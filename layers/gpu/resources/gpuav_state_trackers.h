...
struct DebugPrintfBufferInfo {
    ...
    DebugPrintfBufferInfo(vko::Buffer output_mem_buffer,
                          VkPipelineBindPoint pipeline_bind_point,
                          uint32_t action_command_index) :
        output_mem_buffer(output_mem_buffer),
        pipeline_bind_point(pipeline_bind_point), action_command_index(action_command_index){};
};
...
class CommandBuffer : public vvl::CommandBuffer {
  public:
    ...
    //
    // Note: If the app calls vkCmdBindDescriptorSet 10 times to set descriptor set [0, 9] one at a time instead of
    // setting [0, 9] in a single vkCmdBindDescriptorSet call then this will allocate a lot of redundant memory
    std::vector<DescriptorCommandBinding> descriptor_command_bindings;
    // Information that requires information about the SPIR-V requires pipeline/shaderObject info. This means we need to
    // track things at an action level (draw call) granularity
    ...
    std::string GetDebugLabelRegion(uint32_t label_command_i,
    ...
    using ErrorLoggerFunc =
        stdext::inplace_function<bool(Validator& gpuav,
...
class Buffer : public vvl::Buffer {
  public:
    Buffer(ValidationStateTracker& dev_data,
           VkBuffer buff,
...
class BufferView : public vvl::BufferView {
  public:
    BufferView(const std::shared_ptr<vvl::Buffer>& bf,
               VkBufferView bv,
...
class ImageView : public vvl::ImageView {
  public:
    ImageView(const std::shared_ptr<vvl::Image>& image_state,
              VkImageView iv,
...
class AccelerationStructureKHR : public vvl::AccelerationStructureKHR {
  public:
    AccelerationStructureKHR(VkAccelerationStructureKHR as,
