...
InjectConditionalFunctionPass::InjectConditionalFunctionPass(Module& module) : Pass(module) {
    module.use_bda_ = true;
}

BasicBlockIt InjectConditionalFunctionPass::InjectFunction(Function* function,
    ...
    // If thre is a result, we need to create an additional BasicBlock to hold the |else| case, then after we create a
    // Phi node to hold the result
    ...
    if (target_inst_id != 0) {
        ...
        merge_block.CreateInstruction(
            spv::OpPhi, { phi_type.Id(), phi_id, target_inst_id, valid_block_label, null_id, invalid_block_label });
    }
    ...
    if (target_inst.Opcode() == spv::OpRayQueryInitializeKHR) {
        ...
        invalid_block.CreateInstruction(spv::OpRayQueryInitializeKHR,
                                        { target_inst.Operand(0),
                                          target_inst.Operand(1),
                                          uint32_0_id,
                                          uint32_0_id,
                                          vec3_0_id,
                                          float32_0_id,
    ...
    merge_block.instructions_.insert(merge_block.instructions_.end(),
                                     std::make_move_iterator(inst_it),
    ...
    original_block.CreateInstruction(spv::OpBranchConditional,
                                     { function_result, valid_block_label, invalid_block_label });
        ...
        for (auto block_it = function->blocks_.begin(); block_it != function->blocks_.end(); ++block_it) {
            ...
            for (auto inst_it = block_instructions.begin(); inst_it != block_instructions.end(); ++inst_it) {
                ...
                if (!RequiresInstrumentation(*function, *(inst_it->get()))) {
                    ...
                    // and we get an error "All OpSampledImage instructions must be in the same block in which their
                    // Result <id> are consumed" to get around this we inject a OpCopyObject right after the
                    // OpSampledImage
