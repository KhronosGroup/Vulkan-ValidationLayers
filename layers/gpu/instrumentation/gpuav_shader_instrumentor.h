...
// There is a gpuav::spirv::Instruction that is ONLY intended for shader instrumentation (designed so we can build the
// shader instrumentation as a seperate library). For logging GPU-AV will want to make use of the normal validaiton
// instruction class, just alias it with "Instruction" as that name shouldn't collide with anything.
...
class GpuShaderInstrumentor : public ValidationStateTracker {
  ...
  public:
    ...
    void PreCallRecordDestroyDevice(VkDevice device,
    ...
    bool ValidateCmdWaitEvents(VkCommandBuffer command_buffer,
    ...
    bool PreCallValidateCmdWaitEvents2(VkCommandBuffer commandBuffer,
                                       ...
                                       const ErrorObject& error_obj) const override;
    void PreCallRecordCreatePipelineLayout(VkDevice device,
                                           const VkPipelineLayoutCreateInfo* pCreateInfo,
                                           const VkAllocationCallbacks* pAllocator,
                                           VkPipelineLayout* pPipelineLayout,
                                           const RecordObject& record_obj,
                                           chassis::CreatePipelineLayout& chassis_state) override;
    void PostCallRecordCreatePipelineLayout(VkDevice device,
                                            const VkPipelineLayoutCreateInfo* pCreateInfo,
                                            const VkAllocationCallbacks* pAllocator,
                                            VkPipelineLayout* pPipelineLayout,
                                            const RecordObject& record_obj) override;

    void PostCallRecordCreateShaderModule(VkDevice device,
                                          const VkShaderModuleCreateInfo* pCreateInfo,
                                          const VkAllocationCallbacks* pAllocator,
                                          VkShaderModule* pShaderModule,
                                          const RecordObject& record_obj,
                                          chassis::CreateShaderModule& chassis_state) override;
    void
    PreCallRecordShaderObjectInstrumentation(VkShaderCreateInfoEXT& create_info,
                                             const Location& create_info_loc,
                                             chassis::ShaderObjectInstrumentationData& shader_instrumentation_data);
    void PreCallRecordCreateShadersEXT(VkDevice device,
    ...
    void PostCallRecordCreateShadersEXT(VkDevice device,
                                        uint32_t createInfoCount,
                                        const VkShaderCreateInfoEXT* pCreateInfos,
                                        const VkAllocationCallbacks* pAllocator,
                                        VkShaderEXT* pShaders,
                                        const RecordObject& record_obj,
                                        chassis::ShaderObject& chassis_state) override;
    void PreCallRecordDestroyShaderEXT(VkDevice device,
                                       VkShaderEXT shader,
                                       const VkAllocationCallbacks* pAllocator,
                                       const RecordObject& record_obj) override;

    void PreCallRecordCreateGraphicsPipelines(VkDevice device,
    ...
    void PreCallRecordCreateComputePipelines(VkDevice device,
                                             VkPipelineCache pipelineCache,
                                             uint32_t count,
    ...
    void PreCallRecordCreateRayTracingPipelinesNV(VkDevice device,
                                                  VkPipelineCache pipelineCache,
                                                  uint32_t count,
    ...
    void PreCallRecordCreateRayTracingPipelinesKHR(VkDevice device,
    ...
    void PostCallRecordCreateGraphicsPipelines(VkDevice device,
                                               VkPipelineCache pipelineCache,
                                               uint32_t count,
                                               const VkGraphicsPipelineCreateInfo* pCreateInfos,
                                               const VkAllocationCallbacks* pAllocator,
                                               ...
                                               PipelineStates& pipeline_states,
                                               chassis::CreateGraphicsPipelines& chassis_state) override;
    void PostCallRecordCreateComputePipelines(VkDevice device,
                                              VkPipelineCache pipelineCache,
                                              uint32_t count,
                                              const VkComputePipelineCreateInfo* pCreateInfos,
                                              const VkAllocationCallbacks* pAllocator,
                                              ...
                                              PipelineStates& pipeline_states,
                                              chassis::CreateComputePipelines& chassis_state) override;
    void PostCallRecordCreateRayTracingPipelinesNV(VkDevice device,
                                                   VkPipelineCache pipelineCache,
                                                   uint32_t count,
                                                   const VkRayTracingPipelineCreateInfoNV* pCreateInfos,
                                                   const VkAllocationCallbacks* pAllocator,
    ...
    void PreCallRecordDestroyPipeline(VkDevice device,
                                      VkPipeline pipeline,
                                      const VkAllocationCallbacks* pAllocator,
                                      const RecordObject& record_obj) override;
    ...
    std::string GenerateDebugInfoMessage(VkCommandBuffer commandBuffer,
  ...
  protected:
    ...
    // When instrumenting, we need information about the array of VkDescriptorSetLayouts. The core issue is that for
    // pipelines, we might have to merge 2 pipeline layouts together (because of GPL) and therefore both ShaderObject
    // and PipelineLayout state objects don't have a single way to describe their VkDescriptorSetLayouts. If there are
    // multiple shaders, we also want to only build this information once. This struct is designed to be filled in from
    // both Pipeline and ShaderObject and then passed down to the SPIR-V Instrumentation, and afterwards we don't need
    // to save it.
    ...
    void BuildDescriptorSetLayoutInfo(const VkShaderCreateInfoEXT& create_info,
                                      InstrumentationDescriptorSetLayouts& out_instrumentation_dsl);
    void BuildDescriptorSetLayoutInfo(const vvl::DescriptorSetLayout& set_layout_state,
    ...
    bool InstrumentShader(const vvl::span<const uint32_t>& input_spirv,
  ...
  public:
    ...
    // It should only be used after calls that might abort, not to be used for guarding a function (unless a case is
    // found that make sense too)
