...
void GpuShaderInstrumentor::PostCreateDevice(const VkDeviceCreateInfo* pCreateInfo, const Location& loc) {
    ...
    if (!supported_features.fragmentStoresAndAtomics) {
        InternalError(device,
                      loc,
                      "GPU Shader Instrumentation requires fragmentStoresAndAtomics to allow writting out data inside "
                      "the fragment shader.");
        return;
    }
    ...
    const VkDescriptorSetLayoutCreateInfo debug_desc_layout_info = {
    ...
    VkResult result =
        DispatchCreateDescriptorSetLayout(device, &debug_desc_layout_info, nullptr, &instrumentation_desc_layout_);
    ...
    const VkDescriptorSetLayoutCreateInfo dummy_desc_layout_info = {
        VK_STRUCTURE_TYPE_DESCRIPTOR_SET_LAYOUT_CREATE_INFO, nullptr, 0, 0, nullptr
    };
    ...
    const VkPipelineLayoutCreateInfo debug_pipeline_layout_info = { VK_STRUCTURE_TYPE_PIPELINE_LAYOUT_CREATE_INFO,
                                                                    ...
                                                                    nullptr };
    result =
        DispatchCreatePipelineLayout(device, &debug_pipeline_layout_info, nullptr, &instrumentation_pipeline_layout_);
    if (result != VK_SUCCESS) {
...
void GpuShaderInstrumentor::PreCallRecordDestroyDevice(VkDevice device,
...
void GpuShaderInstrumentor::ReserveBindingSlot(VkPhysicalDevice physicalDevice,
    ...
    if (limits.maxBoundDescriptorSets > kMaxAdjustedBoundDescriptorSet) {
        std::stringstream ss;
        ss << "A descriptor binding slot is required to store GPU-side information, but the device "
              "maxBoundDescriptorSets is "
    ...
    if (enabled[gpu_validation_reserve_binding_slot]) {
        if (limits.maxBoundDescriptorSets > 1) {
            ...
            InternalWarning(
                physicalDevice, loc, "Unable to reserve descriptor binding slot on a device with only one slot.");
        }
    }
}
...
bool GpuShaderInstrumentor::ValidateCmdWaitEvents(VkCommandBuffer command_buffer,
                                                  VkPipelineStageFlags2 src_stage_mask,
                                                  const Location& loc) const {
    if (src_stage_mask & VK_PIPELINE_STAGE_2_HOST_BIT) {
        std::ostringstream error_msg;
        error_msg << loc.Message()
                  << " recorded with VK_PIPELINE_STAGE_HOST_BIT set. GPU-Assisted validation waits on queue "
                     "completion. This wait "
...
bool GpuShaderInstrumentor::PreCallValidateCmdWaitEvents(VkCommandBuffer commandBuffer,
                                                         uint32_t eventCount,
...
bool GpuShaderInstrumentor::PreCallValidateCmdWaitEvents2KHR(VkCommandBuffer commandBuffer,
                                                             uint32_t eventCount,
                                                             const VkEvent* pEvents,
...
bool GpuShaderInstrumentor::PreCallValidateCmdWaitEvents2(VkCommandBuffer commandBuffer,
                                                          uint32_t eventCount,
                                                          const VkEvent* pEvents,
...
void GpuShaderInstrumentor::PreCallRecordCreatePipelineLayout(VkDevice device,
    ...
    if (gpuav_settings.IsSpirvModified()) {
        if (chassis_state.modified_create_info.setLayoutCount > instrumentation_desc_set_bind_index_) {
            ...
            strm << "pCreateInfo::setLayoutCount (" << chassis_state.modified_create_info.setLayoutCount
                 ...
                 << ". "
                 << "This Pipeline Layout has too many descriptor sets that will not allow GPU shader instrumentation "
                    "to be setup "
                    "for "
            ...
            chassis_state.new_layouts.insert(chassis_state.new_layouts.end(),
                                             &pCreateInfo->pSetLayouts[0],
    ...
    BaseClass::PreCallRecordCreatePipelineLayout(
        device, pCreateInfo, pAllocator, pPipelineLayout, record_obj, chassis_state);
}

void GpuShaderInstrumentor::PostCallRecordCreatePipelineLayout(VkDevice device,
                                                               const VkPipelineLayoutCreateInfo* pCreateInfo,
                                                               const VkAllocationCallbacks* pAllocator,
                                                               VkPipelineLayout* pPipelineLayout,
                                                               const RecordObject& record_obj) {
...
void GpuShaderInstrumentor::PostCallRecordCreateShaderModule(VkDevice device,
                                                             const VkShaderModuleCreateInfo* pCreateInfo,
                                                             const VkAllocationCallbacks* pAllocator,
                                                             VkShaderModule* pShaderModule,
                                                             const RecordObject& record_obj,
                                                             chassis::CreateShaderModule& chassis_state) {
    BaseClass::PostCallRecordCreateShaderModule(
        device, pCreateInfo, pAllocator, pShaderModule, record_obj, chassis_state);

    // By default, we instrument everything, but if the setting is enabled, we only will instrument the shaders the app
    // picks
    if (gpuav_settings.select_instrumented_shaders && IsSelectiveInstrumentationEnabled(pCreateInfo->pNext)) {
        // If this is being filled up, likely only a few shaders and the app scope is narrowed down, so no need to spend
        // time removing these later
...
void GpuShaderInstrumentor::PreCallRecordCreateShadersEXT(VkDevice device,
    ...
    BaseClass::PreCallRecordCreateShadersEXT(
    ...
    for (uint32_t i = 0; i < createInfoCount; ++i) {
        ...
        if (new_create_info.flags & VK_SHADER_CREATE_INDIRECT_BINDABLE_BIT_EXT) {
            InternalError(device,
                          create_info_loc,
                          "Unable to instrument shader using VkIndirectExecutionSetEXT validly, things might work, but "
                          "likely will "
        ...
        if (new_create_info.setLayoutCount > instrumentation_desc_set_bind_index_) {
            ...
            strm << "pCreateInfos[" << i << "]::setLayoutCount (" << new_create_info.setLayoutCount
                 << ") will conflicts with validation's descriptor set at slot " << instrumentation_desc_set_bind_index_
                 << ". "
                 << "This Shader Object has too many descriptor sets that will not allow GPU shader instrumentation to "
                    "be setup "
                    "for VkShaderEXT created with it, therefor no validation error will be repored for them by GPU-AV "
                    "at "
                    "runtime.";
            InternalWarning(device, record_obj.location, strm.str().c_str());
        } else {
            ...
            instrumentation_data.new_layouts.insert(instrumentation_data.new_layouts.end(),
                                                    pCreateInfos[i].pSetLayouts,
...
void GpuShaderInstrumentor::PostCallRecordCreateShadersEXT(VkDevice device,
                                                           uint32_t createInfoCount,
                                                           const VkShaderCreateInfoEXT* pCreateInfos,
                                                           const VkAllocationCallbacks* pAllocator,
                                                           VkShaderEXT* pShaders,
                                                           const RecordObject& record_obj,
                                                           chassis::ShaderObject& chassis_state) {
    BaseClass::PostCallRecordCreateShadersEXT(
        device, createInfoCount, pCreateInfos, pAllocator, pShaders, record_obj, chassis_state);
    if (!gpuav_settings.IsSpirvModified())
        return;
    ...
    for (uint32_t i = 0; i < createInfoCount; ++i) {
        ...
        instrumented_shaders_map_.insert_or_assign(instrumentation_data.unique_shader_id,
                                                   VK_NULL_HANDLE,
                                                   VK_NULL_HANDLE,
                                                   pShaders[i],
...
void GpuShaderInstrumentor::PreCallRecordDestroyShaderEXT(VkDevice device,
                                                          VkShaderEXT shader,
                                                          const VkAllocationCallbacks* pAllocator,
                                                          const RecordObject& record_obj) {
    auto to_erase = instrumented_shaders_map_.snapshot(
        [shader](const InstrumentedShader& entry) { return entry.shader_object == shader; });
...
void GpuShaderInstrumentor::PreCallRecordCreateGraphicsPipelines(VkDevice device,
    ...
    BaseClass::PreCallRecordCreateGraphicsPipelines(
        device, pipelineCache, count, pCreateInfos, pAllocator, pPipelines, record_obj, pipeline_states, chassis_state);
    if (!gpuav_settings.IsSpirvModified())
        return;
    ...
    for (uint32_t i = 0; i < count; ++i) {
        ...
        if (pipeline_state->linking_shaders != 0) {
            success = PreCallRecordPipelineCreationShaderInstrumentationGPL(
                pAllocator, *pipeline_state, new_pipeline_ci, create_info_loc, shader_instrumentation_metadata);
        } else {
            success = PreCallRecordPipelineCreationShaderInstrumentation(
                pAllocator, *pipeline_state, new_pipeline_ci, create_info_loc, shader_instrumentation_metadata);
        }
        if (!success) {
            return;
    ...
    chassis_state.pCreateInfos =
        reinterpret_cast<VkGraphicsPipelineCreateInfo*>(chassis_state.modified_create_infos.data());
}

void GpuShaderInstrumentor::PreCallRecordCreateComputePipelines(VkDevice device,
                                                                VkPipelineCache pipelineCache,
                                                                uint32_t count,
    ...
    BaseClass::PreCallRecordCreateComputePipelines(
        device, pipelineCache, count, pCreateInfos, pAllocator, pPipelines, record_obj, pipeline_states, chassis_state);
    if (!gpuav_settings.IsSpirvModified())
        return;
    ...
    for (uint32_t i = 0; i < count; ++i) {
        ...
        bool success = PreCallRecordPipelineCreationShaderInstrumentation(
            pAllocator, *pipeline_state, new_pipeline_ci, create_info_loc, shader_instrumentation_metadata);
        if (!success) {
            return;
        }
    ...
    chassis_state.pCreateInfos =
        reinterpret_cast<VkComputePipelineCreateInfo*>(chassis_state.modified_create_infos.data());
}

void GpuShaderInstrumentor::PreCallRecordCreateRayTracingPipelinesNV(
    VkDevice device,
    VkPipelineCache pipelineCache,
    uint32_t count,
    ...
    BaseClass::PreCallRecordCreateRayTracingPipelinesNV(
        device, pipelineCache, count, pCreateInfos, pAllocator, pPipelines, record_obj, pipeline_states, chassis_state);
    if (!gpuav_settings.IsSpirvModified())
        return;
    ...
    chassis_state.pCreateInfos =
        reinterpret_cast<VkRayTracingPipelineCreateInfoNV*>(chassis_state.modified_create_infos.data());
}
...
void GpuShaderInstrumentor::PreCallRecordCreateRayTracingPipelinesKHR(
    ...
    BaseClass::PreCallRecordCreateRayTracingPipelinesKHR(device,
                                                         deferredOperation,
                                                         pipelineCache,
                                                         count,
                                                         pCreateInfos,
    ...
    chassis_state.pCreateInfos =
        reinterpret_cast<VkRayTracingPipelineCreateInfoKHR*>(chassis_state.modified_create_infos.data());
}
...
static void UtilCopyCreatePipelineFeedbackData(CreateInfos& create_info, SafeCreateInfos& safe_create_info) {
    auto src_feedback_struct =
        vku::FindStructInPNextChain<VkPipelineCreationFeedbackCreateInfo>(safe_create_info.pNext);
    ...
    *dst_feedback_struct->pPipelineCreationFeedback = *src_feedback_struct->pPipelineCreationFeedback;
    for (uint32_t j = 0; j < src_feedback_struct->pipelineStageCreationFeedbackCount; j++) {
        dst_feedback_struct->pPipelineStageCreationFeedbacks[j] =
            src_feedback_struct->pPipelineStageCreationFeedbacks[j];
    }
}

void GpuShaderInstrumentor::PostCallRecordCreateGraphicsPipelines(VkDevice device,
                                                                  VkPipelineCache pipelineCache,
                                                                  uint32_t count,
                                                                  const VkGraphicsPipelineCreateInfo* pCreateInfos,
                                                                  const VkAllocationCallbacks* pAllocator,
                                                                  ...
                                                                  PipelineStates& pipeline_states,
                                                                  chassis::CreateGraphicsPipelines& chassis_state) {
    BaseClass::PostCallRecordCreateGraphicsPipelines(
        device, pipelineCache, count, pCreateInfos, pAllocator, pPipelines, record_obj, pipeline_states, chassis_state);
    if (!gpuav_settings.IsSpirvModified())
        return;
    ...
    for (uint32_t i = 0; i < count; ++i) {
        ...
        auto& shader_instrumentation_metadata = chassis_state.shader_instrumentations_metadata[i];
        if (pipeline_state->linking_shaders != 0) {
            PostCallRecordPipelineCreationShaderInstrumentationGPL(
                *pipeline_state, pAllocator, shader_instrumentation_metadata);
        } else {
...
void GpuShaderInstrumentor::PostCallRecordCreateComputePipelines(VkDevice device,
                                                                 VkPipelineCache pipelineCache,
                                                                 uint32_t count,
                                                                 const VkComputePipelineCreateInfo* pCreateInfos,
                                                                 const VkAllocationCallbacks* pAllocator,
                                                                 ...
                                                                 PipelineStates& pipeline_states,
                                                                 chassis::CreateComputePipelines& chassis_state) {
    BaseClass::PostCallRecordCreateComputePipelines(
        device, pipelineCache, count, pCreateInfos, pAllocator, pPipelines, record_obj, pipeline_states, chassis_state);
    if (!gpuav_settings.IsSpirvModified())
        return;
    // VK_PIPELINE_COMPILE_REQUIRED means that the current pipeline creation call was used to poke the driver cache,
...
void GpuShaderInstrumentor::PostCallRecordCreateRayTracingPipelinesNV(
    ...
    PipelineStates& pipeline_states,
    chassis::CreateRayTracingPipelinesNV& chassis_state) {
    BaseClass::PostCallRecordCreateRayTracingPipelinesNV(
        device, pipelineCache, count, pCreateInfos, pAllocator, pPipelines, record_obj, pipeline_states, chassis_state);
    if (!gpuav_settings.IsSpirvModified())
        return;
    // VK_PIPELINE_COMPILE_REQUIRED means that the current pipeline creation call was used to poke the driver cache,
...
void GpuShaderInstrumentor::PostCallRecordCreateRayTracingPipelinesKHR(
    ...
    BaseClass::PostCallRecordCreateRayTracingPipelinesKHR(device,
                                                          deferredOperation,
                                                          pipelineCache,
                                                          count,
                                                          pCreateInfos,
    ...
    const bool is_operation_deferred =
        deferredOperation != VK_NULL_HANDLE && record_obj.result == VK_OPERATION_DEFERRED_KHR;
    ...
    if (is_operation_deferred) {
        ...
        deferred_op_post_checks.emplace_back([this, held_chassis_state = chassis_state](
                                                 const std::vector<VkPipeline>& vk_pipelines) mutable {
...
void GpuShaderInstrumentor::PreCallRecordDestroyPipeline(VkDevice device,
                                                         VkPipeline pipeline,
                                                         const VkAllocationCallbacks* pAllocator,
                                                         const RecordObject& record_obj) {
    auto to_erase = instrumented_shaders_map_.snapshot(
        [pipeline](const InstrumentedShader& entry) { return entry.pipeline == pipeline; });
    for (const auto& entry : to_erase) {
        instrumented_shaders_map_.erase(entry.first);
    }
...
void SetShaderModule(SafeType& create_info,
                     const vku::safe_VkPipelineShaderStageCreateInfo& stage_info,
...
template <>
vku::safe_VkPipelineShaderStageCreateInfo& GetShaderStageCI(vku::safe_VkComputePipelineCreateInfo& ci,
...
void GpuShaderInstrumentor::BuildDescriptorSetLayoutInfo(const VkShaderCreateInfoEXT& create_info,
    ...
    for (const auto [set_layout_index, set_layout] :
         vvl::enumerate(create_info.pSetLayouts, create_info.setLayoutCount)) {
...
bool GpuShaderInstrumentor::PreCallRecordPipelineCreationShaderInstrumentation(
    ...
    for (uint32_t stage_state_i = 0; stage_state_i < static_cast<uint32_t>(pipeline_state.stage_states.size());
         ++stage_state_i) {
        ...
        vku::safe_VkShaderModuleCreateInfo* modified_shader_module_ci = nullptr;
        {
            ...
            auto& stage_ci = GetShaderStageCI<SafeCreateInfo, vku::safe_VkPipelineShaderStageCreateInfo>(
                modified_pipeline_ci, stage);
            modified_shader_module_ci = const_cast<vku::safe_VkShaderModuleCreateInfo*>(
        ...
        const uint32_t unique_shader_id = unique_shader_module_id_++;
        const bool is_shader_instrumented = InstrumentShader(
            modified_module_state->spirv->words_, unique_shader_id, instrumentation_dsl, loc, instrumented_spirv);
        if (is_shader_instrumented) {
            instrumentation_metadata.unique_shader_id = unique_shader_id;
            if (modified_module_state->VkHandle() != VK_NULL_HANDLE) {
                // If the user used vkCreateShaderModule, we create a new VkShaderModule to replace with the
                // instrumented shader
                ...
                VkResult result = DispatchCreateShaderModule(
                    device, &instrumented_shader_module_ci, pAllocator, &instrumented_shader_module);
                if (result == VK_SUCCESS) {
                    SetShaderModule(modified_pipeline_ci,
                                    *stage_state.pipeline_create_info,
                                    instrumented_shader_module,
                                    stage_state_i);
                    pipeline_state.instrumentation_data.instrumented_shader_module.emplace_back(
                        instrumented_shader_module);
                } else {
                ...
                // pCode memory. It would be much harder to change everything from std::vector and instead to adjust
                // Safe Struct to not double-free the memory on us. If making any changes, we have to consider a case
                // where the user inlines the fragment shader, but use a normal VkShaderModule in the vertex shader.
...
void GpuShaderInstrumentor::PostCallRecordPipelineCreationShaderInstrumentation(
    ...
    for (uint32_t stage_state_i = 0; stage_state_i < static_cast<uint32_t>(pipeline_state.stage_states.size());
        ...
        // we can reference it by the instruction number printed out in the shader. Since the application can destroy
        // the original VkShaderModule, there is a chance this will be gone, we need to copy it now.
        // TODO - in the instrumentation, instead of printing the instruction number only, if we print out debug info,
        ...
        instrumented_shaders_map_.insert_or_assign(instrumentation_metadata.unique_shader_id,
                                                   pipeline_state.VkHandle(),
...
// While have an almost duplicated function is not ideal, the core issue is we have a single, templated function
// designed for Graphics, Compute, and Ray Tracing. GPL is only for graphics, so we end up needing this "side code path"
// for graphics only and it doesn't fit in the "all pipeline" templated flow.
bool GpuShaderInstrumentor::PreCallRecordPipelineCreationShaderInstrumentationGPL(
    const VkAllocationCallbacks* pAllocator,
    vvl::Pipeline& pipeline_state,
    vku::safe_VkGraphicsPipelineCreateInfo& modified_pipeline_ci,
    const Location& loc,
    std::vector<chassis::ShaderInstrumentationMetadata>& shader_instrumentation_metadata) {
    // Init here instead of in chassis so we don't pay cost when GPU-AV is not used
    const size_t total_stages = pipeline_state.stage_states.size();
    ...
    for (uint32_t modified_lib_i = 0; modified_lib_i < modified_pipeline_lib_ci->libraryCount; ++modified_lib_i) {
        ...
        for (uint32_t stage_state_i = 0; stage_state_i < static_cast<uint32_t>(modified_lib->stage_states.size());
            ...
            ASSERT_AND_CONTINUE(modified_module_state);
            chassis::ShaderInstrumentationMetadata& instrumentation_metadata =
                shader_instrumentation_metadata[shader_i++];
            ...
            vku::safe_VkShaderModuleCreateInfo* modified_shader_module_ci = nullptr;
            {
                ...
                if (gpuav_settings.select_instrumented_shaders) {
                    if (modified_shader_module_ci &&
                        !IsSelectiveInstrumentationEnabled(modified_shader_module_ci->pNext)) {
                        continue;
                    } else if (selected_instrumented_shaders.find(modified_module_state->VkHandle()) ==
                               selected_instrumented_shaders.end()) {
                        continue;
            ...
            if (is_shader_instrumented) {
                ...
                if (modified_module_state->VkHandle() != VK_NULL_HANDLE) {
                    ...
                    create_info.codeSize = instrumented_spirv.size() * sizeof(uint32_t);
                    VkResult result =
                        DispatchCreateShaderModule(device, &create_info, pAllocator, &instrumented_shader_module);
                    if (result == VK_SUCCESS) {
                        ...
                        modified_lib->instrumentation_data.instrumented_shader_module.emplace_back(
                            instrumented_shader_module);
                    } else {
                        InternalError(device, loc, "Unable to replace non-instrumented shader with instrumented one.");
                        return false;
                    ...
                    // The user is inlining the Shader Module into the pipeline, so just need to update the spirv
                    instrumentation_metadata.passed_in_shader_stage_ci = true;
                    // TODO - This makes a copy, but could save on Chassis stack instead (then remove function from
                    // VUL). The core issue is we always use std::vector<uint32_t> but Safe Struct manages its own
                    // version of the pCode memory. It would be much harder to change everything from std::vector and
                    // instead to adjust Safe Struct to not double-free the memory on us. If making any changes, we have
                    // to consider a case where the user inlines the fragment shader, but use a normal VkShaderModule in
                    // the vertex shader.
                    modified_shader_module_ci->SetCode(instrumented_spirv);
                } else {
                    assert(false);
        ...
        const VkResult result = DispatchCreateGraphicsPipelines(
            device, VK_NULL_HANDLE, 1, modified_pipeline_ci.ptr(), pAllocator, &instrumented_pipeline_lib);
...
void GpuShaderInstrumentor::PostCallRecordPipelineCreationShaderInstrumentationGPL(
    ...
    for (uint32_t library_i = 0; library_i < pipeline_state.library_create_info->libraryCount; ++library_i) {
        ...
        for (uint32_t stage_state_i = 0; stage_state_i < static_cast<uint32_t>(lib->stage_states.size());
             ++stage_state_i) {
            ...
            // We currently need to store a copy of the original, non-instrumented shader so if there is debug
            // information, we can reference it by the instruction number printed out in the shader. Since the
            // application can destroy the original VkShaderModule, there is a chance this will be gone, we need to copy
            // it now.
            // TODO - in the instrumentation, instead of printing the instruction number only, if we print out debug
            // info, we can remove this copy
            std::vector<uint32_t> code;
            if (module_state && module_state->spirv)
                code = module_state->spirv->words_;
...
static bool GpuValidateShader(const std::vector<uint32_t>& input,
                              bool SetRelaxBlockLayout,
                              bool SetScalarBlockLayout,
...
bool GpuShaderInstrumentor::InstrumentShader(const vvl::span<const uint32_t>& input_spirv,
    ...
    spirv::Module module(
        input_spirv, debug_report, module_settings, instrumentation_dsl.set_index_to_bindings_layout_lut);
    ...
    // Post Process instrumentation passes assume the things inside are valid, but putting at the end, things above will
    // wrap checks in a if/else, this means they will be gaurded as if they were inside the above passes
    ...
    if (gpuav_settings.debug_validate_instrumented_shaders) {
        ...
        if (!GpuValidateShader(out_instrumented_spirv,
                               device_extensions.vk_khr_relaxed_block_layout,
...
void GpuShaderInstrumentor::InternalError(LogObjectList objlist,
                                          const Location& loc,
    ...
    char const* vuid =
        gpuav_settings.debug_printf_only ? "UNASSIGNED-DEBUG-PRINTF" : "UNASSIGNED-GPU-Assisted-Validation";

    LogError(
        vuid, objlist, loc, "Internal Error, %s is being disabled. Details:\n%s", layer_name, error_message.c_str());

    // Once we encounter an internal issue disconnect everything.
    // This prevents need to check "if (aborted)" (which is awful when we easily forget to check somewhere and the user
    // gets spammed with errors making it hard to see the first error with the real source of the problem).
    dispatch_device_->ReleaseValidationObject(LayerObjectTypeGpuAssisted);
}

void GpuShaderInstrumentor::InternalWarning(LogObjectList objlist,
                                            const Location& loc,
                                            const char* const specific_message) const {
...
void GpuShaderInstrumentor::InternalInfo(LogObjectList objlist,
                                         const Location& loc,
                                         const char* const specific_message) const {
...
static void GenerateStageMessage(std::ostringstream& ss,
                                 uint32_t stage_id,
    ...
    switch (stage_id) {
        case glsl::kHeaderStageIdMultiEntryPoint: {
            ...
            for (const auto& insn : instructions) {
                if (insn.Opcode() == spv::OpFunction)
                    break; // early exit when possible
        ...
        case spv::ExecutionModelTessellationControl: {
            ss << "Stage = Tessellation Control.  Invocation ID = " << stage_info_0
               << ", Primitive ID = " << stage_info_1;
        } break;
        case spv::ExecutionModelTessellationEvaluation: {
            ss << "Stage = Tessellation Eval.  Primitive ID = " << stage_info_0 << ", TessCoord (u, v) = ("
               << stage_info_1 << ", " << stage_info_2 << "). ";
        } break;
        ...
        case spv::ExecutionModelAnyHitKHR: {
            ss << "Stage = Any Hit.  Global Launch ID (x,y,z) = (" << stage_info_0 << ", " << stage_info_1 << ", "
               << stage_info_2 << "). ";
        } break;
        ...
        case spv::ExecutionModelMissKHR: {
            ss << "Stage = Miss.  Global Launch ID (x,y,z) = (" << stage_info_0 << ", " << stage_info_1 << ", "
               << stage_info_2 << "). ";
        } break;
        case spv::ExecutionModelCallableKHR: {
            ss << "Stage = Callable.  Global Launch ID (x,y,z) = (" << stage_info_0 << ", " << stage_info_1 << ", "
               << stage_info_2 << "). ";
        } break;
...
static std::string FindShaderSource(std::ostringstream& ss,
                                    const std::vector<Instruction>& instructions,
...
std::string GpuShaderInstrumentor::GenerateDebugInfoMessage(VkCommandBuffer commandBuffer,
    ...
    if (instrumented_shader->shader_module == VK_NULL_HANDLE && instrumented_shader->shader_object == VK_NULL_HANDLE) {
        ...
        ss << "[Internal Error] - Unable to locate shader/pipeline handles used in command buffer "
           << LookupDebugUtilsNameNoLock(debug_report, HandleToUint64(commandBuffer)) << "("
        ...
        if (instrumented_shader->shader_module == VK_NULL_HANDLE) {
            ss << "Shader Object "
               << LookupDebugUtilsNameNoLock(debug_report, HandleToUint64(instrumented_shader->shader_object)) << "("
