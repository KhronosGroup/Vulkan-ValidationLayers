...
struct ComputeValidationPipeline {
    ComputeValidationPipeline(Validator& gpuav,
                              const Location& loc,
        ...
        if (result != VK_SUCCESS) {
            gpuav.InternalError(
                device, loc, "Unable to create descriptor set layout for SharedDrawValidationResources.");
            return;
        }

        std::array<VkDescriptorSetLayout, 2> set_layouts = { { error_output_desc_set_layout,
                                                               specific_desc_set_layout } };
        ...
        result = DispatchCreateComputePipelines(
            device, VK_NULL_HANDLE, 1, &compute_validation_pipeline_ci, nullptr, &pipeline);
        if (result != VK_SUCCESS) {
    ...
    void BindShaderResources(Validator& gpuav,
                             CommandBuffer& cb_state,
                             uint32_t cmd_index,
                             uint32_t error_logger_index,
                             const ShaderResources& shader_resources) {
        // Error logging resources
        BindErrorLoggingDescSet(
            gpuav, cb_state, VK_PIPELINE_BIND_POINT_COMPUTE, pipeline_layout, cmd_index, error_logger_index);
...
struct FirstInstanceValidationShader {
    ...
    static std::vector<VkDescriptorSetLayoutBinding> GetDescriptorSetLayoutBindings() {
        std::vector<VkDescriptorSetLayoutBinding> bindings = {
            { gpuav::glsl::kPreDrawBinding_IndirectBuffer,
              VK_DESCRIPTOR_TYPE_STORAGE_BUFFER,
              1,
              VK_SHADER_STAGE_COMPUTE_BIT,
              nullptr }, // indirect buffer
            { gpuav::glsl::kPreDrawBinding_CountBuffer,
              VK_DESCRIPTOR_TYPE_STORAGE_BUFFER,
              1,
              VK_SHADER_STAGE_COMPUTE_BIT,
...
void FirstInstance(Validator& gpuav,
                   CommandBuffer& cb_state,
                   const Location& loc,
                   VkBuffer draw_buffer,
                   VkDeviceSize draw_buffer_offset,
    ...
    CommandBuffer::ValidationCommandFunc validation_cmd = [draw_buffer,
                                                           draw_buffer_offset,
                                                           draw_cmds_byte_stride,
                                                           first_instance_member_pos,
                                                           draw_count,
                                                           count_buffer,
                                                           count_buffer_offset,
        ...
        if (!draw_buffer_state) {
            gpuav.InternalError(
                LogObjectList(cb_state.VkHandle(), draw_buffer), loc, "buffer must be a valid VkBuffer handle");
            return;
        }
        ...
        // ---
        {
            ...
            if (count_buffer) {
                ...
                shader_resources.push_constants.count_buffer_dwords_offset =
                    (uint32_t)count_buffer_offset / sizeof(uint32_t);
            } else {
                shader_resources.count_buffer_binding.info = { shared_draw_validation_resources.dummy_buffer.VkHandle(),
                                                               0,
                                                               VK_WHOLE_SIZE };
            }
        ...
        // ---
        {
            ...
            if (draw_buffer_state->create_info.size > draw_buffer_offset) {
                ...
                if (draw_count > 1) {
                    max_held_draw_cmds = static_cast<uint32_t>(
                        (draw_buffer_state->create_info.size - draw_buffer_offset) / draw_cmds_byte_stride);
                } else {
            ...
            DispatchCmdPipelineBarrier(cb_state.VkHandle(),
    ...
    CommandBuffer::ErrorLoggerFunc error_logger = [loc, vuid, draw_indirect_struct_name, label_command_i](
                                                      Validator& gpuav,
        ...
        skip |= gpuav.LogError(
            ...
            vvl::String(draw_indirect_struct_name),
            index,
            invalid_first_instance);
...
template <>
void FirstInstance<VkDrawIndirectCommand>(Validator& gpuav,
                                          CommandBuffer& cb_state,
                                          const Location& loc,
                                          VkBuffer draw_buffer,
                                          VkDeviceSize draw_buffer_offset,
...
template <>
void FirstInstance<VkDrawIndexedIndirectCommand>(Validator& gpuav,
                                                 CommandBuffer& cb_state,
                                                 const Location& loc,
                                                 VkBuffer draw_buffer,
                                                 VkDeviceSize draw_buffer_offset,
...
void CountBuffer(Validator& gpuav,
                 CommandBuffer& cb_state,
                 const Location& loc,
                 VkBuffer draw_buffer,
                 VkDeviceSize draw_buffer_offset,
                 uint32_t draw_indirect_struct_byte_size,
                 vvl::Struct draw_indirect_struct_name,
                 uint32_t draw_cmds_byte_stride,
                 VkBuffer count_buffer,
                 VkDeviceSize count_buffer_offset,
    ...
    CommandBuffer::ValidationCommandFunc validation_cmd = [draw_buffer_size = draw_buffer_state->create_info.size,
        ...
        // ---
        {
            ...
            shader_resources.push_constants.device_limit_max_draw_indirect_count =
                gpuav.phys_dev_props.limits.maxDrawIndirectCount;
    ...
    CommandBuffer::ErrorLoggerFunc error_logger =
        [loc,
         draw_buffer,
         draw_buffer_size = draw_buffer_state->create_info.size,
         draw_buffer_offset,
         draw_indirect_struct_byte_size,
         draw_cmds_byte_stride,
         draw_indirect_struct_name,
         vuid_max_draw_count,
         label_command_i](Validator& gpuav,
                          const CommandBuffer& cb_state,
                          const uint32_t* error_record,
                          const LogObjectList& objlist,
                          const std::vector<std::string>& initial_label_stack) {
            ...
            switch (error_record[kHeaderErrorSubCodeOffset]) {
                case kErrorSubCodePreDraw_DrawBufferSize: {
                    ...
                    // Discussed that if drawCount is largeer than the buffer, it is still capped by the maxDrawCount on
                    // the CPU (which we would have checked is in the buffer range). We decided that we still want to
                    // give a warning, but the nothing is invalid here.
                    // https://gitlab.khronos.org/vulkan/vulkan/-/issues/3991
                    skip |= gpuav.LogWarning("WARNING-GPU-AV-drawCount",
                                             objlist,
                                             loc_with_debug_region,
                                             ...
                                             count,
                                             draw_buffer_size,
                                             gpuav.FormatHandle(draw_buffer).c_str(),
                                             draw_cmds_byte_stride,
                                             draw_buffer_offset,
                                             vvl::String(draw_indirect_struct_name),
                                             draw_size);
                ...
                case kErrorSubCodePreDraw_DrawCountLimit: {
                    const uint32_t count = error_record[kPreActionParamOffset_0];
                    skip |= gpuav.LogError(vuid_max_draw_count,
                                           objlist,
                                           loc_with_debug_region,
                                           "Indirect draw count of %" PRIu32
                                           " would exceed maxDrawIndirectCount limit of %" PRIu32 ".",
                                           count,
                                           gpuav.phys_dev_props.limits.maxDrawIndirectCount);
                    break;
                }
...
void DrawMeshIndirect(Validator& gpuav,
                      CommandBuffer& cb_state,
                      const Location& loc,
                      VkBuffer draw_buffer,
                      VkDeviceSize draw_buffer_offset,
    ...
    CommandBuffer::ValidationCommandFunc validation_cmd = [draw_buffer,
                                                           draw_buffer_full_size = draw_buffer_state->create_info.size,
                                                           draw_buffer_offset,
                                                           draw_cmds_byte_stride,
                                                           count_buffer,
                                                           count_buffer_offset,
    ...
    CommandBuffer::ErrorLoggerFunc error_logger = [loc, is_task_shader, label_command_i](
        ...
        const char* group_count_total_name =
            is_task_shader ? "maxTaskWorkGroupTotalCount" : "maxMeshWorkGroupTotalCount";
        ...
        switch (error_record[kHeaderErrorSubCodeOffset]) {
            case kErrorSubCodePreDrawGroupCountX: {
                ...
                const uint32_t limit = is_task_shader
                                           ? gpuav.phys_dev_ext_props.mesh_shader_props_ext.maxTaskWorkGroupCount[0]
                                           : gpuav.phys_dev_ext_props.mesh_shader_props_ext.maxMeshWorkGroupCount[0];
                skip |= gpuav.LogError(vuid_group_count_exceeds_max,
                                       objlist,
                                       loc_with_debug_region,
...
static std::optional<SmallestVertexBufferBinding> SmallestVertexAttributesCount(const vvl::CommandBuffer& cb_state) {
    ...
    const bool dynamic_vertex_input =
        use_shader_objects || pipeline_state->IsDynamic(CB_DYNAMIC_STATE_VERTEX_INPUT_EXT);

    const auto& vertex_binding_descriptions = dynamic_vertex_input ? cb_state.dynamic_state_value.vertex_bindings
                                                                   : pipeline_state->vertex_input_state->bindings;
    ...
    for (const auto& [binding, vertex_binding_desc] : vertex_binding_descriptions) {
        ...
        for (const auto& [Location, attrib] : vertex_binding_desc.locations) {
            ...
            const VkDeviceSize stride =
                vbb.stride != 0 ? vbb.stride
                                : attribute_size; // Tracked stride should already handle all possible value origin
            ...
            // maybe room for one more attribute but not full stride - not having stride space does not matter for last
            // element
...
struct IndexBufferValidationShader {
    ...
    static std::vector<VkDescriptorSetLayoutBinding> GetDescriptorSetLayoutBindings() {
        std::vector<VkDescriptorSetLayoutBinding> bindings = { { gpuav::glsl::kPreDrawBinding_IndexBuffer,
                                                                 VK_DESCRIPTOR_TYPE_STORAGE_BUFFER,
                                                                 1,
                                                                 VK_SHADER_STAGE_COMPUTE_BIT,
                                                                 nullptr } };
...
void DrawIndexed(Validator& gpuav,
                 CommandBuffer& cb_state,
                 const Location& loc,
                 uint32_t index_count,
                 uint32_t first_index,
                 uint32_t vertex_offset,
                 const char* vuid_oob_vertex) {
    ...
    const std::optional<SmallestVertexBufferBinding> smallest_vertex_buffer_binding =
        SmallestVertexAttributesCount(cb_state);
    if (!smallest_vertex_buffer_binding.has_value()) {
        // cannot overrun index buffer, skip validation
        return;
    }

    CommandBuffer::ValidationCommandFunc validation_cmd = [index_buffer_binding = cb_state.index_buffer_binding,
                                                           index_count,
                                                           first_index,
                                                           vertex_offset,
                                                           smallest_vertex_buffer_binding =
                                                               *smallest_vertex_buffer_binding,
                                                           draw_i = cb_state.draw_index,
                                                           error_logger_i =
                                                               uint32_t(cb_state.per_command_error_loggers.size()),
...

        {
            ...
            shader_resources.index_buffer_binding.info = { index_buffer_binding.buffer,
                                                           0,
            ...
            switch (error_record[kHeaderErrorSubCodeOffset]) {
                case kErrorSubCode_OobVertexBuffer: {
                    ...
                    skip |= gpuav.LogError(
                        ...
                        objlist,
                        loc_with_debug_region,
                        "Vertex index %" PRIu32 " is not within the smallest bound vertex buffer.\n"
                        ...
                        "Note: Vertex buffer binding size is the effective, valid one, based on how the VkBuffer was "
                        "created and "
                        ...
                        smallest_vertex_buffer_binding.binding,
                        smallest_vertex_buffer_binding.binding_info.effective_size,
                        smallest_vertex_buffer_binding.binding_info.offset,
                        smallest_vertex_buffer_binding.binding_info.stride,
                        smallest_vertex_buffer_binding.smallest_vertex_attributes_count,

                        // Vertex Attribute info
                        smallest_vertex_buffer_binding.attribute.location,
                        smallest_vertex_buffer_binding.attribute.binding,
...
struct DrawIndexedIndirectIndexBufferShader {
    static size_t GetSpirvSize() {
        return cmd_validation_draw_indexed_indirect_index_buffer_comp_size * sizeof(uint32_t);
    }
    ...
    static std::vector<VkDescriptorSetLayoutBinding> GetDescriptorSetLayoutBindings() {
        std::vector<VkDescriptorSetLayoutBinding> bindings = { { gpuav::glsl::kPreDrawBinding_IndirectBuffer,
                                                                 VK_DESCRIPTOR_TYPE_STORAGE_BUFFER,
                                                                 1,
                                                                 VK_SHADER_STAGE_COMPUTE_BIT,
                                                                 nullptr },
                                                               { gpuav::glsl::kPreDrawBinding_CountBuffer,
                                                                 ...
                                                                 VK_SHADER_STAGE_COMPUTE_BIT,
                                                                 nullptr },
                                                               { gpuav::glsl::kPreDrawBinding_IndexBuffer,
                                                                 VK_DESCRIPTOR_TYPE_STORAGE_BUFFER,
                                                                 1,
                                                                 VK_SHADER_STAGE_COMPUTE_BIT,
                                                                 nullptr } };
...
void DrawIndexedIndirectIndexBuffer(Validator& gpuav,
                                    CommandBuffer& cb_state,
                                    const Location& loc,
                                    VkBuffer draw_buffer,
                                    VkDeviceSize draw_buffer_offset,
        ...
        switch (error_record[kHeaderErrorSubCodeOffset]) {
            case kErrorSubCode_OobIndexBuffer: {
                ...
                skip |= gpuav.LogError(
                    ...
                    "] (.firstIndex = %" PRIu32 ", .indexCount = %" PRIu32 "), stored in %s\n"
...
struct DrawIndexedIndirectVertexBufferShader {
    static size_t GetSpirvSize() {
        return cmd_validation_draw_indexed_indirect_vertex_buffer_comp_size * sizeof(uint32_t);
    }
...
void DrawIndexedIndirectVertexBuffer(Validator& gpuav,
                                     CommandBuffer& cb_state,
                                     const Location& loc,
                                     VkBuffer draw_buffer,
                                     VkDeviceSize draw_buffer_offset,
            ...
            switch (error_record[kHeaderErrorSubCodeOffset]) {
                case kErrorSubCode_OobVertexBuffer: {
                    ...
                    skip |= gpuav.LogError(
                        ...
                        objlist,
                        loc_with_debug_region,
                        "Vertex index %" PRIu32 " is not within the smallest bound vertex buffer. Computed from "
                        "VkDrawIndexedIndirectCommand[ %" PRIu32 " ], stored in %s.\n"
                        "index_buffer[ %" PRIu32 " ] (%" PRIu32 ") + vertexOffset (%" PRIi32 ") = Vertex index %" PRIu32
                        "\n"
