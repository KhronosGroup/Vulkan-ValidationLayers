...
struct Type {
    Type(SpvType spv_type, const Instruction& insn) : spv_type_(spv_type), insn_(insn) {}

    bool operator==(Type const& other) const;
    uint32_t Id() const { return insn_.ResultId(); }

    const SpvType spv_type_;
    const Instruction& insn_;
};

// Represents a OpConstant*
// Will not be a OpSpecConstant* because we don't know if the value will change
struct Constant {
    Constant(const Type& type, const Instruction& insn) : type_(type), insn_(insn) {}
...
struct Variable {
    Variable(const Type& type, const Instruction& insn) : type_(type), insn_(insn) {}

    uint32_t Id() const { return insn_.ResultId(); }
    spv::StorageClass StorageClass() const { return spv::StorageClass(insn_.Word(3)); }
...
class TypeManager {
  public:
    ...
    const Type& AddType(std::unique_ptr<Instruction> new_insn, SpvType spv_type);
    ...
    const Type& GetTypeArray(const Type& element_type, const Type& length_type);
    ...
    const Constant& AddConstant(std::unique_ptr<Instruction> new_insn, const Type& type);
    ...
    const Variable& AddVariable(std::unique_ptr<Instruction> new_insn, const Type& type);
    const Variable* FindVariableById(uint32_t id) const;

  private:
    Module& module_;

    vvl::unordered_map<uint32_t, const Type*> id_to_type_;
    vvl::unordered_map<uint32_t, const Constant*> id_to_constant_;
    vvl::unordered_map<uint32_t, const Variable*> id_to_variable_;
    ...
    // one we fine. We should only be adding a new Type because it currently doesn't exists.
    // Use list over vector because we only use this for storage, but use the fast-lookups below to ever transverse the list
