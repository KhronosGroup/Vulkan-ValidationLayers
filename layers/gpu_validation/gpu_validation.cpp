    ...
    if (validate_descriptor_indexing) {
        ...
        if (descriptor_indexing && !force_buffer_device_address) {
            ...
            LogWarning(
                ...
                "Buffer Device Address from API 1.2+ feature is not available.  No descriptor indexing checking will be attempted");
        }
    }
...
bool GpuAssisted::InstrumentShader(const vvl::span<const uint32_t> &input, std::vector<uint32_t> &new_pgm,
    ...
    // TODO - Expect this interface to substantially change
