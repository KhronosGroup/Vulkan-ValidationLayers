...
bool CommandBufferAccessContext::ValidateDrawVertex(uint32_t vertexCount, uint32_t firstVertex, const char *func_name) const {
    ...
    const auto *pPipe = GetCurrentPipelineShaderGroupFromCommandBuffer(*cb_state_.get(), VK_PIPELINE_BIND_POINT_GRAPHICS, shaderGroup);
    if (!pPipe) {
        return skip;
    }
    ...
    const auto &binding_descriptions_size = pPipe->getVertexState(shaderGroup).binding_descriptions_.size();
    

    for (size_t i = 0; i < binding_descriptions_size; ++i) {
        const auto &binding_description = pPipe->getVertexState(shaderGroup).binding_descriptions_[i];
