...
const uint32_t kNumBindingsInSet = 1;
...
SPFDescriptorSetManager::SPFDescriptorSetManager(ShaderPrintf *dev_data) { dev_data_ = dev_data; }

SPFDescriptorSetManager::~SPFDescriptorSetManager() {
...
VkResult SPFDescriptorSetManager::GetDescriptorSet(VkDescriptorPool *desc_pool, VkDescriptorSet *desc_set) {
...
VkResult SPFDescriptorSetManager::GetDescriptorSets(uint32_t count, VkDescriptorPool *pool,
                                                    std::vector<VkDescriptorSet> *desc_sets) {
...
void SPFDescriptorSetManager::PutBackDescriptorSet(VkDescriptorPool desc_pool, VkDescriptorSet desc_set) {
...
VkResult ShaderPrintf::InitializeVma(VkPhysicalDevice physical_device, VkDevice device, VmaAllocator *pAllocator) {
...
void ShaderPrintf::ReportSetupProblem(const char *const specific_message) const {
...
void ShaderPrintf::PostCallRecordCreateDevice(VkPhysicalDevice physicalDevice, const VkDeviceCreateInfo *pCreateInfo,
    ...
    if (physical_device_properties.apiVersion < VK_API_VERSION_1_1) {
        ReportSetupProblem("Shader Debug Printf requires Vulkan 1.1 or later.  Shader Debug Printf disabled.");
    ...
    if (!supported_features.fragmentStoresAndAtomics || !supported_features.vertexPipelineStoresAndAtomics) {
        ReportSetupProblem(
            "Shader Debug Printf requires fragmentStoresAndAtomics and vertexPipelineStoresAndAtomics.  "
            "Shader Debug Printf disabled.");
        device_shader_printf->aborted = true;
        return;
    }
    ...
    device_shader_printf->vkSetDeviceLoaderData = chain_info->u.pfnSetDeviceLoaderData;
    ...
    device_shader_printf->adjusted_max_desc_sets = physical_device_properties.limits.maxBoundDescriptorSets;
    device_shader_printf->adjusted_max_desc_sets = std::min(33U, device_shader_printf->adjusted_max_desc_sets);
    ...
    if (device_shader_printf->adjusted_max_desc_sets == 1) {
        ReportSetupProblem("Device can bind only a single descriptor set.  GPU-Assisted Validation disabled.");
        device_shader_printf->aborted = true;
        return;
    }
    device_shader_printf->desc_set_bind_index = device_shader_printf->adjusted_max_desc_sets - 1;
    LogInfo(device, "UNASSIGNED-Shader Printf ", "Shaders using descriptor set at index %d. ",
            device_shader_printf->desc_set_bind_index);

    device_shader_printf->output_buffer_size = 1024;  // 256 32-bit words  ToDo - ovverride this with user request
    VkResult result1 = InitializeVma(physicalDevice, *pDevice, &device_shader_printf->vmaAllocator);
    assert(result1 == VK_SUCCESS);
    std::unique_ptr<SPFDescriptorSetManager> desc_set_manager(new SPFDescriptorSetManager(device_shader_printf));

    const VkDescriptorSetLayoutBinding debug_desc_layout_bindings[1] = {
        {
            3,  // printf output
            VK_DESCRIPTOR_TYPE_STORAGE_BUFFER,
            1,
    ...
    result1 = DispatchCreateDescriptorSetLayout(*pDevice, &debug_desc_layout_info, NULL, &device_shader_printf->debug_desc_layout);

    // This is a layout used to "pad" a pipeline layout to fill in any gaps to the selected bind index.
    VkResult result2 =
        DispatchCreateDescriptorSetLayout(*pDevice, &dummy_desc_layout_info, NULL, &device_shader_printf->dummy_desc_layout);
    assert((result1 == VK_SUCCESS) && (result2 == VK_SUCCESS));
    if ((result1 != VK_SUCCESS) || (result2 != VK_SUCCESS)) {
        ReportSetupProblem("Unable to create descriptor set layout.  GPU-Assisted Validation disabled.");
        if (result == VK_SUCCESS) {
            DispatchDestroyDescriptorSetLayout(*pDevice, device_shader_printf->debug_desc_layout, NULL);
        }
        if (result2 == VK_SUCCESS) {
            DispatchDestroyDescriptorSetLayout(*pDevice, device_shader_printf->dummy_desc_layout, NULL);
        }
        device_shader_printf->debug_desc_layout = VK_NULL_HANDLE;
        device_shader_printf->dummy_desc_layout = VK_NULL_HANDLE;
        device_shader_printf->aborted = true;
        return;
    }
    device_shader_printf->desc_set_manager = std::move(desc_set_manager);

    // Register callback to be called at any ResetCommandBuffer time
    device_shader_printf->SetCommandBufferResetCallback([device_shader_printf](VkCommandBuffer command_buffer) -> void {
        device_shader_printf->ResetCommandBuffer(command_buffer);
    });
}
...
void ShaderPrintf::PreCallRecordDestroyDevice(VkDevice device, const VkAllocationCallbacks *pAllocator) {
    for (auto &queue_barrier_command_info_kv : queue_barrier_command_infos) {
        SPFQueueBarrierCommandInfo &queue_barrier_command_info = queue_barrier_command_info_kv.second;
...
void ShaderPrintf::PreCallRecordCreatePipelineLayout(VkDevice device, const VkPipelineLayoutCreateInfo *pCreateInfo,
    ...
    if (cpl_state->modified_create_info.setLayoutCount >= adjusted_max_desc_sets) {
        ...
        ReportSetupProblem(strm.str().c_str());
    } else {
...
void ShaderPrintf::PostCallRecordCreatePipelineLayout(VkDevice device, const VkPipelineLayoutCreateInfo *pCreateInfo,
    ...
    if (result != VK_SUCCESS) {
        ReportSetupProblem("Unable to create pipeline layout.  Device could become unstable.");
        aborted = true;
    }
}
...
void ShaderPrintf::ResetCommandBuffer(VkCommandBuffer commandBuffer) {
    ...
    auto shader_printf_buffer_list = GetSPFBufferInfo(commandBuffer);
...
void ShaderPrintf::PreCallRecordPipelineCreations(uint32_t count, const CreateInfo *pCreateInfos,
                                                  const VkAllocationCallbacks *pAllocator, VkPipeline *pPipelines,
...
//   - Save the shader binary to search for format flags
template <typename CreateInfo>
void ShaderPrintf::PostCallRecordPipelineCreations(const uint32_t count, const CreateInfo *pCreateInfos,
                                                   const VkAllocationCallbacks *pAllocator, VkPipeline *pPipelines,
                                                   const VkPipelineBindPoint bind_point) {
    using Accessor = CreatePipelineTraits<CreateInfo>;
    if (bind_point != VK_PIPELINE_BIND_POINT_GRAPHICS && bind_point != VK_PIPELINE_BIND_POINT_COMPUTE &&
...
bool ShaderPrintf::InstrumentShader(const VkShaderModuleCreateInfo *pCreateInfo, std::vector<unsigned int> &new_pgm,
    ...
    if (!pass) {
        ReportSetupProblem("Failure to instrument shader.  Proceeding with non-instrumented shader.");
    }
...
static void GenerateSourceMessages(const std::vector<unsigned int> &pgm, const uint32_t *debug_record, bool from_printf,
...
void ShaderPrintf::AnalyzeAndPrintfMessages(CMD_BUFFER_STATE *cb_node, VkQueue queue, VkPipelineBindPoint pipeline_bind_point,
    ...
    while (debug_output_buffer[index]) {
        ...
        shader_message << "\"";
        GenerateStageMessage(&debug_output_buffer[index], stage_message);
        GenerateCommonMessage(report_data, cb_node, &debug_output_buffer[index], shader_module_handle, pipeline_handle,
                              pipeline_bind_point, operation_index, common_message);
        GenerateSourceMessages(pgm, &debug_output_buffer[index], true, filename_message, source_message);
...
void ShaderPrintf::ProcessInstrumentationBuffer(VkQueue queue, CMD_BUFFER_STATE *cb_node) {
    if (cb_node && (cb_node->hasDrawCmd || cb_node->hasTraceRaysCmd || cb_node->hasDispatchCmd)) {
        auto gpu_buffer_list = GetSPFBufferInfo(cb_node->commandBuffer);
        ...
        for (auto &buffer_info : gpu_buffer_list) {
            ...
            if (result == VK_SUCCESS) {
                AnalyzeAndPrintfMessages(cb_node, queue, buffer_info.pipeline_bind_point, operation_index, (uint32_t *)pData);
...
void ShaderPrintf::SubmitBarrier(VkQueue queue) {
    auto queue_barrier_command_info_it = queue_barrier_command_infos.emplace(queue, SPFQueueBarrierCommandInfo{});
    if (queue_barrier_command_info_it.second) {
        SPFQueueBarrierCommandInfo &queue_barrier_command_info = queue_barrier_command_info_it.first->second;
        ...
        if (result != VK_SUCCESS) {
            ReportSetupProblem("Unable to create command pool for barrier CB.");
            queue_barrier_command_info.barrier_command_pool = VK_NULL_HANDLE;
            return;
        }
        ...
        if (result != VK_SUCCESS) {
            ReportSetupProblem("Unable to create barrier command buffer.");
...
void ShaderPrintf::PostCallRecordQueueSubmit(VkQueue queue, uint32_t submitCount, const VkSubmitInfo *pSubmits, VkFence fence,
    ...
    for (uint32_t submit_idx = 0; submit_idx < submitCount; submit_idx++) {
        ...
        for (uint32_t i = 0; i < submit->commandBufferCount; i++) {
            ...
            if (GetSPFBufferInfo(cb_node->commandBuffer).size() || cb_node->hasBuildAccelerationStructureCmd)
                buffers_present = true;
            for (auto secondaryCmdBuffer : cb_node->linkedCommandBuffers) {
                if (GetSPFBufferInfo(secondaryCmdBuffer->commandBuffer).size() || cb_node->hasBuildAccelerationStructureCmd)
                    buffers_present = true;
            }
        }
    }
...
void ShaderPrintf::AllocateShaderPrintfResources(const VkCommandBuffer cmd_buffer, const VkPipelineBindPoint bind_point) {
    ...
    assert(result == VK_SUCCESS);
    if (result != VK_SUCCESS) {
        ReportSetupProblem("Unable to allocate descriptor sets.  Device could become unstable.");
        aborted = true;
        return;
    }
    ...
    if (!cb_node) {
        ReportSetupProblem("Unrecognized command buffer");
        aborted = true;
        return;
    }
    ...
    if (result != VK_SUCCESS) {
        ReportSetupProblem("Unable to allocate device memory.  Device could become unstable.");
        aborted = true;
        return;
    }
    ...
    if (iter != cb_node->lastBound.end()) {
        ...
        GetSPFBufferInfo(cmd_buffer).emplace_back(output_block, desc_sets[0], desc_pool, bind_point);
    } else {
        ReportSetupProblem("Unable to find pipeline state");
