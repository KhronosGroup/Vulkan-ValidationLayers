...
typedef struct VkLayerDbgFunctionState_ {
    bool is_messenger;
    union {
    ...
    void *pUserData;
    struct VkLayerDbgFunctionState_ *pNext;
...
typedef struct _debug_report_data {
    VkLayerDbgFunctionState *debug_callback_list{nullptr};
    VkLayerDbgFunctionState *default_debug_callback_list{nullptr};
...
static inline void AddDebugCallbackNode(debug_report_data *debug_data, VkLayerDbgFunctionState **list_head,
                                        VkLayerDbgFunctionState *new_node) {
...
static inline void RemoveDebugUtilsMessageCallback(debug_report_data *debug_data, VkLayerDbgFunctionState **list_head,
...
static inline void RemoveAllMessageCallbacks(debug_report_data *debug_data, VkLayerDbgFunctionState **list_head) {
    VkLayerDbgFunctionState *current_callback = *list_head;

    while (current_callback) {
        VkLayerDbgFunctionState *next_callback = current_callback->pNext;
...
static inline bool debug_log_msg(const debug_report_data *debug_data, VkFlags msg_flags, VkDebugReportObjectTypeEXT object_type,
                                 ...
                                 const char *text_vuid) {
    bool bail = false;
    VkLayerDbgFunctionState *layer_dbg_node = NULL;
...
static inline VkResult layer_create_messenger_callback(debug_report_data *debug_data, bool default_callback,
                                                       ...
                                                       VkDebugUtilsMessengerEXT *messenger) {
    std::unique_lock<std::mutex> lock(debug_data->debug_report_mutex);
    VkLayerDbgFunctionState *pNewDbgFuncNode = (VkLayerDbgFunctionState *)malloc(sizeof(VkLayerDbgFunctionState));
    if (!pNewDbgFuncNode) return VK_ERROR_OUT_OF_HOST_MEMORY;
    memset(pNewDbgFuncNode, 0, sizeof(VkLayerDbgFunctionState));
