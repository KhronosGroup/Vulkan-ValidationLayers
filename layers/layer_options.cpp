...
static std::string GetDeprecatedEnabledDisabledSettingsWarning(const std::vector<std::string>& enabled,
    ...
    const std::array enabled_new_settings = {
    ...
    const std::array disabled_new_settings = {
    ...
    std::stringstream ss;
    ss << "Application is using deprecated \"enables\" (env var: VK_LAYER_ENABLES) and/or \"disables\" (env var: "
          "VK_LAYER_DISABLES) "
    ...
    for (const std::string& deprecated_string : enabled) {
        for (const DS& ds : enabled_new_settings) {
            if (deprecated_string == ds.deprecated_setting) {
                ss << "  Deprecated: " << ds.deprecated_setting << " | New: \"" << ds.new_setting
                   << "\" | Env var: " << ds.new_setting_env_var << "=1\n";
                break;
            }
        }
    }
    for (const std::string& deprecated_string : disabled) {
        for (const DS& ds : disabled_new_settings) {
            if (deprecated_string == ds.deprecated_setting) {
                ss << "  Deprecated: " << ds.deprecated_setting << " | New: \"" << ds.new_setting
                   << "\" | Env var: " << ds.new_setting_env_var << "=0\n";
                break;
            }
        }
    }
...
void ProcessConfigAndEnvSettings(ConfigAndEnvSettings *settings_data) {
    ...
    if (!disabled.empty() || !enabled.empty()) {
        setting_warnings.emplace_back(GetDeprecatedEnabledDisabledSettingsWarning(enabled, disabled));
    }
