...
// Post Process are designed to allow the user to "assume" the access is valid and want to know after the GPU executes
// what happened. These are much lighter checks and can be used while the rest of GPU-AV is turned off
...
void CreateFilterMessageIdList(std::string raw_id_list,
                               const std::string& delimiter,
                               vvl::unordered_set<uint32_t>& filter_list) {
    size_t pos = 0;
    std::string token;
...
// Because VkLayerSettingsCreateInfoEXT/VkLayerSettingEXT are passed in and used before everything else, need to do the
// stateless validation here as a special exception
//
// We want to use the DebugReport object to print VU error messages in here, but we need to parse the settings in
// VkLayerSettingsCreateInfoEXT in order to configure the DebugReport object. Therefor we just settle with printf in
// here. These are very unlikely things to hit validation layers messages (as everything after will likely crumble) so
// should be ok.
//
// Returns if valid
    ...
    if (layer_settings->pSettings) {
        for (const auto [i, setting] : vvl::enumerate(layer_settings->pSettings, layer_settings->settingCount)) {
            ...
            if (!setting->pLayerName) {
                ss << "[ VUID-VkLayerSettingEXT-pLayerName-parameter ] "
                   << setting_loc.dot(vvl::Field::pLayerName).Message() << " is NULL";
                printf("Validation Layer Error: %s\n", ss.str().c_str());
                valid = false;
            }
            if (!setting->pSettingName) {
                ss << "[ VUID-VkLayerSettingEXT-pSettingName-parameter ] "
                   << setting_loc.dot(vvl::Field::pSettingName).Message() << " is NULL";
                printf("Validation Layer Error: %s\n", ss.str().c_str());
                valid = false;
            }
        }
    } else if (layer_settings->settingCount > 0) {
        ss << "[ VUID-VkLayerSettingsCreateInfoEXT-pSettings-parameter ] "
           << create_info_loc.dot(vvl::Field::pSettings).Message() << " is NULL";
        printf("Validation Layer Error: %s\n", ss.str().c_str());
        valid = false;
    }
    return valid;
...
static void ValidateLayerSettingsProvided(const VkLayerSettingsCreateInfoEXT* layer_setting_create_info,
                                          VkuLayerSettingSet layer_setting_set,
    ...
    for (uint32_t i = 0; i < layer_setting_create_info->settingCount; i++) {
        ...
        // Debugging settings are not added here, those are for internal development and not designed for an app to use
        // via VkLayerSettings API
        ...
        if (required_type != VK_LAYER_SETTING_TYPE_MAX_ENUM_EXT && setting.type != required_type) {
            setting_warnings.emplace_back(
                "The setting " + std::string(setting.pSettingName) +
                " in VkLayerSettingsCreateInfoEXT was set to type " +
...
static void SetValidationSetting(VkuLayerSettingSet layer_setting_set,
                                 CHECK_DISABLED& disable_data,
                                 const DisableFlags feature_disable,
                                 const char* setting) {
...
static void SetValidationSetting(VkuLayerSettingSet layer_setting_set,
                                 CHECK_ENABLED& enable_data,
                                 const EnableFlags feature_enable,
                                 const char* setting) {
    if (vkuHasLayerSetting(layer_setting_set, setting)) {
        bool enabled = true;
        vkuGetLayerSettingValue(layer_setting_set, setting, enabled);
...
static void ProcessDebugReportSettings(ConfigAndEnvSettings* settings_data,
                                       VkuLayerSettingSet& layer_setting_set,
                                       std::vector<std::string>& setting_warnings) {
    ...
    if (vkuHasLayerSetting(layer_setting_set, VK_LAYER_MESSAGE_FORMAT_DISPLAY_APPLICATION_NAME)) {
        vkuGetLayerSettingValue(layer_setting_set,
                                VK_LAYER_MESSAGE_FORMAT_DISPLAY_APPLICATION_NAME,
    ...
    if (settings_data->gpuav_settings->debug_printf_enabled) {
        if (settings_data->gpuav_settings->debug_printf_to_stdout && (debug_action & VK_DBG_LAYER_ACTION_LOG_MSG)) {
            if (is_stdout) {
                ...
                setting_warnings.emplace_back("The logging to " + log_filename +
                                              " will not contain any DebugPrintf info because " +
        ...
        if (!settings_data->gpuav_settings->debug_printf_to_stdout && ((report_flags & kInformationBit) == 0)) {
            // Normally it is a lot of spam to use kInformationBit, but if only using DebugPrintf, it should be minimal
            // information printed
            setting_warnings.emplace_back("DebugPrintf logs to the Information message severity, enabling Information "
                                          "level logging otherwise the message "
                                          "will not be seen.");
            report_flags |= kInformationBit;
        }
        // If any non-stdout DebugPrintf is being used, just turn off duplicate_message_limit, it will prevent people
        // thinking DebugPrintf is broken because nothing is printing.
        if (!settings_data->gpuav_settings->debug_printf_to_stdout && debug_report->duplicate_message_limit != 0) {
            ...
            setting_warnings.emplace_back(
                "DebugPrintf logs can possibly print many times, but duplicate_message_limit is set to " +
    ...
    if (debug_action & VK_DBG_LAYER_ACTION_LOG_MSG) {
        ...
        if (log_output != stdout) {
            // This particular warning is designed to show the user where the debug callback is going (which is
            // important to know!), so it makes no sense to put the warning in the callback location. For this one only
            // we attempt to print to the everywhere else possible
...
void ProcessConfigAndEnvSettings(ConfigAndEnvSettings* settings_data) {
    ...
    // We need to send all warnings through the DebugCallback (printf will go into the void and likely is not seen for
    // people), but we also need to get settings setup before creating the DebugCallback. This is the only spot that
    // needs this, so we create a temp list of strings here and shove them through DebugCallback right after we call it.
    // This function should not be a bottleneck so some extra string building should be ok here.
    std::vector<std::string> setting_warnings;
    ...
    if (vkuHasLayerSetting(layer_setting_set, VK_LAYER_DEBUG_DISABLE_SPIRV_VAL)) {
        vkuGetLayerSettingValue(
            layer_setting_set, VK_LAYER_DEBUG_DISABLE_SPIRV_VAL, global_settings.debug_disable_spirv_val);
    }
    ...
    if (vkuHasLayerSetting(layer_setting_set, VK_LAYER_GPUAV_SHADER_INSTRUMENTATION)) {
        vkuGetLayerSettingValue(
            layer_setting_set, VK_LAYER_GPUAV_SHADER_INSTRUMENTATION, shader_instrumentation_enabled);
    }
    ...
    if (!shader_instrumentation_enabled) {
        ...
        if (vkuHasLayerSetting(layer_setting_set, VK_LAYER_GPUAV_DESCRIPTOR_CHECKS)) {
            vkuGetLayerSettingValue(layer_setting_set,
                                    VK_LAYER_GPUAV_DESCRIPTOR_CHECKS,
        ...
        if (vkuHasLayerSetting(layer_setting_set, VK_LAYER_GPUAV_POST_PROCESS_DESCRIPTOR_INDEXING)) {
            vkuGetLayerSettingValue(layer_setting_set,
                                    VK_LAYER_GPUAV_POST_PROCESS_DESCRIPTOR_INDEXING,
        ...
        if (vkuHasLayerSetting(layer_setting_set, VK_LAYER_GPUAV_WARN_ON_ROBUST_OOB)) {
            vkuGetLayerSettingValue(
                layer_setting_set, VK_LAYER_GPUAV_WARN_ON_ROBUST_OOB, gpuav_settings.warn_on_robust_oob);
        } else if (vkuHasLayerSetting(layer_setting_set, DEPRECATED_GPUAV_WARN_ON_ROBUST_OOB)) {
            vkuGetLayerSettingValue(
                layer_setting_set, DEPRECATED_GPUAV_WARN_ON_ROBUST_OOB, gpuav_settings.warn_on_robust_oob);
            setting_warnings.emplace_back("Deprecated " + std::string(DEPRECATED_GPUAV_WARN_ON_ROBUST_OOB) +
                                          " setting was set, use " + std::string(VK_LAYER_GPUAV_WARN_ON_ROBUST_OOB) +
                                          " instead.");
        }

        if (vkuHasLayerSetting(layer_setting_set, VK_LAYER_GPUAV_BUFFER_ADDRESS_OOB)) {
            vkuGetLayerSettingValue(layer_setting_set,
                                    VK_LAYER_GPUAV_BUFFER_ADDRESS_OOB,
        ...
        if (vkuHasLayerSetting(layer_setting_set, VK_LAYER_GPUAV_MAX_BUFFER_DEVICE_ADDRESSES)) {
            ...
            vkuGetLayerSettingValue(
                layer_setting_set, VK_LAYER_GPUAV_MAX_BUFFER_DEVICE_ADDRESSES, gpuav_settings.max_bda_in_use);
        ...
        if (vkuHasLayerSetting(layer_setting_set, VK_LAYER_GPUAV_VALIDATE_RAY_QUERY)) {
            vkuGetLayerSettingValue(
                layer_setting_set, VK_LAYER_GPUAV_VALIDATE_RAY_QUERY, gpuav_settings.shader_instrumentation.ray_query);
        }

        if (vkuHasLayerSetting(layer_setting_set, VK_LAYER_GPUAV_SELECT_INSTRUMENTED_SHADERS)) {
            vkuGetLayerSettingValue(layer_setting_set,
                                    VK_LAYER_GPUAV_SELECT_INSTRUMENTED_SHADERS,
            ...
            vkuGetLayerSettingValue(layer_setting_set,
                                    DEPRECATED_GPUAV_SELECT_INSTRUMENTED_SHADERS,
                                    gpuav_settings.select_instrumented_shaders);
            setting_warnings.emplace_back("Deprecated " + std::string(DEPRECATED_GPUAV_SELECT_INSTRUMENTED_SHADERS) +
                                          " setting was set, use " +
                                          std::string(VK_LAYER_GPUAV_SELECT_INSTRUMENTED_SHADERS) + " instead.");
        }
    ...
    if (!buffers_validation_enabled) {
        ...
        if (vkuHasLayerSetting(layer_setting_set, VK_LAYER_GPUAV_INDIRECT_DRAWS_BUFFERS)) {
            vkuGetLayerSettingValue(layer_setting_set,
                                    VK_LAYER_GPUAV_INDIRECT_DRAWS_BUFFERS,
        ...
        if (vkuHasLayerSetting(layer_setting_set, VK_LAYER_GPUAV_INDIRECT_DISPATCHES_BUFFERS)) {
            vkuGetLayerSettingValue(layer_setting_set,
                                    VK_LAYER_GPUAV_INDIRECT_DISPATCHES_BUFFERS,
        ...
        if (vkuHasLayerSetting(layer_setting_set, VK_LAYER_GPUAV_INDIRECT_TRACE_RAYS_BUFFERS)) {
            vkuGetLayerSettingValue(layer_setting_set,
                                    VK_LAYER_GPUAV_INDIRECT_TRACE_RAYS_BUFFERS,
                                    gpuav_settings.validate_indirect_trace_rays_buffers);
        }
        if (vkuHasLayerSetting(layer_setting_set, VK_LAYER_GPUAV_BUFFER_COPIES)) {
            vkuGetLayerSettingValue(
                layer_setting_set, VK_LAYER_GPUAV_BUFFER_COPIES, gpuav_settings.validate_buffer_copies);
        } else if (vkuHasLayerSetting(layer_setting_set, DEPRECATED_VK_LAYER_GPUAV_VALIDATE_COPIES)) {
            vkuGetLayerSettingValue(
                layer_setting_set, DEPRECATED_VK_LAYER_GPUAV_VALIDATE_COPIES, gpuav_settings.validate_buffer_copies);
            setting_warnings.emplace_back("Deprecated " + std::string(DEPRECATED_VK_LAYER_GPUAV_VALIDATE_COPIES) +
                                          " setting was set, use " + std::string(VK_LAYER_GPUAV_BUFFER_COPIES) +
                                          " instead.");
        }
        if (vkuHasLayerSetting(layer_setting_set, VK_LAYER_GPUAV_INDEX_BUFFERS)) {
            vkuGetLayerSettingValue(
                layer_setting_set, VK_LAYER_GPUAV_INDEX_BUFFERS, gpuav_settings.validate_index_buffers);
        }
    }
    ...
    if (vkuHasLayerSetting(layer_setting_set, VK_LAYER_GPUAV_RESERVE_BINDING_SLOT)) {
        SetValidationSetting(layer_setting_set,
                             settings_data->enables,
        ...
        setting_warnings.emplace_back("Deprecated " + std::string(DEPRECATED_VK_LAYER_RESERVE_BINDING_SLOT) +
                                      " setting was set, use " + std::string(VK_LAYER_GPUAV_RESERVE_BINDING_SLOT) +
                                      " instead.");
    }
    ...
    if (vkuHasLayerSetting(layer_setting_set, VK_LAYER_GPUAV_VMA_LINEAR_OUTPUT)) {
        ...
        vkuGetLayerSettingValue(
            layer_setting_set, DEPRECATED_GPUAV_VMA_LINEAR_OUTPUT, gpuav_settings.vma_linear_output);
        setting_warnings.emplace_back("Deprecated " + std::string(DEPRECATED_GPUAV_VMA_LINEAR_OUTPUT) +
                                      " setting was set, use " + std::string(VK_LAYER_GPUAV_VMA_LINEAR_OUTPUT) +
                                      " instead.");
    }

    if (vkuHasLayerSetting(layer_setting_set, VK_LAYER_GPUAV_DEBUG_VALIDATE_INSTRUMENTED_SHADERS)) {
        vkuGetLayerSettingValue(layer_setting_set,
                                VK_LAYER_GPUAV_DEBUG_VALIDATE_INSTRUMENTED_SHADERS,
    ...
    if (vkuHasLayerSetting(layer_setting_set, VK_LAYER_GPUAV_DEBUG_DUMP_INSTRUMENTED_SHADERS)) {
        vkuGetLayerSettingValue(layer_setting_set,
                                VK_LAYER_GPUAV_DEBUG_DUMP_INSTRUMENTED_SHADERS,
    ...
    if (vkuHasLayerSetting(layer_setting_set, VK_LAYER_GPUAV_DEBUG_MAX_INSTRUMENTATIONS_COUNT)) {
        vkuGetLayerSettingValue(layer_setting_set,
                                VK_LAYER_GPUAV_DEBUG_MAX_INSTRUMENTATIONS_COUNT,
    ...
    if (vkuHasLayerSetting(layer_setting_set, VK_LAYER_GPUAV_DEBUG_PRINT_INSTRUMENTATION_INFO)) {
        vkuGetLayerSettingValue(layer_setting_set,
                                VK_LAYER_GPUAV_DEBUG_PRINT_INSTRUMENTATION_INFO,
    ...
    if (vkuHasLayerSetting(layer_setting_set, VK_LAYER_PRINTF_BUFFER_SIZE)) {
        ...
        vkuGetLayerSettingValue(
            layer_setting_set, VK_LAYER_PRINTF_BUFFER_SIZE, gpuav_settings.debug_printf_buffer_size);
    ...
    if (vkuHasLayerSetting(layer_setting_set, VK_LAYER_SYNCVAL_SUBMIT_TIME_VALIDATION)) {
        vkuGetLayerSettingValue(
            layer_setting_set, VK_LAYER_SYNCVAL_SUBMIT_TIME_VALIDATION, syncval_settings.submit_time_validation);
    } else if (vkuHasLayerSetting(layer_setting_set, DEPRECATED_VK_LAYER_VALIDATE_SYNC_QUEUE_SUBMIT)) {
        vkuGetLayerSettingValue(
            layer_setting_set, DEPRECATED_VK_LAYER_VALIDATE_SYNC_QUEUE_SUBMIT, syncval_settings.submit_time_validation);
    ...
    if (vkuHasLayerSetting(layer_setting_set, VK_LAYER_SYNCVAL_SHADER_ACCESSES_HEURISTIC)) {
        vkuGetLayerSettingValue(
            layer_setting_set, VK_LAYER_SYNCVAL_SHADER_ACCESSES_HEURISTIC, syncval_settings.shader_accesses_heuristic);
    }

    if (vkuHasLayerSetting(layer_setting_set, VK_LAYER_SYNCVAL_MESSAGE_EXTRA_PROPERTIES)) {
        vkuGetLayerSettingValue(
            layer_setting_set, VK_LAYER_SYNCVAL_MESSAGE_EXTRA_PROPERTIES, syncval_settings.message_extra_properties);
    }

    if (vkuHasLayerSetting(layer_setting_set, VK_LAYER_SYNCVAL_MESSAGE_EXTRA_PROPERTIES_PRETTY_PRINT)) {
        vkuGetLayerSettingValue(layer_setting_set,
                                VK_LAYER_SYNCVAL_MESSAGE_EXTRA_PROPERTIES_PRETTY_PRINT,
                                syncval_settings.message_extra_properties_pretty_print);
    }

    const auto* validation_features_ext =
        vku::FindStructInPNextChain<VkValidationFeaturesEXT>(settings_data->create_info);
    ...
    if (use_fine_grained_settings) {
        SetValidationSetting(
            layer_setting_set, settings_data->enables, best_practices, VK_LAYER_VALIDATE_BEST_PRACTICES);
        SetValidationSetting(
            layer_setting_set, settings_data->enables, vendor_specific_arm, VK_LAYER_VALIDATE_BEST_PRACTICES_ARM);
        SetValidationSetting(
            layer_setting_set, settings_data->enables, vendor_specific_amd, VK_LAYER_VALIDATE_BEST_PRACTICES_AMD);
        SetValidationSetting(
            layer_setting_set, settings_data->enables, vendor_specific_img, VK_LAYER_VALIDATE_BEST_PRACTICES_IMG);
        SetValidationSetting(
            layer_setting_set, settings_data->enables, vendor_specific_nvidia, VK_LAYER_VALIDATE_BEST_PRACTICES_NVIDIA);
        ...
        if (vkuHasLayerSetting(layer_setting_set, DEPRECATED_VK_LAYER_VALIDATE_GPU_BASED)) {
            ...
            if (setting_value == "GPU_BASED_GPU_ASSISTED") {
                ...
                setting_warnings.emplace_back("Deprecated " + std::string(DEPRECATED_VK_LAYER_VALIDATE_GPU_BASED) +
                                              " setting was set, use " + std::string(VK_LAYER_GPUAV_ENABLE) +
                                              " instead.");
            } else if (setting_value == "GPU_BASED_DEBUG_PRINTF") {
                settings_data->enables[debug_printf_validation] = true;
                setting_warnings.emplace_back("Deprecated " + std::string(DEPRECATED_VK_LAYER_VALIDATE_GPU_BASED) +
                                              " setting was set, use " + std::string(VK_LAYER_PRINTF_ENABLE) +
                                              " instead.");
            }
        }
    }
    ...
    if (use_fine_grained_settings) {
        ...
        SetValidationSetting(
            layer_setting_set, settings_data->disables, command_buffer_state, VK_LAYER_CHECK_COMMAND_BUFFER);
        ...
        SetValidationSetting(
            layer_setting_set, settings_data->disables, image_layout_validation, VK_LAYER_CHECK_IMAGE_LAYOUT);
        ...
        SetValidationSetting(
            layer_setting_set, settings_data->disables, shader_validation_caching, VK_LAYER_CHECK_SHADERS_CACHING);
    }
    ...
    if (vkuHasLayerSetting(layer_setting_set, VK_LAYER_PRINTF_ONLY_PRESET)) {
        ...
        if (printf_only_preset) {
            ...
            SetValidationFeatureDisable(settings_data->disables,
                                        VK_VALIDATION_FEATURE_DISABLE_SHADER_VALIDATION_CACHE_EXT);
            setting_warnings.emplace_back(
