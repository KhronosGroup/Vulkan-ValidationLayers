...
struct RESTORABLE_PIPELINE_STATE {
    ...
    void Create(CMD_BUFFER_STATE *cb_state, VkPipelineBindPoint bind_point) {
        ...
        if (last_bound.pipeline_state) {
            ...
            descriptor_sets.reserve(last_bound.boundDescriptorSets.size());
            for (std::size_t i = 0; i < last_bound.boundDescriptorSets.size(); i++) {
                const auto *bound_descriptor_set = last_bound.boundDescriptorSets[i];
            ...
            dynamic_offsets = last_bound.dynamicOffsets;
