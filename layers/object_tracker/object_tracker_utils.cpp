...
ObjectLifetimes::ObjectLifetimes(vvl::dispatch::Instance* instance) :
    BaseClass(instance, LayerObjectTypeObjectTracker) {
    WriteLockGuard lock(lifetime_set_mutex);
    lifetime_set.insert(this);
}
...
bool ObjectLifetimes::CheckObjectValidity(uint64_t object_handle,
    ...
    if (TracksObject(object_handle, object_type)) {
        ...
        // If destroying, even if the child libraries are gone, the user still a way to remove the bad parent pipeline
        // library
    ...
    if (!other_lifetimes) {
        return LogError(invalid_handle_vuid,
                        instance,
                        loc,
                        "Invalid %s Object 0x%" PRIxLEAST64 ".",
    ...
    return LogError(
        ...
        string_VulkanObjectType(object_type),
        object_handle,
        other_handle_str.c_str(),
        handle_str.c_str());
}

bool ObjectLifetimes::CheckPipelineObjectValidity(uint64_t object_handle,
                                                  const char* invalid_handle_vuid,
...
void ObjectLifetimes::DestroyObjectSilently(uint64_t object, VulkanObjectType object_type) {
    ...
    if (item == object_map[object_type].end()) {
        ...
        (void)LogError("UNASSIGNED-ObjectTracker-Destroy",
                       device,
                       loc,
...
bool ObjectLifetimes::ValidateAnonymousObject(uint64_t object,
    ...
    return CheckObjectValidity(
        object, object_type, invalid_handle_vuid, wrong_parent_vuid, loc, kVulkanObjectTypeDevice);
}

void ObjectLifetimes::AllocateCommandBuffer(const VkCommandPool command_pool,
    ...
    InsertObject(
        object_map[kVulkanObjectTypeCommandBuffer], command_buffer, kVulkanObjectTypeCommandBuffer, loc, new_obj_node);
    num_objects[kVulkanObjectTypeCommandBuffer]++;
    num_total_objects++;
}

bool ObjectLifetimes::ValidateCommandBuffer(VkCommandPool command_pool,
    ...
    if (iter != object_map[kVulkanObjectTypeCommandBuffer].end()) {
        ...
        if (node->parent_object != HandleToUint64(command_pool)) {
            ...
            skip |= LogError("VUID-vkFreeCommandBuffers-pCommandBuffers-parent",
                             objlist,
                             loc,
                             "attempting to free %s belonging to %s from %s.",
                             FormatHandle(command_buffer).c_str(),
                             FormatHandle(parent_pool).c_str(),
                             FormatHandle(command_pool).c_str());
        }
    } else {
        skip |= LogError("VUID-vkFreeCommandBuffers-pCommandBuffers-00048",
                         command_buffer,
                         loc,
                         "Invalid %s.",
                         FormatHandle(command_buffer).c_str());
...
void ObjectLifetimes::AllocateDescriptorSet(VkDescriptorPool descriptor_pool,
    ...
    InsertObject(
        object_map[kVulkanObjectTypeDescriptorSet], descriptor_set, kVulkanObjectTypeDescriptorSet, loc, new_obj_node);
...
bool ObjectLifetimes::ValidateDescriptorSet(VkDescriptorPool descriptor_pool,
                                            VkDescriptorSet descriptor_set,
                                            const Location& loc) const {
    bool skip = false;
    ...
    if (ds_item != object_map[kVulkanObjectTypeDescriptorSet].end()) {
        if (ds_item->second->parent_object != HandleToUint64(descriptor_pool)) {
            ...
            skip |= LogError("VUID-vkFreeDescriptorSets-pDescriptorSets-parent",
                             objlist,
                             loc,
        ...
        skip |= LogError("VUID-vkFreeDescriptorSets-pDescriptorSets-00310",
                         descriptor_set,
                         loc,
                         "Invalid %s.",
                         FormatHandle(descriptor_set).c_str());
...
bool ObjectLifetimes::ValidateDescriptorWrite(VkWriteDescriptorSet const* desc,
                                              bool isPush,
                                              const Location& loc) const {
    bool skip = false;
    ...
    if (!isPush && desc->dstSet) {
        skip |= ValidateObject(desc->dstSet,
                               kVulkanObjectTypeDescriptorSet,
                               false,
    ...
    switch (desc->descriptorType) {
        ...
        case VK_DESCRIPTOR_TYPE_STORAGE_TEXEL_BUFFER: {
            for (uint32_t i = 0; i < desc->descriptorCount; ++i) {
                skip |= ValidateObject(desc->pTexelBufferView[i],
                                       kVulkanObjectTypeBufferView,
                                       true,
                                       "VUID-VkWriteDescriptorSet-descriptorType-02994",
                                       "VUID-vkUpdateDescriptorSets-pDescriptorWrites-06236",
                                       loc.dot(Field::pTexelBufferView, i));
                if (!null_descriptor_enabled && desc->pTexelBufferView[i] == VK_NULL_HANDLE) {
                    skip |= LogError("VUID-VkWriteDescriptorSet-descriptorType-02995",
                                     desc->dstSet,
                                     loc.dot(Field::pTexelBufferView, i),
                                     "is VK_NULL_HANDLE.");
                }
        ...
        case VK_DESCRIPTOR_TYPE_STORAGE_IMAGE: {
            if (desc->pImageInfo) {
                for (uint32_t i = 0; i < desc->descriptorCount; ++i) {
                    ...
                    skip |= ValidateObject(desc->pImageInfo[i].imageView,
                    ...
                    if (!null_descriptor_enabled && desc->pImageInfo[i].imageView == VK_NULL_HANDLE) {
                        skip |= LogError("VUID-VkWriteDescriptorSet-descriptorType-02997",
                                         desc->dstSet,
                                         loc.dot(Field::pImageInfo, i).dot(Field::imageView),
                                         "is VK_NULL_HANDLE.");
                    }
        ...
        case VK_DESCRIPTOR_TYPE_STORAGE_BUFFER_DYNAMIC: {
            if (desc->pBufferInfo) {
                for (uint32_t i = 0; i < desc->descriptorCount; ++i) {
                    skip |= ValidateObject(desc->pBufferInfo[i].buffer,
                    ...
                    if (!null_descriptor_enabled && desc->pBufferInfo[i].buffer == VK_NULL_HANDLE) {
                        skip |= LogError("VUID-VkDescriptorBufferInfo-buffer-02998",
                                         desc->dstSet,
                                         loc.dot(Field::pBufferInfo, i).dot(Field::buffer),
                                         "is VK_NULL_HANDLE.");
                    }
        ...
        case VK_DESCRIPTOR_TYPE_ACCELERATION_STRUCTURE_KHR: {
            if (const auto* acc_info =
                    vku::FindStructInPNextChain<VkWriteDescriptorSetAccelerationStructureKHR>(desc->pNext)) {
                for (uint32_t i = 0; i < desc->descriptorCount; ++i) {
                    skip |= ValidateObject(
                        acc_info->pAccelerationStructures[i],
                        ...
                        loc.pNext(
                            Struct::VkWriteDescriptorSetAccelerationStructureKHR, Field::pAccelerationStructures, i));
                }
            }
            if (const auto* acc_info_nv =
                    vku::FindStructInPNextChain<VkWriteDescriptorSetAccelerationStructureNV>(desc->pNext)) {
                for (uint32_t i = 0; i < desc->descriptorCount; ++i) {
                    skip |= ValidateObject(
                        acc_info_nv->pAccelerationStructures[i],
                        kVulkanObjectTypeAccelerationStructureNV,
                        true,
                        ...
                        loc.pNext(
                            Struct::VkWriteDescriptorSetAccelerationStructureNV, Field::pAccelerationStructures, i));
                }
            }
            break;
        }
...
bool ObjectLifetimes::PreCallValidateCmdPushDescriptorSet(VkCommandBuffer commandBuffer,
    ...
    skip |= ValidateObject(layout,
                           kVulkanObjectTypePipelineLayout,
                           false,
    ...
    if (pDescriptorWrites) {
        for (uint32_t index0 = 0; index0 < descriptorWriteCount; ++index0) {
            skip |= ValidateDescriptorWrite(
                &pDescriptorWrites[index0], true, error_obj.location.dot(Field::pDescriptorWrites, index0));
        }
    }
    return skip;
}

bool ObjectLifetimes::PreCallValidateCmdPushDescriptorSetKHR(VkCommandBuffer commandBuffer,
                                                             VkPipelineBindPoint pipelineBindPoint,
                                                             VkPipelineLayout layout,
                                                             uint32_t set,
                                                             uint32_t descriptorWriteCount,
...
bool ObjectLifetimes::PreCallValidateCmdPushDescriptorSet2(VkCommandBuffer commandBuffer,
    ...
    skip |= ValidateObject(pPushDescriptorSetInfo->layout,
                           kVulkanObjectTypePipelineLayout,
                           true,
    ...
    if (pPushDescriptorSetInfo->pDescriptorWrites) {
        for (uint32_t index0 = 0; index0 < pPushDescriptorSetInfo->descriptorWriteCount; ++index0) {
            skip |= ValidateDescriptorWrite(
                &pPushDescriptorSetInfo->pDescriptorWrites[index0],
                true,
...
void ObjectLifetimes::CreateSwapchainImageObject(VkImage swapchain_image,
...
bool ObjectLifetimes::ReportLeakedInstanceObjects(VkInstance instance,
                                                  VulkanObjectType object_type,
    ...
    for (const auto& item : snapshot) {
        ...
        skip |= LogError(error_code,
                         objlist,
                         loc,
                         "OBJ ERROR : For %s, %s has not been destroyed.",
                         FormatHandle(instance).c_str(),
                         FormatHandle(ObjTrackStateTypedHandle(*object_info)).c_str());
    }
    return skip;
}

bool ObjectLifetimes::ReportLeakedDeviceObjects(VkDevice device,
                                                VulkanObjectType object_type,
                                                const std::string& error_code,
                                                const Location& loc) const {
    bool skip = false;
...
bool ObjectLifetimes::PreCallValidateDestroyInstance(VkInstance instance,
    ...
    for (const auto& iit : snapshot) {
        ...
        skip |= LogError("VUID-vkDestroyInstance-instance-00629",
                         instance,
                         error_obj.location,
                         "%s object %s has not been destroyed.",
        ...
        auto obj_lifetimes_data =
            static_cast<ObjectLifetimes*>(device_data->GetValidationObject(LayerObjectTypeObjectTracker));
        skip |= obj_lifetimes_data->ReportUndestroyedDeviceObjects(device, error_obj.location);

        skip |= ValidateDestroyObject(device,
    ...
    skip |= ValidateDestroyObject(instance,
...
void ObjectLifetimes::PreCallRecordDestroyInstance(VkInstance instance,
                                                   const VkAllocationCallbacks* pAllocator,
...
void ObjectLifetimes::PostCallRecordDestroyInstance(VkInstance instance,
                                                    const VkAllocationCallbacks* pAllocator,
                                                    const RecordObject& record_obj) {
    RecordDestroyObject(instance, kVulkanObjectTypeInstance);
}

bool ObjectLifetimes::PreCallValidateDestroyDevice(VkDevice device,
                                                   const VkAllocationCallbacks* pAllocator,
                                                   const ErrorObject& error_obj) const {
    bool skip = false;
...
void ObjectLifetimes::PreCallRecordDestroyDevice(VkDevice device,
                                                 const VkAllocationCallbacks* pAllocator,
                                                 const RecordObject& record_obj) {
    auto object_lifetimes =
        static_cast<ObjectLifetimes*>(dispatch_instance_->GetValidationObject(LayerObjectTypeObjectTracker));
    // If ObjectTracker was removed (in an early teardown) this might be null, could search in aborted_object_dispatch
    // but if it is there, no need to record anything else
    if (object_lifetimes) {
...
void ObjectLifetimes::PostCallRecordGetDeviceQueue(
    VkDevice device, uint32_t queueFamilyIndex, uint32_t queueIndex, VkQueue* pQueue, const RecordObject& record_obj) {
    auto lock = WriteSharedLock();
    CreateQueue(*pQueue, record_obj.location);
}

void ObjectLifetimes::PostCallRecordGetDeviceQueue2(VkDevice device,
...
bool ObjectLifetimes::PreCallValidateUpdateDescriptorSets(VkDevice device,
                                                          uint32_t descriptorWriteCount,
                                                          const VkWriteDescriptorSet* pDescriptorWrites,
    ...
    if (pDescriptorCopies) {
        for (uint32_t idx0 = 0; idx0 < descriptorCopyCount; ++idx0) {
            ...
            if (pDescriptorCopies[idx0].dstSet) {
                skip |= ValidateObject(pDescriptorCopies[idx0].dstSet,
                                       kVulkanObjectTypeDescriptorSet,
                                       false,
            ...
            if (pDescriptorCopies[idx0].srcSet) {
                skip |= ValidateObject(pDescriptorCopies[idx0].srcSet,
                                       kVulkanObjectTypeDescriptorSet,
                                       false,
    ...
    if (pDescriptorWrites) {
        for (uint32_t idx1 = 0; idx1 < descriptorWriteCount; ++idx1) {
            skip |= ValidateDescriptorWrite(
                &pDescriptorWrites[idx1], false, error_obj.location.dot(Field::pDescriptorWrites, idx1));
        }
    }
    return skip;
}

bool ObjectLifetimes::PreCallValidateResetDescriptorPool(VkDevice device,
    ...
    skip |= ValidateObject(descriptorPool,
    ...
    if (itr != object_map[kVulkanObjectTypeDescriptorPool].end()) {
        ...
        for (auto set : *pool_node->child_objects) {
            skip |= ValidateDestroyObject((VkDescriptorSet)set,
                                          kVulkanObjectTypeDescriptorSet,
                                          nullptr,
                                          kVUIDUndefined,
                                          kVUIDUndefined,
...
void ObjectLifetimes::PreCallRecordResetDescriptorPool(VkDevice device,
                                                       VkDescriptorPool descriptorPool,
                                                       VkDescriptorPoolResetFlags flags,
                                                       const RecordObject& record_obj) {
    auto lock = WriteSharedLock();
    // A DescriptorPool's descriptor sets are implicitly deleted when the pool is reset. Remove this pool's descriptor
    // sets from our descriptorSet map.
    auto itr = object_map[kVulkanObjectTypeDescriptorPool].find(HandleToUint64(descriptorPool));
    if (itr != object_map[kVulkanObjectTypeDescriptorPool].end()) {
        auto pool_node = itr->second;
        for (auto set : *pool_node->child_objects) {
...
bool ObjectLifetimes::PreCallValidateBeginCommandBuffer(VkCommandBuffer commandBuffer,
    ...
    if (begin_info) {
        ...
        if (iter != object_map[kVulkanObjectTypeCommandBuffer].end()) {
            ...
            if ((begin_info->pInheritanceInfo) && error_obj.handle_data->command_buffer.is_secondary &&
                ...
                skip |= ValidateObject(begin_info->pInheritanceInfo->framebuffer,
                ...
                skip |= ValidateObject(begin_info->pInheritanceInfo->renderPass,
...
void ObjectLifetimes::PostCallRecordGetSwapchainImagesKHR(VkDevice device,
                                                          VkSwapchainKHR swapchain,
    ...
    if (pSwapchainImages != NULL) {
        for (uint32_t i = 0; i < *pSwapchainImageCount; i++) {
            CreateSwapchainImageObject(
                pSwapchainImages[i], swapchain, record_obj.location.dot(Field::pSwapchainImages, i));
        }
    }
}

bool ObjectLifetimes::ValidateDescriptorSetLayoutCreateInfo(const VkDescriptorSetLayoutCreateInfo& create_info,
        ...
        for (uint32_t binding_index = 0; binding_index < create_info.bindingCount; ++binding_index) {
            ...
            if (binding.pImmutableSamplers && is_sampler_type) {
                for (uint32_t index2 = 0; index2 < binding.descriptorCount; ++index2) {
                    ...
                    skip |= ValidateObject(sampler,
                                           kVulkanObjectTypeSampler,
                                           false,
...
bool ObjectLifetimes::PreCallValidateCreateDescriptorSetLayout(VkDevice device,
...
bool ObjectLifetimes::PreCallValidateGetPhysicalDeviceQueueFamilyProperties(
...
void ObjectLifetimes::PostCallRecordGetPhysicalDeviceQueueFamilyProperties(
    VkPhysicalDevice physicalDevice,
    uint32_t* pQueueFamilyPropertyCount,
    VkQueueFamilyProperties* pQueueFamilyProperties,
    const RecordObject& record_obj) {}

void ObjectLifetimes::PostCallRecordCreateInstance(const VkInstanceCreateInfo* pCreateInfo,
                                                   const VkAllocationCallbacks* pAllocator,
...
bool ObjectLifetimes::PreCallValidateAllocateCommandBuffers(VkDevice device,
    ...
    skip |= ValidateObject(pAllocateInfo->commandPool,
                           kVulkanObjectTypeCommandPool,
                           false,
...
void ObjectLifetimes::PostCallRecordAllocateCommandBuffers(VkDevice device,
                                                           const VkCommandBufferAllocateInfo* pAllocateInfo,
                                                           VkCommandBuffer* pCommandBuffers,
                                                           const RecordObject& record_obj) {
    if (record_obj.result < VK_SUCCESS)
    ...
    for (uint32_t i = 0; i < pAllocateInfo->commandBufferCount; i++) {
        AllocateCommandBuffer(pAllocateInfo->commandPool,
                              pCommandBuffers[i],
                              pAllocateInfo->level,
...
bool ObjectLifetimes::PreCallValidateAllocateDescriptorSets(VkDevice device,
    ...
    skip |= ValidateObject(pAllocateInfo->descriptorPool,
                           kVulkanObjectTypeDescriptorPool,
                           false,
                           "VUID-VkDescriptorSetAllocateInfo-descriptorPool-parameter",
                           "VUID-VkDescriptorSetAllocateInfo-commonparent",
                           allocate_info.dot(Field::descriptorPool));
    for (uint32_t i = 0; i < pAllocateInfo->descriptorSetCount; i++) {
        skip |= ValidateObject(pAllocateInfo->pSetLayouts[i],
                               kVulkanObjectTypeDescriptorSetLayout,
                               false,
                               "VUID-VkDescriptorSetAllocateInfo-pSetLayouts-parameter",
                               "VUID-VkDescriptorSetAllocateInfo-commonparent",
                               allocate_info.dot(Field::pSetLayouts, i));
    }
    return skip;
}

void ObjectLifetimes::PostCallRecordAllocateDescriptorSets(VkDevice device,
                                                           const VkDescriptorSetAllocateInfo* pAllocateInfo,
                                                           VkDescriptorSet* pDescriptorSets,
                                                           const RecordObject& record_obj) {
    if (record_obj.result < VK_SUCCESS)
    ...
    for (uint32_t i = 0; i < pAllocateInfo->descriptorSetCount; i++) {
        AllocateDescriptorSet(
            pAllocateInfo->descriptorPool, pDescriptorSets[i], record_obj.location.dot(Field::pDescriptorSets, i));
    }
}

bool ObjectLifetimes::PreCallValidateFreeCommandBuffers(VkDevice device,
    ...
    for (uint32_t i = 0; i < commandBufferCount; i++) {
        if (pCommandBuffers[i] != VK_NULL_HANDLE) {
            ...
            skip |= ValidateDestroyObject(pCommandBuffers[i],
...
void ObjectLifetimes::PreCallRecordFreeCommandBuffers(VkDevice device,
                                                      VkCommandPool commandPool,
                                                      uint32_t commandBufferCount,
                                                      const VkCommandBuffer* pCommandBuffers,
                                                      const RecordObject& record_obj) {
...
void ObjectLifetimes::PreCallRecordDestroySwapchainKHR(VkDevice device,
                                                       VkSwapchainKHR swapchain,
                                                       const VkAllocationCallbacks* pAllocator,
                                                       const RecordObject& record_obj) {
    RecordDestroyObject(swapchain, kVulkanObjectTypeSwapchainKHR);

    auto snapshot = swapchain_image_map.snapshot([swapchain](const std::shared_ptr<ObjTrackState>& pNode) {
        return pNode->parent_object == HandleToUint64(swapchain);
    });
...
bool ObjectLifetimes::PreCallValidateFreeDescriptorSets(VkDevice device,
                                                        VkDescriptorPool descriptorPool,
    ...
    for (uint32_t i = 0; i < descriptorSetCount; i++) {
        if (pDescriptorSets[i] != VK_NULL_HANDLE) {
            ...
            skip |= ValidateDestroyObject(pDescriptorSets[i],
                                          kVulkanObjectTypeDescriptorSet,
                                          nullptr,
                                          kVUIDUndefined,
                                          kVUIDUndefined,
...
void ObjectLifetimes::PreCallRecordFreeDescriptorSets(VkDevice device,
                                                      VkDescriptorPool descriptorPool,
                                                      uint32_t descriptorSetCount,
                                                      const VkDescriptorSet* pDescriptorSets,
                                                      const RecordObject& record_obj) {
...
bool ObjectLifetimes::PreCallValidateDestroyDescriptorPool(VkDevice device,
                                                           VkDescriptorPool descriptorPool,
                                                           const VkAllocationCallbacks* pAllocator,
                                                           const ErrorObject& error_obj) const {
    auto lock = ReadSharedLock();
    ...
    skip |= ValidateDestroyObject(descriptorPool,
                                  kVulkanObjectTypeDescriptorPool,
                                  pAllocator,
...
void ObjectLifetimes::PreCallRecordDestroyDescriptorPool(VkDevice device,
                                                         VkDescriptorPool descriptorPool,
                                                         const VkAllocationCallbacks* pAllocator,
                                                         const RecordObject& record_obj) {
    auto lock = WriteSharedLock();
...
bool ObjectLifetimes::PreCallValidateDestroyCommandPool(VkDevice device,
                                                        VkCommandPool commandPool,
                                                        const VkAllocationCallbacks* pAllocator,
                                                        const ErrorObject& error_obj) const {
    bool skip = false;
    ...
    auto snapshot =
        object_map[kVulkanObjectTypeCommandBuffer].snapshot([commandPool](const std::shared_ptr<ObjTrackState>& pNode) {
            return pNode->parent_object == HandleToUint64(commandPool);
        });
    for (const auto& itr : snapshot) {
        auto node = itr.second;
        skip |= ValidateCommandBuffer(commandPool, reinterpret_cast<VkCommandBuffer>(itr.first), command_pool_loc);
        skip |= ValidateDestroyObject(reinterpret_cast<VkCommandBuffer>(itr.first),
                                      kVulkanObjectTypeCommandBuffer,
                                      nullptr,
                                      kVUIDUndefined,
                                      kVUIDUndefined,
    ...
    skip |= ValidateDestroyObject(commandPool,
                                  kVulkanObjectTypeCommandPool,
                                  pAllocator,
...
void ObjectLifetimes::PreCallRecordDestroyCommandPool(VkDevice device,
                                                      VkCommandPool commandPool,
                                                      const VkAllocationCallbacks* pAllocator,
                                                      const RecordObject& record_obj) {
    auto snapshot =
    ...
    // A CommandPool's cmd buffers are implicitly deleted when pool is deleted. Remove this pool's cmdBuffers from cmd
    // buffer map.
    for (const auto& itr : snapshot) {
...
bool ObjectLifetimes::PreCallValidateGetPhysicalDeviceQueueFamilyProperties2KHR(
    ...
    VkQueueFamilyProperties2* pQueueFamilyProperties,
    const ErrorObject& error_obj) const {
    return PreCallValidateGetPhysicalDeviceQueueFamilyProperties2(
        physicalDevice, pQueueFamilyPropertyCount, pQueueFamilyProperties, error_obj);
}

void ObjectLifetimes::PostCallRecordGetPhysicalDeviceQueueFamilyProperties2(
    VkPhysicalDevice physicalDevice,
    uint32_t* pQueueFamilyPropertyCount,
    VkQueueFamilyProperties2* pQueueFamilyProperties,
    const RecordObject& record_obj) {}
...
void ObjectLifetimes::PostCallRecordGetPhysicalDeviceDisplayPropertiesKHR(VkPhysicalDevice physicalDevice,
    ...
    if (pProperties) {
        for (uint32_t i = 0; i < *pPropertyCount; ++i) {
            AllocateDisplayKHR(physicalDevice,
                               pProperties[i].display,
                               record_obj.location.dot(Field::pProperties, i).dot(Field::display));
        }
    }
}

void ObjectLifetimes::PostCallRecordGetDisplayModePropertiesKHR(VkPhysicalDevice physicalDevice,
    ...
    if (pProperties) {
        for (uint32_t i = 0; i < *pPropertyCount; ++i) {
            CreateObject(pProperties[i].displayMode,
                         kVulkanObjectTypeDisplayModeKHR,
                         nullptr,
...
void ObjectLifetimes::PostCallRecordGetPhysicalDeviceDisplayProperties2KHR(VkPhysicalDevice physicalDevice,
    ...
    if (pProperties) {
        for (uint32_t index = 0; index < *pPropertyCount; ++index) {
            AllocateDisplayKHR(
                physicalDevice,
                pProperties[index].displayProperties.display,
                record_obj.location.dot(Field::pProperties, index).dot(Field::displayProperties).dot(Field::display));
        }
    }
}

void ObjectLifetimes::PostCallRecordGetDisplayModeProperties2KHR(VkPhysicalDevice physicalDevice,
                                                                 VkDisplayKHR display,
                                                                 uint32_t* pPropertyCount,
    ...
    if (pProperties) {
        for (uint32_t index = 0; index < *pPropertyCount; ++index) {
            CreateObject(pProperties[index].displayModeProperties.displayMode,
                         kVulkanObjectTypeDisplayModeKHR,
                         nullptr,
                         record_obj.location.dot(Field::pProperties, index)
                             .dot(Field::displayModeProperties)
                             .dot(Field::displayMode));
        }
    }
}
...
void ObjectLifetimes::PostCallRecordGetPhysicalDeviceDisplayPlanePropertiesKHR(VkPhysicalDevice physicalDevice,
    ...
    if (pProperties) {
        for (uint32_t index = 0; index < *pPropertyCount; ++index) {
            AllocateDisplayKHR(physicalDevice,
                               pProperties[index].currentDisplay,
                               record_obj.location.dot(Field::pProperties, index).dot(Field::currentDisplay));
        }
    }
}

void ObjectLifetimes::PostCallRecordGetPhysicalDeviceDisplayPlaneProperties2KHR(
    VkPhysicalDevice physicalDevice,
    uint32_t* pPropertyCount,
    ...
    if (pProperties) {
        for (uint32_t index = 0; index < *pPropertyCount; ++index) {
            AllocateDisplayKHR(physicalDevice,
                               pProperties[index].displayPlaneProperties.currentDisplay,
                               record_obj.location.dot(Field::pProperties, index)
                                   .dot(Field::displayPlaneProperties)
                                   .dot(Field::currentDisplay));
        }
    }
}
...
bool ObjectLifetimes::PreCallValidateCreateFramebuffer(VkDevice device,
    ...
    skip |= ValidateObject(pCreateInfo->renderPass,
                           kVulkanObjectTypeRenderPass,
                           false,
    ...
    if ((pCreateInfo->flags & VK_FRAMEBUFFER_CREATE_IMAGELESS_BIT) == 0) {
        for (uint32_t index1 = 0; index1 < pCreateInfo->attachmentCount; ++index1) {
            skip |= ValidateObject(pCreateInfo->pAttachments[index1],
                                   kVulkanObjectTypeImageView,
                                   true,
...
bool ObjectLifetimes::PreCallValidateDebugMarkerSetObjectTagEXT(VkDevice device,
    ...
    if (pTagInfo->objectType == VK_DEBUG_REPORT_OBJECT_TYPE_UNKNOWN_EXT) {
        skip |= LogError("VUID-VkDebugMarkerObjectTagInfoEXT-objectType-01493",
                         device,
        ...
        if (pTagInfo->object == (uint64_t)VK_NULL_HANDLE) {
            skip |= LogError("VUID-VkDebugMarkerObjectTagInfoEXT-object-01494",
                             device,
                             error_obj.location.dot(Field::pTagInfo).dot(Field::object),
                             "is VK_NULL_HANDLE.");
            ...
            if (object_type != kVulkanObjectTypeImage || !swapchain_image_map.contains(pTagInfo->object)) {
                skip |= LogError("VUID-VkDebugMarkerObjectTagInfoEXT-object-01495",
                                 device,
                                 error_obj.location.dot(Field::pTagInfo).dot(Field::objectType),
                                 "(%s) doesn't match the object (0x%" PRIx64 ").",
                                 string_VkDebugReportObjectTypeEXT(pTagInfo->objectType),
                                 pTagInfo->object);
            }
        }
    }
...
bool ObjectLifetimes::PreCallValidateDebugMarkerSetObjectNameEXT(VkDevice device,
    ...
    if (pNameInfo->objectType == VK_DEBUG_REPORT_OBJECT_TYPE_UNKNOWN_EXT) {
        skip |= LogError("VUID-VkDebugMarkerObjectNameInfoEXT-objectType-01490",
                         device,
        ...
        if (pNameInfo->object == (uint64_t)VK_NULL_HANDLE) {
            skip |= LogError("VUID-VkDebugMarkerObjectNameInfoEXT-object-01491",
                             device,
                             error_obj.location.dot(Field::pNameInfo).dot(Field::object),
                             "is VK_NULL_HANDLE.");
            ...
            if (object_type != kVulkanObjectTypeImage || !swapchain_image_map.contains(pNameInfo->object)) {
                skip |= LogError("VUID-VkDebugMarkerObjectNameInfoEXT-object-01492",
                                 device,
                                 error_obj.location.dot(Field::pNameInfo).dot(Field::objectType),
                                 "(%s) doesn't match the object (0x%" PRIx64 ").",
                                 string_VkDebugReportObjectTypeEXT(pNameInfo->objectType),
                                 pNameInfo->object);
            }
        }
    }
...
bool ObjectLifetimes::PreCallValidateSetDebugUtilsObjectNameEXT(VkDevice device,
    ...
    if (IsInstanceVkObjectType(object_type)) {
        ...
        if (HandleToUint64(device) != object_handle) {
            skip |= LogError("VUID-vkSetDebugUtilsObjectNameEXT-pNameInfo-07874",
                             device,
                             error_obj.location.dot(Field::objectType),
                             "is VK_OBJECT_TYPE_DEVICE but objectHandle (0x%" PRIx64 ") != device (%s).",
                             object_handle,
                             FormatHandle(device).c_str());
        }
    } else {
        skip |= ValidateAnonymousObject(object_handle,
                                        object_type,
...
bool ObjectLifetimes::PreCallValidateSetDebugUtilsObjectTagEXT(VkDevice device,
    ...
    if (IsInstanceVkObjectType(object_type)) {
        ...
        if (HandleToUint64(device) != object_handle) {
            skip |= LogError("VUID-vkSetDebugUtilsObjectTagEXT-pNameInfo-07877",
                             device,
                             error_obj.location.dot(Field::pTagInfo).dot(Field::objectType),
                             "is VK_OBJECT_TYPE_DEVICE but objectHandle (0x%" PRIx64 ") != device (%s).",
                             object_handle,
...
bool ObjectLifetimes::PreCallValidateCreateDescriptorUpdateTemplate(
    ...
    if (pCreateInfo->templateType == VK_DESCRIPTOR_UPDATE_TEMPLATE_TYPE_DESCRIPTOR_SET) {
        skip |= ValidateObject(pCreateInfo->descriptorSetLayout,
                               kVulkanObjectTypeDescriptorSetLayout,
                               false,
    ...
    if (pCreateInfo->templateType == VK_DESCRIPTOR_UPDATE_TEMPLATE_TYPE_PUSH_DESCRIPTORS) {
        skip |= ValidateObject(pCreateInfo->pipelineLayout,
                               kVulkanObjectTypePipelineLayout,
                               false,
...
void ObjectLifetimes::PostCallRecordCreateDescriptorUpdateTemplate(
    ...
    if (record_obj.result < VK_SUCCESS)
        return;
    CreateObject(
        *pDescriptorUpdateTemplate, kVulkanObjectTypeDescriptorUpdateTemplate, pAllocator, record_obj.location);
}
...
void ObjectLifetimes::PostCallRecordCreateDescriptorUpdateTemplateKHR(
    ...
    VkDescriptorUpdateTemplate* pDescriptorUpdateTemplate,
    const RecordObject& record_obj) {
    return PostCallRecordCreateDescriptorUpdateTemplate(
        device, pCreateInfo, pAllocator, pDescriptorUpdateTemplate, record_obj);
}

bool ObjectLifetimes::ValidateAccelerationStructures(const char* src_handle_vuid,
    ...
    if (infos) {
        ...
        for (uint32_t i = 0; i < count; ++i) {
            ...
            skip |= ValidateObject(infos[i].srcAccelerationStructure,
                                   kVulkanObjectTypeAccelerationStructureKHR,
                                   true,
                                   src_handle_vuid,
                                   device_vuid,
                                   info_loc.dot(Field::srcAccelerationStructure));
            skip |= ValidateObject(infos[i].dstAccelerationStructure,
                                   kVulkanObjectTypeAccelerationStructureKHR,
                                   false,
                                   dst_handle_vuid,
                                   device_vuid,
...
bool ObjectLifetimes::PreCallValidateCmdBuildAccelerationStructuresIndirectKHR(
    ...
    skip |= ValidateAccelerationStructures(
...
bool ObjectLifetimes::PreCallValidateBuildAccelerationStructuresKHR(
    ...
    skip |= ValidateObject(deferredOperation,
...
bool ObjectLifetimes::PreCallValidateCreateRayTracingPipelinesKHR(VkDevice device,
                                                                  VkDeferredOperationKHR deferredOperation,
    ...
    skip |= ValidateObject(deferredOperation,
                           ...
                           error_obj.location.dot(Field::deferredOperation));
    skip |= ValidateObject(pipelineCache,
                           kVulkanObjectTypePipelineCache,
                           true,
        ...
        for (uint32_t index0 = 0; index0 < createInfoCount; ++index0) {
            ...
            if (pCreateInfos[index0].pStages) {
                for (uint32_t index1 = 0; index1 < pCreateInfos[index0].stageCount; ++index1) {
                    skip |= ValidateObject(pCreateInfos[index0].pStages[index1].module,
            ...
            if (pCreateInfos[index0].pLibraryInfo) {
                if (pCreateInfos[index0].pLibraryInfo->pLibraries) {
                    for (uint32_t index2 = 0; index2 < pCreateInfos[index0].pLibraryInfo->libraryCount; ++index2) {
                        skip |= ValidateObject(pCreateInfos[index0].pLibraryInfo->pLibraries[index2],
                                               kVulkanObjectTypePipeline,
                                               false,
            ...
            skip |= ValidateObject(pCreateInfos[index0].layout,
                                   kVulkanObjectTypePipelineLayout,
                                   false,
            ...
            if ((pCreateInfos[index0].flags & VK_PIPELINE_CREATE_DERIVATIVE_BIT) &&
                (pCreateInfos[index0].basePipelineIndex == -1))
                skip |= ValidateObject(pCreateInfos[index0].basePipelineHandle,
                                       kVulkanObjectTypePipeline,
                                       false,
...
bool ObjectLifetimes::PreCallValidateExportMetalObjectsEXT(VkDevice device,
    ...
    while (metal_objects_info_ptr) {
        switch (metal_objects_info_ptr->sType) {
            case VK_STRUCTURE_TYPE_EXPORT_METAL_COMMAND_QUEUE_INFO_EXT: {
                auto metal_command_queue_ptr =
                    reinterpret_cast<const VkExportMetalCommandQueueInfoEXT*>(metal_objects_info_ptr);
                skip |= ValidateObject(metal_command_queue_ptr->queue,
                                       kVulkanObjectTypeQueue,
                                       false,
            ...
            case VK_STRUCTURE_TYPE_EXPORT_METAL_BUFFER_INFO_EXT: {
                ...
                skip |= ValidateObject(metal_buffer_ptr->memory,
                                       kVulkanObjectTypeDeviceMemory,
                                       false,
            ...
            case VK_STRUCTURE_TYPE_EXPORT_METAL_TEXTURE_INFO_EXT: {
                ...
                skip |= ValidateObject(metal_texture_ptr->image,
                                       kVulkanObjectTypeImage,
                                       true,
                                       "VUID-VkExportMetalTextureInfoEXT-image-parameter",
                                       "VUID-VkExportMetalTextureInfoEXT-commonparent",
                                       error_obj.location);
                skip |= ValidateObject(metal_texture_ptr->imageView,
                                       kVulkanObjectTypeImageView,
                                       true,
                                       "VUID-VkExportMetalTextureInfoEXT-imageView-parameter",
                                       "VUID-VkExportMetalTextureInfoEXT-commonparent",
                                       error_obj.location);
                skip |= ValidateObject(metal_texture_ptr->bufferView,
                                       kVulkanObjectTypeBufferView,
                                       true,
            ...
            case VK_STRUCTURE_TYPE_EXPORT_METAL_SHARED_EVENT_INFO_EXT: {
                ...
                skip |= ValidateObject(metal_shared_event_ptr->semaphore,
                                       kVulkanObjectTypeSemaphore,
                                       true,
                                       "VUID-VkExportMetalSharedEventInfoEXT-semaphore-parameter",
                                       "VUID-VkExportMetalSharedEventInfoEXT-commonparent",
                                       error_obj.location);
                skip |= ValidateObject(metal_shared_event_ptr->event,
                                       kVulkanObjectTypeEvent,
                                       true,
...
bool ObjectLifetimes::PreCallValidateGetDescriptorEXT(VkDevice device,
    ...
    skip |= ValidateObject(
        device, kVulkanObjectTypeDevice, false, kVUIDUndefined, kVUIDUndefined, error_obj.location.dot(Field::device));

    return skip;
}

// Need to manually check if objectType and objectHandle are valid
bool ObjectLifetimes::PreCallValidateSetPrivateData(VkDevice device,
    ...
    if (IsInstanceVkObjectType(objectType) || objectType == VK_OBJECT_TYPE_UNKNOWN) {
        skip |= LogError("VUID-vkSetPrivateData-objectHandle-04016",
                         device,
                         error_obj.location.dot(Field::objectType),
                         "is %s.",
        ...
        skip |= ValidateAnonymousObject(objectHandle,
                                        objectType,
    ...
    skip |= ValidateObject(privateDataSlot,
...
bool ObjectLifetimes::PreCallValidateGetPrivateData(VkDevice device,
                                                    VkObjectType objectType,
                                                    uint64_t objectHandle,
                                                    VkPrivateDataSlot privateDataSlot,
                                                    uint64_t* pData,
    ...
    bool skip = false;
    if (IsInstanceVkObjectType(objectType) || objectType == VK_OBJECT_TYPE_UNKNOWN) {
        skip |= LogError("VUID-vkGetPrivateData-objectType-04018",
                         device,
                         error_obj.location.dot(Field::objectType),
                         "is %s.",
                         string_VkObjectType(objectType));
...
void ObjectLifetimes::PostCallRecordCreateGraphicsPipelines(VkDevice device,
                                                            VkPipelineCache pipelineCache,
                                                            uint32_t createInfoCount,
...
bool ObjectLifetimes::PreCallValidateCreateIndirectExecutionSetEXT(
    VkDevice device,
    ...
    if (pCreateInfo->type == VK_INDIRECT_EXECUTION_SET_INFO_TYPE_PIPELINES_EXT && pCreateInfo->info.pPipelineInfo) {
        ...
        skip |= ValidateObject(pCreateInfo->info.pPipelineInfo->initialPipeline,
                               kVulkanObjectTypePipeline,
                               false,
    ...
    if (pCreateInfo->type == VK_INDIRECT_EXECUTION_SET_INFO_TYPE_SHADER_OBJECTS_EXT && pCreateInfo->info.pShaderInfo) {
        ...
        if (shader_info.pSetLayoutInfos && shader_info.pInitialShaders) {
            for (uint32_t i = 0; i < shader_info.shaderCount; ++i) {
                skip |= ValidateObject(shader_info.pInitialShaders[i],
                                       kVulkanObjectTypeShaderEXT,
                                       false,
                ...
                if ((layout_info.setLayoutCount > 0) && (layout_info.pSetLayouts)) {
                    for (uint32_t layout_index = 0; layout_index < layout_info.setLayoutCount; ++layout_index) {
                        skip |=
                            ValidateObject(layout_info.pSetLayouts[layout_index],
                                           kVulkanObjectTypeDescriptorSetLayout,
                                           true,
...
bool ObjectLifetimes::PreCallValidateReleaseCapturedPipelineDataKHR(VkDevice device,
    ...
    if (pInfo) {
        ...
        skip |= ValidateObject(pInfo->pipeline,
                               kVulkanObjectTypePipeline,
                               false,
                               "VUID-VkReleaseCapturedPipelineDataInfoKHR-pipeline-parameter",
                               "UNASSIGNED-VkReleaseCapturedPipelineDataInfoKHR-pipeline-parent",
                               pInfo_loc.dot(Field::pipeline));

        skip |= ValidateDestroyObject(pInfo->pipeline,
                                      kVulkanObjectTypePipeline,
                                      pAllocator,
...
void ObjectLifetimes::PostCallRecordCreatePipelineBinariesKHR(VkDevice device,
    ...
    if (pBinaries->pPipelineBinaries) {
        for (uint32_t index = 0; index < pBinaries->pipelineBinaryCount; index++) {
            CreateObject(pBinaries->pPipelineBinaries[index],
                         kVulkanObjectTypePipelineBinaryKHR,
                         pAllocator,
                         record_obj.location);
        }
    }
