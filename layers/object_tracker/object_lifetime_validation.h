...
typedef vvl::concurrent_unordered_map<uint64_t, small_vector<std::shared_ptr<ObjTrackState>, 4>, 6>
    object_list_map_type;
...
class ObjectLifetimes : public ValidationObject
{
  ...
  public:
    ...
    void InsertObject(object_map_type&               map,
                      T1                             object,
                      ...
                      std::shared_ptr<ObjTrackState> pNode)
    {
        ...
        if (!inserted)
        {
            ...
            (void)LogError("UNASSIGNED-ObjectTracker-Insert",
                           object,
                           loc,
                           ...
                           string_VulkanObjectType(object_type),
                           object_handle);
        }
    }
    ...
    bool ReportLeakedDeviceObjects(VkDevice           device,
                                   VulkanObjectType   object_type,
                                   const std::string& error_code,
                                   const Location&    loc) const;
    bool ReportLeakedInstanceObjects(VkInstance         instance,
                                     VulkanObjectType   object_type,
                                     const std::string& error_code,
                                     const Location&    loc) const;
    ...
    void AllocateCommandBuffer(const VkCommandPool   command_pool,
    ...
    bool
    ValidateDescriptorSet(VkDescriptorPool descriptor_pool, VkDescriptorSet descriptor_set, const Location& loc) const;
    ...
    bool ValidateAnonymousObject(uint64_t        object,
                                 VkObjectType    core_object_type,
                                 const char*     invalid_handle_vuid,
                                 const char*     wrong_parent_vuid,
                                 const Location& loc) const;
    bool ValidateAccelerationStructures(const char*                                        src_handle_vuid,
                                        const char*                                        dst_handle_vuid,
                                        uint32_t                                           count,
                                        const VkAccelerationStructureBuildGeometryInfoKHR* infos,
                                        const Location&                                    loc) const;

    bool TracksObject(uint64_t object_handle, VulkanObjectType object_type) const;
    bool CheckObjectValidity(uint64_t         object_handle,
                             VulkanObjectType object_type,
                             const char*      invalid_handle_vuid,
                             const char*      wrong_parent_vuid,
                             const Location&  loc,
    ...
    bool
    CheckPipelineObjectValidity(uint64_t object_handle, const char* invalid_handle_vuid, const Location& loc) const;

    template <typename T1>
    bool ValidateObject(T1               object,
                        VulkanObjectType object_type,
                        bool             null_allowed,
                        const char*      invalid_handle_vuid,
                        const char*      wrong_parent_vuid,
                        ...
                        VulkanObjectType parent_type = kVulkanObjectTypeDevice) const
    {
        ...
        return CheckObjectValidity(
            HandleToUint64(object), object_type, invalid_handle_vuid, wrong_parent_vuid, loc, parent_type);
    }

    template <typename T1>
    void
    CreateObject(T1 object, VulkanObjectType object_type, const VkAllocationCallbacks* pAllocator, const Location& loc)
    {
        uint64_t   object_handle    = HandleToUint64(object);
    ...
    template <typename T1>
    bool ValidateDestroyObject(T1                           object_handle,
                               VulkanObjectType             object_type,
                               const VkAllocationCallbacks* pAllocator,
                               const char*                  expected_custom_allocator_code,
                               ...
                               const Location&              loc) const
    {
            ...
            object != HandleToUint64(VK_NULL_HANDLE))
        {
            ...
            if (item != object_map[object_type].end())
            {
                ...
                {
                    // This check only verifies that custom allocation callbacks were provided to both Create and
                    // Destroy calls, it cannot verify that these allocation callbacks are compatible with each other.
                    skip |= LogError(expected_custom_allocator_code,
                                     object_handle,
                                     loc,
                         ...
                         expected_default_allocator_code != kVUIDUndefined)
                {
                    skip |= LogError(expected_default_allocator_code,
                                     object_handle,
                                     loc,
