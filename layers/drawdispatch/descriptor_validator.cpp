...
DescriptorValidator::DescriptorValidator(ValidationStateTracker& dev,
                                         ...
                                         const Location&         loc) :
    dev_state(dev),
    cb_state(cb_state), descriptor_set(descriptor_set), set_index(set_index), framebuffer(framebuffer), loc(loc),
                                                    ...
                                                    const T&                                binding) const
{
    ...
    for (uint32_t index = 0; !skip && index < binding.count; index++)
    {
        ...
        if (!binding.updated[index])
        {
            ...
            return dev_state.LogError(vuids.descriptor_buffer_bit_set_08114,
                                      set,
                                      loc,
                                      "the descriptor %s is being used in %s but has never been updated via "
                                                ...
                                                const DescriptorBinding&                binding) const
{
    ...
    switch (binding.descriptor_class)
    {
        ...
        case DescriptorClass::AccelerationStructure:
            skip |=
                ValidateDescriptorsStatic(resource_variable, static_cast<const AccelerationStructureBinding&>(binding));
            break;
...
template <typename T>
bool DescriptorValidator::ValidateDescriptorsDynamic(const spirv::ResourceInterfaceVariable& resource_variable,
                                                     const T&                                binding,
                                                 ...
                                                 const uint32_t                          index)
{
    ...
    switch (binding.descriptor_class)
    {
        ...
        case DescriptorClass::TexelBuffer:
            ...
            break;
        case DescriptorClass::AccelerationStructure:
            skip |= ValidateDescriptorsDynamic(
                resource_variable, static_cast<const AccelerationStructureBinding&>(binding), index);
            break;
        case DescriptorClass::InlineUniform:
...
bool DescriptorValidator::ValidateDescriptor(const spirv::ResourceInterfaceVariable& resource_variable,
                                             const uint32_t                          index,
                                             ...
                                             const BufferDescriptor&                 descriptor) const
{
    ...
    {
        if (dev_state.ValidateProtectedBuffer(
                cb_state, *buffer_node, loc, vuids.unprotected_command_buffer_02707, " (Buffer is in a descriptorSet)"))
        {
            return true;
        }
        if (resource_variable.IsWrittenTo() &&
            dev_state.ValidateUnprotectedBuffer(
                cb_state, *buffer_node, loc, vuids.protected_command_buffer_02712, " (Buffer is in a descriptorSet)"))
        {
            return true;
        }
    }
...
bool DescriptorValidator::ValidateDescriptor(const spirv::ResourceInterfaceVariable& resource_variable,
                                             const uint32_t                          index,
                                             VkDescriptorType                        descriptor_type,
                                             const ImageDescriptor&                  image_descriptor) const
{
    ...
    {
        // TODO - This will skip for GPU-AV because we don't currently capture array of samplers with array of sampled
        // images https://github.com/KhronosGroup/Vulkan-ValidationLayers/issues/8922 To not give false positve, we will
        // skip all Sampler related checks since sampler_states will be empty
    }
    else
    {
        if (index < resource_variable.samplers_used_by_image.size())
        {
            ...
            {
                const auto* descriptor = descriptor_set.GetDescriptorFromBinding(sampler_used.sampler_slot.binding,
                                                                                 sampler_used.sampler_index);
    ...
    if ((!image_view_state && !dev_state.enabled_features.nullDescriptor) ||
    ...
    if (resource_variable.IsImage() && dim != spv::DimSubpassData)
    {
        ...
        // Because you can have a runtime array with different types in it, without extensive GPU-AV tracking, we have
        // no way to detect if the types match up in a given index
        ...
        {
            const bool signed_override   = ((resource_variable.info.image_format_type & spirv::NumericTypeUint) &&
                                          resource_variable.info.is_sign_extended);
            const bool unsigned_override = ((resource_variable.info.image_format_type & spirv::NumericTypeSint) &&
                                            resource_variable.info.is_zero_extended);
            if (!signed_override && !unsigned_override)
            {
                auto                set = descriptor_set.Handle();
                const LogObjectList objlist(set, image_view);
                return dev_state.LogError(
                    vuids.image_view_numeric_format_07753,
                    objlist,
                    loc,
        ...
        if (image_format_width_64)
        {
            if (resource_variable.image_sampled_type_width != 64)
            {
                ...
                const LogObjectList objlist(set, image_view);
                return dev_state.LogError(
                    vuids.image_view_access_64_04470,
                    objlist,
                    loc,
            ...
            else if (!dev_state.enabled_features.sparseImageInt64Atomics && image_state->sparse_residency)
            {
                ...
                return dev_state.LogError(vuids.image_view_sparse_64_04474,
                                          objlist,
                                          loc,
                                          "the descriptor %s has a OpTypeImage's Sampled Type has a width of 64 backed "
                                          "by a sparse Image, but "
        ...
        else if (!image_format_width_64 && resource_variable.image_sampled_type_width != 32)
        {
            ...
            const LogObjectList objlist(set, image_view);
            return dev_state.LogError(
                vuids.image_view_access_32_04471,
                objlist,
                loc,
    ...
    if (!dev_state.disabled[image_layout_validation])
    {
        ...
        dev_state.VerifyImageLayout(
            cb_state, *image_view_state, image_layout, loc, "VUID-VkDescriptorImageInfo-imageLayout-00344", &hit_error);
        if (hit_error)
        {
            auto              set = descriptor_set.Handle();
            ...
            return dev_state.LogError(vuids.descriptor_buffer_bit_set_08114,
                                      set,
                                      loc,
                                      "%s. See previous error callback for specific details.",
                                      msg.str().c_str());
        }
    }
    ...
    if (resource_variable.IsImage())
    {
        if (!resource_variable.info.is_multisampled && image_view_state->samples != VK_SAMPLE_COUNT_1_BIT)
        {
            auto                set = descriptor_set.Handle();
            const LogObjectList objlist(set, image_view);
            return dev_state.LogError("VUID-RuntimeSpirv-samples-08725",
                                      objlist,
                                      loc,
                                      "the descriptor %s has %s created with %s.",
                                      DescribeDescriptor(resource_variable, index).c_str(),
        ...
        if (resource_variable.info.is_multisampled && image_view_state->samples == VK_SAMPLE_COUNT_1_BIT)
        {
            auto                set = descriptor_set.Handle();
            const LogObjectList objlist(set, image_view);
            return dev_state.LogError("VUID-RuntimeSpirv-samples-08726",
                                      objlist,
                                      loc,
                                      "the descriptor %s has %s created with VK_SAMPLE_COUNT_1_BIT.",
                                      DescribeDescriptor(resource_variable, index).c_str(),
                                      dev_state.FormatHandle(image_state->Handle()).c_str());
        }
    }
    ...
    if (image_view_state->samplerConversion)
    {
        if (resource_variable.info.is_not_sampler_sampled)
        {
            ...
            const LogObjectList objlist(set, image_view);
            return dev_state.LogError(
                vuids.image_ycbcr_sampled_06550,
                objlist,
                loc,
        ...
        if (resource_variable.info.is_sampler_offset)
        {
            ...
            const LogObjectList objlist(set, image_view);
            return dev_state.LogError(
                vuids.image_ycbcr_offset_06551,
                objlist,
                loc,
                "the image descriptor %s was created with a sampler Ycbcr conversion, but was accessed with "
    ...
    if (dev_state.has_format_feature2)
    {
        ...
        if (descriptor_type == VK_DESCRIPTOR_TYPE_STORAGE_IMAGE)
        {
                ...
                !(format_features & VK_FORMAT_FEATURE_2_STORAGE_READ_WITHOUT_FORMAT_BIT))
            {
                auto                set = descriptor_set.Handle();
                const LogObjectList objlist(set, image_view);
                return dev_state.LogError(vuids.storage_image_read_without_format_07028,
                                          objlist,
                                          loc,
                ...
                !(format_features & VK_FORMAT_FEATURE_2_STORAGE_WRITE_WITHOUT_FORMAT_BIT))
            {
                auto                set = descriptor_set.Handle();
                const LogObjectList objlist(set, image_view);
                return dev_state.LogError(vuids.storage_image_write_without_format_07027,
                                          objlist,
                                          loc,
                                          "the descriptor %s has %s with format of %s which doesn't support "
        ...
        if ((resource_variable.info.is_dref) &&
            !(format_features & VK_FORMAT_FEATURE_2_SAMPLED_IMAGE_DEPTH_COMPARISON_BIT))
        {
            auto                set = descriptor_set.Handle();
            const LogObjectList objlist(set, image_view);
            return dev_state.LogError(vuids.depth_compare_sample_06479,
                                      objlist,
                                      loc,
                                      "the descriptor %s has %s with format of %s which doesn't support "
        ...
        for (uint32_t att_index = 0; att_index < cb_state.active_attachments.size(); ++att_index)
        {
            ...
            if (read_attachment && descriptor_written_to)
            {
                if (same_view)
                {
                    ...
                    return dev_state.LogError(
                        vuids.image_subresources_subpass_write_06539,
                        objlist,
                        loc,
                        "the descriptor %s has %s which will be read from as %s attachment %" PRIu32 ".",
                        DescribeDescriptor(resource_variable, index).c_str(),
                        dev_state.FormatHandle(image_view).c_str(),
                        dev_state.FormatHandle(framebuffer).c_str(),
                        att_index);
                }
                else if (overlapping_view)
                {
                    ...
                    return dev_state.LogError(
                        ...
                        objlist,
                        loc,
                        "the descriptor %s has %s which will be overlap read from as %s in %s attachment %" PRIu32
                        " overlap.",
                        DescribeDescriptor(resource_variable, index).c_str(),
                        dev_state.FormatHandle(image_view).c_str(),
                        dev_state.FormatHandle(view_state->Handle()).c_str(),
                        dev_state.FormatHandle(framebuffer).c_str(),
                        att_index);
                }
            }
            ...
            if (descriptor_written_to && !layout_read_only)
            {
                if (same_view)
                {
                    ...
                    const LogObjectList objlist(set, image_view, framebuffer);
                    return dev_state.LogError(
                        vuids.image_subresources_render_pass_write_06537,
                        objlist,
                        loc,
    ...
    if (dev_state.enabled_features.protectedMemory == VK_TRUE)
    {
        if (dev_state.ValidateProtectedImage(
                cb_state, *image_state, loc, vuids.unprotected_command_buffer_02707, " (Image is in a descriptorSet)"))
        {
            return true;
        }
        if (resource_variable.IsWrittenTo() &&
            dev_state.ValidateUnprotectedImage(
                cb_state, *image_state, loc, vuids.protected_command_buffer_02712, " (Image is in a descriptorSet)"))
        {
            return true;
        }
    }
    ...
    for (const auto* sampler_state : sampler_states)
    {
        ...
        {
            if (image_view_format == VK_FORMAT_B4G4R4A4_UNORM_PACK16 ||
                image_view_format == VK_FORMAT_B5G6R5_UNORM_PACK16 ||
                image_view_format == VK_FORMAT_B5G5R5A1_UNORM_PACK16 ||
                image_view_format == VK_FORMAT_A1B5G5R5_UNORM_PACK16)
            {
                auto                set = descriptor_set.Handle();
                const LogObjectList objlist(set, sampler_state->Handle(), image_view_state->Handle());
                return dev_state.LogError("VUID-VkSamplerCustomBorderColorCreateInfoEXT-format-04015",
                                          objlist,
                                          loc,
        ...
        // The VU is wording is a bit misleading, if there is no VkSamplerReductionModeCreateInfo we still need to check
        // for linear tiling feature
            ...
            !(image_view_state->format_features & VK_FORMAT_FEATURE_SAMPLED_IMAGE_FILTER_LINEAR_BIT))
        {
            if (sampler_mag_filter == VK_FILTER_LINEAR || sampler_min_filter == VK_FILTER_LINEAR)
            {
                ...
                const LogObjectList objlist(set, sampler_state->Handle(), image_view_state->Handle());
                return dev_state.LogError(
                    vuids.linear_filter_sampler_04553,
                    objlist,
                    loc,
            ...
            if (sampler_state->create_info.mipmapMode == VK_SAMPLER_MIPMAP_MODE_LINEAR)
            {
                ...
                const LogObjectList objlist(set, sampler_state->Handle(), image_view_state->Handle());
                return dev_state.LogError(
                    vuids.linear_mipmap_sampler_04770,
                    objlist,
                    loc,
        ...
        const bool is_minmax =
            sampler_reduction && (sampler_reduction->reductionMode == VK_SAMPLER_REDUCTION_MODE_MIN ||
        ...
        if (is_minmax && !(image_view_state->format_features & VK_FORMAT_FEATURE_SAMPLED_IMAGE_FILTER_MINMAX_BIT))
        {
            if (sampler_mag_filter == VK_FILTER_LINEAR || sampler_min_filter == VK_FILTER_LINEAR)
            {
                ...
                const LogObjectList objlist(set, sampler_state->Handle(), image_view_state->Handle());
                return dev_state.LogError(
                    vuids.linear_filter_sampler_09598,
                    objlist,
                    loc,
            ...
            if (sampler_state->create_info.mipmapMode == VK_SAMPLER_MIPMAP_MODE_LINEAR)
            {
                ...
                const LogObjectList objlist(set, sampler_state->Handle(), image_view_state->Handle());
                return dev_state.LogError(
                    vuids.linear_mipmap_sampler_09599,
                    objlist,
                    loc,
                    "the descriptor %s has %s which is set to use VK_SAMPLER_MIPMAP_MODE_LINEAR with "
        ...
        if (sampler_mag_filter == VK_FILTER_CUBIC_EXT || sampler_min_filter == VK_FILTER_CUBIC_EXT)
        {
            if (!(image_view_state->format_features & VK_FORMAT_FEATURE_SAMPLED_IMAGE_FILTER_CUBIC_BIT_EXT))
            {
                ...
                const LogObjectList objlist(set, sampler_state->Handle(), image_view_state->Handle());
                return dev_state.LogError(
                    vuids.cubic_sampler_02692,
                    objlist,
                    loc,
                    "the descriptor %s has %s which is set to use VK_FILTER_CUBIC_EXT, then image view's (%s) format "
                    "(%s) "
            ...
            if (IsExtEnabled(dev_state.device_extensions.vk_ext_filter_cubic))
            {
                    ...
                    !image_view_state->filter_cubic_props.filterCubicMinmax)
                {
                    ...
                    const LogObjectList objlist(set, sampler_state->Handle(), image_view_state->Handle());
                    return dev_state.LogError(
                        vuids.filter_cubic_min_max_02695,
                        objlist,
                        loc,
            ...
            if (IsExtEnabled(dev_state.device_extensions.vk_img_filter_cubic))
            {
                    ...
                    image_view_state->create_info.viewType == VK_IMAGE_VIEW_TYPE_CUBE_ARRAY)
                {
                    ...
                    const LogObjectList objlist(set, sampler_state->Handle(), image_view_state->Handle());
                    return dev_state.LogError(
                        vuids.img_filter_cubic_02693,
                        objlist,
                        loc,
                        "the descriptor %s has %s which is set to use VK_FILTER_CUBIC_EXT while the "
                        "VK_IMG_filter_cubic "
             ...
             sampler_state->create_info.addressModeW != VK_SAMPLER_ADDRESS_MODE_CLAMP_TO_EDGE))
        {
            ...
            const LogObjectList objlist(
                set, sampler_state->Handle(), image_state->Handle(), image_view_state->Handle());
            return dev_state.LogError(vuids.corner_sampled_address_mode_02696,
                                      objlist,
                                      loc,
        ...
        if (sampler_state->create_info.unnormalizedCoordinates && resource_variable.info.is_sampler_sampled)
        {
                ...
                image_view_ci.viewType == VK_IMAGE_VIEW_TYPE_CUBE_ARRAY)
            {
                ...
                return dev_state.LogError(vuids.sampler_imageview_type_08609,
                                          objlist,
                                          loc,
                                          "the descriptor %s Image View %s, type %s, is used by %s.",
                                          DescribeDescriptor(resource_variable, index).c_str(),
                                          dev_state.FormatHandle(image_view).c_str(),
                                          string_VkImageViewType(image_view_ci.viewType),
                                          dev_state.FormatHandle(sampler_state->Handle()).c_str());
            ...
            if (subresource_range.levelCount != 1)
            {
                ...
                const LogObjectList objlist(set, image_view, sampler_state->Handle());
                return dev_state.LogError(
                    vuids.unnormalized_coordinates_09635,
                    objlist,
                    loc,
                    "the descriptor %s Image View %s was created with levelCount of %s, but the sampler (%s) was "
                    "created with "
            ...
            if (subresource_range.layerCount != 1)
            {
                ...
                return dev_state.LogError(
                    ...
                    objlist,
                    loc,
                    "the descriptor %s Image View %s was created with layerCount of %s, but the sampler (%s) was "
                    "created with "
                    "unnormalizedCoordinates.",
                    DescribeDescriptor(resource_variable, index).c_str(),
                    dev_state.FormatHandle(image_view).c_str(),
            ...
            if (resource_variable.info.is_sampler_implicitLod_dref_proj)
            {
                auto                set = descriptor_set.Handle();
                const LogObjectList objlist(set, image_view, sampler_state->Handle());
                return dev_state.LogError(vuids.sampler_implicitLod_dref_proj_08610,
                                          objlist,
                                          loc,
            ...
            if (resource_variable.info.is_sampler_bias_offset)
            {
                ...
                const LogObjectList objlist(set, image_view, sampler_state->Handle());
                return dev_state.LogError(
                    vuids.sampler_bias_offset_08611,
                    objlist,
                    loc,
    ...
    for (const uint32_t texel_component_count : resource_variable.write_without_formats_component_count_list)
    {
        ...
        if (image_view_format == VK_FORMAT_A8_UNORM)
        {
            if (texel_component_count != 4)
            {
                ...
                const LogObjectList objlist(set, image_view);
                return dev_state.LogError(
                    vuids.storage_image_write_texel_count_08796,
                    objlist,
                    loc,
        ...
        else if (texel_component_count < format_component_count)
        {
            ...
            const LogObjectList objlist(set, image_view);
            return dev_state.LogError(
                vuids.storage_image_write_texel_count_08795,
                objlist,
                loc,
...
bool DescriptorValidator::ValidateDescriptor(const spirv::ResourceInterfaceVariable& resource_variable,
                                             const uint32_t                          index,
                                             VkDescriptorType                        descriptor_type,
                                             const ImageSamplerDescriptor&           descriptor) const
{
    bool skip = false;
    skip |=
        ValidateDescriptor(resource_variable, index, descriptor_type, static_cast<const ImageDescriptor&>(descriptor));
    if (skip)
    {
        return skip;
    }
    skip |= ValidateSamplerDescriptor(resource_variable,
                                      index,
                                      descriptor.GetSampler(),
                                      descriptor.IsImmutableSampler(),
...
bool DescriptorValidator::ValidateDescriptor(const spirv::ResourceInterfaceVariable& resource_variable,
                                             const uint32_t                          index,
                                             VkDescriptorType                        descriptor_type,
                                             const TexelDescriptor&                  texel_descriptor) const
{
    ...
    if ((resource_variable.info.image_format_type & format_bits) == 0)
    {
        ...
        if (!signed_override && !unsigned_override)
        {
            auto set = descriptor_set.Handle();
            return dev_state.LogError(
                vuids.descriptor_buffer_bit_set_08114,
                set,
                loc,
                "the descriptor %s requires %s component type, but bound descriptor format is %s.",
                DescribeDescriptor(resource_variable, index).c_str(),
    ...
    if (buffer_format_width_64 && resource_variable.image_sampled_type_width != 64)
    {
        ...
        return dev_state.LogError(
            vuids.buffer_view_access_64_04472,
            objlist,
            loc,
    ...
    else if (!buffer_format_width_64 && resource_variable.image_sampled_type_width != 32)
    {
        ...
        const LogObjectList objlist(set, buffer_view);
        return dev_state.LogError(
            vuids.buffer_view_access_32_04473,
            objlist,
            loc,
    ...
    if (dev_state.has_format_feature2)
    {
        if (descriptor_type == VK_DESCRIPTOR_TYPE_STORAGE_TEXEL_BUFFER)
        {
                ...
                !(buffer_format_features & VK_FORMAT_FEATURE_2_STORAGE_READ_WITHOUT_FORMAT_BIT_KHR))
            {
                auto                set = descriptor_set.Handle();
                const LogObjectList objlist(set, buffer_view);
                return dev_state.LogError(vuids.storage_texel_buffer_read_without_format_07030,
                                          objlist,
                                          loc,
                ...
                !(buffer_format_features & VK_FORMAT_FEATURE_2_STORAGE_WRITE_WITHOUT_FORMAT_BIT))
            {
                auto                set = descriptor_set.Handle();
                const LogObjectList objlist(set, buffer_view);
                return dev_state.LogError(vuids.storage_texel_buffer_write_without_format_07029,
                                          objlist,
                                          loc,
                                          "the descriptor %s has %s with format of %s which is missing "
                                          "VK_FORMAT_FEATURE_2_STORAGE_WRITE_WITHOUT_FORMAT_BIT.\n"
    ...
    {
        if (dev_state.ValidateProtectedBuffer(cb_state,
                                              *buffer_view_state->buffer_state,
                                              loc,
                                              vuids.unprotected_command_buffer_02707,
                                              " (Buffer is in a descriptorSet)"))
        {
            return true;
        }
        if (resource_variable.IsWrittenTo() && dev_state.ValidateUnprotectedBuffer(cb_state,
                                                                                   *buffer_view_state->buffer_state,
                                                                                   loc,
                                                                                   vuids.protected_command_buffer_02712,
                                                                                   " (Buffer is in a descriptorSet)"))
        {
            return true;
        }
    ...
    for (const uint32_t texel_component_count : resource_variable.write_without_formats_component_count_list)
    {
        ...
        if (texel_component_count < format_component_count)
        {
            ...
            const LogObjectList objlist(set, buffer_view);
            return dev_state.LogError(
                vuids.storage_texel_buffer_write_texel_count_04469,
                objlist,
                loc,
                "the descriptor %s VkImageView is mapped to a OpImage format of %s which has %" PRIu32
                " components, but the OpImageWrite Texel operand only contains %" PRIu32 " components.",
...
bool DescriptorValidator::ValidateSamplerDescriptor(const spirv::ResourceInterfaceVariable& resource_variable,
                                                    uint32_t                                index,
                                                    VkSampler                               sampler,
                                                    bool                                    is_immutable,
                                                    const Sampler*                          sampler_state) const
{
...
{
    return ValidateSamplerDescriptor(resource_variable,
                                     index,
                                     descriptor.GetSampler(),
                                     descriptor.IsImmutableSampler(),
                                     descriptor.GetSamplerState());
