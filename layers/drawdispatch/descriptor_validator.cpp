...
bool DescriptorValidator::ValidateDescriptor(const spirv::ResourceInterfaceVariable &resource_variable, uint32_t index,
    ...
    // TODO / FIXME: Define what is correct here, understanding and maybe updating the specs
    // The following line implements Table 112 from the Vulkan specs *strictly*. But it will trigger VU 09906 with TOSA operators,
    // which require both `OpTypeInt N **0**` in the spirv, and a signed tensor (VkFormat).
    if (loc.Get().function != Func::vkCmdDispatchDataGraphARM &&
        tensor_view_state->create_info.format != resource_variable.info.vk_format) {
    // `IsTensorCompatible` implements a "signless" version of Table 112, *NOT* following the specs, but complies with TOSA operators
    //     !spirv::IsTensorCompatible(resource_variable.info.numeric_type, resource_variable.info.bit_width, tensor_view_state->create_info.format)) {
        const LogObjectList objlist(cb_state.Handle(), this->objlist, descriptor_set.Handle(), tensor_view_state->Handle());
        skip |= LogError(vuids->spirv_OpTypeTensorARM_09906, objlist, loc.Get(),
                         "the %s is using tensor %s that was created with %s but doesn't match the OpTypeTensorARM of type %s and width %" PRIu32 ".%s",
                         DescribeDescriptor(resource_variable, index, descriptor_type).c_str(),
                         FormatHandle(tensor_view_state->Handle()).c_str(),
                         string_VkFormat(tensor_view_state->create_info.format),
                         spirv::string_NumericType(resource_variable.info.numeric_type),
                         resource_variable.info.bit_width, DescribeInstruction().c_str());
    }
