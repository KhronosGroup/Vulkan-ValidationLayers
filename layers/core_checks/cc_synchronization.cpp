...
struct TimelineMaxDiffCheck {
    ...
    bool operator()(const vvl::Semaphore::OpType, uint64_t payload, bool is_pending) {
        return AbsDiff(value, payload) > max_diff;
    }
...
bool SemaphoreSubmitState::CanSignalBinary(const vvl::Semaphore& semaphore_state,
                                           VkQueue& other_queue,
...
bool SemaphoreSubmitState::ValidateBinaryWait(const Location& loc,
                                              VkQueue queue,
                                              const vvl::Semaphore& semaphore_state) {
    ...
    if ((semaphore_state.Scope() == vvl::Semaphore::kInternal || internal_semaphores.count(semaphore))) {
        if (VkQueue other_queue = AnotherQueueWaits(semaphore_state)) {
            ...
            skip |= core.LogError(vuid,
                                  objlist,
                                  loc,
                                  "queue (%s) is already waiting on semaphore (%s).",
                                  core.FormatHandle(other_queue).c_str(),
                                  core.FormatHandle(semaphore).c_str());
            ...
            skip |=
                core.LogError(vuid,
                              objlist,
                              loc,
                              "queue (%s) is waiting on binary semaphore (%s) that has an associated signal but it "
                              "depends on timeline semaphore "
                              "wait (%s, wait value = %" PRIu64 ") that does not have resolving signal submitted yet.",
                              core.FormatHandle(queue).c_str(),
                              core.FormatHandle(semaphore).c_str(),
                              core.FormatHandle(timeline_wait_info->semaphore->VkHandle()).c_str(),
...
bool SemaphoreSubmitState::ValidateWaitSemaphore(const Location& wait_semaphore_loc,
                                                 const vvl::Semaphore& semaphore_state,
                                                 uint64_t value) {
    using sync_vuid_maps::GetQueueSubmitVUID;
    using sync_vuid_maps::SubmitError;
    ...
    switch (semaphore_state.type) {
        ...
        case VK_SEMAPHORE_TYPE_TIMELINE: {
            ...
            std::string where;
            TimelineMaxDiffCheck exceeds_max_diff(value,
                                                  core.phys_dev_props_core12.maxTimelineSemaphoreValueDifference);
            ...
            if (CheckSemaphoreValue(semaphore_state, where, bad_value, exceeds_max_diff)) {
                ...
                skip |=
                    core.LogError(vuid,
                                  ...
                                  value,
                                  where.c_str(),
                                  core.FormatHandle(handle).c_str(),
                                  bad_value);
                break;
            }
...
bool SemaphoreSubmitState::ValidateSignalSemaphore(const Location& signal_semaphore_loc,
                                                   const vvl::Semaphore& semaphore_state,
                                                   uint64_t value) {
    using sync_vuid_maps::GetQueueSubmitVUID;
    using sync_vuid_maps::SubmitError;
        ...
        case VK_SEMAPHORE_TYPE_BINARY: {
            if ((semaphore_state.Scope() == vvl::Semaphore::kInternal || internal_semaphores.count(handle))) {
                ...
                if (!CanSignalBinary(semaphore_state, other_queue, other_command)) {
                    ...
                    skip |= core.LogError("VUID-vkQueueSubmit-pCommandBuffers-00065",
                                          objlist,
                                          signal_semaphore_loc,
                                          "(%s) is being signaled by %s, but it was previously signaled by %s and has "
                                          "not since been waited on",
                                          core.FormatHandle(handle).c_str(),
                                          core.FormatHandle(queue).c_str(),
                                          initiator.str().c_str());
                } else {
        ...
        case VK_SEMAPHORE_TYPE_TIMELINE: {
            ...
            if (CheckSemaphoreValue(semaphore_state, where, bad_value, must_be_greater)) {
                ...
                skip |= core.LogError(vuid,
                                      ...
                                      signal_semaphore_loc,
                                      "signal value (%" PRIu64
                                      ") in %s must be greater than %s timeline semaphore %s value (%" PRIu64 ")",
                                      value,
                                      core.FormatHandle(queue).c_str(),
                                      where.c_str(),
                                      core.FormatHandle(handle).c_str(),
...
bool CoreChecks::ValidateStageMaskHost(const LogObjectList& objlist,
                                       const Location& stage_mask_loc,
                                       VkPipelineStageFlags2KHR stageMask) const {
    bool skip = false;
    if ((stageMask & VK_PIPELINE_STAGE_HOST_BIT) != 0) {
        const auto& vuid =
            sync_vuid_maps::GetQueueSubmitVUID(stage_mask_loc, sync_vuid_maps::SubmitError::kHostStageMask);
        skip |= LogError(
            vuid,
            objlist,
            stage_mask_loc,
...
bool CoreChecks::ValidateFenceForSubmit(const vvl::Fence& fence_state,
    ...
    if (fence_state.Scope() == vvl::Fence::kInternal) {
        switch (fence_state.State()) {
            case vvl::Fence::kInflight:
                skip |= LogError(inflight_vuid,
                                 objlist,
                                 loc,
                                 "(%s) is already in use by another submission.",
                                 FormatHandle(fence_state.Handle()).c_str());
                break;
            case vvl::Fence::kRetired:
                skip |= LogError(retired_vuid,
                                 objlist,
                                 loc,
...
bool CoreChecks::ValidateSemaphoresForSubmit(SemaphoreSubmitState& state,
    ...
    if (const auto d3d12_fence_submit_info = vku::FindStructInPNextChain<VkD3D12FenceSubmitInfoKHR>(submit.pNext)) {
        if (d3d12_fence_submit_info->waitSemaphoreValuesCount != submit.waitSemaphoreCount) {
            skip |=
                LogError("VUID-VkD3D12FenceSubmitInfoKHR-waitSemaphoreValuesCount-00079",
                         state.queue,
                         submit_loc,
                         ...
                         submit_loc.dot(Field::waitSemaphoreCount).Fields().c_str(),
        ...
        if (d3d12_fence_submit_info->signalSemaphoreValuesCount != submit.signalSemaphoreCount) {
            skip |= LogError(
                ...
                submit_loc.dot(Field::signalSemaphoreCount).Fields().c_str(),
    ...
    for (uint32_t i = 0; i < submit.waitSemaphoreCount; ++i) {
        ...
        if (semaphore_state->type == VK_SEMAPHORE_TYPE_TIMELINE) {
            if (timeline_semaphore_submit_info == nullptr) {
                skip |= LogError("VUID-VkSubmitInfo-pWaitSemaphores-03239",
                                 semaphore,
                                 wait_semaphore_loc,
                ...
                skip |= LogError("VUID-VkSubmitInfo-pNext-03240",
                                 semaphore,
                                 wait_semaphore_loc,
                                 "(%s) is a timeline semaphore, %s (%" PRIu32 ") is different than "
                                 "%s (%" PRIu32 ").",
                                 FormatHandle(semaphore).c_str(),
                                 submit_loc.pNext(Struct::VkTimelineSemaphoreSubmitInfo, Field::waitSemaphoreValueCount)
                                     .Fields()
                                     .c_str(),
    ...
    for (uint32_t i = 0; i < submit.signalSemaphoreCount; ++i) {
        ...
        if (semaphore_state->type == VK_SEMAPHORE_TYPE_TIMELINE) {
            if (timeline_semaphore_submit_info == nullptr) {
                ...
                skip |=
                    LogError("VUID-VkSubmitInfo-pNext-03241",
                             semaphore,
                             signal_semaphore_loc,
                             "(%s) is a timeline semaphore, %s (%" PRIu32 ") is different than "
                             "%s (%" PRIu32 ").",
                             FormatHandle(semaphore).c_str(),
                             submit_loc.pNext(Struct::VkTimelineSemaphoreSubmitInfo, Field::signalSemaphoreValueCount)
                                 .Fields()
                                 .c_str(),
...
bool CoreChecks::ValidateSemaphoresForSubmit(SemaphoreSubmitState& state,
    ...
    for (uint32_t i = 0; i < submit.waitSemaphoreInfoCount; ++i) {
        ...
        skip |=
            ValidatePipelineStage(objlist, wait_info_loc.dot(Field::stageMask), state.queue_flags, wait_info.stageMask);
        ...
        if (semaphore_state->type == VK_SEMAPHORE_TYPE_TIMELINE) {
            for (uint32_t sig_index = 0; sig_index < submit.signalSemaphoreInfoCount; sig_index++) {
                ...
                if (wait_info.semaphore == sig_info.semaphore && wait_info.value >= sig_info.value) {
                    ...
                    skip |= LogError("VUID-VkSubmitInfo2-semaphore-03881",
                                     objlist,
                                     wait_info_loc.dot(Field::value),
                                     "(%" PRIu64 ") is less or equal to %s (%" PRIu64 ").",
                                     wait_info.value,
                                     sig_loc.dot(Field::value).Fields().c_str(),
                                     sig_info.value);
                }
            }
        }
    ...
    for (uint32_t i = 0; i < submit.signalSemaphoreInfoCount; ++i) {
        ...
        skip |= ValidatePipelineStage(
            objlist, signal_info_loc.dot(Field::stageMask), state.queue_flags, sem_info.stageMask);
        ...
        if (auto semaphore_state = Get<vvl::Semaphore>(sem_info.semaphore)) {
            skip |=
                state.ValidateSignalSemaphore(signal_info_loc.dot(Field::semaphore), *semaphore_state, sem_info.value);
        }
    }
    return skip;
}
...
bool CoreChecks::ValidateSemaphoresForSubmit(SemaphoreSubmitState& state,
    ...
    for (uint32_t i = 0; i < submit.waitSemaphoreCount; ++i) {
        ...
        if (semaphore_state->type == VK_SEMAPHORE_TYPE_TIMELINE) {
            if (timeline_semaphore_submit_info == nullptr) {
                skip |= LogError("VUID-VkBindSparseInfo-pWaitSemaphores-03246",
                                 semaphore,
                                 wait_semaphore_loc,
                                 "(%s) is a timeline semaphore, but VkSubmitInfo does "
                                 "not include an instance of VkTimelineSemaphoreSubmitInfo",
                ...
                skip |= LogError("VUID-VkBindSparseInfo-pNext-03247",
                                 semaphore,
                                 wait_semaphore_loc,
                                 "(%s) is a timeline semaphore, %s (%" PRIu32 ") is different than "
                                 "%s (%" PRIu32 ").",
    ...
    for (uint32_t i = 0; i < submit.signalSemaphoreCount; ++i) {
        ...
        if (semaphore_state->type == VK_SEMAPHORE_TYPE_TIMELINE) {
            if (timeline_semaphore_submit_info == nullptr) {
                ...
                skip |=
                    LogError("VUID-VkBindSparseInfo-pNext-03248",
                             semaphore,
                             signal_semaphore_loc,
                             "(%s) is a timeline semaphore, %s (%" PRIu32 ") is different than "
                             "%s (%" PRIu32 ").",
...
bool CoreChecks::PreCallValidateCreateFence(VkDevice device,
    ...
    if (fence_export_info && fence_export_info->handleTypes != 0) {
        ...
        auto check_export_support = [&](VkExternalFenceHandleTypeFlagBits flag) {
            ...
            DispatchGetPhysicalDeviceExternalFencePropertiesHelper(
                physical_device, &external_info, &external_properties);
            if ((external_properties.externalFenceFeatures & VK_EXTERNAL_FENCE_FEATURE_EXPORTABLE_BIT) == 0) {
                export_supported = false;
                skip |= LogError("VUID-VkExportFenceCreateInfo-handleTypes-01446",
                                 device,
        ...
        if (export_supported && (fence_export_info->handleTypes & external_properties.compatibleHandleTypes) !=
                                    fence_export_info->handleTypes) {
            skip |= LogError("VUID-VkExportFenceCreateInfo-handleTypes-01446",
                             device,
                             create_info_loc.pNext(Struct::VkExportFenceCreateInfo, Field::handleTypes),
...
bool CoreChecks::PreCallValidateCreateSemaphore(VkDevice device,
    ...
    if (sem_type_create_info) {
        if (sem_type_create_info->semaphoreType == VK_SEMAPHORE_TYPE_TIMELINE && !enabled_features.timelineSemaphore) {
            skip |= LogError("VUID-VkSemaphoreTypeCreateInfo-timelineSemaphore-03252",
                             device,
        ...
        if (sem_type_create_info->semaphoreType == VK_SEMAPHORE_TYPE_BINARY &&
            ...
            skip |= LogError("VUID-VkSemaphoreTypeCreateInfo-semaphoreType-03279",
                             device,
                             create_info_loc.dot(Field::semaphoreType),
                             "is VK_SEMAPHORE_TYPE_BINARY, but initialValue is %" PRIu64 ".",
                             sem_type_create_info->initialValue);
        }
    }
    ...
    if (sem_export_info && sem_export_info->handleTypes != 0) {
        ...
        auto check_export_support = [&](VkExternalSemaphoreHandleTypeFlagBits flag) {
            ...
            external_info.handleType = flag;
            DispatchGetPhysicalDeviceExternalSemaphorePropertiesHelper(
                physical_device, &external_info, &external_properties);
            if ((external_properties.externalSemaphoreFeatures & VK_EXTERNAL_SEMAPHORE_FEATURE_EXPORTABLE_BIT) == 0) {
                export_supported = false;
                skip |= LogError("VUID-VkExportSemaphoreCreateInfo-handleTypes-01124",
                                 device,
        ...
        // Check handle types compatibility
        if (export_supported && (sem_export_info->handleTypes & external_properties.compatibleHandleTypes) !=
                                    sem_export_info->handleTypes) {
            skip |=
                LogError("VUID-VkExportSemaphoreCreateInfo-handleTypes-01124",
                         device,
                         create_info_loc.pNext(Struct::VkExportSemaphoreCreateInfo, Field::handleTypes),
...
bool CoreChecks::PreCallValidateWaitSemaphoresKHR(VkDevice device,
...
bool CoreChecks::PreCallValidateWaitSemaphores(VkDevice device,
                                               const VkSemaphoreWaitInfo* pWaitInfo,
                                               uint64_t timeout,
                                               const ErrorObject& error_obj) const {
    bool skip = false;
    ...
    for (uint32_t i = 0; i < pWaitInfo->semaphoreCount; i++) {
        ...
        ASSERT_AND_CONTINUE(semaphore_state);
        if (semaphore_state->type != VK_SEMAPHORE_TYPE_TIMELINE) {
            skip |= LogError("VUID-VkSemaphoreWaitInfo-pSemaphores-03256",
                             pWaitInfo->pSemaphores[i],
                             error_obj.location.dot(Field::pWaitInfo).dot(Field::pSemaphores, i),
...
bool CoreChecks::PreCallValidateDestroyFence(VkDevice device,
                                             VkFence fence,
                                             const VkAllocationCallbacks* pAllocator,
                                             const ErrorObject& error_obj) const {
    bool skip = false;
...
bool CoreChecks::PreCallValidateResetFences(VkDevice device,
                                            uint32_t fenceCount,
    ...
    for (uint32_t i = 0; i < fenceCount; ++i) {
        ...
        if (fence_state->Scope() == vvl::Fence::kInternal && fence_state->State() == vvl::Fence::kInflight) {
            skip |= LogError("VUID-vkResetFences-pFences-01123",
                             pFences[i],
                             error_obj.location.dot(Field::pFences, i),
                             "(%s) is in use.",
                             FormatHandle(pFences[i]).c_str());
        }
    }
    return skip;
}

bool CoreChecks::PreCallValidateDestroySemaphore(VkDevice device,
...
bool CoreChecks::PreCallValidateDestroyEvent(VkDevice device,
                                             VkEvent event,
                                             const VkAllocationCallbacks* pAllocator,
                                             const ErrorObject& error_obj) const {
    bool skip = false;
    if (auto event_state = Get<vvl::Event>(event)) {
        skip |= ValidateObjectNotInUse(
            event_state.get(), error_obj.location.dot(Field::event), "VUID-vkDestroyEvent-event-01145");
    }
    return skip;
}

bool CoreChecks::PreCallValidateDestroySampler(VkDevice device,
                                               VkSampler sampler,
                                               const VkAllocationCallbacks* pAllocator,
                                               const ErrorObject& error_obj) const {
    bool skip = false;
    if (auto sampler_state = Get<vvl::Sampler>(sampler)) {
        skip |= ValidateObjectNotInUse(
            sampler_state.get(), error_obj.location.dot(Field::sampler), "VUID-vkDestroySampler-sampler-01082");
    }
    return skip;
}

bool CoreChecks::PreCallValidateCmdSetEvent(VkCommandBuffer commandBuffer,
                                            VkEvent event,
...
bool CoreChecks::PreCallValidateCmdSetEvent2(VkCommandBuffer commandBuffer,
                                             VkEvent event,
    ...
    if (!enabled_features.synchronization2) {
        skip |= LogError("VUID-vkCmdSetEvent2-synchronization2-03824",
                         commandBuffer,
                         error_obj.location,
    ...
    if (pDependencyInfo->dependencyFlags != 0) {
        skip |= LogError("VUID-vkCmdSetEvent2-dependencyFlags-03825",
                         objlist,
                         dep_info_loc.dot(Field::dependencyFlags),
                         "(%s) must be 0.",
...
bool CoreChecks::PreCallValidateCmdSetEvent2KHR(VkCommandBuffer commandBuffer,
                                                VkEvent event,
...
bool CoreChecks::PreCallValidateCmdResetEvent(VkCommandBuffer commandBuffer,
                                              VkEvent event,
                                              VkPipelineStageFlags stageMask,
                                              const ErrorObject& error_obj) const {
    auto cb_state = GetRead<vvl::CommandBuffer>(commandBuffer);
...
bool CoreChecks::PreCallValidateCmdResetEvent2(VkCommandBuffer commandBuffer,
                                               VkEvent event,
    ...
    bool skip = false;
    if (!enabled_features.synchronization2) {
        skip |= LogError("VUID-vkCmdResetEvent2-synchronization2-03829",
                         commandBuffer,
                         error_obj.location,
...
bool CoreChecks::PreCallValidateCmdResetEvent2KHR(VkCommandBuffer commandBuffer,
                                                  VkEvent event,
...
struct RenderPassDepState {
    ...
    bool ValidateStage(const Location& barrier_loc,
        ...
        for (const auto self_dep_index : self_dependencies) {
            ...
            const auto subpass_src_stages = sync_utils::ExpandPipelineStages(
                subpass_dep.srcStageMask, sync_utils::kAllQueueTypes, disabled_features);
            const auto barrier_src_stages =
                sync_utils::ExpandPipelineStages(src_stage_mask, sync_utils::kAllQueueTypes, disabled_features);

            const auto subpass_dst_stages = sync_utils::ExpandPipelineStages(
                subpass_dep.dstStageMask, sync_utils::kAllQueueTypes, disabled_features);
            ...
            if (is_subset)
                return false; // subset is found, return skip value (false)
        }
        return core.LogError(
            vuid,
            rp_handle,
            barrier_loc.dot(Field::srcStageMask),
            ...
            string_VkPipelineStageFlags2(dst_stage_mask).c_str(),
            active_subpass,
            core.FormatHandle(rp_handle).c_str());
    }

    bool
    ValidateAccess(const Location& barrier_loc, VkAccessFlags2 src_access_mask, VkAccessFlags2 dst_access_mask) const {
        ...
        return core.LogError(
            vuid,
            rp_handle,
            barrier_loc.dot(Field::srcAccessMask),
            "(%s) and dstAccessMask (%s) is not a subset of subpass dependency's srcAccessMask and dstAccessMask "
            "of subpass %" PRIu32 " of %s.",
            string_VkAccessFlags2(src_access_mask).c_str(),
            string_VkAccessFlags2(dst_access_mask).c_str(),
            active_subpass,
            core.FormatHandle(rp_handle).c_str());
    }
    ...
    bool ValidateDependencyFlag(const Location& dep_flags_loc, VkDependencyFlags dependency_flags) const {
        for (const auto self_dep_index : self_dependencies) {
            ...
            if (match)
                return false; // match is found, return skip value (false)
        }
        return core.LogError(vuid,
                             rp_handle,
                             dep_flags_loc,
...
bool CoreChecks::ValidateRenderPassPipelineStage(VkRenderPass render_pass,
                                                 const Location& loc,
                                                 VkPipelineStageFlags2 src_stage_mask,
                                                 VkPipelineStageFlags2 dst_stage_mask) const {
    bool skip = false;
    ...
    if (src_diff != 0) {
        ...
        skip |= LogError(vuid,
                         render_pass,
                         loc.dot(Field::srcStageMask),
                         "contains non graphics stage %s.",
...
bool CoreChecks::ValidateRenderPassPipelineBarriers(const Location& outer_loc,
                                                    ...
                                                    uint32_t buffer_mem_barrier_count,
    ...
    if (0 != buffer_mem_barrier_count) {
        skip |= LogError("VUID-vkCmdPipelineBarrier-bufferMemoryBarrierCount-01178",
                         state.rp_handle,
                         outer_loc.dot(Field::bufferMemoryBarrierCount),
                         "is non-zero (%" PRIu32 ") for subpass %" PRIu32 " of %s.",
                         buffer_mem_barrier_count,
                         state.active_subpass,
    ...
    for (uint32_t i = 0; i < image_mem_barrier_count; ++i) {
        ...
        if (img_barrier.srcQueueFamilyIndex != img_barrier.dstQueueFamilyIndex) {
            skip |= LogError("VUID-vkCmdPipelineBarrier-srcQueueFamilyIndex-01182",
                             state.rp_handle,
        ...
        if (cb_state.activeFramebuffer) {
            skip |= ValidateImageBarrierAttachment(barrier_loc,
                                                   cb_state,
                                                   *cb_state.activeFramebuffer,
                                                   state.active_subpass,
                                                   sub_desc,
                                                   state.rp_handle,
                                                   img_barrier);
    ...
    if (GetBitSetCount(sub_desc.viewMask) > 1 && ((dependency_flags & VK_DEPENDENCY_VIEW_LOCAL_BIT) == 0)) {
        skip |=
            LogError("VUID-vkCmdPipelineBarrier-None-07893",
                     state.rp_handle,
                     outer_loc.dot(Field::dependencyFlags),
...
bool CoreChecks::ValidateRenderPassPipelineBarriers(const Location& outer_loc,
    ...
    for (uint32_t i = 0; i < dep_info.memoryBarrierCount; ++i) {
        ...
        skip |= state.ValidateAccess(barrier_loc, mem_barrier.srcAccessMask, mem_barrier.dstAccessMask);
        skip |= ValidateRenderPassPipelineStage(
            state.rp_handle, outer_loc, mem_barrier.srcStageMask, mem_barrier.dstStageMask);
    }
    if (0 != dep_info.bufferMemoryBarrierCount) {
        skip |= LogError("VUID-vkCmdPipelineBarrier2-bufferMemoryBarrierCount-01178",
                         state.rp_handle,
                         outer_loc.dot(Field::bufferMemoryBarrierCount),
                         "is non-zero (%" PRIu32 ") for subpass %" PRIu32 " of %s.",
                         dep_info.bufferMemoryBarrierCount,
                         state.active_subpass,
                         FormatHandle(state.rp_handle).c_str());
    }
    for (uint32_t i = 0; i < dep_info.imageMemoryBarrierCount; ++i) {
        ...
        skip |= state.ValidateAccess(barrier_loc, img_barrier.srcAccessMask, img_barrier.dstAccessMask);
        skip |= ValidateRenderPassPipelineStage(
            state.rp_handle, outer_loc, img_barrier.srcAccessMask, img_barrier.dstAccessMask);

        if (img_barrier.srcQueueFamilyIndex != img_barrier.dstQueueFamilyIndex) {
            skip |= LogError("VUID-vkCmdPipelineBarrier2-srcQueueFamilyIndex-01182",
                             state.rp_handle,
                             barrier_loc.dot(Field::srcQueueFamilyIndex),
                             "is %" PRIu32 " and dstQueueFamilyIndex is %" PRIu32 " but they must be equal.",
                             img_barrier.srcQueueFamilyIndex,
    ...
    if (GetBitSetCount(sub_desc.viewMask) > 1 && ((dep_info.dependencyFlags & VK_DEPENDENCY_VIEW_LOCAL_BIT) == 0)) {
        skip |=
            LogError("VUID-vkCmdPipelineBarrier2-None-07893",
                     state.rp_handle,
                     outer_loc.dot(Field::dependencyFlags),
                     "%s is missing VK_DEPENDENCY_VIEW_LOCAL_BIT and subpass %" PRIu32 " has viewMasks 0x%" PRIx32 ".",
                     string_VkDependencyFlags(dep_info.dependencyFlags).c_str(),
                     state.active_subpass,
                     sub_desc.viewMask);
    }
...
bool CoreChecks::ValidateStageMasksAgainstQueueCapabilities(const LogObjectList& objlist,
                                                            const Location& stage_mask_loc,
    ...
    for (const auto& entry : metaFlags) {
        if (((entry.first & stage_mask) != 0) && ((entry.second & queue_flags) == 0)) {
            ...
            skip |= LogError(vuid,
                             ...
                             sync_utils::StringPipelineStageFlags(entry.first).c_str(),
    ...
    for (size_t i = 0; i < sizeof(bad_flags) * 8; i++) {
        ...
        if (bit) {
            ...
            skip |= LogError(vuid,
                             ...
                             stage_mask_loc,
                             "(%s) is not compatible with the queue family properties (%s) of this command buffer.",
                             sync_utils::StringPipelineStageFlags(bit).c_str(),
                             string_VkQueueFlags(queue_flags).c_str());
        }
    }
    return skip;
...
bool CoreChecks::ValidatePipelineStageFeatureEnables(const LogObjectList& objlist,
                                                     const Location& stage_mask_loc,
                                                     VkPipelineStageFlags2KHR stage_mask) const {
    bool skip = false;
    ...
    for (size_t i = 0; i < sizeof(bad_bits) * 8; i++) {
        ...
        if (bit & bad_bits) {
            ...
            skip |= LogError(vuid,
                             objlist,
                             stage_mask_loc,
                             "includes %s when the device does not have %s feature enabled.",
                             sync_utils::StringPipelineStageFlags(bit).c_str(),
                             sync_vuid_maps::GetFeatureNameMap().at(bit).c_str());
        }
    }
    return skip;
}

bool CoreChecks::ValidatePipelineStage(const LogObjectList& objlist,
                                       const Location& stage_mask_loc,
                                       VkQueueFlags queue_flags,
                                       VkPipelineStageFlags2KHR stage_mask) const {
    bool skip = false;
...
bool CoreChecks::ValidateAccessMask(const LogObjectList& objlist,
                                    const Location& access_mask_loc,
                                    const Location& stage_mask_loc,
                                    VkQueueFlags queue_flags,
    ...
    if (!enabled_features.rayQuery && (access_mask & VK_ACCESS_2_ACCELERATION_STRUCTURE_READ_BIT_KHR)) {
        const auto illegal_pipeline_stages =
    ...
    for (size_t i = 0; i < sizeof(bad_accesses) * 8; i++) {
        ...
        if (bad_accesses & bit) {
            ...
            skip |= LogError(vuid,
                             objlist,
                             access_mask_loc,
                             "(%s) is not supported by stage mask (%s).",
                             sync_utils::StringAccessFlags(bit).c_str(),
                             sync_utils::StringPipelineStageFlags(stage_mask).c_str());
        }
    }
...
bool CoreChecks::ValidateWaitEventsAtSubmit(vvl::Func command,
                                            const vvl::CommandBuffer& cb_state,
                                            size_t eventCount,
                                            size_t firstEventIndex,
                                            VkPipelineStageFlags2 sourceStageMask,
                                            const EventMap& local_event_signal_info,
    ...
    for (size_t event_index = firstEventIndex; event_index < max_event; ++event_index) {
        ...
        if (const auto* event_info = vvl::Find(local_event_signal_info, event)) {
            ...
            // The "set event" is found in the current submission (the same queue); there can't be inter-queue usage
            // errors
        } else {
            ...
            if (event_state->signaling_queue != VK_NULL_HANDLE && event_state->signaling_queue != waiting_queue) {
                ...
                skip |= state_data.LogError(
                    ...
                    state_data.FormatHandle(waiting_queue).c_str(),
    ...
    if (sourceStageMask != stage_mask && sourceStageMask != (stage_mask | VK_PIPELINE_STAGE_HOST_BIT)) {
        skip |= state_data.LogError("VUID-vkCmdWaitEvents-srcStageMask-parameter",
                                    cb_state.Handle(),
                                    loc,
                                    "Submitting cmdbuffer with call to VkCmdWaitEvents using srcStageMask %s which "
                                    "must be the bitwise OR of the stageMask "
                                    "parameters used in calls to vkCmdSetEvent and VK_PIPELINE_STAGE_HOST_BIT if used "
                                    "with vkSetEvent but instead is %s.",
                                    string_VkPipelineStageFlags2(sourceStageMask).c_str(),
                                    string_VkPipelineStageFlags2(stage_mask).c_str());
    }
    return skip;
}

bool CoreChecks::PreCallValidateCmdWaitEvents(VkCommandBuffer commandBuffer,
                                              uint32_t eventCount,
    ...
    skip |= ValidateCmd(*cb_state, error_obj.location);
    skip |= ValidateBarriers(error_obj.location,
                             *cb_state,
                             srcStageMask,
                             dstStageMask,
                             memoryBarrierCount,
    ...
    for (uint32_t i = 0; i < bufferMemoryBarrierCount; ++i) {
        if (pBufferMemoryBarriers[i].srcQueueFamilyIndex != pBufferMemoryBarriers[i].dstQueueFamilyIndex) {
            skip |= LogError("VUID-vkCmdWaitEvents-srcQueueFamilyIndex-02803",
                             commandBuffer,
    ...
    if (cb_state->activeRenderPass && ((srcStageMask & VK_PIPELINE_STAGE_HOST_BIT) != 0)) {
        skip |= LogError("VUID-vkCmdWaitEvents-srcStageMask-07308",
                         commandBuffer,
                         error_obj.location.dot(Field::srcStageMask),
                         "is %s.",
...
bool CoreChecks::PreCallValidateCmdWaitEvents2(VkCommandBuffer commandBuffer,
                                               uint32_t eventCount,
                                               const VkEvent* pEvents,
    ...
    bool skip = false;
    if (!enabled_features.synchronization2) {
        skip |= LogError("VUID-vkCmdWaitEvents2-synchronization2-03836",
                         commandBuffer,
                         error_obj.location,
                         "the synchronization2 feature was not enabled.");
    }
    for (uint32_t i = 0; (i < eventCount) && !skip; i++) {
        ...
        if (pDependencyInfos[i].dependencyFlags != 0) {
            ...
            skip |= LogError("UNASSIGNED-vkCmdWaitEvents2-dependencyFlags",
                             objlist,
                             dep_info_loc.dot(Field::dependencyFlags),
                             "(%s) must be 0.",
...
bool CoreChecks::PreCallValidateCmdWaitEvents2KHR(VkCommandBuffer commandBuffer,
                                                  uint32_t eventCount,
                                                  const VkEvent* pEvents,
...
void CoreChecks::PreCallRecordCmdWaitEvents(VkCommandBuffer commandBuffer,
                                            uint32_t eventCount,
                                            const VkEvent* pEvents,
    ...
    BaseClass::PreCallRecordCmdWaitEvents(commandBuffer,
                                          eventCount,
                                          pEvents,
                                          sourceStageMask,
                                          dstStageMask,
...
void CoreChecks::RecordCmdWaitEvents2(VkCommandBuffer commandBuffer,
                                      uint32_t eventCount,
                                      const VkEvent* pEvents,
                                      const VkDependencyInfo* pDependencyInfos,
                                      Func command) {
...
void CoreChecks::PreCallRecordCmdWaitEvents2KHR(VkCommandBuffer commandBuffer,
                                                uint32_t eventCount,
                                                const VkEvent* pEvents,
                                                const VkDependencyInfoKHR* pDependencyInfos,
                                                const RecordObject& record_obj) {
...
void CoreChecks::PreCallRecordCmdWaitEvents2(VkCommandBuffer commandBuffer,
                                             uint32_t eventCount,
                                             const VkEvent* pEvents,
                                             const VkDependencyInfo* pDependencyInfos,
                                             const RecordObject& record_obj) {
...
void CoreChecks::PostCallRecordCmdWaitEvents(VkCommandBuffer commandBuffer,
                                             uint32_t eventCount,
                                             const VkEvent* pEvents,
                                             VkPipelineStageFlags sourceStageMask,
                                             VkPipelineStageFlags dstStageMask,
                                             ...
                                             const RecordObject& record_obj) {
    auto cb_state = GetWrite<vvl::CommandBuffer>(commandBuffer);
    RecordBarriers(record_obj.location.function,
                   *cb_state,
                   sourceStageMask,
                   dstStageMask,
                   bufferMemoryBarrierCount,
...
void CoreChecks::PostCallRecordCmdWaitEvents2KHR(VkCommandBuffer commandBuffer,
                                                 uint32_t eventCount,
                                                 const VkEvent* pEvents,
                                                 const VkDependencyInfoKHR* pDependencyInfos,
                                                 const RecordObject& record_obj) {
...
void CoreChecks::PostCallRecordCmdWaitEvents2(VkCommandBuffer commandBuffer,
                                              uint32_t eventCount,
                                              const VkEvent* pEvents,
                                              const VkDependencyInfo* pDependencyInfos,
                                              const RecordObject& record_obj) {
...
bool CoreChecks::PreCallValidateCmdPipelineBarrier(VkCommandBuffer commandBuffer,
                                                   VkPipelineStageFlags srcStageMask,
                                                   VkPipelineStageFlags dstStageMask,
    ...
    if (cb_state->activeRenderPass && !cb_state->activeRenderPass->UsesDynamicRendering()) {
        skip |= ValidateRenderPassPipelineBarriers(error_obj.location,
                                                   *cb_state,
                                                   srcStageMask,
                                                   dstStageMask,
                                                   dependencyFlags,
                                                   ...
                                                   pImageMemoryBarriers);
        if (skip)
            return true; // Early return to avoid redundant errors from below calls
    } else {
        if (dependencyFlags & VK_DEPENDENCY_VIEW_LOCAL_BIT) {
            skip = LogError("VUID-vkCmdPipelineBarrier-dependencyFlags-01186",
                            objlist,
                            error_obj.location.dot(Field::dependencyFlags),
    ...
    if (cb_state->activeRenderPass && cb_state->activeRenderPass->UsesDynamicRendering()) {
        ...
        skip |= ValidateShaderTileImageBarriers(objlist,
                                                error_obj.location,
                                                dependencyFlags,
                                                memoryBarrierCount,
                                                pMemoryBarriers,
...
bool CoreChecks::PreCallValidateCmdPipelineBarrier2(VkCommandBuffer commandBuffer,
                                                    const VkDependencyInfo* pDependencyInfo,
                                                    const ErrorObject& error_obj) const {
    bool skip = false;
    auto cb_state = GetRead<vvl::CommandBuffer>(commandBuffer);
    ...
    const Location dep_info_loc = error_obj.location.dot(Field::pDependencyInfo);
    if (!enabled_features.synchronization2) {
        skip |= LogError("VUID-vkCmdPipelineBarrier2-synchronization2-03848",
                         commandBuffer,
                         error_obj.location,
                         "the synchronization2 feature was not enabled.");
    }
    ...
    if (cb_state->activeRenderPass) {
        ...
        if (pDependencyInfo->dependencyFlags & VK_DEPENDENCY_VIEW_LOCAL_BIT) {
            skip |= LogError("VUID-vkCmdPipelineBarrier2-dependencyFlags-01186",
                             objlist,
                             dep_info_loc.dot(Field::dependencyFlags),
                             "VK_DEPENDENCY_VIEW_LOCAL_BIT must not be set outside of a render pass instance.");
        }
...
void CoreChecks::PreCallRecordCmdPipelineBarrier(VkCommandBuffer commandBuffer,
                                                 VkPipelineStageFlags srcStageMask,
                                                 VkPipelineStageFlags dstStageMask,
                                                 VkDependencyFlags dependencyFlags,
                                                 uint32_t memoryBarrierCount,
                                                 ...
                                                 const VkImageMemoryBarrier* pImageMemoryBarriers,
                                                 const RecordObject& record_obj) {
    BaseClass::PreCallRecordCmdPipelineBarrier(commandBuffer,
                                               srcStageMask,
                                               dstStageMask,
                                               dependencyFlags,
                                               memoryBarrierCount,
...
void CoreChecks::PreCallRecordCmdPipelineBarrier2KHR(VkCommandBuffer commandBuffer,
                                                     const VkDependencyInfoKHR* pDependencyInfo,
                                                     const RecordObject& record_obj) {
...
bool CoreChecks::PreCallValidateSetEvent(VkDevice device, VkEvent event, const ErrorObject& error_obj) const {
    ...
    if (auto event_state = Get<vvl::Event>(event)) {
        if (event_state->write_in_use) {
            skip |= LogError("VUID-vkSetEvent-event-09543",
                             event,
                             error_obj.location.dot(Field::event),
                             "(%s) that is already in use by a command buffer.",
                             FormatHandle(event).c_str());
        }
        if (event_state->flags & VK_EVENT_CREATE_DEVICE_ONLY_BIT) {
            skip |= LogError("VUID-vkSetEvent-event-03941",
                             event,
                             error_obj.location.dot(Field::event),
                             "(%s) was created with VK_EVENT_CREATE_DEVICE_ONLY_BIT.",
                             FormatHandle(event).c_str());
        }
    }
    return skip;
...
bool CoreChecks::PreCallValidateResetEvent(VkDevice device, VkEvent event, const ErrorObject& error_obj) const {
    ...
    if (auto event_state = Get<vvl::Event>(event)) {
        if (event_state->flags & VK_EVENT_CREATE_DEVICE_ONLY_BIT) {
            skip |= LogError("VUID-vkResetEvent-event-03823",
                             event,
                             error_obj.location.dot(Field::event),
                             "(%s) was created with VK_EVENT_CREATE_DEVICE_ONLY_BIT.",
                             FormatHandle(event).c_str());
...
bool CoreChecks::PreCallValidateGetEventStatus(VkDevice device, VkEvent event, const ErrorObject& error_obj) const {
    ...
    if (auto event_state = Get<vvl::Event>(event)) {
        if (event_state->flags & VK_EVENT_CREATE_DEVICE_ONLY_BIT) {
            skip |= LogError("VUID-vkGetEventStatus-event-03940",
                             event,
                             error_obj.location.dot(Field::event),
                             "(%s) was created with VK_EVENT_CREATE_DEVICE_ONLY_BIT.",
                             FormatHandle(event).c_str());
...
bool CoreChecks::PreCallValidateSignalSemaphore(VkDevice device,
    ...
    if (semaphore_state->type != VK_SEMAPHORE_TYPE_TIMELINE) {
        skip |= LogError("VUID-VkSemaphoreSignalInfo-semaphore-03257",
                         pSignalInfo->semaphore,
                         signal_loc.dot(Field::semaphore),
                         "%s was created with %s.",
    ...
    if (current_payload >= pSignalInfo->value) {
        skip |= LogError("VUID-VkSemaphoreSignalInfo-value-03258",
                         pSignalInfo->semaphore,
                         signal_loc.dot(Field::value),
                         "(%" PRIu64 ") must be greater than current semaphore %s value (%" PRIu64 ").",
                         pSignalInfo->value,
                         FormatHandle(pSignalInfo->semaphore).c_str(),
                         current_payload);
        return skip;
    ...
    if (last_op) {
        skip |= LogError("VUID-VkSemaphoreSignalInfo-value-03259",
                         pSignalInfo->semaphore,
                         signal_loc.dot(Field::value),
                         "(%" PRIu64 ") must be less than value of any pending signal operation (%" PRIu64
                         ") for semaphore %s.",
                         pSignalInfo->value,
                         last_op->payload,
                         FormatHandle(pSignalInfo->semaphore).c_str());
    ...
    TimelineMaxDiffCheck exceeds_max_diff(pSignalInfo->value,
    ...
    if (where) {
        ...
        skip |= LogError(vuid,
                         semaphore_state->Handle(),
                         loc,
                         "(%" PRIu64 ") exceeds limit regarding %s semaphore %s payload (%" PRIu64 ").",
                         pSignalInfo->value,
                         FormatHandle(*semaphore_state).c_str(),
                         where,
                         bad_value);
    }
    return skip;
...
bool CoreChecks::PreCallValidateSignalSemaphoreKHR(VkDevice device,
                                                   const VkSemaphoreSignalInfo* pSignalInfo,
                                                   const ErrorObject& error_obj) const {
    return PreCallValidateSignalSemaphore(device, pSignalInfo, error_obj);
}

bool CoreChecks::PreCallValidateGetSemaphoreCounterValue(VkDevice device,
                                                         VkSemaphore semaphore,
                                                         uint64_t* pValue,
                                                         const ErrorObject& error_obj) const {
    bool skip = false;
    ...
    ASSERT_AND_RETURN_SKIP(semaphore_state);
    if (semaphore_state->type != VK_SEMAPHORE_TYPE_TIMELINE) {
        skip |= LogError("VUID-vkGetSemaphoreCounterValue-semaphore-03255",
                         semaphore,
                         error_obj.location.dot(Field::semaphore),
                         "%s was created with %s.",
                         FormatHandle(semaphore).c_str(),
                         string_VkSemaphoreType(semaphore_state->type));
    }
...
bool CoreChecks::PreCallValidateGetSemaphoreCounterValueKHR(VkDevice device,
                                                            VkSemaphore semaphore,
                                                            uint64_t* pValue,
                                                            const ErrorObject& error_obj) const {
...
static inline VkQueueFlags SubpassToQueueFlags(uint32_t subpass) {
    return subpass == VK_SUBPASS_EXTERNAL ? sync_utils::kAllQueueTypes
                                          : static_cast<VkQueueFlags>(VK_QUEUE_GRAPHICS_BIT);
}

bool CoreChecks::ValidateSubpassDependency(const ErrorObject& error_obj,
                                           const Location& in_loc,
    ...
    skip |= ValidatePipelineStage(
        error_obj.objlist, loc.dot(Field::srcStageMask), src_queue_flags, converted_barrier.srcStageMask);
    skip |= ValidateAccessMask(error_obj.objlist,
                               loc.dot(Field::srcAccessMask),
                               loc.dot(Field::srcStageMask),
                               src_queue_flags,
                               converted_barrier.srcAccessMask,
                               converted_barrier.srcStageMask);

    auto dst_queue_flags = SubpassToQueueFlags(dependency.dstSubpass);
    skip |= ValidatePipelineStage(
        error_obj.objlist, loc.dot(Field::dstStageMask), dst_queue_flags, converted_barrier.dstStageMask);
    skip |= ValidateAccessMask(error_obj.objlist,
                               loc.dot(Field::dstAccessMask),
                               loc.dot(Field::dstStageMask),
                               dst_queue_flags,
                               converted_barrier.dstAccessMask,
...
bool CoreChecks::ValidateBarrierLayoutToImageUsage(const Location& layout_loc,
                                                   VkImage image,
    ...
    if (is_error) {
        ...
        skip |= LogError(vuid,
                         image,
                         layout_loc,
                         "(%s) is not compatible with %s usage flags %s.",
...
bool CoreChecks::ValidateBarriersToImages(const Location& barrier_loc,
                                          const vvl::CommandBuffer& cb_state,
...

    {
        ...
        if ((img_barrier.srcQueueFamilyIndex != img_barrier.dstQueueFamilyIndex) ||
            ...
            skip |= ValidateBarrierLayoutToImageUsage(
                barrier_loc.dot(Field::oldLayout), img_barrier.image, img_barrier.oldLayout, usage_flags);
            skip |= ValidateBarrierLayoutToImageUsage(
                barrier_loc.dot(Field::newLayout), img_barrier.image, img_barrier.newLayout, usage_flags);
        }
        ...
        if (image_state->layout_locked) {
            ...
            skip |= LogError(
                "UNASSIGNED-barrier-shared-presentable",
                img_barrier.image,
                image_loc,
            ...
            if (enabled_features.separateDepthStencilLayouts) {
                if (!has_depth_mask && !has_stencil_mask) {
                    ...
                    skip |= LogError(vuid,
                                     img_barrier.image,
                                     image_loc,
                                     "(%s) has depth/stencil format %s, but its aspectMask is %s.",
                                     FormatHandle(img_barrier.image).c_str(),
        ...
        if (has_depth_mask) {
            if (IsImageLayoutStencilOnly(img_barrier.oldLayout) || IsImageLayoutStencilOnly(img_barrier.newLayout)) {
                ...
                skip |= LogError(vuid,
                                 img_barrier.image,
                                 image_loc,
                                 "(%s) has stencil format %s has depth aspect with stencil only layouts, oldLayout = "
                                 "%s and newLayout = %s.",
                                 FormatHandle(img_barrier.image).c_str(),
                                 string_VkFormat(image_format),
                                 string_VkImageLayout(img_barrier.oldLayout),
        ...
        if (has_stencil_mask) {
            if (IsImageLayoutDepthOnly(img_barrier.oldLayout) || IsImageLayoutDepthOnly(img_barrier.newLayout)) {
                ...
                skip |= LogError(vuid,
                                 img_barrier.image,
                                 image_loc,
                                 "(%s) has depth format %s has stencil aspect with depth only layouts, oldLayout = %s "
                                 "and newLayout = %s.",
                                 FormatHandle(img_barrier.image).c_str(),
                                 string_VkFormat(image_format),
                                 string_VkImageLayout(img_barrier.oldLayout),
        ...
        if (img_barrier.oldLayout == VK_IMAGE_LAYOUT_UNDEFINED) {
            ...
            skip |=
                UpdateCommandBufferImageLayoutMap(cb_state, image_loc, img_barrier, current_map, layout_updates_state);
        }
            ...
            for (auto color_attachment_idx : used_attachments) {
                ...
                if (img_barrier_image == image_view_image_state->VkHandle()) {
                    ...
                    for (const auto& entry : *image_view_image_state->layout_range_map) {
                        if (entry.second != VK_IMAGE_LAYOUT_RENDERING_LOCAL_READ &&
                            entry.second != VK_IMAGE_LAYOUT_GENERAL) {
        ...
        if (vkuFormatIsColor(image_format) && (aspect_mask != VK_IMAGE_ASPECT_COLOR_BIT)) {
            if (!vkuFormatIsMultiplane(image_format)) {
                ...
                skip |= LogError(vuid,
                                 img_barrier.image,
                                 image_loc,
                                 "(%s) has color format %s, but its aspectMask is %s.",
                                 FormatHandle(img_barrier.image).c_str(),
                                 string_VkFormat(image_format),
                                 string_VkImageAspectFlags(aspect_mask).c_str());
        ...
        if ((vkuFormatIsMultiplane(image_format)) && (image_state->disjoint == true)) {
            if (!IsValidPlaneAspect(image_format, aspect_mask) && ((aspect_mask & VK_IMAGE_ASPECT_COLOR_BIT) == 0)) {
                ...
                skip |= LogError(vuid,
                                 img_barrier.image,
                                 image_loc,
                                 "(%s) has Multiplane format %s, but its aspectMask is %s.",
                                 FormatHandle(img_barrier.image).c_str(),
                                 string_VkFormat(image_format),
                                 string_VkImageAspectFlags(aspect_mask).c_str());
            }
...
bool CoreChecks::ValidateImageBarrierAttachment(const Location& barrier_loc,
                                                const vvl::CommandBuffer& cb_state,
                                                const vvl::Framebuffer& fb_state,
                                                uint32_t active_subpass,
                                                const vku::safe_VkSubpassDescription2& sub_desc,
                                                const VkRenderPass rp_handle,
                                                const ImageBarrier& img_barrier,
                                                const vvl::CommandBuffer* primary_cb_state) const {
    using sync_vuid_maps::GetImageBarrierVUID;
    using sync_vuid_maps::ImageError;
    ...
    if (image_match) { // Make sure subpass is referring to matching attachment
                ...
                if (!sub_image_found && sub_desc.pResolveAttachments &&
                    ...
                    if (image_ahb_format == 0) {
                        ...
                        skip |= LogError(vuid,
                                         ...
                                         FormatHandle(img_bar_image).c_str(),
                                         active_subpass);
                    } else if (sub_desc.pColorAttachments &&
                               sub_desc.pColorAttachments[0].attachment != VK_ATTACHMENT_UNUSED) {
                        ...
                        skip |= LogError(vuid,
                                         ...
                                         FormatHandle(img_bar_image).c_str(),
                                         active_subpass,
                                         sub_desc.pColorAttachments[0].attachment);
                    }
                    break;
                }
            }
        ...
        if (!sub_image_found) {
            ...
            skip |= LogError(vuid,
                             rp_handle,
                             image_loc,
                             "(%s) is not referenced by the VkSubpassDescription for active subpass (%" PRIu32
                             ") of current %s.",
                             FormatHandle(img_bar_image).c_str(),
                             active_subpass,
                             FormatHandle(rp_handle).c_str());
        }
    } else { // !image_match
        const auto& vuid = GetImageBarrierVUID(barrier_loc, ImageError::kRenderPassMismatch);
        skip |= LogError(vuid,
                         fb_state.Handle(),
                         image_loc,
                         "(%s) does not match an image from the current %s.",
                         FormatHandle(img_bar_image).c_str(),
                         FormatHandle(fb_state.Handle()).c_str());
    }
    if (img_barrier.oldLayout != img_barrier.newLayout) {
        const auto& vuid = GetImageBarrierVUID(barrier_loc, ImageError::kRenderPassLayoutChange);
        skip |= LogError(vuid,
                         cb_state.Handle(),
                         barrier_loc.dot(Field::oldLayout),
        ...
        if (sub_image_found && sub_image_layout != img_barrier.oldLayout) {
            ...
            skip |=
                LogError(vuid,
                         ...
                         active_subpass,
                         FormatHandle(rp_handle).c_str(),
                         string_VkImageLayout(sub_image_layout),
                         string_VkImageLayout(img_barrier.oldLayout));
        }
    }
    return skip;
...
void CoreChecks::EnqueueSubmitTimeValidateImageBarrierAttachment(const Location& loc,
    ...
    if ((cb_state.activeRenderPass) && (VK_NULL_HANDLE == cb_state.activeFramebuffer) && cb_state.IsSecondary()) {
        ...
        if (active_subpass < rp_state->create_info.subpassCount) {
            ...
            cb_state.cmd_execute_commands_functions.emplace_back([this_ptr,
                                                                  loc_capture,
                                                                  active_subpass,
                                                                  sub_desc,
                                                                  render_pass,
                                                                  barrier](const vvl::CommandBuffer& secondary_cb,
                                                                           const vvl::CommandBuffer* primary_cb,
                                                                           const vvl::Framebuffer* fb) {
                if (!fb)
                    return false;
                return this_ptr->ValidateImageBarrierAttachment(
                    loc_capture.Get(), secondary_cb, *fb, active_subpass, sub_desc, render_pass, barrier, primary_cb);
            });
        }
    }
}

static bool IsQueueFamilyValid(const ValidationStateTracker& device_data, uint32_t queue_family) {
...
static const char* GetFamilyAnnotation(const ValidationStateTracker& device_data, uint32_t family) {
    switch (family) {
...
bool CoreChecks::ValidateHostStage(const LogObjectList& objlist,
                                   const Location& barrier_loc,
    ...
    if (barrier.srcQueueFamilyIndex != barrier.dstQueueFamilyIndex) {
        ...
        if (stage_field != vvl::Field::Empty) {
            ...
            const Location stage_loc =
                is_sync2 ? barrier_loc.dot(stage_field) : Location(barrier_loc.function, stage_field);
            skip |= LogError(vuid,
                             objlist,
                             stage_loc,
...
void CoreChecks::RecordBarrierValidationInfo(const Location& barrier_loc,
                                             vvl::CommandBuffer& cb_state,
...
void CoreChecks::RecordBarriers(Func func_name,
                                vvl::CommandBuffer& cb_state,
                                VkPipelineStageFlags src_stage_mask,
                                VkPipelineStageFlags dst_stage_mask,
...
bool CoreChecks::ValidateAndUpdateQFOScoreboard(const vvl::CommandBuffer& cb_state,
                                                const char* operation,
                                                const TransferBarrier& barrier,
    ...
    if (!inserted.second && inserted.first->second != &cb_state) {
        ...
        skip |= LogWarning(TransferBarrier::DuplicateQFOInSubmit(),
                           objlist,
                           loc,
                           "%s %s queue ownership of %s (%s), from srcQueueFamilyIndex %" PRIu32
                           " to dstQueueFamilyIndex %" PRIu32
                           " duplicates existing barrier submitted in this batch from %s.",
                           TransferBarrier::BarrierName(),
                           operation,
                           TransferBarrier::HandleName(),
                           FormatHandle(barrier.handle).c_str(),
                           barrier.srcQueueFamilyIndex,
                           barrier.dstQueueFamilyIndex,
    ...
    for (const auto& release : cb_barriers.release) {
        ...
        if (set_it != global_release_barriers.cend()) {
            ...
            if (found != set_for_handle.cend()) {
                skip |= LogWarning(
                    TransferBarrier::DuplicateQFOSubmitted(),
                    cb_state.Handle(),
                    loc,
                    ...
                    barrier_name,
                    handle_name,
                    FormatHandle(found->handle).c_str(),
                    found->srcQueueFamilyIndex,
    ...
    for (const auto& acquire : cb_barriers.acquire) {
        ...
        if (!matching_release_found) {
            ...
            skip |= LogError(
                ...
                barrier_name,
                handle_name,
                FormatHandle(acquire.handle).c_str(),
...
bool CoreChecks::ValidateQueuedQFOTransfers(const vvl::CommandBuffer& cb_state,
                                            ...
                                            const Location& loc) const {
    bool skip = false;
    skip |= ValidateQueuedQFOTransferBarriers<QFOImageTransferBarrier>(
        cb_state, qfo_image_scoreboards, qfo_release_image_barrier_map, loc);
    skip |= ValidateQueuedQFOTransferBarriers<QFOBufferTransferBarrier>(
        cb_state, qfo_buffer_scoreboards, qfo_release_buffer_barrier_map, loc);
    return skip;
}
...
void CoreChecks::RecordQueuedQFOTransfers(vvl::CommandBuffer& cb_state) {
    RecordQueuedQFOTransferBarriers<QFOImageTransferBarrier>(cb_state.qfo_transfer_image_barriers,
...
bool CoreChecks::ValidateQFOTransferBarrierUniqueness(
    const Location& barrier_loc,
    const vvl::CommandBuffer& cb_state,
    ...
    if (barrier_record != nullptr) {
        skip |= LogWarning(TransferBarrier::DuplicateQFOInCB(),
                           cb_state.Handle(),
                           barrier_loc,
                           "%s queue ownership of %s (%s), from srcQueueFamilyIndex %" PRIu32
                           " to dstQueueFamilyIndex %" PRIu32
                           " duplicates existing barrier recorded in this command buffer.",
                           transfer_type,
                           handle_name,
                           FormatHandle(barrier_record->handle).c_str(),
                           barrier_record->srcQueueFamilyIndex,
                           barrier_record->dstQueueFamilyIndex);
    }
    return skip;
}

bool CoreChecks::ValidateBarrierQueueFamilies(const LogObjectList& objects,
                                              const Location& barrier_loc,
                                              const Location& field_loc,
                                              const OwnershipTransferBarrier& barrier,
                                              const VulkanTypedHandle& resource_handle,
                                              VkSharingMode sharing_mode,
                                              uint32_t command_pool_queue_family) const {
    bool skip = false;
    using sync_vuid_maps::QueueError;

    auto log_queue_family_error =
        [sharing_mode, resource_handle, &barrier_loc, &field_loc, device_data_ = this, objects_ = objects](
            QueueError vu_index, uint32_t family, const char* param_name) -> bool {
        ...
        return device_data_->LogError(vuid,
                                      objects_,
                                      barrier_loc,
                                      "barrier using %s created with sharingMode %s, has %s %" PRIu32 "%s. %s",
                                      device_data_->FormatHandle(resource_handle).c_str(),
                                      string_VkSharingMode(sharing_mode),
                                      param_name,
                                      family,
                                      annotation,
    ...
    if (!enabled_features.synchronization2 && sharing_mode == VK_SHARING_MODE_CONCURRENT) {
        if (src_queue_family != VK_QUEUE_FAMILY_IGNORED && src_queue_family != VK_QUEUE_FAMILY_EXTERNAL) {
            ...
            skip |= LogError(vuid,
                             ...
                             FormatHandle(resource_handle).c_str(),
                             string_VkSharingMode(sharing_mode),
                             src_queue_family,
                             src_annotation,
                             dst_queue_family,
    ...
    if (sharing_mode == VK_SHARING_MODE_EXCLUSIVE && IsOwnershipTransfer(barrier)) {
        if (src_queue_family != command_pool_queue_family && dst_queue_family != command_pool_queue_family) {
            const std::string vuid =
                GetBarrierQueueVUID(barrier_loc, sync_vuid_maps::QueueError::kSubmitQueueMustMatchSrcOrDst);
            const char* src_annotation = GetFamilyAnnotation(*this, src_queue_family);
            const char* dst_annotation = GetFamilyAnnotation(*this, dst_queue_family);
            skip |= LogError(vuid,
                             objects,
                             ...
                             ". Source or destination queue family must match queue family associated with the command "
                             "buffer's command pool.",
                             src_queue_family,
                             src_annotation,
                             dst_queue_family,
                             dst_annotation,
...
bool CoreChecks::ValidateBufferBarrier(const LogObjectList& objects,
                                       const Location& barrier_loc,
                                       const vvl::CommandBuffer& cb_state,
    ...
    skip |=
        ValidateQFOTransferBarrierUniqueness(barrier_loc, cb_state, mem_barrier, cb_state.qfo_transfer_buffer_barriers);
    ...
    if (auto buffer_state = Get<vvl::Buffer>(mem_barrier.buffer)) {
        ...
        skip |= ValidateBarrierQueueFamilies(objects,
                                             barrier_loc,
                                             buf_loc,
                                             mem_barrier,
                                             buffer_state->Handle(),
        ...
        if (mem_barrier.offset >= buffer_size) {
            ...
            skip |= LogError(vuid,
                             objects,
                             offset_loc,
                             "%s has offset 0x%" PRIx64 " which is not less than total size 0x%" PRIx64 ".",
                             FormatHandle(mem_barrier.buffer).c_str(),
                             HandleToUint64(mem_barrier.offset),
            ...
            skip |= LogError(vuid,
                             objects,
                             size_loc,
                             "%s has offset 0x%" PRIx64 " and size 0x%" PRIx64
                             " whose sum is greater than total size 0x%" PRIx64 ".",
                             FormatHandle(mem_barrier.buffer).c_str(),
                             HandleToUint64(mem_barrier.offset),
                             HandleToUint64(mem_barrier.size),
                             HandleToUint64(buffer_size));
        }
        if (mem_barrier.size == 0) {
            ...
            skip |=
                LogError(vuid, objects, barrier_loc, "%s has a size of 0.", FormatHandle(mem_barrier.buffer).c_str());
        }
    }
    return skip;
}

bool CoreChecks::ValidateImageBarrier(const LogObjectList& objects,
                                      const Location& barrier_loc,
                                      const vvl::CommandBuffer& cb_state,
    ...
    skip |=
        ValidateQFOTransferBarrierUniqueness(barrier_loc, cb_state, mem_barrier, cb_state.qfo_transfer_image_barriers);
    ...
    if (enabled_features.synchronization2) {
        ...
        if (old_layout == VK_IMAGE_LAYOUT_ATTACHMENT_OPTIMAL || old_layout == VK_IMAGE_LAYOUT_READ_ONLY_OPTIMAL) {
            const auto& vuid =
                sync_vuid_maps::GetImageBarrierVUID(barrier_loc, sync_vuid_maps::ImageError::kBadSync2OldLayout);
            skip |= LogError(vuid,
                             objects,
                             barrier_loc.dot(Field::oldLayout),
                             "is %s, but the synchronization2 feature was not enabled.",
        ...
        if (new_layout == VK_IMAGE_LAYOUT_ATTACHMENT_OPTIMAL || new_layout == VK_IMAGE_LAYOUT_READ_ONLY_OPTIMAL) {
            const auto& vuid =
                sync_vuid_maps::GetImageBarrierVUID(barrier_loc, sync_vuid_maps::ImageError::kBadSync2NewLayout);
            skip |= LogError(vuid,
                             objects,
                             barrier_loc.dot(Field::newLayout),
                             "is %s, but the synchronization2 feature was not enabled.",
    ...
    if (is_ilt) {
        if (new_layout == VK_IMAGE_LAYOUT_UNDEFINED || new_layout == VK_IMAGE_LAYOUT_PREINITIALIZED) {
            ...
            skip |=
                LogError(vuid, objects, barrier_loc.dot(Field::newLayout), "is %s.", string_VkImageLayout(new_layout));
        }
    }
    ...
    if (auto image_data = Get<vvl::Image>(mem_barrier.image)) {
        ...
        const auto& vuid_no_memory =
            sync_vuid_maps::GetImageBarrierVUID(barrier_loc, sync_vuid_maps::ImageError::kNoMemory);
        ...
        const auto& vuid_aspect =
            sync_vuid_maps::GetImageBarrierVUID(barrier_loc, sync_vuid_maps::ImageError::kAspectMask);
        skip |= ValidateImageAspectMask(image_data->VkHandle(),
                                        image_data->create_info.format,
                                        mem_barrier.subresourceRange.aspectMask,
                                        image_data->disjoint,
                                        image_loc,
                                        vuid_aspect.c_str());

        skip |= ValidateImageBarrierSubresourceRange(
            image_data->create_info, mem_barrier.subresourceRange, objects, barrier_loc.dot(Field::subresourceRange));
    }
    return skip;
}

bool CoreChecks::ValidateBarriers(const Location& outer_loc,
                                  const vvl::CommandBuffer& cb_state,
                                  VkPipelineStageFlags src_stage_mask,
                                  VkPipelineStageFlags dst_stage_mask,
...
bool CoreChecks::ValidateDependencyInfo(const LogObjectList& objects,
                                        const Location& dep_info_loc,
                                        const vvl::CommandBuffer& cb_state,
                                        const VkDependencyInfo& dep_info) const {
    bool skip = false;
...
bool CoreChecks::ValidatePipelineStageForShaderTileImage(const LogObjectList& objlist,
                                                         const Location& loc,
    ...
    if (HasNonFramebufferStagePipelineStageFlags(stage_mask)) {
        const auto& vuid = sync_vuid_maps::GetShaderTileImageVUID(
            loc, sync_vuid_maps::ShaderTileImageError::kShaderTileImageFramebufferSpace);

        skip |= LogError(vuid,
                         objlist,
                         loc,
                         "(%s) is restricted to framebuffer space stages (%s).",
                         sync_utils::StringPipelineStageFlags(stage_mask).c_str(),
    ...
    if (HasFramebufferStagePipelineStageFlags(stage_mask) && loc.field == Field::srcStageMask &&
        ...
        const auto& vuid = sync_vuid_maps::GetShaderTileImageVUID(
            loc, sync_vuid_maps::ShaderTileImageError::kShaderTileImageDependencyFlags);
...
bool CoreChecks::ValidateShaderTileImageBarriers(const LogObjectList& objlist,
                                                 const Location& outer_loc,
                                                 const VkDependencyInfo& dep_info) const {
    bool skip = false;

    skip |= ValidateShaderTileImageCommon(objlist,
                                          outer_loc,
    ...
    for (uint32_t i = 0; i < dep_info.memoryBarrierCount; ++i) {
        ...
        const auto& mem_barrier = dep_info.pMemoryBarriers[i];
        skip |= ValidatePipelineStageForShaderTileImage(
            objlist, loc.dot(Field::srcStageMask), mem_barrier.srcStageMask, dep_info.dependencyFlags);
        skip |= ValidatePipelineStageForShaderTileImage(
            objlist, loc.dot(Field::dstStageMask), mem_barrier.dstStageMask, dep_info.dependencyFlags);
    }
...
bool CoreChecks::ValidateShaderTileImageBarriers(const LogObjectList& objlist,
    ...
    skip |=
        ValidateShaderTileImageCommon(objlist, outer_loc, dependency_flags, buffer_barrier_count, image_barrier_count);
    skip |= ValidatePipelineStageForShaderTileImage(
        objlist, outer_loc.dot(Field::srcStageMask), src_stage_mask, dependency_flags);
    skip |= ValidatePipelineStageForShaderTileImage(
        objlist, outer_loc.dot(Field::dstStageMask), dst_stage_mask, dependency_flags);

    return skip;
}

bool CoreChecks::ValidateShaderTileImageCommon(const LogObjectList& objlist,
                                               const Location& outer_loc,
                                               VkDependencyFlags dependency_flags,
                                               uint32_t buffer_barrier_count,
    ...
    if (!features_enabled) {
        const auto& feature_error_vuid = sync_vuid_maps::GetShaderTileImageVUID(
            outer_loc, sync_vuid_maps::ShaderTileImageError::kShaderTileImageFeatureError);
        skip |= LogError(feature_error_vuid,
                         objlist,
                         outer_loc,
...
bool CoreChecks::ValidateMemoryBarrier(const LogObjectList& objects,
                                       const Location& barrier_loc,
                                       const vvl::CommandBuffer& cb_state,
    ...
    const bool is_sync2 =
        IsValueIn(barrier_loc.structure,
                  { Struct::VkMemoryBarrier2, Struct::VkBufferMemoryBarrier2, Struct::VkImageMemoryBarrier2 });

    // Validate Sync2 stages in this function because they are defined per barrier structure.
    // Sync1 stages are shared by all barriers (vkCmdPipelineBarrier api) and are validated once per barrier command
    // call.
    if (is_sync2) {
        if (ownership_transfer_op != OwnershipTransferOp::acquire) {
            skip |=
                ValidatePipelineStage(objects, barrier_loc.dot(Field::srcStageMask), queue_flags, barrier.srcStageMask);
        }
        if (ownership_transfer_op != OwnershipTransferOp::release) {
            skip |=
                ValidatePipelineStage(objects, barrier_loc.dot(Field::dstStageMask), queue_flags, barrier.dstStageMask);
        }
    }

    if (ownership_transfer_op != OwnershipTransferOp::acquire) {
        skip |= ValidateAccessMask(objects,
                                   barrier_loc.dot(Field::srcAccessMask),
                                   barrier_loc.dot(Field::srcStageMask),
                                   queue_flags,
    ...
    if (ownership_transfer_op != OwnershipTransferOp::release) {
        skip |= ValidateAccessMask(objects,
                                   barrier_loc.dot(Field::dstAccessMask),
                                   barrier_loc.dot(Field::dstStageMask),
                                   queue_flags,
    ...
    if (barrier_loc.function == Func::vkCmdSetEvent2 || barrier_loc.function == Func::vkCmdSetEvent2KHR) {
        if (barrier.srcStageMask == VK_PIPELINE_STAGE_2_HOST_BIT) {
            skip |= LogError("VUID-vkCmdSetEvent2-srcStageMask-09391",
                             objects,
                             barrier_loc.dot(Field::srcStageMask),
                             "is VK_PIPELINE_STAGE_2_HOST_BIT.");
        }
        if (barrier.dstStageMask == VK_PIPELINE_STAGE_2_HOST_BIT) {
            skip |= LogError("VUID-vkCmdSetEvent2-dstStageMask-09392",
                             objects,
                             barrier_loc.dot(Field::dstStageMask),
                             "is VK_PIPELINE_STAGE_2_HOST_BIT.");
        }
        ...
        if (barrier.srcStageMask == VK_PIPELINE_STAGE_2_HOST_BIT && cb_state.activeRenderPass) {
            skip |= LogError("VUID-vkCmdWaitEvents2-dependencyFlags-03844",
                             objects,
                             barrier_loc.dot(Field::srcStageMask),
