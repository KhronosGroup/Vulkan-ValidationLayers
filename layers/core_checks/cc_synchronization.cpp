...
bool CoreChecks::PreCallValidateSetEvent(VkDevice device, VkEvent event, ErrorObject &error_obj) const {
    bool skip = false;
    if (auto event_state = Get<vvl::Event>(event)) {
...
bool CoreChecks::PreCallValidateResetEvent(VkDevice device, VkEvent event, ErrorObject &error_obj) const {
    bool skip = false;
    if (auto event_state = Get<vvl::Event>(event)) {
        if (event_state->flags & VK_EVENT_CREATE_DEVICE_ONLY_BIT) {
...
bool CoreChecks::PreCallValidateGetEventStatus(VkDevice device, VkEvent event, ErrorObject &error_obj) const {
    bool skip = false;
    if (auto event_state = Get<vvl::Event>(event)) {
        if (event_state->flags & VK_EVENT_CREATE_DEVICE_ONLY_BIT) {
...
bool CoreChecks::ValidateSubpassDependency(ErrorObject &error_obj, const Location &in_loc,
    ...
    skip |=
        ValidatePipelineStage(error_obj.log.objlist, loc.dot(Field::srcStageMask), src_queue_flags, converted_barrier.srcStageMask);
    skip |= ValidateAccessMask(error_obj.log.objlist, loc.dot(Field::srcAccessMask), loc.dot(Field::srcStageMask), src_queue_flags,
    ...
    skip |=
        ValidatePipelineStage(error_obj.log.objlist, loc.dot(Field::dstStageMask), dst_queue_flags, converted_barrier.dstStageMask);
    skip |= ValidateAccessMask(error_obj.log.objlist, loc.dot(Field::dstAccessMask), loc.dot(Field::dstStageMask), dst_queue_flags,
