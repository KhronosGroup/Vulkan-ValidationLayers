...
bool CoreChecks::ValidateDependencyInfo(const LogObjectList &objects, const Location &dep_info_loc,
    ...
    if (const auto tensor_barrier = vku::FindStructInPNextChain<VkTensorMemoryBarrierARM>(dep_info.pNext)) {
        const Location barrier_loc = dep_info_loc.dot(Struct::VkTensorMemoryBarrierARM, Field::pNext);
        const TensorBarrier barrier(*tensor_barrier);
