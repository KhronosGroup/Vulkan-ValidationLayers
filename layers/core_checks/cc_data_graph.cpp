...
bool CoreChecks::PreCallValidateCreateDataGraphPipelinesARM(VkDevice device, VkDeferredOperationKHR deferredOperation,
    ...
    for (uint32_t i = 0; i < createInfoCount; i++) {
        ...
        if (auto* dg_pipeline_shader_module_ci = vku::FindStructInPNextChain<VkDataGraphPipelineShaderModuleCreateInfoARM>(create_info.pNext)) {
                        ...
                        if (!tensor_desc) {
                            ...
                            if (sparsity->dimension >= tensor_desc->dimensionCount) {
                                skip |= LogError(
                                    ...
                                    sparsity->dimension, tensor_desc->dimensionCount);
                            } else {
                                if (tensor_desc->pDimensions[sparsity->dimension] % sparsity->groupSize != 0) {
                                    skip |= LogError(
                                        "VUID-VkDataGraphPipelineConstantARM-pNext-09777", device,
                                        constant_loc.pNext(Struct::VkDataGraphPipelineConstantTensorSemiStructuredSparsityInfoARM,
            ...
            if (auto* shader_module_ci = vku::FindStructInPNextChain<VkShaderModuleCreateInfo>(create_info.pNext)) {
                if (dg_pipeline_shader_module_ci->module != VK_NULL_HANDLE) {
                    skip |= LogError("VUID-VkDataGraphPipelineShaderModuleCreateInfoARM-pNext-09873", device,
                                     create_info_loc.dot(Field::module),
                                     "(%p) is not NULL but the pNext chain of VkDataGraphPipelineCreateInfoARM includes a "
                ...
                skip |= LogError("VUID-VkDataGraphPipelineShaderModuleCreateInfoARM-pNext-09874", device,
                                 create_info_loc.dot(Field::module),
        ...
        if (auto* pipeline_feedback = vku::FindStructInPNextChain<VkPipelineCreationFeedbackCreateInfo>(create_info.pNext)) {
        ...
        for (uint32_t j = 0; j < create_info.resourceInfoCount; j++) {
            ...
            if (resource.arrayElement != 0) {
                skip |= LogError("VUID-VkDataGraphPipelineResourceInfoARM-arrayElement-09779", device,
                                 create_info_loc.dot(Field::arrayElement), "(%" PRIu32 ") not 0", resource.arrayElement);
            }
            ...
            if (!tensor_desc) {
                skip |= LogError("VUID-VkDataGraphPipelineResourceInfoARM-descriptorSet-09851", device,
                                 create_info_loc.pNext(Struct::VkTensorDescriptionARM),
                                 "not found for resource with descriptorSet %" PRIu32 " and binding %" PRIu32,
                                 resource.descriptorSet, resource.binding);
            } else if ((tensor_desc->usage & VK_TENSOR_USAGE_DATA_GRAPH_BIT_ARM) == 0) {
                skip |=
                    LogError("VUID-VkDataGraphPipelineResourceInfoARM-descriptorSet-09851", device,
                             create_info_loc.pNext(Struct::VkTensorDescriptionARM).dot(Field::usage),
                             "(%s) invalid for resource with descriptorSet %" PRIu32 " and binding %" PRIu32,
...
bool CoreChecks::PreCallValidateGetDataGraphPipelinePropertiesARM(VkDevice device, const VkDataGraphPipelineInfoARM* pPipelineInfo,
    ...
    if (VK_STRUCTURE_TYPE_DATA_GRAPH_PIPELINE_CREATE_INFO_ARM != pipeline_ptr->GetCreateInfoSType()) {
        skip |= LogError("VUID-VkDataGraphPipelineInfoARM-dataGraphPipeline-09803", device, pipeline_loc,
...
bool CoreChecks::PreCallValidateCreateDataGraphPipelineSessionARM(VkDevice device,
    ...
    const Location pipeline_loc = error_obj.location.dot(Field::dataGraphPipeline);
    if (VK_STRUCTURE_TYPE_DATA_GRAPH_PIPELINE_CREATE_INFO_ARM != pipeline_ptr->GetCreateInfoSType()) {
        skip |= LogError("VUID-VkDataGraphPipelineSessionCreateInfoARM-dataGraphPipeline-09781", device, pipeline_loc,
                         "was not created with vkCreateDataGraphPipelinesARM. The create info structure type was %s",
                         string_VkStructureType(pipeline_ptr->GetCreateInfoSType()));
    ...
    if ((pCreateInfo->flags & VK_DATA_GRAPH_PIPELINE_SESSION_CREATE_PROTECTED_BIT_ARM) != 0) {
        if (!enabled_features.protectedMemory) {
            skip |= LogError(
                "VUID-VkDataGraphPipelineSessionCreateInfoARM-protectedMemory-09782", device, pipeline_loc.dot(Field::flags),
...
bool CoreChecks::PreCallValidateCmdDispatchDataGraphARM(VkCommandBuffer commandBuffer, VkDataGraphPipelineSessionARM session,
    ...
    for (auto& bpr : session_state.BindPointReqs()) {
        ...
        if (bpr.numObjects != n_bound) {
            skip |= LogError(vuid, objlist, loc, " %zu objects bound at bind point %s, required numObjects %u; session %s.",
                             n_bound, string_VkDataGraphPipelineSessionBindPointARM(bpr.bindPoint), bpr.numObjects,
