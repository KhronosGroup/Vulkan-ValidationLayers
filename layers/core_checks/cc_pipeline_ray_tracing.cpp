                                            ...
                                            const Location&                                       create_info_loc) const
{
    ...
    if (isKHR)
    {
        ...
        {
            skip |= LogError("VUID-VkRayTracingPipelineCreateInfoKHR-maxPipelineRayRecursionDepth-03589",
                             device,
        ...
        if (create_info.pLibraryInfo)
        {
            for (uint32_t i = 0; i < create_info.pLibraryInfo->libraryCount; ++i)
            {
                ...
                {
                    skip |= LogError("VUID-VkRayTracingPipelineCreateInfoKHR-pLibraries-03591",
                                     device,
                                     library_loc,
                                     "was created with maxPipelineRayRecursionDepth (%" PRIu32
                                     ") which is not equal %s (%" PRIu32 ") .",
                                     library_create_info.maxPipelineRayRecursionDepth,
                                     create_info_loc.dot(Field::maxPipelineRayRecursionDepth).Fields().c_str(),
                                     create_info.maxPipelineRayRecursionDepth);
                }
                if (library_create_info.pLibraryInfo &&
                ...
                {
                    skip |= LogError("VUID-VkRayTracingPipelineCreateInfoKHR-pLibraryInfo-03593",
                                     device,
                                     library_loc,
                                     "was created with maxPipelineRayPayloadSize (%" PRIu32 ") and "
                                     "maxPipelineRayHitAttributeSize (%" PRIu32
                                     ") which is not equal to %s values of (%" PRIu32 ") and (%" PRIu32 ").",
                                     library_create_info.pLibraryInterface->maxPipelineRayPayloadSize,
                                     library_create_info.pLibraryInterface->maxPipelineRayHitAttributeSize,
                ...
                if ((flags & VK_PIPELINE_CREATE_RAY_TRACING_SHADER_GROUP_HANDLE_CAPTURE_REPLAY_BIT_KHR) &&
                    !(library_create_info.flags &
                      VK_PIPELINE_CREATE_RAY_TRACING_SHADER_GROUP_HANDLE_CAPTURE_REPLAY_BIT_KHR))
                {
                    skip |= LogError("VUID-VkRayTracingPipelineCreateInfoKHR-flags-03594",
                                     device,
                                     library_loc,
    ...
    else
    {
        ...
        {
            skip |= LogError("VUID-VkRayTracingPipelineCreateInfoNV-maxRecursionDepth-03457",
                             device,
                             create_info_loc.dot(Field::maxRecursionDepth),
                             "(%" PRIu32 ") must be less than or equal to "
                             "maxRecursionDepth (%" PRIu32 ")",
                             create_info.maxRecursionDepth,
    ...
    if (const auto* pipeline_robustness_info =
            vku::FindStructInPNextChain<VkPipelineRobustnessCreateInfo>(create_info.pNext))
    {
    ...
    if ((create_info.flags & VK_PIPELINE_CREATE_LIBRARY_BIT_KHR) == 0)
    {
        ...
        {
            skip |=
                LogError(isKHR ? "VUID-VkRayTracingPipelineCreateInfoKHR-stage-03425"
    ...
    {
        skip |= LogError("VUID-VkRayTracingPipelineCreateInfoKHR-flags-06546",
                         device,
                         create_info_loc.dot(Field::flags),
                         "is %s (contains both SKIP_TRIANGLES and SKIP_AABBS).",
                         string_VkPipelineCreateFlags(flags).c_str());
    }
    ...
    for (uint32_t group_index = 0; group_index < create_info.groupCount; group_index++)
    {
        ...
        {
            if (!GroupHasValidIndex(pipeline,
                                    group.generalShader,
                                        ...
                                        VK_SHADER_STAGE_CALLABLE_BIT_NV))
            {
                skip |= LogError(isKHR ? "VUID-VkRayTracingShaderGroupCreateInfoKHR-type-03474"
                                 ...
                                 device,
                                 group_loc.dot(Field::generalShader),
                                 "is %" PRIu32 ".",
                                 group.generalShader);
            }
                ...
                group.intersectionShader != VK_SHADER_UNUSED_NV)
            {
                skip |= LogError(isKHR ? "VUID-VkRayTracingShaderGroupCreateInfoKHR-type-03475"
                                 ...
                                 device,
                                 group_loc,
                                 "anyHitShader is %" PRIu32 ", closestHitShader is %" PRIu32
                                 ", intersectionShader is %" PRIu32 ".",
                                 group.anyHitShader,
        ...
        else if (group.type == VK_RAY_TRACING_SHADER_GROUP_TYPE_PROCEDURAL_HIT_GROUP_NV)
        {
            if (!GroupHasValidIndex(pipeline, group.intersectionShader, VK_SHADER_STAGE_INTERSECTION_BIT_NV))
            {
                skip |= LogError(isKHR ? "VUID-VkRayTracingShaderGroupCreateInfoKHR-type-03476"
                                 ...
                                 device,
                                 group_loc.dot(Field::intersectionShader),
                                 "is %" PRIu32 ".",
                                 group.intersectionShader);
            }
        }
            ...
            group.type == VK_RAY_TRACING_SHADER_GROUP_TYPE_TRIANGLES_HIT_GROUP_NV)
        {
            if (!GroupHasValidIndex(pipeline, group.anyHitShader, VK_SHADER_STAGE_ANY_HIT_BIT_KHR))
            {
                skip |= LogError(isKHR ? "VUID-VkRayTracingShaderGroupCreateInfoKHR-anyHitShader-03479"
                                 ...
                                 device,
                                 group_loc.dot(Field::anyHitShader),
                                 "is %" PRIu32 ".",
                                 group.anyHitShader);
            }
            if (!GroupHasValidIndex(pipeline, group.closestHitShader, VK_SHADER_STAGE_CLOSEST_HIT_BIT_KHR))
            {
                skip |= LogError(isKHR ? "VUID-VkRayTracingShaderGroupCreateInfoKHR-closestHitShader-03478"
                                 ...
                                 device,
                                 group_loc.dot(Field::closestHitShader),
                                 "is %" PRIu32 ".",
                                 group.closestHitShader);
            }
        }
...
bool CoreChecks::PreCallValidateCreateRayTracingPipelinesNV(VkDevice                                device,
...
{
    bool skip = BaseClass::PreCallValidateCreateRayTracingPipelinesNV(
    ...
    for (uint32_t i = 0; i < count; i++)
    {
        ...
        if (pipeline->create_flags & VK_PIPELINE_CREATE_DERIVATIVE_BIT)
        {
            ...
            {
                skip |= LogError("VUID-vkCreateRayTracingPipelinesNV-flags-03416",
                                 device,
                                 create_info_loc,
        ...
        for (const auto& stage_ci : pipeline->shader_stages_ci)
        {
            skip |= ValidatePipelineShaderStage(
                *pipeline, stage_ci, pCreateInfos[i].pNext, create_info_loc.dot(Field::pStages, stage_index++));
        }
        skip |= ValidatePipelineCacheControlFlags(
...
bool CoreChecks::PreCallValidateCreateRayTracingPipelinesKHR(VkDevice                                 device,
...
{
    bool skip = BaseClass::PreCallValidateCreateRayTracingPipelinesKHR(device,
                                                                       deferredOperation,
                                                                       pipelineCache,
                                                                       count,
                                                                       pCreateInfos,
    ...
    for (uint32_t i = 0; i < count; i++)
    {
        ...
        if (pipeline->create_flags & VK_PIPELINE_CREATE_DERIVATIVE_BIT)
        {
            ...
            if (!base_pipeline || !(base_pipeline->create_flags & VK_PIPELINE_CREATE_ALLOW_DERIVATIVES_BIT))
            {
                skip |= LogError("VUID-vkCreateRayTracingPipelinesKHR-flags-03416",
                                 device,
                                 create_info_loc,
                                 "If the flags member of any element of pCreateInfos contains the "
                                 "VK_PIPELINE_CREATE_DERIVATIVE_BIT flag,"
        ...
        if (create_info.pLibraryInfo)
        {
            ...
            for (uint32_t j = 0; j < create_info.pLibraryInfo->libraryCount; ++j)
            {
                ...
                {
                    skip |= LogError("VUID-VkPipelineLibraryCreateInfoKHR-pLibraries-03381",
                                     device,
                                     library_loc,
                                     "was created with %s.",
                    ...
                    if (pipeline->create_flags & flag)
                    {
                        if ((lib->create_flags & flag) == 0)
                        {
                            skip |= LogError(vuid,
                                             ...
                                             string_VkPipelineCreateFlags2(lib->create_flags).c_str(),
                                             string_VkPipelineCreateFlags2(flag).c_str(),
                                             create_info_loc.dot(Field::flags).Fields().c_str(),
                ...
                {
                    skip |= LogError("VUID-VkPipelineLibraryCreateInfoKHR-pLibraries-08096",
                                     device,
                                     library_loc,
                                     "%s created with VK_PIPELINE_CREATE_DESCRIPTOR_BUFFER_BIT_EXT which is opposite "
                                     "of pLibraries[0].",
