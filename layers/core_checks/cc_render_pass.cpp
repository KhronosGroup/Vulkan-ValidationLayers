...
bool CoreChecks::ValidateAttachmentCompatibility(const VulkanTypedHandle& rp1_object,
                                                 const vvl::RenderPass& rp1_state,
                                                 const VulkanTypedHandle& rp2_object,
                                                 const vvl::RenderPass& rp2_state,
                                                 uint32_t primary_attachment,
                                                 uint32_t secondary_attachment,
                                                 const Location& caller_loc,
    ...
    if (primary_attachment == VK_ATTACHMENT_UNUSED) {
        ...
        skip |= LogError(vuid,
                         ...
                         attachment_loc.Fields().c_str(),
                         FormatHandle(rp1_state).c_str(),
                         FormatHandle(rp1_object).c_str(),
                         FormatHandle(rp2_state).c_str(),
    ...
    if (primary_pass_ci.pAttachments[primary_attachment].format !=
        secondary_pass_ci.pAttachments[secondary_attachment].format) {
        const LogObjectList objlist(rp1_object, rp1_state.Handle(), rp2_object, rp2_state.Handle());
        skip |= LogError(vuid,
                         objlist,
                         caller_loc,
                         ...
                         FormatHandle(rp2_object).c_str(),
                         primary_attachment,
                         string_VkFormat(primary_pass_ci.pAttachments[primary_attachment].format),
                         secondary_attachment,
                         string_VkFormat(secondary_pass_ci.pAttachments[secondary_attachment].format));
    }
    if (primary_pass_ci.pAttachments[primary_attachment].samples !=
        secondary_pass_ci.pAttachments[secondary_attachment].samples) {
        const LogObjectList objlist(rp1_object, rp1_state.Handle(), rp2_object, rp2_state.Handle());
        skip |= LogError(vuid,
                         objlist,
                         caller_loc,
                         ...
                         FormatHandle(rp2_object).c_str(),
                         primary_attachment,
                         string_VkSampleCountFlagBits(primary_pass_ci.pAttachments[primary_attachment].samples),
                         secondary_attachment,
                         string_VkSampleCountFlagBits(secondary_pass_ci.pAttachments[secondary_attachment].samples));
    }
    if (primary_pass_ci.pAttachments[primary_attachment].flags !=
        secondary_pass_ci.pAttachments[secondary_attachment].flags) {
        const LogObjectList objlist(rp1_object, rp1_state.Handle(), rp2_object, rp2_state.Handle());
        skip |= LogError(
            vuid,
            objlist,
...
bool CoreChecks::ValidateSubpassCompatibility(const VulkanTypedHandle& rp1_object,
                                              const vvl::RenderPass& rp1_state,
                                              const VulkanTypedHandle& rp2_object,
                                              const vvl::RenderPass& rp2_state,
                                              const int subpass,
    ...
    uint32_t max_input_attachment_count =
        std::max(primary_desc.inputAttachmentCount, secondary_desc.inputAttachmentCount);
    for (uint32_t i = 0; i < max_input_attachment_count; ++i) {
        ...
        skip |= ValidateAttachmentCompatibility(rp1_object,
                                                rp1_state,
                                                rp2_object,
                                                rp2_state,
                                                primary_input_attach,
                                                secondary_input_attach,
                                                loc,
                                                subpass_loc.dot(Field::pInputAttachments, i).dot(Field::attachment),
                                                vuid);
    }
    uint32_t max_color_attachment_count =
        std::max(primary_desc.colorAttachmentCount, secondary_desc.colorAttachmentCount);
    for (uint32_t i = 0; i < max_color_attachment_count; ++i) {
        ...
        if (rp1_state.create_info.subpassCount > 1) {
            ...
            skip |=
                ValidateAttachmentCompatibility(rp1_object,
                                                rp1_state,
                                                rp2_object,
                                                rp2_state,
                                                primary_resolve_attach,
    ...
    if (primary_desc.flags != secondary_desc.flags) {
        ...
        skip |= LogError(vuid,
                         ...
                         "%s is incompatible between %s (from %s) and %s (from %s), "
                         "%s != %s",
                         subpass_loc.dot(Field::flags).Fields().c_str(),
                         FormatHandle(rp1_state).c_str(),
                         FormatHandle(rp1_object).c_str(),
                         FormatHandle(rp2_state).c_str(),
                         FormatHandle(rp2_object).c_str(),
    ...
    if (primary_desc.viewMask && secondary_desc.viewMask) {
        if (primary_desc.viewMask != secondary_desc.viewMask) {
            ...
            skip |= LogError(vuid,
                             ...
                             "%s is incompatible between %s (from %s) and %s (from %s), "
                             "%" PRIu32 " != %" PRIu32 "",
                             subpass_loc.dot(Field::viewMask).Fields().c_str(),
                             FormatHandle(rp1_state).c_str(),
                             FormatHandle(rp1_object).c_str(),
                             FormatHandle(rp2_state).c_str(),
                             FormatHandle(rp2_object).c_str(),
                             primary_desc.viewMask,
                             secondary_desc.viewMask);
        }
    ...
    if (fsr1 && fsr2) {
        if ((fsr1->shadingRateAttachmentTexelSize.width != fsr2->shadingRateAttachmentTexelSize.width) ||
            ...
            skip |= LogError(
                ...
                FormatHandle(rp2_state).c_str(),
                FormatHandle(rp2_object).c_str(),
                string_VkExtent2D(fsr1->shadingRateAttachmentTexelSize).c_str(),
...
bool CoreChecks::ValidateDependencyCompatibility(const VulkanTypedHandle& rp1_object,
                                                 const vvl::RenderPass& rp1_state,
                                                 const VulkanTypedHandle& rp2_object,
                                                 const vvl::RenderPass& rp2_state,
    ...
    if (const auto primary_barrier = vku::FindStructInPNextChain<VkMemoryBarrier2>(rp1_state.create_info.pNext);
        primary_barrier) {
    ...
    if (primary_dep.srcSubpass != secondary_dep.srcSubpass) {
        ...
        skip |= LogError(vuid,
                         objlist,
                         loc,
                         "pDependencies[%" PRIu32 "].srcSubpass is incompatible between %s (from %s) and %s (from %s), "
                         "%" PRIu32 " != %" PRIu32 ".",
                         dependency,
                         FormatHandle(rp1_state).c_str(),
                         FormatHandle(rp1_object).c_str(),
                         FormatHandle(rp2_state).c_str(),
                         FormatHandle(rp2_object).c_str(),
                         primary_dep.srcSubpass,
                         secondary_dep.srcSubpass);
    }
    if (primary_dep.dstSubpass != secondary_dep.dstSubpass) {
        ...
        skip |= LogError(vuid,
                         objlist,
                         loc,
                         "pDependencies[%" PRIu32 "].dstSubpass is incompatible between %s (from %s) and %s (from %s), "
                         "%" PRIu32 " != %" PRIu32 ".",
                         dependency,
                         FormatHandle(rp1_state).c_str(),
                         FormatHandle(rp1_object).c_str(),
                         FormatHandle(rp2_state).c_str(),
                         FormatHandle(rp2_object).c_str(),
                         primary_dep.dstSubpass,
                         secondary_dep.dstSubpass);
    }
    if (primary_src_stage_mask != secondary_src_stage_mask) {
        ...
        skip |=
            LogError(vuid,
                     objlist,
                     loc,
                     "pDependencies[%" PRIu32 "].srcStageMask is incompatible between %s (from %s) and %s (from %s), "
                     "%s != %s.",
                     dependency,
                     FormatHandle(rp1_state).c_str(),
                     FormatHandle(rp1_object).c_str(),
                     FormatHandle(rp2_state).c_str(),
                     FormatHandle(rp2_object).c_str(),
                     string_VkPipelineStageFlags2(primary_src_stage_mask).c_str(),
    ...
    if (primary_dst_stage_mask != secondary_dst_stage_mask) {
        ...
        skip |=
            LogError(vuid,
                     objlist,
                     loc,
                     "pDependencies[%" PRIu32 "].dstStageMask is incompatible between %s (from %s) and %s (from %s), "
                     "%s != %s.",
                     dependency,
                     FormatHandle(rp1_state).c_str(),
                     FormatHandle(rp1_object).c_str(),
                     FormatHandle(rp2_state).c_str(),
                     FormatHandle(rp2_object).c_str(),
                     string_VkPipelineStageFlags2(primary_dst_stage_mask).c_str(),
    ...
    if (primary_src_access_mask != secondary_src_access_mask) {
        ...
        skip |=
            LogError(vuid,
                     objlist,
                     loc,
                     "pDependencies[%" PRIu32 "].srcAccessMask is incompatible between %s (from %s) and %s (from %s), "
                     "%s != %s.",
                     dependency,
                     FormatHandle(rp1_state).c_str(),
                     FormatHandle(rp1_object).c_str(),
                     FormatHandle(rp2_state).c_str(),
                     FormatHandle(rp2_object).c_str(),
                     string_VkAccessFlags2(primary_src_access_mask).c_str(),
    ...
    if (primary_dst_access_mask != secondary_dst_access_mask) {
        ...
        skip |=
            LogError(vuid,
                     objlist,
                     loc,
                     "pDependencies[%" PRIu32 "].dstAccessMask is incompatible between %s (from %s) and %s (from %s), "
                     "%s != %s.",
                     dependency,
                     FormatHandle(rp1_state).c_str(),
                     FormatHandle(rp1_object).c_str(),
                     FormatHandle(rp2_state).c_str(),
                     FormatHandle(rp2_object).c_str(),
                     string_VkAccessFlags2(primary_dst_access_mask).c_str(),
    ...
    if (primary_dep.dependencyFlags != secondary_dep.dependencyFlags) {
        ...
        skip |= LogError(vuid,
                         ...
                         FormatHandle(rp2_state).c_str(),
                         FormatHandle(rp2_object).c_str(),
                         string_VkDependencyFlags(primary_dep.dependencyFlags).c_str(),
    ...
    if (primary_dep.viewOffset != secondary_dep.viewOffset) {
        ...
        skip |= LogError(vuid,
                         objlist,
                         loc,
                         "pDependencies[%" PRIu32 "].viewOffset is incompatible between %s (from %s) and %s (from %s), "
                         "%" PRIu32 " != %" PRIu32 ".",
                         dependency,
                         FormatHandle(rp1_state).c_str(),
                         FormatHandle(rp1_object).c_str(),
                         FormatHandle(rp2_state).c_str(),
                         FormatHandle(rp2_object).c_str(),
                         primary_dep.viewOffset,
                         secondary_dep.viewOffset);
    }
...
//  This function deals directly with the CreateInfo, there are overloaded versions below that can take the renderPass
//  handle and will then feed into this function
bool CoreChecks::ValidateRenderPassCompatibility(const VulkanTypedHandle& rp1_object,
                                                 const vvl::RenderPass& rp1_state,
                                                 const VulkanTypedHandle& rp2_object,
                                                 const vvl::RenderPass& rp2_state,
                                                 const Location& loc,
    ...
    if (rp1_state.create_info.flags != rp2_state.create_info.flags) {
        ...
        skip |= LogError(vuid,
                         ...
                         FormatHandle(rp2_state).c_str(),
                         FormatHandle(rp2_object).c_str(),
                         string_VkRenderPassCreateFlags(rp1_state.create_info.flags).c_str(),
    ...
    if (rp1_state.create_info.subpassCount != rp2_state.create_info.subpassCount) {
        ...
        skip |= LogError(vuid,
                         ...
                         FormatHandle(rp2_state).c_str(),
                         FormatHandle(rp2_object).c_str(),
                         rp1_state.create_info.subpassCount,
                         rp2_state.create_info.subpassCount);
    } else {
    ...
    if (rp1_state.create_info.dependencyCount != rp2_state.create_info.dependencyCount) {
        ...
        skip |= LogError(vuid,
                         ...
                         FormatHandle(rp2_state).c_str(),
                         FormatHandle(rp2_object).c_str(),
                         rp1_state.create_info.dependencyCount,
    ...
    if (rp1_state.create_info.correlatedViewMaskCount != rp2_state.create_info.correlatedViewMaskCount) {
        ...
        skip |= LogError(vuid,
                         ...
                         FormatHandle(rp2_state).c_str(),
                         FormatHandle(rp2_object).c_str(),
                         rp1_state.create_info.correlatedViewMaskCount,
            ...
            if (rp1_state.create_info.pCorrelatedViewMasks[i] != rp2_state.create_info.pCorrelatedViewMasks[i]) {
                ...
                skip |=
                    LogError(vuid,
                             objlist,
                             loc,
                             "pCorrelatedViewMasks[%" PRIu32 "] is incompatible between %s (from %s) and %s (from %s), "
                             "%" PRIu32 " != %" PRIu32 "",
                             i,
                             FormatHandle(rp1_state).c_str(),
                             FormatHandle(rp1_object).c_str(),
                             FormatHandle(rp2_state).c_str(),
                             FormatHandle(rp2_object).c_str(),
                             rp1_state.create_info.pCorrelatedViewMasks[i],
                             rp2_state.create_info.pCorrelatedViewMasks[i]);
            }
        }
    }
    ...
    const auto fdm1 =
        vku::FindStructInPNextChain<VkRenderPassFragmentDensityMapCreateInfoEXT>(rp1_state.create_info.pNext);
    const auto fdm2 =
        vku::FindStructInPNextChain<VkRenderPassFragmentDensityMapCreateInfoEXT>(rp2_state.create_info.pNext);
...
bool CoreChecks::PreCallValidateDestroyRenderPass(VkDevice device,
...
// If this is a stencil format, make sure the stencil[Load|Store]Op flag is checked, while if it is a depth/color
// attachment the [load|store]Op flag must be checked
// TODO: The memory valid flag in vvl::DeviceMemory should probably be split to track the validity of stencil memory
// separately.
template <typename T>
...
bool CoreChecks::ValidateCmdBeginRenderPass(VkCommandBuffer commandBuffer,
    ...
    if (sample_locations_begin_info) {
        for (uint32_t i = 0; i < sample_locations_begin_info->attachmentInitialSampleLocationsCount; ++i) {
            const Location sampler_loc = rp_begin_loc.pNext(
                Struct::VkRenderPassSampleLocationsBeginInfoEXT, Field::pAttachmentInitialSampleLocations, i);
            const VkAttachmentSampleLocationsEXT& sample_location =
                sample_locations_begin_info->pAttachmentInitialSampleLocations[i];
            skip |= ValidateSampleLocationsInfo(sample_location.sampleLocationsInfo,
                                                sampler_loc.dot(Field::sampleLocationsInfo));
            if (sample_location.attachmentIndex >= rp_state->create_info.attachmentCount) {
                ...
                skip |= LogError("VUID-VkAttachmentSampleLocationsEXT-attachmentIndex-01531",
                                 objlist,
                                 sampler_loc.dot(Field::attachmentIndex),
                                 "(%" PRIu32 ") is greater than the attachment count of %" PRIu32
                                 " for the render pass being begun.",
                                 sample_location.attachmentIndex,
                                 rp_state->create_info.attachmentCount);
            }
        }

        for (uint32_t i = 0; i < sample_locations_begin_info->postSubpassSampleLocationsCount; ++i) {
            const Location sampler_loc = rp_begin_loc.pNext(
                Struct::VkRenderPassSampleLocationsBeginInfoEXT, Field::pPostSubpassSampleLocations, i);
            ...
            if (sample_location.subpassIndex >= rp_state->create_info.subpassCount) {
                const LogObjectList objlist(commandBuffer, pRenderPassBegin->renderPass);
                skip |= LogError("VUID-VkSubpassSampleLocationsEXT-subpassIndex-01532",
                                 objlist,
                                 sampler_loc.dot(Field::subpassIndex),
                                 "(%" PRIu32 ") is greater than the subpass count of %" PRIu32
                                 " for the render pass being begun.",
                                 sample_location.subpassIndex,
                                 rp_state->create_info.subpassCount);
            }
        }
    }
    ...
    for (uint32_t i = 0; i < rp_state->create_info.attachmentCount; ++i) {
        ...
        if (FormatSpecificLoadAndStoreOpSettings(
                attachment->format, attachment->loadOp, attachment->stencilLoadOp, VK_ATTACHMENT_LOAD_OP_CLEAR)) {
            clear_op_size = static_cast<uint32_t>(i) + 1;

            if (vkuFormatHasDepth(attachment->format) && pRenderPassBegin->pClearValues) {
                skip |=
                    ValidateClearDepthStencilValue(commandBuffer,
                                                   pRenderPassBegin->pClearValues[i].depthStencil,
    ...
    if (clear_op_size > pRenderPassBegin->clearValueCount) {
        const LogObjectList objlist(commandBuffer, pRenderPassBegin->renderPass);
        skip |= LogError("VUID-VkRenderPassBeginInfo-clearValueCount-00902",
                         objlist,
                         rp_begin_loc.dot(Field::clearValueCount),
                         ...
                         "VK_ATTACHMENT_LOAD_OP_CLEAR is %" PRIu32 ". Note that the pClearValues array is indexed by "
    ...
    if (fb_state->rp_state->VkHandle() != rp_state->VkHandle()) {
        skip |= ValidateRenderPassCompatibility(rp_state->Handle(),
                                                *rp_state,
                                                fb_state->Handle(),
                                                *fb_state->rp_state,
                                                error_obj.location,
    ...
    auto device_group_begin_info =
        vku::FindStructInPNextChain<VkDeviceGroupRenderPassBeginInfo>(pRenderPassBegin->pNext);
    const bool non_zero_device_render_area =
        device_group_begin_info && device_group_begin_info->deviceRenderAreaCount != 0;
    if (device_group_begin_info) {
        const LogObjectList objlist(commandBuffer, pRenderPassBegin->renderPass);
        const Location device_mask_loc =
            rp_begin_loc.pNext(Struct::VkDeviceGroupRenderPassBeginInfo, Field::deviceMask);
        skip |= ValidateDeviceMaskToPhysicalDeviceCount(device_group_begin_info->deviceMask,
                                                        objlist,
                                                        device_mask_loc,
                                                        "VUID-VkDeviceGroupRenderPassBeginInfo-deviceMask-00905");
        skip |= ValidateDeviceMaskToZero(device_group_begin_info->deviceMask,
                                         objlist,
                                         device_mask_loc,
                                         "VUID-VkDeviceGroupRenderPassBeginInfo-deviceMask-00906");
        skip |= ValidateDeviceMaskToCommandBuffer(cb_state,
                                                  device_group_begin_info->deviceMask,
                                                  objlist,
                                                  device_mask_loc,
        ...
        if (device_group_begin_info->deviceRenderAreaCount != 0 &&
            ...
            skip |= LogError("VUID-VkDeviceGroupRenderPassBeginInfo-deviceRenderAreaCount-00908",
                             objlist,
                             ...
                             device_group_begin_info->deviceRenderAreaCount,
    ...
    if (!non_zero_device_render_area) {
        if (pRenderPassBegin->renderArea.extent.width == 0) {
            skip |= LogError("VUID-VkRenderPassBeginInfo-None-08996",
                             commandBuffer,
                             rp_begin_loc.dot(Field::renderArea).dot(Field::extent).dot(Field::width),
                             "is zero.");
        } else if (pRenderPassBegin->renderArea.extent.height == 0) {
            skip |= LogError("VUID-VkRenderPassBeginInfo-None-08997",
                             commandBuffer,
                             rp_begin_loc.dot(Field::renderArea).dot(Field::extent).dot(Field::height),
                             "is zero.");
        }
    }

    if (contents == VK_SUBPASS_CONTENTS_INLINE_AND_SECONDARY_COMMAND_BUFFERS_KHR &&
        !enabled_features.nestedCommandBuffer && !enabled_features.maintenance7) {
        const char* vuid = error_obj.location.function == Func::vkCmdBeginRenderPass
                               ? "VUID-vkCmdBeginRenderPass-contents-09640"
                               : "VUID-VkSubpassBeginInfo-contents-09382";
        skip |= LogError(vuid,
                         commandBuffer,
                         error_obj.location.dot(Field::contents),
...
bool CoreChecks::PreCallValidateCmdBeginRenderPass(VkCommandBuffer commandBuffer,
                                                   const VkRenderPassBeginInfo* pRenderPassBegin,
                                                   VkSubpassContents contents,
                                                   const ErrorObject& error_obj) const {
...
bool CoreChecks::PreCallValidateCmdBeginRenderPass2KHR(VkCommandBuffer commandBuffer,
                                                       const VkRenderPassBeginInfo* pRenderPassBegin,
...
bool CoreChecks::PreCallValidateCmdBeginRenderPass2(VkCommandBuffer commandBuffer,
                                                    const VkRenderPassBeginInfo* pRenderPassBegin,
                                                    const VkSubpassBeginInfo* pSubpassBeginInfo,
                                                    const ErrorObject& error_obj) const {
...
void CoreChecks::RecordCmdBeginRenderPassLayouts(VkCommandBuffer commandBuffer,
                                                 const VkRenderPassBeginInfo* pRenderPassBegin,
...
void CoreChecks::PreCallRecordCmdBeginRenderPass(VkCommandBuffer commandBuffer,
                                                 const VkRenderPassBeginInfo* pRenderPassBegin,
                                                 VkSubpassContents contents,
...
void CoreChecks::PreCallRecordCmdBeginRenderPass2KHR(VkCommandBuffer commandBuffer,
                                                     const VkRenderPassBeginInfo* pRenderPassBegin,
                                                     const VkSubpassBeginInfo* pSubpassBeginInfo,
                                                     const RecordObject& record_obj) {
...
void CoreChecks::PreCallRecordCmdBeginRenderPass2(VkCommandBuffer commandBuffer,
                                                  const VkRenderPassBeginInfo* pRenderPassBegin,
                                                  const VkSubpassBeginInfo* pSubpassBeginInfo,
                                                  const RecordObject& record_obj) {
...
bool CoreChecks::ValidateCmdEndRenderPass(VkCommandBuffer commandBuffer,
    ...
    if (pSubpassEndInfo && pSubpassEndInfo->pNext) {
        ...
        if (fdm_offset_info && fdm_offset_info->fragmentDensityOffsetCount != 0) {
            if ((!enabled_features.fragmentDensityMapOffset) || (!enabled_features.fragmentDensityMap)) {
                const LogObjectList objlist(commandBuffer, rp_state.Handle());
                skip |= LogError("VUID-VkSubpassFragmentDensityMapOffsetEndInfoQCOM-fragmentDensityMapOffsets-06503",
                                 objlist,
            ...
            for (uint32_t k = 0; k < fdm_offset_info->fragmentDensityOffsetCount; k++) {
                if ((fdm_offset_info->pFragmentDensityOffsets[k].x != 0) ||
                    ...
                    const Location offset_loc = error_obj.location.pNext(
                    ...
                    if (SafeModulo(fdm_offset_info->pFragmentDensityOffsets[k].x, width) != 0) {
                        ...
                        skip |= LogError(
                            ...
                            objlist,
                            offset_loc.dot(Field::x),
                            "(%" PRIu32
                            ") is not an integer multiple of fragmentDensityOffsetGranularity.width (%" PRIu32 ").",
                            fdm_offset_info->pFragmentDensityOffsets[k].x,
                            width);
                    }
                    ...
                    if (SafeModulo(fdm_offset_info->pFragmentDensityOffsets[k].y, height) != 0) {
                        ...
                        skip |= LogError(
                            ...
                            objlist,
                            offset_loc.dot(Field::y),
                            "(%" PRIu32
                            ") is not an integer multiple of fragmentDensityOffsetGranularity.height (%" PRIu32 ").",
                            fdm_offset_info->pFragmentDensityOffsets[k].y,
                            height);
                    }
                }
            }
            ...
            for (uint32_t i = 0; i < rpci->attachmentCount; ++i) {
                ...
                if ((fdm_non_zero_offsets == true) &&
                    ...
                    skip |= LogError("VUID-VkFramebufferCreateInfo-renderPass-06502",
                                     objlist,
                                     error_obj.location,
                                     "pAttachments[%" PRIu32 "] is not created with flag value"
                    ...
                    if (fdm_attachment->fragmentDensityMapAttachment.attachment == i) {
                        if ((ici.flags & VK_IMAGE_CREATE_FRAGMENT_DENSITY_MAP_OFFSET_BIT_QCOM) == 0) {
                            ...
                            skip |= LogError(
                                ...
                                objlist,
                                error_obj.location,
                                "Fragment density map attachment %" PRIu32 " is not created with"
                ...
                if (subpass.pDepthStencilAttachment &&
                    ...
                    const LogObjectList objlist(commandBuffer, rp_state.Handle(), view_state->Handle());
                    skip |=
                        LogError("VUID-VkSubpassFragmentDensityMapOffsetEndInfoQCOM-pDepthStencilAttachment-06505",
                                 objlist,
                                 error_obj.location,
                                 "pDepthStencilAttachment[%" PRIu32 "] is not created with"
                                 " VK_IMAGE_CREATE_FRAGMENT_DENSITY_MAP_OFFSET_BIT_QCOM and fragmentDensityOffsetCount"
                                 " is %" PRIu32 " but must be 0 due to missing fragmentDensityOffset feature bit.",
                                 i,
                                 fdm_offset_info->fragmentDensityOffsetCount);
                ...
                for (uint32_t k = 0; k < subpass.inputAttachmentCount; k++) {
                    ...
                    if ((attachment != VK_ATTACHMENT_UNUSED) && (attachment == i) &&
                        ...
                        skip |= LogError(
                            ...
                            objlist,
                            error_obj.location,
                            "pInputAttachments[%" PRIu32 "] is not created with"
                            " VK_IMAGE_CREATE_FRAGMENT_DENSITY_MAP_OFFSET_BIT_QCOM and fragmentDensityOffsetCount"
                            " is %" PRIu32 " but must be 0 due to missing fragmentDensityOffset feature bit.",
                            i,
                            fdm_offset_info->fragmentDensityOffsetCount);
                ...
                for (uint32_t k = 0; k < subpass.colorAttachmentCount; k++) {
                    ...
                    if ((attachment != VK_ATTACHMENT_UNUSED) && (attachment == i) &&
                        ...
                        skip |= LogError(
                            ...
                            objlist,
                            error_obj.location,
                            "pColorAttachments[%" PRIu32 "] is not created with"
                            " VK_IMAGE_CREATE_FRAGMENT_DENSITY_MAP_OFFSET_BIT_QCOM and fragmentDensityOffsetCount"
                            " is %" PRIu32 " but must be 0 due to missing fragmentDensityOffset feature bit.",
                            i,
                            fdm_offset_info->fragmentDensityOffsetCount);
                    ...
                    for (uint32_t k = 0; k < subpass.colorAttachmentCount; k++) {
                        ...
                        if ((attachment != VK_ATTACHMENT_UNUSED) && (attachment == i) &&
                            ...
                            skip |= LogError(
                                ...
                                objlist,
                                error_obj.location,
                                "pResolveAttachments[%" PRIu32 "] is not created with"
                                " VK_IMAGE_CREATE_FRAGMENT_DENSITY_MAP_OFFSET_BIT_QCOM and fragmentDensityOffsetCount"
                                " is %" PRIu32 " but must be 0 due to missing fragmentDensityOffset feature bit.",
                                i,
                                fdm_offset_info->fragmentDensityOffsetCount);
                ...
                for (uint32_t k = 0; k < subpass.preserveAttachmentCount; k++) {
                    ...
                    if ((attachment != VK_ATTACHMENT_UNUSED) && (attachment == i) &&
                        ...
                        skip |= LogError(
                            ...
                            objlist,
                            error_obj.location,
                            "pPreserveAttachments[%" PRIu32 "] is not created with"
                            " VK_IMAGE_CREATE_FRAGMENT_DENSITY_MAP_OFFSET_BIT_QCOM and fragmentDensityOffsetCount"
                            " is %" PRIu32 " but must be 0 due to missing fragmentDensityOffset feature bit.",
                            i,
                            fdm_offset_info->fragmentDensityOffsetCount);
    ...
    for (const auto& query : cb_state.renderPassQueries) {
        ...
        skip |= LogError(vuid,
                         objlist,
                         error_obj.location,
                         "query %" PRIu32 " from %s was began in subpass %" PRIu32 " but never ended.",
                         query.slot,
...
bool CoreChecks::PreCallValidateCmdEndRenderPass2(VkCommandBuffer commandBuffer,
                                                  const VkSubpassEndInfo* pSubpassEndInfo,
                                                  const ErrorObject& error_obj) const {
...
void CoreChecks::PostCallRecordCmdEndRenderPass2KHR(VkCommandBuffer commandBuffer,
                                                    const VkSubpassEndInfo* pSubpassEndInfo,
                                                    const RecordObject& record_obj) {
    PostCallRecordCmdEndRenderPass2(commandBuffer, pSubpassEndInfo, record_obj);
}

void CoreChecks::PostCallRecordCmdEndRenderPass2(VkCommandBuffer commandBuffer,
                                                 const VkSubpassEndInfo* pSubpassEndInfo,
                                                 const RecordObject& record_obj) {
    RecordCmdEndRenderPassLayouts(commandBuffer);
    ...
    if (device_group_area_count > 0) {
        for (uint32_t i = 0; i < device_group_area_count; ++i) {
            ...
            if (deviceRenderArea.offset.x < 0) {
                skip |= LogError("VUID-VkDeviceGroupRenderPassBeginInfo-offset-06166",
                                 begin_info.renderPass,
                                 render_area_loc.dot(Field::offset).dot(Field::x),
                                 "is negative (%" PRId32 ").",
                                 deviceRenderArea.offset.x);
            }
            if (deviceRenderArea.offset.y < 0) {
                skip |= LogError("VUID-VkDeviceGroupRenderPassBeginInfo-offset-06167",
                                 begin_info.renderPass,
                                 render_area_loc.dot(Field::offset).dot(Field::y),
                                 "is negative (%" PRId32 ").",
            ...
            if ((deviceRenderArea.offset.x + deviceRenderArea.extent.width) > framebuffer_info->width) {
                ...
                skip |= LogError("VUID-VkRenderPassBeginInfo-pNext-02856",
                                 objlist,
                                 render_area_loc,
                                 "offset.x (%" PRId32 ") + extent.width (%" PRId32
                                 ") is greater than framebuffer width (%" PRId32 ").",
                                 deviceRenderArea.offset.x,
                                 deviceRenderArea.extent.width,
                                 framebuffer_info->width);
            }
            if ((deviceRenderArea.offset.y + deviceRenderArea.extent.height) > framebuffer_info->height) {
                const LogObjectList objlist(begin_info.renderPass, framebuffer_state->Handle());
                skip |= LogError("VUID-VkRenderPassBeginInfo-pNext-02857",
                                 objlist,
                                 render_area_loc,
                                 "offset.y (%" PRId32 ") + extent.height (%" PRId32
                                 ") is greater than framebuffer height (%" PRId32 ").",
                                 deviceRenderArea.offset.y,
                                 deviceRenderArea.extent.height,
        ...
        if (begin_info.renderArea.offset.x < 0) {
            skip |= LogError("VUID-VkRenderPassBeginInfo-pNext-02850",
                             begin_info.renderPass,
                             render_area_loc.dot(Field::offset).dot(Field::x),
                             "is %" PRId32 " (offset can't be negative).",
                             begin_info.renderArea.offset.x);
        }
        if (begin_info.renderArea.offset.y < 0) {
            skip |= LogError("VUID-VkRenderPassBeginInfo-pNext-02851",
                             begin_info.renderPass,
                             render_area_loc.dot(Field::offset).dot(Field::y),
                             "is %" PRId32 " (offset can't be negative).",
        ...
        const auto x_adjusted_extent = static_cast<int64_t>(begin_info.renderArea.offset.x) +
                                       static_cast<int64_t>(begin_info.renderArea.extent.width);
        if (x_adjusted_extent > static_cast<int64_t>(framebuffer_info->width)) {
            const LogObjectList objlist(begin_info.renderPass, framebuffer_state->Handle());
            skip |= LogError("VUID-VkRenderPassBeginInfo-pNext-02852",
                             objlist,
                             render_area_loc,
                             "offset.x (%" PRId32 ") + extent.width (%" PRId32
                             ") is greater than framebuffer width (%" PRId32 ").",
        ...
        const auto y_adjusted_extent = static_cast<int64_t>(begin_info.renderArea.offset.y) +
                                       static_cast<int64_t>(begin_info.renderArea.extent.height);
        if (y_adjusted_extent > static_cast<int64_t>(framebuffer_info->height)) {
            const LogObjectList objlist(begin_info.renderPass, framebuffer_state->Handle());
            skip |= LogError("VUID-VkRenderPassBeginInfo-pNext-02853",
                             objlist,
                             render_area_loc,
                             "offset.y (%" PRId32 ") + extent.height (%" PRId32
                             ") is greater than framebuffer height (%" PRId32 ").",
                             begin_info.renderArea.offset.y,
                             begin_info.renderArea.extent.height,
                             framebuffer_info->height);
        }
    }
    return skip;
...
bool CoreChecks::VerifyFramebufferAndRenderPassImageViews(const VkRenderPassBeginInfo& begin_info,
    ...
    bool skip = false;
    const auto* render_pass_attachment_begin_info =
        vku::FindStructInPNextChain<VkRenderPassAttachmentBeginInfo>(begin_info.pNext);
    ...
    if ((framebuffer_create_info.flags & VK_FRAMEBUFFER_CREATE_IMAGELESS_BIT) == 0) {
        ...
        skip |= LogError("VUID-VkRenderPassBeginInfo-framebuffer-03207",
                         objlist,
                         begin_info_loc.pNext(Struct::VkRenderPassAttachmentBeginInfo, Field::attachmentCount),
                         "is %" PRIu32
                         ", but the VkFramebuffer create flags (%s) has no VK_FRAMEBUFFER_CREATE_IMAGELESS_BIT.",
    ...
    if (framebuffer_attachments_create_info->attachmentImageInfoCount !=
        render_pass_attachment_begin_info->attachmentCount) {
        const LogObjectList objlist(begin_info.renderPass, begin_info.framebuffer);
        skip |= LogError("VUID-VkRenderPassBeginInfo-framebuffer-03208",
                         objlist,
                         begin_info_loc.pNext(Struct::VkRenderPassAttachmentBeginInfo, Field::attachmentCount),
                         "is %" PRIu32 ", but VkFramebuffer was created with "
                         "VkFramebufferAttachmentsCreateInfo::attachmentImageInfoCount = %" PRIu32 ".",
                         render_pass_attachment_begin_info->attachmentCount,
    ...
    for (uint32_t i = 0; i < render_pass_attachment_begin_info->attachmentCount; ++i) {
        const Location attachment_loc =
            begin_info_loc.pNext(Struct::VkRenderPassAttachmentBeginInfo, Field::pAttachments, i);
        ...
        const LogObjectList objlist(begin_info.renderPass,
        ...
        if (framebuffer_attachment_image_info->flags != image_create_info->flags) {
            skip |= LogError("VUID-VkRenderPassBeginInfo-framebuffer-03209",
                             objlist,
                             attachment_loc.dot(Field::flags),
        ...
        if (framebuffer_attachment_image_info->usage != image_view_state->inherited_usage) {
            ...
            if (image_create_info->usage == image_view_state->inherited_usage) {
                skip |= LogError("VUID-VkRenderPassBeginInfo-framebuffer-04627",
                                 objlist,
                                 attachment_loc.dot(Field::usage),
                                 "is (%s), but the VkFramebuffer was created with "
        ...
        if (framebuffer_attachment_image_info->width != view_width) {
            skip |= LogError("VUID-VkRenderPassBeginInfo-framebuffer-03211",
                             objlist,
                             attachment_loc,
        ...
        if (framebuffer_attachment_image_info->height != view_height) {
            skip |= LogError("VUID-VkRenderPassBeginInfo-framebuffer-03212",
                             objlist,
                             attachment_loc,
        ...
        if (framebuffer_attachment_image_info->layerCount != layerCount) {
            skip |= LogError("VUID-VkRenderPassBeginInfo-framebuffer-03213",
                             objlist,
                             attachment_loc,
                             "has a subresource range with a layerCount of %" PRIu32
                             ", but VkFramebufferAttachmentsCreateInfo::pAttachments[%" PRIu32
                             "].layerCount is %" PRIu32 ".",
                             layerCount,
                             i,
            ...
            for (uint32_t j = 0; j < image_format_list_create_info->viewFormatCount; ++j) {
                ...
                if (!format_found) {
                    skip |= LogError(
                        ...
                        objlist,
                        attachment_loc,
                        "internal VkImage was created with VkImageFormatListCreateInfo::pViewFormats[%" PRIu32 "] = %s,"
                        "but now found in vkFramebufferAttachmentsCreateInfo::pAttachmentImageInfos[%" PRIu32
                        "].pViewFormats.",
                        j,
                        string_VkFormat(image_format_list_create_info->pViewFormats[j]),
                        i);
                }
            }
        }
        ...
        if (render_pass_create_info->pAttachments[i].format != image_view_create_info->format) {
            skip |= LogError("VUID-VkRenderPassBeginInfo-framebuffer-03216",
                             objlist,
                             attachment_loc.dot(Field::format),
                             "is %s, but the VkRenderPass was created with a pAttachments[%" PRIu32 "].format of %s.",
                             string_VkFormat(image_view_create_info->format),
                             i,
            ...
            const uint64_t attachment_external_format =
                GetExternalFormat(render_pass_create_info->pAttachments[i].pNext);
            if (image_view_state->image_state->ahb_format != attachment_external_format) {
                skip |= LogError("VUID-VkRenderPassBeginInfo-framebuffer-09354",
                                 objlist,
                                 attachment_loc,
                                 "externalFormat is %" PRIu64
                                 ", but the VkRenderPass was created with a pAttachments[%" PRIu32
                                 "] with externalFormat of %" PRIu64 ".",
                                 image_view_state->image_state->ahb_format,
                                 i,
                                 attachment_external_format);
            }
        }
        ...
        if (attachment_samples != image_create_info->samples && !single_sample_enabled) {
            skip |= LogError("VUID-VkRenderPassBeginInfo-framebuffer-09047",
                             objlist,
                             attachment_loc,
                             "internal VkImage was created with %s samples, "
                             "but the VkRenderPass was created with a pAttachments[%" PRIu32 "].samples of %s.",
                             string_VkSampleCountFlagBits(image_create_info->samples),
                             i,
                             string_VkSampleCountFlagBits(render_pass_create_info->pAttachments[i].samples));
        }

        if (subresource_range.levelCount != 1) {
            skip |= LogError("VUID-VkRenderPassAttachmentBeginInfo-pAttachments-03218",
                             objlist,
                             attachment_loc,
                             "was created with multiple mip levels (%" PRIu32 ").",
        ...
        if (image_view_create_info->viewType == VK_IMAGE_VIEW_TYPE_3D) {
            skip |= LogError("VUID-VkRenderPassAttachmentBeginInfo-pAttachments-04114",
                             objlist,
                             attachment_loc,
    ...
    if (enabled_features.externalFormatResolve && !android_external_format_resolve_null_color_attachment_prop) {
        for (const auto [i, subpass] :
             vvl::enumerate(render_pass_create_info->pSubpasses, render_pass_create_info->subpassCount)) {
...
bool CoreChecks::ValidateAttachmentIndex(uint32_t attachment, uint32_t attachment_count, const Location& loc) const {
    ...
    if (attachment >= attachment_count) {
        ...
        skip |= LogError(vuid,
                         device,
                         loc.dot(Field::attachment),
                         "is %" PRIu32 ", but must be less than the total number of attachments (%" PRIu32 ").",
                         attachment,
                         attachment_count);
    }
    return skip;
...
bool CoreChecks::AddAttachmentUse(std::vector<uint8_t>& attachment_uses,
                                  std::vector<VkImageLayout>& attachment_layouts,
                                  uint32_t attachment,
                                  uint8_t new_use,
                                  VkImageLayout new_layout,
                                  const Location loc) const {
    if (attachment >= attachment_uses.size())
        return false; /* out of range, but already reported */
    ...
    if (uses & new_use) {
        if (attachment_layouts[attachment] != new_layout) {
            ...
            skip |= LogError(vuid,
                             device,
                             loc,
                             "already uses attachment %" PRIu32 " with a different image layout (%s vs %s).",
                             attachment,
                             string_VkImageLayout(attachment_layouts[attachment]),
        ...
        skip |= LogError(vuid,
                         device,
                         loc,
                         "uses attachment %" PRIu32 " as both %s and %s attachment.",
                         attachment,
                         StringAttachmentType(uses),
...
bool CoreChecks::ValidateAttachmentReference(VkAttachmentReference2 reference,
        ...
        case VK_IMAGE_LAYOUT_STENCIL_READ_ONLY_OPTIMAL:
            ...
            if (!enabled_features.separateDepthStencilLayouts) {
                ...
                skip |= LogError(vuid,
                                 device,
                                 loc,
                                 "is %s (and separateDepthStencilLayouts was not enabled).",
                ...
                if (attachment_reference_stencil_layout) {
                    ...
                    if (stencil_layout == VK_IMAGE_LAYOUT_UNDEFINED ||
                        ...
                        skip |= LogError("VUID-VkAttachmentReferenceStencilLayout-stencilLayout-03318",
                                         device,
                                         loc.pNext(Struct::VkAttachmentReferenceStencilLayout, Field::stencilLayout),
                                         "(%s) is not a valid VkImageLayout.",
                                         string_VkImageLayout(stencil_layout));
                    }
                }
            }
            break;
        ...
        case VK_IMAGE_LAYOUT_READ_ONLY_OPTIMAL:
            if (!enabled_features.synchronization2) {
                ...
                skip |= LogError(vuid,
                                 device,
                                 loc,
                                 "is %s (and synchronization2 was not enabled).",
                                 string_VkImageLayout(reference.layout));
            }
            break;
...
bool CoreChecks::ValidateRenderpassAttachmentUsage(const VkRenderPassCreateInfo2* pCreateInfo,
                                                   const ErrorObject& error_obj) const {
    bool skip = false;
    ...
    for (uint32_t i = 0; i < pCreateInfo->subpassCount; ++i) {
        ...
        if (subpass.pipelineBindPoint != VK_PIPELINE_BIND_POINT_GRAPHICS &&
            ...
            skip |= LogError(vuid,
                             device,
                             subpass_loc.dot(Field::pipelineBindPoint),
                             "is %s.",
        ...
        for (uint32_t j = 0; j < subpass.inputAttachmentCount; ++j) {
            ...
            if (aspect_mask & VK_IMAGE_ASPECT_METADATA_BIT) {
                ...
                skip |= LogError(vuid,
                                 device,
                                 input_loc.dot(Field::aspectMask),
                                 "is %s.",
            ...
            if (IsValueIn(
                ...
                const char* vuid = use_rp2 ? "VUID-VkSubpassDescription2-attachment-06912"
            ...
            if (IsValueIn(attachment_layout,
                ...
                const char* vuid = use_rp2 ? "VUID-VkSubpassDescription2-attachment-06918"
            ...
            if (attachment_layout == VK_IMAGE_LAYOUT_ATTACHMENT_OPTIMAL) {
                const char* vuid = use_rp2 ? "VUID-VkSubpassDescription2-attachment-06921"
            ...
            if (fragment_density_map_info) {
                ...
                if ((fdm_attachment_index != VK_ATTACHMENT_UNUSED) && (attachment_index == fdm_attachment_index)) {
                    skip |=
                        LogError("VUID-VkRenderPassFragmentDensityMapCreateInfoEXT-fragmentDensityMapAttachment-02548",
                                 device,
                                 input_loc,
            ...
            if (attachment_index < pCreateInfo->attachmentCount) {
                ...
                skip |= AddAttachmentUse(attachment_uses,
                                         attachment_layouts,
                                         attachment_index,
                                         ATTACHMENT_INPUT,
                ...
                {
                    const char* vuid = use_rp2 ? "VUID-VkRenderPassCreateInfo2-attachment-02525"
                                               : "VUID-VkRenderPassCreateInfo-pNext-01963";
                    // Assuming no disjoint image since there's no handle
                    skip |= ValidateImageAspectMask(
                        VK_NULL_HANDLE, attachment_format, aspect_mask, false, error_obj.location, vuid);
                }

                if (attach_first_use[attachment_index]) {
                    skip |= ValidateLayoutVsAttachmentDescription(subpass.pInputAttachments[j].layout,
                                                                  attachment_index,
                    ...
                    if (!used_as_depth && !used_as_color &&
                        ...
                        const char* vuid = use_rp2 ? "VUID-VkSubpassDescription2-loadOp-03064"
                                                   : "VUID-VkSubpassDescription-loadOp-00846";
                        skip |= LogError(
                            vuid, device, attachment_loc.dot(Field::loadOp), "is VK_ATTACHMENT_LOAD_OP_CLEAR.");
                    }
                }
            ...
            if (use_rp2) {
                ...
                // Check for 0
                if (aspect_mask == 0) {
                    skip |= LogError("VUID-VkSubpassDescription2-attachment-02800",
                                     device,
                                     input_loc.dot(Field::aspectMask),
                                     "is zero.");
                } else {
                    ...
                    if (aspect_mask & ~valid_bits) {
                        skip |= LogError("VUID-VkSubpassDescription2-attachment-02799",
                                         device,
                                         input_loc.dot(Field::aspectMask),
                                         "(%s) is invalid.",
                                         string_VkImageAspectFlags(aspect_mask).c_str());
        ...
        for (uint32_t j = 0; j < subpass.preserveAttachmentCount; ++j) {
            ...
            if (attachment == VK_ATTACHMENT_UNUSED) {
                const char* vuid = use_rp2 ? "VUID-VkSubpassDescription2-attachment-03073"
                ...
                if (fragment_density_map_info) {
                    const uint32_t fdm_attachment_index =
        ...
        for (uint32_t j = 0; j < subpass.colorAttachmentCount; ++j) {
            ...
            if (IsValueIn(
                ...
                const char* vuid = use_rp2 ? "VUID-VkSubpassDescription2-attachment-06914"
                                           : "VUID-VkSubpassDescription-attachment-06914";
                skip |= LogError(vuid,
                                 device,
                                 resolve_loc.dot(Field::layout),
                                 "(%s) is invalid.",
                                 string_VkImageLayout(attachment_layout));
            }
            if (IsValueIn(attachment_layout,
                ...
                const char* vuid = use_rp2 ? "VUID-VkSubpassDescription2-attachment-06917"
            ...
            if (IsValueIn(attachment_layout,
                ...
                const char* vuid = use_rp2 ? "VUID-VkSubpassDescription2-attachment-06920"
            ...
            if (attachment_layout == VK_IMAGE_LAYOUT_READ_ONLY_OPTIMAL) {
                const char* vuid = use_rp2 ? "VUID-VkSubpassDescription2-attachment-06923"
            ...
            if (fragment_density_map_info) {
                const auto& fdm_attachment_index = fragment_density_map_info->fragmentDensityMapAttachment.attachment;
                if ((fdm_attachment_index != VK_ATTACHMENT_UNUSED) &&
                    (attachment_ref.attachment == fdm_attachment_index)) {
                    skip |=
                        LogError("VUID-VkRenderPassFragmentDensityMapCreateInfoEXT-fragmentDensityMapAttachment-02548",
                                 device,
                                 resolve_loc,
            ...
            if (attachment_ref.attachment < pCreateInfo->attachmentCount) {
                ...
                if (pCreateInfo->pAttachments[attachment_ref.attachment].samples != VK_SAMPLE_COUNT_1_BIT) {
                    ...
                    skip |= LogError(
                        vuid,
                        device,
                        attachment_loc.dot(Field::samples),
                        "is %s (referenced by %s).",
                ...
                if ((subpass.flags & VK_SUBPASS_DESCRIPTION_SHADER_RESOLVE_BIT_QCOM) != 0) {
                    ...
                    skip |= LogError(vuid,
                                     device,
                                     resolve_loc,
                                     "contains a reference to attachment %" PRIu32
                                     " instead of being VK_ATTACHMENT_UNUSED.",
                                     attachment_ref.attachment);
                }
            }
        }
        ...
        if (subpass.pDepthStencilAttachment && subpass.pDepthStencilAttachment->attachment != VK_ATTACHMENT_UNUSED) {
            ...
            if (IsValueIn(attachment_layout,
                ...
                const char* vuid = use_rp2 ? "VUID-VkSubpassDescription2-attachment-06915"
            ...
            if (use_rp2 &&
                IsValueIn(attachment_layout,
                          { VK_IMAGE_LAYOUT_STENCIL_ATTACHMENT_OPTIMAL, VK_IMAGE_LAYOUT_STENCIL_READ_ONLY_OPTIMAL })) {
                if (vku::FindStructInPNextChain<VkAttachmentReferenceStencilLayout>(
            ...
            if (attachment < pCreateInfo->attachmentCount) {
                ...
                skip |= AddAttachmentUse(
                    attachment_uses, attachment_layouts, attachment, ATTACHMENT_DEPTH, image_layout, ds_loc);

                if (attach_first_use[attachment]) {
                    skip |= ValidateLayoutVsAttachmentDescription(
                        image_layout, attachment, depth_stencil_attachment, ds_loc.dot(Field::layout));
                }
                    ...
                    if ((depth_stencil_sample_count == VK_SAMPLE_COUNT_1_BIT) &&
                        (!subpass_depth_stencil_resolve ||
                         (subpass_depth_stencil_resolve->pDepthStencilResolveAttachment != VK_NULL_HANDLE &&
                          subpass_depth_stencil_resolve->pDepthStencilResolveAttachment->attachment !=
                        ...
                        if (!subpass_depth_stencil_resolve) {
                            ...
                            message
                                << "but the pSubpassResolveAttachment member of the "
                        ...
                        skip |= LogError(
                            "VUID-VkSubpassDescription2-pNext-06871", device, ds_loc, "%s", message.str().c_str());
                    }
                    if (subpass_depth_stencil_resolve) {
                        if (subpass_depth_stencil_resolve->depthResolveMode == VK_RESOLVE_MODE_NONE &&
                            ...
                            skip |= LogError("VUID-VkSubpassDescriptionDepthStencilResolve-pNext-06873",
                                             device,
                                             subpass_loc.pNext(Struct::VkSubpassDescriptionDepthStencilResolve,
                                                               Field::depthResolveMode),
                                             "and stencilResolveMode are VK_RESOLVE_MODE_NONE, and "
                                             "VkMultisampledRenderToSingleSampledInfoEXT::"
                                             "multisampledRenderToSingleSampled was set to "
                                             "VK_TRUE.");
                        }
                        if (vkuFormatHasDepth(attachment_format)) {
                            if (subpass_depth_stencil_resolve->depthResolveMode != VK_RESOLVE_MODE_NONE &&
                                ...
                                skip |= LogError(
                                    ...
                                    "VkMultisampledRenderToSingleSampledInfoEXT::multisampledRenderToSingleSampled was "
                                    "set to "
                                    "VK_TRUE.",
                                    ...
                                    attachment_loc.dot(Field::format).Fields().c_str(),
                                    string_VkFormat(attachment_format));
                            }
                        }
                        if (vkuFormatHasStencil(attachment_format)) {
                            if (subpass_depth_stencil_resolve->stencilResolveMode != VK_RESOLVE_MODE_NONE &&
                                ...
                                skip |= LogError(
                                    ...
                                    string_VkResolveModeFlags(phys_dev_props_core12.supportedStencilResolveModes)
                                        .c_str(),
                                    attachment_loc.dot(Field::format).Fields().c_str(),
                                    string_VkFormat(attachment_format));
                            }
                ...
                if (IsImageLayoutDepthOnly(subpass.pDepthStencilAttachment->layout)) {
                    if (vku::FindStructInPNextChain<VkAttachmentReferenceStencilLayout>(
                            subpass.pDepthStencilAttachment->pNext) == nullptr) {
                        if (vkuFormatIsDepthAndStencil(attachment_format)) {
                            skip |= LogError("VUID-VkRenderPassCreateInfo2-attachment-06244",
                                             device,
                                             attachment_loc.dot(Field::format),
                    ...
                    if (vkuFormatIsStencilOnly(attachment_format)) {
                        skip |= LogError("VUID-VkRenderPassCreateInfo2-attachment-06246",
                                         device,
                                         attachment_loc.dot(Field::format),
                ...
                if (IsImageLayoutStencilOnly(subpass.pDepthStencilAttachment->layout)) {
                    if (!vkuFormatIsStencilOnly(attachment_format)) {
                        skip |= LogError("VUID-VkRenderPassCreateInfo2-attachment-06245",
                                         device,
                                         attachment_loc.dot(Field::format),
        ...
        for (uint32_t j = 0; j < subpass.colorAttachmentCount; ++j) {
            ...
            if (attachment_index != VK_ATTACHMENT_UNUSED) {
                ...
                if (IsValueIn(attachment_layout,
                    ...
                    const char* vuid = use_rp2 ? "VUID-VkSubpassDescription2-attachment-06913"
                ...
                if (IsValueIn(attachment_layout,
                              { VK_IMAGE_LAYOUT_DEPTH_ATTACHMENT_STENCIL_READ_ONLY_OPTIMAL,
                                VK_IMAGE_LAYOUT_DEPTH_READ_ONLY_STENCIL_ATTACHMENT_OPTIMAL })) {
                    const char* vuid = use_rp2 ? "VUID-VkSubpassDescription2-attachment-06916"
                ...
                if (IsValueIn(attachment_layout,
                              { VK_IMAGE_LAYOUT_DEPTH_ATTACHMENT_OPTIMAL,
                                ...
                                VK_IMAGE_LAYOUT_STENCIL_ATTACHMENT_OPTIMAL,
                                VK_IMAGE_LAYOUT_STENCIL_READ_ONLY_OPTIMAL })) {
                    const char* vuid = use_rp2 ? "VUID-VkSubpassDescription2-attachment-06919"
                ...
                if (attachment_layout == VK_IMAGE_LAYOUT_READ_ONLY_OPTIMAL) {
                    const char* vuid = use_rp2 ? "VUID-VkSubpassDescription2-attachment-06922"
                ...
                if (fragment_density_map_info) {
                    const auto& fdm_attachment_index =
                        fragment_density_map_info->fragmentDensityMapAttachment.attachment;
                    if ((fdm_attachment_index != VK_ATTACHMENT_UNUSED) && (attachment_index == fdm_attachment_index)) {
                        skip |= LogError(
                            "VUID-VkRenderPassFragmentDensityMapCreateInfoEXT-fragmentDensityMapAttachment-02548",
                ...
                if (attachment_index < pCreateInfo->attachmentCount) {
                    ...
                    if (use_rp2 && enabled_features.multisampledRenderToSingleSampled && ms_render_to_single_sample &&
                        ...
                        if (current_sample_count != VK_SAMPLE_COUNT_1_BIT &&
                            ...
                            skip |=
                                LogError("VUID-VkSubpassDescription2-pNext-06870",
                                         device,
                    ...
                    if (last_sample_count_attachment != VK_ATTACHMENT_UNUSED) {
                        if (!(IsExtEnabled(device_extensions.vk_amd_mixed_attachment_samples) ||
                            ...
                            VkSampleCountFlagBits last_sample_count =
                                pCreateInfo
                                    ->pAttachments[subpass.pColorAttachments[last_sample_count_attachment].attachment]
                                    .samples;
                            if (current_sample_count != last_sample_count) {
                                const char* vuid =
                                    use_rp2 ? "VUID-VkSubpassDescription2-multisampledRenderToSingleSampled-06872"
                    ...
                    if (subpass.pDepthStencilAttachment &&
                        ...
                        if (IsExtEnabled(device_extensions.vk_amd_mixed_attachment_samples)) {
                            if (current_sample_count > depth_stencil_sample_count) {
                                const char* vuid = use_rp2 ? "VUID-VkSubpassDescription2-None-09456"
                    ...
                    if (attach_first_use[attachment_index]) {
                        skip |= ValidateLayoutVsAttachmentDescription(subpass.pColorAttachments[j].layout,
                                                                      attachment_index,
                                                                      attachment_description,
                                                                      color_loc.dot(Field::layout));
                    }
                ...
                if (enabled_features.externalFormatResolve && resolve_desc.format == VK_FORMAT_UNDEFINED) {
                    if (attachment_index == VK_ATTACHMENT_UNUSED) {
                        ...
                        if (color_desc.samples != VK_SAMPLE_COUNT_1_BIT) {
                            skip |=
                                LogError("VUID-VkSubpassDescription2-externalFormatResolve-09345",
                                         device,
                                         create_info_loc.dot(Field::pAttachments, attachment_index).dot(Field::samples),
                                         "is %s (referenced by %s).",
                                         string_VkSampleCountFlagBits(color_desc.samples),
                                         color_loc.Fields().c_str());
                        }
                    }

                    if (subpass.colorAttachmentCount != 1) {
                        skip |= LogError("VUID-VkSubpassDescription2-externalFormatResolve-09344",
                                         device,
                                         resolve_loc.dot(Field::attachment),
                                         "is %" PRIu32 ", pAttachments[%" PRIu32
                    ...
                    if (subpass.viewMask != 0) {
                        skip |= LogError("VUID-VkSubpassDescription2-externalFormatResolve-09346",
                                         device,
                                         resolve_loc.dot(Field::attachment),
                                         "is %" PRIu32 ", pAttachments[%" PRIu32
                                         "].format is VK_FORMAT_UNDEFINED, but viewMask is %" PRIu32 ".",
                                         resolve_attachment_index,
                                         resolve_attachment_index,
                                         subpass.viewMask);
                    }
                    ...
                    for (uint32_t k = 0; k < subpass.inputAttachmentCount; ++k) {
                        ...
                        if (input_attachment_index == resolve_attachment_index &&
                            ...
                            skip |= LogError("VUID-VkSubpassDescription2-externalFormatResolve-09348",
                                             device,
                                             resolve_loc.dot(Field::attachment),
                        ...
                        } else if (input_attachment_index == attachment_index &&
                                   IsAnyPlaneAspect(attachment_ref.aspectMask)) {
                            skip |= LogError("VUID-VkSubpassDescription2-externalFormatResolve-09348",
                                             device,
                                             ...
                                             attachment_index,
                                             k,
                                             string_VkImageAspectFlags(attachment_ref.aspectMask).c_str());
                        }
                    }
                    ...
                    if (fragment_shading_rate_info && fragment_shading_rate_info->pFragmentShadingRateAttachment &&
                        fragment_shading_rate_info->pFragmentShadingRateAttachment->attachment !=
                            VK_ATTACHMENT_UNUSED) {
                        skip |= LogError("VUID-VkSubpassDescription2-externalFormatResolve-09347",
                                         device,
                                         create_info_loc
                                             .pNext(Struct::VkFragmentShadingRateAttachmentInfoKHR,
                                                    Field::pFragmentShadingRateAttachment)
                    ...
                    if (fragment_density_map_info &&
                        ...
                        skip |= LogError("VUID-VkRenderPassCreateInfo2-pResolveAttachments-09331",
                                         device,
                                         create_info_loc.dot(Field::pAttachments, attachment_index).dot(Field::format),
                                         "is VK_FORMAT_UNDEFINED (referenced by %s) but %s is %" PRIu32 ".",
                                         resolve_loc.Fields().c_str(),
                                         create_info_loc
                                             .pNext(Struct::VkRenderPassFragmentDensityMapCreateInfoEXT,
                    ...
                    skip |= LogError(vuid,
                                     device,
                                     resolve_loc.dot(Field::attachment),
                                     "is %" PRIu32 ", but %s is VK_ATTACHMENT_UNUSED.",
                                     resolve_attachment_index,
                                     color_loc.dot(Field::attachment).Fields().c_str());
                } else {
                    const auto& color_desc = pCreateInfo->pAttachments[attachment_index];
                    if (color_desc.format != resolve_desc.format) {
                        ...
                        skip |= LogError(vuid,
                                         device,
                                         create_info_loc.dot(Field::pAttachments, attachment_index).dot(Field::format),
                                         "(%s) (referenced by %s) is different than pAttachments[%" PRIu32
                                         "].format (%s) (referenced by %s).",
                                         string_VkFormat(color_desc.format),
                                         color_loc.Fields().c_str(),
                                         resolve_attachment_index,
                                         string_VkFormat(resolve_desc.format),
                                         resolve_loc.Fields().c_str());
                    }
                }
            }
        ...
        if (use_rp2 && enabled_features.multisampledRenderToSingleSampled && ms_render_to_single_sample) {
            if (ms_render_to_single_sample->rasterizationSamples == VK_SAMPLE_COUNT_1_BIT) {
                skip |= LogError("VUID-VkMultisampledRenderToSingleSampledInfoEXT-rasterizationSamples-06878",
                                 device,
                                 create_info_loc.pNext(Struct::VkMultisampledRenderToSingleSampledInfoEXT,
                                                       Field::rasterizationSamples),
...
bool CoreChecks::ValidateRenderPassDAG(const VkRenderPassCreateInfo2* pCreateInfo, const ErrorObject& error_obj) const {
    ...
    for (uint32_t i = 0; i < pCreateInfo->dependencyCount; ++i) {
        ...
        // The first subpass here serves as a good proxy for "is multiview enabled" - since all view masks need to be
        // non-zero if any are, which enables multiview.
        if (use_rp2 && (dependency.dependencyFlags & VK_DEPENDENCY_VIEW_LOCAL_BIT) &&
            ...
            skip |=
                LogError("VUID-VkSubpassDependency2-dependencyFlags-03092",
                         device,
                         dependencies_loc,
            ...
            if (dependency.srcSubpass == dependency.dstSubpass) {
                vuid = use_rp2 ? "VUID-VkSubpassDependency2-srcSubpass-03085"
            ...
            if (dependency.viewOffset != 0) {
                vuid =
                    use_rp2 ? "VUID-VkSubpassDependency2-viewOffset-02530" : "VUID-VkRenderPassCreateInfo-pNext-01930";
                skip |= LogError(vuid,
                                 device,
                                 dependencies_loc,
                                 "specifies a self-dependency but has a non-zero view offset of %" PRIu32 "",
                                 dependency.viewOffset);
                ...
                vuid = use_rp2 ? "VUID-VkRenderPassCreateInfo2-pDependencies-03060"
                ...
                skip |= LogError(vuid,
                                 ...
                                 dependency.srcSubpass,
                                 pCreateInfo->pSubpasses[dependency.srcSubpass].viewMask);
                ...
                vuid = use_rp2 ? "VUID-VkSubpassDependency2-srcSubpass-06810"
                ...
                vuid = use_rp2 ? "VUID-VkSubpassDependency2-srcSubpass-02245"
                               : "VUID-VkSubpassDependency-srcSubpass-02243";
                skip |= LogError(
                    vuid,
                    device,
                    dependencies_loc,
                    "specifies a self-dependency for subpass %" PRIu32 " with both stages including a "
                    "framebuffer-space stage, but does not specify VK_DEPENDENCY_BY_REGION_BIT in dependencyFlags.",
                    dependency.srcSubpass);
            }
        } else if ((dependency.srcSubpass < dependency.dstSubpass) &&
                   ((pCreateInfo->pSubpasses[dependency.srcSubpass].flags &
            ...
            skip |= LogError(vuid,
                             device,
                             dependencies_loc,
                             "specifies that subpass %" PRIu32 " has a dependency on a later subpass"
...
bool CoreChecks::ValidateCreateRenderPass(const VkRenderPassCreateInfo2* pCreateInfo,
                                          const ErrorObject& error_obj) const {
    bool skip = false;
    const bool use_rp2 = error_obj.location.function != Func::vkCreateRenderPass;
    const char* vuid;
    ...
    for (uint32_t i = 0; i < pCreateInfo->subpassCount; ++i) {
        ...
        if (subpass.viewMask != 0) {
            ...
            if (!enabled_features.multiview) {
                skip |= LogError("VUID-VkSubpassDescription2-multiview-06558",
                                 device,
                                 subpass_loc.dot(Field::viewMask),
                                 "is %" PRIu32 ", but multiview feature is not enabled.",
                                 subpass.viewMask);
            }
            int highest_view_bit = MostSignificantBit(subpass.viewMask);
            if (highest_view_bit > 0 &&
                static_cast<uint32_t>(highest_view_bit) >= phys_dev_props_core11.maxMultiviewViewCount) {
                skip |= LogError(
        ...
        if ((subpass.flags & VK_SUBPASS_DESCRIPTION_PER_VIEW_POSITION_X_ONLY_BIT_NVX) != 0 &&
            ...
            skip |= LogError(vuid,
                             device,
                             subpass_loc,
                             "The flags parameter of subpass description %" PRIu32 " includes "
    ...
    if (use_rp2) {
        if (view_mask_non_zero && view_mask_zero) {
            skip |= LogError("VUID-VkRenderPassCreateInfo2-viewMask-03058",
                             device,
                             error_obj.location,
        ...
        if (view_mask_zero && pCreateInfo->correlatedViewMaskCount != 0) {
            skip |= LogError("VUID-VkRenderPassCreateInfo2-viewMask-03057",
                             device,
                             error_obj.location,
    ...
    for (uint32_t i = 0; i < pCreateInfo->correlatedViewMaskCount; ++i) {
        if (aggregated_cvms & pCreateInfo->pCorrelatedViewMasks[i]) {
            ...
            skip |= LogError(vuid,
                             device,
                             error_obj.location.dot(Field::pCorrelatedViewMasks, i),
    ...
    if (fragment_density_map_info) {
        if (fragment_density_map_info->fragmentDensityMapAttachment.attachment != VK_ATTACHMENT_UNUSED) {
            const Location fragment_loc = error_obj.location.pNext(Struct::VkRenderPassFragmentDensityMapCreateInfoEXT,
                                                                   Field::fragmentDensityMapAttachment);
            ...
            if (fragment_density_map_info->fragmentDensityMapAttachment.attachment >= pCreateInfo->attachmentCount) {
                ...
                if (!(fragment_density_map_info->fragmentDensityMapAttachment.layout ==
                    ...
                    skip |=
                        LogError("VUID-VkRenderPassFragmentDensityMapCreateInfoEXT-fragmentDensityMapAttachment-02549",
                                 device,
                                 attchment_loc,
                                 "(%" PRIu32 ") layout must be equal to "
                                 "VK_IMAGE_LAYOUT_FRAGMENT_DENSITY_MAP_OPTIMAL_EXT or VK_IMAGE_LAYOUT_GENERAL.",
                                 fragment_density_map_info->fragmentDensityMapAttachment.attachment);
                }
                if (!(pCreateInfo->pAttachments[fragment_density_map_info->fragmentDensityMapAttachment.attachment]
                              .loadOp == VK_ATTACHMENT_LOAD_OP_LOAD ||
                      pCreateInfo->pAttachments[fragment_density_map_info->fragmentDensityMapAttachment.attachment]
                              .loadOp == VK_ATTACHMENT_LOAD_OP_DONT_CARE)) {
                    skip |=
                        LogError("VUID-VkRenderPassFragmentDensityMapCreateInfoEXT-fragmentDensityMapAttachment-02550",
                                 device,
                                 attchment_loc,
                                 "(%" PRIu32 ") must reference an attachment with a loadOp "
                ...
                if (pCreateInfo->pAttachments[fragment_density_map_info->fragmentDensityMapAttachment.attachment]
                    ...
                    skip |=
                        LogError("VUID-VkRenderPassFragmentDensityMapCreateInfoEXT-fragmentDensityMapAttachment-02551",
                                 device,
                                 attchment_loc,
                                 "(%" PRIu32 ") must reference an attachment with a storeOp "
...
bool CoreChecks::PreCallValidateCreateRenderPass(VkDevice device,
    ...
    // Handle extension structs from KHR_multiview and KHR_maintenance2 that can only be validated for RP1 (indices out
    // of bounds)
    ...
    if (multiview_info) {
        if (multiview_info->subpassCount && multiview_info->subpassCount != pCreateInfo->subpassCount) {
            skip |= LogError("VUID-VkRenderPassCreateInfo-pNext-01928",
                             device,
                             error_obj.location,
        ...
        for (uint32_t i = 0; i < multiview_info->subpassCount; ++i) {
            ...
            if (MostSignificantBit(multiview_info->pViewMasks[i]) >=
                ...
                skip |= LogError("VUID-VkRenderPassMultiviewCreateInfo-pViewMasks-06697",
                                 device,
                                 error_obj.location,
        ...
        if (!all_zero && !all_not_zero) {
            skip |= LogError("VUID-VkRenderPassCreateInfo-pNext-02513",
                             device,
                             error_obj.location,
        ...
        if (all_zero && multiview_info->correlationMaskCount != 0) {
            skip |= LogError("VUID-VkRenderPassCreateInfo-pNext-02515",
                             device,
                             error_obj.location,
                             "VkRenderPassCreateInfo::correlationMaskCount is %" PRIu32
        ...
        for (uint32_t i = 0; i < pCreateInfo->dependencyCount; ++i) {
            if ((pCreateInfo->pDependencies[i].dependencyFlags & VK_DEPENDENCY_VIEW_LOCAL_BIT) == 0) {
                if (i < multiview_info->dependencyCount && multiview_info->pViewOffsets[i] != 0) {
                    skip |= LogError("VUID-VkRenderPassCreateInfo-pNext-02512",
                                     device,
                                     ...
                                     i,
                                     multiview_info->pViewOffsets[i]);
                }
            } else if (all_zero) {
                skip |= LogError("VUID-VkRenderPassCreateInfo-pNext-02514",
                                 device,
                                 error_obj.location.dot(Field::pDependencies, i).dot(Field::dependencyFlags),
        ...
        for (uint32_t i = 0; i < input_attachment_aspect_info->aspectReferenceCount; ++i) {
            ...
            if (subpass >= pCreateInfo->subpassCount) {
                skip |= LogError(
                    ...
                    device,
                    error_obj.location
                        .pNext(Struct::VkRenderPassInputAttachmentAspectCreateInfo, Field::pAspectReferences, i)
                        .dot(Field::subpass),
                    "is %" PRIu32 " which is not less than pCreateInfo->subpassCount (%" PRIu32 ").",
                    subpass,
                    pCreateInfo->subpassCount);
                ...
                skip |= LogError(
                    ...
                    "is %" PRIu32 " which is not less then pCreateInfo->pSubpasses[%" PRIu32
                    "].inputAttachmentCount (%" PRIu32 ").",
                    attachment,
                    subpass,
                    pCreateInfo->pSubpasses[subpass].inputAttachmentCount);
            }
        }
    }
...
// VK_KHR_depth_stencil_resolve was added with a requirement on VK_KHR_create_renderpass2 so this will never be able to
// use VkRenderPassCreateInfo
bool CoreChecks::ValidateDepthStencilResolve(const VkRenderPassCreateInfo2* pCreateInfo,
                                             const ErrorObject& error_obj) const {
    bool skip = false;
    ...
    for (uint32_t i = 0; i < pCreateInfo->subpassCount; i++) {
        ...
        if (subpass.pDepthStencilAttachment == nullptr) {
            ...
            skip |= LogError("VUID-VkSubpassDescriptionDepthStencilResolve-pDepthStencilResolveAttachment-03177",
                             device,
                             subpass_loc,
                             "includes a VkSubpassDescriptionDepthStencilResolve "
                             "structure with resolve attachment %" PRIu32
                             ", but pDepthStencilAttachment=VK_ATTACHMENT_UNUSED.",
        ...
        const Location ds_resolve_loc = subpass_loc.pNext(
            Struct::VkSubpassDescriptionDepthStencilResolve, Field::pDepthStencilResolveAttachment, resolve_attachment);
        if (resolve_attachment >= pCreateInfo->attachmentCount) {
            skip |= LogError("VUID-VkRenderPassCreateInfo2-pSubpasses-06473",
                             device,
                             ds_resolve_loc,
        ...
        // "depthResolveMode is ignored if the VkFormat of the pDepthStencilResolveAttachment does not have a depth
        // component"
        const bool resolve_has_depth = vkuFormatHasDepth(resolve_attachment_format);
        // "stencilResolveMode is ignored if the VkFormat of the pDepthStencilResolveAttachment does not have a stencil
        // component"
        ...
        if (resolve_has_depth && !resolve_has_stencil && resolve->depthResolveMode == VK_RESOLVE_MODE_NONE) {
            skip |= LogError("VUID-VkSubpassDescriptionDepthStencilResolve-pDepthStencilResolveAttachment-03178",
                             device,
                             ds_resolve_loc,
        ...
        if (resolve_depth_size > 0 && ((vkuFormatDepthSize(ds_attachment_format) != resolve_depth_size) ||
                                       (vkuFormatDepthNumericalType(ds_attachment_format) !=
                                        vkuFormatDepthNumericalType(ds_attachment_format)))) {
            skip |= LogError("VUID-VkSubpassDescriptionDepthStencilResolve-pDepthStencilResolveAttachment-03181",
                             device,
                             ds_resolve_loc,
                             "has a depth component (size %" PRIu32 "). The depth component "
        ...
        if (resolve_stencil_size > 0 && ((vkuFormatStencilSize(ds_attachment_format) != resolve_stencil_size) ||
                                         (vkuFormatStencilNumericalType(ds_attachment_format) !=
                                          vkuFormatStencilNumericalType(resolve_attachment_format)))) {
            skip |= LogError("VUID-VkSubpassDescriptionDepthStencilResolve-pDepthStencilResolveAttachment-03182",
                             device,
                             ds_resolve_loc,
                             "has a stencil component (size %" PRIu32 "). The stencil component "
                             "of pDepthStencilAttachment must have the same number of bits (currently %" PRIu32
                             ") and the same numerical type.",
                             resolve_stencil_size,
                             vkuFormatStencilSize(ds_attachment_format));
        }

        if (pCreateInfo->pAttachments[ds_attachment].samples == VK_SAMPLE_COUNT_1_BIT) {
            skip |= LogError("VUID-VkSubpassDescriptionDepthStencilResolve-pDepthStencilResolveAttachment-03179",
                             device,
                             ds_resolve_loc,
                             "is not NULL, however pDepthStencilAttachment has sample count=VK_SAMPLE_COUNT_1_BIT.");
        }

        if (pCreateInfo->pAttachments[resolve_attachment].samples != VK_SAMPLE_COUNT_1_BIT) {
            skip |= LogError("VUID-VkSubpassDescriptionDepthStencilResolve-pDepthStencilResolveAttachment-03180",
                             device,
                             ds_resolve_loc,
                             "has sample count of VK_SAMPLE_COUNT_1_BIT.");
        }
        ...
        if ((subpass.flags & VK_SUBPASS_DESCRIPTION_SHADER_RESOLVE_BIT_QCOM) != 0) {
            skip |=
                LogError("VUID-VkRenderPassCreateInfo2-flags-04908",
                         device,
                         subpass_loc,
...
bool CoreChecks::PreCallValidateCreateRenderPass2(VkDevice device,
...
bool CoreChecks::ValidateFragmentShadingRateAttachments(const VkRenderPassCreateInfo2* pCreateInfo,
    ...
    for (uint32_t attachment_description = 0; attachment_description < pCreateInfo->attachmentCount;
         ++attachment_description) {
        ...
        for (uint32_t subpass = 0; subpass < pCreateInfo->subpassCount; ++subpass) {
            const auto* fragment_shading_rate_attachment =
                vku::FindStructInPNextChain<VkFragmentShadingRateAttachmentInfoKHR>(
                    pCreateInfo->pSubpasses[subpass].pNext);
            ...
            const Location fragment_loc = subpass_loc.pNext(Struct::VkFragmentShadingRateAttachmentInfoKHR,
                                                            Field::pFragmentShadingRateAttachment);
            ...
            if (attachment_reference.attachment == attachment_description) {
                ...
                if (pCreateInfo->pAttachments[attachment_reference.attachment].loadOp == VK_ATTACHMENT_LOAD_OP_CLEAR) {
                    skip |= LogError("VUID-VkRenderPassCreateInfo2-pAttachments-09387",
                                     device,
                                     fragment_loc.dot(Field::attachment),
                                     "(%" PRIu32 ") has a loadOp of VK_ATTACHMENT_LOAD_OP_CLEAR",
            ...
            if (attachment_reference.attachment != VK_ATTACHMENT_UNUSED) {
                if ((pCreateInfo->flags & VK_RENDER_PASS_CREATE_TRANSFORM_BIT_QCOM) != 0) {
                    skip |= LogError("VUID-VkRenderPassCreateInfo2-flags-04521",
                                     device,
                                     fragment_loc.dot(Field::attachment),
                                     "is not VK_ATTACHMENT_UNUSED, but render pass includes "
                ...
                if (attachment_reference.layout != VK_IMAGE_LAYOUT_GENERAL &&
                    ...
                    skip |= LogError("VUID-VkFragmentShadingRateAttachmentInfoKHR-pFragmentShadingRateAttachment-04524",
                                     device,
                                     fragment_loc.dot(Field::layout),
                                     "has a layout of %s.",
                ...
                const Location texel_loc = subpass_loc.pNext(Struct::VkFragmentShadingRateAttachmentInfoKHR,
                                                             Field::shadingRateAttachmentTexelSize);
                if (!IsPowerOfTwo(texel_size.width)) {
                    skip |= LogError("VUID-VkFragmentShadingRateAttachmentInfoKHR-pFragmentShadingRateAttachment-04525",
                                     device,
                                     texel_loc.dot(Field::width),
                                     "(%" PRIu32 ") is a non-power-of-two.",
                                     texel_size.width);
                }
                if (texel_size.width <
                    ...
                    skip |= LogError(
                        ...
                        device,
                        texel_loc.dot(Field::width),
                        "(%" PRIu32 ") is lower than the advertised minimum width %" PRIu32 ".",
                        texel_size.width,
                        phys_dev_ext_props.fragment_shading_rate_props.minFragmentShadingRateAttachmentTexelSize.width);
                }
                if (texel_size.width >
                    ...
                    skip |= LogError(
                        ...
                        device,
                        texel_loc.dot(Field::width),
                        "(%" PRIu32 ") is higher than the advertised maximum width %" PRIu32 ".",
                        texel_size.width,
                        phys_dev_ext_props.fragment_shading_rate_props.maxFragmentShadingRateAttachmentTexelSize.width);
                }
                if (!IsPowerOfTwo(texel_size.height)) {
                    skip |= LogError("VUID-VkFragmentShadingRateAttachmentInfoKHR-pFragmentShadingRateAttachment-04528",
                                     device,
                ...
                if (texel_size.height <
                    ...
                    skip |= LogError("VUID-VkFragmentShadingRateAttachmentInfoKHR-pFragmentShadingRateAttachment-04530",
                                     device,
                                     texel_loc.dot(Field::height),
                                     "(%" PRIu32 ") is lower than the advertised minimum height %" PRIu32 ".",
                                     texel_size.height,
                ...
                if (texel_size.height >
                    ...
                    skip |= LogError("VUID-VkFragmentShadingRateAttachmentInfoKHR-pFragmentShadingRateAttachment-04529",
                                     device,
                                     texel_loc.dot(Field::height),
                                     "(%" PRIu32 ") is higher than the advertised maximum height %" PRIu32 ".",
                                     texel_size.height,
                                     phys_dev_ext_props.fragment_shading_rate_props
                ...
                if (aspect_ratio > phys_dev_ext_props.fragment_shading_rate_props
                                       .maxFragmentShadingRateAttachmentTexelSizeAspectRatio) {
                    skip |=
                        LogError("VUID-VkFragmentShadingRateAttachmentInfoKHR-pFragmentShadingRateAttachment-04531",
                                 device,
                                 texel_loc,
                ...
                if (inverse_aspect_ratio > phys_dev_ext_props.fragment_shading_rate_props
                                               .maxFragmentShadingRateAttachmentTexelSizeAspectRatio) {
                    skip |= LogError("VUID-VkFragmentShadingRateAttachmentInfoKHR-pFragmentShadingRateAttachment-04532",
                                     device,
                                     texel_loc,
                                     "has a texel size of %" PRIu32 " by %" PRIu32
                                     ", which has an inverse aspect ratio of %" PRIu32
                                     ", which is higher than the advertised maximum aspect ratio %" PRIu32 ".",
                                     texel_size.width,
                                     texel_size.height,
        ...
        if (!used_as_fragment_shading_rate_attachment.empty()) {
            for (uint32_t subpass = 0; subpass < pCreateInfo->subpassCount; ++subpass) {
                ...
                std::string fsr_attachment_subpasses_string =
                    vector_to_string(used_as_fragment_shading_rate_attachment);
                ...
                for (uint32_t attachment = 0; attachment < subpass_info.colorAttachmentCount; ++attachment) {
                    if (subpass_info.pColorAttachments[attachment].attachment == attachment_description) {
                        skip |= LogError("VUID-VkRenderPassCreateInfo2-pAttachments-04585",
                                         device,
                                         ...
                                         attachment_description,
                                         fsr_attachment_subpasses_string.c_str());
                    }
                }
                for (uint32_t attachment = 0; attachment < subpass_info.colorAttachmentCount; ++attachment) {
                ...
                if (depth_stencil_resolve_attachment &&
                    depth_stencil_resolve_attachment->pDepthStencilResolveAttachment) {
                    if (depth_stencil_resolve_attachment->pDepthStencilResolveAttachment->attachment ==
                        attachment_description) {
                        skip |= LogError("VUID-VkRenderPassCreateInfo2-pAttachments-04585",
                                         device,
                                         subpass_loc
                                             .pNext(Struct::VkSubpassDescriptionDepthStencilResolve,
                                                    Field::pDepthStencilResolveAttachment)
                                             .dot(Field::attachment),
                                         "is also used in pAttachments[%" PRIu32
                                         "] as a fragment shading rate attachment in subpass(es) %s",
                                         attachment_description,
...
bool CoreChecks::PreCallValidateCreateRenderPass2KHR(VkDevice device,
                                                     const VkRenderPassCreateInfo2* pCreateInfo,
                                                     const VkAllocationCallbacks* pAllocator,
                                                     VkRenderPass* pRenderPass,
                                                     const ErrorObject& error_obj) const {
...
bool CoreChecks::ValidateRenderingInfoAttachment(const std::shared_ptr<const vvl::ImageView>& image_view,
    ...
    const bool x_extent_valid = static_cast<int64_t>(image_view->image_state->create_info.extent.width) >=
                                static_cast<int64_t>(pRenderingInfo->renderArea.offset.x) +
                                    static_cast<int64_t>(pRenderingInfo->renderArea.extent.width);
    const bool y_extent_valid = static_cast<int64_t>(image_view->image_state->create_info.extent.height) >=
                                static_cast<int64_t>(pRenderingInfo->renderArea.offset.y) +
                                    static_cast<int64_t>(pRenderingInfo->renderArea.extent.height);

    auto device_group_render_pass_begin_info =
        vku::FindStructInPNextChain<VkDeviceGroupRenderPassBeginInfo>(pRenderingInfo->pNext);
    if (!device_group_render_pass_begin_info || device_group_render_pass_begin_info->deviceRenderAreaCount == 0) {
        if (!x_extent_valid) {
            skip |= LogError("VUID-VkRenderingInfo-pNext-06079",
                             objlist,
                             loc,
                             "width (%" PRIu32 ") is less than pRenderingInfo->renderArea.offset.x (%" PRId32
                             ") + pRenderingInfo->renderArea.extent.width (%" PRIu32 ").",
                             image_view->image_state->create_info.extent.width,
                             pRenderingInfo->renderArea.offset.x,
                             pRenderingInfo->renderArea.extent.width);
        }

        if (!y_extent_valid) {
            skip |= LogError("VUID-VkRenderingInfo-pNext-06080",
                             objlist,
                             loc,
                             "height (%" PRIu32 ") is less than pRenderingInfo->renderArea.offset.y (%" PRId32
                             ") + pRenderingInfo->renderArea.extent.height (%" PRIu32 ").",
                             image_view->image_state->create_info.extent.height,
...
bool CoreChecks::ValidateRenderingAttachmentInfo(VkCommandBuffer commandBuffer,
                                                 const VkRenderingInfo& rendering_info,
                                                 const VkRenderingAttachmentInfo& attachment_info,
                                                 const Location& attachment_loc) const {
    bool skip = false;

    // "If imageView is VK_NULL_HANDLE, and resolveMode is not VK_RESOLVE_MODE_EXTERNAL_FORMAT_DOWNSAMPLE_ANDROID, other
    ...
    if (attachment_info.imageLayout == VK_IMAGE_LAYOUT_PRESENT_SRC_KHR) {
        skip |= LogError("VUID-VkRenderingAttachmentInfo-imageView-06145",
                         commandBuffer,
                         attachment_loc.dot(Field::imageLayout),
                         "must not be VK_IMAGE_LAYOUT_PRESENT_SRC_KHR");
    }

    if ((!vkuFormatIsSINT(image_view_format) && !vkuFormatIsUINT(image_view_format)) &&
        vkuFormatIsColor(image_view_format) &&
        ...
        skip |=
            LogError("VUID-VkRenderingAttachmentInfo-imageView-06129",
                     objlist,
    ...
    if ((vkuFormatIsSINT(image_view_format) || vkuFormatIsUINT(image_view_format)) &&
        vkuFormatIsColor(image_view_format) &&
        !(attachment_info.resolveMode == VK_RESOLVE_MODE_NONE ||
          attachment_info.resolveMode == VK_RESOLVE_MODE_SAMPLE_ZERO_BIT)) {
        const LogObjectList objlist(commandBuffer, attachment_info.imageView);
        skip |=
            LogError("VUID-VkRenderingAttachmentInfo-imageView-06130",
                     objlist,
                     attachment_loc.dot(Field::resolveMode),
    ...
    if (attachment_info.imageLayout == VK_IMAGE_LAYOUT_FRAGMENT_DENSITY_MAP_OPTIMAL_EXT) {
        skip |= LogError("VUID-VkRenderingAttachmentInfo-imageView-06140",
                         commandBuffer,
                         attachment_loc.dot(Field::imageLayout),
    ...
    if (attachment_info.resolveMode != VK_RESOLVE_MODE_NONE && image_view_state->samples == VK_SAMPLE_COUNT_1_BIT) {
        const auto msrtss_info =
            vku::FindStructInPNextChain<VkMultisampledRenderToSingleSampledInfoEXT>(rendering_info.pNext);
        if (!msrtss_info || !msrtss_info->multisampledRenderToSingleSampledEnable) {
            const LogObjectList objlist(commandBuffer, attachment_info.imageView);
            skip |= LogError("VUID-VkRenderingAttachmentInfo-imageView-06861",
                             objlist,
        ...
        if (msrtss_info && msrtss_info->multisampledRenderToSingleSampledEnable &&
            ...
            skip |= LogError(
                ...
                objlist,
                attachment_loc.dot(Field::resolveMode),
                "is %s and VkMultisampledRenderToSingleSampledInfoEXT::multisampledRenderToSingleSampledEnable is "
                "VK_TRUE, and "
                "%s.imageView has a sample count of VK_SAMPLE_COUNT_1_BIT, and resolveImageView (%s) is not "
                "VK_NULL_HANDLE.",
                string_VkResolveModeFlags(attachment_info.resolveMode).c_str(),
    ...
    if (attachment_info.resolveMode != VK_RESOLVE_MODE_NONE && attachment_info.resolveImageView == VK_NULL_HANDLE) {
        const auto msrtss_info =
            vku::FindStructInPNextChain<VkMultisampledRenderToSingleSampledInfoEXT>(rendering_info.pNext);
        if (!msrtss_info || !msrtss_info->multisampledRenderToSingleSampledEnable) {
            skip |= LogError("VUID-VkRenderingAttachmentInfo-imageView-06862",
                             commandBuffer,
                             attachment_loc.dot(Field::resolveMode),
    ...
    if (resolve_view_state && (attachment_info.resolveMode != VK_RESOLVE_MODE_NONE) &&
        ...
        const LogObjectList objlist(commandBuffer, attachment_info.resolveImageView);
        skip |= LogError("VUID-VkRenderingAttachmentInfo-imageView-06864",
                         commandBuffer,
                         attachment_loc.dot(Field::resolveMode),
    ...
    if (attachment_info.resolveMode != VK_RESOLVE_MODE_NONE) {
        if (attachment_info.resolveImageLayout == VK_IMAGE_LAYOUT_PRESENT_SRC_KHR) {
            skip |= LogError("VUID-VkRenderingAttachmentInfo-imageView-06146",
                             commandBuffer,
        ...
        if (attachment_info.resolveImageLayout == VK_IMAGE_LAYOUT_FRAGMENT_DENSITY_MAP_OPTIMAL_EXT) {
            skip |= LogError("VUID-VkRenderingAttachmentInfo-imageView-06141",
                             commandBuffer,
                             attachment_loc.dot(Field::resolveImageLayout),
                             "must not be VK_IMAGE_LAYOUT_FRAGMENT_DENSITY_MAP_OPTIMAL_EXT");
        }
        ...
        if (attachment_info.resolveImageLayout == VK_IMAGE_LAYOUT_READ_ONLY_OPTIMAL) {
            skip |= LogError("VUID-VkRenderingAttachmentInfo-imageView-06142",
                             commandBuffer,
                             attachment_loc.dot(Field::resolveImageLayout),
                             "must not be VK_IMAGE_LAYOUT_READ_ONLY_OPTIMAL");
        }

        if (resolve_view_state && (image_view_format != resolve_view_state->create_info.format)) {
            const LogObjectList objlist(commandBuffer, attachment_info.resolveImageView);
            skip |= LogError("VUID-VkRenderingAttachmentInfo-imageView-06865",
                             objlist,
                             attachment_loc.dot(Field::resolveImageView),
                             "format (%s) and %s format (%s) are different.",
                             string_VkFormat(resolve_view_state->create_info.format),
                             attachment_loc.dot(Field::imageView).Fields().c_str(),
                             string_VkFormat(image_view_format));
        }
        ...
        if (IsValueIn(attachment_info.resolveImageLayout,
            ...
            skip |= LogError("VUID-VkRenderingAttachmentInfo-imageView-06136",
                             commandBuffer,
                             attachment_loc.dot(Field::resolveImageLayout),
                             "is %s.",
        ...
        if (((attachment_info.resolveImageLayout == VK_IMAGE_LAYOUT_DEPTH_READ_ONLY_OPTIMAL) ||
            ...
            skip |= LogError("VUID-VkRenderingAttachmentInfo-imageView-06137",
                             commandBuffer,
                             attachment_loc.dot(Field::resolveImageLayout),
                             "is %s.",
                             string_VkImageLayout(attachment_info.resolveImageLayout));
    ...
    if (IsValueIn(attachment_info.imageLayout,
                  { VK_IMAGE_LAYOUT_UNDEFINED,
                    ...
                    VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL,
                    VK_IMAGE_LAYOUT_PREINITIALIZED })) {
        skip |= LogError("VUID-VkRenderingAttachmentInfo-imageView-06135",
                         commandBuffer,
                         attachment_loc.dot(Field::imageLayout),
                         "is %s.",
                         string_VkImageLayout(attachment_info.imageLayout));
    }
...
bool CoreChecks::ValidateBeginRenderingFragmentDensityMap(VkCommandBuffer commandBuffer,
    ...
    if (!enabled_features.fragmentDensityMapNonSubsampledImages) {
        for (uint32_t j = 0; j < rendering_info.colorAttachmentCount; ++j) {
            if (rendering_info.pColorAttachments[j].imageView != VK_NULL_HANDLE) {
                ...
                if (image_view_state &&
                    !(image_view_state->image_state->create_info.flags & VK_IMAGE_CREATE_SUBSAMPLED_BIT_EXT)) {
        ...
        if (rendering_info.pDepthAttachment && (rendering_info.pDepthAttachment->imageView != VK_NULL_HANDLE)) {
            ...
            if (depth_view_state &&
                !(depth_view_state->image_state->create_info.flags & VK_IMAGE_CREATE_SUBSAMPLED_BIT_EXT)) {
        ...
        if (rendering_info.pStencilAttachment && (rendering_info.pStencilAttachment->imageView != VK_NULL_HANDLE)) {
            ...
            if (stencil_view_state &&
                !(stencil_view_state->image_state->create_info.flags & VK_IMAGE_CREATE_SUBSAMPLED_BIT_EXT)) {
    ...
    if (fragment_density_map_attachment_info->imageView != VK_NULL_HANDLE) {
        ...
        if ((fragment_density_map_view_state->image_state->create_info.flags & VK_IMAGE_CREATE_SUBSAMPLED_BIT_EXT) >
            0) {
            const LogObjectList objlist(commandBuffer,
                                        fragment_density_map_attachment_info->imageView,
        ...
        int32_t layer_count =
            static_cast<int32_t>(fragment_density_map_view_state->normalized_subresource_range.layerCount);
        if (layer_count != 1 && !enabled_features.multiview) {
            ...
            skip |= LogError("VUID-VkRenderingFragmentDensityMapAttachmentInfoEXT-apiVersion-07908",
                             objlist,
                             view_loc,
                             "must have a layer count (%" PRId32 ") equal to 1.",
                             layer_count);
        }
        if ((rendering_info.viewMask == 0) && (layer_count != 1)) {
            ...
            skip |= LogError("VUID-VkRenderingInfo-imageView-06109",
                             objlist,
                             view_loc,
                             "must have a layer count (%" PRId32 ") equal to 1 when viewMask is equal to 0",
                             layer_count);
        }
    ...
    const auto* device_group_begin_info =
        vku::FindStructInPNextChain<VkDeviceGroupRenderPassBeginInfo>(rendering_info.pNext);
    const bool non_zero_device_render_area =
        device_group_begin_info && device_group_begin_info->deviceRenderAreaCount != 0;
    if (!non_zero_device_render_area) {
        if (fragment_density_map_attachment_info && fragment_density_map_attachment_info->imageView != VK_NULL_HANDLE) {
            ...
            if (image_state->create_info.extent.width <
                ...
                const LogObjectList objlist(
                    commandBuffer, fragment_density_map_attachment_info->imageView, image_state->Handle());
                skip |= LogError(
            ...
            if (image_state->create_info.extent.height <
                ...
                skip |= LogError(
                    ...
                    ") + pRenderingInfo->renderArea.extent.height (%" PRIu32
                    ") ) / VkPhysicalDeviceFragmentDensityMapPropertiesEXT::maxFragmentDensityTexelSize.height "
                    "(%" PRIu32 ").",
...
bool CoreChecks::ValidateBeginRenderingFragmentShadingRate(VkCommandBuffer commandBuffer,
                                                           const VkRenderingInfo& rendering_info,
                                                           const Location& rendering_info_loc) const {
    bool skip = false;
    ...
    if (rendering_info.viewMask == 0) {
        ...
        if (layer_count != 1 && layer_count < highest_view_bit) {
            skip |= LogError(
                ...
                "has a layerCount (%" PRId32 ") but must either is equal to 1 or greater than "
    ...
    skip |= ValidateBeginRenderingFragmentShadingRateRenderArea(commandBuffer,
                                                                *view_state,
    ...
    if (!non_zero_device_render_area) {
        ...
        if (static_cast<int64_t>(view_state.image_state->create_info.extent.width) <
            ...
            skip |= LogError(
                ...
                view_state.image_state->create_info.extent.width,
                render_area.offset.x,
                render_area.extent.width,
        ...
        if (static_cast<int64_t>(view_state.image_state->create_info.extent.height) <
            ...
            skip |= LogError(
                ...
                view_state.image_state->create_info.extent.height,
                render_area.offset.y,
                render_area.extent.height,
        ...
        if (device_group_begin_info) {
            for (uint32_t deviceRenderAreaIndex = 0;
                 deviceRenderAreaIndex < device_group_begin_info->deviceRenderAreaCount;
                ...
                const uint32_t height =
                    device_group_begin_info->pDeviceRenderAreas[deviceRenderAreaIndex].extent.height;
                ...
                if (image_state->create_info.extent.height <
                    ...
                    skip |= LogError(
                        ...
                        "].extent.height (%" PRIu32 ") ) / shadingRateAttachmentTexelSize.height "
                        "(%" PRIu32 ").",
                        image_state->create_info.extent.height,
                        deviceRenderAreaIndex,
                        offset_y,
...
bool CoreChecks::ValidateBeginRenderingDeviceGroup(VkCommandBuffer commandBuffer,
                                                   const VkRenderingInfo& rendering_info,
                                                   const Location& rendering_info_loc) const {
    bool skip = false;
    ...
    if (!non_zero_device_render_area) {
        ...
        if (render_area.offset.x < 0) {
            skip |= LogError("VUID-VkRenderingInfo-pNext-06077",
                             commandBuffer,
                             rendering_info_loc.dot(Field::renderArea).dot(Field::offset).dot(Field::x),
                             "is %" PRId32 " (offset can't be negative).",
                             render_area.offset.x);
        } else if (render_area.offset.y < 0) {
            skip |= LogError("VUID-VkRenderingInfo-pNext-06078",
                             commandBuffer,
                             rendering_info_loc.dot(Field::renderArea).dot(Field::offset).dot(Field::y),
                             "is %" PRId32 " (offset can't be negative).",
                             render_area.offset.y);
        } else if (render_area.extent.width == 0) {
            skip |= LogError("VUID-VkRenderingInfo-None-08994",
                             commandBuffer,
                             rendering_info_loc.dot(Field::renderArea).dot(Field::extent).dot(Field::width),
                             "is zero.");
        } else if (render_area.extent.height == 0) {
            skip |= LogError("VUID-VkRenderingInfo-None-08995",
                             commandBuffer,
                             rendering_info_loc.dot(Field::renderArea).dot(Field::extent).dot(Field::height),
                             "is zero.");
        }
        ...
        if (x_adjusted_extent > phys_dev_props.limits.maxFramebufferWidth) {
            skip |= LogError("VUID-VkRenderingInfo-pNext-07815",
                             commandBuffer,
                             rendering_info_loc.dot(Field::renderArea),
        ...
        if (y_adjusted_extent > phys_dev_props.limits.maxFramebufferHeight) {
            skip |= LogError("VUID-VkRenderingInfo-pNext-07816",
                             commandBuffer,
                             rendering_info_loc.dot(Field::renderArea),
    ...
    for (uint32_t dra_i = 0; dra_i < device_group_begin_info->deviceRenderAreaCount; ++dra_i) {
        ...
        if (offset_x < 0) {
            skip |= LogError("VUID-VkDeviceGroupRenderPassBeginInfo-offset-06166",
                             commandBuffer,
                             group_loc.dot(Field::offset).dot(Field::x),
                             "is %" PRId32 " (offset can't be negative).",
                             offset_x);
        } else if (offset_y < 0) {
            skip |= LogError("VUID-VkDeviceGroupRenderPassBeginInfo-offset-06167",
                             commandBuffer,
                             group_loc.dot(Field::offset).dot(Field::y),
                             "is %" PRId32 " (offset can't be negative).",
                             offset_y);
        } else if (width == 0) {
            skip |= LogError("VUID-VkDeviceGroupRenderPassBeginInfo-extent-08998",
                             commandBuffer,
                             group_loc.dot(Field::extent).dot(Field::width),
                             "is zero.");
        } else if (height == 0) {
            skip |= LogError("VUID-VkDeviceGroupRenderPassBeginInfo-extent-08999",
                             commandBuffer,
                             group_loc.dot(Field::extent).dot(Field::height),
                             "is zero.");
        }

        if ((offset_x + width) > phys_dev_props.limits.maxFramebufferWidth) {
            skip |= LogError("VUID-VkDeviceGroupRenderPassBeginInfo-offset-06168",
                             commandBuffer,
                             group_loc,
        ...
        if ((offset_y + height) > phys_dev_props.limits.maxFramebufferHeight) {
            skip |= LogError("VUID-VkDeviceGroupRenderPassBeginInfo-offset-06169",
                             commandBuffer,
                             group_loc,
        ...
        for (uint32_t j = 0; j < rendering_info.colorAttachmentCount; ++j) {
            ...
            if (image_state->create_info.extent.width < offset_x + width) {
                ...
                skip |= LogError("VUID-VkRenderingInfo-pNext-06083",
                                 objlist,
                                 rendering_info_loc.dot(Field::pColorAttachments, j).dot(Field::imageView),
                                 "width (%" PRIu32 ") must be greater than or equal to"
            ...
            if (image_state->create_info.extent.height < offset_y + height) {
                ...
                skip |= LogError("VUID-VkRenderingInfo-pNext-06084",
                                 objlist,
                                 rendering_info_loc.dot(Field::pColorAttachments, j).dot(Field::imageView),
                                 "height (%" PRIu32 ") must be greater than or equal to"
...
bool CoreChecks::PreCallValidateCmdBeginRendering(VkCommandBuffer commandBuffer,
    ...
    if (cb_state->IsSecondary() &&
        ((pRenderingInfo->flags & VK_RENDERING_CONTENTS_SECONDARY_COMMAND_BUFFERS_BIT) != 0) &&
    ...
    if (!(IsExtEnabled(device_extensions.vk_amd_mixed_attachment_samples) ||
        ...
        for (uint32_t j = 0; j < pRenderingInfo->colorAttachmentCount; ++j) {
            ...
            if (first_sample_count_attachment != image_view->samples) {
                skip |= LogError("VUID-VkRenderingInfo-multisampledRenderToSingleSampled-06857",
                                 objlist,
                                 image_view_loc,
                                 "has sample count %s, whereas first used color "
                                 "attachment ref has sample count %" PRIu32 ".",
                                 string_VkSampleCountFlagBits(image_view->samples),
                                 first_sample_count_attachment);
            }
        ...
        if (pRenderingInfo->pDepthAttachment && pRenderingInfo->pDepthAttachment->imageView != VK_NULL_HANDLE) {
            ...
            skip |=
                ValidateRenderingInfoAttachment(image_view,
                                                pRenderingInfo,
                                                objlist,
                                                rendering_info_loc.dot(Field::pDepthAttachment).dot(Field::imageView));
        }
        if (pRenderingInfo->pStencilAttachment && pRenderingInfo->pStencilAttachment->imageView != VK_NULL_HANDLE) {
            ...
            skip |= ValidateRenderingInfoAttachment(
                image_view,
                pRenderingInfo,
                objlist,
                rendering_info_loc.dot(Field::pStencilAttachment).dot(Field::imageView));
    ...
    if (MostSignificantBit(pRenderingInfo->viewMask) >=
        static_cast<int32_t>(phys_dev_props_core11.maxMultiviewViewCount)) {
        skip |= LogError("VUID-VkRenderingInfo-viewMask-06128",
                         commandBuffer,
                         rendering_info_loc.dot(Field::viewMask),
...
bool CoreChecks::ValidateBeginRenderingColorAttachment(VkCommandBuffer commandBuffer,
                                                       const VkRenderingInfo& rendering_info,
                                                       const Location& rendering_info_loc) const {
    bool skip = false;
    for (uint32_t i = 0; i < rendering_info.colorAttachmentCount; ++i) {
        ...
        if (color_attachment.imageView != VK_NULL_HANDLE) {
            ...
            if (!(image_state->create_info.usage & VK_IMAGE_USAGE_COLOR_ATTACHMENT_BIT)) {
                const LogObjectList objlist(commandBuffer, image_view_state->Handle(), image_state->Handle());
                skip |= LogError("VUID-VkRenderingInfo-colorAttachmentCount-06087",
                                 objlist,
            ...
            if (!IsIdentitySwizzle(components)) {
                ...
                skip |= LogError("VUID-VkRenderingInfo-colorAttachmentCount-09479",
                                 objlist,
                                 color_attachment_loc.dot(Field::imageView),
                                 "has a non-identiy swizzle component, here are the actual swizzle values:\n%s",
                                 string_VkComponentMapping(components).c_str());
        ...
        if (color_attachment.resolveMode == VK_RESOLVE_MODE_EXTERNAL_FORMAT_DOWNSAMPLE_ANDROID) {
            if (!enabled_features.externalFormatResolve) {
                skip |= LogError("VUID-VkRenderingAttachmentInfo-externalFormatResolve-09323",
                                 commandBuffer,
            ...
            if (fragment_density_info_ext && fragment_density_info_ext->imageView != VK_NULL_HANDLE) {
                skip |= LogError(
                    ...
                    rendering_info_loc.pNext(Struct::VkRenderingFragmentDensityMapAttachmentInfoEXT, Field::imageView),
                    "is not null (%s).",
                    FormatHandle(fragment_density_info_ext->imageView).c_str());
            }
            ...
            if (fragment_shading_rate_info_khr && fragment_shading_rate_info_khr->imageView != VK_NULL_HANDLE) {
                skip |= LogError(
                    ...
                    rendering_info_loc.pNext(Struct::VkRenderingFragmentShadingRateAttachmentInfoKHR, Field::imageView),
                    "is not null (%s).",
                    FormatHandle(fragment_shading_rate_info_khr->imageView).c_str());
            }

            auto resolve_view_state = Get<vvl::ImageView>(color_attachment.resolveImageView);
            if (!resolve_view_state) {
                skip |= LogError("VUID-VkRenderingAttachmentInfo-resolveMode-09324",
                                 commandBuffer,
                ...
                if (android_external_format_resolve_null_color_attachment_prop &&
                    ...
                    skip |=
                        LogError("VUID-VkRenderingAttachmentInfo-nullColorAttachmentWithExternalFormatResolve-09325",
                                 commandBuffer,
                                 color_attachment_loc.dot(Field::resolveImageView),
                ...
                if (!resolve_view_state->image_state->HasAHBFormat()) {
                    skip |= LogError("VUID-VkRenderingAttachmentInfo-resolveMode-09326",
                                     commandBuffer,
                                     color_attachment_loc.dot(Field::resolveImageView),
                                     "was not created with an external format.");
                }
                if (resolve_view_state->create_info.subresourceRange.layerCount != 1) {
                    skip |= LogError("VUID-VkRenderingAttachmentInfo-resolveMode-09327",
                                     commandBuffer,
                                     color_attachment_loc.dot(Field::resolveImageView),
                ...
                if (auto color_view_state = Get<vvl::ImageView>(color_attachment.imageView)) {
                    if (android_external_format_resolve_null_color_attachment_prop) {
                        skip |= LogError("VUID-VkRenderingAttachmentInfo-resolveMode-09328",
                                         commandBuffer,
                                         color_attachment_loc.dot(Field::imageView),
                                         "is not null (%s).",
                        ...
                        if (it != ahb_ext_resolve_formats_map.end()) {
                            if (it->second != color_view_state->create_info.format) {
                                skip |= LogError("VUID-VkRenderingAttachmentInfo-resolveMode-09330",
                                                 commandBuffer,
                                                 color_attachment_loc.dot(Field::imageView),
                                                 "has externalFormat %" PRIu64
                                                 " which corresponds to needing a color attachment format of %s, but "
                                                 "the format is %s.",
                                                 resolve_view_state->image_state->ahb_format,
                                                 string_VkFormat(it->second),
                    ...
                    skip |= LogError("VUID-VkRenderingAttachmentInfo-resolveMode-09329",
                                     commandBuffer,
                                     color_attachment_loc.dot(Field::imageView),
                                     "is not valid.");
                }
        ...
        if (color_attachment.resolveMode != VK_RESOLVE_MODE_NONE) {
            if (auto resolve_view_state = Get<vvl::ImageView>(color_attachment.resolveImageView)) {
                ...
                if (!IsIdentitySwizzle(components)) {
                    ...
                    skip |= LogError("VUID-VkRenderingInfo-colorAttachmentCount-09480",
                                     objlist,
                                     color_attachment_loc.dot(Field::resolveImageView),
                                     "has a non-identiy swizzle component, here are the actual swizzle values:\n%s",
                                     string_VkComponentMapping(components).c_str());
                ...
                if ((image_usage & VK_IMAGE_USAGE_COLOR_ATTACHMENT_BIT) == 0) {
                    const LogObjectList objlist(
                        commandBuffer, resolve_view_state->Handle(), resolve_view_state->image_state->Handle());
                    skip |= LogError("VUID-VkRenderingInfo-colorAttachmentCount-09476",
                                     objlist,
                                     color_attachment_loc.dot(Field::resolveImageView),
                                     "image was created with %s.",
...
bool CoreChecks::ValidateBeginRenderingDepthAttachment(VkCommandBuffer commandBuffer,
                                                       const VkRenderingInfo& rendering_info,
                                                       const Location& rendering_info_loc) const {
    bool skip = false;
    ...
    skip |= ValidateRenderingAttachmentInfo(
        commandBuffer, rendering_info, depth_attachment, rendering_info_loc.dot(Field::pDepthAttachment));
    ...
    if (depth_attachment.imageView != VK_NULL_HANDLE) {
        ...
        if (!IsIdentitySwizzle(components)) {
            const LogObjectList objlist(commandBuffer, depth_view_state->Handle());
            skip |= LogError("VUID-VkRenderingInfo-pDepthAttachment-09481",
                             objlist,
                             rendering_info_loc.dot(Field::pDepthAttachment).dot(Field::imageView),
                             "has a non-identiy swizzle component, here are the actual swizzle values:\n%s",
                             string_VkComponentMapping(components).c_str());
    ...
    if (depth_attachment.resolveMode != VK_RESOLVE_MODE_NONE) {
        if (depth_attachment.resolveMode == VK_RESOLVE_MODE_EXTERNAL_FORMAT_DOWNSAMPLE_ANDROID) {
            skip |= LogError("VUID-VkRenderingInfo-pDepthAttachment-09318",
                             commandBuffer,
        ...
        if (auto depth_resolve_view_state = Get<vvl::ImageView>(depth_attachment.resolveImageView)) {
            ...
            if (!IsIdentitySwizzle(components)) {
                ...
                skip |= LogError("VUID-VkRenderingInfo-pDepthAttachment-09482",
                                 objlist,
            ...
            if ((image_usage & VK_IMAGE_USAGE_DEPTH_STENCIL_ATTACHMENT_BIT) == 0) {
                const LogObjectList objlist(
                    commandBuffer, depth_resolve_view_state->Handle(), depth_resolve_view_state->image_state->Handle());
                skip |= LogError("VUID-VkRenderingInfo-pDepthAttachment-09477",
                                 objlist,
                                 rendering_info_loc.dot(Field::pDepthAttachment).dot(Field::resolveImageView),
                                 "image was created with %s.",
                                 string_VkImageUsageFlags(image_usage).c_str());
...
bool CoreChecks::ValidateBeginRenderingStencilAttachment(VkCommandBuffer commandBuffer,
                                                         const VkRenderingInfo& rendering_info,
                                                         const Location& rendering_info_loc) const {
    bool skip = false;
    ...
    skip |= ValidateRenderingAttachmentInfo(
        commandBuffer, rendering_info, stencil_attachment, rendering_info_loc.dot(Field::pStencilAttachment));
    ...
    if (stencil_attachment.imageView != VK_NULL_HANDLE) {
        ...
        if (!IsIdentitySwizzle(components)) {
            const LogObjectList objlist(commandBuffer, stencil_view_state->Handle());
            skip |= LogError("VUID-VkRenderingInfo-pStencilAttachment-09483",
                             objlist,
                             rendering_info_loc.dot(Field::pStencilAttachment).dot(Field::imageView),
                             "has a non-identiy swizzle component, here are the actual swizzle values:\n%s",
                             string_VkComponentMapping(components).c_str());
    ...
    if (stencil_attachment.resolveMode != VK_RESOLVE_MODE_NONE) {
        if (stencil_attachment.resolveMode == VK_RESOLVE_MODE_EXTERNAL_FORMAT_DOWNSAMPLE_ANDROID) {
            skip |= LogError("VUID-VkRenderingInfo-pStencilAttachment-09319",
                             commandBuffer,
        ...
        if (auto stencil_resolve_view_state = Get<vvl::ImageView>(stencil_attachment.resolveImageView)) {
            ...
            if (!IsIdentitySwizzle(components)) {
                ...
                skip |= LogError("VUID-VkRenderingInfo-pStencilAttachment-09484",
                                 objlist,
            ...
            if ((image_usage & VK_IMAGE_USAGE_DEPTH_STENCIL_ATTACHMENT_BIT) == 0) {
                const LogObjectList objlist(commandBuffer,
                                            stencil_resolve_view_state->Handle(),
                                            stencil_resolve_view_state->image_state->Handle());
                skip |= LogError("VUID-VkRenderingInfo-pStencilAttachment-09478",
                                 objlist,
                                 rendering_info_loc.dot(Field::pStencilAttachment).dot(Field::resolveImageView),
                                 "image was created with %s.",
                                 string_VkImageUsageFlags(image_usage).c_str());
...
bool CoreChecks::ValidateBeginRenderingDepthAndStencilAttachment(VkCommandBuffer commandBuffer,
    ...
    if (depth_attachment.imageView != VK_NULL_HANDLE && stencil_attachment.imageView != VK_NULL_HANDLE) {
        if (depth_attachment.imageView != stencil_attachment.imageView) {
            ...
            skip |= LogError("VUID-VkRenderingInfo-pDepthAttachment-06085",
                             objlist,
                             rendering_info_loc,
        ...
        if ((phys_dev_props_core12.independentResolveNone == VK_FALSE) &&
            ...
            skip |= LogError("VUID-VkRenderingInfo-pDepthAttachment-06104",
                             commandBuffer,
                             rendering_info_loc,
                             "values of pDepthAttachment->resolveMode (%s) and pStencilAttachment->resolveMode (%s) "
                             "must be identical.",
        ...
        if ((phys_dev_props_core12.independentResolve == VK_FALSE) &&
            ...
            skip |=
                LogError("VUID-VkRenderingInfo-pDepthAttachment-06105",
                         commandBuffer,
                         rendering_info_loc,
    ...
    if (depth_attachment.resolveMode != VK_RESOLVE_MODE_NONE &&
        ...
        if (depth_attachment.resolveImageView != stencil_attachment.resolveImageView) {
            const LogObjectList objlist(
                commandBuffer, depth_attachment.resolveImageView, stencil_attachment.resolveImageView);
            skip |= LogError("VUID-VkRenderingInfo-pDepthAttachment-06086",
                             objlist,
                             rendering_info_loc,
...
// Flags validation error if the associated call is made inside a render pass. The apiName routine should ONLY be called
...
bool CoreChecks::InsideRenderPass(const vvl::CommandBuffer& cb_state, const Location& loc, const char* vuid) const {
    ...
    if (cb_state.activeRenderPass) {
        inside = LogError(vuid,
                          cb_state.Handle(),
                          loc,
                          "It is invalid to issue this call inside an active %s.",
...
bool CoreChecks::PreCallValidateCmdEndRendering(VkCommandBuffer commandBuffer, const ErrorObject& error_obj) const {
    ...
    if (!cb_state->activeRenderPass->UsesDynamicRendering()) {
        skip |= LogError("VUID-vkCmdEndRendering-None-06161",
                         commandBuffer,
                         error_obj.location,
    ...
    if (cb_state->activeRenderPass->use_dynamic_rendering_inherited == true) {
        skip |= LogError("VUID-vkCmdEndRendering-commandBuffer-06162",
                         commandBuffer,
                         error_obj.location,
    ...
    for (const auto& query : cb_state->renderPassQueries) {
        ...
        skip |= LogError("VUID-vkCmdEndRendering-None-06999",
                         objlist,
                         error_obj.location,
                         "query %" PRIu32 " from %s was began in the render pass, but never ended.",
                         query.slot,
                         FormatHandle(query.pool).c_str());
    }
    return skip;
...
bool CoreChecks::ValidateMultisampledRenderToSingleSampleView(
    VkCommandBuffer commandBuffer,
    ...
    if ((image_view_state.samples != VK_SAMPLE_COUNT_1_BIT) &&
        ...
        skip |= LogError("VUID-VkRenderingInfo-imageView-06858",
                         objlist,
    ...
    if (!image_state->image_format_properties.sampleCounts) {
        if (GetPhysicalDeviceImageFormatProperties(
                *image_state, "VUID-VkMultisampledRenderToSingleSampledInfoEXT-pNext-06880", attachment_loc))
            return true;
    }
...
bool CoreChecks::ValidateCmdSubpassState(const vvl::CommandBuffer& cb_state,
                                         const Location& loc,
                                         const char* vuid) const {
    ...
    if (cb_state.IsPrimary() && cb_state.activeSubpassContents == VK_SUBPASS_CONTENTS_SECONDARY_COMMAND_BUFFERS &&
        ...
        skip |=
            LogError(vuid, cb_state.Handle(), loc, "cannot be called in a subpass using secondary command buffers.");
    }
    return skip;
}
...
bool CoreChecks::PreCallValidateCmdNextSubpass(VkCommandBuffer commandBuffer,
                                               VkSubpassContents contents,
                                               const ErrorObject& error_obj) const {
    return ValidateCmdNextSubpass(commandBuffer, error_obj);
}

bool CoreChecks::PreCallValidateCmdNextSubpass2KHR(VkCommandBuffer commandBuffer,
                                                   const VkSubpassBeginInfo* pSubpassBeginInfo,
                                                   const VkSubpassEndInfo* pSubpassEndInfo,
                                                   const ErrorObject& error_obj) const {
...
bool CoreChecks::PreCallValidateCmdNextSubpass2(VkCommandBuffer commandBuffer,
                                                const VkSubpassBeginInfo* pSubpassBeginInfo,
                                                const VkSubpassEndInfo* pSubpassEndInfo,
                                                const ErrorObject& error_obj) const {
    return ValidateCmdNextSubpass(commandBuffer, error_obj);
...
void CoreChecks::PostCallRecordCmdNextSubpass(VkCommandBuffer commandBuffer,
                                              VkSubpassContents contents,
                                              const RecordObject& record_obj) {
...
void CoreChecks::PostCallRecordCmdNextSubpass2KHR(VkCommandBuffer commandBuffer,
                                                  const VkSubpassBeginInfo* pSubpassBeginInfo,
                                                  const VkSubpassEndInfo* pSubpassEndInfo,
                                                  const RecordObject& record_obj) {
...
void CoreChecks::PostCallRecordCmdNextSubpass2(VkCommandBuffer commandBuffer,
                                               const VkSubpassBeginInfo* pSubpassBeginInfo,
                                               const VkSubpassEndInfo* pSubpassEndInfo,
                                               const RecordObject& record_obj) {
...
bool CoreChecks::MatchUsage(uint32_t count,
                            const VkAttachmentReference2* attachments,
                            const VkFramebufferCreateInfo& fbci,
                            VkImageUsageFlagBits usage_flag,
                            const char* vuid,
        ...
        if ((fbci.flags & VK_FRAMEBUFFER_CREATE_IMAGELESS_BIT) == 0) {
            ...
            if (auto view_state = Get<vvl::ImageView>(*image_view)) {
                ...
                if ((creation_usage & usage_flag) == 0) {
                    skip |= LogError(vuid,
                                     device,
                                     create_info_loc.dot(Field::pAttachments, fb_attachment),
                                     "expected usage (%s) conflicts with the image's flags (%s).",
                                     string_VkImageUsageFlagBits(usage_flag),
                                     string_VkImageUsageFlags(creation_usage).c_str());
                }
            }
        } else {
            ...
            if (fbaci != nullptr && fbaci->pAttachmentImageInfos != nullptr &&
                fbaci->attachmentImageInfoCount > fb_attachment) {
...
bool CoreChecks::MsRenderedToSingleSampledValidateFBAttachments(uint32_t count,
                                                                const VkAttachmentReference2* attachments,
                                                                const VkFramebufferCreateInfo& fbci,
                                                                const VkRenderPassCreateInfo2& rpci,
                                                                uint32_t subpass,
    ...
    for (uint32_t attach = 0; attach < count; attach++) {
        ...
        if (renderpass_samples == VK_SAMPLE_COUNT_1_BIT) {
            ...
            if (!(image_state->create_info.flags & VK_IMAGE_CREATE_MULTISAMPLED_RENDER_TO_SINGLE_SAMPLED_BIT_EXT)) {
                skip |=
                    LogError("VUID-VkFramebufferCreateInfo-samples-06881",
                             device,
                             create_info_loc,
                             "Renderpass subpass %" PRIu32 " enables "
                             "multisampled-render-to-single-sampled and attachment %" PRIu32 ", is specified from with "
            ...
            if (!image_state->image_format_properties.sampleCounts) {
                skip |= GetPhysicalDeviceImageFormatProperties(
                    *image_state.get(), "VUID-VkFramebufferCreateInfo-samples-07009", create_info_loc);
            }
            if (!(image_state->image_format_properties.sampleCounts & sample_count)) {
                skip |=
                    LogError("VUID-VkFramebufferCreateInfo-samples-07009",
                             device,
                             create_info_loc,
                             "Renderpass subpass %" PRIu32 " enables "
                             "multisampled-render-to-single-sampled and attachment %" PRIu32 ", is specified from with "
...
bool CoreChecks::PreCallValidateCreateFramebuffer(VkDevice device,
    ...
    if (rpci->attachmentCount != pCreateInfo->attachmentCount) {
        skip |= LogError("VUID-VkFramebufferCreateInfo-attachmentCount-00876",
                         pCreateInfo->renderPass,
                         create_info_loc.dot(Field::attachmentCount),
                         "%" PRIu32 " does not match attachmentCount of %" PRIu32
                         " of %s being used to create Framebuffer.",
                         pCreateInfo->attachmentCount,
                         rpci->attachmentCount,
    ...
    if (framebuffer_attachments_create_info) {
        for (const auto [i, attachment_image_info] :
            ...
            if (attachment_image_info->pNext != nullptr) {
                skip |= LogError("VUID-VkFramebufferAttachmentImageInfo-pNext-pNext",
                                 device,
                                 attachment_image_info_loc.dot(Field::pNext),
                                 "is not NULL.");
            }
            for (const auto [j, view_format] :
                ...
                if (*view_format == VK_FORMAT_UNDEFINED &&
                    rp_state->create_info.pAttachments[i].format != VK_FORMAT_UNDEFINED) {
                    skip |= LogError("VUID-VkFramebufferAttachmentImageInfo-viewFormatCount-09536",
                                     device,
                                     attachment_image_info_loc.dot(Field::pViewFormats, j),
    ...
    if ((pCreateInfo->flags & VK_FRAMEBUFFER_CREATE_IMAGELESS_BIT) == 0) {
        ...
        skip |= ValidateFrameBufferAttachmentsImageless(
            *pCreateInfo, create_info_loc, *rpci, *framebuffer_attachments_create_info);
    }

    if (rp_state->has_multiview_enabled && pCreateInfo->layers != 1) {
        skip |= LogError("VUID-VkFramebufferCreateInfo-renderPass-02531",
                         pCreateInfo->renderPass,
                         create_info_loc.dot(Field::layers),
                         "is %" PRIu32 " but renderPass (%s) was specified with non-zero view masks.",
                         pCreateInfo->layers,
                         FormatHandle(pCreateInfo->renderPass).c_str());
    }
...
bool CoreChecks::ValidateFrameBufferAttachments(const VkFramebufferCreateInfo& create_info,
    ...
    for (uint32_t i = 0; i < create_info.attachmentCount; ++i) {
        ...
        if (ivci.format != rpci.pAttachments[i].format) {
            ...
            skip |= LogError(
                ...
                string_VkFormat(rpci.pAttachments[i].format),
        ...
        if (ici.samples != rpci.pAttachments[i].samples) {
            ...
            skip |=
                LogError("VUID-VkFramebufferCreateInfo-pAttachments-00881",
                         objlist,
                         attachment_loc,
                         "has %s samples that do not match the %s samples used by the corresponding attachment for %s.",
                         string_VkSampleCountFlagBits(ici.samples),
        ...
        if (auto image_data = Get<vvl::Image>(ivci.image)) {
            ...
            skip |= ValidateMemoryIsBoundToImage(LogObjectList(ivci.image),
                                                 *image_data,
                                                 attachment_loc,
        ...
        if (subresource_range.levelCount != 1) {
            LogObjectList objlist(create_info.renderPass, image_views[i], ivci.image);
            skip |= LogError("VUID-VkFramebufferCreateInfo-pAttachments-00883",
                             objlist,
                             attachment_loc,
        ...
        for (uint32_t j = 0; j < rpci.subpassCount; ++j) {
            ...
            if (used_as_input_color_resolve_depth_stencil_attachment) {
                if (static_cast<int32_t>(subresource_range.layerCount) <= highest_view_bit) {
                    LogObjectList objlist(create_info.renderPass, image_views[i], ivci.image);
                    skip |= LogError("VUID-VkFramebufferCreateInfo-renderPass-04536",
                                     objlist,
                                     attachment_loc,
            ...
            if (enabled_features.attachmentFragmentShadingRate) {
                ...
                if (fsr_attachment && fsr_attachment->pFragmentShadingRateAttachment &&
                    ...
                    if (validate_render_area &&
                        ...
                        LogObjectList objlist(create_info.renderPass, image_views[i], ivci.image);
                        skip |=
                            LogError("VUID-VkFramebufferCreateInfo-flags-04539",
                                     objlist,
                                     attachment_loc,
                                     "mip level %" PRIu32 " is used as a "
                                     "fragment shading rate attachment in subpass %" PRIu32 ", but the product of its "
                                     "width (%" PRIu32 ") and the "
                                     "specified shading rate texel width (%" PRIu32 ") are smaller than the "
                                     "corresponding framebuffer width (%" PRIu32 ").",
                                     subresource_range.baseMipLevel,
                                     j,
                                     mip_width,
                                     fsr_attachment->shadingRateAttachmentTexelSize.width,
                                     create_info.width);
                    }
                    if (validate_render_area &&
                        ...
                        skip |= LogError(
                            ...
                            "mip level %" PRIu32 " is used as a "
                            "fragment shading rate attachment in subpass %" PRIu32 ", but the product of its "
                            "height (%" PRIu32 ") and the "
                            "specified shading rate texel height (%" PRIu32 ") are smaller than the corresponding "
                            "framebuffer height (%" PRIu32 ").",
                            subresource_range.baseMipLevel,
                            j,
                            mip_height,
                            fsr_attachment->shadingRateAttachmentTexelSize.height,
                            create_info.height);
                    }
                    if (highest_view_bit >= 0) {
            ...
            if (enabled_features.fragmentDensityMap && api_version >= VK_API_VERSION_1_1) {
                ...
                if (fdm_attachment && fdm_attachment->fragmentDensityMapAttachment.attachment == i) {
                    ...
                    if (rp_state.has_multiview_enabled && layer_count != 1 && layer_count <= highest_view_bit) {
                        LogObjectList objlist(create_info.renderPass, image_views[i], ivci.image);
                        skip |= LogError("VUID-VkFramebufferCreateInfo-renderPass-02746",
                                         objlist,
                                         attachment_loc,
                    ...
                    if (!rp_state.has_multiview_enabled && layer_count != 1) {
                        LogObjectList objlist(create_info.renderPass, image_views[i], ivci.image);
                        skip |=
                            LogError("VUID-VkFramebufferCreateInfo-renderPass-02746",
                                     objlist,
                                     attachment_loc,
                                     "has a layer count (%" PRIu32
            ...
            if (enabled_features.externalFormatResolve && !android_external_format_resolve_null_color_attachment_prop &&
                subpass.pResolveAttachments && subpass.pResolveAttachments[0].attachment == i &&
                ...
                if (it != ahb_ext_resolve_formats_map.end()) {
                    ...
                    if (it->second != color_format) {
                        ...
                        skip |= LogError(
                            ...
                            objlist,
                            attachment_loc,
                            "subpass[%" PRIu32 "].pResolveAttachments[0].attachment %" PRIu32
                            " has externalFormat %" PRIu64
                            " which corresponds to needing a color attachment format of %s, but the format is %s.",
                            j,
                            i,
            ...
            if (fdm_attachment && fdm_attachment->fragmentDensityMapAttachment.attachment != VK_ATTACHMENT_UNUSED) {
                if (fdm_attachment->fragmentDensityMapAttachment.attachment == i) {
                    ...
                    if (mip_width < ceiling_width) {
                        LogObjectList objlist(create_info.renderPass, image_views[i], ivci.image);
                        skip |= LogError("VUID-VkFramebufferCreateInfo-pAttachments-02555",
                                         objlist,
                                         attachment_loc,
                                         "mip level %" PRIu32 " has width "
                    ...
                    if (mip_height < ceiling_height) {
                        ...
                        skip |= LogError(
                            ...
                            "Here are the respective dimensions for attachment #%" PRIu32 ", the ceiling value:\n "
                            "attachment #%" PRIu32 ", framebuffer:\n"
                    ...
                    if ((ici.flags & VK_IMAGE_CREATE_SUBSAMPLED_BIT_EXT) != 0) {
                        LogObjectList objlist(create_info.renderPass, image_views[i], ivci.image);
                        skip |=
                            LogError("VUID-VkFramebufferCreateInfo-pAttachments-02552",
                                     objlist,
                                     attachment_loc,
                    ...
                    LogObjectList objlist(create_info.renderPass, image_views[i], ivci.image);
                    skip |= LogError("VUID-VkFramebufferCreateInfo-renderPass-02553",
                                     objlist,
                                     attachment_loc,
        ...
        if (used_as_input_color_resolve_depth_stencil_attachment) {
            if (mip_width < create_info.width) {
                LogObjectList objlist(create_info.renderPass, image_views[i], ivci.image);
                skip |= LogError("VUID-VkFramebufferCreateInfo-flags-04533",
                                 objlist,
                                 attachment_loc,
            ...
            if (mip_height < create_info.height) {
                LogObjectList objlist(create_info.renderPass, image_views[i], ivci.image);
                skip |= LogError("VUID-VkFramebufferCreateInfo-flags-04534",
                                 objlist,
                                 attachment_loc,
            ...
            if (layerCount < create_info.layers) {
                LogObjectList objlist(create_info.renderPass, image_views[i], ivci.image);
                skip |= LogError("VUID-VkFramebufferCreateInfo-flags-04535",
                                 objlist,
                                 attachment_loc,
                                 "has a layer count (%" PRIu32
        ...
        if (used_as_fragment_shading_rate_attachment && !fsr_non_zero_viewmasks) {
            if (subresource_range.layerCount != 1 && subresource_range.layerCount < create_info.layers) {
                LogObjectList objlist(create_info.renderPass, image_views[i], ivci.image);
                skip |= LogError("VUID-VkFramebufferCreateInfo-flags-04538",
                                 objlist,
                                 attachment_loc,
                                 "has a layer count (%" PRIu32 ") "
                                 "smaller than the corresponding framebuffer layer count (%" PRIu32 ").",
        ...
        if ((ivci.viewType == VK_IMAGE_VIEW_TYPE_2D) || (ivci.viewType == VK_IMAGE_VIEW_TYPE_2D)) {
            ...
            if (image_state && image_state->create_info.imageType == VK_IMAGE_TYPE_3D) {
                if (vkuFormatIsDepthOrStencil(ivci.format)) {
                    LogObjectList objlist(create_info.renderPass, image_views[i], ivci.image);
                    skip |= LogError("VUID-VkFramebufferCreateInfo-pAttachments-00891",
                                     objlist,
                                     attachment_loc,
                                     ...
                                     string_VkImageViewType(ivci.viewType),
                                     FormatHandle(ivci.image).c_str(),
                                     string_VkFormat(ivci.format));
                }
            }
        }
        if (ivci.viewType == VK_IMAGE_VIEW_TYPE_3D) {
            LogObjectList objlist(create_info.renderPass, image_views[i]);
            skip |= LogError("VUID-VkFramebufferCreateInfo-flags-04113",
                             objlist,
                             attachment_loc,
...
bool CoreChecks::ValidateFrameBufferAttachmentsImageless(
    ...
    for (uint32_t i = 0; i < create_info.attachmentCount; ++i) {
        ...
        for (uint32_t j = 0; j < rpci.subpassCount; ++j) {
            ...
            if (enabled_features.attachmentFragmentShadingRate) {
                ...
                if (fsr_attachment && fsr_attachment->pFragmentShadingRateAttachment->attachment == i) {
                    ...
                    if (validate_render_area &&
                        ...
                        skip |= LogError("VUID-VkFramebufferCreateInfo-flags-04543",
                                         create_info.renderPass,
                                         attachment_loc,
                                         "is used as a fragment shading rate attachment in subpass %" PRIu32
                                         ", but the product of its width (%" PRIu32 ") and the "
                    ...
                    if (validate_render_area &&
                        ...
                        skip |= LogError("VUID-VkFramebufferCreateInfo-flags-04544",
                                         create_info.renderPass,
                                         attachment_loc,
                                         "is used as a fragment shading rate attachment in subpass %" PRIu32
            ...
            if (enabled_features.fragmentDensityMap) {
                ...
                if (fdm_attachment && fdm_attachment->fragmentDensityMapAttachment.attachment == i) {
                    ...
                    const uint32_t ceiling_width =
                        vvl::GetQuotientCeil(create_info.width, maxFragmentDensityTexelSize.width);
                    if (aii.width < ceiling_width) {
                        LogObjectList objlist(create_info.renderPass);
                        skip |= LogError("VUID-VkFramebufferCreateInfo-flags-03196",
                                         objlist,
                                         attachment_loc,
                    ...
                    const uint32_t ceiling_height =
                        vvl::GetQuotientCeil(create_info.height, maxFragmentDensityTexelSize.height);
                    if (aii.height < ceiling_height) {
                        LogObjectList objlist(create_info.renderPass);
                        skip |= LogError("VUID-VkFramebufferCreateInfo-flags-03197",
                                         objlist,
                                         attachment_loc,
                                         "is used as a fragment density map attachment in subpass %" PRIu32
        ...
        if (used_as_input_color_resolve_depth_stencil_attachment) {
            if (aii.width < create_info.width) {
                skip |= LogError("VUID-VkFramebufferCreateInfo-flags-04541",
                                 create_info.renderPass,
                                 attachment_loc,
            ...
            if (aii.height < create_info.height) {
                skip |= LogError("VUID-VkFramebufferCreateInfo-flags-04542",
                                 create_info.renderPass,
                                 attachment_loc,
            ...
            if ((rpci.subpassCount == 0) || (rpci.pSubpasses[0].viewMask == 0)) {
                if (aii.layerCount < create_info.layers) {
                    skip |= LogError("VUID-VkFramebufferCreateInfo-renderPass-04546",
                                     create_info.renderPass,
                                     attachment_loc,
        ...
        if (used_as_fragment_shading_rate_attachment && !fsr_non_zero_viewmasks) {
            if (aii.layerCount != 1 && aii.layerCount < create_info.layers) {
                skip |= LogError("VUID-VkFramebufferCreateInfo-flags-04545",
                                 create_info.renderPass,
                                 attachment_loc,
                                 "has a layer count (%" PRIu32
                                 ") smaller than the corresponding framebuffer layer count (%" PRIu32 ").",
    ...
    for (uint32_t i = 0; i < rpci.subpassCount; ++i) {
        ...
        skip |= MatchUsage(subpass.colorAttachmentCount,
                           subpass.pColorAttachments,
                           create_info,
        ...
        skip |= MatchUsage(1,
                           ...
                           create_info_loc);
        skip |= MatchUsage(subpass.inputAttachmentCount,
        ...
        if (enabled_features.attachmentFragmentShadingRate && fragment_shading_rate_attachment_info != nullptr) {
            skip |= MatchUsage(1,
                               fragment_shading_rate_attachment_info->pFragmentShadingRateAttachment,
                               create_info,
        ...
        for (uint32_t i = 0; i < rpci.subpassCount; ++i) {
                        ...
                        skip |= LogError(
                            ...
                            create_info_loc
                                .pNext(Struct::VkFramebufferAttachmentsCreateInfo,
                                       Field::pAttachmentImageInfos,
                                       attachment_index)
                                .dot(Field::layerCount),
                            "is %" PRId32 ", but the view mask for subpass %" PRIu32 " in renderPass (%s) "
            ...
            if (subpass.pDepthStencilAttachment != nullptr) {
                ...
                if (attachment_index != VK_ATTACHMENT_UNUSED) {
                    ...
                    if (layer_count <= highest_view_bit) {
                        skip |=
                            LogError("VUID-VkFramebufferCreateInfo-renderPass-03198",
                                     create_info.renderPass,
                                     create_info_loc
                                         .pNext(Struct::VkFramebufferAttachmentsCreateInfo,
                                                Field::pAttachmentImageInfos,
...
bool CoreChecks::ValidateFrameBufferSubpasses(const VkFramebufferCreateInfo& create_info,
                                              const Location& create_info_loc,
                                              const VkRenderPassCreateInfo2& rpci) const {
    bool skip = false;
    for (uint32_t subpass = 0; subpass < rpci.subpassCount; subpass++) {
        ...
        skip |= MatchUsage(subpass_description.inputAttachmentCount,
        ...
        skip |= MatchUsage(subpass_description.colorAttachmentCount,
        ...
        if (ms_rendered_to_single_sampled && ms_rendered_to_single_sampled->multisampledRenderToSingleSampledEnable) {
            ...
            if (subpass_description.pDepthStencilAttachment) {
                skip |=
                    MsRenderedToSingleSampledValidateFBAttachments(1,
                                                                   subpass_description.pDepthStencilAttachment,
                                                                   create_info,
                                                                   rpci,
                                                                   subpass,
...
bool CoreChecks::PreCallValidateDestroyFramebuffer(VkDevice device,
    ...
    if (auto framebuffer_state = Get<vvl::Framebuffer>(framebuffer)) {
        skip |= ValidateObjectNotInUse(
            framebuffer_state.get(), error_obj.location, "VUID-vkDestroyFramebuffer-framebuffer-00892");
    }
    return skip;
}

bool CoreChecks::ValidateInheritanceInfoFramebuffer(VkCommandBuffer primaryBuffer,
...
bool CoreChecks::ValidateRenderingAttachmentLocations(const VkRenderingAttachmentLocationInfo& location_info,
    ...
    if (location_info.pColorAttachmentLocations) {
        ...
        for (uint32_t i = 0; i < location_info.colorAttachmentCount; ++i) {
            ...
            const Location loc =
                loc_info.dot(Struct::VkRenderingAttachmentLocationInfo, Field::pColorAttachmentLocations, i);

            if (!enabled_features.dynamicRenderingLocalRead && location != i) {
                skip |= LogError("VUID-VkRenderingAttachmentLocationInfo-dynamicRenderingLocalRead-09512",
                                 objlist,
                                 loc,
            ...
            if (unique.find(location) != unique.end()) {
                skip |=
                    LogError("VUID-VkRenderingAttachmentLocationInfo-pColorAttachmentLocations-09513",
                             objlist,
                             loc,
                             "(%" PRIu32 ") has same value as pColorAttachmentLocations[%" PRIu32 "] (%" PRIu32 ").",
                             location,
                             unique[location],
                             location);
            } else
            ...
            if (location >= phys_dev_props.limits.maxColorAttachments) {
                skip |= LogError("VUID-VkRenderingAttachmentLocationInfo-pColorAttachmentLocations-09515",
                                 objlist,
                                 loc,
                                 "(%" PRIu32 ") is greater than maxColorAttachments (%" PRIu32 ")",
                                 location,
    ...
    if (location_info.colorAttachmentCount > phys_dev_props.limits.maxColorAttachments) {
        skip |= LogError("VUID-VkRenderingAttachmentLocationInfo-colorAttachmentCount-09514",
                         objlist,
...
bool CoreChecks::PreCallValidateCmdSetRenderingAttachmentLocations(
    VkCommandBuffer commandBuffer,
    ...
    if (!enabled_features.dynamicRenderingLocalRead) {
        skip |= LogError("VUID-vkCmdSetRenderingAttachmentLocations-dynamicRenderingLocalRead-09509",
                         commandBuffer,
                         loc_info,
    ...
    if (!rp_state.UsesDynamicRendering()) {
        ...
        skip |= LogError("VUID-vkCmdSetRenderingAttachmentLocations-commandBuffer-09511",
                         objlist,
                         loc_info,
    ...
    if (pLocationInfo->colorAttachmentCount != rp_state.dynamic_rendering_begin_rendering_info.colorAttachmentCount) {
        const LogObjectList objlist(commandBuffer, rp_state.VkHandle());
        skip |= LogError("VUID-vkCmdSetRenderingAttachmentLocations-pLocationInfo-09510",
                         objlist,
...
bool CoreChecks::PreCallValidateCmdSetRenderingAttachmentLocationsKHR(
    VkCommandBuffer commandBuffer,
...
bool CoreChecks::ValidateRenderingInputAttachmentIndices(const VkRenderingInputAttachmentIndexInfo& index_info,
        ...
        if (index_info.pDepthInputAttachmentIndex) {
            if (*index_info.pDepthInputAttachmentIndex != VK_ATTACHMENT_UNUSED) {
                skip |= LogError(
                    ...
                    "is %" PRIu32 " but must be VK_ATTACHMENT_UNUSED",
    ...
    if (index_info.pColorAttachmentInputIndices) {
        ...
        for (uint32_t i = 0; i < index_info.colorAttachmentCount; ++i) {
            ...
            if (unique.find(index) != unique.end()) {
                skip |= LogError(
                    ...
                    index,
                    unique[index],
                    index_info.pColorAttachmentInputIndices[unique[index]]);
            } else
                unique[index] = i;
        }
        if (index_info.pDepthInputAttachmentIndex && *index_info.pDepthInputAttachmentIndex != VK_ATTACHMENT_UNUSED &&
            ...
            const Location loc =
                loc_info.dot(Struct::VkRenderingInputAttachmentIndexInfo, Field::pDepthInputAttachmentIndex, 0);
            skip |=
                LogError("VUID-VkRenderingInputAttachmentIndexInfo-pColorAttachmentInputIndices-09523",
                         objlist,
                         loc,
                         "(%" PRIu32 ") has same value as in pColorAttachmentInputIndices[%" PRIu32 "] (%" PRIu32 ").",
                         *index_info.pDepthInputAttachmentIndex,
        ...
        if (index_info.pStencilInputAttachmentIndex &&
            *index_info.pStencilInputAttachmentIndex != VK_ATTACHMENT_UNUSED &&
            unique.find(*index_info.pStencilInputAttachmentIndex) != unique.end()) {
            const Location loc =
                loc_info.dot(Struct::VkRenderingInputAttachmentIndexInfo, Field::pStencilInputAttachmentIndex, 0);
            skip |=
                LogError("VUID-VkRenderingInputAttachmentIndexInfo-pColorAttachmentInputIndices-09524",
                         objlist,
                         loc,
                         "(%" PRIu32 ") has same value as in pColorAttachmentInputIndices[%" PRIu32 "] (%" PRIu32 ").",
                         *index_info.pStencilInputAttachmentIndex,
                         unique[*index_info.pStencilInputAttachmentIndex],
    ...
    if (index_info.colorAttachmentCount > phys_dev_props.limits.maxColorAttachments) {
        ...
        skip |= LogError("VUID-VkRenderingInputAttachmentIndexInfo-colorAttachmentCount-09525",
                         objlist,
                         loc,
                         "(%" PRIu32 ") is greater than maxColorAttachments (%" PRIu32 ").",
...
bool CoreChecks::PreCallValidateCmdSetRenderingInputAttachmentIndices(
    VkCommandBuffer commandBuffer,
    ...
    if (!enabled_features.dynamicRenderingLocalRead) {
        skip |= LogError("VUID-vkCmdSetRenderingInputAttachmentIndices-dynamicRenderingLocalRead-09516",
                         commandBuffer,
                         error_obj.location,
                         "dynamicRenderingLocalRead was not enabled.");
    }
    ...
    if (!rp_state.UsesDynamicRendering()) {
        ...
        skip |= LogError("VUID-vkCmdSetRenderingInputAttachmentIndices-commandBuffer-09518",
                         objlist,
                         error_obj.location,
                         "vkCmdBeginRendering was not called.");
    }
    if (pLocationInfo->colorAttachmentCount != rp_state.dynamic_rendering_begin_rendering_info.colorAttachmentCount) {
        const LogObjectList objlist(commandBuffer, rp_state.VkHandle());
        const Location loc =
            error_obj.location.dot(Struct::VkRenderingInputAttachmentIndexInfo, Field::colorAttachmentCount);

        skip |= LogError("VUID-vkCmdSetRenderingInputAttachmentIndices-pInputAttachmentIndexInfo-09517",
                         objlist,
                         loc,
