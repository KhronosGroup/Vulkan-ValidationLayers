    ...
    for (uint32_t set_idx = 0; set_idx < setCount; set_idx++)
    {
        ...
        if (auto descriptor_set = Get<vvl::DescriptorSet>(pDescriptorSets[set_idx]))
        {
            ...
            if (set_dynamic_descriptor_count)
            {
                ...
                { // Validate dynamic offsets and Dynamic Offset Minimums
                                   ...
                                   const vvl::DrawDispatchVuid& vuids) const
{
    ...
    for (const auto& [binding_index, desc_set_reqs] : binding_req_map)
    {
        ...
        if (!binding)
        { //  End at construction is the condition for an invalid binding.
...
bool CoreChecks::PreCallValidateGetBufferOpaqueCaptureDescriptorDataEXT(
    VkDevice device, const VkBufferCaptureDescriptorDataInfoEXT* pInfo, void* pData, const ErrorObject& error_obj) const
{
    bool skip = false;
