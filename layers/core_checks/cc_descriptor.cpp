...
bool CoreChecks::ImmutableSamplersAreEqual(const VkDescriptorSetLayoutBinding& b1,
...
bool CoreChecks::VerifySetLayoutCompatibility(const vvl::DescriptorSetLayout& layout_dsl,
                                              const vvl::DescriptorSetLayout& bound_dsl,
                                              std::string& error_msg) const {
    // Short circuit the detailed check.
    if (layout_dsl.IsCompatible(&bound_dsl))
        return true;
    ...
    if (layout_ds_layout_def->GetTotalDescriptorCount() != bound_ds_layout_def->GetTotalDescriptorCount()) {
        ...
        error_str << FormatHandle(layout_dsl_handle) << " from pipeline layout has "
                  ...
                  << FormatHandle(bound_dsl_handle) << ", which is bound, has " << bound_total_count
                  << " total descriptors.";
    ...
    for (const auto& layout_binding : layout_ds_layout_def->GetBindings()) {
        ...
        if (layout_binding.descriptorCount != bound_binding->descriptorCount) {
            ...
            error_str << "Binding " << layout_binding.binding << " for " << FormatHandle(layout_dsl_handle)
                      << " from pipeline layout has a descriptorCount of " << layout_binding.descriptorCount
            ...
            error_msg = "Immutable samplers from binding " + std::to_string(layout_binding.binding) +
                        " in pipeline layout " + FormatHandle(layout_dsl_handle) +
    ...
    if (bound_layout_flags != ds_layout_flags) {
        ...
        for (i = 0; i < ds_layout_flags.size(); i++) {
            if (ds_layout_flags[i] != bound_layout_flags[i])
                break;
        }
        error_str << FormatHandle(layout_dsl_handle)
                  << " from pipeline layout does not have the same binding flags at binding " << i << " ( "
                  << string_VkDescriptorBindingFlags(ds_layout_flags[i]) << " ) as " << FormatHandle(bound_dsl_handle)
    ...
    //
    // There are exceptions where this is valid, example is the pImmutableSamplers pointing to 2 different handles are
    // not hashed. It would be ugly to pass in the state object when hashing with hash_utils, so we just defer until
    // here.
    assert(exception);
    return true;
}
...
bool CoreChecks::VerifySetLayoutCompatibility(
    ...
    if (layout_node) {
        ...
        // It's possible the DSL is null when creating a graphics pipeline library, in which case we can't verify
        // compatibility here.
        return true;
    }
}

bool CoreChecks::VerifySetLayoutCompatibility(const vvl::PipelineLayout& layout_a,
                                              const vvl::PipelineLayout& layout_b,
                                              std::string& error_msg) const {
...
bool CoreChecks::VerifySetLayoutCompatibilityUnion(const vvl::PipelineLayout& layout,
                                                   const vvl::PipelineLayout& pre_raster_layout,
                                                   const vvl::PipelineLayout& fs_layout,
                                                   std::string& error_msg) const {
    ...
    uint32_t num_sets =
        static_cast<uint32_t>(std::min(pre_raster_layout.set_layouts.size(), fs_layout.set_layouts.size()));
...
bool CoreChecks::ValidateCmdBindDescriptorSets(const vvl::CommandBuffer& cb_state,
    ...
    for (uint32_t set_idx = 0; set_idx < setCount; set_idx++) {
        ...
        if (auto descriptor_set = Get<vvl::DescriptorSet>(pDescriptorSets[set_idx])) {
            ...
            if (!VerifySetLayoutCompatibility(*descriptor_set,
                ...
                skip |= LogError(vuid,
                                 ...
                                 "descriptorSetLayout at index %" PRIu32 " of "
                                 "%s due to: %s.",
            ...
            if (dsl->GetCreateFlags() & VK_DESCRIPTOR_SET_LAYOUT_CREATE_DESCRIPTOR_BUFFER_BIT_EXT) {
                ...
                skip |= LogError(vuid,
                                 objlist,
                                 set_loc,
                                 "was allocated from VkDescriptorSetLayout with %s flags.",
            ...
            if (set_dynamic_descriptor_count) {
                ...
                if ((total_dynamic_descriptors + set_dynamic_descriptor_count) > dynamicOffsetCount) {
                    ...
                    skip |= LogError(vuid,
                                     ...
                                     "pDynamicOffsets array. There must be one dynamic offset for each dynamic "
                                     "descriptor being bound.",
                                     FormatHandle(pDescriptorSets[set_idx]).c_str(),
                                     descriptor_set->GetDynamicDescriptorCount(),
                                     (dynamicOffsetCount - total_dynamic_descriptors));
                    // Set the number found to the maximum to prevent duplicate messages, or subsquent descriptor sets
                    // from testing against the "short tail" we're skipping below.
                    ...
                    for (uint32_t i = 0; i < binding_count; i++) {
                        ...
                        for (uint32_t j = 0; j < descriptorCount; j++) {
                            ...
                            if ((binding->descriptorType == VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER_DYNAMIC) &&
                                ...
                                skip |= LogError(vuid,
                                                 cb_state.Handle(),
                                                 loc.dot(Field::pDynamicOffsets, cur_dyn_offset),
                                                 "is %" PRIu32 ", but must be a multiple of "
                            ...
                            if (descriptor->GetClass() == vvl::DescriptorClass::GeneralBuffer) {
                                ...
                                if ((bound_range == VK_WHOLE_SIZE) && (offset > 0)) {
                                    const LogObjectList objlist(
                                        cb_state.Handle(), pDescriptorSets[set_idx], buffer_descriptor->GetBuffer());
                                    ...
                                    skip |= LogError(
                                        ...
                                        objlist,
                                        loc.dot(Field::pDynamicOffsets, cur_dyn_offset),
                                        "is %" PRIu32 ", but must be zero since "
            ...
            const LogObjectList objlist(cb_state.Handle(), pDescriptorSets[set_idx]);
            const char* vuid = is_2 ? "VUID-VkBindDescriptorSetsInfo-pDescriptorSets-06563"
    ...
    if (total_dynamic_descriptors != dynamicOffsetCount) {
        ...
        skip |= LogError(vuid,
                         cb_state.Handle(),
                         loc,
                         "Attempting to bind %" PRIu32 " descriptorSets with %" PRIu32 " dynamic descriptors, but "
                         "dynamicOffsetCount is %" PRIu32 ". It should "
                         "exactly match the number of dynamic descriptors.",
                         setCount,
                         total_dynamic_descriptors,
                         dynamicOffsetCount);
    }
    ...
    if ((firstSet + setCount) > static_cast<uint32_t>(pipeline_layout->set_layouts.size())) {
        const char* vuid =
            is_2 ? "VUID-VkBindDescriptorSetsInfo-firstSet-00360" : "VUID-vkCmdBindDescriptorSets-firstSet-00360";
        skip |= LogError(vuid,
                         cb_state.Handle(),
                         loc,
                         "Sum of firstSet (%" PRIu32 ") and descriptorSetCount (%" PRIu32 ") is greater than "
                         "VkPipelineLayoutCreateInfo::setLayoutCount "
                         "(%zu) when pipeline layout was created",
                         firstSet,
                         setCount,
                         pipeline_layout->set_layouts.size());
    }

    return skip;
}

bool CoreChecks::PreCallValidateCmdBindDescriptorSets(VkCommandBuffer commandBuffer,
    ...
    skip |= ValidateCmdBindDescriptorSets(*cb_state,
                                          layout,
                                          firstSet,
                                          setCount,
                                          pDescriptorSets,
...
bool CoreChecks::ValidateDescriptorSetLayoutBindingFlags(const VkDescriptorSetLayoutCreateInfo& create_info,
                                                         uint32_t max_binding,
                                                         uint32_t* update_after_bind,
                                                         const Location& create_info_loc) const {
    bool skip = false;
    const auto* flags_info =
        vku::FindStructInPNextChain<VkDescriptorSetLayoutBindingFlagsCreateInfo>(create_info.pNext);
    if (!flags_info) {
        return skip;
    }
    if (flags_info->bindingCount != 0 && flags_info->bindingCount != create_info.bindingCount) {
        skip |=
            LogError("VUID-VkDescriptorSetLayoutBindingFlagsCreateInfo-bindingCount-03002",
                     device,
                     create_info_loc.pNext(Struct::VkDescriptorSetLayoutBindingFlagsCreateInfo, Field::bindingCount),
                     "(%" PRIu32 ") is different from pCreateInfo->bindingCount (%" PRIu32 ").",
                     flags_info->bindingCount,
    ...
    for (uint32_t i = 0; i < create_info.bindingCount; ++i) {
        ...
        if (flags_info->pBindingFlags[i] & VK_DESCRIPTOR_BINDING_UPDATE_AFTER_BIND_BIT) {
            *update_after_bind = i;
            if ((create_info.flags & VK_DESCRIPTOR_SET_LAYOUT_CREATE_UPDATE_AFTER_BIND_POOL_BIT) == 0) {
                skip |= LogError("VUID-VkDescriptorSetLayoutCreateInfo-flags-03000",
                                 device,
                                 binding_flags_loc,
            ...
            if (binding_info.descriptorType == VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER &&
                ...
                skip |= LogError("VUID-VkDescriptorSetLayoutBindingFlagsCreateInfo-"
            ...
            if ((binding_info.descriptorType == VK_DESCRIPTOR_TYPE_INPUT_ATTACHMENT ||
                ...
                skip |= LogError("VUID-VkDescriptorSetLayoutBindingFlagsCreateInfo-None-03011",
                                 device,
                                 binding_flags_loc,
                                 "includes VK_DESCRIPTOR_BINDING_UPDATE_AFTER_BIND_BIT but pBindings[%" PRIu32
                                 "].descriptorType is %s.",
            ...
            if ((binding_info.descriptorType == VK_DESCRIPTOR_TYPE_ACCELERATION_STRUCTURE_KHR ||
                ...
                skip |= LogError("VUID-VkDescriptorSetLayoutBindingFlagsCreateInfo-"
                                 ...
                                 binding_flags_loc,
                                 "includes VK_DESCRIPTOR_BINDING_UPDATE_AFTER_BIND_BIT but pBindings[%" PRIu32
                                 "].descriptorType is %s, but the "
                                 "descriptorBindingAccelerationStructureUpdateAfterBind was not enabled.",
                                 i,
                                 string_VkDescriptorType(binding_info.descriptorType));
            }
        }
        ...
        if (flags_info->pBindingFlags[i] & VK_DESCRIPTOR_BINDING_PARTIALLY_BOUND_BIT) {
            if (!enabled_features.descriptorBindingPartiallyBound) {
                skip |=
                    LogError("VUID-VkDescriptorSetLayoutBindingFlagsCreateInfo-descriptorBindingPartiallyBound-03013",
                             device,
                             binding_flags_loc,
        ...
        if (flags_info->pBindingFlags[i] & VK_DESCRIPTOR_BINDING_VARIABLE_DESCRIPTOR_COUNT_BIT) {
            if (binding_info.binding != max_binding) {
                skip |= LogError("VUID-VkDescriptorSetLayoutBindingFlagsCreateInfo-pBindingFlags-03004",
                                 device,
                                 binding_flags_loc,
            ...
            if ((binding_info.descriptorType == VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER_DYNAMIC) ||
                (binding_info.descriptorType == VK_DESCRIPTOR_TYPE_STORAGE_BUFFER_DYNAMIC)) {
                skip |= LogError("VUID-VkDescriptorSetLayoutBindingFlagsCreateInfo-pBindingFlags-03015",
                                 device,
                                 binding_flags_loc,
        ...
        if (push_descriptor_set &&
            (flags_info->pBindingFlags[i] &
             (VK_DESCRIPTOR_BINDING_UPDATE_AFTER_BIND_BIT | VK_DESCRIPTOR_BINDING_UPDATE_UNUSED_WHILE_PENDING_BIT |
              VK_DESCRIPTOR_BINDING_VARIABLE_DESCRIPTOR_COUNT_BIT))) {
            skip |= LogError("VUID-VkDescriptorSetLayoutBindingFlagsCreateInfo-flags-03003",
                             device,
                             binding_flags_loc,
...
bool CoreChecks::ValidateDescriptorSetLayoutCreateInfo(const VkDescriptorSetLayoutCreateInfo& create_info,
    ...
    for (uint32_t i = 0; i < create_info.bindingCount; ++i) {
        ...
        if (!bindings.insert(binding_info.binding).second) {
            skip |= LogError("VUID-VkDescriptorSetLayoutCreateInfo-binding-00279",
                             device,
                             binding_loc.dot(Field::binding),
                             "is duplicated at pBindings[%" PRIu32 "].binding.",
                             binding_info.binding);
        }
        ...
        if (binding_info.descriptorType == VK_DESCRIPTOR_TYPE_INLINE_UNIFORM_BLOCK) {
            if (!enabled_features.inlineUniformBlock) {
                ...
                skip |= LogError("VUID-VkDescriptorSetLayoutCreateInfo-flags-02208",
                                 device,
                                 binding_loc.dot(Field::descriptorType),
                ...
                if ((binding_info.descriptorCount % 4) != 0) {
                    skip |= LogError("VUID-VkDescriptorSetLayoutBinding-descriptorType-02209",
                                     device,
                ...
                if ((binding_info.descriptorCount > phys_dev_props_core13.maxInlineUniformBlockSize) &&
                    ...
                    skip |= LogError("VUID-VkDescriptorSetLayoutBinding-descriptorType-08004",
                                     device,
                                     binding_loc.dot(Field::descriptorCount),
                                     "(%" PRIu32
                                     ") but must be less than or equal to maxInlineUniformBlockSize (%" PRIu32 "), but "
                                     "pCreateInfo->flags is %s.",
                                     binding_info.descriptorCount,
                                     phys_dev_props_core13.maxInlineUniformBlockSize,
                                     string_VkDescriptorSetLayoutCreateFlags(create_info.flags).c_str());
                }
            }
            ...
            if (push_descriptor_set) {
                skip |= LogError("VUID-VkDescriptorSetLayoutCreateInfo-flags-00280",
                                 device,
                                 binding_loc.dot(Field::descriptorType),
        ...
        if ((binding_info.descriptorType == VK_DESCRIPTOR_TYPE_SAMPLER ||
            ...
            for (uint32_t j = 0; j < binding_info.descriptorCount; j++) {
                ...
                if (sampler_state && (sampler_state->create_info.borderColor == VK_BORDER_COLOR_INT_CUSTOM_EXT ||
                    ...
                    skip |= LogError("VUID-VkDescriptorSetLayoutBinding-pImmutableSamplers-04009",
                                     device,
        ...
        if (binding_info.descriptorType == VK_DESCRIPTOR_TYPE_MUTABLE_EXT &&
            ...
            skip |= LogError("VUID-VkDescriptorSetLayoutBinding-descriptorType-04605",
                             device,
                             binding_loc.dot(Field::descriptorType),
        ...
        if (create_info.flags & VK_DESCRIPTOR_SET_LAYOUT_CREATE_EMBEDDED_IMMUTABLE_SAMPLERS_BIT_EXT) {
            if (binding_info.descriptorType != VK_DESCRIPTOR_TYPE_SAMPLER) {
                skip |= LogError("VUID-VkDescriptorSetLayoutBinding-flags-08005",
                                 device,
                                 binding_loc.dot(Field::descriptorType),
            ...
            if (binding_info.descriptorCount > 1) {
                skip |= LogError("VUID-VkDescriptorSetLayoutBinding-flags-08006",
                                 device,
                                 binding_loc.dot(Field::descriptorCount),
                                 "is %" PRIu32 " but pCreateInfo->flags includes "
                                 "VK_DESCRIPTOR_SET_LAYOUT_CREATE_EMBEDDED_IMMUTABLE_SAMPLERS_BIT_EXT.",
                                 binding_info.descriptorCount);
            }

            if ((binding_info.descriptorCount == 1) && (binding_info.pImmutableSamplers == nullptr)) {
                skip |= LogError("VUID-VkDescriptorSetLayoutBinding-flags-08007",
                                 device,
                                 binding_loc.dot(Field::descriptorCount),
    ...
    if (update_after_bind < create_info.bindingCount) {
        if (uniform_buffer_dynamic < create_info.bindingCount) {
            skip |= LogError("VUID-VkDescriptorSetLayoutCreateInfo-descriptorType-03001",
                             device,
                             create_info_loc.dot(Field::pBindings, update_after_bind),
                             "has VK_DESCRIPTOR_BINDING_UPDATE_AFTER_BIND_BIT "
                             "flag, but pBindings[%" PRIu32
                             "] has descriptor type VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER_DYNAMIC.",
        ...
        if (storage_buffer_dynamic < create_info.bindingCount) {
            skip |= LogError("VUID-VkDescriptorSetLayoutCreateInfo-descriptorType-03001",
                             ...
                             "has VK_DESCRIPTOR_BINDING_UPDATE_AFTER_BIND_BIT "
                             "flag, but pBindings[%" PRIu32
                             "] has descriptor type VK_DESCRIPTOR_TYPE_STORAGE_BUFFER_DYNAMIC.",
                             storage_buffer_dynamic);
        }
    }

    if ((push_descriptor_set) && (total_descriptors > phys_dev_props_core14.maxPushDescriptors)) {
        skip |= LogError("VUID-VkDescriptorSetLayoutCreateInfo-flags-00281",
                         device,
                         create_info_loc.dot(Field::flags),
                         "contains VK_DESCRIPTOR_SET_LAYOUT_CREATE_PUSH_DESCRIPTOR_BIT, but the total descriptor count "
                         "in layout (%" PRIu64 ") must not be greater than maxPushDescriptors (%" PRIu32 ").",
                         total_descriptors,
                         phys_dev_props_core14.maxPushDescriptors);
    }

    return skip;
}

bool CoreChecks::PreCallValidateCreateDescriptorSetLayout(VkDevice device,
...
bool CoreChecks::PreCallValidateGetDescriptorSetLayoutSupport(VkDevice device,
                                                              const VkDescriptorSetLayoutCreateInfo* pCreateInfo,
...
bool CoreChecks::ValidateDrawState(const vvl::DescriptorSet& descriptor_set,
                                   uint32_t set_index,
    ...
    const VkFramebuffer framebuffer =
        cb_state.activeFramebuffer ? cb_state.activeFramebuffer->VkHandle() : VK_NULL_HANDLE;
    // NOTE: GPU-AV needs non-const state objects to do lazy updates of descriptor state of only the dynamically used
    // descriptors, via the non-const version of ValidateBindingDynamic(), this code uses the const path only even it
    // gives up non-const versions of its state objects here.
    const vvl::DescriptorValidator desc_val(const_cast<CoreChecks&>(*this),
                                            const_cast<vvl::CommandBuffer&>(cb_state),
                                            const_cast<vvl::DescriptorSet&>(descriptor_set),
                                            set_index,
                                            framebuffer,
                                            loc);
    ...
    for (const auto& [binding_index, desc_set_reqs] : binding_req_map) {
        ...
        if (!binding) { //  End at construction is the condition for an invalid binding.
            ...
            result |= LogError(vuids.descriptor_buffer_bit_set_08114,
                               set,
                               loc,
                               "%s %s is invalid.",
...
//  descriptor updates and verify that for any binding boundaries that are crossed, the next binding(s) are all
//  consistent Consistency means that their type, stage flags, and whether or not they use immutable samplers matches
bool CoreChecks::VerifyUpdateConsistency(const vvl::DescriptorSet& set,
                                         uint32_t binding,
                                         uint32_t offset,
    ...
    while (!skip && update_count) {
        ...
        if (offset > 0) {
            // index_range.start + offset is which descriptor is needed to update. If it > index_range.end, it means the
            // descriptor isn't in this binding, maybe in next binding.
        ...
        if (update_count) {
            // Starting offset is beyond the current binding. Check consistency, update counters and advance to the next
            // binding, looking for the start point. All bindings (even those skipped) must be consistent with the
            // update and with the original binding.
            offset = 0;
            ++current_iter;
            // Verify next consecutive binding matches type, stage flags & immutable sampler use and if AtEnd
            if (current_iter == set.end() || !orig_binding.IsConsistent(**current_iter)) {
    ...
    if (skip) {
        ...
        error_str << " binding #" << orig_binding.binding << " with #" << update_count
                  << " descriptors being updated but this update oversteps the bounds of this binding and the next "
                     "binding is "
        ...
        const char* vuid =
            is_copy ? "VUID-VkCopyDescriptorSet-srcSet-00349" : "VUID-VkWriteDescriptorSet-dstArrayElement-00321";
...
bool CoreChecks::ValidateCopyUpdate(const VkCopyDescriptorSet& update, const Location& copy_loc) const {
    ...
    uint32_t src_start_idx = 0;
    {
        if (src_layout->Destroyed()) {
            ...
            return LogError("VUID-VkCopyDescriptorSet-srcSet-parameter",
                            objlist,
                            copy_loc.dot(Field::srcSet),
                            "(%s) has been destroyed.",
                            FormatHandle(src_layout->Handle()).c_str());
        }
        if (!src_set->HasBinding(update.srcBinding)) {
            const LogObjectList objlist(update.srcSet, src_layout->Handle());
            return LogError("VUID-VkCopyDescriptorSet-srcBinding-00345",
                            objlist,
                            copy_loc.dot(Field::srcBinding),
                            "(%" PRIu32 ") does not exist in %s.",
                            update.srcBinding,
                            FormatHandle(src_set->Handle()).c_str());
        }
        ...
        if ((src_start_idx + update.descriptorCount) > src_set->GetTotalDescriptorCount()) {
            const LogObjectList objlist(update.srcSet, src_layout->Handle());
            skip |= LogError("VUID-VkCopyDescriptorSet-srcArrayElement-00346",
                             objlist,
                             copy_loc.dot(Field::srcArrayElement),
                             "(%" PRIu32 ") + descriptorCount (%" PRIu32 ") + offset index (%" PRIu32
                             ") is larger than the total descriptors count (%" PRIu32
                             ") for the binding at srcBinding (%" PRIu32 ").",
                             update.srcArrayElement,
                             update.descriptorCount,
                             src_set->GetGlobalIndexRangeFromBinding(update.srcBinding).start,
                             src_set->GetTotalDescriptorCount(),
                             update.srcBinding);
        }
    }
    ...
    uint32_t dst_start_idx = 0;
    {
        if (dst_layout->Destroyed()) {
            ...
            return LogError("VUID-VkCopyDescriptorSet-dstSet-parameter",
                            objlist,
        ...
        if (!dst_layout->HasBinding(update.dstBinding)) {
            const LogObjectList objlist(update.dstSet, dst_layout->Handle());
            return LogError("VUID-VkCopyDescriptorSet-dstBinding-00347",
                            objlist,
                            copy_loc.dot(Field::dstBinding),
                            "(%" PRIu32 ") does not exist in %s.",
                            update.dstBinding,
                            FormatHandle(dst_set->Handle()).c_str());
        }
        ...
        if ((dst_start_idx + update.descriptorCount) > dst_layout->GetTotalDescriptorCount()) {
            const LogObjectList objlist(update.dstSet, dst_layout->Handle());
            skip |= LogError("VUID-VkCopyDescriptorSet-dstArrayElement-00348",
                             objlist,
                             copy_loc.dot(Field::dstArrayElement),
                             "(%" PRIu32 ") + descriptorCount (%" PRIu32 ") + offset index (%" PRIu32
                             ") is larger than the total descriptors count (%" PRIu32
                             ") for the binding at dstBinding (%" PRIu32 ").",
                             update.dstArrayElement,
                             update.descriptorCount,
                             dst_set->GetGlobalIndexRangeFromBinding(update.dstBinding).start,
                             dst_set->GetTotalDescriptorCount(),
                             update.dstBinding);
        }
    }
    ...
    skip |= VerifyUpdateConsistency(
        *src_set, update.srcBinding, update.srcArrayElement, update.descriptorCount, true, copy_loc.dot(Field::dstSet));
    skip |= VerifyUpdateConsistency(
        *dst_set, update.dstBinding, update.dstArrayElement, update.descriptorCount, true, copy_loc.dot(Field::srcSet));
...
bool CoreChecks::ValidateCopyUpdateDescriptorSetLayoutFlags(const VkCopyDescriptorSet& update,
    ...
    if ((src_flags & VK_DESCRIPTOR_SET_LAYOUT_CREATE_UPDATE_AFTER_BIND_POOL_BIT) &&
        ...
        skip |= LogError("VUID-VkCopyDescriptorSet-srcSet-01918",
                         objlist,
                         copy_loc.dot(Field::srcSet),
    ...
    if (!(src_flags & (VK_DESCRIPTOR_SET_LAYOUT_CREATE_UPDATE_AFTER_BIND_POOL_BIT |
        ...
        (dst_flags & VK_DESCRIPTOR_SET_LAYOUT_CREATE_UPDATE_AFTER_BIND_POOL_BIT)) {
        const LogObjectList objlist(update.srcSet, update.dstSet, src_layout.Handle(), dst_layout.Handle());
        skip |= LogError("VUID-VkCopyDescriptorSet-srcSet-04885",
                         objlist,
                         copy_loc.dot(Field::srcSet),
                         "layout was created with %s, but dstSet layout was created with %s.",
                         string_VkDescriptorSetLayoutCreateFlags(src_flags).c_str(),
...
bool CoreChecks::ValidateCopyUpdateDescriptorPoolFlags(const VkCopyDescriptorSet& update,
                                                       const vvl::DescriptorSet& src_set,
                                                       const vvl::DescriptorSet& dst_set,
                                                       const Location& copy_loc) const {
    bool skip = false;
    ...
    if ((src_flags & VK_DESCRIPTOR_POOL_CREATE_UPDATE_AFTER_BIND_BIT) &&
        ...
        skip |= LogError("VUID-VkCopyDescriptorSet-srcSet-01920",
                         objlist,
                         copy_loc.dot(Field::srcSet),
                         "descriptor pool was created with %s, but dstSet descriptor pool was created with %s.",
                         string_VkDescriptorPoolCreateFlags(src_flags).c_str(),
                         string_VkDescriptorPoolCreateFlags(dst_flags).c_str());
    }
    ...
    if (!(src_flags &
        ...
        (dst_flags & VK_DESCRIPTOR_POOL_CREATE_UPDATE_AFTER_BIND_BIT)) {
        const LogObjectList objlist(update.srcSet, update.dstSet, src_pool->Handle(), dst_pool->Handle());
        skip |= LogError("VUID-VkCopyDescriptorSet-srcSet-04887",
                         objlist,
                         copy_loc.dot(Field::srcSet),
                         "descriptor pool was created with %s, but dstSet descriptor pool was created with %s.",
                         string_VkDescriptorPoolCreateFlags(src_flags).c_str(),
...
bool CoreChecks::ValidateCopyUpdateDescriptorTypes(const VkCopyDescriptorSet& update,
                                                   const vvl::DescriptorSet& src_set,
                                                   const vvl::DescriptorSet& dst_set,
                                                   const vvl::DescriptorSetLayout& src_layout,
                                                   const vvl::DescriptorSetLayout& dst_layout,
    ...
    if (src_type != VK_DESCRIPTOR_TYPE_MUTABLE_EXT && dst_type != VK_DESCRIPTOR_TYPE_MUTABLE_EXT &&
        src_type != dst_type) {
        const LogObjectList objlist(update.srcSet, update.dstSet, src_layout.Handle(), dst_layout.Handle());
        skip |= LogError("VUID-VkCopyDescriptorSet-dstBinding-02632",
                         objlist,
                         copy_loc.dot(Field::dstBinding),
                         "(%" PRIu32 ") (from %s) is %s, but srcBinding (%" PRIu32 ") (from %s) is %s.",
                         update.dstBinding,
                         FormatHandle(dst_set.Handle()).c_str(),
    ...
    if (src_type == VK_DESCRIPTOR_TYPE_INLINE_UNIFORM_BLOCK && ((update.srcArrayElement % 4) != 0)) {
        const LogObjectList objlist(update.srcSet, src_layout.Handle());
        skip |= LogError("VUID-VkCopyDescriptorSet-srcBinding-02223",
                         objlist,
                         copy_loc.dot(Field::srcArrayElement),
    ...
    if (dst_type == VK_DESCRIPTOR_TYPE_INLINE_UNIFORM_BLOCK && ((update.dstArrayElement % 4) != 0)) {
        const LogObjectList objlist(update.dstSet, dst_layout.Handle());
        skip |= LogError("VUID-VkCopyDescriptorSet-dstBinding-02224",
                         objlist,
                         copy_loc.dot(Field::dstArrayElement),
    ...
    if (src_type == VK_DESCRIPTOR_TYPE_INLINE_UNIFORM_BLOCK || dst_type == VK_DESCRIPTOR_TYPE_INLINE_UNIFORM_BLOCK) {
        if ((update.descriptorCount % 4) != 0) {
            const LogObjectList objlist(update.srcSet, update.dstSet, src_layout.Handle(), dst_layout.Handle());
            skip |= LogError("VUID-VkCopyDescriptorSet-srcBinding-02225",
                             objlist,
                             copy_loc.dot(Field::descriptorCount),
                             "is %" PRIu32 " (not a multiple of 4), but srcBinding (%" PRIu32
    ...
    if (dst_type == VK_DESCRIPTOR_TYPE_MUTABLE_EXT) {
        if (src_type != VK_DESCRIPTOR_TYPE_MUTABLE_EXT) {
            if (!dst_layout.IsTypeMutable(src_type, update.dstBinding)) {
                const LogObjectList objlist(update.srcSet, update.dstSet, src_layout.Handle(), dst_layout.Handle());
                skip |= LogError("VUID-VkCopyDescriptorSet-dstSet-04612",
                                 objlist,
                                 copy_loc.dot(Field::dstBinding),
                                 "(%" PRIu32
                                 ") descriptor type is VK_DESCRIPTOR_TYPE_MUTABLE_EXT, but the srcBinding (%" PRIu32
        ...
        for (uint32_t i = 0; i < update.descriptorCount; i++, ++src_iter) {
            ...
            if (mutable_src.ActiveType() != dst_type) {
                ...
                skip |= LogError(
                    ...
                    update.srcBinding,
                    string_VkDescriptorType(mutable_src.ActiveType()),
                    update.dstBinding,
                    string_VkDescriptorType(dst_type));
            }
        }
    ...
    if (dst_type == VK_DESCRIPTOR_TYPE_MUTABLE_EXT && src_type == VK_DESCRIPTOR_TYPE_MUTABLE_EXT) {
        ...
        if (mutable_src_types != mutable_dst_types) {
            ...
            skip |= LogError(
                ...
                update.srcBinding,
                update.dstBinding,
                src_layout.PrintMutableTypes(update.srcBinding).c_str(),
                dst_layout.PrintMutableTypes(update.dstBinding).c_str());
        }
    }
    ...
    if (dst_type == VK_DESCRIPTOR_TYPE_SAMPLER) {
        ...
        for (uint32_t di = 0; di < update.descriptorCount; ++di, ++dst_iter) {
            if (dst_iter.updated() && dst_iter->IsImmutableSampler()) {
                ...
                skip |= LogError("VUID-VkCopyDescriptorSet-dstBinding-02753",
                                 objlist,
                                 copy_loc.dot(Field::dstBinding),
                                 "(%" PRIu32 ") is type VK_DESCRIPTOR_TYPE_SAMPLER, but the dstSet was created with a "
                                 "non-null pImmutableSamplers.",
                                 update.dstBinding);
            }
        }
    }
...
bool CoreChecks::ValidateImageUpdate(const vvl::ImageView& view_state,
    ...
    const auto image_view_usage_info =
        vku::FindStructInPNextChain<VkImageViewUsageCreateInfo>(view_state.create_info.pNext);
    const auto stencil_usage_info =
        vku::FindStructInPNextChain<VkImageStencilUsageCreateInfo>(image_node->create_info.pNext);
    ...
    skip |= ValidateMemoryIsBoundToImage(LogObjectList(image),
                                         *image_node,
                                         image_info_loc.dot(Field::image),
    ...
    if (view_state.IsDepthSliced() && image_node->create_info.imageType == VK_IMAGE_TYPE_3D) {
        ...
        if (view_state.create_info.viewType == VK_IMAGE_VIEW_TYPE_2D_ARRAY) {
            skip |= LogError("VUID-VkDescriptorImageInfo-imageView-06712",
                             objlist,
                             image_info_loc.dot(Field::imageView),
            ...
            if (type == VK_DESCRIPTOR_TYPE_STORAGE_IMAGE && !enabled_features.image2DViewOf3D) {
                skip |=
                    LogError("VUID-VkDescriptorImageInfo-descriptorType-06713",
                             objlist,
                             image_info_loc.dot(Field::imageView),
    ...
    if (IsValueIn(image_layout,
        ...
        if (aspect_mask & VK_IMAGE_ASPECT_COLOR_BIT) {
            skip |= LogError("VUID-VkDescriptorImageInfo-imageLayout-09426",
                             objlist,
                             image_info_loc.dot(Field::imageView),
                             "uses layout %s but aspectMask (%s) includes color bit.",
    ...
    if (vkuFormatIsDepthOrStencil(image_node->create_info.format)) {
        if (aspect_mask & VK_IMAGE_ASPECT_DEPTH_BIT) {
            if (aspect_mask & VK_IMAGE_ASPECT_STENCIL_BIT) {
                skip |=
                    LogError("VUID-VkDescriptorImageInfo-imageView-01976",
                             objlist,
                             image_info_loc.dot(Field::imageView),
                             "use layout %s and the image format (%s), but it has both STENCIL and DEPTH aspects set",
                             string_VkImageLayout(image_layout),
                             string_VkFormat(image_node->create_info.format));
            }
        }
    }
    ...
    switch (type) {
        case VK_DESCRIPTOR_TYPE_SAMPLED_IMAGE:
            if (view_state.samplerConversion != VK_NULL_HANDLE) {
                skip |= LogError("VUID-VkWriteDescriptorSet-descriptorType-01946",
                                 objlist,
                                 image_info_loc.dot(Field::imageView),
        ...
        case VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER: {
            if (!(usage & VK_IMAGE_USAGE_SAMPLED_BIT)) {
                skip |= LogError("VUID-VkWriteDescriptorSet-descriptorType-00337",
                                 objlist,
                                 image_info_loc.dot(Field::imageView),
        ...
        case VK_DESCRIPTOR_TYPE_STORAGE_IMAGE: {
            if (!(usage & VK_IMAGE_USAGE_STORAGE_BIT)) {
                skip |= LogError("VUID-VkWriteDescriptorSet-descriptorType-00339",
                                 objlist,
                                 image_info_loc.dot(Field::imageView),
        ...
        case VK_DESCRIPTOR_TYPE_INPUT_ATTACHMENT: {
            if (!(usage & VK_IMAGE_USAGE_INPUT_ATTACHMENT_BIT)) {
                skip |= LogError("VUID-VkWriteDescriptorSet-descriptorType-00338",
                                 objlist,
                                 image_info_loc.dot(Field::imageView),
        ...
        case VK_DESCRIPTOR_TYPE_SAMPLE_WEIGHT_IMAGE_QCOM: {
            if (!(usage & VK_IMAGE_USAGE_SAMPLE_WEIGHT_BIT_QCOM)) {
                skip |=
                    LogError("VUID-VkWriteDescriptorSet-descriptorType-06942",
                             objlist,
                             image_info_loc.dot(Field::imageView),
        ...
        case VK_DESCRIPTOR_TYPE_BLOCK_MATCH_IMAGE_QCOM: {
            if (!(usage & VK_IMAGE_USAGE_SAMPLE_BLOCK_MATCH_BIT_QCOM)) {
                skip |=
                    LogError("VUID-VkWriteDescriptorSet-descriptorType-06943",
                             objlist,
                             image_info_loc.dot(Field::imageView),
    ...
    if ((type == VK_DESCRIPTOR_TYPE_SAMPLED_IMAGE) || (type == VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER) ||
        ...
        const static std::array<VkImageLayout, 3> valid_layouts = { { VK_IMAGE_LAYOUT_DEPTH_STENCIL_READ_ONLY_OPTIMAL,
        ...
        const bool valid_layout =
            (std::find(valid_layouts.cbegin(), valid_layouts.cend(), image_layout) != valid_layouts.cend()) ||
        ...
        if (!valid_layout) {
            ...
            error_str << "Descriptor update with descriptorType " << string_VkDescriptorType(type)
                      ...
                      << " for image " << FormatHandle(image) << " in imageView " << FormatHandle(view_state.Handle())
    ...
    if ((type == VK_DESCRIPTOR_TYPE_STORAGE_IMAGE) || (type == VK_DESCRIPTOR_TYPE_INPUT_ATTACHMENT)) {
        ...
        if (IsIdentitySwizzle(components) == false) {
            skip |= LogError("VUID-VkWriteDescriptorSet-descriptorType-00336",
                             objlist,
                             image_info_loc.dot(Field::imageView),
    ...
    if ((type == VK_DESCRIPTOR_TYPE_INPUT_ATTACHMENT) && (view_state.min_lod != 0.0f)) {
        skip |= LogError("VUID-VkWriteDescriptorSet-descriptorType-06450",
                         objlist,
                         image_info_loc.dot(Field::imageView),
                         "was created with minLod %f, but descriptorType is VK_DESCRIPTOR_TYPE_INPUT_ATTACHMENT.",
                         view_state.min_lod);
    }
...
bool CoreChecks::ValidateUpdateDescriptorSets(uint32_t descriptorWriteCount,
...
vvl::DecodedTemplateUpdate::DecodedTemplateUpdate(const ValidationStateTracker& device_data,
    ...
    for (uint32_t i = 0; i < create_info.descriptorUpdateEntryCount; i++) {
        auto binding_count =
            ds_layout_state->GetDescriptorCountFromBinding(create_info.pDescriptorUpdateEntries[i].dstBinding);
        ...
        for (uint32_t j = 0; j < create_info.pDescriptorUpdateEntries[i].descriptorCount; j++) {
            ...
            switch (create_info.pDescriptorUpdateEntries[i].descriptorType) {
                ...
                case VK_DESCRIPTOR_TYPE_INLINE_UNIFORM_BLOCK: {
                    ...
                    // descriptorCount must match the dataSize member of the VkWriteDescriptorSetInlineUniformBlock
                    // structure
                ...
                case VK_DESCRIPTOR_TYPE_ACCELERATION_STRUCTURE_KHR: {
                    ...
                    inline_info_khr->accelerationStructureCount =
                        create_info.pDescriptorUpdateEntries[i].descriptorCount;
                    inline_info_khr->pAccelerationStructures =
                        reinterpret_cast<VkAccelerationStructureKHR*>(update_entry);
                ...
                case VK_DESCRIPTOR_TYPE_ACCELERATION_STRUCTURE_NV: {
                    ...
                    inline_info_nv->accelerationStructureCount =
                        create_info.pDescriptorUpdateEntries[i].descriptorCount;
                    inline_info_nv->pAccelerationStructures =
                        reinterpret_cast<VkAccelerationStructureNV*>(update_entry);
...
bool CoreChecks::ValidatePushDescriptorsUpdate(const vvl::DescriptorSet& push_set,
                                               uint32_t descriptorWriteCount,
                                               const VkWriteDescriptorSet* pDescriptorWrites,
                                               const vvl::DslErrorSource& dsl_error_source,
                                               const Location& loc) const {
    bool skip = false;
    for (uint32_t i = 0; i < descriptorWriteCount; i++) {
        skip |=
            ValidateWriteUpdate(push_set, pDescriptorWrites[i], loc.dot(Field::pDescriptorWrites, i), dsl_error_source);
    }
    return skip;
}
...
bool CoreChecks::ValidateBufferUsage(const vvl::Buffer& buffer_state,
                                     VkDescriptorType type,
    ...
    switch (type) {
        case VK_DESCRIPTOR_TYPE_UNIFORM_TEXEL_BUFFER:
            if (!(buffer_state.usage & VK_BUFFER_USAGE_UNIFORM_TEXEL_BUFFER_BIT)) {
                skip |= LogError("VUID-VkWriteDescriptorSet-descriptorType-08765",
                                 buffer_state.Handle(),
                                 buffer_loc,
        ...
        case VK_DESCRIPTOR_TYPE_STORAGE_TEXEL_BUFFER:
            if (!(buffer_state.usage & VK_BUFFER_USAGE_STORAGE_TEXEL_BUFFER_BIT)) {
                skip |= LogError("VUID-VkWriteDescriptorSet-descriptorType-08766",
                                 buffer_state.Handle(),
                                 buffer_loc,
        ...
        case VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER_DYNAMIC:
            if (!(buffer_state.usage & VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT)) {
                skip |= LogError("VUID-VkWriteDescriptorSet-descriptorType-00330",
                                 buffer_state.Handle(),
                                 buffer_loc,
                                 "was created with %s, but descriptorType is %s.",
                                 string_VkBufferUsageFlags2(buffer_state.usage).c_str(),
        ...
        case VK_DESCRIPTOR_TYPE_STORAGE_BUFFER_DYNAMIC:
            if (!(buffer_state.usage & VK_BUFFER_USAGE_STORAGE_BUFFER_BIT)) {
                skip |= LogError("VUID-VkWriteDescriptorSet-descriptorType-00331",
                                 buffer_state.Handle(),
                                 buffer_loc,
                                 "was created with %s, but descriptorType is %s.",
                                 string_VkBufferUsageFlags2(buffer_state.usage).c_str(),
...
bool CoreChecks::ValidateBufferUpdate(const VkDescriptorBufferInfo& buffer_info,
                                      VkDescriptorType type,
    ...
    skip |= ValidateMemoryIsBoundToBuffer(
        device, *buffer_state, buffer_info_loc.dot(Field::buffer), "VUID-VkWriteDescriptorSet-descriptorType-00329");
    ...
    if (buffer_info.offset >= buffer_state->create_info.size) {
        skip |= LogError("VUID-VkDescriptorBufferInfo-offset-00340",
                         buffer_info.buffer,
                         buffer_info_loc.dot(Field::offset),
                         "(%" PRIu64 ") is greater than or equal to buffer size (%" PRIu64 ").",
                         buffer_info.offset,
                         buffer_state->create_info.size);
    }
    if (buffer_info.range != VK_WHOLE_SIZE) {
        if (buffer_info.range == 0) {
            skip |= LogError("VUID-VkDescriptorBufferInfo-range-00341",
                             buffer_info.buffer,
        ...
        if (buffer_info.range > (buffer_state->create_info.size - buffer_info.offset)) {
            skip |= LogError("VUID-VkDescriptorBufferInfo-range-00342",
                             buffer_info.buffer,
                             buffer_info_loc.dot(Field::range),
                             "(%" PRIu64 ") is larger than buffer size (%" PRIu64 ") + offset (%" PRIu64 ").",
                             buffer_info.range,
                             buffer_state->create_info.size,
                             buffer_info.offset);
        }
    ...
    if (VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER == type || VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER_DYNAMIC == type) {
        ...
        if (buffer_info.range != VK_WHOLE_SIZE && buffer_info.range > max_ub_range) {
            skip |= LogError("VUID-VkWriteDescriptorSet-descriptorType-00332",
                             buffer_info.buffer,
                             buffer_info_loc.dot(Field::range),
                             ...
                             max_ub_range,
                             string_VkDescriptorType(type));
        } else if (buffer_info.range == VK_WHOLE_SIZE &&
                   (buffer_state->create_info.size - buffer_info.offset) > max_ub_range) {
            skip |=
                LogError("VUID-VkWriteDescriptorSet-descriptorType-00332",
                         buffer_info.buffer,
                         buffer_info_loc.dot(Field::range),
                         "is VK_WHOLE_SIZE, but the effective range [size (%" PRIu64 ") - offset (%" PRIu64
                         ") = %" PRIu64 "] is greater than maxUniformBufferRange (%" PRIu32 ") for descriptorType %s.",
                         buffer_state->create_info.size,
                         buffer_info.offset,
        ...
        if (buffer_info.range != VK_WHOLE_SIZE && buffer_info.range > max_sb_range) {
            skip |= LogError("VUID-VkWriteDescriptorSet-descriptorType-00333",
                             buffer_info.buffer,
                             buffer_info_loc.dot(Field::range),
                   ...
                   (buffer_state->create_info.size - buffer_info.offset) > max_sb_range) {
            skip |=
                LogError("VUID-VkWriteDescriptorSet-descriptorType-00333",
                         buffer_info.buffer,
                         buffer_info_loc.dot(Field::range),
                         "is VK_WHOLE_SIZE, but the effective range [size (%" PRIu64 ") - offset (%" PRIu64
                         ") = %" PRIu64 "] is greater than maxStorageBufferRange (%" PRIu32 ") for descriptorType %s.",
...
bool CoreChecks::ValidateWriteUpdate(const vvl::DescriptorSet& dst_set,
                                     const VkWriteDescriptorSet& update,
                                     const Location& write_loc,
    ...
    if (dst_layout->Destroyed()) {
        return LogError("VUID-VkWriteDescriptorSet-dstSet-00320",
                        dst_layout->Handle(),
                        dsl_error_source.ds_loc_,
                        "%s has been destroyed.%s",
                        FormatHandle(dst_layout->Handle()).c_str(),
    ...
    if (dst_layout->GetBindingCount() == 0) {
        return LogError("VUID-VkWriteDescriptorSet-dstBinding-10009",
                        dst_layout->Handle(),
                        dsl_error_source.ds_loc_,
                        "%s was created with bindingCount of zero.%s",
                        FormatHandle(dst_layout->Handle()).c_str(),
                        dsl_error_source.PrintMessage(*this).c_str());
    } else if (update.dstBinding > dst_layout->GetMaxBinding()) {
        return LogError("VUID-VkWriteDescriptorSet-dstBinding-00315",
                        dst_layout->Handle(),
                        dst_binding_loc,
                        "(%" PRIu32 ") is larger than bindingCount (%" PRIu32 ") used to create %s.%s",
                        update.dstBinding,
                        dst_layout->GetBindingCount(),
                        FormatHandle(dst_layout->Handle()).c_str(),
                        dsl_error_source.PrintMessage(*this).c_str());
    }
    ...
    if (!dst_binding) {
        ...
        skip |= LogError("VUID-VkWriteDescriptorSet-dstBinding-00316",
                         dst_layout->Handle(),
                         dst_binding_loc,
    ...
    if (!vvl::IsBindless(dst_binding->binding_flags)) {
        if (const auto* used_handle = dst_set.InUse()) {
            const LogObjectList objlist(update.dstSet, dst_layout->Handle());
            skip |= LogError("VUID-vkUpdateDescriptorSets-None-03047",
                             objlist,
                             dst_binding_loc,
                             "(%" PRIu32 ") was created with %s, but %s is in use by %s.",
                             update.dstBinding,
                             string_VkDescriptorBindingFlags(dst_binding->binding_flags).c_str(),
                             FormatHandle(update.dstSet).c_str(),
    ...
    if (dst_binding->type == VK_DESCRIPTOR_TYPE_MUTABLE_EXT) {
        ...
        if (!dst_layout->IsTypeMutable(update.descriptorType, update.dstBinding)) {
            skip |= LogError("VUID-VkWriteDescriptorSet-dstSet-04611",
                             dst_layout->Handle(),
        ...
        skip |= LogError("VUID-VkWriteDescriptorSet-descriptorType-00319",
                         dst_layout->Handle(),
                         write_loc.dot(Field::descriptorType),
                         "(%s) is different from pBindings[%" PRIu32 "].descriptorType (%s) of %s.%s",
                         string_VkDescriptorType(update.descriptorType),
                         update.dstBinding,
                         string_VkDescriptorType(dst_binding->type),
                         FormatHandle(dst_layout->Handle()).c_str(),
                         dsl_error_source.PrintMessage(*this).c_str());
    }
    ...
    if (update.descriptorCount == 0) {
        skip |= LogError("VUID-VkWriteDescriptorSet-descriptorCount-arraylength",
                         device,
                         write_loc.dot(Field::descriptorCount),
                         "is zero.");
    } else {
        ...
        for (uint32_t i = 0; i < update.descriptorCount;) {
            ...
            // All consecutive bindings updated, except those with a descriptorCount of zero, must have identical
            // descType and stageFlags
            if (current_binding->count > 0) {
                ...
                if ((current_binding->stage_flags != stage_flags) || (current_binding->type != descriptor_type)) {
                    ...
                    if (descriptor_type == VK_DESCRIPTOR_TYPE_INLINE_UNIFORM_BLOCK) {
                        extra << " For inline uniforms blocks, you might have your "
                                 "VkWriteDescriptorSet::dstArrayElement ("
                              << update.dstArrayElement << ") + VkWriteDescriptorSet::descriptorCount ("
                              << update.descriptorCount
                              << ") larger than your VkDescriptorSetLayoutBinding::descriptorCount so this is trying "
                                 "to update the "
                                 "next binding.";
                    }

                    const LogObjectList objlist(update.dstSet, dst_layout->Handle());
                    skip |= LogError("VUID-VkWriteDescriptorSet-descriptorCount-00317",
                                     objlist,
                                     write_loc,
                                     "binding #%" PRIu32 " (started on dstBinding [%" PRIu32 "] + %" PRIu32
                                     " descriptors offset) has stageFlags of %s and descriptorType of %s, but previous "
                                     "binding was %s and %s.%s",
                                     current_binding->binding,
                                     update.dstBinding,
                                     i,
                                     string_VkShaderStageFlags(current_binding->stage_flags).c_str(),
                                     string_VkDescriptorType(current_binding->type),
    ...
    if (dst_binding->IsVariableCount()) {
        if ((update.dstArrayElement + update.descriptorCount) > dst_set.GetVariableDescriptorCount()) {
            ...
            const LogObjectList objlist(update.dstSet, dst_layout->Handle());
            skip |= LogError("VUID-VkWriteDescriptorSet-dstArrayElement-00321",
                             objlist,
                             write_loc.dot(Field::dstArrayElement),
                             "(%" PRIu32 ") + descriptorCount (%" PRIu32 ") is larger than (%" PRIu32
                             ") for dstBinding (%" PRIu32 ") in %s (allocated with %s).",
                             update.dstArrayElement,
                             update.descriptorCount,
                             dst_set.GetVariableDescriptorCount(),
                             update.dstBinding,
                             FormatHandle(dst_set.Handle()).c_str(),
                             FormatHandle(dst_layout->Handle()).c_str());
        }
    } else {
        skip |= VerifyUpdateConsistency(dst_set,
                                        update.dstBinding,
                                        update.dstArrayElement,
                                        update.descriptorCount,
                                        false,
    ...
    // The pipeline_layout is there for PushDescriptors as the proxy VkDescriptorSet comes from there and not
    // update.dstSet
...
bool CoreChecks::ValidateWriteUpdateDescriptorType(const VkWriteDescriptorSet& update,
    ...
    if (descriptor_type == VK_DESCRIPTOR_TYPE_INLINE_UNIFORM_BLOCK) {
        ...
        if (update.pImageInfo == nullptr) {
            const char* vuid = (write_loc.function == Func::vkCmdPushDescriptorSet ||
                                write_loc.function == Func::vkCmdPushDescriptorSetKHR)
                                   ? "VUID-vkCmdPushDescriptorSet-pDescriptorWrites-06494"
                               : (write_loc.function == Func::vkCmdPushDescriptorSet2 ||
                                  write_loc.function == Func::vkCmdPushDescriptorSet2KHR)
                                   ? "VUID-VkPushDescriptorSetInfo-pDescriptorWrites-06494"
                                   : "VUID-vkUpdateDescriptorSets-pDescriptorWrites-06493";
            skip |= LogError(vuid,
                             device,
                             write_loc.dot(Field::descriptorType),
                             "is %s but pImageInfo is NULL.",
                             string_VkDescriptorType(descriptor_type));
        }
    }
...
bool CoreChecks::ValidateWriteUpdateBufferInfo(const VkWriteDescriptorSet& update, const Location& write_loc) const {
    ...
    if (!update.pBufferInfo) {
        skip |= LogError("VUID-VkWriteDescriptorSet-descriptorType-00324",
                         device,
                         write_loc.dot(Field::descriptorType),
                         "is %s but pBufferInfo is NULL.",
                         string_VkDescriptorType(descriptor_type));
        return skip;
    }
    ...
    for (uint32_t i = 0; i < update.descriptorCount; ++i) {
        ...
        if (enabled_features.nullDescriptor) {
            if (buffer_info.buffer == VK_NULL_HANDLE &&
                (buffer_info.offset != 0 || buffer_info.range != VK_WHOLE_SIZE)) {
                skip |= LogError("VUID-VkDescriptorBufferInfo-buffer-02999",
                                 device,
        ...
        if (IsValueIn(descriptor_type,
            ...
            if (SafeModulo(buffer_info.offset, uniform_alignment) != 0) {
                skip |= LogError("VUID-VkWriteDescriptorSet-descriptorType-00327",
                                 device,
            ...
            if (SafeModulo(buffer_info.offset, storage_alignment) != 0) {
                skip |= LogError("VUID-VkWriteDescriptorSet-descriptorType-00328",
                                 device,
                                 write_loc.dot(Field::pBufferInfo, i).dot(Field::offset),
                                 "(%" PRIu64
...
bool CoreChecks::ValidateWriteUpdateInlineUniformBlock(const VkWriteDescriptorSet& update,
                                                       const Location& write_loc) const {
    bool skip = false;
    if ((update.dstArrayElement % 4) != 0) {
        skip |=
            LogError("VUID-VkWriteDescriptorSet-descriptorType-02219",
                     device,
                     write_loc.dot(Field::dstBinding),
    ...
    if ((update.descriptorCount % 4) != 0) {
        skip |=
            LogError("VUID-VkWriteDescriptorSet-descriptorType-02220",
                     device,
                     write_loc.dot(Field::dstBinding),
    ...
    if (!write_inline_info) {
        skip |= LogError("VUID-VkWriteDescriptorSet-descriptorType-02221",
                         device,
                         write_loc.dot(Field::dstBinding),
                         "(%" PRIu32 ") is of type VK_DESCRIPTOR_TYPE_INLINE_UNIFORM_BLOCK, but there is no "
        ...
        skip |= LogError("VUID-VkWriteDescriptorSetInlineUniformBlock-dataSize-02222",
                         device,
                         write_loc.pNext(Struct::VkWriteDescriptorSetInlineUniformBlock, Field::dataSize),
                         "is %" PRIu32 ".",
                         write_inline_info->dataSize);
...
bool CoreChecks::ValidateWriteUpdateAccelerationStructureKHR(const VkWriteDescriptorSet& update,
                                                             const Location& write_loc) const {
    bool skip = false;

    const auto* pnext_struct = vku::FindStructInPNextChain<VkWriteDescriptorSetAccelerationStructureKHR>(update.pNext);
    if (!pnext_struct) {
        skip |= LogError("VUID-VkWriteDescriptorSet-descriptorType-02382",
                         device,
                         write_loc.dot(Field::descriptorType),
    ...
    if (pnext_struct->accelerationStructureCount != update.descriptorCount) {
        skip |= LogError(
            ...
            "(%" PRIu32 ") is not equal to descriptorCount (%" PRIu32 ").",
            pnext_struct->accelerationStructureCount,
            update.descriptorCount);
    }
    ...
    for (uint32_t j = 0; j < pnext_struct->accelerationStructureCount; ++j) {
        if (pnext_struct->pAccelerationStructures[j] == VK_NULL_HANDLE && !enabled_features.nullDescriptor) {
            skip |=
                LogError("VUID-VkWriteDescriptorSetAccelerationStructureKHR-pAccelerationStructures-03580",
                         device,
                         write_loc.pNext(
                             Struct::VkWriteDescriptorSetAccelerationStructureKHR, Field::pAccelerationStructures, j),
        ...
        if (as_state->create_info.type != VK_ACCELERATION_STRUCTURE_TYPE_TOP_LEVEL_KHR &&
            ...
            skip |=
                LogError("VUID-VkWriteDescriptorSetAccelerationStructureKHR-pAccelerationStructures-03579",
                         as_state->Handle(),
                         write_loc.pNext(
                             Struct::VkWriteDescriptorSetAccelerationStructureKHR, Field::pAccelerationStructures, j),
                         "was created with %s.",
                         string_VkAccelerationStructureTypeKHR(as_state->create_info.type));
        }
    }
...
bool CoreChecks::ValidateWriteUpdateAccelerationStructureNV(const VkWriteDescriptorSet& update,
                                                            const Location& write_loc) const {
    bool skip = false;
    ...
    if (!pnext_struct || (pnext_struct->accelerationStructureCount != update.descriptorCount)) {
        skip |= LogError("VUID-VkWriteDescriptorSet-descriptorType-03817",
                         device,
                         write_loc,
                         ...
                         pnext_struct ? pnext_struct->accelerationStructureCount : -1,
                         update.descriptorCount);
        return skip;
    }
    ...
    for (uint32_t j = 0; j < pnext_struct->accelerationStructureCount; ++j) {
        ...
        if (as_state->create_info.info.type != VK_ACCELERATION_STRUCTURE_TYPE_TOP_LEVEL_NV) {
            skip |= LogError(
                ...
                write_loc.pNext(Struct::VkWriteDescriptorSetAccelerationStructureNV, Field::pAccelerationStructures, j),
                "was created with %s.",
                string_VkAccelerationStructureTypeKHR(as_state->create_info.info.type));
        }
    }
...
bool CoreChecks::VerifyWriteUpdateContents(const vvl::DescriptorSet& dst_set,
                                           const VkWriteDescriptorSet& update,
                                           const Location& write_loc,
    ...
    switch (update.descriptorType) {
            ...
            for (uint32_t di = 0; di < update.descriptorCount && !iter.AtEnd(); ++di, ++iter) {
                ...
                if (image_view == VK_NULL_HANDLE) {
                    if (desc.IsImmutableSampler()) {
                        ...
                        skip |= LogError("VUID-VkWriteDescriptorSet-descriptorType-09506",
                                         objlist,
                                         image_info_loc.dot(Field::imageView),
                                         "is VK_NULL_HANDLE.");
                    }
                ...
                if (desc.IsImmutableSampler()) {
                    ...
                    if (iv_state && sampler_state) {
                        if (iv_state->samplerConversion != sampler_state->samplerConversion) {
                            ...
                            skip |= LogError("VUID-VkWriteDescriptorSet-descriptorType-01948",
                                             objlist,
                                             image_info_loc.dot(Field::sampler),
                                             "was created with %s which is not identical to %s that was used to create "
                                             "the imageView.",
                    ...
                    skip |=
                        LogError("VUID-VkWriteDescriptorSet-descriptorType-02738",
                                 objlist,
                                 write_loc.dot(Field::dstSet),
                ...
                if (sampler && !desc.IsImmutableSampler() && vkuFormatIsMultiplane(image_state->create_info.format)) {
                    ...
                    if (0 == (image_state->create_info.flags & VK_IMAGE_CREATE_MUTABLE_FORMAT_BIT)) {
                        ...
                        skip |= LogError("VUID-VkDescriptorImageInfo-sampler-01564",
                                         objlist,
                                         write_loc,
                    ...
                    if (!IsValidPlaneAspect(image_format, image_aspect)) {
                        ...
                        skip |= LogError(
                            ...
                            string_VkFormat(image_format),
                            string_VkImageAspectFlags(image_aspect).c_str());
                    }
                }
                ...
                if (auto sampler_state = Get<vvl::Sampler>(sampler)) {
                    if (IsExtEnabled(device_extensions.vk_khr_portability_subset)) {
                        if ((VK_FALSE == enabled_features.mutableComparisonSamplers) &&
                            ...
                            skip |= LogError("VUID-VkDescriptorImageInfo-mutableComparisonSamplers-04450",
                                             device,
                                             write_loc,
                ...
                if (!desc.IsImmutableSampler()) {
                    ...
                    if (Get<vvl::Sampler>(sampler_handle).get() == nullptr) {
                        if (sampler_handle == VK_NULL_HANDLE) {
                            ...
                            skip |= LogError("VUID-VkWriteDescriptorSet-descriptorType-00325",
                                             objlist,
                                             image_info_loc.dot(Field::sampler),
                    ...
                    skip |= LogError("VUID-VkWriteDescriptorSet-descriptorType-02752",
                                     update.dstSet,
                                     image_info_loc,
        ...
        case VK_DESCRIPTOR_TYPE_BLOCK_MATCH_IMAGE_QCOM: {
            ...
            for (uint32_t di = 0; di < update.descriptorCount; ++di) {
                ...
                if (auto iv_state = Get<vvl::ImageView>(image_view)) {
                    skip |= ValidateImageUpdate(
                        *iv_state, image_layout, update.descriptorType, write_loc.dot(Field::pImageInfo, di));
                }
            }
            break;
        }
            ...
            for (uint32_t di = 0; di < update.descriptorCount; ++di) {
                ...
                if (!buffer_state) {
                    skip |= LogError(
                        ...
                        FormatHandle(buffer_handle).c_str(),
                        string_VkDescriptorType(update.descriptorType));
                    break;
                }
                skip |= ValidateBufferUsage(
                    *buffer_state, update.descriptorType, write_loc.dot(Field::pTexelBufferView, di));
            }
            break;
        }
        case VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER:
        ...
        case VK_DESCRIPTOR_TYPE_STORAGE_BUFFER_DYNAMIC: {
            if (!update.pBufferInfo)
                break;
            for (uint32_t di = 0; di < update.descriptorCount; ++di) {
                skip |= ValidateBufferUpdate(
                    update.pBufferInfo[di], update.descriptorType, write_loc.dot(Field::pBufferInfo, di));
            }
            break;
        }
        case VK_DESCRIPTOR_TYPE_INLINE_UNIFORM_BLOCK:
        ...
        case VK_DESCRIPTOR_TYPE_ACCELERATION_STRUCTURE_NV: {
            const auto* acc_info =
                vku::FindStructInPNextChain<VkWriteDescriptorSetAccelerationStructureNV>(update.pNext);
            if (!acc_info)
                break;
            for (uint32_t di = 0; di < update.descriptorCount; ++di) {
                ...
                if (auto as_state = Get<vvl::AccelerationStructureNV>(as)) {
                    skip |= VerifyBoundMemoryIsValid(
                        ...
                        as_state->Handle(),
                        write_loc.pNext(
                            Struct::VkWriteDescriptorSetAccelerationStructureNV, Field::pAccelerationStructures, di),
                        kVUIDUndefined);
                }
            }
        } break;
...
bool CoreChecks::ValidateCmdSetDescriptorBufferOffsets(const vvl::CommandBuffer& cb_state,
                                                       VkPipelineLayout layout,
                                                       uint32_t firstSet,
                                                       uint32_t setCount,
    ...
    if ((firstSet + setCount) > pipeline_layout->set_layouts.size()) {
        ...
        skip |= LogError(vuid,
                         ...
                         ") is greater than VkPipelineLayoutCreateInfo::setLayoutCount (%" PRIuLEAST64
                         ") when layout was created.",
                         firstSet,
                         setCount,
                         (uint64_t)pipeline_layout->set_layouts.size());
    ...
    for (uint32_t i = 0; i < setCount; i++) {
        ...
        if ((set_layout->GetCreateFlags() & VK_DESCRIPTOR_SET_LAYOUT_CREATE_DESCRIPTOR_BUFFER_BIT_EXT) == 0) {
            ...
            skip |= LogError(vuid,
                             ...
                             FormatHandle(set_layout->Handle()).c_str(),
                             firstSet + i);
            continue;
        }
        ...
        if (buffer_index < cb_state.descriptor_buffer_binding_info.size()) {
            ...
            if (!buffer_states.empty()) {
                ...
                if (!buffer_state_starts.empty()) {
                    ...
                    if (pSetLayoutSize == nullptr) {
                        ...
                        DispatchGetDescriptorSetLayoutSizeEXT(
                            pool->dev_data.device, set_layout->VkHandle(), &setLayoutSize);
                    } else {
            ...
            if (!valid_binding) {
                ...
                skip |= LogError(vuid,
                                 cb_state.Handle(),
                                 loc.dot(Field::pOffsets, i),
                                 "%" PRIuLEAST64 " must be small enough such that any descriptor binding"
        ...
        if (!valid_buffer) {
            ...
            skip |= LogError(vuid,
                             cb_state.Handle(),
                             loc.dot(Field::pBufferIndices, i),
                             "(%" PRIu32
        ...
        if (SafeModulo(offset, phys_dev_ext_props.descriptor_buffer_props.descriptorBufferOffsetAlignment) != 0) {
            ...
            skip |= LogError(vuid,
                             cb_state.Handle(),
                             loc.dot(Field::pOffsets, i),
                             "(%" PRIuLEAST64 ") is not aligned to descriptorBufferOffsetAlignment"
                             " (%" PRIuLEAST64 ")",
                             offset,
...
bool CoreChecks::PreCallValidateCmdSetDescriptorBufferOffsetsEXT(VkCommandBuffer commandBuffer,
    ...
    skip |= ValidateCmdSetDescriptorBufferOffsets(
        *cb_state, layout, firstSet, setCount, pBufferIndices, pOffsets, error_obj.location);
    return skip;
}
...
bool CoreChecks::PreCallValidateCmdSetDescriptorBufferOffsets2EXT(
    ...
    skip |= ValidateCmdSetDescriptorBufferOffsets(*cb_state,
    ...
    if (IsStageInPipelineBindPoint(pSetDescriptorBufferOffsetsInfo->stageFlags,
...
bool CoreChecks::ValidateCmdBindDescriptorBufferEmbeddedSamplers(const vvl::CommandBuffer& cb_state,
                                                                 VkPipelineLayout layout,
                                                                 uint32_t set,
                                                                 const Location& loc) const {
    bool skip = false;
...
bool CoreChecks::PreCallValidateCmdBindDescriptorBufferEmbeddedSamplers2EXT(
    ...
    skip |= ValidateCmdBindDescriptorBufferEmbeddedSamplers(*cb_state,
                                                            pBindDescriptorBufferEmbeddedSamplersInfo->layout,
                                                            pBindDescriptorBufferEmbeddedSamplersInfo->set,
                                                            error_obj.location);

    if (IsStageInPipelineBindPoint(pBindDescriptorBufferEmbeddedSamplersInfo->stageFlags,
...
bool CoreChecks::PreCallValidateCmdBindDescriptorBuffersEXT(VkCommandBuffer commandBuffer,
    ...
    // A "descriptor buffer" binding is seperate from a "VkBuffer" so you can have the same address to the same VkBuffer
    // and it will count as 2, not 1, towards the limit. (more info at
    // https://gitlab.khronos.org/vulkan/vulkan/-/issues/4086)
    ...
    for (uint32_t i = 0; i < bufferCount; i++) {
        ...
        // If none if found, output each violated VUIDs, with the list of buffers that violate it.
        {
                ...
                { "VUID-vkCmdBindDescriptorBuffersEXT-pBindingInfos-08055",
                  [binding_usage = binding_info.usage](vvl::Buffer* const buffer_state, std::string* out_error_msg) {
                      if ((buffer_state->usage & (VK_BUFFER_USAGE_SAMPLER_DESCRIPTOR_BUFFER_BIT_EXT |
                          ...
                          (binding_usage & (VK_BUFFER_USAGE_SAMPLER_DESCRIPTOR_BUFFER_BIT_EXT |
                                            VK_BUFFER_USAGE_RESOURCE_DESCRIPTOR_BUFFER_BIT_EXT |
                                            VK_BUFFER_USAGE_PUSH_DESCRIPTORS_DESCRIPTOR_BUFFER_BIT_EXT))) {
                          if (out_error_msg) {
                  ...
                  [binding_usage = binding_info.usage, i]() {
                      return "The following buffers have a usage that does not match pBindingInfos[" +
                             std::to_string(i) + "].usage (" + string_VkBufferUsageFlags2(binding_usage) + "):";
                  } },
                  ...
                  [binding_usage = binding_info.usage, &resource_buffers](vvl::Buffer* const buffer_state,
                      ...
                      if (binding_usage & VK_BUFFER_USAGE_RESOURCE_DESCRIPTOR_BUFFER_BIT_EXT) {
                          ...
                          if (!(buffer_state->usage & VK_BUFFER_USAGE_RESOURCE_DESCRIPTOR_BUFFER_BIT_EXT)) {
                              if (out_error_msg) {
                                  *out_error_msg +=
                                      "buffer has usage " + string_VkBufferUsageFlags2(buffer_state->usage);
                              }
                              return false;
                          }
                      }
            ...
            skip |= buffer_address_validator.LogErrorsIfNoValidBuffer(
                *this, buffer_states, binding_loc.dot(Field::address), LogObjectList(device), binding_info.address);
        }
        ...
        if (!phys_dev_ext_props.descriptor_buffer_props.bufferlessPushDescriptors &&
            ...
            skip |= LogError("VUID-VkDescriptorBufferBindingInfoEXT-bufferlessPushDescriptors-08056",
                             commandBuffer,
        ...
        if (SafeModulo(pBindingInfos[i].address,
                       phys_dev_ext_props.descriptor_buffer_props.descriptorBufferOffsetAlignment) != 0) {
            skip |= LogError("VUID-VkDescriptorBufferBindingInfoEXT-address-08057",
                             commandBuffer,
                             binding_loc.dot(Field::address),
                             "(%" PRIuLEAST64 ") is not aligned "
                             "to descriptorBufferOffsetAlignment (%" PRIuLEAST64 ")",
                             pBindingInfos[i].address,
                             phys_dev_ext_props.descriptor_buffer_props.descriptorBufferOffsetAlignment);
        }

        if (buffer_handle && phys_dev_ext_props.descriptor_buffer_props.bufferlessPushDescriptors) {
            skip |=
                LogError("VUID-VkDescriptorBufferBindingPushDescriptorBufferHandleEXT-bufferlessPushDescriptors-08059",
                         commandBuffer,
                         binding_loc.dot(Field::pNext),
    ...
    auto list_buffers = [this](std::vector<VkBuffer>& buffer_list) {
        ...
        if (unique_buffers.size() != buffer_list.size()) {
            msg << "Addresses pointing to the same VkBuffer still count as multiple 'descriptor buffer bindings' "
                   "towards the "
                   "limits.\n";
        }
        return msg.str();
    ...
    if (sampler_buffers.size() > phys_dev_ext_props.descriptor_buffer_props.maxSamplerDescriptorBufferBindings) {
        skip |= LogError("VUID-vkCmdBindDescriptorBuffersEXT-maxSamplerDescriptorBufferBindings-08048",
                         commandBuffer,
                         error_obj.location,
                         ...
                         sampler_buffers.size(),
                         phys_dev_ext_props.descriptor_buffer_props.maxSamplerDescriptorBufferBindings,
    ...
    if (resource_buffers.size() > phys_dev_ext_props.descriptor_buffer_props.maxResourceDescriptorBufferBindings) {
        skip |=
            LogError("VUID-vkCmdBindDescriptorBuffersEXT-maxResourceDescriptorBufferBindings-08049",
                     commandBuffer,
                     error_obj.location,
                     ...
                     phys_dev_ext_props.descriptor_buffer_props.maxResourceDescriptorBufferBindings,
                     list_buffers(resource_buffers).c_str());
    }

    if (push_descriptor_buffers.size() > 1) {
        skip |= LogError("VUID-vkCmdBindDescriptorBuffersEXT-None-08050",
                         commandBuffer,
                         error_obj.location,
                         "Number of push descriptor buffers is %zu. "
                         "There must be no more than 1 push descriptor buffer bound that was created "
                         "with the VK_BUFFER_USAGE_PUSH_DESCRIPTORS_DESCRIPTOR_BUFFER_BIT_EXT bit set. List of push "
                         "descriptor buffers:\n%s",
                         push_descriptor_buffers.size(),
                         list_buffers(push_descriptor_buffers).c_str());
    }

    if (bufferCount > phys_dev_ext_props.descriptor_buffer_props.maxDescriptorBufferBindings) {
        skip |= LogError("VUID-vkCmdBindDescriptorBuffersEXT-bufferCount-08051",
                         commandBuffer,
...
bool CoreChecks::PreCallValidateGetDescriptorSetLayoutSizeEXT(VkDevice device,
    ...
    if (!enabled_features.descriptorBuffer) {
        skip |= LogError("VUID-vkGetDescriptorSetLayoutSizeEXT-None-08011",
                         device,
                         error_obj.location,
    ...
    if (auto ds_layout_state = Get<vvl::DescriptorSetLayout>(layout)) {
        ...
        if (!(create_flags & VK_DESCRIPTOR_SET_LAYOUT_CREATE_DESCRIPTOR_BUFFER_BIT_EXT)) {
            skip |= LogError("VUID-vkGetDescriptorSetLayoutSizeEXT-layout-08012",
                             layout,
...
bool CoreChecks::PreCallValidateGetDescriptorSetLayoutBindingOffsetEXT(VkDevice device,
                                                                       VkDescriptorSetLayout layout,
                                                                       uint32_t binding,
    ...
    if (!enabled_features.descriptorBuffer) {
        skip |= LogError("VUID-vkGetDescriptorSetLayoutBindingOffsetEXT-None-08013",
                         device,
                         error_obj.location,
                         "descriptorBuffer feature was not enabled.");
    }
    ...
    if (auto ds_layout_state = Get<vvl::DescriptorSetLayout>(layout)) {
        ...
        if (!(ds_layout_state->GetCreateFlags() & VK_DESCRIPTOR_SET_LAYOUT_CREATE_DESCRIPTOR_BUFFER_BIT_EXT)) {
            skip |= LogError("VUID-vkGetDescriptorSetLayoutBindingOffsetEXT-layout-08014",
                             layout,
                             error_obj.location.dot(Field::layout),
                             "was created with %s.",
                             string_VkDescriptorSetLayoutCreateFlags(create_flags).c_str());
...
bool CoreChecks::PreCallValidateGetBufferOpaqueCaptureDescriptorDataEXT(
    VkDevice device,
    ...
    if (!enabled_features.descriptorBufferCaptureReplay) {
        skip |= LogError("VUID-vkGetBufferOpaqueCaptureDescriptorDataEXT-None-08072",
                         pInfo->buffer,
                         error_obj.location,
    ...
    if (physical_device_count > 1 && !enabled_features.bufferDeviceAddressMultiDevice &&
        ...
        skip |= LogError("VUID-vkGetBufferOpaqueCaptureDescriptorDataEXT-device-08074",
                         pInfo->buffer,
                         error_obj.location,
                         "device was created with multiple physical devices (%" PRIu32 "), but the "
    ...
    if (auto buffer_state = Get<vvl::Buffer>(pInfo->buffer)) {
        if (!(buffer_state->create_info.flags & VK_BUFFER_CREATE_DESCRIPTOR_BUFFER_CAPTURE_REPLAY_BIT_EXT)) {
            skip |= LogError("VUID-VkBufferCaptureDescriptorDataInfoEXT-buffer-08075",
                             pInfo->buffer,
...
bool CoreChecks::PreCallValidateGetImageOpaqueCaptureDescriptorDataEXT(VkDevice device,
    ...
    if (!enabled_features.descriptorBufferCaptureReplay) {
        skip |= LogError("VUID-vkGetImageOpaqueCaptureDescriptorDataEXT-None-08076",
                         pInfo->image,
                         error_obj.location,
                         "descriptorBufferCaptureReplay feature was not enabled.");
    }
    ...
    if (physical_device_count > 1 && !enabled_features.bufferDeviceAddressMultiDevice &&
        !enabled_features.bufferDeviceAddressMultiDeviceEXT) {
        skip |= LogError("VUID-vkGetImageOpaqueCaptureDescriptorDataEXT-device-08078",
                         pInfo->image,
                         error_obj.location,
                         "device was created with multiple physical devices (%" PRIu32 "), but the "
                         "bufferDeviceAddressMultiDevice feature was not enabled.",
    ...
    if (auto image_state = Get<vvl::Image>(pInfo->image)) {
        if (!(image_state->create_info.flags & VK_IMAGE_CREATE_DESCRIPTOR_BUFFER_CAPTURE_REPLAY_BIT_EXT)) {
            skip |= LogError("VUID-VkImageCaptureDescriptorDataInfoEXT-image-08079",
                             pInfo->image,
                             error_obj.location.dot(Field::pInfo).dot(Field::image),
                             "is %s.",
                             string_VkImageCreateFlags(image_state->create_info.flags).c_str());
        }
    }
...
bool CoreChecks::PreCallValidateGetImageViewOpaqueCaptureDescriptorDataEXT(
    VkDevice device,
    ...
    if (!enabled_features.descriptorBufferCaptureReplay) {
        skip |= LogError("VUID-vkGetImageViewOpaqueCaptureDescriptorDataEXT-None-08080",
                         pInfo->imageView,
                         error_obj.location,
                         "descriptorBufferCaptureReplay feature was not enabled.");
    }
    ...
    if (physical_device_count > 1 && !enabled_features.bufferDeviceAddressMultiDevice &&
        !enabled_features.bufferDeviceAddressMultiDeviceEXT) {
        skip |= LogError("VUID-vkGetImageViewOpaqueCaptureDescriptorDataEXT-device-08082",
                         pInfo->imageView,
                         error_obj.location,
                         "device was created with multiple physical devices (%" PRIu32 "), but the "
                         "bufferDeviceAddressMultiDevice feature was not enabled.",
    ...
    if (auto image_view_state = Get<vvl::ImageView>(pInfo->imageView)) {
        if (!(image_view_state->create_info.flags & VK_IMAGE_VIEW_CREATE_DESCRIPTOR_BUFFER_CAPTURE_REPLAY_BIT_EXT)) {
            skip |= LogError("VUID-VkImageViewCaptureDescriptorDataInfoEXT-imageView-08083",
                             pInfo->imageView,
                             error_obj.location.dot(Field::pInfo).dot(Field::imageView),
                             "is %s.",
...
bool CoreChecks::PreCallValidateGetSamplerOpaqueCaptureDescriptorDataEXT(
    VkDevice device,
    ...
    if (!enabled_features.descriptorBufferCaptureReplay) {
        skip |= LogError("VUID-vkGetSamplerOpaqueCaptureDescriptorDataEXT-None-08084",
                         pInfo->sampler,
                         error_obj.location,
                         "descriptorBufferCaptureReplay feature was not enabled.");
    }
    ...
    if (physical_device_count > 1 && !enabled_features.bufferDeviceAddressMultiDevice &&
        !enabled_features.bufferDeviceAddressMultiDeviceEXT) {
        skip |= LogError("VUID-vkGetSamplerOpaqueCaptureDescriptorDataEXT-device-08086",
                         pInfo->sampler,
                         error_obj.location,
                         "device was created with multiple physical devices (%" PRIu32 "), but the "
                         "bufferDeviceAddressMultiDevice feature was not enabled.",
    ...
    if (auto sampler_state = Get<vvl::Sampler>(pInfo->sampler)) {
        if (!(sampler_state->create_info.flags & VK_SAMPLER_CREATE_DESCRIPTOR_BUFFER_CAPTURE_REPLAY_BIT_EXT)) {
            skip |= LogError("VUID-VkSamplerCaptureDescriptorDataInfoEXT-sampler-08087",
                             pInfo->sampler,
                             error_obj.location.dot(Field::pInfo).dot(Field::sampler),
                             "is %s.",
...
bool CoreChecks::PreCallValidateGetAccelerationStructureOpaqueCaptureDescriptorDataEXT(
    ...
    if (!enabled_features.descriptorBufferCaptureReplay) {
        skip |= LogError("VUID-vkGetAccelerationStructureOpaqueCaptureDescriptorDataEXT-None-08088",
                         device,
                         error_obj.location,
                         "descriptorBufferCaptureReplay feature was not enabled.");
    }
    ...
    if (physical_device_count > 1 && !enabled_features.bufferDeviceAddressMultiDevice &&
        !enabled_features.bufferDeviceAddressMultiDeviceEXT) {
        skip |= LogError("VUID-vkGetAccelerationStructureOpaqueCaptureDescriptorDataEXT-device-08090",
                         device,
                         error_obj.location,
                         "device was created with multiple physical devices (%" PRIu32 "), but the "
                         "bufferDeviceAddressMultiDevice feature was not enabled.",
        ...
        if (auto acceleration_structure_state = Get<vvl::AccelerationStructureKHR>(pInfo->accelerationStructure)) {
            if (!(acceleration_structure_state->create_info.createFlags &
                ...
                skip |= LogError(
                    ...
                    error_obj.location,
                    "pInfo->accelerationStructure was %s.",
                    string_VkAccelerationStructureCreateFlagsKHR(acceleration_structure_state->create_info.createFlags)
                        .c_str());
            }
        }
        ...
        if (auto acceleration_structure_state = Get<vvl::AccelerationStructureNV>(pInfo->accelerationStructureNV)) {
            if (!(acceleration_structure_state->create_info.info.flags &
                ...
                skip |= LogError(
                    ...
                    pInfo->accelerationStructureNV,
                    error_obj.location,
                    "pInfo->accelerationStructure was %s.",
                    string_VkAccelerationStructureCreateFlagsKHR(acceleration_structure_state->create_info.info.flags)
                        .c_str());
            }
        }
...
bool CoreChecks::ValidateDescriptorAddressInfoEXT(const VkDescriptorAddressInfoEXT* address_info,
    ...
    if (address_info->range == 0) {
        skip |=
            LogError("VUID-VkDescriptorAddressInfoEXT-range-08940", device, address_loc.dot(Field::range), "is zero.");
    }

    if (address_info->address == 0) {
        if (!enabled_features.nullDescriptor) {
            skip |= LogError("VUID-VkDescriptorAddressInfoEXT-address-08043",
                             device,
                             address_loc.dot(Field::address),
                             "is zero, but the nullDescriptor feature was not enabled.");
        } else if (address_info->range != VK_WHOLE_SIZE) {
            skip |= LogError("VUID-VkDescriptorAddressInfoEXT-nullDescriptor-08938",
                             device,
                             address_loc.dot(Field::range),
                             "(%" PRIu64 ") is not VK_WHOLE_SIZE, but address is zero.",
                             address_info->range);
        }
    } else {
        if (address_info->range == VK_WHOLE_SIZE) {
            skip |= LogError("VUID-VkDescriptorAddressInfoEXT-nullDescriptor-08939",
                             device,
                             address_loc.dot(Field::range),
                             "is VK_WHOLE_SIZE.");
        }
    ...
    if ((address_info->address != 0) && buffer_states.empty()) {
        skip |= LogError("VUID-VkDescriptorAddressInfoEXT-None-08044",
                         device,
                         address_loc.dot(Field::address),
        ...
        BufferAddressValidation<1> buffer_address_validator = {
            { { { "VUID-VkDescriptorAddressInfoEXT-range-08045",
                  ...
                  [&address_info]() {
                      const sparse_container::range<VkDeviceAddress> address_range{
                          address_info->address, address_info->address + address_info->range
                      };
        ...
        skip |= buffer_address_validator.LogErrorsIfNoValidBuffer(
            *this, buffer_states, address_loc.dot(Field::address), LogObjectList(device), address_info->address);
    }

    return skip;
}

bool CoreChecks::ValidateGetDescriptorDataSize(const VkDescriptorGetInfoEXT& descriptor_info,
    ...
    switch (descriptor_info.type) {
        ...
        case VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER:
            ...
            field_name = enabled_features.robustBufferAccess ? Field::robustUniformBufferDescriptorSize
                                                             : Field::uniformBufferDescriptorSize;
            break;
        ...
        case VK_DESCRIPTOR_TYPE_STORAGE_BUFFER:
            ...
            field_name = enabled_features.robustBufferAccess ? Field::robustStorageBufferDescriptorSize
                                                             : Field::storageBufferDescriptorSize;
            break;
        ...
        if (combined_image_sampler->imageView == VK_NULL_HANDLE) {
            ...
            if (size != data_size) {
                skip |= LogError(
                    ...
                    phys_dev_ext_props.descriptor_buffer_props.combinedImageSamplerDescriptorSize,
                    data_size);
            }
        } else {
            ...
            if (image_view_state && image_view_state->samplerConversion != VK_NULL_HANDLE) {
                ...
                VkImageFormatProperties2 image_format_properties =
                    vku::InitStructHelper(&sampler_ycbcr_image_format_info);
                DispatchGetPhysicalDeviceImageFormatProperties2Helper(
                ...
                if (size != data_size) {
                    skip |= LogError("VUID-vkGetDescriptorEXT-descriptorType-09469",
                                     device,
                                     descriptor_info_loc.dot(Field::type),
                                     ...
                                     FormatHandle(image_view_state->samplerConversion).c_str(),
                                     size,
    ...
    if (size != data_size) {
        skip |= LogError("VUID-vkGetDescriptorEXT-dataSize-08125",
                         device,
                         descriptor_info_loc.dot(Field::type),
                         "(%s) has a size of %zu (determined by %s::%s), but dataSize is %zu",
                         string_VkDescriptorType(descriptor_info.type),
                         size,
                         String(struct_name),
                         String(field_name),
                         data_size);
...
bool CoreChecks::PreCallValidateGetDescriptorEXT(VkDevice device,
    ...
    switch (pDescriptorInfo->type) {
        case VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER:
            ...
            if (Get<vvl::Sampler>(pDescriptorInfo->data.pCombinedImageSampler->sampler).get() == nullptr) {
                skip |= LogError("VUID-VkDescriptorGetInfoEXT-type-08019",
                                 device,
                                 descriptor_info_loc.dot(Field::type),
            ...
            if ((pDescriptorInfo->data.pCombinedImageSampler->imageView != VK_NULL_HANDLE) &&
                ...
                skip |= LogError("VUID-VkDescriptorGetInfoEXT-type-08020",
                                 device,
                                 descriptor_info_loc.dot(Field::type),
                                 "is VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER, but "
        ...
        case VK_DESCRIPTOR_TYPE_INPUT_ATTACHMENT:
            ...
            if (Get<vvl::ImageView>(pDescriptorInfo->data.pInputAttachmentImage->imageView).get() == nullptr) {
                skip |= LogError("VUID-VkDescriptorGetInfoEXT-type-08021",
                                 device,
                                 descriptor_info_loc.dot(Field::type),
        ...
        case VK_DESCRIPTOR_TYPE_SAMPLED_IMAGE:
            ...
            if (pDescriptorInfo->data.pSampledImage &&
                ...
                skip |= LogError("VUID-VkDescriptorGetInfoEXT-type-08022",
                                 device,
                                 descriptor_info_loc.dot(Field::type),
        ...
        case VK_DESCRIPTOR_TYPE_STORAGE_IMAGE:
            ...
            if (pDescriptorInfo->data.pStorageImage &&
                (pDescriptorInfo->data.pStorageImage->imageView != VK_NULL_HANDLE) &&
                (Get<vvl::ImageView>(pDescriptorInfo->data.pStorageImage->imageView).get() == nullptr)) {
                skip |= LogError("VUID-VkDescriptorGetInfoEXT-type-08023",
                                 device,
                                 descriptor_info_loc.dot(Field::type),
                                 "is VK_DESCRIPTOR_TYPE_SAMPLED_IMAGE, but "
        ...
        case VK_DESCRIPTOR_TYPE_UNIFORM_TEXEL_BUFFER:
            ...
            if (address_info && (address_info->address != 0) && (GetBuffersByAddress(address_info->address).empty())) {
                skip |= LogError("VUID-VkDescriptorGetInfoEXT-type-08024",
                                 device,
                                 descriptor_info_loc.dot(Field::type),
                                 "is VK_DESCRIPTOR_TYPE_UNIFORM_TEXEL_BUFFER, but "
                                 "pUniformTexelBuffer->address (%" PRIu64 ") is not zero or "
        ...
        case VK_DESCRIPTOR_TYPE_STORAGE_TEXEL_BUFFER:
            ...
            if (address_info && (address_info->address != 0) && (GetBuffersByAddress(address_info->address).empty())) {
                skip |= LogError("VUID-VkDescriptorGetInfoEXT-type-08025",
                                 device,
                                 descriptor_info_loc.dot(Field::type),
                                 "is VK_DESCRIPTOR_TYPE_STORAGE_TEXEL_BUFFER, but "
                                 "pStorageTexelBuffer->address (%" PRIu64 ") is not zero or "
                                 "an address within a buffer.",
                                 address_info->address);
            }
            break;
        case VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER:
            ...
            if (address_info && (address_info->address != 0) && (GetBuffersByAddress(address_info->address).empty())) {
                skip |= LogError("VUID-VkDescriptorGetInfoEXT-type-08026",
                                 device,
                                 descriptor_info_loc.dot(Field::type),
                                 "is VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER, but "
                                 "pUniformBuffer->address (%" PRIu64 ") is not zero or "
                                 "an address within a buffer.",
                                 address_info->address);
            }
            break;
        case VK_DESCRIPTOR_TYPE_STORAGE_BUFFER:
            ...
            if (address_info && (address_info->address != 0) && (GetBuffersByAddress(address_info->address).empty())) {
                skip |= LogError("VUID-VkDescriptorGetInfoEXT-type-08027",
                                 device,
                                 descriptor_info_loc.dot(Field::type),
                                 "is VK_DESCRIPTOR_TYPE_STORAGE_BUFFER, but "
                                 "pStorageBuffer->address (%" PRIu64 ") is not zero or "
                                 "an address within a buffer.",
                                 address_info->address);
            }
            break;
        ...
        case VK_DESCRIPTOR_TYPE_ACCELERATION_STRUCTURE_NV:
            ...
            if (pDescriptorInfo->data.accelerationStructure) {
                const VkAccelerationStructureNV as =
                    (VkAccelerationStructureNV)pDescriptorInfo->data.accelerationStructure;
    ...
    if (address_info && address_info->range != VK_WHOLE_SIZE &&
        ...
        const VkDeviceSize texels = SafeDivision(address_info->range, format_info.block_size) *
                                    static_cast<VkDeviceSize>(format_info.texel_per_block);
        if (texels > static_cast<VkDeviceSize>(phys_dev_props.limits.maxTexelBufferElements)) {
            ...
            skip |= LogError(vuid,
                             device,
                             data_loc.dot(data_field).dot(Field::range),
    ...
    switch (pDescriptorInfo->type) {
        case VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER:
            ...
            if (!address_info && !enabled_features.nullDescriptor) {
                skip |= LogError("VUID-VkDescriptorDataEXT-type-08039",
                                 device,
                                 descriptor_info_loc.dot(Field::type),
                                 "is VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER, but "
        ...
        case VK_DESCRIPTOR_TYPE_STORAGE_BUFFER:
            ...
            if (!address_info && !enabled_features.nullDescriptor) {
                skip |= LogError("VUID-VkDescriptorDataEXT-type-08040",
                                 device,
                                 descriptor_info_loc.dot(Field::type),
                                 "is VK_DESCRIPTOR_TYPE_STORAGE_BUFFER, but "
        ...
        case VK_DESCRIPTOR_TYPE_UNIFORM_TEXEL_BUFFER:
            ...
            if (!address_info && !enabled_features.nullDescriptor) {
                skip |= LogError("VUID-VkDescriptorDataEXT-type-08037",
                                 device,
                                 descriptor_info_loc.dot(Field::type),
                                 "is VK_DESCRIPTOR_TYPE_UNIFORM_TEXEL_BUFFER, but "
        ...
        case VK_DESCRIPTOR_TYPE_STORAGE_TEXEL_BUFFER:
            ...
            if (!address_info && !enabled_features.nullDescriptor) {
                skip |= LogError("VUID-VkDescriptorDataEXT-type-08038",
                                 device,
                                 descriptor_info_loc.dot(Field::type),
                                 "is VK_DESCRIPTOR_TYPE_STORAGE_TEXEL_BUFFER, but "
        ...
        case VK_DESCRIPTOR_TYPE_ACCELERATION_STRUCTURE_KHR:
            if ((pDescriptorInfo->data.accelerationStructure == 0) && !enabled_features.nullDescriptor) {
                skip |= LogError("VUID-VkDescriptorDataEXT-type-08041",
                                 device,
                                 descriptor_info_loc.dot(Field::type),
        ...
        case VK_DESCRIPTOR_TYPE_ACCELERATION_STRUCTURE_NV:
            if ((pDescriptorInfo->data.accelerationStructure == 0) && !enabled_features.nullDescriptor) {
                skip |= LogError("VUID-VkDescriptorDataEXT-type-08042",
                                 device,
                                 descriptor_info_loc.dot(Field::type),
        ...
        case VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER:
            if ((pDescriptorInfo->data.pCombinedImageSampler->imageView == VK_NULL_HANDLE) &&
                !enabled_features.nullDescriptor) {
                skip |= LogError("VUID-VkDescriptorDataEXT-type-08034",
                                 device,
                                 descriptor_info_loc.dot(Field::type),
                                 "is VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER, but "
                                 "pCombinedImageSampler->imageView is VK_NULL_HANDLE and the nullDescriptor feature is "
                                 "not enabled.");
            }
            break;
        case VK_DESCRIPTOR_TYPE_SAMPLED_IMAGE:
        ...
        case VK_DESCRIPTOR_TYPE_STORAGE_BUFFER:
            if (address_info) {
                ...
                if (!buffer_states.empty()) {
                    ...
                    skip |=
                        buffer_address_validator.LogErrorsIfNoValidBuffer(*this,
                                                                          buffer_states,
...
bool CoreChecks::PreCallValidateResetDescriptorPool(VkDevice device,
    ...
    if (auto ds_pool_state = Get<vvl::DescriptorPool>(descriptorPool)) {
        skip |= ValidateObjectNotInUse(ds_pool_state.get(),
                                       error_obj.location.dot(Field::descriptorPool),
                                       "VUID-vkResetDescriptorPool-descriptorPool-00313");
    }
    return skip;
}

bool CoreChecks::PreCallValidateDestroyDescriptorPool(VkDevice device,
                                                      VkDescriptorPool descriptorPool,
                                                      const VkAllocationCallbacks* pAllocator,
                                                      const ErrorObject& error_obj) const {
    bool skip = false;
    if (auto ds_pool_state = Get<vvl::DescriptorPool>(descriptorPool)) {
        skip |= ValidateObjectNotInUse(
            ds_pool_state.get(), error_obj.location, "VUID-vkDestroyDescriptorPool-descriptorPool-00303");
    }
    return skip;
}
...
bool CoreChecks::PreCallValidateAllocateDescriptorSets(VkDevice device,
    ...
    for (uint32_t i = 0; i < pAllocateInfo->descriptorSetCount; i++) {
        ...
        if (ds_layout_state->GetCreateFlags() & VK_DESCRIPTOR_SET_LAYOUT_CREATE_DESCRIPTOR_BUFFER_BIT_EXT) {
            skip |= LogError("VUID-VkDescriptorSetAllocateInfo-pSetLayouts-08009",
                             pAllocateInfo->pSetLayouts[i],
                             set_layout_loc,
        ...
        if (ds_layout_state->GetCreateFlags() & VK_DESCRIPTOR_SET_LAYOUT_CREATE_UPDATE_AFTER_BIND_POOL_BIT &&
            ...
            skip |= LogError("VUID-VkDescriptorSetAllocateInfo-pSetLayouts-03044",
                             objlist,
                             set_layout_loc,
        ...
        if (ds_layout_state->GetCreateFlags() & VK_DESCRIPTOR_SET_LAYOUT_CREATE_HOST_ONLY_POOL_BIT_EXT &&
            ...
            const LogObjectList objlist(pAllocateInfo->descriptorPool, pAllocateInfo->pSetLayouts[i]);
            skip |= LogError("VUID-VkDescriptorSetAllocateInfo-pSetLayouts-04610",
                             objlist,
                             set_layout_loc,
                             "was created with %s but the descriptorPool was created with %s",
                             string_VkDescriptorSetLayoutCreateFlags(ds_layout_state->GetCreateFlags()).c_str(),
    ...
    if (!IsExtEnabled(device_extensions.vk_khr_maintenance1)) {
        ...
        if (ds_pool_state->GetAvailableSets() < pAllocateInfo->descriptorSetCount) {
            skip |= LogError("VUID-VkDescriptorSetAllocateInfo-apiVersion-07895",
                             ds_pool_state->Handle(),
                             error_obj.location,
                             "Unable to allocate %" PRIu32 " descriptorSets from %s"
                             ". This pool only has %" PRIu32 " descriptorSets remaining.",
                             pAllocateInfo->descriptorSetCount,
                             FormatHandle(*ds_pool_state).c_str(),
                             ds_pool_state->GetAvailableSets());
        }
        // Determine whether descriptor counts are satisfiable
        for (auto it = ds_data.required_descriptors_by_type.begin(); it != ds_data.required_descriptors_by_type.end();
             ++it) {
            ...
            if (ds_data.required_descriptors_by_type.at(it->first) > available_count) {
                skip |= LogError("VUID-VkDescriptorSetAllocateInfo-apiVersion-07896",
                                 ds_pool_state->Handle(),
                                 error_obj.location,
                                 "Unable to allocate %" PRIu32 " descriptors of type %s from %s"
    ...
    if (count_allocate_info) {
        if (count_allocate_info->descriptorSetCount != 0 &&
            ...
            skip |= LogError("VUID-VkDescriptorSetVariableDescriptorCountAllocateInfo-descriptorSetCount-03045",
                             device,
                             allocate_info_loc.pNext(Struct::VkDescriptorSetVariableDescriptorCountAllocateInfo,
                                                     Field::descriptorSetCount),
                             "(%" PRIu32 ") != pAllocateInfo->descriptorSetCount (%" PRIu32 ").",
                             count_allocate_info->descriptorSetCount,
                             pAllocateInfo->descriptorSetCount);
        }
        if (count_allocate_info->descriptorSetCount == pAllocateInfo->descriptorSetCount) {
            for (uint32_t i = 0; i < pAllocateInfo->descriptorSetCount; i++) {
                ...
                if (count_allocate_info->pDescriptorCounts[i] >
                    ...
                    skip |= LogError("VUID-VkDescriptorSetAllocateInfo-pSetLayouts-09380",
                                     device,
                                     allocate_info_loc.pNext(Struct::VkDescriptorSetVariableDescriptorCountAllocateInfo,
                                                             Field::pDescriptorCounts,
                                                             i),
...
void CoreChecks::PostCallRecordAllocateDescriptorSets(VkDevice device,
                                                      const VkDescriptorSetAllocateInfo* pAllocateInfo,
                                                      VkDescriptorSet* pDescriptorSets,
                                                      const RecordObject& record_obj,
                                                      vvl::AllocateDescriptorSetsData& ads_state) {
    // Discussed in https://gitlab.khronos.org/vulkan/vulkan/-/issues/3347
    // The issue if users see VK_ERROR_OUT_OF_POOL_MEMORY they think they over-allocated, but if they instead allocated
    // type not avaiable (so the pool size is zero), they will just keep getting this error mistakenly thinking they ran
        ...
        for (uint32_t i = 0; i < pAllocateInfo->descriptorSetCount; i++) {
            ...
            for (uint32_t j = 0; j < binding_count; ++j) {
                ...
                if (!ds_pool_state->IsAvailableType(type)) {
                    ...
                    LogWarning(
                        "WARNING-CoreValidation-AllocateDescriptorSets-WrongType",
                        ds_pool_state->Handle(),
                        record_obj.location.dot(Field::pAllocateInfo).dot(Field::pSetLayouts, i),
                        "binding %" PRIu32 " was created with %s but the "
...
bool CoreChecks::PreCallValidateFreeDescriptorSets(VkDevice device,
                                                   VkDescriptorPool descriptorPool,
                                                   uint32_t count,
                                                   const VkDescriptorSet* pDescriptorSets,
                                                   const ErrorObject& error_obj) const {
    ...
    if (!(VK_DESCRIPTOR_POOL_CREATE_FREE_DESCRIPTOR_SET_BIT & ds_pool_state->create_info.flags)) {
        ...
        skip |= LogError("VUID-vkFreeDescriptorSets-descriptorPool-00312",
                         descriptorPool,
                         error_obj.location.dot(Field::descriptorPool),
                         "with a pool created with %s.",
                         string_VkDescriptorPoolCreateFlags(ds_pool_state->create_info.flags).c_str());
    }
    return skip;
}

bool CoreChecks::PreCallValidateUpdateDescriptorSets(VkDevice device,
                                                     uint32_t descriptorWriteCount,
                                                     const VkWriteDescriptorSet* pDescriptorWrites,
                                                     uint32_t descriptorCopyCount,
                                                     const VkCopyDescriptorSet* pDescriptorCopies,
    ...
    return ValidateUpdateDescriptorSets(
        descriptorWriteCount, pDescriptorWrites, descriptorCopyCount, pDescriptorCopies, error_obj.location);
}

bool CoreChecks::ValidateCmdPushDescriptorSet(const vvl::CommandBuffer& cb_state,
                                              VkPipelineLayout layout,
                                              uint32_t set,
                                              uint32_t descriptorWriteCount,
                                              const VkWriteDescriptorSet* pDescriptorWrites,
    ...
    if (set >= set_layouts.size()) {
        ...
        skip |= LogError(vuid,
                         ...
                         loc.dot(Field::set),
                         "(%" PRIu32
                         ") is indexing outside the range for %s (which had a setLayoutCount of only %" PRIu32 ").",
                         set,
                         FormatHandle(layout).c_str(),
...
bool CoreChecks::PreCallValidateCmdPushDescriptorSet(VkCommandBuffer commandBuffer,
                                                     VkPipelineBindPoint pipelineBindPoint,
                                                     VkPipelineLayout layout,
                                                     uint32_t set,
                                                     uint32_t descriptorWriteCount,
    ...
    skip |= ValidateCmd(*cb_state, error_obj.location);
    skip |= ValidatePipelineBindPoint(*cb_state, pipelineBindPoint, error_obj.location);
    skip |= ValidateCmdPushDescriptorSet(
        *cb_state, layout, set, descriptorWriteCount, pDescriptorWrites, error_obj.location);
    return skip;
}

bool CoreChecks::PreCallValidateCmdPushDescriptorSetKHR(VkCommandBuffer commandBuffer,
                                                        VkPipelineBindPoint pipelineBindPoint,
                                                        VkPipelineLayout layout,
                                                        uint32_t set,
                                                        uint32_t descriptorWriteCount,
                                                        const VkWriteDescriptorSet* pDescriptorWrites,
                                                        const ErrorObject& error_obj) const {
    return PreCallValidateCmdPushDescriptorSet(
        commandBuffer, pipelineBindPoint, layout, set, descriptorWriteCount, pDescriptorWrites, error_obj);
}
...
bool CoreChecks::PreCallValidateCmdPushDescriptorSet2(VkCommandBuffer commandBuffer,
    ...
    skip |= ValidateCmdPushDescriptorSet(*cb_state,
...
bool CoreChecks::PreCallValidateCreateDescriptorUpdateTemplate(VkDevice device,
    ...
    if (VK_DESCRIPTOR_UPDATE_TEMPLATE_TYPE_DESCRIPTOR_SET == pCreateInfo->templateType && !ds_layout_state) {
        skip |= LogError("VUID-VkDescriptorUpdateTemplateCreateInfo-templateType-00350",
        ...
        const bool valid_bp = (bind_point == VK_PIPELINE_BIND_POINT_GRAPHICS) ||
                              (bind_point == VK_PIPELINE_BIND_POINT_COMPUTE) ||
                              (bind_point == VK_PIPELINE_BIND_POINT_RAY_TRACING_KHR);
        if (!valid_bp) {
            skip |= LogError("VUID-VkDescriptorUpdateTemplateCreateInfo-templateType-00351",
                             device,
                             create_info_loc.dot(Field::pipelineBindPoint),
                             "is %s.",
                             string_VkPipelineBindPoint(bind_point));
        }
        auto pipeline_layout = Get<vvl::PipelineLayout>(pCreateInfo->pipelineLayout);
        if (!pipeline_layout) {
            skip |= LogError("VUID-VkDescriptorUpdateTemplateCreateInfo-templateType-00352",
            ...
            if ((pd_set >= pipeline_layout->set_layouts.size()) || !pipeline_layout->set_layouts[pd_set] ||
                ...
                skip |=
                    LogError("VUID-VkDescriptorUpdateTemplateCreateInfo-templateType-00353",
                             pCreateInfo->pipelineLayout,
        ...
        for (const auto& binding : ds_layout_state->GetBindings()) {
            if (binding.descriptorType == VK_DESCRIPTOR_TYPE_MUTABLE_EXT) {
                skip |= LogError("VUID-VkDescriptorUpdateTemplateCreateInfo-templateType-04615",
                                 device,
                                 create_info_loc.dot(Field::templateType),
                                 "is VK_DESCRIPTOR_UPDATE_TEMPLATE_TYPE_DESCRIPTOR_SET, but "
                                 "pCreateInfo->descriptorSetLayout contains a binding with descriptor type "
    ...
    for (uint32_t i = 0; i < pCreateInfo->descriptorUpdateEntryCount; ++i) {
        ...
        if (descriptor_update.descriptorType == VK_DESCRIPTOR_TYPE_INLINE_UNIFORM_BLOCK) {
            if (descriptor_update.dstArrayElement & 3) {
                skip |=
                    LogError("VUID-VkDescriptorUpdateTemplateEntry-descriptor-02226",
                             pCreateInfo->pipelineLayout,
            ...
            if (descriptor_update.descriptorCount & 3) {
                skip |=
                    LogError("VUID-VkDescriptorUpdateTemplateEntry-descriptor-02227",
                             pCreateInfo->pipelineLayout,
                             create_info_loc.dot(Field::pDescriptorUpdateEntries, i),
...
bool CoreChecks::PreCallValidateCreateDescriptorUpdateTemplateKHR(
    VkDevice device,
    const VkDescriptorUpdateTemplateCreateInfo* pCreateInfo,
    const VkAllocationCallbacks* pAllocator,
    VkDescriptorUpdateTemplate* pDescriptorUpdateTemplate,
    const ErrorObject& error_obj) const {
    return PreCallValidateCreateDescriptorUpdateTemplate(
        device, pCreateInfo, pAllocator, pDescriptorUpdateTemplate, error_obj);
}

bool CoreChecks::PreCallValidateUpdateDescriptorSetWithTemplate(VkDevice device,
                                                                VkDescriptorSet descriptorSet,
    ...
    if (template_state->create_info.templateType == VK_DESCRIPTOR_UPDATE_TEMPLATE_TYPE_DESCRIPTOR_SET) {
        ...
        return ValidateUpdateDescriptorSets(static_cast<uint32_t>(decoded_update.desc_writes.size()),
                                            decoded_update.desc_writes.data(),
                                            0,
                                            nullptr,
                                            error_obj.location);
    }
...
bool CoreChecks::PreCallValidateUpdateDescriptorSetWithTemplateKHR(VkDevice device,
                                                                   VkDescriptorSet descriptorSet,
                                                                   VkDescriptorUpdateTemplate descriptorUpdateTemplate,
                                                                   const void* pData,
                                                                   const ErrorObject& error_obj) const {
    return PreCallValidateUpdateDescriptorSetWithTemplate(
        device, descriptorSet, descriptorUpdateTemplate, pData, error_obj);
}
...
bool CoreChecks::ValidateCmdPushDescriptorSetWithTemplate(VkCommandBuffer commandBuffer,
    ...
    const bool is_2 = loc.function != Func::vkCmdPushDescriptorSetWithTemplateKHR &&
                      loc.function != Func::vkCmdPushDescriptorSetWithTemplate;
    auto pipeline_layout = Get<vvl::PipelineLayout>(layout);
    if (!pipeline_layout)
        return skip; // dynamicPipelineLayout
    const auto& set_layouts = pipeline_layout->set_layouts;
    if (set >= set_layouts.size()) {
        const char* vuid = is_2 ? "VUID-VkPushDescriptorSetWithTemplateInfo-set-07304"
    ...
    if (!dsl->IsPushDescriptor()) {
        const char* vuid = is_2 ? "VUID-VkPushDescriptorSetWithTemplateInfo-set-07305"
    ...
    if (template_ci.templateType != VK_DESCRIPTOR_UPDATE_TEMPLATE_TYPE_PUSH_DESCRIPTORS) {
        ...
        skip |= LogError(vuid,
                         commandBuffer,
                         loc.dot(Field::descriptorUpdateTemplate),
    ...
    if (template_ci.set != set) {
        const char* vuid = is_2 ? "VUID-VkPushDescriptorSetWithTemplateInfo-set-07995"
                                : "VUID-vkCmdPushDescriptorSetWithTemplate-set-07995";
        skip |= LogError(vuid,
                         commandBuffer,
                         loc.dot(Field::set),
                         "(%" PRIu32 ") does not match the set (%" PRIu32
                         ") that the descriptorUpdateTemplate (%s) was created with.",
                         set,
                         template_ci.set,
                         FormatHandle(descriptorUpdateTemplate).c_str());
    }
    ...
    if (!IsPipelineLayoutSetCompatible(set, pipeline_layout.get(), template_layout.get())) {
        ...
        skip |=
            LogError(vuid,
                     ...
                     "%s for set %" PRIu32 ".\n%s",
                     FormatHandle(descriptorUpdateTemplate).c_str(),
                     FormatHandle(template_ci.pipelineLayout).c_str(),
                     FormatHandle(layout).c_str(),
                     set,
    ...
    if (!Get<vvl::DescriptorSetLayout>(dsl->VkHandle())) {
        ...
        vvl::DecodedTemplateUpdate decoded_template(
            *this, VK_NULL_HANDLE, template_state.get(), pData, dsl->VkHandle());
        // Validate the decoded update against the proxy_ds
        vvl::DslErrorSource dsl_error_source(loc, layout, set);
        skip |= ValidatePushDescriptorsUpdate(proxy_ds,
...
bool CoreChecks::PreCallValidateCmdPushDescriptorSetWithTemplate(VkCommandBuffer commandBuffer,
                                                                 ...
                                                                 const void* pData,
                                                                 const ErrorObject& error_obj) const {
    return ValidateCmdPushDescriptorSetWithTemplate(
        commandBuffer, descriptorUpdateTemplate, layout, set, pData, error_obj.location);
}
...
bool CoreChecks::PreCallValidateCmdPushDescriptorSetWithTemplateKHR(VkCommandBuffer commandBuffer,
                                                                    ...
                                                                    const void* pData,
                                                                    const ErrorObject& error_obj) const {
    return PreCallValidateCmdPushDescriptorSetWithTemplate(
        commandBuffer, descriptorUpdateTemplate, layout, set, pData, error_obj);
}
...
bool CoreChecks::PreCallValidateCmdPushDescriptorSetWithTemplate2(
    ...
    const ErrorObject& error_obj) const {
    bool skip = false;
    skip |= ValidateCmdPushDescriptorSetWithTemplate(commandBuffer,
...
bool CoreChecks::PreCallValidateCmdPushDescriptorSetWithTemplate2KHR(
    ...
    const ErrorObject& error_obj) const {
    return PreCallValidateCmdPushDescriptorSetWithTemplate2(
        commandBuffer, pPushDescriptorSetWithTemplateInfo, error_obj);
}
...
// Returns an array of size DSL_NUM_DESCRIPTOR_GROUPS of the maximum number of descriptors used in any single pipeline
// stage
std::valarray<uint32_t>
GetDescriptorCountMaxPerStage(const DeviceFeatures* enabled_features,
                              const std::vector<std::shared_ptr<vvl::DescriptorSetLayout const>>& set_layouts,
    ...
    for (auto stage : stage_flags) {
        ...
        for (const auto& dsl : set_layouts) {
            ...
            if (skip_update_after_bind &&
                (dsl->GetCreateFlags() & VK_DESCRIPTOR_SET_LAYOUT_CREATE_UPDATE_AFTER_BIND_POOL_BIT)) {
                continue;
            }

            for (uint32_t binding_idx = 0; binding_idx < dsl->GetBindingCount(); binding_idx++) {
                const VkDescriptorSetLayoutBinding* binding =
                    dsl->GetDescriptorSetLayoutBindingPtrFromIndex(binding_idx);
...
GetDescriptorSum(const std::vector<std::shared_ptr<vvl::DescriptorSetLayout const>>& set_layouts,
                 bool skip_update_after_bind) {
...
uint32_t
GetInlineUniformBlockBindingCount(const std::vector<std::shared_ptr<vvl::DescriptorSetLayout const>>& set_layouts,
                                  bool skip_update_after_bind) {
    uint32_t sum = 0;
    for (const auto& dsl : set_layouts) {
        if (!dsl) {
...
bool CoreChecks::PreCallValidateCreatePipelineLayout(VkDevice device,
    ...
    for (uint32_t i = 0; i < pCreateInfo->setLayoutCount; ++i) {
        ...
        if (set_layouts[i]->IsPushDescriptor()) {
            if (push_descriptor_set_found < pCreateInfo->setLayoutCount) {
                skip |= LogError("VUID-VkPipelineLayoutCreateInfo-pSetLayouts-00293",
                                 set_layouts[i]->VkHandle(),
                                 create_info_loc.dot(Field::pSetLayouts, i),
                                 "and pSetLayouts[%" PRIu32 "] both have push descriptor sets.",
                                 push_descriptor_set_found);
            }
            push_descriptor_set_found = i;
        }
        if (set_layouts[i]->GetCreateFlags() & VK_DESCRIPTOR_SET_LAYOUT_CREATE_HOST_ONLY_POOL_BIT_EXT) {
            skip |= LogError("VUID-VkPipelineLayoutCreateInfo-pSetLayouts-04606",
                             set_layouts[i]->VkHandle(),
                             create_info_loc.dot(Field::pSetLayouts, i),
    ...
    if ((descriptor_buffer_set_count != 0) && (valid_set_count != descriptor_buffer_set_count)) {
        skip |= LogError("VUID-VkPipelineLayoutCreateInfo-pSetLayouts-08008",
                         device,
                         error_obj.location,
    ...
    std::valarray<uint32_t> max_descriptors_per_stage =
        GetDescriptorCountMaxPerStage(&enabled_features, set_layouts, true);
    // Samplers
    if (max_descriptors_per_stage[DSL_TYPE_SAMPLERS] > phys_dev_props.limits.maxPerStageDescriptorSamplers) {
        skip |= LogError("VUID-VkPipelineLayoutCreateInfo-descriptorType-03016",
                         device,
                         error_obj.location,
                         "max per-stage sampler bindings count (%" PRIu32 ") exceeds device "
                         "maxPerStageDescriptorSamplers limit (%" PRIu32 ").",
                         max_descriptors_per_stage[DSL_TYPE_SAMPLERS],
    ...
    if (max_descriptors_per_stage[DSL_TYPE_UNIFORM_BUFFERS] >
        ...
        skip |= LogError("VUID-VkPipelineLayoutCreateInfo-descriptorType-03017",
                         device,
                         error_obj.location,
                         "max per-stage uniform buffer bindings count (%" PRIu32 ") exceeds device "
    ...
    if (max_descriptors_per_stage[DSL_TYPE_STORAGE_BUFFERS] >
        ...
        skip |= LogError("VUID-VkPipelineLayoutCreateInfo-descriptorType-03018",
                         device,
                         error_obj.location,
                         "max per-stage storage buffer bindings count (%" PRIu32 ") exceeds device "
    ...
    if (max_descriptors_per_stage[DSL_TYPE_SAMPLED_IMAGES] > phys_dev_props.limits.maxPerStageDescriptorSampledImages) {
        skip |= LogError("VUID-VkPipelineLayoutCreateInfo-descriptorType-06939",
                         device,
                         error_obj.location,
                         "max per-stage sampled image bindings count (%" PRIu32 ") exceeds device "
    ...
    if (max_descriptors_per_stage[DSL_TYPE_STORAGE_IMAGES] > phys_dev_props.limits.maxPerStageDescriptorStorageImages) {
        skip |= LogError("VUID-VkPipelineLayoutCreateInfo-descriptorType-03020",
                         device,
                         error_obj.location,
                         "max per-stage storage image bindings count (%" PRIu32 ") exceeds device "
    ...
    if (max_descriptors_per_stage[DSL_TYPE_INPUT_ATTACHMENTS] >
        ...
        skip |= LogError("VUID-VkPipelineLayoutCreateInfo-descriptorType-03021",
                         device,
                         error_obj.location,
                         "max per-stage input attachment bindings count (%" PRIu32 ") exceeds device "
    ...
    if (max_descriptors_per_stage[DSL_TYPE_INLINE_UNIFORM_BLOCK] >
        ...
        skip |= LogError("VUID-VkPipelineLayoutCreateInfo-descriptorType-02214",
                         device,
                         error_obj.location,
                         "max per-stage inline uniform block bindings count (%" PRIu32 ") exceeds device "
    ...
    if (max_descriptors_per_stage[DSL_TYPE_ACCELERATION_STRUCTURE] >
        ...
        skip |= LogError("VUID-VkPipelineLayoutCreateInfo-descriptorType-03571",
                         device,
                         error_obj.location,
                         "max per-stage acceleration structure bindings count (%" PRIu32 ") exceeds device "
    ...
    if (sum > phys_dev_props.limits.maxDescriptorSetSamplers) {
        skip |= LogError("VUID-VkPipelineLayoutCreateInfo-descriptorType-03028",
                         device,
                         error_obj.location,
                         "sum of sampler bindings among all stages (%" PRIu32 ") exceeds device "
    ...
    if (sum_all_stages[VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER] > phys_dev_props.limits.maxDescriptorSetUniformBuffers) {
        skip |= LogError("VUID-VkPipelineLayoutCreateInfo-descriptorType-03029",
                         device,
                         error_obj.location,
                         "sum of uniform buffer bindings among all stages (%" PRIu32 ") exceeds device "
    ...
    if (sum_all_stages[VK_DESCRIPTOR_TYPE_STORAGE_BUFFER] > phys_dev_props.limits.maxDescriptorSetStorageBuffers) {
        skip |= LogError("VUID-VkPipelineLayoutCreateInfo-descriptorType-03031",
                         device,
                         error_obj.location,
                         "sum of storage buffer bindings among all stages (%" PRIu32 ") exceeds device "
    ...
    if (enabled_features.maintenance7) {
        ...
        if (sum_all_uniform_buffer_dynamic >
            phys_dev_ext_props.maintenance7_props.maxDescriptorSetTotalUniformBuffersDynamic) {
            skip |= LogError("VUID-VkPipelineLayoutCreateInfo-maintenance7-10003",
                             device,
                             error_obj.location,
                             "sum of dynamic uniform buffer bindings among all stages (%" PRIu32 ") exceeds device "
        ...
        if (sum_all_storage_buffer_dynamic >
            phys_dev_ext_props.maintenance7_props.maxDescriptorSetTotalStorageBuffersDynamic) {
            skip |= LogError("VUID-VkPipelineLayoutCreateInfo-maintenance7-10004",
                             device,
                             error_obj.location,
                             "sum of dynamic storage buffer bindings among all stages (%" PRIu32 ") exceeds device "
        ...
        // prevent overflow
        const uint64_t total_sum = static_cast<uint64_t>(sum_all_uniform_buffer_dynamic) +
                                   static_cast<uint64_t>(sum_all_storage_buffer_dynamic);
        if (total_sum > phys_dev_ext_props.maintenance7_props.maxDescriptorSetTotalBuffersDynamic) {
            skip |= LogError("VUID-VkPipelineLayoutCreateInfo-None-10005",
                             device,
                             error_obj.location,
        ...
        if (sum_all_uniform_buffer_dynamic > phys_dev_props.limits.maxDescriptorSetUniformBuffersDynamic) {
            skip |= LogError("VUID-VkPipelineLayoutCreateInfo-descriptorType-03030",
                             device,
                             error_obj.location,
                             "sum of dynamic uniform buffer bindings among all stages (%" PRIu32 ") exceeds device "
        ...
        if (sum_all_storage_buffer_dynamic > phys_dev_props.limits.maxDescriptorSetStorageBuffersDynamic) {
            skip |= LogError("VUID-VkPipelineLayoutCreateInfo-descriptorType-03032",
                             device,
                             error_obj.location,
                             "sum of dynamic storage buffer bindings among all stages (%" PRIu32 ") exceeds device "
    ...
    if (sum > phys_dev_props.limits.maxDescriptorSetSampledImages) {
        skip |= LogError("VUID-VkPipelineLayoutCreateInfo-descriptorType-03033",
                         device,
                         error_obj.location,
                         "sum of sampled image bindings among all stages (%" PRIu32 ") exceeds device "
    ...
    if (sum > phys_dev_props.limits.maxDescriptorSetStorageImages) {
        skip |= LogError("VUID-VkPipelineLayoutCreateInfo-descriptorType-03034",
                         device,
                         error_obj.location,
                         "sum of storage image bindings among all stages (%" PRIu32 ") exceeds device "
    ...
    if (sum_all_stages[VK_DESCRIPTOR_TYPE_INPUT_ATTACHMENT] > phys_dev_props.limits.maxDescriptorSetInputAttachments) {
        skip |= LogError("VUID-VkPipelineLayoutCreateInfo-descriptorType-03035",
                         device,
                         error_obj.location,
                         "sum of input attachment bindings among all stages (%" PRIu32 ") exceeds device "
    ...
    if (inline_uniform_block_bindings_all_stages > phys_dev_props_core13.maxDescriptorSetInlineUniformBlocks) {
        skip |= LogError("VUID-VkPipelineLayoutCreateInfo-descriptorType-02216",
                         device,
                         error_obj.location,
                         "sum of inline uniform block bindings among all stages (%" PRIu32 ") exceeds device "
    ...
    if (api_version >= VK_API_VERSION_1_3 &&
        ...
        skip |= LogError("VUID-VkPipelineLayoutCreateInfo-descriptorType-06531",
                         device,
                         error_obj.location,
                         "sum of inline uniform block bytes among all stages (%" PRIu32 ") exceeds device "
                         "maxInlineUniformTotalSize limit (%" PRIu32 ").",
                         sum_all_stages[VK_DESCRIPTOR_TYPE_INLINE_UNIFORM_BLOCK],
                         phys_dev_props_core13.maxInlineUniformTotalSize);
    }
    ...
    if (sum_all_stages[VK_DESCRIPTOR_TYPE_ACCELERATION_STRUCTURE_KHR] >
        ...
        skip |= LogError("VUID-VkPipelineLayoutCreateInfo-descriptorType-03573",
                         device,
                         error_obj.location,
                         "sum of acceleration structures bindings among all stages (%" PRIu32 ") exceeds device "
    ...
    if (sum_all_stages[VK_DESCRIPTOR_TYPE_ACCELERATION_STRUCTURE_NV] >
        ...
        skip |= LogError(
            ...
            device,
            error_obj.location,
            "sum of acceleration structures NV bindings among all stages (%" PRIu32 ") exceeds device "
    ...
    if (IsExtEnabled(device_extensions.vk_ext_descriptor_indexing)) {
        ...
        if (max_descriptors_per_stage_update_after_bind[DSL_TYPE_SAMPLERS] >
            ...
            skip |= LogError("VUID-VkPipelineLayoutCreateInfo-descriptorType-03022",
                             device,
                             error_obj.location,
                             "max per-stage sampler bindings count (%" PRIu32 ") exceeds device "
        ...
        if (max_descriptors_per_stage_update_after_bind[DSL_TYPE_UNIFORM_BUFFERS] >
            ...
            skip |= LogError("VUID-VkPipelineLayoutCreateInfo-descriptorType-03023",
                             device,
                             error_obj.location,
                             "max per-stage uniform buffer bindings count (%" PRIu32 ") exceeds device "
        ...
        if (max_descriptors_per_stage_update_after_bind[DSL_TYPE_STORAGE_BUFFERS] >
            ...
            skip |= LogError("VUID-VkPipelineLayoutCreateInfo-descriptorType-03024",
                             device,
                             error_obj.location,
                             "max per-stage storage buffer bindings count (%" PRIu32 ") exceeds device "
        ...
        if (max_descriptors_per_stage_update_after_bind[DSL_TYPE_SAMPLED_IMAGES] >
            ...
            skip |= LogError("VUID-VkPipelineLayoutCreateInfo-descriptorType-03025",
                             device,
                             error_obj.location,
                             "max per-stage sampled image bindings count (%" PRIu32 ") exceeds device "
        ...
        if (max_descriptors_per_stage_update_after_bind[DSL_TYPE_STORAGE_IMAGES] >
            ...
            skip |= LogError("VUID-VkPipelineLayoutCreateInfo-descriptorType-03026",
                             device,
                             error_obj.location,
                             "max per-stage storage image bindings count (%" PRIu32 ") exceeds device "
        ...
        if (max_descriptors_per_stage_update_after_bind[DSL_TYPE_INPUT_ATTACHMENTS] >
            ...
            skip |= LogError("VUID-VkPipelineLayoutCreateInfo-descriptorType-03027",
                             device,
                             error_obj.location,
                             "max per-stage input attachment bindings count (%" PRIu32 ") exceeds device "
        ...
        if (max_descriptors_per_stage_update_after_bind[DSL_TYPE_INLINE_UNIFORM_BLOCK] >
            ...
            skip |= LogError("VUID-VkPipelineLayoutCreateInfo-descriptorType-02215",
                             device,
                             error_obj.location,
                             "max per-stage inline uniform block bindings count (%" PRIu32 ") exceeds device "
        ...
        if (sum > phys_dev_props_core12.maxDescriptorSetUpdateAfterBindSamplers) {
            skip |= LogError("VUID-VkPipelineLayoutCreateInfo-pSetLayouts-03036",
                             device,
                             error_obj.location,
                             "sum of sampler bindings among all stages (%" PRIu32 ") exceeds device "
        ...
        if (sum_all_stages_update_after_bind[VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER] >
            ...
            skip |= LogError("VUID-VkPipelineLayoutCreateInfo-pSetLayouts-03037",
                             device,
                             error_obj.location,
                             "sum of uniform buffer bindings among all stages (%" PRIu32 ") exceeds device "
        ...
        if (sum_all_stages_update_after_bind[VK_DESCRIPTOR_TYPE_STORAGE_BUFFER] >
            ...
            skip |= LogError("VUID-VkPipelineLayoutCreateInfo-pSetLayouts-03039",
                             device,
                             error_obj.location,
                             "sum of storage buffer bindings among all stages (%" PRIu32 ") exceeds device "
        ...
        if (enabled_features.maintenance7) {
            ...
            if (total_sum > phys_dev_ext_props.maintenance7_props.maxDescriptorSetUpdateAfterBindTotalBuffersDynamic) {
                skip |= LogError(
                    ...
                    error_obj.location,
                    "sum of both dynamic storage buffer bindings (%" PRIu32
                    ") and dynamic uniform buffer bindings (%" PRIu32 ") among all stages (%" PRIu64 ") exceeds device "
            ...
            if (sum_all_after_bind_uniform_buffer_dynamic >
                ...
                skip |= LogError("VUID-VkPipelineLayoutCreateInfo-pSetLayouts-03038",
                                 device,
                                 error_obj.location,
                                 "sum of dynamic uniform buffer bindings among all stages (%" PRIu32 ") exceeds device "
            ...
            if (sum_all_after_bind_storage_buffer_dynamic >
                ...
                skip |= LogError("VUID-VkPipelineLayoutCreateInfo-pSetLayouts-03040",
                                 device,
                                 error_obj.location,
                                 "sum of dynamic storage buffer bindings among all stages (%" PRIu32 ") exceeds device "
        ...
        if (sum > phys_dev_props_core12.maxDescriptorSetUpdateAfterBindSampledImages) {
            skip |= LogError("VUID-VkPipelineLayoutCreateInfo-pSetLayouts-03041",
                             device,
                             error_obj.location,
                             "sum of sampled image bindings among all stages (%" PRIu32 ") exceeds device "
        ...
        if (sum > phys_dev_props_core12.maxDescriptorSetUpdateAfterBindStorageImages) {
            skip |= LogError("VUID-VkPipelineLayoutCreateInfo-pSetLayouts-03042",
                             device,
                             error_obj.location,
                             "sum of storage image bindings among all stages (%" PRIu32 ") exceeds device "
        ...
        if (sum_all_stages_update_after_bind[VK_DESCRIPTOR_TYPE_INPUT_ATTACHMENT] >
            ...
            skip |= LogError("VUID-VkPipelineLayoutCreateInfo-pSetLayouts-03043",
                             device,
                             error_obj.location,
                             "sum of input attachment bindings among all stages (%" PRIu32 ") exceeds device "
        ...
        if (inline_uniform_block_bindings > phys_dev_props_core13.maxDescriptorSetUpdateAfterBindInlineUniformBlocks) {
            skip |= LogError("VUID-VkPipelineLayoutCreateInfo-descriptorType-02217",
                             device,
                             error_obj.location,
                             "sum of inline uniform block bindings among all stages (%" PRIu32 ") exceeds device "
        ...
        if (sum_all_stages_update_after_bind[VK_DESCRIPTOR_TYPE_ACCELERATION_STRUCTURE_KHR] >
            ...
            skip |=
                LogError("VUID-VkPipelineLayoutCreateInfo-descriptorType-03574",
                         device,
                         error_obj.location,
                         "sum of acceleration structures bindings among all stages (%" PRIu32 ") exceeds device "
    ...
    if (IsExtEnabled(device_extensions.vk_ext_fragment_density_map2)) {
            ...
            for (uint32_t binding_idx = 0; binding_idx < dsl->GetBindingCount(); binding_idx++) {
                ...
                if (((binding->descriptorType == VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER) ||
                    ...
                    for (uint32_t sampler_idx = 0; sampler_idx < binding->descriptorCount; sampler_idx++) {
                        ...
                        if (state &&
                            (state->create_info.flags & (VK_SAMPLER_CREATE_SUBSAMPLED_BIT_EXT |
        ...
        if (sum_subsampled_samplers >
            phys_dev_ext_props.fragment_density_map2_props.maxDescriptorSetSubsampledSamplers) {
            skip |= LogError("VUID-VkPipelineLayoutCreateInfo-pImmutableSamplers-03566",
                             device,
                             error_obj.location,
    ...
    if (!enabled_features.graphicsPipelineLibrary) {
        for (uint32_t i = 0; i < pCreateInfo->setLayoutCount; ++i) {
            if (!pCreateInfo->pSetLayouts[i]) {
                skip |= LogError("VUID-VkPipelineLayoutCreateInfo-graphicsPipelineLibrary-06753",
                                 device,
                                 create_info_loc.dot(Field::pSetLayouts, i),
...
bool CoreChecks::ValidateCmdPushConstants(VkCommandBuffer commandBuffer,
                                          VkPipelineLayout layout,
    ...
    // Check if pipeline_layout VkPushConstantRange(s) overlapping offset, size have stageFlags set for each stage in
    // the command stageFlags argument, *and* that the command stageFlags argument has bits set for the stageFlags in
    ...
    for (const auto& range : ranges) {
        if ((offset >= range.offset) && (offset + size <= range.offset + range.size)) {
            ...
            if (matching_stages != range.stageFlags) {
                const char* vuid =
                    is_2 ? "VUID-VkPushConstantsInfo-offset-01796" : "VUID-vkCmdPushConstants-offset-01796";
                skip |= LogError(
                    vuid,
                    commandBuffer,
                    loc,
                    ...
                    string_VkShaderStageFlags(stageFlags).c_str(),
                    offset,
                    size,
                    FormatHandle(layout).c_str(),
                    string_VkShaderStageFlags(range.stageFlags).c_str(),
                    range.offset,
                    range.size);
            }
...
bool CoreChecks::PreCallValidateCmdPushConstants(VkCommandBuffer commandBuffer,
                                                 VkPipelineLayout layout,
                                                 VkShaderStageFlags stageFlags,
                                                 uint32_t offset,
                                                 uint32_t size,
...
bool CoreChecks::PreCallValidateCmdPushConstants2(VkCommandBuffer commandBuffer,
...
bool CoreChecks::PreCallValidateCreateSampler(VkDevice device,
    ...
    if (enabled_features.samplerYcbcrConversion == VK_TRUE) {
        if (const auto* conversion_info =
                vku::FindStructInPNextChain<VkSamplerYcbcrConversionInfo>(pCreateInfo->pNext)) {
            ...
            if (ycbcr_state &&
                ...
                if (pCreateInfo->minFilter != chroma_filter) {
                    skip |=
                        LogError("VUID-VkSamplerCreateInfo-minFilter-01645",
                                 device,
                                 ...
                                 "chromaFilter (%s)",
                                 FormatHandle(sampler_ycbcr_conversion).c_str(),
                                 string_VkFormat(ycbcr_state->format),
                                 string_VkFilter(pCreateInfo->minFilter),
                                 string_VkFilter(chroma_filter));
                }
                if (pCreateInfo->magFilter != chroma_filter) {
                    skip |=
                        LogError("VUID-VkSamplerCreateInfo-minFilter-01645",
                                 ...
                                 FormatHandle(sampler_ycbcr_conversion).c_str(),
                                 string_VkFormat(ycbcr_state->format),
                                 string_VkFilter(pCreateInfo->magFilter),
                                 string_VkFilter(chroma_filter));
                }
            }
        }
    ...
    if (pCreateInfo->borderColor == VK_BORDER_COLOR_INT_CUSTOM_EXT ||
        ...
        if (custom_border_color_sampler_count >=
            phys_dev_ext_props.custom_border_color_props.maxCustomBorderColorSamplers) {
            skip |= LogError("VUID-VkSamplerCreateInfo-None-04012",
                             device,
                             error_obj.location,
    ...
    if (IsExtEnabled(device_extensions.vk_khr_portability_subset)) {
        if ((VK_FALSE == enabled_features.samplerMipLodBias) && pCreateInfo->mipLodBias != 0) {
            skip |= LogError("VUID-VkSamplerCreateInfo-samplerMipLodBias-04467",
                             device,
                             error_obj.location,
                             "(portability error) mipLodBias is %f, but samplerMipLodBias not supported.",
