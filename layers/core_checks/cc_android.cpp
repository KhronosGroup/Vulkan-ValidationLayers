...
bool CoreChecks::PreCallValidateGetAndroidHardwareBufferPropertiesANDROID(
    ...
    bool skip = false;
    //  buffer must be a valid Android hardware buffer object with at least one of the AHARDWAREBUFFER_USAGE_GPU_* usage
    //  flags.
    ...
    if (0 == (ahb_desc.usage & required_flags)) {
        skip |= LogError("VUID-vkGetAndroidHardwareBufferPropertiesANDROID-buffer-01884",
                         device,
                         error_obj.location.dot(Field::buffer),
                         "AHardwareBuffer_Desc.usage (0x%" PRIx64
                         ") does not have any AHARDWAREBUFFER_USAGE_GPU_* flags set. (AHB = %p).",
                         ahb_desc.usage,
                         buffer);
    }
    return skip;
...
bool CoreChecks::PreCallValidateGetMemoryAndroidHardwareBufferANDROID(
    VkDevice device,
    ...
    if (!mem_info->IsExport() ||
        ...
        skip |= LogError("VUID-VkMemoryGetAndroidHardwareBufferInfoANDROID-handleTypes-01882",
                         device,
    ...
    if (dedicated_image != VK_NULL_HANDLE) {
        ...
        if (!image_state || (0 == (image_state->CountDeviceMemory(mem_info->VkHandle())))) {
            ...
            skip |= LogError("VUID-VkMemoryGetAndroidHardwareBufferInfoANDROID-pNext-01883",
                             objlist,
                             error_obj.location.dot(Field::pInfo).dot(Field::memory),
...
//
bool CoreChecks::ValidateAllocateMemoryANDROID(const VkMemoryAllocateInfo& allocate_info,
    ...
    if ((import_ahb_info) && (NULL != import_ahb_info->buffer)) {
        ...
        if ((AHARDWAREBUFFER_FORMAT_BLOB != ahb_desc.format) ||
            ...
            uint64_t ahb_equiv_usage_bits = AHARDWAREBUFFER_USAGE_GPU_SAMPLED_IMAGE |
                                            AHARDWAREBUFFER_USAGE_GPU_FRAMEBUFFER | AHARDWAREBUFFER_USAGE_GPU_CUBE_MAP |
                                            AHARDWAREBUFFER_USAGE_GPU_MIPMAP_COMPLETE |
                                            AHARDWAREBUFFER_USAGE_PROTECTED_CONTENT;
            if (0 == (ahb_desc.usage & ahb_equiv_usage_bits)) {
                skip |=
                    LogError("VUID-VkImportAndroidHardwareBufferInfoANDROID-buffer-01881",
                             device,
                             ahb_loc,
        ...
        if (0 == (ext_buf_props.externalMemoryProperties.externalMemoryFeatures &
            ...
            if (0 < ahb_format_map_a2v.count(ahb_desc.format))
                pdifi2.format = ahb_format_map_a2v[ahb_desc.format];
            ...
            VkResult fmt_lookup_result =
                DispatchGetPhysicalDeviceImageFormatProperties2Helper(physical_device, &pdifi2, &ifp2);

            if ((VK_SUCCESS != fmt_lookup_result) ||
                (0 == (ext_img_fmt_props.externalMemoryProperties.externalMemoryFeatures &
                       VK_EXTERNAL_MEMORY_FEATURE_IMPORTABLE_BIT))) {
                skip |= LogError("VUID-VkImportAndroidHardwareBufferInfoANDROID-buffer-01880",
                                 device,
                                 allocate_info_loc,
                                 "Neither the VkExternalImageFormatProperties nor the VkExternalBufferProperties "
                                 "structs for the AHardwareBuffer include the "
                                 "VK_EXTERNAL_MEMORY_FEATURE_IMPORTABLE_BIT flag. (AHB = %p).",
                                 import_ahb_info->buffer);
            }
        }
        ...
        // allocationSize must be the size returned by vkGetAndroidHardwareBufferPropertiesANDROID for the Android
        // hardware buffer
        if (allocate_info.allocationSize != ahb_props.allocationSize) {
            skip |= LogError("VUID-VkMemoryAllocateInfo-allocationSize-02383",
                             device,
                             allocate_info_loc.dot(Field::allocationSize),
                             "(%" PRIu64 ") does not match the %s AHardwareBuffer's allocationSize (%" PRIu64
                             "). (AHB = %p).",
                             allocate_info.allocationSize,
                             ahb_loc.Fields().c_str(),
                             ahb_props.allocationSize,
                             import_ahb_info->buffer);
        }

        // memoryTypeIndex must be one of those returned by vkGetAndroidHardwareBufferPropertiesANDROID for the
        // AHardwareBuffer Note: memoryTypeIndex is an index, memoryTypeBits is a bitmask
        uint32_t mem_type_bitmask = 1 << allocate_info.memoryTypeIndex;
        if (0 == (mem_type_bitmask & ahb_props.memoryTypeBits)) {
            skip |= LogError("VUID-VkMemoryAllocateInfo-memoryTypeIndex-02385",
                             device,
                             allocate_info_loc.dot(Field::memoryTypeIndex),
                             "(%" PRIu32 ") does not correspond to a bit set in %s "
        ...
        if ((nullptr == mem_ded_alloc_info) || (VK_NULL_HANDLE == mem_ded_alloc_info->image)) {
            ...
            if (((uint64_t)AHARDWAREBUFFER_FORMAT_BLOB != ahb_desc.format) ||
                (0 == (ahb_desc.usage & AHARDWAREBUFFER_USAGE_GPU_DATA_BUFFER))) {
                skip |= LogError("VUID-VkMemoryAllocateInfo-pNext-02384",
                                 device,
                                 ahb_loc,
            ...
            if (0 ==
                (ahb_desc.usage & (AHARDWAREBUFFER_USAGE_GPU_FRAMEBUFFER | AHARDWAREBUFFER_USAGE_GPU_SAMPLED_IMAGE |
                                   AHARDWAREBUFFER_USAGE_GPU_DATA_BUFFER))) {
                skip |= LogError("VUID-VkMemoryAllocateInfo-pNext-02386",
                                 device,
                                 ahb_loc,
                                 "AHardwareBuffer's usage is 0x%" PRIx64 ". (AHB = %p).",
                                 ahb_desc.usage,
                                 import_ahb_info->buffer);
            }
            ...
            const Location& dedicated_image_loc =
            ...
            if (VK_FORMAT_UNDEFINED != ici->format) {
                ...
                if (ici->format != ahb_format_props.format) {
                    skip |= LogError("VUID-VkMemoryAllocateInfo-pNext-02387",
                                     mem_ded_alloc_info->image,
                                     dedicated_image_loc,
                                     "was created with format (%s) which does not match the %s AHardwareBuffer's "
                                     "format (%s). (AHB = %p).",
                                     string_VkFormat(ici->format),
                                     ahb_loc.Fields().c_str(),
                                     string_VkFormat(ahb_format_props.format),
                                     import_ahb_info->buffer);
                }
            }
            ...
            if ((ici->extent.width != ahb_desc.width) || (ici->extent.height != ahb_desc.height) ||
                ...
                skip |=
                    LogError("VUID-VkMemoryAllocateInfo-pNext-02388",
                             mem_ded_alloc_info->image,
                             dedicated_image_loc,
                             "was created with width (%" PRId32 "),  height (%" PRId32 "), and arrayLayers (%" PRId32
                             ") which not match those of the %s AHardwareBuffer (%" PRId32 " %" PRId32 " %" PRId32
                             "). (AHB = %p).",
                             ici->extent.width,
                             ici->extent.height,
                             ici->arrayLayers,
            ...
            if ((ahb_desc.usage & AHARDWAREBUFFER_USAGE_GPU_MIPMAP_COMPLETE) != 0) {
                if ((ici->mipLevels != 1) && (ici->mipLevels != FullMipChainLevels(ici->extent))) {
                    skip |= LogError("VUID-VkMemoryAllocateInfo-pNext-02389",
                                     mem_ded_alloc_info->image,
                                     ahb_loc,
                                     "AHardwareBuffer_Desc's usage includes AHARDWAREBUFFER_USAGE_GPU_MIPMAP_COMPLETE "
                                     "but %s mipLevels (%" PRIu32 ") is neither 1 nor full mip "
                                     "chain levels (%" PRIu32 "). (AHB = %p).",
                                     dedicated_image_loc.Fields().c_str(),
                                     ici->mipLevels,
                                     FullMipChainLevels(ici->extent),
                                     import_ahb_info->buffer);
                }
            } else {
                if (ici->mipLevels != 1) {
                    skip |= LogError("VUID-VkMemoryAllocateInfo-pNext-02586",
                                     mem_ded_alloc_info->image,
                                     ahb_loc,
                                     "AHardwareBuffer_Desc's  usage is 0x%" PRIx64 " but %s mipLevels is %" PRIu32
                                     ". (AHB = %p).",
                                     ahb_desc.usage,
                                     dedicated_image_loc.Fields().c_str(),
                                     ici->mipLevels,
            ...
            if (ici->usage & ~(VK_IMAGE_USAGE_SAMPLED_BIT | VK_IMAGE_USAGE_INPUT_ATTACHMENT_BIT |
                ...
                skip |= LogError("VUID-VkMemoryAllocateInfo-pNext-02390",
                                 mem_ded_alloc_info->image,
                                 dedicated_image_loc,
                                 "usage bits (%s) include an issue not listed in the AHardwareBuffer Usage Equivalence "
                                 "table. (AHB = %p).",
                                 string_VkImageUsageFlags(ici->usage).c_str(),
                                 import_ahb_info->buffer);
            }

            std::vector<VkImageUsageFlags> usages = { VK_IMAGE_USAGE_SAMPLED_BIT,
            ...
            for (VkImageUsageFlags ubit : usages) {
                if (ici->usage & ubit) {
                    ...
                    if (0 == (ahb_usage & ahb_desc.usage)) {
                        skip |= LogError("VUID-VkMemoryAllocateInfo-pNext-02390",
                                         mem_ded_alloc_info->image,
                                         dedicated_image_loc,
                                         " usage bit %s equivalent is not in AHardwareBuffer_Desc.usage (0x%" PRIx64
                                         "). (AHB = %p).",
                                         string_VkImageUsageFlags(ubit).c_str(),
                                         ahb_desc.usage,
                                         import_ahb_info->buffer);
                    }
                }
             ...
             // auto exp_mem_alloc_info = vku::FindStructInPNextChain<VkExportMemoryAllocateInfo>(allocate_info.pNext);
             // auto mem_ded_alloc_info =
             // vku::FindStructInPNextChain<VkMemoryDedicatedAllocateInfo>(allocate_info.pNext);
        ...
        if (exp_mem_alloc_info &&
            ...
            if (mem_ded_alloc_info) {
                if (mem_ded_alloc_info->image != VK_NULL_HANDLE && allocate_info.allocationSize != 0) {
                    skip |= LogError("VUID-VkMemoryAllocateInfo-pNext-01874",
                                     mem_ded_alloc_info->image,
                                     allocate_info_loc.pNext(Struct::VkMemoryDedicatedAllocateInfo, Field::image),
                                     "is %s but allocationSize is %" PRIu64 ".",
                                     FormatHandle(mem_ded_alloc_info->image).c_str(),
                                     allocate_info.allocationSize);
                }
                if (mem_ded_alloc_info->buffer != VK_NULL_HANDLE && allocate_info.allocationSize == 0) {
                    skip |= LogError("VUID-VkMemoryAllocateInfo-pNext-07901",
                                     mem_ded_alloc_info->buffer,
                                     allocate_info_loc.pNext(Struct::VkMemoryDedicatedAllocateInfo, Field::buffer),
                                     "is %s but allocationSize is 0.",
                                     FormatHandle(mem_ded_alloc_info->buffer).c_str());
                }
...
bool CoreChecks::ValidateGetPhysicalDeviceImageFormatProperties2ANDROID(
    ...
    const ErrorObject& error_obj) const {
    bool skip = false;
    const auto* ahb_usage =
        vku::FindStructInPNextChain<VkAndroidHardwareBufferUsageANDROID>(pImageFormatProperties->pNext);
    if (ahb_usage) {
        const auto* pdeifi =
        ...
        if ((!pdeifi) || (VK_EXTERNAL_MEMORY_HANDLE_TYPE_ANDROID_HARDWARE_BUFFER_BIT_ANDROID != pdeifi->handleType)) {
            skip |=
                LogError("VUID-vkGetPhysicalDeviceImageFormatProperties2-pNext-01868",
                         physical_device,
...
bool CoreChecks::ValidateBufferImportedHandleANDROID(VkExternalMemoryHandleTypeFlags handle_types,
...
bool CoreChecks::ValidateImageImportedHandleANDROID(VkExternalMemoryHandleTypeFlags handle_types,
                                                    VkDeviceMemory memory,
                                                    VkImage image,
                                                    const Location& loc) const {
    bool skip = false;
...
bool CoreChecks::ValidateCreateImageANDROID(const VkImageCreateInfo& create_info,
                                            const Location& create_info_loc) const {
    bool skip = false;

    const VkExternalFormatANDROID* ext_fmt_android =
        vku::FindStructInPNextChain<VkExternalFormatANDROID>(create_info.pNext);
    if (ext_fmt_android && (0 != ext_fmt_android->externalFormat)) {
        if (VK_FORMAT_UNDEFINED != create_info.format) {
            skip |= LogError("VUID-VkImageCreateInfo-pNext-01974",
                             device,
                             create_info_loc.pNext(Struct::VkExternalFormatANDROID, Field::externalFormat),
                             "(%" PRIu64 ") is non-zero, format is %s.",
        ...
        if (0 != (VK_IMAGE_CREATE_MUTABLE_FORMAT_BIT & create_info.flags)) {
            skip |= LogError("VUID-VkImageCreateInfo-pNext-02396",
                             device,
                             create_info_loc.pNext(Struct::VkExternalFormatANDROID, Field::externalFormat),
                             "(%" PRIu64 ") is non-zero, but flags is %s.",
                             ext_fmt_android->externalFormat,
        ...
        if (0 !=
            (~(VK_IMAGE_USAGE_SAMPLED_BIT | VK_IMAGE_USAGE_COLOR_ATTACHMENT_BIT | VK_IMAGE_USAGE_INPUT_ATTACHMENT_BIT) &
             create_info.usage)) {
            skip |= LogError("VUID-VkImageCreateInfo-pNext-02397",
                             device,
                             create_info_loc.pNext(Struct::VkExternalFormatANDROID, Field::externalFormat),
                             "(%" PRIu64 ") is non-zero, but usage is %s.",
                             ext_fmt_android->externalFormat,
            ...
            skip |= LogError("VUID-VkImageCreateInfo-pNext-09457",
                             device,
                             create_info_loc.pNext(Struct::VkExternalFormatANDROID, Field::externalFormat),
                             "(%" PRIu64 ") is non-zero, but usage is %s (without externalFormatResolve, only "
        ...
        if (VK_IMAGE_TILING_OPTIMAL != create_info.tiling) {
            skip |= LogError("VUID-VkImageCreateInfo-pNext-02398",
                             device,
                             create_info_loc.pNext(Struct::VkExternalFormatANDROID, Field::externalFormat),
                             "(%" PRIu64 ") is non-zero, but layout is %s.",
                             ext_fmt_android->externalFormat,
        ...
        if (ahb_ext_formats_map.find(ext_fmt_android->externalFormat) == ahb_ext_formats_map.end()) {
            skip |= LogError("VUID-VkExternalFormatANDROID-externalFormat-01894",
                             device,
                             create_info_loc.pNext(Struct::VkExternalFormatANDROID, Field::externalFormat),
                             "(%" PRIu64
                             ") has not been previously retrieved by vkGetAndroidHardwareBufferPropertiesANDROID().",
                             ext_fmt_android->externalFormat);
        }
    }
    ...
    if ((nullptr == ext_fmt_android) || (0 == ext_fmt_android->externalFormat)) {
        if (VK_FORMAT_UNDEFINED == create_info.format) {
            if (ext_fmt_android) {
                ...
                skip |= LogError("VUID-VkImageCreateInfo-pNext-01975",
                                 device,
                                 create_info_loc.dot(Field::format),
    ...
    const VkExternalMemoryImageCreateInfo* emici =
        vku::FindStructInPNextChain<VkExternalMemoryImageCreateInfo>(create_info.pNext);
    if (emici && (emici->handleTypes & VK_EXTERNAL_MEMORY_HANDLE_TYPE_ANDROID_HARDWARE_BUFFER_BIT_ANDROID)) {
        ...
        if ((create_info.mipLevels != 1) && (create_info.mipLevels != FullMipChainLevels(create_info.extent))) {
            skip |= LogError("VUID-VkImageCreateInfo-pNext-02394",
                             device,
                             create_info_loc.pNext(Struct::VkExternalMemoryImageCreateInfo, Field::handleTypes),
                             ...
                             FullMipChainLevels(create_info.extent));
        }
    }
...
bool CoreChecks::ValidateCreateImageViewANDROID(const VkImageViewCreateInfo& create_info,
    ...
    if (image_state->HasAHBFormat()) {
        if (VK_FORMAT_UNDEFINED != create_info.format) {
            skip |= LogError("VUID-VkImageViewCreateInfo-image-02399",
                             create_info.image,
                             create_info_loc.dot(Field::format),
        ...
        if (!conv_found) {
            ...
            skip |= LogError("VUID-VkImageViewCreateInfo-image-02400",
                             objlist,
            ...
            skip |=
                LogError("VUID-VkImageViewCreateInfo-image-02400",
                         objlist,
                         create_info_loc.pNext(Struct::VkSamplerYcbcrConversionInfo, Field::conversion),
                         ...
                         FormatHandle(ycbcr_conv_info->conversion).c_str(),
                         external_format,
        ...
        if (IsIdentitySwizzle(create_info.components) == false) {
            skip |= LogError("VUID-VkImageViewCreateInfo-image-02401",
                             create_info.image,
