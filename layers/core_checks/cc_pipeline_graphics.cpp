...
bool CoreChecks::PreCallValidateCreateGraphicsPipelines(VkDevice device,
    ...
    bool skip = BaseClass::PreCallValidateCreateGraphicsPipelines(
        device, pipelineCache, count, pCreateInfos, pAllocator, pPipelines, error_obj, pipeline_states, chassis_state);
    ...
    for (uint32_t i = 0; i < count; i++) {
        ...
        // From dumping traces, we found almost all apps only create one pipeline at a time. To greatly simplify the
        // logic, only check the stateless validation in the pNext chain for the first pipeline. (The core issue is
        // because we parse the SPIR-V at state tracking time, and we state track pipelines first)
        if (i == 0) {
            ...
            for (uint32_t stage = 0; stage < stage_count; stage++) {
                if (chassis_state.stateless_data[stage].pipeline_pnext_module) {
                    skip |= ValidateSpirvStateless(*chassis_state.stateless_data[stage].pipeline_pnext_module,
                                                   chassis_state.stateless_data[stage],
                                                   create_info_loc.dot(Field::pStages, stage)
                                                       .pNext(Struct::VkShaderModuleCreateInfo, Field::pCode));
                }
            }
        }
    }
...
bool CoreChecks::ValidateGraphicsPipeline(const vvl::Pipeline& pipeline,
                                          const void* pipeline_ci_pnext,
                                          const Location& create_info_loc) const {
    bool skip = false;
    // It would be ideal to split all pipeline checks into Dynamic and Non-Dynamic renderpasses, but with GPL it gets a
    // bit tricky. Also you might be deep in a function and it is easier to do a if/else check if it is dynamic
    // rendering or not there.
    ...
    if (rp_state && rp_state->UsesDynamicRendering()) {
        ...
        if (subpass >= rp_state->create_info.subpassCount) {
            skip |= LogError("VUID-VkGraphicsPipelineCreateInfo-renderPass-06046",
                             rp_state->Handle(),
                             create_info_loc.dot(Field::subpass),
                             "(%" PRIu32 ") is out of range for this renderpass (0..%" PRIu32 ").",
                             subpass,
                             rp_state->create_info.subpassCount - 1);
            ...
            skip |=
                ValidateGraphicsPipelineRenderPassRasterization(pipeline, *rp_state, *subpass_desc, create_info_loc);

            if (subpass_desc->viewMask != 0) {
                skip |= ValidateMultiViewShaders(pipeline,
                                                 create_info_loc.dot(Field::pSubpasses, subpass).dot(Field::viewMask),
    ...
    if (pipeline.OwnsSubState(pipeline.pre_raster_state) || pipeline.OwnsSubState(pipeline.fragment_shader_state)) {
        ...
        for (const auto& stage_ci : pipeline.shader_stages_ci) {
            ...
            if (it != unique_stage_map.end()) {
                skip |= LogError("VUID-VkGraphicsPipelineCreateInfo-stage-06897",
                                 device,
                                 create_info_loc.dot(Field::pStages, index),
                                 "and pStages[%" PRIu32 "] both have %s",
                                 it->second,
    ...
    if (pipeline.OwnsSubState(pipeline.fragment_shader_state) || pipeline.OwnsSubState(pipeline.pre_raster_state)) {
        ...
        for (const auto& stage_ci : pipeline.shader_stages_ci) {
            skip |= ValidatePipelineShaderStage(
                pipeline, stage_ci, pipeline_ci_pnext, create_info_loc.dot(Field::pStages, stage_index++));
        }
    }

    skip |= ValidatePipelineCacheControlFlags(pipeline.create_flags,
    ...
    if (const auto attachment_sample_count_info =
            vku::FindStructInPNextChain<VkAttachmentSampleCountInfoAMD>(pipeline_pnext)) {
    ...
    if (const auto* pipeline_robustness_info =
            vku::FindStructInPNextChain<VkPipelineRobustnessCreateInfo>(pipeline_pnext)) {
    ...
    if (const auto* fragment_shading_rate_state =
        ...
        skip |=
            ValidateGraphicsPipelineFragmentShadingRateState(pipeline, *fragment_shading_rate_state, create_info_loc);
    }

    return skip;
}

bool CoreChecks::ValidateGraphicsPipelinePortability(const vvl::Pipeline& pipeline,
                                                     const Location& create_info_loc) const {
    bool skip = false;
    if (!enabled_features.triangleFans && (pipeline.topology_at_rasterizer == VK_PRIMITIVE_TOPOLOGY_TRIANGLE_FAN)) {
        skip |= LogError("VUID-VkPipelineInputAssemblyStateCreateInfo-triangleFans-04452",
                         device,
                         create_info_loc,
        ...
        for (const auto& state : pipeline.vertex_input_state->bindings) {
            ...
            if (min_alignment != 0 && (desc.stride % min_alignment != 0)) {
                skip |= LogError(
                    ...
                    device,
                    create_info_loc,
                    "(portability error): Vertex input stride (%" PRIu32 ") must be at least as large as and a "
            ...
            for (const auto& binding_state : pipeline.vertex_input_state->bindings) {
                for (const auto& attrib_state : binding_state.second.locations) {
                    ...
                    if ((attrib.offset + vkuFormatElementSize(attrib.format)) > desc.stride) {
                        skip |=
                            LogError("VUID-VkVertexInputAttributeDescription-vertexAttributeAccessBeyondStride-04457",
                                     device,
                                     create_info_loc,
                                     ...
                                     attrib.offset,
                                     vkuFormatElementSize(attrib.format),
                                     desc.stride);
                    }
                }
            }
    ...
    if (const auto raster_state_ci = pipeline.RasterizationState()) {
        ...
        if (!enabled_features.pointPolygons && !raster_state_ci->rasterizerDiscardEnable &&
            ...
            skip |= LogError("VUID-VkPipelineRasterizationStateCreateInfo-pointPolygons-04458",
                             device,
                             create_info_loc,
        ...
        if (!enabled_features.separateStencilMaskRef && (raster_state_ci->cullMode == VK_CULL_MODE_NONE)) {
            ...
            if (ds_state && ds_state->stencilTestEnable && (ds_state->front.reference != ds_state->back.reference)) {
                skip |= LogError("VUID-VkPipelineDepthStencilStateCreateInfo-separateStencilMaskRef-04453",
                                 device,
                                 create_info_loc,
        ...
        if (!enabled_features.constantAlphaColorBlendFactors && !ignore_color_blend_state && color_blend_state) {
            ...
            for (uint32_t color_attachment_index = 0; color_attachment_index < color_blend_state->attachmentCount;
                ...
                if ((attachments[color_attachment_index].srcColorBlendFactor == VK_BLEND_FACTOR_CONSTANT_ALPHA) ||
                    (attachments[color_attachment_index].srcColorBlendFactor ==
                     VK_BLEND_FACTOR_ONE_MINUS_CONSTANT_ALPHA)) {
                    skip |=
                        LogError("VUID-VkPipelineColorBlendAttachmentState-constantAlphaColorBlendFactors-04454",
                                 device,
                                 create_info_loc,
                                 "(portability error): srcColorBlendFactor for color attachment %" PRIu32 " must "
                ...
                if ((attachments[color_attachment_index].dstColorBlendFactor == VK_BLEND_FACTOR_CONSTANT_ALPHA) ||
                    (attachments[color_attachment_index].dstColorBlendFactor ==
                     VK_BLEND_FACTOR_ONE_MINUS_CONSTANT_ALPHA)) {
                    skip |=
                        LogError("VUID-VkPipelineColorBlendAttachmentState-constantAlphaColorBlendFactors-04455",
                                 device,
                                 create_info_loc,
                                 "(portability error): dstColorBlendFactor for color attachment %" PRIu32 " must "
                                 "not be VK_BLEND_FACTOR_CONSTANT_ALPHA or VK_BLEND_FACTOR_ONE_MINUS_CONSTANT_ALPHA",
                                 color_attachment_index);
                }
            }
...
bool CoreChecks::ValidatePipelineLibraryCreateInfo(const vvl::Pipeline& pipeline,
    ...
    const bool has_retain_link_time_opt =
        (pipeline_flags & VK_PIPELINE_CREATE_2_RETAIN_LINK_TIME_OPTIMIZATION_INFO_BIT_EXT) != 0;
    const bool has_capture_internal =
        (pipeline_flags & VK_PIPELINE_CREATE_2_CAPTURE_INTERNAL_REPRESENTATIONS_BIT_KHR) != 0;
    bool uses_descriptor_buffer = false;
    bool lib_all_has_capture_internal = false;

    const auto gpl_info =
        vku::FindStructInPNextChain<VkGraphicsPipelineLibraryCreateInfoEXT>(pipeline.GetCreateInfoPNext());
    ...
    for (uint32_t i = 0; i < library_create_info.libraryCount; ++i) {
        ...
        if (has_link_time_opt && !lib_has_retain_link_time_opt) {
            ...
            skip |= LogError("VUID-VkGraphicsPipelineCreateInfo-flags-06609",
                             objlist,
                             library_loc,
        ...
        if (has_retain_link_time_opt && !lib_has_retain_link_time_opt) {
            const LogObjectList objlist(device, lib->Handle());
            skip |= LogError("VUID-VkGraphicsPipelineCreateInfo-flags-06610",
                             objlist,
                             library_loc,
                             "(%s) was created with %s, which is missing "
                             "VK_PIPELINE_CREATE_RETAIN_LINK_TIME_OPTIMIZATION_INFO_BIT_EXT, %s is %s.",
        ...
        if (lib_has_capture_internal) {
            ...
            if (!has_capture_internal && non_zero_gpl) {
                const Location& gpl_flags_loc =
                    create_info_loc.pNext(Struct::VkGraphicsPipelineLibraryCreateInfoEXT, Field::flags);
                const LogObjectList objlist(device, lib->Handle());
                skip |= LogError("VUID-VkGraphicsPipelineCreateInfo-pLibraries-06647",
                                 objlist,
                                 library_loc,
            ...
            if (lib_all_has_capture_internal) {
                const LogObjectList objlist(device, lib->Handle());
                skip |= LogError("VUID-VkGraphicsPipelineCreateInfo-pLibraries-06646",
                                 objlist,
                                 library_loc,
                                 "(%s) was created with %s.",
                                 string_VkGraphicsPipelineLibraryFlagsEXT(lib->graphics_lib_type).c_str(),
                ...
                const Location& gpl_flags_loc =
                    create_info_loc.pNext(Struct::VkGraphicsPipelineLibraryCreateInfoEXT, Field::flags);
                const LogObjectList objlist(device, lib->Handle());
                skip |= LogError("VUID-VkGraphicsPipelineCreateInfo-flags-06645",
                                 objlist,
                                 library_loc,
                                 "(%s) was created with %s\n"
                                 "%s is %s\n"
        ...
        if ((lib->uses_shader_module_id) &&
            !(pipeline_flags & VK_PIPELINE_CREATE_2_FAIL_ON_PIPELINE_COMPILE_REQUIRED_BIT)) {
        ...
        for (const auto& check_info : check_infos) {
            if ((pipeline_flags & check_info.bit)) {
                if (!(lib_pipeline_flags & check_info.bit)) {
                    const LogObjectList objlist(device, lib->Handle());
                    skip |= LogError(check_info.first_vuid.c_str(),
                                     objlist,
                                     library_loc,
                                     "(%s) was created with %s, which is missing %s included in %s (%s).",
                                     string_VkGraphicsPipelineLibraryFlagsEXT(lib->graphics_lib_type).c_str(),
                                     string_VkPipelineCreateFlags2(lib_pipeline_flags).c_str(),
                                     string_VkPipelineCreateFlagBits2(check_info.bit),
                                     create_info_loc.dot(Field::flags).Fields().c_str(),
                                     string_VkPipelineCreateFlags2(pipeline_flags).c_str());
                }
            } else {
                if ((lib_pipeline_flags & check_info.bit)) {
                    const LogObjectList objlist(device, lib->Handle());
                    skip |= LogError(check_info.second_vuid.c_str(),
                                     objlist,
                                     library_loc,
    ...
    if (pipeline.GraphicsCreateInfo().renderPass == VK_NULL_HANDLE) {
        if (gpl_info) {
            skip |= ValidatePipelineLibraryFlags(gpl_info->flags,
                                                 library_create_info,
                                                 pipeline.rendering_create_info,
                                                 create_info_loc,
                                                 -1,
        ...
        for (uint32_t i = 0; i < library_create_info.libraryCount; ++i) {
            ...
            skip |= ValidatePipelineLibraryFlags(lib->graphics_lib_type,
                                                 library_create_info,
                                                 lib_rendering_struct,
                                                 create_info_loc,
                                                 i,
...
bool CoreChecks::ValidateGraphicsPipelineVertexInputState(const vvl::Pipeline& pipeline,
                                                          const Location& create_info_loc) const {
    bool skip = false;
    ...
    const bool ignore_input_assembly_state =
    ...
    if (invalid_input_state && invalid_assembly_state && pipeline.IsGraphicsLibrary()) {
        ...
        skip |= LogError("VUID-VkGraphicsPipelineCreateInfo-pStages-02097",
                         device,
                         create_info_loc.dot(Field::pVertexInputState),
                         "is NULL.");
    } else if (invalid_assembly_state) {
        skip |= LogError("VUID-VkGraphicsPipelineCreateInfo-dynamicPrimitiveTopologyUnrestricted-09031",
                         device,
...
bool CoreChecks::ValidateGraphicsPipelineNullRenderPass(const vvl::Pipeline& pipeline,
                                                        const Location& create_info_loc) const {
    bool skip = false;
    ...
    if (pipeline.IsRenderPassNull()) {
        if (!enabled_features.dynamicRendering) {
            skip |= LogError("VUID-VkGraphicsPipelineCreateInfo-dynamicRendering-06576",
                             device,
                             create_info_loc.dot(Field::renderPass),
                             "is NULL, but the dynamicRendering feature was not enabled");
        }
    } else if (!pipeline.RenderPassState()) {
        // If the vvl::RenderPass object is not found AND the handle is not null, this we know this is an invalid render
        // pass
        const auto gpl_info =
            vku::FindStructInPNextChain<VkGraphicsPipelineLibraryCreateInfoEXT>(pipeline.GetCreateInfoPNext());
        ...
        const char* vuid = has_flags ? "VUID-VkGraphicsPipelineCreateInfo-flags-06643"
...
bool CoreChecks::ValidateGraphicsPipelineLibrary(const vvl::Pipeline& pipeline, const Location& create_info_loc) const {
    ...
    if (pipeline.OwnsSubState(pipeline.pre_raster_state) && !pipeline.OwnsSubState(pipeline.fragment_shader_state) &&
        ...
        skip |=
            LogError("VUID-VkGraphicsPipelineCreateInfo-pStages-06894",
                     device,
                     create_info_loc,
    ...
    if (!pipeline.OwnsSubState(pipeline.fragment_shader_state) && !pipeline.OwnsSubState(pipeline.pre_raster_state) &&
        ...
        skip |= LogError("VUID-VkGraphicsPipelineCreateInfo-stageCount-09587",
                         device,
    ...
    if (is_create_library) {
        if (!enabled_features.graphicsPipelineLibrary) {
            skip |= LogError("VUID-VkGraphicsPipelineCreateInfo-graphicsPipelineLibrary-06606",
                             device,
                             create_info_loc.dot(Field::flags),
    ...
    if (pipeline.OwnsSubState(pipeline.fragment_shader_state) && !pipeline.OwnsSubState(pipeline.pre_raster_state) &&
        ...
        skip |= LogError("VUID-VkGraphicsPipelineCreateInfo-pStages-06895",
                         device,
                         create_info_loc,
    ...
    if (pipeline.HasFullState()) {
        if (is_create_library) {
            skip |= LogError("VUID-VkGraphicsPipelineCreateInfo-flags-06608",
                             device,
                             create_info_loc.dot(Field::flags),
        ...
        if (!pipeline_layout_state &&
            (!pipeline.library_create_info || pipeline.library_create_info->libraryCount == 0)) {
            skip |= LogError("VUID-VkGraphicsPipelineCreateInfo-None-07826",
                             device,
        ...
        if (from_libraries_only) {
            ...
            if (!pre_raster_independent_set && !fs_independent_set) {
                // The layout defined at link time must be compatible with each (pre-raster and fragment shader) sub
                // state's layout (vertex input and fragment output state do not contain a layout)
                if (pipeline_layout_state) {
                    if (std::string err_msg; !VerifySetLayoutCompatibility(
                        ...
                        LogObjectList objlist(pipeline_layout_state->Handle(),
                                              pipeline.PreRasterPipelineLayoutState()->Handle());
                        skip |=
                            LogError("VUID-VkGraphicsPipelineCreateInfo-layout-07827",
                                     objlist,
                                     create_info_loc.dot(Field::layout),
                                     "is incompatible with the layout specified in the pre-rasterization library: %s",
                                     err_msg.c_str());
                    }
                    if (std::string err_msg; !VerifySetLayoutCompatibility(
                        ...
                        skip |= LogError("VUID-VkGraphicsPipelineCreateInfo-layout-07827",
                                         objlist,
                                         create_info_loc.dot(Field::layout),
                                         "is incompatible with the layout specified in the fragment shader library: %s",
                                         err_msg.c_str());
                    }
                } else {
                    skip |= LogError("VUID-VkGraphicsPipelineCreateInfo-layout-07827",
            ...
            const bool has_link_time_opt =
                (pipeline.create_flags & VK_PIPELINE_CREATE_2_LINK_TIME_OPTIMIZATION_BIT_EXT) != 0;
            if (!has_link_time_opt && (pre_raster_independent_set && fs_independent_set)) {
                if (pipeline_layout_state) {
                    if (std::string err_msg;
                        !VerifySetLayoutCompatibilityUnion(*pipeline_layout_state,
                                                           *pipeline.PreRasterPipelineLayoutState(),
                                                           *pipeline.FragmentShaderPipelineLayoutState(),
                                                           err_msg)) {
                        LogObjectList objlist(pipeline_layout_state->Handle(),
                                              pipeline.PreRasterPipelineLayoutState()->Handle(),
                                              pipeline.FragmentShaderPipelineLayoutState()->Handle());
                    ...
                    skip |= LogError("VUID-VkGraphicsPipelineCreateInfo-flags-06730",
                                     device,
                                     create_info_loc.dot(Field::layout),
    ...
    if (gpl_info) {
        ...
        if (gpl_info->flags & VK_GRAPHICS_PIPELINE_LIBRARY_FRAGMENT_SHADER_BIT_EXT) {
            ...
            frag_shader_info.flags = (pipeline.FragmentShaderPipelineLayoutState())
    ...
    if ((pipeline.OwnsSubState(pipeline.pre_raster_state) || pipeline.OwnsSubState(pipeline.fragment_shader_state)) &&
        ...
        const char* vuid =
            (pre_raster_info.init == GPLInitType::gpl_flags || frag_shader_info.init == GPLInitType::gpl_flags)
    ...
    if (pipeline_layout_state && pre_raster_info.init == GPLInitType::gpl_flags) {
        for (uint32_t i = 0; i < pipeline_layout_state->set_layouts.size(); i++) {
            ...
            if (!pipeline.IsDynamic(CB_DYNAMIC_STATE_RASTERIZER_DISCARD_ENABLE) && pipeline.RasterizationDisabled()) {
                skip |= LogError("VUID-VkGraphicsPipelineCreateInfo-flags-06683",
                                 device,
                                 create_info_loc.pNext(Struct::VkGraphicsPipelineLibraryCreateInfoEXT, Field::flags),
                ...
                skip |= LogError("VUID-VkGraphicsPipelineCreateInfo-flags-06682",
                                 device,
                                 create_info_loc.pNext(Struct::VkGraphicsPipelineLibraryCreateInfoEXT, Field::flags),
                                 "is %s, but layout was created with pSetLayouts[%" PRIu32 "] == VK_NULL_HANDLE",
                                 string_VkGraphicsPipelineLibraryFlagsEXT(gpl_info->flags).c_str(),
    ...
    if ((pre_raster_info.init != GPLInitType::uninitialized) && (frag_shader_info.init != GPLInitType::uninitialized)) {
        // For VUs saying "includes only one... pLibraries includes the other flag" this would be when |only_libs ==
        // false| This is because it is not valid to have both init be from |gpl_flags|
        const bool only_libs = (pre_raster_info.init == GPLInitType::link_libraries) &&
                               (frag_shader_info.init == GPLInitType::link_libraries);

        // Check for consistent independent sets across libraries
        const auto pre_raster_independant_set =
            (pre_raster_info.flags & VK_PIPELINE_LAYOUT_CREATE_INDEPENDENT_SETS_BIT_EXT);
        ...
        if (pre_raster_independant_set ^ fs_independant_set) {
            const char* vuid = only_libs ? "VUID-VkGraphicsPipelineCreateInfo-pLibraries-06615"
            ...
            skip |= LogError(vuid,
                             objlist,
                             create_info_loc,
                             "is attempting to create a graphics pipeline library with pre-raster and fragment shader "
                             "state. However "
                             "the pre-raster layout create flags (%s) are %s defined with "
                             "VK_PIPELINE_LAYOUT_CREATE_INDEPENDENT_SETS_BIT_EXT, "
                             "and the fragment shader layout create flags (%s) are %s defined with "
                             "VK_PIPELINE_LAYOUT_CREATE_INDEPENDENT_SETS_BIT_EXT",
                             string_VkPipelineLayoutCreateFlags(pre_raster_info.flags).c_str(),
                             (pre_raster_independant_set != 0) ? "" : "not",
                             string_VkPipelineLayoutCreateFlags(frag_shader_info.flags).c_str(),
                             (fs_independant_set != 0) ? "" : "not");
        } else if (not_independent_sets) {
            // "layout used by this pipeline and the library must be identically defined"
            // Inside VkPipelineLayoutCreateInfo, |pSetLayouts| and |pPushConstantRanges| are checked below, this leaves
            // this VU to cover everything else. Currently no valid pNext structs are extending pipeline layout creation
        ...
        if (!pre_raster_info.layout->push_constant_ranges_layout->empty() &&
            ...
            const uint32_t pre_raster_count =
                static_cast<uint32_t>(pre_raster_info.layout->push_constant_ranges_layout->size());
            const uint32_t frag_shader_count =
                static_cast<uint32_t>(frag_shader_info.layout->push_constant_ranges_layout->size());
            ...
            if (pre_raster_count != frag_shader_count) {
                ...
                for (uint32_t i = 0; i < pre_raster_count; i++) {
                    ...
                    if (pre_raster_range.stageFlags != frag_shader_range.stageFlags ||
                        pre_raster_range.offset != frag_shader_range.offset ||
                        pre_raster_range.size != frag_shader_range.size) {
                        const char* vuid = only_libs ? "VUID-VkGraphicsPipelineCreateInfo-pLibraries-06621"
                                                     : "VUID-VkGraphicsPipelineCreateInfo-flags-06620";
                        LogObjectList objlist(pre_raster_info.layout->Handle(), frag_shader_info.layout->Handle());
                        skip |= LogError(
                            ...
                            ", %" PRIu32 "}.",
                            i,
                            string_VkShaderStageFlags(pre_raster_range.stageFlags).c_str(),
                            pre_raster_range.offset,
                            pre_raster_range.size,
                            i,
                            string_VkShaderStageFlags(frag_shader_range.stageFlags).c_str(),
                            frag_shader_range.offset,
        ...
        if (frag_shader_info.shading_rate_state || pre_raster_info.shading_rate_state) {
            const char* vuid = only_libs ? "VUID-VkGraphicsPipelineCreateInfo-pLibraries-06639"
                                         : "VUID-VkGraphicsPipelineCreateInfo-flags-06638";
            if (!pre_raster_info.shading_rate_state) {
                skip |= LogError(vuid,
                                 device,
                                 create_info_loc,
                                 "Fragment Shader has a valid VkPipelineFragmentShadingRateStateCreateInfoKHR, but Pre "
                                 "Rasterization has "
                ...
                skip |= LogError(vuid,
                                 device,
                                 create_info_loc,
                                 "Pre Rasterization has a valid VkPipelineFragmentShadingRateStateCreateInfoKHR, but "
                                 "Fragment Shader has "
                                 "no VkPipelineFragmentShadingRateStateCreateInfoKHR in the pNext chain.");
        ...
        for (uint32_t i = 0; i < num_set_layouts; ++i) {
            ...
            if (!pre_raster_dsl && fs_dsl) {
                // Null DSL at pSetLayouts[i] in pre-raster state. Make sure that shader bindings in corresponding DSL
                // in fragment shader state do not overlap.
                ...
                // Null DSL at pSetLayouts[i] in FS state. Make sure that shader bindings in corresponding DSL in
                // pre-raster state do not overlap.
                ...
                if (frag_shader_info.init == GPLInitType::gpl_flags) {
                    ...
                    msg << "is linking libraries with pre-raster and fragment shader state. The descriptor set "
                        ...
                        << " in the fragment shader state is NULL. "
                        << "However, the descriptor set layout at the same index (" << i << ") in "
                        << pre_raster_layout_handle_str << " in the pre-raster state is NULL too.";
                }
                LogObjectList objlist(pre_raster_info.layout->Handle(), frag_shader_info.layout->Handle());
                skip |= LogError(vuid, objlist, create_info_loc, "%s", msg.str().c_str());
                break;
            } else if (not_independent_sets) {
                // both handles are valid, but without VK_PIPELINE_LAYOUT_CREATE_INDEPENDENT_SETS_BIT_EXT, need to check
                // everything is identically defined
                if (pre_raster_dsl->GetCreateFlags() != fs_dsl->GetCreateFlags()) {
                    ...
                    skip |= LogError(
                        ...
                        i,
                        pre_raster_dsl->GetBindingCount(),
                        i,
                        fs_dsl->GetBindingCount());
                } else {
                    ...
                    for (uint32_t binding_index = 0; binding_index < binding_count; binding_index++) {
                        ...
                        if (!CompareDescriptorSetLayoutBinding(*pre_raster_binding.ptr(), *fs_binding.ptr())) {
                            ...
                            skip |= LogError(vuid,
                                             objlist,
                                             create_info_loc,
                                             "VK_PIPELINE_LAYOUT_CREATE_INDEPENDENT_SETS_BIT_EXT was not set and the "
                                             "graphics pipeline "
                                             "library have different pipeline layouts, pre-raster layout has "
                                             "pSetLayouts[%" PRIu32 "] pBindings[%" PRIu32 "] of:\n"
                                             "\tbinding: %" PRIu32 "\n"
                                             "\tdescriptorType: %s\n"
                                             ...
                                             i,
                                             binding_index,
                                             pre_raster_binding.binding,
                                             string_VkDescriptorType(pre_raster_binding.descriptorType),
                                             pre_raster_binding.descriptorCount,
                                             string_VkShaderStageFlags(pre_raster_binding.stageFlags).c_str(),
                                             pre_raster_binding.pImmutableSamplers,
                                             i,
                                             binding_index,
                                             fs_binding.binding,
                                             string_VkDescriptorType(fs_binding.descriptorType),
                                             fs_binding.descriptorCount,
                                             string_VkShaderStageFlags(fs_binding.stageFlags).c_str(),
    ...
    if ((frag_shader_info.init != GPLInitType::uninitialized) &&
        (frag_output_info.init != GPLInitType::uninitialized)) {
        if (!frag_shader_info.ms_state && frag_output_info.ms_state) {
            ...
            if (frag_output_info.ms_state->sampleShadingEnable) {
                const char* vuid = (frag_shader_info.init == GPLInitType::gpl_flags)
                ...
                skip |= LogError(vuid,
                                 device,
                                 create_info_loc,
                                 "Fragment Output Interface were created with "
                                 "VkPipelineMultisampleStateCreateInfo::sampleShadingEnable to "
            ...
            if (!ComparePipelineMultisampleStateCreateInfo(*frag_shader_info.ms_state, *frag_output_info.ms_state)) {
                ...
                skip |= LogError(vuid,
                                 ...
                                 frag_shader_info.ms_state->sampleShadingEnable,
                                 frag_shader_info.ms_state->minSampleShading,
                                 frag_shader_info.ms_state->pSampleMask,
                                 frag_shader_info.ms_state->alphaToCoverageEnable,
                                 frag_shader_info.ms_state->alphaToOneEnable,
                                 frag_output_info.ms_state->pNext,
                                 string_VkSampleCountFlagBits(frag_output_info.ms_state->rasterizationSamples),
                                 frag_output_info.ms_state->sampleShadingEnable,
                                 frag_output_info.ms_state->minSampleShading,
                                 frag_output_info.ms_state->pSampleMask,
                                 frag_output_info.ms_state->alphaToCoverageEnable,
...
bool CoreChecks::ValidateGraphicsPipelineBlendEnable(const vvl::Pipeline& pipeline,
                                                     const Location& create_info_loc) const {
    bool skip = false;
    const auto rp_state = pipeline.RenderPassState();
    if (!rp_state || rp_state->UsesDynamicRendering())
    ...
    for (uint32_t i = 0; i < pipeline.AttachmentStates().size() && i < subpass_desc->colorAttachmentCount; ++i) {
        ...
        if (!pipeline.RasterizationDisabled() && pipeline.AttachmentStates()[i].blendEnable &&
            ...
            skip |= LogError("VUID-VkGraphicsPipelineCreateInfo-renderPass-06041",
                             device,
                             ...
                             string_VkFormat(attachment_desc.format),
                             subpass,
                             attachment);
        }
    }
...
bool CoreChecks::ValidateGraphicsPipelineMeshTask(const vvl::Pipeline& pipeline,
                                                  const Location& create_info_loc) const {
    bool skip = false;
    ...
    if (has_mesh && has_task) {
        for (const auto& stage : pipeline.stage_states) {
            if (stage.GetStage() == VK_SHADER_STAGE_MESH_BIT_EXT && stage.spirv_state &&
                ...
                skip |= LogError("VUID-VkGraphicsPipelineCreateInfo-pStages-09631",
                                 device,
                                 create_info_loc,
...
bool CoreChecks::ValidateGraphicsPipelineExternalFormatResolve(const vvl::Pipeline& pipeline,
    ...
    if (!pipeline.IsDynamic(CB_DYNAMIC_STATE_RASTERIZATION_SAMPLES_EXT) && multisample_state) {
        if (multisample_state->rasterizationSamples != VK_SAMPLE_COUNT_1_BIT) {
            skip |= LogError("VUID-VkGraphicsPipelineCreateInfo-externalFormatResolve-09313",
                             device,
        ...
        for (uint32_t i = 0; i < color_blend_state->attachmentCount; i++) {
            if (color_blend_state->pAttachments[i].blendEnable) {
                skip |= LogError(
                    ...
                    "is VK_TRUE, but externalFormat is %" PRIu64 " for subpass %" PRIu32 ".",
                    external_format,
                    pipeline.Subpass());
            }
        }
        ...
        if (fragment_shading_rate->fragmentSize.width != 1) {
            skip |= LogError(
                ...
                create_info_loc.pNext(Struct::VkPipelineFragmentShadingRateStateCreateInfoKHR, Field::fragmentSize)
                    .dot(Field::width),
                "is %" PRIu32 ", but externalFormat is %" PRIu64 " for subpass %" PRIu32 ".",
                fragment_shading_rate->fragmentSize.width,
                external_format,
                pipeline.Subpass());
        }
        if (fragment_shading_rate->fragmentSize.height != 1) {
            skip |= LogError(
                ...
                create_info_loc.pNext(Struct::VkPipelineFragmentShadingRateStateCreateInfoKHR, Field::fragmentSize)
                    .dot(Field::height),
                "is %" PRIu32 ", but externalFormat is %" PRIu64 " for subpass %" PRIu32 ".",
                fragment_shading_rate->fragmentSize.height,
                external_format,
                pipeline.Subpass());
        }
    }
...
bool CoreChecks::ValidateGraphicsPipelineExternalFormatResolveDynamicRendering(const vvl::Pipeline& pipeline,
    ...
    if (rendering_struct->viewMask != 0) {
        skip |= LogError("VUID-VkGraphicsPipelineCreateInfo-externalFormatResolve-09301",
                         device,
                         create_info_loc.pNext(Struct::VkPipelineRenderingCreateInfo, Field::viewMask),
                         "is 0x%" PRIx32 ", but externalFormat is %" PRIu64 ".",
                         rendering_struct->viewMask,
                         external_format);
    }
    if (rendering_struct->colorAttachmentCount != 1) {
        skip |= LogError("VUID-VkGraphicsPipelineCreateInfo-externalFormatResolve-09309",
                         device,
                         create_info_loc.pNext(Struct::VkPipelineRenderingCreateInfo, Field::colorAttachmentCount),
                         "is %" PRIu32 ", but externalFormat is %" PRIu64 ".",
    ...
    if (pipeline.OwnsSubState(pipeline.fragment_shader_state) && pipeline.fragment_shader_state->fragment_entry_point) {
        ...
        if (entrypoint->execution_mode.Has(spirv::ExecutionModeSet::depth_replacing_bit)) {
            skip |= LogError("VUID-VkGraphicsPipelineCreateInfo-externalFormatResolve-09310",
                             device,
                             create_info_loc.pNext(Struct::VkExternalFormatANDROID, Field::externalFormat),
                             "is %" PRIu64 " but the fragment shader declares DepthReplacing.",
                             external_format);
        } else if (entrypoint->execution_mode.Has(spirv::ExecutionModeSet::stencil_ref_replacing_bit)) {
            skip |= LogError("VUID-VkGraphicsPipelineCreateInfo-externalFormatResolve-09310",
                             device,
                             create_info_loc.pNext(Struct::VkExternalFormatANDROID, Field::externalFormat),
                             "is %" PRIu64 " but the fragment shader declares StencilRefReplacingEXT.",
                             external_format);
        }
    }
    ...
    if (!pipeline.IsDynamic(CB_DYNAMIC_STATE_RASTERIZATION_SAMPLES_EXT) && multisample_state) {
        if (multisample_state->rasterizationSamples != VK_SAMPLE_COUNT_1_BIT) {
            skip |= LogError("VUID-VkGraphicsPipelineCreateInfo-externalFormatResolve-09304",
                             device,
                             create_info_loc.dot(Field::pMultisampleState).dot(Field::rasterizationSamples),
        ...
        for (uint32_t i = 0; i < color_blend_state->attachmentCount; i++) {
            if (color_blend_state->pAttachments[i].blendEnable) {
                skip |= LogError(
                    ...
                    device,
                    create_info_loc.dot(Field::pColorBlendState).dot(Field::pAttachments, i).dot(Field::blendEnable),
                    "is VK_TRUE, but externalFormat is %" PRIu64 ".",
                    external_format);
            }
        }
    }
...
bool CoreChecks::ValidateGraphicsPipelineInputAssemblyState(const vvl::Pipeline& pipeline,
                                                            const Location& create_info_loc) const {
    bool skip = false;
    ...
    if (ia_state) {
        ...
        if (!pipeline.IsDynamic(CB_DYNAMIC_STATE_PRIMITIVE_RESTART_ENABLE) &&
            ...
            if (topology == VK_PRIMITIVE_TOPOLOGY_PATCH_LIST) {
                if (!enabled_features.primitiveTopologyPatchListRestart) {
                    skip |= LogError("VUID-VkPipelineInputAssemblyStateCreateInfo-topology-06253",
                                     device,
                                     ia_loc,
        ...
        if ((enabled_features.geometryShader == VK_FALSE) &&
            ...
            skip |= LogError("VUID-VkPipelineInputAssemblyStateCreateInfo-topology-00429",
                             device,
                             ia_loc,
        ...
        if ((enabled_features.tessellationShader == VK_FALSE) && (topology == VK_PRIMITIVE_TOPOLOGY_PATCH_LIST)) {
            skip |= LogError("VUID-VkPipelineInputAssemblyStateCreateInfo-topology-00430",
                             device,
                             ia_loc,
        ...
        if (!phys_dev_ext_props.conservative_rasterization_props.conservativePointAndLineRasterization &&
            ...
            if (rasterization_conservative_state_ci &&
                ...
                skip |= LogError("VUID-VkGraphicsPipelineCreateInfo-conservativePointAndLineRasterization-08892",
                                 device,
                                 ia_loc,
                                 "topology is %s, %s, but conservativeRasterizationMode is %s.",
                                 string_VkPrimitiveTopology(topology),
                                 msg.c_str(),
    ...
    if (!ignore_topology && pipeline.pre_raster_state && pipeline.vertex_input_state) {
        ...
        if (has_tessellation && (!ia_state || ia_state->topology != VK_PRIMITIVE_TOPOLOGY_PATCH_LIST)) {
            skip |=
                LogError("VUID-VkGraphicsPipelineCreateInfo-pStages-08888",
                         device,
                         ia_loc.dot(Field::topology),
        ...
        if (!has_tessellation && (ia_state && ia_state->topology == VK_PRIMITIVE_TOPOLOGY_PATCH_LIST)) {
            skip |= LogError("VUID-VkGraphicsPipelineCreateInfo-topology-08889",
                             device,
                             ia_loc.dot(Field::topology),
...
bool CoreChecks::ValidateGraphicsPipelineTessellationState(const vvl::Pipeline& pipeline,
                                                           const Location& create_info_loc) const {
    bool skip = false;
    ...
    if (pipeline.OwnsSubState(pipeline.pre_raster_state) &&
        ...
        if (!pipeline.TessellationState() && (!pipeline.IsDynamic(CB_DYNAMIC_STATE_PATCH_CONTROL_POINTS_EXT) ||
                                              !IsExtEnabled(device_extensions.vk_ext_extended_dynamic_state3))) {
            skip |= LogError("VUID-VkGraphicsPipelineCreateInfo-pStages-09022",
                             device,
                             create_info_loc.dot(Field::pStages),
...
bool CoreChecks::ValidateGraphicsPipelinePreRasterizationState(const vvl::Pipeline& pipeline,
    ...
    if ((stages & PreRasterState::ValidShaderStages()) == 0) {
        skip |= LogError("VUID-VkGraphicsPipelineCreateInfo-pStages-06896",
                         device,
                         create_info_loc,
    ...
    if (!enabled_features.geometryShader && (stages & VK_SHADER_STAGE_GEOMETRY_BIT)) {
        skip |= LogError("VUID-VkPipelineShaderStageCreateInfo-stage-00704",
                         device,
                         create_info_loc,
    ...
    if (!enabled_features.tessellationShader &&
        ...
        skip |= LogError("VUID-VkPipelineShaderStageCreateInfo-stage-00705",
                         device,
                         create_info_loc,
    ...
    if (!(stages & (VK_SHADER_STAGE_VERTEX_BIT | VK_SHADER_STAGE_MESH_BIT_EXT))) {
        skip |= LogError("VUID-VkGraphicsPipelineCreateInfo-stage-02096",
                         device,
                         create_info_loc,
    ...
    if ((stages & (VK_SHADER_STAGE_MESH_BIT_EXT | VK_SHADER_STAGE_TASK_BIT_EXT)) &&
        ...
        skip |= LogError("VUID-VkGraphicsPipelineCreateInfo-pStages-02095",
                         device,
                         create_info_loc,
    ...
    if (!(enabled_features.meshShader) && (stages & VK_SHADER_STAGE_MESH_BIT_EXT)) {
        skip |= LogError("VUID-VkPipelineShaderStageCreateInfo-stage-02091",
                         device,
                         create_info_loc,
    ...
    if (!(enabled_features.taskShader) && (stages & VK_SHADER_STAGE_TASK_BIT_EXT)) {
        skip |= LogError("VUID-VkPipelineShaderStageCreateInfo-stage-02092",
                         device,
                         create_info_loc,
    ...
    if (has_control && !has_eval) {
        skip |= LogError("VUID-VkGraphicsPipelineCreateInfo-pStages-00729",
                         device,
                         create_info_loc,
    ...
    if (!has_control && has_eval) {
        skip |= LogError("VUID-VkGraphicsPipelineCreateInfo-pStages-00730",
                         device,
                         create_info_loc,
    ...
    if (!enabled_features.independentBlend && attachment_states.size() > 1) {
        for (size_t i = 1; i < attachment_states.size(); i++) {
            if (!ComparePipelineColorBlendAttachmentState(attachment_states[0], attachment_states[i])) {
                skip |= LogError("VUID-VkPipelineColorBlendStateCreateInfo-pAttachments-00605",
                                 device,
    ...
    for (size_t i = 0; i < attachment_states.size(); i++) {
        ...
        if (IsSecondaryColorInputBlendFactor(attachment_state.srcColorBlendFactor)) {
            if (!enabled_features.dualSrcBlend) {
                skip |= LogError("VUID-VkPipelineColorBlendAttachmentState-srcColorBlendFactor-00608",
                                 device,
        ...
        if (IsSecondaryColorInputBlendFactor(attachment_state.dstColorBlendFactor)) {
            if (!enabled_features.dualSrcBlend) {
                skip |= LogError("VUID-VkPipelineColorBlendAttachmentState-dstColorBlendFactor-00609",
                                 device,
        ...
        if (IsSecondaryColorInputBlendFactor(attachment_state.srcAlphaBlendFactor)) {
            if (!enabled_features.dualSrcBlend) {
                skip |= LogError("VUID-VkPipelineColorBlendAttachmentState-srcAlphaBlendFactor-00610",
                                 device,
        ...
        if (IsSecondaryColorInputBlendFactor(attachment_state.dstAlphaBlendFactor)) {
            if (!enabled_features.dualSrcBlend) {
                skip |= LogError("VUID-VkPipelineColorBlendAttachmentState-dstAlphaBlendFactor-00611",
                                 device,
        ...
        if (attachment_state.blendEnable) {
            ...
            if (IsAdvanceBlendOperation(attachment_state.colorBlendOp)) {
                ...
                if (phys_dev_ext_props.blend_operation_advanced_props.advancedBlendAllOperations == VK_FALSE) {
                    ...
                    switch (attachment_state.colorBlendOp) {
                        ...
                        case VK_BLEND_OP_BLUE_EXT: {
                            skip |=
                                LogError("VUID-VkPipelineColorBlendAttachmentState-advancedBlendAllOperations-01409",
                                         device,
                ...
                if (phys_dev_ext_props.blend_operation_advanced_props.advancedBlendIndependentBlend == VK_FALSE &&
                    ...
                    skip |= LogError("VUID-VkPipelineColorBlendAttachmentState-advancedBlendIndependentBlend-01407",
                                     device,
                                     attachment_loc.dot(Field::colorBlendOp),
                                     "(%s) is not same the other attachments (%s).",
                                     string_VkBlendOp(attachment_state.colorBlendOp),
            ...
            if (IsAdvanceBlendOperation(attachment_state.alphaBlendOp)) {
                ...
                if (phys_dev_ext_props.blend_operation_advanced_props.advancedBlendIndependentBlend == VK_FALSE &&
                    ...
                    skip |= LogError("VUID-VkPipelineColorBlendAttachmentState-advancedBlendIndependentBlend-01408",
                                     device,
                                     attachment_loc.dot(Field::alphaBlendOp),
                                     "(%s) is not same the other attachments (%s).",
            ...
            if (advance_blend) {
                if (attachment_state.colorBlendOp != attachment_state.alphaBlendOp) {
                    skip |= LogError("VUID-VkPipelineColorBlendAttachmentState-colorBlendOp-01406",
                                     device,
                                     attachment_loc,
...
bool CoreChecks::IsColorBlendStateAttachmentCountIgnore(const vvl::Pipeline& pipeline) const {
    return pipeline.IsDynamic(CB_DYNAMIC_STATE_COLOR_BLEND_ENABLE_EXT) &&
           ...
           (pipeline.IsDynamic(CB_DYNAMIC_STATE_COLOR_BLEND_ADVANCED_EXT) ||
            !enabled_features.advancedBlendCoherentOperations);
}
bool CoreChecks::ValidatePipelineColorBlendAdvancedStateCreateInfo(
    const vvl::Pipeline& pipeline,
    const VkPipelineColorBlendAdvancedStateCreateInfoEXT& color_blend_advanced,
    const Location& color_loc) const {
    bool skip = false;
    ...
    if (!prop.advancedBlendCorrelatedOverlap &&
        color_blend_advanced.blendOverlap != VK_BLEND_OVERLAP_UNCORRELATED_EXT) {
        skip |= LogError("VUID-VkPipelineColorBlendAdvancedStateCreateInfoEXT-blendOverlap-01426",
                         device,
    ...
    if (!prop.advancedBlendNonPremultipliedDstColor && color_blend_advanced.dstPremultiplied != VK_TRUE) {
        skip |=
            LogError("VUID-VkPipelineColorBlendAdvancedStateCreateInfoEXT-dstPremultiplied-01425",
                     device,
    ...
    if (!prop.advancedBlendNonPremultipliedSrcColor && color_blend_advanced.srcPremultiplied != VK_TRUE) {
        skip |=
            LogError("VUID-VkPipelineColorBlendAdvancedStateCreateInfoEXT-srcPremultiplied-01424",
                     device,
...
bool CoreChecks::ValidateGraphicsPipelineColorBlendState(const vvl::Pipeline& pipeline,
    ...
    if (!null_rp && !rp_state)
        return skip; // invalid render pass
    ...
    if (null_rp && rendering_struct && attachment_sample_count_info &&
        ...
        skip |= LogError(
            ...
            "(%" PRIu32 ") is different then %s (%" PRIu32 ").",
    ...
    if (!null_rp && subpass_desc && color_blend_state->attachmentCount != subpass_desc->colorAttachmentCount) {
        if (!IsColorBlendStateAttachmentCountIgnore(pipeline)) {
            skip |= LogError("VUID-VkGraphicsPipelineCreateInfo-renderPass-07609",
                             device,
                             color_loc.dot(Field::attachmentCount),
    ...
    if (!enabled_features.logicOp && (color_blend_state->logicOpEnable == VK_TRUE) &&
        ...
        skip |= LogError("VUID-VkPipelineColorBlendStateCreateInfo-logicOpEnable-00606",
                         device,
                         color_loc.dot(Field::logicOpEnable),
    ...
    if (auto color_write = vku::FindStructInPNextChain<VkPipelineColorWriteCreateInfoEXT>(color_blend_state->pNext)) {
        if (color_write->attachmentCount > phys_dev_props.limits.maxColorAttachments) {
            skip |= LogError("VUID-VkPipelineColorWriteCreateInfoEXT-attachmentCount-06655",
                             device,
...
bool CoreChecks::ValidatePipelineRasterizationStateStreamCreateInfo(
    const vvl::Pipeline& pipeline,
    const VkPipelineRasterizationStateStreamCreateInfoEXT& rasterization_state_stream_ci,
    const Location& raster_loc) const {
    bool skip = false;
    if (!enabled_features.geometryStreams) {
        skip |= LogError("VUID-VkPipelineRasterizationStateStreamCreateInfoEXT-geometryStreams-02324",
                         device,
        ...
        if (props.transformFeedbackRasterizationStreamSelect == VK_FALSE &&
            ...
            skip |= LogError(
                ...
                rasterization_state_stream_ci.rasterizationStream,
                props.maxTransformFeedbackStreams);
        }
    }
    return skip;
}
        ...
        if (IsValueIn(pipeline.topology_at_rasterizer,
            ...
            if (rasterization_conservative_state_ci.conservativeRasterizationMode !=
                ...
                if ((pipeline.create_info_shaders & VK_SHADER_STAGE_GEOMETRY_BIT) != 0) {
                    skip |= LogError("VUID-VkGraphicsPipelineCreateInfo-conservativePointAndLineRasterization-06760",
                                     device,
                                     raster_loc.pNext(Struct::VkPipelineRasterizationConservativeStateCreateInfoEXT,
                ...
                if ((pipeline.create_info_shaders & VK_SHADER_STAGE_MESH_BIT_EXT) != 0) {
                    skip |= LogError("VUID-VkGraphicsPipelineCreateInfo-conservativePointAndLineRasterization-06761",
                                     device,
                                     raster_loc.pNext(Struct::VkPipelineRasterizationConservativeStateCreateInfoEXT,
                                                      Field::conservativeRasterizationMode),
...
bool CoreChecks::ValidateGraphicsPipelineRasterizationState(const vvl::Pipeline& pipeline,
    ...
    if ((raster_state->depthClampEnable == VK_TRUE) && !enabled_features.depthClamp &&
        ...
        skip |= LogError("VUID-VkPipelineRasterizationStateCreateInfo-depthClampEnable-00782",
                         device,
                         raster_loc.dot(Field::depthClampEnable),
                         "is VK_TRUE, but the depthClamp feature was not enabled.");
    }

    if (!pipeline.IsDynamic(CB_DYNAMIC_STATE_DEPTH_BIAS) && (raster_state->depthBiasClamp != 0.0) &&
        !enabled_features.depthBiasClamp) {
        skip |= LogError("VUID-VkGraphicsPipelineCreateInfo-pDynamicStates-00754",
                         device,
                         raster_loc.dot(Field::depthBiasClamp),
                         "is %f, but the depthBiasClamp feature was not enabled",
    ...
    if (raster_state->rasterizerDiscardEnable == VK_FALSE) {
        ...
        if (has_ms_state) {
            ...
            if (ms_state && (ms_state->alphaToOneEnable == VK_TRUE) && !enabled_features.alphaToOne &&
                ...
                skip |= LogError("VUID-VkPipelineMultisampleStateCreateInfo-alphaToOneEnable-00785",
                                 device,
    ...
    if (auto provoking_vertex_state_ci =
        ...
        if (provoking_vertex_state_ci->provokingVertexMode == VK_PROVOKING_VERTEX_MODE_LAST_VERTEX_EXT &&
            ...
            skip |= LogError(
                ...
                device,
                raster_loc.pNext(Struct::VkPipelineRasterizationProvokingVertexStateCreateInfoEXT,
                                 Field::provokingVertexMode),
    ...
    if (const auto rasterization_state_stream_ci =
        ...
        skip |=
            ValidatePipelineRasterizationStateStreamCreateInfo(pipeline, *rasterization_state_stream_ci, raster_loc);
    }
    ...
    if (const auto rasterization_conservative_state_ci =
        ...
        skip |= ValidatePipelineRasterizationConservativeStateCreateInfo(
            pipeline, *rasterization_conservative_state_ci, raster_loc);
    } else {
        if (pipeline.IsDynamic(CB_DYNAMIC_STATE_CONSERVATIVE_RASTERIZATION_MODE_EXT) &&
            ...
            skip |=
                LogError("VUID-VkGraphicsPipelineCreateInfo-pDynamicState-09639",
                         device,
                         raster_loc.dot(Field::pNext),
    ...
    if (const auto* depth_bias_representation =
            vku::FindStructInPNextChain<VkDepthBiasRepresentationInfoEXT>(raster_state->pNext);
...
bool CoreChecks::ValidateGraphicsPipelineRenderPassRasterization(const vvl::Pipeline& pipeline,
                                                                 const vvl::RenderPass& rp_state,
                                                                 const vku::safe_VkSubpassDescription2& subpass_desc,
                                                                 const Location& create_info_loc) const {
    bool skip = false;
    ...
    if (pipeline.fragment_shader_state) {
        if (subpass_desc.pDepthStencilAttachment &&
            ...
            const auto ds_state = pipeline.DepthStencilState();
            if (!ds_state) {
                if (!pipeline.IsDepthStencilStateDynamic() ||
                    !IsExtEnabled(device_extensions.vk_ext_extended_dynamic_state3)) {
                    skip |= LogError("VUID-VkGraphicsPipelineCreateInfo-renderPass-09028",
                                     rp_state.Handle(),
                                     ds_loc,
                ...
                if (!enabled_features.depthBounds) {
                    skip |= LogError("VUID-VkPipelineDepthStencilStateCreateInfo-depthBoundsTestEnable-00598",
                                     device,
    ...
    if (pipeline.fragment_output_state && !pipeline.fragment_output_state->color_blend_state &&
        ...
        for (uint32_t i = 0; i < subpass_desc.colorAttachmentCount; ++i) {
            if (subpass_desc.pColorAttachments[i].attachment != VK_ATTACHMENT_UNUSED) {
                skip |= LogError("VUID-VkGraphicsPipelineCreateInfo-renderPass-09030",
                                 rp_state.Handle(),
...
bool CoreChecks::ValidateSampleLocationsInfo(const VkSampleLocationsInfoEXT& sample_location_info,
    ...
    const uint32_t sample_total_size = sample_location_info.sampleLocationGridSize.width *
                                       sample_location_info.sampleLocationGridSize.height *
                                       SampleCountSize(sample_count);
    if (sample_location_info.sampleLocationsCount != sample_total_size) {
        skip |= LogError("VUID-VkSampleLocationsInfoEXT-sampleLocationsCount-01527",
                         device,
                         loc.dot(Field::sampleLocationsCount),
                         "(%" PRIu32 ") must equal grid width * grid height * pixel "
                         "sample rate which currently is (%" PRIu32 " * %" PRIu32 " * %" PRIu32 ").",
                         sample_location_info.sampleLocationsCount,
                         sample_location_info.sampleLocationGridSize.width,
                         sample_location_info.sampleLocationGridSize.height,
                         SampleCountSize(sample_count));
    }
    if ((phys_dev_ext_props.sample_locations_props.sampleLocationSampleCounts & sample_count) == 0) {
        skip |= LogError(
            ...
            device,
            loc.dot(Field::sampleLocationsPerPixel),
...
bool CoreChecks::ValidateGraphicsPipelineMultisampleState(const vvl::Pipeline& pipeline,
    ...
    if (!pipeline.IsDynamic(CB_DYNAMIC_STATE_RASTERIZATION_SAMPLES_EXT)) {
        ...
        if (!(IsExtEnabled(device_extensions.vk_amd_mixed_attachment_samples) ||
            ...
            // subpass_num_samples is 0 when the subpass has no attachments or if all attachments are
            // VK_ATTACHMENT_UNUSED. Only validate the value of subpass_num_samples if the subpass has attachments that
            // are not VK_ATTACHMENT_UNUSED.
            if (subpass_num_samples &&
                (!IsPowerOfTwo(subpass_num_samples) || (subpass_num_samples != raster_samples))) {
                skip |= LogError(
        ...
        if (IsExtEnabled(device_extensions.vk_amd_mixed_attachment_samples)) {
            ...
            if (!pipeline.RasterizationDisabled() && (max_sample_count != static_cast<VkSampleCountFlagBits>(0)) &&
                ...
                skip |=
                    LogError("VUID-VkGraphicsPipelineCreateInfo-subpass-01505",
                             device,
                             ms_loc.dot(Field::rasterizationSamples),
                ...
                if (ds_state) {
                    ...
                    if (ds_test_enabled &&
                        ...
                        skip |= LogError(
                            ...
                            ms_loc.dot(Field::rasterizationSamples),
                            "(%" PRIu32
                            ") does not match the number of samples of the RenderPass depth attachment (%" PRIu32 ").",
                            raster_samples,
                            subpass_depth_samples);
                    }
                }
            }
            ...
            if (IsPowerOfTwo(subpass_color_samples)) {
                if (raster_samples < subpass_color_samples) {
                    skip |= LogError(
                        ...
                        ").",
                        raster_samples,
                        subpass_color_samples);
                }

                if (multisample_state) {
                    if ((raster_samples > subpass_color_samples) &&
                        (multisample_state->sampleShadingEnable == VK_TRUE)) {
                        skip |= LogError("VUID-VkPipelineMultisampleStateCreateInfo-rasterizationSamples-01415",
                                         device,
                                         ms_loc.dot(Field::rasterizationSamples),
                    ...
                    const auto* coverage_modulation_state =
                        vku::FindStructInPNextChain<VkPipelineCoverageModulationStateCreateInfoNV>(
                            multisample_state->pNext);

                    if (coverage_modulation_state &&
                        (coverage_modulation_state->coverageModulationTableEnable == VK_TRUE)) {
                        if (coverage_modulation_state->coverageModulationTableCount !=
                            (raster_samples / subpass_color_samples)) {
                            skip |= LogError("VUID-VkPipelineCoverageModulationStateCreateInfoNV-"
        ...
        if (IsExtEnabled(device_extensions.vk_nv_coverage_reduction_mode)) {
            ...
            if (multisample_state && IsPowerOfTwo(subpass_color_samples) &&
                ...
                if (coverage_reduction_state) {
                    const VkCoverageReductionModeNV coverage_reduction_mode =
                        coverage_reduction_state->coverageReductionMode;
                    ...
                    DispatchGetPhysicalDeviceSupportedFramebufferMixedSamplesCombinationsNV(
                        physical_device, &combination_count, nullptr);
                    combinations.resize(combination_count);
                    DispatchGetPhysicalDeviceSupportedFramebufferMixedSamplesCombinationsNV(
                        physical_device, &combination_count, &combinations[0]);
                    ...
                    if (!combination_found) {
                        skip |= LogError(
                            ...
                            string_VkCoverageReductionModeNV(coverage_reduction_mode),
                            raster_samples);
                    }
                }
            }
        }
        const auto msrtss_info =
            vku::FindStructInPNextChain<VkMultisampledRenderToSingleSampledInfoEXT>(subpass_desc.pNext);
        if (msrtss_info && msrtss_info->multisampledRenderToSingleSampledEnable &&
            ...
            skip |=
                LogError("VUID-VkGraphicsPipelineCreateInfo-renderPass-06854",
                         rp_state.Handle(),
        ...
        if (rp_state.UsesNoAttachment(pipeline.Subpass())) {
            if ((multisample_state->rasterizationSamples &
    ...
    const auto coverage_to_color_state =
        vku::FindStructInPNextChain<VkPipelineCoverageToColorStateCreateInfoNV>(multisample_state);
    ...
    if (sample_location_state != nullptr) {
        if ((sample_location_state->sampleLocationsEnable == VK_TRUE) &&
            ...
            DispatchGetPhysicalDeviceMultisamplePropertiesEXT(
                physical_device, multisample_state->rasterizationSamples, &multisample_prop);
            ...
            if (SafeModulo(max_grid_size.height, grid_size.height) != 0) {
                skip |= LogError(
                    ...
                    ").",
                    grid_size.height,
                    max_grid_size.height);
            }
            if (sample_location_info.sampleLocationsPerPixel != multisample_state->rasterizationSamples) {
                skip |= LogError("VUID-VkGraphicsPipelineCreateInfo-pDynamicStates-07612",
                                 device,
                                 sample_info_loc.dot(Field::sampleLocationsPerPixel),
    ...
    if (IsExtEnabled(device_extensions.vk_qcom_render_pass_shader_resolve)) {
        ...
        if ((subpass_desc.flags & VK_SUBPASS_DESCRIPTION_FRAGMENT_REGION_BIT_QCOM) != 0) {
            ...
            if ((raster_samples != subpass_input_attachment_samples) &&
                !pipeline.IsDynamic(CB_DYNAMIC_STATE_RASTERIZATION_SAMPLES_EXT)) {
                skip |= LogError("VUID-VkGraphicsPipelineCreateInfo-rasterizationSamples-04899",
                                 device,
                                 ms_loc.dot(Field::rasterizationSamples),
            ...
            if (multisample_state->sampleShadingEnable == VK_TRUE) {
                skip |= LogError("VUID-VkGraphicsPipelineCreateInfo-sampleShadingEnable-04900",
                                 device,
...
bool CoreChecks::ValidateGraphicsPipelineNullState(const vvl::Pipeline& pipeline,
                                                   const Location& create_info_loc) const {
    bool skip = false;
    ...
    if (null_rp) {
        if (!pipeline.DepthStencilState()) {
            if (pipeline.fragment_shader_state && !pipeline.fragment_output_state) {
                if (!pipeline.IsDepthStencilStateDynamic() ||
                    !IsExtEnabled(device_extensions.vk_ext_extended_dynamic_state3)) {
                    skip |= LogError("VUID-VkGraphicsPipelineCreateInfo-renderPass-09035",
                                     device,
                                     create_info_loc.dot(Field::pDepthStencilState),
                                     "is NULL.");
                }
            }
        }
    ...
    if (IsExtEnabled(device_extensions.vk_ext_graphics_pipeline_library)) {
        if (pipeline.OwnsSubState(pipeline.fragment_output_state) && !pipeline.MultisampleState()) {
            ...
            if (!null_rp) {
                skip |= LogError("VUID-VkGraphicsPipelineCreateInfo-renderpass-06631",
                                 device,
    ...
    if (!pipeline_ci.pMultisampleState && pipeline.OwnsSubState(pipeline.fragment_output_state)) {
        ...
        if (!pipeline.IsDynamic(CB_DYNAMIC_STATE_RASTERIZATION_SAMPLES_EXT) ||
            ...
            skip |= LogError("VUID-VkGraphicsPipelineCreateInfo-pMultisampleState-09026",
                             device,
                             create_info_loc.dot(Field::pMultisampleState),
                             "is NULL.");
        }
    ...
    if (!pipeline.RasterizationState()) {
        if (!pipeline_ci.pRasterizationState && pipeline.OwnsSubState(pipeline.pre_raster_state)) {
            if (!IsExtEnabled(device_extensions.vk_ext_extended_dynamic_state3) ||
                ...
                !pipeline.IsDynamic(CB_DYNAMIC_STATE_DEPTH_BIAS_ENABLE) ||
                !pipeline.IsDynamic(CB_DYNAMIC_STATE_DEPTH_BIAS) || !pipeline.IsDynamic(CB_DYNAMIC_STATE_LINE_WIDTH)) {
                skip |= LogError("VUID-VkGraphicsPipelineCreateInfo-pRasterizationState-06601",
                                 device,
...
bool CoreChecks::ValidateGraphicsPipelineRasterizationOrderAttachmentAccess(
    const vvl::Pipeline& pipeline,
    const vku::safe_VkSubpassDescription2* subpass_desc,
    const Location& create_info_loc) const {
    bool skip = false;
    ...
    if (const auto ds_state = pipeline.DepthStencilState()) {
        ...
        if (null_rp) {
            if (!enabled_features.dynamicRenderingLocalRead &&
                (raster_order_attach_depth || raster_order_attach_stencil)) {
                skip |= LogError("VUID-VkGraphicsPipelineCreateInfo-None-09526",
                                 device,
            ...
            if (raster_order_attach_depth) {
                ...
                if (subpass_desc && (subpass_desc->flags &
                                     VK_SUBPASS_DESCRIPTION_RASTERIZATION_ORDER_ATTACHMENT_DEPTH_ACCESS_BIT_EXT) == 0) {
                    skip |= LogError("VUID-VkGraphicsPipelineCreateInfo-renderPass-09528",
                                     rp_state->Handle(),
                                     ds_loc.dot(Field::flags),
            ...
            if (raster_order_attach_stencil) {
                ...
                if (subpass_desc &&
                    (subpass_desc->flags &
                     VK_SUBPASS_DESCRIPTION_RASTERIZATION_ORDER_ATTACHMENT_STENCIL_ACCESS_BIT_EXT) == 0) {
                    skip |= LogError("VUID-VkGraphicsPipelineCreateInfo-renderPass-09529",
                                     rp_state->Handle(),
                                     ds_loc.dot(Field::flags),
                                     "is (%s) but VkRenderPassCreateInfo::VkSubpassDescription::flags == %s",
                                     string_VkPipelineDepthStencilStateCreateFlags(ds_state->flags).c_str(),
    ...
    if (const auto color_blend_state = pipeline.ColorBlendState()) {
        ...
        if (color_blend_state->flags &
            ...
            if (!enabled_features.dynamicRenderingLocalRead && null_rp) {
                skip |=
                    LogError("VUID-VkGraphicsPipelineCreateInfo-flags-06482",
                             device,
                             color_loc.dot(Field::flags),
                             "includes "
            ...
            if (!null_rp && subpass_desc &&
                (subpass_desc->flags & VK_SUBPASS_DESCRIPTION_RASTERIZATION_ORDER_ATTACHMENT_COLOR_ACCESS_BIT_EXT) ==
                    0) {
                skip |= LogError("VUID-VkGraphicsPipelineCreateInfo-renderPass-09527",
                                 rp_state->Handle(),
                                 color_loc.dot(Field::flags),
...
bool CoreChecks::ValidateGraphicsPipelineDynamicState(const vvl::Pipeline& pipeline,
                                                      const Location& create_info_loc) const {
    ...
    if (pipeline.create_info_shaders & VK_SHADER_STAGE_MESH_BIT_EXT) {
        if (pipeline.IsDynamic(CB_DYNAMIC_STATE_PRIMITIVE_TOPOLOGY)) {
            skip |= LogError("VUID-VkGraphicsPipelineCreateInfo-pDynamicStates-07065",
                             device,
            ...
            skip |=
                LogError("VUID-VkGraphicsPipelineCreateInfo-pDynamicStates-07065",
                         device,
                         create_info_loc.dot(Field::pDynamicState)
        ...
        if (pipeline.IsDynamic(CB_DYNAMIC_STATE_PRIMITIVE_RESTART_ENABLE)) {
            skip |=
                LogError("VUID-VkGraphicsPipelineCreateInfo-pDynamicStates-07066",
                         device,
                         create_info_loc.dot(Field::pDynamicState)
        ...
        if (pipeline.IsDynamic(CB_DYNAMIC_STATE_VERTEX_INPUT_EXT)) {
            skip |= LogError("VUID-VkGraphicsPipelineCreateInfo-pDynamicStates-07067",
                             device,
                             create_info_loc.dot(Field::pDynamicState)
    ...
    if (api_version < VK_API_VERSION_1_3 && !enabled_features.extendedDynamicState &&
        (pipeline.IsDynamic(CB_DYNAMIC_STATE_CULL_MODE) || pipeline.IsDynamic(CB_DYNAMIC_STATE_FRONT_FACE) ||
         pipeline.IsDynamic(CB_DYNAMIC_STATE_PRIMITIVE_TOPOLOGY) ||
         pipeline.IsDynamic(CB_DYNAMIC_STATE_VIEWPORT_WITH_COUNT) ||
         pipeline.IsDynamic(CB_DYNAMIC_STATE_SCISSOR_WITH_COUNT) ||
         pipeline.IsDynamic(CB_DYNAMIC_STATE_VERTEX_INPUT_BINDING_STRIDE) ||
         pipeline.IsDynamic(CB_DYNAMIC_STATE_DEPTH_TEST_ENABLE) ||
         pipeline.IsDynamic(CB_DYNAMIC_STATE_DEPTH_WRITE_ENABLE) ||
         pipeline.IsDynamic(CB_DYNAMIC_STATE_DEPTH_COMPARE_OP) ||
         pipeline.IsDynamic(CB_DYNAMIC_STATE_DEPTH_BOUNDS_TEST_ENABLE) ||
         pipeline.IsDynamic(CB_DYNAMIC_STATE_STENCIL_TEST_ENABLE) || pipeline.IsDynamic(CB_DYNAMIC_STATE_STENCIL_OP))) {
        skip |= LogError("VUID-VkGraphicsPipelineCreateInfo-pDynamicStates-03378",
                         device,
                         create_info_loc.dot(Field::pDynamicState),
                         "contains dynamic states from VK_EXT_extended_dynamic_state, but the extendedDynamicState "
    ...
    if (api_version < VK_API_VERSION_1_3 && !enabled_features.extendedDynamicState2) {
        if (pipeline.IsDynamic(CB_DYNAMIC_STATE_RASTERIZER_DISCARD_ENABLE)) {
            skip |=
                LogError("VUID-VkGraphicsPipelineCreateInfo-pDynamicStates-04868",
                         device,
                         create_info_loc.dot(Field::pDynamicState)
                             .dot(Field::pDynamicStates, get_state_index(VK_DYNAMIC_STATE_RASTERIZER_DISCARD_ENABLE)),
                         "is VK_DYNAMIC_STATE_RASTERIZER_DISCARD_ENABLE, but the extendedDynamicState2 feature was not "
                         "enabled.");
    ...
    if (!enabled_features.extendedDynamicState2LogicOp && pipeline.IsDynamic(CB_DYNAMIC_STATE_LOGIC_OP_EXT)) {
        skip |=
            LogError("VUID-VkGraphicsPipelineCreateInfo-pDynamicStates-04869",
                     device,
                     create_info_loc.dot(Field::pDynamicState)
                         .dot(Field::pDynamicStates, get_state_index(VK_DYNAMIC_STATE_LOGIC_OP_EXT)),
    ...
    if (!enabled_features.extendedDynamicState3TessellationDomainOrigin &&
        ...
        skip |=
            LogError("VUID-VkGraphicsPipelineCreateInfo-extendedDynamicState3TessellationDomainOrigin-07370",
                     device,
                     create_info_loc.dot(Field::pDynamicState)
    ...
    if (!enabled_features.extendedDynamicState3DepthClampEnable &&
        pipeline.IsDynamic(CB_DYNAMIC_STATE_DEPTH_CLAMP_ENABLE_EXT)) {
        skip |= LogError("VUID-VkGraphicsPipelineCreateInfo-extendedDynamicState3DepthClampEnable-07371",
                         device,
                         create_info_loc.dot(Field::pDynamicState)
                             .dot(Field::pDynamicStates, get_state_index(VK_DYNAMIC_STATE_DEPTH_CLAMP_ENABLE_EXT)),
                         "is VK_DYNAMIC_STATE_DEPTH_CLAMP_ENABLE_EXT, but the extendedDynamicState3DepthClampEnable "
                         "feature was not enabled.");
    }
    ...
    if (!enabled_features.extendedDynamicState3SampleMask && pipeline.IsDynamic(CB_DYNAMIC_STATE_SAMPLE_MASK_EXT)) {
        skip |= LogError(
            ...
            device,
            create_info_loc.dot(Field::pDynamicState)
                .dot(Field::pDynamicStates, get_state_index(VK_DYNAMIC_STATE_SAMPLE_MASK_EXT)),
    ...
    if (!enabled_features.extendedDynamicState3AlphaToOneEnable &&
        pipeline.IsDynamic(CB_DYNAMIC_STATE_ALPHA_TO_ONE_ENABLE_EXT)) {
        skip |= LogError("VUID-VkGraphicsPipelineCreateInfo-extendedDynamicState3AlphaToOneEnable-07376",
                         device,
                         create_info_loc.dot(Field::pDynamicState)
                             .dot(Field::pDynamicStates, get_state_index(VK_DYNAMIC_STATE_ALPHA_TO_ONE_ENABLE_EXT)),
                         "is VK_DYNAMIC_STATE_ALPHA_TO_ONE_ENABLE_EXT but the extendedDynamicState3AlphaToOneEnable "
                         "feature is not enabled.");
    }

    if (!enabled_features.extendedDynamicState3LogicOpEnable &&
        pipeline.IsDynamic(CB_DYNAMIC_STATE_LOGIC_OP_ENABLE_EXT)) {
        skip |= LogError("VUID-VkGraphicsPipelineCreateInfo-extendedDynamicState3LogicOpEnable-07377",
                         device,
                         create_info_loc.dot(Field::pDynamicState)
                             .dot(Field::pDynamicStates, get_state_index(VK_DYNAMIC_STATE_LOGIC_OP_ENABLE_EXT)),
                         "is VK_DYNAMIC_STATE_LOGIC_OP_ENABLE_EXT but the extendedDynamicState3LogicOpEnable feature "
                         "is not enabled.");
    }

    if (!enabled_features.extendedDynamicState3ColorBlendEnable &&
        pipeline.IsDynamic(CB_DYNAMIC_STATE_COLOR_BLEND_ENABLE_EXT)) {
        skip |= LogError("VUID-VkGraphicsPipelineCreateInfo-extendedDynamicState3ColorBlendEnable-07378",
                         device,
                         create_info_loc.dot(Field::pDynamicState)
                             .dot(Field::pDynamicStates, get_state_index(VK_DYNAMIC_STATE_COLOR_BLEND_ENABLE_EXT)),
                         "is VK_DYNAMIC_STATE_COLOR_BLEND_ENABLE_EXT but the extendedDynamicState3ColorBlendEnable "
                         "feature is not enabled.");
    }

    if (!enabled_features.extendedDynamicState3ColorBlendEquation &&
        pipeline.IsDynamic(CB_DYNAMIC_STATE_COLOR_BLEND_EQUATION_EXT)) {
        skip |= LogError("VUID-VkGraphicsPipelineCreateInfo-extendedDynamicState3ColorBlendEquation-07379",
                         device,
                         create_info_loc.dot(Field::pDynamicState)
                             .dot(Field::pDynamicStates, get_state_index(VK_DYNAMIC_STATE_COLOR_BLEND_EQUATION_EXT)),
                         "is VK_DYNAMIC_STATE_COLOR_BLEND_EQUATION_EXT but the extendedDynamicState3ColorBlendEquation "
                         "feature is not enabled.");
    }

    if (!enabled_features.extendedDynamicState3ColorWriteMask &&
        pipeline.IsDynamic(CB_DYNAMIC_STATE_COLOR_WRITE_MASK_EXT)) {
        skip |= LogError("VUID-VkGraphicsPipelineCreateInfo-extendedDynamicState3ColorWriteMask-07380",
                         device,
                         create_info_loc.dot(Field::pDynamicState)
                             .dot(Field::pDynamicStates, get_state_index(VK_DYNAMIC_STATE_COLOR_WRITE_MASK_EXT)),
                         "is VK_DYNAMIC_STATE_COLOR_WRITE_MASK_EXT but the extendedDynamicState3ColorWriteMask feature "
                         "is not enabled.");
    }
    ...
    if (!enabled_features.extendedDynamicState3DepthClipEnable &&
        pipeline.IsDynamic(CB_DYNAMIC_STATE_DEPTH_CLIP_ENABLE_EXT)) {
        skip |= LogError("VUID-VkGraphicsPipelineCreateInfo-extendedDynamicState3DepthClipEnable-07384",
                         device,
                         create_info_loc.dot(Field::pDynamicState)
                             .dot(Field::pDynamicStates, get_state_index(VK_DYNAMIC_STATE_DEPTH_CLIP_ENABLE_EXT)),
                         "is VK_DYNAMIC_STATE_DEPTH_CLIP_ENABLE_EXT but the extendedDynamicState3DepthClipEnable "
                         "feature is not enabled.");
    }
    ...
    if (!enabled_features.extendedDynamicState3ColorBlendAdvanced &&
        pipeline.IsDynamic(CB_DYNAMIC_STATE_COLOR_BLEND_ADVANCED_EXT)) {
        skip |= LogError("VUID-VkGraphicsPipelineCreateInfo-extendedDynamicState3ColorBlendAdvanced-07386",
                         device,
                         create_info_loc.dot(Field::pDynamicState)
                             .dot(Field::pDynamicStates, get_state_index(VK_DYNAMIC_STATE_COLOR_BLEND_ADVANCED_EXT)),
                         "is VK_DYNAMIC_STATE_COLOR_BLEND_ADVANCED_EXT but the extendedDynamicState3ColorBlendAdvanced "
                         "feature is not enabled.");
    }
    ...
    if (!enabled_features.extendedDynamicState3LineStippleEnable &&
        pipeline.IsDynamic(CB_DYNAMIC_STATE_LINE_STIPPLE_ENABLE_EXT)) {
        skip |= LogError("VUID-VkGraphicsPipelineCreateInfo-extendedDynamicState3LineStippleEnable-07389",
                         device,
                         create_info_loc.dot(Field::pDynamicState)
                             .dot(Field::pDynamicStates, get_state_index(VK_DYNAMIC_STATE_LINE_STIPPLE_ENABLE_EXT)),
                         "is VK_DYNAMIC_STATE_LINE_STIPPLE_ENABLE_EXT but the extendedDynamicState3LineStippleEnable "
                         "feature is not enabled.");
    }
    ...
    if (!enabled_features.extendedDynamicState3ViewportSwizzle &&
        pipeline.IsDynamic(CB_DYNAMIC_STATE_VIEWPORT_SWIZZLE_NV)) {
        skip |= LogError("VUID-VkGraphicsPipelineCreateInfo-extendedDynamicState3ViewportSwizzle-07392",
                         device,
                         create_info_loc.dot(Field::pDynamicState)
                             .dot(Field::pDynamicStates, get_state_index(VK_DYNAMIC_STATE_VIEWPORT_SWIZZLE_NV)),
                         "is VK_DYNAMIC_STATE_VIEWPORT_SWIZZLE_NV but the extendedDynamicState3ViewportSwizzle feature "
                         "is not enabled.");
    }
    ...
    if (!enabled_features.vertexInputDynamicState && pipeline.IsDynamic(CB_DYNAMIC_STATE_VERTEX_INPUT_EXT)) {
        skip |=
            LogError("VUID-VkGraphicsPipelineCreateInfo-pDynamicStates-04807",
                     device,
                     create_info_loc.dot(Field::pDynamicState)
                         .dot(Field::pDynamicStates, get_state_index(VK_DYNAMIC_STATE_VERTEX_INPUT_EXT)),
    ...
    if (!enabled_features.colorWriteEnable && pipeline.IsDynamic(CB_DYNAMIC_STATE_COLOR_WRITE_ENABLE_EXT)) {
        skip |= LogError("VUID-VkGraphicsPipelineCreateInfo-pDynamicStates-04800",
                         device,
                         create_info_loc.dot(Field::pDynamicState)
...
bool CoreChecks::ValidateGraphicsPipelineFragmentShadingRateState(
    ...
    if (fragment_shading_rate_state.fragmentSize.width == 0) {
        skip |= LogError("VUID-VkGraphicsPipelineCreateInfo-pDynamicState-04494",
                         device,
                         fragment_loc.dot(Field::width),
                         "is zero.");
    }

    if (fragment_shading_rate_state.fragmentSize.height == 0) {
        skip |= LogError("VUID-VkGraphicsPipelineCreateInfo-pDynamicState-04495",
                         device,
                         fragment_loc.dot(Field::height),
                         "is zero.");
    }

    if (fragment_shading_rate_state.fragmentSize.width != 0 &&
        !IsPowerOfTwo(fragment_shading_rate_state.fragmentSize.width)) {
        skip |= LogError("VUID-VkGraphicsPipelineCreateInfo-pDynamicState-04496",
                         device,
                         fragment_loc.dot(Field::width),
                         "is %" PRIu32 ".",
    ...
    if (fragment_shading_rate_state.fragmentSize.height != 0 &&
        ...
        skip |= LogError("VUID-VkGraphicsPipelineCreateInfo-pDynamicState-04497",
                         device,
                         fragment_loc.dot(Field::height),
                         "is %" PRIu32 ".",
    ...
    if (fragment_shading_rate_state.fragmentSize.width > 4) {
        skip |= LogError("VUID-VkGraphicsPipelineCreateInfo-pDynamicState-04498",
                         device,
                         fragment_loc.dot(Field::width),
                         "is %" PRIu32 ".",
                         fragment_shading_rate_state.fragmentSize.width);
    ...
    if (fragment_shading_rate_state.fragmentSize.height > 4) {
        skip |= LogError("VUID-VkGraphicsPipelineCreateInfo-pDynamicState-04499",
                         device,
                         fragment_loc.dot(Field::height),
                         "is %" PRIu32 ".",
                         fragment_shading_rate_state.fragmentSize.height);
    ...
    if (!enabled_features.pipelineFragmentShadingRate && fragment_shading_rate_state.fragmentSize.width != 1) {
        skip |= LogError("VUID-VkGraphicsPipelineCreateInfo-pDynamicState-04500",
                         device,
                         fragment_loc.dot(Field::width),
...
bool CoreChecks::ValidateGraphicsPipelineDynamicRendering(const vvl::Pipeline& pipeline,
                                                          const Location& create_info_loc) const {
    bool skip = false;
    const auto& pipeline_ci = pipeline.GraphicsCreateInfo();
    ...
    if (!rendering_struct) {
        ...
        if (pipeline.OwnsSubState(pipeline.fragment_output_state) && color_blend_state &&
            color_blend_state->attachmentCount > 0) {
            skip |= LogError("VUID-VkGraphicsPipelineCreateInfo-renderPass-06055",
                             device,
                             create_info_loc.dot(Field::pColorBlendState).dot(Field::attachmentCount),
                             "is %" PRIu32 ", but VkPipelineRenderingCreateInfo::colorAttachmentCount is zero because "
                             "the pNext chain was not included.",
                             color_blend_state->attachmentCount);
        }
        return skip;
    }
    ...
    if (!pipeline.RasterizationDisabled()) {
        if (pipeline.fragment_shader_state && pipeline.fragment_output_state &&
            ...
            !IsExtEnabled(device_extensions.vk_ext_extended_dynamic_state3)) {
            skip |= LogError("VUID-VkGraphicsPipelineCreateInfo-renderPass-09033",
                             device,
                             create_info_loc.dot(Field::pDepthStencilState),
        ...
        if (pipeline.fragment_shader_state && pipeline.fragment_output_state) {
            const auto input_attachment_index =
                vku::FindStructInPNextChain<VkRenderingInputAttachmentIndexInfo>(pipeline_ci.pNext);
            if (input_attachment_index) {
                ...
                if (input_attachment_index->colorAttachmentCount != rendering_struct->colorAttachmentCount) {
                    ...
                    skip |=
                        LogError("VUID-VkGraphicsPipelineCreateInfo-renderPass-09531",
                                 device,
                                 loc,
                                 "(%" PRIu32
                                 ") does not match VkPipelineRenderingCreateInfo.colorAttachmentCount (%" PRIu32 ").",
                                 input_attachment_index->colorAttachmentCount,
                                 rendering_struct->colorAttachmentCount);
                }
            }

            const auto attachment_location =
                vku::FindStructInPNextChain<VkRenderingAttachmentLocationInfo>(pipeline_ci.pNext);
            if (attachment_location) {
                skip |= ValidateRenderingAttachmentLocations(
                    *attachment_location, device, create_info_loc.pNext(Struct::VkRenderingAttachmentLocationInfo));
                if (attachment_location->colorAttachmentCount != rendering_struct->colorAttachmentCount) {
                    ...
                    skip |=
                        LogError("VUID-VkGraphicsPipelineCreateInfo-renderPass-09532",
                                 device,
                                 loc,
                                 "(%" PRIu32
                                 ") does not match VkPipelineRenderingCreateInfo.colorAttachmentCount (%" PRIu32 ").",
                                 attachment_location->colorAttachmentCount,
                                 rendering_struct->colorAttachmentCount);
                }
            }
        }
    ...
    if (pipeline.OwnsSubState(pipeline.fragment_output_state)) {
        for (uint32_t color_index = 0; color_index < rendering_struct->colorAttachmentCount; color_index++) {
            ...
            if (color_format != VK_FORMAT_UNDEFINED) {
                ...
                if (((format_features & VK_FORMAT_FEATURE_COLOR_ATTACHMENT_BLEND_BIT) == 0) &&
                    ...
                    skip |= LogError("VUID-VkGraphicsPipelineCreateInfo-renderPass-06062",
                                     device,
                                     create_info_loc.dot(Field::pColorBlendState)
                ...
                if ((format_features & (VK_FORMAT_FEATURE_2_COLOR_ATTACHMENT_BIT |
                    ...
                    skip |= LogError("VUID-VkGraphicsPipelineCreateInfo-renderPass-06582",
                                     device,
                                     create_info_loc.pNext(Struct::VkPipelineRenderingCreateInfo,
                                                           Field::pColorAttachmentFormats,
                                                           color_index),
                                     "(%s) potential format features are %s.",
        ...
        if (rendering_struct->depthAttachmentFormat != VK_FORMAT_UNDEFINED) {
            ...
            if ((format_features & VK_FORMAT_FEATURE_2_DEPTH_STENCIL_ATTACHMENT_BIT) == 0) {
                skip |=
                    LogError("VUID-VkGraphicsPipelineCreateInfo-renderPass-06585",
                             device,
                             create_info_loc.pNext(Struct::VkPipelineRenderingCreateInfo, Field::depthAttachmentFormat),
                             "(%s) potential format features are %s.",
                             string_VkFormat(rendering_struct->depthAttachmentFormat),
        ...
        if (rendering_struct->stencilAttachmentFormat != VK_FORMAT_UNDEFINED) {
            VkFormatFeatureFlags2 format_features =
                GetPotentialFormatFeatures(rendering_struct->stencilAttachmentFormat);
            if ((format_features & VK_FORMAT_FEATURE_2_DEPTH_STENCIL_ATTACHMENT_BIT) == 0) {
                skip |= LogError(
        ...
        if ((rendering_struct->depthAttachmentFormat != VK_FORMAT_UNDEFINED) &&
            ...
            skip |= LogError("VUID-VkGraphicsPipelineCreateInfo-renderPass-06589",
                             device,
                             create_info_loc.pNext(Struct::VkPipelineRenderingCreateInfo, Field::depthAttachmentFormat),
    ...
    if (pipeline.IsRenderPassStateRequired()) {
        if ((enabled_features.multiview == VK_FALSE) && (rendering_struct->viewMask != 0)) {
            skip |= LogError("VUID-VkGraphicsPipelineCreateInfo-multiview-06577",
                             device,
                             create_info_loc.pNext(Struct::VkPipelineRenderingCreateInfo, Field::viewMask),
        ...
        if (MostSignificantBit(rendering_struct->viewMask) >=
            ...
            skip |= LogError("VUID-VkGraphicsPipelineCreateInfo-renderPass-06578",
                             device,
                             create_info_loc.pNext(Struct::VkPipelineRenderingCreateInfo, Field::viewMask),
                             "is 0x%" PRIx32 ", but maxMultiviewViewCount is %" PRIu32 "",
                             rendering_struct->viewMask,
...
bool CoreChecks::ValidateGraphicsPipelineBindPoint(const vvl::CommandBuffer& cb_state,
                                                   const vvl::Pipeline& pipeline,
                                                   const Location& loc) const {
    bool skip = false;
    ...
    bool dyn_scissor =
        pipeline.IsDynamic(CB_DYNAMIC_STATE_SCISSOR_WITH_COUNT) || pipeline.IsDynamic(CB_DYNAMIC_STATE_SCISSOR);
    if (!dyn_viewport || !dyn_scissor) {
        const LogObjectList objlist(cb_state.Handle(), pipeline.Handle());
        skip |= LogError("VUID-vkCmdBindPipeline-commandBuffer-04808",
                         objlist,
                         loc,
    ...
    if ((discard_rectangle_state && discard_rectangle_state->discardRectangleCount != 0) ||
        ...
        if (!pipeline.IsDynamic(CB_DYNAMIC_STATE_DISCARD_RECTANGLE_EXT)) {
            ...
            skip |= LogError(
                ...
                objlist,
                loc.dot(Field::commandBuffer),
                "is a secondary command buffer with VkCommandBufferInheritanceViewportScissorInfoNV::viewportScissor2D "
                "enabled, pipelineBindPoint is VK_PIPELINE_BIND_POINT_GRAPHICS and pipeline was created with %s, but "
                "without "
...
bool CoreChecks::ValidateDrawPipelineFragmentShadingRate(const vvl::CommandBuffer& cb_state,
                                                         const vvl::Pipeline& pipeline,
    ...
    for (auto& stage_state : pipeline.stage_states) {
        ...
        if (!IsValueIn(stage,
                       { VK_SHADER_STAGE_VERTEX_BIT, VK_SHADER_STAGE_GEOMETRY_BIT, VK_SHADER_STAGE_MESH_BIT_EXT })) {
            continue;
        }
        if (!phys_dev_ext_props.fragment_shading_rate_props.primitiveFragmentShadingRateWithMultipleViewports &&
            pipeline.IsDynamic(CB_DYNAMIC_STATE_VIEWPORT_WITH_COUNT) &&
            cb_state.dynamic_state_value.viewport_count != 1) {
            if (stage_state.entrypoint && stage_state.entrypoint->written_builtin_primitive_shading_rate_khr) {
                skip |= LogError(
                    vuid.viewport_count_primitive_shading_rate_04552,
                    stage_state.module_state->Handle(),
                    vuid.loc(),
...
bool CoreChecks::ValidateDrawPipeline(const LastBound& last_bound_state,
                                      const vvl::Pipeline& pipeline,
                                      const vvl::DrawDispatchVuid& vuid) const {
    bool skip = false;
    ...
    if (cb_state.activeRenderPass->UsesDynamicRendering()) {
        const VkRenderingInfo& rendering_info =
            *(cb_state.activeRenderPass->dynamic_rendering_begin_rendering_info.ptr());
    ...
    if (!pipeline.IsDynamic(CB_DYNAMIC_STATE_RASTERIZATION_SAMPLES_EXT) &&
        ...
        if (msrtss_info && msrtss_info->multisampledRenderToSingleSampledEnable &&
            ...
            const LogObjectList objlist(cb_state.Handle(), pipeline.Handle());
            skip |= LogError(
                vuid.rasterization_samples_07935,
                objlist,
                vuid.loc(),
    ...
    if (pipeline.IsDynamic(CB_DYNAMIC_STATE_ALPHA_TO_COVERAGE_ENABLE_EXT) &&
        ...
        if (fragment_entry_point && !fragment_entry_point->has_alpha_to_coverage_variable) {
            const LogObjectList objlist(cb_state.Handle(), pipeline.Handle());
            skip |= LogError(
                vuid.dynamic_alpha_to_coverage_component_08919,
                objlist,
                vuid.loc(),
    ...
    if ((pipeline.create_info_shaders & (VK_SHADER_STAGE_VERTEX_BIT | VK_SHADER_STAGE_TESSELLATION_CONTROL_BIT |
        ...
        for (const auto& query : cb_state.activeQueries) {
            ...
            if (query_pool_state &&
                query_pool_state->create_info.queryType == VK_QUERY_TYPE_MESH_PRIMITIVES_GENERATED_EXT) {
                const LogObjectList objlist(cb_state.Handle(), pipeline.Handle(), query.pool);
                skip |= LogError(vuid.mesh_shader_queries_07073,
                                 objlist,
                                 vuid.loc(),
                                 "Query (slot %" PRIu32
                                 ") with type VK_QUERY_TYPE_MESH_PRIMITIVES_GENERATED_EXT is active.",
                                 query.slot);
            }
        }
    }
...
bool CoreChecks::ValidateDrawPipelineRenderpass(const LastBound& last_bound_state,
                                                const vvl::Pipeline& pipeline,
                                                const vvl::DrawDispatchVuid& vuid) const {
    bool skip = false;
    const vvl::CommandBuffer& cb_state = last_bound_state.cb_state;
    ...
    if (cb_state.activeRenderPass->VkHandle() != rp_state->VkHandle()) {
        ...
        skip |= ValidateRenderPassCompatibility(cb_state.Handle(),
                                                *cb_state.activeRenderPass.get(),
                                                pipeline.Handle(),
                                                *rp_state.get(),
                                                vuid.loc(),
                                                vuid.render_pass_compatible_02684);
    }
    const auto subpass = pipeline.Subpass();
    if (subpass != cb_state.GetActiveSubpass()) {
        ...
        skip |= LogError(vuid.subpass_index_02685,
                         objlist,
                         vuid.loc(),
                         "Pipeline was built for subpass %" PRIu32 " but used in subpass %" PRIu32 ".",
                         subpass,
    ...
    if (ds_attachment != nullptr) {
            ...
            if (attachment != VK_ATTACHMENT_UNUSED) {
                ...
                if (imageview_state != nullptr) {
                    ...
                    if (image_state != nullptr) {
                        if ((image_state->create_info.flags &
                             VK_IMAGE_CREATE_SAMPLE_LOCATIONS_COMPATIBLE_DEPTH_BIT_EXT) == 0) {
                            const LogObjectList objlist(
                                cb_state.Handle(), pipeline.Handle(), cb_state.activeRenderPass->Handle());
                            skip |=
                                LogError(vuid.sample_location_02689,
                                         objlist,
                                         vuid.loc(),
        ...
        if (ds_state) {
            if (IsImageLayoutDepthReadOnly(ds_attachment->layout) && last_bound_state.IsDepthWriteEnable()) {
                ...
                skip |= LogError(vuid.depth_read_only_06886,
                                 objlist,
                                 vuid.loc(),
            ...
            const bool all_keep_op =
                ((front.failOp == VK_STENCIL_OP_KEEP) && (front.passOp == VK_STENCIL_OP_KEEP) &&
            ...
            if (!all_keep_op && write_mask_enabled) {
                ...
                if (is_stencil_layout_read_only) {
                    const LogObjectList objlist(
                        pipeline.Handle(), cb_state.activeRenderPass->Handle(), cb_state.Handle());
                    skip |= LogError(
                        vuid.stencil_read_only_06887,
                        objlist,
                        vuid.loc(),
                        ...
                        string_VkImageLayout(ds_attachment->layout),
                        string_VkStencilOp(front.failOp),
                        string_VkStencilOp(front.passOp),
                        string_VkStencilOp(front.depthFailOp),
                        string_VkStencilOp(back.failOp),
                        string_VkStencilOp(back.passOp),
...
bool CoreChecks::ValidateDrawPipelineDynamicRenderpass(const LastBound& last_bound_state,
                                                       const vvl::Pipeline& pipeline,
                                                       const VkRenderingInfo& rendering_info,
                                                       const vvl::DrawDispatchVuid& vuid) const {
    bool skip = false;
    ...
    if (rp_state->VkHandle() != VK_NULL_HANDLE) {
        ...
        skip |= LogError(vuid.dynamic_rendering_06198,
                         objlist,
                         vuid.loc(),
    ...
    const VkPipelineRenderingCreateInfo& pipeline_rendering_ci =
        *(rp_state->dynamic_pipeline_rendering_create_info.ptr());

    skip |= ValidateDrawPipelineDynamicRenderpassSampleCount(last_bound_state, pipeline, rendering_info, vuid);
    skip |=
        ValidateDrawPipelineDynamicRenderpassExternalFormatResolve(last_bound_state, pipeline, rendering_info, vuid);
    ...
    skip |= ValidateDrawPipelineDynamicRenderpassUnusedAttachments(
        last_bound_state, pipeline, rendering_info, pipeline_rendering_ci, vuid);
    skip |= ValidateDrawPipelineDynamicRenderpassDepthStencil(
        last_bound_state, pipeline, rendering_info, pipeline_rendering_ci, vuid);
    ...
    if ((pipeline.active_shaders & VK_SHADER_STAGE_FRAGMENT_BIT) != 0) {
        for (uint32_t i = 0; i < rendering_info.colorAttachmentCount; ++i) {
            const bool statically_writes_to_color_attachment =
                pipeline.fragmentShader_writable_output_location_list.find(i) !=
            ...
            if (statically_writes_to_color_attachment && mask_and_write_enabled &&
                ...
                if (i >= pipeline_rendering_ci.colorAttachmentCount ||
                    ...
                    const LogObjectList objlist(
                        cb_state.Handle(), pipeline.Handle(), rendering_info.pColorAttachments[i].imageView);
                    skip |= LogError(vuid.color_attachment_08963,
                                     objlist,
                                     vuid.loc(),
                                     ...
                                     i,
                                     FormatHandle(rendering_info.pColorAttachments[i].imageView).c_str(),
                                     FormatHandle(pipeline).c_str(),
                                     i);
                }
            }
...
bool CoreChecks::ValidateDrawPipelineDynamicRenderpassDepthStencil(
    const LastBound& last_bound_state,
    const vvl::Pipeline& pipeline,
    const VkRenderingInfo& rendering_info,
    const VkPipelineRenderingCreateInfo& pipeline_rendering_ci,
    const vvl::DrawDispatchVuid& vuid) const {
    ...
    if (last_bound_state.IsDepthWriteEnable() && rendering_info.pDepthAttachment &&
        ...
        if (pipeline_rendering_ci.depthAttachmentFormat == VK_FORMAT_UNDEFINED) {
            ...
            skip |= LogError(
                ...
                FormatHandle(rendering_info.pDepthAttachment->imageView).c_str(),
                FormatHandle(pipeline).c_str());
        }
    }
    if (last_bound_state.IsStencilTestEnable() && rendering_info.pStencilAttachment &&
        ...
        if (pipeline_rendering_ci.stencilAttachmentFormat == VK_FORMAT_UNDEFINED) {
            ...
            skip |= LogError(
                ...
                FormatHandle(rendering_info.pStencilAttachment->imageView).c_str(),
                FormatHandle(pipeline).c_str());
        }
    }
...
bool CoreChecks::ValidateDrawPipelineDynamicRenderpassUnusedAttachments(
    const LastBound& last_bound_state,
    const vvl::Pipeline& pipeline,
    const VkRenderingInfo& rendering_info,
    const VkPipelineRenderingCreateInfo& pipeline_rendering_ci,
    ...
    if (!enabled_features.dynamicRenderingUnusedAttachments) {
        ...
        const auto rendering_color_attachment_count =
        ...
        if (color_attachment_count && (color_attachment_count != rendering_color_attachment_count)) {
            const LogObjectList objlist(cb_state.Handle(), pipeline.Handle());
            skip |= LogError(vuid.dynamic_rendering_color_count_06179,
                             objlist,
                             vuid.loc(),
                             ...
                             FormatHandle(pipeline).c_str(),
                             pipeline_rendering_ci.colorAttachmentCount,
        ...
        if (enabled_features.dynamicRenderingUnusedAttachments) {
            if (rendering_info.pColorAttachments[i].imageView != VK_NULL_HANDLE) {
                ...
                if ((pipeline_rendering_ci.colorAttachmentCount > i) &&
                    ...
                    const LogObjectList objlist(cb_state.Handle(), pipeline.Handle());
                    skip |= LogError(vuid.dynamic_rendering_unused_attachments_08911,
                                     objlist,
                                     vuid.loc(),
                                     "VkRenderingInfo::pColorAttachments[%" PRIu32
            ...
            if (rendering_info.pColorAttachments[i].imageView == VK_NULL_HANDLE) {
                if ((pipeline_rendering_ci.colorAttachmentCount > i) &&
                    (pipeline_rendering_ci.pColorAttachmentFormats[i] != VK_FORMAT_UNDEFINED)) {
                    const LogObjectList objlist(cb_state.Handle(), pipeline.Handle());
                    skip |= LogError(vuid.dynamic_rendering_undefined_color_formats_08912,
                                     objlist,
                                     vuid.loc(),
                                     "VkRenderingInfo::pColorAttachments[%" PRIu32
                ...
                if ((pipeline_rendering_ci.colorAttachmentCount > i) &&
                    view_state->create_info.format != pipeline_rendering_ci.pColorAttachmentFormats[i]) {
                    const LogObjectList objlist(cb_state.Handle(), pipeline.Handle());
                    skip |= LogError(vuid.dynamic_rendering_color_formats_08910,
                                     objlist,
                                     vuid.loc(),
                                     "VkRenderingInfo::pColorAttachments[%" PRIu32
                                     "].imageView format (%s) must match corresponding format in "
    ...
    if (rendering_info.pDepthAttachment) {
        if (enabled_features.dynamicRenderingUnusedAttachments) {
            if (rendering_info.pDepthAttachment->imageView != VK_NULL_HANDLE) {
                ...
                if (view_state && (view_state->create_info.format != VK_FORMAT_UNDEFINED) &&
                    ...
                    const LogObjectList objlist(cb_state.Handle(), pipeline.Handle());
                    skip |= LogError(
                        vuid.dynamic_rendering_unused_attachments_08915,
                        objlist,
                        vuid.loc(),
            ...
            if (rendering_info.pDepthAttachment->imageView == VK_NULL_HANDLE) {
                if (pipeline_rendering_ci.depthAttachmentFormat != VK_FORMAT_UNDEFINED) {
                    const LogObjectList objlist(cb_state.Handle(), pipeline.Handle());
                    skip |= LogError(
                        vuid.dynamic_rendering_undefined_depth_format_08913,
                        objlist,
                        vuid.loc(),
                ...
                if (view_state && view_state->create_info.format != pipeline_rendering_ci.depthAttachmentFormat) {
                    const LogObjectList objlist(cb_state.Handle(), pipeline.Handle());
                    skip |= LogError(
                        vuid.dynamic_rendering_depth_format_08914,
                        objlist,
                        vuid.loc(),
                        "VkRenderingInfo::pDepthAttachment->imageView format (%s) must match corresponding format "
                        "in VkPipelineRenderingCreateInfo::depthAttachmentFormat (%s)",
        ...
        if (cb_state.activeRenderPass->inheritance_rendering_info.depthAttachmentFormat !=
            ...
            const LogObjectList objlist(cb_state.Handle(), pipeline.Handle());
            skip |=
                LogError(vuid.dynamic_rendering_depth_format_08914,
                         objlist,
                         vuid.loc(),
    ...
    if (rendering_info.pStencilAttachment) {
        if (enabled_features.dynamicRenderingUnusedAttachments) {
            if (rendering_info.pStencilAttachment->imageView != VK_NULL_HANDLE) {
                ...
                if (view_state && (view_state->create_info.format != VK_FORMAT_UNDEFINED) &&
                    ...
                    const LogObjectList objlist(cb_state.Handle(), pipeline.Handle());
                    skip |=
                        LogError(vuid.dynamic_rendering_unused_attachments_08918,
                                 objlist,
                                 vuid.loc(),
            ...
            if (rendering_info.pStencilAttachment->imageView == VK_NULL_HANDLE) {
                if (pipeline_rendering_ci.stencilAttachmentFormat != VK_FORMAT_UNDEFINED) {
                    const LogObjectList objlist(cb_state.Handle(), pipeline.Handle());
                    skip |= LogError(vuid.dynamic_rendering_undefined_stencil_format_08916,
                                     objlist,
                                     vuid.loc(),
                ...
                if (view_state && view_state->create_info.format != pipeline_rendering_ci.stencilAttachmentFormat) {
                    const LogObjectList objlist(cb_state.Handle(), pipeline.Handle());
                    skip |=
                        LogError(vuid.dynamic_rendering_stencil_format_08917,
                                 objlist,
                                 vuid.loc(),
                                 "VkRenderingInfo::pStencilAttachment->imageView format (%s) must match corresponding "
                                 "format in VkPipelineRenderingCreateInfo::stencilAttachmentFormat (%s)",
        ...
        if (cb_state.activeRenderPass->inheritance_rendering_info.stencilAttachmentFormat !=
            ...
            const LogObjectList objlist(cb_state.Handle(), pipeline.Handle());
            skip |= LogError(
                vuid.dynamic_rendering_stencil_format_08917,
                objlist,
                vuid.loc(),
...
bool CoreChecks::ValidateDrawPipelineDynamicRenderpassFragmentShadingRate(const LastBound& last_bound_state,
    ...
    if (rendering_fragment_shading_rate_attachment_info &&
        ...
        if (!(pipeline.create_flags & VK_PIPELINE_CREATE_2_RENDERING_FRAGMENT_SHADING_RATE_ATTACHMENT_BIT_KHR)) {
            const LogObjectList objlist(cb_state.Handle(), pipeline.Handle());
            skip |= LogError(vuid.dynamic_rendering_fsr_06183,
                             objlist,
                             vuid.loc(),
    ...
    if (rendering_fragment_shading_rate_density_map &&
        ...
        if (!(pipeline.create_flags & VK_PIPELINE_RASTERIZATION_STATE_CREATE_FRAGMENT_DENSITY_MAP_ATTACHMENT_BIT_EXT)) {
            const LogObjectList objlist(cb_state.Handle(), pipeline.Handle());
            skip |= LogError(vuid.dynamic_rendering_fdm_06184,
                             objlist,
                             vuid.loc(),
                             "Currently bound graphics pipeline %s must have been created with "
...
bool CoreChecks::ValidateDrawPipelineDynamicRenderpassLegacyDithering(const LastBound& last_bound_state,
    ...
    const bool has_legacy_dithering_rendering =
        (rendering_info.flags & VK_RENDERING_ENABLE_LEGACY_DITHERING_BIT_EXT) != 0;
    if (has_legacy_dithering_pipeline && !has_legacy_dithering_rendering) {
        const LogObjectList objlist(cb_state.Handle(), pipeline.Handle());
        skip |= LogError(
            vuid.dynamic_rendering_dithering_09643,
            objlist,
            vuid.loc(),
        ...
        const LogObjectList objlist(cb_state.Handle(), pipeline.Handle());
        skip |= LogError(
            vuid.dynamic_rendering_dithering_09642,
            objlist,
            vuid.loc(),
...
bool CoreChecks::ValidateDrawPipelineDynamicRenderpassExternalFormatResolve(const LastBound& last_bound_state,
    ...
    if (rendering_info.colorAttachmentCount == 1 &&
        ...
        if (auto resolve_image_view_state = Get<vvl::ImageView>(rendering_info.pColorAttachments[0].resolveImageView)) {
            if (resolve_image_view_state->image_state->ahb_format != pipeline_external_format) {
                skip |= LogError(vuid.external_format_resolve_09362,
                                 objlist,
                                 vuid.loc(),
                                 ...
                                 resolve_image_view_state->image_state->ahb_format);
            }
        }
        ...
        if (auto color_image_view_state = Get<vvl::ImageView>(rendering_info.pColorAttachments[0].imageView)) {
            if (color_image_view_state->image_state->ahb_format != pipeline_external_format) {
                skip |= LogError(vuid.external_format_resolve_09363,
                                 objlist,
                                 vuid.loc(),
                                 "pipeline externalFormat is %" PRIu64
                                 " but the imageView's image was created with externalFormat %" PRIu64 "",
                                 pipeline_external_format,
                                 color_image_view_state->image_state->ahb_format);
            }
        }
    }
    ...
    if (pipeline.IsDynamic(CB_DYNAMIC_STATE_COLOR_BLEND_ENABLE_EXT) &&
        ...
        skip |= LogError(vuid.external_format_resolve_09364,
                         objlist,
                         vuid.loc(),
                         "pipeline externalFormat is %" PRIu64
    ...
    if (pipeline.IsDynamic(CB_DYNAMIC_STATE_RASTERIZATION_SAMPLES_EXT) &&
        ...
        skip |= LogError(vuid.external_format_resolve_09365,
                         objlist,
                         vuid.loc(),
    ...
    if (pipeline.IsDynamic(CB_DYNAMIC_STATE_FRAGMENT_SHADING_RATE_KHR)) {
        if (cb_state.dynamic_state_value.fragment_size.width != 1) {
            skip |= LogError(vuid.external_format_resolve_09368,
                             objlist,
                             vuid.loc(),
        ...
        if (cb_state.dynamic_state_value.fragment_size.height != 1) {
            skip |= LogError(vuid.external_format_resolve_09369,
                             objlist,
                             vuid.loc(),
    ...
    if (auto fragment_entry_point = last_bound_state.GetFragmentEntryPoint()) {
        if (fragment_entry_point->execution_mode.Has(spirv::ExecutionModeSet::depth_replacing_bit)) {
            skip |= LogError(vuid.external_format_resolve_09372,
                             objlist,
                             vuid.loc(),
            ...
            skip |= LogError(vuid.external_format_resolve_09372,
                             ...
                             vuid.loc(),
                             "pipeline externalFormat is %" PRIu64
                             " but the fragment shader declares StencilRefReplacingEXT.",
                             pipeline_external_format);
        }
    }
...
bool CoreChecks::ValidateDrawPipelineDynamicRenderpassSampleCount(const LastBound& last_bound_state,
                                                                  const vvl::Pipeline& pipeline,
                                                                  const VkRenderingInfo& rendering_info,
                                                                  const vvl::DrawDispatchVuid& vuid) const {
    bool skip = false;
    ...
    if (auto p_attachment_sample_count_info =
        ...
        for (uint32_t i = 0; i < rendering_info.colorAttachmentCount; ++i) {
            ...
            if (p_attachment_sample_count_info &&
                ...
                const LogObjectList objlist(cb_state.Handle(), pipeline.Handle());
                skip |=
                    LogError(vuid.dynamic_rendering_color_sample_06185,
                             objlist,
                             vuid.loc(),
                             ...
                             "sample count (%s)",
                             i,
                             string_VkSampleCountFlagBits(color_image_samples),
        ...
        if (rendering_info.pDepthAttachment != nullptr) {
            ...
            if (p_attachment_sample_count_info) {
                if (depth_image_samples != p_attachment_sample_count_info->depthStencilAttachmentSamples) {
                    const LogObjectList objlist(cb_state.Handle(), pipeline.Handle());
                    skip |= LogError(
                        vuid.dynamic_rendering_depth_sample_06186,
                        objlist,
                        vuid.loc(),
        ...
        for (uint32_t i = 0; i < rendering_info.colorAttachmentCount; ++i) {
            ...
            if (samples != rasterization_samples) {
                const LogObjectList objlist(cb_state.Handle(), pipeline.Handle());
                skip |= LogError(vuid.dynamic_rendering_07285,
                                 objlist,
                                 vuid.loc(),
                                 "Color attachment (%" PRIu32
        ...
        if ((rendering_info.pDepthAttachment != nullptr) &&
            ...
            if (depth_image_samples != rasterization_samples) {
                const LogObjectList objlist(cb_state.Handle(), pipeline.Handle());
                skip |= LogError(vuid.dynamic_rendering_07286,
                                 objlist,
                                 vuid.loc(),
                                 "Depth attachment sample count (%s) must match corresponding "
        ...
        if ((rendering_info.pStencilAttachment != nullptr) &&
            ...
            ASSERT_AND_RETURN_SKIP(stencil_view_state);
            const auto& stencil_image_samples =
                Get<vvl::Image>(stencil_view_state->create_info.image)->create_info.samples;
            ASSERT_AND_RETURN_SKIP(stencil_image_samples);

            if (stencil_image_samples != rasterization_samples) {
                const LogObjectList objlist(cb_state.Handle(), pipeline.Handle());
                skip |= LogError(vuid.dynamic_rendering_07287,
                                 objlist,
                                 vuid.loc(),
                                 "Stencil attachment sample count (%s) must match corresponding "
                                 "VkPipelineMultisampleStateCreateInfo::rasterizationSamples count (%s)",
...
bool CoreChecks::ValidatePipelineVertexDivisors(const vvl::Pipeline& pipeline, const Location& create_info_loc) const {
    ...
    const auto divisor_state_info =
        vku::FindStructInPNextChain<VkPipelineVertexInputDivisorStateCreateInfo>(input_state->pNext);
    ...
    for (uint32_t j = 0; j < divisor_state_info->vertexBindingDivisorCount; j++) {
        ...
        if (vibdd->binding >= device_limits->maxVertexInputBindings) {
            skip |= LogError("VUID-VkVertexInputBindingDivisorDescription-binding-01869",
                             device,
                             divisor_loc.dot(Field::binding),
                             "(%" PRIu32 ") exceeds device maxVertexInputBindings (%" PRIu32 ").",
                             vibdd->binding,
                             device_limits->maxVertexInputBindings);
        }
        if (vibdd->divisor > phys_dev_props_core14.maxVertexAttribDivisor) {
            skip |= LogError("VUID-VkVertexInputBindingDivisorDescription-divisor-01870",
                             device,
                             divisor_loc.dot(Field::divisor),
                             "(%" PRIu32 ") exceeds device maxVertexAttribDivisor (%" PRIu32 ").",
                             vibdd->divisor,
                             phys_dev_props_core14.maxVertexAttribDivisor);
        }
        if ((0 == vibdd->divisor) && !enabled_features.vertexAttributeInstanceRateZeroDivisor) {
            skip |= LogError("VUID-VkVertexInputBindingDivisorDescription-vertexAttributeInstanceRateZeroDivisor-02228",
                             device,
                             divisor_loc.dot(Field::divisor),
        ...
        if ((1 != vibdd->divisor) && !enabled_features.vertexAttributeInstanceRateDivisor) {
            skip |= LogError("VUID-VkVertexInputBindingDivisorDescription-vertexAttributeInstanceRateDivisor-02229",
                             device,
                             divisor_loc.dot(Field::divisor),
                             "is (%" PRIu32 ") but vertexAttributeInstanceRateDivisor feature was not enabled.",
                             vibdd->divisor);
        }
        ...
        if (!found_input_rate) { // Description not found, or has incorrect inputRate value
            skip |= LogError("VUID-VkVertexInputBindingDivisorDescription-inputRate-01871",
                             device,
                             divisor_loc.dot(Field::binding),
                             "is %" PRIu32 ", but inputRate is not VK_VERTEX_INPUT_RATE_INSTANCE.",
                             vibdd->binding);
        }
    }
    return skip;
...
bool CoreChecks::ValidatePipelineLibraryFlags(const VkGraphicsPipelineLibraryFlagsEXT lib_flags,
    ...
    const uint32_t flags_count =
        GetBitSetCount(lib_flags & (VK_GRAPHICS_PIPELINE_LIBRARY_PRE_RASTERIZATION_SHADERS_BIT_EXT |
                                    VK_GRAPHICS_PIPELINE_LIBRARY_FRAGMENT_SHADER_BIT_EXT |
                                    VK_GRAPHICS_PIPELINE_LIBRARY_FRAGMENT_OUTPUT_INTERFACE_BIT_EXT));
    if (flags_count == 0 || flags_count > 3)
        return skip;

    // We start iterating at the index after lib_index to avoid duplicating checks, because the caller will iterate the
    // same loop
    for (int i = lib_index + 1; i < static_cast<int>(link_info.libraryCount); ++i) {
        ...
        if (current_pipeline) {
            if (lib->GraphicsCreateInfo().renderPass != VK_NULL_HANDLE) {
                skip |= LogError("VUID-VkGraphicsPipelineCreateInfo-renderpass-06625",
                                 device,
                                 loc.dot(Field::renderPass),
                                 ...
                                 string_VkGraphicsPipelineLibraryFlagsEXT(lib_flags).c_str(),
                                 i,
                                 string_VkGraphicsPipelineLibraryFlagsEXT(lib->graphics_lib_type).c_str());
            }
        }
        ...
        if (view_mask != lib_view_mask) {
            skip |= LogError(vuid,
                             device,
                             loc,
                             "pLibraries[%" PRIu32 "] is (flags = %s and viewMask = %" PRIu32
                             "), but pLibraries[%" PRIu32 "] is (flags = %s and viewMask %" PRIu32 ").",
                             lib_index,
                             string_VkGraphicsPipelineLibraryFlagsEXT(lib_flags).c_str(),
                             view_mask,
                             i,
...
bool CoreChecks::ValidateGraphicsPipelineDerivatives(PipelineStates& pipeline_states,
                                                     uint32_t pipe_index,
                                                     const Location& loc) const {
    bool skip = false;
    ...
    if (pipeline.create_flags & VK_PIPELINE_CREATE_2_DERIVATIVE_BIT) {
        ...
        if (base_index != -1 && base_index < static_cast<int32_t>(pipeline_states.size())) {
            if (static_cast<uint32_t>(base_index) >= pipe_index) {
                skip |= LogError("VUID-vkCreateGraphicsPipelines-flags-00720",
                                 base_handle,
                                 loc,
        ...
        if (base_pipeline && !(base_pipeline->create_flags & VK_PIPELINE_CREATE_2_ALLOW_DERIVATIVES_BIT)) {
            skip |= LogError("VUID-vkCreateGraphicsPipelines-flags-00721",
                             base_pipeline->Handle(),
                             loc,
...
bool CoreChecks::ValidateMultiViewShaders(const vvl::Pipeline& pipeline,
                                          const Location& multiview_loc,
                                          uint32_t view_mask,
    ...
    if (!enabled_features.multiviewMeshShader && (stages & VK_SHADER_STAGE_MESH_BIT_EXT)) {
        ...
        skip |= LogError(vuid,
                         ...
                         multiview_loc,
                         "is %" PRIu32
                         " and pStages contains mesh shader, but the multiviewMeshShader feature was not enabled.",
                         view_mask);
    }

    for (const auto& stage : pipeline.stage_states) {
        // Stage may not have SPIR-V data (e.g. due to the use of shader module identifier or in Vulkan SC)
        if (!stage.spirv_state)
            continue;
        ...
        if (stage.spirv_state->static_data_.has_builtin_layer) {
            ...
            skip |= LogError(vuid,
                             device,
                             multiview_loc,
                             "is %" PRIu32 " but %s stage contains a Layer decorated OpVariable.",
                             view_mask,
                             string_VkShaderStageFlagBits(stage.GetStage()));
        }
    }
...
bool CoreChecks::ValidateDrawPipelineVertexAttribute(const vvl::CommandBuffer& cb_state,
                                                     const vvl::Pipeline& pipeline,
                                                     const vvl::DrawDispatchVuid& vuid) const {
    bool skip = false;
    ...
    const auto& vertex_bindings = pipeline.IsDynamic(CB_DYNAMIC_STATE_VERTEX_INPUT_EXT)
                                      ? cb_state.dynamic_state_value.vertex_bindings
    ...
    for (const auto& binding_state : vertex_bindings) {
        const auto& binding_index = binding_state.second.index;
        const auto& binding_desc = binding_state.second.desc;
        const auto binding_num = binding_desc.binding;

        const auto* vertex_buffer = vvl::Find(cb_state.current_vertex_buffer_binding_info, binding_num);
        if (!vertex_buffer) {
            const LogObjectList objlist(cb_state.Handle(), pipeline.Handle());
            skip |=
                LogError(vuid.vertex_binding_attribute_02721,
                         objlist,
                         vuid.loc(),
                         "pVertexBindingDescriptions[%" PRIu32 "] (binding %" PRIu32 " ) was not bound to a buffer.",
                         binding_index,
                         binding_num);
            break;
        }
        if (vertex_buffer->buffer == VK_NULL_HANDLE && !enabled_features.nullDescriptor) {
            const LogObjectList objlist(cb_state.Handle(), pipeline.Handle());
            skip |= LogError(vuid.vertex_binding_attribute_02721,
                             objlist,
                             vuid.loc(),
                             "pVertexBindingDescriptions[%" PRIu32 "] (binding %" PRIu32
                             ") points to a VK_NULL_HANDLE buffer.",
                             binding_index,
                             binding_num);
            break;
        }
        auto const buffer_state = Get<vvl::Buffer>(vertex_buffer->buffer);
        if (!buffer_state) {
            const LogObjectList objlist(cb_state.Handle(), pipeline.Handle());
            skip |=
                LogError(vuid.vertex_binding_attribute_02721,
                         objlist,
                         vuid.loc(),
                         "pVertexBindingDescriptions[%" PRIu32 "] (binding %" PRIu32 ") points to an invalid buffer.",
                         binding_index,
                         binding_num);
            break;
        }
        for (const auto& location : binding_state.second.locations) {
            const auto attr_index = location.second.index;
            const auto& attr_desc = location.second.desc;

            const VkDeviceSize attribute_offset = attr_desc.offset;
            ...
            if (pipeline.IsDynamic(CB_DYNAMIC_STATE_VERTEX_INPUT_BINDING_STRIDE)) {
                ...
                if (vertex_buffer_stride != 0 && vertex_buffer_stride < attribute_binding_extent) {
                    ...
                    skip |= LogError("VUID-vkCmdBindVertexBuffers2-pStrides-06209",
                                     ...
                                     String(vuid.function),
                                     attribute_binding_extent);
                }
            }
            const VkDeviceSize vertex_buffer_offset = vertex_buffer->offset;
            ...
            if (vkuFormatElementIsTexel(attribute_format)) {
                vtx_attrib_req_alignment =
                    SafeDivision(vtx_attrib_req_alignment, vkuFormatComponentCount(attribute_format));
            }
            ...
            if (SafeModulo(attrib_address, vtx_attrib_req_alignment) != 0) {
                ...
                skip |=
                    LogError(vuid.vertex_binding_attribute_02721,
                             ...
                             string_VkFormat(attribute_format),
                             vtx_attrib_req_alignment,
                             attrib_address,
                             attr_index,
                             attr_desc.binding,
...
bool CoreChecks::ValidateDrawPipelineFramebuffer(const vvl::CommandBuffer& cb_state,
                                                 const vvl::Pipeline& pipeline,
                                                 const vvl::DrawDispatchVuid& vuid) const {
    bool skip = false;
        ...
        for (uint32_t i = 0; i < cb_state.active_attachments.size(); i++) {
            ...
            if (subpass.used && view_state && !view_state->Destroyed()) {
                ...
                if (subpass.usage != VK_IMAGE_USAGE_INPUT_ATTACHMENT_BIT &&
                    ...
                    skip |= ValidateUnprotectedImage(cb_state,
                                                     *view_state->image_state,
                                                     vuid.loc(),
                                                     vuid.protected_command_buffer_02712,
                                                     image_desc.c_str());
                }
                skip |= ValidateProtectedImage(cb_state,
                                               *view_state->image_state,
                                               vuid.loc(),
                                               vuid.unprotected_command_buffer_02707,
                                               image_desc.c_str());
            }
        }
    }
...
bool CoreChecks::ValidateDrawPipelineVertexBinding(const vvl::CommandBuffer& cb_state,
                                                   const vvl::Pipeline& pipeline,
                                                   const vvl::DrawDispatchVuid& vuid) const {
    bool skip = false;
    ...
    const auto& vertex_bindings = pipeline.vertex_input_state->bindings;
    ...
    for (const auto& vertex_buffer_binding : cb_state.current_vertex_buffer_binding_info) {
        ...
        if (!pipeline.IsDynamic(CB_DYNAMIC_STATE_VERTEX_INPUT_EXT) &&
            !vvl::Find(vertex_bindings, vertex_buffer_binding.first)) {
            continue;
        }
        ...
        if (vertex_buffer_binding.second.buffer == VK_NULL_HANDLE) {
            if (!enabled_features.nullDescriptor) {
                const LogObjectList objlist(cb_state.Handle(), pipeline.Handle());
                skip |= LogError(vuid.vertex_binding_null_04008,
                                 objlist,
                                 vuid.loc(),
            ...
            const LogObjectList objlist(cb_state.Handle(), pipeline.Handle());
            skip |= LogError(vuid.vertex_binding_04007,
                             objlist,
                             vuid.loc(),
                             "Vertex binding %" PRIu32
                             " is not a valid VkBuffer. (Check the buffer set in vkCmdBindVertexBuffers)",
                             vertex_buffer_binding.first);
        }
    }

    skip |= ValidateDrawPipelineVertexAttribute(cb_state, pipeline, vuid);

    return skip;
}

bool CoreChecks::ValidateDrawPipelineRasterizationState(const LastBound& last_bound_state,
                                                        const vvl::Pipeline& pipeline,
                                                        const vvl::DrawDispatchVuid& vuid) const {
    bool skip = false;
    const vvl::CommandBuffer& cb_state = last_bound_state.cb_state;
    ...
    if (cb_state.activeRenderPass->UsesDynamicRendering()) {
            ...
            if (imageview_state && color_blend_state && (attachment < color_blend_state->attachmentCount)) {
                if ((imageview_state->format_features & VK_FORMAT_FEATURE_2_COLOR_ATTACHMENT_BLEND_BIT) == 0 &&
                    ...
                    const LogObjectList objlist(
                        cb_state.Handle(), pipeline.Handle(), cb_state.activeRenderPass->Handle());
                    skip |=
                        LogError(vuid.blend_enable_04727,
                                 objlist,
                                 vuid.loc(),
        ...
        if (!(IsExtEnabled(device_extensions.vk_amd_mixed_attachment_samples) ||
            ...
            const LogObjectList objlist(cb_state.Handle(), pipeline.Handle(), cb_state.activeRenderPass->Handle());
            skip |= LogError(vuid.msrtss_rasterization_samples_07284,
                             objlist,
                             vuid.loc(),
        ...
        if (dynamic_line_stipple_enable || dynamic_line_raster_mode) {
            const auto raster_line_state = vku::FindStructInPNextChain<VkPipelineRasterizationLineStateCreateInfo>(
                pipeline.RasterizationStatePNext());

            const VkLineRasterizationMode line_rasterization_mode =
            ...
            if (stippled_line_enable) {
                if (line_rasterization_mode == VK_LINE_RASTERIZATION_MODE_RECTANGULAR &&
                    ...
                    const LogObjectList objlist(
                        cb_state.Handle(), pipeline.Handle(), cb_state.activeRenderPass->Handle());
                    skip |=
                        LogError(vuid.stippled_rectangular_lines_07495,
                                 objlist,
                                 vuid.loc(),
                ...
                if (line_rasterization_mode == VK_LINE_RASTERIZATION_MODE_BRESENHAM &&
                    (!enabled_features.stippledBresenhamLines)) {
                    const LogObjectList objlist(
                        cb_state.Handle(), pipeline.Handle(), cb_state.activeRenderPass->Handle());
                    skip |=
                        LogError(vuid.stippled_bresenham_lines_07496,
                                 objlist,
                                 vuid.loc(),
                ...
                if (line_rasterization_mode == VK_LINE_RASTERIZATION_MODE_RECTANGULAR_SMOOTH &&
                    ...
                    const LogObjectList objlist(
                        cb_state.Handle(), pipeline.Handle(), cb_state.activeRenderPass->Handle());
                    skip |=
                        LogError(vuid.stippled_smooth_lines_07497,
                                 objlist,
                                 vuid.loc(),
                ...
                if (line_rasterization_mode == VK_LINE_RASTERIZATION_MODE_DEFAULT &&
                    ...
                    const LogObjectList objlist(
                        cb_state.Handle(), pipeline.Handle(), cb_state.activeRenderPass->Handle());
                    skip |= LogError(vuid.stippled_default_strict_07498,
                                     objlist,
                                     vuid.loc(),
                                     "lineRasterizationMode = VK_LINE_RASTERIZATION_MODE_DEFAULT (set %s) with "
                                     "stippledLineEnable (set %s), the stippledRectangularLines features is %s and "
                                     "strictLines is %s.",
                                     dynamic_line_raster_mode ? "dynamically" : "in pipeline",
                                     dynamic_line_stipple_enable ? "dynamically" : "in pipeline",
...
bool CoreChecks::ValidatePipelineDiscardRectangleStateCreateInfo(
    const vvl::Pipeline& pipeline,
    const VkPipelineDiscardRectangleStateCreateInfoEXT& discard_rectangle_state,
    const Location& create_info_loc) const {
    bool skip = false;
...
bool CoreChecks::ValidatePipelineAttachmentSampleCountInfo(
    const vvl::Pipeline& pipeline,
    ...
    if (pipeline.fragment_output_state && attachment_sample_count_info.depthStencilAttachmentSamples != 0 &&
        ...
        skip |= LogError(
            ...
            "(0x%" PRIx32 ") is invalid.",
            attachment_sample_count_info.depthStencilAttachmentSamples);
    }
    return skip;
}
