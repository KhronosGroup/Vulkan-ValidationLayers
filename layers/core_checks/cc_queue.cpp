...
struct CommandBufferSubmitState {
    ...
    bool Validate(const Location& loc, const vvl::CommandBuffer& cb_state, uint32_t perf_pass) {
        ...
        skip |= core.ValidatePrimaryCommandBufferState(
            loc,
            cb_state,
            static_cast<uint32_t>(std::count(current_cmds.begin(), current_cmds.end(), cmd)),
            &qfo_image_scoreboards,
        ...
        // Call submit-time functions to validate or update local mirrors of state (to preserve const-ness at validate
        // time)
        ...
        for (auto& function : cb_state.event_updates) {
            skip |= function(const_cast<vvl::CommandBuffer&>(cb_state),
                             /*do_validate*/ true,
                             local_event_signal_info,
                             queue_state.VkHandle(),
                             loc);
        }
        ...
        for (const auto& it : cb_state.video_session_updates) {
            ...
            if (video_session_state && (local_state_it == local_video_session_state.end())) {
                local_state_it =
                    local_video_session_state.insert({ it.first, video_session_state->DeviceStateCopy() }).first;
            }
    ...
    bool ValidateCmdBufLabelMatching(const Location& loc, const vvl::CommandBuffer& cb_state) {
        ...
        if (found_unbalanced_cmdbuf_label) {
            ...
            if (last_closed_cmdbuf_label.empty()) {
                ...
                previous_debug_region = std::string("The previous debug region before the invalid command is '") +
                                        last_closed_cmdbuf_label + "'.";
            }
            skip |= core.LogError(
                ...
                core.FormatHandle(cb_state).c_str(),
                previous_debug_region.c_str());
        }
        return skip;
    }
};

bool CoreChecks::PreCallValidateQueueSubmit(VkQueue queue,
    ...
    if (auto fence_state = Get<vvl::Fence>(fence)) {
        ...
        skip |= ValidateFenceForSubmit(*fence_state,
    ...
    for (uint32_t submit_idx = 0; submit_idx < submitCount; submit_idx++) {
        ...
        if (protected_submit_info) {
            ...
            if ((protected_submit == true) &&
                ((queue_state->create_flags & VK_DEVICE_QUEUE_CREATE_PROTECTED_BIT) == 0)) {
                skip |= LogError("VUID-vkQueueSubmit-queue-06448",
                                 queue,
                                 submit_loc,
        ...
        for (uint32_t i = 0; i < submit.commandBufferCount; i++) {
            ...
            if ((cb_state->unprotected == true) && (protected_submit == true)) {
                ...
                skip |= LogError("VUID-VkSubmitInfo-pNext-04148",
                                 objlist,
                                 cb_loc,
            ...
            if ((cb_state->unprotected == false) && (protected_submit == false)) {
                const LogObjectList objlist(cb_state->Handle(), queue);
                skip |= LogError("VUID-VkSubmitInfo-pNext-04120",
                                 objlist,
                                 cb_loc,
            ...
            if (suspended_render_pass_instance && cb_state->hasRenderPassInstance &&
                !cb_state->resumesRenderPassInstance) {
                skip |= LogError("VUID-VkSubmitInfo-pCommandBuffers-06016",
                                 queue,
                                 submit_loc,
            ...
            if (cb_state->resumesRenderPassInstance) {
                if (!suspended_render_pass_instance) {
                    const LogObjectList objlist(cb_state->Handle(), queue);
                    skip |= LogError("VUID-VkSubmitInfo-pCommandBuffers-06193",
                                     objlist,
                                     cb_loc,
        ...
        if (suspended_render_pass_instance) {
            skip |= LogError("VUID-VkSubmitInfo-pCommandBuffers-06014",
                             queue,
                             submit_loc,
        ...
        if (chained_device_group_struct && chained_device_group_struct->commandBufferCount > 0) {
            for (uint32_t i = 0; i < chained_device_group_struct->commandBufferCount; ++i) {
                ...
                skip |= ValidateDeviceMaskToPhysicalDeviceCount(
                    chained_device_group_struct->pCommandBufferDeviceMasks[i],
                    objlist,
            ...
            if (chained_device_group_struct->signalSemaphoreCount != submit.signalSemaphoreCount) {
                skip |= LogError("VUID-VkDeviceGroupSubmitInfo-signalSemaphoreCount-00084",
                                 queue,
                                 ...
                                 chained_device_group_struct->signalSemaphoreCount,
                                 submit.signalSemaphoreCount);
            }
            if (chained_device_group_struct->waitSemaphoreCount != submit.waitSemaphoreCount) {
                skip |= LogError("VUID-VkDeviceGroupSubmitInfo-waitSemaphoreCount-00082",
                                 queue,
                                 ...
                                 chained_device_group_struct->waitSemaphoreCount,
                                 submit.waitSemaphoreCount);
            }
            if (chained_device_group_struct->commandBufferCount != submit.commandBufferCount) {
                skip |= LogError("VUID-VkDeviceGroupSubmitInfo-commandBufferCount-00083",
                                 queue,
                                 ...
                                 chained_device_group_struct->commandBufferCount,
...
bool CoreChecks::ValidateRenderPassStripeSubmitInfo(VkQueue queue,
    ...
    const VkRenderPassStripeSubmitInfoARM* rp_submit_info =
        vku::FindStructInPNextChain<VkRenderPassStripeSubmitInfoARM>(pNext);
    if (!rp_submit_info) {
        if (cb_state.has_render_pass_striped && !cb_state.resumesRenderPassInstance) {
            skip |= LogError("VUID-VkCommandBufferSubmitInfo-commandBuffer-09445",
                             objlist,
                             loc.dot(Field::pNext),
    ...
    if (rp_submit_info->stripeSemaphoreInfoCount != cb_state.striped_count && !cb_state.resumesRenderPassInstance) {
        skip |=
            LogError("VUID-VkCommandBufferSubmitInfo-pNext-09446",
                     objlist,
                     ...
                     rp_submit_info->stripeSemaphoreInfoCount,
                     cb_state.striped_count);
    }
    ...
    for (uint32_t count = 0; count < rp_submit_info->stripeSemaphoreInfoCount; ++count) {
        ...
        if (semaphore_state->type != VK_SEMAPHORE_TYPE_BINARY) {
            ...
            skip |= LogError("VUID-VkRenderPassStripeSubmitInfoARM-semaphore-09447",
                             objlist,
...
bool CoreChecks::ValidateQueueSubmit2(VkQueue queue,
                                      uint32_t submitCount,
    ...
    if (!enabled_features.synchronization2) {
        skip |= LogError("VUID-vkQueueSubmit2-synchronization2-03866",
                         queue,
                         error_obj.location,
    ...
    for (uint32_t submit_idx = 0; submit_idx < submitCount; submit_idx++) {
        ...
        if ((protected_submit == true) && ((queue_state->create_flags & VK_DEVICE_QUEUE_CREATE_PROTECTED_BIT)) == 0) {
            skip |= LogError("VUID-vkQueueSubmit2-queue-06447",
                             queue,
                             submit_loc,
                             "contains a protected submission to %s which was not created with "
                             "VK_DEVICE_QUEUE_CREATE_PROTECTED_BIT",
        ...
        for (uint32_t i = 0; i < submit.commandBufferInfoCount; i++) {
            ...
            {
                const LogObjectList objlist(queue);
                skip |= ValidateDeviceMaskToPhysicalDeviceCount(submit.pCommandBufferInfos[i].deviceMask,
                                                                queue,
            ...
            if ((cb_state->unprotected == true) && (protected_submit == true)) {
                const LogObjectList objlist(cb_state->Handle(), queue);
                skip |= LogError("VUID-VkSubmitInfo2-flags-03886",
                                 objlist,
                                 cb_loc,
                                 "is unprotected while %s.flags (%s) has VK_SUBMIT_PROTECTED_BIT set",
                                 submit_loc.Fields().c_str(),
                                 string_VkSubmitFlags(submit.flags).c_str());
            }
            if ((cb_state->unprotected == false) && (protected_submit == false)) {
                const LogObjectList objlist(cb_state->Handle(), queue);
                skip |= LogError("VUID-VkSubmitInfo2-flags-03887",
                                 objlist,
                                 cb_loc,
            ...
            if (suspended_render_pass_instance && cb_state->hasRenderPassInstance &&
                !cb_state->resumesRenderPassInstance) {
                skip |= LogError("VUID-VkSubmitInfo2-commandBuffer-06012",
                                 queue,
                                 submit_loc,
                                 "has a suspended render pass instance, but pCommandBuffers[%" PRIu32
                                 "] has its own render pass instance that does not resume it.",
            ...
            if (cb_state->resumesRenderPassInstance) {
                if (!suspended_render_pass_instance) {
                    skip |= LogError("VUID-VkSubmitInfo2-commandBuffer-06192",
                                     queue,
                                     cb_loc,
                                     "resumes a render pass instance, but there is no suspended render pass instance.");
                }
        ...
        if (suspended_render_pass_instance) {
            skip |= LogError("VUID-VkSubmitInfo2-commandBuffer-06010",
                             queue,
                             submit_loc,
                             "has a suspended render pass instance that was not resumed.");
        }
...
bool CoreChecks::PreCallValidateQueueSubmit2KHR(VkQueue queue,
                                                uint32_t submitCount,
...
bool CoreChecks::PreCallValidateQueueSubmit2(VkQueue queue,
                                             uint32_t submitCount,
                                             const VkSubmitInfo2* pSubmits,
                                             VkFence fence,
                                             const ErrorObject& error_obj) const {
...
void CoreChecks::PostCallRecordQueueSubmit(
    VkQueue queue, uint32_t submitCount, const VkSubmitInfo* pSubmits, VkFence fence, const RecordObject& record_obj) {
...
void CoreChecks::RecordQueueSubmit2(
...
void CoreChecks::PostCallRecordQueueSubmit2KHR(
    VkQueue queue, uint32_t submitCount, const VkSubmitInfo2* pSubmits, VkFence fence, const RecordObject& record_obj) {
    PostCallRecordQueueSubmit2(queue, submitCount, pSubmits, fence, record_obj);
}

void CoreChecks::PostCallRecordQueueSubmit2(
    VkQueue queue, uint32_t submitCount, const VkSubmitInfo2* pSubmits, VkFence fence, const RecordObject& record_obj) {
...
bool CoreChecks::ValidImageBufferQueue(const vvl::CommandBuffer& cb_state,
                                       const VulkanTypedHandle& object,
...
bool CoreChecks::ValidateQueueFamilyIndices(const Location& loc,
                                            const vvl::CommandBuffer& cb_state,
    ...
    if (pool->queueFamilyIndex != queue_state.queue_family_index) {
        ...
        skip |= LogError(vuid,
                         ...
                         FormatHandle(queue_state.Handle()).c_str(),
                         queue_state.queue_family_index);
    }

    // Ensure that any bound images or buffers created with SHARING_MODE_CONCURRENT have access to the current queue
    // family
        ...
        switch (state_object->Type()) {
            case kVulkanObjectTypeImage: {
                ...
                if (image_state && image_state->create_info.sharingMode == VK_SHARING_MODE_CONCURRENT) {
                    skip |= ValidImageBufferQueue(cb_state,
                                                  image_state->Handle(),
                                                  queue_state.queue_family_index,
                                                  image_state->create_info.queueFamilyIndexCount,
                                                  image_state->create_info.pQueueFamilyIndices,
                                                  loc);
                }
                break;
            }
            case kVulkanObjectTypeBuffer: {
                ...
                if (buffer_state && buffer_state->create_info.sharingMode == VK_SHARING_MODE_CONCURRENT) {
                    skip |= ValidImageBufferQueue(cb_state,
                                                  ...
                                                  buffer_state->create_info.pQueueFamilyIndices,
                                                  loc);
                }
                break;
            }
...
bool CoreChecks::ValidateCommandBufferState(const vvl::CommandBuffer& cb_state,
                                            const Location& loc,
                                            uint32_t current_submit_count,
    ...
    if (const uint64_t submissions = cb_state.submitCount + current_submit_count;
        ...
        skip |= LogError("UNASSIGNED-DrawState-CommandBufferSingleSubmitViolation",
                         cb_state.Handle(),
                         loc,
                         "%s recorded with VK_COMMAND_BUFFER_USAGE_ONE_TIME_SUBMIT_BIT has been submitted %" PRIu64
                         " times.",
                         FormatHandle(cb_state).c_str(),
                         submissions);
    }
    ...
    switch (cb_state.state) {
        ...
        case CbState::New:
            skip |= LogError(vuid,
                             cb_state.Handle(),
                             loc,
                             "%s is unrecorded and contains no commands.",
                             FormatHandle(cb_state).c_str());
            break;
        ...
        case CbState::Recording:
            skip |= LogError(vuid,
                             cb_state.Handle(),
                             loc,
                             "You must call vkEndCommandBuffer() on %s before this call.",
                             FormatHandle(cb_state).c_str());
            break;
...
bool CoreChecks::ValidateCommandBufferSimultaneousUse(const Location& loc,
                                                      const vvl::CommandBuffer& cb_state,
...
bool CoreChecks::ValidatePrimaryCommandBufferState(
    ...
    if (cb_state.IsSecondary()) {
        ...
        skip |= LogError(vuid,
                         cb_state.Handle(),
                         loc,
                         "Command buffer %s must be allocated with VK_COMMAND_BUFFER_LEVEL_PRIMARY.",
                         FormatHandle(cb_state).c_str());
    } else {
        for (const auto* sub_cb : cb_state.linkedCommandBuffers) {
            ...
            if (sub_cb->state != CbState::Recorded) {
                ...
                const LogObjectList objlist(device, cb_state.Handle(), sub_cb->Handle(), sub_cb->primaryCommandBuffer);
                skip |= LogError(finished_cb_vuid,
                                 objlist,
                                 loc,
...
bool CoreChecks::PreCallValidateQueueBindSparse(VkQueue queue,
    ...
    if (!(queue_flags & VK_QUEUE_SPARSE_BINDING_BIT)) {
        skip |= LogError("VUID-vkQueueBindSparse-queuetype",
                         queue,
                         error_obj.location,
                         "queueFamilyIndex %" PRIu32 " queueFlags are %s.",
                         queue_state->queue_family_index,
    ...
    for (uint32_t bind_idx = 0; bind_idx < bindInfoCount; ++bind_idx) {
            ...
            for (uint32_t buffer_idx = 0; buffer_idx < bind_info.bufferBindCount; ++buffer_idx) {
                ...
                if (buffer_bind.pBinds) {
                    ...
                    for (uint32_t buffer_bind_idx = 0; buffer_bind_idx < buffer_bind.bindCount; ++buffer_bind_idx) {
                        ...
                        skip |= ValidateBufferSparseMemoryBindAlignments(
                            memory_bind, *buffer_state, bind_loc, buffer_bind_info_loc);
                        skip |= ValidateSparseMemoryBind(memory_bind,
                                                         buffer_state->requirements,
                                                         buffer_state->requirements.size,
                                                         buffer_state->external_memory_handle_types,
                                                         buffer_state->Handle(),
                                                         bind_loc);
                    }
                }
        ...
        if (bind_info.pImageOpaqueBinds) {
            for (uint32_t image_opaque_idx = 0; image_opaque_idx < bind_info.imageOpaqueBindCount; ++image_opaque_idx) {
                const VkSparseImageOpaqueMemoryBindInfo& image_opaque_bind =
                    bind_info.pImageOpaqueBinds[image_opaque_idx];
                if (image_opaque_bind.pBinds) {
                    ...
                    for (uint32_t image_opaque_bind_idx = 0; image_opaque_bind_idx < image_opaque_bind.bindCount;
                        ...
                        const Location image_bind_info_loc =
                            bind_info_loc.dot(Field::pImageOpaqueBinds, image_opaque_idx);
                        ...
                        // confirmation
                        skip |= ValidateImageSparseMemoryBindAlignments(
                            memory_bind, *image_state, bind_loc, image_bind_info_loc);
                        skip |= ValidateSparseMemoryBind(memory_bind,
                                                         image_state->requirements[0],
                                                         image_state->requirements[0].size,
                                                         image_state->external_memory_handle_types,
                                                         image_state->Handle(),
                                                         bind_loc);
                    }
                }
        ...
        if (bind_info.pImageBinds) {
            for (uint32_t image_idx = 0; image_idx < bind_info.imageBindCount; ++image_idx) {
                ...
                if (!image_state->sparse_residency) {
                    skip |= LogError("VUID-VkSparseImageMemoryBindInfo-image-02901",
                                     image_bind.image,
                                     bind_loc.dot(Field::image),
                                     "was created with flags %s.",
