...
struct CommandBufferSubmitState {
    ...
    bool Validate(const Location &loc, const vvl::CommandBuffer &cb_state, uint32_t perf_pass) {
        ...
        for (auto &barrier : cb_state.tensor_barriers) {
            ...
            if (!tensor_state_ptr) continue;
            auto &tensor_state = *tensor_state_ptr;
            if (VK_SHARING_MODE_EXCLUSIVE == tensor_state.SharingMode()) {
                if (barrier.srcQueueFamilyIndex != queue_state.queue_family_index &&
                    barrier.srcQueueFamilyIndex != VK_QUEUE_FAMILY_IGNORED) {
                    if (barrier.dstQueueFamilyIndex != queue_state.queue_family_index &&
