...
VkShaderStageFlags
FindNextStage(uint32_t createInfoCount, const VkShaderCreateInfoEXT* pCreateInfos, VkShaderStageFlagBits stage) {
    constexpr uint32_t graphicsStagesCount = 5;
    constexpr uint32_t meshStagesCount = 3;
    const VkShaderStageFlagBits graphicsStages[graphicsStagesCount] = { VK_SHADER_STAGE_VERTEX_BIT,
                                                                        VK_SHADER_STAGE_TESSELLATION_CONTROL_BIT,
                                                                        VK_SHADER_STAGE_TESSELLATION_EVALUATION_BIT,
                                                                        VK_SHADER_STAGE_GEOMETRY_BIT,
                                                                        VK_SHADER_STAGE_FRAGMENT_BIT };
    const VkShaderStageFlagBits meshStages[meshStagesCount] = { VK_SHADER_STAGE_TASK_BIT_EXT,
...
bool CoreChecks::ValidateCreateShadersLinking(uint32_t createInfoCount,
    ...
    for (uint32_t i = 0; i < createInfoCount; ++i) {
        ...
        if (create_info.stage == VK_SHADER_STAGE_TESSELLATION_CONTROL_BIT ||
            ...
            if (enabled_features.tessellationShader == VK_FALSE) {
                skip |= LogError("VUID-VkShaderCreateInfoEXT-stage-08419",
                                 device,
                                 create_info_loc.dot(Field::stage),
            ...
            if (enabled_features.geometryShader == VK_FALSE) {
                skip |= LogError("VUID-VkShaderCreateInfoEXT-stage-08420",
                                 device,
                                 create_info_loc.dot(Field::stage),
            ...
            if (enabled_features.taskShader == VK_FALSE) {
                skip |= LogError("VUID-VkShaderCreateInfoEXT-stage-08421",
                                 device,
                                 create_info_loc.dot(Field::stage),
            ...
            if (enabled_features.meshShader == VK_FALSE) {
                skip |= LogError("VUID-VkShaderCreateInfoEXT-stage-08422",
                                 device,
                                 create_info_loc.dot(Field::stage),
        ...
        if ((create_info.flags & VK_SHADER_CREATE_FRAGMENT_SHADING_RATE_ATTACHMENT_BIT_EXT) != 0 &&
            ...
            skip |= LogError("VUID-VkShaderCreateInfoEXT-flags-08487",
                             device,
        ...
        if ((create_info.flags & VK_SHADER_CREATE_FRAGMENT_DENSITY_MAP_ATTACHMENT_BIT_EXT) != 0 &&
            ...
            skip |= LogError("VUID-VkShaderCreateInfoEXT-flags-08489",
                             device,
                             create_info_loc.dot(Field::flags),
        ...
        if ((create_info.flags & VK_SHADER_CREATE_LINK_STAGE_BIT_EXT) != 0) {
            ...
            if (nextStage != 0 && create_info.nextStage != nextStage) {
                skip |=
                    LogError("VUID-vkCreateShadersEXT-pCreateInfos-08409",
                             device,
                             create_info_loc.dot(Field::flags),
            ...
            for (uint32_t j = i; j < createInfoCount; ++j) {
                if (i != j && create_info.stage == pCreateInfos[j].stage) {
                    skip |= LogError("VUID-vkCreateShadersEXT-pCreateInfos-08410",
                                     device,
                                     create_info_loc,
        ...
        if (enabled_features.tessellationShader == VK_FALSE &&
            ...
            skip |= LogError("VUID-VkShaderCreateInfoEXT-nextStage-08428",
                             device,
                             create_info_loc.dot(Field::nextStage),
        ...
        if (enabled_features.geometryShader == VK_FALSE && create_info.nextStage == VK_SHADER_STAGE_GEOMETRY_BIT) {
            skip |= LogError("VUID-VkShaderCreateInfoEXT-nextStage-08429",
                             device,
                             create_info_loc.dot(Field::nextStage),
        ...
        if (create_info.stage == VK_SHADER_STAGE_TESSELLATION_CONTROL_BIT &&
            ...
            skip |= LogError("VUID-VkShaderCreateInfoEXT-nextStage-08430",
                             device,
                             create_info_loc.dot(Field::stage),
        ...
        if (create_info.stage == VK_SHADER_STAGE_TESSELLATION_EVALUATION_BIT &&
            ...
            skip |= LogError("VUID-VkShaderCreateInfoEXT-nextStage-08431",
                             device,
                             create_info_loc.dot(Field::stage),
                             "is VK_SHADER_STAGE_TESSELLATION_EVALUATION_BIT, but nextStage is %s.",
                             string_VkShaderStageFlags(create_info.nextStage).c_str());
        }
        if (create_info.stage == VK_SHADER_STAGE_GEOMETRY_BIT &&
            (create_info.nextStage & ~VK_SHADER_STAGE_FRAGMENT_BIT) > 0) {
            skip |= LogError("VUID-VkShaderCreateInfoEXT-nextStage-08433",
                             device,
                             create_info_loc.dot(Field::stage),
        ...
        if ((create_info.stage == VK_SHADER_STAGE_FRAGMENT_BIT || create_info.stage == VK_SHADER_STAGE_COMPUTE_BIT) &&
            ...
            skip |= LogError("VUID-VkShaderCreateInfoEXT-nextStage-08434",
                             device,
                             create_info_loc.dot(Field::stage),
                             "is %s, but nextStage is %s.",
                             string_VkShaderStageFlagBits(create_info.stage),
                             string_VkShaderStageFlags(create_info.nextStage).c_str());
        }
        if (create_info.stage == VK_SHADER_STAGE_TASK_BIT_EXT &&
            (create_info.nextStage & ~VK_SHADER_STAGE_MESH_BIT_EXT) > 0) {
            skip |= LogError("VUID-VkShaderCreateInfoEXT-nextStage-08435",
                             device,
                             create_info_loc.dot(Field::stage),
        ...
        if (create_info.stage == VK_SHADER_STAGE_MESH_BIT_EXT &&
            (create_info.nextStage & ~VK_SHADER_STAGE_FRAGMENT_BIT) > 0) {
            skip |= LogError("VUID-VkShaderCreateInfoEXT-nextStage-08436",
                             device,
                             create_info_loc.dot(Field::stage),
        ...
        if ((create_info.flags & VK_SHADER_CREATE_ALLOW_VARYING_SUBGROUP_SIZE_BIT_EXT) != 0 &&
            ...
            skip |= LogError("VUID-VkShaderCreateInfoEXT-flags-09404",
                             device,
                             create_info_loc.dot(Field::flags),
                             "contains VK_SHADER_CREATE_ALLOW_VARYING_SUBGROUP_SIZE_BIT_EXT, but subgroupSizeControl "
                             "feature is not enabled.");
        }
        if ((create_info.flags & VK_SHADER_CREATE_REQUIRE_FULL_SUBGROUPS_BIT_EXT) != 0 &&
            enabled_features.computeFullSubgroups == VK_FALSE) {
            skip |= LogError("VUID-VkShaderCreateInfoEXT-flags-09405",
                             device,
                             create_info_loc.dot(Field::flags),
                             "contains VK_SHADER_CREATE_REQUIRE_FULL_SUBGROUPS_BIT_EXT, but computeFullSubgroups "
                             "feature is not enabled.");
        }
        if ((create_info.flags & VK_SHADER_CREATE_INDIRECT_BINDABLE_BIT_EXT) != 0 &&
            enabled_features.deviceGeneratedCommands == VK_FALSE) {
            skip |= LogError(" VUID-VkShaderCreateInfoEXT-flags-11005",
                             device,
                             create_info_loc.dot(Field::flags),
                             "contains VK_SHADER_CREATE_INDIRECT_BINDABLE_BIT_EXT, but deviceGeneratedCommands feature "
                             "is not enabled.");
        }
    }

    if (linked_stage != invalid && non_linked_graphics_stage != invalid) {
        skip |= LogError("VUID-vkCreateShadersEXT-pCreateInfos-08402",
                         device,
    ...
    if (linked_stage != invalid && non_linked_task_mesh_stage != invalid) {
        skip |= LogError("VUID-vkCreateShadersEXT-pCreateInfos-08403",
                         device,
                         loc.dot(Field::pCreateInfos, linked_stage).dot(Field::flags),
                         "contains VK_SHADER_CREATE_LINK_STAGE_BIT_EXT, but pCreateInfos[%" PRIu32
                         "] stage is %s and does not have VK_SHADER_CREATE_LINK_STAGE_BIT_EXT.",
                         non_linked_task_mesh_stage,
    ...
    if (linked_vert_stage != invalid && linked_task_mesh_stage != invalid) {
        skip |= LogError("VUID-vkCreateShadersEXT-pCreateInfos-08404",
                         device,
                         loc.dot(Field::pCreateInfos, linked_vert_stage).dot(Field::stage),
                         "is %s and pCreateInfos[%" PRIu32
                         "].stage is %s, but both contain VK_SHADER_CREATE_LINK_STAGE_BIT_EXT.",
                         string_VkShaderStageFlagBits(pCreateInfos[linked_vert_stage].stage),
                         linked_task_mesh_stage,
    ...
    if (linked_task_stage != invalid && linked_mesh_no_task_stage != invalid) {
        skip |= LogError("VUID-vkCreateShadersEXT-pCreateInfos-08405",
                         device,
                         loc.dot(Field::pCreateInfos, linked_task_stage),
    ...
    if (linked_spirv_index != invalid && linked_binary_index != invalid) {
        skip |= LogError("VUID-vkCreateShadersEXT-pCreateInfos-08411",
                         device,
                         loc.dot(Field::pCreateInfos, linked_spirv_index),
...
bool CoreChecks::ValidateCreateShadersMesh(const VkShaderCreateInfoEXT& create_info,
                                           const spirv::Module& spirv,
                                           const Location& create_info_loc) const {
    bool skip = false;
    if (create_info.flags & VK_SHADER_CREATE_NO_TASK_SHADER_BIT_EXT)
        return skip;
    if (spirv.static_data_.has_builtin_draw_index) {
        skip |= LogError("VUID-vkCreateShadersEXT-pCreateInfos-09632",
                         device,
                         create_info_loc,
                         "the mesh Shader Object being created uses DrawIndex (gl_DrawID) which will be an undefined "
                         "value when reading.");
    }
    return skip;
}

bool CoreChecks::PreCallValidateCreateShadersEXT(VkDevice device,
    ...
    if (enabled_features.shaderObject == VK_FALSE) {
        skip |= LogError("VUID-vkCreateShadersEXT-None-08400",
                         device,
                         error_obj.location,
    ...
    for (uint32_t i = 0; i < createInfoCount; ++i) {
        ...
        spv_const_binary_t binary{ static_cast<const uint32_t*>(create_info.pCode),
                                   create_info.codeSize / sizeof(uint32_t) };
        skip |= RunSpirvValidation(binary, create_info_loc, cache);

        const auto spirv =
            std::make_shared<spirv::Module>(create_info.codeSize, static_cast<const uint32_t*>(create_info.pCode));
        ...
        if (stage_state.entrypoint && (create_info.stage == VK_SHADER_STAGE_TESSELLATION_CONTROL_BIT ||
            ...
            if (create_info.stage == VK_SHADER_STAGE_TESSELLATION_EVALUATION_BIT) {
                if (stage_state.entrypoint->execution_mode.tessellation_subdivision == 0) {
                    skip |=
                        LogError("VUID-VkShaderCreateInfoEXT-codeType-08872",
                                 device,
                                 create_info_loc.dot(Field::stage),
                ...
                if (stage_state.entrypoint->execution_mode.tessellation_orientation == 0) {
                    skip |=
                        LogError("VUID-VkShaderCreateInfoEXT-codeType-08873",
                                 device,
                                 create_info_loc.dot(Field::stage),
                ...
                if (stage_state.entrypoint->execution_mode.tessellation_spacing == 0) {
                    skip |= LogError("VUID-VkShaderCreateInfoEXT-codeType-08874",
                                     device,
                                     create_info_loc.dot(Field::stage),
            ...
            if (stage_state.entrypoint->execution_mode.output_vertices != vvl::kU32Max &&
                ...
                skip |= LogError("VUID-VkShaderCreateInfoEXT-pCode-08453",
                                 device,
                                 create_info_loc.dot(Field::pCode),
    ...
    if (tesc_linked_subdivision != 0 && tese_linked_subdivision != 0 &&
        tesc_linked_subdivision != tese_linked_subdivision) {
        skip |=
            LogError("VUID-vkCreateShadersEXT-pCreateInfos-08867",
                     device,
                     error_obj.location,
                     ...
                     string_SpvExecutionMode(tesc_linked_subdivision),
                     string_SpvExecutionMode(tese_linked_subdivision));
    }
    if (tesc_linked_orientation != 0 && tese_linked_orientation != 0 &&
        tesc_linked_orientation != tese_linked_orientation) {
        skip |=
            LogError("VUID-vkCreateShadersEXT-pCreateInfos-08868",
                     device,
                     error_obj.location,
                     "The orientation specified in tessellation control shader (%s) does not match the orientation in "
                     "tessellation evaluation shader (%s).",
                     string_SpvExecutionMode(tesc_linked_orientation),
                     string_SpvExecutionMode(tese_linked_orientation));
    }
    ...
    if (tesc_linked_spacing != 0 && tese_linked_spacing != 0 && tesc_linked_spacing != tese_linked_spacing) {
        skip |= LogError("VUID-vkCreateShadersEXT-pCreateInfos-08870",
                         device,
                         error_obj.location,
                         "The spacing specified in tessellation control shader (%s) does not match the spacing in "
                         "tessellation evaluation shader (%s).",
                         string_SpvExecutionMode(tesc_linked_spacing),
                         string_SpvExecutionMode(tese_linked_spacing));
    }
    ...
    if ((physical_device_state->supported_queues & queue_flag) == 0) {
        ...
        skip |= LogError(vuid,
                         device,
                         error_obj.location,
                         "device only supports (%s) but require %s.",
                         string_VkQueueFlags(physical_device_state->supported_queues).c_str(),
                         string_VkQueueFlags(queue_flag).c_str());
    }

    return skip;
}

bool CoreChecks::PreCallValidateDestroyShaderEXT(VkDevice device,
                                                 VkShaderEXT shader,
                                                 const VkAllocationCallbacks* pAllocator,
                                                 const ErrorObject& error_obj) const {
    bool skip = false;
    ...
    if (enabled_features.shaderObject == VK_FALSE) {
        skip |= LogError("VUID-vkDestroyShaderEXT-None-08481",
                         device,
                         error_obj.location,
                         "the shaderObject feature was not enabled.");
    }
    ...
    if (const auto shader_state = Get<vvl::ShaderObject>(shader)) {
        skip |= ValidateObjectNotInUse(
            shader_state.get(), error_obj.location.dot(Field::shader), "VUID-vkDestroyShaderEXT-shader-08482");
    }

    return skip;
}

bool CoreChecks::PreCallValidateCmdBindShadersEXT(VkCommandBuffer commandBuffer,
                                                  uint32_t stageCount,
    ...
    if (enabled_features.shaderObject == VK_FALSE) {
        skip |= LogError("VUID-vkCmdBindShadersEXT-None-08462",
                         commandBuffer,
                         error_obj.location,
                         "the shaderObject feature was not enabled.");
    }
    ...
    for (uint32_t i = 0; i < stageCount; ++i) {
        ...
        for (uint32_t j = i; j < stageCount; ++j) {
            if (i != j && stage == pStages[j]) {
                skip |= LogError("VUID-vkCmdBindShadersEXT-pStages-08463",
                                 commandBuffer,
                                 stage_loc,
                                 "and pStages[%" PRIu32 "] are both %s.",
                                 j,
        ...
        if (stage == VK_SHADER_STAGE_VERTEX_BIT && shader != VK_NULL_HANDLE) {
            ...
            skip |= LogError("VUID-vkCmdBindShadersEXT-pShaders-08474",
                             objlist,
                             stage_loc,
                             "is %s and pShaders[%" PRIu32
                             "] is not VK_NULL_HANDLE, but tessellationShader feature was not enabled.",
                             string_VkShaderStageFlagBits(stage),
                             i);
        } else if (enabled_features.geometryShader == VK_FALSE && stage == VK_SHADER_STAGE_GEOMETRY_BIT &&
                   shader != VK_NULL_HANDLE) {
            const LogObjectList objlist(commandBuffer, shader);
            skip |= LogError("VUID-vkCmdBindShadersEXT-pShaders-08475",
                             objlist,
                             stage_loc,
        ...
        if ((stage & (VK_SHADER_STAGE_VERTEX_BIT | VK_SHADER_STAGE_TESSELLATION_CONTROL_BIT |
            ...
            if ((cb_state->command_pool->queue_flags & VK_QUEUE_GRAPHICS_BIT) == 0) {
                const LogObjectList objlist(commandBuffer, cb_state->command_pool->Handle());
                skip |= LogError("VUID-vkCmdBindShadersEXT-pShaders-08477",
                                 objlist,
                                 stage_loc,
        ...
        if ((stage & (VK_SHADER_STAGE_MESH_BIT_EXT | VK_SHADER_STAGE_TASK_BIT_EXT)) > 0) {
            if ((cb_state->command_pool->queue_flags & VK_QUEUE_GRAPHICS_BIT) == 0) {
                const LogObjectList objlist(commandBuffer, cb_state->command_pool->Handle());
                skip |= LogError("VUID-vkCmdBindShadersEXT-pShaders-08478",
                                 objlist,
                                 stage_loc,
                                 "is %s, but the command pool the command buffer %s was allocated from "
                                 "does not support graphics operations (%s).",
        ...
        if (stage == VK_SHADER_STAGE_TASK_BIT_EXT && enabled_features.taskShader == VK_FALSE &&
            shader != VK_NULL_HANDLE) {
            const LogObjectList objlist(commandBuffer, shader);
            skip |= LogError("VUID-vkCmdBindShadersEXT-pShaders-08490",
                             objlist,
                             stage_loc,
                             "is %s and pShaders[%" PRIu32
                             "] is not VK_NULL_HANDLE, but taskShader feature was not enabled.",
                             string_VkShaderStageFlagBits(stage),
                             i);
        } else if (stage == VK_SHADER_STAGE_MESH_BIT_EXT && enabled_features.meshShader == VK_FALSE &&
                   shader != VK_NULL_HANDLE) {
            const LogObjectList objlist(commandBuffer, shader);
            skip |= LogError("VUID-vkCmdBindShadersEXT-pShaders-08491",
                             objlist,
                             stage_loc,
                             "is %s and pShaders[%" PRIu32
                             "] is not VK_NULL_HANDLE, but meshShader feature was not enabled.",
                             string_VkShaderStageFlagBits(stage),
                             i);
        }
        if (stage == VK_SHADER_STAGE_ALL_GRAPHICS || stage == VK_SHADER_STAGE_ALL) {
            skip |= LogError("VUID-vkCmdBindShadersEXT-pStages-08464",
                             commandBuffer,
                             stage_loc,
                             "is %s.",
                             string_VkShaderStageFlagBits(stage));
        ...
        if (stage & kShaderStageAllRayTracing) {
            skip |= LogError("VUID-vkCmdBindShadersEXT-pStages-08465",
                             commandBuffer,
                             stage_loc,
                             "is %s.",
                             string_VkShaderStageFlagBits(stage));
        ...
        if (stage == VK_SHADER_STAGE_SUBPASS_SHADING_BIT_HUAWEI) {
            skip |= LogError("VUID-vkCmdBindShadersEXT-pStages-08467",
                             commandBuffer,
                             stage_loc,
                             "is %s.",
                             string_VkShaderStageFlagBits(stage));
        ...
        if (stage == VK_SHADER_STAGE_CLUSTER_CULLING_BIT_HUAWEI) {
            skip |= LogError("VUID-vkCmdBindShadersEXT-pStages-08468",
                             commandBuffer,
                             stage_loc,
                             "is %s.",
                             string_VkShaderStageFlagBits(stage));
        ...
        if (shader != VK_NULL_HANDLE) {
            ...
            if (shader_state && shader_state->create_info.stage != stage) {
                const LogObjectList objlist(commandBuffer, shader);
                skip |= LogError("VUID-vkCmdBindShadersEXT-pShaders-08469",
                                 objlist,
                                 stage_loc,
    ...
    if (vertexStageIndex != stageCount && taskStageIndex != stageCount) {
        skip |= LogError("VUID-vkCmdBindShadersEXT-pShaders-08470",
                         commandBuffer,
                         error_obj.location,
    ...
    if (vertexStageIndex != stageCount && meshStageIndex != stageCount) {
        skip |= LogError("VUID-vkCmdBindShadersEXT-pShaders-08471",
                         commandBuffer,
                         error_obj.location,
                         "pStages[%" PRIu32 "] is VK_SHADER_STAGE_VERTEX_BIT and pStages[%" PRIu32
...
bool CoreChecks::PreCallValidateGetShaderBinaryDataEXT(
    VkDevice device, VkShaderEXT shader, size_t* pDataSize, void* pData, const ErrorObject& error_obj) const {
    bool skip = false;

    if (enabled_features.shaderObject == VK_FALSE) {
        skip |= LogError("VUID-vkGetShaderBinaryDataEXT-None-08461",
                         shader,
                         error_obj.location,
                         "the shaderObject feature was not enabled.");
    }
...
bool CoreChecks::ValidateShaderObjectBoundShader(const LastBound& last_bound_state,
    ...
    if (!last_bound_state.ValidShaderObjectCombination(bind_point, enabled_features)) {
        skip |= LogError(vuid.pipeline_or_shaders_bound_08607,
                         cb_state.Handle(),
                         vuid.loc(),
    ...
    if (bind_point == VK_PIPELINE_BIND_POINT_GRAPHICS) {
        ...
        if (enabled_features.tessellationShader &&
            ...
            skip |= LogError(vuid.tessellation_control_shader_08685,
                             cb_state.Handle(),
                             vuid.loc(),
                             "There is no graphics pipeline bound and vkCmdBindShadersEXT() was not called with stage "
        ...
        if (enabled_features.tessellationShader &&
            ...
            skip |= LogError(vuid.tessellation_evaluation_shader_08686,
                             cb_state.Handle(),
                             vuid.loc(),
                             "There is no graphics pipeline bound and vkCmdBindShadersEXT() was not called with stage "
        ...
        if (enabled_features.geometryShader &&
            !last_bound_state.IsValidShaderOrNullBound(ShaderObjectStage::GEOMETRY)) {
            skip |= LogError(vuid.geometry_shader_08687,
                             cb_state.Handle(),
                             vuid.loc(),
                             "There is no graphics pipeline bound and vkCmdBindShadersEXT() was not called with stage "
                             "VK_SHADER_STAGE_GEOMETRY_BIT and either VK_NULL_HANDLE or a valid "
                             "VK_SHADER_STAGE_GEOMETRY_BIT shader.");
        }
        if (!last_bound_state.IsValidShaderOrNullBound(ShaderObjectStage::FRAGMENT)) {
            skip |= LogError(vuid.fragment_shader_08688,
                             cb_state.Handle(),
                             vuid.loc(),
                             "There is no graphics pipeline bound and vkCmdBindShadersEXT() was not called with stage "
                             "VK_SHADER_STAGE_FRAGMENT_BIT and either VK_NULL_HANDLE or a valid "
...
bool CoreChecks::ValidateDrawShaderObject(const LastBound& last_bound_state, const vvl::DrawDispatchVuid& vuid) const {
    ...
    if (cb_state.activeRenderPass && !cb_state.activeRenderPass->UsesDynamicRendering()) {
        skip |= LogError(vuid.render_pass_began_08876,
                         cb_state.Handle(),
                         vuid.loc(),
...
bool CoreChecks::ValidateDrawShaderObjectLinking(const LastBound& last_bound_state,
                                                 const vvl::DrawDispatchVuid& vuid) const {
    bool skip = false;
    const vvl::CommandBuffer& cb_state = last_bound_state.cb_state;
    ...
    const std::array graphics_stages = { VK_SHADER_STAGE_VERTEX_BIT,
                                         VK_SHADER_STAGE_TESSELLATION_CONTROL_BIT,
                                         VK_SHADER_STAGE_TESSELLATION_EVALUATION_BIT,
                                         VK_SHADER_STAGE_GEOMETRY_BIT,
                                         VK_SHADER_STAGE_FRAGMENT_BIT };
    ...
    for (const auto stage : graphics_stages) {
        ...
        if (next_stage != VK_SHADER_STAGE_ALL && consumer->create_info.stage != next_stage) {
            skip |=
                LogError(vuid.linked_shaders_08699,
                         cb_state.Handle(),
                         loc,
                         ...
                         string_VkShaderStageFlagBits(prev_stage),
                         string_VkShaderStageFlagBits(next_stage),
                         string_VkShaderStageFlagBits(stage),
                         string_VkShaderStageFlagBits(stage));
            break;
        }
        ...
        if (producer && consumer->spirv && producer->spirv && consumer->entrypoint && producer->entrypoint) {
            skip |= ValidateInterfaceBetweenStages(
                *producer->spirv, *producer->entrypoint, *consumer->spirv, *consumer->entrypoint, loc);
        }
        producer = consumer;
    }
...
bool CoreChecks::ValidateDrawShaderObjectPushConstantAndLayout(const LastBound& last_bound_state,
    ...
    for (const auto shader_state : last_bound_state.shader_object_states) {
        ...
        if (push_constant_different) {
            skip |= LogError(vuid.shaders_push_constants_08878,
                             cb_state.Handle(),
                             vuid.loc(),
        ...
        bool descriptor_layouts_different =
            first->create_info.setLayoutCount != shader_state->create_info.setLayoutCount;
        ...
        if (descriptor_layouts_different) {
            skip |= LogError(vuid.shaders_descriptor_layouts_08879,
                             cb_state.Handle(),
                             vuid.loc(),
...
bool CoreChecks::ValidateDrawShaderObjectMesh(const LastBound& last_bound_state,
                                              const vvl::DrawDispatchVuid& vuid) const {
    bool skip = false;
    const vvl::CommandBuffer& cb_state = last_bound_state.cb_state;

    const bool has_vertex_shader = last_bound_state.GetShader(ShaderObjectStage::VERTEX);
    const bool has_task_shader = last_bound_state.GetShader(ShaderObjectStage::TASK);
    const bool has_mesh_shader = last_bound_state.GetShader(ShaderObjectStage::MESH);
    ...
    if (has_task_shader || has_mesh_shader) {
        if (!IsValueIn(vuid.function,
            ...
            std::stringstream ss;
            if (has_task_shader && has_mesh_shader) {
                ss << "Task and mesh shaders are bound.";
            } else if (has_mesh_shader) {
                ss << "Task shader is bound.";
            } else {
                ss << "Mesh shader is bound.";
            }
            skip |=
                LogError(vuid.draw_shaders_no_task_mesh_08885, cb_state.Handle(), vuid.loc(), "%s", ss.str().c_str());
        }
    }

    if (enabled_features.taskShader || enabled_features.meshShader) {
        if (has_vertex_shader && has_mesh_shader) {
            skip |= LogError(vuid.vert_mesh_shader_08693,
                             cb_state.Handle(),
                             vuid.loc(),
                             "Both vertex shader and mesh shader are bound");
    ...
    if (enabled_features.taskShader && enabled_features.meshShader) {
        const auto shader_object = last_bound_state.GetShaderState(ShaderObjectStage::MESH);
        if (has_mesh_shader && (shader_object->create_info.flags & VK_SHADER_CREATE_NO_TASK_SHADER_BIT_EXT) == 0 &&
            !has_task_shader) {
            skip |= LogError(vuid.task_mesh_shader_08694,
                             cb_state.Handle(),
                             vuid.loc(),
                             "Mesh shader %s was created without VK_SHADER_CREATE_NO_TASK_SHADER_BIT_EXT, but no task "
                             "shader is bound.",
                             debug_report->FormatHandle(last_bound_state.GetShader(ShaderObjectStage::MESH)).c_str());
        } else if (has_mesh_shader &&
                   (shader_object->create_info.flags & VK_SHADER_CREATE_NO_TASK_SHADER_BIT_EXT) != 0 &&
                   has_task_shader) {
            skip |= LogError(
                vuid.task_mesh_shader_08695,
                cb_state.Handle(),
                vuid.loc(),
    ...
    if (has_vertex_shader && (has_task_shader || has_mesh_shader)) {
        ...
        skip |= LogError(vuid.vert_task_mesh_shader_08696,
                         cb_state.Handle(),
                         vuid.loc(),
                         "Vertex shader %s is bound, but %s.",
                         debug_report->FormatHandle(last_bound_state.GetShader(ShaderObjectStage::MESH)).c_str(),
                         msg.str().c_str());
    }
    return skip;
