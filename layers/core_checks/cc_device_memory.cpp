...
bool CoreChecks::PreCallValidateCmdDecompressMemoryEXT(VkCommandBuffer commandBuffer,
    ...
    for (uint32_t i = 0; i < pDecompressMemoryInfoEXT->regionCount; ++i) {
        ...
        const Location region_loc = error_obj.location.dot(Field::pDecompressMemoryInfoEXT).dot(Field::pRegions, i);
        if (region.compressedSize > 0) {
            ...
            BufferAddressValidation<2> buffer_address_validator = {
                {{{
                  ...
                  },
                  {
                      ...
                      "UNASSIGNED-VkDecompressMemoryRegionEXT-srcAddress-memory-decompression-usage",
                      [](const vvl::Buffer& buffer_state) {
            ...
            skip |= buffer_address_validator.ValidateDeviceAddress(*this, region_loc.dot(Field::srcAddress), objlist, start);
        }
        ...
        if (region.decompressedSize > 0) {
            ...
            BufferAddressValidation<2> dst_range_validator = {
                {{{
                  ...
                  {
                      "UNASSIGNED-VkDecompressMemoryRegionEXT-dstAddress-memory-decompression-usage",
                      [](const vvl::Buffer& buffer_state) {
                          return (buffer_state.usage & VK_BUFFER_USAGE_2_MEMORY_DECOMPRESSION_BIT_EXT) == 0;
                      },
                      []() { return "The following buffers are missing VK_BUFFER_USAGE_2_MEMORY_DECOMPRESSION_BIT_EXT"; },
            ...
            skip |= dst_range_validator.ValidateDeviceAddress(*this, region_loc.dot(Field::dstAddress), objlist, start);
        }
    }
...
bool CoreChecks::PreCallValidateCmdDecompressMemoryIndirectCountEXT(VkCommandBuffer commandBuffer,
...

    {
        BufferAddressValidation<3> buffer_address_validator = {
            {{{"VUID-vkCmdDecompressMemoryIndirectCountEXT-indirectCommandsAddress-07694",
              ...
              {"UNASSIGNED-vkCmdDecompressMemoryIndirectCountEXT-indirectCommandsAddress-range-same-buffer",
        ...
        skip |= buffer_address_validator.ValidateDeviceAddress(*this, error_obj.location.dot(Field::indirectCommandsAddress),
                                                               objlist, indirectCommandsAddress);
    }

    {
        ...
        skip |= buffer_address_validator.ValidateDeviceAddress(*this, error_obj.location.dot(Field::indirectCommandsCountAddress),
                                                               objlist, indirectCommandsCountAddress);
    }
