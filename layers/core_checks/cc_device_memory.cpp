...
bool CoreChecks::VerifyBoundMemoryIsValid(const vvl::DeviceMemory* mem_state,
    ...
    if (!mem_state) {
        ...
        skip |= LogError(vuid,
                         objlist,
                         loc,
                         "(%s) is used with no memory bound. Memory should be bound by calling vkBind%sMemory().",
...
bool CoreChecks::VerifyBoundMemoryIsDeviceVisible(const vvl::DeviceMemory* mem_state,
                                                  const LogObjectList& objlist,
                                                  const VulkanTypedHandle& typed_handle,
                                                  const Location& loc,
                                                  const char* vuid) const {
    ...
    if (mem_state) {
        if ((phys_dev_mem_props.memoryTypes[mem_state->allocate_info.memoryTypeIndex].propertyFlags &
            ...
            result |= LogError(vuid,
                               objlist,
                               loc,
                               "(%s) used with memory that is not device visible.",
                               FormatHandle(typed_handle).c_str());
        }
    }
    return result;
...
bool CoreChecks::ValidateMemoryIsBoundToImage(const LogObjectList& objlist,
    ...
    if (image_state.create_from_swapchain != VK_NULL_HANDLE) {
        if (!image_state.bind_swapchain) {
            result |= LogError(vuid,
                               objlist,
                               loc,
                               "(%s) is created by %s, and the image should be bound by calling vkBindImageMemory2(), "
                               "and the pNext chain "
            ...
            result |= LogError(vuid,
                               objlist,
                               loc,
                               "(%s) is created by %s, but the image is bound by %s. The image should be created and "
                               "bound by the same "
                               "swapchain",
                               FormatHandle(image_state).c_str(),
                               FormatHandle(image_state.create_from_swapchain).c_str(),
        ...
        if (memory_states.empty()) {
            result |= LogError(vuid,
                               objlist,
                               loc,
                               "%s used with no memory bound. Memory should be bound by calling vkBindImageMemory().",
                               FormatHandle(image_state).c_str());
        } else {
...
bool CoreChecks::ValidateAccelStructsMemoryDoNotOverlap(const Location& function_loc,
    ...
    const sparse_container::range<VkDeviceSize> range_a(accel_struct_a.create_info.offset,
                                                        accel_struct_a.create_info.size);
    const sparse_container::range<VkDeviceSize> range_b(accel_struct_b.create_info.offset,
                                                        accel_struct_b.create_info.size);
    ...
    if (const auto [memory, overlap_range] = buffer_a.GetResourceMemoryOverlap(range_a, &buffer_b, range_b);
        ...
        skip |= LogError(
            ...
            FormatHandle(buffer_a).c_str(),
            loc_a.Fields().c_str(),
            FormatHandle(accel_struct_a.Handle()).c_str(),
            FormatHandle(buffer_b).c_str(),
            loc_b.Fields().c_str(),
            FormatHandle(accel_struct_b.Handle()).c_str(),
            FormatHandle(memory).c_str(),
            string_range_hex(overlap_range).c_str());
    }

    return skip;
}

static bool ValidateBufferAndAccelStructsMemoryDoNotOverlap(const CoreChecks& validator,
                                                            const vvl::Buffer& buffer_a,
    ...
    if (const auto [memory, overlap_range] = buffer_a.GetResourceMemoryOverlap(range_a, &buffer_b, range_b);
        ...
        if (err_msg) {
            ...
            err_msg_strm << "memory backing buffer (" << validator.FormatHandle(buffer_a)
                         << ") overlaps memory backing buffer (" << validator.FormatHandle(buffer_b)
                         << ") used as storage for " << loc_b.Fields() << " ("
...
bool CoreChecks::ValidateScratchMemoryNoOverlap(const Location& function_loc,
                                                LogObjectList objlist,
    ...
    const char* scratch_dst_accel_struct_overlap =
    ...
    BufferAddressValidation<3> buffer_address_validator = { { {
        { scratch_dst_accel_struct_overlap,
          [this, scratch_a_address, scratch_a_size, &dst_accel_struct, &loc_dst_accel_struct](
              vvl::Buffer* const scratch_a, std::string* out_error_msg) {
              const VkDeviceSize scratch_a_offset = scratch_a_address - scratch_a->deviceAddress;
              const sparse_container::range<VkDeviceSize> scratch_a_range(scratch_a_offset,
                                                                          scratch_a_offset + scratch_a_size);
          ...
          [scratch_a_size, &loc_dst_accel_struct]() {
              return std::string("The following scratch buffers associated to this device address (assumed scratch "
                                 "byte size: ") +
    ...
    if (src_accel_struct) {
        buffer_address_validator.AddVuidValidation(
            { scratch_src_accel_struct_overlap,
              [this, scratch_a_address, scratch_a_size, src_accel_struct, &loc_src_accel_struct](
                  vvl::Buffer* const scratch_a, std::string* out_error_msg) {
                  const VkDeviceSize scratch_a_offset = scratch_a_address - scratch_a->deviceAddress;
                  const sparse_container::range<VkDeviceSize> scratch_a_range(scratch_a_offset,
                                                                              scratch_a_offset + scratch_a_size);
              ...
              [scratch_a_size, &loc_src_accel_struct]() {
                  return std::string("The following scratch buffers associated to this device address (assumed scratch "
                                     "byte size: ") +
                         std::to_string(scratch_a_size) + ") overlap with memory backing " +
                         loc_src_accel_struct.Fields();
              } });
    }
    ...
    if (!scratches_b.empty()) {
        ...
        buffer_address_validator.AddVuidValidation(
            { scratch_scratch_overlap,
              [this, scratch_a_address, scratch_a_size, scratches_b, scratch_b_address, scratch_b_size](
                  vvl::Buffer* const scratch_a, std::string* out_error_msg) {
                  const VkDeviceSize scratch_a_offset = scratch_a_address - scratch_a->deviceAddress;
                  const sparse_container::range<VkDeviceSize> scratch_a_range(scratch_a_offset,
                                                                              scratch_a_offset + scratch_a_size);
                  ...
                  for (auto scratch_b : scratches_b) {
                      ...
                      if (auto [mem, mem_range] =
                              scratch_b->GetResourceMemoryOverlap(scratch_b_range, scratch_a, scratch_a_range);
                          mem != VK_NULL_HANDLE) {
                          if (out_error_msg) {
                              *out_error_msg += "Memory (" + FormatHandle(mem) + ") overlap on memory range " +
                                                string_range_hex(mem_range);
                          }
                          return false;
                      }
                  }
              ...
              [loc_scratches_b, scratch_a_size, scratch_b_size, &scratch_b_address_str]() {
                  return std::string("The following scratch buffers associated to this device address (assumed scratch "
                                     "byte size: " +
                                     std::to_string(scratch_a_size) +
                                     ") overlap with at least one scratch buffer associated to ") +
    ...
    skip |= buffer_address_validator.LogErrorsIfNoValidBuffer(
        *this, scratches_a, loc_scratches_a, objlist, scratch_a_address);
    ...
    if (!result) {
        ...
        if (mem_state) {
            if ((phys_dev_mem_props.memoryTypes[mem_state->allocate_info.memoryTypeIndex].propertyFlags &
                ...
                const LogObjectList objlist(
                    accel_struct.Handle(), accel_struct.buffer_state->Handle(), mem_state->Handle());
                result |= LogError(vuid,
                                   objlist,
                                   buffer_loc,
                                   "has been created with a buffer whose bound memory is not host visible.");
            }
        }
    }
    return result;
...
bool CoreChecks::ValidateAccelStructBufferMemoryIsNotMultiInstance(const vvl::AccelerationStructureKHR& accel_struct,
    ...
    if (const vvl::DeviceMemory* buffer_mem = accel_struct.buffer_state->MemState()) {
        if (buffer_mem->multi_instance) {
            const LogObjectList objlist(
                accel_struct.Handle(), accel_struct.buffer_state->Handle(), buffer_mem->Handle());
            skip |= LogError(
                vuid,
                objlist,
                accel_struct_loc,
...
bool CoreChecks::ValidateSetMemBinding(const vvl::DeviceMemory& memory_state,
    ...
    if (prev_binding || mem_binding.indeterminate_state) {
        ...
        if (mem_binding.indeterminate_state) {
            ...
            skip |= LogError(
                ...
                objlist,
                loc,
                "attempting to bind %s to %s which is in an indeterminate (possibly bound) state. A previous call to "
                "%s failed and "
            ...
            skip |= LogError(vuid,
                             objlist,
                             loc,
                             "attempting to bind %s to %s which has already been bound to %s.",
                             FormatHandle(memory_state.Handle()).c_str(),
                             FormatHandle(typed_handle).c_str(),
...
bool CoreChecks::HasExternalMemoryImportSupport(const vvl::Buffer& buffer,
    ...
    return (properties.externalMemoryProperties.externalMemoryFeatures & VK_EXTERNAL_MEMORY_FEATURE_IMPORTABLE_BIT) !=
           0;
}

bool CoreChecks::HasExternalMemoryImportSupport(const vvl::Image& image,
                                                VkExternalMemoryHandleTypeFlagBits handle_type) const {
    ...
    if (image.create_info.tiling != VK_IMAGE_TILING_DRM_FORMAT_MODIFIER_EXT) {
        ...
        if (DispatchGetImageDrmFormatModifierPropertiesEXT(device, image.VkHandle(), &drm_format_properties) !=
            VK_SUCCESS) {
            return false;
        }
...
bool CoreChecks::PreCallValidateAllocateMemory(VkDevice device,
    ...
    if (Count<vvl::DeviceMemory>() >= phys_dev_props.limits.maxMemoryAllocationCount) {
        skip |= LogError("VUID-vkAllocateMemory-maxMemoryAllocationCount-04101",
                         device,
                         error_obj.location,
                         "vkAllocateMemory: Number of currently valid memory objects is not less than "
                         "maxMemoryAllocationCount (%" PRIu32 ").",
    ...
    if (IsExtEnabled(device_extensions.vk_android_external_memory_android_hardware_buffer)) {
        ...
        if (!IgnoreAllocationSize(*pAllocateInfo) && 0 == pAllocateInfo->allocationSize) {
            skip |= LogError("VUID-VkMemoryAllocateInfo-allocationSize-07899",
                             device,
    ...
    if (pAllocateInfo->memoryTypeIndex >= phys_dev_mem_props.memoryTypeCount) {
        ...
        if (!IgnoreAllocationSize(*pAllocateInfo) &&
            ...
            skip |= LogError("VUID-vkAllocateMemory-pAllocateInfo-01713",
                             device,
                             allocate_info_loc.dot(Field::allocationSize),
        ...
        if (!enabled_features.deviceCoherentMemory &&
            ...
            skip |= LogError("VUID-vkAllocateMemory-deviceCoherentMemory-02790",
                             device,
                             allocate_info_loc.dot(Field::memoryTypeIndex),
                             "%" PRIu32 " includes the VK_MEMORY_PROPERTY_DEVICE_COHERENT_BIT_AMD memory property, but "
                             "the deviceCoherentMemory feature "
                             "is not enabled.",
                             pAllocateInfo->memoryTypeIndex);
        }
    ...
    //  "memory is not an imported Android Hardware Buffer" refers to VkImportAndroidHardwareBufferInfoANDROID with a
    //  non-NULL buffer value. Memory imported has another VUID to check size and allocationSize match up
    if (auto imported_ahb_info =
            vku::FindStructInPNextChain<VkImportAndroidHardwareBufferInfoANDROID>(pAllocateInfo->pNext);
    ...
    if (dedicated_allocate_info) {
        ...
        if ((dedicated_buffer != VK_NULL_HANDLE) && (dedicated_image != VK_NULL_HANDLE)) {
            skip |= LogError("VUID-VkMemoryDedicatedAllocateInfo-image-01432",
                             device,
                             allocate_info_loc,
            ...
            if (image_state->disjoint == true) {
                skip |= LogError("VUID-VkMemoryDedicatedAllocateInfo-image-01797",
                                 objlist,
                                 image_loc,
            ...
            if (auto buffer_state = Get<vvl::Buffer>(dedicated_buffer)) {
                if (!IgnoreAllocationSize(*pAllocateInfo) &&
                    ...
                    !imported_qnx_buffer) {
                    skip |=
                        LogError("VUID-VkMemoryDedicatedAllocateInfo-buffer-02965",
                                 objlist,
                                 allocate_info_loc.dot(Field::allocationSize),
                                 "(%" PRIu64 ") needs to be equal to %s (%s) VkMemoryRequirements::size (%" PRIu64 ").",
                                 pAllocateInfo->allocationSize,
                                 buffer_loc.Fields().c_str(),
                                 FormatHandle(dedicated_buffer).c_str(),
                                 buffer_state->requirements.size);
                }
                if ((buffer_state->create_info.flags & VK_BUFFER_CREATE_SPARSE_BINDING_BIT) != 0) {
                    skip |= LogError("VUID-VkMemoryDedicatedAllocateInfo-buffer-01436",
                                     objlist,
                                     buffer_loc,
    ...
    if (imported_opaque_fd) {
        ...
        if (const auto payload_info = GetOpaqueInfoFromFdHandle(import_memory_fd_info->fd)) {
            if (pAllocateInfo->allocationSize != payload_info->allocation_size) {
                skip |= LogError("VUID-VkMemoryAllocateInfo-allocationSize-01742",
                                 device,
                                 allocate_info_loc.dot(Field::allocationSize),
                                 "allocationSize (%" PRIu64 ") does not match %s (%d) allocationSize (%" PRIu64 ").",
                                 pAllocateInfo->allocationSize,
                                 import_loc.Fields().c_str(),
                                 import_memory_fd_info->fd,
            ...
            if (dedicated_image != VK_NULL_HANDLE) {
                if (payload_info->dedicated_image == VK_NULL_HANDLE) {
                    skip |= LogError("VUID-VkMemoryDedicatedAllocateInfo-image-01878",
                                     dedicated_image,
            ...
            if (dedicated_buffer != VK_NULL_HANDLE) {
                if (payload_info->dedicated_buffer == VK_NULL_HANDLE) {
                    skip |= LogError("VUID-VkMemoryDedicatedAllocateInfo-buffer-01879",
                                     dedicated_buffer,
        ...
        // There is no reasonable way to query all variations of Image/Buffer creation to see what is supported, but if
        // the import has dedicated Image/Buffer, we can at least validate that it has import support
        ...
        if (dedicated_image != VK_NULL_HANDLE) {
            ...
            if (dedicated_image_state &&
                ...
                skip |= LogError("VUID-VkImportMemoryFdInfoKHR-handleType-00667",
                                 dedicated_image,
                                 allocate_info_loc.pNext(Struct::VkMemoryDedicatedAllocateInfo, Field::image),
        ...
        if (dedicated_buffer != VK_NULL_HANDLE) {
            auto dedicated_buffer_state = Get<vvl::Buffer>(dedicated_buffer);
            if (dedicated_buffer_state && !HasExternalMemoryImportSupport(
                                              *dedicated_buffer_state, VK_EXTERNAL_MEMORY_HANDLE_TYPE_OPAQUE_FD_BIT)) {
                skip |= LogError("VUID-VkImportMemoryFdInfoKHR-handleType-00667",
                                 dedicated_buffer,
                                 allocate_info_loc.pNext(Struct::VkMemoryDedicatedAllocateInfo, Field::buffer),
...
#ifdef VK_USE_PLATFORM_WIN32_KHR
    if (const auto import_memory_win32_info =
            vku::FindStructInPNextChain<VkImportMemoryWin32HandleInfoKHR>(pAllocateInfo->pNext)) {
        if (const auto payload_info = GetOpaqueInfoFromWin32Handle(import_memory_win32_info->handle)) {
            if (IsValueIn(import_memory_win32_info->handleType,
                ...
                const Location import_loc =
                    allocate_info_loc.pNext(Struct::VkImportMemoryWin32HandleInfoKHR, Field::handle);
                static_assert(sizeof(HANDLE) == sizeof(uintptr_t)); // to use PRIxPTR for HANDLE formatting
                if (pAllocateInfo->allocationSize != payload_info->allocation_size) {
                    skip |= LogError("VUID-VkMemoryAllocateInfo-allocationSize-01743",
                                     device,
                                     allocate_info_loc.dot(Field::allocationSize),
            ...
            if (dedicated_image != VK_NULL_HANDLE) {
                if (IsValueIn(import_memory_win32_info->handleType,
                              { VK_EXTERNAL_MEMORY_HANDLE_TYPE_OPAQUE_WIN32_BIT,
                                ...
                                VK_EXTERNAL_MEMORY_HANDLE_TYPE_D3D12_RESOURCE_BIT })) {
                    if (payload_info->dedicated_image == VK_NULL_HANDLE) {
                        skip |= LogError("VUID-VkMemoryDedicatedAllocateInfo-image-01876",
                                         dedicated_image,
                                         allocate_info_loc.pNext(Struct::VkMemoryDedicatedAllocateInfo, Field::image),
                        ...
                        if (!dedicated_image_state || !payload_image_state ||
                            ...
                            const LogObjectList objlist(payload_info->dedicated_image, dedicated_image);
                            skip |=
                                LogError("VUID-VkMemoryDedicatedAllocateInfo-image-01876",
                                         objlist,
                                         allocate_info_loc.pNext(Struct::VkMemoryDedicatedAllocateInfo, Field::image),
                    ...
                    if (payload_info->dedicated_buffer == VK_NULL_HANDLE) {
                        ...
                        if (!dedicated_buffer_state || !payload_buffer_state ||
                            ...
                            skip |= LogError(
                                ...
                                objlist,
                                allocate_info_loc.pNext(Struct::VkMemoryDedicatedAllocateInfo, Field::buffer),
                                "is %s but %s (0x%" PRIxPTR
                                ") was created with a dedicated buffer %s with handleType %s.",
                                FormatHandle(dedicated_buffer).c_str(),
                                import_loc.Fields().c_str(),
                                reinterpret_cast<std::uintptr_t>(import_memory_win32_info->handle),
                                FormatHandle(payload_info->dedicated_buffer).c_str(),
            ...
            if ((import_memory_win32_info->handleType &
                ...
                DispatchGetMemoryWin32HandlePropertiesKHR(
                    device, import_memory_win32_info->handleType, import_memory_win32_info->handle, &handle_properties);
                if (((1 << pAllocateInfo->memoryTypeIndex) & handle_properties.memoryTypeBits) == 0) {
                    skip |= LogError("VUID-VkMemoryAllocateInfo-memoryTypeIndex-00645",
                                     device,
                                     allocate_info_loc.dot(Field::memoryTypeIndex),
...
void CoreChecks::PostCallRecordAllocateMemory(VkDevice device,
                                              const VkMemoryAllocateInfo* pAllocateInfo,
                                              const VkAllocationCallbacks* pAllocator,
                                              VkDeviceMemory* pMemory,
...
bool CoreChecks::PreCallValidateFreeMemory(VkDevice device,
...
bool CoreChecks::ValidateInsertMemoryRange(const VulkanTypedHandle& typed_handle,
    ...
    if (!IgnoreAllocationSize(mem_info.allocate_info) && memoryOffset >= mem_info.allocate_info.allocationSize) {
        ...
        if (typed_handle.type == kVulkanObjectTypeBuffer) {
            vuid = bind_2 ? "VUID-VkBindBufferMemoryInfo-memoryOffset-01031"
                          : "VUID-vkBindBufferMemory-memoryOffset-01031";
        } else if (typed_handle.type == kVulkanObjectTypeImage) {
            vuid =
                bind_2 ? "VUID-VkBindImageMemoryInfo-memoryOffset-01046" : "VUID-vkBindImageMemory-memoryOffset-01046";
...
bool CoreChecks::ValidateInsertImageMemoryRange(VkImage image,
                                                const vvl::DeviceMemory& mem_info,
...
bool CoreChecks::ValidateInsertBufferMemoryRange(VkBuffer buffer,
                                                 const vvl::DeviceMemory& mem_info,
                                                 VkDeviceSize mem_offset,
                                                 const Location& loc) const {
...
bool CoreChecks::ValidateMemoryTypes(const vvl::DeviceMemory& mem_info,
    ...
    if (((1 << mem_info.allocate_info.memoryTypeIndex) & memory_type_bits) == 0) {
        skip |= LogError(vuid,
                         ...
                         memory_type_bits,
                         FormatHandle(mem_info.Handle()).c_str(),
                         mem_info.allocate_info.memoryTypeIndex);
    }
    return skip;
}

bool CoreChecks::ValidateBindBufferMemory(
    VkBuffer buffer, VkDeviceMemory memory, VkDeviceSize memoryOffset, const void* pNext, const Location& loc) const {
    bool skip = false;

    // Validate device group information
    if (const auto* bind_buffer_memory_device_group_info =
            vku::FindStructInPNextChain<VkBindBufferMemoryDeviceGroupInfo>(pNext)) {
        if (bind_buffer_memory_device_group_info->deviceIndexCount != 0 &&
            ...
            skip |= LogError("VUID-VkBindBufferMemoryDeviceGroupInfo-deviceIndexCount-01606",
                             objlist,
                             loc.pNext(Struct::VkBindBufferMemoryDeviceGroupInfo, Field::deviceIndexCount),
                             "(%" PRIu32
                             ") is not the same as the number of physical devices in the logical device (%" PRIu32 ").",
            ...
            for (uint32_t i = 0; i < bind_buffer_memory_device_group_info->deviceIndexCount; ++i) {
                if (bind_buffer_memory_device_group_info->pDeviceIndices[i] >=
                    device_group_create_info.physicalDeviceCount) {
                    const LogObjectList objlist(buffer, memory);
                    skip |= LogError(
    ...
    if (SafeModulo(memoryOffset, buffer_state->requirements.alignment) != 0) {
        const char* vuid = bind_buffer_mem_2 ? "VUID-VkBindBufferMemoryInfo-memoryOffset-01036"
                                             : "VUID-vkBindBufferMemory-memoryOffset-01036";
        const LogObjectList objlist(buffer, memory);
        skip |= LogError(vuid,
                         objlist,
                         loc.dot(Field::memoryOffset),
                         "is %" PRIu64
                         " but must be an integer multiple of the VkMemoryRequirements::alignment value %" PRIu64
                         ", returned from a call to vkGetBufferMemoryRequirements with buffer.",
                         memoryOffset,
                         buffer_state->requirements.alignment);
    }
    ...
    if (auto mem_info = Get<vvl::DeviceMemory>(memory)) {
        ...
        if (mem_info->IsExport()) {
            ...
            IterateFlags<VkExternalMemoryHandleTypeFlagBits>(mem_info->export_handle_types,
                                                             validate_export_handle_types);
            ...
            if (export_supported &&
                (mem_info->export_handle_types & compatible_types) != mem_info->export_handle_types) {
                const LogObjectList objlist(buffer, memory);
                skip |= LogError(
                    "VUID-VkExportMemoryAllocateInfo-handleTypes-00656",
                    objlist,
        ...
        skip |= ValidateMemoryTypes(
            *mem_info, buffer_state->requirements.memoryTypeBits, loc.dot(Field::buffer), mem_type_vuid);
        ...
        auto chained_flags_struct =
            vku::FindStructInPNextChain<VkMemoryAllocateFlagsInfo>(mem_info->allocate_info.pNext);
        ...
        if (buffer_state->create_info.flags & VK_BUFFER_CREATE_DESCRIPTOR_BUFFER_CAPTURE_REPLAY_BIT_EXT) {
            ...
            if (!(memory_allocate_flags & VK_MEMORY_ALLOCATE_DEVICE_ADDRESS_CAPTURE_REPLAY_BIT)) {
                const char* vuid = bind_buffer_mem_2 ? "VUID-VkBindBufferMemoryInfo-buffer-09201"
            ...
            if (enabled_features.descriptorBufferCaptureReplay) {
                if (!(memory_allocate_flags & VK_MEMORY_ALLOCATE_DEVICE_ADDRESS_CAPTURE_REPLAY_BIT)) {
                    const char* vuid = bind_buffer_mem_2
        ...
        if (mem_info->IsImportAHB()) {
            skip |=
                ValidateBufferImportedHandleANDROID(buffer_state->external_memory_handle_types, memory, buffer, loc);
        } else if (mem_info->IsImport()) {
            if ((mem_info->import_handle_type.value() & buffer_state->external_memory_handle_types) == 0) {
                const char* vuid = bind_buffer_mem_2 ? "VUID-VkBindBufferMemoryInfo-memory-02985"
            ...
            if (!HasExternalMemoryImportSupport(*buffer_state, mem_info->import_handle_type.value())) {
                const LogObjectList objlist(buffer, memory);
                skip |= LogError("VUID-VkImportMemoryWin32HandleInfoKHR-handleType-00658",
                                 objlist,
                                 loc.dot(Field::memory),
                                 "(%s) was imported from handleType %s but VkExternalBufferProperties does not report "
                                 "it as importable.",
                                 FormatHandle(memory).c_str(),
                                 string_VkExternalMemoryHandleTypeFlagBits(mem_info->import_handle_type.value()));
            }
...
bool CoreChecks::PreCallValidateBindBufferMemory(VkDevice device,
                                                 VkBuffer buffer,
                                                 VkDeviceMemory memory,
                                                 VkDeviceSize memoryOffset,
                                                 const ErrorObject& error_obj) const {
...
bool CoreChecks::PreCallValidateBindBufferMemory2(VkDevice device,
    ...
    for (uint32_t i = 0; i < bindInfoCount; i++) {
        ...
        skip |= ValidateBindBufferMemory(
            pBindInfos[i].buffer, pBindInfos[i].memory, pBindInfos[i].memoryOffset, pBindInfos[i].pNext, loc);
    }
    return skip;
}

bool CoreChecks::PreCallValidateBindBufferMemory2KHR(VkDevice device,
                                                     uint32_t bindInfoCount,
                                                     const VkBindBufferMemoryInfo* pBindInfos,
                                                     const ErrorObject& error_obj) const {
...
bool CoreChecks::PreCallValidateGetImageMemoryRequirements(VkDevice device,
                                                           VkImage image,
    ...
    if (auto image_state = Get<vvl::Image>(image)) {
        ...
        if (image_state->disjoint == true) {
            skip |= LogError("VUID-vkGetImageMemoryRequirements-image-01588",
                             image,
                             image_loc,
...
bool CoreChecks::PreCallValidateGetImageMemoryRequirements2(VkDevice device,
    ...
    if (!image_plane_info && image_state->disjoint) {
        if (vkuFormatIsMultiplane(image_format)) {
            skip |= LogError("VUID-VkImageMemoryRequirementsInfo2-image-01589",
                             pInfo->image,
                             image_loc,
        ...
        if (image_state->create_info.tiling == VK_IMAGE_TILING_DRM_FORMAT_MODIFIER_EXT) {
            skip |= LogError("VUID-VkImageMemoryRequirementsInfo2-image-02279",
                             pInfo->image,
                             image_loc,
        ...
        if ((image_state->disjoint == false)) {
            skip |= LogError("VUID-VkImageMemoryRequirementsInfo2-image-01590",
                             pInfo->image,
                             image_loc,
        ...
        if ((vkuFormatIsMultiplane(image_format) == false) &&
            (image_tiling != VK_IMAGE_TILING_DRM_FORMAT_MODIFIER_EXT)) {
            skip |= LogError("VUID-VkImageMemoryRequirementsInfo2-image-02280",
                             pInfo->image,
                             image_loc,
        ...
        if ((image_tiling == VK_IMAGE_TILING_LINEAR) || (image_tiling == VK_IMAGE_TILING_OPTIMAL)) {
            ...
            if (vkuFormatIsMultiplane(image_format) && !IsOnlyOneValidPlaneAspect(image_format, aspect)) {
                skip |= LogError("VUID-VkImagePlaneMemoryRequirementsInfo-planeAspect-02281",
                                 pInfo->image,
                                 info_loc.pNext(Struct::VkImagePlaneMemoryRequirementsInfo, Field::planeAspect),
                                 "%s but is invalid for %s.",
            ...
            if (GetBitSetCount(aspect) > 1 || !IsValueIn(VkImageAspectFlagBits(aspect),
                                                         { VK_IMAGE_ASPECT_MEMORY_PLANE_0_BIT_EXT,
                                                           VK_IMAGE_ASPECT_MEMORY_PLANE_1_BIT_EXT,
                                                           VK_IMAGE_ASPECT_MEMORY_PLANE_2_BIT_EXT,
                                                           VK_IMAGE_ASPECT_MEMORY_PLANE_3_BIT_EXT })) {
                skip |= LogError("VUID-VkImagePlaneMemoryRequirementsInfo-planeAspect-02282",
                                 pInfo->image,
                                 info_loc.pNext(Struct::VkImagePlaneMemoryRequirementsInfo, Field::planeAspect),
                                 "%s but is invalid for %s.",
                                 string_VkImageAspectFlags(aspect).c_str(),
...
bool CoreChecks::PreCallValidateGetImageMemoryRequirements2KHR(VkDevice device,
                                                               const VkImageMemoryRequirementsInfo2* pInfo,
                                                               VkMemoryRequirements2* pMemoryRequirements,
                                                               const ErrorObject& error_obj) const {
...
bool CoreChecks::ValidateMapMemory(const vvl::DeviceMemory& mem_info,
                                   VkDeviceSize offset,
                                   VkDeviceSize size,
    ...
    if ((propertyFlags & VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT) == 0) {
        skip |= LogError(map2 ? "VUID-VkMemoryMapInfo-memory-07962" : "VUID-vkMapMemory-memory-00682",
                         memory,
                         loc,
                         "Mapping memory without VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT set. "
                         "Memory has type %" PRIu32 " which has properties %s.",
                         memoryTypeIndex,
                         string_VkMemoryPropertyFlags(propertyFlags).c_str());
    }

    if (mem_info.multi_instance) {
        skip |= LogError(map2 ? "VUID-VkMemoryMapInfo-memory-07963" : "VUID-vkMapMemory-memory-00683",
                         instance,
                         loc,
    ...
    if (size == 0) {
        skip |= LogError(
            map2 ? "VUID-VkMemoryMapInfo-size-07960" : "VUID-vkMapMemory-size-00680", memory, size_loc, "is zero.");
    }

    // It is an application error to call VkMapMemory on an object that is already mapped
    if (mem_info.mapped_range.size != 0) {
        skip |= LogError(map2 ? "VUID-VkMemoryMapInfo-memory-07958" : "VUID-vkMapMemory-memory-00678",
                         memory,
                         loc,
    ...
    if (offset >= allocationSize) {
        skip |= LogError(map2 ? "VUID-VkMemoryMapInfo-offset-07959" : "VUID-vkMapMemory-offset-00679",
                         memory,
                         offset_loc,
                         "0x%" PRIx64 " is larger than the total array size 0x%" PRIx64,
                         offset,
                         allocationSize);
    }
    ...
    if (size != VK_WHOLE_SIZE) {
        if ((offset + size) > allocationSize) {
            skip |= LogError(map2 ? "VUID-VkMemoryMapInfo-size-07961" : "VUID-vkMapMemory-size-00681",
                             memory,
                             offset_loc,
                             "0x%" PRIx64 " plus size 0x%" PRIx64 " (total 0x%" PRIx64
                             ") oversteps total array size 0x%" PRIx64 ".",
                             offset,
                             size,
                             size + offset,
                             allocationSize);
...
bool CoreChecks::PreCallValidateMapMemory(VkDevice device,
                                          VkDeviceMemory memory,
                                          VkDeviceSize offset,
                                          VkDeviceSize size,
                                          VkFlags flags,
    ...
    bool skip = false;
    if (auto mem_info = Get<vvl::DeviceMemory>(memory)) {
        skip |= ValidateMapMemory(
            *mem_info.get(), offset, size, error_obj.location.dot(Field::offset), error_obj.location.dot(Field::size));

        if (flags & VK_MEMORY_MAP_PLACED_BIT_EXT) {
            skip |= LogError("VUID-vkMapMemory-flags-09568",
                             memory,
...
bool CoreChecks::PreCallValidateMapMemory2(VkDevice device,
    ...
    skip |= ValidateMapMemory(*mem_info.get(),
                              pMemoryMapInfo->offset,
                              pMemoryMapInfo->size,
                              info_loc.dot(Field::offset),
                              info_loc.dot(Field::size));

    if (pMemoryMapInfo->flags & VK_MEMORY_MAP_PLACED_BIT_EXT) {
        if (!enabled_features.memoryMapPlaced) {
            skip |= LogError("VUID-VkMemoryMapInfo-flags-09569",
        ...
        if (enabled_features.memoryMapRangePlaced) {
            if (pMemoryMapInfo->offset % phys_dev_ext_props.map_memory_placed_props.minPlacedMemoryMapAlignment != 0) {
                skip |=
                    LogError("VUID-VkMemoryMapInfo-flags-09573",
                             pMemoryMapInfo->memory,
                             info_loc.dot(Field::offset),
                             "(0x%" PRIx64 ") is not an integer multiple of "
            ...
            if (pMemoryMapInfo->offset != 0) {
                skip |= LogError("VUID-VkMemoryMapInfo-flags-09571",
                                 pMemoryMapInfo->memory,
                                 info_loc.dot(Field::offset),
                                 "(0x%" PRIx64 ") is not zero while VK_MEMORY_MAP_PLACED_BIT_EXT is set and "
            ...
            if (pMemoryMapInfo->size != VK_WHOLE_SIZE &&
                ...
                skip |= LogError("VUID-VkMemoryMapInfo-flags-09572",
                                 pMemoryMapInfo->memory,
                                 info_loc.dot(Field::size),
        ...
        if (pMemoryMapInfo->size == VK_WHOLE_SIZE) {
            if (mem_info->allocate_info.allocationSize %
                    phys_dev_ext_props.map_memory_placed_props.minPlacedMemoryMapAlignment !=
                0) {
                skip |= LogError("VUID-VkMemoryMapInfo-flags-09651",
                                 pMemoryMapInfo->memory,
                                 info_loc.dot(Field::size),
            ...
            if (pMemoryMapInfo->size % phys_dev_ext_props.map_memory_placed_props.minPlacedMemoryMapAlignment != 0) {
                skip |=
                    LogError("VUID-VkMemoryMapInfo-flags-09574",
                             pMemoryMapInfo->memory,
                             info_loc.dot(Field::size),
                             "(0x%" PRIx64 ") is not VK_WHOLE_SIZE and is not an integer multiple of "
                             "minPlacedMemoryMapAlignment (0x%" PRIx64 ") while VK_MEMORY_MAP_PLACED_BIT_EXT is set",
                             pMemoryMapInfo->size,
                             phys_dev_ext_props.map_memory_placed_props.minPlacedMemoryMapAlignment);
            }
        ...
        if (mem_info->IsImport() &&
            (mem_info->import_handle_type.value() == VK_EXTERNAL_MEMORY_HANDLE_TYPE_HOST_ALLOCATION_BIT_EXT ||
             mem_info->import_handle_type.value() ==
                 VK_EXTERNAL_MEMORY_HANDLE_TYPE_HOST_MAPPED_FOREIGN_MEMORY_BIT_EXT)) {
            skip |=
                LogError("VUID-VkMemoryMapInfo-flags-09575",
                         pMemoryMapInfo->memory,
        ...
        if (placed_info == NULL) {
            ...
            skip |= LogError("VUID-VkMemoryMapInfo-flags-09570",
                             pMemoryMapInfo->memory,
                             addr_loc,
                       ...
                       phys_dev_ext_props.map_memory_placed_props.minPlacedMemoryMapAlignment !=
                   0) {
            skip |= LogError("VUID-VkMemoryMapPlacedInfoEXT-pPlacedAddress-09577",
                             pMemoryMapInfo->memory,
                             addr_loc,
...
bool CoreChecks::PreCallValidateMapMemory2KHR(VkDevice device,
...
bool CoreChecks::PreCallValidateUnmapMemory(VkDevice device,
                                            VkDeviceMemory memory,
                                            const ErrorObject& error_obj) const {
    bool skip = false;
    ...
    if (!mem_info->mapped_range.size) {
        skip |= LogError("VUID-vkUnmapMemory-memory-00689",
                         memory,
                         error_obj.location,
...
bool CoreChecks::PreCallValidateUnmapMemory2(VkDevice device,
    ...
    if (!mem_info->mapped_range.size) {
        ...
        skip |= LogError("VUID-VkMemoryUnmapInfo-memory-07964",
        ...
        if (pMemoryUnmapInfo->flags & VK_MEMORY_UNMAP_RESERVE_BIT_EXT) {
            if (!enabled_features.memoryUnmapReserve) {
                skip |= LogError("VUID-VkMemoryUnmapInfo-flags-09579",
                                 pMemoryUnmapInfo->memory,
                                 info_loc.dot(Field::flags),
            ...
            if (mem_info->IsImport() &&
                (mem_info->import_handle_type.value() == VK_EXTERNAL_MEMORY_HANDLE_TYPE_HOST_ALLOCATION_BIT_EXT ||
                 mem_info->import_handle_type.value() ==
                     VK_EXTERNAL_MEMORY_HANDLE_TYPE_HOST_MAPPED_FOREIGN_MEMORY_BIT_EXT)) {
                skip |= LogError("VUID-VkMemoryUnmapInfo-flags-09580",
                                 pMemoryUnmapInfo->memory,
                                 info_loc.dot(Field::flags),
...
bool CoreChecks::PreCallValidateUnmapMemory2KHR(VkDevice device,
                                                const VkMemoryUnmapInfoKHR* pMemoryUnmapInfo,
                                                const ErrorObject& error_obj) const {
    return PreCallValidateUnmapMemory2(device, pMemoryUnmapInfo, error_obj);
}

bool CoreChecks::ValidateMemoryIsMapped(uint32_t mem_range_count,
    ...
    for (uint32_t i = 0; i < mem_range_count; ++i) {
        ...
        if (mem_info->mapped_range.size == 0) {
            skip |= LogError("VUID-VkMappedMemoryRange-memory-00684",
                             mem_ranges[i].memory,
                             memory_range_loc,
        ...
        if (mem_ranges[i].size == VK_WHOLE_SIZE) {
            if (mem_info->mapped_range.offset > mem_ranges[i].offset) {
                skip |= LogError("VUID-VkMappedMemoryRange-size-00686",
                                 mem_ranges[i].memory,
                                 memory_range_loc.dot(Field::offset),
                                 "(%" PRIu64 ") is less than the mapped memory offset (%" PRIu64
                                 ") (and size is VK_WHOLE_SIZE).",
                                 mem_ranges[i].offset,
                                 mem_info->mapped_range.offset);
            }
        } else {
            if (mem_info->mapped_range.offset > mem_ranges[i].offset) {
                skip |= LogError("VUID-VkMappedMemoryRange-size-00685",
                                 mem_ranges[i].memory,
                                 memory_range_loc.dot(Field::offset),
                                 "(%" PRIu64 ") is less than the mapped memory offset (%" PRIu64
                                 ") (and size is not VK_WHOLE_SIZE).",
                                 mem_ranges[i].offset,
                                 mem_info->mapped_range.offset);
            }
...
bool CoreChecks::ValidateMappedMemoryRangeDeviceLimits(uint32_t mem_range_count,
                                                       const VkMappedMemoryRange* mem_ranges,
                                                       const ErrorObject& error_obj) const {
    bool skip = false;
    for (uint32_t i = 0; i < mem_range_count; ++i) {
        ...
        if (SafeModulo(offset, atom_size) != 0) {
            skip |= LogError("VUID-VkMappedMemoryRange-offset-00687",
                             mem_ranges->memory,
                             memory_range_loc.dot(Field::offset),
        ...
        if (size == VK_WHOLE_SIZE) {
            ...
            const auto mapping_end =
                ((mapping_size == VK_WHOLE_SIZE) ? allocation_size : mapping_offset + mapping_size);
            if (SafeModulo(mapping_end, atom_size) != 0 && mapping_end != allocation_size) {
                skip |= LogError("VUID-VkMappedMemoryRange-size-01389",
                                 mem_ranges->memory,
                                 memory_range_loc.dot(Field::size),
            ...
            if (range_end != allocation_size && SafeModulo(size, atom_size) != 0) {
                skip |=
                    LogError("VUID-VkMappedMemoryRange-size-01390",
                             mem_ranges->memory,
                             memory_range_loc.dot(Field::size),
                             "(%" PRIu64 ") is not a multiple of VkPhysicalDeviceLimits::nonCoherentAtomSize (%" PRIu64
...
bool CoreChecks::PreCallValidateFlushMappedMemoryRanges(VkDevice device,
...
bool CoreChecks::PreCallValidateInvalidateMappedMemoryRanges(VkDevice device,
                                                             uint32_t memoryRangeCount,
                                                             const VkMappedMemoryRange* pMemoryRanges,
                                                             const ErrorObject& error_obj) const {
    bool skip = false;
...
bool CoreChecks::PreCallValidateGetDeviceMemoryCommitment(VkDevice device,
                                                          VkDeviceMemory memory,
    ...
    if (auto mem_info = Get<vvl::DeviceMemory>(memory)) {
        if ((phys_dev_mem_props.memoryTypes[mem_info->allocate_info.memoryTypeIndex].propertyFlags &
            ...
            skip |= LogError("VUID-vkGetDeviceMemoryCommitment-memory-00690",
                             memory,
                             error_obj.location,
...
bool CoreChecks::ValidateBindImageMemory(uint32_t bindInfoCount,
    ...
    for (uint32_t i = 0; i < bindInfoCount; i++) {
        const Location loc =
            bind_image_mem_2 ? error_obj.location.dot(Field::pBindInfos, i) : error_obj.location.function;
        ...
        if (auto image_state = Get<vvl::Image>(bind_info.image)) {
            ...
            if (image_state->disjoint && plane_info == nullptr) {
                ...
                skip |= LogError("VUID-VkBindImageMemoryInfo-image-07736",
                                 objlist,
                                 loc.dot(Field::image),
            ...
            if ((plane_info == nullptr) && (image_state->disjoint == false)) {
                ...
                if (image_state->IsExternalBuffer() == false) {
                    ...
                    if (mem_info) {
                        ...
                        {
                            const char* vuid = bind_image_mem_2 ? "VUID-VkBindImageMemoryInfo-pNext-01615"
                ...
                if (bind_image_mem_2 == true) {
                    ...
                    if (it == resources_bound.end()) {
                        ...
                        const LogObjectList objlist(bind_info.image, bind_info.memory);
                        skip |= LogError("VUID-vkBindImageMemory2-pBindInfos-04006",
                                         objlist,
                                         loc.dot(Field::image),
                                         "is non-disjoint and is being bound twice at pBindInfos[%d]",
                                         it->second[0]);
                    }
                }
                ...
                // All validation using the image_state->plane*_requirements for external AHB is check in android only
                // section
                if (image_state->IsExternalBuffer() == false) {
                    ...
                    if (SafeModulo(bind_info.memoryOffset, disjoint_mem_req.alignment) != 0) {
                        ...
                        skip |= LogError(
                            ...
                            "is %" PRIu64
                            " but must be an integer multiple of the VkMemoryRequirements::alignment value %" PRIu64
                            ", returned from a call to vkGetImageMemoryRequirements2 with disjoint image for aspect "
                            "plane %s.",
                            bind_info.memoryOffset,
                            disjoint_mem_req.alignment,
                    ...
                    if (mem_info) {
                        ...
                        if (disjoint_mem_req.size > allocate_info.allocationSize - bind_info.memoryOffset) {
                            const LogObjectList objlist(bind_info.image, bind_info.memory);
                            skip |=
                                LogError("VUID-VkBindImageMemoryInfo-pNext-01621",
                                         objlist,
                                         loc,
                                         "allocationSize (%" PRIu64 ") minus memoryOffset (%" PRIu64 ") is %" PRIu64
                                         " but must be at least as large as VkMemoryRequirements::size value %" PRIu64
                                         ", returned from a call to vkGetImageMemoryRequirements with disjoint image "
                                         "for aspect plane %s.",
                                         allocate_info.allocationSize,
                                         bind_info.memoryOffset,
                                         allocate_info.allocationSize - bind_info.memoryOffset,
                                         disjoint_mem_req.size,
                                         string_VkImageAspectFlagBits(aspect));
                        }
                        ...
                        // Validate memory type used
                        {
                            skip |= ValidateMemoryTypes(*mem_info,
                                                        disjoint_mem_req.memoryTypeBits,
                                                        loc.dot(Field::image),
            ...
            if (mem_info) {
                // Validate bound memory range information
                // if memory is exported to an AHB then the mem_info->allocationSize must be zero and this check is not
                // needed
                if ((mem_info->IsExport() == false) ||
                    ((mem_info->export_handle_types &
                    ...
                    if (enabled_features.dedicatedAllocationImageAliasing) {
                        ...
                        if ((bind_info.memoryOffset != 0) || !current_image_state ||
                            ...
                            skip |= LogError(vuid,
                                             objlist,
                                             loc.dot(Field::memory),
                                             "(%s) is a dedicated memory allocation, but "
                                             "VkMemoryDedicatedAllocateInfo:: %s must compatible "
                        ...
                        if ((bind_info.memoryOffset != 0) || (dedicated_image != bind_info.image)) {
                            ...
                            skip |= LogError(vuid,
                                             ...
                                             loc.dot(Field::memory),
                                             "(%s) is a dedicated memory allocation, but "
                                             "VkMemoryDedicatedAllocateInfo:: %s must be equal "
                                             "to %s and memoryOffset %" PRIu64 " must be zero.",
                                             FormatHandle(bind_info.memory).c_str(),
                                             FormatHandle(dedicated_image).c_str(),
                                             FormatHandle(bind_info.image).c_str(),
                ...
                auto chained_flags_struct =
                    vku::FindStructInPNextChain<VkMemoryAllocateFlagsInfo>(mem_info->allocate_info.pNext);
                const VkMemoryAllocateFlags memory_allocate_flags =
                    chained_flags_struct ? chained_flags_struct->flags : 0;
                if ((image_state->create_info.flags & VK_IMAGE_CREATE_DESCRIPTOR_BUFFER_CAPTURE_REPLAY_BIT_EXT) &&
                    ...
                    const char* vuid = bind_image_mem_2
                ...
                if ((image_state->create_info.flags & VK_IMAGE_CREATE_DESCRIPTOR_BUFFER_CAPTURE_REPLAY_BIT_EXT) &&
                    !(memory_allocate_flags & VK_MEMORY_ALLOCATE_DEVICE_ADDRESS_BIT)) {
                    const char* vuid = bind_image_mem_2 ? "VUID-VkBindImageMemoryInfo-image-09202"
                ...
                if (mem_info->IsExport()) {
                    ...
                    auto validate_export_handle_types = [&](VkExternalMemoryHandleTypeFlagBits flag) {
                        ...
                        if (image_state->create_info.tiling == VK_IMAGE_TILING_DRM_FORMAT_MODIFIER_EXT) {
                            ...
                            auto result = DispatchGetImageDrmFormatModifierPropertiesEXT(
                                device, bind_info.image, &drm_modifier_properties);
                        ...
                        auto result = DispatchGetPhysicalDeviceImageFormatProperties2Helper(
                            physical_device, &image_info, &image_properties);
                        if (result != VK_SUCCESS) {
                            export_supported = false;
                            const LogObjectList objlist(bind_info.image, bind_info.memory);
                            skip |= LogError("VUID-VkExportMemoryAllocateInfo-handleTypes-00656",
                                             objlist,
                                             loc,
                                             "The handle type (%s) specified by the memory's "
                                             "VkExportMemoryAllocateInfo, and format (%s), "
                                             "type (%s), tiling (%s), usage (%s), flags (%s) specified by the image's "
                                             "VkImageCreateInfo is not "
                                             "supported combination of parameters. "
                                             "vkGetPhysicalDeviceImageFormatProperties2 returned back %s.",
                                             string_VkExternalMemoryHandleTypeFlagBits(flag),
                                             string_VkFormat(image_info.format),
                                             string_VkImageType(image_info.type),
                                             string_VkImageTiling(image_info.tiling),
                    ...
                    if ((mem_info->export_handle_types & image_state->external_memory_handle_types) == 0) {
                        const char* vuid = bind_image_mem_2 ? "VUID-VkBindImageMemoryInfo-memory-02728"
                ...
                if (mem_info->IsImportAHB() == true) {
                    skip |= ValidateImageImportedHandleANDROID(
                        image_state->external_memory_handle_types, bind_info.memory, bind_info.image, loc);
                } else if (mem_info->IsImport() == true) {
                    if ((mem_info->import_handle_type.value() & image_state->external_memory_handle_types) == 0) {
                        const char* vuid = bind_image_mem_2 ? "VUID-VkBindImageMemoryInfo-memory-02989"
                ...
                if ((image_state->unprotected == false) && (mem_info->unprotected == true)) {
                    const char* vuid = bind_image_mem_2 ? "VUID-VkBindImageMemoryInfo-None-01901"
                    ...
                    const char* vuid = bind_image_mem_2 ? "VUID-VkBindImageMemoryInfo-None-01902"
            ...
            if (swapchain_info) {
                if (bind_info.memory != VK_NULL_HANDLE) {
                    const LogObjectList objlist(bind_info.image, bind_info.memory);
                    skip |= LogError("VUID-VkBindImageMemoryInfo-pNext-01631",
                                     objlist,
                                     loc.dot(Field::memory),
                                     "(%s) is not VK_NULL_HANDLE.",
                                     FormatHandle(bind_info.memory).c_str());
                }
                if (image_state->create_from_swapchain != swapchain_info->swapchain) {
                    const LogObjectList objlist(
                        bind_info.image, image_state->create_from_swapchain, swapchain_info->swapchain);
                    // VU being worked on https://gitlab.khronos.org/vulkan/vulkan/-/merge_requests/5078
                    skip |= LogError("UNASSIGNED-CoreValidation-BindImageMemory-Swapchain",
                                     objlist,
                                     loc.dot(Field::image),
                                     "(%s) is created by %s, but the image is bound by %s. The image should be created "
                ...
                if (auto swapchain_state = Get<vvl::Swapchain>(swapchain_info->swapchain)) {
                    if (swapchain_state->images.size() <= swapchain_info->imageIndex) {
                        const LogObjectList objlist(bind_info.image, bind_info.memory);
                        skip |= LogError("VUID-VkBindImageMemorySwapchainInfoKHR-imageIndex-01644",
                                         objlist,
                    ...
                    if (IsExtEnabled(device_extensions.vk_ext_swapchain_maintenance1) &&
                        ...
                        if (swapchain_state->images[swapchain_info->imageIndex].acquired == false) {
                            const LogObjectList objlist(bind_info.image, bind_info.memory);
                            skip |=
                                LogError("VUID-VkBindImageMemorySwapchainInfoKHR-swapchain-07756",
                                         objlist,
                                         loc.pNext(Struct::VkBindImageMemorySwapchainInfoKHR, Field::swapchain),
                ...
                if (image_state->create_from_swapchain) {
                    const LogObjectList objlist(bind_info.image, bind_info.memory);
                    skip |= LogError("VUID-VkBindImageMemoryInfo-image-01630",
                                     objlist,
                                     loc,
                ...
                if (!mem_info) {
                    const LogObjectList objlist(bind_info.image, bind_info.memory);
                    skip |= LogError("VUID-VkBindImageMemoryInfo-pNext-01632",
                                     objlist,
                                     loc.dot(Field::memory),
                                     "(%s) is invalid.",
                                     FormatHandle(bind_info.memory).c_str());
            ...
            if (bind_image_memory_device_group_info &&
                bind_image_memory_device_group_info->splitInstanceBindRegionCount != 0) {
                if (!(image_state->create_info.flags & VK_IMAGE_CREATE_SPLIT_INSTANCE_BIND_REGIONS_BIT)) {
                    ...
                    skip |= LogError(
                        ...
                        "(%" PRId32 ") is not 0 and %s is not created with "
                ...
                if (device_group_create_info.physicalDeviceCount > 0) {
                    phy_dev_square =
                        device_group_create_info.physicalDeviceCount * device_group_create_info.physicalDeviceCount;
                }
                if (bind_image_memory_device_group_info->splitInstanceBindRegionCount != phy_dev_square) {
                    ...
                    skip |= LogError(
                        ...
                        objlist,
                        loc.pNext(Struct::VkBindImageMemoryDeviceGroupInfo, Field::splitInstanceBindRegionCount),
                        "(%" PRId32 ") is not 0 and different from the number of physical devices in the logical "
                        "device squared (%" PRIu32 ").",
                        bind_image_memory_device_group_info->splitInstanceBindRegionCount,
                        phy_dev_square);
                }
            }
            ...
            if (plane_info) {
                ...
                if (image_state->disjoint == false) {
                    const LogObjectList objlist(bind_info.image, bind_info.memory);
                    skip |= LogError("VUID-VkBindImageMemoryInfo-pNext-01618",
                                     objlist,
                                     loc.dot(Field::image),
                                     "(%s) is not created with VK_IMAGE_CREATE_DISJOINT_BIT, but pNext contains "
                ...
                if ((image_tiling == VK_IMAGE_TILING_LINEAR) || (image_tiling == VK_IMAGE_TILING_OPTIMAL)) {
                    if (vkuFormatIsMultiplane(image_format) && !IsOnlyOneValidPlaneAspect(image_format, aspect)) {
                        const LogObjectList objlist(bind_info.image, bind_info.memory);
                        skip |= LogError("VUID-VkBindImagePlaneMemoryInfo-planeAspect-02283",
                                         objlist,
                    ...
                    if (GetBitSetCount(aspect) > 1 || !IsValueIn(VkImageAspectFlagBits(aspect),
                                                                 { VK_IMAGE_ASPECT_MEMORY_PLANE_0_BIT_EXT,
                                                                   ...
                                                                   VK_IMAGE_ASPECT_MEMORY_PLANE_3_BIT_EXT })) {
                        const LogObjectList objlist(bind_info.image, bind_info.memory);
                        skip |= LogError("VUID-VkBindImagePlaneMemoryInfo-planeAspect-02284",
                                         objlist,
                                         loc.pNext(Struct::VkBindImagePlaneMemoryInfo, Field::planeAspect),
                                         "is %s but is invalid for %s.",
                                         string_VkImageAspectFlags(aspect).c_str(),
        ...
        const auto bind_image_memory_device_group =
            vku::FindStructInPNextChain<VkBindImageMemoryDeviceGroupInfo>(bind_info.pNext);
        if (bind_image_memory_device_group) {
            if (bind_image_memory_device_group->deviceIndexCount > 0 &&
                ...
                const LogObjectList objlist(bind_info.image, bind_info.memory);
                skip |= LogError("VUID-VkBindImageMemoryDeviceGroupInfo-deviceIndexCount-01633",
                                 objlist,
                                 loc,
            ...
            if (bind_image_memory_device_group->deviceIndexCount != 0 &&
                ...
                const LogObjectList objlist(bind_info.image, bind_info.memory);
                skip |=
                    LogError("VUID-VkBindImageMemoryDeviceGroupInfo-deviceIndexCount-01634",
                             objlist,
...
bool CoreChecks::PreCallValidateBindImageMemory(VkDevice device,
                                                VkImage image,
                                                VkDeviceMemory memory,
                                                VkDeviceSize memoryOffset,
                                                const ErrorObject& error_obj) const {
    ...
    if (auto image_state = Get<vvl::Image>(image)) {
        ...
        if (image_state->disjoint == true) {
            ...
            skip |= LogError("VUID-vkBindImageMemory-image-01608",
                             objlist,
...
void CoreChecks::PostCallRecordBindImageMemory(
...
bool CoreChecks::PreCallValidateBindImageMemory2(VkDevice device,
                                                 uint32_t bindInfoCount,
                                                 const VkBindImageMemoryInfo* pBindInfos,
                                                 const ErrorObject& error_obj) const {
...
void CoreChecks::PostCallRecordBindImageMemory2(VkDevice device,
                                                uint32_t bindInfoCount,
                                                const VkBindImageMemoryInfo* pBindInfos,
                                                const RecordObject& record_obj) {
...
bool CoreChecks::PreCallValidateBindImageMemory2KHR(VkDevice device,
                                                    uint32_t bindInfoCount,
                                                    const VkBindImageMemoryInfo* pBindInfos,
                                                    const ErrorObject& error_obj) const {
...
void CoreChecks::PostCallRecordBindImageMemory2KHR(VkDevice device,
                                                   uint32_t bindInfoCount,
                                                   const VkBindImageMemoryInfo* pBindInfos,
                                                   const RecordObject& record_obj) {
...
bool CoreChecks::ValidateBufferSparseMemoryBindAlignments(const VkSparseMemoryBind& bind,
    ...
    if (SafeModulo(bind.resourceOffset, buffer.requirements.alignment) != 0) {
        ...
        skip |= LogError("VUID-VkSparseMemoryBind-resourceOffset-09491",
                         objlist,
                         buffer_bind_info_loc.dot(Field::buffer),
                         "(%s) is being bound, but %s.resourceOffset (%" PRIu64
                         ") is not a multiple of required memory alignment (%" PRIu64 ").",
                         FormatHandle(buffer).c_str(),
                         bind_loc.Fields().c_str(),
                         bind.resourceOffset,
    ...
    if (SafeModulo(bind.size, buffer.requirements.alignment) != 0) {
        ...
        skip |= LogError("VUID-VkSparseMemoryBind-resourceOffset-09491",
                         objlist,
                         buffer_bind_info_loc.dot(Field::buffer),
                         "(%s) is being bound, but %s.size (%" PRIu64
                         ") is not a multiple of required memory alignment (%" PRIu64 ").",
                         FormatHandle(buffer).c_str(),
                         bind_loc.Fields().c_str(),
                         bind.size,
...
bool CoreChecks::ValidateImageSparseMemoryBindAlignments(const VkSparseMemoryBind& bind,
    ...
    if (SafeModulo(bind.resourceOffset, image.requirements[0].alignment) != 0) {
        ...
        skip |= LogError("VUID-VkSparseMemoryBind-resourceOffset-09492",
                         objlist,
                         image_bind_info_loc.dot(Field::image),
                         "(%s) is being bound, but %s.resourceOffset (%" PRIu64
                         ") is not a multiple of required memory alignment (%" PRIu64 ").",
                         FormatHandle(image).c_str(),
                         bind_loc.Fields().c_str(),
                         bind.resourceOffset,
                         image.requirements[0].alignment);
    }
...
bool CoreChecks::ValidateSparseMemoryBind(const VkSparseMemoryBind& bind,
    ...
    if (auto mem_state = Get<vvl::DeviceMemory>(bind.memory)) {
        if (!((uint32_t(1) << mem_state->allocate_info.memoryTypeIndex) & requirements.memoryTypeBits)) {
            ...
            skip |= LogError("VUID-VkSparseMemoryBind-memory-01096",
                             objlist,
                             loc.dot(Field::memory),
        ...
        if (SafeModulo(bind.memoryOffset, requirements.alignment) != 0) {
            ...
            skip |= LogError("VUID-VkSparseMemoryBind-memory-01096",
                             objlist,
                             loc.dot(Field::memoryOffset),
                             "(%" PRIu64 ") is not a multiple of required memory alignment (%" PRIu64 ")",
                             bind.memoryOffset,
        ...
        if (phys_dev_mem_props.memoryTypes[mem_state->allocate_info.memoryTypeIndex].propertyFlags &
            ...
            const LogObjectList objlist(bind.memory, resource_handle);
            skip |= LogError("VUID-VkSparseMemoryBind-memory-01097",
                             objlist,
                             loc.dot(Field::memory),
        ...
        if (bind.memoryOffset >= mem_state->allocate_info.allocationSize) {
            const LogObjectList objlist(bind.memory, resource_handle);
            skip |= LogError("VUID-VkSparseMemoryBind-memoryOffset-01101",
                             objlist,
                             loc.dot(Field::memoryOffset),
                             "(%" PRIu64 ") must be less than the size of memory (%" PRIu64 ")",
                             bind.memoryOffset,
        ...
        if ((mem_state->allocate_info.allocationSize - bind.memoryOffset) < bind.size) {
            const LogObjectList objlist(bind.memory, resource_handle);
            skip |= LogError("VUID-VkSparseMemoryBind-size-01102",
                             objlist,
                             loc.dot(Field::size),
        ...
        if (mem_state->IsExport()) {
            if (!(mem_state->export_handle_types & external_handle_types)) {
                const LogObjectList objlist(bind.memory, resource_handle);
                skip |=
                    LogError("VUID-VkSparseMemoryBind-memory-02730",
                             objlist,
    ...
    if (bind.size <= 0) {
        const LogObjectList objlist(bind.memory, resource_handle);
        skip |= LogError("VUID-VkSparseMemoryBind-size-01098",
                         objlist,
                         loc.dot(Field::size),
                         "(%" PRIu64 ") must be greater than 0.",
                         bind.size);
    }

    if (resource_size <= bind.resourceOffset) {
        const LogObjectList objlist(bind.memory, resource_handle);
        skip |= LogError("VUID-VkSparseMemoryBind-resourceOffset-01099",
                         objlist,
                         loc.dot(Field::resourceOffset),
                         "(%" PRIu64 ") must be less than the size of the resource (%" PRIu64 ").",
                         bind.resourceOffset,
                         resource_size);
    }
    ...
    if ((resource_size - bind.resourceOffset) < bind.size) {
        const LogObjectList objlist(bind.memory, resource_handle);
        skip |= LogError("VUID-VkSparseMemoryBind-size-01100",
                         objlist,
                         loc.dot(Field::size),
...
bool CoreChecks::ValidateImageSubresourceSparseImageMemoryBind(vvl::Image const& image_state,
    ...
    skip |= ValidateImageAspectMask(image_state.VkHandle(),
                                    image_state.create_info.format,
    ...
    if (subresource.mipLevel >= image_state.create_info.mipLevels) {
        skip |= LogError("VUID-VkSparseImageMemoryBindInfo-subresource-01722",
                         image_state.Handle(),
                         subresource_loc.dot(Field::mipLevel),
                         "(%" PRIu32 ") is not less than mipLevels (%" PRIu32 ") of %s.image.",
    ...
    if (subresource.arrayLayer >= image_state.create_info.arrayLayers) {
        skip |= LogError("VUID-VkSparseImageMemoryBindInfo-subresource-01723",
                         image_state.Handle(),
                         subresource_loc.dot(Field::arrayLayer),
                         "(%" PRIu32 ") is not less than arrayLayers (%" PRIu32 ") of %s.image.",
...
bool CoreChecks::ValidateSparseImageMemoryBind(vvl::Image const* image_state,
    ...
    if (auto const mem_state = Get<vvl::DeviceMemory>(bind.memory)) {
        ...
        if (bind.memoryOffset >= mem_state->allocate_info.allocationSize) {
            skip |= LogError("VUID-VkSparseMemoryBind-memoryOffset-01101",
                             bind.memory,
                             bind_loc.dot(Field::memoryOffset),
                             "(%" PRIu64 ") is not less than the size (%" PRIu64 ") of memory.",
                             bind.memoryOffset,
                             mem_state->allocate_info.allocationSize);
        }
        ...
        if (SafeModulo(bind.memoryOffset, requirement.alignment) != 0) {
            skip |= LogError("VUID-VkSparseImageMemoryBind-memory-01105",
                             bind.memory,
                             memory_loc.dot(Field::memoryOffset),
                             "(%" PRIu64 ") is not a multiple of the required alignment (%" PRIu64 ").",
                             bind.memoryOffset,
                             requirement.alignment);
        }

        skip |= ValidateMemoryTypes(*mem_state.get(),
                                    requirement.memoryTypeBits,
                                    memory_loc.dot(Field::memory),
    ...
    skip |= ValidateImageSubresourceSparseImageMemoryBind(
        *image_state, bind.subresource, bind_loc, memory_loc.dot(Field::subresource));
    ...
    for (auto const& requirements : image_state->sparse_requirements) {
        ...
        if (SafeModulo(bind.offset.x, granularity.width) != 0) {
            skip |= LogError("VUID-VkSparseImageMemoryBind-offset-01107",
                             image_state->Handle(),
                             bind_loc.dot(Field::offset).dot(Field::x),
                             "(%" PRId32 ") must be a multiple of the sparse image block width "
        ...
        if (SafeModulo(bind.offset.y, granularity.height) != 0) {
            skip |= LogError("VUID-VkSparseImageMemoryBind-offset-01109",
                             image_state->Handle(),
                             bind_loc.dot(Field::offset).dot(Field::y),
                             "(%" PRId32 ") must be a multiple of the sparse image block height "
        ...
        if (SafeModulo(bind.offset.z, granularity.depth) != 0) {
            skip |= LogError("VUID-VkSparseImageMemoryBind-offset-01111",
                             image_state->Handle(),
                             bind_loc.dot(Field::offset).dot(Field::z),
                             "(%" PRId32 ") must be a multiple of the sparse image block depth "
        ...
        if ((SafeModulo(bind.extent.width, granularity.width) != 0) &&
            ...
            skip |= LogError("VUID-VkSparseImageMemoryBind-extent-01108",
                             image_state->Handle(),
                             bind_loc.dot(Field::extent).dot(Field::width),
                             "(%" PRIu32 ") must either be a multiple of the sparse image block width "
                             ...
                             bind.extent.width,
                             granularity.width,
                             bind.extent.width + bind.offset.x,
        ...
        if ((SafeModulo(bind.extent.height, granularity.height) != 0) &&
            ...
            skip |= LogError("VUID-VkSparseImageMemoryBind-extent-01110",
                             image_state->Handle(),
                             bind_loc.dot(Field::extent).dot(Field::height),
                             "(%" PRIu32 ") must either be a multiple of the sparse image block height "
                             ...
                             bind.extent.height,
                             granularity.height,
                             bind.extent.height + bind.offset.y,
        ...
        if ((SafeModulo(bind.extent.depth, granularity.depth) != 0) &&
            ...
            skip |= LogError("VUID-VkSparseImageMemoryBind-extent-01112",
                             image_state->Handle(),
                             bind_loc.dot(Field::extent).dot(Field::depth),
                             "(%" PRIu32 ") must either be a multiple of the sparse image block depth "
                             ...
                             bind.extent.depth,
                             granularity.depth,
                             bind.extent.depth + bind.offset.z,
...
bool CoreChecks::PreCallValidateGetBufferDeviceAddress(VkDevice device,
                                                       const VkBufferDeviceAddressInfo* pInfo,
                                                       const ErrorObject& error_obj) const {
    bool skip = false;
    if (!enabled_features.bufferDeviceAddress && !enabled_features.bufferDeviceAddressEXT) {
        skip |= LogError("VUID-vkGetBufferDeviceAddress-bufferDeviceAddress-03324",
                         pInfo->buffer,
                         error_obj.location,
    ...
    if (physical_device_count > 1 && !enabled_features.bufferDeviceAddressMultiDevice &&
        ...
        skip |= LogError("VUID-vkGetBufferDeviceAddress-device-03325",
                         pInfo->buffer,
                         error_obj.location,
    ...
    if (auto buffer_state = Get<vvl::Buffer>(pInfo->buffer)) {
        ...
        if (!(buffer_state->create_info.flags & VK_BUFFER_CREATE_DEVICE_ADDRESS_CAPTURE_REPLAY_BIT)) {
            skip |= ValidateMemoryIsBoundToBuffer(
                device, *buffer_state, info_loc.dot(Field::buffer), "VUID-VkBufferDeviceAddressInfo-buffer-02600");
        }

        skip |= ValidateBufferUsageFlags(LogObjectList(device),
                                         *buffer_state,
                                         VK_BUFFER_USAGE_SHADER_DEVICE_ADDRESS_BIT,
                                         true,
                                         "VUID-VkBufferDeviceAddressInfo-buffer-02601",
                                         info_loc.dot(Field::buffer));
    }

    return skip;
}

bool CoreChecks::PreCallValidateGetBufferDeviceAddressEXT(VkDevice device,
                                                          const VkBufferDeviceAddressInfo* pInfo,
                                                          const ErrorObject& error_obj) const {
    return PreCallValidateGetBufferDeviceAddress(device, pInfo, error_obj);
}

bool CoreChecks::PreCallValidateGetBufferDeviceAddressKHR(VkDevice device,
                                                          const VkBufferDeviceAddressInfo* pInfo,
                                                          const ErrorObject& error_obj) const {
    return PreCallValidateGetBufferDeviceAddress(device, pInfo, error_obj);
}

bool CoreChecks::PreCallValidateGetBufferOpaqueCaptureAddress(VkDevice device,
                                                              const VkBufferDeviceAddressInfo* pInfo,
                                                              const ErrorObject& error_obj) const {
    bool skip = false;
    ...
    if (!enabled_features.bufferDeviceAddress) {
        skip |= LogError("VUID-vkGetBufferOpaqueCaptureAddress-None-03326",
                         objlist,
                         error_obj.location,
                         "The bufferDeviceAddress feature must be enabled.");
    }
    ...
    if (physical_device_count > 1 && !enabled_features.bufferDeviceAddressMultiDevice) {
        skip |= LogError("VUID-vkGetBufferOpaqueCaptureAddress-device-03327",
                         objlist,
                         error_obj.location,
                         "If device was created with multiple physical devices, then the "
                         "bufferDeviceAddressMultiDevice feature must be enabled.");
...
bool CoreChecks::PreCallValidateGetBufferOpaqueCaptureAddressKHR(VkDevice device,
                                                                 const VkBufferDeviceAddressInfo* pInfo,
                                                                 const ErrorObject& error_obj) const {
...
bool CoreChecks::PreCallValidateGetDeviceMemoryOpaqueCaptureAddress(VkDevice device,
    ...
    if (!enabled_features.bufferDeviceAddress) {
        skip |= LogError("VUID-vkGetDeviceMemoryOpaqueCaptureAddress-None-03334",
                         objlst,
                         error_obj.location,
    ...
    if (physical_device_count > 1 && !enabled_features.bufferDeviceAddressMultiDevice) {
        skip |= LogError("VUID-vkGetDeviceMemoryOpaqueCaptureAddress-device-03335",
                         objlst,
                         error_obj.location,
                         "If device was created with multiple physical devices, then the "
    ...
    if (auto mem_info = Get<vvl::DeviceMemory>(pInfo->memory)) {
        ...
        if (!chained_flags_struct || !(chained_flags_struct->flags & VK_MEMORY_ALLOCATE_DEVICE_ADDRESS_BIT)) {
            skip |= LogError("VUID-VkDeviceMemoryOpaqueCaptureAddressInfo-memory-03336",
                             objlst,
                             error_obj.location,
...
bool CoreChecks::PreCallValidateGetDeviceMemoryOpaqueCaptureAddressKHR(
    VkDevice device, const VkDeviceMemoryOpaqueCaptureAddressInfo* pInfo, const ErrorObject& error_obj) const {
    return PreCallValidateGetDeviceMemoryOpaqueCaptureAddress(device, pInfo, error_obj);
}

bool CoreChecks::ValidateMemoryIsBoundToBuffer(LogObjectList objlist,
