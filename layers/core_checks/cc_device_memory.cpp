...
bool CoreChecks::ValidateBindDataGraphPipelineSessionMemoryARM(const VkBindDataGraphPipelineSessionMemoryInfoARM &bind_info,
    ...
    const auto loc = error_obj.location.dot(Field::pBindInfos);
    ...
    if (bpr_match == bp_requirements.end()) {
        skip |= LogError("VUID-VkBindDataGraphPipelineSessionMemoryInfoARM-bindPoint-09786", objlist, loc.dot(Field::bindPoint),
            "bindPoint (%s) not found in requirements.",
            string_VkDataGraphPipelineSessionBindPointARM(bind_info.bindPoint));
    } else {
        if (bind_info.objectIndex > bpr_match->numObjects) {
            skip |= LogError(
                "VUID-VkBindDataGraphPipelineSessionMemoryInfoARM-objectIndex-09805", objlist, loc.dot(Field::objectIndex),
                "objectIndex (%" PRIu32 ") is greater than numObjects (%" PRIu32 ") defined for bindPoint (%s)",
    ...
    if (bound_memory_map.find(bind_info.bindPoint) != bound_memory_map.end()) {
        for (const auto &bound_mem : bound_memory_map.at(bind_info.bindPoint)) {
            if (bound_mem.memory_state->VkHandle() == bind_info.memory) {
                skip |= LogError("VUID-VkBindDataGraphPipelineSessionMemoryInfoARM-session-09785",
                    objlist, loc.dot(Field::bindPoint), "attempting to bind %s to %s which has already been bound to %s.",
                    FormatHandle(mem_info->Handle()).c_str(),
                    FormatHandle(session_state->Handle()).c_str(),
                    FormatHandle(bound_mem.memory_state->Handle()).c_str());
            }
        }
    }

    skip |= ValidateInsertMemoryRange(VulkanTypedHandle(bind_info.session, kVulkanObjectTypeDataGraphPipelineSessionARM),
                                        *mem_info, bind_info.memoryOffset, loc.dot(Field::memoryOffset));
    if (mem_reqs_map.find(bind_info.bindPoint) != mem_reqs_map.end()) {
        ...
        skip |= ValidateMemoryTypes(*mem_info, mem_reqs.memoryTypeBits, loc,
                                    "VUID-VkBindDataGraphPipelineSessionMemoryInfoARM-memory-09788");
        if (SafeModulo(bind_info.memoryOffset, mem_reqs.alignment) != 0) {
            skip |= LogError("VUID-VkBindDataGraphPipelineSessionMemoryInfoARM-memoryOffset-09789", objlist, loc,
                            "memoryOffset must be an integer multiple of the alignment member of the VkMemoryRequirements "
                            "structure returned from a call to vkGetDataGraphPipelineSessionMemoryRequirementsARM with session");
        }
        if (mem_reqs.size > (mem_info->allocate_info.allocationSize - bind_info.memoryOffset)) {
            skip |= LogError("VUID-VkBindDataGraphPipelineSessionMemoryInfoARM-size-09790", objlist, loc.dot(Field::size),
                                "(%" PRIu64 ") must be less than or equal to the size of memory (%" PRIu64
                                ") minus memoryOffset (%" PRIu64 ").",
                                mem_reqs.size, mem_info->allocate_info.allocationSize,
...
bool CoreChecks::PreCallValidateBindDataGraphPipelineSessionMemoryARM(VkDevice device, uint32_t bindInfoCount,
    ...
    bool skip = false;
    for (uint32_t i = 0; i < bindInfoCount; i++) {
        skip |= ValidateBindDataGraphPipelineSessionMemoryARM(pBindInfos[i], error_obj);
    }
    return skip;
}
