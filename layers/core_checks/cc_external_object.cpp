...
bool CoreChecks::ValidateAllocateMemoryMetal(const VkMemoryAllocateInfo &allocate_info,
    ...
    if ((import_memory_metal_info->handleType != VK_EXTERNAL_MEMORY_HANDLE_TYPE_MTLBUFFER_BIT_EXT) &&
        ...
        skip |= LogError("VUID-VK_EXT_external_memory-incorrect-handle-type", device,
                         allocate_info_loc.dot(Struct::VkImportMemoryMetalHandleInfoEXT, Field::handleType), "current value is %s",
    ...
    if (import_memory_metal_info->handleType == VK_EXTERNAL_MEMORY_HANDLE_TYPE_MTLTEXTURE_BIT_EXT) {
        if (allocate_info.allocationSize != 0) {
            skip |= LogError("VUID-VK_EXT_external_memory-VkMemoryAllocateInfo-allocationSize-not-0", device,
                             allocate_info_loc.dot(Field::allocationSize), "VkMemoryAllocateInfo::allocationSize is %" PRId64,
        ...
        DispatchGetPhysicalDeviceFormatProperties2Helper(physical_device, image_format, &format_properties);
