 ...
 *
 * This file deals with anything related to Phyiscal Devices, Logical Devices, or Device Queues Families, Device Masks,
 * etc
 */
...
bool CoreChecks::ValidateDeviceQueueFamily(uint32_t        queue_family,
                                           const Location& loc,
                                           const char*     vuid,
                                           bool            optional = false) const
{
    ...
    else if (queue_family_index_set.find(queue_family) == queue_family_index_set.end())
    {
        skip |= LogError(vuid,
                         device,
                         loc,
                         "(%" PRIu32 ") is not one of the queue families given via VkDeviceQueueCreateInfo structures "
...
bool CoreChecks::ValidatePhysicalDeviceQueueFamilies(uint32_t        queue_family_count,
    ...
    if (queue_families)
    {
        ...
        for (uint32_t i = 0; i < queue_family_count; ++i)
        {
            if (set.count(queue_families[i]))
            {
                skip |= LogError(vuid,
                                 device,
                                 loc.dot(Field::pQueueFamilyIndices, i),
                                 "(%" PRIu32 ") is also in pQueueFamilyIndices[0].",
                                 queue_families[i]);
            }
            else
            {
                ...
                else if (queue_families[i] >= physical_device_state->queue_family_known_count)
                {
                    ...
                    skip |= LogError(vuid,
                                     objlist,
                                     loc.dot(Field::pQueueFamilyIndices, i),
                                     "(%" PRIu32 ") is not one of the queue families supported by the parent "
                                     "PhysicalDevice %s of this device %s.",
                                     queue_families[i],
                                     FormatHandle(physical_device).c_str(),
...
bool CoreChecks::GetPhysicalDeviceImageFormatProperties(vvl::Image&     image_state,
                                                        ...
                                                        const Location& loc) const
{
    ...
    {
        image_properties_result = DispatchGetPhysicalDeviceImageFormatProperties(physical_device,
    ...
    if (image_properties_result != VK_SUCCESS)
    {
        skip |= LogError(vuid_string,
                         ...
                         "flags: %s.",
                         String(command),
                         string_VkResult(image_properties_result),
...
bool CoreChecks::ValidateDeviceMaskToPhysicalDeviceCount(uint32_t             deviceMask,
                                                         ...
                                                         const char*          vuid) const
{
    ...
    if (count <= deviceMask)
    {
        skip |= LogError(vuid,
                         objlist,
                         loc,
                         "(0x%" PRIx32 ") is invalid, Physical device count is %" PRIu32 ".",
                         deviceMask,
...
bool CoreChecks::ValidateDeviceMaskToZero(uint32_t             deviceMask,
                                          const LogObjectList& objlist,
                                          const Location       loc,
                                          const char*          vuid) const
{
...
bool CoreChecks::ValidateDeviceMaskToCommandBuffer(const vvl::CommandBuffer& cb_state,
                                                   uint32_t                  deviceMask,
                                                   const LogObjectList&      objlist,
                                                   const Location            loc,
                                                   const char*               vuid) const
{
    ...
    if ((deviceMask & cb_state.initial_device_mask) != deviceMask)
    {
        skip |= LogError(vuid,
                         objlist,
                         loc,
                         "(0x%" PRIx32 ") is not a subset of %s initial device mask (0x%" PRIx32 ").",
                         deviceMask,
                         FormatHandle(cb_state).c_str(),
                         cb_state.initial_device_mask);
    }
    return skip;
}

bool CoreChecks::ValidateDeviceMaskToRenderPass(const vvl::CommandBuffer& cb_state,
                                                uint32_t                  deviceMask,
                                                const Location            loc,
                                                const char*               vuid) const
{
    ...
    if ((deviceMask & cb_state.active_render_pass_device_mask) != deviceMask)
    {
        skip |= LogError(vuid,
                         cb_state.Handle(),
                         loc,
                         "(0x%" PRIx32 ") is not a subset of %s device mask (0x%" PRIx32 ").",
                         deviceMask,
...
bool CoreChecks::ValidateQueueFamilyIndex(const vvl::PhysicalDevice& pd_state,
                                          uint32_t                   requested_queue_family,
                                          const char*                vuid,
                                          const Location&            loc) const
{
    bool skip = false;
    ...
    if (requested_queue_family >= pd_state.queue_family_known_count)
    {
        ...
        skip |= LogError(vuid,
                         ...
                         pd_state.queue_family_known_count,
                         conditional_ext_cmd);
    }
    return skip;
}

bool CoreChecks::ValidateDeviceQueueCreateInfos(const vvl::PhysicalDevice&     pd_state,
                                                uint32_t                       info_count,
                                                ...
                                                const Location&                loc) const
{
    ...
    for (uint32_t i = 0; i < info_count; ++i)
    {
        ...
        skip |= ValidateQueueFamilyIndex(pd_state,
                                         requested_queue_family,
                                         "VUID-VkDeviceQueueCreateInfo-queueFamilyIndex-00381",
                                         info_loc.dot(Field::queueFamilyIndex));
        if (skip)
        { // Skip if queue family index is invalid, as it will be used as index in arrays
            continue;
        }
        ...
        if (api_version == VK_API_VERSION_1_0)
        {
            ...
            {
                skip |= LogError("VUID-VkDeviceCreateInfo-queueFamilyIndex-02802",
                                 pd_state.Handle(),
                                 info_loc.dot(Field::queueFamilyIndex),
                                 "(%" PRIu32
                                 ") is not unique and was also used in pCreateInfo->pQueueCreateInfos[%" PRIu32 "].",
                                 requested_queue_family,
        ...
        const auto*           global_priority_ci =
            vku::FindStructInPNextChain<VkDeviceQueueGlobalPriorityCreateInfo>(infos[i].pNext);
        ...
        if (prev_global_priority != global_priorities.end())
        {
            ...
            {
                skip |= LogError("VUID-VkDeviceCreateInfo-pQueueCreateInfos-06654",
                                 pd_state.Handle(),
                                 info_loc,
                                 ...
                                 string_VkQueueGlobalPriority(prev_global_priority->second),
        ...
        const VkQueueFamilyProperties requested_queue_family_props =
            pd_state.queue_family_properties[requested_queue_family];
        ...
        {
            skip |=
                LogError("VUID-VkDeviceQueueCreateInfo-flags-06449",
                         pd_state.Handle(),
                         info_loc.dot(Field::queueFamilyIndex),
            ...
            if (requested_queue_count > available_queue_count)
            {
                ...
                const std::string count_note =
                    queue_family_has_props
                        ...
                        : "the pQueueFamilyProperties[" + std::to_string(requested_queue_family) +
                              "] was never obtained";

                skip |= LogError("VUID-VkDeviceQueueCreateInfo-queueCount-00382",
                                 pd_state.Handle(),
                                 info_loc.dot(Field::queueCount),
                                 "(%" PRIu32 ") is not less than or equal to available queue count for this "
                                 "pCreateInfo->pQueueCreateInfos[%" PRIu32 "].queueFamilyIndex} (%" PRIu32
                                 ") obtained previously from vkGetPhysicalDeviceQueueFamilyProperties%s (%s).",
    ...
    for (uint32_t i = 0; i < static_cast<uint32_t>(queue_counts.size()); ++i)
    {
        ...
        {
            skip |= LogError("VUID-VkDeviceCreateInfo-pQueueCreateInfos-06755",
                             pd_state.Handle(),
                             loc,
                             ...
                             i,
                             queue_counts[i],
                             pd_state.queue_family_properties[i].queueCount);
        }
    }
...
bool CoreChecks::PreCallValidateCreateDevice(VkPhysicalDevice             gpu,
                                             ...
                                             const ErrorObject&           error_obj) const
{
    ...
    if (!pd_state)
    {
        skip |= LogError("VUID-vkCreateDevice-physicalDevice-parameter",
                         device,
                         error_obj.location,
    ...
    skip |= ValidateDeviceQueueCreateInfos(*pd_state,
...
void CoreChecks::PostCreateDevice(const VkDeviceCreateInfo* pCreateInfo, const Location& loc)
{
    ...
    if (!disabled[shader_validation_caching] && !disabled[shader_validation] && !core_validation_cache)
    {
        ...
        {
            LogInfo("WARNING-cache-file-error",
                    device,
                    loc,
...
void CoreChecks::PreCallRecordDestroyDevice(VkDevice                     device,
                                            const VkAllocationCallbacks* pAllocator,
                                            const RecordObject&          record_obj)
{
    ...
    if (core_validation_cache)
    {
        ...
        VkResult result = CoreLayerGetValidationCacheDataEXT(
            device, core_validation_cache, &validation_cache_size, validation_cache_data);
        ...
        if (validation_cache_path.size() > 0)
        {
            ...
            {
                LogInfo("WARNING-cache-write-error",
                        device,
                        loc,
                        "Cannot open shader validation cache at %s for writing",
                        validation_cache_path.c_str());
            }
        }
        free(validation_cache_data);
...
bool CoreChecks::PreCallValidateGetDeviceQueue(VkDevice           device,
                                               ...
                                               const ErrorObject& error_obj) const
{
    ...
    for (size_t i = 0; i < device_queue_info_list.size(); i++)
    {
        ...
        {
            skip |= LogError("VUID-vkGetDeviceQueue-flags-01841",
                             device,
                             error_obj.location.dot(Field::queueFamilyIndex),
                             "(%" PRIu32 ") was created with a non-zero VkDeviceQueueCreateFlags in "
        ...
        {
            skip |=
                LogError("VUID-vkGetDeviceQueue-queueIndex-00385",
                         device,
                         error_obj.location.dot(Field::queueIndex),
...
bool CoreChecks::PreCallValidateGetDeviceQueue2(VkDevice                  device,
                                                ...
                                                const ErrorObject&        error_obj) const
{
    ...
    if (pQueueInfo)
    {
        ...
        skip |= ValidateDeviceQueueFamily(queueFamilyIndex,
                                          queue_info_loc.dot(Field::queueFamilyIndex),
        ...
        for (size_t i = 0; i < device_queue_info_list.size(); i++)
        {
            ...
            if (device_queue_info.queue_count <= queueIndex)
            {
                skip |= LogError(
                    ...
                    error_obj.location.dot(Field::queueFamilyIndex),
                    "(%" PRIu32 ") is not less than the number of queues requested from [queueFamilyIndex (%" PRIu32
                    "), flags (%s)] combination when the device was created "
                    "vkCreateDevice::pCreateInfo->pQueueCreateInfos[%" PRIu32 "] (requested %" PRIu32 " queues).",
                    queueIndex,
                    queueFamilyIndex,
                    string_VkDeviceQueueCreateFlags(flags).c_str(),
                    device_queue_info.index,
                    device_queue_info.queue_count);
            }
        }
        ...
        if (!valid_flags && !skip)
        {
            skip |= LogError("VUID-VkDeviceQueueInfo2-flags-06225",
                             device,
                             error_obj.location,
...
bool CoreChecks::ValidateGetPhysicalDeviceImageFormatProperties2(
    const VkPhysicalDeviceImageFormatInfo2* pImageFormatInfo,
    VkImageFormatProperties2*               pImageFormatProperties,
    const ErrorObject&                      error_obj) const
{
    bool        skip = false;
    const auto* copy_perf_query =
        vku::FindStructInPNextChain<VkHostImageCopyDevicePerformanceQuery>(pImageFormatProperties->pNext);
    if (copy_perf_query)
    {
...
bool CoreChecks::PreCallValidateGetPhysicalDeviceImageFormatProperties2(
...
bool CoreChecks::PreCallValidateGetPhysicalDeviceImageFormatProperties2KHR(
    VkPhysicalDevice                        physicalDevice,
    const VkPhysicalDeviceImageFormatInfo2* pImageFormatInfo,
    VkImageFormatProperties2*               pImageFormatProperties,
    const ErrorObject&                      error_obj) const
{
    return PreCallValidateGetPhysicalDeviceImageFormatProperties2(
        physicalDevice, pImageFormatInfo, pImageFormatProperties, error_obj);
}
...
VkResult CoreChecks::CoreLayerCreateValidationCacheEXT(VkDevice                              device,
...
void CoreChecks::CoreLayerDestroyValidationCacheEXT(VkDevice                     device,
...
VkResult CoreChecks::CoreLayerGetValidationCacheDataEXT(VkDevice             device,
                                                        VkValidationCacheEXT validationCache,
                                                        size_t*              pDataSize,
                                                        void*                pData)
{
...
VkResult CoreChecks::CoreLayerMergeValidationCachesEXT(VkDevice                    device,
                                                       ...
                                                       const VkValidationCacheEXT* pSrcCaches)
{
    ...
    for (uint32_t i = 0; i < srcCacheCount; i++)
    {
        ...
        if (src == dst)
        {
            ...
            skip |= LogError("VUID-vkMergeValidationCachesEXT-dstCache-01536",
                             device,
                             loc,
                             "(0x%" PRIx64 ") must not appear in pSrcCaches array.",
...
bool CoreChecks::PreCallValidateCmdSetDeviceMask(VkCommandBuffer    commandBuffer,
                                                 uint32_t           deviceMask,
                                                 const ErrorObject& error_obj) const
{
    bool skip         = false;
    ...
    skip |=
        ValidateDeviceMaskToPhysicalDeviceCount(deviceMask, objlist, loc, "VUID-vkCmdSetDeviceMask-deviceMask-00108");
    ...
    skip |= ValidateDeviceMaskToCommandBuffer(
        cb_state, deviceMask, objlist, loc, "VUID-vkCmdSetDeviceMask-deviceMask-00110");
...
bool CoreChecks::PreCallValidateCmdSetDeviceMaskKHR(VkCommandBuffer    commandBuffer,
                                                    uint32_t           deviceMask,
                                                    const ErrorObject& error_obj) const
{
...
bool CoreChecks::PreCallValidateCreatePrivateDataSlotEXT(VkDevice                              device,
...
bool CoreChecks::PreCallValidateCreatePrivateDataSlot(VkDevice                           device,
                                                      ...
                                                      const ErrorObject&                 error_obj) const
{
    ...
    {
        skip |= LogError("VUID-vkCreatePrivateDataSlot-privateData-04564",
                         device,
                         error_obj.location,
...
bool CoreChecks::PreCallValidateCreateCommandPool(VkDevice                       device,
                                                  ...
                                                  const ErrorObject&             error_obj) const
{
    ...
    skip |= ValidateDeviceQueueFamily(pCreateInfo->queueFamilyIndex,
                                      create_info_loc.dot(Field::queueFamilyIndex),
                                      "VUID-vkCreateCommandPool-queueFamilyIndex-01937");
    if ((enabled_features.protectedMemory == VK_FALSE) &&
        ((pCreateInfo->flags & VK_COMMAND_POOL_CREATE_PROTECTED_BIT) != 0))
    {
        skip |=
            LogError("VUID-VkCommandPoolCreateInfo-flags-02860",
                     device,
...
bool CoreChecks::PreCallValidateDestroyCommandPool(VkDevice                     device,
                                                   ...
                                                   const ErrorObject&           error_obj) const
{
    ...
    for (auto& entry : cp_state->commandBuffers)
    {
        ...
        if (cb_state->InUse())
        {
            ...
            skip |= LogError("VUID-vkDestroyCommandPool-commandPool-00041",
                             objlist,
                             error_obj.location,
                             "(%s) is in use.",
...
bool CoreChecks::PreCallValidateResetCommandPool(VkDevice                device,
                                                 VkCommandPool           commandPool,
                                                 ...
                                                 const ErrorObject&      error_obj) const
{
    ...
    for (auto& entry : cp_state->commandBuffers)
    {
        ...
        {
            const LogObjectList objlist(cb_state->Handle(), commandPool);
            skip |= LogError("VUID-vkResetCommandPool-commandPool-00040",
                             objlist,
                             error_obj.location,
                             "(%s) is in use.",
                             FormatHandle(cb_state->Handle()).c_str());
...
bool CoreChecks::ValidateObjectNotInUse(const vvl::StateObject* obj_node,
                                        const Location&         loc,
                                        const char*             error_code) const
{
    ...
    if (used_handle)
    {
        skip |= LogError(error_code,
                         device,
                         loc,
                         "can't be called on %s that is currently in use by %s.",
...
bool CoreChecks::PreCallValidateGetCalibratedTimestampsEXT(VkDevice                            device,
                                                           ...
                                                           const ErrorObject&                  error_obj) const
{
    return PreCallValidateGetCalibratedTimestampsKHR(
        device, timestampCount, pTimestampInfos, pTimestamps, pMaxDeviation, error_obj);
}

bool CoreChecks::PreCallValidateGetCalibratedTimestampsKHR(VkDevice                            device,
                                                           uint32_t                            timestampCount,
                                                           ...
                                                           const ErrorObject&                  error_obj) const
{
    ...
    for (uint32_t i = 0; i < timestampCount; i++)
    {
        ...
        {
            skip |= LogError("VUID-vkGetCalibratedTimestampsEXT-timeDomain-09246",
                             device,
                             error_obj.location.dot(Field::pTimestampInfos, i).dot(Field::timeDomain),
                             "and pTimestampInfos[%" PRIu32 "].timeDomain are both %s.",
                             it->second,
        ...
        {
            skip |= LogError("VUID-VkCalibratedTimestampInfoKHR-timeDomain-02354",
                             device,
                             error_obj.location.dot(Field::pTimestampInfos, i).dot(Field::timeDomain),
                             "is %s.",
                             string_VkTimeDomainKHR(time_domain));
...
bool CoreChecks::ValidateDeviceQueueSupport(const Location& loc) const
{
    ...
    switch (loc.function)
    {
        ...
        case Func::vkCreateQueryPool:
            ...
            flags = VK_QUEUE_VIDEO_ENCODE_BIT_KHR | VK_QUEUE_VIDEO_DECODE_BIT_KHR | VK_QUEUE_COMPUTE_BIT |
    ...
    if ((physical_device_state->supported_queues & flags) == 0)
    {
        skip |= LogError(vuid,
                         device,
                         loc,
                         "device only supports (%s) but require one of (%s).",
                         string_VkQueueFlags(physical_device_state->supported_queues).c_str(),
                         string_VkQueueFlags(flags).c_str());
    }

    return skip;
}

bool CoreChecks::PreCallValidateGetDeviceFaultInfoEXT(VkDevice                device,
                                                      ...
                                                      const ErrorObject&      error_obj) const
{
    ...
    if (!is_device_lost)
    {
        skip |= LogError("VUID-vkGetDeviceFaultInfoEXT-device-07336",
                         device,
                         error_obj.location,
...
bool CoreChecks::PreCallValidateCreatePipelineBinariesKHR(VkDevice                             device,
                                                          ...
                                                          const ErrorObject&                   error_obj) const
{
    ...
    if (pCreateInfo->pipeline != VK_NULL_HANDLE)
    {
        ...
        {
            skip |= LogError("VUID-VkPipelineBinaryCreateInfoKHR-pipeline-09607",
                             pipeline,
                             create_info_loc.dot(Field::pipeline),
        ...
        {
            skip |= LogError("VUID-VkPipelineBinaryCreateInfoKHR-pipeline-09608",
                             pipeline,
                             create_info_loc.dot(Field::pipeline),
    ...
    if (pCreateInfo->pPipelineCreateInfo != nullptr)
    {
        ...
        {
            skip |= LogError("VUID-VkPipelineBinaryCreateInfoKHR-pipelineBinaryInternalCache-09609",
                             device,
                             create_info_loc.dot(Field::pPipelineCreateInfo),
                             "is not NULL, but pipelineBinaryInternalCache is false.");
        }

        if (props->pipelineBinaryInternalCacheControl && disable_internal_pipeline_cache)
        {
            skip |= LogError("VUID-VkPipelineBinaryCreateInfoKHR-device-09610",
                             device,
                             create_info_loc.dot(Field::pPipelineCreateInfo),
                             "is not NULL, but disableInternalCache is true.");
        }

        const auto* binary_info =
            vku::FindStructInPNextChain<VkPipelineBinaryInfoKHR>(pCreateInfo->pPipelineCreateInfo);
        if (binary_info && (binary_info->binaryCount > 0))
        {
            skip |= LogError("VUID-VkPipelineBinaryCreateInfoKHR-pPipelineCreateInfo-09606",
                             device,
                             create_info_loc.dot(Field::pPipelineCreateInfo).dot(Field::binaryCount),
                             "(%" PRIu32 ") is not zero",
    ...
    if (pointerCount != 1)
    {
        skip |= LogError("VUID-VkPipelineBinaryCreateInfoKHR-pKeysAndDataInfo-09619",
                         device,
                         create_info_loc,
