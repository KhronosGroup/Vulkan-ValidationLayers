...
bool CoreChecks::ValidateGraphicsIndexedCmd(const vvl::CommandBuffer& cb_state, const Location& loc) const {
    ...
    if (!buffer_state && !enabled_features.maintenance6 && !enabled_features.nullDescriptor) {
        skip |= LogError(vuid.index_binding_07312,
...
bool CoreChecks::ValidateCmdDrawInstance(const vvl::CommandBuffer& cb_state,
    ...
    if (cb_state.activeRenderPass && cb_state.activeRenderPass->has_multiview_enabled &&
        ...
        skip |= LogError(vuid.max_multiview_instance_index_02688,
                         objlist,
                         loc,
                         ...
                         phys_dev_props_core11.maxMultiviewInstanceIndex,
                         instanceCount,
                         firstInstance);
    }

    if (pipeline_state && pipeline_state->GraphicsCreateInfo().pVertexInputState) {
        const auto* vertex_input_divisor_state =
            vku::FindStructInPNextChain<VkPipelineVertexInputDivisorStateCreateInfo>(
        ...
        if (vertex_input_divisor_state && phys_dev_props_core14.supportsNonZeroFirstInstance == VK_FALSE &&
            ...
            for (uint32_t i = 0; i < vertex_input_divisor_state->vertexBindingDivisorCount; ++i) {
                if (vertex_input_divisor_state->pVertexBindingDivisors[i].divisor != 1u) {
                    ...
                    skip |= LogError(vuid.vertex_input_09461,
                                     objlist,
                                     loc,
                                     ...
                                     i,
                                     vertex_input_divisor_state->pVertexBindingDivisors[i].divisor,
                                     firstInstance);
        ...
        if (cb_state.IsDynamicStateSet(CB_DYNAMIC_STATE_VERTEX_INPUT_EXT) &&
            ...
            for (const auto& binding_state : cb_state.dynamic_state_value.vertex_bindings) {
                ...
                if (desc.divisor != 1u) {
                    ...
                    skip |= LogError(vuid.vertex_input_09462,
                                     objlist,
                                     loc,
...
bool CoreChecks::ValidateVTGShaderStages(const vvl::CommandBuffer& cb_state, const Location& loc) const {
    ...
    const auto* pipeline_state = cb_state.GetCurrentPipeline(VK_PIPELINE_BIND_POINT_GRAPHICS);
    if (pipeline_state &&
        pipeline_state->active_shaders & (VK_SHADER_STAGE_TASK_BIT_EXT | VK_SHADER_STAGE_MESH_BIT_EXT)) {
        skip |= LogError(vuid.invalid_mesh_shader_stages_06481,
                         cb_state.GetObjectList(VK_PIPELINE_BIND_POINT_GRAPHICS),
                         loc,
                         "The bound graphics pipeline must not have been created with "
                         "VK_SHADER_STAGE_TASK_BIT_EXT or VK_SHADER_STAGE_MESH_BIT_EXT. Active shader stages on the "
...
bool CoreChecks::ValidateMeshShaderStage(const vvl::CommandBuffer& cb_state, const Location& loc, bool is_NV) const {
    ...
    if (pipeline_state && !(pipeline_state->active_shaders & VK_SHADER_STAGE_MESH_BIT_EXT)) {
        skip |= LogError(
            vuid.missing_mesh_shader_stages_07080,
            cb_state.GetObjectList(VK_PIPELINE_BIND_POINT_GRAPHICS),
            loc,
            ...
            is_NV ? "MeshNV" : "MeshEXT",
            string_VkShaderStageFlags(pipeline_state->active_shaders).c_str());
    }
    if (pipeline_state && (pipeline_state->active_shaders &
                           (VK_SHADER_STAGE_VERTEX_BIT | VK_SHADER_STAGE_TESSELLATION_CONTROL_BIT |
                            VK_SHADER_STAGE_TESSELLATION_EVALUATION_BIT | VK_SHADER_STAGE_GEOMETRY_BIT))) {
        skip |= LogError(
            vuid.mesh_shader_stages_06480,
            cb_state.GetObjectList(VK_PIPELINE_BIND_POINT_GRAPHICS),
            loc,
            "The bound graphics pipeline must not have been created with "
            "VK_SHADER_STAGE_VERTEX_BIT, VK_SHADER_STAGE_TESSELLATION_CONTROL_BIT, "
    ...
    for (const auto& query : cb_state.activeQueries) {
        ...
        if (query_pool_state->create_info.queryType == VK_QUERY_TYPE_TRANSFORM_FEEDBACK_STREAM_EXT) {
            skip |= LogError(vuid.xfb_queries_07074,
                             cb_state.Handle(),
                             loc,
        ...
        if (query_pool_state->create_info.queryType == VK_QUERY_TYPE_PRIMITIVES_GENERATED_EXT) {
            skip |= LogError(vuid.pg_queries_07075,
                             cb_state.Handle(),
                             loc,
                             "Query with type %s is active.",
                             string_VkQueryType(query_pool_state->create_info.queryType));
...
bool CoreChecks::PreCallValidateCmdDrawMultiEXT(VkCommandBuffer commandBuffer,
                                                uint32_t drawCount,
    ...
    if (!enabled_features.multiDraw) {
        skip |= LogError("VUID-vkCmdDrawMultiEXT-None-04933",
                         cb_state.GetObjectList(VK_PIPELINE_BIND_POINT_GRAPHICS),
                         error_obj.location,
                         "The multiDraw feature was not enabled.");
    }
    if (drawCount > phys_dev_ext_props.multi_draw_props.maxMultiDrawCount) {
        skip |= LogError("VUID-vkCmdDrawMultiEXT-drawCount-04934",
                         cb_state.GetObjectList(VK_PIPELINE_BIND_POINT_GRAPHICS),
                         error_obj.location.dot(Field::drawCount),
                         "(%" PRIu32 ") must be less than maxMultiDrawCount (%" PRIu32 ").",
                         drawCount,
                         phys_dev_ext_props.multi_draw_props.maxMultiDrawCount);
    }
    if (drawCount > 1) {
        skip |= ValidateCmdDrawStrideWithStruct(cb_state,
    ...
    if (drawCount != 0 && !pVertexInfo) {
        skip |= LogError("VUID-vkCmdDrawMultiEXT-drawCount-04935",
                         cb_state.GetObjectList(VK_PIPELINE_BIND_POINT_GRAPHICS),
                         error_obj.location.dot(Field::drawCount),
                         "is %" PRIu32 " but pVertexInfo is NULL.",
                         drawCount);
    }
...
bool CoreChecks::ValidateCmdDrawIndexedBufferSize(const vvl::CommandBuffer& cb_state,
                                                  uint32_t indexCount,
                                                  uint32_t firstIndex,
                                                  const Location& loc,
    ...
    if (const auto buffer_state = Get<vvl::Buffer>(index_buffer_binding.buffer)) {
        ...
        // This doesn't exactly match the pseudocode of the VUID, but the binding size is the *bound* size, such that
        // the offset has already been accounted for (subtracted from the buffer size), and is consistent with the use
        // of BufferBinding::size for vertex buffer bindings (which record the *bound* size, not the size of the bound
        // buffer)
        ...
        if (end_offset > index_buffer_binding.size) {
            ...
            skip |= LogError(first_index_vuid,
                             objlist,
                             loc,
                             ...
                             end_offset + index_buffer_binding.offset,
                             index_buffer_binding.size + index_buffer_binding.offset);
        }
    }
    return skip;
}

bool CoreChecks::PreCallValidateCmdDrawIndexed(VkCommandBuffer commandBuffer,
                                               uint32_t indexCount,
                                               uint32_t instanceCount,
                                               uint32_t firstIndex,
    ...
    skip |= ValidateCmdDrawIndexedBufferSize(
        cb_state, indexCount, firstIndex, error_obj.location, "VUID-vkCmdDrawIndexed-robustBufferAccess2-08798");
    skip |= ValidateVTGShaderStages(cb_state, error_obj.location);
    return skip;
}

bool CoreChecks::PreCallValidateCmdDrawMultiIndexedEXT(VkCommandBuffer commandBuffer,
                                                       uint32_t drawCount,
    ...
    if (!enabled_features.multiDraw) {
        skip |= LogError("VUID-vkCmdDrawMultiIndexedEXT-None-04937",
                         cb_state.GetObjectList(VK_PIPELINE_BIND_POINT_GRAPHICS),
                         error_obj.location,
                         "multiDraw feature was not enabled.");
    }
    if (drawCount > phys_dev_ext_props.multi_draw_props.maxMultiDrawCount) {
        skip |= LogError("VUID-vkCmdDrawMultiIndexedEXT-drawCount-04939",
                         cb_state.GetObjectList(VK_PIPELINE_BIND_POINT_GRAPHICS),
                         error_obj.location.dot(Field::drawCount),
                         "(%" PRIu32
    ...
    if (drawCount != 0 && !pIndexInfo) {
        skip |= LogError("VUID-vkCmdDrawMultiIndexedEXT-drawCount-04940",
                         cb_state.GetObjectList(VK_PIPELINE_BIND_POINT_GRAPHICS),
                         error_obj.location.dot(Field::drawCount),
                         "is %" PRIu32 " but pIndexInfo is NULL.",
                         drawCount);
    } else {
        ...
        for (uint32_t i = 0; i < drawCount; i++) {
            ...
            skip |= ValidateCmdDrawIndexedBufferSize(cb_state,
                                                     info_ptr->indexCount,
                                                     info_ptr->firstIndex,
...
bool CoreChecks::PreCallValidateCmdDrawIndirect(VkCommandBuffer commandBuffer,
    ...
    if (!enabled_features.multiDrawIndirect && ((drawCount > 1))) {
        skip |= LogError("VUID-vkCmdDrawIndirect-drawCount-02718",
                         cb_state.GetObjectList(VK_PIPELINE_BIND_POINT_GRAPHICS),
                         error_obj.location.dot(Field::drawCount),
                         "(%" PRIu32 ") must be 0 or 1 if multiDrawIndirect feature is not enabled.",
                         drawCount);
    }
    if (drawCount > phys_dev_props.limits.maxDrawIndirectCount) {
        skip |= LogError("VUID-vkCmdDrawIndirect-drawCount-02719",
                         cb_state.GetObjectList(VK_PIPELINE_BIND_POINT_GRAPHICS),
                         error_obj.location.dot(Field::drawCount),
                         "(%" PRIu32 ") is not less than or equal to the maximum allowed (%" PRIu32 ").",
                         drawCount,
                         phys_dev_props.limits.maxDrawIndirectCount);
    }
    if (offset & 3) {
        skip |= LogError("VUID-vkCmdDrawIndirect-offset-02710",
                         cb_state.GetObjectList(VK_PIPELINE_BIND_POINT_GRAPHICS),
                         error_obj.location.dot(Field::offset),
    ...
    if (drawCount > 1) {
        skip |= ValidateCmdDrawStrideWithStruct(cb_state,
                                                ...
                                                error_obj.location);
        skip |= ValidateCmdDrawStrideWithBuffer(cb_state,
        ...
        LogObjectList objlist = cb_state.GetObjectList(VK_PIPELINE_BIND_POINT_GRAPHICS);
        objlist.add(buffer);
        skip |= LogError("VUID-vkCmdDrawIndirect-drawCount-00487",
                         objlist,
                         error_obj.location.dot(Field::drawCount),
                         "is 1 and (offset + sizeof(VkDrawIndirectCommand)) (%" PRIu64 ") is not less than "
...
bool CoreChecks::PreCallValidateCmdDrawIndexedIndirect(VkCommandBuffer commandBuffer,
                                                       VkBuffer buffer,
                                                       VkDeviceSize offset,
                                                       uint32_t drawCount,
                                                       uint32_t stride,
    ...
    if (!enabled_features.multiDrawIndirect && ((drawCount > 1))) {
        skip |= LogError("VUID-vkCmdDrawIndexedIndirect-drawCount-02718",
                         cb_state.GetObjectList(VK_PIPELINE_BIND_POINT_GRAPHICS),
                         error_obj.location.dot(Field::drawCount),
                         "(%" PRIu32 ") must be 0 or 1 if multiDrawIndirect feature is not enabled.",
                         drawCount);
    ...
    if (drawCount > phys_dev_props.limits.maxDrawIndirectCount) {
        skip |= LogError("VUID-vkCmdDrawIndexedIndirect-drawCount-02719",
                         cb_state.GetObjectList(VK_PIPELINE_BIND_POINT_GRAPHICS),
                         error_obj.location.dot(Field::drawCount),
                         "(%" PRIu32 ") is not less than or equal to the maximum allowed (%" PRIu32 ").",
                         drawCount,
    ...
    if (drawCount > 1) {
        ...
        skip |= LogError("VUID-vkCmdDrawIndexedIndirect-offset-02710",
                         cb_state.GetObjectList(VK_PIPELINE_BIND_POINT_GRAPHICS),
                         error_obj.location.dot(Field::offset),
                         "(%" PRIu64 ") must be a multiple of 4.",
                         offset);
        ...
        LogObjectList objlist = cb_state.GetObjectList(VK_PIPELINE_BIND_POINT_GRAPHICS);
        objlist.add(buffer);
        skip |= LogError("VUID-vkCmdDrawIndexedIndirect-drawCount-00539",
                         objlist,
                         error_obj.location.dot(Field::drawCount),
                         "is 1 and (offset + sizeof(VkDrawIndexedIndirectCommand)) (%" PRIu64 ") is not less than "
                         "or equal to the size of buffer (%" PRIu64 ").",
                         (offset + sizeof(VkDrawIndexedIndirectCommand)),
                         buffer_state->create_info.size);
    }
    // TODO: If the drawIndirectFirstInstance feature is not enabled, all the firstInstance members of the
    // VkDrawIndexedIndirectCommand structures accessed by this command must be 0, which will require access to the
    // contents of 'buffer'.
    return skip;
}
...
bool CoreChecks::PreCallValidateCmdDispatch(VkCommandBuffer commandBuffer,
    ...
    if (groupCountX > phys_dev_props.limits.maxComputeWorkGroupCount[0]) {
        skip |= LogError("VUID-vkCmdDispatch-groupCountX-00386",
                         cb_state.GetObjectList(VK_SHADER_STAGE_COMPUTE_BIT),
                         error_obj.location.dot(Field::groupCountX),
                         "(%" PRIu32 ") exceeds device limit maxComputeWorkGroupCount[0] (%" PRIu32 ").",
                         groupCountX,
                         phys_dev_props.limits.maxComputeWorkGroupCount[0]);
    }

    if (groupCountY > phys_dev_props.limits.maxComputeWorkGroupCount[1]) {
        skip |= LogError("VUID-vkCmdDispatch-groupCountY-00387",
                         cb_state.GetObjectList(VK_SHADER_STAGE_COMPUTE_BIT),
                         error_obj.location.dot(Field::groupCountY),
                         "(%" PRIu32 ") exceeds device limit maxComputeWorkGroupCount[1] (%" PRIu32 ").",
                         groupCountY,
                         phys_dev_props.limits.maxComputeWorkGroupCount[1]);
    }

    if (groupCountZ > phys_dev_props.limits.maxComputeWorkGroupCount[2]) {
        skip |= LogError("VUID-vkCmdDispatch-groupCountZ-00388",
                         cb_state.GetObjectList(VK_SHADER_STAGE_COMPUTE_BIT),
                         error_obj.location.dot(Field::groupCountZ),
                         "(%" PRIu32 ") exceeds device limit maxComputeWorkGroupCount[2] (%" PRIu32 ").",
                         groupCountZ,
...
bool CoreChecks::PreCallValidateCmdDispatchBase(VkCommandBuffer commandBuffer,
                                                uint32_t baseGroupX,
                                                uint32_t baseGroupY,
                                                uint32_t baseGroupZ,
                                                uint32_t groupCountX,
    ...
    if (baseGroupX >= limit) {
        skip |= LogError("VUID-vkCmdDispatchBase-baseGroupX-00421",
                         cb_state.GetObjectList(VK_SHADER_STAGE_COMPUTE_BIT),
                         error_obj.location.dot(Field::baseGroupX),
                         "(%" PRIu32 ") equals or exceeds device limit maxComputeWorkGroupCount[0] (%" PRIu32 ").",
                         baseGroupX,
                         limit);
    } else if (groupCountX > (limit - baseGroupX)) {
        skip |= LogError("VUID-vkCmdDispatchBase-groupCountX-00424",
                         cb_state.GetObjectList(VK_SHADER_STAGE_COMPUTE_BIT),
                         error_obj.location.dot(Field::baseGroupX),
                         "(%" PRIu32 ") + groupCountX (%" PRIu32
    ...
    if (baseGroupY >= limit) {
        skip |= LogError("VUID-vkCmdDispatchBase-baseGroupX-00422",
                         cb_state.GetObjectList(VK_SHADER_STAGE_COMPUTE_BIT),
                         error_obj.location.dot(Field::baseGroupY),
                         "(%" PRIu32 ") equals or exceeds device limit maxComputeWorkGroupCount[1] (%" PRIu32 ").",
                         baseGroupY,
                         limit);
    } else if (groupCountY > (limit - baseGroupY)) {
        skip |= LogError("VUID-vkCmdDispatchBase-groupCountY-00425",
                         cb_state.GetObjectList(VK_SHADER_STAGE_COMPUTE_BIT),
                         error_obj.location.dot(Field::baseGroupY),
                         "(%" PRIu32 ") + groupCountY (%" PRIu32
    ...
    if (baseGroupZ >= limit) {
        skip |= LogError("VUID-vkCmdDispatchBase-baseGroupZ-00423",
                         cb_state.GetObjectList(VK_SHADER_STAGE_COMPUTE_BIT),
                         error_obj.location.dot(Field::baseGroupZ),
                         "(%" PRIu32 ") equals or exceeds device limit maxComputeWorkGroupCount[2] (%" PRIu32 ").",
                         baseGroupZ,
                         limit);
    } else if (groupCountZ > (limit - baseGroupZ)) {
        skip |= LogError("VUID-vkCmdDispatchBase-groupCountZ-00426",
                         cb_state.GetObjectList(VK_SHADER_STAGE_COMPUTE_BIT),
                         error_obj.location.dot(Field::baseGroupZ),
                         "(%" PRIu32 ") + groupCountZ (%" PRIu32
    ...
    if (baseGroupX || baseGroupY || baseGroupZ) {
        ...
        if (pipeline_state) {
            if (!(pipeline_state->create_flags & VK_PIPELINE_CREATE_DISPATCH_BASE)) {
                skip |= LogError("VUID-vkCmdDispatchBase-baseGroupX-00427",
                                 cb_state.GetObjectList(VK_SHADER_STAGE_COMPUTE_BIT),
                                 error_obj.location,
...
bool CoreChecks::PreCallValidateCmdDispatchBaseKHR(VkCommandBuffer commandBuffer,
                                                   uint32_t baseGroupX,
                                                   uint32_t baseGroupY,
                                                   uint32_t baseGroupZ,
                                                   uint32_t groupCountX,
                                                   ...
                                                   uint32_t groupCountZ,
                                                   const ErrorObject& error_obj) const {
    return PreCallValidateCmdDispatchBase(
        commandBuffer, baseGroupX, baseGroupY, baseGroupZ, groupCountX, groupCountY, groupCountZ, error_obj);
}

bool CoreChecks::PreCallValidateCmdDispatchIndirect(VkCommandBuffer commandBuffer,
                                                    VkBuffer buffer,
                                                    VkDeviceSize offset,
                                                    const ErrorObject& error_obj) const {
    bool skip = false;
    ...
    skip |= ValidateIndirectCmd(cb_state, *buffer_state, error_obj.location);
    if (offset & 3) {
        skip |= LogError("VUID-vkCmdDispatchIndirect-offset-02710",
                         cb_state.GetObjectList(VK_SHADER_STAGE_COMPUTE_BIT),
                         error_obj.location.dot(Field::offset),
                         "(%" PRIu64 ") must be a multiple of 4.",
                         offset);
    ...
    if ((offset + sizeof(VkDispatchIndirectCommand)) > buffer_state->create_info.size) {
        skip |= LogError("VUID-vkCmdDispatchIndirect-offset-00407",
                         cb_state.GetObjectList(VK_SHADER_STAGE_COMPUTE_BIT),
                         error_obj.location,
                         "The (offset + sizeof(VkDrawIndexedIndirectCommand)) (%" PRIu64 ")  is greater than the "
...
bool CoreChecks::PreCallValidateCmdDrawIndirectCount(VkCommandBuffer commandBuffer,
                                                     VkBuffer buffer,
                                                     VkDeviceSize offset,
    ...
    if (offset & 3) {
        skip |= LogError("VUID-vkCmdDrawIndirectCount-offset-02710",
                         cb_state.GetObjectList(VK_PIPELINE_BIND_POINT_GRAPHICS),
                         error_obj.location.dot(Field::offset),
                         "(%" PRIu64 "), is not a multiple of 4.",
                         offset);
    }
    ...
    if (countBufferOffset & 3) {
        skip |= LogError("VUID-vkCmdDrawIndirectCount-countBufferOffset-02716",
                         cb_state.GetObjectList(VK_PIPELINE_BIND_POINT_GRAPHICS),
                         error_obj.location.dot(Field::countBufferOffset),
                         "(%" PRIu64 "), is not a multiple of 4.",
                         countBufferOffset);
    }
...
bool CoreChecks::PreCallValidateCmdDrawIndirectCountKHR(VkCommandBuffer commandBuffer,
                                                        VkBuffer buffer,
                                                        VkDeviceSize offset,
                                                        VkBuffer countBuffer,
                                                        VkDeviceSize countBufferOffset,
                                                        ...
                                                        uint32_t stride,
                                                        const ErrorObject& error_obj) const {
    return PreCallValidateCmdDrawIndirectCount(
        commandBuffer, buffer, offset, countBuffer, countBufferOffset, maxDrawCount, stride, error_obj);
}

bool CoreChecks::PreCallValidateCmdDrawIndexedIndirectCount(VkCommandBuffer commandBuffer,
                                                            VkBuffer buffer,
                                                            VkDeviceSize offset,
                                                            VkBuffer countBuffer,
                                                            VkDeviceSize countBufferOffset,
    ...
    if (offset & 3) {
        skip |= LogError("VUID-vkCmdDrawIndexedIndirectCount-offset-02710",
                         cb_state.GetObjectList(VK_PIPELINE_BIND_POINT_GRAPHICS),
                         error_obj.location.dot(Field::offset),
                         "(%" PRIu64 "), is not a multiple of 4.",
                         offset);
...
bool CoreChecks::PreCallValidateCmdDrawIndexedIndirectCountKHR(VkCommandBuffer commandBuffer,
                                                               VkBuffer buffer,
                                                               VkDeviceSize offset,
                                                               VkBuffer countBuffer,
                                                               VkDeviceSize countBufferOffset,
                                                               ...
                                                               uint32_t stride,
                                                               const ErrorObject& error_obj) const {
    return PreCallValidateCmdDrawIndexedIndirectCount(
        commandBuffer, buffer, offset, countBuffer, countBufferOffset, maxDrawCount, stride, error_obj);
}

bool CoreChecks::PreCallValidateCmdDrawIndirectByteCountEXT(VkCommandBuffer commandBuffer,
                                                            uint32_t instanceCount,
                                                            uint32_t firstInstance,
    ...
    if ((vertexStride <= 0) ||
        ...
        skip |= LogError("VUID-vkCmdDrawIndirectByteCountEXT-vertexStride-02289",
                         ...
                         "(%" PRIu32 ") must be between 0 and maxTransformFeedbackBufferDataStride (%" PRIu32 ").",
                         vertexStride,
    ...
    if (SafeModulo(counterBufferOffset, 4) != 0) {
        skip |= LogError("VUID-vkCmdDrawIndirectByteCountEXT-counterBufferOffset-04568",
                         cb_state.GetObjectList(VK_PIPELINE_BIND_POINT_GRAPHICS),
                         error_obj.location.dot(Field::counterBufferOffset),
                         "(%" PRIu64 ") must be a multiple of 4.",
                         counterBufferOffset);
    }
    ...
    if (SafeModulo(counterOffset, 4) != 0) {
        skip |= LogError("VUID-vkCmdDrawIndirectByteCountEXT-counterOffset-09474",
                         cb_state.GetObjectList(VK_PIPELINE_BIND_POINT_GRAPHICS),
                         error_obj.location.dot(Field::counterOffset),
                         "(%" PRIu32 ") must be a multiple of 4.",
                         counterOffset);
    }
...
bool CoreChecks::PreCallValidateCmdTraceRaysNV(VkCommandBuffer commandBuffer,
    ...
    if (SafeModulo(callableShaderBindingOffset, phys_dev_ext_props.ray_tracing_props_nv.shaderGroupBaseAlignment) !=
        0) {
    ...
    if (width > phys_dev_props.limits.maxComputeWorkGroupCount[0]) {
        skip |= LogError("VUID-vkCmdTraceRaysNV-width-02469",
                         cb_state.GetObjectList(VK_PIPELINE_BIND_POINT_RAY_TRACING_KHR),
    ...
    if (height > phys_dev_props.limits.maxComputeWorkGroupCount[1]) {
        skip |= LogError("VUID-vkCmdTraceRaysNV-height-02470",
                         cb_state.GetObjectList(VK_PIPELINE_BIND_POINT_RAY_TRACING_KHR),
    ...
    if (depth > phys_dev_props.limits.maxComputeWorkGroupCount[2]) {
        skip |= LogError("VUID-vkCmdTraceRaysNV-depth-02471",
                         cb_state.GetObjectList(VK_PIPELINE_BIND_POINT_RAY_TRACING_KHR),
    ...
    if (callable_shader_buffer_state && callableShaderBindingOffset >= callable_shader_buffer_state->create_info.size) {
        ...
        skip |= LogError("VUID-vkCmdTraceRaysNV-callableShaderBindingOffset-02461",
                         objlist,
                         error_obj.location.dot(Field::callableShaderBindingOffset),
                         "%" PRIu64 " must be less than the size of callableShaderBindingTableBuffer %" PRIu64 " .",
                         callableShaderBindingOffset,
                         callable_shader_buffer_state->create_info.size);
    }
    ...
    if (hit_shader_buffer_state && hitShaderBindingOffset >= hit_shader_buffer_state->create_info.size) {
        ...
        skip |= LogError("VUID-vkCmdTraceRaysNV-hitShaderBindingOffset-02459",
                         objlist,
                         error_obj.location.dot(Field::hitShaderBindingOffset),
                         "%" PRIu64 " must be less than the size of hitShaderBindingTableBuffer %" PRIu64 " .",
                         hitShaderBindingOffset,
                         hit_shader_buffer_state->create_info.size);
    }
    ...
    if (miss_shader_buffer_state && missShaderBindingOffset >= miss_shader_buffer_state->create_info.size) {
        ...
        skip |= LogError("VUID-vkCmdTraceRaysNV-missShaderBindingOffset-02457",
                         objlist,
                         error_obj.location.dot(Field::missShaderBindingOffset),
                         "%" PRIu64 " must be less than the size of missShaderBindingTableBuffer %" PRIu64 " .",
                         missShaderBindingOffset,
                         miss_shader_buffer_state->create_info.size);
    }
    ...
    if (raygenShaderBindingOffset >= raygen_shader_buffer_state->create_info.size) {
        ...
        skip |= LogError("VUID-vkCmdTraceRaysNV-raygenShaderBindingOffset-02455",
                         objlist,
                         error_obj.location.dot(Field::raygenShaderBindingOffset),
                         "%" PRIu64 " must be less than the size of raygenShaderBindingTableBuffer %" PRIu64 " .",
                         raygenShaderBindingOffset,
                         raygen_shader_buffer_state->create_info.size);
    }
    return skip;
}

bool CoreChecks::ValidateCmdTraceRaysKHR(const Location& loc,
    ...
    if (!pipeline_state)
        return skip; // possible wasn't bound correctly, check caught elsewhere
        ...
        if (pipeline_state->create_flags & VK_PIPELINE_CREATE_RAY_TRACING_NO_NULL_INTERSECTION_SHADERS_BIT_KHR) {
            ...
            if ((pHitShaderBindingTable->size == 0 || pHitShaderBindingTable->stride == 0)) {
                ...
                skip |= LogError(vuid,
                                 cb_state.Handle(),
                                 table_loc,
                                 "either size (%" PRIu64 ") and stride (%" PRIu64 ") is zero.",
...
bool CoreChecks::PreCallValidateCmdTraceRaysKHR(VkCommandBuffer commandBuffer,
    ...
    skip |= ValidateCmdTraceRaysKHR(error_obj.location,
                                    cb_state,
                                    pRaygenShaderBindingTable,
                                    pMissShaderBindingTable,
                                    pHitShaderBindingTable,
...
bool CoreChecks::PreCallValidateCmdDrawMeshTasksNV(VkCommandBuffer commandBuffer,
                                                   uint32_t taskCount,
                                                   uint32_t firstTask,
                                                   const ErrorObject& error_obj) const {
    bool skip = false;
    ...
    if (taskCount > phys_dev_ext_props.mesh_shader_props_nv.maxDrawMeshTasksCount) {
        skip |= LogError("VUID-vkCmdDrawMeshTasksNV-taskCount-02119",
                         cb_state.GetObjectList(VK_PIPELINE_BIND_POINT_GRAPHICS),
                         error_obj.location.dot(Field::taskCount),
                         "(0x%" PRIxLEAST32 "), must be less than or equal to "
                         "VkPhysicalDeviceMeshShaderPropertiesNV::maxDrawMeshTasksCount (0x%" PRIxLEAST32 ").",
                         taskCount,
                         phys_dev_ext_props.mesh_shader_props_nv.maxDrawMeshTasksCount);
    }
    skip |= ValidateActionState(cb_state, VK_PIPELINE_BIND_POINT_GRAPHICS, error_obj.location);
...
bool CoreChecks::PreCallValidateCmdDrawMeshTasksIndirectNV(VkCommandBuffer commandBuffer,
                                                           VkBuffer buffer,
                                                           VkDeviceSize offset,
                                                           uint32_t drawCount,
                                                           uint32_t stride,
    ...
    if (drawCount > 1) {
        ...
        if ((stride & 3) || stride < sizeof(VkDrawMeshTasksIndirectCommandNV)) {
            skip |= LogError("VUID-vkCmdDrawMeshTasksIndirectNV-drawCount-02146",
               ...
               ((offset + sizeof(VkDrawMeshTasksIndirectCommandNV)) > buffer_state.get()->create_info.size)) {
        LogObjectList objlist = cb_state.GetObjectList(VK_PIPELINE_BIND_POINT_GRAPHICS);
        objlist.add(buffer);
        skip |= LogError("VUID-vkCmdDrawMeshTasksIndirectNV-drawCount-02156",
                         objlist,
                         error_obj.location,
                         "(offset + sizeof(VkDrawMeshTasksIndirectNV)) (%" PRIu64
                         ") is greater than the size of buffer (%" PRIu64 ").",
                         offset + sizeof(VkDrawMeshTasksIndirectCommandNV),
                         buffer_state->create_info.size);
    }
    if (offset & 3) {
        skip |= LogError("VUID-vkCmdDrawMeshTasksIndirectNV-offset-02710",
                         cb_state.GetObjectList(VK_PIPELINE_BIND_POINT_GRAPHICS),
                         error_obj.location.dot(Field::offset),
                         "(%" PRIu64 "), is not a multiple of 4.",
                         offset);
    ...
    if (drawCount > phys_dev_props.limits.maxDrawIndirectCount) {
        skip |= LogError("VUID-vkCmdDrawMeshTasksIndirectNV-drawCount-02719",
                         cb_state.GetObjectList(VK_PIPELINE_BIND_POINT_GRAPHICS),
                         error_obj.location.dot(Field::drawCount),
                         "(%" PRIu32 ") is not less than or equal to maxDrawIndirectCount (%" PRIu32 ").",
                         drawCount,
                         phys_dev_props.limits.maxDrawIndirectCount);
    }
    skip |= ValidateMeshShaderStage(cb_state, error_obj.location, true);
...
bool CoreChecks::PreCallValidateCmdDrawMeshTasksIndirectCountNV(VkCommandBuffer commandBuffer,
                                                                VkBuffer buffer,
                                                                VkDeviceSize offset,
                                                                VkBuffer countBuffer,
                                                                VkDeviceSize countBufferOffset,
    ...
    if (offset & 3) {
        skip |= LogError("VUID-vkCmdDrawMeshTasksIndirectCountNV-offset-02710",
                         cb_state.GetObjectList(VK_PIPELINE_BIND_POINT_GRAPHICS),
                         error_obj.location.dot(Field::offset),
                         "(%" PRIu64 "), is not a multiple of 4.",
                         offset);
...
bool CoreChecks::PreCallValidateCmdDrawMeshTasksEXT(VkCommandBuffer commandBuffer,
                                                    uint32_t groupCountX,
                                                    uint32_t groupCountY,
                                                    uint32_t groupCountZ,
                                                    const ErrorObject& error_obj) const {
    ...
    if (groupCountX > phys_dev_ext_props.mesh_shader_props_ext.maxTaskWorkGroupCount[0]) {
        skip |= LogError("VUID-vkCmdDrawMeshTasksEXT-TaskEXT-07322",
                         cb_state.GetObjectList(VK_PIPELINE_BIND_POINT_GRAPHICS),
                         error_obj.location.dot(Field::groupCountX),
                         "(0x%" PRIxLEAST32 "), must be less than or equal to "
                         "VkPhysicalDeviceMeshShaderPropertiesEXT::maxTaskWorkGroupCount[0] (0x%" PRIxLEAST32 ").",
                         groupCountX,
                         phys_dev_ext_props.mesh_shader_props_ext.maxTaskWorkGroupCount[0]);
    }
    if (groupCountY > phys_dev_ext_props.mesh_shader_props_ext.maxTaskWorkGroupCount[1]) {
        skip |= LogError("VUID-vkCmdDrawMeshTasksEXT-TaskEXT-07323",
                         cb_state.GetObjectList(VK_PIPELINE_BIND_POINT_GRAPHICS),
                         error_obj.location.dot(Field::groupCountY),
                         "(0x%" PRIxLEAST32 "), must be less than or equal to "
                         "VkPhysicalDeviceMeshShaderPropertiesEXT::maxTaskWorkGroupCount[1] (0x%" PRIxLEAST32 ").",
                         groupCountY,
                         phys_dev_ext_props.mesh_shader_props_ext.maxTaskWorkGroupCount[1]);
    }
    if (groupCountZ > phys_dev_ext_props.mesh_shader_props_ext.maxTaskWorkGroupCount[2]) {
        skip |= LogError("VUID-vkCmdDrawMeshTasksEXT-TaskEXT-07324",
                         cb_state.GetObjectList(VK_PIPELINE_BIND_POINT_GRAPHICS),
                         error_obj.location.dot(Field::groupCountZ),
                         "(0x%" PRIxLEAST32 "), must be less than or equal to "
                         "VkPhysicalDeviceMeshShaderPropertiesEXT::maxTaskWorkGroupCount[2] (0x%" PRIxLEAST32 ").",
                         groupCountZ,
                         phys_dev_ext_props.mesh_shader_props_ext.maxTaskWorkGroupCount[2]);
    }
    ...
    if (fail) {
        skip |= LogError("VUID-vkCmdDrawMeshTasksEXT-TaskEXT-07325",
                         cb_state.GetObjectList(VK_PIPELINE_BIND_POINT_GRAPHICS),
                         error_obj.location,
                         "The product of groupCountX (0x%" PRIxLEAST32 "), groupCountY (0x%" PRIxLEAST32
                         ") and groupCountZ (0x%" PRIxLEAST32 ") must be less than or equal to "
...
bool CoreChecks::PreCallValidateCmdDrawMeshTasksIndirectEXT(VkCommandBuffer commandBuffer,
                                                            VkBuffer buffer,
                                                            VkDeviceSize offset,
                                                            uint32_t drawCount,
                                                            uint32_t stride,
    ...
    if ((drawCount == 1) && (offset + sizeof(VkDrawMeshTasksIndirectCommandEXT)) > buffer_state->create_info.size) {
        LogObjectList objlist = cb_state.GetObjectList(VK_PIPELINE_BIND_POINT_GRAPHICS);
        objlist.add(buffer);
        skip |= LogError("VUID-vkCmdDrawMeshTasksIndirectEXT-drawCount-07089",
                         objlist,
                         error_obj.location.dot(Field::drawCount),
                         "is 1 and (offset + sizeof(vkCmdDrawMeshTasksIndirectEXT)) (%" PRIu64 ") is not less than "
                         "or equal to the size of buffer (%" PRIu64 ").",
                         (offset + sizeof(VkDrawMeshTasksIndirectCommandEXT)),
                         buffer_state->create_info.size);
    }
...
bool CoreChecks::PreCallValidateCmdDrawMeshTasksIndirectCountEXT(VkCommandBuffer commandBuffer,
                                                                 VkBuffer buffer,
                                                                 VkDeviceSize offset,
                                                                 VkBuffer countBuffer,
                                                                 VkDeviceSize countBufferOffset,
    ...
    skip |= ValidateBufferUsageFlags(LogObjectList(commandBuffer, countBuffer),
                                     *count_buffer_state,
...
bool CoreChecks::ValidateActionState(const vvl::CommandBuffer& cb_state,
    ...
    if (!pipeline && !enabled_features.shaderObject) {
        return LogError(vuid.pipeline_bound_08606,
                        cb_state.GetObjectList(bind_point),
                        loc,
    ...
    if (!pipeline) {
        ...
        if (skip)
            return skip; // if shaders are bound wrong, likely to give false positives after
    }
    ...
    if (bind_point == VK_PIPELINE_BIND_POINT_GRAPHICS) {
        ...
        if (!cb_state.unprotected) {
            skip |= LogError(vuid.ray_query_protected_cb_03635,
                             cb_state.GetObjectList(bind_point),
                             loc,
    ...
    if (pipeline) {
        ...
        // TODO - VkPipeline have VK_PIPELINE_CREATE_DESCRIPTOR_BUFFER_BIT_EXT (descriptor_buffer_mode) to know if using
        // descriptor buffers, but VK_EXT_shader_object has no flag. For now, if the command buffer ever calls
        // vkCmdBindDescriptorBuffersEXT, we just assume things are bound until we add some form of GPU side tracking
...
static bool NeedDrawStateValidated(const vvl::CommandBuffer& cb_state,
                                   const vvl::DescriptorSet* descriptor_set,
                                   const LastBound::DescriptorSetSlot& ds_slot,
                                   bool disabled_image_layout_validation) {
...
bool CoreChecks::ValidateActionStateDescriptorsPipeline(const LastBound& last_bound_state,
                                                        const VkPipelineBindPoint bind_point,
    ...
    for (const auto& ds_slot : last_bound_state.ds_slots) {
        ...
        if (pipeline.descriptor_buffer_mode) {
            if (ds_slot.ds_state && !ds_slot.ds_state->IsPushDescriptor()) {
                ...
                skip |= LogError(vuid.descriptor_buffer_bit_not_set_08115,
                                 objlist,
                                 vuid.loc(),
                                 ...
                                 string_VkPipelineBindPoint(bind_point),
                                 FormatHandle(ds_slot.ds_state->Handle()).c_str());
                break;
            }
        } else if (ds_slot.descriptor_buffer_binding.has_value()) {
            const LogObjectList objlist(cb_state.Handle(), pipeline.Handle());
            skip |= LogError(vuid.descriptor_buffer_set_offset_missing_08117,
                             objlist,
                             vuid.loc(),
                             ...
                             string_VkPipelineBindPoint(bind_point),
                             ds_slot.descriptor_buffer_binding->index,
    ...
    if (!pipeline.active_slots.empty() &&
        !last_bound_state.IsBoundSetCompatible(pipeline.max_active_slot, *pipeline_layout)) {
        ...
        std::string range = pipeline.max_active_slot == 0
                                ? "set 0 is"
                                : "all sets 0 to " + std::to_string(pipeline.max_active_slot) + " are";
        skip |= LogError(vuid.compatible_pipeline_08600,
                         objlist,
                         vuid.loc(),
                         ...
                         pipe_layouts_log.str().c_str(),
                         pipeline.max_active_slot,
                         range.c_str(),
        ...
        for (const auto& [set_index, binding_req_map] : pipeline.active_slots) {
            ...
            if (!ds_slot.ds_state) {
                skip |= LogError(
                    vuid.compatible_pipeline_08600,
                    cb_state.GetObjectList(bind_point),
                    vuid.loc(),
                    "%s uses set #%" PRIu32
                    ...
                    FormatHandle(pipeline).c_str(),
                    set_index);
            } else if (!VerifySetLayoutCompatibility(*ds_slot.ds_state,
                ...
                skip |= LogError(vuid.compatible_pipeline_08600,
                                 ...
                                 set_index,
                                 FormatHandle(*pipeline_layout).c_str(),
                                 error_string.c_str());
...
bool CoreChecks::ValidateActionStateDescriptorsShaderObject(const LastBound& last_bound_state,
                                                            const VkPipelineBindPoint bind_point,
                                                            const vvl::DrawDispatchVuid& vuid) const {
    bool skip = false;
    const vvl::CommandBuffer& cb_state = last_bound_state.cb_state;
    ...
    for (const auto& shader_state : last_bound_state.shader_object_states) {
        ...
        if (shader_state && !shader_state->active_slots.empty() &&
            ...
            skip |= LogError(
                ...
                FormatHandle(shader_state->Handle()).c_str(),
                shader_state->max_active_slot,
            ...
            for (const auto& [set_index, binding_req_map] : shader_state->active_slots) {
                ...
                if (!ds_slot.ds_state) {
                    ...
                    if (need_validate) {
                        skip |=
                            ValidateDrawState(*descriptor_set, set_index, binding_req_map, cb_state, vuid.loc(), vuid);
                    }
                }
            }
        }
...
bool CoreChecks::ValidateActionStatePushConstant(const LastBound& last_bound_state,
                                                 const vvl::Pipeline* pipeline,
                                                 const vvl::DrawDispatchVuid& vuid) const {
    bool skip = false;
    const vvl::CommandBuffer& cb_state = last_bound_state.cb_state;
    ...
    // NOTE: Currently not checking whether active push constants are compatible with the active pipeline, nor whether
    // the
    //       "life times" of push constants are correct.
    //       Discussion on validity of these checks can be found at
    //       https://gitlab.khronos.org/vulkan/vulkan/-/issues/2602.
    if (pipeline) {
        ...
        if (!cb_state.push_constant_ranges_layout ||
            ...
            for (const auto& stage : pipeline->stage_states) {
                ...
                // Edge case where if the shader is using push constants statically and there never was a
                // vkCmdPushConstants
...
bool CoreChecks::ValidateActionStateProtectedMemory(const LastBound& last_bound_state,
                                                    const VkPipelineBindPoint bind_point,
                                                    const vvl::Pipeline* pipeline,
                                                    const vvl::DrawDispatchVuid& vuid) const {
    bool skip = false;
    ...
    if (pipeline) {
        for (const auto& stage : pipeline->stage_states) {
            ...
            if (stage.spirv_state->HasCapability(spv::CapabilityRayQueryKHR)) {
                skip |= LogError(vuid.ray_query_04617,
                                 last_bound_state.cb_state.GetObjectList(bind_point),
                                 vuid.loc(),
...
bool CoreChecks::MatchSampleLocationsInfo(const VkSampleLocationsInfoEXT& info_1,
                                          const VkSampleLocationsInfoEXT& info_2) const {
...
bool CoreChecks::ValidateIndirectCmd(const vvl::CommandBuffer& cb_state,
    ...
    skip |= ValidateMemoryIsBoundToBuffer(
        cb_state.VkHandle(), buffer_state, loc.dot(Field::buffer), vuid.indirect_contiguous_memory_02708);
    ...
    if (cb_state.unprotected == false) {
        skip |= LogError(vuid.indirect_protected_cb_02711,
                         objlist,
                         loc,
...
bool CoreChecks::ValidateIndirectCountCmd(const vvl::CommandBuffer& cb_state,
                                          const vvl::Buffer& count_buffer_state,
    ...
    skip |= ValidateMemoryIsBoundToBuffer(cb_state.VkHandle(),
                                          count_buffer_state,
    ...
    if (count_buffer_offset + sizeof(uint32_t) > count_buffer_state.create_info.size) {
        skip |= LogError(vuid.indirect_count_offset_04129,
                         objlist,
                         loc,
                         "countBufferOffset (%" PRIu64
                         ") + sizeof(uint32_t) is greater than the buffer size of %" PRIu64 ".",
                         count_buffer_offset,
                         count_buffer_state.create_info.size);
    }
    return skip;
}

bool CoreChecks::ValidateDrawPrimitivesGeneratedQuery(const LastBound& last_bound_state,
                                                      const vvl::DrawDispatchVuid& vuid) const {
    bool skip = false;
    const vvl::CommandBuffer& cb_state = last_bound_state.cb_state;
    ...
    if (primitives_generated_query) {
        if (!with_rasterizer_discard && last_bound_state.IsRasterizationDisabled()) {
            skip |= LogError(vuid.primitives_generated_06708,
                             cb_state.GetObjectList(VK_PIPELINE_BIND_POINT_GRAPHICS),
                             vuid.loc(),
        ...
        if (!with_non_zero_streams && pipeline) {
            const auto rasterization_state_stream_ci =
                vku::FindStructInPNextChain<VkPipelineRasterizationStateStreamCreateInfoEXT>(
                    pipeline->RasterizationStatePNext());
            if (rasterization_state_stream_ci && rasterization_state_stream_ci->rasterizationStream != 0) {
                skip |=
                    LogError(vuid.primitives_generated_streams_06709,
                             cb_state.GetObjectList(VK_PIPELINE_BIND_POINT_GRAPHICS),
                             vuid.loc(),
                             "a VK_QUERY_TYPE_PRIMITIVES_GENERATED_EXT query is active and pipeline was created with "
...
bool CoreChecks::ValidateDrawProtectedMemory(const LastBound& last_bound_state,
                                             const vvl::DrawDispatchVuid& vuid) const {
    bool skip = false;
    const vvl::CommandBuffer& cb_state = last_bound_state.cb_state;
    ...
    for (const auto& vertex_buffer_binding : cb_state.current_vertex_buffer_binding_info) {
        if (const auto buffer_state = Get<vvl::Buffer>(vertex_buffer_binding.second.buffer)) {
            skip |= ValidateProtectedBuffer(cb_state,
                                            *buffer_state,
                                            vuid.loc(),
...
bool CoreChecks::ValidateDrawDualSourceBlend(const LastBound& last_bound_state,
                                             const vvl::DrawDispatchVuid& vuid) const {
    bool skip = false;
    const vvl::CommandBuffer& cb_state = last_bound_state.cb_state;
    ...
    for (uint32_t i = 0; i < attachment_count; ++i) {
        ...
        if (dynamic_blend_equation) {
            ...
            if (IsSecondaryColorInputBlendFactor(color_blend_equation.srcColorBlendFactor) ||
                ...
                skip |= LogError(vuid.blend_dual_source_09239,
                                 objlist,
                                 vuid.loc(),
            ...
            if (IsSecondaryColorInputBlendFactor(attachment.srcColorBlendFactor) ||
                ...
                skip |= LogError(
                    ...
                    max_fragment_location,
                    phys_dev_props.limits.maxFragmentDualSrcAttachments,
                    string_VkBlendFactor(attachment.srcColorBlendFactor),
                    string_VkBlendFactor(attachment.dstColorBlendFactor),
                    string_VkBlendFactor(attachment.srcAlphaBlendFactor),
                    string_VkBlendFactor(attachment.dstAlphaBlendFactor));
                break;
            }
        }
    }
