...
bool CoreChecks::InsideVideoCodingScope(const vvl::CommandBuffer& cb_state,
                                        const Location& loc,
                                        const char* vuid) const {
    bool inside = false;
    if (cb_state.bound_video_session) {
        inside =
            LogError(vuid, cb_state.Handle(), loc, "It is invalid to issue this call inside a video coding block.");
    }
    return inside;
}
...
bool CoreChecks::OutsideVideoCodingScope(const vvl::CommandBuffer& cb_state,
                                         const Location& loc,
                                         const char* vuid) const {
    bool outside = false;
    if (!cb_state.bound_video_session) {
...
CoreChecks::GetVideoFormatProperties(VkImageUsageFlags image_usage,
    ...
    DispatchGetPhysicalDeviceVideoFormatPropertiesKHR(
        physical_device, &format_info, &format_count, format_props.data());

    return format_props;
}

std::vector<VkVideoFormatPropertiesKHR>
CoreChecks::GetVideoFormatProperties(VkImageUsageFlags image_usage, const VkVideoProfileInfoKHR* profile) const {
...
bool CoreChecks::IsSupportedVideoFormat(const VkImageCreateInfo& image_ci,
                                        const VkVideoProfileListInfoKHR* profile_list) const {
    auto format_props_list = GetVideoFormatProperties(image_ci.usage, profile_list);

    for (auto& format_props : format_props_list) {
        const VkImageCreateFlags allowed_flags =
            format_props.imageCreateFlags | VK_IMAGE_CREATE_VIDEO_PROFILE_INDEPENDENT_BIT_KHR;
        ...
        if (image_ci.format == format_props.format && (image_ci.flags & allowed_flags) == image_ci.flags &&
            image_ci.imageType == format_props.imageType && image_ci.tiling == format_props.imageTiling &&
            compatible_usage) {
            return true;
        }
    }
...
bool CoreChecks::IsVideoFormatSupported(VkFormat format,
    ...
    for (const auto& format_props : format_props_list) {
        if (format_props.format == format)
            return true;
    }
    return false;
}

bool CoreChecks::IsBufferCompatibleWithVideoSession(const vvl::Buffer& buffer_state,
                                                    const vvl::VideoSession& vs_state) const {
...
bool CoreChecks::IsImageCompatibleWithVideoSession(const vvl::Image& image_state,
                                                   const vvl::VideoSession& vs_state) const {
    if (image_state.create_info.flags & VK_IMAGE_CREATE_VIDEO_PROFILE_INDEPENDENT_BIT_KHR) {
        ...
        return image_state.supported_video_profiles.find(vs_state.profile) !=
               image_state.supported_video_profiles.end();
    }
}

void CoreChecks::EnqueueVerifyVideoSessionInitialized(vvl::CommandBuffer& cb_state,
                                                      vvl::VideoSession& vs_state,
                                                      const Location& loc,
                                                      const char* vuid) {
    cb_state.video_session_updates[vs_state.VkHandle()].emplace_back(
        [loc, vuid](const ValidationStateTracker& dev_data,
                    const vvl::VideoSession* vs_state,
                    vvl::VideoSessionDeviceState& dev_state,
                    bool do_validate) {
            bool skip = false;
            if (!dev_state.IsInitialized()) {
                skip |= dev_data.LogError(vuid,
                                          vs_state->Handle(),
                                          loc,
...
void CoreChecks::EnqueueVerifyVideoInlineQueryUnavailable(vvl::CommandBuffer& cb_state,
                                                          const VkVideoInlineQueryInfoKHR& query_info,
                                                          Func command) {
    if (disabled[query_validation])
        return;
    cb_state.query_updates.emplace_back([query_info, command](vvl::CommandBuffer& cb_state_arg,
                                                              bool do_validate,
...
bool CoreChecks::ValidateVideoInlineQueryInfo(const vvl::QueryPool& query_pool_state,
                                              const VkVideoInlineQueryInfoKHR& query_info,
                                              const Location& loc) const {
    bool skip = false;

    if (query_info.firstQuery >= query_pool_state.create_info.queryCount) {
        skip |= LogError("VUID-VkVideoInlineQueryInfoKHR-queryPool-08372",
                         query_pool_state.Handle(),
                         loc.dot(Field::firstQuery),
                         "(%u) is greater than or equal to the number of queries (%u) in %s.",
                         query_info.firstQuery,
    ...
    if (query_info.firstQuery + query_info.queryCount > query_pool_state.create_info.queryCount) {
        skip |= LogError("VUID-VkVideoInlineQueryInfoKHR-queryPool-08373",
                         query_pool_state.Handle(),
                         loc.dot(Field::firstQuery),
                         "(%u) plus queryCount (%u) is greater than the number of queries (%u) in %s.",
                         query_info.firstQuery,
                         query_info.queryCount,
                         query_pool_state.create_info.queryCount,
                         FormatHandle(query_pool_state).c_str());
    }
...
bool CoreChecks::ValidateVideoEncodeRateControlInfo(const VkVideoEncodeRateControlInfoKHR& rc_info,
                                                    const void* pNext,
    ...
    if (rc_info.layerCount > profile_caps.encode.maxRateControlLayers) {
        ...
        skip |= LogError("VUID-VkVideoEncodeRateControlInfoKHR-layerCount-08245",
                         objlist,
                         rc_info_loc.dot(Field::layerCount),
                         "(%u) is greater than the maxRateControlLayers (%u) "
                         "supported by the video profile %s was created with.",
                         rc_info.layerCount,
                         profile_caps.encode.maxRateControlLayers,
    ...
    if ((rc_info.rateControlMode == VK_VIDEO_ENCODE_RATE_CONTROL_MODE_DEFAULT_KHR ||
        ...
        skip |= LogError("VUID-VkVideoEncodeRateControlInfoKHR-rateControlMode-08248",
                         cmdbuf,
                         rc_info_loc.dot(Field::rateControlMode),
                         "is %s, but %s (%u) is not zero.",
    ...
    if (rc_info.rateControlMode == VK_VIDEO_ENCODE_RATE_CONTROL_MODE_CBR_BIT_KHR ||
        ...
        if (rc_info.layerCount != 0) {
            for (uint32_t layer_idx = 0; layer_idx < rc_info.layerCount; ++layer_idx) {
                skip |=
                    ValidateVideoEncodeRateControlLayerInfo(layer_idx, rc_info, pNext, cmdbuf, vs_state, rc_info_loc);
            }
        } else {
            skip |= LogError("VUID-VkVideoEncodeRateControlInfoKHR-rateControlMode-08275",
                             cmdbuf,
                             rc_info_loc.dot(Field::rateControlMode),
                             "is %s, but %s is zero.",
                             string_VkVideoEncodeRateControlModeFlagBitsKHR(rc_info.rateControlMode),
    ...
    if (rc_info.rateControlMode != VK_VIDEO_ENCODE_RATE_CONTROL_MODE_DEFAULT_KHR) {
        ...
        if ((profile_caps.encode.rateControlModes & rc_info.rateControlMode) == 0) {
            const LogObjectList objlist(cmdbuf, vs_state.Handle());
            skip |= LogError("VUID-VkVideoEncodeRateControlInfoKHR-rateControlMode-08244",
                             objlist,
                             rc_info_loc.dot(Field::rateControlMode),
                             "(%s) is not supported by the video profile %s was created with.",
                             string_VkVideoEncodeRateControlModeFlagBitsKHR(rc_info.rateControlMode),
                             FormatHandle(vs_state).c_str());
        }
    }
    ...
    if (rc_info.layerCount != 0) {
        if (rc_info.virtualBufferSizeInMs == 0) {
            skip |= LogError("VUID-VkVideoEncodeRateControlInfoKHR-layerCount-08357",
                             cmdbuf,
                             rc_info_loc.dot(Field::virtualBufferSizeInMs),
                             "must not be zero if %s (%u) is not zero.",
                             rc_info_loc.dot(Field::layerCount).Fields().c_str(),
                             rc_info.layerCount);
        }
        if (rc_info.initialVirtualBufferSizeInMs > rc_info.virtualBufferSizeInMs) {
            skip |= LogError("VUID-VkVideoEncodeRateControlInfoKHR-layerCount-08358",
                             cmdbuf,
...
bool CoreChecks::ValidateVideoEncodeRateControlInfoH264(const VkVideoEncodeRateControlInfoKHR& rc_info,
                                                        const void* pNext,
                                                        VkCommandBuffer cmdbuf,
                                                        const vvl::VideoSession& vs_state,
                                                        const Location& loc) const {
    ...
    if (rc_info_h264->flags & VK_VIDEO_ENCODE_H264_RATE_CONTROL_ATTEMPT_HRD_COMPLIANCE_BIT_KHR &&
        ...
        const LogObjectList objlist(cmdbuf, vs_state.Handle());
        skip |= LogError("VUID-VkVideoEncodeH264RateControlInfoKHR-flags-08280",
                         objlist,
                         rc_info_h264_loc.dot(Field::flags),
    ...
    if ((rc_info_h264->flags & VK_VIDEO_ENCODE_H264_RATE_CONTROL_REFERENCE_PATTERN_FLAT_BIT_KHR ||
        ...
        skip |= LogError("VUID-VkVideoEncodeH264RateControlInfoKHR-flags-08281",
                         cmdbuf,
                         rc_info_h264_loc.dot(Field::flags),
    ...
    if (rc_info_h264->flags & VK_VIDEO_ENCODE_H264_RATE_CONTROL_REFERENCE_PATTERN_FLAT_BIT_KHR &&
        rc_info_h264->flags & VK_VIDEO_ENCODE_H264_RATE_CONTROL_REFERENCE_PATTERN_DYADIC_BIT_KHR) {
        skip |= LogError("VUID-VkVideoEncodeH264RateControlInfoKHR-flags-08282",
                         cmdbuf,
                         rc_info_h264_loc.dot(Field::flags),
    ...
    if (rc_info_h264->flags & VK_VIDEO_ENCODE_H264_RATE_CONTROL_REGULAR_GOP_BIT_KHR &&
        rc_info_h264->gopFrameCount == 0) {
        skip |= LogError("VUID-VkVideoEncodeH264RateControlInfoKHR-flags-08283",
                         cmdbuf,
                         rc_info_h264_loc.dot(Field::flags),
    ...
    if (rc_info_h264->idrPeriod != 0 && rc_info_h264->idrPeriod < rc_info_h264->gopFrameCount) {
        skip |=
            LogError("VUID-VkVideoEncodeH264RateControlInfoKHR-idrPeriod-08284",
                     cmdbuf,
                     rc_info_h264_loc.dot(Field::idrPeriod),
                     "(%u) is not zero and the specified IDR period smaller than the GOP size specified in %s (%u).",
                     rc_info_h264->idrPeriod,
                     rc_info_h264_loc.dot(Field::gopFrameCount).Fields().c_str(),
                     rc_info_h264->gopFrameCount);
    }

    if (rc_info_h264->consecutiveBFrameCount != 0 &&
        rc_info_h264->consecutiveBFrameCount >= rc_info_h264->gopFrameCount) {
        skip |= LogError("VUID-VkVideoEncodeH264RateControlInfoKHR-consecutiveBFrameCount-08285",
                         cmdbuf,
                         rc_info_h264_loc.dot(Field::consecutiveBFrameCount),
                         "(%u) is greater than or equal to the GOP size specified in %s (%u).",
                         rc_info_h264->consecutiveBFrameCount,
                         rc_info_h264_loc.dot(Field::gopFrameCount).Fields().c_str(),
                         rc_info_h264->gopFrameCount);
    }

    if (rc_info.layerCount > 1 && rc_info.layerCount != rc_info_h264->temporalLayerCount) {
        skip |= LogError("VUID-VkVideoEncodeRateControlInfoKHR-videoCodecOperation-07022",
                         cmdbuf,
                         rc_info_h264_loc.dot(Field::temporalLayerCount),
                         "(%u) does not match %s (%u).",
                         rc_info_h264->temporalLayerCount,
                         loc.pNext(Struct::VkVideoEncodeRateControlInfoKHR, Field::layerCount).Fields().c_str(),
                         rc_info.layerCount);
    }
...
bool CoreChecks::ValidateVideoEncodeRateControlInfoH265(const VkVideoEncodeRateControlInfoKHR& rc_info,
                                                        const void* pNext,
                                                        VkCommandBuffer cmdbuf,
                                                        const vvl::VideoSession& vs_state,
                                                        const Location& loc) const {
    ...
    if (rc_info_h265->flags & VK_VIDEO_ENCODE_H265_RATE_CONTROL_ATTEMPT_HRD_COMPLIANCE_BIT_KHR &&
        ...
        const LogObjectList objlist(cmdbuf, vs_state.Handle());
        skip |= LogError("VUID-VkVideoEncodeH265RateControlInfoKHR-flags-08291",
                         objlist,
                         rc_info_h265_loc.dot(Field::flags),
    ...
    if ((rc_info_h265->flags & VK_VIDEO_ENCODE_H265_RATE_CONTROL_REFERENCE_PATTERN_FLAT_BIT_KHR ||
        ...
        skip |= LogError("VUID-VkVideoEncodeH265RateControlInfoKHR-flags-08292",
                         cmdbuf,
                         rc_info_h265_loc.dot(Field::flags),
                         "(%s) specifies a reference pattern but does not indicate the use of a regular GOP structure.",
    ...
    if (rc_info_h265->flags & VK_VIDEO_ENCODE_H265_RATE_CONTROL_REFERENCE_PATTERN_FLAT_BIT_KHR &&
        rc_info_h265->flags & VK_VIDEO_ENCODE_H265_RATE_CONTROL_REFERENCE_PATTERN_DYADIC_BIT_KHR) {
        skip |= LogError("VUID-VkVideoEncodeH265RateControlInfoKHR-flags-08293",
                         cmdbuf,
                         rc_info_h265_loc.dot(Field::flags),
                         "(%s) indicates conflicting reference patterns.",
                         string_VkVideoEncodeH265RateControlFlagsKHR(rc_info_h265->flags).c_str());
    ...
    if (rc_info_h265->flags & VK_VIDEO_ENCODE_H265_RATE_CONTROL_REGULAR_GOP_BIT_KHR &&
        rc_info_h265->gopFrameCount == 0) {
        skip |= LogError("VUID-VkVideoEncodeH265RateControlInfoKHR-flags-08294",
                         cmdbuf,
                         rc_info_h265_loc.dot(Field::flags),
    ...
    if (rc_info_h265->idrPeriod != 0 && rc_info_h265->idrPeriod < rc_info_h265->gopFrameCount) {
        skip |=
            LogError("VUID-VkVideoEncodeH265RateControlInfoKHR-idrPeriod-08295",
                     cmdbuf,
                     rc_info_h265_loc.dot(Field::idrPeriod),
                     "(%u) is not zero and the specified IDR period smaller than the GOP size specified in %s (%u).",
                     rc_info_h265->idrPeriod,
                     rc_info_h265_loc.dot(Field::gopFrameCount).Fields().c_str(),
                     rc_info_h265->gopFrameCount);
    }

    if (rc_info_h265->consecutiveBFrameCount != 0 &&
        rc_info_h265->consecutiveBFrameCount >= rc_info_h265->gopFrameCount) {
        skip |= LogError("VUID-VkVideoEncodeH265RateControlInfoKHR-consecutiveBFrameCount-08296",
                         cmdbuf,
                         rc_info_h265_loc.dot(Field::consecutiveBFrameCount),
                         "(%u) is greater than or equal to the GOP size specified in %s (%u).",
                         rc_info_h265->consecutiveBFrameCount,
                         rc_info_h265_loc.dot(Field::gopFrameCount).Fields().c_str(),
                         rc_info_h265->gopFrameCount);
    }

    if (rc_info.layerCount > 1 && rc_info.layerCount != rc_info_h265->subLayerCount) {
        skip |= LogError("VUID-VkVideoEncodeRateControlInfoKHR-videoCodecOperation-07025",
                         cmdbuf,
                         rc_info_h265_loc.dot(Field::subLayerCount),
                         "(%u) does not match %s (%u).",
                         rc_info_h265->subLayerCount,
                         loc.pNext(Struct::VkVideoEncodeRateControlInfoKHR, Field::layerCount).Fields().c_str(),
                         rc_info.layerCount);
    }
...
bool CoreChecks::ValidateVideoEncodeRateControlInfoAV1(const VkVideoEncodeRateControlInfoKHR& rc_info,
                                                       const void* pNext,
                                                       VkCommandBuffer cmdbuf,
                                                       const vvl::VideoSession& vs_state,
                                                       const Location& loc) const {
    ...
    if ((rc_info_av1->flags & VK_VIDEO_ENCODE_AV1_RATE_CONTROL_REFERENCE_PATTERN_FLAT_BIT_KHR ||
        ...
        skip |= LogError("VUID-VkVideoEncodeAV1RateControlInfoKHR-flags-10294",
                         cmdbuf,
                         rc_info_av1_loc.dot(Field::flags),
                         "(%s) specifies a reference pattern but does not indicate the use of a regular GOP structure.",
    ...
    if (rc_info_av1->flags & VK_VIDEO_ENCODE_AV1_RATE_CONTROL_REFERENCE_PATTERN_FLAT_BIT_KHR &&
        rc_info_av1->flags & VK_VIDEO_ENCODE_AV1_RATE_CONTROL_REFERENCE_PATTERN_DYADIC_BIT_KHR) {
        skip |= LogError("VUID-VkVideoEncodeAV1RateControlInfoKHR-flags-10295",
                         cmdbuf,
                         rc_info_av1_loc.dot(Field::flags),
                         "(%s) indicates conflicting reference patterns.",
                         string_VkVideoEncodeAV1RateControlFlagsKHR(rc_info_av1->flags).c_str());
    ...
    if (rc_info_av1->flags & VK_VIDEO_ENCODE_AV1_RATE_CONTROL_REGULAR_GOP_BIT_KHR && rc_info_av1->gopFrameCount == 0) {
        skip |= LogError("VUID-VkVideoEncodeAV1RateControlInfoKHR-flags-10296",
                         cmdbuf,
                         rc_info_av1_loc.dot(Field::flags),
    ...
    if (rc_info_av1->consecutiveBipredictiveFrameCount != 0 &&
        ...
        skip |= LogError("VUID-VkVideoEncodeAV1RateControlInfoKHR-consecutiveBipredictiveFrameCount-10298",
                         cmdbuf,
                         rc_info_av1_loc.dot(Field::consecutiveBipredictiveFrameCount),
                         "(%u) is greater than or equal to the GOP size specified in %s (%u).",
                         rc_info_av1->consecutiveBipredictiveFrameCount,
                         rc_info_av1_loc.dot(Field::gopFrameCount).Fields().c_str(),
                         rc_info_av1->gopFrameCount);
    }

    if (rc_info_av1->temporalLayerCount > profile_caps.encode_av1.maxTemporalLayerCount) {
        skip |= LogError("VUID-VkVideoEncodeAV1RateControlInfoKHR-temporalLayerCount-10299",
                         cmdbuf,
                         ...
                         profile_caps.encode_av1.maxTemporalLayerCount,
                         FormatHandle(vs_state).c_str());
    }

    if (rc_info.layerCount > 1 && rc_info.layerCount != rc_info_av1->temporalLayerCount) {
        skip |= LogError("VUID-VkVideoEncodeRateControlInfoKHR-videoCodecOperation-10351",
                         cmdbuf,
                         rc_info_av1_loc.dot(Field::temporalLayerCount),
                         "(%u) does not match %s (%u).",
                         rc_info_av1->temporalLayerCount,
...
bool CoreChecks::ValidateVideoEncodeRateControlLayerInfo(uint32_t layer_index,
                                                         const VkVideoEncodeRateControlInfoKHR& rc_info,
                                                         const void* pNext,
                                                         VkCommandBuffer cmdbuf,
                                                         const vvl::VideoSession& vs_state,
    ...
    if (rc_layer_info.averageBitrate < 1 || rc_layer_info.averageBitrate > profile_caps.encode.maxBitrate) {
        const LogObjectList objlist(cmdbuf, vs_state.Handle());
        skip |= LogError("VUID-VkVideoEncodeRateControlInfoKHR-pLayers-08276",
                         objlist,
                         rc_layer_info_loc.dot(Field::averageBitrate),
                         ...
                         rc_layer_info.averageBitrate,
                         profile_caps.encode.maxBitrate,
                         FormatHandle(vs_state).c_str());
    }

    if (rc_layer_info.maxBitrate < 1 || rc_layer_info.maxBitrate > profile_caps.encode.maxBitrate) {
        const LogObjectList objlist(cmdbuf, vs_state.Handle());
        skip |= LogError("VUID-VkVideoEncodeRateControlInfoKHR-pLayers-08277",
                         objlist,
                         rc_layer_info_loc.dot(Field::maxBitrate),
                         "(%" PRIu64 ") must be between 1 and VkVideoEncodeCapabilitiesKHR::maxBitrate (%" PRIu64
                         ") limit supported by the video profile %s was created with.",
                         rc_layer_info.maxBitrate,
                         profile_caps.encode.maxBitrate,
                         FormatHandle(vs_state).c_str());
    }
    ...
    if (rc_info.rateControlMode == VK_VIDEO_ENCODE_RATE_CONTROL_MODE_CBR_BIT_KHR &&
        ...
        skip |= LogError("VUID-VkVideoEncodeRateControlInfoKHR-rateControlMode-08356",
                         cmdbuf,
                         rc_info_loc.dot(Field::rateControlMode),
                         ...
                         rc_layer_info.maxBitrate,
                         rc_layer_info.averageBitrate,
                         rc_layer_info_loc.Fields().c_str());
    }

    if (rc_info.rateControlMode == VK_VIDEO_ENCODE_RATE_CONTROL_MODE_VBR_BIT_KHR &&
        rc_layer_info.averageBitrate > rc_layer_info.maxBitrate) {
        skip |= LogError("VUID-VkVideoEncodeRateControlInfoKHR-rateControlMode-08278",
                         cmdbuf,
                         rc_info_loc.dot(Field::rateControlMode),
    ...
    if (rc_layer_info.frameRateNumerator == 0) {
        skip |= LogError("VUID-VkVideoEncodeRateControlLayerInfoKHR-frameRateNumerator-08350",
                         cmdbuf,
                         rc_layer_info_loc.dot(Field::frameRateNumerator),
                         "is zero.");
    }

    if (rc_layer_info.frameRateDenominator == 0) {
        skip |= LogError("VUID-VkVideoEncodeRateControlLayerInfoKHR-frameRateDenominator-08351",
                         cmdbuf,
                         rc_layer_info_loc.dot(Field::frameRateDenominator),
                         "is zero.");
    }

    switch (vs_state.GetCodecOp()) {
        case VK_VIDEO_CODEC_OPERATION_ENCODE_H264_BIT_KHR:
            skip |= ValidateVideoEncodeRateControlLayerInfoH264(
                layer_index, rc_info, pNext, cmdbuf, vs_state, rc_layer_info_loc);
            break;

        case VK_VIDEO_CODEC_OPERATION_ENCODE_H265_BIT_KHR:
            skip |= ValidateVideoEncodeRateControlLayerInfoH265(
                layer_index, rc_info, pNext, cmdbuf, vs_state, rc_layer_info_loc);
            break;

        case VK_VIDEO_CODEC_OPERATION_ENCODE_AV1_BIT_KHR:
            skip |= ValidateVideoEncodeRateControlLayerInfoAV1(
                layer_index, rc_info, pNext, cmdbuf, vs_state, rc_layer_info_loc);
            break;
...
bool CoreChecks::ValidateVideoEncodeRateControlH26xQp(VkCommandBuffer cmdbuf,
                                                      const vvl::VideoSession& vs_state,
                                                      const RateControlLayerInfo& rc_layer_info,
                                                      const char* min_qp_range_vuid,
                                                      const char* max_qp_range_vuid,
                                                      int32_t min_qp,
                                                      int32_t max_qp,
                                                      const char* min_qp_per_pic_type_vuid,
                                                      const char* max_qp_per_pic_type_vuid,
                                                      bool qp_per_picture_type,
                                                      const char* min_max_qp_compare_vuid,
                                                      const Location& loc) const {
    bool skip = false;
    ...
    auto qp_range_error = [&](const char* vuid, const Location& field_loc, int32_t value) {
        ...
        return LogError(vuid,
                        objlist,
                        field_loc,
                        "(%d) is outside of the range [%d, %d] supported by the video profile %s was created with.",
                        value,
                        min_qp,
                        max_qp,
                        FormatHandle(vs_state).c_str());
    ...
    auto qp_per_pic_type_error =
        [&](const char* vuid, const Location& struct_loc, int32_t qp_i, int32_t qp_p, int32_t qp_b) {
            const LogObjectList objlist(cmdbuf, vs_state.Handle());
            return LogError(
                vuid,
                objlist,
    ...
    auto min_max_qp_compare_error = [&](const char* which, int32_t min_value, int32_t max_value) {
        return LogError(min_max_qp_compare_vuid,
                        cmdbuf,
                        loc,
                        "minQp.%s (%d) is greater than maxQp.%s (%d).",
                        which,
                        min_value,
                        which,
                        max_value);
    ...
    if (rc_layer_info.useMinQp) {
        ...
        if ((rc_layer_info.minQp.qpI != rc_layer_info.minQp.qpP ||
             rc_layer_info.minQp.qpI != rc_layer_info.minQp.qpB) &&
            !qp_per_picture_type) {
            skip |= qp_per_pic_type_error(min_qp_per_pic_type_vuid,
                                          loc.dot(Field::minQp),
                                          rc_layer_info.minQp.qpI,
                                          rc_layer_info.minQp.qpP,
                                          rc_layer_info.minQp.qpB);
        }
    }
    ...
    if (rc_layer_info.useMaxQp) {
        ...
        if ((rc_layer_info.maxQp.qpI != rc_layer_info.maxQp.qpP ||
             rc_layer_info.maxQp.qpI != rc_layer_info.maxQp.qpB) &&
            !qp_per_picture_type) {
            skip |= qp_per_pic_type_error(max_qp_per_pic_type_vuid,
                                          loc.dot(Field::maxQp),
                                          rc_layer_info.maxQp.qpI,
                                          rc_layer_info.maxQp.qpP,
                                          rc_layer_info.maxQp.qpB);
        }
    }
...
bool CoreChecks::ValidateVideoEncodeRateControlAV1QIndex(VkCommandBuffer cmdbuf,
                                                         const vvl::VideoSession& vs_state,
                                                         const VkVideoEncodeAV1RateControlLayerInfoKHR& rc_layer_info,
                                                         const char* min_q_index_range_vuid,
                                                         const char* max_q_index_range_vuid,
                                                         uint32_t min_q_index,
                                                         uint32_t max_q_index,
                                                         const char* min_q_index_per_rc_group_vuid,
                                                         const char* max_q_index_per_rc_group_vuid,
                                                         bool q_index_per_rc_group,
                                                         const char* min_max_q_index_compare_vuid,
                                                         const Location& loc) const {
    bool skip = false;
    ...
    auto q_index_per_rc_group_error =
        [&](const char* vuid, const Location& struct_loc, uint32_t qi_i, uint32_t qi_p, uint32_t qi_b) {
            const LogObjectList objlist(cmdbuf, vs_state.Handle());
            return LogError(vuid,
                            objlist,
                            struct_loc,
    ...
    auto min_max_q_index_compare_error = [&](const char* which, uint32_t min_value, uint32_t max_value) {
        return LogError(min_max_q_index_compare_vuid,
                        cmdbuf,
                        loc,
                        "minQIndex.%s (%u) is greater than maxQIndex.%s (%u).",
                        which,
                        min_value,
                        which,
                        max_value);
    ...
    if (rc_layer_info.useMinQIndex) {
        if (rc_layer_info.minQIndex.intraQIndex < min_q_index || rc_layer_info.minQIndex.intraQIndex > max_q_index) {
            skip |= q_index_range_error(min_q_index_range_vuid,
                                        loc.dot(Field::minQIndex).dot(Field::intraQIndex),
                                        rc_layer_info.minQIndex.intraQIndex);
        }

        if (rc_layer_info.minQIndex.predictiveQIndex < min_q_index ||
            rc_layer_info.minQIndex.predictiveQIndex > max_q_index) {
            skip |= q_index_range_error(min_q_index_range_vuid,
                                        loc.dot(Field::minQIndex).dot(Field::qpP),
                                        rc_layer_info.minQIndex.predictiveQIndex);
        }

        if (rc_layer_info.minQIndex.bipredictiveQIndex < min_q_index ||
            rc_layer_info.minQIndex.bipredictiveQIndex > max_q_index) {
            skip |= q_index_range_error(min_q_index_range_vuid,
                                        loc.dot(Field::minQIndex).dot(Field::bipredictiveQIndex),
        ...
        if ((rc_layer_info.minQIndex.intraQIndex != rc_layer_info.minQIndex.predictiveQIndex ||
            ...
            skip |= q_index_per_rc_group_error(min_q_index_per_rc_group_vuid,
                                               loc.dot(Field::minQIndex),
                                               rc_layer_info.minQIndex.intraQIndex,
                                               rc_layer_info.minQIndex.predictiveQIndex,
                                               rc_layer_info.minQIndex.bipredictiveQIndex);
        }
    ...
    if (rc_layer_info.useMaxQIndex) {
        if (rc_layer_info.maxQIndex.intraQIndex < min_q_index || rc_layer_info.maxQIndex.intraQIndex > max_q_index) {
            skip |= q_index_range_error(max_q_index_range_vuid,
                                        loc.dot(Field::maxQIndex).dot(Field::intraQIndex),
                                        rc_layer_info.maxQIndex.intraQIndex);
        }

        if (rc_layer_info.maxQIndex.predictiveQIndex < min_q_index ||
            rc_layer_info.maxQIndex.predictiveQIndex > max_q_index) {
            skip |= q_index_range_error(max_q_index_range_vuid,
                                        loc.dot(Field::maxQIndex).dot(Field::predictiveQIndex),
                                        rc_layer_info.maxQIndex.predictiveQIndex);
        }

        if (rc_layer_info.maxQIndex.bipredictiveQIndex < min_q_index ||
            rc_layer_info.maxQIndex.bipredictiveQIndex > max_q_index) {
            skip |= q_index_range_error(max_q_index_range_vuid,
                                        loc.dot(Field::maxQIndex).dot(Field::bipredictiveQIndex),
        ...
        if ((rc_layer_info.maxQIndex.intraQIndex != rc_layer_info.maxQIndex.predictiveQIndex ||
            ...
            !q_index_per_rc_group) {
            skip |= q_index_per_rc_group_error(max_q_index_per_rc_group_vuid,
                                               loc.dot(Field::maxQIndex),
                                               rc_layer_info.maxQIndex.intraQIndex,
                                               rc_layer_info.maxQIndex.predictiveQIndex,
                                               rc_layer_info.maxQIndex.bipredictiveQIndex);
        }
    ...
    if (rc_layer_info.useMinQIndex && rc_layer_info.useMaxQIndex) {
        if (rc_layer_info.minQIndex.intraQIndex > rc_layer_info.maxQIndex.intraQIndex) {
            skip |= min_max_q_index_compare_error(
                String(Field::intraQIndex), rc_layer_info.minQIndex.intraQIndex, rc_layer_info.maxQIndex.intraQIndex);
        }

        if (rc_layer_info.minQIndex.predictiveQIndex > rc_layer_info.maxQIndex.predictiveQIndex) {
            skip |= min_max_q_index_compare_error(String(Field::predictiveQIndex),
                                                  rc_layer_info.minQIndex.predictiveQIndex,
                                                  rc_layer_info.maxQIndex.predictiveQIndex);
        }

        if (rc_layer_info.minQIndex.bipredictiveQIndex > rc_layer_info.maxQIndex.bipredictiveQIndex) {
            skip |= min_max_q_index_compare_error(String(Field::bipredictiveQIndex),
                                                  rc_layer_info.minQIndex.bipredictiveQIndex,
                                                  rc_layer_info.maxQIndex.bipredictiveQIndex);
        }
    }
...
bool CoreChecks::ValidateVideoEncodeRateControlLayerInfoH264(uint32_t layer_index,
                                                             const VkVideoEncodeRateControlInfoKHR& rc_info,
                                                             const void* pNext,
                                                             VkCommandBuffer cmdbuf,
                                                             const vvl::VideoSession& vs_state,
    ...
    skip |= ValidateVideoEncodeRateControlH26xQp(
        ...
        *rc_layer_info_h264,
        ...
        profile_caps.encode_h264.minQp,
        profile_caps.encode_h264.maxQp,
        ...
        rc_layer_info_h264_loc);

    return skip;
}

bool CoreChecks::ValidateVideoEncodeRateControlLayerInfoH265(uint32_t layer_index,
                                                             const VkVideoEncodeRateControlInfoKHR& rc_info,
                                                             const void* pNext,
                                                             VkCommandBuffer cmdbuf,
                                                             const vvl::VideoSession& vs_state,
    ...
    skip |= ValidateVideoEncodeRateControlH26xQp(
        ...
        *rc_layer_info_h265,
        ...
        profile_caps.encode_h265.minQp,
        profile_caps.encode_h265.maxQp,
        ...
        rc_layer_info_h265_loc);

    return skip;
}

bool CoreChecks::ValidateVideoEncodeRateControlLayerInfoAV1(uint32_t layer_index,
                                                            const VkVideoEncodeRateControlInfoKHR& rc_info,
                                                            const void* pNext,
                                                            VkCommandBuffer cmdbuf,
                                                            const vvl::VideoSession& vs_state,
    ...
    skip |= ValidateVideoEncodeRateControlAV1QIndex(
        ...
        *rc_layer_info_av1,
        ...
        profile_caps.encode_av1.minQIndex,
        profile_caps.encode_av1.maxQIndex,
        ...
        rc_layer_info_av1_loc);

    return skip;
}

bool CoreChecks::ValidateVideoPictureResource(const vvl::VideoPictureResource& picture_resource,
                                              VkCommandBuffer cmdbuf,
                                              const vvl::VideoSession& vs_state,
                                              const Location& loc,
                                              const char* coded_offset_vuid,
    ...
    if (coded_offset_vuid) {
        ...
        if (!IsIntegerMultipleOf(picture_resource.coded_offset, offset_granularity)) {
            const LogObjectList objlist(cmdbuf, vs_state.Handle());
            skip |= LogError(coded_offset_vuid,
                             objlist,
                             loc.dot(Field::codedExtent),
                             "(%s) is not an integer multiple of the codedOffsetGranularity (%s).",
                             string_VkOffset2D(picture_resource.coded_offset).c_str(),
                             string_VkOffset2D(offset_granularity).c_str());
        }
    }

    if (coded_extent_vuid && !IsBetweenInclusive(picture_resource.coded_extent,
                                                 profile_caps.base.minCodedExtent,
                                                 vs_state.create_info.maxCodedExtent)) {
        const LogObjectList objlist(cmdbuf, vs_state.Handle());
        skip |= LogError(coded_extent_vuid,
                         objlist,
                         loc.dot(Field::codedExtent),
                         "(%s) is outside of the range (%s)-(%s) supported by %s.",
                         string_VkExtent2D(picture_resource.coded_extent).c_str(),
                         string_VkExtent2D(profile_caps.base.minCodedExtent).c_str(),
                         string_VkExtent2D(vs_state.create_info.maxCodedExtent).c_str(),
                         FormatHandle(vs_state).c_str());
    }

    if (picture_resource.base_array_layer >=
        picture_resource.image_view_state->create_info.subresourceRange.layerCount) {
        const LogObjectList objlist(cmdbuf,
                                    vs_state.Handle(),
                                    picture_resource.image_view_state->Handle(),
                                    picture_resource.image_state->Handle());
        skip |= LogError("VUID-VkVideoPictureResourceInfoKHR-baseArrayLayer-07175",
                         objlist,
                         loc.dot(Field::baseArrayLayer),
                         "(%u) is greater than or equal to the layerCount (%u) "
                         "the %s specified in imageViewBinding was created with.",
                         picture_resource.base_array_layer,
                         picture_resource.image_view_state->create_info.subresourceRange.layerCount,
...
template bool CoreChecks::ValidateVideoProfileInfo<VkDevice>(const VkVideoProfileInfoKHR* profile,
                                                             const VkDevice object,
                                                             const Location& loc) const;
template bool CoreChecks::ValidateVideoProfileInfo<VkPhysicalDevice>(const VkVideoProfileInfoKHR* profile,
                                                                     const VkPhysicalDevice object,
                                                                     const Location& loc) const;

template <typename HandleT>
bool CoreChecks::ValidateVideoProfileInfo(const VkVideoProfileInfoKHR* profile,
                                          const HandleT object,
                                          const Location& loc) const {
    bool skip = false;
    ...
    if (GetBitSetCount(profile->chromaSubsampling) != 1) {
        skip |= LogError("VUID-VkVideoProfileInfoKHR-chromaSubsampling-07013",
                         object,
                         loc.dot(Field::chromaSubsampling),
                         "must have a single bit set.");
    }

    if (GetBitSetCount(profile->lumaBitDepth) != 1) {
        skip |= LogError("VUID-VkVideoProfileInfoKHR-lumaBitDepth-07014",
                         object,
                         loc.dot(Field::lumaBitDepth),
                         "must have a single bit set.");
    }

    if (profile->chromaSubsampling != VK_VIDEO_CHROMA_SUBSAMPLING_MONOCHROME_BIT_KHR) {
        if (GetBitSetCount(profile->chromaBitDepth) != 1) {
            skip |= LogError("VUID-VkVideoProfileInfoKHR-chromaSubsampling-07015",
                             object,
                             loc.dot(Field::chromaBitDepth),
                             "must have a single bit set.");
        }
    }
    ...
    switch (profile->videoCodecOperation) {
        case VK_VIDEO_CODEC_OPERATION_DECODE_H264_BIT_KHR: {
            ...
            if (decode_h264 == nullptr) {
                skip |= LogError("VUID-VkVideoProfileInfoKHR-videoCodecOperation-07179",
                                 object,
                                 loc.dot(Field::pNext),
                                 profile_pnext_msg,
        ...
        case VK_VIDEO_CODEC_OPERATION_DECODE_H265_BIT_KHR: {
            ...
            if (decode_h265 == nullptr) {
                skip |= LogError("VUID-VkVideoProfileInfoKHR-videoCodecOperation-07180",
                                 object,
                                 loc.dot(Field::pNext),
                                 profile_pnext_msg,
        ...
        case VK_VIDEO_CODEC_OPERATION_DECODE_AV1_BIT_KHR: {
            ...
            if (decode_av1 == nullptr) {
                skip |= LogError("VUID-VkVideoProfileInfoKHR-videoCodecOperation-09256",
                                 object,
                                 loc.dot(Field::pNext),
                                 profile_pnext_msg,
        ...
        case VK_VIDEO_CODEC_OPERATION_ENCODE_H264_BIT_KHR: {
            ...
            if (encode_h264 == nullptr) {
                skip |= LogError("VUID-VkVideoProfileInfoKHR-videoCodecOperation-07181",
                                 object,
                                 loc.dot(Field::pNext),
                                 profile_pnext_msg,
        ...
        case VK_VIDEO_CODEC_OPERATION_ENCODE_H265_BIT_KHR: {
            ...
            if (encode_h265 == nullptr) {
                skip |= LogError("VUID-VkVideoProfileInfoKHR-videoCodecOperation-07182",
                                 object,
                                 loc.dot(Field::pNext),
                                 profile_pnext_msg,
        ...
        case VK_VIDEO_CODEC_OPERATION_ENCODE_AV1_BIT_KHR: {
            if constexpr (std::is_same_v<HandleT, VkDevice>) {
                if (!enabled_features.videoEncodeAV1) {
                    ...
                    skip |= LogError(vuid,
                                     object,
                                     loc.dot(Field::videoCodecOperation),
                                     codec_feature_not_enabled_msg,
                                     string_VkVideoCodecOperationFlagBitsKHR(profile->videoCodecOperation),
                                     "videoEncodeAV1");
                }
            }
            ...
            if (encode_av1 == nullptr) {
                skip |= LogError("VUID-VkVideoProfileInfoKHR-videoCodecOperation-10262",
                                 object,
                                 loc.dot(Field::pNext),
                                 profile_pnext_msg,
...
template bool CoreChecks::ValidateVideoProfileListInfo<VkDevice>(const VkVideoProfileListInfoKHR* profile_list,
                                                                 const VkDevice object,
                                                                 const Location& loc,
...
template bool
CoreChecks::ValidateVideoProfileListInfo<VkPhysicalDevice>(const VkVideoProfileListInfoKHR* profile_list,
                                                           const VkPhysicalDevice object,
                                                           const Location& loc,
                                                           bool expect_decode_profile,
                                                           const char* missing_decode_profile_msg_code,
...
template <typename HandleT>
bool CoreChecks::ValidateVideoProfileListInfo(const VkVideoProfileListInfoKHR* profile_list,
                                              const HandleT object,
                                              const Location& loc,
                                              bool expect_decode_profile,
                                              const char* missing_decode_profile_msg_code,
            ...
            switch (profile_list->pProfiles[i].videoCodecOperation) {
                ...
                case VK_VIDEO_CODEC_OPERATION_DECODE_AV1_BIT_KHR:
                    if (has_decode_profile) {
                        skip |= LogError("VUID-VkVideoProfileListInfoKHR-pProfiles-06813",
                                         object,
                                         loc,
    ...
    if (expect_decode_profile && !has_decode_profile) {
        skip |= LogError(missing_decode_profile_msg_code,
                         object,
                         loc.dot(Field::pProfiles),
                         "contains no video profile specifying a video decode operation.");
    }

    if (expect_encode_profile && !has_encode_profile) {
        skip |= LogError(missing_encode_profile_msg_code,
                         object,
                         loc.dot(Field::pProfiles),
...
bool CoreChecks::ValidateDecodeH264ParametersAddInfo(const vvl::VideoSession& vs_state,
                                                     const VkVideoDecodeH264SessionParametersAddInfoKHR* add_info,
                                                     VkDevice device,
                                                     const Location& loc,
    ...
    if (add_info) {
        ...
        for (uint32_t i = 0; i < add_info->stdSPSCount; ++i) {
            ...
            if (!keys.emplace(key).second) {
                skip |= LogError("VUID-VkVideoDecodeH264SessionParametersAddInfoKHR-None-04825",
                                 device,
                                 loc.dot(Field::pStdSPSs),
                                 "keys are not unique.");
                break;
            }
        }
    ...
    if (add_info) {
        ...
        for (uint32_t i = 0; i < add_info->stdPPSCount; ++i) {
            ...
            if (!keys.emplace(key).second) {
                skip |= LogError("VUID-VkVideoDecodeH264SessionParametersAddInfoKHR-None-04826",
                                 device,
                                 loc.dot(Field::pStdPPSs),
                                 "keys are not unique.");
                break;
            }
        }
...
bool CoreChecks::ValidateDecodeH265ParametersAddInfo(const vvl::VideoSession& vs_state,
                                                     const VkVideoDecodeH265SessionParametersAddInfoKHR* add_info,
                                                     VkDevice device,
                                                     const Location& loc,
    ...
    if (add_info) {
        ...
        for (uint32_t i = 0; i < add_info->stdVPSCount; ++i) {
            ...
            if (!keys.emplace(key).second) {
                skip |= LogError("VUID-VkVideoDecodeH265SessionParametersAddInfoKHR-None-04833",
                                 device,
                                 loc.dot(Field::pStdVPSs),
                                 "keys are not unique.");
                break;
            }
        }
    ...
    if (add_info) {
        ...
        for (uint32_t i = 0; i < add_info->stdSPSCount; ++i) {
            ...
            if (!keys.emplace(key).second) {
                skip |= LogError("VUID-VkVideoDecodeH265SessionParametersAddInfoKHR-None-04834",
                                 device,
                                 loc.dot(Field::pStdSPSs),
                                 "keys are not unique.");
                break;
    ...
    if (create_info) {
        ...
        if (keys.size() > create_info->maxStdSPSCount) {
            skip |= LogError("VUID-VkVideoSessionParametersCreateInfoKHR-videoSession-07208",
                             device,
                             loc.function,
    ...
    if (add_info) {
        ...
        for (uint32_t i = 0; i < add_info->stdPPSCount; ++i) {
            ...
            if (!keys.emplace(key).second) {
                skip |= LogError("VUID-VkVideoDecodeH265SessionParametersAddInfoKHR-None-04835",
                                 device,
                                 loc.dot(Field::pStdPPSs),
                                 "keys are not unique.");
                break;
    ...
    if (create_info) {
        ...
        if (keys.size() > create_info->maxStdPPSCount) {
            skip |= LogError("VUID-VkVideoSessionParametersCreateInfoKHR-videoSession-07209",
                             device,
                             loc.function,
...
bool CoreChecks::ValidateEncodeH264ParametersAddInfo(const vvl::VideoSession& vs_state,
                                                     const VkVideoEncodeH264SessionParametersAddInfoKHR* add_info,
                                                     VkDevice device,
                                                     const Location& loc,
    ...
    if (add_info) {
        ...
        for (uint32_t i = 0; i < add_info->stdSPSCount; ++i) {
            auto key = vvl::VideoSessionParameters::GetH264SPSKey(add_info->pStdSPSs[i]);
            if (!keys.emplace(key).second) {
                skip |= LogError("VUID-VkVideoEncodeH264SessionParametersAddInfoKHR-None-04837",
                                 device,
                                 loc.dot(Field::pStdSPSs),
                                 "keys are not unique.");
                break;
    ...
    if (create_info) {
        ...
        if (keys.size() > create_info->maxStdSPSCount) {
            skip |= LogError("VUID-VkVideoSessionParametersCreateInfoKHR-videoSession-04839",
                             device,
                             loc.function,
                             "number of H.264 SPS entries to add (%zu) is larger than "
    ...
    if (add_info) {
        ...
        for (uint32_t i = 0; i < add_info->stdPPSCount; ++i) {
            auto key = vvl::VideoSessionParameters::GetH264PPSKey(add_info->pStdPPSs[i]);
            if (!keys.emplace(key).second) {
                skip |= LogError("VUID-VkVideoEncodeH264SessionParametersAddInfoKHR-None-04838",
                                 device,
                                 loc.dot(Field::pStdPPSs),
                                 "keys are not unique.");
                break;
    ...
    if (create_info) {
        ...
        if (keys.size() > create_info->maxStdPPSCount) {
            skip |= LogError("VUID-VkVideoSessionParametersCreateInfoKHR-videoSession-04840",
                             device,
                             loc.function,
                             "number of H.264 PPS entries to add (%zu) is larger than "
...
bool CoreChecks::ValidateEncodeH265ParametersAddInfo(const vvl::VideoSession& vs_state,
                                                     const VkVideoEncodeH265SessionParametersAddInfoKHR* add_info,
                                                     VkDevice device,
                                                     const Location& loc,
    ...
    if (add_info) {
        ...
        for (uint32_t i = 0; i < add_info->stdVPSCount; ++i) {
            auto key = vvl::VideoSessionParameters::GetH265VPSKey(add_info->pStdVPSs[i]);
            if (!keys.emplace(key).second) {
                skip |= LogError("VUID-VkVideoEncodeH265SessionParametersAddInfoKHR-None-06438",
                                 device,
                                 loc.dot(Field::pStdVPSs),
                                 "keys are not unique.");
                break;
    ...
    if (create_info) {
        ...
        if (keys.size() > create_info->maxStdVPSCount) {
            skip |= LogError("VUID-VkVideoSessionParametersCreateInfoKHR-videoSession-04841",
                             device,
                             loc.function,
                             "number of H.265 VPS entries to add (%zu) is larger than "
    ...
    if (add_info) {
        ...
        for (uint32_t i = 0; i < add_info->stdSPSCount; ++i) {
            auto key = vvl::VideoSessionParameters::GetH265SPSKey(add_info->pStdSPSs[i]);
            if (!keys.emplace(key).second) {
                skip |= LogError("VUID-VkVideoEncodeH265SessionParametersAddInfoKHR-None-06439",
                                 device,
                                 loc.dot(Field::pStdSPSs),
                                 "keys are not unique.");
                break;
    ...
    if (create_info) {
        ...
        if (keys.size() > create_info->maxStdSPSCount) {
            skip |= LogError("VUID-VkVideoSessionParametersCreateInfoKHR-videoSession-04842",
                             device,
                             loc.function,
                             "number of H.265 SPS entries to add (%zu) is larger than "
    ...
    if (add_info) {
        ...
        for (uint32_t i = 0; i < add_info->stdPPSCount; ++i) {
            auto key = vvl::VideoSessionParameters::GetH265PPSKey(add_info->pStdPPSs[i]);
            if (!keys.emplace(key).second) {
                skip |= LogError("VUID-VkVideoEncodeH265SessionParametersAddInfoKHR-None-06440",
                                 device,
                                 loc.dot(Field::pStdPPSs),
                                 "keys are not unique.");
                break;
    ...
    if (create_info) {
        ...
        if (keys.size() > create_info->maxStdPPSCount) {
            skip |= LogError("VUID-VkVideoSessionParametersCreateInfoKHR-videoSession-04843",
                             device,
                             loc.function,
                             "number of H.265 PPS entries to add (%zu) is larger than "
        ...
        for (uint32_t i = 0; i < add_info->stdPPSCount; ++i) {
            if (add_info->pStdPPSs[i].num_tile_columns_minus1 >= profile_caps.encode_h265.maxTiles.width) {
                ...
                skip |= LogError(vuid,
                                 ...
                                 loc.dot(Field::pStdPPSs, i).Fields().c_str(),
                                 add_info->pStdPPSs[i].num_tile_columns_minus1,
                                 profile_caps.encode_h265.maxTiles.width,
                                 FormatHandle(vs_state).c_str());
            }
            if (add_info->pStdPPSs[i].num_tile_rows_minus1 >= profile_caps.encode_h265.maxTiles.height) {
                ...
                skip |= LogError(vuid,
                                 ...
                                 "supported by the H.265 encode profile %s was created with.",
                                 loc.dot(Field::pStdPPSs, i).Fields().c_str(),
                                 add_info->pStdPPSs[i].num_tile_rows_minus1,
                                 profile_caps.encode_h265.maxTiles.height,
                                 FormatHandle(vs_state).c_str());
            }
        }
    }
...
bool CoreChecks::ValidateEncodeQuantizationMapParametersCreateInfo(
    const vvl::VideoSession& vs_state,
    const VkVideoEncodeQuantizationMapSessionParametersCreateInfoKHR& quantization_map_info,
    VkDevice device,
    const Location& loc,
    const vvl::VideoSessionParameters* template_state) const {
    bool skip = false;
    ...
    if (template_state != nullptr &&
        ...
        skip |= LogError("VUID-VkVideoSessionParametersCreateInfoKHR-videoSessionParametersTemplate-10276",
                         objlist,
                         loc.dot(Field::quantizationMapTexelSize),
...
bool CoreChecks::ValidateDecodeDistinctOutput(const vvl::CommandBuffer& cb_state,
        ...
        switch (vs_state.GetCodecOp()) {
            case VK_VIDEO_CODEC_OPERATION_DECODE_AV1_BIT_KHR: {
                ...
                if (!vs_state.profile->HasAV1FilmGrainSupport()) {
                    skip |= LogError("VUID-vkCmdDecodeVideoKHR-pDecodeInfo-07141",
                                     objlist,
                                     cmd_loc,
...
bool CoreChecks::ValidateVideoDecodeInfoH264(const vvl::CommandBuffer& cb_state,
                                             const VkVideoDecodeInfoKHR& decode_info,
                                             const Location& loc) const {
    bool skip = false;
    ...
    if (picture_info) {
        ...
        if (!interlaced_frame_support && std_picture_info->flags.field_pic_flag) {
            const LogObjectList objlist(cb_state.Handle(), vs_state.Handle());
            skip |= LogError("VUID-vkCmdDecodeVideoKHR-None-07258",
                             objlist,
                             loc.function,
        ...
        for (uint32_t i = 0; i < picture_info->sliceCount; ++i) {
            if (picture_info->pSliceOffsets[i] >= decode_info.srcBufferRange) {
                skip |= LogError("VUID-vkCmdDecodeVideoKHR-pSliceOffsets-07153",
                                 cb_state.Handle(),
        ...
        if (session_params.GetH264SPS(std_picture_info->seq_parameter_set_id) == nullptr) {
            ...
            skip |= LogError("VUID-vkCmdDecodeVideoKHR-StdVideoH264SequenceParameterSet-07154",
                             objlist,
                             loc.function,
        ...
        if (session_params.GetH264PPS(std_picture_info->seq_parameter_set_id, std_picture_info->pic_parameter_set_id) ==
            nullptr) {
            const LogObjectList objlist(cb_state.Handle(), vsp_state.Handle());
            skip |= LogError("VUID-vkCmdDecodeVideoKHR-StdVideoH264PictureParameterSet-07155",
                             objlist,
                             loc.function,
                             "no H.264 PPS with seq_parameter_set_id = %u "
                             "and pic_parameter_set_id = %u exists in the bound video session parameters object %s.",
                             std_picture_info->seq_parameter_set_id,
                             std_picture_info->pic_parameter_set_id,
                             FormatHandle(vsp_state).c_str());
        }
    } else {
        skip |= LogError("VUID-vkCmdDecodeVideoKHR-pNext-07152",
                         cb_state.Handle(),
                         loc.dot(Field::pNext),
                         pnext_msg,
    ...
    if (decode_info.pSetupReferenceSlot) {
        auto dpb_slot_info =
            vku::FindStructInPNextChain<VkVideoDecodeH264DpbSlotInfoKHR>(decode_info.pSetupReferenceSlot->pNext);
        if (dpb_slot_info) {
            ...
            if (!interlaced_frame_support && !picture_id.IsFrame()) {
                const LogObjectList objlist(cb_state.Handle(), vs_state.Handle());
                skip |= LogError("VUID-vkCmdDecodeVideoKHR-pDecodeInfo-07259",
                                 objlist,
                                 loc.function,
            ...
            if (picture_info) {
                ...
                if (!dst_is_field && !picture_id.IsFrame()) {
                    skip |= LogError("VUID-vkCmdDecodeVideoKHR-pDecodeInfo-07261",
                                     cb_state.Handle(),
                                     loc.function,
                ...
                if (dst_is_field && !dst_is_bottom_field && !picture_id.IsTopField()) {
                    skip |= LogError("VUID-vkCmdDecodeVideoKHR-pDecodeInfo-07262",
                                     cb_state.Handle(),
                                     loc.function,
                ...
                if (dst_is_field && dst_is_bottom_field && !picture_id.IsBottomField()) {
                    skip |= LogError("VUID-vkCmdDecodeVideoKHR-pDecodeInfo-07263",
                                     cb_state.Handle(),
                                     loc.function,
            ...
            skip |= LogError("VUID-vkCmdDecodeVideoKHR-pDecodeInfo-07156",
                             cb_state.Handle(),
                             loc.dot(Field::pSetupReferenceSlot).dot(Field::pNext),
                             pnext_msg,
    ...
    for (uint32_t i = 0; i < decode_info.referenceSlotCount; ++i) {
        auto dpb_slot_info =
            vku::FindStructInPNextChain<VkVideoDecodeH264DpbSlotInfoKHR>(decode_info.pReferenceSlots[i].pNext);
        if (dpb_slot_info) {
            vvl::VideoPictureID picture_id(*vs_state.profile, decode_info.pReferenceSlots[i]);
            if (!interlaced_frame_support && !picture_id.IsFrame()) {
                const LogObjectList objlist(cb_state.Handle(), vs_state.Handle());
                skip |= LogError("VUID-vkCmdDecodeVideoKHR-pDecodeInfo-07260",
                                 objlist,
                                 loc.function,
            ...
            skip |= LogError("VUID-vkCmdDecodeVideoKHR-pNext-07157",
                             cb_state.Handle(),
                             loc.dot(Field::pReferenceSlots, i).dot(Field::pNext),
                             pnext_msg,
                             "VkVideoDecodeH264DpbSlotInfoKHR");
        }
    }
...
bool CoreChecks::ValidateVideoDecodeInfoH265(const vvl::CommandBuffer& cb_state,
                                             const VkVideoDecodeInfoKHR& decode_info,
                                             const Location& loc) const {
    bool skip = false;
    ...
    if (picture_info) {
        ...
        for (uint32_t i = 0; i < picture_info->sliceSegmentCount; ++i) {
            if (picture_info->pSliceSegmentOffsets[i] >= decode_info.srcBufferRange) {
                skip |= LogError("VUID-vkCmdDecodeVideoKHR-pSliceSegmentOffsets-07159",
                                 cb_state.Handle(),
                                 loc.pNext(Struct::VkVideoDecodeH265PictureInfoKHR, Field::pSliceSegmentOffsets, i),
                                 "(%u) is greater than or equal to pDecodeInfo->srcBufferRange (%" PRIu64 ").",
                                 picture_info->pSliceSegmentOffsets[i],
                                 decode_info.srcBufferRange);
            }
        }
        ...
        if (session_params.GetH265VPS(std_picture_info->sps_video_parameter_set_id) == nullptr) {
            const LogObjectList objlist(cb_state.Handle(), vsp_state.Handle());
            skip |= LogError("VUID-vkCmdDecodeVideoKHR-StdVideoH265VideoParameterSet-07160",
                             objlist,
                             loc.function,
        ...
        if (session_params.GetH265SPS(std_picture_info->sps_video_parameter_set_id,
                                      std_picture_info->pps_seq_parameter_set_id) == nullptr) {
            const LogObjectList objlist(cb_state.Handle(), vsp_state.Handle());
            skip |= LogError("VUID-vkCmdDecodeVideoKHR-StdVideoH265SequenceParameterSet-07161",
                             objlist,
                             loc.function,
                             ...
                             "parameters object %s.",
                             std_picture_info->sps_video_parameter_set_id,
                             std_picture_info->pps_seq_parameter_set_id,
                             FormatHandle(vsp_state).c_str());
        }

        if (session_params.GetH265PPS(std_picture_info->sps_video_parameter_set_id,
                                      std_picture_info->pps_seq_parameter_set_id,
                                      std_picture_info->pps_pic_parameter_set_id) == nullptr) {
            const LogObjectList objlist(cb_state.Handle(), vsp_state.Handle());
            skip |= LogError("VUID-vkCmdDecodeVideoKHR-StdVideoH265PictureParameterSet-07162",
                             objlist,
                             loc.function,
                             "no H.265 SPS with sps_video_parameter_set_id = %u, "
        ...
        skip |= LogError("VUID-vkCmdDecodeVideoKHR-pNext-07158",
                         cb_state.Handle(),
                         loc.dot(Field::pNext),
                         pnext_msg,
    ...
    if (decode_info.pSetupReferenceSlot) {
        auto dpb_slot_info =
            vku::FindStructInPNextChain<VkVideoDecodeH265DpbSlotInfoKHR>(decode_info.pSetupReferenceSlot->pNext);
        if (!dpb_slot_info) {
            skip |= LogError("VUID-vkCmdDecodeVideoKHR-pDecodeInfo-07163",
                             cb_state.Handle(),
                             loc.dot(Field::pSetupReferenceSlot).dot(Field::pNext),
                             pnext_msg,
    ...
    for (uint32_t i = 0; i < decode_info.referenceSlotCount; ++i) {
        auto dpb_slot_info =
            vku::FindStructInPNextChain<VkVideoDecodeH265DpbSlotInfoKHR>(decode_info.pReferenceSlots[i].pNext);
        if (!dpb_slot_info) {
            skip |= LogError("VUID-vkCmdDecodeVideoKHR-pNext-07164",
                             cb_state.Handle(),
                             loc.dot(Field::pReferenceSlots, i).dot(Field::pNext),
                             pnext_msg,
                             "VkVideoDecodeH265DpbSlotInfoKHR");
...
bool CoreChecks::ValidateVideoDecodeInfoAV1(const vvl::CommandBuffer& cb_state,
                                            const VkVideoDecodeInfoKHR& decode_info,
                                            const Location& loc) const {
    bool skip = false;
    ...
    if (decode_info.pSetupReferenceSlot) {
        auto dpb_slot_info =
            vku::FindStructInPNextChain<VkVideoDecodeAV1DpbSlotInfoKHR>(decode_info.pSetupReferenceSlot->pNext);
        if (!dpb_slot_info) {
            skip |= LogError("VUID-vkCmdDecodeVideoKHR-pDecodeInfo-09254",
                             cb_state.Handle(),
                             loc.dot(Field::pSetupReferenceSlot).dot(Field::pNext),
                             pnext_msg,
    ...
    for (uint32_t i = 0; i < decode_info.referenceSlotCount; ++i) {
        auto dpb_slot_info =
            vku::FindStructInPNextChain<VkVideoDecodeAV1DpbSlotInfoKHR>(decode_info.pReferenceSlots[i].pNext);
        if (!dpb_slot_info) {
            skip |= LogError("VUID-vkCmdDecodeVideoKHR-pNext-09255",
                             cb_state.Handle(),
                             loc.dot(Field::pReferenceSlots, i).dot(Field::pNext),
                             pnext_msg,
                             "VkVideoDecodeAV1DpbSlotInfoKHR");
    ...
    if (picture_info) {
        ...
        if (std_picture_info->flags.apply_grain) {
            if (!vs_state.profile->HasAV1FilmGrainSupport()) {
                skip |= LogError("VUID-vkCmdDecodeVideoKHR-filmGrainSupport-09248",
                                 cb_state.Handle(),
            ...
            if (decode_info.pSetupReferenceSlot != nullptr &&
                decode_info.pSetupReferenceSlot->pPictureResource != nullptr) {
                auto dst_resource = vvl::VideoPictureResource(*this, decode_info.dstPictureResource);
                auto setup_resource =
                    vvl::VideoPictureResource(*this, *decode_info.pSetupReferenceSlot->pPictureResource);
                if (dst_resource == setup_resource) {
                    skip |= LogError("VUID-vkCmdDecodeVideoKHR-pDecodeInfo-09249",
                                     cb_state.Handle(),
                                     loc.pNext(Struct::VkVideoDecodeAV1PictureInfoKHR, Field::pStdPictureInfo),
        ...
        for (uint32_t i = 0; i < VK_MAX_VIDEO_AV1_REFERENCES_PER_FRAME_KHR; ++i) {
            if (picture_info->referenceNameSlotIndices[i] >= 0) {
                if (reference_slot_indices.find(picture_info->referenceNameSlotIndices[i]) ==
                    reference_slot_indices.end()) {
                    skip |= LogError(
        ...
        for (uint32_t i = 0; i < decode_info.referenceSlotCount; ++i) {
            if (reference_name_slot_indices.find(decode_info.pReferenceSlots[i].slotIndex) ==
                reference_name_slot_indices.end()) {
                skip |= LogError("VUID-vkCmdDecodeVideoKHR-slotIndex-09263",
                                 cb_state.Handle(),
...
bool CoreChecks::ValidateVideoEncodeH264PicType(const vvl::VideoSession& vs_state,
    ...
    if (profile_caps.encode_h264.maxPPictureL0ReferenceCount == 0 && pic_type == STD_VIDEO_H264_PICTURE_TYPE_P) {
        skip |= LogError("VUID-vkCmdEncodeVideoKHR-maxPPictureL0ReferenceCount-08340",
                         vs_state.Handle(),
                         loc,
    ...
    if (profile_caps.encode_h264.maxBPictureL0ReferenceCount == 0 &&
        profile_caps.encode_h264.maxL1ReferenceCount == 0 && pic_type == STD_VIDEO_H264_PICTURE_TYPE_B) {
        skip |= LogError("VUID-vkCmdEncodeVideoKHR-maxBPictureL0ReferenceCount-08341",
                         vs_state.Handle(),
                         loc,
...
bool CoreChecks::ValidateVideoEncodeInfoH264(const vvl::CommandBuffer& cb_state,
    ...
    if (encode_info.pSetupReferenceSlot) {
        auto dpb_slot_info =
            vku::FindStructInPNextChain<VkVideoEncodeH264DpbSlotInfoKHR>(encode_info.pSetupReferenceSlot->pNext);
        if (!dpb_slot_info) {
            skip |= LogError("VUID-vkCmdEncodeVideoKHR-pEncodeInfo-08228",
                             cb_state.Handle(),
                             loc.dot(Field::pSetupReferenceSlot).dot(Field::pNext),
                             pnext_msg,
    ...
    for (uint32_t i = 0; i < encode_info.referenceSlotCount; ++i) {
        auto dpb_slot_info =
            vku::FindStructInPNextChain<VkVideoEncodeH264DpbSlotInfoKHR>(encode_info.pReferenceSlots[i].pNext);
        if (!dpb_slot_info) {
            skip |= LogError("VUID-vkCmdEncodeVideoKHR-pNext-08229",
                             cb_state.Handle(),
                             loc.dot(Field::pReferenceSlots, i).dot(Field::pNext),
                             pnext_msg,
                             "VkVideoEncodeH264DpbSlotInfoKHR");
    ...
    if (picture_info) {
        ...
        auto std_pps =
            session_params.GetH264PPS(std_picture_info->seq_parameter_set_id, std_picture_info->pic_parameter_set_id);
        ...
        if (std_sps == nullptr) {
            const LogObjectList objlist(cb_state.Handle(), vsp_state.Handle());
            skip |= LogError("VUID-vkCmdEncodeVideoKHR-StdVideoH264SequenceParameterSet-08226",
                             objlist,
                             loc.function,
                             "no H.264 SPS with seq_parameter_set_id = %u "
                             "exists in the bound video session parameters object %s.",
        ...
        if (std_pps == nullptr) {
            const LogObjectList objlist(cb_state.Handle(), vsp_state.Handle());
            skip |= LogError("VUID-vkCmdEncodeVideoKHR-StdVideoH264PictureParameterSet-08227",
                             objlist,
                             loc.function,
                             "no H.264 PPS with seq_parameter_set_id = %u "
                             "and pic_parameter_set_id = %u exists in the bound video session parameters object %s.",
        ...
        if (picture_info->naluSliceEntryCount > profile_caps.encode_h264.maxSliceCount) {
            ...
            skip |= LogError(
                ...
                profile_caps.encode_h264.maxSliceCount,
                FormatHandle(vs_state).c_str());
        }
        ...
        if (profile_caps.encode_h264.flags & VK_VIDEO_ENCODE_H264_CAPABILITY_ROW_UNALIGNED_SLICE_BIT_KHR) {
            if (picture_info->naluSliceEntryCount > min_coding_block_extent.width * min_coding_block_extent.height) {
                skip |= LogError("VUID-vkCmdEncodeVideoKHR-naluSliceEntryCount-08302",
                                 cb_state.Handle(),
                                 slice_count_loc,
                                 ...
                                 picture_info->naluSliceEntryCount,
                                 string_VkExtent2D(min_coding_block_extent).c_str(),
            ...
            if (picture_info->naluSliceEntryCount > min_coding_block_extent.height) {
                skip |= LogError("VUID-vkCmdEncodeVideoKHR-naluSliceEntryCount-08312",
                                 cb_state.Handle(),
                                 slice_count_loc,
                                 ...
                                 "pEncodeInfo->srcPictureResource (codedExtent = {%s}).",
                                 picture_info->naluSliceEntryCount,
                                 min_coding_block_extent.height,
                                 string_VkExtent2D(encode_info.srcPictureResource.codedExtent).c_str());
            }
        }
        ...
        for (uint32_t slice_idx = 0; slice_idx < picture_info->naluSliceEntryCount; ++slice_idx) {
            ...
            const Location slice_info_loc =
                loc.pNext(Struct::VkVideoEncodeH264PictureInfoKHR, Field::pNaluSliceEntries, slice_idx);
            ...
            if (rc_state.base.rateControlMode == VK_VIDEO_ENCODE_RATE_CONTROL_MODE_DISABLED_BIT_KHR) {
                if (slice_info.constantQp < profile_caps.encode_h264.minQp ||
                    ...
                    const LogObjectList objlist(cb_state.Handle(), vs_state.Handle());
                    skip |= LogError("VUID-vkCmdEncodeVideoKHR-constantQp-08270",
                                     objlist,
                                     slice_info_loc.dot(Field::constantQp),
                                     "(%d) is outside of the range [%d, %d] supported by the video "
                                     "profile %s was created with.",
                                     slice_info.constantQp,
                                     profile_caps.encode_h264.minQp,
                                     profile_caps.encode_h264.maxQp,
                                     FormatHandle(vs_state).c_str());
                }
                ...
                if (slice_info.constantQp != 0) {
                    const LogObjectList objlist(cb_state.Handle(), vs_state.Handle());
                    skip |= LogError("VUID-vkCmdEncodeVideoKHR-constantQp-08269",
                                     objlist,
                                     slice_info_loc.dot(Field::constantQp),
            ...
            if (std_pps != nullptr &&
                (profile_caps.encode_h264.flags &
                ...
                if (std_slice_header->pWeightTable == nullptr && weighted_pred_error_msg != nullptr) {
                    const LogObjectList objlist(cb_state.Handle(), vs_state.Handle());
                    skip |= LogError("VUID-VkVideoEncodeH264PictureInfoKHR-flags-08314",
                                     objlist,
                                     loc.function,
                                     ...
                                     "is not supported by the H.264 encode profile %s was created with.",
                                     weighted_pred_error_msg,
                                     slice_info_loc.dot(Field::pStdSliceHeader).Fields().c_str(),
                                     FormatHandle(vs_state).c_str());
                }
            }
        }
        ...
        if ((profile_caps.encode_h264.flags & VK_VIDEO_ENCODE_H264_CAPABILITY_DIFFERENT_SLICE_TYPE_BIT_KHR) == 0 &&
            ...
            const LogObjectList objlist(cb_state.Handle(), vs_state.Handle());
            skip |= LogError("VUID-VkVideoEncodeH264PictureInfoKHR-flags-08315",
                             objlist,
                             loc.function,
        ...
        if ((profile_caps.encode_h264.flags & VK_VIDEO_ENCODE_H264_CAPABILITY_PER_SLICE_CONSTANT_QP_BIT_KHR) == 0 &&
            ...
            const LogObjectList objlist(cb_state.Handle(), vs_state.Handle());
            skip |= LogError("VUID-vkCmdEncodeVideoKHR-constantQp-08271",
                             objlist,
                             loc.function,
        ...
        skip |= ValidateVideoEncodeH264PicType(vs_state,
                                               std_picture_info->primary_pic_type,
                                               loc.function,
        ...
        if (std_picture_info->pRefLists != nullptr) {
            ...
            for (uint8_t i = 0; i < STD_VIDEO_H264_MAX_NUM_LIST_REF; ++i) {
                ...
                if (ref_slot != reference_slots.end()) {
                    if (ref_slot->second != nullptr) {
                        ...
                        skip |= ValidateVideoEncodeH264PicType(vs_state,
                                                               std_reference_info->primary_pic_type,
                                                               loc.function,
                                                               "primary_pic_type for L0 reference");

                        if (std_reference_info->primary_pic_type == STD_VIDEO_H264_PICTURE_TYPE_B &&
                            (profile_caps.encode_h264.flags &
                             VK_VIDEO_ENCODE_H264_CAPABILITY_B_FRAME_IN_L0_LIST_BIT_KHR) == 0) {
                            const LogObjectList objlist(cb_state.Handle(), vs_state.Handle());
                            skip |= LogError("VUID-vkCmdEncodeVideoKHR-flags-08342",
                                             objlist,
                                             loc.function,
            ...
            for (uint8_t i = 0; i < STD_VIDEO_H264_MAX_NUM_LIST_REF; ++i) {
                ...
                if (ref_slot != reference_slots.end()) {
                    if (ref_slot->second != nullptr) {
                        ...
                        if (std_reference_info->primary_pic_type == STD_VIDEO_H264_PICTURE_TYPE_B &&
                            (profile_caps.encode_h264.flags &
                             VK_VIDEO_ENCODE_H264_CAPABILITY_B_FRAME_IN_L1_LIST_BIT_KHR) == 0) {
                            const LogObjectList objlist(cb_state.Handle(), vs_state.Handle());
                            skip |= LogError("VUID-vkCmdEncodeVideoKHR-flags-08343",
                                             objlist,
                                             loc.function,
            ...
            skip |=
                LogError("VUID-vkCmdEncodeVideoKHR-pNext-08352",
                         cb_state.Handle(),
                         loc.function,
        ...
        if (picture_info->generatePrefixNalu &&
            ...
            const LogObjectList objlist(cb_state.Handle(), vs_state.Handle());
            skip |= LogError("VUID-VkVideoEncodeH264PictureInfoKHR-flags-08304",
                             objlist,
        ...
        skip |= LogError("VUID-vkCmdEncodeVideoKHR-pNext-08225",
                         cb_state.Handle(),
                         loc.dot(Field::pNext),
                         pnext_msg,
...
bool CoreChecks::ValidateVideoEncodeH265PicType(const vvl::VideoSession& vs_state,
    ...
    if (profile_caps.encode_h265.maxPPictureL0ReferenceCount == 0 && pic_type == STD_VIDEO_H265_PICTURE_TYPE_P) {
        skip |= LogError("VUID-vkCmdEncodeVideoKHR-maxPPictureL0ReferenceCount-08345",
                         vs_state.Handle(),
                         loc,
    ...
    if (profile_caps.encode_h265.maxBPictureL0ReferenceCount == 0 &&
        profile_caps.encode_h265.maxL1ReferenceCount == 0 && pic_type == STD_VIDEO_H265_PICTURE_TYPE_B) {
        skip |= LogError("VUID-vkCmdEncodeVideoKHR-maxBPictureL0ReferenceCount-08346",
                         vs_state.Handle(),
                         loc,
...
bool CoreChecks::ValidateVideoEncodeInfoH265(const vvl::CommandBuffer& cb_state,
                                             const VkVideoEncodeInfoKHR& encode_info,
                                             const Location& loc) const {
    bool skip = false;
    ...
    if (encode_info.pSetupReferenceSlot) {
        auto dpb_slot_info =
            vku::FindStructInPNextChain<VkVideoEncodeH265DpbSlotInfoKHR>(encode_info.pSetupReferenceSlot->pNext);
        if (!dpb_slot_info) {
            skip |= LogError("VUID-vkCmdEncodeVideoKHR-pEncodeInfo-08234",
                             cb_state.Handle(),
                             loc.dot(Field::pSetupReferenceSlot).dot(Field::pNext),
                             pnext_msg,
    ...
    for (uint32_t i = 0; i < encode_info.referenceSlotCount; ++i) {
        auto dpb_slot_info =
            vku::FindStructInPNextChain<VkVideoEncodeH265DpbSlotInfoKHR>(encode_info.pReferenceSlots[i].pNext);
        if (!dpb_slot_info) {
            skip |= LogError("VUID-vkCmdEncodeVideoKHR-pNext-08235",
                             cb_state.Handle(),
                             loc.dot(Field::pReferenceSlots, i).dot(Field::pNext),
                             pnext_msg,
                             "VkVideoEncodeH265DpbSlotInfoKHR");
    ...
    if (picture_info) {
        ...
        auto std_sps = session_params.GetH265SPS(std_picture_info->sps_video_parameter_set_id,
                                                 std_picture_info->pps_seq_parameter_set_id);
        auto std_pps = session_params.GetH265PPS(std_picture_info->sps_video_parameter_set_id,
                                                 std_picture_info->pps_seq_parameter_set_id,
                                                 std_picture_info->pps_pic_parameter_set_id);

        const Location slice_seg_count_loc =
            loc.pNext(Struct::VkVideoEncodeH265PictureInfoKHR, Field::naluSliceSegmentEntryCount);
        const Location slice_seg_list_loc =
            loc.pNext(Struct::VkVideoEncodeH265PictureInfoKHR, Field::pNaluSliceSegmentEntries);

        if (std_vps == nullptr) {
            const LogObjectList objlist(cb_state.Handle(), vsp_state.Handle());
            skip |= LogError("VUID-vkCmdEncodeVideoKHR-StdVideoH265VideoParameterSet-08231",
                             objlist,
                             loc.function,
                             "no H.265 VPS with sps_video_parameter_set_id = %u "
                             "exists in the bound video session parameters object %s.",
        ...
        if (std_sps == nullptr) {
            const LogObjectList objlist(cb_state.Handle(), vsp_state.Handle());
            skip |= LogError("VUID-vkCmdEncodeVideoKHR-StdVideoH265SequenceParameterSet-08232",
                             objlist,
                             loc.function,
                             "no H.265 SPS with sps_video_parameter_set_id = %u "
                             "and pps_seq_parameter_set_id = %u exists in the bound video session "
        ...
        if (std_pps == nullptr) {
            const LogObjectList objlist(cb_state.Handle(), vsp_state.Handle());
            skip |= LogError("VUID-vkCmdEncodeVideoKHR-StdVideoH265PictureParameterSet-08233",
                             objlist,
                             loc.function,
                             "no H.265 SPS with sps_video_parameter_set_id = %u, "
                             "pps_seq_parameter_set_id = %u, and pps_pic_parameter_set_id = %u exists in "
        ...
        if (picture_info->naluSliceSegmentEntryCount > profile_caps.encode_h265.maxSliceSegmentCount) {
            const LogObjectList objlist(cb_state.Handle(), vs_state.Handle());
            skip |= LogError("VUID-VkVideoEncodeH265PictureInfoKHR-naluSliceSegmentEntryCount-08306",
                             objlist,
                             slice_seg_count_loc,
                             "(%u) exceeds the maxSliceSegmentCount (%u) limit supported by the H.265 encode profile "
                             "%s was created with.",
                             picture_info->naluSliceSegmentEntryCount,
                             profile_caps.encode_h265.maxSliceSegmentCount,
                             FormatHandle(vs_state).c_str());
        }
        ...
        if (profile_caps.encode_h265.flags & VK_VIDEO_ENCODE_H265_CAPABILITY_ROW_UNALIGNED_SLICE_SEGMENT_BIT_KHR) {
            if (picture_info->naluSliceSegmentEntryCount >
                min_coding_block_extent.width * min_coding_block_extent.height) {
                const LogObjectList objlist(cb_state.Handle(), vs_state.Handle());
                skip |= LogError("VUID-vkCmdEncodeVideoKHR-naluSliceSegmentEntryCount-08307",
                                 objlist,
                                 slice_seg_count_loc,
            ...
            if (picture_info->naluSliceSegmentEntryCount > min_coding_block_extent.height) {
                const LogObjectList objlist(cb_state.Handle(), vs_state.Handle());
                skip |=
                    LogError("VUID-vkCmdEncodeVideoKHR-naluSliceSegmentEntryCount-08313",
                             objlist,
                             slice_seg_count_loc,
        ...
        if (std_pps != nullptr) {
            ...
            if ((profile_caps.encode_h265.flags &
                ...
                const LogObjectList objlist(cb_state.Handle(), vs_state.Handle());
                skip |= LogError("VUID-VkVideoEncodeH265PictureInfoKHR-flags-08323",
                                 objlist,
                                 slice_seg_count_loc,
                                 ...
                                 picture_info->naluSliceSegmentEntryCount,
                                 num_tiles,
                                 std_pps->num_tile_columns_minus1,
                                 std_pps->num_tile_rows_minus1,
                                 FormatHandle(vs_state).c_str());
            }
            ...
            if ((profile_caps.encode_h265.flags &
                ...
                picture_info->naluSliceSegmentEntryCount > num_tiles) {
                const LogObjectList objlist(cb_state.Handle(), vs_state.Handle());
                skip |= LogError("VUID-VkVideoEncodeH265PictureInfoKHR-flags-08324",
                                 objlist,
                                 slice_seg_count_loc,
        ...
        for (uint32_t slice_seg_idx = 0; slice_seg_idx < picture_info->naluSliceSegmentEntryCount; ++slice_seg_idx) {
            ...
            if (rc_state.base.rateControlMode == VK_VIDEO_ENCODE_RATE_CONTROL_MODE_DISABLED_BIT_KHR) {
                if (slice_segment_info.constantQp < profile_caps.encode_h265.minQp ||
                    ...
                    const LogObjectList objlist(cb_state.Handle(), vs_state.Handle());
                    skip |= LogError("VUID-vkCmdEncodeVideoKHR-constantQp-08273",
                                     objlist,
                                     slice_seg_info_loc.dot(Field::constantQp),
                                     "(%d) is outside of the range [%d, %d] supported by the video "
                                     "profile %s was created with.",
                                     slice_segment_info.constantQp,
                                     profile_caps.encode_h265.minQp,
                                     profile_caps.encode_h265.maxQp,
                                     FormatHandle(vs_state).c_str());
                }
                ...
                if (slice_segment_info.constantQp != 0) {
                    const LogObjectList objlist(cb_state.Handle(), vs_state.Handle());
                    skip |= LogError("VUID-vkCmdEncodeVideoKHR-constantQp-08272",
                                     objlist,
                                     slice_seg_info_loc.dot(Field::constantQp),
                                     "(%d) is not zero but the currently set video encode rate control mode for %s "
                                     "was specified to be %s when beginning the video coding scope.",
            ...
            if (std_pps != nullptr &&
                (profile_caps.encode_h265.flags &
                 VK_VIDEO_ENCODE_H265_CAPABILITY_PREDICTION_WEIGHT_TABLE_GENERATED_BIT_KHR) == 0) {
                const char* weighted_pred_error_msg = nullptr;

                if (std_slice_segment_header->slice_type == STD_VIDEO_H265_SLICE_TYPE_P &&
                    std_pps->flags.weighted_pred_flag) {
                    weighted_pred_error_msg =
                    ...
                    weighted_pred_error_msg = "weighted_bipred_flag is set in the active H.265 PPS, slice_type is "
                                              "STD_VIDEO_H265_SLICE_TYPE_B";
                }

                if (std_slice_segment_header->pWeightTable == nullptr && weighted_pred_error_msg != nullptr) {
                    const LogObjectList objlist(cb_state.Handle(), vs_state.Handle());
                    skip |= LogError("VUID-VkVideoEncodeH265PictureInfoKHR-flags-08316",
                                     objlist,
                                     loc.function,
                                     "%s, and pWeightTable is NULL in %s but "
                                     ...
                                     "is not supported by the H.265 encode profile %s was created with.",
                                     weighted_pred_error_msg,
                                     slice_seg_info_loc.dot(Field::pStdSliceSegmentHeader).Fields().c_str(),
                                     FormatHandle(vs_state).c_str());
                }
            }
        }

        if ((profile_caps.encode_h265.flags & VK_VIDEO_ENCODE_H265_CAPABILITY_DIFFERENT_SLICE_SEGMENT_TYPE_BIT_KHR) ==
                0 &&
            different_slice_segment_types) {
            const LogObjectList objlist(cb_state.Handle(), vs_state.Handle());
            skip |=
                LogError("VUID-VkVideoEncodeH265PictureInfoKHR-flags-08317",
                         objlist,
                         loc.function,
                         ...
                         "profile %s was created with.",
                         slice_seg_list_loc.Fields().c_str(),
                         FormatHandle(vs_state).c_str());
        }

        if ((profile_caps.encode_h265.flags & VK_VIDEO_ENCODE_H265_CAPABILITY_PER_SLICE_SEGMENT_CONSTANT_QP_BIT_KHR) ==
                0 &&
            different_constant_qp_per_slice_segment) {
            const LogObjectList objlist(cb_state.Handle(), vs_state.Handle());
            skip |= LogError("VUID-vkCmdEncodeVideoKHR-constantQp-08274",
                             objlist,
                             loc.function,
                             "constantQp does not match across the elements of %s "
        ...
        skip |= ValidateVideoEncodeH265PicType(vs_state,
                                               std_picture_info->pic_type,
                                               loc.function,
        ...
        if (std_picture_info->pRefLists != nullptr) {
            ...
            for (uint8_t i = 0; i < STD_VIDEO_H265_MAX_NUM_LIST_REF; ++i) {
                ...
                if (ref_slot != reference_slots.end()) {
                    if (ref_slot->second != nullptr) {
                        ...
                        skip |= ValidateVideoEncodeH265PicType(
                            vs_state, std_reference_info->pic_type, loc.function, "pic_type for L0 reference");

                        if (std_reference_info->pic_type == STD_VIDEO_H265_PICTURE_TYPE_B &&
                            (profile_caps.encode_h265.flags &
                             VK_VIDEO_ENCODE_H265_CAPABILITY_B_FRAME_IN_L0_LIST_BIT_KHR) == 0) {
                            const LogObjectList objlist(cb_state.Handle(), vs_state.Handle());
                            skip |= LogError("VUID-vkCmdEncodeVideoKHR-flags-08347",
                                             objlist,
                                             loc.function,
            ...
            for (uint8_t i = 0; i < STD_VIDEO_H265_MAX_NUM_LIST_REF; ++i) {
                ...
                if (ref_slot != reference_slots.end()) {
                    if (ref_slot->second != nullptr) {
                        ...
                        skip |= ValidateVideoEncodeH265PicType(
                            vs_state, std_reference_info->pic_type, loc.function, "pic_type for L1 reference");

                        if (std_reference_info->pic_type == STD_VIDEO_H265_PICTURE_TYPE_B &&
                            (profile_caps.encode_h265.flags &
                             VK_VIDEO_ENCODE_H265_CAPABILITY_B_FRAME_IN_L1_LIST_BIT_KHR) == 0) {
                            const LogObjectList objlist(cb_state.Handle(), vs_state.Handle());
                            skip |= LogError("VUID-vkCmdEncodeVideoKHR-flags-08348",
                                             objlist,
                                             loc.function,
            ...
            skip |=
                LogError("VUID-vkCmdEncodeVideoKHR-pNext-08354",
                         cb_state.Handle(),
                         loc.function,
                         "%s->pRefLists is NULL but pEncodeInfo->referenceSlotCount (%u) is not zero.",
                         loc.pNext(Struct::VkVideoEncodeH265PictureInfoKHR, Field::pStdPictureInfo).Fields().c_str(),
        ...
        skip |= LogError("VUID-vkCmdEncodeVideoKHR-pNext-08230",
                         cb_state.Handle(),
                         loc.dot(Field::pNext),
                         pnext_msg,
...
bool CoreChecks::ValidateVideoEncodeInfoAV1(const vvl::CommandBuffer& cb_state,
                                            const VkVideoEncodeInfoKHR& encode_info,
                                            const Location& loc) const {
    bool skip = false;
    ...
    if (encode_info.pSetupReferenceSlot) {
        setup_dpb_slot_info =
            vku::FindStructInPNextChain<VkVideoEncodeAV1DpbSlotInfoKHR>(encode_info.pSetupReferenceSlot->pNext);
        if (!setup_dpb_slot_info) {
            skip |= LogError("VUID-vkCmdEncodeVideoKHR-pEncodeInfo-10318",
                             cb_state.Handle(),
                             loc.dot(Field::pSetupReferenceSlot).dot(Field::pNext),
                             pnext_msg,
    ...
    for (uint32_t i = 0; i < encode_info.referenceSlotCount; ++i) {
        auto dpb_slot_info =
            vku::FindStructInPNextChain<VkVideoEncodeAV1DpbSlotInfoKHR>(encode_info.pReferenceSlots[i].pNext);
        if (dpb_slot_info) {
            if (dpb_slot_info->pStdReferenceInfo->pExtensionHeader != nullptr) {
                auto reference_loc = loc.dot(Field::pReferenceSlots, i);
                auto std_reference_info_loc =
                    reference_loc.pNext(Struct::VkVideoEncodeAV1DpbSlotInfoKHR, Field::pStdReferenceInfo);
                ...
                if (av1_obu_ext_header->temporal_id >= profile_caps.encode_av1.maxTemporalLayerCount) {
                    skip |= LogError(
                        ...
                        av1_obu_ext_header->temporal_id,
                        profile_caps.encode_av1.maxTemporalLayerCount,
                        FormatHandle(vs_state).c_str());
                }
                ...
                if (av1_obu_ext_header->spatial_id >= profile_caps.encode_av1.maxSpatialLayerCount) {
                    skip |= LogError(
                        ...
                        "limit supported by the AV1 encode profile the bound video session %s was created with.",
                        av1_obu_ext_header->spatial_id,
                        profile_caps.encode_av1.maxSpatialLayerCount,
                        FormatHandle(vs_state).c_str());
                }
            }
        } else {
            skip |= LogError("VUID-vkCmdEncodeVideoKHR-pNext-10319",
                             cb_state.Handle(),
                             loc.dot(Field::pReferenceSlots, i).dot(Field::pNext),
                             pnext_msg,
                             "VkVideoEncodeAV1DpbSlotInfoKHR");
    ...
    if ((profile_caps.encode_av1.flags & VK_VIDEO_ENCODE_AV1_CAPABILITY_FRAME_SIZE_OVERRIDE_BIT_KHR) == 0) {
        if (encode_info.srcPictureResource.codedExtent.width != std_seq_header->max_frame_width_minus_1 + 1u) {
            const LogObjectList objlist(cb_state.Handle(), vs_state.Handle());
            skip |= LogError("VUID-vkCmdEncodeVideoKHR-flags-10323",
                             objlist,
        ...
        if (encode_info.srcPictureResource.codedExtent.height != std_seq_header->max_frame_height_minus_1 + 1u) {
            ...
            skip |= LogError("VUID-vkCmdEncodeVideoKHR-flags-10324",
                             ...
                             "VK_VIDEO_ENCODE_AV1_CAPABILITY_FRAME_SIZE_OVERRIDE_BIT_KHR is not supported "
                             "by the AV1 encode profile the bound video session %s was created with.",
                             encode_info.srcPictureResource.codedExtent.height,
                             std_seq_header->max_frame_height_minus_1,
                             FormatHandle(vs_state).c_str());
        }
    }
    ...
    if ((profile_caps.encode_av1.flags & VK_VIDEO_ENCODE_AV1_CAPABILITY_MOTION_VECTOR_SCALING_BIT_KHR) == 0) {
        for (uint32_t i = 0; i < encode_info.referenceSlotCount; ++i) {
            ...
            if (reference_resource != nullptr &&
                ...
                const LogObjectList objlist(cb_state.Handle(), vs_state.Handle());
                skip |=
                    LogError("VUID-vkCmdEncodeVideoKHR-flags-10325",
                             objlist,
                             ...
                             "by the AV1 encode profile the bound video session %s was created with.",
                             reference_resource->codedExtent.width,
                             reference_resource->codedExtent.height,
    ...
    if (picture_info) {
        ...
        if (rc_state.base.rateControlMode == VK_VIDEO_ENCODE_RATE_CONTROL_MODE_DISABLED_BIT_KHR) {
            ...
            if (picture_info->constantQIndex != 0) {
                const LogObjectList objlist(cb_state.Handle(), vs_state.Handle());
                skip |= LogError("VUID-vkCmdEncodeVideoKHR-constantQIndex-10320",
                                 objlist,
                                 loc.pNext(Struct::VkVideoEncodeAV1PictureInfoKHR, Field::constantQIndex),
        ...
        if (std_picture_info->primary_ref_frame < VK_MAX_VIDEO_AV1_REFERENCES_PER_FRAME_KHR &&
            ...
            skip |= LogError("VUID-VkVideoEncodeAV1PictureInfoKHR-pStdPictureInfo-10291",
                             cb_state.Handle(),
        ...
        if (picture_info->primaryReferenceCdfOnly == VK_TRUE) {
            if ((profile_caps.encode_av1.flags & VK_VIDEO_ENCODE_AV1_CAPABILITY_PRIMARY_REFERENCE_CDF_ONLY_BIT_KHR) ==
                0) {
                skip |=
                    LogError("VUID-VkVideoEncodeAV1PictureInfoKHR-flags-10289",
                             cb_state.Handle(),
        ...
        if (picture_info->generateObuExtensionHeader == VK_TRUE) {
            if ((profile_caps.encode_av1.flags &
                 VK_VIDEO_ENCODE_AV1_CAPABILITY_GENERATE_OBU_EXTENSION_HEADER_BIT_KHR) == 0) {
                skip |=
                    LogError("VUID-VkVideoEncodeAV1PictureInfoKHR-flags-10292",
                             cb_state.Handle(),
            ...
            if (std_picture_info->pExtensionHeader == nullptr) {
                skip |= LogError("VUID-VkVideoEncodeAV1PictureInfoKHR-generateObuExtensionHeader-10293",
                                 cb_state.Handle(),
                                 loc.pNext(Struct::VkVideoEncodeAV1PictureInfoKHR, Field::generateObuExtensionHeader),
                                 "is VK_TRUE but no OBU extension header information is specified in "
                                 "pStdPictureInfo->pExtensionHeader.");
            }
        }
        ...
        if (picture_info->predictionMode == VK_VIDEO_ENCODE_AV1_PREDICTION_MODE_INTRA_ONLY_KHR &&
            ...
            skip |= LogError("VUID-vkCmdEncodeVideoKHR-predictionMode-10326",
                             cb_state.Handle(),
        ...
        switch (picture_info->predictionMode) {
            case VK_VIDEO_ENCODE_AV1_PREDICTION_MODE_SINGLE_REFERENCE_KHR: {
                if (profile_caps.encode_av1.maxSingleReferenceCount == 0) {
                    skip |= LogError("VUID-vkCmdEncodeVideoKHR-maxSingleReferenceCount-10328",
                                     cb_state.Handle(),
                                     loc.pNext(Struct::VkVideoEncodeAV1PictureInfoKHR, Field::predictionMode),
            ...
            case VK_VIDEO_ENCODE_AV1_PREDICTION_MODE_UNIDIRECTIONAL_COMPOUND_KHR: {
                if (profile_caps.encode_av1.maxUnidirectionalCompoundReferenceCount == 0) {
                    skip |= LogError("VUID-vkCmdEncodeVideoKHR-maxUnidirectionalCompoundReferenceCount-10330",
                                     cb_state.Handle(),
                                     loc.pNext(Struct::VkVideoEncodeAV1PictureInfoKHR, Field::predictionMode),
            ...
            case VK_VIDEO_ENCODE_AV1_PREDICTION_MODE_BIDIRECTIONAL_COMPOUND_KHR: {
                if (profile_caps.encode_av1.maxBidirectionalCompoundReferenceCount == 0) {
                    skip |= LogError("VUID-vkCmdEncodeVideoKHR-maxBidirectionalCompoundReferenceCount-10332",
                                     cb_state.Handle(),
                                     loc.pNext(Struct::VkVideoEncodeAV1PictureInfoKHR, Field::predictionMode),
                ...
                for (uint32_t i = 0; i < VK_MAX_VIDEO_AV1_REFERENCES_PER_FRAME_KHR; ++i) {
                    if (i != cdf_only_ref_index && picture_info->referenceNameSlotIndices[i] >= 0 &&
                        ...
                        if (i < STD_VIDEO_AV1_REFERENCE_NAME_BWDREF_FRAME - 1) {
                            ...
                            if (has_supported_group2_reference_name)
                                break;
                        } else {
                            has_supported_group2_reference_name = true;
                            if (has_supported_group1_reference_name)
                                break;
                        }
                    }
                }
        ...
        for (uint32_t i = 0; i < encode_info.referenceSlotCount; ++i) {
            if (reference_name_slot_indices.find(encode_info.pReferenceSlots[i].slotIndex) ==
                reference_name_slot_indices.end()) {
                skip |= LogError("VUID-vkCmdEncodeVideoKHR-slotIndex-10335",
                                 cb_state.Handle(),
                                 loc.dot(Field::pReferenceSlots, i),
                                 "(%d) does not match any of the elements of "
        ...
        if (std_picture_info->pExtensionHeader != nullptr) {
            if (std_picture_info->pExtensionHeader->temporal_id >= profile_caps.encode_av1.maxTemporalLayerCount) {
                skip |=
                    LogError("VUID-vkCmdEncodeVideoKHR-pExtensionHeader-10336",
                             cb_state.Handle(),
                             std_picture_info_loc,
                             "pExtensionHeader->temporal_id (%u) is greater than or equal to the maxTemporalLayer (%u) "
                             "limit supported by the AV1 encode profile the bound video session %s was created with.",
                             std_picture_info->pExtensionHeader->temporal_id,
                             profile_caps.encode_av1.maxTemporalLayerCount,
                             FormatHandle(vs_state).c_str());
            }
            ...
            if (std_picture_info->pExtensionHeader->spatial_id >= profile_caps.encode_av1.maxSpatialLayerCount) {
                skip |= LogError(
                    ...
                    "pExtensionHeader->spatial_id (%u) is greater than or equal to the maxSpatialLayerCount (%u) "
                    "limit supported by the AV1 encode profile the bound video session %s was created with.",
                    std_picture_info->pExtensionHeader->spatial_id,
                    profile_caps.encode_av1.maxSpatialLayerCount,
                    FormatHandle(vs_state).c_str());
            }
            ...
            if (setup_dpb_slot_info != nullptr) {
                if (setup_dpb_slot_info->pStdReferenceInfo->pExtensionHeader != nullptr) {
                    ...
                    if (std_picture_info->pExtensionHeader->spatial_id != av1_setup_obu_ext_header->spatial_id) {
                        skip |= LogError("VUID-vkCmdEncodeVideoKHR-pEncodeInfo-10340",
                                         cb_state.Handle(),
                                         std_picture_info_loc,
                                         "pExtensionHeader->spatial_id (%u) does not match the "
                                         "pExtensionHeader->spatial_id (%u) specified in %s.",
                                         std_picture_info->pExtensionHeader->spatial_id,
                                         av1_setup_obu_ext_header->spatial_id,
                                         std_setup_reference_info_loc.Fields().c_str());
                    }
                } else {
                    skip |= LogError("VUID-vkCmdEncodeVideoKHR-pEncodeInfo-10338",
                                     cb_state.Handle(),
                                     std_picture_info_loc,
        ...
        } else if (setup_dpb_slot_info != nullptr &&
                   setup_dpb_slot_info->pStdReferenceInfo->pExtensionHeader != nullptr) {
            skip |= LogError("VUID-vkCmdEncodeVideoKHR-pEncodeInfo-10338",
                             cb_state.Handle(),
                             std_setup_reference_info_loc,
                             "includes AV1 OBU extension header information (pExtensionHeader != NULL) but "
        ...
        if (std_picture_info->pTileInfo != nullptr) {
            auto tile_size = [](uint32_t extent, uint32_t tile_count) {
                return (extent + tile_count - 1) / tile_count;
            };
            ...
            if (std_picture_info->pTileInfo->TileCols != 0) {
                if (std_picture_info->pTileInfo->TileCols > profile_caps.encode_av1.maxTiles.width) {
                    skip |= LogError(
                        ...
                        std_picture_info->pTileInfo->TileCols,
                        profile_caps.encode_av1.maxTiles.width,
                        FormatHandle(vs_state).c_str());
                }
                ...
                if (tile_width < profile_caps.encode_av1.minTileSize.width ||
                    ...
                    skip |= LogError(
                        ...
                        cb_state.Handle(),
                        std_picture_info_loc,
                        "effective tile width (%u) calculated as "
                        "ceil(pEncodeInfo->srcPictureResource.codedExtent.width [%u] / "
                        "pTileInfo->TileCols [%u]) must be between the minTileSize.width (%u) and maxTileSize.width "
                        "(%u) "
                        "limits supported by the AV1 encode profile the bound video session %s was created with.",
                        tile_width,
                        encode_info.srcPictureResource.codedExtent.width,
                        std_picture_info->pTileInfo->TileCols,
                        profile_caps.encode_av1.minTileSize.width,
                        profile_caps.encode_av1.maxTileSize.width,
                        FormatHandle(vs_state).c_str());
                }
            } else {
                skip |= LogError(
            ...
            if (std_picture_info->pTileInfo->TileRows != 0) {
                if (std_picture_info->pTileInfo->TileRows > profile_caps.encode_av1.maxTiles.height) {
                    skip |= LogError(
                        ...
                        "bound video session %s was created with.",
                        std_picture_info->pTileInfo->TileRows,
                        profile_caps.encode_av1.maxTiles.height,
                        FormatHandle(vs_state).c_str());
                }
                ...
                if (tile_height < profile_caps.encode_av1.minTileSize.height ||
                    ...
                    skip |= LogError(
                        ...
                        cb_state.Handle(),
                        std_picture_info_loc,
                        "effective tile height (%u) calculated as "
                        "ceil(pEncodeInfo->srcPictureResource.codedExtent.height [%u] / "
                        "pTileInfo->TileRows [%u]) must be between the minTileSize.height (%u) and maxTileSize.height "
                        "(%u) "
                        "limits supported by the AV1 encode profile the bound video session %s was created with.",
                        tile_height,
                        encode_info.srcPictureResource.codedExtent.height,
                        std_picture_info->pTileInfo->TileRows,
                        profile_caps.encode_av1.minTileSize.height,
                        profile_caps.encode_av1.maxTileSize.height,
                        FormatHandle(vs_state).c_str());
                }
            } else {
                skip |= LogError(
        ...
        if ((profile_caps.encode_av1.flags & VK_VIDEO_ENCODE_AV1_CAPABILITY_FRAME_SIZE_OVERRIDE_BIT_KHR) == 0 &&
            ...
            const LogObjectList objlist(cb_state.Handle(), vs_state.Handle());
            skip |= LogError("VUID-vkCmdEncodeVideoKHR-flags-10322",
                             objlist,
                             std_picture_info_loc,
        ...
        if (std_picture_info->flags.segmentation_enabled != 0) {
            skip |=
                LogError("VUID-vkCmdEncodeVideoKHR-pStdPictureInfo-10349",
                         cb_state.Handle(),
                         std_picture_info_loc,
        ...
        if (std_picture_info->pSegmentation != nullptr) {
            skip |= LogError("VUID-vkCmdEncodeVideoKHR-pStdPictureInfo-10350",
                             cb_state.Handle(),
                             std_picture_info_loc,
                             "AV1 encoding with segmentation is not supported but pSegmentation is not NULL.");
        }
    } else {
        skip |= LogError("VUID-vkCmdEncodeVideoKHR-pNext-10317",
                         cb_state.Handle(),
                         loc.dot(Field::pNext),
                         pnext_msg,
...
bool CoreChecks::ValidateVideoEncodeQuantizationMapInfo(
    ...
    if (vsp_state->create_info.flags & VK_VIDEO_SESSION_PARAMETERS_CREATE_QUANTIZATION_MAP_COMPATIBLE_BIT_KHR) {
        ...
        if (quantization_map_info.quantizationMapExtent.width != expected_extent.width ||
            ...
            const LogObjectList objlist(cb_state.Handle(), vsp_state->Handle());
            skip |= LogError("VUID-vkCmdEncodeVideoKHR-pNext-10316",
                             objlist,
                             loc.dot(Field::quantizationMapExtent),
                             ...
                             string_VkExtent2D(coded_extent).c_str(),
                             string_VkExtent2D(texel_size).c_str(),
    ...
    if (iv_state) {
        ...
        if (iv_extent.width < quantization_map_info.quantizationMapExtent.width) {
            ...
            skip |= LogError("VUID-VkVideoEncodeQuantizationMapInfoKHR-quantizationMapExtent-10352",
                             objlist,
                             ...
                             iv_extent.width,
                             loc.dot(Field::quantizationMap).Fields().c_str(),
                             FormatHandle(iv_state->Handle()).c_str(),
                             FormatHandle(iv_state->image_state->Handle()).c_str());
        }

        if (iv_extent.height < quantization_map_info.quantizationMapExtent.height) {
            const LogObjectList objlist(cb_state.Handle(), iv_state->Handle(), iv_state->image_state->Handle());
            skip |= LogError("VUID-VkVideoEncodeQuantizationMapInfoKHR-quantizationMapExtent-10353",
                             objlist,
        ...
        skip |= ValidateProtectedImage(
            cb_state, *iv_state->image_state, loc.dot(Field::quantizationMap), "VUID-vkCmdEncodeVideoKHR-pNext-10313");

        skip |= VerifyImageLayout(cb_state,
                                  *iv_state->image_state,
                                  iv_state->normalized_subresource_range,
                                  VK_IMAGE_LAYOUT_VIDEO_ENCODE_QUANTIZATION_MAP_KHR,
                                  loc.dot(Field::quantizationMap),
        ...
        if (!IsImageCompatibleWithVideoSession(*iv_state->image_state, *vs_state)) {
            ...
            skip |= LogError("VUID-vkCmdEncodeVideoKHR-pEncodeInfo-10310",
                             objlist,
                             loc.dot(Field::quantizationMap),
...
bool CoreChecks::ValidateActiveReferencePictureCount(const vvl::CommandBuffer& cb_state,
    ...
    if (active_reference_picture_count > vs_state.create_info.maxActiveReferencePictures) {
        const LogObjectList objlist(cb_state.Handle(), vs_state.Handle());
        skip |= LogError("VUID-vkCmdDecodeVideoKHR-activeReferencePictureCount-07150",
                         objlist,
                         loc,
                         ...
                         active_reference_picture_count,
                         vs_state.create_info.maxActiveReferencePictures,
                         FormatHandle(vs_state).c_str());
    }
...
bool CoreChecks::ValidateActiveReferencePictureCount(const vvl::CommandBuffer& cb_state,
    ...
    if (active_reference_picture_count > vs_state.create_info.maxActiveReferencePictures) {
        const LogObjectList objlist(cb_state.Handle(), vs_state.Handle());
        skip |= LogError("VUID-vkCmdEncodeVideoKHR-activeReferencePictureCount-08216",
                         objlist,
                         loc,
                         "more active reference pictures (%u) were specified than "
                         "the maxActiveReferencePictures (%u) the bound video session %s was created with.",
...
bool CoreChecks::ValidateReferencePictureUseCount(const vvl::CommandBuffer& cb_state,
    ...
    for (uint32_t i = 0; i < vs_state.create_info.maxDpbSlots; ++i) {
        if (dpb_frame_use_count[i] > 1) {
            skip |= LogError("VUID-vkCmdDecodeVideoKHR-dpbFrameUseCount-07176",
                             cb_state.Handle(),
                             loc,
        ...
        if (interlaced_frame_support) {
            if (dpb_top_field_use_count[i] > 1) {
                skip |= LogError("VUID-vkCmdDecodeVideoKHR-dpbTopFieldUseCount-07177",
                                 cb_state.Handle(),
                                 loc,
            ...
            if (dpb_bottom_field_use_count[i] > 1) {
                skip |= LogError("VUID-vkCmdDecodeVideoKHR-dpbBottomFieldUseCount-07178",
                                 cb_state.Handle(),
                                 loc,
...
bool CoreChecks::ValidateReferencePictureUseCount(const vvl::CommandBuffer& cb_state,
    ...
    for (uint32_t i = 0; i < vs_state.create_info.maxDpbSlots; ++i) {
        if (dpb_frame_use_count[i] > 1) {
            skip |= LogError("VUID-vkCmdEncodeVideoKHR-dpbFrameUseCount-08221",
                             cb_state.Handle(),
                             loc,
                             "frame in DPB slot %u is referred to multiple times across "
...
bool CoreChecks::PreCallValidateGetPhysicalDeviceVideoCapabilitiesKHR(VkPhysicalDevice physicalDevice,
    ...
    switch (pVideoProfile->videoCodecOperation) {
        case VK_VIDEO_CODEC_OPERATION_DECODE_H264_BIT_KHR:
            ...
            if (!vku::FindStructInPNextChain<VkVideoDecodeH264CapabilitiesKHR>(pCapabilities->pNext)) {
                skip |= LogError("VUID-vkGetPhysicalDeviceVideoCapabilitiesKHR-pVideoProfile-07184",
                                 physicalDevice,
                                 caps_loc,
                                 caps_pnext_msg,
        ...
        case VK_VIDEO_CODEC_OPERATION_DECODE_H265_BIT_KHR:
            ...
            if (!vku::FindStructInPNextChain<VkVideoDecodeH265CapabilitiesKHR>(pCapabilities->pNext)) {
                skip |= LogError("VUID-vkGetPhysicalDeviceVideoCapabilitiesKHR-pVideoProfile-07185",
                                 physicalDevice,
                                 caps_loc,
                                 caps_pnext_msg,
        ...
        case VK_VIDEO_CODEC_OPERATION_DECODE_AV1_BIT_KHR:
            ...
            if (!vku::FindStructInPNextChain<VkVideoDecodeAV1CapabilitiesKHR>(pCapabilities->pNext)) {
                skip |= LogError("VUID-vkGetPhysicalDeviceVideoCapabilitiesKHR-pVideoProfile-09257",
                                 physicalDevice,
                                 caps_loc,
                                 caps_pnext_msg,
        ...
        case VK_VIDEO_CODEC_OPERATION_ENCODE_H264_BIT_KHR:
            ...
            if (!vku::FindStructInPNextChain<VkVideoEncodeH264CapabilitiesKHR>(pCapabilities->pNext)) {
                skip |= LogError("VUID-vkGetPhysicalDeviceVideoCapabilitiesKHR-pVideoProfile-07187",
                                 physicalDevice,
                                 caps_loc,
                                 caps_pnext_msg,
        ...
        case VK_VIDEO_CODEC_OPERATION_ENCODE_H265_BIT_KHR:
            ...
            if (!vku::FindStructInPNextChain<VkVideoEncodeH265CapabilitiesKHR>(pCapabilities->pNext)) {
                skip |= LogError("VUID-vkGetPhysicalDeviceVideoCapabilitiesKHR-pVideoProfile-07188",
                                 physicalDevice,
                                 caps_loc,
                                 caps_pnext_msg,
        ...
        case VK_VIDEO_CODEC_OPERATION_ENCODE_AV1_BIT_KHR:
            ...
            if (!vku::FindStructInPNextChain<VkVideoEncodeAV1CapabilitiesKHR>(pCapabilities->pNext)) {
                skip |= LogError("VUID-vkGetPhysicalDeviceVideoCapabilitiesKHR-pVideoProfile-10263",
                                 physicalDevice,
                                 caps_loc,
                                 caps_pnext_msg,
    ...
    if (is_decode && !vku::FindStructInPNextChain<VkVideoDecodeCapabilitiesKHR>(pCapabilities->pNext)) {
        skip |= LogError("VUID-vkGetPhysicalDeviceVideoCapabilitiesKHR-pVideoProfile-07183",
                         physicalDevice,
                         caps_loc,
                         caps_pnext_msg,
    ...
    if (is_encode && !vku::FindStructInPNextChain<VkVideoEncodeCapabilitiesKHR>(pCapabilities->pNext)) {
        skip |= LogError("VUID-vkGetPhysicalDeviceVideoCapabilitiesKHR-pVideoProfile-07186",
                         physicalDevice,
                         caps_loc,
                         caps_pnext_msg,
...
bool CoreChecks::PreCallValidateGetPhysicalDeviceVideoFormatPropertiesKHR(
    ...
    if (video_profiles && video_profiles->profileCount != 0) {
        skip |= ValidateVideoProfileListInfo(
            video_profiles,
            physicalDevice,
        ...
        skip |= LogError("VUID-vkGetPhysicalDeviceVideoFormatPropertiesKHR-pNext-06812",
                         physicalDevice,
                         error_obj.location,
                         "%s",
                         msg);
...
bool CoreChecks::PreCallValidateGetPhysicalDeviceVideoEncodeQualityLevelPropertiesKHR(
    ...
    skip |= ValidateVideoProfileInfo(
        pQualityLevelInfo->pVideoProfile, physicalDevice, quality_level_info_loc.dot(Field::pVideoProfile));
    ...
    if (!profile_desc.IsEncode()) {
        skip |= LogError("VUID-VkPhysicalDeviceVideoEncodeQualityLevelInfoKHR-pVideoProfile-08260",
                         physicalDevice,
                         quality_level_info_loc.dot(Field::pVideoProfile),
                         "does not specify an encode profile.");
    }

    if (profile_caps.supported) {
        if (profile_desc.IsEncode() && pQualityLevelInfo->qualityLevel >= profile_caps.encode.maxQualityLevels) {
            skip |= LogError("VUID-VkPhysicalDeviceVideoEncodeQualityLevelInfoKHR-qualityLevel-08261",
                             physicalDevice,
                             ...
                             pQualityLevelInfo->qualityLevel,
                             profile_caps.encode.maxQualityLevels);
        }
    } else {
        skip |= LogError("VUID-VkPhysicalDeviceVideoEncodeQualityLevelInfoKHR-pVideoProfile-08259",
                         physicalDevice,
                         quality_level_info_loc.dot(Field::pVideoProfile),
                         "is not supported.");
    }
    ...
    switch (pQualityLevelInfo->pVideoProfile->videoCodecOperation) {
        case VK_VIDEO_CODEC_OPERATION_ENCODE_H264_BIT_KHR:
            if (!vku::FindStructInPNextChain<VkVideoEncodeH264QualityLevelPropertiesKHR>(
                    pQualityLevelProperties->pNext)) {
                skip |= LogError("VUID-vkGetPhysicalDeviceVideoEncodeQualityLevelPropertiesKHR-pQualityLevelInfo-08257",
                                 physicalDevice,
                                 quality_level_props_loc,
                                 props_pnext_msg,
        ...
        case VK_VIDEO_CODEC_OPERATION_ENCODE_H265_BIT_KHR:
            if (!vku::FindStructInPNextChain<VkVideoEncodeH265QualityLevelPropertiesKHR>(
                    pQualityLevelProperties->pNext)) {
                skip |= LogError("VUID-vkGetPhysicalDeviceVideoEncodeQualityLevelPropertiesKHR-pQualityLevelInfo-08258",
                                 physicalDevice,
                                 quality_level_props_loc,
                                 props_pnext_msg,
                                 "VkVideoEncodeH264QualityLevelPropertiesKHR");
        ...
        case VK_VIDEO_CODEC_OPERATION_ENCODE_AV1_BIT_KHR:
            if (!vku::FindStructInPNextChain<VkVideoEncodeAV1QualityLevelPropertiesKHR>(
                    pQualityLevelProperties->pNext)) {
                skip |= LogError("VUID-vkGetPhysicalDeviceVideoEncodeQualityLevelPropertiesKHR-pQualityLevelInfo-10305",
                                 physicalDevice,
                                 quality_level_props_loc,
                                 props_pnext_msg,
...
bool CoreChecks::PreCallValidateCreateVideoSessionKHR(VkDevice device,
    ...
    if (profile_caps.supported) {
        if (pCreateInfo->flags & VK_VIDEO_SESSION_CREATE_PROTECTED_CONTENT_BIT_KHR) {
            ...
            if (error_msg != nullptr) {
                skip |= LogError("VUID-VkVideoSessionCreateInfoKHR-protectedMemory-07189",
                                 device,
                                 create_info_loc.dot(Field::flags),
                                 "has VK_VIDEO_SESSION_CREATE_PROTECTED_CONTENT_BIT_KHR set but %s.",
                                 error_msg);
            }
        }

        if (pCreateInfo->flags & VK_VIDEO_SESSION_CREATE_INLINE_QUERIES_BIT_KHR &&
            !enabled_features.videoMaintenance1) {
            skip |= LogError("VUID-VkVideoSessionCreateInfoKHR-flags-08371",
                             device,
                             create_info_loc.dot(Field::flags),
        ...
        if (pCreateInfo->flags & (VK_VIDEO_SESSION_CREATE_ALLOW_ENCODE_QUANTIZATION_DELTA_MAP_BIT_KHR |
            ...
            if (!enabled_features.videoEncodeQuantizationMap) {
                skip |= LogError("VUID-VkVideoSessionCreateInfoKHR-flags-10264",
                                 device,
                                 create_info_loc.dot(Field::flags),
            ...
            if ((pCreateInfo->flags & VK_VIDEO_SESSION_CREATE_ALLOW_ENCODE_QUANTIZATION_DELTA_MAP_BIT_KHR) &&
                ...
                skip |= LogError("VUID-VkVideoSessionCreateInfoKHR-flags-10266",
                                 device,
                                 create_info_loc.dot(Field::flags),
            ...
            if (profile_desc.IsEncode()) {
                ...
                if ((pCreateInfo->flags & VK_VIDEO_SESSION_CREATE_ALLOW_ENCODE_EMPHASIS_MAP_BIT_KHR) &&
                    ...
                    skip |= LogError("VUID-VkVideoSessionCreateInfoKHR-flags-10268",
                                     device,
                                     create_info_loc.dot(Field::flags),
                ...
                skip |= LogError("VUID-VkVideoSessionCreateInfoKHR-flags-10265",
                                 device,
                                 create_info_loc.dot(Field::flags),
                                 "has VK_VIDEO_SESSION_CREATE_ALLOW_ENCODE_EMPHASIS_MAP_BIT_KHR or "
                                 "VK_VIDEO_SESSION_CREATE_ALLOW_ENCODE_QUANTIZATION_MAP_BIT_KHR set but "
        ...
        if (!IsBetweenInclusive(
                pCreateInfo->maxCodedExtent, profile_caps.base.minCodedExtent, profile_caps.base.maxCodedExtent)) {
            skip |= LogError("VUID-VkVideoSessionCreateInfoKHR-maxCodedExtent-04851",
                             device,
                             create_info_loc.dot(Field::maxCodedExtent),
                             "(%s) is outside of the "
                             "range (%s)-(%s) supported by the video profile.",
                             string_VkExtent2D(pCreateInfo->maxCodedExtent).c_str(),
                             string_VkExtent2D(profile_caps.base.minCodedExtent).c_str(),
                             string_VkExtent2D(profile_caps.base.maxCodedExtent).c_str());
        }

        if (pCreateInfo->maxDpbSlots > profile_caps.base.maxDpbSlots) {
            skip |= LogError("VUID-VkVideoSessionCreateInfoKHR-maxDpbSlots-04847",
                             device,
                             create_info_loc.dot(Field::maxDpbSlots),
                             "(%u) is greater than the "
                             "maxDpbSlots (%u) supported by the video profile.",
                             pCreateInfo->maxDpbSlots,
                             profile_caps.base.maxDpbSlots);
        }

        if (pCreateInfo->maxActiveReferencePictures > profile_caps.base.maxActiveReferencePictures) {
            skip |= LogError("VUID-VkVideoSessionCreateInfoKHR-maxActiveReferencePictures-04849",
                             device,
                             error_obj.location,
                             "pCreateInfo->maxActiveReferencePictures (%u) is greater "
                             "than the maxActiveReferencePictures (%u) supported by the video profile.",
                             pCreateInfo->maxActiveReferencePictures,
                             profile_caps.base.maxActiveReferencePictures);
        }
        ...
        if ((pCreateInfo->maxDpbSlots == 0 && pCreateInfo->maxActiveReferencePictures != 0) ||
            ...
            skip |= LogError("VUID-VkVideoSessionCreateInfoKHR-maxDpbSlots-04850",
                             device,
                             error_obj.location,
        ...
        if (profile_desc.IsDecode() && pCreateInfo->maxActiveReferencePictures > 0 &&
            !IsVideoFormatSupported(pCreateInfo->referencePictureFormat,
                                    VK_IMAGE_USAGE_VIDEO_DECODE_DPB_BIT_KHR,
                                    pCreateInfo->pVideoProfile)) {
            skip |= LogError("VUID-VkVideoSessionCreateInfoKHR-referencePictureFormat-04852",
                             device,
        ...
        if (profile_desc.IsEncode() && pCreateInfo->maxActiveReferencePictures > 0 &&
            !IsVideoFormatSupported(pCreateInfo->referencePictureFormat,
                                    ...
                                    pCreateInfo->pVideoProfile)) {
            skip |= LogError("VUID-VkVideoSessionCreateInfoKHR-referencePictureFormat-06814",
                             device,
                             create_info_loc.dot(Field::referencePictureFormat),
                             "(%s) is not a supported "
        ...
        if (profile_desc.IsDecode() && !IsVideoFormatSupported(pCreateInfo->pictureFormat,
                                                               VK_IMAGE_USAGE_VIDEO_DECODE_DST_BIT_KHR,
                                                               pCreateInfo->pVideoProfile)) {
            skip |= LogError("VUID-VkVideoSessionCreateInfoKHR-pictureFormat-04853",
                             device,
        ...
        if (profile_desc.IsEncode() && !IsVideoFormatSupported(pCreateInfo->pictureFormat,
                                                               VK_IMAGE_USAGE_VIDEO_ENCODE_SRC_BIT_KHR,
                                                               pCreateInfo->pVideoProfile)) {
            skip |= LogError("VUID-VkVideoSessionCreateInfoKHR-pictureFormat-04854",
                             device,
                             create_info_loc.dot(Field::pictureFormat),
                             "(%s) is not a supported "
        ...
        if (strncmp(pCreateInfo->pStdHeaderVersion->extensionName,
                    profile_caps.base.stdHeaderVersion.extensionName,
                    VK_MAX_EXTENSION_NAME_SIZE)) {
            skip |= LogError("VUID-VkVideoSessionCreateInfoKHR-pStdHeaderVersion-07190",
                             device,
                             create_info_loc.dot(Field::pStdHeaderVersion).dot(Field::extensionName),
                             "'%.*s' is an unsupported Video Std header name, expected '%.*s'.",
                             VK_MAX_EXTENSION_NAME_SIZE,
                             pCreateInfo->pStdHeaderVersion->extensionName,
                             VK_MAX_EXTENSION_NAME_SIZE,
                             profile_caps.base.stdHeaderVersion.extensionName);
        }

        if (pCreateInfo->pStdHeaderVersion->specVersion > profile_caps.base.stdHeaderVersion.specVersion) {
            skip |= LogError("VUID-VkVideoSessionCreateInfoKHR-pStdHeaderVersion-07191",
                             device,
                             create_info_loc.dot(Field::pStdHeaderVersion).dot(Field::specVersion),
                             "(0x%08x) is larger than the supported version (0x%08x).",
                             pCreateInfo->pStdHeaderVersion->specVersion,
                             profile_caps.base.stdHeaderVersion.specVersion);
        }
    } else {
        skip |= LogError("VUID-VkVideoSessionCreateInfoKHR-pVideoProfile-04845",
                         device,
                         create_info_loc.dot(Field::pVideoProfile),
                         "is not supported.");
    }
    ...
    switch (pCreateInfo->pVideoProfile->videoCodecOperation) {
        case VK_VIDEO_CODEC_OPERATION_ENCODE_H264_BIT_KHR: {
            ...
            if (h264_create_info != nullptr && h264_create_info->maxLevelIdc > profile_caps.encode_h264.maxLevelIdc) {
                skip |=
                    LogError("VUID-VkVideoSessionCreateInfoKHR-pVideoProfile-08251",
                             device,
                             ...
                             h264_create_info->maxLevelIdc,
                             profile_caps.encode_h264.maxLevelIdc);
            }
            break;
        }
        ...
        case VK_VIDEO_CODEC_OPERATION_ENCODE_H265_BIT_KHR: {
            ...
            if (h265_create_info != nullptr && h265_create_info->maxLevelIdc > profile_caps.encode_h265.maxLevelIdc) {
                skip |=
                    LogError("VUID-VkVideoSessionCreateInfoKHR-pVideoProfile-08252",
                             device,
                             ...
                             h265_create_info->maxLevelIdc,
                             profile_caps.encode_h265.maxLevelIdc);
            }
            break;
        }
        ...
        case VK_VIDEO_CODEC_OPERATION_ENCODE_AV1_BIT_KHR: {
            ...
            if (av1_create_info != nullptr && av1_create_info->maxLevel > profile_caps.encode_av1.maxLevel) {
                skip |= LogError("VUID-VkVideoSessionCreateInfoKHR-pVideoProfile-10270",
                                 device,
                                 ...
                                 av1_create_info->maxLevel,
                                 profile_caps.encode_av1.maxLevel);
            }
            break;
        }
...
bool CoreChecks::PreCallValidateDestroyVideoSessionKHR(VkDevice device,
    ...
    if (auto video_session_state = Get<vvl::VideoSession>(videoSession)) {
        skip |= ValidateObjectNotInUse(
            video_session_state.get(), error_obj.location, "VUID-vkDestroyVideoSessionKHR-videoSession-07192");
    }
    return skip;
}

bool CoreChecks::PreCallValidateBindVideoSessionMemoryKHR(
    VkDevice device,
    VkVideoSessionKHR videoSession,
    ...
    if (pBindSessionMemoryInfos) {
        {
            ...
            for (uint32_t i = 0; i < bindSessionMemoryInfoCount; ++i) {
                ...
                if (memory_bind_indices.find(mem_bind_index) != memory_bind_indices.end()) {
                    skip |=
                        LogError("VUID-vkBindVideoSessionMemoryKHR-memoryBindIndex-07196",
                                 videoSession,
        ...
        for (uint32_t i = 0; i < bindSessionMemoryInfoCount; ++i) {
            ...
            if (mem_binding_info != nullptr) {
                if (auto mem_state = Get<vvl::DeviceMemory>(bind_info.memory)) {
                    if (((1 << mem_state->allocate_info.memoryTypeIndex) &
                         mem_binding_info->requirements.memoryTypeBits) == 0) {
                        const LogObjectList objlist(videoSession, mem_state->Handle());
                        skip |= LogError("VUID-vkBindVideoSessionMemoryKHR-pBindSessionMemoryInfos-07198",
                                         objlist,
                                         error_obj.location,
                                         ...
                                         mem_binding_info->requirements.memoryTypeBits,
                                         bind_info.memoryBindIndex,
                    ...
                    if (bind_info.memoryOffset >= mem_state->allocate_info.allocationSize) {
                        const LogObjectList objlist(videoSession, mem_state->Handle());
                        skip |=
                            LogError("VUID-VkBindVideoSessionMemoryInfoKHR-memoryOffset-07201",
                                     objlist,
                                     ...
                                     FormatHandle(*mem_state).c_str());
                    } else if (bind_info.memoryOffset + bind_info.memorySize >
                               mem_state->allocate_info.allocationSize) {
                        const LogObjectList objlist(videoSession, mem_state->Handle());
                        skip |=
                            LogError("VUID-VkBindVideoSessionMemoryInfoKHR-memorySize-07202",
                                     objlist,
                                     error_obj.location.dot(Field::pBindSessionMemoryInfos, i).dot(Field::memoryOffset),
                                     "(%" PRIuLEAST64 ") + memory size (%" PRIuLEAST64
                                     ") must be less than or equal to the size (%" PRIuLEAST64 ") of %s.",
                                     bind_info.memoryOffset,
                                     bind_info.memorySize,
                                     mem_state->allocate_info.allocationSize,
                                     FormatHandle(*mem_state).c_str());
                    }
                }
                ...
                if (SafeModulo(bind_info.memoryOffset, mem_binding_info->requirements.alignment) != 0) {
                    skip |= LogError("VUID-vkBindVideoSessionMemoryKHR-pBindSessionMemoryInfos-07199",
                                     videoSession,
                                     error_obj.location.dot(Field::pBindSessionMemoryInfos, i).dot(Field::memoryOffset),
                                     "(%" PRIuLEAST64
                                     ...
                                     bind_info.memoryOffset,
                                     mem_binding_info->requirements.alignment,
                                     bind_info.memoryBindIndex,
                                     FormatHandle(videoSession).c_str());
                }

                if (bind_info.memorySize != mem_binding_info->requirements.size) {
                    skip |= LogError("VUID-vkBindVideoSessionMemoryKHR-pBindSessionMemoryInfos-07200",
                                     videoSession,
                                     ...
                                     ") for the memory binding index %u of %s.",
                                     bind_info.memorySize,
                                     mem_binding_info->requirements.size,
                                     bind_info.memoryBindIndex,
                                     FormatHandle(videoSession).c_str());
                }

                if (mem_binding_info->bound) {
                    skip |= LogError("VUID-vkBindVideoSessionMemoryKHR-videoSession-07195",
                                     videoSession,
                                     error_obj.location,
                ...
                skip |= LogError("VUID-vkBindVideoSessionMemoryKHR-pBindSessionMemoryInfos-07197",
                                 videoSession,
                                 error_obj.location,
...
bool CoreChecks::PreCallValidateCreateVideoSessionParametersKHR(
    VkDevice device,
    ...
    if (pCreateInfo->videoSessionParametersTemplate != VK_NULL_HANDLE) {
        ...
        if (template_state && (template_state->vs_state->VkHandle() != pCreateInfo->videoSession)) {
            ...
            skip |= LogError("VUID-VkVideoSessionParametersCreateInfoKHR-videoSessionParametersTemplate-04855",
                             objlist,
                             create_info_loc.dot(Field::videoSessionParametersTemplate),
                             "(%s) was not created against the same %s.",
    ...
    switch (vs_state->GetCodecOp()) {
        case VK_VIDEO_CODEC_OPERATION_DECODE_H264_BIT_KHR: {
            auto codec_info =
                vku::FindStructInPNextChain<VkVideoDecodeH264SessionParametersCreateInfoKHR>(pCreateInfo->pNext);
            if (codec_info) {
                skip |= ValidateDecodeH264ParametersAddInfo(
                    *vs_state,
                    codec_info->pParametersAddInfo,
                    device,
                    create_info_loc.pNext(Struct::VkVideoDecodeH264SessionParametersCreateInfoKHR,
                                          Field::pParametersAddInfo),
                    codec_info,
                    template_state.get());
            } else {
                skip |= LogError("VUID-VkVideoSessionParametersCreateInfoKHR-videoSession-07203",
                                 device,
        ...
        case VK_VIDEO_CODEC_OPERATION_DECODE_H265_BIT_KHR: {
            auto codec_info =
                vku::FindStructInPNextChain<VkVideoDecodeH265SessionParametersCreateInfoKHR>(pCreateInfo->pNext);
            if (codec_info) {
                skip |= ValidateDecodeH265ParametersAddInfo(
                    *vs_state,
                    codec_info->pParametersAddInfo,
                    device,
                    create_info_loc.pNext(Struct::VkVideoDecodeH265SessionParametersCreateInfoKHR,
                                          Field::pParametersAddInfo),
                    codec_info,
                    template_state.get());
            } else {
                skip |= LogError("VUID-VkVideoSessionParametersCreateInfoKHR-videoSession-07206",
                                 device,
                                 create_info_loc.dot(Field::pNext),
                                 pnext_chain_msg,
        ...
        case VK_VIDEO_CODEC_OPERATION_DECODE_AV1_BIT_KHR: {
            auto codec_info =
                vku::FindStructInPNextChain<VkVideoDecodeAV1SessionParametersCreateInfoKHR>(pCreateInfo->pNext);
            if (!codec_info) {
                skip |= LogError("VUID-VkVideoSessionParametersCreateInfoKHR-videoSession-09259",
                                 device,
                                 create_info_loc.dot(Field::pNext),
                                 pnext_chain_msg,
            ...
            if (pCreateInfo->videoSessionParametersTemplate != VK_NULL_HANDLE) {
                skip |= LogError("VUID-VkVideoSessionParametersCreateInfoKHR-videoSession-09258",
                                 device,
                                 create_info_loc.dot(Field::videoSessionParametersTemplate),
        ...
        case VK_VIDEO_CODEC_OPERATION_ENCODE_H264_BIT_KHR: {
            auto codec_info =
                vku::FindStructInPNextChain<VkVideoEncodeH264SessionParametersCreateInfoKHR>(pCreateInfo->pNext);
            if (codec_info) {
                skip |= ValidateEncodeH264ParametersAddInfo(
                    *vs_state,
                    codec_info->pParametersAddInfo,
                    device,
                    create_info_loc.pNext(Struct::VkVideoEncodeH264SessionParametersCreateInfoKHR,
                                          Field::pParametersAddInfo),
                    codec_info,
                    template_state.get());
            } else {
                skip |= LogError("VUID-VkVideoSessionParametersCreateInfoKHR-videoSession-07210",
                                 device,
                                 create_info_loc.dot(Field::pNext),
                                 pnext_chain_msg,
        ...
        case VK_VIDEO_CODEC_OPERATION_ENCODE_H265_BIT_KHR: {
            auto codec_info =
                vku::FindStructInPNextChain<VkVideoEncodeH265SessionParametersCreateInfoKHR>(pCreateInfo->pNext);
            if (codec_info) {
                skip |= ValidateEncodeH265ParametersAddInfo(
                    *vs_state,
                    codec_info->pParametersAddInfo,
                    device,
                    create_info_loc.pNext(Struct::VkVideoEncodeH265SessionParametersCreateInfoKHR,
                                          Field::pParametersAddInfo),
                    codec_info,
                    template_state.get());
            } else {
                skip |= LogError("VUID-VkVideoSessionParametersCreateInfoKHR-videoSession-07211",
                                 device,
                                 create_info_loc.dot(Field::pNext),
                                 pnext_chain_msg,
        ...
        case VK_VIDEO_CODEC_OPERATION_ENCODE_AV1_BIT_KHR: {
            auto codec_info =
                vku::FindStructInPNextChain<VkVideoEncodeAV1SessionParametersCreateInfoKHR>(pCreateInfo->pNext);
            if (codec_info) {
                if (codec_info->stdOperatingPointCount >
                    vs_state->profile->GetCapabilities().encode_av1.maxOperatingPoints) {
                    skip |= LogError(
                        ...
                        "(%u) is greater than the VkVideoEncodeAV1Capabilities::maxOperatingPoints (%u) supported by "
                        "the "
                        "AV1 encode profile VkVideoSessionParametersCreateInfoKHR::videoSession (%s) was created with.",
                        codec_info->stdOperatingPointCount,
                        vs_state->profile->GetCapabilities().encode_av1.maxOperatingPoints,
                        FormatHandle(*vs_state).c_str());
                }
                ...
                if (codec_info->pStdSequenceHeader != nullptr &&
                    ...
                    skip |= LogError("VUID-VkVideoEncodeAV1SessionParametersCreateInfoKHR-pStdSequenceHeader-10288",
                                     device,
                                     create_info_loc.pNext(Struct::VkVideoEncodeAV1SessionParametersCreateInfoKHR,
                                                           Field::pStdSequenceHeader),
                                     "AV1 encoding with film grain is not supported but "
                                     "flags.film_grain_params_present is not zero.");
                }
            } else {
                skip |= LogError("VUID-VkVideoSessionParametersCreateInfoKHR-videoSession-10279",
                                 device,
                                 create_info_loc.dot(Field::pNext),
                                 pnext_chain_msg,
            ...
            if (pCreateInfo->videoSessionParametersTemplate != VK_NULL_HANDLE) {
                skip |= LogError("VUID-VkVideoSessionParametersCreateInfoKHR-videoSession-10278",
                                 device,
                                 create_info_loc.dot(Field::videoSessionParametersTemplate),
    ...
    if (vs_state->IsEncode()) {
        ...
        if (quality_level_info != nullptr) {
            ...
            if (encode_quality_level >= max_quality_levels) {
                skip |= LogError("VUID-VkVideoEncodeQualityLevelInfoKHR-qualityLevel-08311",
                                 pCreateInfo->videoSession,
        ...
        if (pCreateInfo->flags & VK_VIDEO_SESSION_PARAMETERS_CREATE_QUANTIZATION_MAP_COMPATIBLE_BIT_KHR) {
            ...
            if ((vs_state->create_info.flags & (VK_VIDEO_SESSION_CREATE_ALLOW_ENCODE_QUANTIZATION_DELTA_MAP_BIT_KHR |
                ...
                skip |= LogError("VUID-VkVideoSessionParametersCreateInfoKHR-flags-10271",
                                 device,
                                 create_info_loc.dot(Field::flags),
                ...
                const auto* quantization_map_info =
                    vku::FindStructInPNextChain<VkVideoEncodeQuantizationMapSessionParametersCreateInfoKHR>(
                        pCreateInfo->pNext);
                if (quantization_map_info) {
                    ...
                    skip |=
                        LogError("VUID-VkVideoSessionParametersCreateInfoKHR-flags-10272",
                                 device,
                                 create_info_loc.dot(Field::flags),
                                 "has VK_VIDEO_SESSION_PARAMETERS_CREATE_QUANTIZATION_MAP_COMPATIBLE_BIT_KHR set but "
...
bool CoreChecks::PreCallValidateUpdateVideoSessionParametersKHR(
    VkDevice device,
    ...
    if (pUpdateInfo->updateSequenceCount != vsp_data->update_sequence_counter + 1) {
        skip |= LogError("VUID-vkUpdateVideoSessionParametersKHR-pUpdateInfo-07215",
                         device,
                         update_info_loc.dot(Field::updateSequenceCount),
                         "(%" PRIu32 ") should be %" PRIu32 ".",
                         pUpdateInfo->updateSequenceCount,
                         vsp_data->update_sequence_counter + 1);
    }

    switch (vsp_state->GetCodecOp()) {
        case VK_VIDEO_CODEC_OPERATION_DECODE_H264_BIT_KHR: {
            auto add_info =
                vku::FindStructInPNextChain<VkVideoDecodeH264SessionParametersAddInfoKHR>(pUpdateInfo->pNext);
            if (add_info) {
                skip |= ValidateDecodeH264ParametersAddInfo(
                    *vsp_state->vs_state,
                    add_info,
                ...
                for (uint32_t i = 0; i < add_info->stdSPSCount; ++i) {
                    if (vsp_data.GetH264SPS(add_info->pStdSPSs[i].seq_parameter_set_id)) {
                        skip |= LogError("VUID-vkUpdateVideoSessionParametersKHR-videoSessionParameters-07216",
                                         ...
                                         add_info->pStdSPSs[i].seq_parameter_set_id,
                                         FormatHandle(videoSessionParameters).c_str());
                    }
                }

                if (add_info->stdSPSCount + vsp_data->h264.sps.size() > vsp_data->h264.sps_capacity) {
                    skip |= LogError("VUID-vkUpdateVideoSessionParametersKHR-videoSessionParameters-07217",
                                     videoSessionParameters,
                                     error_obj.location,
                                     ...
                                     add_info->stdSPSCount,
                                     vsp_data->h264.sps.size(),
                                     vsp_data->h264.sps_capacity,
                                     FormatHandle(videoSessionParameters).c_str());
                }
                ...
                for (uint32_t i = 0; i < add_info->stdPPSCount; ++i) {
                    if (vsp_data.GetH264PPS(add_info->pStdPPSs[i].seq_parameter_set_id,
                        ...
                        skip |= LogError("VUID-vkUpdateVideoSessionParametersKHR-videoSessionParameters-07218",
                                         ...
                                         add_info->pStdPPSs[i].seq_parameter_set_id,
                                         add_info->pStdPPSs[i].pic_parameter_set_id,
                                         FormatHandle(videoSessionParameters).c_str());
                    }
                }
                ...
                if (add_info->stdPPSCount + vsp_data->h264.pps.size() > vsp_data->h264.pps_capacity) {
                    skip |= LogError("VUID-vkUpdateVideoSessionParametersKHR-videoSessionParameters-07219",
                                     videoSessionParameters,
                                     error_obj.location,
                                     "number of H.264 PPS entries to add "
                                     "(%u) plus the already used capacity (%zu) is greater than the maximum H.264 "
                                     "PPS capacity (%u) the %s was created with.",
                                     add_info->stdPPSCount,
                                     vsp_data->h264.pps.size(),
                                     vsp_data->h264.pps_capacity,
                                     FormatHandle(videoSessionParameters).c_str());
                }
            }
            break;
        ...
        case VK_VIDEO_CODEC_OPERATION_DECODE_H265_BIT_KHR: {
            auto add_info =
                vku::FindStructInPNextChain<VkVideoDecodeH265SessionParametersAddInfoKHR>(pUpdateInfo->pNext);
            if (add_info) {
                skip |= ValidateDecodeH265ParametersAddInfo(
                    *vsp_state->vs_state,
                    add_info,
                ...
                if (add_info->stdVPSCount + vsp_data->h265.vps.size() > vsp_data->h265.vps_capacity) {
                    skip |= LogError("VUID-vkUpdateVideoSessionParametersKHR-videoSessionParameters-07221",
                                     videoSessionParameters,
                                     error_obj.location,
                                     ...
                                     add_info->stdVPSCount,
                                     vsp_data->h265.vps.size(),
                                     vsp_data->h265.vps_capacity,
                                     FormatHandle(videoSessionParameters).c_str());
                }
                ...
                for (uint32_t i = 0; i < add_info->stdSPSCount; ++i) {
                    if (vsp_data.GetH265SPS(add_info->pStdSPSs[i].sps_video_parameter_set_id,
                        ...
                        skip |= LogError("VUID-vkUpdateVideoSessionParametersKHR-videoSessionParameters-07222",
                                         videoSessionParameters,
                                         error_obj.location,
                                         "H.265 SPS with key "
                ...
                if (add_info->stdSPSCount + vsp_data->h265.sps.size() > vsp_data->h265.sps_capacity) {
                    skip |= LogError("VUID-vkUpdateVideoSessionParametersKHR-videoSessionParameters-07223",
                                     videoSessionParameters,
                                     error_obj.location,
                                     ...
                                     "SPS capacity (%u) the %s was created with.",
                                     add_info->stdSPSCount,
                                     vsp_data->h265.sps.size(),
                                     vsp_data->h265.sps_capacity,
                                     FormatHandle(videoSessionParameters).c_str());
                }
                ...
                for (uint32_t i = 0; i < add_info->stdPPSCount; ++i) {
                    if (vsp_data.GetH265PPS(add_info->pStdPPSs[i].sps_video_parameter_set_id,
                        ...
                        skip |= LogError("VUID-vkUpdateVideoSessionParametersKHR-videoSessionParameters-07224",
                                         videoSessionParameters,
                                         error_obj.location,
                                         "H.265 PPS with key "
                ...
                if (add_info->stdPPSCount + vsp_data->h265.pps.size() > vsp_data->h265.pps_capacity) {
                    skip |= LogError("VUID-vkUpdateVideoSessionParametersKHR-videoSessionParameters-07225",
                                     videoSessionParameters,
                                     error_obj.location,
                                     ...
                                     "PPS capacity (%u) the %s was created with.",
                                     add_info->stdPPSCount,
                                     vsp_data->h265.pps.size(),
                                     vsp_data->h265.pps_capacity,
                                     FormatHandle(videoSessionParameters).c_str());
                }
            }
            break;
        ...
        case VK_VIDEO_CODEC_OPERATION_DECODE_AV1_BIT_KHR: {
            skip |= LogError("VUID-vkUpdateVideoSessionParametersKHR-videoSessionParameters-09260",
                             videoSessionParameters,
                             error_obj.location,
        ...
        case VK_VIDEO_CODEC_OPERATION_ENCODE_H264_BIT_KHR: {
            auto add_info =
                vku::FindStructInPNextChain<VkVideoEncodeH264SessionParametersAddInfoKHR>(pUpdateInfo->pNext);
            if (add_info) {
                skip |= ValidateEncodeH264ParametersAddInfo(
                    *vsp_state->vs_state,
                    add_info,
                ...
                if (add_info->stdSPSCount + vsp_data->h264.sps.size() > vsp_data->h264.sps_capacity) {
                    skip |= LogError("VUID-vkUpdateVideoSessionParametersKHR-videoSessionParameters-06441",
                                     videoSessionParameters,
                                     error_obj.location,
                                     "number of H.264 SPS entries to add "
                                     "(%u) plus the already used capacity (%zu) is greater than the maximum H.264 "
                ...
                if (add_info->stdPPSCount + vsp_data->h264.pps.size() > vsp_data->h264.pps_capacity) {
                    skip |= LogError("VUID-vkUpdateVideoSessionParametersKHR-videoSessionParameters-06442",
                                     videoSessionParameters,
                                     error_obj.location,
                                     "number of H.264 PPS entries to add "
                                     "(%u) plus the already used capacity (%zu) is greater than the maximum H.264 "
        ...
        case VK_VIDEO_CODEC_OPERATION_ENCODE_H265_BIT_KHR: {
            auto add_info =
                vku::FindStructInPNextChain<VkVideoEncodeH265SessionParametersAddInfoKHR>(pUpdateInfo->pNext);
            if (add_info) {
                skip |= ValidateEncodeH265ParametersAddInfo(
                    *vsp_state->vs_state,
                    add_info,
                ...
                if (add_info->stdVPSCount + vsp_data->h265.vps.size() > vsp_data->h265.vps_capacity) {
                    skip |= LogError("VUID-vkUpdateVideoSessionParametersKHR-videoSessionParameters-06443",
                                     videoSessionParameters,
                                     error_obj.location,
                                     "number of H.265 VPS entries to add "
                                     "(%u) plus the already used capacity (%zu) is greater than the maximum H.265 "
                ...
                for (uint32_t i = 0; i < add_info->stdSPSCount; ++i) {
                    if (vsp_data.GetH265SPS(add_info->pStdSPSs[i].sps_video_parameter_set_id,
                                            add_info->pStdSPSs[i].sps_seq_parameter_set_id)) {
                        skip |= LogError("VUID-vkUpdateVideoSessionParametersKHR-videoSessionParameters-07229",
                                         videoSessionParameters,
                                         error_obj.location,
                                         "H.265 SPS with key "
                                         "(VPS ID = %u, SPS ID = %u) already exists in %s.",
                ...
                if (add_info->stdSPSCount + vsp_data->h265.sps.size() > vsp_data->h265.sps_capacity) {
                    skip |= LogError("VUID-vkUpdateVideoSessionParametersKHR-videoSessionParameters-06444",
                                     videoSessionParameters,
                                     error_obj.location,
                                     "number of H.265 SPS entries to add "
                                     "(%u) plus the already used capacity (%zu) is greater than the maximum H.265 "
                ...
                for (uint32_t i = 0; i < add_info->stdPPSCount; ++i) {
                    if (vsp_data.GetH265PPS(add_info->pStdPPSs[i].sps_video_parameter_set_id,
                                            add_info->pStdPPSs[i].pps_seq_parameter_set_id,
                                            add_info->pStdPPSs[i].pps_pic_parameter_set_id)) {
                        skip |= LogError("VUID-vkUpdateVideoSessionParametersKHR-videoSessionParameters-07230",
                                         videoSessionParameters,
                                         error_obj.location,
                                         "H.265 PPS with key "
                                         "(VPS ID = %u, SPS ID = %u, PPS ID = %u) already exists in %s.",
                ...
                if (add_info->stdPPSCount + vsp_data->h265.pps.size() > vsp_data->h265.pps_capacity) {
                    skip |= LogError("VUID-vkUpdateVideoSessionParametersKHR-videoSessionParameters-06445",
                                     videoSessionParameters,
                                     error_obj.location,
                                     "number of H.265 PPS entries to add "
                                     "(%u) plus the already used capacity (%zu) is greater than the maximum H.265 "
        ...
        case VK_VIDEO_CODEC_OPERATION_ENCODE_AV1_BIT_KHR: {
            skip |= LogError("VUID-vkUpdateVideoSessionParametersKHR-videoSessionParameters-10281",
                             videoSessionParameters,
                             error_obj.location,
...
bool CoreChecks::PreCallValidateDestroyVideoSessionParametersKHR(VkDevice device,
    ...
    if (auto video_session_parameters_state = Get<vvl::VideoSessionParameters>(videoSessionParameters)) {
        skip |= ValidateObjectNotInUse(video_session_parameters_state.get(),
                                       error_obj.location,
...
bool CoreChecks::PreCallValidateGetEncodedVideoSessionParametersKHR(
    ...
    if (vsp_state->IsEncode()) {
        switch (vsp_state->GetCodecOp()) {
            case VK_VIDEO_CODEC_OPERATION_ENCODE_H264_BIT_KHR: {
                auto h264_info = vku::FindStructInPNextChain<VkVideoEncodeH264SessionParametersGetInfoKHR>(
                    pVideoSessionParametersInfo->pNext);
                if (h264_info) {
                    const Location h264_info_loc =
                        params_info_loc.pNext(Struct::VkVideoEncodeH264SessionParametersGetInfoKHR);
                    ...
                    if (h264_info->writeStdPPS &&
                        vsp_data.GetH264PPS(h264_info->stdSPSId, h264_info->stdPPSId) == nullptr) {
                    ...
                    skip |= LogError("VUID-vkGetEncodedVideoSessionParametersKHR-pVideoSessionParametersInfo-08262",
                                     pVideoSessionParametersInfo->videoSessionParameters,
                                     params_info_loc.dot(Field::pNext),
                                     pnext_msg,
            ...
            case VK_VIDEO_CODEC_OPERATION_ENCODE_H265_BIT_KHR: {
                auto h265_info = vku::FindStructInPNextChain<VkVideoEncodeH265SessionParametersGetInfoKHR>(
                    pVideoSessionParametersInfo->pNext);
                if (h265_info) {
                    const Location h265_info_loc =
                        params_info_loc.pNext(Struct::VkVideoEncodeH264SessionParametersGetInfoKHR);
                    ...
                    if (h265_info->writeStdSPS &&
                        vsp_data.GetH265SPS(h265_info->stdVPSId, h265_info->stdSPSId) == nullptr) {
        ...
        skip |= LogError("VUID-vkGetEncodedVideoSessionParametersKHR-pVideoSessionParametersInfo-08359",
                         pVideoSessionParametersInfo->videoSessionParameters,
                         params_info_loc.dot(Field::videoSessionParameters),
...
bool CoreChecks::PreCallValidateCmdBeginVideoCodingKHR(VkCommandBuffer commandBuffer,
    ...
    if (cb_state->activeQueries.size() > 0) {
        skip |= LogError("VUID-vkCmdBeginVideoCodingKHR-None-07232",
                         commandBuffer,
                         error_obj.location.dot(Field::commandBuffer),
                         "%s has active queries.",
    ...
    if ((qf_ext_props.video_props.videoCodecOperations & vs_state->GetCodecOp()) == 0) {
        ...
        skip |= LogError("VUID-vkCmdBeginVideoCodingKHR-commandBuffer-07231",
                         objlist,
                         begin_info_loc.dot(Field::videoSession),
    ...
    if (vs_state->GetUnboundMemoryBindingCount() > 0) {
        ...
        skip |= LogError("VUID-VkVideoBeginCodingInfoKHR-videoSession-07237",
                         objlist,
                         begin_info_loc.dot(Field::videoSession),
                         "%s has %u unbound memory binding indices.",
                         FormatHandle(pBeginInfo->videoSession).c_str(),
    ...
    if ((!phys_dev_props_core11.protectedNoFault) && (cb_state->unprotected == true) &&
        ...
        const LogObjectList objlist(commandBuffer, pBeginInfo->videoSession);
        skip |= LogError("VUID-vkCmdBeginVideoCodingKHR-commandBuffer-07233",
                         objlist,
                         error_obj.location.dot(Field::commandBuffer),
    ...
    if ((!phys_dev_props_core11.protectedNoFault) && (cb_state->unprotected == false) &&
        ((vs_state->create_info.flags & VK_VIDEO_SESSION_CREATE_PROTECTED_CONTENT_BIT_KHR) == 0)) {
        const LogObjectList objlist(commandBuffer, pBeginInfo->videoSession);
        skip |= LogError("VUID-vkCmdBeginVideoCodingKHR-commandBuffer-07234",
                         objlist,
                         error_obj.location.dot(Field::commandBuffer),
    ...
    if (pBeginInfo->pReferenceSlots) {
        ...
        for (uint32_t i = 0; i < pBeginInfo->referenceSlotCount; ++i) {
            ...
            if (slot.slotIndex >= 0 && (uint32_t)slot.slotIndex >= vs_state->create_info.maxDpbSlots) {
                const LogObjectList objlist(commandBuffer, pBeginInfo->videoSession);
                skip |= LogError("VUID-VkVideoBeginCodingInfoKHR-slotIndex-04856",
                                 objlist,
                                 reference_slot_loc.dot(Field::slotIndex),
                                 "(%d) is greater than the maxDpbSlots %s was created with.",
                                 slot.slotIndex,
                                 FormatHandle(pBeginInfo->videoSession).c_str());
            }
            ...
            if (slot.pPictureResource != nullptr) {
                ...
                skip |= ValidateVideoPictureResource(reference_resource,
                                                     commandBuffer,
                                                     *vs_state,
                                                     reference_resource_loc,
                ...
                if (reference_resource) {
                    ...
                    skip |= ValidateProtectedImage(*cb_state,
                                                   *reference_resource.image_state,
                                                   reference_image_view_loc,
                    ...
                    if (reference_resource.image_view_state->create_info.format !=
                        vs_state->create_info.referencePictureFormat) {
                        const LogObjectList objlist(commandBuffer,
                                                    pBeginInfo->videoSession,
                                                    reference_resource.image_view_state->Handle(),
                                                    reference_resource.image_state->Handle());
                        skip |= LogError("VUID-VkVideoBeginCodingInfoKHR-pPictureResource-07241",
                                         objlist,
                                         reference_image_view_loc,
                    ...
                    if (vs_state->IsDecode() && (supported_usage & VK_IMAGE_USAGE_VIDEO_DECODE_DPB_BIT_KHR) == 0) {
                        const LogObjectList objlist(commandBuffer,
                                                    ...
                                                    reference_resource.image_view_state->Handle(),
                                                    reference_resource.image_state->Handle());
                        skip |= LogError("VUID-VkVideoBeginCodingInfoKHR-slotIndex-07245",
                                         objlist,
                                         reference_image_view_loc,
                    ...
                    if (vs_state->IsEncode() && (supported_usage & VK_IMAGE_USAGE_VIDEO_ENCODE_DPB_BIT_KHR) == 0) {
                        const LogObjectList objlist(commandBuffer,
                                                    ...
                                                    reference_resource.image_view_state->Handle(),
                                                    reference_resource.image_state->Handle());
                        skip |= LogError("VUID-VkVideoBeginCodingInfoKHR-slotIndex-07246",
                                         objlist,
                                         reference_image_view_loc,
                                         "(%s created from %s) was not created "
        ...
        if (!resources_unique) {
            const LogObjectList objlist(commandBuffer, pBeginInfo->videoSession);
            skip |= LogError("VUID-VkVideoBeginCodingInfoKHR-pPictureResource-07238",
                             objlist,
                             error_obj.location,
        ...
        if ((supported_cap_flags & VK_VIDEO_CAPABILITY_SEPARATE_REFERENCE_IMAGES_BIT_KHR) == 0 && has_separate_images) {
            const LogObjectList objlist(commandBuffer, pBeginInfo->videoSession);
            skip |= LogError("VUID-VkVideoBeginCodingInfoKHR-flags-07244",
                             objlist,
                             error_obj.location,
    ...
    if (vsp_state && vsp_state->vs_state->VkHandle() != vs_state->VkHandle()) {
        ...
        skip |= LogError("VUID-VkVideoBeginCodingInfoKHR-videoSessionParameters-04857",
                         objlist,
                         begin_info_loc.dot(Field::videoSessionParameters),
                         "%s was not created for %s.",
                         FormatHandle(pBeginInfo->videoSessionParameters).c_str(),
                         FormatHandle(pBeginInfo->videoSession).c_str());
    }
    ...
    if ((codec_op_requires_params_vuid != nullptr) && (vsp_state == nullptr)) {
        const LogObjectList objlist(commandBuffer, pBeginInfo->videoSession);
        skip |= LogError(codec_op_requires_params_vuid,
                         objlist,
                         begin_info_loc.dot(Field::videoSession),
    ...
    if (vs_state->IsEncode()) {
        ...
        if (rc_info != nullptr) {
            skip |= ValidateVideoEncodeRateControlInfo(
                *rc_info, pBeginInfo->pNext, commandBuffer, *vs_state, begin_info_loc);
                ...
                switch (vs_state->GetCodecOp()) {
                    case VK_VIDEO_CODEC_OPERATION_ENCODE_H264_BIT_KHR: {
                        ...
                        if (vs_state->profile->GetCapabilities().encode_h264.requiresGopRemainingFrames &&
                            ...
                            const LogObjectList objlist(commandBuffer, pBeginInfo->videoSession);
                            skip |= LogError("VUID-vkCmdBeginVideoCodingKHR-pBeginInfo-08255",
                                             objlist,
                                             error_obj.location,
                    ...
                    case VK_VIDEO_CODEC_OPERATION_ENCODE_H265_BIT_KHR: {
                        ...
                        if (vs_state->profile->GetCapabilities().encode_h265.requiresGopRemainingFrames &&
                            ...
                            const LogObjectList objlist(commandBuffer, pBeginInfo->videoSession);
                            skip |= LogError("VUID-vkCmdBeginVideoCodingKHR-pBeginInfo-08256",
                                             objlist,
                                             error_obj.location,
                                             "requiresGopRemainingFrames is VK_TRUE for "
                    ...
                    case VK_VIDEO_CODEC_OPERATION_ENCODE_AV1_BIT_KHR: {
                        ...
                        if (vs_state->profile->GetCapabilities().encode_av1.requiresGopRemainingFrames &&
                            ...
                            const LogObjectList objlist(commandBuffer, pBeginInfo->videoSession);
                            skip |= LogError("VUID-vkCmdBeginVideoCodingKHR-pBeginInfo-10282",
                                             objlist,
                                             error_obj.location,
                                             "requiresGopRemainingFrames is VK_TRUE for "
...
void CoreChecks::PreCallRecordCmdBeginVideoCodingKHR(VkCommandBuffer commandBuffer,
                                                     const VkVideoBeginCodingInfoKHR* pBeginInfo,
    ...
    if (pBeginInfo->referenceSlotCount > 0) {
        ...
        cb_state->video_session_updates[vs_state->VkHandle()].emplace_back([expected_slots, loc](
            ...
            for (const auto& slot : expected_slots) {
                if (!dev_state.IsSlotActive(slot.index)) {
                    skip |= dev_data.LogError("VUID-vkCmdBeginVideoCodingKHR-slotIndex-07239",
                                              vs_state->Handle(),
                                              loc,
                                              "DPB slot index %d is not active in %s.",
                                              slot.index,
                                              dev_data.FormatHandle(*vs_state).c_str());
                } else if (slot.resource && !dev_state.IsSlotPicture(slot.index, slot.resource)) {
                    skip |= dev_data.LogError("VUID-vkCmdBeginVideoCodingKHR-pPictureResource-07265",
                                              vs_state->Handle(),
                                              loc,
    ...
    if (vs_state->IsEncode()) {
        ...
        cb_state->video_session_updates[vs_state->VkHandle()].emplace_back(
            [begin_info, loc](const ValidationStateTracker& dev_data,
                              const vvl::VideoSession* vs_state,
                              vvl::VideoSessionDeviceState& dev_state,
                              bool do_validate) {
                if (!do_validate)
...
bool CoreChecks::PreCallValidateCmdEndVideoCodingKHR(VkCommandBuffer commandBuffer,
    ...
    if (cb_state->activeQueries.size() > 0) {
        skip |= LogError("VUID-vkCmdEndVideoCodingKHR-None-07251",
                         commandBuffer,
                         error_obj.location.dot(Field::commandBuffer),
                         "%s has active queries.",
                         FormatHandle(commandBuffer).c_str());
...
bool CoreChecks::PreCallValidateCmdControlVideoCodingKHR(VkCommandBuffer commandBuffer,
    ...
    if (pCodingControlInfo->flags & VK_VIDEO_CODING_CONTROL_ENCODE_RATE_CONTROL_BIT_KHR) {
        if (vs_state->IsEncode()) {
            const auto rc_info =
                vku::FindStructInPNextChain<VkVideoEncodeRateControlInfoKHR>(pCodingControlInfo->pNext);
            if (rc_info != nullptr) {
                skip |= ValidateVideoEncodeRateControlInfo(
                    *rc_info, pCodingControlInfo->pNext, commandBuffer, *vs_state, control_info_loc);
            } else {
                skip |= LogError("VUID-VkVideoCodingControlInfoKHR-flags-07018",
                                 commandBuffer,
                                 control_info_loc.dot(Field::flags),
                                 flags_pnext_msg,
            ...
            skip |= LogError("VUID-vkCmdControlVideoCodingKHR-pCodingControlInfo-08243",
                             objlist,
                             control_info_loc.dot(Field::flags),
                             flags_require_encode_msg,
                             "VK_VIDEO_CODING_CONTROL_ENCODE_RATE_CONTROL_BIT_KHR",
                             FormatHandle(*vs_state).c_str());
        }
    }
        ...
        if (vs_state->IsEncode()) {
            ...
            if (quality_level_info != nullptr) {
                if (quality_level_info->qualityLevel >= profile_caps.encode.maxQualityLevels) {
                    const LogObjectList objlist(commandBuffer, vs_state->Handle());
                    skip |=
                        LogError("VUID-VkVideoEncodeQualityLevelInfoKHR-qualityLevel-08311",
                                 objlist,
                ...
                skip |= LogError("VUID-VkVideoCodingControlInfoKHR-flags-08349",
                                 commandBuffer,
                                 control_info_loc.dot(Field::flags),
                                 flags_pnext_msg,
...
void CoreChecks::PreCallRecordCmdControlVideoCodingKHR(VkCommandBuffer commandBuffer,
    ...
    if ((pCodingControlInfo->flags & VK_VIDEO_CODING_CONTROL_RESET_BIT_KHR) == 0) {
        EnqueueVerifyVideoSessionInitialized(
            *cb_state, *vs_state, record_obj.location, "VUID-vkCmdControlVideoCodingKHR-flags-07017");
    }
}

bool CoreChecks::PreCallValidateCmdDecodeVideoKHR(VkCommandBuffer commandBuffer,
    ...
    if (!vs_state->IsDecode()) {
        const LogObjectList objlist(commandBuffer, vs_state->Handle());
        skip |= LogError("VUID-vkCmdDecodeVideoKHR-None-08249",
                         objlist,
                         error_obj.location,
    ...
    if (auto buffer_state = Get<vvl::Buffer>(pDecodeInfo->srcBuffer)) {
        skip |= ValidateProtectedBuffer(*cb_state,
                                        *buffer_state,
                                        decode_info_loc.dot(Field::srcBuffer),
        ...
        if ((buffer_state->usage & VK_BUFFER_USAGE_VIDEO_DECODE_SRC_BIT_KHR) == 0) {
            ...
            skip |= LogError("VUID-VkVideoDecodeInfoKHR-srcBuffer-07165",
                             objlist,
                             decode_info_loc.dot(Field::srcBuffer),
        ...
        if (!IsBufferCompatibleWithVideoSession(*buffer_state, *vs_state)) {
            const LogObjectList objlist(commandBuffer, vs_state->Handle(), pDecodeInfo->srcBuffer);
            skip |= LogError("VUID-vkCmdDecodeVideoKHR-pDecodeInfo-07135",
                             objlist,
                             decode_info_loc.dot(Field::srcBuffer),
                             "(%s) is not compatible "
        ...
        if (pDecodeInfo->srcBufferOffset >= buffer_state->create_info.size) {
            const LogObjectList objlist(commandBuffer, vs_state->Handle(), pDecodeInfo->srcBuffer);
            skip |= LogError("VUID-VkVideoDecodeInfoKHR-srcBufferOffset-07166",
                             objlist,
                             decode_info_loc.dot(Field::srcBufferOffset),
        ...
        if (!IsIntegerMultipleOf(pDecodeInfo->srcBufferOffset, profile_caps.base.minBitstreamBufferOffsetAlignment)) {
            const LogObjectList objlist(commandBuffer, vs_state->Handle());
            skip |=
                LogError("VUID-vkCmdDecodeVideoKHR-pDecodeInfo-07138",
                         objlist,
                         decode_info_loc.dot(Field::srcBufferOffset),
                         ...
                         pDecodeInfo->srcBufferOffset,
                         profile_caps.base.minBitstreamBufferOffsetAlignment,
                         FormatHandle(*vs_state).c_str());
        }

        if (pDecodeInfo->srcBufferOffset + pDecodeInfo->srcBufferRange > buffer_state->create_info.size) {
            const LogObjectList objlist(commandBuffer, vs_state->Handle(), pDecodeInfo->srcBuffer);
            skip |= LogError("VUID-VkVideoDecodeInfoKHR-srcBufferRange-07167",
                             objlist,
                             decode_info_loc.dot(Field::srcBufferOffset),
    ...
    if (!IsIntegerMultipleOf(pDecodeInfo->srcBufferRange, profile_caps.base.minBitstreamBufferSizeAlignment)) {
        const LogObjectList objlist(commandBuffer, vs_state->Handle());
        skip |= LogError("VUID-vkCmdDecodeVideoKHR-pDecodeInfo-07139",
                         objlist,
                         decode_info_loc.dot(Field::srcBufferRange),
                         "(%" PRIu64 ") is not an integer multiple of the minBitstreamBufferSizeAlignment (%" PRIu64
                         ") required by the video profile %s was created with.",
                         pDecodeInfo->srcBufferRange,
                         profile_caps.base.minBitstreamBufferSizeAlignment,
                         FormatHandle(*vs_state).c_str());
    }

    if (vs_state->create_info.maxDpbSlots > 0 && pDecodeInfo->pSetupReferenceSlot == nullptr) {
        const LogObjectList objlist(commandBuffer, vs_state->Handle());
        skip |= LogError("VUID-vkCmdDecodeVideoKHR-pDecodeInfo-08376",
                         objlist,
                         decode_info_loc.dot(Field::pSetupReferenceSlot),
                         "is NULL but the bound video session %s was created with maxDpbSlot %u.",
                         FormatHandle(*vs_state).c_str(),
    ...
    if (pDecodeInfo->pSetupReferenceSlot) {
        if (pDecodeInfo->pSetupReferenceSlot->slotIndex < 0) {
            skip |= LogError("VUID-VkVideoDecodeInfoKHR-pSetupReferenceSlot-07168",
                             commandBuffer,
            ...
            skip |= LogError("VUID-vkCmdDecodeVideoKHR-pDecodeInfo-07170",
                             commandBuffer,
                             decode_info_loc.dot(Field::pSetupReferenceSlot).dot(Field::slotIndex),
        ...
        if (pDecodeInfo->pSetupReferenceSlot->pPictureResource != nullptr) {
            ...
            if (setup_resource) {
                ...
                if (bound_resources.find(setup_resource) == bound_resources.end()) {
                    skip |= LogError("VUID-vkCmdDecodeVideoKHR-pDecodeInfo-07149",
                                     commandBuffer,
                                     decode_info_loc.dot(Field::pSetupReferenceSlot).dot(Field::pPictureResource),
                ...
                skip |= VerifyImageLayout(*cb_state,
                                          *setup_resource.image_state,
                                          setup_resource.range,
            ...
            skip |= LogError("VUID-VkVideoDecodeInfoKHR-pSetupReferenceSlot-07169",
                             commandBuffer,
                             decode_info_loc.dot(Field::pSetupReferenceSlot).dot(Field::pPictureResource),
                             "is NULL");
        }
    ...
    skip |= ValidateVideoPictureResource(dst_resource,
                                         commandBuffer,
                                         *vs_state,
                                         dst_resource_loc,
    ...
    if (dst_resource) {
        ...
        skip |= ValidateProtectedImage(
            *cb_state, *dst_resource.image_state, dst_image_view_loc, "VUID-vkCmdDecodeVideoKHR-commandBuffer-07147");
        skip |= ValidateUnprotectedImage(
            *cb_state, *dst_resource.image_state, dst_image_view_loc, "VUID-vkCmdDecodeVideoKHR-commandBuffer-07148");

        if (!IsImageCompatibleWithVideoSession(*dst_resource.image_state, *vs_state)) {
            const LogObjectList objlist(commandBuffer,
                                        vs_state->Handle(),
                                        dst_resource.image_view_state->Handle(),
                                        dst_resource.image_state->Handle());
            skip |= LogError("VUID-vkCmdDecodeVideoKHR-pDecodeInfo-07142",
                             objlist,
                             dst_image_view_loc,
                             "(%s created from %s) is not compatible with the video profile the bound "
                             "video session %s was created with.",
        ...
        if (dst_resource.image_view_state->create_info.format != vs_state->create_info.pictureFormat) {
            const LogObjectList objlist(commandBuffer,
                                        ...
                                        dst_resource.image_view_state->Handle(),
                                        dst_resource.image_state->Handle());
            skip |= LogError("VUID-vkCmdDecodeVideoKHR-pDecodeInfo-07143",
                             objlist,
                             dst_image_view_loc,
        ...
        if ((supported_usage & VK_IMAGE_USAGE_VIDEO_DECODE_DST_BIT_KHR) == 0) {
            const LogObjectList objlist(commandBuffer,
                                        ...
                                        dst_resource.image_view_state->Handle(),
                                        dst_resource.image_state->Handle());
            skip |= LogError("VUID-vkCmdDecodeVideoKHR-pDecodeInfo-07146",
                             objlist,
                             dst_image_view_loc,
        ...
        const char* vuid = dst_same_as_setup ? "VUID-vkCmdDecodeVideoKHR-pDecodeInfo-07253"
                                             : "VUID-vkCmdDecodeVideoKHR-pDecodeInfo-07252";

        skip |= VerifyImageLayout(*cb_state,
                                  *dst_resource.image_state,
                                  dst_resource.range,
                                  expected_layout,
                                  error_obj.location,
                                  vuid,
        ...
        if (setup_resource) {
            if ((profile_caps.decode.flags & VK_VIDEO_DECODE_CAPABILITY_DPB_AND_OUTPUT_COINCIDE_BIT_KHR) == 0 &&
                dst_same_as_setup) {
                const LogObjectList objlist(commandBuffer, vs_state->Handle());
                skip |= LogError("VUID-vkCmdDecodeVideoKHR-pDecodeInfo-07140",
                                 objlist,
                                 error_obj.location,
                                 "the video profile %s was created with does not support "
    ...
    if (pDecodeInfo->pReferenceSlots) {
        ...
        for (uint32_t i = 0; i < pDecodeInfo->referenceSlotCount; ++i) {
            if (pDecodeInfo->pReferenceSlots[i].slotIndex < 0) {
                skip |= LogError("VUID-VkVideoDecodeInfoKHR-slotIndex-07171",
                                 commandBuffer,
                ...
                const LogObjectList objlist(commandBuffer, vs_state->Handle());
                skip |= LogError("VUID-vkCmdDecodeVideoKHR-slotIndex-07256",
                                 objlist,
                                 decode_info_loc.dot(Field::pReferenceSlots, i).dot(Field::slotIndex),
                                 "(%d) must be smaller than the maxDpbSlots (%u) the bound video session %s "
                                 "was created with.",
            ...
            if (pDecodeInfo->pReferenceSlots[i].pPictureResource != nullptr) {
                auto reference_resource =
                    vvl::VideoPictureResource(*this, *pDecodeInfo->pReferenceSlots[i].pPictureResource);
                if (reference_resource) {
                    if (!unique_resources.emplace(reference_resource).second) {
                        resources_unique = false;
                    }
                    ...
                    if (it == bound_resources.end()) {
                        skip |= LogError("VUID-vkCmdDecodeVideoKHR-pDecodeInfo-07151",
                                         commandBuffer,
                                         error_obj.location,
                ...
                skip |= LogError("VUID-VkVideoDecodeInfoKHR-pPictureResource-07172",
                                 commandBuffer,
                                 decode_info_loc.dot(Field::pReferenceSlots, i).dot(Field::pPictureResource),
                                 "is NULL.");
            }
        ...
        if (!resources_unique) {
            skip |= LogError("VUID-vkCmdDecodeVideoKHR-pDecodeInfo-07264",
                             commandBuffer,
                             error_obj.location,
    ...
    for (const auto& query : cb_state->activeQueries) {
        if (query.active_query_index + op_count > query.last_activatable_query_index + 1) {
            ...
            skip |= LogError("VUID-vkCmdDecodeVideoKHR-opCount-07134",
                             commandBuffer,
                             error_obj.location,
    ...
    if (vs_state->create_info.flags & VK_VIDEO_SESSION_CREATE_INLINE_QUERIES_BIT_KHR) {
        ...
        if (inline_query_info != nullptr) {
            if (auto query_pool_state = Get<vvl::QueryPool>(inline_query_info->queryPool)) {
                skip |= ValidateVideoInlineQueryInfo(
                    *query_pool_state, *inline_query_info, decode_info_loc.pNext(Struct::VkVideoInlineQueryInfoKHR));

                if (inline_query_info->queryCount != op_count) {
                    const LogObjectList objlist(commandBuffer, vs_state->Handle());
                    skip |= LogError("VUID-vkCmdDecodeVideoKHR-pNext-08365",
                                     objlist,
                                     decode_info_loc.pNext(Struct::VkVideoInlineQueryInfoKHR, Field::queryCount),
                                     "(%u) is not equal to opCount (%u).",
                                     inline_query_info->queryCount,
                                     op_count);
                }
                ...
                if (query_pool_state->create_info.queryType != VK_QUERY_TYPE_RESULT_STATUS_ONLY_KHR) {
                    ...
                    skip |= LogError("VUID-vkCmdDecodeVideoKHR-queryType-08367",
                                     objlist,
                ...
                if (vs_state->profile != query_pool_state->supported_video_profile) {
                    ...
                    skip |= LogError("VUID-vkCmdDecodeVideoKHR-queryPool-08368",
                                     objlist,
                                     decode_info_loc.pNext(Struct::VkVideoInlineQueryInfoKHR, Field::queryPool),
                ...
                if (!qf_ext_props.query_result_status_props.queryResultStatusSupport) {
                    const LogObjectList objlist(commandBuffer, inline_query_info->queryPool);
                    skip |= LogError("VUID-vkCmdDecodeVideoKHR-queryType-08369",
                                     objlist,
                                     error_obj.location,
...
void CoreChecks::PreCallRecordCmdDecodeVideoKHR(VkCommandBuffer commandBuffer,
                                                const VkVideoDecodeInfoKHR* pDecodeInfo,
                                                const RecordObject& record_obj) {
    auto cb_state = GetWrite<vvl::CommandBuffer>(commandBuffer);
    if (!cb_state)
    ...
    if (vs_state->GetCodecOp() == VK_VIDEO_CODEC_OPERATION_DECODE_H264_BIT_KHR) {
        ...
        cb_state->video_session_updates[vs_state->VkHandle()].emplace_back([reference_slots, loc](
                                                                               const ValidationStateTracker& dev_data,
                                                                               const vvl::VideoSession* vs_state,
                                                                               vvl::VideoSessionDeviceState& dev_state,
                                                                               bool do_validate) {
            if (!do_validate)
                return false;
            bool skip = false;
            const auto log_picture_kind_error =
                [&](const vvl::VideoReferenceSlot& slot, const char* vuid, const char* picture_kind) -> bool {
                return dev_data.LogError(vuid,
                                         vs_state->Handle(),
                                         loc,
...
bool CoreChecks::PreCallValidateCmdEncodeVideoKHR(VkCommandBuffer commandBuffer,
    ...
    if (!vs_state->IsEncode()) {
        const LogObjectList objlist(commandBuffer, vs_state->Handle());
        skip |= LogError("VUID-vkCmdEncodeVideoKHR-None-08250",
                         objlist,
                         error_obj.location,
                         "the video codec operation (%s) the bound video session %s "
    ...
    if (vs_state->IsEncode() && vsp_state && cb_state->video_encode_quality_level.has_value()) {
        ...
        if (vsp_state->GetEncodeQualityLevel() != cb_state->video_encode_quality_level.value()) {
            ...
            skip |= LogError("VUID-vkCmdEncodeVideoKHR-None-08318",
                             objlist,
                             error_obj.location,
                             "the currently configured encode quality level (%u) for %s "
                             "does not match the encode quality level (%u) %s was created with.",
                             cb_state->video_encode_quality_level.value(),
                             FormatHandle(*vs_state).c_str(),
                             vsp_state->GetEncodeQualityLevel(),
                             FormatHandle(*vsp_state).c_str());
        }
    }

    const auto* quantization_map_info =
        vku::FindStructInPNextChain<VkVideoEncodeQuantizationMapInfoKHR>(pEncodeInfo->pNext);
    ...
    for (auto& valid_usage : quantization_map_info_valid_usages) {
        if ((pEncodeInfo->flags & valid_usage.encode_flag) == 0)
            continue;

        if ((vs_state->create_info.flags & valid_usage.session_flag) == 0) {
            const LogObjectList objlist(commandBuffer, vs_state->Handle());
            skip |= LogError(valid_usage.session_mismatch_vuid,
                             objlist,
                             encode_info_loc.dot(Field::flags),
        ...
        if (quantization_map_info != nullptr && quantization_map_info->quantizationMap != VK_NULL_HANDLE) {
            ...
            if ((iv_state->inherited_usage & valid_usage.image_usage_flag) == 0) {
                ...
                skip |= LogError(
                    valid_usage.image_view_mismatch_vuid,
                    objlist,
                    encode_info_loc.dot(Field::flags),
                    ...
                    encode_info_loc.pNext(Struct::VkVideoEncodeQuantizationMapInfoKHR, Field::quantizationMap)
                        .Fields()
                        .c_str(),
                    FormatHandle(*iv_state).c_str(),
                    FormatHandle(*iv_state->image_state).c_str(),
    ...
    if (pEncodeInfo->flags &
        (VK_VIDEO_ENCODE_WITH_QUANTIZATION_DELTA_MAP_BIT_KHR | VK_VIDEO_ENCODE_WITH_EMPHASIS_MAP_BIT_KHR)) {
        if (quantization_map_info != nullptr && quantization_map_info->quantizationMap != VK_NULL_HANDLE) {
            if (vsp_state && (vsp_state->create_info.flags &
                              VK_VIDEO_SESSION_PARAMETERS_CREATE_QUANTIZATION_MAP_COMPATIBLE_BIT_KHR) == 0) {
                const LogObjectList objlist(commandBuffer, vsp_state->Handle());
                skip |=
                    LogError("VUID-vkCmdEncodeVideoKHR-pNext-10315",
                             objlist,
                             encode_info_loc.pNext(Struct::VkVideoEncodeQuantizationMapInfoKHR, Field::quantizationMap),
    ...
    if (pEncodeInfo->flags & VK_VIDEO_ENCODE_WITH_EMPHASIS_MAP_BIT_KHR) {
        ...
        if (rc_mode == VK_VIDEO_ENCODE_RATE_CONTROL_MODE_DISABLED_BIT_KHR ||
            ...
            const LogObjectList objlist(commandBuffer, vs_state->Handle());
            skip |=
                LogError("VUID-vkCmdEncodeVideoKHR-pEncodeInfo-10308",
                         objlist,
                         encode_info_loc.dot(Field::flags),
    ...
    if (auto buffer_state = Get<vvl::Buffer>(pEncodeInfo->dstBuffer)) {
        skip |= ValidateProtectedBuffer(*cb_state,
                                        *buffer_state,
                                        encode_info_loc.dot(Field::dstBuffer),
                                        "VUID-vkCmdEncodeVideoKHR-commandBuffer-08202");
        skip |= ValidateUnprotectedBuffer(*cb_state,
                                          *buffer_state,
                                          encode_info_loc.dot(Field::dstBuffer),
        ...
        if ((buffer_state->usage & VK_BUFFER_USAGE_VIDEO_ENCODE_DST_BIT_KHR) == 0) {
            ...
            skip |= LogError("VUID-VkVideoEncodeInfoKHR-dstBuffer-08236",
                             objlist,
                             encode_info_loc.dot(Field::dstBuffer),
                             "(%s) was not created with "
        ...
        if (!IsBufferCompatibleWithVideoSession(*buffer_state, *vs_state)) {
            const LogObjectList objlist(commandBuffer, vs_state->Handle(), pEncodeInfo->dstBuffer);
            skip |= LogError("VUID-vkCmdEncodeVideoKHR-pEncodeInfo-08201",
                             objlist,
                             encode_info_loc.dot(Field::dstBuffer),
                             "(%s) is not compatible "
                             "with the video profile %s was created with.",
        ...
        if (pEncodeInfo->dstBufferOffset >= buffer_state->create_info.size) {
            const LogObjectList objlist(commandBuffer, vs_state->Handle(), pEncodeInfo->dstBuffer);
            skip |= LogError("VUID-VkVideoEncodeInfoKHR-dstBufferOffset-08237",
                             objlist,
                             encode_info_loc.dot(Field::dstBufferOffset),
                             "(%" PRIu64 ") must be less than the size (%" PRIu64 ") of pEncodeInfo->dstBuffer (%s).",
                             pEncodeInfo->dstBufferOffset,
                             buffer_state->create_info.size,
                             FormatHandle(pEncodeInfo->dstBuffer).c_str());
        }
        ...
        if (!IsIntegerMultipleOf(pEncodeInfo->dstBufferOffset, profile_caps.base.minBitstreamBufferOffsetAlignment)) {
            const LogObjectList objlist(commandBuffer, vs_state->Handle());
            skip |=
                LogError("VUID-vkCmdEncodeVideoKHR-pEncodeInfo-08204",
                         objlist,
                         encode_info_loc.dot(Field::dstBufferOffset),
                         "(%" PRIu64 ") is not an integer multiple of the minBitstreamBufferOffsetAlignment (%" PRIu64
                         ") required by the video profile %s was created with.",
    ...
    if (!IsIntegerMultipleOf(pEncodeInfo->dstBufferRange, profile_caps.base.minBitstreamBufferSizeAlignment)) {
        const LogObjectList objlist(commandBuffer, vs_state->Handle());
        skip |= LogError("VUID-vkCmdEncodeVideoKHR-pEncodeInfo-08205",
                         objlist,
                         encode_info_loc.dot(Field::dstBufferRange),
                         "(%" PRIu64 ") is not an integer multiple of the minBitstreamBufferSizeAlignment (%" PRIu64
                         ") required by the video profile %s was created with.",
                         pEncodeInfo->dstBufferRange,
                         profile_caps.base.minBitstreamBufferSizeAlignment,
    ...
    if (vs_state->create_info.maxDpbSlots > 0 && pEncodeInfo->pSetupReferenceSlot == nullptr) {
        const LogObjectList objlist(commandBuffer, vs_state->Handle());
        skip |= LogError("VUID-vkCmdEncodeVideoKHR-pEncodeInfo-08377",
                         objlist,
                         encode_info_loc.dot(Field::pSetupReferenceSlot),
                         "is NULL but the bound video session %s was created with maxDpbSlot %u.",
                         FormatHandle(*vs_state).c_str(),
                         vs_state->create_info.maxDpbSlots);
    }
    ...
    if (pEncodeInfo->pSetupReferenceSlot) {
        if (pEncodeInfo->pSetupReferenceSlot->slotIndex < 0) {
            skip |= LogError("VUID-VkVideoEncodeInfoKHR-pSetupReferenceSlot-08239",
                             commandBuffer,
            ...
            skip |= LogError("VUID-vkCmdEncodeVideoKHR-pEncodeInfo-08213",
                             commandBuffer,
                             encode_info_loc.dot(Field::pSetupReferenceSlot).dot(Field::slotIndex),
                             "(%d) must be smaller than the maxDpbSlots (%u) the bound video session %s "
                             "was created with.",
        ...
        if (pEncodeInfo->pSetupReferenceSlot->pPictureResource != nullptr) {
            ...
            if (setup_resource) {
                ...
                if (bound_resources.find(setup_resource) == bound_resources.end()) {
                    skip |= LogError("VUID-vkCmdEncodeVideoKHR-pEncodeInfo-08215",
                                     commandBuffer,
                                     error_obj.location,
                                     "the video picture resource specified in "
            ...
            skip |= LogError("VUID-VkVideoEncodeInfoKHR-pSetupReferenceSlot-08240",
                             commandBuffer,
                             encode_info_loc.dot(Field::pSetupReferenceSlot).dot(Field::pPictureResource),
                             "is NULL.");
        }
    ...
    skip |= ValidateVideoPictureResource(src_resource,
                                         commandBuffer,
                                         *vs_state,
                                         src_resource_loc,
    ...
    if (src_resource) {
        ...
        skip |= ValidateProtectedImage(
            *cb_state, *src_resource.image_state, src_image_view_loc, "VUID-vkCmdEncodeVideoKHR-commandBuffer-08211");

        if (!IsImageCompatibleWithVideoSession(*src_resource.image_state, *vs_state)) {
            const LogObjectList objlist(commandBuffer,
                                        vs_state->Handle(),
                                        src_resource.image_view_state->Handle(),
                                        src_resource.image_state->Handle());
            skip |= LogError("VUID-vkCmdEncodeVideoKHR-pEncodeInfo-08206",
                             objlist,
                             src_image_view_loc,
                             "(%s created from %s) is not compatible with the video profile the bound "
                             "video session %s was created with.",
        ...
        if (src_resource.image_view_state->create_info.format != vs_state->create_info.pictureFormat) {
            const LogObjectList objlist(commandBuffer,
                                        ...
                                        src_resource.image_view_state->Handle(),
                                        src_resource.image_state->Handle());
            skip |= LogError("VUID-vkCmdEncodeVideoKHR-pEncodeInfo-08207",
                             objlist,
                             src_image_view_loc,
                             "(%s created from %s) format (%s) does not match the pictureFormat (%s) "
                             "the bound video session %s was created with.",
        ...
        if ((supported_usage & VK_IMAGE_USAGE_VIDEO_ENCODE_SRC_BIT_KHR) == 0) {
            const LogObjectList objlist(commandBuffer,
                                        ...
                                        src_resource.image_view_state->Handle(),
                                        src_resource.image_state->Handle());
            skip |= LogError("VUID-vkCmdEncodeVideoKHR-pEncodeInfo-08210",
                             objlist,
                             src_image_view_loc,
    ...
    if (pEncodeInfo->pReferenceSlots) {
        ...
        for (uint32_t i = 0; i < pEncodeInfo->referenceSlotCount; ++i) {
            if (pEncodeInfo->pReferenceSlots[i].slotIndex < 0) {
                skip |= LogError("VUID-VkVideoEncodeInfoKHR-slotIndex-08241",
                                 commandBuffer,
                ...
                const LogObjectList objlist(commandBuffer, vs_state->Handle());
                skip |= LogError("VUID-vkCmdEncodeVideoKHR-slotIndex-08217",
                                 objlist,
                                 encode_info_loc.dot(Field::pReferenceSlots, i).dot(Field::slotIndex),
                                 "(%d) "
            ...
            if (pEncodeInfo->pReferenceSlots[i].pPictureResource != nullptr) {
                auto reference_resource =
                    vvl::VideoPictureResource(*this, *pEncodeInfo->pReferenceSlots[i].pPictureResource);
                if (reference_resource) {
                    if (!unique_resources.emplace(reference_resource).second) {
                        resources_unique = false;
                    }
                    ...
                    const auto& it = bound_resources.find(reference_resource);
                    if (it == bound_resources.end()) {
                        skip |= LogError("VUID-vkCmdEncodeVideoKHR-pPictureResource-08219",
                                         commandBuffer,
                                         error_obj.location,
                                         "the video picture resource specified in "
                ...
                skip |= LogError("VUID-VkVideoEncodeInfoKHR-pPictureResource-08242",
                                 commandBuffer,
                                 encode_info_loc.dot(Field::pReferenceSlots, i).dot(Field::pPictureResource),
                                 "is NULL.");
            }
        ...
        if (!resources_unique) {
            skip |= LogError("VUID-vkCmdEncodeVideoKHR-pPictureResource-08220",
                             commandBuffer,
                             error_obj.location,
    ...
    for (const auto& query : cb_state->activeQueries) {
        if (query.active_query_index + op_count > query.last_activatable_query_index + 1) {
            auto query_pool_state = Get<vvl::QueryPool>(query.pool);
            skip |= LogError("VUID-vkCmdEncodeVideoKHR-opCount-07174",
                             commandBuffer,
                             error_obj.location,
                             "not enough activatable queries for query type %s "
                             "with opCount %u, active query index %u, and last activatable query index %u.",
    ...
    if (vs_state->create_info.flags & VK_VIDEO_SESSION_CREATE_INLINE_QUERIES_BIT_KHR) {
        ...
        if (inline_query_info != nullptr) {
            if (auto query_pool_state = Get<vvl::QueryPool>(inline_query_info->queryPool)) {
                skip |= ValidateVideoInlineQueryInfo(
                    *query_pool_state, *inline_query_info, encode_info_loc.pNext(Struct::VkVideoInlineQueryInfoKHR));

                if (inline_query_info->queryCount != op_count) {
                    const LogObjectList objlist(commandBuffer, vs_state->Handle());
                    skip |= LogError("VUID-vkCmdEncodeVideoKHR-pNext-08360",
                                     objlist,
                ...
                if (query_pool_state->create_info.queryType != VK_QUERY_TYPE_RESULT_STATUS_ONLY_KHR &&
                    ...
                    const LogObjectList objlist(commandBuffer, inline_query_info->queryPool);
                    skip |= LogError("VUID-vkCmdEncodeVideoKHR-queryType-08362",
                                     objlist,
                ...
                if (vs_state->profile != query_pool_state->supported_video_profile) {
                    const LogObjectList objlist(commandBuffer, inline_query_info->queryPool, vs_state->Handle());
                    skip |= LogError("VUID-vkCmdEncodeVideoKHR-queryPool-08363",
                                     objlist,
                                     encode_info_loc.pNext(Struct::VkVideoInlineQueryInfoKHR, Field::queryPool),
                                     "video profile %s that was created with does not "
                                     "match the video profile of the bound video session %s.",
                ...
                if (query_pool_state->create_info.queryType == VK_QUERY_TYPE_RESULT_STATUS_ONLY_KHR &&
                    ...
                    const LogObjectList objlist(commandBuffer, inline_query_info->queryPool);
                    skip |= LogError("VUID-vkCmdEncodeVideoKHR-queryType-08364",
                                     objlist,
                                     error_obj.location,
                                     "the command pool's queue family (index %u) the command "
                                     "buffer %s was allocated from does not support result status queries.",
...
void CoreChecks::PreCallRecordCmdEncodeVideoKHR(VkCommandBuffer commandBuffer,
                                                const VkVideoEncodeInfoKHR* pEncodeInfo,
                                                const RecordObject& record_obj) {
    auto cb_state = GetWrite<vvl::CommandBuffer>(commandBuffer);
    if (!cb_state)
    ...
    if (vs_state->IsEncode() && cb_state->bound_video_session_parameters) {
        if (!cb_state->video_encode_quality_level.has_value()) {
            ...
            cb_state->video_session_updates[vs_state->VkHandle()].emplace_back(
                [vsp_state = cb_state->bound_video_session_parameters, loc](const ValidationStateTracker& dev_data,
                                                                            const vvl::VideoSession* vs_state,
                                                                            vvl::VideoSessionDeviceState& dev_state,
                                                                            bool do_validate) {
                    if (!do_validate)
                    ...
                    if (vsp_state->GetEncodeQualityLevel() != dev_state.GetEncodeQualityLevel()) {
                        const LogObjectList objlist(vs_state->Handle(), vsp_state->Handle());
                        skip |= dev_data.LogError("VUID-vkCmdEncodeVideoKHR-None-08318",
                                                  objlist,
                                                  loc,
                                                  "The currently configured encode quality level (%u) for %s "
                                                  "does not match the encode quality level (%u) %s was created with.",
                                                  dev_state.GetEncodeQualityLevel(),
                                                  dev_data.FormatHandle(*vs_state).c_str(),
                                                  vsp_state->GetEncodeQualityLevel(),
                                                  dev_data.FormatHandle(*vsp_state).c_str());
                    }
                    return skip;
                });
        }
