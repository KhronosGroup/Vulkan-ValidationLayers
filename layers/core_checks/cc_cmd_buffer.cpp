...
bool CoreChecks::ValidateCmdExecuteCommandsRenderPassInheritance(const vvl::CommandBuffer &cb_state,
    ...
    if (auto *tile_mem_bind_info = vku::FindStructInPNextChain<VkTileMemoryBindInfoQCOM>(inheritance_info.pNext)) {
        ...
        const VkDeviceSize active_tile_mem_size = (active_tile_mem != nullptr) ? active_tile_mem->allocate_info.allocationSize : 0;
        auto tile_mem_bind = Get<vvl::DeviceMemory>(tile_mem_bind_info->memory);
        const VkDeviceSize tile_mem_bind_size = tile_mem_bind->allocate_info.allocationSize;
