...
bool CoreChecks::ReportInvalidCommandBuffer(const vvl::CommandBuffer& cb_state,
                                            const Location& loc,
    ...
    for (const auto& entry : cb_state.broken_bindings) {
        ...
        skip |= LogError(vuid,
                         objlist,
                         loc,
                         "was called in %s which is invalid because bound %s was destroyed%s.",
                         FormatHandle(cb_state).c_str(),
                         FormatHandle(obj).c_str(),
                         cause_str);
    }
    return skip;
}

bool CoreChecks::PreCallValidateFreeCommandBuffers(VkDevice device,
    ...
    for (uint32_t i = 0; i < commandBufferCount; i++) {
        ...
        if (cb_state && cb_state->InUse()) {
            ...
            skip |= LogError("VUID-vkFreeCommandBuffers-pCommandBuffers-00047",
                             objlist,
                             error_obj.location.dot(Field::pCommandBuffers, i),
                             "(%s) is in use.",
...
bool CoreChecks::PreCallValidateBeginCommandBuffer(VkCommandBuffer commandBuffer,
    ...
    bool skip = false;
    if (cb_state->InUse()) {
        skip |= LogError("VUID-vkBeginCommandBuffer-commandBuffer-00049",
                         commandBuffer,
                         error_obj.location,
    ...
    if (cb_state->IsPrimary()) {
        ...
        if ((pBeginInfo->flags & invalid_usage) == invalid_usage) {
            skip |= LogError("VUID-vkBeginCommandBuffer-commandBuffer-02840",
                             commandBuffer,
        ...
        if (!info) {
            skip |= LogError("VUID-vkBeginCommandBuffer-commandBuffer-00051",
                             commandBuffer,
                             inheritance_loc,
                             "is null for Secondary %s.",
                             FormatHandle(commandBuffer).c_str());
        } else {
            auto p_inherited_rendering_info =
                vku::FindStructInPNextChain<VkCommandBufferInheritanceRenderingInfo>(info->pNext);
            ...
            if (pBeginInfo->flags & VK_COMMAND_BUFFER_USAGE_RENDER_PASS_CONTINUE_BIT) {
                if (info->renderPass != VK_NULL_HANDLE) {
                    if (auto framebuffer = Get<vvl::Framebuffer>(info->framebuffer)) {
                        if (framebuffer->create_info.renderPass != info->renderPass) {
                            if (auto render_pass = Get<vvl::RenderPass>(info->renderPass)) {
                                ...
                                skip |= ValidateRenderPassCompatibility(framebuffer->Handle(),
                                                                        *framebuffer->rp_state.get(),
                                                                        cb_state->Handle(),
                                                                        *render_pass.get(),
                                                                        inheritance_loc,
                    ...
                    if (!render_pass) {
                        skip |= LogError("VUID-VkCommandBufferBeginInfo-flags-06000",
                                         commandBuffer,
                                         inheritance_loc.dot(Field::renderPass),
                                         "is not a valid VkRenderPass.");
                    } else {
                        if (info->subpass >= render_pass->create_info.subpassCount) {
                            skip |= LogError("VUID-VkCommandBufferBeginInfo-flags-06001",
                                             commandBuffer,
                                             inheritance_loc.dot(Field::subpass),
                                             "(%" PRIu32
                                             ") is not valid, renderPass was created with subpassCount %" PRIu32 ".",
                                             info->subpass,
                                             render_pass->create_info.subpassCount);
                        }
                    }
                } else {
                    if (!p_inherited_rendering_info) {
                        skip |= LogError("VUID-VkCommandBufferBeginInfo-flags-06002",
                                         commandBuffer,
            ...
            if (p_inherited_rendering_info) {
                auto p_attachment_sample_count_info_amd =
                    vku::FindStructInPNextChain<VkAttachmentSampleCountInfoAMD>(info->pNext);
                if (p_attachment_sample_count_info_amd && p_attachment_sample_count_info_amd->colorAttachmentCount !=
                ...
                if ((p_inherited_rendering_info->colorAttachmentCount != 0) &&
                    ...
                    skip |= LogError("VUID-VkCommandBufferInheritanceRenderingInfo-colorAttachmentCount-06004",
                                     commandBuffer,
                                     inheritance_loc.pNext(Struct::VkCommandBufferInheritanceRenderingInfo,
                                                           Field::colorAttachmentCount),
                ...
                if ((!enabled_features.variableMultisampleRate) &&
                    (p_inherited_rendering_info->rasterizationSamples & AllVkSampleCountFlagBits) == 0) {
                    skip |= LogError("VUID-VkCommandBufferInheritanceRenderingInfo-variableMultisampleRate-06005",
                                     commandBuffer,
                                     inheritance_loc.pNext(Struct::VkCommandBufferInheritanceRenderingInfo,
                                                           Field::rasterizationSamples),
                                     "is not valid (0x%" PRIx32
                                     ") and the variableMultisampleRate feature was not enabled.",
                                     p_inherited_rendering_info->rasterizationSamples);
                }
                            ...
                            if ((potential_format_features & (VK_FORMAT_FEATURE_2_COLOR_ATTACHMENT_BIT |
                                ...
                                skip |= LogError(
                                    ...
                                    inheritance_loc.pNext(Struct::VkCommandBufferInheritanceRenderingInfo,
                                                          ...
                                                          i),
                                    "is %s with potential format features %s.",
                ...
                const VkFormatFeatureFlags2 valid_depth_stencil_format =
                ...
                if (depth_format != VK_FORMAT_UNDEFINED) {
                    ...
                    if ((potential_format_features & valid_depth_stencil_format) == 0) {
                        skip |= LogError("VUID-VkCommandBufferInheritanceRenderingInfo-depthAttachmentFormat-06007",
                                         commandBuffer,
                                         inheritance_loc.pNext(Struct::VkCommandBufferInheritanceRenderingInfo,
                                                               Field::depthAttachmentFormat),
                                         "is %s with potential format features %s.",
                                         string_VkFormat(depth_format),
                                         string_VkFormatFeatureFlags2(potential_format_features).c_str());
                    }
                    if (!vkuFormatHasDepth(depth_format)) {
                        skip |= LogError("VUID-VkCommandBufferInheritanceRenderingInfo-depthAttachmentFormat-06540",
                                         commandBuffer,
                                         inheritance_loc.pNext(Struct::VkCommandBufferInheritanceRenderingInfo,
                                                               Field::depthAttachmentFormat),
                                         "%s is not a depth format.",
                                         string_VkFormat(depth_format));
                    }
                }
                ...
                if (stencil_format != VK_FORMAT_UNDEFINED) {
                    ...
                    if ((potential_format_features & valid_depth_stencil_format) == 0) {
                        skip |= LogError("VUID-VkCommandBufferInheritanceRenderingInfo-stencilAttachmentFormat-06199",
                                         commandBuffer,
                                         inheritance_loc.pNext(Struct::VkCommandBufferInheritanceRenderingInfo,
                                                               Field::stencilAttachmentFormat),
                                         "is %s with potential format features %s.",
                                         string_VkFormat(stencil_format),
                                         string_VkFormatFeatureFlags2(potential_format_features).c_str());
                    }
                    if (!vkuFormatHasStencil(stencil_format)) {
                        skip |= LogError("VUID-VkCommandBufferInheritanceRenderingInfo-stencilAttachmentFormat-06541",
                                         commandBuffer,
                                         inheritance_loc.pNext(Struct::VkCommandBufferInheritanceRenderingInfo,
                                                               Field::stencilAttachmentFormat),
                                         "%s is not a stencil format.",
                                         string_VkFormat(stencil_format));
                    }
                }
                ...
                if ((depth_format != VK_FORMAT_UNDEFINED && stencil_format != VK_FORMAT_UNDEFINED) &&
                    ...
                    skip |= LogError("VUID-VkCommandBufferInheritanceRenderingInfo-depthAttachmentFormat-06200",
                                     commandBuffer,
                                     inheritance_loc.pNext(Struct::VkCommandBufferInheritanceRenderingInfo,
                                                           Field::depthAttachmentFormat),
        ...
        if (info) {
            if ((info->occlusionQueryEnable == VK_FALSE || enabled_features.occlusionQueryPrecise == VK_FALSE) &&
                ...
                skip |=
                    LogError("VUID-vkBeginCommandBuffer-commandBuffer-00052",
                             commandBuffer,
                             inheritance_loc,
            ...
            if (p_inherited_viewport_scissor_info != nullptr && p_inherited_viewport_scissor_info->viewportScissor2D) {
                if (!enabled_features.inheritedViewportScissor2D) {
                    skip |= LogError("VUID-VkCommandBufferInheritanceViewportScissorInfoNV-viewportScissor2D-04782",
                                     commandBuffer,
                                     inheritance_loc.pNext(Struct::VkCommandBufferInheritanceViewportScissorInfoNV,
                                                           Field::viewportScissor2D),
                                     "is VK_TRUE, but the inheritedViewportScissor2D feature was not enabled.");
                }
                if (!(pBeginInfo->flags & VK_COMMAND_BUFFER_USAGE_RENDER_PASS_CONTINUE_BIT)) {
                    skip |= LogError("VUID-VkCommandBufferInheritanceViewportScissorInfoNV-viewportScissor2D-04786",
                                     commandBuffer,
                                     begin_info_loc.dot(Field::flags),
                                     "is %s for Secondary %s (and viewportScissor2D is VK_TRUE).",
                                     string_VkCommandBufferUsageFlags(pBeginInfo->flags).c_str(),
                                     FormatHandle(commandBuffer).c_str());
                }
                if (p_inherited_viewport_scissor_info->viewportDepthCount == 0) {
                    skip |= LogError("VUID-VkCommandBufferInheritanceViewportScissorInfoNV-viewportScissor2D-04784",
                                     commandBuffer,
                                     inheritance_loc.pNext(Struct::VkCommandBufferInheritanceViewportScissorInfoNV,
                                                           Field::viewportDepthCount),
            ...
            if ((api_version < VK_API_VERSION_1_3) && (!enabled_features.dynamicRendering)) {
                if (pBeginInfo->flags & VK_COMMAND_BUFFER_USAGE_RENDER_PASS_CONTINUE_BIT) {
                    if (info->renderPass == VK_NULL_HANDLE) {
                        skip |= LogError("VUID-VkCommandBufferBeginInfo-flags-09240",
                                         commandBuffer,
                                         begin_info_loc.dot(Field::flags),
    ...
    if (CbState::Recording == cb_state->state) {
        ...
        if (!(VK_COMMAND_POOL_CREATE_RESET_COMMAND_BUFFER_BIT & pool->createFlags)) {
            ...
            skip |= LogError("VUID-vkBeginCommandBuffer-commandBuffer-00050",
                             objlist,
                             error_obj.location,
                             ...
                             FormatHandle(commandBuffer).c_str(),
                             FormatHandle(cmd_pool).c_str());
        }
    }
    auto chained_device_group_struct =
        vku::FindStructInPNextChain<VkDeviceGroupCommandBufferBeginInfo>(pBeginInfo->pNext);
    if (chained_device_group_struct) {
        const LogObjectList objlist(commandBuffer);
        skip |= ValidateDeviceMaskToPhysicalDeviceCount(
            chained_device_group_struct->deviceMask,
            objlist,
            begin_info_loc.pNext(Struct::VkDeviceGroupCommandBufferBeginInfo, Field::deviceMask),
            "VUID-VkDeviceGroupCommandBufferBeginInfo-deviceMask-00106");
        skip |= ValidateDeviceMaskToZero(
            chained_device_group_struct->deviceMask,
            objlist,
            begin_info_loc.pNext(Struct::VkDeviceGroupCommandBufferBeginInfo, Field::deviceMask),
    ...
    if ((pBeginInfo->flags & VK_COMMAND_BUFFER_USAGE_RENDER_PASS_CONTINUE_BIT) != 0) {
        if ((cb_state->command_pool->queue_flags & VK_QUEUE_GRAPHICS_BIT) == 0) {
            ...
            skip |=
                LogError("VUID-VkCommandBufferBeginInfo-flags-09123",
                         objlist,
                         begin_info_loc.dot(Field::flags),
...
bool CoreChecks::PreCallValidateEndCommandBuffer(VkCommandBuffer commandBuffer, const ErrorObject& error_obj) const {
    ...
    if (cb_state.state == CbState::InvalidComplete || cb_state.state == CbState::InvalidIncomplete) {
        ...
        skip |=
            LogError("VUID-vkEndCommandBuffer-commandBuffer-00059",
                     commandBuffer,
                     error_obj.location,
    ...
    for (const auto& query_obj : cb_state.activeQueries) {
        skip |= LogError("VUID-vkEndCommandBuffer-commandBuffer-00061",
                         commandBuffer,
                         error_obj.location,
    ...
    if (cb_state.conditional_rendering_active) {
        skip |= LogError("VUID-vkEndCommandBuffer-None-01978",
                         commandBuffer,
                         error_obj.location,
...
bool CoreChecks::PreCallValidateResetCommandBuffer(VkCommandBuffer commandBuffer,
    ...
    if (!(VK_COMMAND_POOL_CREATE_RESET_COMMAND_BUFFER_BIT & pool->createFlags)) {
        const LogObjectList objlist(commandBuffer, cmd_pool);
        skip |= LogError("VUID-vkResetCommandBuffer-commandBuffer-00046",
                         objlist,
                         error_obj.location,
                         "%s was created from %s  which was created with %s.",
                         FormatHandle(commandBuffer).c_str(),
                         FormatHandle(cmd_pool).c_str(),
                         string_VkCommandPoolCreateFlags(pool->createFlags).c_str());
    }

    if (cb_state->InUse()) {
        const LogObjectList objlist(commandBuffer, cmd_pool);
        skip |= LogError("VUID-vkResetCommandBuffer-commandBuffer-00045",
                         objlist,
                         error_obj.location,
                         "(%s) is in use.",
                         FormatHandle(commandBuffer).c_str());
...
bool CoreChecks::ValidateCmdBindIndexBuffer(const vvl::CommandBuffer& cb_state,
                                            VkBuffer buffer,
                                            VkDeviceSize offset,
    ...
    if (!buffer_state)
        return skip; // if using nullDescriptors
    ...
    skip |= ValidateBufferUsageFlags(
        objlist, *buffer_state, VK_BUFFER_USAGE_INDEX_BUFFER_BIT, true, vuid, loc.dot(Field::buffer));
    ...
    if (!IsIntegerMultipleOf(offset, offset_align)) {
        ...
        skip |= LogError(vuid,
                         objlist,
                         loc.dot(Field::offset),
                         "(%" PRIu64 ") does not fall on alignment (%s) boundary.",
                         offset,
    ...
    if (offset >= buffer_state->create_info.size) {
        ...
        skip |= LogError(vuid,
                         objlist,
                         loc.dot(Field::offset),
                         "(%" PRIu64 ") is not less than the size (%" PRIu64 ").",
                         offset,
...
bool CoreChecks::PreCallValidateCmdBindIndexBuffer(VkCommandBuffer commandBuffer,
                                                   VkBuffer buffer,
                                                   VkDeviceSize offset,
                                                   VkIndexType indexType,
                                                   const ErrorObject& error_obj) const {
...
bool CoreChecks::PreCallValidateCmdBindIndexBuffer2(VkCommandBuffer commandBuffer,
                                                    VkBuffer buffer,
                                                    VkDeviceSize offset,
                                                    VkDeviceSize size,
                                                    VkIndexType indexType,
    ...
    if (size != VK_WHOLE_SIZE && buffer != VK_NULL_HANDLE) {
        ...
        if (!IsIntegerMultipleOf(size, offset_align)) {
            ...
            skip |= LogError("VUID-vkCmdBindIndexBuffer2-size-08767",
                             objlist,
        ...
        if ((offset + size) > buffer_state->create_info.size) {
            const LogObjectList objlist(commandBuffer, buffer);
            skip |= LogError("VUID-vkCmdBindIndexBuffer2-size-08768",
                             objlist,
                             error_obj.location.dot(Field::size),
                             "(%" PRIu64 ") + offset (%" PRIu64 ") is larger than the buffer size (%" PRIu64 ").",
                             size,
                             offset,
                             buffer_state->create_info.size);
        }
...
bool CoreChecks::PreCallValidateCmdBindIndexBuffer2KHR(VkCommandBuffer commandBuffer,
                                                       VkBuffer buffer,
                                                       VkDeviceSize offset,
                                                       VkDeviceSize size,
                                                       VkIndexType indexType,
...
bool CoreChecks::PreCallValidateCmdBindVertexBuffers(VkCommandBuffer commandBuffer,
    ...
    for (uint32_t i = 0; i < bindingCount; ++i) {
        ...
        if (!buffer_state)
            continue; // if using nullDescriptors

        const LogObjectList objlist(commandBuffer, buffer_state->Handle());
        skip |= ValidateBufferUsageFlags(objlist,
                                         *buffer_state,
                                         VK_BUFFER_USAGE_VERTEX_BUFFER_BIT,
                                         true,
                                         "VUID-vkCmdBindVertexBuffers-pBuffers-00627",
                                         error_obj.location.dot(Field::pBuffers, i));
        skip |= ValidateMemoryIsBoundToBuffer(commandBuffer,
                                              *buffer_state,
                                              error_obj.location.dot(Field::pBuffers, i),
                                              "VUID-vkCmdBindVertexBuffers-pBuffers-00628");
        if (pOffsets[i] >= buffer_state->create_info.size) {
            skip |= LogError("VUID-vkCmdBindVertexBuffers-pOffsets-00626",
                             objlist,
                             error_obj.location.dot(Field::pOffsets, i),
                             "(%" PRIu64 ") is larger than the buffer size (%" PRIu64 ").",
                             pOffsets[i],
                             buffer_state->create_info.size);
        }
    }
...
bool CoreChecks::PreCallValidateCmdUpdateBuffer(VkCommandBuffer commandBuffer,
                                                VkBuffer dstBuffer,
    ...
    skip |= ValidateMemoryIsBoundToBuffer(
        commandBuffer, *dst_buffer_state, buffer_loc, "VUID-vkCmdUpdateBuffer-dstBuffer-00035");
    ...
    skip |= ValidateCmd(*cb_state, error_obj.location);
    skip |=
        ValidateProtectedBuffer(*cb_state, *dst_buffer_state, buffer_loc, "VUID-vkCmdUpdateBuffer-commandBuffer-01813");
    skip |= ValidateUnprotectedBuffer(
        *cb_state, *dst_buffer_state, buffer_loc, "VUID-vkCmdUpdateBuffer-commandBuffer-01814");
    if (dstOffset >= dst_buffer_state->create_info.size) {
        skip |= LogError("VUID-vkCmdUpdateBuffer-dstOffset-00032",
                         objlist,
        ...
        skip |= LogError("VUID-vkCmdUpdateBuffer-dataSize-00033",
                         objlist,
                         error_obj.location.dot(Field::dataSize),
                         "(%" PRIu64 ") is not less than the buffer size (%" PRIu64 ") minus dstOffset (%" PRIu64 ").",
                         dataSize,
                         dst_buffer_state->create_info.size,
                         dstOffset);
    }
...
bool CoreChecks::ValidatePrimaryCommandBuffer(const vvl::CommandBuffer& cb_state,
                                              const Location& loc,
                                              const char* vuid) const {
    bool skip = false;
...
bool CoreChecks::ValidateSecondaryCommandBufferState(const vvl::CommandBuffer& cb_state,
    ...
    if (!disabled[query_validation]) {
        for (const auto& query_object : cb_state.activeQueries) {
            ...
            if (query_pool_state->create_info.queryType == VK_QUERY_TYPE_PIPELINE_STATISTICS &&
                ...
                VkQueryPipelineStatisticFlags cmd_buf_statistics =
                    sub_cb_state.beginInfo.pInheritanceInfo->pipelineStatistics;
        ...
        for (const auto& query_object : sub_cb_state.startedQueries) {
            ...
            if (active_types.count(query_pool_state->create_info.queryType)) {
                const LogObjectList objlist(cb_state.Handle(), sub_cb_state.Handle(), query_object.pool);
                skip |=
                    LogError("VUID-vkCmdExecuteCommands-pCommandBuffers-00105",
                             objlist,
                             cb_loc,
                             ...
                             FormatHandle(sub_cb_state).c_str());
            }
        }
    }
    ...
    if (primary_pool && secondary_pool && (primary_pool->queueFamilyIndex != secondary_pool->queueFamilyIndex)) {
        ...
        skip |= LogError("VUID-vkCmdExecuteCommands-pCommandBuffers-00094",
                         objlist,
                         cb_loc,
...
// Object that simulates the inherited viewport/scissor state as the device executes the called secondary command
// buffers. Visit the calling primary command buffer first, then the called secondaries in order. Contact David Zhao
// Akeley <dakeley@nvidia.com> for clarifications and bug fixes.
class CoreChecks::ViewportScissorInheritanceTracker {
    static_assert(4 == sizeof(vvl::CommandBuffer::viewportMask),
                  "Adjust max_viewports to match viewportMask bit width");
  ...
  private:
    ...
    bool VisitSecondaryNoInheritance(uint32_t cmd_buffer_idx, const vvl::CommandBuffer& secondary_state) {
        ...
        for (uint32_t n = 0; n < kMaxViewports; ++n) {
            ...
            // Check trashing dynamic viewport/scissor count in VisitSecondary (at end) as even secondary command
            // buffers enabling viewport/scissor state inheritance may define this state statically in bound graphics
            // pipelines.
        }
        return false;
    }
    ...
    bool VisitSecondaryInheritance(uint32_t cmd_buffer_idx,
                                   const Location& cb_loc,
                                   const vvl::CommandBuffer& secondary_state) {
        bool skip = false;
        ...
        auto check_missing_inherit = [&](uint32_t was_ever_defined,
                                         uint32_t trashed_by,
                                         VkDynamicState state,
                                         uint32_t index = 0,
                                         uint32_t static_use_count = 0,
            ...
            if (was_ever_defined && trashed_by == kNotTrashed) {
                ...
                if (inherited_viewport->minDepth != expected_viewport_depth->minDepth ||
                    ...
                    return validation_.LogError(
                        ...
                        primary_state_->Handle(),
                        cb_loc,
                        "(%s) consume inherited viewport %" PRIu32 " %s"
                        "but this state was not inherited as its depth range [%f, %f] does not match "
                        "pViewportDepths[%" PRIu32 "] = [%f, %f]",
                        validation_.FormatHandle(secondary_state.Handle()).c_str(),
                        unsigned(index),
                        index >= static_use_count ? "(with count) " : "",
                        inherited_viewport->minDepth,
                        inherited_viewport->maxDepth,
                        unsigned(cmd_buffer_idx),
                        expected_viewport_depth->minDepth,
            ...
            std::stringstream ss;
            ss << "(" << validation_.FormatHandle(secondary_state.Handle()).c_str() << ") consume inherited "
               << state_name << " ";
            if (format_index) {
            ...
            return validation_.LogError(
                "VUID-vkCmdDraw-None-07850", primary_state_->Handle(), cb_loc, "%s", ss.str().c_str());
        };
        ...
        if (secondary_state.usedDynamicViewportCount) {
            if (viewport_count_to_inherit_ == 0 || viewport_count_trashed_by_ != kNotTrashed) {
                skip |= check_missing_inherit(
                    viewport_count_to_inherit_, viewport_count_trashed_by_, VK_DYNAMIC_STATE_VIEWPORT_WITH_COUNT);
            } else {
        ...
        if (secondary_state.usedDynamicScissorCount) {
            if (scissor_count_to_inherit_ == 0 || scissor_count_trashed_by_ != kNotTrashed) {
                skip |= check_missing_inherit(
                    scissor_count_to_inherit_, scissor_count_trashed_by_, VK_DYNAMIC_STATE_SCISSOR_WITH_COUNT);
            } else {
        ...
        check_viewport_count =
            std::min(std::min(kMaxViewports, uint32_t(secondary_state.inheritedViewportDepths.size())),
                     std::max(check_viewport_count, secondary_state.usedViewportScissorCount));
        check_scissor_count =
            std::min(kMaxViewports, std::max(check_scissor_count, secondary_state.usedViewportScissorCount));

        if (secondary_state.usedDynamicViewportCount &&
            viewport_count_to_inherit_ > secondary_state.inheritedViewportDepths.size()) {
            skip |= validation_.LogError("VUID-vkCmdDraw-None-07850",
                                         primary_state_->Handle(),
                                         cb_loc,
...
bool CoreChecks::PreCallValidateCmdExecuteCommands(VkCommandBuffer commandBuffer,
    ...
    if (!cb_state.IsPrimary()) {
        if (!enabled_features.nestedCommandBuffer) {
            skip |= LogError("VUID-vkCmdExecuteCommands-commandBuffer-09375",
                             commandBuffer,
            ...
            skip |=
                LogError("VUID-vkCmdExecuteCommands-nestedCommandBufferRendering-09377",
                         commandBuffer,
                         error_obj.location.dot(Field::commandBuffer),
                         "is a secondary command buffer and was recorded with VkCommandBufferBeginInfo::flag including "
                         "VK_COMMAND_BUFFER_USAGE_RENDER_PASS_CONTINUE_BIT, but the nestedCommandBufferRendering "
    ...
    for (const auto& active_query : cb_state.activeQueries) {
        ...
        if (queryType != VK_QUERY_TYPE_OCCLUSION && queryType != VK_QUERY_TYPE_PIPELINE_STATISTICS) {
            skip |= LogError("VUID-vkCmdExecuteCommands-commandBuffer-07594",
                             commandBuffer,
                             error_obj.location,
                             "query with type %s is active.",
        ...
        if (!cb_state.activeRenderPass->UsesDynamicRendering() && cb_state.IsPrimary()) {
            ...
            if (cb_state.activeSubpassContents != VK_SUBPASS_CONTENTS_SECONDARY_COMMAND_BUFFERS &&
                ...
                if (cb_state.GetActiveSubpass() == 0) {
                    ...
                    skip |= LogError("VUID-vkCmdExecuteCommands-contents-09680",
                                     objlist,
                                     error_obj.location,
    ...
    for (uint32_t i = 0; i < commandBuffersCount; i++) {
        ...
        if (!sub_cb_state.IsSecondary()) {
            ...
            skip |= LogError("VUID-vkCmdExecuteCommands-pCommandBuffers-00088",
                             objlist,
                             cb_loc,
                             "(%s) is not VK_COMMAND_BUFFER_LEVEL_SECONDARY.",
                             FormatHandle(pCommandBuffers[i]).c_str());
        } else {
            if (cb_state.activeRenderPass) {
                if (cb_state.hasRenderPassInstance && cb_state.activeRenderPass->UsesDynamicRendering() &&
                    ...
                    if (location_info) {
                        ...
                        if (location_info->colorAttachmentCount !=
                            cb_state.rendering_attachments.color_indexes.size()) {
                            skip |= LogError(
                                vuid_090504,
                                objlist,
                            ...
                            for (uint32_t idx = 0; idx < location_info->colorAttachmentCount; idx++) {
                                if (location_info->pColorAttachmentLocations &&
                                    ...
                                    skip |= LogError(vuid_090504,
                                                     objlist,
                                                     cb_loc.pNext(Struct::VkRenderingAttachmentLocationInfo,
                                                                  Field::pColorAttachmentInputIndices,
                                                                  idx),
                                                     "(%" PRIu32 ") does not match the implicit or explicit state in "
                                                     "the primary command buffer (%" PRIu32 ").",
                                                     location_info->pColorAttachmentLocations[idx],
                                                     cb_state.rendering_attachments.color_locations[idx]);
                                }
                            }
                    ...
                    if (index_info) {
                        ...
                        if (cb_state.rendering_attachments.depth_index && index_info->pDepthInputAttachmentIndex &&
                            *cb_state.rendering_attachments.depth_index != *index_info->pDepthInputAttachmentIndex) {
                            skip |= LogError(
                                ...
                                objlist,
                                cb_loc.pNext(Struct::VkRenderingInputAttachmentIndexInfo,
                                             Field::pDepthInputAttachmentIndex),
                                "(%" PRIu32
                                ") does not match the implicit or explicit state in the primary command buffer ("
                                "%" PRIu32 ").",
                                *index_info->pDepthInputAttachmentIndex,
                        ...
                        if (cb_state.rendering_attachments.stencil_index && index_info->pStencilInputAttachmentIndex &&
                            *cb_state.rendering_attachments.stencil_index !=
                                *index_info->pStencilInputAttachmentIndex) {
                            skip |=
                                LogError(vuid_090505,
                                         objlist,
                                         cb_loc.pNext(Struct::VkRenderingInputAttachmentIndexInfo,
                                                      Field::pStencilInputAttachmentIndex),
                                         "(%" PRIu32
                                         ") does not match the implicit or explicit state in the primary command buffer"
                                         "(%" PRIu32 ").",
                                         *index_info->pStencilInputAttachmentIndex,
            ...
            if (!cb_state.activeRenderPass) {
                ...
                if (!(sub_cb_state.beginInfo.flags & VK_COMMAND_BUFFER_USAGE_RENDER_PASS_CONTINUE_BIT)) {
                    ...
                    if (!cb_state.activeRenderPass->UsesDynamicRendering()) {
                        // Make sure render pass is compatible with parent command buffer pass if secondary command
                        // buffer has "render pass continue" usage flag
                        auto secondary_rp_state = Get<vvl::RenderPass>(inheritance_render_pass);
                        if (secondary_rp_state &&
                            (cb_state.activeRenderPass->VkHandle() != secondary_rp_state->VkHandle())) {
                            skip |= ValidateRenderPassCompatibility(cb_state.Handle(),
                                                                    *cb_state.activeRenderPass.get(),
                                                                    secondary_rp_state->Handle(),
                                                                    *secondary_rp_state.get(),
                                                                    cb_loc,
                                                                    "VUID-vkCmdExecuteCommands-pBeginInfo-06020");
                        }
                        //  If framebuffer for secondary CB is not NULL, then it must match active FB from primaryCB
                        skip |= ValidateInheritanceInfoFramebuffer(
                            commandBuffer, cb_state, pCommandBuffers[i], sub_cb_state, error_obj.location);
                    }
                ...
                if ((sub_cb_state.beginInfo.flags & VK_COMMAND_BUFFER_USAGE_SIMULTANEOUS_USE_BIT) &&
                    ...
                    skip |= LogError("VUID-vkCmdExecuteCommands-nestedCommandBufferSimultaneousUse-09378",
                                     pCommandBuffers[i],
                                     cb_loc,
                                     "(%s) was recorded with VkCommandBufferBeginInfo::flag including "
                                     "VK_COMMAND_BUFFER_USAGE_SIMULTANEOUS_USE_BIT, but the "
                                     "nestedCommandBufferSimultaneousUse feature "
                                     "was not enabled.",
                                     FormatHandle(pCommandBuffers[i]).c_str());
                }

                if (!cb_state.activeRenderPass->UsesDynamicRendering() &&
                    (cb_state.GetActiveSubpass() != inheritance_subpass)) {
                    const LogObjectList objlist(commandBuffer, pCommandBuffers[i], cb_state.activeRenderPass->Handle());
                    skip |= LogError("VUID-vkCmdExecuteCommands-pCommandBuffers-06019",
                                     objlist,
                                     cb_loc,
                                     "(%s) is executed within a %s "
                                     "instance scope begun by vkCmdBeginRenderPass(), but "
                                     "VkCommandBufferInheritanceInfo::subpass (%" PRIu32 ") does not "
                    ...
                    if (inheritance_render_pass != VK_NULL_HANDLE) {
                        const LogObjectList objlist(
                            commandBuffer, pCommandBuffers[i], cb_state.activeRenderPass->Handle());
                        skip |= LogError(
                    ...
                    if (cb_state.activeRenderPass->use_dynamic_rendering && sub_cb_state.activeRenderPass &&
                        ...
                        const auto inheritance_rendering_info =
                            sub_cb_state.activeRenderPass->inheritance_rendering_info;
                        if ((inheritance_rendering_info.flags & ~(VK_RENDERING_CONTENTS_SECONDARY_COMMAND_BUFFERS_BIT |
                                                                  VK_RENDERING_CONTENTS_INLINE_BIT_KHR)) !=
                            (rendering_info.flags & ~(VK_RENDERING_CONTENTS_SECONDARY_COMMAND_BUFFERS_BIT |
                                                      VK_RENDERING_CONTENTS_INLINE_BIT_KHR))) {
                            const LogObjectList objlist(
                                commandBuffer, pCommandBuffers[i], cb_state.activeRenderPass->Handle());
                            skip |= LogError("VUID-vkCmdExecuteCommands-flags-06026",
                                             objlist,
                                             cb_loc,
                                             "(%s) is executed within a dynamic renderpass instance scope begun "
                                             "by vkCmdBeginRendering(), but "
                                             "VkCommandBufferInheritanceRenderingInfo::flags (%s) does "
                        ...
                        if (inheritance_rendering_info.colorAttachmentCount != rendering_info.colorAttachmentCount) {
                            const LogObjectList objlist(
                                commandBuffer, pCommandBuffers[i], cb_state.activeRenderPass->Handle());
                            skip |= LogError("VUID-vkCmdExecuteCommands-colorAttachmentCount-06027",
                                             objlist,
                                             cb_loc,
                                             "(%s) is executed within a dynamic renderpass instance scope begun "
                                             "by vkCmdBeginRendering(), but "
                                             ...
                                             FormatHandle(pCommandBuffers[i]).c_str(),
                                             inheritance_rendering_info.colorAttachmentCount,
                                             rendering_info.colorAttachmentCount);
                        }

                        for (uint32_t color_i = 0,
                                      count = std::min(inheritance_rendering_info.colorAttachmentCount,
                                                       rendering_info.colorAttachmentCount);
                             color_i < count;
                             color_i++) {
                            if (rendering_info.pColorAttachments[color_i].imageView == VK_NULL_HANDLE) {
                                if (inheritance_rendering_info.pColorAttachmentFormats[color_i] !=
                                    VK_FORMAT_UNDEFINED) {
                                    const LogObjectList objlist(
                                        commandBuffer, pCommandBuffers[i], cb_state.activeRenderPass->Handle());
                                    skip |= LogError(
                                ...
                                auto image_view_state =
                                    Get<vvl::ImageView>(rendering_info.pColorAttachments[color_i].imageView);

                                if (image_view_state &&
                                    image_view_state->create_info.format !=
                        ...
                        if ((rendering_info.pDepthAttachment != nullptr) &&
                            ...
                            if (image_view_state && image_view_state->create_info.format !=
                                                        inheritance_rendering_info.depthAttachmentFormat) {
                                const LogObjectList objlist(
                                    commandBuffer, pCommandBuffers[i], cb_state.activeRenderPass->Handle());
                                skip |= LogError(
                        ...
                        if ((rendering_info.pStencilAttachment != nullptr) &&
                            ...
                            if (image_view_state && image_view_state->create_info.format !=
                                                        inheritance_rendering_info.stencilAttachmentFormat) {
                                const LogObjectList objlist(
                                    commandBuffer, pCommandBuffers[i], cb_state.activeRenderPass->Handle());
                                skip |= LogError(
                        ...
                        if (rendering_info.viewMask != inheritance_rendering_info.viewMask) {
                            const LogObjectList objlist(
                                commandBuffer, pCommandBuffers[i], cb_state.activeRenderPass->Handle());
                            skip |= LogError("VUID-vkCmdExecuteCommands-viewMask-06031",
                                             objlist,
                                             cb_loc,
                                             "(%s) is executed within a dynamic renderpass instance scope begun "
                                             "by vkCmdBeginRendering(), but "
                                             "VkCommandBufferInheritanceRenderingInfo::viewMask (%" PRIu32 ") does "
                                             "not match VkRenderingInfo::viewMask (%" PRIu32 ").",
                                             FormatHandle(pCommandBuffers[i]).c_str(),
                                             inheritance_rendering_info.viewMask,
                                             rendering_info.viewMask);
                        }

                        // VkAttachmentSampleCountInfoAMD == VkAttachmentSampleCountInfoNV
                        const auto amd_sample_count = vku::FindStructInPNextChain<VkAttachmentSampleCountInfoAMD>(
                        ...
                        if (amd_sample_count) {
                            for (uint32_t index = 0; index < rendering_info.colorAttachmentCount; index++) {
                                ...
                                auto image_view_state =
                                    Get<vvl::ImageView>(rendering_info.pColorAttachments[index].imageView);

                                if (image_view_state &&
                                    image_view_state->samples != amd_sample_count->pColorAttachmentSamples[index]) {
                                    const LogObjectList objlist(
                                        commandBuffer, pCommandBuffers[i], cb_state.activeRenderPass->Handle());
                                    skip |= LogError("VUID-vkCmdExecuteCommands-pNext-06032",
                                                     objlist,
                                                     cb_loc,
                                                     "(%s) is executed within a dynamic renderpass instance "
                                                     "scope begun "
                                                     "by vkCmdBeginRenderingKHR(), but "
                                                     "VkAttachmentSampleCountInfo(AMD/NV)::pColorAttachmentSamples at "
                                                     "index (%" PRIu32 ") "
                                                     "does "
                                                     "not match the sample count of the imageView in "
                            ...
                            for (uint32_t index = 0; index < rendering_info.colorAttachmentCount; index++) {
                                ...
                                if (image_view_state &&
                                    ...
                                    const LogObjectList objlist(
                                        commandBuffer, pCommandBuffers[i], cb_state.activeRenderPass->Handle());
                                    skip |= LogError("VUID-vkCmdExecuteCommands-pNext-06035",
                                                     objlist,
                                                     cb_loc,
                                                     "(%s) is executed within a dynamic renderpass instance "
                                                     "scope begun "
                                                     "by vkCmdBeginRenderingKHR(), but the sample count of the image "
                                                     "view at index (%" PRIu32 ") of "
        ...
        if (!(sub_cb_state.beginInfo.flags & VK_COMMAND_BUFFER_USAGE_SIMULTANEOUS_USE_BIT)) {
            if (sub_cb_state.InUse()) {
                const LogObjectList objlist(commandBuffer, pCommandBuffers[i]);
                skip |= LogError("VUID-vkCmdExecuteCommands-pCommandBuffers-00091",
                                 objlist,
                                 cb_loc,
                                 "Cannot execute pending %s without VK_COMMAND_BUFFER_USAGE_SIMULTANEOUS_USE_BIT set.",
                                 FormatHandle(pCommandBuffers[i]).c_str());
            }
            // We use an const_cast, because one cannot query a container keyed on a non-const pointer using a const
            // pointer
            if (cb_state.linkedCommandBuffers.count(const_cast<vvl::CommandBuffer*>(&sub_cb_state))) {
                const LogObjectList objlist(commandBuffer, pCommandBuffers[i]);
                skip |= LogError("VUID-vkCmdExecuteCommands-pCommandBuffers-00092",
                                 objlist,
                                 cb_loc,
            ...
            if (!insert_pair.second) {
                const LogObjectList objlist(commandBuffer, pCommandBuffers[i]);
                skip |= LogError("VUID-vkCmdExecuteCommands-pCommandBuffers-00093",
                                 objlist,
                                 cb_loc,
        ...
        if (!cb_state.activeQueries.empty() && !enabled_features.inheritedQueries) {
            const LogObjectList objlist(commandBuffer, pCommandBuffers[i]);
            skip |= LogError("VUID-vkCmdExecuteCommands-commandBuffer-00101",
                             objlist,
                             cb_loc,
        ...
        for (const auto& sub_layout_map_entry : sub_cb_state.image_layout_map) {
            ...
            if (!cb_image_layout_registry)
                continue;
            if (!sub_layout_map_entry.second)
                continue;
            ...
            for (sparse_container::parallel_iterator<const ImageLayoutRegistry::LayoutMap> iter(
                     sub_layout_map, cb_layout_map, 0);
                 !iter->range.empty();
                 ++iter) {
                VkImageLayout cb_layout = kInvalidLayout, sub_layout = kInvalidLayout;
                const char* layout_type;

                if (!iter->pos_A->valid || !iter->pos_B->valid)
                    continue;

                // pos_A denotes the sub CB map in the parallel iterator
                sub_layout = iter->pos_A->lower_bound->second.initial_layout;
                if (VK_IMAGE_LAYOUT_UNDEFINED == sub_layout)
                    continue; // secondary doesn't care about current or initial
                ...
                if (sub_layout != cb_layout) {
                    ...
                    for (auto index = iter->range.begin; index < iter->range.end; index++) {
                        ...
                        skip |= LogError("UNASSIGNED-vkCmdExecuteCommands-commandBuffer-00001",
                                         objlist,
                                         cb_loc,
                                         "was executed using %s (subresource: aspectMask %s, array layer %" PRIu32
                                         ", mip level %" PRIu32
                                         ") which expects layout %s--instead, image %s layout is %s.",
                                         FormatHandle(image).c_str(),
                                         string_VkImageAspectFlags(subresource.aspectMask).c_str(),
                                         subresource.arrayLayer,
                                         subresource.mipLevel,
                                         string_VkImageLayout(sub_layout),
                                         layout_type,
        ...
        if ((cb_state.unprotected == false) && (sub_cb_state.unprotected == true)) {
            const LogObjectList objlist(commandBuffer, pCommandBuffers[i]);
            skip |= LogError("VUID-vkCmdExecuteCommands-commandBuffer-01820",
                             objlist,
                             cb_loc,
            ...
            const LogObjectList objlist(commandBuffer, pCommandBuffers[i]);
            skip |= LogError("VUID-vkCmdExecuteCommands-commandBuffer-01821",
                             objlist,
                             cb_loc,
    ...
    if (cb_state.transform_feedback_active) {
        skip |= LogError(
            "VUID-vkCmdExecuteCommands-None-02286", commandBuffer, error_obj.location, "transform feedback is active.");
    }
...
bool CoreChecks::ValidateCmdDrawStrideWithStruct(const vvl::CommandBuffer& cb_state,
    ...
    if ((stride & condition_multiples) || (stride < struct_size)) {
        skip |= LogError(vuid,
                         cb_state.GetObjectList(VK_PIPELINE_BIND_POINT_GRAPHICS),
                         loc.dot(Field::stride),
                         "%" PRIu32 " is invalid or less than sizeof(%s) %" PRIu32 ".",
                         stride,
                         String(struct_name),
                         struct_size);
    }
...
bool CoreChecks::ValidateCmdDrawStrideWithBuffer(const vvl::CommandBuffer& cb_state,
                                                 const std::string& vuid,
                                                 const uint32_t stride,
                                                 Struct struct_name,
                                                 const uint32_t struct_size,
    ...
    if (validation_value > buffer_state.create_info.size) {
        ...
        skip |= LogError(vuid,
                         objlist,
                         loc,
                         "stride (%" PRIu32 ") * [drawCount (%" PRIu32 ") - 1] + offset (%" PRIu64
                         ") + sizeof(%s) (%" PRIu32 ") is %" PRIu64 ", which is greater than the buffer size (%" PRIu64
                         ").",
                         stride,
                         drawCount,
                         offset,
...
bool CoreChecks::PreCallValidateCmdBindTransformFeedbackBuffersEXT(VkCommandBuffer commandBuffer,
                                                                   uint32_t firstBinding,
                                                                   uint32_t bindingCount,
                                                                   const VkBuffer* pBuffers,
                                                                   const VkDeviceSize* pOffsets,
    ...
    auto cb_state = GetRead<vvl::CommandBuffer>(commandBuffer);
    if (cb_state->transform_feedback_active) {
        skip |= LogError("VUID-vkCmdBindTransformFeedbackBuffersEXT-None-02365",
                         commandBuffer,
                         error_obj.location,
    ...
    for (uint32_t i = 0; i < bindingCount; ++i) {
        ...
        if (pOffsets[i] >= buffer_state->create_info.size) {
            ...
            skip |=
                LogError("VUID-vkCmdBindTransformFeedbackBuffersEXT-pOffsets-02358",
                         objlist,
                         error_obj.location.dot(Field::pOffsets, i),
        ...
        if ((buffer_state->usage & VK_BUFFER_USAGE_TRANSFORM_FEEDBACK_BUFFER_BIT_EXT) == 0) {
            const LogObjectList objlist(commandBuffer, pBuffers[i]);
            skip |= LogError("VUID-vkCmdBindTransformFeedbackBuffersEXT-pBuffers-02360",
                             objlist,
                             buffer_loc,
                             "was created with %s.",
        ...
        if ((pSizes != nullptr) && (pSizes[i] != VK_WHOLE_SIZE)) {
            ...
            if (pSizes[i] > buffer_state->create_info.size) {
                const LogObjectList objlist(commandBuffer, pBuffers[i]);
                skip |= LogError("VUID-vkCmdBindTransformFeedbackBuffersEXT-pSizes-02362",
                                 objlist,
                                 error_obj.location.dot(Field::pSizes, i),
                                 "(%" PRIu64 ") is greater than the size of pBuffers[%" PRIu32 "](%" PRIu64 ").",
                                 pSizes[i],
                                 i,
                                 buffer_state->create_info.size);
            } else if (pOffsets[i] + pSizes[i] > buffer_state->create_info.size) {
                const LogObjectList objlist(commandBuffer, pBuffers[i]);
                skip |= LogError("VUID-vkCmdBindTransformFeedbackBuffersEXT-pOffsets-02363",
                                 objlist,
                                 error_obj.location,
        ...
        skip |= ValidateMemoryIsBoundToBuffer(
            commandBuffer, *buffer_state, buffer_loc, "VUID-vkCmdBindTransformFeedbackBuffersEXT-pBuffers-02364");
    }

    return skip;
}

bool CoreChecks::PreCallValidateCmdBeginTransformFeedbackEXT(VkCommandBuffer commandBuffer,
    ...
    skip |= ValidateCmd(*cb_state, error_obj.location);
    if (skip)
        return skip; // basic validation failed, might have null pointers

    const auto* pipe = cb_state->lastBound[VK_PIPELINE_BIND_POINT_GRAPHICS].pipeline_state;
    if (!pipe && !enabled_features.shaderObject) {
        skip |= LogError("VUID-vkCmdBeginTransformFeedbackEXT-None-06233",
                         commandBuffer,
                         error_obj.location,
        ...
        for (const auto& stage_state : pipe->stage_states) {
            ...
            if (!stage_state.entrypoint->execution_mode.Has(spirv::ExecutionModeSet::xfb_bit)) {
                ...
                skip |= LogError("VUID-vkCmdBeginTransformFeedbackEXT-None-04128",
                                 objlist,
                                 error_obj.location,
    ...
    if (cb_state->transform_feedback_active) {
        skip |= LogError("VUID-vkCmdBeginTransformFeedbackEXT-None-02367",
                         commandBuffer,
                         error_obj.location,
                         "transform feedback is active.");
    }
    ...
    for (uint32_t i = 0; i < rp_ci.subpassCount; ++i) {
        ...
        if (rp_ci.pSubpasses[i].viewMask > 0) {
            skip |= LogError("VUID-vkCmdBeginTransformFeedbackEXT-None-02373",
                             commandBuffer,
                             error_obj.location,
    ...
    if ((counterBufferCount + firstCounterBuffer) > cb_state->transform_feedback_buffers_bound) {
        skip |= LogError("VUID-vkCmdBeginTransformFeedbackEXT-firstCounterBuffer-09630",
                         commandBuffer,
                         ...
                         firstCounterBuffer,
                         counterBufferCount,
                         cb_state->transform_feedback_buffers_bound);
    }

    // pCounterBuffers and pCounterBufferOffsets are optional and may be nullptr. Additionaly, pCounterBufferOffsets
    // must be nullptr if pCounterBuffers is nullptr.
    if (pCounterBuffers == nullptr) {
        if (pCounterBufferOffsets != nullptr) {
            skip |= LogError("VUID-vkCmdBeginTransformFeedbackEXT-pCounterBuffer-02371",
                             commandBuffer,
                             error_obj.location,
        ...
        for (uint32_t i = 0; i < counterBufferCount; ++i) {
            ...
            if (pCounterBufferOffsets != nullptr && pCounterBufferOffsets[i] + 4 > buffer_state->create_info.size) {
                ...
                skip |= LogError("VUID-vkCmdBeginTransformFeedbackEXT-pCounterBufferOffsets-02370",
                                 objlist,
            ...
            if ((buffer_state->usage & VK_BUFFER_USAGE_TRANSFORM_FEEDBACK_COUNTER_BUFFER_BIT_EXT) == 0) {
                const LogObjectList objlist(commandBuffer, pCounterBuffers[i]);
                skip |= LogError("VUID-vkCmdBeginTransformFeedbackEXT-pCounterBuffers-02372",
                                 objlist,
                                 error_obj.location.dot(Field::pCounterBuffers, i),
                                 "was created with %s.",
                                 string_VkBufferUsageFlags2(buffer_state->usage).c_str());
...
bool CoreChecks::PreCallValidateCmdEndTransformFeedbackEXT(VkCommandBuffer commandBuffer,
                                                           uint32_t firstCounterBuffer,
                                                           uint32_t counterBufferCount,
                                                           const VkBuffer* pCounterBuffers,
                                                           const VkDeviceSize* pCounterBufferOffsets,
    ...
    auto cb_state = GetRead<vvl::CommandBuffer>(commandBuffer);
    if (!cb_state->transform_feedback_active) {
        skip |= LogError("VUID-vkCmdEndTransformFeedbackEXT-None-02375",
                         commandBuffer,
                         error_obj.location,
    ...
    if (pCounterBuffers) {
        for (uint32_t i = 0; i < counterBufferCount; ++i) {
            ...
            if (pCounterBufferOffsets != nullptr && pCounterBufferOffsets[i] + 4 > buffer_state->create_info.size) {
                const LogObjectList objlist(commandBuffer, pCounterBuffers[i]);
                skip |= LogError("VUID-vkCmdEndTransformFeedbackEXT-pCounterBufferOffsets-02378",
                                 objlist,
                                 error_obj.location.dot(Field::pCounterBuffers, i),
                                 "is not large enough to hold 4 bytes at pCounterBufferOffsets[%" PRIu32 "](0x%" PRIx64
                                 ").",
            ...
            if ((buffer_state->usage & VK_BUFFER_USAGE_TRANSFORM_FEEDBACK_COUNTER_BUFFER_BIT_EXT) == 0) {
                const LogObjectList objlist(commandBuffer, pCounterBuffers[i]);
                skip |= LogError("VUID-vkCmdEndTransformFeedbackEXT-pCounterBuffers-02380",
                                 objlist,
                                 error_obj.location.dot(Field::pCounterBuffers, i),
                                 "was created with %s.",
                                 string_VkBufferUsageFlags2(buffer_state->usage).c_str());
...
bool CoreChecks::PreCallValidateCmdBindVertexBuffers2(VkCommandBuffer commandBuffer,
                                                      uint32_t firstBinding,
                                                      uint32_t bindingCount,
                                                      const VkBuffer* pBuffers,
                                                      const VkDeviceSize* pOffsets,
    ...
    for (uint32_t i = 0; i < bindingCount; ++i) {
        ...
        skip |= ValidateBufferUsageFlags(objlist,
                                         *buffer_state,
                                         ...
                                         buffer_loc);
        skip |= ValidateMemoryIsBoundToBuffer(
            commandBuffer, *buffer_state, buffer_loc, "VUID-vkCmdBindVertexBuffers2-pBuffers-03360");

        const VkDeviceSize offset = pOffsets[i];
        if (pSizes) {
            if (offset >= buffer_state->create_info.size) {
                skip |= LogError("VUID-vkCmdBindVertexBuffers2-pOffsets-03357",
                                 objlist,
                                 error_obj.location.dot(Field::pOffsets, i),
                                 "(0x%" PRIu64 ") is beyond the end of the buffer of size (%" PRIu64 ").",
                                 offset,
                                 buffer_state->create_info.size);
            }
            ...
            if (size == VK_WHOLE_SIZE) {
                ...
                skip |= LogError("VUID-vkCmdBindVertexBuffers2-pSizes-03358",
                                 objlist,
                                 error_obj.location.dot(Field::pOffsets, i),
...
bool CoreChecks::PreCallValidateCmdBindVertexBuffers2EXT(VkCommandBuffer commandBuffer,
                                                         uint32_t firstBinding,
                                                         uint32_t bindingCount,
                                                         const VkBuffer* pBuffers,
                                                         const VkDeviceSize* pOffsets,
                                                         ...
                                                         const VkDeviceSize* pStrides,
                                                         const ErrorObject& error_obj) const {
    return PreCallValidateCmdBindVertexBuffers2(
        commandBuffer, firstBinding, bindingCount, pBuffers, pOffsets, pSizes, pStrides, error_obj);
}
...
bool CoreChecks::PreCallValidateCmdBeginConditionalRenderingEXT(
    ...
    if (cb_state && cb_state->conditional_rendering_active) {
        skip |= LogError("VUID-vkCmdBeginConditionalRenderingEXT-None-01980",
                         commandBuffer,
                         error_obj.location,
    ...
    if (pConditionalRenderingBegin) {
        if (auto buffer_state = Get<vvl::Buffer>(pConditionalRenderingBegin->buffer)) {
            ...
            skip |= ValidateMemoryIsBoundToBuffer(commandBuffer,
                                                  *buffer_state,
                                                  conditional_loc.dot(Field::buffer),
            ...
            if ((buffer_state->usage & VK_BUFFER_USAGE_CONDITIONAL_RENDERING_BIT_EXT) == 0) {
                const LogObjectList objlist(commandBuffer, buffer_state->Handle());
                skip |= LogError("VUID-VkConditionalRenderingBeginInfoEXT-buffer-01982",
                                 objlist,
                                 conditional_loc.dot(Field::buffer),
            ...
            if (pConditionalRenderingBegin->offset + 4 > buffer_state->create_info.size) {
                const LogObjectList objlist(commandBuffer, buffer_state->Handle());
                skip |= LogError("VUID-VkConditionalRenderingBeginInfoEXT-offset-01983",
                                 objlist,
                                 conditional_loc.dot(Field::offset),
                                 "(%" PRIu64
...
bool CoreChecks::PreCallValidateCmdEndConditionalRenderingEXT(VkCommandBuffer commandBuffer,
                                                              const ErrorObject& error_obj) const {
    bool skip = false;
    ...
    if (!cb_state->conditional_rendering_active) {
        skip |= LogError("VUID-vkCmdEndConditionalRenderingEXT-None-01985",
                         commandBuffer,
                         error_obj.location,
    ...
    if (!cb_state->conditional_rendering_inside_render_pass && cb_state->activeRenderPass != nullptr) {
        skip |= LogError("VUID-vkCmdEndConditionalRenderingEXT-None-01986",
                         commandBuffer,
                         error_obj.location,
    ...
    if (cb_state->conditional_rendering_inside_render_pass && cb_state->activeRenderPass != nullptr &&
        ...
        skip |=
            LogError("VUID-vkCmdEndConditionalRenderingEXT-None-01987",
                     commandBuffer,
                     error_obj.location,
                     "Conditional rendering was begun in subpass %" PRIu32 ", but the current subpass is %" PRIu32 ".",
                     cb_state->conditional_rendering_subpass,
                     cb_state->GetActiveSubpass());
    }
...
bool CoreChecks::PreCallValidateCmdBindShadingRateImageNV(VkCommandBuffer commandBuffer,
    ...
    if (!enabled_features.shadingRateImage) {
        skip |= LogError("VUID-vkCmdBindShadingRateImageNV-None-02058",
                         commandBuffer,
                         error_obj.location,
    ...
    if (!view_state) {
        ...
        skip |= LogError("VUID-vkCmdBindShadingRateImageNV-imageView-02059",
                         objlist,
                         error_obj.location,
    ...
    if (ivci.format != VK_FORMAT_R8_UINT) {
        const LogObjectList objlist(commandBuffer, imageView);
        skip |= LogError("VUID-vkCmdBindShadingRateImageNV-imageView-02060",
                         objlist,
                         error_obj.location,
    ...
    if (!(usage & VK_IMAGE_USAGE_SHADING_RATE_IMAGE_BIT_NV)) {
        const LogObjectList objlist(commandBuffer, imageView);
        skip |= LogError("VUID-vkCmdBindShadingRateImageNV-imageView-02061",
                         objlist,
                         error_obj.location,
    ...
    if (image_state) {
        if (imageLayout != VK_IMAGE_LAYOUT_SHADING_RATE_OPTIMAL_NV && imageLayout != VK_IMAGE_LAYOUT_GENERAL) {
            ...
            skip |= LogError("VUID-vkCmdBindShadingRateImageNV-imageLayout-02063",
                             objlist,
        ...
        skip |= VerifyImageLayoutSubresource(*cb_state,
                                             *image_state,
                                             subresource,
                                             imageLayout,
                                             error_obj.location.dot(Field::imageView),
...
bool CoreChecks::PreCallValidateCmdEndDebugUtilsLabelEXT(VkCommandBuffer commandBuffer,
                                                         const ErrorObject& error_obj) const {
    auto cb_state = GetRead<vvl::CommandBuffer>(commandBuffer);
    bool skip = false;
    ...
    if (cb_state->GetLabelStackDepth() < 1) {
        skip |= LogError("VUID-vkCmdEndDebugUtilsLabelEXT-commandBuffer-01913",
                         commandBuffer,
                         error_obj.location,
