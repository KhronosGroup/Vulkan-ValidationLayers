...
bool CoreChecks::ValidatePipelineCacheControlFlags(VkPipelineCreateFlags2KHR flags,
                                                   const Location& loc,
                                                   const char* vuid) const {
    bool skip = false;
    if (enabled_features.pipelineCreationCacheControl == VK_FALSE) {
        const VkPipelineCreateFlags invalid_flags = VK_PIPELINE_CREATE_2_FAIL_ON_PIPELINE_COMPILE_REQUIRED_BIT |
                                                    VK_PIPELINE_CREATE_2_EARLY_RETURN_ON_FAILURE_BIT;
        if ((flags & invalid_flags) != 0) {
            skip |= LogError(vuid,
                             device,
                             loc,
                             "is %s but pipelineCreationCacheControl feature was not enabled.",
...
bool CoreChecks::ValidatePipelineIndirectBindableFlags(VkPipelineCreateFlags2KHR flags,
                                                       const Location& loc,
                                                       const char* vuid) const {
    bool skip = false;
    if (enabled_features.deviceGeneratedComputePipelines == VK_FALSE) {
        if ((flags & VK_PIPELINE_CREATE_2_INDIRECT_BINDABLE_BIT_NV) != 0) {
            skip |= LogError(vuid,
                             device,
                             loc,
                             "is %s but deviceGeneratedComputePipelines feature was not enabled.",
                             string_VkPipelineCreateFlags2(flags).c_str());
        }
    }
    return skip;
...
bool CoreChecks::ValidatePipelineProtectedAccessFlags(VkPipelineCreateFlags2KHR flags, const Location& loc) const {
    ...
    if (enabled_features.pipelineProtectedAccess == VK_FALSE) {
        ...
        if ((flags & invalid_flags) != 0) {
            skip |= LogError("VUID-VkGraphicsPipelineCreateInfo-pipelineProtectedAccess-07368",
                             device,
                             loc,
    ...
    if ((flags & VK_PIPELINE_CREATE_2_NO_PROTECTED_ACCESS_BIT) &&
        (flags & VK_PIPELINE_CREATE_2_PROTECTED_ACCESS_ONLY_BIT)) {
        skip |= LogError("VUID-VkGraphicsPipelineCreateInfo-flags-07369",
                         device,
                         loc,
...
bool CoreChecks::ValidatePipelineRobustnessCreateInfo(const vvl::Pipeline& pipeline,
    ...
    if (!enabled_features.pipelineRobustness) {
        if (pipeline_robustness_info.storageBuffers != VK_PIPELINE_ROBUSTNESS_BUFFER_BEHAVIOR_DEVICE_DEFAULT) {
            skip |= LogError("VUID-VkPipelineRobustnessCreateInfo-pipelineRobustness-06926",
                             device,
        ...
        if (pipeline_robustness_info.uniformBuffers != VK_PIPELINE_ROBUSTNESS_BUFFER_BEHAVIOR_DEVICE_DEFAULT) {
            skip |= LogError("VUID-VkPipelineRobustnessCreateInfo-pipelineRobustness-06927",
                             device,
        ...
        if (pipeline_robustness_info.vertexInputs != VK_PIPELINE_ROBUSTNESS_BUFFER_BEHAVIOR_DEVICE_DEFAULT) {
            skip |= LogError("VUID-VkPipelineRobustnessCreateInfo-pipelineRobustness-06928",
                             device,
        ...
        if (pipeline_robustness_info.images != VK_PIPELINE_ROBUSTNESS_IMAGE_BEHAVIOR_DEVICE_DEFAULT) {
            skip |= LogError("VUID-VkPipelineRobustnessCreateInfo-pipelineRobustness-06929",
                             device,
    ...
    if (!has_robust_image_access &&
        ...
        skip |= LogError("VUID-VkPipelineRobustnessCreateInfo-robustImageAccess-06930",
                         device,
                         loc.pNext(Struct::VkPipelineRobustnessCreateInfo, Field::images),
...
bool CoreChecks::PreCallValidateGetPipelineExecutablePropertiesKHR(VkDevice device,
                                                                   const VkPipelineInfoKHR* pPipelineInfo,
                                                                   uint32_t* pExecutableCount,
                                                                   VkPipelineExecutablePropertiesKHR* pProperties,
                                                                   const ErrorObject& error_obj) const {
    bool skip = false;
    skip |= ValidatePipelineExecutableInfo(
        device, nullptr, error_obj.location, "VUID-vkGetPipelineExecutablePropertiesKHR-pipelineExecutableInfo-03270");
    return skip;
}

bool CoreChecks::ValidatePipelineExecutableInfo(VkDevice device,
    ...
    if (!enabled_features.pipelineExecutableInfo) {
        ...
        if (pExecutableInfo->executableIndex >= executable_count) {
            skip |= LogError("VUID-VkPipelineExecutableInfoKHR-executableIndex-03275",
                             pExecutableInfo->pipeline,
                             loc.dot(Field::pExecutableInfo).dot(Field::executableIndex),
                             "(%" PRIu32 ") must be less than the number of executables associated with "
...
bool CoreChecks::PreCallValidateGetPipelineExecutableStatisticsKHR(VkDevice device,
                                                                   ...
                                                                   const ErrorObject& error_obj) const {
    bool skip = false;
    skip |= ValidatePipelineExecutableInfo(device,
                                           pExecutableInfo,
                                           error_obj.location,
    ...
    if (!(pipeline_state->create_flags & VK_PIPELINE_CREATE_CAPTURE_STATISTICS_BIT_KHR)) {
        skip |= LogError("VUID-vkGetPipelineExecutableStatisticsKHR-pipeline-03274",
                         pExecutableInfo->pipeline,
                         error_obj.location,
...
bool CoreChecks::PreCallValidateGetPipelineExecutableInternalRepresentationsKHR(
    ...
    if (!(pipeline_state->create_flags & VK_PIPELINE_CREATE_CAPTURE_INTERNAL_REPRESENTATIONS_BIT_KHR)) {
        skip |= LogError("VUID-vkGetPipelineExecutableInternalRepresentationsKHR-pipeline-03278",
                         pExecutableInfo->pipeline,
                         error_obj.location,
                         "called on a pipeline created without the "
...
bool CoreChecks::PreCallValidateDestroyPipeline(VkDevice device,
                                                VkPipeline pipeline,
    ...
    if (auto pipeline_state = Get<vvl::Pipeline>(pipeline)) {
        skip |=
            ValidateObjectNotInUse(pipeline_state.get(), error_obj.location, "VUID-vkDestroyPipeline-pipeline-00765");
    }
    return skip;
}

bool CoreChecks::PreCallValidateCmdBindPipeline(VkCommandBuffer commandBuffer,
    ...
    if (pipelineBindPoint != pipeline_state.pipeline_type) {
        if (pipelineBindPoint == VK_PIPELINE_BIND_POINT_GRAPHICS) {
            ...
            skip |= LogError("VUID-vkCmdBindPipeline-pipelineBindPoint-00779",
                             objlist,
                             error_obj.location,
            ...
            const LogObjectList objlist(cb_state->Handle(), pipeline);
            skip |= LogError("VUID-vkCmdBindPipeline-pipelineBindPoint-00780",
                             objlist,
                             error_obj.location,
            ...
            const LogObjectList objlist(cb_state->Handle(), pipeline);
            skip |= LogError("VUID-vkCmdBindPipeline-pipelineBindPoint-02392",
                             objlist,
                             error_obj.location,
        ...
        if (pipelineBindPoint == VK_PIPELINE_BIND_POINT_GRAPHICS) {
            ...
            if (cb_state->activeRenderPass &&
                phys_dev_ext_props.sample_locations_props.variableSampleLocations == VK_FALSE) {
                ...
                if (sample_locations && sample_locations->sampleLocationsEnable == VK_TRUE &&
                    ...
                    const auto* sample_locations_begin_info =
                        vku::FindStructInPNextChain<VkRenderPassSampleLocationsBeginInfoEXT>(
            ...
            if (enabled_features.variableMultisampleRate == VK_FALSE) {
                if (const auto* multisample_state = pipeline_state.MultisampleState(); multisample_state) {
                    if (const auto& render_pass = cb_state->activeRenderPass; render_pass) {
                        ...
                        // if render pass uses no attachment, verify that all bound pipelines referencing this subpass
                        // have the same pMultisampleState->rasterizationSamples.
                        if (render_pass->UsesNoAttachment(subpass)) {
                            // If execution ends up here, GetActiveSubpassRasterizationSampleCount() can still be empty
                            // if this is the first bound pipeline with the previous conditions holding. Rasterization
                            // samples count for the subpass will be updated in PostCallRecordCmdBindPipeline, if it is
                            // empty.
                            if (std::optional<VkSampleCountFlagBits> subpass_rasterization_samples =
                                ...
                                *subpass_rasterization_samples != multisample_state->rasterizationSamples) {
                                ...
                                skip |= LogError("VUID-vkCmdBindPipeline-pipeline-00781",
                                                 objlist,
                                                 error_obj.location,
                                                 "variableMultisampleRate is VK_FALSE "
                                                 "and "
                                                 "pipeline has pMultisampleState->rasterizationSamples equal to %s, "
                                                 "while a previously bound "
                                                 "pipeline in the current subpass (%" PRIu32 ") used "
            ...
            if (cb_state->GetCurrentPipeline(pipelineBindPoint) &&
                ...
                skip |= LogError("UNASSIGNED-vkCmdBindPipeline-Pipeline-Rebind",
                                 objlist,
                                 error_obj.location,
                                 "The pipeline being bound (%s) is the same as the currently bound pipeline and "
                                 "between the calls, a "
                                 "dynamic state was set which is static in this pipeline. This might not work "
                                 "correctly on drivers with "
        ...
        if (pipeline_state.create_flags & VK_PIPELINE_CREATE_LIBRARY_BIT_KHR) {
            const LogObjectList objlist(cb_state->Handle(), pipeline);
            skip |=
                LogError("VUID-vkCmdBindPipeline-pipeline-03382",
                         objlist,
                         error_obj.location,
        ...
        if (cb_state->transform_feedback_active) {
            const LogObjectList objlist(cb_state->Handle(), pipeline);
            skip |= LogError(
                "VUID-vkCmdBindPipeline-None-02323", objlist, error_obj.location, "transform feedback is active.");
        }
        if (enabled_features.pipelineProtectedAccess) {
            if (cb_state->unprotected) {
                const LogObjectList objlist(cb_state->Handle(), pipeline);
                if (pipeline_state.create_flags & VK_PIPELINE_CREATE_PROTECTED_ACCESS_ONLY_BIT) {
                    skip |= LogError("VUID-vkCmdBindPipeline-pipelineProtectedAccess-07409",
                                     objlist,
                                     error_obj.location,
                ...
                if (pipeline_state.create_flags & VK_PIPELINE_CREATE_NO_PROTECTED_ACCESS_BIT) {
                    skip |= LogError("VUID-vkCmdBindPipeline-pipelineProtectedAccess-07408",
                                     objlist,
                                     error_obj.location,
                                     "Binding pipeline created with "
...
bool CoreChecks::ValidatePipelineBindPoint(const vvl::CommandBuffer& cb_state,
    ...
    if (0 == (qfp.queueFlags & required_mask)) {
        ...
        switch (loc.function) {
            ...
            case Func::vkCmdBindDescriptorBufferEmbeddedSamplers2EXT:
                vuid = "VUID-vkCmdBindDescriptorBufferEmbeddedSamplers2EXT-pBindDescriptorBufferEmbeddedSamplersInfo-"
                       "09473";
                break;
        ...
        skip |= LogError(vuid,
                         objlist,
                         loc,
                         "%s was allocated from %s that does not support bindpoint %s.",
                         FormatHandle(cb_state.Handle()).c_str(),
                         FormatHandle(cb_state.allocate_info.commandPool).c_str(),
                         string_VkPipelineBindPoint(bind_point));
    }
    return skip;
}

bool CoreChecks::ValidateShaderSubgroupSizeControl(VkShaderStageFlagBits stage,
    ...
    if (stage_state.HasPipeline()) {
        ...
        if ((flags & VK_PIPELINE_SHADER_STAGE_CREATE_ALLOW_VARYING_SUBGROUP_SIZE_BIT) != 0 &&
            ...
            skip |= LogError("VUID-VkPipelineShaderStageCreateInfo-flags-02784",
                             device,
                             loc.dot(Field::flags),
                             "includes "
        ...
        if ((flags & VK_PIPELINE_SHADER_STAGE_CREATE_REQUIRE_FULL_SUBGROUPS_BIT) != 0) {
            if (!enabled_features.computeFullSubgroups) {
                skip |= LogError(
                    ...
                    "was not enabled");
            } else if ((stage & (VK_SHADER_STAGE_MESH_BIT_EXT | VK_SHADER_STAGE_TASK_BIT_EXT |
                                 VK_SHADER_STAGE_COMPUTE_BIT)) == 0) {
                skip |= LogError("VUID-VkPipelineShaderStageCreateInfo-flags-08988",
                                 device,
                                 loc.dot(Field::flags),
                                 "includes "
        ...
        if ((flags & VK_SHADER_CREATE_REQUIRE_FULL_SUBGROUPS_BIT_EXT) != 0) {
            if ((stage & (VK_SHADER_STAGE_MESH_BIT_EXT | VK_SHADER_STAGE_TASK_BIT_EXT | VK_SHADER_STAGE_COMPUTE_BIT)) ==
                0) {
                skip |= LogError(
...
bool CoreChecks::ValidateSpecializations(const vku::safe_VkSpecializationInfo* spec, const Location& loc) const {
    ...
    for (auto i = 0u; i < spec->mapEntryCount; i++) {
        ...
        if (map_entry.offset >= spec->dataSize) {
            skip |= LogError("VUID-VkSpecializationInfo-offset-00773",
                             device,
                             map_loc.dot(Field::offset),
                             "is %" PRIu32 " but dataSize is %zu (for constantID %" PRIu32 ").",
                             map_entry.offset,
                             spec->dataSize,
        ...
        if (map_entry.offset + map_entry.size > spec->dataSize) {
            skip |=
                LogError("VUID-VkSpecializationInfo-pMapEntries-00774",
                         device,
                         map_loc.dot(Field::size),
        ...
        for (uint32_t j = i + 1; j < spec->mapEntryCount; ++j) {
            if (map_entry.constantID == spec->pMapEntries[j].constantID) {
                skip |= LogError("VUID-VkSpecializationInfo-constantID-04911",
                                 device,
                                 map_loc,
                                 "and pMapEntries[%" PRIu32 "] both have constantID (%" PRIu32 ").",
                                 j,
                                 map_entry.constantID);
            }
        }
...
bool CoreChecks::ValidateShaderStageMaxResources(VkShaderStageFlagBits stage,
    ...
    // TODO: This reuses a lot of GetDescriptorCountMaxPerStage but currently would need to make it agnostic in a way to
    // handle input from CreatePipeline and CreatePipelineLayout level
        ...
        for (const auto& set_layout : layout_state->set_layouts) {
            ...
            for (uint32_t binding_idx = 0; binding_idx < set_layout->GetBindingCount(); binding_idx++) {
                const VkDescriptorSetLayoutBinding* binding =
                    set_layout->GetDescriptorSetLayoutBindingPtrFromIndex(binding_idx);
    ...
    if (const auto module_identifier =
        ...
        if (module_identifier->identifierSize > 0) {
            ...
            if (stage_ci.module != VK_NULL_HANDLE) {
                skip |= LogError("VUID-VkPipelineShaderStageCreateInfo-stage-06848",
                                 device,
                                 loc.dot(Field::pNext),
        ...
        if (module_create_info) {
            skip |= LogError("VUID-VkPipelineShaderStageCreateInfo-stage-06844",
                             device,
                             loc.dot(Field::pNext),
                             "has both a "
                             "VkPipelineShaderStageModuleIdentifierCreateInfoEXT "
        ...
        if (!enabled_features.maintenance5 && !enabled_features.graphicsPipelineLibrary) {
            skip |= LogError("VUID-VkPipelineShaderStageCreateInfo-stage-08771",
                             device,
                             loc.dot(Field::module),
            ...
            skip |=
                LogError("VUID-VkPipelineShaderStageCreateInfo-stage-06845",
                         device,
                         loc.dot(Field::module),
...
bool CoreChecks::PreCallValidateGetPipelineKeyKHR(VkDevice device,
    ...
    const VkBaseOutStructure* pipeline_create_info =
        reinterpret_cast<const VkBaseOutStructure*>(pPipelineCreateInfo->pNext);
    if (pipeline_create_info) {
        if (pipeline_create_info->sType != VK_STRUCTURE_TYPE_EXECUTION_GRAPH_PIPELINE_CREATE_INFO_AMDX &&
            ...
            skip |= LogError("VUID-VkPipelineCreateInfoKHR-pNext-09604",
                             device,
    ...
    if (binary_info && (binary_info->binaryCount > 0)) {
        skip |= LogError("VUID-vkGetPipelineKeyKHR-pNext-09605",
                         device,
                         error_obj.location.dot(Field::pPipelineCreateInfo)
                             .pNext(Struct::VkPipelineBinaryInfoKHR, Field::binaryCount),
                         "(%" PRIu32 ") is greater than zero",
...
bool CoreChecks::PreCallValidateReleaseCapturedPipelineDataKHR(VkDevice device,
    ...
    if (!(pipeline_state->create_flags & VK_PIPELINE_CREATE_2_CAPTURE_DATA_BIT_KHR)) {
        skip |= LogError("VUID-VkReleaseCapturedPipelineDataInfoKHR-pipeline-09613",
                         pInfo->pipeline,
    ...
    if (pipeline_state->binary_data_released) {
        skip |= LogError("VUID-VkReleaseCapturedPipelineDataInfoKHR-pipeline-09618",
                         pInfo->pipeline,
                         error_obj.location.dot(Field::pInfo).dot(Field::pipeline),
...
void CoreChecks::PostCallRecordReleaseCapturedPipelineDataKHR(VkDevice device,
                                                              const VkReleaseCapturedPipelineDataInfoKHR* pInfo,
                                                              const VkAllocationCallbacks* pAllocator,
