...
bool CoreChecks::ValidateImageSubresourceRange(const uint32_t image_mip_count, const uint32_t image_layer_count,
    ...
    if (subresourceRange.baseArrayLayer >= image_layer_count) {
        const auto vuid = image_layer_count_var == Field::depth
                                ? GetSubresourceRangeVUID(subresource_loc, vvl::SubresourceRangeError::BaseLayer_02724)
    ...
    if (image_layer_count_var == Field::depth && subresourceRange.levelCount != 1u) {
        const auto vuid = GetSubresourceRangeVUID(subresource_loc, vvl::SubresourceRangeError::levelCount_Todo);
        skip |= LogError(vuid, objlist, subresource_loc.dot(Field::levelCount), "is (%" PRIu32 ").", subresourceRange.levelCount);
    }
    ...
    if (subresourceRange.layerCount != VK_REMAINING_ARRAY_LAYERS) {
        if (subresourceRange.layerCount == 0) {
            ...
            if (necessary_layer_count > image_layer_count) {
                const auto vuid = image_layer_count_var == Field::depth
                                      ? GetSubresourceRangeVUID(subresource_loc, vvl::SubresourceRangeError::LayerCount_02725)
