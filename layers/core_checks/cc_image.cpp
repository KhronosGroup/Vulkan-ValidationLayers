...
bool CoreChecks::ValidateImageFormatFeatures(const VkImageCreateInfo& create_info, const Location& loc) const {
    ...
    if (vkuFormatIsMultiplane(image_format) && ((create_info.flags & VK_IMAGE_CREATE_DISJOINT_BIT) != 0) &&
        ...
        skip |= LogError("VUID-VkImageCreateInfo-imageCreateFormatFeatures-02260",
                         device,
                         loc.dot(Field::usage),
    ...
    if (((tiling_features & VK_FORMAT_FEATURE_2_HOST_IMAGE_TRANSFER_BIT) == 0) &&
        ...
        skip |= LogError("VUID-VkImageCreateInfo-imageCreateFormatFeatures-09048",
                         device,
                         loc.dot(Field::usage),
...
bool CoreChecks::ValidateImageAlignmentControlCreateInfo(const VkImageCreateInfo& create_info,
    ...
    if (!enabled_features.imageAlignmentControl) {
        skip |= LogError("VUID-VkImageAlignmentControlCreateInfoMESA-imageAlignmentControl-09657",
                         device,
                         create_info_loc.dot(Field::pNext),
                         "contains a VkImageAlignmentControlCreateInfoMESA struct but the imageAlignmentControl "
    ...
    if (create_info.tiling != VK_IMAGE_TILING_OPTIMAL) {
        skip |= LogError("VUID-VkImageCreateInfo-pNext-09653",
                         device,
                         create_info_loc.dot(Field::tiling),
                         "is %s but needs to be VK_IMAGE_TILING_OPTIMAL",
...
bool CoreChecks::PreCallValidateCreateImage(VkDevice device,
    ...
    if (IsExtEnabled(device_extensions.vk_android_external_memory_android_hardware_buffer)) {
        ...
        if (pCreateInfo->format == VK_FORMAT_UNDEFINED) {
            return LogError("VUID-VkImageCreateInfo-pNext-01975",
                            device,
    ...
    const VkImageUsageFlags attach_flags =
    ...
    if (pCreateInfo->usage & attach_flags) {
        if (pCreateInfo->extent.width > device_limits->maxFramebufferWidth) {
            skip |= LogError("VUID-VkImageCreateInfo-usage-00964",
                             device,
        ...
        if (pCreateInfo->extent.height > device_limits->maxFramebufferHeight) {
            skip |= LogError("VUID-VkImageCreateInfo-usage-00965",
                             device,
                             create_info_loc.dot(Field::usage),
    ...
    if (!enabled_features.fragmentDensityMapOffset &&
        (pCreateInfo->usage & VK_IMAGE_USAGE_FRAGMENT_DENSITY_MAP_BIT_EXT)) {
        uint32_t ceiling_width = static_cast<uint32_t>(
            ceilf(static_cast<float>(device_limits->maxFramebufferWidth) /
        ...
        if (pCreateInfo->extent.width > ceiling_width) {
            skip |= LogError("VUID-VkImageCreateInfo-fragmentDensityMapOffset-06514",
                             device,
                             create_info_loc.dot(Field::usage),
                             "includes VK_IMAGE_USAGE_FRAGMENT_DENSITY_MAP_BIT_EXT and image width (%" PRIu32
                             ") is greater than %" PRIu32 ".\n"
                             "This is ceiling value of maxFramebufferWidth (%" PRIu32
                             ") / minFragmentDensityTexelSize.width (%" PRIu32 ").",
                             pCreateInfo->extent.width,
                             ceiling_width,
                             device_limits->maxFramebufferWidth,
                             phys_dev_ext_props.fragment_density_map_props.minFragmentDensityTexelSize.width);
        ...
        uint32_t ceiling_height = static_cast<uint32_t>(
            ceilf(static_cast<float>(device_limits->maxFramebufferHeight) /
                  std::max(static_cast<float>(
        ...
        if (pCreateInfo->extent.height > ceiling_height) {
            skip |= LogError("VUID-VkImageCreateInfo-fragmentDensityMapOffset-06515",
                             device,
                             create_info_loc.dot(Field::usage),
                             "includes VK_IMAGE_USAGE_FRAGMENT_DENSITY_MAP_BIT_EXT and image height (%" PRIu32
                             ") is greater than %" PRIu32 ".\n"
                             "This is ceiling value of maxFramebufferHeight (%" PRIu32
                             ") / minFragmentDensityTexelSize.height (%" PRIu32 ").",
                             pCreateInfo->extent.height,
                             ceiling_height,
                             device_limits->maxFramebufferHeight,
                             phys_dev_ext_props.fragment_density_map_props.minFragmentDensityTexelSize.height);
    ...
    if (pCreateInfo->tiling != VK_IMAGE_TILING_DRM_FORMAT_MODIFIER_EXT) {
        if (IsExtEnabled(instance_extensions.vk_khr_get_physical_device_properties2)) {
            result = DispatchGetPhysicalDeviceImageFormatProperties2Helper(
                physical_device, &image_format_info, &image_format_properties);
        } else {
            result = DispatchGetPhysicalDeviceImageFormatProperties(physical_device,
                                                                    pCreateInfo->format,
        ...
        if (result != VK_SUCCESS) {
            ...
            if (!vku::FindStructInPNextChain<VkExternalFormatANDROID>(pCreateInfo->pNext)) {
                ...
                skip |= LogError("VUID-VkImageCreateInfo-imageCreateMaxMipLevels-02251",
                                 device,
                                 create_info_loc,
        ...
        if (result != VK_SUCCESS) {
            ...
            std::string drm_source =
                modifier_list ? "pDrmFormatModifiers[]" : "VkImageDrmFormatModifierExplicitCreateInfoEXT";
            skip |= LogError("VUID-VkImageCreateInfo-imageCreateMaxMipLevels-02251",
                             device,
                             create_info_loc,
                             "The following parameters -\n"
    ...
    if (result == VK_SUCCESS) {
        ...
        if (pCreateInfo->mipLevels > format_limits.maxMipLevels) {
            skip |= LogError("VUID-VkImageCreateInfo-mipLevels-02255",
                             device,
                             create_info_loc.dot(Field::mipLevels),
                             "(%d) exceed image format maxMipLevels (%d) for format %s.",
                             pCreateInfo->mipLevels,
                             format_limits.maxMipLevels,
                             string_VkFormat(pCreateInfo->format));
        }
        ...
        if (!vkuFormatIsDepthAndStencil(pCreateInfo->format)) {
            uint64_t total_size = static_cast<uint64_t>(
                std::ceil(vkuFormatTexelSize(pCreateInfo->format) * static_cast<double>(texel_count)));
        ...
        if (pCreateInfo->arrayLayers > format_limits.maxArrayLayers) {
            skip |=
                LogError("VUID-VkImageCreateInfo-arrayLayers-02256",
                         device,
        ...
        if ((pCreateInfo->samples & format_limits.sampleCounts) == 0) {
            skip |= LogError("VUID-VkImageCreateInfo-samples-02258",
                             device,
                             create_info_loc.dot(Field::samples),
        ...
        if (pCreateInfo->extent.height > format_limits.maxExtent.height) {
            skip |= LogError("VUID-VkImageCreateInfo-extent-02253",
                             ...
                             pCreateInfo->extent.height,
                             format_limits.maxExtent.height,
                             string_VkFormat(pCreateInfo->format));
        }

        if (pCreateInfo->extent.depth > format_limits.maxExtent.depth) {
            skip |= LogError("VUID-VkImageCreateInfo-extent-02254",
                             device,
    ...
    if (FormatRequiresYcbcrConversionExplicitly(pCreateInfo->format)) {
        if (pCreateInfo->mipLevels != 1) {
            skip |= LogError("VUID-VkImageCreateInfo-format-06410",
                             device,
                             create_info_loc.dot(Field::mipLevels),
                             "(%d), but when using a YCbCr Conversion format (%s), mipLevels must be 1.",
                             pCreateInfo->mipLevels,
                             string_VkFormat(pCreateInfo->format));
        }

        if (pCreateInfo->samples != VK_SAMPLE_COUNT_1_BIT) {
            skip |= LogError("VUID-VkImageCreateInfo-format-06411",
                             device,
                             create_info_loc.dot(Field::samples),
        ...
        if (pCreateInfo->imageType != VK_IMAGE_TYPE_2D) {
            skip |= LogError("VUID-VkImageCreateInfo-format-06412",
                             device,
    ...
    if (IsExtEnabled(device_extensions.vk_khr_maintenance2)) {
        if (pCreateInfo->flags & VK_IMAGE_CREATE_BLOCK_TEXEL_VIEW_COMPATIBLE_BIT) {
            if (!vkuFormatIsCompressed(pCreateInfo->format)) {
                skip |= LogError("VUID-VkImageCreateInfo-flags-01572",
                                 device,
                                 create_info_loc.dot(Field::flags),
                                 "contains VK_IMAGE_CREATE_BLOCK_TEXEL_VIEW_COMPATIBLE_BIT, but format (%s) must be a "
                                 "compressed image format.",
                                 string_VkFormat(pCreateInfo->format));
            }
            if (!(pCreateInfo->flags & VK_IMAGE_CREATE_MUTABLE_FORMAT_BIT)) {
                skip |= LogError("VUID-VkImageCreateInfo-flags-01573",
                                 device,
                                 create_info_loc.dot(Field::flags),
    ...
    if (!vkuFormatIsMultiplane(pCreateInfo->format) && !(pCreateInfo->flags & VK_IMAGE_CREATE_ALIAS_BIT) &&
        ...
        skip |= LogError("VUID-VkImageCreateInfo-format-01577",
                         device,
                         create_info_loc,
    ...
    if (swapchain_create_info != nullptr && swapchain_create_info->swapchain != VK_NULL_HANDLE) {
        if (auto swapchain_state = Get<vvl::Swapchain>(swapchain_create_info->swapchain)) {
            ...
            const VkImageCreateFlags mutable_flags =
            ...
            if (pCreateInfo->format != swapchain_state->create_info.imageFormat) {
                skip |= LogError(vuid,
                                 ...
                                 string_VkFormat(swapchain_state->create_info.imageFormat),
                                 string_VkFormat(pCreateInfo->format));
            }
            if (pCreateInfo->extent.width != swapchain_state->create_info.imageExtent.width ||
                pCreateInfo->extent.height != swapchain_state->create_info.imageExtent.height ||
            ...
            if (pCreateInfo->arrayLayers != swapchain_state->create_info.imageArrayLayers) {
                skip |= LogError(vuid,
                                 ...
                                 swapchain_state->create_info.imageArrayLayers,
                                 pCreateInfo->arrayLayers);
            }
            ...
            if (pCreateInfo->queueFamilyIndexCount != swapchain_state->create_info.queueFamilyIndexCount) {
                skip |= LogError(vuid,
                                 ...
                                 swapchain_state->create_info.queueFamilyIndexCount,
                                 pCreateInfo->queueFamilyIndexCount);
            } else {
                for (uint32_t i = 0; i < pCreateInfo->queueFamilyIndexCount; ++i) {
                    bool found = false;
                    for (uint32_t j = 0; j < pCreateInfo->queueFamilyIndexCount; ++j) {
                        if (pCreateInfo->pQueueFamilyIndices[j] ==
                            swapchain_state->create_info.pQueueFamilyIndices[i]) {
                            found = true;
                            break;
                        }
                    }
    ...
    if ((pCreateInfo->flags & VK_IMAGE_CREATE_PROTECTED_BIT) != 0) {
        ...
        const VkImageCreateFlags invalid_flags = VK_IMAGE_CREATE_SPARSE_BINDING_BIT |
                                                 VK_IMAGE_CREATE_SPARSE_RESIDENCY_BIT |
                                                 VK_IMAGE_CREATE_SPARSE_ALIASED_BIT;
        if ((pCreateInfo->flags & invalid_flags) != 0) {
            skip |= LogError("VUID-VkImageCreateInfo-None-01891",
                             device,
                             create_info_loc.dot(Field::flags),
    ...
    if ((pCreateInfo->flags & VK_IMAGE_CREATE_MULTISAMPLED_RENDER_TO_SINGLE_SAMPLED_BIT_EXT) != 0) {
        if (!(enabled_features.multisampledRenderToSingleSampled)) {
            skip |= LogError("VUID-VkImageCreateInfo-multisampledRenderToSingleSampled-06882",
                             device,
                             create_info_loc.dot(Field::flags),
    ...
    if (IsExtEnabled(device_extensions.vk_khr_portability_subset)) {
        if (VK_IMAGE_CREATE_2D_ARRAY_COMPATIBLE_BIT & pCreateInfo->flags &&
            VK_FALSE == enabled_features.imageView2DOn3DImage) {
            skip |= LogError("VUID-VkImageCreateInfo-imageView2DOn3DImage-04459",
                             device,
                             create_info_loc,
        ...
        if ((VK_SAMPLE_COUNT_1_BIT != pCreateInfo->samples) && (1 != pCreateInfo->arrayLayers) &&
            ...
            skip |= LogError("VUID-VkImageCreateInfo-multisampleArrayImage-04460",
                             device,
                             create_info_loc,
    ...
    const auto external_memory_create_info_nv =
        vku::FindStructInPNextChain<VkExternalMemoryImageCreateInfoNV>(pCreateInfo->pNext);
    const auto external_memory_create_info =
        vku::FindStructInPNextChain<VkExternalMemoryImageCreateInfo>(pCreateInfo->pNext);
    if (external_memory_create_info_nv != nullptr && external_memory_create_info != nullptr) {
        skip |= LogError("VUID-VkImageCreateInfo-pNext-00988",
                         device,
                         create_info_loc,
    ...
    if (external_memory_create_info && external_memory_create_info->handleTypes != 0 &&
        ...
        if (pCreateInfo->initialLayout != VK_IMAGE_LAYOUT_UNDEFINED) {
            skip |= LogError("VUID-VkImageCreateInfo-pNext-01443",
                             device,
                             create_info_loc.pNext(Struct::VkExternalMemoryImageCreateInfo, Field::handleTypes),
                             "is %" PRIu32 " but the initialLayout is %s.",
        ...
        if (pCreateInfo->tiling != VK_IMAGE_TILING_DRM_FORMAT_MODIFIER_EXT) {
            result = DispatchGetPhysicalDeviceImageFormatProperties2Helper(
                physical_device, &image_format_info, &image_properties);
        ...
        if (result != VK_SUCCESS) {
            skip |= LogError("VUID-VkImageCreateInfo-pNext-00990",
                             device,
                             create_info_loc,
                             "The handle type (%s), format (%s), type (%s), tiling (%s), usage (%s), flags (%s) "
                             "is not supported combination of parameters and vkGetPhysicalDeviceImageFormatProperties2 "
                             "returned back %s.",
                             string_VkExternalMemoryHandleTypeFlagBits(external_image_info.handleType),
                             string_VkFormat(image_format_info.format),
                             string_VkImageType(image_format_info.type),
                             string_VkImageTiling(image_format_info.tiling),
                             string_VkImageUsageFlags(image_format_info.usage).c_str(),
                             string_VkImageCreateFlags(image_format_info.flags).c_str(),
                             string_VkResult(result));
        } else if ((external_memory_create_info->handleTypes & compatible_types) !=
                   external_memory_create_info->handleTypes) {
            skip |= LogError("VUID-VkImageCreateInfo-pNext-00990",
                             device,
                             create_info_loc.pNext(Struct::VkExternalMemoryImageCreateInfo, Field::handleTypes),
                             "(%s) is not reported as compatible by vkGetPhysicalDeviceImageFormatProperties2. "
                             "Compatible types are %s.",
        ...
        if (pCreateInfo->initialLayout != VK_IMAGE_LAYOUT_UNDEFINED) {
            skip |= LogError("VUID-VkImageCreateInfo-pNext-01443",
                             ...
                             "is %" PRIu32 " but pCreateInfo->initialLayout is %s.",
        ...
        result = DispatchGetPhysicalDeviceExternalImageFormatPropertiesNV(physical_device,
                                                                          pCreateInfo->format,
                                                                          pCreateInfo->imageType,
                                                                          pCreateInfo->tiling,
                                                                          pCreateInfo->usage,
        ...
        if (result != VK_SUCCESS) {
            skip |= LogError(
                ...
                string_VkResult(result));
                   ...
                   external_memory_create_info_nv->handleTypes) {
            skip |=
                LogError("VUID-VkImageCreateInfo-pNext-00991",
                         device,
                         create_info_loc.pNext(Struct::VkExternalMemoryImageCreateInfoNV, Field::handleTypes),
    ...
    if (device_group_create_info.physicalDeviceCount == 1) {
        if (pCreateInfo->flags & VK_IMAGE_CREATE_SPLIT_INSTANCE_BIND_REGIONS_BIT) {
            skip |=
                LogError("VUID-VkImageCreateInfo-physicalDeviceCount-01421",
                         device,
                         create_info_loc.dot(Field::flags),
    ...
    if (opaque_capture_descriptor_buffer &&
        ...
        skip |= LogError("VUID-VkImageCreateInfo-pNext-08105",
                         device,
                         create_info_loc.dot(Field::flags),
    ...
    if (video_profile_independent && !enabled_features.videoMaintenance1) {
        skip |= LogError("VUID-VkImageCreateInfo-flags-08328",
                         device,
                         create_info_loc.dot(Field::flags),
    ...
    if (video_profile_independent && (pCreateInfo->usage & VK_IMAGE_USAGE_VIDEO_DECODE_DPB_BIT_KHR) &&
        ...
        skip |= LogError("VUID-VkImageCreateInfo-flags-08329",
                         device,
                         create_info_loc.dot(Field::flags),
    ...
    if (video_profile_independent && (pCreateInfo->usage & VK_IMAGE_USAGE_VIDEO_ENCODE_DPB_BIT_KHR)) {
        skip |= LogError("VUID-VkImageCreateInfo-flags-08331",
                         device,
                         create_info_loc.dot(Field::flags),
                         "has VK_IMAGE_CREATE_VIDEO_PROFILE_INDEPENDENT_BIT_KHR set but usage (%s) contains "
                         "VK_IMAGE_USAGE_VIDEO_ENCODE_DPB_BIT_KHR.",
    ...
    if (has_quantization_map_usage) {
        if (!enabled_features.videoEncodeQuantizationMap) {
            skip |= LogError("VUID-VkImageCreateInfo-usage-10251",
                             device,
                             create_info_loc.dot(Field::usage),
        ...
        if (pCreateInfo->imageType != VK_IMAGE_TYPE_2D) {
            skip |= LogError("VUID-VkImageCreateInfo-usage-10252",
                             device,
                             create_info_loc.dot(Field::usage),
                             "has VK_IMAGE_USAGE_VIDEO_ENCODE_QUANTIZATION_DELTA_MAP_BIT_KHR or "
        ...
        if (pCreateInfo->samples != VK_SAMPLE_COUNT_1_BIT) {
            skip |= LogError("VUID-VkImageCreateInfo-usage-10253",
                             device,
                             create_info_loc.dot(Field::usage),
                             "has VK_IMAGE_USAGE_VIDEO_ENCODE_QUANTIZATION_DELTA_MAP_BIT_KHR or "
        ...
        if (video_profiles == nullptr) {
            skip |= LogError("VUID-VkImageCreateInfo-usage-10254",
                             device,
                             create_info_loc.dot(Field::usage),
                             "has VK_IMAGE_USAGE_VIDEO_ENCODE_QUANTIZATION_DELTA_MAP_BIT_KHR or "
            ...
            skip |=
                LogError("VUID-VkImageCreateInfo-usage-10254",
                         device,
                         create_info_loc.dot(Field::usage),
                         "has VK_IMAGE_USAGE_VIDEO_ENCODE_QUANTIZATION_DELTA_MAP_BIT_KHR or "
            ...
            const Location profile_info_loc =
                create_info_loc.pNext(Struct::VkVideoProfileListInfoKHR, Field::pProfiles, 0);
            skip |= ValidateVideoProfileInfo(
                &video_profiles->pProfiles[0], device, create_info_loc.dot(Field::pProfiles, 0));
            ...
            if (profile_desc.IsEncode()) {
                if (pCreateInfo->usage == VK_IMAGE_USAGE_VIDEO_ENCODE_QUANTIZATION_DELTA_MAP_BIT_KHR &&
                    ...
                    skip |= LogError("VUID-VkImageCreateInfo-usage-10255",
                                     device,
                                     create_info_loc.dot(Field::usage),
                ...
                if (pCreateInfo->usage == VK_IMAGE_USAGE_VIDEO_ENCODE_EMPHASIS_MAP_BIT_KHR &&
                    ...
                    skip |= LogError("VUID-VkImageCreateInfo-usage-10256",
                                     device,
                                     create_info_loc.dot(Field::usage),
                ...
                if (valid_quantization_map_format &&
                    pCreateInfo->extent.width >
                        profile_caps.encode_ext.quantization_map.maxQuantizationMapExtent.width) {
                    skip |= LogError("VUID-VkImageCreateInfo-usage-10257",
                                     device,
                                     create_info_loc.dot(Field::usage),
                                     "has VK_IMAGE_USAGE_VIDEO_ENCODE_QUANTIZATION_DELTA_MAP_BIT_KHR or "
                ...
                if (valid_quantization_map_format &&
                    ...
                    skip |= LogError("VUID-VkImageCreateInfo-usage-10258",
                                     device,
                                     create_info_loc.dot(Field::usage),
                                     "has VK_IMAGE_USAGE_VIDEO_ENCODE_QUANTIZATION_DELTA_MAP_BIT_KHR or "
    ...
    if (has_decode_usage || has_encode_usage || valid_quantization_map_format) {
        ...
        if (video_profiles && video_profiles->profileCount > 0) {
            if (!IsSupportedVideoFormat(*pCreateInfo, video_profiles)) {
                skip |= LogError("VUID-VkImageCreateInfo-pNext-06811",
                                 device,
                                 create_info_loc,
    ...
    const auto image_compression_control =
        vku::FindStructInPNextChain<VkImageCompressionControlEXT>(pCreateInfo->pNext);
    if (image_compression_control &&
        (image_compression_control->flags & VK_IMAGE_COMPRESSION_FIXED_RATE_EXPLICIT_EXT) != 0) {
...
void CoreChecks::PostCallRecordCreateImage(VkDevice device,
                                           const VkImageCreateInfo* pCreateInfo,
                                           const VkAllocationCallbacks* pAllocator,
                                           VkImage* pImage,
...
bool CoreChecks::PreCallValidateDestroyImage(VkDevice device,
                                             VkImage image,
                                             const VkAllocationCallbacks* pAllocator,
                                             const ErrorObject& error_obj) const {
    bool skip = false;
    if (auto image_state = Get<vvl::Image>(image)) {
        if (image_state->IsSwapchainImage() && image_state->owned_by_swapchain) {
            skip |= LogError("VUID-vkDestroyImage-image-04882",
                             image,
                             error_obj.location.dot(Field::image),
...
void CoreChecks::PreCallRecordDestroyImage(VkDevice device,
                                           VkImage image,
                                           const VkAllocationCallbacks* pAllocator,
                                           const RecordObject& record_obj) {
...
bool CoreChecks::ValidateClearImageSubresourceRange(const LogObjectList& objlist,
                                                    const VkImageSubresourceRange& range,
                                                    const Location& loc) const {
    bool skip = false;

    if (range.aspectMask != VK_IMAGE_ASPECT_COLOR_BIT) {
        skip |= LogError("VUID-vkCmdClearColorImage-aspectMask-02498",
                         objlist,
                         loc.dot(Field::aspectMask),
                         "is %s (must only include COLOR_BIT).",
                         string_VkImageAspectFlags(range.aspectMask).c_str());
    }

    return skip;
}

bool CoreChecks::PreCallValidateCmdClearColorImage(VkCommandBuffer commandBuffer,
                                                   VkImage image,
    ...
    if (IsExtEnabled(device_extensions.vk_khr_maintenance1)) {
        skip |= ValidateImageFormatFeatureFlags(commandBuffer,
                                                image_state,
    ...
    if (vkuFormatIsDepthOrStencil(format)) {
        skip |= LogError("VUID-vkCmdClearColorImage-image-00007",
                         objlist,
                         image_loc,
                         "(%s) was created with a depth/stencil format (%s).",
        ...
        skip |= LogError("VUID-vkCmdClearColorImage-image-00007",
                         objlist,
                         image_loc,
                         "(%s) was created with a compressed format (%s).",
                         FormatHandle(image).c_str(),
                         string_VkFormat(format));
    }
    ...
    if (!(image_state.create_info.usage & VK_IMAGE_USAGE_TRANSFER_DST_BIT)) {
        skip |= LogError("VUID-vkCmdClearColorImage-image-00002",
                         objlist,
                         image_loc,
                         "(%s) was created with usage %s (missing VK_IMAGE_USAGE_TRANSFER_DST_BIT).",
                         FormatHandle(image).c_str(),
    ...
    if (FormatRequiresYcbcrConversionExplicitly(format)) {
        skip |= LogError("VUID-vkCmdClearColorImage-image-01545",
                         objlist,
                         image_loc,
                         "(%s) was created with format %s.",
                         FormatHandle(image).c_str(),
                         string_VkFormat(format));
    }
    return skip;
...
void CoreChecks::PostCallRecordCmdClearColorImage(VkCommandBuffer commandBuffer,
                                                  VkImage image,
                                                  VkImageLayout imageLayout,
                                                  const VkClearColorValue* pColor,
                                                  uint32_t rangeCount,
                                                  const VkImageSubresourceRange* pRanges,
                                                  const RecordObject& record_obj) {
    BaseClass::PostCallRecordCmdClearColorImage(
        commandBuffer, image, imageLayout, pColor, rangeCount, pRanges, record_obj);
...
bool CoreChecks::ValidateClearDepthStencilValue(VkCommandBuffer commandBuffer,
...
bool CoreChecks::PreCallValidateCmdClearDepthStencilImage(VkCommandBuffer commandBuffer,
                                                          VkImage image,
                                                          VkImageLayout imageLayout,
    ...
    skip |=
        ValidateMemoryIsBoundToImage(objlist, image_state, image_loc, "VUID-vkCmdClearDepthStencilImage-image-00010");
    skip |= ValidateCmd(cb_state, error_obj.location);
    if (IsExtEnabled(device_extensions.vk_khr_maintenance1)) {
        skip |= ValidateImageFormatFeatureFlags(commandBuffer,
                                                image_state,
    ...
    skip |= ValidateProtectedImage(
        cb_state, image_state, image_loc, "VUID-vkCmdClearDepthStencilImage-commandBuffer-01807");
    skip |= ValidateUnprotectedImage(
        cb_state, image_state, image_loc, "VUID-vkCmdClearDepthStencilImage-commandBuffer-01808");
    ...
    for (uint32_t i = 0; i < rangeCount; ++i) {
        ...
        if (((pRanges[i].aspectMask & valid_aspects) == 0) || ((pRanges[i].aspectMask & ~valid_aspects) != 0)) {
            skip |= LogError("VUID-vkCmdClearDepthStencilImage-aspectMask-02824",
                             objlist,
                             range_loc.dot(Field::aspectMask),
                             "is %s (can only be DEPTH_BIT or STENCIL_BIT).",
                             string_VkImageAspectFlags(pRanges[i].aspectMask).c_str());
        }
        if ((pRanges[i].aspectMask & VK_IMAGE_ASPECT_DEPTH_BIT) != 0) {
            if (vkuFormatHasDepth(image_format) == false) {
                skip |= LogError("VUID-vkCmdClearDepthStencilImage-image-02826",
                                 objlist,
                                 range_loc.dot(Field::aspectMask),
            ...
            if ((image_state.create_info.usage & VK_IMAGE_USAGE_TRANSFER_DST_BIT) == 0) {
                skip |= LogError("VUID-vkCmdClearDepthStencilImage-pRanges-02660",
                                 objlist,
                                 range_loc.dot(Field::aspectMask),
                                 "includes VK_IMAGE_ASPECT_DEPTH_BIT, but the image was not created with "
                                 "VK_IMAGE_USAGE_TRANSFER_DST_BIT.");
            }
        }
        if ((pRanges[i].aspectMask & VK_IMAGE_ASPECT_STENCIL_BIT) != 0) {
            if (vkuFormatHasStencil(image_format) == false) {
                skip |= LogError("VUID-vkCmdClearDepthStencilImage-image-02825",
                                 objlist,
                                 range_loc.dot(Field::aspectMask),
            ...
            if (image_stencil_struct != nullptr) {
                if ((image_stencil_struct->stencilUsage & VK_IMAGE_USAGE_TRANSFER_DST_BIT) == 0) {
                    skip |= LogError("VUID-vkCmdClearDepthStencilImage-pRanges-02658",
                                     objlist,
                                     range_loc.dot(Field::aspectMask),
                ...
                skip |= LogError("VUID-vkCmdClearDepthStencilImage-pRanges-02659",
                                 objlist,
                                 range_loc.dot(Field::aspectMask),
                                 "includes VK_IMAGE_ASPECT_STENCIL_BIT and "
    ...
    if (!vkuFormatIsDepthOrStencil(image_format)) {
        skip |= LogError("VUID-vkCmdClearDepthStencilImage-image-00014",
                         objlist,
                         image_loc,
                         "(%s) doesn't have a depth/stencil format (%s).",
                         FormatHandle(image).c_str(),
                         string_VkFormat(image_format));
    }
...
void CoreChecks::PostCallRecordCmdClearDepthStencilImage(VkCommandBuffer commandBuffer,
                                                         VkImage image,
                                                         VkImageLayout imageLayout,
                                                         const VkClearDepthStencilValue* pDepthStencil,
                                                         uint32_t rangeCount,
...
bool CoreChecks::ValidateClearAttachmentExtent(const vvl::CommandBuffer& cb_state,
                                               const VkRect2D& render_area,
                                               uint32_t render_pass_layer_count,
                                               uint32_t rect_count,
    ...
    for (uint32_t i = 0; i < rect_count; i++) {
        if (!ContainsRect(render_area, clear_rects[i].rect)) {
            skip |= LogError("VUID-vkCmdClearAttachments-pRects-00016",
                             cb_state.Handle(),
                             loc.dot(Field::pRects, i).dot(Field::rect),
                             "(%s) is not contained in the area of "
                             "the current render pass instance (%s).",
                             string_VkRect2D(clear_rects[i].rect).c_str(),
                             string_VkRect2D(render_area).c_str());
        }
        ...
        if (rect_base_layer + rect_layer_count > render_pass_layer_count) {
            skip |= LogError(
                ...
                cb_state.Handle(),
                loc.dot(Field::pRects, i).dot(Field::baseArrayLayer),
                ...
                rect_base_layer,
                rect_layer_count,
                rect_base_layer + rect_layer_count,
                render_pass_layer_count);
        }
    }
    return skip;
}

bool CoreChecks::PreCallValidateCmdClearAttachments(VkCommandBuffer commandBuffer,
    ...
    if (cb_state.IsPrimary()) {
        ...
        skip |=
            ValidateClearAttachmentExtent(cb_state, render_area, layer_count, rectCount, pRects, error_obj.location);
    }
    ...
    for (uint32_t attachment_index = 0; attachment_index < attachmentCount; attachment_index++) {
        ...
        if (is_dynamic_rendering) {
            ...
            if ((clear_desc->aspectMask & VK_IMAGE_ASPECT_COLOR_BIT) != 0 &&
                cb_state.rendering_attachments.set_color_locations &&
            ...
            color_view_state = cb_state.GetActiveAttachmentImageViewState(
                cb_state.GetDynamicColorAttachmentImageIndex(colorAttachment));
            color_attachment_count = cb_state.GetDynamicColorAttachmentCount();

            depth_view_state =
                cb_state.GetActiveAttachmentImageViewState(cb_state.GetDynamicDepthAttachmentImageIndex());
            stencil_view_state =
                cb_state.GetActiveAttachmentImageViewState(cb_state.GetDynamicStencilAttachmentImageIndex());
                        ...
                        if (subpass_desc->pResolveAttachments) {
                            ...
                            if (resolve_attachment != VK_ATTACHMENT_UNUSED) {
                                external_format_resolve =
                                    GetExternalFormat(renderpass_create_info->pAttachments[resolve_attachment].pNext) !=
                                    0;
                            }
                        }
                    }
                    ...
                    if (depth_view_state) {
                        ...
                        if ((aspect_mask & VK_IMAGE_ASPECT_DEPTH_BIT) && !vkuFormatHasDepth(image_view_format)) {
                            const LogObjectList objlist(
                                commandBuffer, cb_state.activeRenderPass->Handle(), depth_view_state->Handle());
                            skip |= LogError(
                        ...
                        if ((aspect_mask & VK_IMAGE_ASPECT_STENCIL_BIT) && !vkuFormatHasStencil(image_view_format)) {
                            const LogObjectList objlist(
                                commandBuffer, cb_state.activeRenderPass->Handle(), stencil_view_state->Handle());
                            skip |= LogError(
        ...
        if (aspect_mask & VK_IMAGE_ASPECT_METADATA_BIT) {
            ...
            skip |= LogError("VUID-VkClearAttachment-aspectMask-00020",
                             objlist,
                             attachment_loc.dot(Field::aspectMask),
                             "is %s.",
            ...
            const LogObjectList objlist(commandBuffer, cb_state.activeRenderPass->Handle());
            skip |= LogError("VUID-VkClearAttachment-aspectMask-02246",
                             objlist,
                             attachment_loc.dot(Field::aspectMask),
                             "is %s.",
                             string_VkImageAspectFlags(aspect_mask).c_str());
            ...
            if (clear_desc->colorAttachment == VK_ATTACHMENT_UNUSED) {
                const LogObjectList objlist(commandBuffer, cb_state.activeRenderPass->Handle());
                skip |= LogError("VUID-vkCmdClearAttachments-aspectMask-07271",
                                 objlist,
                                 attachment_loc.dot(Field::colorAttachment),
                ...
                skip |= LogError("VUID-vkCmdClearAttachments-aspectMask-07271",
                                 objlist,
                                 attachment_loc.dot(Field::colorAttachment),
                                 "is index %" PRIu32 ", which is not less than colorAttachmentCount (%" PRIu32
                                 ") which was set by %s",
                                 clear_desc->colorAttachment,
                                 color_attachment_count,
                                 describe_color_count().c_str());
            }

            if ((aspect_mask & VK_IMAGE_ASPECT_DEPTH_BIT) || (aspect_mask & VK_IMAGE_ASPECT_STENCIL_BIT)) {
                const LogObjectList objlist(commandBuffer, cb_state.activeRenderPass->Handle());
                skip |= LogError("VUID-VkClearAttachment-aspectMask-00019",
                                 objlist,
                                 attachment_loc.dot(Field::aspectMask),
                                 "is %s.",
                                 string_VkImageAspectFlags(aspect_mask).c_str());
            ...
            skip |= ValidateClearDepthStencilValue(commandBuffer,
                                                   clear_desc->clearValue.depthStencil,
                                                   attachment_loc.dot(Field::clearValue).dot(Field::depthStencil));
        } else if (external_format_resolve && IsAnyPlaneAspect(aspect_mask)) {
            const LogObjectList objlist(commandBuffer, cb_state.activeRenderPass->Handle());
            skip |= LogError("VUID-vkCmdClearAttachments-aspectMask-09298",
                             objlist,
                             attachment_loc.dot(Field::aspectMask),
                             "is %s.",
                             string_VkImageAspectFlags(aspect_mask).c_str());
        ...
        for (auto image_view : image_views) {
            if (!image_view || !image_view->image_state)
                continue;
            skip |= ValidateProtectedImage(
                cb_state, *image_view->image_state, attachment_loc, "VUID-vkCmdClearAttachments-commandBuffer-02504");
            skip |= ValidateUnprotectedImage(
                cb_state, *image_view->image_state, attachment_loc, "VUID-vkCmdClearAttachments-commandBuffer-02505");
            ...
            if (vkuFormatIs64bit(image_view_format) && vkuFormatComponentCount(image_view_format) > 2) {
                ...
                skip |= LogError(
                    ...
                    FormatHandle(image_view->Handle()).c_str(),
                    string_VkFormat(image_view_format));
            }
        }
        ...
        if (view_mask > 0) {
            for (uint32_t i = 0; i < rectCount; ++i) {
                if (pRects[i].baseArrayLayer != 0 || pRects[i].layerCount != 1) {
                    const LogObjectList objlist(commandBuffer, cb_state.activeRenderPass->Handle());
                    skip |= LogError("VUID-vkCmdClearAttachments-baseArrayLayer-00018",
                                     objlist,
                                     error_obj.location.dot(Field::pRects, i).dot(Field::baseArrayLayer),
                                     "is %" PRIu32 " and layerCount is %" PRIu32
                                     ", but the render pass instance uses multiview.",
                                     pRects[i].baseArrayLayer,
                                     pRects[i].layerCount);
                }
            }
        }
...
void CoreChecks::PostCallRecordCmdClearAttachments(VkCommandBuffer commandBuffer,
                                                   uint32_t attachmentCount,
                                                   const VkClearAttachment* pAttachments,
                                                   uint32_t rectCount,
                                                   const VkClearRect* pRects,
    ...
    BaseClass::PostCallRecordCmdClearAttachments(
        commandBuffer, attachmentCount, pAttachments, rectCount, pRects, record_obj);
            ...
            if (image_index != -1) {
                ...
                auto val_fn = [this, rectCount, clear_rect_copy, record_obj](const vvl::CommandBuffer& secondary,
                    ...
                    return ValidateClearAttachmentExtent(
                        ...
                        prim_cb->activeRenderPass->dynamic_rendering_begin_rendering_info.layerCount,
                        rectCount,
            ...
            if (fb_attachment != VK_ATTACHMENT_UNUSED) {
                ...
                auto val_fn = [this, rectCount, clear_rect_copy, record_obj](const vvl::CommandBuffer& secondary,
                    ...
                    if (fb && prim_cb->IsPrimary()) {
                        skip |= ValidateClearAttachmentExtent(secondary,
                                                              render_area,
                                                              fb->create_info.layers,
                                                              rectCount,
                                                              clear_rect_copy->data(),
                                                              record_obj.location);
                    }
...
bool CoreChecks::ValidateImageUsageFlags(VkCommandBuffer cb,
                                         vvl::Image const& image_state,
                                         VkImageUsageFlags desired,
                                         bool strict,
                                         const char* vuid,
...
bool CoreChecks::ValidateImageFormatFeatureFlags(VkCommandBuffer cb,
                                                 vvl::Image const& image_state,
                                                 VkFormatFeatureFlags2KHR desired,
    ...
    if ((image_format_features & desired) != desired) {
        ...
        if (image_state.HasAHBFormat()) {
            skip |= LogError(vuid,
                             objlist,
                             image_loc,
                             "(%s) was created with an external format having VkFormatFeatureFlags2 (%s) which is "
                             "missing the required feature "
...
bool CoreChecks::ValidateImageAspectMask(VkImage image,
                                         VkFormat format,
                                         VkImageAspectFlags aspect_mask,
                                         bool is_image_disjoint,
                                         const Location& loc,
                                         const char* vuid) const {
    bool skip = false;
    ...
    if ((vkuFormatIsColor(format)) && ((vkuFormatIsMultiplane(format) == false) || (is_image_disjoint == false))) {
        if ((aspect_mask & VK_IMAGE_ASPECT_COLOR_BIT) != VK_IMAGE_ASPECT_COLOR_BIT) {
            skip |= LogError(vuid,
                             image,
                             loc,
                             "Using format (%s) with aspect flags (%s) but color image formats must have the "
        ...
        VkImageAspectFlags valid_flags =
...
bool CoreChecks::ValidateImageSubresourceRange(const uint32_t image_mip_count,
                                               const uint32_t image_layer_count,
                                               const VkImageSubresourceRange& subresourceRange,
                                               vvl::Field image_layer_count_var,
                                               const LogObjectList& objlist,
    ...
    if (subresourceRange.baseMipLevel >= image_mip_count) {
        skip |= LogError(GetSubresourceRangeVUID(subresource_loc, vvl::SubresourceRangeError::BaseMip_01486),
                         objlist,
    ...
    if (subresourceRange.levelCount != VK_REMAINING_MIP_LEVELS) {
        if (subresourceRange.levelCount == 0) {
            skip |= LogError("VUID-VkImageSubresourceRange-levelCount-01720",
                             objlist,
                             subresource_loc.dot(Field::levelCount),
                             "is zero.");
        } else {
            const uint64_t necessary_mip_count =
                uint64_t{ subresourceRange.baseMipLevel } + uint64_t{ subresourceRange.levelCount };

            if (necessary_mip_count > image_mip_count) {
                skip |= LogError(GetSubresourceRangeVUID(subresource_loc, vvl::SubresourceRangeError::MipCount_01724),
                                 objlist,
                                 subresource_loc.dot(Field::baseMipLevel),
    ...
    if (subresourceRange.aspectMask & VK_IMAGE_ASPECT_COLOR_BIT) {
        if (subresourceRange.aspectMask &
            ...
            skip |= LogError("VUID-VkImageSubresourceRange-aspectMask-01670",
                             objlist,
    ...
    if (subresourceRange.aspectMask &
        (VK_IMAGE_ASPECT_MEMORY_PLANE_0_BIT_EXT | VK_IMAGE_ASPECT_MEMORY_PLANE_1_BIT_EXT |
         VK_IMAGE_ASPECT_MEMORY_PLANE_2_BIT_EXT | VK_IMAGE_ASPECT_MEMORY_PLANE_3_BIT_EXT)) {
        skip |= LogError("VUID-VkImageSubresourceRange-aspectMask-02278",
                         objlist,
                         subresource_loc.dot(Field::aspectMask),
                         "is %s.",
                         string_VkImageAspectFlags(subresourceRange.aspectMask).c_str());
...
bool CoreChecks::ValidateCreateImageViewSubresourceRange(const vvl::Image& image_state,
                                                         bool is_imageview_2d_type,
                                                         const VkImageSubresourceRange& subresourceRange,
                                                         const Location& loc) const {
    const bool is_khr_maintenance1 = IsExtEnabled(device_extensions.vk_khr_maintenance1);
    const bool is_2d_compatible = image_state.create_info.flags & (VK_IMAGE_CREATE_2D_ARRAY_COMPATIBLE_BIT |
    ...
    return ValidateImageSubresourceRange(image_state.create_info.mipLevels,
                                         image_layer_count,
                                         subresourceRange,
                                         image_layer_count_var,
                                         image_state.VkHandle(),
...
bool CoreChecks::ValidateCmdClearColorSubresourceRange(const VkImageCreateInfo& create_info,
                                                       ...
                                                       const LogObjectList& objlist,
                                                       const Location& loc) const {
    return ValidateImageSubresourceRange(create_info.mipLevels,
...
bool CoreChecks::ValidateImageViewFormatFeatures(const vvl::Image& image_state,
                                                 const VkFormat view_format,
                                                 const VkImageUsageFlags image_usage,
                                                 const Location& create_info_loc) const {
    // Pass in image_usage here instead of extracting it from image_state in case there's a chained
    ...
    if (tiling_features == 0) {
        ...
        skip |= LogError("VUID-VkImageViewCreateInfo-usage-02275",
                         image_state.Handle(),
                         create_info_loc.dot(Field::format),
               ...
               !(tiling_features &
                 (VK_FORMAT_FEATURE_COLOR_ATTACHMENT_BIT | VK_FORMAT_FEATURE_2_LINEAR_COLOR_ATTACHMENT_BIT_NV))) {
        skip |= LogError("VUID-VkImageViewCreateInfo-usage-08931",
                         image_state.Handle(),
                         create_info_loc.dot(Field::format),
        ...
        skip |= LogError("VUID-VkImageViewCreateInfo-usage-02277",
                         image_state.Handle(),
                         create_info_loc.dot(Field::format),
        ...
        if (!enabled_features.externalFormatResolve && !android_external_format_resolve_null_color_attachment_prop &&
            ...
            skip |= LogError("VUID-VkImageViewCreateInfo-usage-08932",
                             image_state.Handle(),
                             create_info_loc.dot(Field::format),
        ...
        skip |= LogError("VUID-VkImageViewCreateInfo-usage-10260",
                         image_state.Handle(),
                         create_info_loc.dot(Field::usage),
    ...
    if (image_state.create_info.flags & VK_IMAGE_CREATE_VIDEO_PROFILE_INDEPENDENT_BIT_KHR) {
        if ((image_usage & VK_IMAGE_USAGE_VIDEO_DECODE_DST_BIT_KHR) &&
            ...
            skip |= LogError("VUID-VkImageViewCreateInfo-image-08333",
                             image_state.Handle(),
                             create_info_loc.dot(Field::format),
            ...
            skip |= LogError("VUID-VkImageViewCreateInfo-image-08334",
                             image_state.Handle(),
                             create_info_loc.dot(Field::format),
            ...
            skip |= LogError("VUID-VkImageViewCreateInfo-image-08336",
                             image_state.Handle(),
                             create_info_loc.dot(Field::format),
            ...
            skip |= LogError("VUID-VkImageViewCreateInfo-image-08337",
                             image_state.Handle(),
                             create_info_loc.dot(Field::format),
...
bool CoreChecks::PreCallValidateCreateImageView(VkDevice device,
                                                const VkImageViewCreateInfo* pCreateInfo,
                                                [[maybe_unused]] const VkAllocationCallbacks* pAllocator,
                                                [[maybe_unused]] VkImageView* pView,
                                                const ErrorObject& error_obj) const {
    bool skip = false;
    ...
    const VkImageUsageFlags valid_usage_flags =
        ...
        VK_IMAGE_USAGE_VIDEO_ENCODE_EMPHASIS_MAP_BIT_KHR;
    skip |= ValidateImageUsageFlags(VK_NULL_HANDLE,
                                    image_state,
                                    valid_usage_flags,
                                    false,
    ...
    skip |= ValidateMemoryIsBoundToImage(LogObjectList(device, pCreateInfo->image),
                                         image_state,
                                         create_info_loc.dot(Field::image),
                                         "VUID-VkImageViewCreateInfo-image-01020");
    // Checks imported from image layer
    skip |= ValidateCreateImageViewSubresourceRange(image_state,
        ...
        if (IsExtEnabled(device_extensions.vk_khr_maintenance2)) {
            ...
            if (image_stencil_struct == nullptr) {
                if ((image_usage | chained_ivuci_struct->usage) != image_usage) {
                    skip |= LogError("VUID-VkImageViewCreateInfo-pNext-02662",
                                     pCreateInfo->image,
                                     create_info_loc.pNext(Struct::VkImageViewUsageCreateInfo, Field::usage),
                                     "(%s) must not include any bits that were not set in VkImageCreateInfo::usage "
                                     "(%s) of the image.",
                ...
                if ((aspect_mask & VK_IMAGE_ASPECT_STENCIL_BIT) == VK_IMAGE_ASPECT_STENCIL_BIT &&
                    ...
                    skip |= LogError("VUID-VkImageViewCreateInfo-pNext-02663",
                                     pCreateInfo->image,
                                     create_info_loc.pNext(Struct::VkImageViewUsageCreateInfo, Field::usage),
                                     "(%s) must not include any bits that were not set in "
                ...
                if ((aspect_mask & ~VK_IMAGE_ASPECT_STENCIL_BIT) != 0 &&
                    ...
                    skip |= LogError(
                        ...
                        pCreateInfo->image,
                        create_info_loc.pNext(Struct::VkImageViewUsageCreateInfo, Field::usage),
                        "(%s) must not include any bits that were not set in VkImageCreateInfo::usage (%s) of the "
                        "image "
    ...
    if (const auto sliced_create_info_ext =
            vku::FindStructInPNextChain<VkImageViewSlicedCreateInfoEXT>(pCreateInfo->pNext);
        sliced_create_info_ext) {
        const bool feature_disabled = (enabled_features.imageSlicedViewOf3D == VK_FALSE);
        if (feature_disabled) {
            skip |= LogError("VUID-VkImageViewSlicedCreateInfoEXT-None-07871",
                             pCreateInfo->image,
                             create_info_loc,
        ...
        if (normalized_subresource_range.levelCount != 1) {
            skip |= LogError("VUID-VkImageViewSlicedCreateInfoEXT-None-07870",
                             pCreateInfo->image,
        ...
        const uint32_t effective_view_depth =
            image_state.GetEffectiveSubresourceExtent(pCreateInfo->subresourceRange).depth;
        ...
        if (slice_offset >= effective_view_depth) {
            skip |= LogError("VUID-VkImageViewSlicedCreateInfoEXT-sliceOffset-07867",
                             pCreateInfo->image,
                             create_info_loc.pNext(Struct::VkImageViewSlicedCreateInfoEXT, Field::sliceOffset),
                             "(%" PRIu32 ") must be less than the effective view depth (%" PRIu32 ").",
                             slice_offset,
        ...
        if (slice_count != VK_REMAINING_3D_SLICES_EXT) {
            if (slice_count == 0) {
                skip |= LogError("VUID-VkImageViewSlicedCreateInfoEXT-sliceCount-07868",
                                 pCreateInfo->image,
                                 create_info_loc.pNext(Struct::VkImageViewSlicedCreateInfoEXT, Field::sliceCount),
                                 "is 0.");
            }
            ...
            if ((slice_offset + slice_count) > effective_view_depth) {
                skip |= LogError("VUID-VkImageViewSlicedCreateInfoEXT-sliceCount-07868",
                                 pCreateInfo->image,
                                 create_info_loc.pNext(Struct::VkImageViewSlicedCreateInfoEXT, Field::sliceOffset),
                                 "(%" PRIu32 ") + sliceCount (%" PRIu32
                                 ") is greater than effective view depth (%" PRIu32 ").",
                                 slice_offset,
                                 slice_count,
                                 effective_view_depth);
            }
    ...
    if (const auto format_list_info =
        ...
        if (found_format == false) {
            skip |= LogError("VUID-VkImageViewCreateInfo-pNext-01585",
                             pCreateInfo->image,
                             create_info_loc.pNext(Struct::VkImageFormatListCreateInfo, Field::pViewFormats),
                             "has no formats that match the VkImageViewCreateInfo::format (%s).",
                             string_VkFormat(view_format));
        }
    }
    ...
    if (multiplane_image && IsMultiplePlaneAspect(aspect_mask)) {
        skip |= LogError("VUID-VkImageViewCreateInfo-subresourceRange-07818",
                         pCreateInfo->image,
                         create_info_loc.dot(Field::subresourceRange).dot(Field::aspectMask),
                         "(%s) is invalid for %s.",
                         string_VkImageAspectFlags(aspect_mask).c_str(),
                         string_VkFormat(image_format));
    }
    ...
    if ((image_flags & VK_IMAGE_CREATE_MUTABLE_FORMAT_BIT) && (image_format != view_format)) {
        ...
        if (multiplane_image) {
            ...
            if (has_valid_aspect &&
                ((image_class != view_class) || (image_class == VKU_FORMAT_COMPATIBILITY_CLASS_NONE))) {
                ...
                skip |= LogError("VUID-VkImageViewCreateInfo-image-01586",
                                 pCreateInfo->image,
                                 create_info_loc.dot(Field::format),
                                 "(%s) is not compatible with plane %" PRIu32
                                 " of the %s format %s, must be compatible with %s.",
                                 string_VkFormat(view_format),
                                 vkuGetPlaneIndex(static_cast<VkImageAspectFlagBits>(aspect_mask)),
                                 FormatHandle(pCreateInfo->image).c_str(),
                                 string_VkFormat(image_format),
                                 string_VkFormat(compat_format));
            }
        } else if (!(image_flags & VK_IMAGE_CREATE_BLOCK_TEXEL_VIEW_COMPATIBLE_BIT)) {
            // Format MUST be compatible (in the same format compatibility class) as the format the image was created
            // with
            ...
            if ((image_class != view_class) || (image_class == VKU_FORMAT_COMPATIBILITY_CLASS_NONE)) {
                skip |= LogError(
                    ...
                    "VK_IMAGE_CREATE_MUTABLE_FORMAT BIT can support ImageViews with differing formats but they must be "
                    "in "
    ...
    if (image_state.create_info.samples != VK_SAMPLE_COUNT_1_BIT && view_type != VK_IMAGE_VIEW_TYPE_2D &&
        ...
        skip |= LogError("VUID-VkImageViewCreateInfo-image-04972",
                         pCreateInfo->image,
                         create_info_loc.dot(Field::image),
                         "was created with sample count %s, but pCreateInfo->viewType is %s.",
                         string_VkSampleCountFlagBits(image_state.create_info.samples),
                         string_VkImageViewType(view_type));
    }

    // Validate correct image aspect bits for desired formats and format consistency
    skip |= ValidateImageAspectMask(image_state.VkHandle(),
                                    image_format,
                                    aspect_mask,
                                    image_state.disjoint,
                                    error_obj.location,
    ...
    switch (image_type) {
        case VK_IMAGE_TYPE_1D:
            if (view_type != VK_IMAGE_VIEW_TYPE_1D && view_type != VK_IMAGE_VIEW_TYPE_1D_ARRAY) {
                skip |= LogError("VUID-VkImageViewCreateInfo-subResourceRange-01021",
                                 pCreateInfo->image,
                                 create_info_loc.dot(Field::viewType),
        ...
        case VK_IMAGE_TYPE_2D:
            if (view_type != VK_IMAGE_VIEW_TYPE_2D && view_type != VK_IMAGE_VIEW_TYPE_2D_ARRAY) {
                if ((view_type == VK_IMAGE_VIEW_TYPE_CUBE || view_type == VK_IMAGE_VIEW_TYPE_CUBE_ARRAY) &&
                    ...
                    skip |= LogError("VUID-VkImageViewCreateInfo-image-01003",
                                     pCreateInfo->image,
                                     create_info_loc.dot(Field::viewType),
                                     "(%s) is not compatible with image type %s.",
                                     string_VkImageViewType(view_type),
                        ...
                        if (IsExtEnabled(device_extensions.vk_ext_image_2d_view_of_3d)) {
                            if (!(image_flags & VK_IMAGE_CREATE_2D_ARRAY_COMPATIBLE_BIT)) {
                                if (view_type == VK_IMAGE_VIEW_TYPE_2D_ARRAY) {
                                    skip |= LogError("VUID-VkImageViewCreateInfo-image-06723",
                                                     pCreateInfo->image,
                                                     create_info_loc.dot(Field::viewType),
                                                     "(%s) is not compatible with image type "
                                                     "%s since the image doesn't have "
                                                     "VK_IMAGE_CREATE_2D_ARRAY_COMPATIBLE_BIT flag set.",
                                                     string_VkImageViewType(view_type),
                                                     string_VkImageType(image_type));
    ...
    if (enabled_features.shadingRateImage) {
        if (image_usage & VK_IMAGE_USAGE_SHADING_RATE_IMAGE_BIT_NV) {
            if (view_format != VK_FORMAT_R8_UINT) {
                skip |= LogError("VUID-VkImageViewCreateInfo-image-02087",
                                 pCreateInfo->image,
                                 create_info_loc.dot(Field::image),
    ...
    if (enabled_features.shadingRateImage || enabled_features.attachmentFragmentShadingRate) {
        if (image_usage & VK_IMAGE_USAGE_FRAGMENT_SHADING_RATE_ATTACHMENT_BIT_KHR) {
            if (view_type != VK_IMAGE_VIEW_TYPE_2D && view_type != VK_IMAGE_VIEW_TYPE_2D_ARRAY) {
                skip |= LogError("VUID-VkImageViewCreateInfo-image-02086",
                                 pCreateInfo->image,
                                 create_info_loc.dot(Field::image),
                                 "was created with usage containing "
    ...
    if (enabled_features.attachmentFragmentShadingRate &&
        ...
        normalized_subresource_range.layerCount != 1) {
        skip |= LogError("VUID-VkImageViewCreateInfo-usage-04551",
                         pCreateInfo->image,
    ...
    if (pCreateInfo->subresourceRange.layerCount == VK_REMAINING_ARRAY_LAYERS) {
        const uint32_t remaining_layers =
            image_state.create_info.arrayLayers - pCreateInfo->subresourceRange.baseArrayLayer;
        if (view_type == VK_IMAGE_VIEW_TYPE_CUBE && remaining_layers != 6) {
            skip |= LogError("VUID-VkImageViewCreateInfo-viewType-02962",
                             pCreateInfo->image,
                             create_info_loc.dot(Field::subresourceRange).dot(Field::layerCount),
                             "is VK_REMAINING_ARRAY_LAYERS (%d) but must be 6.",
                             remaining_layers);
        }
        if (view_type == VK_IMAGE_VIEW_TYPE_CUBE_ARRAY && ((remaining_layers) % 6) != 0) {
            skip |= LogError("VUID-VkImageViewCreateInfo-viewType-02963",
                             pCreateInfo->image,
                             create_info_loc.dot(Field::subresourceRange).dot(Field::layerCount),
                             "is VK_REMAINING_ARRAY_LAYERS (%d) but must be a multiple of 6.",
                             remaining_layers);
        }
        ...
        if ((pCreateInfo->subresourceRange.layerCount != 1) &&
            ((view_type == VK_IMAGE_VIEW_TYPE_1D) || (view_type == VK_IMAGE_VIEW_TYPE_2D) ||
             (view_type == VK_IMAGE_VIEW_TYPE_3D))) {
            skip |= LogError("VUID-VkImageViewCreateInfo-imageViewType-04973",
                             pCreateInfo->image,
                             create_info_loc.dot(Field::subresourceRange).dot(Field::layerCount),
                             "(%" PRIu32
    ...
    if (pCreateInfo->flags & VK_IMAGE_VIEW_CREATE_FRAGMENT_DENSITY_MAP_DYNAMIC_BIT_EXT) {
        ...
        if (image_usage & VK_IMAGE_USAGE_FRAGMENT_DENSITY_MAP_BIT_EXT) {
            if (image_flags & (VK_IMAGE_CREATE_PROTECTED_BIT | VK_IMAGE_CREATE_SPARSE_BINDING_BIT |
                ...
                skip |= LogError("VUID-VkImageViewCreateInfo-flags-04116",
                                 pCreateInfo->image,
                                 create_info_loc.dot(Field::image),
                                 "was created with usage containing "
    ...
    if (image_flags & VK_IMAGE_CREATE_BLOCK_TEXEL_VIEW_COMPATIBLE_BIT) {
        ...
        const bool class_compatible =
            vkuFormatCompatibilityClass(view_format) == vkuFormatCompatibilityClass(image_format);
        ...
        if (!class_compatible && !size_compatible) {
            skip |= LogError("VUID-VkImageViewCreateInfo-image-01583",
                             pCreateInfo->image,
                             create_info_loc.dot(Field::image),
    ...
    if (pCreateInfo->flags & VK_IMAGE_VIEW_CREATE_FRAGMENT_DENSITY_MAP_DEFERRED_BIT_EXT) {
        if (!enabled_features.fragmentDensityMapDeferred) {
            skip |= LogError("VUID-VkImageViewCreateInfo-flags-03567",
                             pCreateInfo->image,
                             create_info_loc.dot(Field::flags),
        ...
        if (pCreateInfo->flags & VK_IMAGE_VIEW_CREATE_FRAGMENT_DENSITY_MAP_DYNAMIC_BIT_EXT) {
            skip |= LogError("VUID-VkImageViewCreateInfo-flags-03568",
                             pCreateInfo->image,
                             create_info_loc.dot(Field::flags),
    ...
    if (const auto astc_decode_mode = vku::FindStructInPNextChain<VkImageViewASTCDecodeModeEXT>(pCreateInfo->pNext);
        ...
        if ((enabled_features.decodeModeSharedExponent == VK_FALSE) &&
            ...
            skip |=
                LogError("VUID-VkImageViewASTCDecodeModeEXT-decodeMode-02231",
                         pCreateInfo->image,
    ...
    if (IsExtEnabled(device_extensions.vk_khr_portability_subset)) {
        ...
        //       However, issue https://github.com/KhronosGroup/Vulkan-Portability/issues/27 points out that the
        //       identity can also be defined via R, G, B, A enums in the correct order. Spec change is at
        //       https://gitlab.khronos.org/vulkan/vulkan/-/merge_requests/4600
        if ((VK_FALSE == enabled_features.imageViewFormatSwizzle) && !IsIdentitySwizzle(pCreateInfo->components)) {
            skip |= LogError("VUID-VkImageViewCreateInfo-imageViewFormatSwizzle-04465",
                             pCreateInfo->image,
                             create_info_loc,
                             "(portability error): swizzle is disabled for this device.");
        }

        // Ensure ImageView's format has the same number of bits and components as Image's format if format
        // reinterpretation is disabled
        if (!enabled_features.imageViewFormatReinterpretation &&
            ...
            skip |= LogError("VUID-VkImageViewCreateInfo-imageViewFormatReinterpretation-04466",
                             pCreateInfo->image,
                             create_info_loc,
    ...
    if (const auto image_view_min_lod = vku::FindStructInPNextChain<VkImageViewMinLodCreateInfoEXT>(pCreateInfo->pNext);
        ...
        if ((!enabled_features.minLod) && (image_view_min_lod->minLod != 0)) {
            skip |= LogError("VUID-VkImageViewMinLodCreateInfoEXT-minLod-06455",
                             pCreateInfo->image,
                             create_info_loc.pNext(Struct::VkImageViewMinLodCreateInfoEXT, Field::minLod),
                             "%f, but the minLod feature is not enabled.",
                             image_view_min_lod->minLod);
        }
        const float max_level = static_cast<float>(pCreateInfo->subresourceRange.baseMipLevel +
                                                   (normalized_subresource_range.levelCount - 1));
        if (image_view_min_lod->minLod > max_level) {
            skip |= LogError("VUID-VkImageViewMinLodCreateInfoEXT-minLod-06456",
                             pCreateInfo->image,
                             create_info_loc.pNext(Struct::VkImageViewMinLodCreateInfoEXT, Field::minLod),
    ...
    if (ycbcr_conversion && ycbcr_conversion->conversion != VK_NULL_HANDLE) {
        ...
        if (ycbcr_state && (pCreateInfo->format != ycbcr_state->format)) {
            skip |= LogError("VUID-VkImageViewCreateInfo-pNext-06658",
                             pCreateInfo->image,
        ...
        skip |= LogError("VUID-VkImageViewCreateInfo-format-06415",
                         pCreateInfo->image,
                         create_info_loc.dot(Field::image),
    ...
    if (pCreateInfo->viewType != VK_IMAGE_VIEW_TYPE_2D && pCreateInfo->viewType != VK_IMAGE_VIEW_TYPE_2D_ARRAY) {
        VkImageUsageFlags decode_usage = VK_IMAGE_USAGE_VIDEO_DECODE_DST_BIT_KHR |
                                         VK_IMAGE_USAGE_VIDEO_DECODE_SRC_BIT_KHR |
                                         VK_IMAGE_USAGE_VIDEO_DECODE_DPB_BIT_KHR;
        if (image_usage & decode_usage) {
            skip |= LogError("VUID-VkImageViewCreateInfo-image-04817",
                             pCreateInfo->image,
                             create_info_loc.dot(Field::viewType),
                             "%s is incompatible with video decode usage.",
                             string_VkImageViewType(pCreateInfo->viewType));
        }

        VkImageUsageFlags encode_usage = VK_IMAGE_USAGE_VIDEO_ENCODE_DST_BIT_KHR |
                                         VK_IMAGE_USAGE_VIDEO_ENCODE_SRC_BIT_KHR |
                                         VK_IMAGE_USAGE_VIDEO_ENCODE_DPB_BIT_KHR;
        if (image_usage & encode_usage) {
            skip |= LogError("VUID-VkImageViewCreateInfo-image-04818",
                             pCreateInfo->image,
                             create_info_loc.dot(Field::viewType),
                             "%s is incompatible with video encode usage.",
                             string_VkImageViewType(pCreateInfo->viewType));
        }

        VkImageUsageFlags quantization_map_usage = VK_IMAGE_USAGE_VIDEO_ENCODE_QUANTIZATION_DELTA_MAP_BIT_KHR |
                                                   VK_IMAGE_USAGE_VIDEO_ENCODE_EMPHASIS_MAP_BIT_KHR;
        if (image_usage & quantization_map_usage) {
            skip |= LogError("VUID-VkImageViewCreateInfo-image-10261",
                             pCreateInfo->image,
                             create_info_loc.dot(Field::viewType),
    ...
    if ((pCreateInfo->flags & VK_IMAGE_VIEW_CREATE_DESCRIPTOR_BUFFER_CAPTURE_REPLAY_BIT_EXT) &&
        !enabled_features.descriptorBufferCaptureReplay) {
        skip |= LogError("VUID-VkImageViewCreateInfo-flags-08106",
                         pCreateInfo->image,
                         create_info_loc.dot(Field::flags),
    ...
    if (const auto opaque_capture_descriptor_buffer =
        ...
        skip |= LogError("VUID-VkImageViewCreateInfo-pNext-08107",
                         pCreateInfo->image,
                         create_info_loc.dot(Field::flags),
    ...
    if (vkuFormatIsXChromaSubsampled(view_format) && (SafeModulo(image_state.create_info.extent.width, 2) != 0)) {
        skip |= LogError("VUID-VkImageViewCreateInfo-format-04714",
                         device,
                         create_info_loc.dot(Field::format),
    ...
    if (vkuFormatIsYChromaSubsampled(view_format) && (SafeModulo(image_state.create_info.extent.height, 2) != 0)) {
        skip |= LogError("VUID-VkImageViewCreateInfo-format-04715",
                         device,
                         create_info_loc.dot(Field::format),
                         "(%s) is Y Chroma Subsampled (has _420 suffix) so the image height (%" PRIu32
                         ") must be a multiple of 2.",
                         string_VkFormat(view_format),
...
bool CoreChecks::PreCallValidateDestroyImageView(VkDevice device,
    ...
    if (auto image_view_state = Get<vvl::ImageView>(imageView)) {
        skip |= ValidateObjectNotInUse(
            image_view_state.get(), error_obj.location, "VUID-vkDestroyImageView-imageView-01026");
    }
    return skip;
}

bool CoreChecks::ValidateGetImageSubresourceLayout(const vvl::Image& image_state,
    ...
    if (GetBitSetCount(aspect_mask) != 1) {
        const char* vuid = is_2 ? "VUID-vkGetImageSubresourceLayout2-aspectMask-00997"
    ...
    if (subresource.mipLevel >= image_state.create_info.mipLevels) {
        const char* vuid = is_2 ? "VUID-vkGetImageSubresourceLayout2-mipLevel-01716"
        ...
        skip |= LogError(vuid,
                         ...
                         image_state.create_info.mipLevels);
    }
    ...
    if (subresource.arrayLayer >= image_state.create_info.arrayLayers) {
        const char* vuid = is_2 ? "VUID-vkGetImageSubresourceLayout2-arrayLayer-01717"
        ...
        skip |= LogError(vuid,
                         ...
                         image_state.create_info.arrayLayers);
    }

    const VkFormat image_format = image_state.create_info.format;
    const bool tiling_linear_optimal = image_state.create_info.tiling == VK_IMAGE_TILING_LINEAR ||
                                       image_state.create_info.tiling == VK_IMAGE_TILING_OPTIMAL;
    if (vkuFormatIsColor(image_format) && !vkuFormatIsMultiplane(image_format) &&
        ...
        skip |= LogError(vuid,
                         image_state.Handle(),
                         subresource_loc.dot(Field::aspectMask),
                         "is %s but image was created with color format %s.",
                         string_VkImageAspectFlags(aspect_mask).c_str(),
                         string_VkFormat(image_format));
    }
    ...
    if (vkuFormatHasDepth(image_format) && ((aspect_mask & VK_IMAGE_ASPECT_DEPTH_BIT) == 0)) {
        ...
        skip |= LogError(vuid,
                         image_state.Handle(),
                         subresource_loc.dot(Field::aspectMask),
                         "is %s but image was created with depth format %s.",
                         string_VkImageAspectFlags(aspect_mask).c_str(),
                         string_VkFormat(image_format));
    }
    ...
    if (vkuFormatHasStencil(image_format) && ((aspect_mask & VK_IMAGE_ASPECT_STENCIL_BIT) == 0)) {
        ...
        skip |= LogError(vuid,
                         image_state.Handle(),
                         subresource_loc.dot(Field::aspectMask),
                         "is %s but image was created with stencil format %s.",
                         string_VkImageAspectFlags(aspect_mask).c_str(),
                         string_VkFormat(image_format));
    }
    ...
    if (!vkuFormatHasDepth(image_format) && !vkuFormatHasStencil(image_format)) {
        if ((aspect_mask & (VK_IMAGE_ASPECT_DEPTH_BIT | VK_IMAGE_ASPECT_STENCIL_BIT)) != 0) {
            const char* vuid = is_2 ? "VUID-vkGetImageSubresourceLayout2-format-04464"
                                    : "VUID-vkGetImageSubresourceLayout-format-04464";
            skip |= LogError(vuid,
                             image_state.Handle(),
                             subresource_loc.dot(Field::aspectMask),
                             "is %s but image was created with format %s.",
                             string_VkImageAspectFlags(aspect_mask).c_str(),
                             string_VkFormat(image_format));
        }
    }
    ...
    if (image_state.create_info.tiling == VK_IMAGE_TILING_LINEAR) {
        if (vkuFormatIsMultiplane(image_format) && !IsOnlyOneValidPlaneAspect(image_format, aspect_mask)) {
            const char* vuid = is_2 ? "VUID-vkGetImageSubresourceLayout2-tiling-08717"
                                    : "VUID-vkGetImageSubresourceLayout-tiling-08717";
            skip |= LogError(vuid,
                             image_state.Handle(),
                             subresource_loc.dot(Field::aspectMask),
                             "(%s) is invalid for format %s.",
                             string_VkImageAspectFlags(aspect_mask).c_str(),
                             string_VkFormat(image_format));
        }
    } else if (image_state.create_info.tiling == VK_IMAGE_TILING_DRM_FORMAT_MODIFIER_EXT) {
        if ((aspect_mask != VK_IMAGE_ASPECT_MEMORY_PLANE_0_BIT_EXT) &&
            (aspect_mask != VK_IMAGE_ASPECT_MEMORY_PLANE_1_BIT_EXT) &&
            (aspect_mask != VK_IMAGE_ASPECT_MEMORY_PLANE_2_BIT_EXT) &&
            (aspect_mask != VK_IMAGE_ASPECT_MEMORY_PLANE_3_BIT_EXT)) {
            const char* vuid = is_2 ? "VUID-vkGetImageSubresourceLayout2-tiling-09435"
                                    : "VUID-vkGetImageSubresourceLayout-tiling-09433";
            skip |= LogError(vuid,
                             image_state.Handle(),
                             subresource_loc.dot(Field::aspectMask),
                             "(%s) must be VK_IMAGE_ASPECT_MEMORY_PLANE_i_BIT_EXT.",
                             string_VkImageAspectFlags(aspect_mask).c_str());
        } else {
            VkImageDrmFormatModifierPropertiesEXT drm_format_properties = vku::InitStructHelper();
            DispatchGetImageDrmFormatModifierPropertiesEXT(device, image_state.VkHandle(), &drm_format_properties);
            ...
            VkDrmFormatModifierPropertiesListEXT fmt_drm_props = vku::InitStructHelper();
            VkFormatProperties2 fmt_props_2 = vku::InitStructHelper(&fmt_drm_props);
            DispatchGetPhysicalDeviceFormatProperties2Helper(
                physical_device, image_state.create_info.format, &fmt_props_2);
            ...
            VkImageAspectFlagBits allowed_plane_indices[] = { VK_IMAGE_ASPECT_MEMORY_PLANE_0_BIT_EXT,
    ...
    if (image_state.IsExternalBuffer() && image_state.GetBoundMemoryStates().empty()) {
        const char* vuid =
            is_2 ? "VUID-vkGetImageSubresourceLayout2-image-09434" : "VUID-vkGetImageSubresourceLayout-image-09432";
        skip |= LogError(
            vuid,
            image_state.Handle(),
            subresource_loc,
...
bool CoreChecks::PreCallValidateGetImageSubresourceLayout(VkDevice device,
                                                          VkImage image,
    ...
    if (pSubresource && pLayout && image_state) {
        skip |=
            ValidateGetImageSubresourceLayout(*image_state, *pSubresource, error_obj.location.dot(Field::pSubresource));
        if ((image_state->create_info.tiling != VK_IMAGE_TILING_LINEAR) &&
            (image_state->create_info.tiling != VK_IMAGE_TILING_DRM_FORMAT_MODIFIER_EXT)) {
            skip |= LogError("VUID-vkGetImageSubresourceLayout-image-07790",
                             image,
                             error_obj.location,
                             "image was created with tiling %s.",
                             string_VkImageTiling(image_state->create_info.tiling));
        }
    }
    return skip;
...
bool CoreChecks::PreCallValidateGetImageSubresourceLayout2(VkDevice device,
                                                           VkImage image,
    ...
    if (pSubresource && pLayout && image_state) {
        skip |=
            ValidateGetImageSubresourceLayout(*image_state,
...
bool CoreChecks::PreCallValidateGetImageSubresourceLayout2KHR(VkDevice device,
                                                              VkImage image,
...
bool CoreChecks::PreCallValidateGetImageSubresourceLayout2EXT(VkDevice device,
                                                              VkImage image,
...
bool CoreChecks::PreCallValidateGetImageDrmFormatModifierPropertiesEXT(
    VkDevice device,
    VkImage image,
    ...
    if (auto image_state = Get<vvl::Image>(image)) {
        if (image_state->create_info.tiling != VK_IMAGE_TILING_DRM_FORMAT_MODIFIER_EXT) {
            skip |= LogError("VUID-vkGetImageDrmFormatModifierPropertiesEXT-image-02272",
                             image,
                             error_obj.location.dot(Field::image),
                             "was created with tiling %s.",
                             string_VkImageTiling(image_state->create_info.tiling));
        }
    }
    return skip;
...
bool CoreChecks::PreCallValidateTransitionImageLayout(VkDevice device,
    ...
    for (uint32_t i = 0; i < transitionCount; ++i) {
        ...
        if ((image_state->create_info.usage & VK_IMAGE_USAGE_HOST_TRANSFER_BIT) == 0) {
            ...
            skip |= LogError("VUID-VkHostImageLayoutTransitionInfo-image-09055",
                             objlist,
                             transition_loc.dot(Field::image),
                             "was created with usage (%s) which does not contain "
                             "VK_IMAGE_USAGE_HOST_TRANSFER_BIT.",
                             string_VkImageUsageFlags(image_state->create_info.usage).c_str());
        }

        skip |= ValidateImageSubresourceRange(image_state->create_info.mipLevels,
                                              image_state->create_info.arrayLayers,
                                              transition.subresourceRange,
                                              Field::arrayLayers,
                                              image_state->VkHandle(),
                                              transition_loc.dot(Field::subresourceRange));
        skip |= ValidateMemoryIsBoundToImage(LogObjectList(device, transition.image),
                                             *image_state,
                                             transition_loc.dot(Field::image),
        ...
        if (vkuFormatIsColor(image_format) && (aspect_mask != VK_IMAGE_ASPECT_COLOR_BIT)) {
            if (!vkuFormatIsMultiplane(image_format)) {
                const LogObjectList objlist(device, image_state->Handle());
                skip |= LogError("VUID-VkHostImageLayoutTransitionInfo-image-09241",
                                 objlist,
                ...
                const LogObjectList objlist(device, image_state->Handle());
                skip |= LogError("VUID-VkHostImageLayoutTransitionInfo-image-09242",
                                 objlist,
                                 transition_loc.dot(Field::subresourceRange).dot(Field::aspectMask),
                                 "is %s (not VK_IMAGE_ASPECT_COLOR_BIT) and image was created with format %s.",
                                 string_VkImageAspectFlags(aspect_mask).c_str(),
        ...
        if ((vkuFormatIsMultiplane(image_format)) && (image_state->disjoint)) {
            if (!IsValidPlaneAspect(image_format, aspect_mask) && ((aspect_mask & VK_IMAGE_ASPECT_COLOR_BIT) == 0)) {
                const LogObjectList objlist(device, image_state->Handle());
                skip |= LogError("VUID-VkHostImageLayoutTransitionInfo-image-01672",
                                 objlist,
                                 transition_loc.dot(Field::subresourceRange).dot(Field::aspectMask),
                                 "is %s and image was created with format %s.",
                                 string_VkImageAspectFlags(aspect_mask).c_str(),
                                 string_VkFormat(image_format));
            }
        }
        if (vkuFormatIsDepthAndStencil(image_format)) {
            if (enabled_features.separateDepthStencilLayouts) {
                if (!has_depth_mask && !has_stencil_mask) {
                    const LogObjectList objlist(device, image_state->Handle());
                    skip |= LogError("VUID-VkHostImageLayoutTransitionInfo-image-03319",
                                     objlist,
                                     transition_loc.dot(Field::subresourceRange).dot(Field::aspectMask),
                                     "is %s and image was created with format %s.",
                                     string_VkImageAspectFlags(aspect_mask).c_str(),
                ...
                if (!has_depth_mask || !has_stencil_mask) {
                    const LogObjectList objlist(device, image_state->Handle());
                    skip |= LogError("VUID-VkHostImageLayoutTransitionInfo-image-03320",
                                     objlist,
                                     transition_loc.dot(Field::subresourceRange).dot(Field::aspectMask),
                                     "is %s and image was created with format %s.",
                                     string_VkImageAspectFlags(aspect_mask).c_str(),
        ...
        if (aspect_mask & VK_IMAGE_ASPECT_DEPTH_BIT) {
            if ((transition.oldLayout == VK_IMAGE_LAYOUT_STENCIL_ATTACHMENT_OPTIMAL) ||
                ...
                const LogObjectList objlist(device, image_state->Handle());
                skip |=
                    LogError("VUID-VkHostImageLayoutTransitionInfo-aspectMask-08702",
                             objlist,
                             transition_loc.dot(Field::subresourceRange).dot(Field::aspectMask),
        ...
        if (aspect_mask & VK_IMAGE_ASPECT_STENCIL_BIT) {
            if ((transition.oldLayout == VK_IMAGE_LAYOUT_DEPTH_ATTACHMENT_OPTIMAL) ||
                ...
                const LogObjectList objlist(device, image_state->Handle());
                skip |= LogError("VUID-VkHostImageLayoutTransitionInfo-aspectMask-08703",
                                 objlist,
                                 transition_loc.dot(Field::subresourceRange).dot(Field::aspectMask),
                                 "is %s meaning that neither oldLayout nor newLayout can be "
        ...
        if ((transition.oldLayout != VK_IMAGE_LAYOUT_UNDEFINED) &&
            (transition.oldLayout != VK_IMAGE_LAYOUT_PREINITIALIZED)) {
            skip |= ValidateHostCopyImageLayout(transition.image,
                                                phys_dev_props_core14.copySrcLayoutCount,
                                                phys_dev_props_core14.pCopySrcLayouts,
        ...
        skip |= ValidateHostCopyImageLayout(transition.image,
                                            phys_dev_props_core14.copyDstLayoutCount,
                                            phys_dev_props_core14.pCopyDstLayouts,
        ...
        if (transition.oldLayout != VK_IMAGE_LAYOUT_UNDEFINED) {
            skip |= ValidateHostCopyCurrentLayout(
                transition.oldLayout, transition.subresourceRange, *image_state, transition_loc.dot(Field::oldLayout));
        }
    }
    return skip;
}

bool CoreChecks::PreCallValidateTransitionImageLayoutEXT(VkDevice device,
                                                         uint32_t transitionCount,
...
void CoreChecks::PostCallRecordTransitionImageLayout(VkDevice device,
                                                     uint32_t transitionCount,
                                                     const VkHostImageLayoutTransitionInfo* pTransitions,
                                                     const RecordObject& record_obj) {
...
void CoreChecks::PostCallRecordTransitionImageLayoutEXT(VkDevice device,
                                                        uint32_t transitionCount,
                                                        const VkHostImageLayoutTransitionInfoEXT* pTransitions,
                                                        const RecordObject& record_obj) {
...
bool CoreChecks::ValidateProtectedImage(const vvl::CommandBuffer& cb_state,
    ...
    if ((!phys_dev_props_core11.protectedNoFault) && (cb_state.unprotected == true) &&
        (image_state.unprotected == false)) {
        const LogObjectList objlist(cb_state.Handle(), image_state.Handle());
        skip |= LogError(vuid,
                         objlist,
                         loc,
                         "(%s) is a protected image, but command buffer (%s) is unprotected.%s",
                         FormatHandle(image_state).c_str(),
...
bool CoreChecks::ValidateUnprotectedImage(const vvl::CommandBuffer& cb_state,
                                          const vvl::Image& image_state,
                                          const Location& loc,
                                          const char* vuid,
                                          const char* more_message) const {
    ...
    if ((!phys_dev_props_core11.protectedNoFault) && (cb_state.unprotected == false) &&
        (image_state.unprotected == true)) {
        const LogObjectList objlist(cb_state.Handle(), image_state.Handle());
        skip |= LogError(vuid,
                         objlist,
                         loc,
                         "(%s) is an unprotected image, but command buffer (%s) is protected.%s",
                         FormatHandle(image_state).c_str(),
                         FormatHandle(cb_state).c_str(),
                         more_message);
    }
...
bool CoreChecks::ValidateImageViewSampleWeightQCOM(const VkImageViewCreateInfo& create_info,
                                                   const vvl::Image& image_state,
                                                   const Location& create_info_loc) const {
    bool skip = false;
    ...
    if ((enabled_features.textureSampleWeighted == VK_FALSE)) {
        skip |= LogError("VUID-VkImageViewCreateInfo-pNext-06944",
                         create_info.image,
                         create_info_loc.dot(Field::pNext),
    ...
    if ((image_usage & VK_IMAGE_USAGE_SAMPLE_WEIGHT_BIT_QCOM) == 0) {
        skip |= LogError("VUID-VkImageViewCreateInfo-pNext-06945",
                         create_info.image,
                         create_info_loc.dot(Field::pNext),
                         "chain includes VkImageViewSampleWeightCreateInfoQCOM "
    ...
    if (!IsIdentitySwizzle(create_info.components)) {
        skip |= LogError("VUID-VkImageViewCreateInfo-pNext-06946",
                         create_info.image,
                         create_info_loc.dot(Field::pNext),
                         "chain includes VkImageViewSampleWeightCreateInfoQCOM "
                         "but not all members of pCreateInfo->components have identity swizzle. Here are the actual "
                         "swizzle values:\n%s",
                         string_VkComponentMapping(create_info.components).c_str());
    }

    if (aspect_mask != VK_IMAGE_ASPECT_COLOR_BIT) {
        skip |= LogError("VUID-VkImageViewCreateInfo-pNext-06947",
                         create_info.image,
                         create_info_loc.dot(Field::pNext),
                         "chain includes VkImageViewSampleWeightCreateInfoQCOM "
    ...
    if (normalized_subresource_range.levelCount != 1) {
        skip |= LogError("VUID-VkImageViewCreateInfo-pNext-06948",
                         create_info.image,
                         create_info_loc.dot(Field::pNext),
                         "chain includes VkImageViewSampleWeightCreateInfoQCOM "
    ...
    if ((create_info.viewType != VK_IMAGE_VIEW_TYPE_1D_ARRAY) &&
        (create_info.viewType != VK_IMAGE_VIEW_TYPE_2D_ARRAY)) {
        skip |= LogError("VUID-VkImageViewCreateInfo-pNext-06949",
                         create_info.image,
                         create_info_loc.dot(Field::pNext),
                         "chain includes VkImageViewSampleWeightCreateInfoQCOM "
    ...
    if ((create_info.viewType == VK_IMAGE_VIEW_TYPE_1D_ARRAY) && (image_type != VK_IMAGE_TYPE_1D)) {
        skip |=
            LogError("VUID-VkImageViewCreateInfo-pNext-06950",
                     create_info.image,
                     create_info_loc.dot(Field::pNext),
                     "chain includes VkImageViewSampleWeightCreateInfoQCOM "
    ...
    if ((create_info.viewType == VK_IMAGE_VIEW_TYPE_1D_ARRAY) && (normalized_subresource_range.layerCount != 2)) {
        skip |= LogError("VUID-VkImageViewCreateInfo-pNext-06951",
                         create_info.image,
                         create_info_loc.dot(Field::pNext),
                         "chain includes VkImageViewSampleWeightCreateInfoQCOM "
                         "and the view type is VK_IMAGE_VIEW_TYPE_1D_ARRAY so teh subresourceRange.layerCount must be "
                         "2, but it is %s.",
    ...
    if ((create_info.viewType == VK_IMAGE_VIEW_TYPE_1D_ARRAY) && (image_extent.width < min_image_width)) {
        skip |=
            LogError("VUID-VkImageViewCreateInfo-pNext-06952",
                     create_info.image,
                     create_info_loc.dot(Field::pNext),
                     "chain includes VkImageViewSampleWeightCreateInfoQCOM "
    ...
    if ((create_info.viewType == VK_IMAGE_VIEW_TYPE_2D_ARRAY) && (image_type != VK_IMAGE_TYPE_2D)) {
        skip |= LogError("VUID-VkImageViewCreateInfo-pNext-06953",
                         create_info.image,
                         create_info_loc.dot(Field::pNext),
                         "chain includes VkImageViewSampleWeightCreateInfoQCOM "
    ...
    if ((create_info.viewType == VK_IMAGE_VIEW_TYPE_2D_ARRAY) &&
        ...
        skip |= LogError("VUID-VkImageViewCreateInfo-pNext-06955",
                         create_info.image,
                         create_info_loc.dot(Field::pNext),
                         "chain includes VkImageViewSampleWeightCreateInfoQCOM "
    ...
    if ((create_info.viewType == VK_IMAGE_VIEW_TYPE_2D_ARRAY) &&
        (image_extent.width < sample_weight_info->filterSize.height)) {
        skip |= LogError("VUID-VkImageViewCreateInfo-pNext-06956",
                         create_info.image,
                         create_info_loc.dot(Field::pNext),
                         "chain includes VkImageViewSampleWeightCreateInfoQCOM "
    ...
    const Location filter_size_loc =
        create_info_loc.pNext(vvl::Struct::VkImageViewSampleWeightCreateInfoQCOM, Field::filterSize);
    if ((sample_weight_info->filterSize.height >
         phys_dev_ext_props.image_processing_props.maxWeightFilterDimension.height)) {
        skip |=
            LogError("VUID-VkImageViewCreateInfo-pNext-06957",
                     create_info.image,
                     filter_size_loc.dot(Field::height),
                     "(%" PRIu32 ") is not less than or equal to "
    ...
    if ((sample_weight_info->filterSize.width >
         phys_dev_ext_props.image_processing_props.maxWeightFilterDimension.width)) {
        skip |= LogError("VUID-VkImageViewSampleWeightCreateInfoQCOM-filterSize-06958",
                         create_info.image,
                         filter_size_loc.dot(Field::width),
                         "(%" PRIu32 ") is not less than or equal to "
    ...
    if ((sample_weight_info->filterSize.height >
         phys_dev_ext_props.image_processing_props.maxWeightFilterDimension.height)) {
        skip |=
            LogError("VUID-VkImageViewSampleWeightCreateInfoQCOM-filterSize-06959",
                     create_info.image,
                     filter_size_loc.dot(Field::height),
                     "(%" PRIu32 ") is not less than or equal to "
                     "VkPhysicalDeviceImageProcessingPropertiesQCOM::maxWeightFilterDimension.height (%" PRIu32 ").",
    ...
    if ((sample_weight_info->numPhases > phys_dev_ext_props.image_processing_props.maxWeightFilterPhases)) {
        skip |= LogError("VUID-VkImageViewSampleWeightCreateInfoQCOM-numPhases-06963",
                         create_info.image,
                         create_info_loc.pNext(vvl::Struct::VkImageViewSampleWeightCreateInfoQCOM, Field::numPhases),
                         "(%" PRIu32 ") is not less than or equal to "
                         "VkPhysicalDeviceImageProcessingPropertiesQCOM::maxWeightFilterPhases (%" PRIu32 ")",
                         sample_weight_info->numPhases,
                         phys_dev_ext_props.image_processing_props.maxWeightFilterPhases);
    }
    return skip;
}
