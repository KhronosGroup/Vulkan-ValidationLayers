...
bool CoreChecks::PreCallValidateCreateComputePipelines(VkDevice device,
    ...
    bool skip = BaseClass::PreCallValidateCreateComputePipelines(
    ...
    for (uint32_t i = 0; i < count; i++) {
        ...
        if (stage_state.pipeline_create_info) {
            skip |= ValidatePipelineShaderStage(
                *pipeline, *stage_state.pipeline_create_info, pCreateInfos[i].pNext, stage_info);
        }

        skip |= ValidateComputePipelineDerivatives(pipeline_states, i, create_info_loc);
        skip |=
            ValidatePipelineCacheControlFlags(pipeline->create_flags,
                                              create_info_loc.dot(Field::flags),
                                              "VUID-VkComputePipelineCreateInfo-pipelineCreationCacheControl-02875");
        skip |= ValidatePipelineIndirectBindableFlags(
            pipeline->create_flags, create_info_loc.dot(Field::flags), "VUID-VkComputePipelineCreateInfo-flags-09007");
        ...
        if (i == 0 && chassis_state.stateless_data.pipeline_pnext_module) {
            skip |= ValidateSpirvStateless(
                *chassis_state.stateless_data.pipeline_pnext_module,
...
bool CoreChecks::ValidateComputePipelineDerivatives(PipelineStates& pipeline_states,
                                                    uint32_t pipe_index,
    ...
    if (pipeline.create_flags & VK_PIPELINE_CREATE_2_DERIVATIVE_BIT) {
        ...
        if (base_index != -1 && base_index < static_cast<int32_t>(pipeline_states.size())) {
            if (static_cast<uint32_t>(base_index) >= pipe_index) {
                skip |= LogError("VUID-vkCreateComputePipelines-flags-00695",
                                 base_handle,
                                 loc,
        ...
        if (base_pipeline && !(base_pipeline->create_flags & VK_PIPELINE_CREATE_2_ALLOW_DERIVATIVES_BIT)) {
            skip |= LogError("VUID-vkCreateComputePipelines-flags-00696",
