...
struct ImageRegionIntersection {
    ...
    std::string String() const noexcept {
        ...
        ss << "\nsubresource : { aspectMask: " << string_VkImageAspectFlags(subresource.aspectMask)
           ...
           << "},\nextent : {" << string_VkExtent3D(extent) << "}\n";
        return ss.str();
    }
};
...
static ImageRegionIntersection
GetRegionIntersection(const RegionType& region0, const RegionType& region1, VkImageType type, bool is_multiplane) {
    ...
    auto intersection = GetRangeIntersection(region0.srcSubresource.baseArrayLayer,
                                             region0.srcSubresource.layerCount,
                                             region1.dstSubresource.baseArrayLayer,
                                             region1.dstSubresource.layerCount);
    if ((region0.srcSubresource.mipLevel == region1.dstSubresource.mipLevel) && intersection.non_empty()) {
        ...
        switch (type) {
            case VK_IMAGE_TYPE_3D:
                intersection = GetRangeIntersection(
                    region0.srcOffset.z, region0.extent.depth, region1.dstOffset.z, region1.extent.depth);
            ...
            case VK_IMAGE_TYPE_2D:
                intersection = GetRangeIntersection(
                    region0.srcOffset.y, region0.extent.height, region1.dstOffset.y, region1.extent.height);
                if (intersection.non_empty()) {
            ...
            case VK_IMAGE_TYPE_1D:
                intersection = GetRangeIntersection(
                    region0.srcOffset.x, region0.extent.width, region1.dstOffset.x, region1.extent.width);
                if (intersection.non_empty()) {
...
template <typename RegionType>
static bool
RegionIntersects(const RegionType* region0, const RegionType* region1, VkImageType type, bool is_multiplane) {
    return GetRegionIntersection(region0, region1, type, is_multiplane).has_instersection;
}

template <typename RegionType>
static bool
RegionIntersectsBlit(const RegionType* region0, const RegionType* region1, VkImageType type, bool is_multiplane) {
    bool result = false;
    ...
    if ((region0->srcSubresource.mipLevel == region1->dstSubresource.mipLevel) &&
        (RangesIntersect(region0->srcSubresource.baseArrayLayer,
                         region0->srcSubresource.layerCount,
                         region1->dstSubresource.baseArrayLayer,
                         region1->dstSubresource.layerCount))) {
        result = true;
        switch (type) {
            case VK_IMAGE_TYPE_3D:
                result &= RangesIntersect(region0->srcOffsets[0].z,
                                          region0->srcOffsets[1].z - region0->srcOffsets[0].z,
                                          region1->dstOffsets[0].z,
                                          region1->dstOffsets[1].z - region1->dstOffsets[0].z);
                [[fallthrough]];
            case VK_IMAGE_TYPE_2D:
                result &= RangesIntersect(region0->srcOffsets[0].y,
                                          region0->srcOffsets[1].y - region0->srcOffsets[0].y,
                                          region1->dstOffsets[0].y,
                                          region1->dstOffsets[1].y - region1->dstOffsets[0].y);
                [[fallthrough]];
            case VK_IMAGE_TYPE_1D:
                result &= RangesIntersect(region0->srcOffsets[0].x,
                                          region0->srcOffsets[1].x - region0->srcOffsets[0].x,
                                          region1->dstOffsets[0].x,
                                          region1->dstOffsets[1].x - region1->dstOffsets[0].x);
                break;
            default:
                // Unrecognized or new IMAGE_TYPE enums will be caught in parameter_validation
                assert(false);
...
VkExtent3D CoreChecks::GetScaledItg(const vvl::CommandBuffer& cb_state, const vvl::Image& image_state) const {
    ...
    if (cb_state.command_pool) {
        granularity = physical_device_state->queue_family_properties[cb_state.command_pool->queueFamilyIndex]
...
bool CoreChecks::ValidateTransferGranularityOffset(const LogObjectList& objlist,
    ...
    if (IsExtentAllZeroes(granularity)) {
        ...
        // If the queue family image transfer granularity is not (0, 0, 0), then the offset dimensions must always be
        // even integer multiples of the image transfer granularity.
        skip |= LogError(vuid,
                         objlist,
                         offset_loc,
                         ...
                         string_VkOffset3D(offset).c_str(),
                         string_VkExtent3D(granularity).c_str());
    }
    return skip;
}
...
static inline bool IsExtentEqual(const VkExtent3D& extent, const VkExtent3D& other_extent) {
    return (extent.width == other_extent.width) && (extent.height == other_extent.height) &&
...
bool CoreChecks::ValidateTransferGranularityExtent(const LogObjectList& objlist,
    ...
    if (IsExtentAllZeroes(granularity)) {
        ...
        if (IsExtentEqual(extent, subresource_extent) == false) {
            skip |= LogError(vuid,
                             ...
                             string_VkExtent3D(extent).c_str(),
                             string_VkExtent3D(subresource_extent).c_str());
        }
    } else {
        // If the queue family image transfer granularity is not (0, 0, 0), then the extent dimensions must always be
        // even integer multiples of the image transfer granularity or the offset + extent dimensions must always match
        // the image subresource extent dimensions.
...
bool CoreChecks::ValidateImageMipLevel(const HandleT handle,
    ...
    if (mip_level >= image_state.create_info.mipLevels) {
        ...
        skip |= LogError(vvl::GetImageMipLevelVUID(subresource_loc),
                         objlist,
                         subresource_loc.dot(Field::mipLevel),
                         "is %" PRIu32 ", but provided %s has %" PRIu32 " mip levels.",
                         mip_level,
...
template <typename HandleT>
bool CoreChecks::ValidateImageArrayLayerRange(const HandleT handle,
                                              const vvl::Image& image_state,
    ...
    if (base_layer >= image_state.create_info.arrayLayers || layer_count > image_state.create_info.arrayLayers ||
        ...
        if (layer_count != VK_REMAINING_ARRAY_LAYERS) {
            const LogObjectList objlist(handle, image_state.Handle());
            skip |=
                LogError(vvl::GetImageArrayLayerRangeVUID(subresource_loc),
                         objlist,
...
bool VerifyAspectsPresent(VkImageAspectFlags aspect_mask, VkFormat format) {
    if ((aspect_mask & VK_IMAGE_ASPECT_COLOR_BIT) != 0) {
        if (!(vkuFormatIsColor(format) || vkuFormatIsMultiplane(format)))
            return false;
    }
    ...
    if (0 !=
        (aspect_mask & (VK_IMAGE_ASPECT_PLANE_0_BIT | VK_IMAGE_ASPECT_PLANE_1_BIT | VK_IMAGE_ASPECT_PLANE_2_BIT))) {
...
bool CoreChecks::ValidateHeterogeneousCopyData(const HandleT handle,
                                               const RegionType& region,
                                               const vvl::Image& image_state,
                                               const Location& region_loc) const {
    bool skip = false;
    const bool is_memory = IsValueIn(
        region_loc.function, { Func::vkCopyMemoryToImageEXT, Func::vkCopyImageToMemory, Func::vkCopyImageToMemoryEXT });
    ...
    if (image_state.create_info.imageType == VK_IMAGE_TYPE_1D) {
        if ((region.imageOffset.y != 0) || (region.imageExtent.height != 1)) {
            const LogObjectList objlist(handle, image_state.Handle());
            skip |= LogError(GetCopyBufferImageVUID(region_loc, vvl::CopyError::Image1D_07979),
                             objlist,
                             region_loc.dot(Field::imageOffset).dot(Field::y),
                             "is %" PRId32 " and imageExtent.height is %" PRIu32 ". For 1D images these must be 0 "
                             "and 1, respectively.",
    ...
    if ((image_state.create_info.imageType == VK_IMAGE_TYPE_1D) ||
        (image_state.create_info.imageType == VK_IMAGE_TYPE_2D)) {
        if ((region.imageOffset.z != 0) || (region.imageExtent.depth != 1)) {
            const LogObjectList objlist(handle, image_state.Handle());
            skip |= LogError(GetCopyBufferImageVUID(region_loc, vvl::CopyError::Image1D_07980),
                             objlist,
                             region_loc.dot(Field::imageOffset).dot(Field::z),
                             "is %" PRId32 " and imageExtent.depth is %" PRIu32 ". For 1D and 2D images these "
    ...
    if (image_state.create_info.imageType == VK_IMAGE_TYPE_3D) {
        if ((0 != region.imageSubresource.baseArrayLayer) || (1 != region.imageSubresource.layerCount)) {
            const LogObjectList objlist(handle, image_state.Handle());
            skip |= LogError(GetCopyBufferImageVUID(region_loc, vvl::CopyError::Image3D_07983),
                             objlist,
                             subresource_loc.dot(Field::baseArrayLayer),
    ...
    if (region.imageExtent.width == 0) {
        const LogObjectList objlist(handle, image_state.Handle());
        skip |= LogError(GetCopyBufferImageDeviceVUID(region_loc, vvl::CopyError::ImageExtentWidthZero_06659),
                         objlist,
    ...
    if (region.imageExtent.height == 0) {
        const LogObjectList objlist(handle, image_state.Handle());
        skip |= LogError(GetCopyBufferImageDeviceVUID(region_loc, vvl::CopyError::ImageExtentHeightZero_06660),
                         objlist,
    ...
    if (region.imageExtent.depth == 0) {
        const LogObjectList objlist(handle, image_state.Handle());
        skip |= LogError(GetCopyBufferImageDeviceVUID(region_loc, vvl::CopyError::ImageExtentDepthZero_06661),
                         objlist,
    ...
    if ((row_length != 0) && (row_length < region.imageExtent.width)) {
        ...
        skip |= LogError(GetCopyBufferImageDeviceVUID(region_loc, vvl::CopyError::ImageExtentRowLength_09101),
                         objlist,
                         region_loc.dot(field),
                         "(%" PRIu32 ") must be zero or greater-than-or-equal-to imageExtent.width (%" PRIu32 ").",
                         row_length,
    ...
    if ((image_height != 0) && (image_height < region.imageExtent.height)) {
        ...
        skip |= LogError(GetCopyBufferImageDeviceVUID(region_loc, vvl::CopyError::ImageExtentImageHeight_09102),
                         objlist,
                         region_loc.dot(field),
                         "(%" PRIu32 ") must be zero or greater-than-or-equal-to imageExtent.height (%" PRIu32 ").",
                         image_height,
                         region.imageExtent.height);
    }
    ...
    if (GetBitSetCount(region_aspect_mask) != 1) {
        const LogObjectList objlist(handle, image_state.Handle());
        skip |= LogError(GetCopyBufferImageDeviceVUID(region_loc, vvl::CopyError::AspectMaskSingleBit_09103),
                         objlist,
                         subresource_loc.dot(Field::aspectMask),
                         "is %s (only one bit allowed).",
    ...
    if ((region.imageOffset.x < 0) || (region.imageOffset.x > static_cast<int32_t>(adjusted_image_extent.width)) ||
        ...
        const LogObjectList objlist(handle, image_state.Handle());
        skip |= LogError(GetCopyBufferImageVUID(region_loc, vvl::CopyError::ImageOffest_07971),
                         objlist,
                         region_loc.dot(Field::imageOffset).dot(Field::x),
                         "(%" PRId32 ") and (imageExtent.width + imageOffset.x) (%" PRIu32 ") must be >= "
    ...
    if ((region.imageOffset.y < 0) || (region.imageOffset.y > static_cast<int32_t>(adjusted_image_extent.height)) ||
        ...
        const LogObjectList objlist(handle, image_state.Handle());
        skip |= LogError(GetCopyBufferImageVUID(region_loc, vvl::CopyError::ImageOffest_07972),
                         objlist,
                         region_loc.dot(Field::imageOffset).dot(Field::y),
                         "(%" PRId32 ") and (imageExtent.height + imageOffset.y) (%" PRIu32 ") must be >= "
    ...
    if ((region.imageOffset.z < 0) || (region.imageOffset.z > static_cast<int32_t>(adjusted_image_extent.depth)) ||
        ...
        const LogObjectList objlist(handle, image_state.Handle());
        skip |= LogError(GetCopyBufferImageVUID(region_loc, vvl::CopyError::ImageOffest_09104),
                         objlist,
                         region_loc.dot(Field::imageOffset).dot(Field::z),
                         "(%" PRId32 ") and (imageExtent.depth + imageOffset.z) (%" PRIu32 ") must be >= "
    ...
    if (!VerifyAspectsPresent(region_aspect_mask, image_format)) {
        const LogObjectList objlist(handle, image_state.Handle());
        skip |= LogError(GetCopyBufferImageVUID(region_loc, vvl::CopyError::AspectMask_09105),
                         objlist,
                         subresource_loc.dot(Field::aspectMask),
                         "%s invalid for image format %s.",
                         string_VkImageAspectFlags(region_aspect_mask).c_str(),
    ...
    if (SafeModulo(row_length, block_extent.width) != 0) {
        const LogObjectList objlist(handle, image_state.Handle());
        Field field = is_memory ? Field::memoryRowLength : Field::bufferRowLength;
        skip |= LogError(GetCopyBufferImageVUID(region_loc, vvl::CopyError::bufferRowLength_09106),
                         objlist,
                         region_loc.dot(field),
                         "(%" PRIu32 ") must be a multiple of the image (%s) texel block extent width (%" PRIu32 ").",
                         row_length,
                         string_VkFormat(image_format),
                         block_extent.width);
    }
    ...
    if (SafeModulo(image_height, block_extent.height) != 0) {
        const LogObjectList objlist(handle, image_state.Handle());
        Field field = is_memory ? Field::memoryImageHeight : Field::bufferImageHeight;
        skip |= LogError(GetCopyBufferImageVUID(region_loc, vvl::CopyError::bufferImageHeight_09107),
                         objlist,
                         region_loc.dot(field),
                         "(%" PRIu32 ") must be a multiple of the image (%s) texel block extent height (%" PRIu32 ").",
                         image_height,
                         string_VkFormat(image_format),
                         block_extent.height);
    }
    ...
    if (SafeModulo(region.imageOffset.x, block_extent.width) != 0) {
        const LogObjectList objlist(handle, image_state.Handle());
        skip |= LogError(GetCopyBufferImageVUID(region_loc, vvl::CopyError::TexelBlockExtentWidth_07274),
                         objlist,
                         region_loc.dot(Field::imageOffset).dot(Field::x),
                         "(%" PRId32 ") must be a multiple of the image (%s) texel block extent "
                         "width (%" PRIu32 ").",
                         region.imageOffset.x,
                         string_VkFormat(image_format),
                         block_extent.width);
    ...
    if (SafeModulo(region.imageOffset.y, block_extent.height) != 0) {
        const LogObjectList objlist(handle, image_state.Handle());
        skip |= LogError(GetCopyBufferImageVUID(region_loc, vvl::CopyError::TexelBlockExtentHeight_07275),
                         objlist,
                         region_loc.dot(Field::imageOffset).dot(Field::y),
    ...
    if (SafeModulo(region.imageOffset.z, block_extent.depth) != 0) {
        const LogObjectList objlist(handle, image_state.Handle());
        skip |= LogError(GetCopyBufferImageVUID(region_loc, vvl::CopyError::TexelBlockExtentDepth_07276),
                         objlist,
                         region_loc.dot(Field::imageOffset).dot(Field::z),
    ...
    if ((SafeModulo(region.imageExtent.width, block_extent.width) != 0) &&
        ...
        const LogObjectList objlist(handle, image_state.Handle());
        skip |= LogError(GetCopyBufferImageVUID(region_loc, vvl::CopyError::TexelBlockExtentWidth_00207),
                         objlist,
                         region_loc.dot(Field::imageExtent).dot(Field::width),
                         "(%" PRIu32 ") must be a multiple of the image (%s) texel block width "
    ...
    if ((SafeModulo(region.imageExtent.height, block_extent.height) != 0) &&
        ...
        const LogObjectList objlist(handle, image_state.Handle());
        skip |= LogError(GetCopyBufferImageVUID(region_loc, vvl::CopyError::TexelBlockExtentHeight_00208),
                         objlist,
                         region_loc.dot(Field::imageExtent).dot(Field::height),
                         "(%" PRIu32 ") must be a multiple of the image (%s) texel block height "
    ...
    if ((SafeModulo(region.imageExtent.depth, block_extent.depth) != 0) &&
        ...
        const LogObjectList objlist(handle, image_state.Handle());
        skip |= LogError(GetCopyBufferImageVUID(region_loc, vvl::CopyError::TexelBlockExtentDepth_00209),
                         objlist,
                         region_loc.dot(Field::imageExtent).dot(Field::depth),
                         "(%" PRIu32 ") must be a multiple of the image (%s) texel block depth "
    ...
    // *RowLength divided by the texel block extent width and then multiplied by the texel block size of the image must
    ...
    if (test_value > two_to_31_minus_1) {
        ...
        Field field = is_memory ? Field::memoryRowLength : Field::bufferRowLength;
        skip |=
            LogError(GetCopyBufferImageVUID(region_loc, vvl::CopyError::bufferRowLength_09108),
                     objlist,
                     region_loc.dot(field),
                     "(%" PRIu32 ") divided by the  image (%s) texel block width (%" PRIu32 ") then multiplied by the "
    ...
    if (vkuFormatIsMultiplane(image_format) && !IsOnlyOneValidPlaneAspect(image_format, region_aspect_mask)) {
        const LogObjectList objlist(handle, image_state.Handle());
        skip |= LogError(GetCopyBufferImageVUID(region_loc, vvl::CopyError::MultiPlaneAspectMask_07981),
                         objlist,
                         subresource_loc.dot(Field::aspectMask),
                         "(%s) is invalid for multi-planar format %s.",
                         string_VkImageAspectFlags(region_aspect_mask).c_str(),
                         string_VkFormat(image_format));
    }
...
template <typename RegionType>
bool CoreChecks::ValidateBufferImageCopyData(const vvl::CommandBuffer& cb_state,
                                             const RegionType& region,
                                             const vvl::Image& image_state,
                                             const Location& region_loc) const {
    bool skip = false;
    ...
    if (vkuFormatIsDepthOrStencil(image_format)) {
        if (SafeModulo(bufferOffset, 4) != 0) {
            ...
            skip |= LogError(GetCopyBufferImageDeviceVUID(region_loc, vvl::CopyError::BufferOffset_07978),
                             objlist,
                             region_loc.dot(Field::bufferOffset),
                             "(%" PRIu64 ") must be a multiple 4 if using a depth/stencil format (%s).",
                             bufferOffset,
                             string_VkFormat(image_format));
        }
    } else {
        ...
        if (!vkuFormatIsMultiplane(image_format) && (SafeModulo(bufferOffset, element_size) != 0)) {
            const LogObjectList objlist(cb_state.Handle(), image_state.Handle());
            skip |= LogError(GetCopyBufferImageDeviceVUID(region_loc, vvl::CopyError::TexelBlockSize_07975),
                             objlist,
                             region_loc.dot(Field::bufferOffset),
                             "(%" PRIu64 ") must be a multiple of %s texel size (%" PRIu32 ").",
                             bufferOffset,
                             string_VkFormat(image_format),
                             element_size);
        }
    ...
    if (vkuFormatIsMultiplane(image_format) && IsAnyPlaneAspect(region_aspect_mask)) {
        ...
        if (SafeModulo(bufferOffset, compatible_size) != 0) {
            const LogObjectList objlist(cb_state.Handle(), image_state.Handle());
            skip |=
                LogError(GetCopyBufferImageDeviceVUID(region_loc, vvl::CopyError::MultiPlaneCompatible_07976),
                         objlist,
                         region_loc.dot(Field::bufferOffset),
    ...
    if (SafeModulo(bufferOffset, 4) != 0) {
        ...
        if (!HasRequiredQueueFlags(cb_state, *physical_device_state, required_flags)) {
            ...
            skip |= LogError(vuid,
                             objlist,
                             region_loc.dot(Field::bufferOffset),
                             "(%" PRIu64 ") is not a multiple of 4, but is %s",
                             bufferOffset,
...
template <typename RegionType>
bool CoreChecks::ValidateCmdCopyBufferBounds(VkCommandBuffer cb,
    ...
    for (uint32_t i = 0; i < regionCount; i++) {
        ...
        if (region.srcOffset >= src_buffer_size) {
            ...
            skip |= LogError(vuid,
                             src_objlist,
                             region_loc.dot(Field::srcOffset),
                             "(%" PRIuLEAST64 ") is greater than size of srcBuffer (%" PRIuLEAST64 ").",
                             region.srcOffset,
                             src_buffer_size);
        }
        ...
        if (region.dstOffset >= dst_buffer_size) {
            ...
            skip |= LogError(vuid,
                             dst_objlist,
                             region_loc.dot(Field::dstOffset),
                             "(%" PRIuLEAST64 ") is greater than size of dstBuffer (%" PRIuLEAST64 ").",
                             region.dstOffset,
                             dst_buffer_size);
        }

        // The size member of each element of pRegions must be less than or equal to the size of srcBuffer minus
        // srcOffset
        ...
        // The size member of each element of pRegions must be less than or equal to the size of dstBuffer minus
        // dstOffset
        ...
        if (validate_no_memory_overlaps) {
            // Sort copy ranges
            {
                ...
                auto insert_pos =
                    std::lower_bound(src_memory_ranges.begin(), src_memory_ranges.end(), src_buffer_memory_range);
...

            {
                ...
                auto insert_pos =
                    std::lower_bound(dst_memory_ranges.begin(), dst_memory_ranges.end(), dst_buffer_memory_range);
        ...
        while (src_ranges_it != src_memory_ranges.cend() && dst_ranges_it != dst_memory_ranges.cend()) {
            if (src_ranges_it->intersects(*dst_ranges_it)) {
                ...
                const LogObjectList objlist(
                    cb, src_binding->memory_state->Handle(), src_buffer_state.Handle(), dst_buffer_state.Handle());
                const char* vuid =
                    is_2 ? "VUID-VkCopyBufferInfo2-pRegions-00117" : "VUID-vkCmdCopyBuffer-pRegions-00117";
                skip |= LogError(vuid,
                                 objlist,
                                 loc,
                                 "Copy source buffer range %s (from buffer %s) and destination buffer range %s (from "
                                 "buffer %s) are bound to "
                                 "the same memory (%s), "
                                 ...
                                 FormatHandle(dst_buffer_state.VkHandle()).c_str(),
...
template <typename RegionType>
bool CoreChecks::ValidateCmdCopyBuffer(VkCommandBuffer commandBuffer,
                                       VkBuffer srcBuffer,
                                       VkBuffer dstBuffer,
                                       uint32_t regionCount,
                                       const RegionType* pRegions,
    ...
    skip |=
        ValidateCmdCopyBufferBounds(commandBuffer, *src_buffer_state, *dst_buffer_state, regionCount, pRegions, loc);
    ...
    // src buffer
    {
        ...
        skip |= ValidateBufferUsageFlags(LogObjectList(commandBuffer, srcBuffer),
                                         *src_buffer_state,
    ...
    // dst buffer
    {
        ...
        skip |= ValidateBufferUsageFlags(LogObjectList(commandBuffer, dstBuffer),
                                         *dst_buffer_state,
...
bool CoreChecks::PreCallValidateCmdCopyBuffer(VkCommandBuffer commandBuffer,
                                              VkBuffer srcBuffer,
                                              VkBuffer dstBuffer,
                                              uint32_t regionCount,
...
bool CoreChecks::PreCallValidateCmdCopyBuffer2(VkCommandBuffer commandBuffer,
                                               const VkCopyBufferInfo2* pCopyBufferInfo,
                                               const ErrorObject& error_obj) const {
    return ValidateCmdCopyBuffer(commandBuffer,
...
// Check valid usage Image Transfer Granularity requirements for elements of a VkBufferImageCopy/VkBufferImageCopy2
// structure
template <typename RegionType>
bool CoreChecks::ValidateCopyBufferImageTransferGranularityRequirements(const vvl::CommandBuffer& cb_state,
    ...
    skip |= ValidateTransferGranularityOffset(
        objlist, region.imageOffset, granularity, region_loc.dot(Field::imageOffset), vuid.c_str());
    VkExtent3D subresource_extent = image_state.GetEffectiveSubresourceExtent(region.imageSubresource);
    skip |= ValidateTransferGranularityExtent(objlist,
                                              region.imageExtent,
                                              region.imageOffset,
                                              granularity,
                                              subresource_extent,
                                              image_state.create_info.imageType,
                                              region_loc.dot(Field::imageExtent),
                                              vuid.c_str());
    return skip;
}

template <typename HandleT>
bool CoreChecks::ValidateImageSubresourceLayers(HandleT handle,
                                                const VkImageSubresourceLayers& subresource_layers,
                                                const Location& subresource_loc) const {
    bool skip = false;
    if (subresource_layers.layerCount == VK_REMAINING_ARRAY_LAYERS) {
        if (!enabled_features.maintenance5) {
            skip |= LogError("VUID-VkImageSubresourceLayers-layerCount-09243",
                             handle,
                             subresource_loc.dot(Field::layerCount),
                             "is VK_REMAINING_ARRAY_LAYERS.");
        }
    } else if (subresource_layers.layerCount == 0) {
        skip |= LogError("VUID-VkImageSubresourceLayers-layerCount-01700",
                         handle,
                         subresource_loc.dot(Field::layerCount),
                         "is zero.");
    }
    ...
    if (aspect_mask & VK_IMAGE_ASPECT_METADATA_BIT) {
        skip |= LogError("VUID-VkImageSubresourceLayers-aspectMask-00168",
                         handle,
                         subresource_loc.dot(Field::aspectMask),
                         "is %s.",
    ...
    if ((aspect_mask & VK_IMAGE_ASPECT_COLOR_BIT) &&
        ...
        skip |= LogError("VUID-VkImageSubresourceLayers-aspectMask-00167",
                         handle,
                         subresource_loc.dot(Field::aspectMask),
                         "is %s.",
                         string_VkImageAspectFlags(aspect_mask).c_str());
    ...
    if (aspect_mask & (VK_IMAGE_ASPECT_MEMORY_PLANE_0_BIT_EXT | VK_IMAGE_ASPECT_MEMORY_PLANE_1_BIT_EXT |
        ...
        skip |= LogError("VUID-VkImageSubresourceLayers-aspectMask-02247",
                         handle,
                         subresource_loc.dot(Field::aspectMask),
                         "is %s.",
                         string_VkImageAspectFlags(aspect_mask).c_str());
...
bool CoreChecks::ValidateCopyImageTransferGranularityRequirements(const vvl::CommandBuffer& cb_state,
    ...
    const VkExtent3D extent = region.extent;
    {
        ...
        skip |= ValidateTransferGranularityOffset(
            objlist, region.srcOffset, granularity, region_loc.dot(Field::srcOffset), vuid);
        ...
        skip |= ValidateTransferGranularityExtent(objlist,
                                                  ...
                                                  granularity,
                                                  subresource_extent,
                                                  src_image_state.create_info.imageType,
                                                  region_loc.dot(Field::extent),
                                                  vuid);
    }

    {
        ...
        skip |= ValidateTransferGranularityOffset(
            objlist, region.dstOffset, granularity, region_loc.dot(Field::dstOffset), vuid);
        ...
        skip |= ValidateTransferGranularityExtent(objlist,
                                                  dst_effective_extent,
                                                  region.dstOffset,
                                                  granularity,
                                                  subresource_extent,
                                                  dst_image_state.create_info.imageType,
                                                  region_loc.dot(Field::extent),
                                                  vuid);
    }
    return skip;
...
template <typename HandleT, typename RegionType>
bool CoreChecks::ValidateImageCopyData(const HandleT handle,
                                       const RegionType& region,
                                       const vvl::Image& src_image_state,
                                       const vvl::Image& dst_image_state,
                                       bool is_host,
    ...
    const VkExtent3D dst_copy_extent = GetAdjustedDstImageExtent(
        src_image_state.create_info.format, dst_image_state.create_info.format, region.extent);
    ...
    if (src_image_state.create_info.imageType == VK_IMAGE_TYPE_1D) {
        if ((0 != region.srcOffset.y) || (1 != src_copy_extent.height)) {
            ...
            skip |= LogError(GetCopyImageVUID(region_loc, vvl::CopyError::SrcImage1D_00146),
                             objlist,
                             region_loc.dot(Field::srcOffset).dot(Field::y),
                             "is %" PRId32 " and extent.height is %" PRIu32 ". For 1D images these must "
                             "be 0 and 1, respectively.",
                             region.srcOffset.y,
                             src_copy_extent.height);
        }
    }
    ...
    if (((src_image_state.create_info.imageType == VK_IMAGE_TYPE_1D) ||
        ...
        skip |= LogError(GetCopyImageVUID(region_loc, vvl::CopyError::SrcImage1D_01785),
                         objlist,
                         region_loc.dot(Field::srcOffset).dot(Field::z),
                         "is %" PRId32 " and extent.depth is %" PRIu32 ". For %s images "
    ...
    if ((src_image_state.create_info.imageType == VK_IMAGE_TYPE_2D) && (0 != region.srcOffset.z) && (!is_host)) {
        ...
        skip |= LogError(vuid,
                         objlist,
                         region_loc.dot(Field::srcOffset).dot(Field::z),
                         "is %" PRId32 ". For 2D images the z-offset must be 0.",
                         region.srcOffset.z);
    }
    ...
    { // Used to be compressed checks, now apply to all
        ...
        if (SafeModulo(region.srcOffset.x, block_extent.width) != 0) {
            const LogObjectList objlist(handle, src_image_state.Handle());
            skip |= LogError(GetCopyImageVUID(region_loc, vvl::CopyError::SrcOffset_07278),
                             objlist,
                             region_loc.dot(Field::srcOffset).dot(Field::x),
                             "(%" PRId32 ") must be a multiple of the image (%s) texel block "
                             "width (%" PRIu32 ").",
                             region.srcOffset.x,
                             string_VkFormat(src_image_format),
                             block_extent.width);
        }
        ...
        if (SafeModulo(region.srcOffset.y, block_extent.height) != 0) {
            const LogObjectList objlist(handle, src_image_state.Handle());
            skip |= LogError(GetCopyImageVUID(region_loc, vvl::CopyError::SrcOffset_07279),
                             objlist,
                             region_loc.dot(Field::srcOffset).dot(Field::y),
                             "(%" PRId32 ") must be a multiple of the image (%s) texel block "
                             "height (%" PRIu32 ").",
        ...
        if (SafeModulo(region.srcOffset.z, block_extent.depth) != 0) {
            const LogObjectList objlist(handle, src_image_state.Handle());
            skip |= LogError(GetCopyImageVUID(region_loc, vvl::CopyError::SrcOffset_07280),
                             objlist,
                             region_loc.dot(Field::srcOffset).dot(Field::z),
                             "(%" PRId32 ") must be a multiple of the image (%s) texel block "
                             "depth (%" PRIu32 ").",
        ...
        if ((SafeModulo(src_copy_extent.width, block_extent.width) != 0) &&
            ...
            const LogObjectList objlist(handle, src_image_state.Handle());
            skip |= LogError(GetCopyImageVUID(region_loc, vvl::CopyError::SrcOffset_01728),
                             objlist,
                             region_loc.dot(Field::extent).dot(Field::width),
                             "(%" PRIu32 ") must be a multiple of the image (%s) texel block "
                             "width (%" PRIu32 "), or when added to srcOffset.x (%" PRId32
                             ") must equal the image subresource width (%" PRIu32 ").",
                             src_copy_extent.width,
                             string_VkFormat(src_image_format),
                             block_extent.width,
                             region.srcOffset.x,
                             mip_extent.width);
        ...
        if ((SafeModulo(src_copy_extent.height, block_extent.height) != 0) &&
            ...
            const LogObjectList objlist(handle, src_image_state.Handle());
            skip |= LogError(GetCopyImageVUID(region_loc, vvl::CopyError::SrcOffset_01729),
                             objlist,
        ...
        if ((SafeModulo(copy_depth, block_extent.depth) != 0) &&
            (copy_depth + region.srcOffset.z != mip_extent.depth)) {
            const LogObjectList objlist(handle, src_image_state.Handle());
            skip |= LogError(GetCopyImageVUID(region_loc, vvl::CopyError::SrcOffset_01730),
                             objlist,
    ...
    if (dst_image_state.create_info.imageType == VK_IMAGE_TYPE_1D) {
        if ((0 != region.dstOffset.y) || (1 != dst_copy_extent.height)) {
            ...
            skip |= LogError(GetCopyImageVUID(region_loc, vvl::CopyError::DstImage1D_00152),
                             objlist,
                             region_loc.dot(Field::dstOffset).dot(Field::y),
                             "is %" PRId32 " and extent.height is %" PRIu32 ". For 1D images "
                             "these must be 0 and 1, respectively.",
                             region.dstOffset.y,
                             dst_copy_extent.height);
        }
    }
    ...
    if (((dst_image_state.create_info.imageType == VK_IMAGE_TYPE_1D) ||
        ...
        const LogObjectList objlist(handle, dst_image_state.Handle());
        const char* image_type = is_host ? "1D or 2D" : "1D";
        skip |= LogError(GetCopyImageVUID(region_loc, vvl::CopyError::DstImage1D_01786),
                         objlist,
                         region_loc.dot(Field::dstOffset).dot(Field::z),
                         "is %" PRId32 " and extent.depth is %" PRIu32 ". For %s images these must be 0 "
                         "and 1, respectively.",
                         region.dstOffset.z,
                         dst_copy_extent.depth,
                         image_type);
    }
    ...
    if (IsExtEnabled(device_extensions.vk_khr_maintenance1) || is_host) {
        if (src_image_state.create_info.imageType == VK_IMAGE_TYPE_3D) {
            ...
            if ((0 != region.srcSubresource.baseArrayLayer) || (1 != region.srcSubresource.layerCount)) {
                skip |= LogError(GetCopyImageVUID(region_loc, vvl::CopyError::SrcImage3D_04443),
                                 objlist,
        ...
        if (dst_image_state.create_info.imageType == VK_IMAGE_TYPE_3D) {
            ...
            if ((0 != region.dstSubresource.baseArrayLayer) || (1 != region.dstSubresource.layerCount)) {
                skip |= LogError(GetCopyImageVUID(region_loc, vvl::CopyError::DstImage3D_04444),
                                 objlist,
...

    {
        ...
        if (SafeModulo(region.dstOffset.y, block_extent.height) != 0) {
            const LogObjectList objlist(handle, src_image_state.Handle());
            skip |= LogError(GetCopyImageVUID(region_loc, vvl::CopyError::DstOffset_07282),
                             objlist,
                             region_loc.dot(Field::dstOffset).dot(Field::y),
                             "(%" PRId32 ") must be a multiple of the image (%s) texel block height (%" PRIu32 ").",
                             region.dstOffset.y,
        ...
        if (SafeModulo(region.dstOffset.z, block_extent.depth) != 0) {
            const LogObjectList objlist(handle, src_image_state.Handle());
            skip |= LogError(GetCopyImageVUID(region_loc, vvl::CopyError::DstOffset_07283),
                             objlist,
                             region_loc.dot(Field::dstOffset).dot(Field::z),
                             "(%" PRId32 ") must be a multiple of the image (%s) texel block depth (%" PRIu32 ").",
                             region.dstOffset.z,
        ...
        if ((SafeModulo(dst_copy_extent.width, block_extent.width) != 0) &&
            ...
            const LogObjectList objlist(handle, dst_image_state.Handle());
            skip |= LogError(GetCopyImageVUID(region_loc, vvl::CopyError::DstOffset_01732),
                             objlist,
                             region_loc.dot(Field::extent).dot(Field::width),
                             "(%" PRIu32 ") must be a multiple of the image (%s) texel block width (%" PRIu32
                             "), or when added to dstOffset.x (%" PRId32
                             ") must equal the image subresource width (%" PRIu32 ").",
                             dst_copy_extent.width,
                             string_VkFormat(dst_image_format),
                             block_extent.width,
                             region.dstOffset.x,
                             mip_extent.width);
        ...
        // Extent height must be a multiple of block height, or dst_copy_extent+offset height must equal subresource
        // height
        if ((SafeModulo(dst_copy_extent.height, block_extent.height) != 0) &&
            (dst_copy_extent.height + region.dstOffset.y != mip_extent.height)) {
            const LogObjectList objlist(handle, dst_image_state.Handle());
            skip |= LogError(GetCopyImageVUID(region_loc, vvl::CopyError::DstOffset_01733),
                             objlist,
                             region_loc.dot(Field::extent).dot(Field::height),
                             "(%" PRIu32 ") must be a multiple of the image (%s) texel block height (%" PRIu32
                             "), or when added to dstOffset.y (%" PRId32 ") must equal the image subresource "
                             "height (%" PRIu32 ").",
                             dst_copy_extent.height,
                             string_VkFormat(dst_image_format),
                             block_extent.height,
        ...
        if ((SafeModulo(copy_depth, block_extent.depth) != 0) &&
            (copy_depth + region.dstOffset.z != mip_extent.depth)) {
            const LogObjectList objlist(handle, dst_image_state.Handle());
            skip |= LogError(GetCopyImageVUID(region_loc, vvl::CopyError::DstOffset_01734),
                             objlist,
                             region_loc.dot(Field::extent).dot(Field::depth),
                             "(%" PRIu32 ") must be a multiple of the image (%s) texel block depth (%" PRIu32
                             "), or when added to dstOffset.z (%" PRId32
                             ") must equal the image subresource depth (%" PRIu32 ").",
                             dst_copy_extent.depth,
                             string_VkFormat(dst_image_format),
                             block_extent.depth,
...
template <typename HandleT, typename RegionType>
bool CoreChecks::ValidateCopyImageRegionCommon(HandleT handle,
                                               const vvl::Image& src_image_state,
                                               const vvl::Image& dst_image_state,
                                               const RegionType& region,
                                               const Location& region_loc) const {
    ...
    // src
    {
        ...
        skip |= ValidateImageArrayLayerRange(handle,
                                             src_image_state,
                                             region.srcSubresource.baseArrayLayer,
                                             region.srcSubresource.layerCount,
                                             src_subresource_loc);
        ...
        if (!VerifyAspectsPresent(region.srcSubresource.aspectMask, src_format)) {
            ...
            skip |= LogError(GetCopyImageVUID(region_loc, vvl::CopyError::SrcSubresource_00142),
                             src_objlist,
    ...
    // dst
    {
        ...
        if (!VerifyAspectsPresent(region.dstSubresource.aspectMask, dst_format)) {
            ...
            skip |= LogError(GetCopyImageVUID(region_loc, vvl::CopyError::DstSubresource_00143),
                             dst_objlist,
    ...
    const bool is_2 = region_loc.function == Func::vkCmdCopyImage2 || region_loc.function == Func::vkCmdCopyImage2KHR;
    const bool is_host =
        region_loc.function == Func::vkCopyImageToImage || region_loc.function == Func::vkCopyImageToImageEXT;
    ...
    if (api_version < VK_API_VERSION_1_1) {
        if (!IsExtEnabled(device_extensions.vk_khr_maintenance1)) {
            ...
            if (region.srcSubresource.layerCount != region.dstSubresource.layerCount) {
                ...
                skip |= LogError(vuid,
                                 objlist,
                                 src_subresource_loc.dot(Field::layerCount),
    ...
    if (extent_check & kYBit) {
        const LogObjectList src_objlist(handle, src_image_state.VkHandle());
        skip |= LogError(GetCopyImageVUID(region_loc, vvl::CopyError::SrcOffset_00145),
                         src_objlist,
                         region_loc.dot(Field::srcOffset).dot(Field::y),
    ...
    if (extent_check & kZBit) {
        const LogObjectList src_objlist(handle, src_image_state.VkHandle());
        skip |= LogError(GetCopyImageVUID(region_loc, vvl::CopyError::SrcOffset_00147),
                         src_objlist,
                         region_loc.dot(Field::srcOffset).dot(Field::z),
    ...
    if (slice_override)
        dst_copy_extent.depth = depth_slices;

    extent_check = ExceedsBounds(&(region.dstOffset), &dst_copy_extent, &subresource_extent);
    if (extent_check & kXBit) {
        const LogObjectList dst_objlist(handle, dst_image_state.VkHandle());
        skip |= LogError(GetCopyImageVUID(region_loc, vvl::CopyError::DstOffset_00150),
                         dst_objlist,
                         region_loc.dot(Field::dstOffset).dot(Field::x),
                         "(%" PRId32 ") + extent (%" PRIu32 ") exceeds miplevel %" PRIu32 " width (%" PRIu32 ").",
                         region.dstOffset.x,
    ...
    if (extent_check & kYBit) {
        const LogObjectList dst_objlist(handle, dst_image_state.VkHandle());
        skip |= LogError(GetCopyImageVUID(region_loc, vvl::CopyError::DstOffset_00151),
                         dst_objlist,
                         region_loc.dot(Field::dstOffset).dot(Field::y),
                         "(%" PRId32 ") + extent (%" PRIu32 ") exceeds miplevel %" PRIu32 " height (%" PRIu32 ").",
                         region.dstOffset.y,
    ...
    if (extent_check & kZBit) {
        const LogObjectList dst_objlist(handle, dst_image_state.VkHandle());
        skip |= LogError(GetCopyImageVUID(region_loc, vvl::CopyError::DstOffset_00153),
                         dst_objlist,
                         region_loc.dot(Field::dstOffset).dot(Field::z),
                         "(%" PRId32 ") + extent (%" PRIu32 ") exceeds miplevel %" PRIu32 " depth (%" PRIu32 ").",
                         region.dstOffset.z,
...
template <typename HandleT>
bool CoreChecks::ValidateCopyImageCommon(HandleT handle,
                                         const vvl::Image& src_image_state,
                                         const vvl::Image& dst_image_state,
                                         const Location& loc) const {
    bool skip = false;
    ...
    // src
    {
        ...
        skip |= ValidateMemoryIsBoundToImage(src_objlist,
                                             src_image_state,
                                             src_image_loc,
        ...
        if (src_image_state.create_info.flags & VK_IMAGE_CREATE_SUBSAMPLED_BIT_EXT) {
            skip |= LogError(GetCopyImageVUID(loc, vvl::CopyError::SrcImageSubsampled_07969),
                             src_objlist,
                             src_image_loc,
    ...
    // dst
    {
        ...
        skip |= ValidateMemoryIsBoundToImage(dst_objlist,
                                             dst_image_state,
                                             dst_image_loc,
        ...
        if (dst_image_state.create_info.flags & VK_IMAGE_CREATE_SUBSAMPLED_BIT_EXT) {
            skip |= LogError(GetCopyImageVUID(loc, vvl::CopyError::DstImageSubsampled_07969),
                             dst_objlist,
                             dst_image_loc,
                             "was created with flags including VK_IMAGE_CREATE_SUBSAMPLED_BIT_EXT");
        }
...
template <typename RegionType>
bool CoreChecks::ValidateCmdCopyImage(VkCommandBuffer commandBuffer,
                                      VkImage srcImage,
    ...
    for (uint32_t i = 0; i < regionCount; i++) {
        ...
        if (IsExtEnabled(device_extensions.vk_khr_maintenance1)) {
            ...
            if (!slice_override) {
                ...
                if (src_slices == VK_REMAINING_ARRAY_LAYERS || dst_slices == VK_REMAINING_ARRAY_LAYERS) {
                    if (src_slices != VK_REMAINING_ARRAY_LAYERS) {
                        if (src_slices != (dst_image_state->create_info.arrayLayers - dst_subresource.baseArrayLayer)) {
                            ...
                            skip |= LogError(vuid,
                                             ...
                                             src_slices,
                                             dst_image_state->create_info.arrayLayers,
                        ...
                        if (dst_slices != (src_image_state->create_info.arrayLayers - src_subresource.baseArrayLayer)) {
                            ...
                            skip |= LogError(vuid,
                                             src_objlist,
                                             dst_subresource_loc.dot(Field::layerCount),
                                             "(%" PRIu32 ") does not match srcImage arrayLayers (%" PRIu32
                                             ") minus baseArrayLayer (%" PRIu32 ").",
                                             dst_slices,
                                             src_image_state->create_info.arrayLayers,
                    ...
                    skip |= LogError(vuid,
                                     all_objlist,
                                     region_loc,
                                     "%s (%" PRIu32 ") is different from %s (%" PRIu32 ").",
                                     src_is_3d ? "extent.depth" : "srcSubresource.layerCount",
                                     src_slices,
                                     dst_is_3d ? "extent.depth" : "dstSubresource.layerCount",
                                     dst_slices);
                }
            }
            ...
            if (src_image_type != dst_image_type) {
                ...
                const bool valid =
                    (src_is_2d && dst_is_3d) || (src_is_3d && dst_is_2d) || enabled_features.maintenance5;
                if (!valid) {
                    vuid = is_2 ? "VUID-VkCopyImageInfo2-srcImage-07743" : "VUID-vkCmdCopyImage-srcImage-07743";
                    skip |= LogError(vuid,
                                     all_objlist,
                                     region_loc,
                                     "srcImage type (%s) must be equal to dstImage type (%s) or else one must be 2D "
                                     "and the other 3D",
            ...
            skip |= ValidateImageFormatFeatureFlags(
                commandBuffer, *src_image_state, VK_FORMAT_FEATURE_2_TRANSFER_SRC_BIT, src_image_loc, vuid);
            vuid = is_2 ? "VUID-VkCopyImageInfo2-dstImage-01996" : "VUID-vkCmdCopyImage-dstImage-01996";
            skip |= ValidateImageFormatFeatureFlags(
                commandBuffer, *dst_image_state, VK_FORMAT_FEATURE_2_TRANSFER_DST_BIT, dst_image_loc, vuid);
            ...
            if (src_is_2d && dst_is_3d && (src_copy_extent.depth != src_subresource.layerCount)) {
                ...
                skip |= LogError(vuid,
                                 all_objlist,
                                 region_loc,
                                 "srcImage is 2D, dstImage is 3D and extent.depth is %" PRIu32 " and has to be "
                ...
                skip |= LogError(vuid,
                                 all_objlist,
                                 region_loc,
                                 "srcImage is 3D, dstImage is 2D and extent.depth is %" PRIu32 " and has to be "
            ...
            if ((src_is_2d || dst_is_2d) && (src_copy_extent.depth != 1)) {
                ...
                skip |= LogError(vuid,
                                 all_objlist,
                                 region_loc,
                                 "srcImage is %s is dstImage is %s but extent.depth is %" PRIu32 ".",
                                 string_VkImageType(src_image_type),
                                 string_VkImageType(dst_image_type),
                                 src_copy_extent.depth);
            }
        ...
        // regions, must not overlap in memory Validation is only performed when source image is the same as destination
        // image. In the general case, the mapping between an image and its underlying memory is undefined, so checking
        // for memory overlaps is not possible.
        ...
        const bool same_subresource = (same_image && (src_subresource.mipLevel == dst_subresource.mipLevel) &&
                                       RangesIntersect(src_subresource.baseArrayLayer,
                                                       src_subresource.layerCount,
                                                       dst_subresource.baseArrayLayer,
                                                       dst_subresource.layerCount));
        if (same_subresource) {
        ...
        skip |= VerifyImageLayoutSubresource(
            cb_state, *src_image_state, src_subresource, srcImageLayout, src_image_loc, vuid);
        vuid = is_2 ? "VUID-VkCopyImageInfo2-dstImageLayout-00133" : "VUID-vkCmdCopyImage-dstImageLayout-00133";
        skip |= VerifyImageLayoutSubresource(
            cb_state, *dst_image_state, dst_subresource, dstImageLayout, dst_image_loc, vuid);
        skip |= ValidateCopyImageTransferGranularityRequirements(
            cb_state, *src_image_state, *dst_image_state, region, region_loc);
        skip |= ValidateCopyImageRegionCommon(commandBuffer, *src_image_state, *dst_image_state, region, region_loc);
    }

    // The formats of non-multiplane src_image and dst_image must be compatible. Formats are considered compatible if
    // their texel size in bytes is the same between both formats. For example, VK_FORMAT_R8G8B8A8_UNORM is compatible
    // with VK_FORMAT_R32_UINT because because both texels are 4 bytes in size.
    if (!vkuFormatIsMultiplane(src_format) && !vkuFormatIsMultiplane(dst_format)) {
        ...
        if (vkuFormatIsDepthOrStencil(src_format) || vkuFormatIsDepthOrStencil(dst_format)) {
            if (src_format != dst_format) {
                skip |= LogError(compatible_vuid,
                                 all_objlist,
                                 loc,
                                 "srcImage format (%s) is different from dstImage format (%s).",
                                 string_VkFormat(src_format),
                                 string_VkFormat(dst_format));
            }
        } else {
            if (vkuFormatElementSize(src_format) != vkuFormatElementSize(dst_format)) {
                skip |= LogError(compatible_vuid,
                                 all_objlist,
                                 loc,
                                 "srcImage format %s has size of %" PRIu32
                                 " and dstImage format %s has size of %" PRIu32 ".",
                                 string_VkFormat(src_format),
                                 vkuFormatElementSize(src_format),
                                 string_VkFormat(dst_format),
    ...
    if (vkuFormatIsCompressed(src_format) && vkuFormatIsCompressed(dst_format)) {
        ...
        if (src_block_extent.width != dst_block_extent.width || src_block_extent.height != dst_block_extent.height ||
            ...
            const char* compatible_vuid =
                is_2 ? "VUID-VkCopyImageInfo2-srcImage-09247" : "VUID-vkCmdCopyImage-srcImage-09247";
            skip |= LogError(
                compatible_vuid,
                all_objlist,
                loc,
                ...
                string_VkFormat(src_format),
                string_VkExtent3D(src_block_extent).c_str(),
                string_VkFormat(dst_format),
    ...
    if (!IsExtEnabled(device_extensions.vk_ext_separate_stencil_usage)) {
        ...
        skip |= ValidateImageUsageFlags(
            commandBuffer, *src_image_state, VK_IMAGE_USAGE_TRANSFER_SRC_BIT, false, vuid, src_image_loc);
        vuid = is_2 ? "VUID-VkCopyImageInfo2-aspect-06663" : "VUID-vkCmdCopyImage-aspect-06663";
        skip |= ValidateImageUsageFlags(
            commandBuffer, *dst_image_state, VK_IMAGE_USAGE_TRANSFER_DST_BIT, false, vuid, dst_image_loc);
    } else {
        auto src_separate_stencil =
            vku::FindStructInPNextChain<VkImageStencilUsageCreateInfo>(src_image_state->create_info.pNext);
        if (src_separate_stencil && has_stencil_aspect &&
            ...
            skip = LogError(vuid,
                            src_objlist,
                            src_image_loc,
                            "(%s) was created with %s but requires VK_IMAGE_USAGE_TRANSFER_SRC_BIT.",
        ...
        auto dst_separate_stencil =
            vku::FindStructInPNextChain<VkImageStencilUsageCreateInfo>(dst_image_state->create_info.pNext);
        if (dst_separate_stencil && has_stencil_aspect &&
            ...
            skip = LogError(vuid,
                            dst_objlist,
                            dst_image_loc,
                            "(%s) was created with %s but requires VK_IMAGE_USAGE_TRANSFER_DST_BIT.",
    ...
    if (src_image_state->create_info.samples != dst_image_state->create_info.samples) {
        ...
        skip |= LogError(vuid,
                         all_objlist,
                         src_image_loc,
                         "was created with (%s) but the dstImage was created with (%s).",
    ...
    if (!IsValueIn(
        ...
        skip |=
            LogError(vuid, src_objlist, loc.dot(Field::srcImageLayout), "is %s.", string_VkImageLayout(srcImageLayout));
    }
    ...
    if (!IsValueIn(
        ...
        skip |=
            LogError(vuid, dst_objlist, loc.dot(Field::dstImageLayout), "is %s.", string_VkImageLayout(dstImageLayout));
    }

    return skip;
}

bool CoreChecks::PreCallValidateCmdCopyImage(VkCommandBuffer commandBuffer,
                                             VkImage srcImage,
                                             VkImageLayout srcImageLayout,
                                             VkImage dstImage,
                                             VkImageLayout dstImageLayout,
                                             ...
                                             const ErrorObject& error_obj) const {
    return ValidateCmdCopyImage(
        commandBuffer, srcImage, srcImageLayout, dstImage, dstImageLayout, regionCount, pRegions, error_obj.location);
}

bool CoreChecks::PreCallValidateCmdCopyImage2KHR(VkCommandBuffer commandBuffer,
                                                 const VkCopyImageInfo2KHR* pCopyImageInfo,
                                                 const ErrorObject& error_obj) const {
    return PreCallValidateCmdCopyImage2(commandBuffer, pCopyImageInfo, error_obj);
}

bool CoreChecks::PreCallValidateCmdCopyImage2(VkCommandBuffer commandBuffer,
                                              const VkCopyImageInfo2* pCopyImageInfo,
                                              const ErrorObject& error_obj) const {
    return ValidateCmdCopyImage(commandBuffer,
...
void CoreChecks::PostCallRecordCmdCopyImage(VkCommandBuffer commandBuffer,
                                            VkImage srcImage,
                                            VkImageLayout srcImageLayout,
                                            VkImage dstImage,
                                            VkImageLayout dstImageLayout,
...
void CoreChecks::PostCallRecordCmdCopyImage2KHR(VkCommandBuffer commandBuffer,
                                                const VkCopyImageInfo2KHR* pCopyImageInfo,
                                                const RecordObject& record_obj) {
    PostCallRecordCmdCopyImage2(commandBuffer, pCopyImageInfo, record_obj);
}

void CoreChecks::PostCallRecordCmdCopyImage2(VkCommandBuffer commandBuffer,
                                             const VkCopyImageInfo2* pCopyImageInfo,
                                             const RecordObject& record_obj) {
    ...
    for (uint32_t i = 0; i < pCopyImageInfo->regionCount; ++i) {
        cb_state->SetImageInitialLayout(
            *src_image_state, pCopyImageInfo->pRegions[i].srcSubresource, pCopyImageInfo->srcImageLayout);
        cb_state->SetImageInitialLayout(
            *dst_image_state, pCopyImageInfo->pRegions[i].dstSubresource, pCopyImageInfo->dstImageLayout);
    }
}

template <typename RegionType>
void CoreChecks::RecordCmdCopyBuffer(VkCommandBuffer commandBuffer,
                                     VkBuffer srcBuffer,
                                     VkBuffer dstBuffer,
                                     uint32_t regionCount,
                                     const RegionType* pRegions,
    ...
    if (regionCount > 0 && (src_buffer_state->sparse || dst_buffer_state->sparse)) {
        ...
        auto queue_submit_validation = [this,
                                        commandBuffer,
                                        src_buffer_state,
                                        dst_buffer_state,
                                        src_ranges = std::move(src_ranges),
                                        dst_ranges = std::move(dst_ranges),
                                        src_ranges_bounds,
                                        dst_ranges_bounds,
                                        loc,
                                        vuid](const ValidationStateTracker& device_data,
                                              const class vvl::Queue& queue_state,
                ...
                if (const auto find_mem_it = dst_vk_memory_to_ranges_map.find(vk_memory);
                    ...
                    while (src_ranges_it != src_ranges.cend() && dst_ranges_it != dst_ranges_vec.cend()) {
                        if (src_ranges_it->first.intersects(dst_ranges_it->first)) {
                            ...
                            const LogObjectList objlist(
                                commandBuffer, src_buffer_state->Handle(), dst_buffer_state->Handle(), vk_memory);
                            skip |= this->LogError(vuid,
                                                   objlist,
                                                   loc,
                                                   "Copy source buffer range %s (from buffer %s) and destination "
                                                   "buffer range %s (from buffer %s) are "
...
void CoreChecks::PostCallRecordCmdCopyBuffer(VkCommandBuffer commandBuffer,
                                             VkBuffer srcBuffer,
                                             VkBuffer dstBuffer,
                                             uint32_t regionCount,
                                             const VkBufferCopy* pRegions,
...
void CoreChecks::PostCallRecordCmdCopyBuffer2KHR(VkCommandBuffer commandBuffer,
                                                 const VkCopyBufferInfo2KHR* pCopyBufferInfo,
                                                 const RecordObject& record_obj) {
    return PostCallRecordCmdCopyBuffer2(commandBuffer, pCopyBufferInfo, record_obj);
}

void CoreChecks::PostCallRecordCmdCopyBuffer2(VkCommandBuffer commandBuffer,
                                              const VkCopyBufferInfo2* pCopyBufferInfo,
                                              const RecordObject& record_obj) {
    BaseClass::PostCallRecordCmdCopyBuffer2(commandBuffer, pCopyBufferInfo, record_obj);
    RecordCmdCopyBuffer(commandBuffer,
                        pCopyBufferInfo->srcBuffer,
                        pCopyBufferInfo->dstBuffer,
                        pCopyBufferInfo->regionCount,
                        pCopyBufferInfo->pRegions,
...
template <typename HandleT, typename RegionType>
bool CoreChecks::ValidateImageBounds(const HandleT handle,
                                     const vvl::Image& image_state,
                                     const RegionType& region,
...
template <typename RegionType>
bool CoreChecks::ValidateBufferBounds(VkCommandBuffer cb,
                                      const vvl::Image& image_state,
    ...
    if (buffer_copy_size != 0) {
        ...
        if (buffer_state.create_info.size < max_buffer_copy) {
            ...
            skip |= LogError(GetCopyBufferImageDeviceVUID(region_loc, vvl::CopyError::ExceedBufferBounds_00171),
                             objlist,
                             region_loc,
                             "is trying to copy %" PRIu64 " bytes + %" PRIu64
                             " offset to/from the VkBuffer (%s) which exceeds the VkBuffer total size of %" PRIu64
                             " bytes.",
                             buffer_copy_size,
                             region.bufferOffset,
...
bool CoreChecks::ValidateImageSampleCount(const HandleT handle,
    ...
    if (image_state.create_info.samples != sample_count) {
        ...
        skip |= LogError(vuid,
                         objlist,
                         loc,
                         "%s was created with a sample count of %s but must be %s.",
                         FormatHandle(image_state).c_str(),
...
template <typename RegionType>
bool CoreChecks::ValidateImageBufferCopyMemoryOverlap(const vvl::CommandBuffer& cb_state,
                                                      const RegionType& region,
                                                      const vvl::Image& image_state,
                                                      const vvl::Buffer& buffer_state,
                                                      const Location& region_loc) const {
    ...
    auto texel_size = vkuFormatTexelSizeWithAspect(
        image_state.create_info.format, static_cast<VkImageAspectFlagBits>(region.imageSubresource.aspectMask));
    ...
    if (image_state.create_info.tiling == VK_IMAGE_TILING_LINEAR) {
        ...
        if (image_state.create_info.arrayLayers == 1)
            srl.arrayPitch = 0;
        if (image_state.create_info.imageType != VK_IMAGE_TYPE_3D)
            srl.depthPitch = 0;
        image_offset = (region.imageSubresource.baseArrayLayer * srl.arrayPitch) +
                       static_cast<VkDeviceSize>((region.imageOffset.x * texel_size)) +
    ...
    if (image_state.DoesResourceMemoryOverlap(image_region, &buffer_state, buffer_region)) {
        const LogObjectList objlist(cb_state.Handle(), image_state.Handle());
        skip |= LogError(GetCopyBufferImageDeviceVUID(region_loc, vvl::CopyError::MemoryOverlap_00173),
                         objlist,
                         region_loc,
...
template <typename RegionType>
bool CoreChecks::ValidateCmdCopyImageToBuffer(VkCommandBuffer commandBuffer,
                                              VkImage srcImage,
                                              VkImageLayout srcImageLayout,
                                              VkBuffer dstBuffer,
                                              uint32_t regionCount,
    ...
    // dst buffer
    {
        ...
        skip |= ValidateBufferUsageFlags(
            dst_objlist, *dst_buffer_state, VK_BUFFER_USAGE_TRANSFER_DST_BIT, true, vuid, dst_buffer_loc);

        vuid = is_2 ? "VUID-vkCmdCopyImageToBuffer2-commandBuffer-01832"
                    : "VUID-vkCmdCopyImageToBuffer-commandBuffer-01832";
        skip |= ValidateProtectedBuffer(cb_state, *dst_buffer_state, dst_buffer_loc, vuid);

        vuid = is_2 ? "VUID-vkCmdCopyImageToBuffer2-commandBuffer-01833"
    ...
    // src image
    {
        ...
        skip |= ValidateImageUsageFlags(
            commandBuffer, *src_image_state, VK_IMAGE_USAGE_TRANSFER_SRC_BIT, true, vuid, src_image_loc);

        vuid = is_2 ? "VUID-vkCmdCopyImageToBuffer2-commandBuffer-01831"
        ...
        if (!IsValueIn(srcImageLayout,
            ...
            vuid = is_2 ? "VUID-VkCopyImageToBufferInfo2-srcImageLayout-01397"
                        : "VUID-vkCmdCopyImageToBuffer-srcImageLayout-01397";
            skip |= LogError(
                vuid, src_objlist, loc.dot(Field::srcImageLayout), "is %s.", string_VkImageLayout(srcImageLayout));
        }
    }
    ...
    for (uint32_t i = 0; i < regionCount; ++i) {
        ...
        vuid = is_2 ? "VUID-VkCopyImageToBufferInfo2-srcImageLayout-00189"
                    : "VUID-vkCmdCopyImageToBuffer-srcImageLayout-00189";
        skip |= VerifyImageLayoutSubresource(
            cb_state, *src_image_state, region.imageSubresource, srcImageLayout, src_image_loc, vuid);
        skip |= ValidateCopyBufferImageTransferGranularityRequirements(cb_state, *src_image_state, region, region_loc);
        skip |=
            ValidateImageMipLevel(commandBuffer, *src_image_state, region.imageSubresource.mipLevel, subresource_loc);
        skip |= ValidateImageArrayLayerRange(commandBuffer,
                                             *src_image_state,
                                             region.imageSubresource.baseArrayLayer,
                                             region.imageSubresource.layerCount,
                                             subresource_loc);

        vuid = is_2 ? "VUID-VkCopyImageToBufferInfo2-pRegions-04566"
...
bool CoreChecks::PreCallValidateCmdCopyImageToBuffer(VkCommandBuffer commandBuffer,
                                                     VkImage srcImage,
                                                     VkImageLayout srcImageLayout,
                                                     VkBuffer dstBuffer,
                                                     uint32_t regionCount,
                                                     ...
                                                     const ErrorObject& error_obj) const {
    return ValidateCmdCopyImageToBuffer(
        commandBuffer, srcImage, srcImageLayout, dstBuffer, regionCount, pRegions, error_obj.location);
}
...
bool CoreChecks::PreCallValidateCmdCopyImageToBuffer2(VkCommandBuffer commandBuffer,
                                                      ...
                                                      const ErrorObject& error_obj) const {
    return ValidateCmdCopyImageToBuffer(commandBuffer,
                                        ...
                                        error_obj.location.dot(Field::pCopyImageToBufferInfo));
}

void CoreChecks::PostCallRecordCmdCopyImageToBuffer(VkCommandBuffer commandBuffer,
                                                    VkImage srcImage,
                                                    VkImageLayout srcImageLayout,
                                                    VkBuffer dstBuffer,
                                                    uint32_t regionCount,
                                                    const VkBufferImageCopy* pRegions,
                                                    const RecordObject& record_obj) {
    BaseClass::PostCallRecordCmdCopyImageToBuffer(
        commandBuffer, srcImage, srcImageLayout, dstBuffer, regionCount, pRegions, record_obj);
...
void CoreChecks::PostCallRecordCmdCopyImageToBuffer2(VkCommandBuffer commandBuffer,
    ...
    for (uint32_t i = 0; i < pCopyImageToBufferInfo->regionCount; ++i) {
        cb_state->SetImageInitialLayout(*src_image_state,
                                        pCopyImageToBufferInfo->pRegions[i].imageSubresource,
                                        pCopyImageToBufferInfo->srcImageLayout);
    }
}

template <typename RegionType>
bool CoreChecks::ValidateCmdCopyBufferToImage(VkCommandBuffer commandBuffer,
                                              VkBuffer srcBuffer,
                                              VkImage dstImage,
                                              VkImageLayout dstImageLayout,
                                              uint32_t regionCount,
    ...
    // src buffer
    {
        ...
        skip |= ValidateBufferUsageFlags(
            src_objlist, *src_buffer_state, VK_BUFFER_USAGE_TRANSFER_SRC_BIT, true, vuid, src_buffer_loc);

        vuid = is_2 ? "VUID-vkCmdCopyBufferToImage2-commandBuffer-01828"
    ...
    // dst image
    {
        ...
        skip |= ValidateImageUsageFlags(
            commandBuffer, *dst_image_state, VK_IMAGE_USAGE_TRANSFER_DST_BIT, true, vuid, dst_image_loc);

        vuid = is_2 ? "VUID-vkCmdCopyBufferToImage2-commandBuffer-01829"
                    : "VUID-vkCmdCopyBufferToImage-commandBuffer-01829";
        skip |= ValidateProtectedImage(cb_state, *dst_image_state, dst_image_loc, vuid);

        vuid = is_2 ? "VUID-vkCmdCopyBufferToImage-commandBuffer-01830"
        ...
        if (!IsValueIn(dstImageLayout,
                       { VK_IMAGE_LAYOUT_SHARED_PRESENT_KHR,
                         ...
                         VK_IMAGE_LAYOUT_GENERAL })) {
            vuid = is_2 ? "VUID-VkCopyBufferToImageInfo2-dstImageLayout-01396"
                        : "VUID-vkCmdCopyBufferToImage-dstImageLayout-01396";
            skip |= LogError(
                vuid, dst_objlist, loc.dot(Field::dstImageLayout), "is %s.", string_VkImageLayout(dstImageLayout));
        }
    }
    ...
    for (uint32_t i = 0; i < regionCount; ++i) {
        ...
        skip |= ValidateImageSubresourceLayers(commandBuffer, region.imageSubresource, subresource_loc);
        vuid = is_2 ? "VUID-VkCopyBufferToImageInfo2-dstImageLayout-00180"
                    : "VUID-vkCmdCopyBufferToImage-dstImageLayout-00180";
        skip |= VerifyImageLayoutSubresource(
            cb_state, *dst_image_state, region.imageSubresource, dstImageLayout, dst_image_loc, vuid);
        skip |= ValidateCopyBufferImageTransferGranularityRequirements(cb_state, *dst_image_state, region, region_loc);
        skip |=
            ValidateImageMipLevel(commandBuffer, *dst_image_state, region.imageSubresource.mipLevel, subresource_loc);
        skip |= ValidateImageArrayLayerRange(commandBuffer,
                                             *dst_image_state,
                                             region.imageSubresource.baseArrayLayer,
                                             region.imageSubresource.layerCount,
        ...
        vuid = is_2 ? "VUID-VkCopyBufferToImageInfo2-pRegions-04565"
...
bool CoreChecks::PreCallValidateCmdCopyBufferToImage(VkCommandBuffer commandBuffer,
                                                     VkBuffer srcBuffer,
                                                     VkImage dstImage,
                                                     VkImageLayout dstImageLayout,
                                                     uint32_t regionCount,
                                                     const VkBufferImageCopy* pRegions,
                                                     const ErrorObject& error_obj) const {
    return ValidateCmdCopyBufferToImage(
        commandBuffer, srcBuffer, dstImage, dstImageLayout, regionCount, pRegions, error_obj.location);
}
...
bool CoreChecks::PreCallValidateCmdCopyBufferToImage2(VkCommandBuffer commandBuffer,
                                                      ...
                                                      const ErrorObject& error_obj) const {
    return ValidateCmdCopyBufferToImage(commandBuffer,
                                        ...
                                        error_obj.location.dot(Field::pCopyBufferToImageInfo));
}

void CoreChecks::PostCallRecordCmdCopyBufferToImage(VkCommandBuffer commandBuffer,
                                                    VkBuffer srcBuffer,
                                                    VkImage dstImage,
                                                    VkImageLayout dstImageLayout,
                                                    uint32_t regionCount,
                                                    const VkBufferImageCopy* pRegions,
                                                    const RecordObject& record_obj) {
    BaseClass::PostCallRecordCmdCopyBufferToImage(
        commandBuffer, srcBuffer, dstImage, dstImageLayout, regionCount, pRegions, record_obj);
...
void CoreChecks::PostCallRecordCmdCopyBufferToImage2(VkCommandBuffer commandBuffer,
    ...
    for (uint32_t i = 0; i < pCopyBufferToImageInfo->regionCount; ++i) {
        cb_state->SetImageInitialLayout(*dst_image_state,
                                        pCopyBufferToImageInfo->pRegions[i].imageSubresource,
                                        pCopyBufferToImageInfo->dstImageLayout);
    }
}

bool CoreChecks::UsageHostTransferCheck(const vvl::Image& image_state,
...
VkImageLayout GetImageLayout(VkCopyMemoryToImageInfo data) {
    return data.dstImageLayout;
}
VkImageLayout GetImageLayout(VkCopyImageToMemoryInfo data) {
    return data.srcImageLayout;
}

VkImage GetImage(VkCopyMemoryToImageInfo data) {
    return data.dstImage;
}
VkImage GetImage(VkCopyImageToMemoryInfo data) {
...
bool CoreChecks::ValidateMemoryImageCopyCommon(InfoPointer info_ptr, const Location& loc) const {
    ...
    skip |= ValidateMemoryIsBoundToImage(image,
                                         *image_state,
                                         image_loc,
                                         from_image ? "VUID-VkCopyImageToMemoryInfo-srcImage-07966"
    ...
    skip |= ValidateImageSampleCount(device,
                                     *image_state,
                                     VK_SAMPLE_COUNT_1_BIT,
                                     image_loc,
                                     from_image ? "VUID-VkCopyImageToMemoryInfo-srcImage-07973"
    ...
    for (uint32_t i = 0; i < regionCount; i++) {
        ...
        skip |= ValidateImageArrayLayerRange(device,
                                             *image_state,
                                             region.imageSubresource.baseArrayLayer,
                                             region.imageSubresource.layerCount,
                                             subresource_loc);
        skip |= ValidateImageSubresourceLayers(device, region.imageSubresource, subresource_loc);
        skip |= UsageHostTransferCheck(
            *image_state, region.imageSubresource.aspectMask, vuid_09111, vuid_09112, vuid_09113, subresource_loc);
        ...
        if (check_memcpy) {
            if (region.imageOffset.x != 0 || region.imageOffset.y != 0 || region.imageOffset.z != 0) {
                ...
                skip |= LogError(vuid,
                                 device,
                                 loc.dot(info_type).dot(Field::flags),
                                 "contains VK_HOST_IMAGE_COPY_MEMCPY which "
                                 "means that pRegions[%" PRIu32 "].imageOffset x(%" PRIu32 "), y(%" PRIu32
                                 ") and z(%" PRIu32 ") must all be zero",
                                 i,
                                 region.imageOffset.x,
                                 region.imageOffset.y,
                                 region.imageOffset.z);
            ...
            if (!IsExtentEqual(region.imageExtent, subresource_extent)) {
                const char* vuid = from_image ? "VUID-VkCopyImageToMemoryInfo-srcImage-09115"
                ...
                skip |= LogError(
                    ...
                    string_VkExtent3D(region.imageExtent).c_str(),
                    string_VkExtent3D(subresource_extent).c_str(),
                    String(info_type));
            }
            if ((region.memoryRowLength != 0) || (region.memoryImageHeight != 0)) {
                const char* vuid = from_image ? "VUID-VkCopyImageToMemoryInfo-flags-09394"
                                              : "VUID-VkCopyMemoryToImageInfo-flags-09393";
                skip |= LogError(vuid,
                                 image_state->Handle(),
                                 region_loc.dot(Field::memoryRowLength),
        ...
        skip |= ValidateImageBounds(device,
                                    *image_state,
                                    region,
                                    region_loc,
        ...
        skip |= ValidateHostCopyCurrentLayout(
            image_layout, region.imageSubresource, *image_state, region_loc.dot(image_layout_field));
    }
    ...
    for (const auto& state : memory_states) {
        ...
        for (uint32_t i = 0; i < regionCount; i++) {
            ...
            if (region.memoryRowLength != 0 && region.memoryImageHeight != 0) {
                ...
                copy_size =
                    ((region.imageExtent.width * region.imageExtent.height * region.imageExtent.depth) * element_size);
            }
            const void* copy_start = region.pHostPointer;
            const void* copy_end = static_cast<const char*>(copy_start) + copy_size;

            if ((copy_start >= mapped_start && copy_start < mapped_end) ||
                (copy_end >= mapped_start && copy_end < mapped_end) ||
                ...
                skip |= LogError(vuid,
                                 image_state->Handle(),
                                 loc.dot(Field::pRegions, i).dot(Field::pHostPointer),
                                 "points to %" PRIu64
                                 " bytes in host memory spanning\n[%p : %p]\nwhich overlaps with %" PRIu64
...
bool CoreChecks::ValidateHostCopyImageCreateInfos(const vvl::Image& src_image_state,
                                                  const vvl::Image& dst_image_state,
                                                  const Location& loc) const {
    bool skip = false;
    ...
    if (src_info.mipLevels != dst_info.mipLevels) {
        mismatch_stream << "srcImage mipLevels = " << src_info.mipLevels
                        << "and dstImage mipLevels = " << dst_info.mipLevels << '\n';
    }
...
bool CoreChecks::ValidateHostCopyImageLayout(const VkImage image,
    ...
    skip |= LogError(vuid,
                     ...
                     String(supported_name));

    return skip;
}

bool CoreChecks::PreCallValidateCopyMemoryToImage(VkDevice device,
    ...
    skip |= ValidateHostCopyImageLayout(dst_image,
                                        props.copyDstLayoutCount,
                                        props.pCopyDstLayouts,
                                        pCopyMemoryToImageInfo->dstImageLayout,
                                        copy_loc.dot(Field::dstImageLayout),
...
bool CoreChecks::PreCallValidateCopyImageToMemory(VkDevice device,
    ...
    const auto& props = phys_dev_props_core14;
    skip |= ValidateHostCopyImageLayout(src_image,
                                        props.copySrcLayoutCount,
                                        props.pCopySrcLayouts,
                                        pCopyImageToMemoryInfo->srcImageLayout,
                                        copy_loc.dot(Field::srcImageLayout),
...
bool CoreChecks::ValidateMemcpyExtents(const VkImageCopy2& region,
                                       const vvl::Image& src_image_state,
                                       const vvl::Image& dst_image_state,
                                       const Location& region_loc) const {
    bool skip = false;
    if (region.srcOffset.x != 0 || region.srcOffset.y != 0 || region.srcOffset.z != 0) {
        skip |= LogError("VUID-VkCopyImageToImageInfo-srcOffset-09114",
                         device,
                         region_loc.dot(Field::srcOffset),
                         "is (%s) but flags contains VK_HOST_IMAGE_COPY_MEMCPY.",
                         string_VkOffset3D(region.srcOffset).c_str());
    }
    if (!IsExtentEqual(region.extent, src_image_state.create_info.extent)) {
        skip |= LogError("VUID-VkCopyImageToImageInfo-srcImage-09115",
                         src_image_state.Handle(),
                         region_loc.dot(Field::imageExtent),
                         "(%s) must match the image's subresource "
                         "extents (%s) when VkCopyImageToImageInfo->flags contains VK_HOST_IMAGE_COPY_MEMCPY",
                         string_VkExtent3D(region.extent).c_str(),
                         string_VkExtent3D(src_image_state.create_info.extent).c_str());
    }

    if (region.dstOffset.x != 0 || region.dstOffset.y != 0 || region.dstOffset.z != 0) {
        skip |= LogError("VUID-VkCopyImageToImageInfo-dstOffset-09114",
                         device,
                         region_loc.dot(Field::dstOffset),
                         "is (%s) but flags contains VK_HOST_IMAGE_COPY_MEMCPY.",
                         string_VkOffset3D(region.dstOffset).c_str());
    }
    if (!IsExtentEqual(region.extent, dst_image_state.create_info.extent)) {
        skip |= LogError("VUID-VkCopyImageToImageInfo-dstImage-09115",
                         dst_image_state.Handle(),
                         region_loc.dot(Field::imageExtent),
                         "(%s) must match the image's subresource "
                         "extents (%s) when VkCopyImageToImageInfo->flags contains VK_HOST_IMAGE_COPY_MEMCPY",
                         string_VkExtent3D(region.extent).c_str(),
                         string_VkExtent3D(dst_image_state.create_info.extent).c_str());
    }
    return skip;
}

bool CoreChecks::ValidateHostCopyMultiplane(const VkImageCopy2& region,
                                            const vvl::Image& src_image_state,
                                            const vvl::Image& dst_image_state,
                                            const Location& region_loc) const {
    bool skip = false;
    ...
    if (vkuFormatPlaneCount(src_image_state.create_info.format) == 2 &&
        ...
        skip |= LogError("VUID-VkCopyImageToImageInfo-srcImage-07981",
                         src_image_state.Handle(),
                         region_loc.dot(Field::srcSubresource),
                         "is %s but srcImage has 2-plane format (%s).",
                         string_VkImageAspectFlags(src_aspect_mask).c_str(),
    ...
    if (vkuFormatPlaneCount(src_image_state.create_info.format) == 3 &&
        ...
        skip |= LogError("VUID-VkCopyImageToImageInfo-srcImage-07981",
                         src_image_state.Handle(),
                         region_loc.dot(Field::srcSubresource),
                         "is %s but srcImage has 3-plane format (%s).",
                         string_VkImageAspectFlags(src_aspect_mask).c_str(),
                         string_VkFormat(src_image_state.create_info.format));
    }
    ...
    if (vkuFormatPlaneCount(dst_image_state.create_info.format) == 2 &&
        ...
        skip |= LogError("VUID-VkCopyImageToImageInfo-dstImage-07981",
                         dst_image_state.Handle(),
                         region_loc.dot(Field::dstSubresource),
                         "is %s but dstImage has 2-plane format (%s).",
                         string_VkImageAspectFlags(dst_aspect_mask).c_str(),
    ...
    if (vkuFormatPlaneCount(dst_image_state.create_info.format) == 3 &&
        ...
        skip |= LogError("VUID-VkCopyImageToImageInfo-dstImage-07981",
                         dst_image_state.Handle(),
                         region_loc.dot(Field::dstSubresource),
                         "is %s but dstImage has 3-plane format (%s).",
                         string_VkImageAspectFlags(dst_aspect_mask).c_str(),
                         string_VkFormat(dst_image_state.create_info.format));
    }
    return skip;
...
bool CoreChecks::PreCallValidateCopyImageToImage(VkDevice device,
    ...
    bool check_multiplane =
        ((src_plane_count == 2 || src_plane_count == 3) || (dst_plane_count == 2 || dst_plane_count == 3));
    bool check_memcpy = (info_ptr->flags & VK_HOST_IMAGE_COPY_MEMCPY);
    ...
    const auto& props = phys_dev_props_core14;
    skip |= ValidateHostCopyImageLayout(info_ptr->srcImage,
                                        props.copySrcLayoutCount,
                                        props.pCopySrcLayouts,
                                        info_ptr->srcImageLayout,
                                        loc.dot(Field::srcImageLayout),
                                        Field::pCopySrcLayouts,
                                        "VUID-VkCopyImageToImageInfo-srcImageLayout-09072");
    skip |= ValidateHostCopyImageLayout(info_ptr->dstImage,
                                        props.copyDstLayoutCount,
                                        props.pCopyDstLayouts,
                                        info_ptr->dstImageLayout,
                                        loc.dot(Field::dstImageLayout),
                                        Field::pCopyDstLayouts,
                                        "VUID-VkCopyImageToImageInfo-dstImageLayout-09073");

    if (src_image_state->sparse && (!src_image_state->HasFullRangeBound())) {
        skip |= LogError("VUID-VkCopyImageToImageInfo-srcImage-09109",
                         src_image_state->Handle(),
                         loc.dot(Field::srcImage),
                         "is a sparse image with no memory bound");
    }
    if (dst_image_state->sparse && (!dst_image_state->HasFullRangeBound())) {
        skip |= LogError("VUID-VkCopyImageToImageInfo-dstImage-09109",
                         dst_image_state->Handle(),
                         loc.dot(Field::dstImage),
                         "is a sparse image with no memory bound");
    }
    ...
    for (uint32_t i = 0; i < region_count; i++) {
        ...
        skip |= UsageHostTransferCheck(*src_image_state,
                                       ...
                                       region_loc.dot(Field::srcSubresource));
        skip |= UsageHostTransferCheck(*dst_image_state,
        ...
        skip |= ValidateHostCopyCurrentLayout(
            info_ptr->srcImageLayout, region.srcSubresource, *src_image_state, region_loc.dot(Field::srcImageLayout));
        skip |= ValidateHostCopyCurrentLayout(
            info_ptr->dstImageLayout, region.dstSubresource, *dst_image_state, region_loc.dot(Field::dstImageLayout));

        skip |= ValidateImageBounds(
            device, *src_image_state, region, region_loc, "VUID-VkCopyImageToImageInfo-srcSubresource-07970", true);
        skip |= ValidateImageBounds(
            device, *dst_image_state, region, region_loc, "VUID-VkCopyImageToImageInfo-dstSubresource-07970", false);
...
bool CoreChecks::PreCallValidateCopyImageToImageEXT(VkDevice device,
...
template <typename RegionType>
bool CoreChecks::ValidateCmdBlitImage(VkCommandBuffer commandBuffer,
                                      VkImage srcImage,
                                      VkImageLayout srcImageLayout,
                                      VkImage dstImage,
                                      VkImageLayout dstImageLayout,
    ...
    const VkImageType src_type = src_image_state->create_info.imageType;
    {
        ...
        skip |= ValidateImageFormatFeatureFlags(
            commandBuffer, *src_image_state, VK_FORMAT_FEATURE_2_BLIT_SRC_BIT, src_image_loc, vuid);
        ...
        if ((VK_FILTER_CUBIC_IMG == filter) && (VK_IMAGE_TYPE_2D != src_type)) {
            ...
            skip |= LogError(vuid,
                             src_objlist,
                             loc.dot(Field::filter),
                             "is VK_FILTER_CUBIC_IMG but srcImage was created with %s.",
        ...
        if (vkuFormatIsDepthOrStencil(src_format) && (filter != VK_FILTER_NEAREST)) {
            ...
            skip |= LogError(vuid,
                             src_objlist,
                             src_image_loc,
                             "has depth-stencil format %s but filter is %s.",
                             string_VkFormat(src_format),
                             string_VkFilter(filter));
        }
    ...
    const VkImageType dst_type = dst_image_state->create_info.imageType;
    {
        ...
        skip |= ValidateImageFormatFeatureFlags(
            commandBuffer, *dst_image_state, VK_FORMAT_FEATURE_2_BLIT_DST_BIT, dst_image_loc, vuid);
    ...
    if (vkuFormatIsUINT(src_format) != vkuFormatIsUINT(dst_format)) {
        ...
        skip |= LogError(vuid,
                         all_objlist,
                         loc,
                         "srcImage format %s is different than dstImage format %s.",
                         string_VkFormat(src_format),
                         string_VkFormat(dst_format));
    }
    ...
    for (uint32_t i = 0; i < regionCount; i++) {
        ...
        if (srcImage == dstImage) {
            for (uint32_t j = 0; j < regionCount; j++) {
                if (RegionIntersectsBlit(&region,
                                         &pRegions[j],
                                         src_image_state->create_info.imageType,
                                         vkuFormatIsMultiplane(src_format))) {
                    ...
                    skip |= LogError(vuid,
                                     all_objlist,
                                     loc,
                                     "pRegion[%" PRIu32 "] src overlaps with pRegions[%" PRIu32 "] dst.",
                                     i,
                                     j);
                }
            }
...
bool CoreChecks::PreCallValidateCmdBlitImage(VkCommandBuffer commandBuffer,
                                             VkImage srcImage,
                                             VkImageLayout srcImageLayout,
                                             VkImage dstImage,
                                             VkImageLayout dstImageLayout,
...
bool CoreChecks::PreCallValidateCmdBlitImage2KHR(VkCommandBuffer commandBuffer,
                                                 const VkBlitImageInfo2KHR* pBlitImageInfo,
                                                 const ErrorObject& error_obj) const {
    return PreCallValidateCmdBlitImage2(commandBuffer, pBlitImageInfo, error_obj);
}

bool CoreChecks::PreCallValidateCmdBlitImage2(VkCommandBuffer commandBuffer,
...
template <typename RegionType>
void CoreChecks::RecordCmdBlitImage(VkCommandBuffer commandBuffer,
                                    VkImage srcImage,
                                    VkImageLayout srcImageLayout,
                                    VkImage dstImage,
                                    VkImageLayout dstImageLayout,
...
void CoreChecks::PostCallRecordCmdBlitImage(VkCommandBuffer commandBuffer,
                                            VkImage srcImage,
                                            VkImageLayout srcImageLayout,
                                            VkImage dstImage,
                                            VkImageLayout dstImageLayout,
                                            ...
                                            VkFilter filter,
                                            const RecordObject& record_obj) {
    BaseClass::PostCallRecordCmdBlitImage(
        commandBuffer, srcImage, srcImageLayout, dstImage, dstImageLayout, regionCount, pRegions, filter, record_obj);
    RecordCmdBlitImage(
        commandBuffer, srcImage, srcImageLayout, dstImage, dstImageLayout, regionCount, pRegions, filter);
}

void CoreChecks::PostCallRecordCmdBlitImage2KHR(VkCommandBuffer commandBuffer,
                                                const VkBlitImageInfo2KHR* pBlitImageInfo,
                                                const RecordObject& record_obj) {
    PostCallRecordCmdBlitImage2(commandBuffer, pBlitImageInfo, record_obj);
}

void CoreChecks::PostCallRecordCmdBlitImage2(VkCommandBuffer commandBuffer,
                                             const VkBlitImageInfo2KHR* pBlitImageInfo,
                                             const RecordObject& record_obj) {
    BaseClass::PostCallRecordCmdBlitImage2(commandBuffer, pBlitImageInfo, record_obj);
    RecordCmdBlitImage(commandBuffer,
                       pBlitImageInfo->srcImage,
                       pBlitImageInfo->srcImageLayout,
                       pBlitImageInfo->dstImage,
                       pBlitImageInfo->dstImageLayout,
...
template <typename RegionType>
bool CoreChecks::ValidateCmdResolveImage(VkCommandBuffer commandBuffer,
                                         VkImage srcImage,
                                         VkImageLayout srcImageLayout,
                                         VkImage dstImage,
                                         VkImageLayout dstImageLayout,
    ...
    // src image
    {
        ...
        skip |= ValidateImageFormatFeatureFlags(
            commandBuffer, *src_image_state, VK_FORMAT_FEATURE_TRANSFER_SRC_BIT, src_image_loc, vuid);
        ...
        if (!IsValueIn(srcImageLayout,
            ...
            const LogObjectList objlist(cb_state.Handle(), src_image_state->Handle());
            skip |=
                LogError(vuid, objlist, loc.dot(Field::srcImageLayout), "is %s.", string_VkImageLayout(srcImageLayout));
        }
    ...
    // dst image
    {
        ...
        skip |= ValidateImageFormatFeatureFlags(
            commandBuffer, *dst_image_state, VK_FORMAT_FEATURE_2_COLOR_ATTACHMENT_BIT, dst_image_loc, vuid);
        ...
        skip |= ValidateImageFormatFeatureFlags(
            commandBuffer, *dst_image_state, VK_FORMAT_FEATURE_TRANSFER_DST_BIT, dst_image_loc, vuid);
        ...
        if (!IsValueIn(dstImageLayout,
            ...
            const LogObjectList objlist(cb_state.Handle(), dst_image_state->Handle());
            skip |=
                LogError(vuid, objlist, loc.dot(Field::dstImageLayout), "is %s.", string_VkImageLayout(dstImageLayout));
        }
        ...
        if (dst_image_state->create_info.samples != VK_SAMPLE_COUNT_1_BIT) {
            ...
            skip |= LogError(vuid,
                             dst_objlist,
                             dst_image_loc,
                             "was created with sample count (%s) (not VK_SAMPLE_COUNT_1_BIT).",
                             string_VkSampleCountFlagBits(dst_image_state->create_info.samples));
        }
    }
    ...
    for (uint32_t i = 0; i < regionCount; i++) {
        ...
        if (VK_IMAGE_TYPE_3D == dst_image_type) {
            ...
            if ((dst_subresource.baseArrayLayer != 0) || (dst_subresource.layerCount != 1)) {
                ...
                skip |= LogError(vuid,
                                 dst_objlist,
                                 dst_subresource_loc.dot(Field::baseArrayLayer),
    ...
    if (src_image_state->create_info.format != dst_image_state->create_info.format) {
        ...
        skip |= LogError(vuid,
                         all_objlist,
                         src_image_loc,
                         "was created with format %s but dstImage format is %s.",
                         string_VkFormat(src_image_state->create_info.format),
                         string_VkFormat(dst_image_state->create_info.format));
    }
    return skip;
...
bool CoreChecks::PreCallValidateCmdResolveImage(VkCommandBuffer commandBuffer,
                                                VkImage srcImage,
                                                VkImageLayout srcImageLayout,
                                                VkImage dstImage,
                                                VkImageLayout dstImageLayout,
                                                ...
                                                const ErrorObject& error_obj) const {
    return ValidateCmdResolveImage(
        commandBuffer, srcImage, srcImageLayout, dstImage, dstImageLayout, regionCount, pRegions, error_obj.location);
}
...
bool CoreChecks::PreCallValidateCmdResolveImage2(VkCommandBuffer commandBuffer,
                                                 ...
                                                 const ErrorObject& error_obj) const {
    return ValidateCmdResolveImage(commandBuffer,
