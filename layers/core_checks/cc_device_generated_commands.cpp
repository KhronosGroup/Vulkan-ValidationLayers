...
bool CoreChecks::PreCallValidateCreateIndirectCommandsLayoutEXT(
    VkDevice device,
    ...
    for (uint32_t i = 0; i < pCreateInfo->tokenCount; i++) {
        ...
        if (token.type == VK_INDIRECT_COMMANDS_TOKEN_TYPE_PUSH_CONSTANT_EXT) {
            ...
            if (!dynamic_layout_create && pipeline_layout_state) {
                ...
                // TODO - Because of custom PushConstantRangesId, can't share logic when using
            ...
            if (vertex_binding_unit_unique.find(vertex_binding_uint) != vertex_binding_unit_unique.end()) {
                skip |=
                    LogError("VUID-VkIndirectCommandsLayoutCreateInfoEXT-pTokens-11097",
                             device,
                             ...
                             vertex_binding_uint,
                             vertex_binding_unit_unique[vertex_binding_uint]);
            } else {
        ...
        if (token.type == VK_INDIRECT_COMMANDS_TOKEN_TYPE_EXECUTION_SET_EXT) {
            ...
            if (execution_set_token_index == kNotFound) {
                ...
                if (i != 0) {
                    skip |= LogError("VUID-VkIndirectCommandsLayoutCreateInfoEXT-pTokens-11139",
                                     device,
                                     token_loc.dot(Field::type),
                                     "is VK_INDIRECT_COMMANDS_TOKEN_TYPE_EXECUTION_SET_EXT but it can only be the "
                                     "first token at pTokens[0].");
                }
            } else {
                skip |= LogError("VUID-VkIndirectCommandsLayoutCreateInfoEXT-pTokens-11093",
                                 device,
                                 token_loc.dot(Field::type),
            ...
            if (index_buffer_token_index == kNotFound) {
                ...
                skip |= LogError("VUID-VkIndirectCommandsLayoutCreateInfoEXT-pTokens-11094",
                                 device,
                                 token_loc.dot(Field::type),
            ...
            if (sequence_index_token_index == kNotFound) {
                ...
                skip |= LogError("VUID-VkIndirectCommandsLayoutCreateInfoEXT-pTokens-11145",
                                 device,
                                 token_loc.dot(Field::type),
        ...
        if (token.type == VK_INDIRECT_COMMANDS_TOKEN_TYPE_PUSH_CONSTANT_EXT ||
            ...
            if (pCreateInfo->pipelineLayout == VK_NULL_HANDLE) {
                if (!enabled_features.dynamicGeneratedPipelineLayout) {
                    skip |= LogError("VUID-VkIndirectCommandsLayoutCreateInfoEXT-pTokens-11101",
                                     device,
                                     token_loc.dot(Field::type),
                    ...
                    skip |= LogError("VUID-VkIndirectCommandsLayoutCreateInfoEXT-pTokens-11102",
                                     device,
                                     token_loc.dot(Field::type),
            ...
            for (const auto& [past_index, past_range] : token_ranges) {
                if (RangesIntersect(past_range.offset, past_range.size, token_range.offset, token_range.size)) {
                    skip |=
                        LogError("VUID-VkIndirectCommandsLayoutCreateInfoEXT-pTokens-11099",
                                 device,
    ...
    if (!IsActionCommand(final_token_type)) {
        skip |= LogError("VUID-VkIndirectCommandsLayoutCreateInfoEXT-pTokens-11100",
                         device,
        ...
        if (!IsValueIn(final_token_type,
            ...
            skip |= LogError("VUID-VkIndirectCommandsLayoutCreateInfoEXT-pTokens-11095",
                             device,
                             create_info_loc.dot(Field::pTokens, final_token_index).dot(Field::type),
...
bool CoreChecks::PreCallValidateDestroyIndirectCommandsLayoutEXT(VkDevice device,
    ...
    if (auto indirect_commands_layout = Get<vvl::IndirectCommandsLayout>(indirectCommandsLayout)) {
        skip |= ValidateObjectNotInUse(indirect_commands_layout.get(),
                                       error_obj.location,
...
bool CoreChecks::ValidateIndirectExecutionSetPipelineInfo(const VkIndirectExecutionSetPipelineInfoEXT& pipeline_info,
    ...
    if (initial_pipeline->pipeline_type == VK_PIPELINE_BIND_POINT_COMPUTE &&
        ...
        skip |= LogError(
            ...
            initial_pipeline->Handle(),
            pipeline_info_loc.dot(Field::initialPipeline),
            "is type VK_PIPELINE_BIND_POINT_COMPUTE, but supportedIndirectCommandsShaderStagesPipelineBinding (%s) "
            "doesn't "
        ...
        skip |= LogError(
            ...
            initial_pipeline->Handle(),
            pipeline_info_loc.dot(Field::initialPipeline),
            "is type VK_PIPELINE_BIND_POINT_GRAPHICS, but supportedIndirectCommandsShaderStagesPipelineBinding (%s) "
            "doesn't "
        ...
        skip |= LogError(
            ...
            initial_pipeline->Handle(),
            pipeline_info_loc.dot(Field::initialPipeline),
            "is type VK_PIPELINE_BIND_POINT_RAY_TRACING_KHR, but supportedIndirectCommandsShaderStagesPipelineBinding "
            "(%s) doesn't "
    ...
    for (uint32_t i = 0; i < pipeline_layout->set_layouts.size(); i++) {
        if (pipeline_layout->set_layouts[i] == nullptr)
            continue;
        ...
        for (uint32_t j = 0; j < bindings.size(); j++) {
            if (bindings[j].descriptorType == VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER_DYNAMIC ||
                ...
                skip |= LogError(
                    ...
                    string_VkDescriptorType(bindings[j].descriptorType),
                    i,
                    j);
            }
        }
...
bool CoreChecks::ValidateIndirectExecutionSetShaderInfo(const VkIndirectExecutionSetShaderInfoEXT& shader_info,
    ...
    for (uint32_t i = 0; i < shader_info.shaderCount; i++) {
        ...
        if ((shader_object->create_info.flags & VK_SHADER_CREATE_INDIRECT_BINDABLE_BIT_EXT) == 0) {
            skip |= LogError("VUID-VkIndirectExecutionSetShaderInfoEXT-pInitialShaders-11154",
                             shader_object->Handle(),
        ...
        if ((stage & props.supportedIndirectCommandsShaderStagesShaderBinding) == 0) {
            skip |=
                LogError("VUID-VkIndirectExecutionSetShaderInfoEXT-pInitialShaders-11020",
                         shader_handle,
                         shader_info_loc.dot(Field::pInitialShaders, i),
                         "was created with stage %s, but supportedIndirectCommandsShaderStagesShaderBinding (%s) "
                         "doesn't indicate support.",
        ...
        if (unique_stages.find(stage) != unique_stages.end()) {
            skip |= LogError("VUID-VkIndirectExecutionSetShaderInfoEXT-stage-11023",
                             shader_handle,
                             shader_info_loc.dot(Field::pInitialShaders, i),
                             "is the same stage as pInitialShaders[%" PRIu32 "] (%s)",
                             unique_stages[stage],
                             string_VkShaderStageFlagBits(stage));
        } else {
...
bool CoreChecks::PreCallValidateCreateIndirectExecutionSetEXT(VkDevice device,
    ...
    if (pCreateInfo->type == VK_INDIRECT_EXECUTION_SET_INFO_TYPE_PIPELINES_EXT) {
        ...
        skip |= ValidateIndirectExecutionSetPipelineInfo(*pCreateInfo->info.pPipelineInfo,
        ...
        skip |=
            ValidateIndirectExecutionSetShaderInfo(*pCreateInfo->info.pShaderInfo, info_loc.dot(Field::pShaderInfo));
    }

    return skip;
}

bool CoreChecks::PreCallValidateDestroyIndirectExecutionSetEXT(VkDevice device,
                                                               VkIndirectExecutionSetEXT indirectExecutionSet,
                                                               const VkAllocationCallbacks* pAllocator,
                                                               const ErrorObject& error_obj) const {
    bool skip = false;
    if (auto indirect_execution_set = Get<vvl::IndirectExecutionSet>(indirectExecutionSet)) {
        skip |= ValidateObjectNotInUse(indirect_execution_set.get(),
                                       error_obj.location,
...
bool CoreChecks::ValidateGeneratedCommandsInfo(const vvl::CommandBuffer& cb_state,
    ...
    if (indirect_commands_layout.has_multi_draw_count_token) {
        ...
        const uint64_t count =
            (uint64_t)generated_commands_info.maxDrawCount * (uint64_t)generated_commands_info.maxSequenceCount;
        if (count >= limit) {
            skip |= LogError("VUID-VkGeneratedCommandsInfoEXT-maxDrawCount-11078",
                             cb_state.Handle(),
                             info_loc.dot(Field::maxDrawCount),
                             "(%" PRIu32 ") time maxSequenceCount (%" PRIu32 ") is %" PRIu64
                             " which is over the limit of 2^24 (16777216)",
                             generated_commands_info.maxDrawCount,
                             generated_commands_info.maxSequenceCount,
                             count);
        }
    }
    ...
    auto* pipeline_info =
        vku::FindStructInPNextChain<VkGeneratedCommandsPipelineInfoEXT>(generated_commands_info.pNext);
    ...
    if (generated_commands_info.indirectExecutionSet == VK_NULL_HANDLE) {
        if (!pipeline_info && !shader_info) {
            skip |= LogError("VUID-VkGeneratedCommandsInfoEXT-indirectExecutionSet-11080",
                             cb_state.Handle(),
                             info_loc.dot(Field::indirectExecutionSet),
                             "is VK_NULL_HANDLE but the pNext chain does not contain an instance of "
                             "VkGeneratedCommandsPipelineInfoEXT or "
                             "VkGeneratedCommandsShaderInfoEXT.");
            valid_dispatch = false;
        } else if (indirect_commands_layout.has_execution_set_token) {
            skip |= LogError("VUID-VkGeneratedCommandsInfoEXT-indirectCommandsLayout-11083",
        ...
        if (!indirect_commands_layout.has_execution_set_token) {
            skip |= LogError("VUID-VkGeneratedCommandsInfoEXT-indirectCommandsLayout-10241",
                             indirect_commands_layout.Handle(),
                             info_loc.dot(Field::indirectExecutionSet),
            ...
            if (indirect_execution_set->shader_stage_flags !=
                indirect_commands_layout.execution_set_token_shader_stage_flags) {
                skip |= LogError(
    ...
    if (valid_dispatch) {
        ...
        if (generated_commands_info.preprocessSize < mem_reqs.memoryRequirements.size) {
            skip |= LogError(
                ...
                cb_state.Handle(),
                info_loc.dot(Field::preprocessSize),
                "(%" PRIu64
                ") is less then the size returned from vkGetGeneratedCommandsMemoryRequirementsEXT (%" PRIu64 ").",
                generated_commands_info.preprocessSize,
                mem_reqs.memoryRequirements.size);
        }
    }

    if (generated_commands_info.maxSequenceCount >
        phys_dev_ext_props.device_generated_commands_props.maxIndirectSequenceCount) {
        skip |= LogError("VUID-VkGeneratedCommandsInfoEXT-maxSequenceCount-11067",
                         cb_state.Handle(),
                         info_loc.dot(Field::maxSequenceCount),
                         "(%" PRIu32 ") is larger than maxIndirectSequenceCount (%" PRIu32 ").",
                         generated_commands_info.maxSequenceCount,
                         phys_dev_ext_props.device_generated_commands_props.maxIndirectSequenceCount);
    }
    ...
    if (!sequence_buffer_states.empty()) {
        ...
        skip |= buffer_address_validator.LogErrorsIfNoValidBuffer(*this,
                                                                  sequence_buffer_states,
                                                                  info_loc.dot(Field::sequenceCountAddress),
                                                                  LogObjectList(cb_state.Handle()),
...
bool CoreChecks::PreCallValidateCmdExecuteGeneratedCommandsEXT(VkCommandBuffer commandBuffer,
    ...
    if (!cb_state.unprotected) {
        skip |= LogError("VUID-vkCmdExecuteGeneratedCommandsEXT-commandBuffer-11045",
                         commandBuffer,
    ...
    if (cb_state.transform_feedback_active) {
        ...
        if (pGeneratedCommandsInfo->indirectExecutionSet != VK_NULL_HANDLE) {
            skip |= LogError("VUID-vkCmdExecuteGeneratedCommandsEXT-indirectExecutionSet-11058",
                             commandBuffer,
                             error_obj.location.dot(Field::commandBuffer),
                             "has Transform Feedback active, but pGeneratedCommandsInfo->indirectExecutionSet is not "
                             "VK_NULL_HANDLE.");
        }
    }

    if (cb_state.beginInfo.flags & VK_COMMAND_BUFFER_USAGE_SIMULTANEOUS_USE_BIT) {
        LogError("VUID-vkCmdExecuteGeneratedCommandsEXT-commandBuffer-11143",
                 commandBuffer,
                 error_obj.location.dot(Field::commandBuffer),
    ...
    const auto indirect_commands_layout =
        Get<vvl::IndirectCommandsLayout>(pGeneratedCommandsInfo->indirectCommandsLayout);
    ASSERT_AND_RETURN_SKIP(indirect_commands_layout);

    const bool preprocess_usage_flag = (indirect_commands_layout->create_info.flags &
    ...
    if (isPreprocessed && !preprocess_usage_flag) {
        ...
        skip |= LogError("VUID-vkCmdExecuteGeneratedCommandsEXT-isPreprocessed-11047",
                         objlist,
                         info_loc.dot(Field::indirectCommandsLayout),
                         "was not created with VK_INDIRECT_COMMANDS_LAYOUT_USAGE_EXPLICIT_PREPROCESS_BIT_EXT but "
                         "isPreprocessed is VK_TRUE.");
    } else if (!isPreprocessed && preprocess_usage_flag) {
        const LogObjectList objlist(commandBuffer, indirect_commands_layout->Handle());
        skip |= LogError("VUID-vkCmdExecuteGeneratedCommandsEXT-indirectCommandsLayout-11141",
                         objlist,
                         info_loc.dot(Field::indirectCommandsLayout),
                         "was created with VK_INDIRECT_COMMANDS_LAYOUT_USAGE_EXPLICIT_PREPROCESS_BIT_EXT but "
                         "isPreprocessed is VK_FALSE.");
    }
    ...
    if (cb_state.activeRenderPass) {
        ...
        if (view_mask != 0) {
            skip |= LogError("VUID-vkCmdExecuteGeneratedCommandsEXT-None-11062",
                             commandBuffer,
                             error_obj.location,
                             "The active render pass contains a non-zero view mask (%" PRIu32 ").",
                             view_mask);
        }
    }
    ...
    if (auto indirect_execution_set = Get<vvl::IndirectExecutionSet>(pGeneratedCommandsInfo->indirectExecutionSet)) {
        const LogObjectList objlist(
            commandBuffer, indirect_commands_layout->Handle(), indirect_execution_set->Handle());
        skip |= ValidateGeneratedCommandsInitialShaderState(cb_state,
    ...
    if ((bound_stages | props.supportedIndirectCommandsShaderStages) != props.supportedIndirectCommandsShaderStages) {
        skip |= LogError("VUID-vkCmdExecuteGeneratedCommandsEXT-supportedIndirectCommandsShaderStages-11060",
                         cb_state.GetObjectList(indirect_commands_layout->bind_point),
                         error_obj.location.dot(Field::commandBuffer),
    ...
    skip |= ValidateGeneratedCommandsInfo(
        cb_state, *indirect_commands_layout, *pGeneratedCommandsInfo, isPreprocessed, info_loc);

    return skip;
}

bool CoreChecks::ValidateGeneratedCommandsInitialShaderState(
    ...
    if (indirect_execution_set.is_pipeline) {
        ...
        if (!pipeline) {
            skip |= LogError(
                vuid, objlist, cb_loc, "has not had a pipeline bound for %s.", string_VkPipelineBindPoint(bind_point));
...
bool CoreChecks::ValidatePreprocessGeneratedCommandsStateCommandBuffer(
    ...
    if (state_command_buffer.state == CbState::InvalidComplete ||
        state_command_buffer.state == CbState::InvalidIncomplete) {
        skip |= ReportInvalidCommandBuffer(state_command_buffer,
        ...
        skip |= LogError("VUID-vkCmdPreprocessGeneratedCommandsEXT-stateCommandBuffer-11138",
                         objlist,
                         loc.dot(Field::stateCommandBuffer),
                         "is not in a recording state.");
    }

    if (auto indirect_execution_set = Get<vvl::IndirectExecutionSet>(generated_commands_info.indirectExecutionSet)) {
        const LogObjectList objlist(command_buffer.Handle(),
                                    state_command_buffer.Handle(),
                                    indirect_commands_layout.Handle(),
                                    indirect_execution_set->Handle());
        skip |= ValidateGeneratedCommandsInitialShaderState(state_command_buffer,
                                                            indirect_commands_layout,
                                                            *indirect_execution_set,
                                                            generated_commands_info.shaderStages,
                                                            objlist,
                                                            loc.dot(Field::stateCommandBuffer));
    }
...
bool CoreChecks::PreCallValidateCmdPreprocessGeneratedCommandsEXT(
    ...
    skip |= ValidateCmd(cb_state, error_obj.location);
    if (!cb_state.unprotected) {
        skip |= LogError("VUID-vkCmdPreprocessGeneratedCommandsEXT-commandBuffer-11081",
                         commandBuffer,
                         error_obj.location.dot(Field::commandBuffer),
                         "is protected.");
    }
    ...
    if ((indirect_commands_layout->create_info.flags & VK_INDIRECT_COMMANDS_LAYOUT_USAGE_EXPLICIT_PREPROCESS_BIT_EXT) ==
        0) {
        const LogObjectList objlist(commandBuffer, indirect_commands_layout->Handle());
        skip |= LogError("VUID-vkCmdPreprocessGeneratedCommandsEXT-pGeneratedCommandsInfo-11082",
                         objlist,
                         info_loc.dot(Field::indirectCommandsLayout),
    ...
    skip |= ValidatePreprocessGeneratedCommandsStateCommandBuffer(
        cb_state, *state_cb_state, *indirect_commands_layout, *pGeneratedCommandsInfo, error_obj.location);

    skip |=
        ValidateGeneratedCommandsInfo(cb_state, *indirect_commands_layout, *pGeneratedCommandsInfo, false, info_loc);

    return skip;
}

bool CoreChecks::PreCallValidateGetGeneratedCommandsMemoryRequirementsEXT(
    VkDevice device,
    ...
    if (pInfo->maxSequenceCount > phys_dev_ext_props.device_generated_commands_props.maxIndirectSequenceCount) {
        skip |= LogError("VUID-VkGeneratedCommandsMemoryRequirementsInfoEXT-maxSequencesCount-11009",
                         device,
                         info_loc.dot(Field::maxSequenceCount),
                         "(%" PRIu32 ") is larger than maxIndirectSequenceCount (%" PRIu32 ").",
    ...
    if (indirect_commands_layout->has_multi_draw_count_token) {
        ...
        if (count >= limit) {
            skip |= LogError("VUID-VkGeneratedCommandsMemoryRequirementsInfoEXT-maxDrawCount-11146",
                             device,
                             info_loc.dot(Field::maxDrawCount),
                             "(%" PRIu32 ") time maxSequenceCount (%" PRIu32 ") is %" PRIu64
                             " which is over the limit of 2^24 (16777216)",
        ...
        if (!indirect_commands_layout->has_execution_set_token) {
            ...
            if (indirect_execution_set->shader_stage_flags !=
                ...
                skip |= LogError(
                    ...
                    "was created with shader stage %s but indirectCommandsLayout was created with shader stage %s.",
                    string_VkShaderStageFlags(indirect_execution_set->shader_stage_flags).c_str(),
                    string_VkShaderStageFlags(indirect_commands_layout->execution_set_token_shader_stage_flags)
                        .c_str());
            }
        }
    }
...
bool CoreChecks::PreCallValidateUpdateIndirectExecutionSetPipelineEXT(
    ...
    if (executionSetWriteCount > indirect_execution_set->max_pipeline_count) {
        skip |= LogError("VUID-vkUpdateIndirectExecutionSetPipelineEXT-executionSetWriteCount-11037",
                         indirect_execution_set->Handle(),
                         error_obj.location.dot(Field::executionSetWriteCount),
                         "(%" PRIu32
                         ") is larger than the maxPipelineCount used to create indirectExecutionSet (%" PRIu32 ").",
                         executionSetWriteCount,
                         indirect_execution_set->max_pipeline_count);
    }
    ...
    for (uint32_t i = 0; i < executionSetWriteCount; i++) {
        ...
        if (unique_indexes.find(set_pipeline.index) != unique_indexes.end()) {
            skip |= LogError("VUID-vkUpdateIndirectExecutionSetPipelineEXT-pExecutionSetWrites-11042",
                             device,
                             set_write_loc.dot(Field::index),
                             "(%" PRIu32 ") has same value as pExecutionSetWrites[%" PRIu32 "].index",
                             set_pipeline.index,
                             unique_indexes[set_pipeline.index]);
        } else {
        ...
        if (set_pipeline.index >= indirect_execution_set->max_pipeline_count) {
            skip |=
                LogError("VUID-VkWriteIndirectExecutionSetPipelineEXT-index-11026",
                         indirect_execution_set->Handle(),
                         set_write_loc.dot(Field::index),
                         "(%" PRIu32
                         ") is not less than the maxPipelineCount used to create indirectExecutionSet (%" PRIu32 ").",
                         set_pipeline.index,
                         indirect_execution_set->max_pipeline_count);
        }
        ...
        if (const auto initial_pipeline = indirect_execution_set->initial_pipeline) {
            // Want to throw this error first, in case one is missing a fragment shader, might have extra errors follow
            // below
            if (initial_pipeline->active_shaders != update_pipeline->active_shaders) {
                ...
                skip |= LogError("VUID-vkUpdateIndirectExecutionSetPipelineEXT-initialPipeline-11152",
                                 objlist,
                                 set_write_loc.dot(Field::pipeline),
            ...
            if (initial_pipeline->dynamic_state != update_pipeline->dynamic_state) {
                LogObjectList objlist(initial_pipeline->Handle(), update_pipeline->Handle());
                skip |= LogError("VUID-vkUpdateIndirectExecutionSetPipelineEXT-None-11040",
                                 objlist,
                                 set_write_loc.dot(Field::pipeline),
            ...
            if (initial_pipeline->fragmentShader_writable_output_location_list !=
                ...
                LogObjectList objlist(initial_pipeline->Handle(), update_pipeline->Handle());
                skip |= LogError("VUID-vkUpdateIndirectExecutionSetPipelineEXT-initialPipeline-11147",
                                 objlist,
                                 set_write_loc.dot(Field::pipeline),
            ...
            if (initial_pipeline->fragment_shader_state &&
                initial_pipeline->fragment_shader_state->fragment_entry_point) {
            ...
            if (update_pipeline->fragment_shader_state &&
                update_pipeline->fragment_shader_state->fragment_entry_point) {
            ...
            if (initial_pipeline_frag_depth != ies_pipeline_frag_depth) {
                LogObjectList objlist(initial_pipeline->Handle(), update_pipeline->Handle());
                skip |= LogError("VUID-vkUpdateIndirectExecutionSetPipelineEXT-initialPipeline-11098",
                                 objlist,
                                 set_write_loc.dot(Field::pipeline),
                                 "was %screated with FragDepth, but the initialPipeline was %screated with FragDepth.",
                                 ies_pipeline_frag_depth ? "" : "not ",
                                 initial_pipeline_frag_depth ? "" : "not ");
            }
            if (initial_pipeline_sample_mask != ies_pipeline_sample_mask) {
                LogObjectList objlist(initial_pipeline->Handle(), update_pipeline->Handle());
                skip |=
                    LogError("VUID-vkUpdateIndirectExecutionSetPipelineEXT-initialPipeline-11086",
                             objlist,
                             set_write_loc.dot(Field::pipeline),
                             "was %screated with SampleMask, but the initialPipeline was %screated with SampleMask.",
                             ies_pipeline_sample_mask ? "" : "not ",
                             initial_pipeline_sample_mask ? "" : "not ");
            }
            if (initial_pipeline_stencil_export != ies_pipeline_stencil_export) {
                ...
                skip |= LogError(
                    ...
                    ies_pipeline_stencil_export ? "" : "not ",
                    initial_pipeline_stencil_export ? "" : "not ");
            }
            ...
            if (initial_pipeline_layout && update_pipeline_layout) {
                ...
                for (uint32_t set = 0; set < set_count; set++) {
                    if (!IsPipelineLayoutSetCompatible(
                            set, initial_pipeline_layout.get(), update_pipeline_layout.get())) {
                        LogObjectList objlist(initial_pipeline->Handle(),
                                              initial_pipeline_layout->Handle(),
                                              update_pipeline->Handle(),
                                              update_pipeline_layout->Handle());
                        skip |= LogError("VUID-vkUpdateIndirectExecutionSetPipelineEXT-None-11039",
                                         objlist,
                                         set_write_loc.dot(Field::pipeline),
                                         "%s was created with a layout %s which is not compatible with the "
                                         "initialPipeline layout %s for set "
                                         "%" PRIu32 ".\n%s",
                                         FormatHandle(update_pipeline->VkHandle()).c_str(),
                                         FormatHandle(update_pipeline_layout->VkHandle()).c_str(),
                                         FormatHandle(initial_pipeline_layout->VkHandle()).c_str(),
                                         set,
...
bool CoreChecks::PreCallValidateUpdateIndirectExecutionSetShaderEXT(
    VkDevice device,
    VkIndirectExecutionSetEXT indirectExecutionSet,
    uint32_t executionSetWriteCount,
    ...
    if (!indirect_execution_set->is_shader_objects) {
        skip |= LogError("VUID-vkUpdateIndirectExecutionSetShaderEXT-indirectExecutionSet-11041",
                         indirect_execution_set->Handle(),
                         error_obj.location.dot(Field::indirectExecutionSet),
    ...
    for (uint32_t i = 0; i < executionSetWriteCount; i++) {
        ...
        if (unique_indexes.find(set_shader.index) != unique_indexes.end()) {
            skip |= LogError("VUID-vkUpdateIndirectExecutionSetShaderEXT-pExecutionSetWrites-11043",
                             device,
                             set_write_loc.dot(Field::index),
                             "(%" PRIu32 ") has same value as pExecutionSetWrites[%" PRIu32 "].index",
                             set_shader.index,
                             unique_indexes[set_shader.index]);
        } else {
        ...
        if (set_shader.index >= indirect_execution_set->max_shader_count) {
            skip |= LogError(
                ...
                set_shader.index,
                indirect_execution_set->max_shader_count);
        }
        ...
        if ((update_shader_object->create_info.flags & VK_SHADER_CREATE_INDIRECT_BINDABLE_BIT_EXT) == 0) {
            ...
            skip |= LogError("VUID-VkWriteIndirectExecutionSetShaderEXT-shader-11032",
                             update_shader_object->Handle(),
        ...
        if ((update_shader_object->create_info.stage & indirect_execution_set->shader_stage_flags) == 0) {
            skip |= LogError("VUID-VkWriteIndirectExecutionSetShaderEXT-pInitialShaders-11033",
                             update_shader_object->Handle(),
                             set_write_loc.dot(Field::shader),
                             "was created with %s but none of the VkIndirectExecutionSetShaderInfoEXT::pShaderStages "
                             "contained that stage (%s).",
        ...
        if (initial_fragment_shader_object && update_shader_object->create_info.stage == VK_SHADER_STAGE_FRAGMENT_BIT) {
            if (update_shader_object->entrypoint && initial_fragment_shader_object->entrypoint) {
                ...
                for (const auto* variable :
                     initial_fragment_shader_object->entrypoint->user_defined_interface_variables) {
                    if ((variable->storage_class != spv::StorageClassOutput) || variable->interface_slots.empty()) {
                        continue; // not an output interface
                    }
                ...
                const bool ies_shader_sample_mask =
                    update_shader_object->entrypoint->HasBuiltIn(spv::BuiltInSampleMask);
                const bool ies_shader_stencil_export =
                    update_shader_object->spirv &&
                    update_shader_object->spirv->HasCapability(spv::CapabilityStencilExportEXT);

                if (initial_shader_frag_depth != ies_shader_frag_depth) {
                    LogObjectList objlist(initial_fragment_shader_object->Handle(), update_shader_object->Handle());
                    skip |= LogError("VUID-vkUpdateIndirectExecutionSetShaderEXT-FragDepth-11054",
                                     objlist,
                                     set_write_loc.dot(Field::shader),
                                     "was %screated with FragDepth, but the initial Fragment VkShaderEXT was %screated "
                                     "with FragDepth.",
                                     ies_shader_frag_depth ? "" : "not ",
                                     initial_shader_frag_depth ? "" : "not ");
                }
                if (initial_shader_sample_mask != ies_shader_sample_mask) {
                    LogObjectList objlist(initial_fragment_shader_object->Handle(), update_shader_object->Handle());
                    skip |= LogError("VUID-vkUpdateIndirectExecutionSetShaderEXT-SampleMask-11050",
                                     objlist,
                                     set_write_loc.dot(Field::shader),
                                     "was %screated with SampleMask, but the initial Fragment VkShaderEXT was "
                                     "%screated with SampleMask.",
                                     ies_shader_sample_mask ? "" : "not ",
                                     initial_shader_sample_mask ? "" : "not ");
                }
                if (initial_shader_stencil_export != ies_shader_stencil_export) {
                    ...
                    skip |= LogError(
                        ...
                        "StencilExportEXT.",
                        ies_shader_stencil_export ? "" : "not ",
                        initial_shader_stencil_export ? "" : "not ");
                }
            }
        }
    }
