        ...
        else if (layout_entry.initial_layout != kInvalidLayout)
        {
            if (!ImageLayoutMatches(aspect_mask, expected_layout, layout_entry.initial_layout))
            {
                ...
                if (!((layout_entry.state->aspect_mask & kDepthOrStencil) &&
                      ImageLayoutMatches(
                          layout_entry.state->aspect_mask, expected_layout, layout_entry.initial_layout)))
                {
...
bool CoreChecks::VerifyImageLayoutRange(const vvl::CommandBuffer& cb_state,
                                        const vvl::Image&         image_state,
                                        VkImageAspectFlags        aspect_mask,
                                        VkImageLayout             explicit_layout,
                                        const RangeFactory&       range_factory,
                                        const Location&           loc,
                                        const char*               mismatch_layout_vuid,
                                        bool*                     error) const
{
    bool       skip                  = false;
        ...
        [this, image_layout_registry, &cb_state, &image_state, &layout_check, mismatch_layout_vuid, loc, error](
            ...
            if (!layout_check.Check(state))
            {
                *error                     = true;
                ...
                subres_skip |= LogError(mismatch_layout_vuid,
                                        objlist,
                                        loc,
                                        ...
                                        subres.arrayLayer,
                                        subres.mipLevel,
                                        string_VkImageLayout(layout_check.expected_layout),
                                        layout_check.message,
...
bool CoreChecks::VerifyImageLayoutSubresource(const vvl::CommandBuffer&       cb_state,
                                              const vvl::Image&               image_state,
                                              ...
                                              const char*                     vuid) const
{
    ...
    auto                    range_factory  = [&normalized_isr](const ImageLayoutRegistry& registry) {
        return registry.RangeGen(normalized_isr);
    };
    bool unused_error = false;
    skip |= VerifyImageLayoutRange(
        cb_state, image_state, normalized_isr.aspectMask, explicit_layout, range_factory, loc, vuid, &unused_error);
    return skip;
}

bool CoreChecks::VerifyImageLayout(const vvl::CommandBuffer& cb_state,
                                   ...
                                   bool*                     error) const
{
    ...
    return VerifyImageLayoutRange(cb_state,
                                  *image_view_state.image_state,
                                  image_view_state.create_info.subresourceRange.aspectMask,
                                  explicit_layout,
                                  range_factory,
                                  loc,
                                  mismatch_layout_vuid,
                                  error);
}
                                   ...
                                   bool*                          error) const
{
    ...
    return VerifyImageLayoutRange(
        cb_state, image_state, range.aspectMask, explicit_layout, range_factory, loc, mismatch_layout_vuid, error);
}
...
static GlobalImageLayoutRangeMap* GetLayoutRangeMap(GlobalImageLayoutMap& global_image_layout_map,
                                                    const vvl::Image&     image_state)
{
...
// This validates that the initial layout specified in the command buffer for the IMAGE is the same as the global IMAGE
// layout
bool CoreChecks::ValidateCmdBufImageLayouts(const Location&           loc,
                                            ...
                                            GlobalImageLayoutMap&     global_image_layout_map) const
{
    ...
    for (const auto& [image, image_layout_registry] : cb_state.image_layout_map)
    {
        ...
        sparse_container::parallel_iterator<const GlobalImageLayoutRangeMap> current_layout(
            *overlay_map, *global_range_map, pos->first.begin);
        while (pos != end)
        {
            ...
            if (current_layout->range.empty())
                break; // When we are past the end of data in overlay and global... stop looking
            if (current_layout->pos_A->valid)
            { // pos_A denotes the overlay map in the parallel iterator
                image_layout = current_layout->pos_A->lower_bound->second;
            }
            else if (current_layout->pos_B->valid)
            { // pos_B denotes the global map in the parallel iterator
            ...
            else if (image_layout != initial_layout)
            {
                ...
                if (!matches)
                {
                    ...
                    for (auto index : sparse_container::range_view<decltype(intersected_range)>(intersected_range))
                    {
                        ...
                        skip |= LogError(vuid.image_layout_09600,
                                         objlist,
                                         loc,
                                         "command buffer %s expects %s (subresource: %s) to be in layout %s--instead, "
                                         "current layout is %s.",
...
void CoreChecks::UpdateCmdBufImageLayouts(const vvl::CommandBuffer& cb_state)
{
    for (const auto& [image, image_layout_registry] : cb_state.image_layout_map)
    {
        ...
        if (image_state && image_layout_registry && image_state->GetId() == image_layout_registry->GetImageId())
        {
            ...
            sparse_container::splice(
                *image_state->layout_range_map, image_layout_registry->GetLayoutMap(), GlobalLayoutUpdater());
        }
    }
}
...
bool CoreChecks::ValidateLayoutVsAttachmentDescription(const VkImageLayout             first_layout,
                                                       ...
                                                       const Location&                 layout_loc) const
{
    ...
    // for both loadOp and stencilLoaOp rp2 has it in 1 VU while rp1 has it in 2 VU with half behind Maintenance2
    // extension Each is VUID is below in following order: rp2 -> rp1 with Maintenance2 -> rp1 with no extenstion
    if (attachment_description.loadOp == VK_ATTACHMENT_LOAD_OP_CLEAR)
    {
        ...
        {
            skip |= LogError("VUID-VkRenderPassCreateInfo2-pAttachments-02522",
                             device,
                             layout_loc,
                 ...
                 (first_layout == VK_IMAGE_LAYOUT_DEPTH_READ_ONLY_STENCIL_ATTACHMENT_OPTIMAL))
        {
            skip |= LogError("VUID-VkRenderPassCreateInfo-pAttachments-01566",
                             device,
                             layout_loc,
                             "(%s) is an invalid for pAttachments[%d] (first attachment to have LOAD_OP_CLEAR).",
                             string_VkImageLayout(first_layout),
                             attachment);
                                        ...
                                        (first_layout == VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL)))
        {
            skip |= LogError("VUID-VkRenderPassCreateInfo-pAttachments-00836",
                             device,
                             layout_loc,
                             "(%s) is an invalid for pAttachments[%d] (first attachment to have LOAD_OP_CLEAR).",
                             string_VkImageLayout(first_layout),
    ...
    if (attachment_description.stencilLoadOp == VK_ATTACHMENT_LOAD_OP_CLEAR)
    {
        ...
        {
            skip |= LogError("VUID-VkRenderPassCreateInfo2-pAttachments-02523",
                             device,
                             layout_loc,
                             "(%s) is an invalid for pAttachments[%d] (first attachment to have LOAD_OP_CLEAR).",
                             string_VkImageLayout(first_layout),
                 ...
                 (first_layout == VK_IMAGE_LAYOUT_DEPTH_ATTACHMENT_STENCIL_READ_ONLY_OPTIMAL))
        {
            skip |= LogError("VUID-VkRenderPassCreateInfo-pAttachments-01567",
                             device,
                             layout_loc,
                             "(%s) is an invalid for pAttachments[%d] (first attachment to have LOAD_OP_CLEAR).",
                             string_VkImageLayout(first_layout),
                             attachment);
                                        ...
                                        (first_layout == VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL)))
        {
            skip |= LogError("VUID-VkRenderPassCreateInfo-pAttachments-02511",
                             device,
                             layout_loc,
                             "(%s) is an invalid for pAttachments[%d] (first attachment to have LOAD_OP_CLEAR).",
                             string_VkImageLayout(first_layout),
...
bool CoreChecks::ValidateMultipassRenderedToSingleSampledSampleCount(VkFramebuffer         framebuffer,
                                                                     VkRenderPass          renderpass,
                                                                     vvl::Image&           image_state,
                                                                     ...
                                                                     const Location& rasterization_samples_loc) const
{
    ...
    {
        skip |= GetPhysicalDeviceImageFormatProperties(
            image_state, "VUID-VkRenderPassAttachmentBeginInfo-pAttachments-07010", rasterization_samples_loc);
    }
    if (!(image_state.image_format_properties.sampleCounts & msrtss_samples))
    {
        ...
        skip |= LogError("VUID-VkRenderPassAttachmentBeginInfo-pAttachments-07010",
                         objlist,
                         ...
                         "usage: %s\n"
                         "flags: %s\n",
                         string_VkSampleCountFlagBits(msrtss_samples),
                         FormatHandle(image_state).c_str(),
                         string_VkFormat(image_create_info.format),
                         string_VkImageType(image_create_info.imageType),
                                                                      ...
                                                                      const Location& attachment_reference_loc) const
{
    ...
    {
        vuid = use_rp2 ? "VUID-vkCmdBeginRenderPass2-initialLayout-03094"
    ...
    {
        vuid = use_rp2 ? "VUID-vkCmdBeginRenderPass2-initialLayout-03097"
    ...
    {
        vuid = use_rp2 ? "VUID-vkCmdBeginRenderPass2-initialLayout-03098"
    ...
    {
        vuid = use_rp2 ? "VUID-vkCmdBeginRenderPass2-initialLayout-03096"
    ...
    {
        vuid = use_rp2 ? "VUID-vkCmdBeginRenderPass2-initialLayout-09538"
        ...
        skip = true;
             ...
             !(image_usage & VK_IMAGE_USAGE_DEPTH_STENCIL_ATTACHMENT_BIT))
    {
        vuid = use_rp2 ? "VUID-vkCmdBeginRenderPass2-initialLayout-02844"
    ...
    if (skip)
    {
        ...
        return LogError(vuid,
                        objlist,
                        rp_loc,
                        "(%s) was created with %s = %s, but %s pAttachments[%" PRIu32 "] (%s) usage is %s%s.",
                        FormatHandle(renderpass).c_str(),
                        attachment_reference_loc.Fields().c_str(),
                        string_VkImageLayout(layout),
                        FormatHandle(framebuffer).c_str(),
                        attachment_index,
                        FormatHandle(image_view).c_str(),
                        string_VkImageUsageFlags(image_usage).c_str(),
                        stencil_usage_message.str().c_str());
    }
                                                       ...
                                                       const Location&              rp_begin_loc) const
{
    ...
    for (uint32_t i = 0; i < render_pass_info->attachmentCount && i < framebuffer_info.attachmentCount; ++i)
    {
        ...
        if (!view_state)
        {
            ...
            skip |= LogError("VUID-VkRenderPassBeginInfo-framebuffer-parameter",
                             objlist,
                             attachment_loc,
                             "%s is invalid.",
                             FormatHandle(image_view).c_str());
        ...
        if (const auto* attachment_description_stencil_layout =
                vku::FindStructInPNextChain<VkAttachmentDescriptionStencilLayout>(
                    render_pass_info->pAttachments[i].pNext);
        ...
        for (uint32_t aspect_index = 0; aspect_index < 32; aspect_index++)
        {
            ...
            skip |= image_layout_registry->AnyInRange(
                ...
                [this,
                 ...
                 framebuffer = framebuffer_state.Handle(),
                 image       = view_state->image_state->Handle(),
                 image_view  = view_state->Handle(),
                 attachment_loc,
                 rp_begin_loc](const LayoutRange& range, const LayoutEntry& state) {
                    ...
                    if (!layout_check.Check(state))
                    {
                        ...
                        subres_skip |= LogError(vuid,
                                                objlist,
                                                attachment_loc,
                                                "You cannot start a render pass using attachment %" PRIu32
                                                " where the render pass initial layout is %s and the %s layout of the "
                                                "attachment is %s. The layouts "
                                                "must match, or the render pass initial layout for the attachment must "
        ...
        skip |= ValidateRenderPassLayoutAgainstFramebufferImageUsage(attachment_initial_layout,
                                                                     *view_state,
                                                                     framebuffer,
                                                                     render_pass,
                                                                     i,
                                                                     rp_loc,
                                                                     attachment_loc.dot(Field::initialLayout));

        skip |= ValidateRenderPassLayoutAgainstFramebufferImageUsage(attachment_final_layout,
                                                                     *view_state,
                                                                     framebuffer,
                                                                     render_pass,
                                                                     i,
                                                                     rp_loc,
                                                                     attachment_loc.dot(Field::finalLayout));

        if (attachment_desc_stencil_layout != nullptr)
        {
            skip |= ValidateRenderPassStencilLayoutAgainstFramebufferImageUsage(
                attachment_desc_stencil_layout->stencilInitialLayout,
                *view_state,
                framebuffer,
                render_pass,
                attachment_loc.pNext(Struct::VkAttachmentDescriptionStencilLayout, Field::stencilInitialLayout));
            skip |= ValidateRenderPassStencilLayoutAgainstFramebufferImageUsage(
                attachment_desc_stencil_layout->stencilFinalLayout,
                *view_state,
                framebuffer,
                render_pass,
    ...
    for (uint32_t j = 0; j < render_pass_info->subpassCount; ++j)
    {
        ...
        const auto*    ms_rendered_to_single_sampled =
            vku::FindStructInPNextChain<VkMultisampledRenderToSingleSampledInfoEXT>(
                render_pass_info->pSubpasses[j].pNext);
        for (uint32_t k = 0; k < render_pass_info->pSubpasses[j].inputAttachmentCount; ++k)
        {
            ...
            {
                skip |= ValidateRenderPassLayoutAgainstFramebufferImageUsage(attachment_ref.layout,
                                                                             *view_state,
                                                                             framebuffer,
                                                                             render_pass,
                ...
                if (ms_rendered_to_single_sampled &&
                    ms_rendered_to_single_sampled->multisampledRenderToSingleSampledEnable)
                {
                    if (render_pass_info->pAttachments[attachment_ref.attachment].samples == VK_SAMPLE_COUNT_1_BIT)
                    {
                        skip |= ValidateMultipassRenderedToSingleSampledSampleCount(
                            ...
                            *view_state->image_state,
                            ...
                            subpass_loc.pNext(Struct::VkMultisampledRenderToSingleSampledInfoEXT,
...
void CoreChecks::TransitionAttachmentRefLayout(vvl::CommandBuffer&                     cb_state,
                                               const vku::safe_VkAttachmentReference2& ref)
{
...
void CoreChecks::TransitionSubpassLayouts(vvl::CommandBuffer&    cb_state,
                                          const vvl::RenderPass& render_pass_state,
...
void CoreChecks::TransitionBeginRenderPassLayouts(vvl::CommandBuffer&    cb_state,
                                                  const vvl::RenderPass& render_pass_state)
{
    ...
    for (uint32_t i = 0; i < rpci->attachmentCount; ++i)
    {
        ...
        {
            // If layoutStencil is kInvalidLayout (meaning no separate depth/stencil layout), image view format has both
            // depth and stencil aspects, and subresource has only one of aspect out of depth or stencil, then the
            // missing aspect will also be transitioned and thus must be included explicitly
...
bool CoreChecks::VerifyClearImageLayout(const vvl::CommandBuffer&      cb_state,
                                        const vvl::Image&              image_state,
                                        const VkImageSubresourceRange& range,
                                        ...
                                        const Location&                loc) const
{
    ...
    if (loc.function == Func::vkCmdClearDepthStencilImage)
    {
        if ((dest_image_layout != VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL) &&
            (dest_image_layout != VK_IMAGE_LAYOUT_GENERAL))
        {
            LogObjectList objlist(cb_state.Handle(), image_state.Handle());
            skip |= LogError("VUID-vkCmdClearDepthStencilImage-imageLayout-00012",
                             objlist,
                             loc,
    ...
    {
        if ((dest_image_layout != VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL) &&
            (dest_image_layout != VK_IMAGE_LAYOUT_GENERAL) && (dest_image_layout != VK_IMAGE_LAYOUT_SHARED_PRESENT_KHR))
        {
            LogObjectList objlist(cb_state.Handle(), image_state.Handle());
            skip |= LogError(
    ...
    if (image_layout_registry)
    {
        ...
        // IncrementInterval skips over all the subresources that have the same state as we just checked, incrementing
        // to the next "constant value" range
        skip |= image_layout_registry->AnyInRange(
            normalized_isr,
...
bool CoreChecks::UpdateCommandBufferImageLayoutMap(const vvl::CommandBuffer&                 cb_state,
                                                   const Location&                           image_loc,
                                                   ...
                                                   vvl::CommandBuffer::ImageLayoutMap&       layout_updates) const
{
    ...
    for (uint32_t aspect_index = 0; aspect_index < 32; aspect_index++)
    {
        ...
        skip |= read_subresource_map->AnyInRange(
            normalized_isr,
            [this, read_subresource_map, &cb_state, &layout_check, &image_loc, &img_barrier](const LayoutRange& range,
                                                                                             const LayoutEntry& state) {
                bool subres_skip = false;
                if (!layout_check.Check(state))
                {
                    ...
                    subres_skip = LogError(vuid,
                                           objlist,
                                           image_loc,
                                           "(%s) cannot transition the layout of aspect=%" PRIu32 ", level=%" PRIu32
                                           ", layer=%" PRIu32 " from %s when the "
                                           "%s layout is %s.",
...
void CoreChecks::RecordTransitionImageLayout(vvl::CommandBuffer& cb_state, const ImageBarrier& mem_barrier)
{
    ...
    VkImageLayout initial_layout =
        NormalizeSynchronization2Layout(mem_barrier.subresourceRange.aspectMask, mem_barrier.oldLayout);
    VkImageLayout new_layout =
        NormalizeSynchronization2Layout(mem_barrier.subresourceRange.aspectMask, mem_barrier.newLayout);
...
void CoreChecks::TransitionImageLayouts(vvl::CommandBuffer&          cb_state,
...
bool CoreChecks::IsCompliantSubresourceRange(const VkImageSubresourceRange& subres_range,
                                             const vvl::Image&              image_state) const
{
    if (!(subres_range.layerCount) || !(subres_range.levelCount))
        return false;
    if (subres_range.baseMipLevel + subres_range.levelCount > image_state.create_info.mipLevels)
        return false;
    if ((subres_range.baseArrayLayer + subres_range.layerCount) > image_state.create_info.arrayLayers)
    {
        return false;
    }
    if (!VerifyAspectsPresent(subres_range.aspectMask, image_state.create_info.format))
        return false;
    if (((vkuFormatPlaneCount(image_state.create_info.format) < 3) &&
         (subres_range.aspectMask & VK_IMAGE_ASPECT_PLANE_2_BIT)) ||
        ((vkuFormatPlaneCount(image_state.create_info.format) < 2) &&
         (subres_range.aspectMask & VK_IMAGE_ASPECT_PLANE_1_BIT)))
    {
        return false;
    }
...
bool CoreChecks::ValidateHostCopyCurrentLayout(const VkImageLayout             expected_layout,
...
bool CoreChecks::ValidateHostCopyCurrentLayout(const VkImageLayout            expected_layout,
                                               const VkImageSubresourceRange& validate_range,
                                               const vvl::Image&              image_state,
                                               const Location&                loc) const
{
    bool skip = false;
    if (disabled[image_layout_validation])
        return false;
    if (!(image_state.layout_range_map))
        return false;
    ...
    if (!IsCompliantSubresourceRange(subres_range, image_state))
        return false;
    ...
    struct CheckState
    {
        ...
        CheckState(VkImageLayout expected_layout_, VkImageAspectFlags aspect_mask_) :
            expected_layout(expected_layout_), aspect_mask(aspect_mask_), found_range({ 0, 0 }),
    ...
    if (check_state.found_range.non_empty())
    {
        const VkImageSubresource subres =
            image_state.subresource_encoder.IndexToVkSubresource(check_state.found_range.begin);
        skip |= LogError(vvl::GetImageImageLayoutVUID(loc),
                         image_state.Handle(),
                         loc,
                         "is currently %s but expected to be %s for %s (subresource: %s)",
                         string_VkImageLayout(check_state.found_layout),
                         string_VkImageLayout(expected_layout),
                         debug_report->FormatHandle(image_state.Handle()).c_str(),
                         string_VkImageSubresource(subres).c_str());
    }
    return skip;
}
