...
// Helper function to validate usage flags for buffers. For given buffer_state send actual vs. desired usage off to
// helper above where an error will be flagged if usage is not correct
bool CoreChecks::ValidateBufferUsageFlags(const LogObjectList& objlist,
                                          vvl::Buffer const& buffer_state,
                                          VkBufferUsageFlags desired,
                                          bool strict,
                                          const char* vuid,
    ...
    if (!correct_usage) {
        skip |= LogError(vuid,
                         objlist,
                         buffer_loc,
                         "(%s) was created with %s but requires %s.",
...
bool CoreChecks::ValidateBufferViewRange(const vvl::Buffer& buffer_state,
    ...
    if (range != VK_WHOLE_SIZE) {
        ...
        if (range <= 0) {
            skip |= LogError("VUID-VkBufferViewCreateInfo-range-00928",
                             buffer_state.Handle(),
                             loc.dot(Field::range),
                             "(%" PRIuLEAST64 ") does not equal VK_WHOLE_SIZE, range must be greater than 0.",
                             range);
        }
        // Range must be a multiple of the element size of format
        if (SafeModulo(range, format_info.block_size) != 0) {
            skip |= LogError("VUID-VkBufferViewCreateInfo-range-00929",
                             buffer_state.Handle(),
                             loc.dot(Field::range),
                             "(%" PRIuLEAST64
        ...
        if (texels > static_cast<VkDeviceSize>(phys_dev_props.limits.maxTexelBufferElements)) {
            skip |= LogError("VUID-VkBufferViewCreateInfo-range-00930",
                             buffer_state.Handle(),
                             loc.dot(Field::range),
                             "(%" PRIuLEAST64 "), %s texel block size (%" PRIu32
                             "), and texels-per-block (%" PRIuLEAST64 ") is a total of (%" PRIuLEAST64
        ...
        if (range + create_info.offset > buffer_state.create_info.size) {
            skip |= LogError("VUID-VkBufferViewCreateInfo-offset-00931",
                             buffer_state.Handle(),
                             loc.dot(Field::range),
        ...
        if (texels > static_cast<VkDeviceSize>(phys_dev_props.limits.maxTexelBufferElements)) {
            skip |= LogError("VUID-VkBufferViewCreateInfo-range-04059",
                             buffer_state.Handle(),
                             loc.dot(Field::range),
...
bool CoreChecks::ValidateBufferViewBuffer(const vvl::Buffer& buffer_state,
                                          const VkBufferViewCreateInfo& create_info,
                                          const Location& loc) const {
    bool skip = false;
    const VkFormat format = create_info.format;
    ...
    if ((usage & VK_BUFFER_USAGE_UNIFORM_TEXEL_BUFFER_BIT) &&
        ...
        skip |= LogError("VUID-VkBufferViewCreateInfo-format-08778",
                         buffer_state.Handle(),
    ...
    if ((usage & VK_BUFFER_USAGE_STORAGE_TEXEL_BUFFER_BIT) &&
        ...
        skip |= LogError("VUID-VkBufferViewCreateInfo-format-08779",
                         buffer_state.Handle(),
                         loc.dot(Field::buffer),
...
bool CoreChecks::ValidateCreateBufferBufferDeviceAddress(const VkBufferCreateInfo& create_info,
    ...
    if (auto chained_devaddr_struct =
            vku::FindStructInPNextChain<VkBufferDeviceAddressCreateInfoEXT>(create_info.pNext)) {
        if (!(create_info.flags & VK_BUFFER_CREATE_DEVICE_ADDRESS_CAPTURE_REPLAY_BIT) &&
            chained_devaddr_struct->deviceAddress != 0) {
            skip |=
                LogError("VUID-VkBufferCreateInfo-deviceAddress-02604",
                         device,
    ...
    if (auto chained_opaqueaddr_struct =
            vku::FindStructInPNextChain<VkBufferOpaqueCaptureAddressCreateInfo>(create_info.pNext)) {
        if (!(create_info.flags & VK_BUFFER_CREATE_DEVICE_ADDRESS_CAPTURE_REPLAY_BIT) &&
...
bool CoreChecks::ValidateCreateBufferDescriptorBuffer(const VkBufferCreateInfo& create_info,
    ...
    if (usage & VK_BUFFER_USAGE_SAMPLER_DESCRIPTOR_BUFFER_BIT_EXT) {
        if (create_info.size + samplerDescriptorBufferAddressSpaceSize >
            ...
            skip |= LogError(
                ...
                device,
                create_info_loc.dot(Field::size),
                ...
                ").",
                create_info.size,
                samplerDescriptorBufferAddressSpaceSize.load(),
                phys_dev_ext_props.descriptor_buffer_props.samplerDescriptorBufferAddressSpaceSize);
            ...
            skip |= LogError(
                ...
                create_info.size,
                descriptorBufferAddressSpaceSize.load(),
                phys_dev_ext_props.descriptor_buffer_props.descriptorBufferAddressSpaceSize);
        }
    }
    ...
    if (usage & VK_BUFFER_USAGE_RESOURCE_DESCRIPTOR_BUFFER_BIT_EXT) {
        if (create_info.size + resourceDescriptorBufferAddressSpaceSize >
            ...
            skip |= LogError("VUID-VkBufferCreateInfo-usage-08098",
                             device,
                             create_info_loc.dot(Field::size),
                             "(%" PRIuLEAST64 ") + current total (%" PRIuLEAST64
                             ") is greater than specified in properties field resourceDescriptorBufferAddressSpaceSize "
                             "(%" PRIuLEAST64 ").",
                             create_info.size,
                             resourceDescriptorBufferAddressSpaceSize.load(),
                             phys_dev_ext_props.descriptor_buffer_props.resourceDescriptorBufferAddressSpaceSize);
        } else if (create_info.size + descriptorBufferAddressSpaceSize >
                   phys_dev_ext_props.descriptor_buffer_props.descriptorBufferAddressSpaceSize) {
            skip |= LogError(
    ...
    if (vku::FindStructInPNextChain<VkOpaqueCaptureDescriptorDataCreateInfoEXT>(create_info.pNext)) {
        if (!(create_info.flags & VK_BUFFER_CREATE_DESCRIPTOR_BUFFER_CAPTURE_REPLAY_BIT_EXT)) {
            skip |= LogError("VUID-VkBufferCreateInfo-pNext-08100",
                             device,
                             create_info_loc.dot(Field::flags),
    ...
    if (usage & VK_BUFFER_USAGE_PUSH_DESCRIPTORS_DESCRIPTOR_BUFFER_BIT_EXT) {
        if (!enabled_features.descriptorBufferPushDescriptors) {
            skip |= LogError("VUID-VkBufferCreateInfo-usage-08101",
                             device,
                             create_info_loc.dot(Field::usage),
        ...
        if (!(usage & (VK_BUFFER_USAGE_RESOURCE_DESCRIPTOR_BUFFER_BIT_EXT |
            ...
            skip |= LogError("VUID-VkBufferCreateInfo-usage-08103",
                             device,
                             create_info_loc.dot(Field::usage),
                             "is (%s).",
                             string_VkBufferUsageFlags2(usage).c_str());
...
bool CoreChecks::PreCallValidateCreateBuffer(VkDevice device,
    ...
    auto dedicated_allocation_buffer =
        vku::FindStructInPNextChain<VkDedicatedAllocationBufferCreateInfoNV>(pCreateInfo->pNext);
    if (dedicated_allocation_buffer && dedicated_allocation_buffer->dedicatedAllocation == VK_TRUE) {
        if (pCreateInfo->flags & (VK_BUFFER_CREATE_SPARSE_BINDING_BIT | VK_BUFFER_CREATE_SPARSE_RESIDENCY_BIT |
            ...
            skip |= LogError("VUID-VkBufferCreateInfo-pNext-01571",
                             device,
                             create_info_loc.dot(Field::flags),
    ...
    if (pCreateInfo->sharingMode == VK_SHARING_MODE_CONCURRENT && pCreateInfo->pQueueFamilyIndices) {
        skip |= ValidatePhysicalDeviceQueueFamilies(pCreateInfo->queueFamilyIndexCount,
    ...
    if ((pCreateInfo->flags & VK_BUFFER_CREATE_PROTECTED_BIT) != 0) {
        ...
        const VkBufferCreateFlags invalid_flags = VK_BUFFER_CREATE_SPARSE_BINDING_BIT |
                                                  VK_BUFFER_CREATE_SPARSE_RESIDENCY_BIT |
                                                  VK_BUFFER_CREATE_SPARSE_ALIASED_BIT;
        if ((pCreateInfo->flags & invalid_flags) != 0) {
            skip |= LogError("VUID-VkBufferCreateInfo-None-01888",
                             device,
                             create_info_loc.dot(Field::flags),
    ...
    const bool has_decode_usage =
        usage & (VK_BUFFER_USAGE_VIDEO_DECODE_SRC_BIT_KHR | VK_BUFFER_USAGE_VIDEO_DECODE_DST_BIT_KHR);
    const bool has_encode_usage =
        usage & (VK_BUFFER_USAGE_VIDEO_ENCODE_SRC_BIT_KHR | VK_BUFFER_USAGE_VIDEO_ENCODE_DST_BIT_KHR);
    ...
    if (video_profile_independent && !enabled_features.videoMaintenance1) {
        skip |= LogError("VUID-VkBufferCreateInfo-flags-08325",
                         device,
                         create_info_loc.dot(Field::flags),
    ...
    if (has_decode_usage || has_encode_usage) {
        ...
        skip |= ValidateVideoProfileListInfo(video_profiles,
                                             device,
                                             create_info_loc.pNext(Struct::VkVideoProfileListInfoKHR),
                                             expect_decode_profile,
                                             "VUID-VkBufferCreateInfo-usage-04813",
                                             expect_encode_profile,
    ...
    if (external_memory_info && external_memory_info->handleTypes) {
        ...
        DispatchGetPhysicalDeviceExternalBufferPropertiesHelper(
        ...
        if ((external_memory_info->handleTypes & compatible_types) != external_memory_info->handleTypes) {
            skip |= LogError("VUID-VkBufferCreateInfo-pNext-00920",
                             device,
    ...
    if (enabled_features.maintenance4 && pCreateInfo->size > phys_dev_props_core13.maxBufferSize) {
        skip |= LogError("VUID-VkBufferCreateInfo-size-06409",
                         device,
                         create_info_loc.dot(Field::size),
                         "(%" PRIu64 ") is larger than the maximum allowed buffer size "
                         "VkPhysicalDeviceMaintenance4Properties.maxBufferSize (%" PRIu64 ").",
                         pCreateInfo->size,
                         phys_dev_props_core13.maxBufferSize);
    }
...
bool CoreChecks::PreCallValidateCreateBufferView(VkDevice device,
    ...
    if (vkuFormatIsDepthOrStencil(pCreateInfo->format)) {
        ...
        skip |= LogError("UNASSIGNED-VkBufferViewCreateInfo-depthStencil-format",
                         device,
    ...
    skip |= ValidateMemoryIsBoundToBuffer(
        device, buffer_state, create_info_loc.dot(Field::buffer), "VUID-VkBufferViewCreateInfo-buffer-00935");
    // In order to create a valid buffer view, the buffer must have been created with at least one of the following
    // flags: UNIFORM_TEXEL_BUFFER_BIT or STORAGE_TEXEL_BUFFER_BIT
    skip |=
        ValidateBufferUsageFlags(objlist,
                                 buffer_state,
    ...
    if (pCreateInfo->offset >= buffer_state.create_info.size) {
        skip |= LogError("VUID-VkBufferViewCreateInfo-offset-00925",
                         buffer_state.Handle(),
                         create_info_loc.dot(Field::offset),
                         "(%" PRIuLEAST64 ") must be less than the size of the buffer (%" PRIuLEAST64 ").",
                         pCreateInfo->offset,
                         buffer_state.create_info.size);
    }
    ...
    if ((pCreateInfo->offset % phys_dev_props.limits.minTexelBufferOffsetAlignment) != 0 &&
        ...
        skip |= LogError("VUID-VkBufferViewCreateInfo-offset-02749",
                         objlist,
                         create_info_loc.dot(Field::offset),
                         "(%" PRIuLEAST64
    ...
    if (enabled_features.texelBufferAlignment) {
        ...
        if (buffer_state.usage & VK_BUFFER_USAGE_STORAGE_TEXEL_BUFFER_BIT) {
            ...
            if (SafeModulo(pCreateInfo->offset, alignment_requirement) != 0) {
                skip |= LogError("VUID-VkBufferViewCreateInfo-buffer-02750",
                                 objlist,
                                 create_info_loc,
                                 "If buffer was created with usage containing "
                                 "VK_BUFFER_USAGE_STORAGE_TEXEL_BUFFER_BIT, "
                                 "VkBufferViewCreateInfo offset (%" PRIuLEAST64 ") must be a multiple of the lesser of "
                                 "storageTexelBufferOffsetAlignmentBytes (%" PRIuLEAST64
                                 ") or, if storageTexelBufferOffsetSingleTexelAlignment "
                                 "(%" PRId32 ") is VK_TRUE, the size of a texel of the requested format. "
        ...
        if (buffer_state.usage & VK_BUFFER_USAGE_UNIFORM_TEXEL_BUFFER_BIT) {
            ...
            if (SafeModulo(pCreateInfo->offset, alignment_requirement) != 0) {
                skip |= LogError("VUID-VkBufferViewCreateInfo-buffer-02751",
                                 objlist,
                                 create_info_loc,
                                 "If buffer was created with usage containing "
    ...
    if (auto buffer_usage_flags2 = vku::FindStructInPNextChain<VkBufferUsageFlags2CreateInfo>(pCreateInfo->pNext)) {
        ...
        if ((usage & ~(VK_BUFFER_USAGE_2_UNIFORM_TEXEL_BUFFER_BIT | VK_BUFFER_USAGE_2_STORAGE_TEXEL_BUFFER_BIT)) != 0) {
            skip |= LogError("VUID-VkBufferViewCreateInfo-pNext-08780",
                             objlist,
                             create_info_loc.pNext(Struct::VkBufferUsageFlags2CreateInfo, Field::usage),
                             "is %s.",
                             string_VkBufferUsageFlags2(usage).c_str());
        } else if ((usage & buffer_state.usage) != usage) {
            skip |= LogError("VUID-VkBufferViewCreateInfo-pNext-08781",
                             objlist,
                             create_info_loc.pNext(Struct::VkBufferUsageFlags2CreateInfo, Field::usage),
                             "(%s) is not a subset of the buffer's usage (%s).",
                             string_VkBufferUsageFlags2(usage).c_str(),
                             string_VkBufferUsageFlags2(buffer_state.usage).c_str());
        }
    }
...
bool CoreChecks::PreCallValidateDestroyBuffer(VkDevice device,
                                              VkBuffer buffer,
                                              const VkAllocationCallbacks* pAllocator,
                                              const ErrorObject& error_obj) const {
    bool skip = false;
...
bool CoreChecks::PreCallValidateDestroyBufferView(VkDevice device,
    ...
    if (auto buffer_view_state = Get<vvl::BufferView>(bufferView)) {
        skip |= ValidateObjectNotInUse(
            buffer_view_state.get(), error_obj.location, "VUID-vkDestroyBufferView-bufferView-00936");
    }
    return skip;
}

bool CoreChecks::PreCallValidateCmdFillBuffer(VkCommandBuffer commandBuffer,
                                              VkBuffer dstBuffer,
    ...
    skip |=
        ValidateMemoryIsBoundToBuffer(commandBuffer, *buffer_state, buffer_loc, "VUID-vkCmdFillBuffer-dstBuffer-00031");
    ...
    if (dstOffset >= buffer_state->create_info.size) {
        skip |= LogError("VUID-vkCmdFillBuffer-dstOffset-00024",
                         objlist,
                         error_obj.location.dot(Field::dstOffset),
                         "(%" PRIu64 ") is not less than destination buffer (%s) size (%" PRIu64 ").",
                         dstOffset,
                         FormatHandle(dstBuffer).c_str(),
                         buffer_state->create_info.size);
    }

    if ((size != VK_WHOLE_SIZE) && (size > (buffer_state->create_info.size - dstOffset))) {
        skip |=
            LogError("VUID-vkCmdFillBuffer-size-00027",
                     objlist,
                     error_obj.location.dot(Field::size),
                     "(%" PRIu64 ") is greater than dstBuffer (%s) size (%" PRIu64 ") minus dstOffset (%" PRIu64 ").",
                     size,
                     FormatHandle(dstBuffer).c_str(),
                     buffer_state->create_info.size,
                     dstOffset);
    ...
    if (!IsExtEnabled(device_extensions.vk_khr_maintenance1)) {
        ...
        if (!HasRequiredQueueFlags(cb_state, *physical_device_state, required_flags)) {
            ...
            skip |= LogError("VUID-vkCmdFillBuffer-apiVersion-07894",
                             objlist_pool,
                             error_obj.location,
                             "%s",
...
bool CoreChecks::ValidateProtectedBuffer(const vvl::CommandBuffer& cb_state,
                                         const vvl::Buffer& buffer_state,
                                         const Location& buffer_loc,
                                         const char* vuid,
    ...
    if ((!phys_dev_props_core11.protectedNoFault) && (cb_state.unprotected == true) &&
        (buffer_state.unprotected == false)) {
...
bool CoreChecks::ValidateUnprotectedBuffer(const vvl::CommandBuffer& cb_state,
                                           const vvl::Buffer& buffer_state,
                                           const Location& buffer_loc,
                                           const char* vuid,
                                           const char* more_message) const {
