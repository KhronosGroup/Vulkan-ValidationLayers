...
static QueryState
GetLocalQueryState(const QueryMap* localQueryToStateMap, VkQueryPool queryPool, uint32_t queryIndex, uint32_t perfPass)
{
    QueryObject query = QueryObject(queryPool, queryIndex, perfPass);

    auto iter = localQueryToStateMap->find(query);
    if (iter != localQueryToStateMap->end())
        return iter->second;

    return QUERYSTATE_UNKNOWN;
}

bool CoreChecks::PreCallValidateDestroyQueryPool(VkDevice                     device,
                                                 ...
                                                 const ErrorObject&           error_obj) const
{
    ...
    if (!completed_by_get_results)
    {
        skip |= ValidateObjectNotInUse(
            query_pool_state.get(), error_obj.location, "VUID-vkDestroyQueryPool-queryPool-00793");
    }
    return skip;
}

bool CoreChecks::ValidatePerformanceQueryResults(const vvl::QueryPool& query_pool_state,
                                                 uint32_t              firstQuery,
                                                 uint32_t              queryCount,
                                                 ...
                                                 const Location&       loc) const
{
    ...
    if (flags & (VK_QUERY_RESULT_WITH_AVAILABILITY_BIT | VK_QUERY_RESULT_WITH_STATUS_BIT_KHR |
                 VK_QUERY_RESULT_PARTIAL_BIT | VK_QUERY_RESULT_64_BIT))
    {
        std::string invalid_flags_string;
        for (auto flag : { VK_QUERY_RESULT_WITH_AVAILABILITY_BIT,
        ...
        const char* vuid = loc.function == Func::vkGetQueryPoolResults
                               ? "VUID-vkGetQueryPoolResults-queryType-09440"
                               : "VUID-vkCmdCopyQueryPoolResults-queryType-09440";
        skip |= LogError(vuid,
                         query_pool_state.Handle(),
                         loc.dot(Field::queryPool),
...
bool CoreChecks::ValidateQueryPoolWasReset(const vvl::QueryPool& query_pool_state,
                                           uint32_t              firstQuery,
                                           uint32_t              queryCount,
                                           const Location&       loc,
...
bool CoreChecks::PreCallValidateGetQueryPoolResults(VkDevice           device,
                                                    VkQueryPool        queryPool,
                                                    uint32_t           firstQuery,
                                                    uint32_t           queryCount,
                                                    size_t             dataSize,
                                                    ...
                                                    const ErrorObject& error_obj) const
{
    ...
    {
        skip |= LogError("VUID-vkGetQueryPoolResults-queryCount-09438",
                         queryPool,
                         error_obj.location.dot(Field::queryCount),
                         "is %" PRIu32 " but stride is zero.",
                         queryCount);
    }
    ...
    skip |= ValidateQueryPoolIndex(device,
                                   *query_pool_state,
                                   firstQuery,
                                   queryCount,
                                   error_obj.location,
    ...
    {
        skip |= ValidateQueryPoolStride("VUID-vkGetQueryPoolResults-flags-02828",
    ...
    {
        skip |=
            LogError("VUID-vkGetQueryPoolResults-queryType-09439",
                     queryPool,
                     error_obj.location.dot(Field::flags),
                     "(%s) includes VK_QUERY_RESULT_PARTIAL_BIT, but queryPool (%s) was created with a queryType of "
                     "VK_QUERY_TYPE_TIMESTAMP.",
                     string_VkQueryResultFlags(flags).c_str(),
    ...
    {
        skip |= LogError("VUID-vkGetQueryPoolResults-queryType-09442",
                         queryPool,
                         error_obj.location.dot(Field::flags),
    ...
    switch (query_pool_state->create_info.queryType)
    {
        ...
        case VK_QUERY_TYPE_PIPELINE_STATISTICS:
            ...
            // pipelineStatistics when the pool is created
            {
        ...
        case VK_QUERY_TYPE_PERFORMANCE_QUERY_KHR:
            ...
            {
                skip |= LogError("VUID-vkGetQueryPoolResults-queryType-04519",
                                 queryPool,
                                 error_obj.location.dot(Field::queryPool),
                                 "(%s) specified stride %" PRIu64 " which must be at least counterIndexCount (%d) "
            ...
            {
                skip |= LogError("VUID-vkGetQueryPoolResults-queryType-03229",
                                 queryPool,
                                 error_obj.location.dot(Field::queryPool),
                                 "(%s) was created with a queryType of "
            ...
            skip |=
                ValidatePerformanceQueryResults(*query_pool_state, firstQuery, queryCount, flags, error_obj.location);
    ...
    {
        skip |= LogError("VUID-vkGetQueryPoolResults-dataSize-00817",
                         queryPool,
                         error_obj.location.dot(Field::queryPool),
...
bool CoreChecks::PreCallValidateCreateQueryPool(VkDevice                     device,
                                                ...
                                                const ErrorObject&           error_obj) const
{
    ...
    switch (pCreateInfo->queryType)
    {
            ...
            if (auto perf_ci = vku::FindStructInPNextChain<VkQueryPoolPerformanceCreateInfoKHR>(pCreateInfo->pNext))
            {
                ...
                if (perf_counter_iter != physical_device_state->perf_counters.end())
                {
                    ...
                    for (uint32_t idx = 0; idx < perf_ci->counterIndexCount; idx++)
                    {
                        ...
                        {
                            skip |=
                                LogError("VUID-VkQueryPoolPerformanceCreateInfoKHR-pCounterIndices-03321",
                                         device,
                                         create_info_loc.pNext(
                                             Struct::VkQueryPoolPerformanceCreateInfoKHR, Field::pCounterIndices, idx),
                                         "(%" PRIu32 ") is not a valid counter index.",
                                         perf_ci->pCounterIndices[idx]);
                        }
                    }
                }
            }
        ...
        case VK_QUERY_TYPE_RESULT_STATUS_ONLY_KHR:
        {
            ...
            {
                skip |= ValidateVideoProfileInfo(
                    video_profile, device, create_info_loc.pNext(Struct::VkVideoProfileInfoKHR));
            }
            break;
        }
        case VK_QUERY_TYPE_VIDEO_ENCODE_FEEDBACK_KHR:
        {
            ...
            {
                skip |= LogError("VUID-VkQueryPoolCreateInfo-queryType-07133",
                                 device,
            ...
            {
                skip |= LogError("VUID-VkQueryPoolCreateInfo-queryType-07906",
                                 device,
                                 create_info_loc.dot(Field::queryType),
                                 pnext_chain_msg,
...
bool CoreChecks::HasRequiredQueueFlags(const vvl::CommandBuffer&  cb_state,
                                                  ...
                                                  VkQueueFlags               required_flags) const
{
    ...
    ss << "called in " << FormatHandle(cb_state) << " which was allocated from the " << FormatHandle(pool->Handle())
       << " which was created with "
          "queueFamilyIndex "
       << queue_family_index << " which contains the capability flags " << string_VkQueueFlags(queue_flags)
       << " (but requires " << required_flags_string << ").";
    return ss.str();
}

bool CoreChecks::ValidateBeginQuery(const vvl::CommandBuffer& cb_state,
                                    const QueryObject&        query_obj,
                                    ...
                                    const Location&           loc) const
{
    ...
    switch (query_pool_ci.queryType)
    {
        case VK_QUERY_TYPE_TIMESTAMP:
        {
            ...
            skip |= LogError(vuid,
                             objlist,
                             loc.dot(Field::queryPool),
                             "(%s) was created with VK_QUERY_TYPE_TIMESTAMP.",
        ...
        case VK_QUERY_TYPE_TRANSFORM_FEEDBACK_STREAM_EXT:
        {
            ...
            {
                const char*         vuid = is_indexed ? "VUID-vkCmdBeginQueryIndexedEXT-queryType-02338"
            ...
            {
                const char*         vuid = is_indexed ? "VUID-vkCmdBeginQueryIndexedEXT-queryType-02341"
        ...
        case VK_QUERY_TYPE_OCCLUSION:
        {
            if (!HasRequiredQueueFlags(cb_state, *physical_device_state, VK_QUEUE_GRAPHICS_BIT))
            {
                const char*         vuid = is_indexed ? "VUID-vkCmdBeginQueryIndexedEXT-queryType-00803"
        ...
        case VK_QUERY_TYPE_PERFORMANCE_QUERY_KHR:
        {
            ...
            {
                const char*         vuid = is_indexed ? "VUID-vkCmdBeginQueryIndexedEXT-queryPool-03223"
            ...
            {
                const char*         vuid = is_indexed ? "VUID-vkCmdBeginQueryIndexedEXT-queryPool-03224"
            ...
            {
                const char*         vuid = is_indexed ? "VUID-vkCmdBeginQueryIndexedEXT-queryPool-03225"
            ...
            {
                const char*         vuid = is_indexed ? "VUID-vkCmdBeginQueryIndexedEXT-queryPool-07289"
                ...
                skip |= LogError(vuid,
                                 ...
                                 query_pool_state->perf_counter_queue_family_index,
        ...
        case VK_QUERY_TYPE_ACCELERATION_STRUCTURE_SERIALIZATION_SIZE_KHR:
        {
            ...
            skip |= LogError(vuid,
                             objlist,
                             loc.dot(Field::queryPool),
                             "(%s) was created with queryType %s.",
                             FormatHandle(query_obj.pool).c_str(),
        ...
        case VK_QUERY_TYPE_PIPELINE_STATISTICS:
        {
            if ((cb_state.command_pool->queue_flags & VK_QUEUE_GRAPHICS_BIT) == 0)
            {
                ...
                {
                    const char*         vuid = is_indexed ? "VUID-vkCmdBeginQueryIndexedEXT-queryType-00804"
                                                          : "VUID-vkCmdBeginQuery-queryType-00804";
                    const LogObjectList objlist(cb_state.Handle(), query_obj.pool);
                    skip |= LogError(vuid,
                                     objlist,
                                     loc.dot(Field::queryPool),
                                     "(%s) was created with queryType VK_QUERY_TYPE_PIPELINE_STATISTICS (%s) and "
                                     "indicates graphics operations, "
                                     "but "
                                     "the command pool the command buffer %s was allocated from does not support "
                                     "graphics operations (%s).",
                                     FormatHandle(query_obj.pool).c_str(),
            ...
            if ((cb_state.command_pool->queue_flags & VK_QUEUE_COMPUTE_BIT) == 0)
            {
                ...
                {
                    const char*         vuid = is_indexed ? "VUID-vkCmdBeginQueryIndexedEXT-queryType-00805"
                    ...
                    skip |= LogError(vuid,
                                     ...
                                     loc.dot(Field::queryPool),
                                     "(%s) was created with queryType VK_QUERY_TYPE_PIPELINE_STATISTICS (%s) and "
                                     "indicates compute operations, "
                                     "but "
                                     "the command pool the command buffer %s was allocated from does not support "
                                     "compute operations (%s).",
                                     FormatHandle(query_obj.pool).c_str(),
                                     string_VkQueryPipelineStatisticFlags(query_pool_ci.pipelineStatistics).c_str(),
                                     FormatHandle(cb_state).c_str(),
                                     string_VkQueueFlags(cb_state.command_pool->queue_flags).c_str());
        ...
        case VK_QUERY_TYPE_PRIMITIVES_GENERATED_EXT:
        {
            if ((cb_state.command_pool->queue_flags & VK_QUEUE_GRAPHICS_BIT) == 0)
            {
                const char*         vuid = is_indexed ? "VUID-vkCmdBeginQueryIndexedEXT-queryType-06689"
                ...
                skip |= LogError(vuid,
                                 ...
                                 "the command pool the command buffer %s was allocated from does not support graphics "
                                 "operations (%s).",
                                 FormatHandle(query_obj.pool).c_str(),
                                 FormatHandle(cb_state).c_str(),
                                 string_VkQueueFlags(cb_state.command_pool->queue_flags).c_str());
        ...
        case VK_QUERY_TYPE_RESULT_STATUS_ONLY_KHR:
        {
            ...
            {
                const char*         vuid = is_indexed ? "VUID-vkCmdBeginQueryIndexedEXT-queryType-07126"
        ...
        case VK_QUERY_TYPE_MESH_PRIMITIVES_GENERATED_EXT:
        {
            ...
            {
                const LogObjectList objlist(cb_state.Handle(), query_obj.pool);
                skip |= LogError("VUID-vkCmdBeginQueryIndexedEXT-queryType-07071",
                                 objlist,
                                 loc.dot(Field::queryPool),
                                 "(%s) was created with queryType %s.",
                                 FormatHandle(query_obj.pool).c_str(),
    ...
    {
        const char* vuid =
            is_indexed ? "VUID-vkCmdBeginQueryIndexedEXT-query-00802" : "VUID-vkCmdBeginQuery-query-00802";
        const LogObjectList objlist(cb_state.Handle(), query_obj.pool);
        skip |= LogError(vuid,
                         objlist,
                         loc,
                         "Query index %" PRIu32 " must be less than query count %" PRIu32 " of %s.",
                         query_obj.slot,
                         query_pool_ci.queryCount,
                         FormatHandle(query_obj.pool).c_str());
    }

    if (cb_state.unprotected == false)
    {
        const char* vuid = is_indexed ? "VUID-vkCmdBeginQueryIndexedEXT-commandBuffer-01885"
    ...
    if (cb_state.bound_video_session)
    {
        ...
        {
            const char* vuid =
                is_indexed ? "VUID-vkCmdBeginQueryIndexedEXT-None-08370" : "VUID-vkCmdBeginQuery-None-08370";
        ...
        {
            const char* vuid =
                is_indexed ? "VUID-vkCmdBeginQueryIndexedEXT-None-07127" : "VUID-vkCmdBeginQuery-None-07127";
            const LogObjectList objlist(cb_state.Handle(), cb_state.bound_video_session->Handle());
            skip |= LogError(vuid,
                             objlist,
                             loc,
        ...
        switch (query_pool_ci.queryType)
        {
            case VK_QUERY_TYPE_RESULT_STATUS_ONLY_KHR:
            {
                ...
                {
                    const char*         vuid = is_indexed ? "VUID-vkCmdBeginQueryIndexedEXT-queryType-07128"
                                                          : "VUID-vkCmdBeginQuery-queryType-07128";
                    const LogObjectList objlist(
                        cb_state.Handle(), query_pool_state->Handle(), cb_state.bound_video_session->Handle());
                    skip |= LogError(vuid,
                                     objlist,
                                     loc,
            ...
            case VK_QUERY_TYPE_VIDEO_ENCODE_FEEDBACK_KHR:
            {
                if (cb_state.bound_video_session->profile != query_pool_state->supported_video_profile)
                {
                    const char*         vuid = is_indexed ? "VUID-vkCmdBeginQueryIndexedEXT-queryType-07130"
            ...
            default:
            {
                const char*         vuid = is_indexed ? "VUID-vkCmdBeginQueryIndexedEXT-queryType-07131"
    ...
    {
        const char* vuid =
            is_indexed ? "VUID-vkCmdBeginQueryIndexedEXT-queryType-07129" : "VUID-vkCmdBeginQuery-queryType-07129";
        const LogObjectList objlist(cb_state.Handle(), query_obj.pool);
        skip |= LogError(vuid,
                         objlist,
                         loc,
...
bool CoreChecks::PreCallValidateCmdBeginQuery(VkCommandBuffer     commandBuffer,
                                              VkQueryPool         queryPool,
                                              uint32_t            slot,
                                              VkQueryControlFlags flags,
                                              const ErrorObject&  error_obj) const
{
    ...
    if (query_pool_state->create_info.queryType == VK_QUERY_TYPE_PRIMITIVES_GENERATED_EXT)
    {
        if (!enabled_features.primitivesGeneratedQuery)
        {
            ...
            skip |= LogError("VUID-vkCmdBeginQuery-queryType-06688",
                             objlist,
                             error_obj.location.dot(Field::queryPool),
...
bool CoreChecks::VerifyQueryIsReset(const vvl::CommandBuffer& cb_state,
                                    const QueryObject&        query_obj,
                                    Func                      command,
                                    ...
                                    QueryMap*                 localQueryToStateMap)
{
    ...
    if (state != QUERYSTATE_RESET)
    {
        ...
        skip |= state_data.LogError(vuid,
                                    objlist,
                                    loc,
                                    "%s and query %" PRIu32 ": query not reset. "
                                    "After query pool creation, each query must be reset (with vkCmdResetQueryPool or "
                                    "vkResetQueryPool) before it is used. "
...
bool CoreChecks::ValidatePerformanceQuery(const vvl::CommandBuffer& cb_state,
                                          const QueryObject&        query_obj,
                                          Func                      command,
                                          VkQueryPool&              firstPerfQueryPool,
                                          uint32_t                  perfPass,
                                          QueryMap*                 localQueryToStateMap)
{
    ...
    {
        const LogObjectList objlist(cb_state.Handle(), query_obj.pool);
        skip |= state_data.LogError("VUID-VkPerformanceQuerySubmitInfoKHR-counterPassIndex-03221",
                                    objlist,
                                    loc,
                                    "Invalid counterPassIndex (%u, maximum allowed %u) value for query pool %s.",
                                    perfPass,
                                    query_pool_state->n_performance_passes,
                                    state_data.FormatHandle(query_obj.pool).c_str());
    }
    ...
    {
        const LogObjectList objlist(cb_state.Handle(), query_obj.pool);
        skip |= state_data.LogError("VUID-vkQueueSubmit-pCommandBuffers-03220",
                                    objlist,
                                    loc,
    ...
    QueryState command_buffer_state =
        GetLocalQueryState(localQueryToStateMap, query_obj.pool, query_obj.slot, perfPass);
    if (command_buffer_state == QUERYSTATE_RESET)
    {
        const LogObjectList objlist(cb_state.Handle(), query_obj.pool);
        skip |= state_data.LogError(query_obj.indexed ? "VUID-vkCmdBeginQueryIndexedEXT-None-02863"
    ...
    if (firstPerfQueryPool != VK_NULL_HANDLE)
    {
        ...
        {
            const LogObjectList objlist(cb_state.Handle(), query_obj.pool);
            skip |= state_data.LogError(query_obj.indexed ? "VUID-vkCmdBeginQueryIndexedEXT-queryPool-03226"
...
void CoreChecks::EnqueueVerifyBeginQuery(VkCommandBuffer command_buffer, const QueryObject& query_obj, Func command)
{
    ...
    // Enqueue the submit time validation here, ahead of the submit time state update in the StateTracker's
    // PostCallRecord
    cb_state->query_updates.emplace_back([query_obj, command](vvl::CommandBuffer& cb_state_arg,
                                                              bool                do_validate,
                                                              VkQueryPool&        firstPerfQueryPool,
                                                              uint32_t            perfPass,
                                                              QueryMap*           localQueryToStateMap) {
        if (!do_validate)
            return false;
        bool skip = false;
        skip |= ValidatePerformanceQuery(
            cb_state_arg, query_obj, command, firstPerfQueryPool, perfPass, localQueryToStateMap);
        skip |=
            VerifyQueryIsReset(cb_state_arg, query_obj, command, firstPerfQueryPool, perfPass, localQueryToStateMap);
        return skip;
    });
}

// Need to enqueue work prior to PostCallRecord
void CoreChecks::PreCallRecordCmdBeginQuery(VkCommandBuffer     commandBuffer,
                                            VkQueryPool         queryPool,
                                            uint32_t            slot,
                                            VkQueryControlFlags flags,
...
void CoreChecks::EnqueueVerifyEndQuery(vvl::CommandBuffer& cb_state, const QueryObject& query_obj, Func command)
{
    // Enqueue the submit time validation here, ahead of the submit time state update in the StateTracker's
    // PostCallRecord
    cb_state.query_updates.emplace_back([this, query_obj, command](vvl::CommandBuffer& cb_state_arg,
                                                                   bool                do_validate,
                                                                   VkQueryPool&        firstPerfQueryPool,
                                                                   uint32_t            perfPass,
                                                                   QueryMap*           localQueryToStateMap) {
        ...
        ASSERT_AND_RETURN_SKIP(query_pool_state);
        if (query_pool_state->has_perf_scope_command_buffer &&
            (cb_state_arg.command_count - 1) != query_obj.end_command_index)
        {
            const LogObjectList objlist(cb_state_arg.Handle(), query_pool_state->Handle());
            const Location      loc(command);
            skip |= LogError("VUID-vkCmdEndQuery-queryPool-03227",
                             objlist,
                             loc,
                             ...
                             FormatHandle(query_obj.pool).c_str(),
                             FormatHandle(cb_state_arg).c_str());
        }
        return skip;
    });
}

bool CoreChecks::ValidateCmdEndQuery(
    const vvl::CommandBuffer& cb_state, VkQueryPool queryPool, uint32_t slot, uint32_t index, const Location& loc) const
{
    bool       skip          = false;
    ...
    if (query_payload == cb_state.activeQueries.end())
    {
        ...
        skip |= LogError(vuid,
                         objlist,
                         loc,
                         "Ending a query before it was started: %s, index %d.",
                         FormatHandle(queryPool).c_str(),
                         slot);
    }
    auto query_pool_state = Get<vvl::QueryPool>(queryPool);
    ...
    if (query_pool_ci.queryType == VK_QUERY_TYPE_PERFORMANCE_QUERY_KHR)
    {
        ...
        {
            const LogObjectList objlist(cb_state.Handle(), queryPool);
            skip |= LogError("VUID-vkCmdEndQuery-queryPool-03228",
                             objlist,
                             loc,
                             "Query pool %s was created with a counter of scope "
                             "VK_PERFORMANCE_COUNTER_SCOPE_RENDER_PASS_KHR but %s is inside a render pass.",
    ...
    if (cb_state.activeRenderPass && (query_payload != cb_state.activeQueries.end()))
    {
        ...
        if (!cb_state.activeRenderPass->UsesDynamicRendering())
        {
            ...
            {
                const char* vuid =
                    is_indexed ? "VUID-vkCmdEndQueryIndexedEXT-None-07007" : "VUID-vkCmdEndQuery-None-07007";
                const LogObjectList objlist(cb_state.Handle(), queryPool, cb_state.activeRenderPass->Handle());
                skip |= LogError(vuid,
                                 objlist,
                                 loc,
                                 "query (%" PRIu32 ") was started in subpass %" PRIu32
                                 ", but ending in subpass %" PRIu32 ".",
                                 slot,
                                 query_payload->subpass,
                                 subpass);
            }
            ...
            if (subpass_desc)
            {
                ...
                {
                    const char* vuid =
                        is_indexed ? "VUID-vkCmdEndQueryIndexedEXT-query-02345" : "VUID-vkCmdEndQuery-query-00812";
                    const LogObjectList objlist(cb_state.Handle(), queryPool, cb_state.activeRenderPass->Handle());
                    skip |=
                        LogError(vuid,
                                 objlist,
...
bool CoreChecks::PreCallValidateCmdEndQuery(VkCommandBuffer    commandBuffer,
                                            VkQueryPool        queryPool,
                                            uint32_t           slot,
                                            const ErrorObject& error_obj) const
{
    ...
    {
        const LogObjectList objlist(commandBuffer, queryPool);
        skip |= LogError("VUID-vkCmdEndQuery-query-00810",
                         objlist,
                         error_obj.location.dot(Field::query),
                         "(%u) is greater or equal to the queryPool size (%u).",
                         slot,
...
void CoreChecks::PreCallRecordCmdEndQuery(VkCommandBuffer     commandBuffer,
                                          VkQueryPool         queryPool,
                                          uint32_t            slot,
                                          const RecordObject& record_obj)
{
...
bool CoreChecks::ValidateQueryPoolIndex(LogObjectList         objlist,
                                        const vvl::QueryPool& query_pool_state,
                                        uint32_t              firstQuery,
                                        uint32_t              queryCount,
                                        const Location&       loc,
                                        const char*           first_vuid,
...
bool CoreChecks::ValidateQueriesNotActive(const vvl::CommandBuffer& cb_state,
                                          VkQueryPool               queryPool,
                                          uint32_t                  firstQuery,
                                          uint32_t                  queryCount,
                                          const Location&           loc,
    ...
    for (uint32_t i = 0; i < queryCount; i++)
    {
        ...
        if (cb_state.activeQueries.count(query_obj))
        {
            ...
            skip |= LogError(vuid,
                             objlist,
                             loc,
                             "Query index %" PRIu32 " is still active (firstQuery = %" PRIu32 ", queryCount = %" PRIu32
                             ").",
                             slot,
                             firstQuery,
                             queryCount);
...
bool CoreChecks::PreCallValidateCmdResetQueryPool(VkCommandBuffer    commandBuffer,
                                                  VkQueryPool        queryPool,
                                                  uint32_t           firstQuery,
                                                  uint32_t           queryCount,
                                                  const ErrorObject& error_obj) const
{
    ...
    const auto query_pool_state = Get<vvl::QueryPool>(queryPool);
    ASSERT_AND_RETURN_SKIP(query_pool_state);
    skip |= ValidateQueryPoolIndex(commandBuffer,
                                   *query_pool_state,
                                   firstQuery,
                                   queryCount,
                                   error_obj.location,
    ...
    skip |= ValidateQueriesNotActive(
        *cb_state, queryPool, firstQuery, queryCount, error_obj.location, "VUID-vkCmdResetQueryPool-None-02841");

    return skip;
}

// Use PreCallRecord to view query object before resetting it
void CoreChecks::PreCallRecordCmdResetQueryPool(VkCommandBuffer     commandBuffer,
                                                VkQueryPool         queryPool,
                                                uint32_t            firstQuery,
                                                uint32_t            queryCount,
                                                const RecordObject& record_obj)
{
    ...
    if (query_pool_state->create_info.queryType == VK_QUERY_TYPE_PERFORMANCE_QUERY_KHR)
    {
        cb_state->query_updates.emplace_back(
            [queryPool, firstQuery, queryCount, record_obj](vvl::CommandBuffer& cb_state_arg,
                                                            bool                do_validate,
                                                            VkQueryPool&        firstPerfQueryPool,
                                                            uint32_t            perfPass,
                                                            QueryMap*           localQueryToStateMap) {
                ...
                for (uint32_t i = 0; i < queryCount; i++)
                {
                    ...
                    if (state == QUERYSTATE_ENDED)
                    {
                        ...
                        skip |= state_data.LogError("VUID-vkCmdResetQueryPool-firstQuery-02862",
                                                    objlist,
                                                    record_obj.location,
                                                    "Query index %" PRIu32
                                                    " was begun and reset in the same command buffer.",
                                                    firstQuery + i);
                        break;
                    }
                }
...
bool CoreChecks::PreCallValidateCmdCopyQueryPoolResults(VkCommandBuffer    commandBuffer,
                                                        VkQueryPool        queryPool,
                                                        uint32_t           firstQuery,
                                                        uint32_t           queryCount,
                                                        VkBuffer           dstBuffer,
                                                        ...
                                                        const ErrorObject& error_obj) const
{
    ...
    skip |= ValidateMemoryIsBoundToBuffer(commandBuffer,
                                          *dst_buff_state,
                                          error_obj.location.dot(Field::dstBuffer),
                                          "VUID-vkCmdCopyQueryPoolResults-dstBuffer-00826");
    skip |= ValidateQueryPoolStride("VUID-vkCmdCopyQueryPoolResults-flags-00822",
    ...
    skip |= ValidateBufferUsageFlags(buffer_objlist,
                                     *dst_buff_state,
    ...
    {
        skip |= LogError("VUID-vkCmdCopyQueryPoolResults-dstOffset-00819",
                         buffer_objlist,
                         error_obj.location.dot(Field::dstOffset),
                         "(%" PRIu64 ") is not less than the size (%" PRIu64 ") of buffer (%s).",
                         dstOffset,
                         dst_buff_state->requirements.size,
                         FormatHandle(dst_buff_state->Handle()).c_str());
    }
    else if (dstOffset + (queryCount * stride) > dst_buff_state->requirements.size)
    {
        skip |= LogError("VUID-vkCmdCopyQueryPoolResults-dstBuffer-00824",
                         buffer_objlist,
                         error_obj.location,
                         "storage required (%" PRIu64
                         ") equal to dstOffset + (queryCount * stride) is greater than the size (%" PRIu64
                         ") of buffer (%s).",
                         dstOffset + (queryCount * stride),
                         dst_buff_state->requirements.size,
                         FormatHandle(dst_buff_state->Handle()).c_str());
    }
    ...
    {
        skip |= LogError("VUID-vkCmdCopyQueryPoolResults-flags-09443",
                         commandBuffer,
                         error_obj.location.dot(Field::flags),
                         "(%s) include both STATUS_BIT and AVAILABILITY_BIT.",
                         string_VkQueryResultFlags(flags).c_str());
    }
    ...
    {
        const LogObjectList objlist(commandBuffer, queryPool);
        skip |= LogError("VUID-vkCmdCopyQueryPoolResults-queryCount-09438",
                         objlist,
                         error_obj.location.dot(Field::queryCount),
                         "is %" PRIu32 " but stride is zero.",
                         queryCount);
    ...
    skip |= ValidateQueriesNotActive(
        *cb_state, queryPool, firstQuery, queryCount, error_obj.location, "VUID-vkCmdCopyQueryPoolResults-None-07429");
    ...
    {
        const LogObjectList objlist(commandBuffer, queryPool);
        skip |= LogError("VUID-vkCmdCopyQueryPoolResults-queryType-09439",
                         objlist,
                         error_obj.location.dot(Field::flags),
    ...
    {
        const LogObjectList objlist(commandBuffer, queryPool);
        skip |= LogError("VUID-vkCmdCopyQueryPoolResults-queryType-02734",
                         objlist,
                         error_obj.location.dot(Field::queryPool),
                         "(%s) was created with queryType VK_QUERY_TYPE_PERFORMANCE_QUERY_INTEL.",
                         FormatHandle(queryPool).c_str());
    }
    if (query_pool_state->create_info.queryType == VK_QUERY_TYPE_RESULT_STATUS_ONLY_KHR &&
        (flags & VK_QUERY_RESULT_WITH_STATUS_BIT_KHR) == 0)
    {
        const LogObjectList objlist(commandBuffer, queryPool);
        skip |= LogError("VUID-vkCmdCopyQueryPoolResults-queryType-09442",
                         objlist,
                         error_obj.location.dot(Field::flags),
...
void CoreChecks::PreCallRecordCmdCopyQueryPoolResults(VkCommandBuffer     commandBuffer,
                                                      VkQueryPool         queryPool,
                                                      uint32_t            firstQuery,
                                                      uint32_t            queryCount,
                                                      VkBuffer            dstBuffer,
                                                      ...
                                                      const RecordObject& record_obj)
{
    ...
    auto cb_state = GetWrite<vvl::CommandBuffer>(commandBuffer);
    cb_state->query_updates.emplace_back(
        [queryPool, firstQuery, queryCount, flags, record_obj, this](vvl::CommandBuffer& cb_state_arg,
                                                                     bool                do_validate,
                                                                     VkQueryPool&        firstPerfQueryPool,
                                                                     uint32_t            perfPass,
                                                                     QueryMap*           localQueryToStateMap) {
            ...
            for (uint32_t i = 0; i < queryCount; i++)
            {
                ...
                {
                    const LogObjectList objlist(cb_state_arg.Handle(), queryPool);
                    skip |= state_data.LogError("VUID-vkCmdCopyQueryPoolResults-None-08752",
                                                objlist,
                                                record_obj.location,
                                                "Requesting a copy from query to buffer on %s query %" PRIu32 ": %s",
                                                state_data.FormatHandle(queryPool).c_str(),
                                                firstQuery + i,
                                                string_QueryResultType(result_type));
                }
            }

            // NOTE: dev_data == this, but the compiler "Visual Studio 16" complains Get is ambiguous if dev_data isn't
            // used
            auto query_pool_state = cb_state_arg.dev_data.Get<vvl::QueryPool>(queryPool);
            if (query_pool_state)
            {
                skip |= ValidateQueryPoolWasReset(
                    *query_pool_state, firstQuery, queryCount, record_obj.location, localQueryToStateMap, perfPass);
            }
...
bool CoreChecks::ValidateCmdWriteTimestamp(const vvl::CommandBuffer& cb_state,
                                           VkQueryPool               queryPool,
                                           uint32_t                  slot,
                                           const Location&           loc) const
{
    ...
    {
        const char*         vuid = is_2 ? "VUID-vkCmdWriteTimestamp2-timestampValidBits-03863"
                                        : "VUID-vkCmdWriteTimestamp-timestampValidBits-00829";
        const LogObjectList objlist(cb_state.Handle(), queryPool);
        skip |= LogError(vuid,
                         objlist,
                         loc,
                         "Query Pool %s has a timestampValidBits value of zero for queueFamilyIndex %u.",
                         FormatHandle(queryPool).c_str(),
                         cb_state.command_pool->queueFamilyIndex);
    }
    ...
    {
        const char* vuid =
            is_2 ? "VUID-vkCmdWriteTimestamp2-queryPool-03861" : "VUID-vkCmdWriteTimestamp-queryPool-01416";
        const LogObjectList objlist(cb_state.Handle(), queryPool);
        skip |= LogError(vuid,
                         objlist,
                         loc,
                         "Query Pool %s was not created with VK_QUERY_TYPE_TIMESTAMP.",
                         FormatHandle(queryPool).c_str());
    }
    ...
    if (slot >= query_pool_state->create_info.queryCount)
    {
        ...
        skip |= LogError(vuid,
                         objlist,
                         loc,
                         "query (%" PRIu32 ") is not lower than the number of queries (%" PRIu32 ") in Query pool %s.",
                         slot,
                         query_pool_state->create_info.queryCount,
                         FormatHandle(queryPool).c_str());
    }
    if (cb_state.activeRenderPass && slot + cb_state.activeRenderPass->GetViewMaskBits(cb_state.GetActiveSubpass()) >
                                         query_pool_state->create_info.queryCount)
    {
        ...
        skip |= LogError(vuid,
                         ...
                         slot,
                         cb_state.activeRenderPass->GetViewMaskBits(cb_state.GetActiveSubpass()),
                         query_pool_state->create_info.queryCount,
                         FormatHandle(queryPool).c_str());
    }
...
bool CoreChecks::PreCallValidateCmdWriteTimestamp(VkCommandBuffer         commandBuffer,
...
bool CoreChecks::PreCallValidateCmdWriteTimestamp2(VkCommandBuffer       commandBuffer,
                                                   ...
                                                   const ErrorObject&    error_obj) const
{
    ...
    {
        skip |= LogError("VUID-vkCmdWriteTimestamp2-synchronization2-03858",
                         commandBuffer,
                         error_obj.location,
    ...
    if ((stage & (stage - 1)) != 0)
    {
        skip |= LogError("VUID-vkCmdWriteTimestamp2-stage-03859",
                         commandBuffer,
                         stage_loc,
                         "(%s) must only set a single pipeline stage.",
                         string_VkPipelineStageFlags2(stage).c_str());
    }
    skip |= ValidatePipelineStage(LogObjectList(commandBuffer), stage_loc, cb_state->GetQueueFlags(), stage);
    return skip;
}

bool CoreChecks::PreCallValidateCmdWriteTimestamp2KHR(VkCommandBuffer          commandBuffer,
...
void CoreChecks::RecordCmdWriteTimestamp2(vvl::CommandBuffer& cb_state,
                                          VkQueryPool         queryPool,
                                          uint32_t            slot,
                                          Func                command) const
{
...
void CoreChecks::PreCallRecordCmdWriteTimestamp(VkCommandBuffer         commandBuffer,
                                                VkPipelineStageFlagBits pipelineStage,
                                                VkQueryPool             queryPool,
                                                uint32_t                slot,
                                                const RecordObject&     record_obj)
...
void CoreChecks::PreCallRecordCmdWriteTimestamp2KHR(VkCommandBuffer          commandBuffer,
                                                    VkPipelineStageFlags2KHR stage,
                                                    VkQueryPool              queryPool,
                                                    uint32_t                 slot,
                                                    const RecordObject&      record_obj)
...
void CoreChecks::PreCallRecordCmdWriteTimestamp2(VkCommandBuffer          commandBuffer,
                                                 VkPipelineStageFlags2KHR stage,
                                                 VkQueryPool              queryPool,
                                                 uint32_t                 slot,
                                                 const RecordObject&      record_obj)
...
bool CoreChecks::PreCallValidateCmdBeginQueryIndexedEXT(VkCommandBuffer     commandBuffer,
                                                        VkQueryPool         queryPool,
                                                        uint32_t            slot,
                                                        VkQueryControlFlags flags,
                                                        uint32_t            index,
                                                        const ErrorObject&  error_obj) const
{
    ...
    if (query_pool_ci.queryType == VK_QUERY_TYPE_PRIMITIVES_GENERATED_EXT)
    {
        ...
        {
            const LogObjectList objlist(commandBuffer, queryPool);
            skip |= LogError("VUID-vkCmdBeginQueryIndexedEXT-queryType-06693",
                             objlist,
                             error_obj.location.dot(Field::queryPool),
        ...
        if (index >= phys_dev_ext_props.transform_feedback_props.maxTransformFeedbackStreams)
        {
            const LogObjectList objlist(commandBuffer, queryPool);
            skip |=
                LogError("VUID-vkCmdBeginQueryIndexedEXT-queryType-06690",
                         objlist,
                         error_obj.location.dot(Field::queryPool),
                         "was created with queryType of VK_QUERY_TYPE_PRIMITIVES_GENERATED_EXT, but "
                         "index (%" PRIu32 ") is greater than or equal to "
        ...
        {
            const LogObjectList objlist(commandBuffer, queryPool);
            skip |= LogError("VUID-vkCmdBeginQueryIndexedEXT-queryType-06691",
                             objlist,
                             error_obj.location.dot(Field::queryPool),
                             "was created with queryType of VK_QUERY_TYPE_PRIMITIVES_GENERATED_EXT, but "
                             "index (%" PRIu32
    ...
    else if (query_pool_ci.queryType == VK_QUERY_TYPE_TRANSFORM_FEEDBACK_STREAM_EXT)
    {
        ...
        {
            skip |= LogError("VUID-vkCmdBeginQueryIndexedEXT-queryType-02339",
                             commandBuffer,
                             error_obj.location.dot(Field::index),
                             "(%" PRIu32 ") must be less than "
                             "VkPhysicalDeviceTransformFeedbackPropertiesEXT::maxTransformFeedbackStreams %" PRIu32 ".",
                             index,
                             phys_dev_ext_props.transform_feedback_props.maxTransformFeedbackStreams);
        }
    ...
    else if (index != 0)
    {
        ...
        skip |= LogError(
            ...
            objlist,
            error_obj.location.dot(Field::index),
            "(%" PRIu32 ") must be zero if %s was not created with type VK_QUERY_TYPE_TRANSFORM_FEEDBACK_STREAM_EXT or "
...
// Need to enqueue work prior to PostCallRecord
void CoreChecks::PreCallRecordCmdBeginQueryIndexedEXT(VkCommandBuffer     commandBuffer,
                                                      VkQueryPool         queryPool,
                                                      uint32_t            slot,
                                                      VkQueryControlFlags flags,
                                                      uint32_t            index,
...
void CoreChecks::PreCallRecordCmdEndQueryIndexedEXT(
    VkCommandBuffer commandBuffer, VkQueryPool queryPool, uint32_t slot, uint32_t index, const RecordObject& record_obj)
{
    if (disabled[query_validation])
        return;
    auto        cb_state        = GetWrite<vvl::CommandBuffer>(commandBuffer);
...
bool CoreChecks::PreCallValidateCmdEndQueryIndexedEXT(VkCommandBuffer    commandBuffer,
                                                      VkQueryPool        queryPool,
                                                      uint32_t           slot,
                                                      uint32_t           index,
                                                      const ErrorObject& error_obj) const
{
    ...
    {
        const LogObjectList objlist(commandBuffer, queryPool);
        skip |= LogError("VUID-vkCmdEndQueryIndexedEXT-query-02343",
                         objlist,
                         error_obj.location.dot(Field::index),
                         "(%" PRIu32 ") is greater or equal to the queryPool size (%" PRIu32 ").",
                         index,
                         available_query_count);
    }
        ...
        query_pool_ci.queryType == VK_QUERY_TYPE_PRIMITIVES_GENERATED_EXT)
    {
        if (index >= phys_dev_ext_props.transform_feedback_props.maxTransformFeedbackStreams)
        {
            skip |= LogError("VUID-vkCmdEndQueryIndexedEXT-queryType-06694",
                             commandBuffer,
                             error_obj.location.dot(Field::index),
                             "(%" PRIu32 ") must be less than "
                             "VkPhysicalDeviceTransformFeedbackPropertiesEXT::maxTransformFeedbackStreams %" PRIu32 ".",
        ...
        for (const auto& query_object : cb_state->startedQueries)
        {
            if (query_object.pool == queryPool && query_object.slot == slot)
            {
                if (query_object.index != index)
                {
                    const LogObjectList objlist(commandBuffer, queryPool);
                    skip |=
                        LogError("VUID-vkCmdEndQueryIndexedEXT-queryType-06696",
                                 objlist,
                                 error_obj.location,
...
bool CoreChecks::PreCallValidateResetQueryPool(VkDevice           device,
                                               VkQueryPool        queryPool,
                                               uint32_t           firstQuery,
                                               uint32_t           queryCount,
                                               const ErrorObject& error_obj) const
{
    ...
    {
        skip |= LogError(
            "VUID-vkResetQueryPool-None-02665", device, error_obj.location, "hostQueryReset feature was not enabled.");
    }
    ...
    {
        skip |= LogError("VUID-vkResetQueryPool-firstQuery-09436",
                         queryPool,
                         error_obj.location.dot(Field::firstQuery),
                         "(%" PRIu32 ") is greater than or equal to query pool count (%" PRIu32 ") for %s.",
                         firstQuery,
                         query_pool_state->create_info.queryCount,
                         FormatHandle(queryPool).c_str());
    }
    ...
    {
        skip |= LogError("VUID-vkResetQueryPool-firstQuery-09437",
                         queryPool,
                         error_obj.location,
                         "Query range [%" PRIu32 ", %" PRIu32 ") goes beyond query pool count (%" PRIu32 ") for %s.",
                         firstQuery,
...
bool CoreChecks::PreCallValidateResetQueryPoolEXT(VkDevice           device,
                                                  VkQueryPool        queryPool,
                                                  uint32_t           firstQuery,
                                                  uint32_t           queryCount,
                                                  const ErrorObject& error_obj) const
...
bool CoreChecks::ValidateQueryPoolStride(const std::string&       vuid_not_64,
                                         const std::string&       vuid_64,
                                         const VkDeviceSize       stride,
                                         vvl::Field               field_name,
                                         ...
                                         const Location&          loc) const
{
    ...
    if (flags & VK_QUERY_RESULT_64_BIT)
    {
        ...
        {
            skip |= LogError(vuid_64,
                             objlist,
                             loc,
                             "%" PRIu64 " or %s %" PRIu64 " is invalid.",
                             stride,
                             String(field_name),
                             parameter_value);
        }
...
void CoreChecks::PostCallRecordGetQueryPoolResults(VkDevice            device,
                                                   VkQueryPool         queryPool,
                                                   uint32_t            firstQuery,
                                                   uint32_t            queryCount,
                                                   size_t              dataSize,
...
bool CoreChecks::PreCallValidateReleaseProfilingLockKHR(VkDevice device, const ErrorObject& error_obj) const
{
    ...
    {
        skip |= LogError("VUID-vkReleaseProfilingLockKHR-device-03235",
                         device,
                         error_obj.location,
