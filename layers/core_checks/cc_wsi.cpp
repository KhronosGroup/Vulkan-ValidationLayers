...
static bool IsExtentInsideBounds(VkExtent2D extent, VkExtent2D min, VkExtent2D max) {
    if ((extent.width < min.width) || (extent.width > max.width) || (extent.height < min.height) ||
...
static VkImageCreateInfo GetSwapchainImpliedImageCreateInfo(const VkSwapchainCreateInfoKHR& create_info) {
    ...
    if (create_info.flags & VK_SWAPCHAIN_CREATE_PROTECTED_BIT_KHR)
        result.flags |= VK_IMAGE_CREATE_PROTECTED_BIT;
...
bool CoreChecks::ValidateSwapchainImageExtent(const VkSwapchainCreateInfoKHR& create_info,
    ...
    if (create_info.imageExtent.width == 0 || create_info.imageExtent.height == 0) {
        skip |= LogError("VUID-VkSwapchainCreateInfoKHR-imageExtent-01689",
                         device,
                         create_info_loc.dot(Field::imageExtent),
                         "(%s) is invalid.",
                         string_VkExtent2D(create_info.imageExtent).c_str());
        return skip; // do not continue, other extent checks will fail
    }

    const auto present_scaling_ci =
    ...
    if (no_scaling) {
        if (!IsExtentInsideBounds(create_info.imageExtent, surface_caps.minImageExtent, surface_caps.maxImageExtent)) {
            skip |= LogError("VUID-VkSwapchainCreateInfoKHR-pNext-07781",
                             device,
                             create_info_loc.dot(Field::imageExtent),
                             "(%s), which is outside the bounds returned by "
                             "vkGetPhysicalDeviceSurfaceCapabilitiesKHR(): currentExtent = (%s), minImageExtent = "
                             "(%s), maxImageExtent = (%s).",
                             string_VkExtent2D(create_info.imageExtent).c_str(),
                             string_VkExtent2D(surface_caps.currentExtent).c_str(),
                             string_VkExtent2D(surface_caps.minImageExtent).c_str(),
                             string_VkExtent2D(surface_caps.maxImageExtent).c_str());
        }
    } else {
        ...
        if (!IsExtentInsideBounds(
                create_info.imageExtent, scaling_caps.minScaledImageExtent, scaling_caps.maxScaledImageExtent)) {
            skip |= LogError("VUID-VkSwapchainCreateInfoKHR-pNext-07782",
                             device,
                             create_info_loc.dot(Field::imageExtent),
...
bool CoreChecks::ValidateSwapchainPresentModesCreateInfo(VkPresentModeKHR present_mode,
                                                         const Location& create_info_loc,
    ...
    for (uint32_t i = 0; i < swapchain_present_modes_ci->presentModeCount; i++) {
        ...
        if (swapchain_present_mode == VK_PRESENT_MODE_FIFO_LATEST_READY_EXT &&
            !enabled_features.presentModeFifoLatestReady) {
            skip |=
                LogError("VUID-VkSwapchainPresentModesCreateInfoEXT-presentModeFifoLatestReady-10160",
                         device,
        ...
        if (std::find(present_modes.begin(), present_modes.end(), swapchain_present_mode) == present_modes.end()) {
            if (LogError("VUID-VkSwapchainPresentModesCreateInfoEXT-None-07762",
                         device,
                         create_info_loc.pNext(Struct::VkSwapchainPresentModesCreateInfoEXT, Field::pPresentModes, i),
                         "%s is a non-supported presentMode.",
        ...
        if (std::find(compatible_present_modes.begin(), compatible_present_modes.end(), swapchain_present_mode) ==
            ...
            if (LogError("VUID-VkSwapchainPresentModesCreateInfoEXT-pPresentModes-07763",
                         device,
                         create_info_loc.pNext(Struct::VkSwapchainPresentModesCreateInfoEXT, Field::pPresentModes, i),
                         "%s is a non-compatible presentMode.",
                         string_VkPresentModeKHR(swapchain_present_mode))) {
                skip |= true;
            }
        }
...
bool CoreChecks::ValidateSwapchainPresentScalingCreateInfo(VkPresentModeKHR present_mode,
                                                           const Location& create_info_loc,
    ...
    if (pres_scale_ci) {
        if ((pres_scale_ci->presentGravityX == 0) && (pres_scale_ci->presentGravityY != 0)) {
            if (LogError("VUID-VkSwapchainPresentScalingCreateInfoEXT-presentGravityX-07765",
                         device,
                         create_info_loc.pNext(Struct::VkSwapchainPresentScalingCreateInfoEXT, Field::presentGravityX),
                         "is zero but presentGravityY (%" PRIu32 ") is not zero.",
                         pres_scale_ci->presentGravityY)) {
                skip |= true;
            }
        }

        if ((pres_scale_ci->presentGravityX != 0) && (pres_scale_ci->presentGravityY == 0)) {
            if (LogError("VUID-VkSwapchainPresentScalingCreateInfoEXT-presentGravityX-07766",
                         device,
                         create_info_loc.pNext(Struct::VkSwapchainPresentScalingCreateInfoEXT, Field::presentGravityY),
                         "is zero but presentGravityX (%" PRIu32 ") is not zero.",
                         pres_scale_ci->presentGravityX)) {
                skip |= true;
            }
        }

        if (GetBitSetCount(pres_scale_ci->scalingBehavior) > 1) {
            if (LogError("VUID-VkSwapchainPresentScalingCreateInfoEXT-scalingBehavior-07767",
                         device,
        ...
        if (GetBitSetCount(pres_scale_ci->presentGravityX) > 1) {
            if (LogError("VUID-VkSwapchainPresentScalingCreateInfoEXT-presentGravityX-07768",
                         device,
                         create_info_loc.pNext(Struct::VkSwapchainPresentScalingCreateInfoEXT, Field::presentGravityX),
                         "(%s) must not have more than one bit set.",
        ...
        if (GetBitSetCount(pres_scale_ci->presentGravityY) > 1) {
            if (LogError("VUID-VkSwapchainPresentScalingCreateInfoEXT-presentGravityY-07769",
                         device,
                         create_info_loc.pNext(Struct::VkSwapchainPresentScalingCreateInfoEXT, Field::presentGravityY),
                         "(%s) must not have more than one bit set.",
        ...
        if ((scaling_caps.supportedPresentScaling != 0) && (pres_scale_ci->scalingBehavior != 0) &&
            ...
            if (LogError("VUID-VkSwapchainPresentScalingCreateInfoEXT-scalingBehavior-07770",
                         device,
                         create_info_loc.pNext(Struct::VkSwapchainPresentScalingCreateInfoEXT, Field::scalingBehavior),
                         "(%s) is not among the scaling methods for the surface as returned in "
                         "VkSurfacePresentScalingCapabilitiesEXT::supportedPresentScaling for the specified "
                         "presentMode: (%s).",
                         string_VkPresentScalingFlagsEXT(pres_scale_ci->scalingBehavior).c_str(),
        ...
        const auto* present_modes_ci =
            vku::FindStructInPNextChain<VkSwapchainPresentModesCreateInfoEXT>(create_info.pNext);
        if (present_modes_ci) {
            for (uint32_t i = 0; i < present_modes_ci->presentModeCount; i++) {
                const Location present_mode_loc =
                    create_info_loc.pNext(Struct::VkSwapchainPresentModesCreateInfoEXT, Field::pPresentModes, i);
                scaling_caps = surface_state->GetPresentModeScalingCapabilities(physical_device,
                                                                                present_modes_ci->pPresentModes[i]);

                if ((scaling_caps.supportedPresentScaling != 0) && (pres_scale_ci->scalingBehavior != 0) &&
                    (scaling_caps.supportedPresentScaling & pres_scale_ci->scalingBehavior) == 0) {
                    if (LogError("VUID-VkSwapchainPresentScalingCreateInfoEXT-scalingBehavior-07771",
                                 device,
                                 create_info_loc.pNext(Struct::VkSwapchainPresentScalingCreateInfoEXT,
                                                       Field::scalingBehavior),
                ...
                if ((scaling_caps.supportedPresentGravityX != 0) && (pres_scale_ci->presentGravityX != 0) &&
                    (scaling_caps.supportedPresentGravityX & pres_scale_ci->presentGravityX) == 0) {
                    if (LogError("VUID-VkSwapchainPresentScalingCreateInfoEXT-presentGravityX-07773",
                                 device,
                                 create_info_loc.pNext(Struct::VkSwapchainPresentScalingCreateInfoEXT,
                                                       Field::presentGravityX),
                ...
                if ((scaling_caps.supportedPresentGravityY != 0) && (pres_scale_ci->presentGravityY != 0) &&
                    (scaling_caps.supportedPresentGravityY & pres_scale_ci->presentGravityY) == 0) {
                    if (LogError("VUID-VkSwapchainPresentScalingCreateInfoEXT-presentGravityY-07775",
                                 device,
                                 create_info_loc.pNext(Struct::VkSwapchainPresentScalingCreateInfoEXT,
                                                       Field::presentGravityY),
...
bool CoreChecks::ValidateCreateSwapchain(const VkSwapchainCreateInfoKHR& create_info,
                                         const vvl::Surface* surface_state,
                                         const vvl::Swapchain* old_swapchain_state,
    ...
    if (!IsExtEnabled(instance_extensions.vk_khr_android_surface)) {
        ...
        if (!is_supported) {
            ...
            if (LogError("VUID-VkSwapchainCreateInfoKHR-surface-01270",
                         objlist,
                         create_info_loc.dot(Field::surface),
    ...
    if (old_swapchain_state) {
        if (old_swapchain_state->create_info.surface != create_info.surface) {
            if (LogError("VUID-VkSwapchainCreateInfoKHR-oldSwapchain-01933",
                         create_info.oldSwapchain,
                         create_info_loc.dot(Field::oldSwapchain),
                         "surface is not pCreateInfo->surface")) {
                return true;
            }
        }
    ...
    if (full_screen_info &&
        full_screen_info->fullScreenExclusive == VK_FULL_SCREEN_EXCLUSIVE_APPLICATION_CONTROLLED_EXT) {
        ...
        if (IsExtEnabled(device_extensions.vk_khr_win32_surface)) {
            ...
            if (!win32_full_screen_info) {
                ...
                if (LogError("VUID-VkSwapchainCreateInfoKHR-pNext-02679",
                             objlist,
    ...
    const auto surface_caps =
        surface_state->GetSurfaceCapabilities(physical_device_state->VkHandle(), surface_info_pnext);
    ...
    if ((create_info.preTransform & current_transform) != create_info.preTransform) {
        skip |= LogPerformanceWarning(
            "WARNING-Swapchain-PreTransform",
            physical_device,
            create_info_loc.dot(Field::preTransform),
    ...
    if ((create_info.minImageCount < surface_caps.minImageCount) && !shared_present_mode) {
        if (LogError("VUID-VkSwapchainCreateInfoKHR-presentMode-02839",
                     device,
                     create_info_loc.dot(Field::minImageCount),
    ...
    if ((surface_caps.maxImageCount > 0) && (create_info.minImageCount > surface_caps.maxImageCount)) {
        if (LogError("VUID-VkSwapchainCreateInfoKHR-minImageCount-01272",
                     device,
                     create_info_loc.dot(Field::minImageCount),
                     "%" PRIu32 ", which is outside the bounds returned by "
                     "vkGetPhysicalDeviceSurfaceCapabilitiesKHR() (i.e. minImageCount = %d, maxImageCount = %d).",
    ...
    if (!create_info.preTransform || (create_info.preTransform & (create_info.preTransform - 1)) ||
        ...
        return LogError("VUID-VkSwapchainCreateInfoKHR-preTransform-01279",
                        device,
                        create_info_loc.dot(Field::preTransform),
    ...
    if (!create_info.compositeAlpha || (create_info.compositeAlpha & (create_info.compositeAlpha - 1)) ||
        ...
        return LogError("VUID-VkSwapchainCreateInfoKHR-compositeAlpha-01280",
                        device,
                        create_info_loc.dot(Field::compositeAlpha),
                        "is not supported, support values are:\n%s.",
                        ss.str().c_str());
    }
    ...
    if (create_info.imageArrayLayers > surface_caps.maxImageArrayLayers) {
        if (LogError("VUID-VkSwapchainCreateInfoKHR-imageArrayLayers-01275",
                     device,
                     create_info_loc.dot(Field::imageArrayLayers),
                     "%" PRIu32 " is more than maxImageArrayLayers %" PRIu32 ".",
    ...
    if ((image_usage != (image_usage & surface_caps.supportedUsageFlags)) && !shared_present_mode) {
        if (LogError("VUID-VkSwapchainCreateInfoKHR-presentMode-01427",
                     device,
                     create_info_loc.dot(Field::imageUsage),
                     "(%s) are not in supportedUsageFlags (%s).",
    ...
    // Validate pCreateInfo values with the results of vkGetPhysicalDeviceSurfaceFormats2KHR():
    {
        ...
        if (surface_state) {
            formats = surface_state->GetFormats(IsExtEnabled(instance_extensions.vk_khr_get_surface_capabilities2),
                                                physical_device_state->VkHandle(),
                                                surface_info_pnext,
                                                create_info_loc,
                                                this);
        ...
        if (!found_match) {
            if (!found_format) {
                if (LogError("VUID-VkSwapchainCreateInfoKHR-imageFormat-01273",
                             device,
                             create_info_loc.dot(Field::imageFormat),
                             "is %s.",
            ...
            if (!found_color_space) {
                if (LogError("VUID-VkSwapchainCreateInfoKHR-imageFormat-01273",
                             device,
                             create_info_loc.dot(Field::imageColorSpace),
                             "is %s.",
                             string_VkColorSpaceKHR(create_info.imageColorSpace))) {
                    return true;
                }
            }
        }
    ...
    if (std::find(present_modes.begin(), present_modes.end(), present_mode) == present_modes.end()) {
        ...
        skip |= LogError("VUID-VkSwapchainCreateInfoKHR-presentMode-01281",
                         device,
                         create_info_loc.dot(Field::presentMode),
                         "(%s) is not supported (the following are supported %s).",
                         string_VkPresentModeKHR(present_mode),
                         ss.str().c_str());
    }

    if (IsExtEnabled(device_extensions.vk_ext_swapchain_maintenance1)) {
        skip |= ValidateSwapchainPresentModesCreateInfo(
            present_mode, create_info_loc, create_info, present_modes, surface_state);
        skip |= ValidateSwapchainPresentScalingCreateInfo(
    ...
    if (shared_present_mode) {
        if (create_info.minImageCount != 1) {
            if (LogError("VUID-VkSwapchainCreateInfoKHR-minImageCount-01383",
                         device,
                         create_info_loc,
                         "called with presentMode %s, but minImageCount value is %d. For shared presentable image, "
                         "minImageCount "
                         "must be 1.",
                         string_VkPresentModeKHR(present_mode),
                         create_info.minImageCount)) {
        ...
        DispatchGetPhysicalDeviceSurfaceCapabilities2KHR(
            physical_device_state->VkHandle(), &surface_info, &capabilities2);
    ...
    if ((create_info.imageSharingMode == VK_SHARING_MODE_CONCURRENT) && create_info.pQueueFamilyIndices) {
        bool skip1 = ValidatePhysicalDeviceQueueFamilies(create_info.queueFamilyIndexCount,
    ...
    if (tiling_features == 0) {
        if (LogError("VUID-VkSwapchainCreateInfoKHR-imageFormat-01778",
                     device,
                     create_info_loc.dot(Field::imageFormat),
    ...
    } else if ((image_usage & VK_IMAGE_USAGE_SAMPLED_BIT) &&
               !(tiling_features & VK_FORMAT_FEATURE_2_SAMPLED_IMAGE_BIT)) {
        if (LogError("VUID-VkSwapchainCreateInfoKHR-imageFormat-01778",
                     device,
                     create_info_loc.dot(Field::imageFormat),
    ...
    } else if ((image_usage & VK_IMAGE_USAGE_STORAGE_BIT) &&
               !(tiling_features & VK_FORMAT_FEATURE_2_STORAGE_IMAGE_BIT)) {
        if (LogError("VUID-VkSwapchainCreateInfoKHR-imageFormat-01778",
                     device,
                     create_info_loc.dot(Field::imageFormat),
                     "%s with tiling VK_IMAGE_TILING_OPTIMAL does not support usage that includes "
    ...
    const VkResult image_properties_result = DispatchGetPhysicalDeviceImageFormatProperties(physical_device,
    ...
    if (create_info.imageArrayLayers > image_properties.maxArrayLayers) {
        if (LogError("VUID-VkSwapchainCreateInfoKHR-imageFormat-01778",
                     ...
                     create_info.imageArrayLayers,
                     image_properties.maxArrayLayers,
                     string_VkFormat(create_info.imageFormat))) {
            return true;
        }
    }
    ...
    if ((create_info.imageExtent.width > image_properties.maxExtent.width) ||
        ...
        if (LogError("VUID-VkSwapchainCreateInfoKHR-imageFormat-01778",
                     ...
                     " returned by vkGetPhysicalDeviceImageFormatProperties() for imageFormat %s with tiling "
                     "VK_IMAGE_TILING_OPTIMAL.",
                     string_VkExtent2D(create_info.imageExtent).c_str(),
                     string_VkExtent3D(image_properties.maxExtent).c_str(),
                     string_VkFormat(create_info.imageFormat))) {
            return true;
        }
    }
    ...
    if ((create_info.flags & VK_SWAPCHAIN_CREATE_SPLIT_INSTANCE_BIND_REGIONS_BIT_KHR) && physical_device_count == 1) {
        if (LogError(
                ...
                "The logical device may have been created without explicitly using VkDeviceGroupDeviceCreateInfo, or "
                "with"
    ...
    if (image_compression_control && !enabled_features.imageCompressionControlSwapchain) {
        skip |= LogError("VUID-VkSwapchainCreateInfoKHR-pNext-06752",
                         device,
                         create_info_loc.dot(Field::pNext),
    ...
    if (swapchain_counter) {
        ...
        const VkResult result = DispatchGetPhysicalDeviceSurfaceCapabilities2EXT(
            physical_device, create_info.surface, &surface_capabilities);
        if (result != VK_SUCCESS) {
            skip |=
                LogError("VUID-VkSwapchainCounterCreateInfoEXT-surfaceCounters-01244",
                         device,
                         create_info_loc.pNext(Struct::VkSwapchainPresentModesCreateInfoEXT, Field::surfaceCounters),
                         "is %s, but the counters are not supported because the "
                         "vkGetPhysicalDeviceSurfaceCapabilities2EXT query failed",
...
bool CoreChecks::PreCallValidateCreateSwapchainKHR(VkDevice device,
    ...
    return ValidateCreateSwapchain(
        *pCreateInfo, surface_state.get(), old_swapchain_state.get(), error_obj.location.dot(Field::pCreateInfo));
}

void CoreChecks::PreCallRecordDestroySwapchainKHR(VkDevice device,
...
bool CoreChecks::ValidateImageAcquireWait(const vvl::SwapchainImage& swapchain_image,
    ...
    if (!semaphore_was_waited && !fence_was_waited) {
        // TODO: Replace UNASSIGNED with official VUID when ready:
        // https://gitlab.khronos.org/vulkan/vulkan/-/issues/3616
        ...
        if (semaphore && fence) {
            ...
            skip |=
                LogError(missing_acquire_wait_vuid,
                         objlist,
                         image_index_loc,
            ...
            skip |= LogError(missing_acquire_wait_vuid,
                             objlist,
                             image_index_loc,
                             "was acquired with a fence %s that has not since been waited on",
                             FormatHandle(fence->Handle()).c_str());
        }
    }
    return skip;
...
bool CoreChecks::PreCallValidateQueuePresentKHR(VkQueue queue,
    ...
    for (uint32_t i = 0; i < pPresentInfo->waitSemaphoreCount; ++i) {
        ...
        if (semaphore_state->type != VK_SEMAPHORE_TYPE_BINARY) {
            skip |= LogError("VUID-vkQueuePresentKHR-pWaitSemaphores-03267",
                             pPresentInfo->pWaitSemaphores[i],
                             present_info_loc.dot(Field::pWaitSemaphores, i),
                             "(%s) is not a VK_SEMAPHORE_TYPE_BINARY",
                             FormatHandle(pPresentInfo->pWaitSemaphores[i]).c_str());
            continue;
        }
        skip |= sem_submit_state.ValidateWaitSemaphore(
            present_info_loc.dot(Field::pWaitSemaphores, i), *semaphore_state, 0);
    }
    ...
    for (uint32_t i = 0; i < pPresentInfo->swapchainCount; ++i) {
        ...
        if (pPresentInfo->pImageIndices[i] >= swapchain_data->images.size()) {
            ...
            skip |= LogError("VUID-VkPresentInfoKHR-pImageIndices-01430",
                             pPresentInfo->pSwapchains[i],
                             swapchain_loc,
                             "image at index %" PRIu32 " was not acquired from the swapchain.",
                             pPresentInfo->pImageIndices[i]);
        } else {
            ...
            const auto* display_present_info =
                vku::FindStructInPNextChain<VkDisplayPresentInfoKHR>(pPresentInfo->pNext);
            if (display_present_info) {
                if (display_present_info->srcRect.offset.x < 0 || display_present_info->srcRect.offset.y < 0 ||
                    ...
                    skip |= LogError("VUID-VkDisplayPresentInfoKHR-srcRect-01257",
                                     queue,
            ...
            skip |= ValidateImageAcquireWait(
                swapchain_data->images[pPresentInfo->pImageIndices[i]], i, *pPresentInfo, present_info_loc);
        }
        ...
        if (!IsExtEnabled(instance_extensions.vk_khr_android_surface)) {
            ...
            if (surface_state && !surface_state->GetQueueSupport(physical_device, queue_state->queue_family_index)) {
                skip |= LogError("VUID-vkQueuePresentKHR-pSwapchains-01292",
                                 pPresentInfo->pSwapchains[i],
                                 swapchain_loc,
    ...
    if (pPresentInfo->pNext) {
            ...
            for (uint32_t i = 0; i < present_regions->swapchainCount; ++i) {
                ...
                for (uint32_t j = 0; j < region.rectangleCount; ++j) {
                    ...
                    if ((rect.offset.x + rect.extent.width) > swapchain_data->create_info.imageExtent.width) {
                        skip |= LogError("VUID-VkRectLayerKHR-offset-04864",
                                         pPresentInfo->pSwapchains[i],
                                         rect_loc,
                    ...
                    if (rect.layer >= swapchain_data->create_info.imageArrayLayers) {
                        skip |=
                            LogError("VUID-VkRectLayerKHR-layer-01262",
                                     pPresentInfo->pSwapchains[i],
                                     rect_loc.dot(Field::layer),
                                     "layer (%" PRIu32
                                     ") is greater than the corresponding swapchain's imageArrayLayers (%" PRIu32 ").",
                                     rect.layer,
                                     swapchain_data->create_info.imageArrayLayers);
                    }
                }
            }
        }
        ...
        if (present_times_info) {
            if (pPresentInfo->swapchainCount != present_times_info->swapchainCount) {
                skip |= LogError("VUID-VkPresentTimesInfoGOOGLE-swapchainCount-01247",
                                 ...
                                 present_times_info->swapchainCount,
        ...
        if (present_id_info) {
            if (!enabled_features.presentId) {
                for (uint32_t i = 0; i < present_id_info->swapchainCount; i++) {
                    if (present_id_info->pPresentIds[i] != 0) {
                        skip |= LogError("VUID-VkPresentInfoKHR-pNext-06235",
                                         pPresentInfo->pSwapchains[0],
            ...
            if (pPresentInfo->swapchainCount != present_id_info->swapchainCount) {
                skip |= LogError("VUID-VkPresentIdKHR-swapchainCount-04998",
                                 pPresentInfo->pSwapchains[0],
                                 present_info_loc.pNext(Struct::VkPresentIdKHR, Field::swapchainCount),
                                 "(%" PRIu32 ") is not equal to pPresentInfo->swapchainCount (%" PRIu32 ").",
                                 present_id_info->swapchainCount,
                                 pPresentInfo->swapchainCount);
            }
            for (uint32_t i = 0; i < present_id_info->swapchainCount; i++) {
                ...
                if ((present_id_info->pPresentIds[i] != 0) &&
                    ...
                    skip |= LogError(
                        ...
                        present_id_info->pPresentIds[i],
                        swapchain_state->max_present_id);
                }
            }
        }

        const auto* swapchain_present_fence_info =
            vku::FindStructInPNextChain<VkSwapchainPresentFenceInfoEXT>(pPresentInfo->pNext);
        if (swapchain_present_fence_info) {
            if (pPresentInfo->swapchainCount != swapchain_present_fence_info->swapchainCount) {
                skip |= LogError("VUID-VkSwapchainPresentFenceInfoEXT-swapchainCount-07757",
                                 pPresentInfo->pSwapchains[0],
                                 present_info_loc.pNext(Struct::VkSwapchainPresentFenceInfoEXT, Field::swapchainCount),
                                 "(%" PRIu32 ") is not equal to pPresentInfo->swapchainCount (%" PRIu32 ").",
                                 swapchain_present_fence_info->swapchainCount,
                                 pPresentInfo->swapchainCount);
            }
        ...
        const auto* swapchain_present_mode_info =
            vku::FindStructInPNextChain<VkSwapchainPresentModeInfoEXT>(pPresentInfo->pNext);
        if (swapchain_present_mode_info) {
            if (pPresentInfo->swapchainCount != swapchain_present_mode_info->swapchainCount) {
                skip |= LogError("VUID-VkSwapchainPresentModeInfoEXT-swapchainCount-07760",
                                 pPresentInfo->pSwapchains[0],
                                 present_info_loc.pNext(Struct::VkSwapchainPresentModeInfoEXT, Field::swapchainCount),
                                 "(%" PRIu32 ") is not equal to pPresentInfo->swapchainCount (%" PRIu32 ").",
                                 swapchain_present_mode_info->swapchainCount,
                                 pPresentInfo->swapchainCount);
            }
            ...
            for (uint32_t i = 0; i < swapchain_present_mode_info->swapchainCount; i++) {
                ...
                if (!swapchain_state->present_modes.empty()) {
                    bool found_match = std::find(swapchain_state->present_modes.begin(),
                                                 swapchain_state->present_modes.end(),
                                                 present_mode) != swapchain_state->present_modes.end();
                    if (!found_match) {
                        skip |=
                            LogError("VUID-VkSwapchainPresentModeInfoEXT-pPresentModes-07761",
                                     pPresentInfo->pSwapchains[i],
...
bool CoreChecks::PreCallValidateReleaseSwapchainImagesEXT(VkDevice device,
    ...
    for (uint32_t i = 0; i < pReleaseInfo->imageIndexCount; i++) {
        ...
        if (image_index >= swapchain_state->images.size()) {
            skip |= LogError("VUID-VkReleaseSwapchainImagesInfoEXT-pImageIndices-07785",
                             pReleaseInfo->swapchain,
            ...
            if (!swapchain_state->images[image_index].acquired) {
                ...
                skip |= LogError("VUID-VkReleaseSwapchainImagesInfoEXT-pImageIndices-07785",
                                 pReleaseInfo->swapchain,
                                 release_info_loc.dot(Field::pImageIndices, i),
                                 "%" PRIu32 " was not acquired from the swapchain.",
                                 image_index);
            }
    ...
    if (image_in_use) {
        skip |= LogError("VUID-VkReleaseSwapchainImagesInfoEXT-pImageIndices-07786",
                         pReleaseInfo->swapchain,
                         release_info_loc,
...
bool CoreChecks::PreCallValidateCreateSharedSwapchainsKHR(VkDevice device,
    ...
    if (pCreateInfos) {
        for (uint32_t i = 0; i < swapchainCount; i++) {
            ...
            skip |= ValidateCreateSwapchain(pCreateInfos[i],
                                            surface_state.get(),
...
bool CoreChecks::ValidateAcquireNextImage(VkDevice device,
                                          VkSwapchainKHR swapchain,
                                          uint64_t timeout,
    ...
    if (auto semaphore_state = Get<vvl::Semaphore>(semaphore)) {
        if (semaphore_state->type != VK_SEMAPHORE_TYPE_BINARY) {
            skip |= LogError(semaphore_type_vuid,
                             semaphore,
                             loc,
                             "%s is not a VK_SEMAPHORE_TYPE_BINARY.",
            ...
            if (!semaphore_state->CanBinaryBeSignaled()) {
                const char* vuid = version_2 ? "VUID-VkAcquireNextImageInfoKHR-semaphore-01288"
                                             : "VUID-vkAcquireNextImageKHR-semaphore-01286";
                skip |= LogError(vuid, semaphore, loc, "Semaphore must not be currently signaled.");
            }
            if (semaphore_state->InUse()) {
                const char* vuid = version_2 ? "VUID-VkAcquireNextImageInfoKHR-semaphore-01781"
    ...
    if (auto swapchain_data = Get<vvl::Swapchain>(swapchain)) {
        if (swapchain_data->retired) {
            const char* vuid = version_2 ? "VUID-VkAcquireNextImageInfoKHR-swapchain-01675"
        ...
        if (present_modes_ci) {
            ...
            min_image_count = 0;
            for (uint32_t i = 0; i < present_modes_ci->presentModeCount; i++) {
                surface_capabilities = surface_state->GetPresentModeSurfaceCapabilities(
                    physical_device, present_modes_ci->pPresentModes[i]);
        ...
        if (timeout == vvl::kU64Max && too_many_already_acquired) {
            const char* vuid =
                version_2 ? "VUID-vkAcquireNextImage2KHR-surface-07784" : "VUID-vkAcquireNextImageKHR-surface-07783";
...
bool CoreChecks::PreCallValidateAcquireNextImageKHR(VkDevice device,
                                                    VkSwapchainKHR swapchain,
                                                    uint64_t timeout,
                                                    VkSemaphore semaphore,
                                                    VkFence fence,
                                                    ...
                                                    const ErrorObject& error_obj) const {
    return ValidateAcquireNextImage(
        device, swapchain, timeout, semaphore, fence, error_obj.location, "VUID-vkAcquireNextImageKHR-semaphore-03265");
}

bool CoreChecks::PreCallValidateAcquireNextImage2KHR(VkDevice device,
    ...
    skip |= ValidateDeviceMaskToPhysicalDeviceCount(pAcquireInfo->deviceMask,
                                                    objlist,
                                                    acquire_info_loc.dot(Field::deviceMask),
                                                    "VUID-VkAcquireNextImageInfoKHR-deviceMask-01290");
    skip |= ValidateDeviceMaskToZero(pAcquireInfo->deviceMask,
                                     objlist,
                                     acquire_info_loc.dot(Field::deviceMask),
                                     "VUID-VkAcquireNextImageInfoKHR-deviceMask-01291");
    skip |= ValidateAcquireNextImage(device,
...
bool CoreChecks::PreCallValidateWaitForPresentKHR(VkDevice device,
                                                  VkSwapchainKHR swapchain,
                                                  uint64_t presentId,
                                                  uint64_t timeout,
                                                  const ErrorObject& error_obj) const {
    ...
    if (!enabled_features.presentWait) {
        skip |= LogError("VUID-vkWaitForPresentKHR-presentWait-06234",
                         swapchain,
                         error_obj.location,
    ...
    if (auto swapchain_state = Get<vvl::Swapchain>(swapchain)) {
        if (swapchain_state->retired) {
            skip |= LogError("VUID-vkWaitForPresentKHR-swapchain-04997",
                             swapchain,
                             error_obj.location,
...
bool CoreChecks::PreCallValidateDestroySurfaceKHR(VkInstance instance,
    ...
    if (surface_state && surface_state->swapchain) {
        skip |= LogError("VUID-vkDestroySurfaceKHR-surface-01266",
                         instance,
                         error_obj.location,
...
bool CoreChecks::PreCallValidateGetPhysicalDeviceWaylandPresentationSupportKHR(VkPhysicalDevice physicalDevice,
    ...
    return ValidateQueueFamilyIndex(*pd_state,
                                    queueFamilyIndex,
...
bool CoreChecks::PreCallValidateGetPhysicalDeviceSurfaceSupportKHR(VkPhysicalDevice physicalDevice,
                                                                   uint32_t queueFamilyIndex,
                                                                   VkSurfaceKHR surface,
                                                                   VkBool32* pSupported,
                                                                   const ErrorObject& error_obj) const {
...
bool CoreChecks::PreCallValidateGetDisplayPlaneSupportedDisplaysKHR(VkPhysicalDevice physicalDevice,
                                                                    uint32_t planeIndex,
    ...
    skip |= ValidateGetPhysicalDeviceDisplayPlanePropertiesKHRQuery(
        physicalDevice, planeIndex, error_obj.location.dot(Field::planeIndex));
    return skip;
}

bool CoreChecks::PreCallValidateGetDisplayPlaneCapabilitiesKHR(VkPhysicalDevice physicalDevice,
...
bool CoreChecks::PreCallValidateGetDisplayPlaneCapabilities2KHR(VkPhysicalDevice physicalDevice,
    ...
    skip |= ValidateGetPhysicalDeviceDisplayPlanePropertiesKHRQuery(
        ...
        error_obj.location.dot(Field::pDisplayPlaneInfo).dot(Field::planeIndex));
    return skip;
}

bool CoreChecks::PreCallValidateCreateDisplayPlaneSurfaceKHR(VkInstance instance,
    ...
    if (pCreateInfo->alphaMode == VK_DISPLAY_PLANE_ALPHA_GLOBAL_BIT_KHR) {
        ...
        if ((global_alpha > 1.0f) || (global_alpha < 0.0f)) {
            skip |= LogError("VUID-VkDisplaySurfaceCreateInfoKHR-alphaMode-01254",
                             display_mode,
                             create_info_loc.dot(Field::globalAlpha),
                             "is %f, but alphaMode is VK_DISPLAY_PLANE_ALPHA_GLOBAL_BIT_KHR.",
                             global_alpha);
        }
    }
    ...
    if (width >= device_properties.limits.maxImageDimension2D) {
        skip |= LogError("VUID-VkDisplaySurfaceCreateInfoKHR-width-01256",
                         display_mode,
                         create_info_loc.dot(Field::imageExtent).dot(Field::width),
                         "(%" PRIu32 ") exceeds device limit maxImageDimension2D (%" PRIu32 ").",
                         width,
    ...
    if (pd_state->vkGetPhysicalDeviceDisplayPlanePropertiesKHR_called) {
        if (plane_index >= pd_state->display_plane_property_count) {
            skip |= LogError("VUID-VkDisplaySurfaceCreateInfoKHR-planeIndex-01252",
                             display_mode,
                             create_info_loc.dot(Field::planeIndex),
                             "(%" PRIu32 ") must be in the range [0, %" PRIu32 "] that was returned by "
                             "vkGetPhysicalDeviceDisplayPlanePropertiesKHR "
                             "or vkGetPhysicalDeviceDisplayPlaneProperties2KHR. Do you have the plane index hardcoded?",
                             plane_index,
                             pd_state->display_plane_property_count - 1);
        } else {
            ...
            if ((pCreateInfo->alphaMode & plane_capabilities.supportedAlpha) == 0) {
                skip |= LogError("VUID-VkDisplaySurfaceCreateInfoKHR-alphaMode-01255",
                                 display_mode,
                                 create_info_loc,
                                 "alphaMode is %s but planeIndex %" PRIu32 " supportedAlpha (%s) "
                                 "does not support the mode.",
                                 string_VkDisplayPlaneAlphaFlagBitsKHR(pCreateInfo->alphaMode),
                                 plane_index,
...
#ifdef VK_USE_PLATFORM_WIN32_KHR
bool CoreChecks::PreCallValidateAcquireFullScreenExclusiveModeEXT(VkDevice device,
                                                                  VkSwapchainKHR swapchain,
                                                                  const ErrorObject& error_obj) const {
    bool skip = false;
    ...
    if (swapchain_state->retired) {
        skip |= LogError("VUID-vkAcquireFullScreenExclusiveModeEXT-swapchain-02674",
                         device,
                         error_obj.location,
                         "swapchain %s is retired.",
    ...
    if (!surface_full_screen_exclusive_info || surface_full_screen_exclusive_info->fullScreenExclusive !=
    ...
    if (swapchain_state->exclusive_full_screen_access) {
        skip |= LogError("VUID-vkAcquireFullScreenExclusiveModeEXT-swapchain-02676",
                         device,
                         error_obj.location,
                         "swapchain %s already has exclusive full-screen access.",
                         FormatHandle(swapchain).c_str());
    }
...
bool CoreChecks::PreCallValidateReleaseFullScreenExclusiveModeEXT(VkDevice device,
                                                                  VkSwapchainKHR swapchain,
                                                                  const ErrorObject& error_obj) const {
    bool skip = false;
    ...
    if (swapchain_state->retired) {
        skip |= LogError("VUID-vkReleaseFullScreenExclusiveModeEXT-swapchain-02677",
                         device,
                         error_obj.location,
                         "swapchain %s is retired.",
                         FormatHandle(swapchain).c_str());
...
bool CoreChecks::ValidatePhysicalDeviceSurfaceSupport(VkPhysicalDevice physicalDevice,
                                                      VkSurfaceKHR surface,
                                                      const char* vuid,
...
bool CoreChecks::PreCallValidateGetDeviceGroupSurfacePresentModes2EXT(
    VkDevice device,
    ...
    if (physical_device_count == 1) {
        skip |= ValidatePhysicalDeviceSurfaceSupport(physical_device,
        ...
        for (uint32_t i = 0; i < physical_device_count; ++i) {
            skip |=
                ValidatePhysicalDeviceSurfaceSupport(device_group_create_info.pPhysicalDevices[i],
                                                     pSurfaceInfo->surface,
                                                     "VUID-vkGetDeviceGroupSurfacePresentModes2EXT-pSurfaceInfo-06213",
                                                     error_obj.location);
        }
...
bool CoreChecks::PreCallValidateGetPhysicalDeviceSurfacePresentModes2EXT(
    ...
    skip |= ValidatePhysicalDeviceSurfaceSupport(physicalDevice,
                                                 pSurfaceInfo->surface,
...
bool CoreChecks::PreCallValidateGetDeviceGroupSurfacePresentModesKHR(VkDevice device,
                                                                     VkSurfaceKHR surface,
                                                                     VkDeviceGroupPresentModeFlagsKHR* pModes,
                                                                     const ErrorObject& error_obj) const {
    bool skip = false;
    ...
    if (physical_device_count == 1) {
        ...
        for (uint32_t i = 0; i < physical_device_count; ++i) {
            skip |= ValidatePhysicalDeviceSurfaceSupport(device_group_create_info.pPhysicalDevices[i],
                                                         surface,
...
bool CoreChecks::PreCallValidateGetPhysicalDevicePresentRectanglesKHR(VkPhysicalDevice physicalDevice,
                                                                      VkSurfaceKHR surface,
                                                                      uint32_t* pRectCount,
                                                                      VkRect2D* pRects,
                                                                      const ErrorObject& error_obj) const {
    ...
    skip |= ValidatePhysicalDeviceSurfaceSupport(
        physicalDevice, surface, "VUID-vkGetPhysicalDevicePresentRectanglesKHR-surface-06211", error_obj.location);

    return skip;
}

bool CoreChecks::PreCallValidateGetPhysicalDeviceSurfaceCapabilities2EXT(
    VkPhysicalDevice physicalDevice,
    VkSurfaceKHR surface,
...
bool CoreChecks::PreCallValidateGetPhysicalDeviceSurfaceCapabilities2KHR(
    VkPhysicalDevice physicalDevice,
    const VkPhysicalDeviceSurfaceInfo2KHR* pSurfaceInfo,
    ...
    if (IsExtEnabled(device_extensions.vk_ext_surface_maintenance1)) {
        ...
        if (surface_present_mode) {
            ...
            bool found_match =
                std::find(present_modes.begin(), present_modes.end(), present_mode) != present_modes.end();
            if (!found_match) {
                skip |= LogError("VUID-VkSurfacePresentModeEXT-presentMode-07780",
                                 device,
                                 error_obj.location,
                                 "is called with VK_EXT_surface_maintenance1 enabled and "
                                 "a VkSurfacePresentModeEXT structure included in "
                                 "the pNext chain of VkPhysicalDeviceSurfaceInfo2KHR, but the specified presentMode "
                                 "(%s) is not among "
...
#if defined(VK_USE_PLATFORM_WIN32_KHR)
    if (IsExtEnabled(device_extensions.vk_khr_win32_surface) &&
        IsExtEnabled(device_extensions.vk_ext_full_screen_exclusive)) {
        if (const auto* full_screen_info =
                vku::FindStructInPNextChain<VkSurfaceFullScreenExclusiveInfoEXT>(pSurfaceInfo->pNext);
            full_screen_info &&
            full_screen_info->fullScreenExclusive == VK_FULL_SCREEN_EXCLUSIVE_APPLICATION_CONTROLLED_EXT) {
            if (const auto* win32_full_screen_info =
                ...
                skip |= LogError("VUID-VkPhysicalDeviceSurfaceInfo2KHR-pNext-02672",
                                 objlist,
...
bool CoreChecks::PreCallValidateGetPhysicalDeviceSurfaceCapabilitiesKHR(VkPhysicalDevice physicalDevice,
                                                                        VkSurfaceKHR surface,
...
bool CoreChecks::PreCallValidateGetPhysicalDeviceSurfaceFormatsKHR(VkPhysicalDevice physicalDevice,
                                                                   VkSurfaceKHR surface,
                                                                   uint32_t* pSurfaceFormatCount,
    ...
    skip |= ValidatePhysicalDeviceSurfaceSupport(
        physicalDevice, surface, "VUID-vkGetPhysicalDeviceSurfaceFormatsKHR-surface-06525", error_obj.location);

    return skip;
}

bool CoreChecks::PreCallValidateGetPhysicalDeviceSurfacePresentModesKHR(VkPhysicalDevice physicalDevice,
                                                                        VkSurfaceKHR surface,
                                                                        uint32_t* pPresentModeCount,
                                                                        VkPresentModeKHR* pPresentModes,
                                                                        const ErrorObject& error_obj) const {
...
bool CoreChecks::ValidateGetPhysicalDeviceDisplayPlanePropertiesKHRQuery(VkPhysicalDevice physicalDevice,
                                                                         uint32_t planeIndex,
                                                                         const Location& loc) const {
    bool skip = false;
    auto pd_state = Get<vvl::PhysicalDevice>(physicalDevice);
