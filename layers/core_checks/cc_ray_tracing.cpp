...
bool CoreChecks::ValidateInsertAccelerationStructureMemoryRange(VkAccelerationStructureNV as,
                                                                const vvl::DeviceMemory& mem_info,
                                                                VkDeviceSize mem_offset,
                                                                const Location& loc) const {
    return ValidateInsertMemoryRange(
        VulkanTypedHandle(as, kVulkanObjectTypeAccelerationStructureNV), mem_info, mem_offset, loc);
}
...
bool CoreChecks::PreCallValidateCreateAccelerationStructureNV(VkDevice device,
    ...
    if (pCreateInfo != nullptr && pCreateInfo->info.type == VK_ACCELERATION_STRUCTURE_TYPE_BOTTOM_LEVEL_NV) {
        for (uint32_t i = 0; i < pCreateInfo->info.geometryCount; i++) {
            skip |= ValidateGeometryNV(
                pCreateInfo->info.pGeometries[i],
...
bool CoreChecks::PreCallValidateCreateAccelerationStructureKHR(VkDevice device,
    ...
    if (!(buffer_state->usage & VK_BUFFER_USAGE_ACCELERATION_STRUCTURE_STORAGE_BIT_KHR)) {
        skip |= LogError("VUID-VkAccelerationStructureCreateInfoKHR-buffer-03614",
                         buffer_state->Handle(),
                         error_obj.location.dot(Field::pCreateInfo).dot(Field::buffer),
                         "was created with %s.",
    ...
    if (buffer_state->create_info.flags & VK_BUFFER_CREATE_SPARSE_RESIDENCY_BIT) {
        skip |= LogError("VUID-VkAccelerationStructureCreateInfoKHR-buffer-03615",
                         buffer_state->Handle(),
                         error_obj.location.dot(Field::pCreateInfo).dot(Field::buffer),
                         "was created with %s.",
    ...
    if (pCreateInfo->offset + pCreateInfo->size > buffer_state->create_info.size) {
        skip |= LogError("VUID-VkAccelerationStructureCreateInfoKHR-offset-03616",
                         buffer_state->Handle(),
...
bool CoreChecks::PreCallValidateBindAccelerationStructureMemoryNV(
    VkDevice device,
    ...
    for (uint32_t i = 0; i < bindInfoCount; i++) {
        ...
        if (as_state->HasFullRangeBound()) {
            skip |= LogError("VUID-VkBindAccelerationStructureMemoryInfoNV-accelerationStructure-03620",
        ...
        if (mem_info) {
            skip |= ValidateInsertAccelerationStructureMemoryRange(
                info.accelerationStructure, *mem_info, info.memoryOffset, bind_info_loc.dot(Field::memoryOffset));
            skip |= ValidateMemoryTypes(*mem_info,
                                        as_state->memory_requirements.memoryTypeBits,
                                        bind_info_loc.dot(Field::accelerationStructure),
        ...
        if (SafeModulo(info.memoryOffset, as_state->memory_requirements.alignment) != 0) {
            skip |= LogError(
                ...
                info.memoryOffset,
                as_state->memory_requirements.alignment,
        ...
        if (mem_info) {
            ...
            if (as_state->memory_requirements.size > (mem_info->allocate_info.allocationSize - info.memoryOffset)) {
                skip |=
                    LogError("VUID-VkBindAccelerationStructureMemoryInfoNV-size-03624",
                             info.accelerationStructure,
                             ...
                             as_state->memory_requirements.size,
                             bind_info_loc.dot(Field::memoryOffset).Fields().c_str(),
                             info.memoryOffset,
                             mem_info->allocate_info.allocationSize - info.memoryOffset,
                             FormatHandle(info.accelerationStructure).c_str(),
                             as_state->memory_requirements.size);
            }
        }
...
bool CoreChecks::PreCallValidateGetAccelerationStructureHandleNV(VkDevice device,
    ...
    if (auto as_state = Get<vvl::AccelerationStructureNV>(accelerationStructure)) {
        skip |= VerifyBoundMemoryIsValid(as_state->MemState(),
                                         LogObjectList(accelerationStructure),
                                         as_state->Handle(),
...
bool CoreChecks::ValidateAccelerationStructuresMemoryAlisasing(
    ...
    if (info_in_mode_update && info.srcAccelerationStructure != info.dstAccelerationStructure && src_as_state &&
        dst_as_state) {
        ...
        skip |= ValidateAccelStructsMemoryDoNotOverlap(error_obj.location,
                                                       objlist,
                                                       *src_as_state,
    ...
    for (auto [other_info_j, other_info] : vvl::enumerate(pInfos + info_i + 1, infoCount - (info_i + 1))) {
        ...
        // Here validation is pessimistic: if one buffer associated to pInfos[other_info_j].scratchData.deviceAddress
        // has an overlap, an error will be logged.
        ...
        // Validate destination acceleration structure's memory is not overlapped by another source acceleration
        // structure's memory that is going to be updated by this cmd
        if (dst_as_state && other_src_as_state) {
            if (other_info->mode == VK_BUILD_ACCELERATION_STRUCTURE_MODE_UPDATE_KHR) {
                const char* vuid =
                    error_obj.location.function == Func::vkCmdBuildAccelerationStructuresKHR
                ...
                skip |= ValidateAccelStructsMemoryDoNotOverlap(error_obj.location,
                                                               ...
                                                               *dst_as_state,
                                                               info_i_loc.dot(Field::dstAccelerationStructure),
                                                               *other_src_as_state,
                                                               other_info_j_loc.dot(Field::srcAccelerationStructure),
                                                               vuid);
            }
        }
        ...
        if (dst_as_state && other_dst_as_state) {
            ...
            skip |= ValidateAccelStructsMemoryDoNotOverlap(error_obj.location,
                                                           ...
                                                           *dst_as_state,
                                                           info_i_loc.dot(Field::dstAccelerationStructure),
                                                           *other_dst_as_state,
                                                           other_info_j_loc.dot(Field::dstAccelerationStructure),
                                                           vuid);
        }
    }
...
bool CoreChecks::ValidateAccelerationStructuresDeviceScratchBufferMemoryAlisasing(
    ...
    uint32_t info_i,
    const VkAccelerationStructureBuildRangeInfoKHR* const* pp_range_infos,
    const ErrorObject& error_obj) const {
    using sparse_container::range;
    ...
    const auto src_as_state = Get<vvl::AccelerationStructureKHR>(info.srcAccelerationStructure);
    const auto dst_as_state = Get<vvl::AccelerationStructureKHR>(info.dstAccelerationStructure);
    const rt::BuildType rt_build_type = error_obj.location.function == Func::vkBuildAccelerationStructuresKHR
                                            ? rt::BuildType::Host
                                            : rt::BuildType::Device;
    ...
    const VkDeviceSize assumed_scratch_size =
        rt::ComputeScratchSize(rt_build_type, device, info, pp_range_infos[info_i]);

    if (dst_as_state) {
        vvl::span<vvl::Buffer* const> dummy(nullptr, 0);
        skip |= ValidateScratchMemoryNoOverlap(error_obj.location,
                                               objlist,
                                               info_scratches,
    ...
    for (uint32_t other_info_j = info_i + 1; other_info_j < info_count; ++other_info_j) {
        ...
        if (other_dst_as_state) {
            ...
            skip |= ValidateScratchMemoryNoOverlap(error_obj.location,
                                                   ...
                                                   *other_dst_as_state,
                                                   other_info_j_loc.dot(Field::dstAccelerationStructure),
                                                   other_info_scratches,
                                                   other_info.scratchData.deviceAddress,
                                                   assumed_other_scratch_size,
                                                   &other_scratch_address_loc);
        }
    }
...
bool CoreChecks::PreCallValidateGetAccelerationStructureDeviceAddressKHR(
    ...
    if (!enabled_features.accelerationStructure) {
        skip |= LogError("VUID-vkGetAccelerationStructureDeviceAddressKHR-accelerationStructure-08935",
                         device,
                         error_obj.location,
    ...
    if (physical_device_count > 1 && !enabled_features.bufferDeviceAddressMultiDevice &&
        ...
        skip |= LogError("VUID-vkGetAccelerationStructureDeviceAddressKHR-device-03504",
                         device,
                         error_obj.location,
    ...
    if (const auto accel_struct = Get<vvl::AccelerationStructureKHR>(pInfo->accelerationStructure)) {
        ...
        skip |= ValidateMemoryIsBoundToBuffer(device,
                                              *accel_struct->buffer_state,
        ...
        if (!(accel_struct->buffer_state->usage & VK_BUFFER_USAGE_SHADER_DEVICE_ADDRESS_BIT)) {
            skip |= LogError("VUID-vkGetAccelerationStructureDeviceAddressKHR-pInfo-09542",
                             LogObjectList(device),
                             info_loc.dot(Field::accelerationStructure).dot(Field::buffer),
                             "was created with usage flag(s) %s.",
...
bool CoreChecks::ValidateAccelerationBuffers(VkCommandBuffer cmd_buffer,
                                             uint32_t info_i,
    ...
    auto buffer_check =
        [this, &pick_vuid](uint32_t gi, const VkDeviceOrHostAddressConstKHR address, const Location& geom_loc) -> bool {
        const auto buffer_states = GetBuffersByAddress(address.deviceAddress);
        const bool no_valid_buffer_found =
            !buffer_states.empty() &&
            std::none_of(buffer_states.begin(), buffer_states.end(), [](const vvl::Buffer* buffer_state) {
        ...
        if (no_valid_buffer_found) {
            ...
            return LogError(pick_vuid("VUID-vkCmdBuildAccelerationStructuresKHR-geometry-03673",
                                      "VUID-vkCmdBuildAccelerationStructuresIndirectKHR-geometry-03673"),
                            objlist,
                            geom_loc,
                            "has no buffer which created with "
    ...
    for (uint32_t geom_i = 0; geom_i < info.geometryCount; ++geom_i) {
        ...
        switch (geom_data.geometryType) {
            ...
            {
                skip |= buffer_check(
                    geom_i, geom_data.geometry.triangles.vertexData, p_geom_geom_triangles_loc.dot(Field::vertexData));
                skip |= buffer_check(
                    geom_i, geom_data.geometry.triangles.indexData, p_geom_geom_triangles_loc.dot(Field::indexData));
                skip |= buffer_check(geom_i,
                                     geom_data.geometry.triangles.transformData,
                ...
                if (vertex_buffer_states.empty() && geometry_build_ranges[geom_i].primitiveCount > 0) {
                    ...
                    BufferAddressValidation<1> buffer_address_validator = {
                        { { { pick_vuid("VUID-vkCmdBuildAccelerationStructuresKHR-pInfos-03805",
                              ...
                              [this](vvl::Buffer* const buffer_state, std::string* out_error_msg) {
                                  return BufferAddressValidation<1>::ValidateMemoryBoundToBuffer(
                                      *this, buffer_state, out_error_msg);
                              },
                              []() {
                    ...
                    skip |= buffer_address_validator.LogErrorsIfNoValidBuffer(
                        *this,
                        vertex_buffer_states,
                        p_geom_geom_triangles_loc.dot(Field::vertexData).dot(Field::deviceAddress),
                        LogObjectList(cmd_buffer),
                        geom_data.geometry.triangles.vertexData.deviceAddress);
                }

                if (geom_data.geometry.triangles.indexType != VK_INDEX_TYPE_NONE_KHR) {
                    auto index_buffer_states =
                        GetBuffersByAddress(geom_data.geometry.triangles.indexData.deviceAddress);
                            ...
                            if (geom_i < src_as_state->build_range_infos.size()) {
                                if (const uint32_t recorded_primitive_count =
                                    ...
                                    skip |= LogError(
                                        ...
                                        pp_build_range_info_loc.Fields().c_str(),
                                        geom_i,
                                        geometry_build_ranges[geom_i].primitiveCount,
                ...
                const VkFormatProperties3KHR format_properties =
                    GetPDFormatProperties(geom_data.geometry.triangles.vertexFormat);
                ...
                // Only try to get format info if vertex format is valid
                    ...
                    for (uint32_t component_i = 1; component_i < format_info.component_count; ++component_i) {
                        min_component_bits_size =
                            std::min(format_info.components[component_i].size, min_component_bits_size);
                    }
                    const uint32_t min_component_byte_size = min_component_bits_size / 8;
                    if (SafeModulo(geom_data.geometry.triangles.vertexData.deviceAddress, min_component_byte_size) !=
                        0) {
                        skip |=
            ...
            {
                skip |= buffer_check(
                    geom_i, geom_data.geometry.aabbs.data, p_geom_geom_loc.dot(Field::aabbs).dot(Field::data));
    ...
    if (info_loc.function == Func::vkCmdBuildAccelerationStructuresKHR &&
        ...
        if (const auto dst_as_state = Get<vvl::AccelerationStructureKHR>(info.dstAccelerationStructure)) {
            ...
            if (dst_as_state->create_info.size < as_minimum_size) {
                ...
                skip |= LogError("VUID-vkCmdBuildAccelerationStructuresKHR-pInfos-10126",
                                 objlist,
    ...
    if (buffer_states.empty()) {
        skip |= LogError(pick_vuid("VUID-vkCmdBuildAccelerationStructuresKHR-pInfos-03802",
                         ...
                         "(0x%" PRIx64 ") has no buffer is associated with it.",
                         info.scratchData.deviceAddress);
    } else {
        ...
        const VkDeviceSize scratch_size =
            info_loc.function == Func::vkCmdBuildAccelerationStructuresKHR
                ? rt::ComputeScratchSize(rt::BuildType::Device, device, info, geometry_build_ranges)
                : 1;
        const sparse_container::range<VkDeviceSize> scratch_address_range(
            info.scratchData.deviceAddress, info.scratchData.deviceAddress + scratch_size);
...
bool CoreChecks::CommonBuildAccelerationStructureValidation(const VkAccelerationStructureBuildGeometryInfoKHR& info,
                                                            ...
                                                            LogObjectList object_list) const {
    bool skip = false;
    ...
    if (info.mode == VK_BUILD_ACCELERATION_STRUCTURE_MODE_UPDATE_KHR) {
        if (!src_as_state->is_built) {
            ...
            skip |= LogError(vuid,
                             ...
                             "is VK_BUILD_ACCELERATION_STRUCTURE_MODE_UPDATE_KHR, srcAccelerationStructure must have "
            ...
            if (info.geometryCount != src_as_state->build_info_khr->geometryCount) {
                ...
                skip |= LogError(vuid,
                                 ...
                                 " but geometryCount (%" PRIu32 ") must have the same value as specified when "
                                 "srcAccelerationStructure was last built (%" PRIu32 ").",
                                 info.geometryCount,
                                 src_as_state->build_info_khr->geometryCount);
                ...
                for (uint32_t geom_i = 0; geom_i < info.geometryCount; ++geom_i) {
                    ...
                    if (updated_geometry.geometryType != last_geometry.geometryType) {
                        ...
                        skip |= LogError(vuid,
                                         objlist,
                                         geom_loc.dot(Field::geometryType),
                    ...
                    if (updated_geometry.geometryType == VK_GEOMETRY_TYPE_TRIANGLES_KHR) {
                        if (updated_geometry.geometry.triangles.vertexFormat !=
                            last_geometry.geometry.triangles.vertexFormat) {
                            const char* vuid = info_loc.function == Func::vkCmdBuildAccelerationStructuresKHR
                        ...
                        if (updated_geometry.geometry.triangles.maxVertex !=
                        ...
                        if (updated_geometry.geometry.triangles.indexType !=
                            last_geometry.geometry.triangles.indexType) {
                            const char* vuid = info_loc.function == Func::vkCmdBuildAccelerationStructuresKHR
...
bool CoreChecks::PreCallValidateCmdBuildAccelerationStructuresKHR(
    ...
    for (const auto [info_i, info] : vvl::enumerate(pInfos, infoCount)) {
        ...
        if (const auto src_as_state = Get<vvl::AccelerationStructureKHR>(info->srcAccelerationStructure)) {
            if (info->mode == VK_BUILD_ACCELERATION_STRUCTURE_MODE_UPDATE_KHR) {
                if (!src_as_state->buffer_state) {
                    ...
                    skip |=
                        LogError("VUID-vkCmdBuildAccelerationStructuresKHR-pInfos-03708",
                                 objlist,
                                 info_loc.dot(Field::mode),
        ...
        if (const auto dst_as_state = Get<vvl::AccelerationStructureKHR>(info->dstAccelerationStructure)) {
            ...
            if (info->type == VK_ACCELERATION_STRUCTURE_TYPE_BOTTOM_LEVEL_KHR) {
                if (dst_as_state->create_info.type != VK_ACCELERATION_STRUCTURE_TYPE_BOTTOM_LEVEL_KHR &&
                    ...
                    skip |= LogError("VUID-vkCmdBuildAccelerationStructuresKHR-pInfos-03700",
                                     objlist,
                                     info_loc.dot(Field::type),
                                     "is VK_ACCELERATION_STRUCTURE_TYPE_BOTTOM_LEVEL_KHR, but its "
                                     "dstAccelerationStructure was created with %s.",
            ...
            if (info->type == VK_ACCELERATION_STRUCTURE_TYPE_TOP_LEVEL_KHR) {
                if (dst_as_state->create_info.type != VK_ACCELERATION_STRUCTURE_TYPE_TOP_LEVEL_KHR &&
                    dst_as_state->create_info.type != VK_ACCELERATION_STRUCTURE_TYPE_GENERIC_KHR) {
                    const LogObjectList objlist(device, commandBuffer);
                    skip |= LogError("VUID-vkCmdBuildAccelerationStructuresKHR-pInfos-03699",
                                     objlist,
                                     info_loc.dot(Field::type),
                                     "is VK_ACCELERATION_STRUCTURE_TYPE_TOP_LEVEL_KHR, but its "
                                     "dstAccelerationStructure was created with %s.",
                                     string_VkAccelerationStructureTypeKHR(dst_as_state->create_info.type));
                }
            }
        ...
        skip |= ValidateAccelerationStructuresDeviceScratchBufferMemoryAlisasing(
            commandBuffer, infoCount, pInfos, info_i, ppBuildRangeInfos, error_obj);
    }
...
bool CoreChecks::PreCallValidateBuildAccelerationStructuresKHR(
    ...
    const ErrorObject& error_obj) const {
    bool skip = false;
    skip |= ValidateDeferredOperation(device,
                                      deferredOperation,
    ...
    for (const auto [info_i, info] : vvl::enumerate(pInfos, infoCount)) {
        ...
        if (src_as_state) {
            if (info->mode == VK_BUILD_ACCELERATION_STRUCTURE_MODE_UPDATE_KHR) {
                skip |=
                    ValidateAccelStructBufferMemoryIsHostVisible(*src_as_state,
                                                                 info_loc.dot(Field::srcAccelerationStructure),
        ...
        if (dst_as_state) {
            skip |= ValidateAccelStructBufferMemoryIsHostVisible(*dst_as_state,
                                                                 info_loc.dot(Field::dstAccelerationStructure),
                                                                 "VUID-vkBuildAccelerationStructuresKHR-pInfos-03722");

            skip |=
                ValidateAccelStructBufferMemoryIsNotMultiInstance(*dst_as_state,
                                                                  info_loc.dot(Field::dstAccelerationStructure),
            ...
            if (!(info->mode == VK_BUILD_ACCELERATION_STRUCTURE_MODE_UPDATE_KHR &&
                ...
                if (dst_as_state->create_info.size < as_minimum_size) {
                    ...
                    skip |=
                        LogError("VUID-vkBuildAccelerationStructuresKHR-pInfos-10126",
                                 objlist,
                                 info_loc.dot(Field::dstAccelerationStructure),
                                 " was created with size (%" PRIu64
                                 "), but an acceleration structure build with corresponding ppBuildRangeInfos[%" PRIu32
            ...
            if (info->type == VK_ACCELERATION_STRUCTURE_TYPE_TOP_LEVEL_KHR) {
                if (dst_as_state->create_info.type != VK_ACCELERATION_STRUCTURE_TYPE_TOP_LEVEL_KHR &&
                    dst_as_state->create_info.type != VK_ACCELERATION_STRUCTURE_TYPE_GENERIC_KHR) {
                    skip |= LogError("VUID-vkBuildAccelerationStructuresKHR-pInfos-03699",
                                     info->dstAccelerationStructure,
                                     info_loc.dot(Field::type),
                                     "is VK_ACCELERATION_STRUCTURE_TYPE_TOP_LEVEL_KHR, but its "
                                     "dstAccelerationStructure was built with type %s.",
                                     string_VkAccelerationStructureTypeKHR(dst_as_state->create_info.type));
                }
            }
        }
        ...
        const VkDeviceSize scratch_i_size =
            rt::ComputeScratchSize(rt::BuildType::Host, device, *info, ppBuildRangeInfos[info_i]);
        auto scratch_i_host_addr = reinterpret_cast<uint64_t>(info->scratchData.hostAddress);
        const sparse_container::range<uint64_t> scratch_addr_range(scratch_i_host_addr,
                                                                   scratch_i_host_addr + scratch_i_size);
        ...
        for (auto [other_info_j, other_info] : vvl::enumerate(pInfos + info_i + 1, infoCount - (info_i + 1))) {
            ...
            const sparse_container::range<uint64_t> other_scratch_addr_range(
                other_scratch_host_addr, other_scratch_host_addr + other_scratch_size);
            ...
            if (scratch_addr_range.intersects(other_scratch_addr_range)) {
                ...
                skip |= LogError("VUID-vkBuildAccelerationStructuresKHR-scratchData-03704",
                                 device,
                                 info_loc.dot(Field::scratchData),
                                 "overlaps with %s on host address range %s.\n"
                                 "%s.hostAddress is %p and assumed scratch size is %" PRIu64 ".\n"
                                 "%s.hostAddress is %p and assumed scratch size is %" PRIu64 ".",
                                 other_info_j_scratch_str.c_str(),
                                 string_range_hex(scratch_addr_range & other_scratch_addr_range).c_str(),
                                 info_i_scratch_str.c_str(),
                                 info->scratchData.hostAddress,
                                 scratch_i_size,
                                 other_info_j_scratch_str.c_str(),
                                 other_info->scratchData.hostAddress,
                                 other_scratch_size);
            }
        }
            ...
            if (info->mode == VK_BUILD_ACCELERATION_STRUCTURE_MODE_UPDATE_KHR && src_as_state &&
                ...
                if (geom_i < src_as_state->build_range_infos.size()) {
                    if (const uint32_t recorded_primitive_count =
                        ...
                        skip |= LogError("VUID-vkCmdBuildAccelerationStructuresKHR-primitiveCount-03769",
                                         objlist,
                                         geometry_loc,
                                         " has corresponding VkAccelerationStructureBuildRangeInfoKHR %s[%" PRIu32
                                         "], but this build range has its primitiveCount member set to (%" PRIu32
                                         ") when it was last specified as (%" PRIu32 ").",
                                         error_obj.location.dot(Field::ppBuildRangeInfos, info_i).Fields().c_str(),
                                         geom_i,
                                         ppBuildRangeInfos[info_i][geom_i].primitiveCount,
                                         recorded_primitive_count);
                    }
                }
            }
            ...
            for (uint32_t instance_i = 0; instance_i < ppBuildRangeInfos[info_i][geom_i].primitiveCount; ++instance_i) {
                ...
                if (geom.geometry.instances.data.hostAddress) {
                    if (geom.geometry.instances.arrayOfPointers) {
                        auto instance_pointers_array =
                            reinterpret_cast<VkAccelerationStructureInstanceKHR const* const*>(
                                geom.geometry.instances.data.hostAddress);
                        instance = instance_pointers_array[instance_i];
                    } else {
                        auto instances_array = reinterpret_cast<VkAccelerationStructureInstanceKHR const*>(
                            geom.geometry.instances.data.hostAddress);
                    ...
                    if (!instance) {
                        skip |=
                            LogError("VUID-vkBuildAccelerationStructuresKHR-pInfos-03779",
                                     device,
                                     geometry_loc.dot(Field::geometry)
                                         .dot(Field::instances)
                                         .dot(Field::data)
                                         .dot(Field::hostAddress, instance_i),
                                     "(0x%p) does not reference a valid VkAccelerationStructureKHR object. %s is %s.",
                                     instance,
                                     geometry_loc.dot(Field::geometry)
                                         .dot(Field::instances)
                                         .dot(Field::arrayOfPointers)
                                         .Fields()
                                         .c_str(),
                    ...
                    if (!accel_struct_state) {
                        skip |= LogError("VUID-vkBuildAccelerationStructuresKHR-pInfos-03779",
                                         device,
                                         geometry_loc.dot(Field::geometry)
                                             .dot(Field::instances)
                                             .dot(Field::data)
                                         ...
                                         "(%" PRIu64
                                         ") does not reference a valid VkAccelerationStructureKHR object. %s is %s.",
...
bool CoreChecks::PreCallValidateCmdBuildAccelerationStructuresIndirectKHR(
    VkCommandBuffer commandBuffer,
    uint32_t infoCount,
    const VkAccelerationStructureBuildGeometryInfoKHR* pInfos,
    ...
    if (!cb_state->unprotected) {
        skip |= LogError("VUID-vkCmdBuildAccelerationStructuresIndirectKHR-commandBuffer-09547",
                         commandBuffer,
                         error_obj.location,
    ...
    for (const auto [info_i, info] : vvl::enumerate(pInfos, infoCount)) {
        ...
        if (auto dst_as_state = Get<vvl::AccelerationStructureKHR>(info->dstAccelerationStructure)) {
            ...
            if (info->type == VK_ACCELERATION_STRUCTURE_TYPE_TOP_LEVEL_KHR) {
                if (dst_as_state->create_info.type != VK_ACCELERATION_STRUCTURE_TYPE_TOP_LEVEL_KHR &&
                    dst_as_state->create_info.type != VK_ACCELERATION_STRUCTURE_TYPE_GENERIC_KHR) {
                    skip |= LogError("VUID-vkCmdBuildAccelerationStructuresIndirectKHR-pInfos-03699",
                                     info->dstAccelerationStructure,
                                     info_loc.dot(Field::type),
                                     "is VK_ACCELERATION_STRUCTURE_TYPE_TOP_LEVEL_KHR, but its "
                                     "dstAccelerationStructure was built with type %s.",
...
bool CoreChecks::PreCallValidateCmdBuildAccelerationStructureNV(VkCommandBuffer commandBuffer,
    ...
    if (pInfo != nullptr && pInfo->type == VK_ACCELERATION_STRUCTURE_TYPE_BOTTOM_LEVEL_NV) {
        for (uint32_t i = 0; i < pInfo->geometryCount; i++) {
            skip |= ValidateGeometryNV(pInfo->pGeometries[i],
                                       error_obj.location.dot(Field::pInfo).dot(Field::pGeometries, i));
        }
    }

    if (pInfo != nullptr && pInfo->geometryCount > phys_dev_ext_props.ray_tracing_props_nv.maxGeometryCount) {
        skip |= LogError("VUID-vkCmdBuildAccelerationStructureNV-geometryCount-02241",
                         commandBuffer,
                         error_obj.location,
                         "geometryCount [%" PRIu32 "] must be less than or equal to "
    ...
    if (dst_as_state && pInfo) {
        if (dst_as_state->create_info.info.type != pInfo->type) {
            skip |= LogError("VUID-vkCmdBuildAccelerationStructureNV-dst-02488",
                             commandBuffer,
                             error_obj.location,
        ...
        if (dst_as_state->create_info.info.instanceCount < pInfo->instanceCount) {
            skip |= LogError(
                ...
                "].",
                dst_as_state->create_info.info.instanceCount,
                pInfo->instanceCount);
        }
        if (dst_as_state->create_info.info.geometryCount < pInfo->geometryCount) {
            skip |= LogError(
                ...
                "].",
                dst_as_state->create_info.info.geometryCount,
                pInfo->geometryCount);
        } else {
            for (uint32_t i = 0; i < pInfo->geometryCount; i++) {
                ...
                if (create_geometry_data.triangles.vertexCount < build_geometry_data.triangles.vertexCount) {
                    skip |= LogError("VUID-vkCmdBuildAccelerationStructureNV-dst-02488",
                                     ...
                                     i,
                                     create_geometry_data.triangles.vertexCount,
                                     i,
                                     build_geometry_data.triangles.vertexCount);
                    break;
                }
                if (create_geometry_data.triangles.indexCount < build_geometry_data.triangles.indexCount) {
                    skip |= LogError("VUID-vkCmdBuildAccelerationStructureNV-dst-02488",
                                     ...
                                     i,
                                     create_geometry_data.triangles.indexCount,
                                     i,
                                     build_geometry_data.triangles.indexCount);
                    break;
                }
                if (create_geometry_data.aabbs.numAABBs < build_geometry_data.aabbs.numAABBs) {
                    skip |= LogError("VUID-vkCmdBuildAccelerationStructureNV-dst-02488",
                                     ...
                                     i,
                                     create_geometry_data.aabbs.numAABBs,
                                     i,
                                     build_geometry_data.aabbs.numAABBs);
                    break;
                }
            }
        }
    ...
    if (dst_as_state) {
        skip |= VerifyBoundMemoryIsValid(dst_as_state->MemState(),
                                         LogObjectList(commandBuffer, dst),
                                         dst_as_state->Handle(),
    ...
    if (update == VK_TRUE) {
        if (src == VK_NULL_HANDLE) {
            skip |= LogError("VUID-vkCmdBuildAccelerationStructureNV-update-02489",
                             commandBuffer,
                             error_obj.location,
            ...
            if (!src_as_state || !src_as_state->built ||
                ...
                skip |= LogError("VUID-vkCmdBuildAccelerationStructureNV-update-02490",
                                 commandBuffer,
                                 error_obj.location,
        ...
        if (dst_as_state && !dst_as_state->update_scratch_memory_requirements_checked) {
            skip |= LogWarning("WARNING-vkCmdBuildAccelerationStructureNV-update-requirements",
                               dst,
                               error_obj.location,
        ...
        if (scratch_buffer_state && dst_as_state &&
            dst_as_state->update_scratch_memory_requirements.size >
                (scratch_buffer_state->create_info.size - scratchOffset)) {
            skip |= LogError("VUID-vkCmdBuildAccelerationStructureNV-update-02492",
                             commandBuffer,
                             error_obj.location,
        ...
        if (dst_as_state && !dst_as_state->build_scratch_memory_requirements_checked) {
            skip |=
                LogWarning("WARNING-vkCmdBuildAccelerationStructureNV-scratch-requirements",
                           dst,
                           error_obj.location,
        ...
        if (scratch_buffer_state && dst_as_state &&
            dst_as_state->build_scratch_memory_requirements.size >
                (scratch_buffer_state->create_info.size - scratchOffset)) {
            skip |= LogError("VUID-vkCmdBuildAccelerationStructureNV-update-02491",
                             commandBuffer,
                             error_obj.location,
    ...
    if (instanceData != VK_NULL_HANDLE) {
        if (auto buffer_state = Get<vvl::Buffer>(instanceData)) {
            skip |= ValidateBufferUsageFlags(LogObjectList(commandBuffer, instanceData),
                                             *buffer_state,
                                             VK_BUFFER_USAGE_RAY_TRACING_BIT_NV,
                                             true,
                                             "VUID-VkAccelerationStructureInfoNV-instanceData-02782",
                                             error_obj.location.dot(Field::instanceData));
        }
    }
    if (scratch_buffer_state) {
        skip |= ValidateBufferUsageFlags(LogObjectList(commandBuffer, scratch),
                                         *scratch_buffer_state,
                                         VK_BUFFER_USAGE_RAY_TRACING_BIT_NV,
                                         true,
                                         "VUID-VkAccelerationStructureInfoNV-scratch-02781",
                                         error_obj.location.dot(Field::scratch));
    }
    return skip;
}

bool CoreChecks::PreCallValidateCmdCopyAccelerationStructureNV(VkCommandBuffer commandBuffer,
                                                               VkAccelerationStructureNV dst,
                                                               VkAccelerationStructureNV src,
    ...
    if (dst_as_state) {
        ...
        skip |= VerifyBoundMemoryIsDeviceVisible(dst_as_state->MemState(),
                                                 objlist,
                                                 dst_as_state->Handle(),
                                                 error_obj.location.dot(Field::dst),
    ...
    if (src_as_state) {
        ...
        skip |= VerifyBoundMemoryIsDeviceVisible(src_as_state->MemState(),
                                                 objlist,
                                                 src_as_state->Handle(),
                                                 error_obj.location.dot(Field::src),
                                                 "VUID-vkCmdCopyAccelerationStructureNV-buffer-03718");
        if (!src_as_state->built) {
            skip |= LogError("VUID-vkCmdCopyAccelerationStructureNV-src-04963",
                             commandBuffer,
                             error_obj.location,
    ...
    if (mode == VK_COPY_ACCELERATION_STRUCTURE_MODE_COMPACT_NV) {
        if (src_as_state && (!src_as_state->built || !(src_as_state->build_info.flags &
                                                       VK_BUILD_ACCELERATION_STRUCTURE_ALLOW_COMPACTION_BIT_NV))) {
            skip |= LogError("VUID-vkCmdCopyAccelerationStructureNV-src-03411",
                             commandBuffer,
                             error_obj.location,
    ...
    if (!(mode == VK_COPY_ACCELERATION_STRUCTURE_MODE_COMPACT_NV ||
        ...
        skip |= LogError("VUID-vkCmdCopyAccelerationStructureNV-mode-03410",
                         commandBuffer,
                         error_obj.location,
...
bool CoreChecks::PreCallValidateDestroyAccelerationStructureNV(VkDevice device,
                                                               VkAccelerationStructureNV accelerationStructure,
                                                               const VkAllocationCallbacks* pAllocator,
                                                               const ErrorObject& error_obj) const {
    bool skip = false;
    if (auto as_state = Get<vvl::AccelerationStructureNV>(accelerationStructure)) {
        skip |= ValidateObjectNotInUse(
            as_state.get(), error_obj.location, "VUID-vkDestroyAccelerationStructureNV-accelerationStructure-03752");
    }
    return skip;
}

bool CoreChecks::PreCallValidateDestroyAccelerationStructureKHR(VkDevice device,
    ...
    if (auto as_state = Get<vvl::AccelerationStructureKHR>(accelerationStructure)) {
        skip |= ValidateObjectNotInUse(
            as_state.get(), error_obj.location, "VUID-vkDestroyAccelerationStructureKHR-accelerationStructure-02442");
    }
    return skip;
}
...
void CoreChecks::PostCallRecordCmdWriteAccelerationStructuresPropertiesKHR(
    ...
    cb_state->query_updates.emplace_back(
        [accelerationStructureCount, firstQuery, queryPool](vvl::CommandBuffer& cb_state_arg,
                                                            bool do_validate,
                                                            VkQueryPool& firstPerfQueryPool,
                                                            uint32_t perfPass,
            ...
            for (uint32_t i = 0; i < accelerationStructureCount; i++) {
                ...
                skip |= VerifyQueryIsReset(cb_state_arg,
                                           query_obj,
...
bool CoreChecks::PreCallValidateWriteAccelerationStructuresPropertiesKHR(
    VkDevice device,
    uint32_t accelerationStructureCount,
    const VkAccelerationStructureKHR* pAccelerationStructures,
    VkQueryType queryType,
    ...
    for (uint32_t i = 0; i < accelerationStructureCount; ++i) {
        ...
        skip |= ValidateAccelStructBufferMemoryIsHostVisible(
            *as_state, as_loc, "VUID-vkWriteAccelerationStructuresPropertiesKHR-buffer-03733");

        skip |= ValidateAccelStructBufferMemoryIsNotMultiInstance(
            *as_state, as_loc, "VUID-vkWriteAccelerationStructuresPropertiesKHR-buffer-03784");

        if (!as_state->is_built) {
            skip |= LogError("VUID-vkWriteAccelerationStructuresPropertiesKHR-pAccelerationStructures-04964",
                             device,
                             as_loc,
                             "has not been built.");
            ...
            if (queryType == VK_QUERY_TYPE_ACCELERATION_STRUCTURE_COMPACTED_SIZE_KHR) {
                if (!(as_state->build_info_khr->flags & VK_BUILD_ACCELERATION_STRUCTURE_ALLOW_COMPACTION_BIT_KHR)) {
                    ...
                    skip |=
                        LogError("VUID-vkWriteAccelerationStructuresPropertiesKHR-accelerationStructures-03431",
                                 objlist,
                                 as_loc,
                                 "has flags %s.",
...
bool CoreChecks::PreCallValidateCmdWriteAccelerationStructuresPropertiesKHR(
    ...
    if (query_pool_ci.queryType != queryType) {
        skip |= LogError("VUID-vkCmdWriteAccelerationStructuresPropertiesKHR-queryPool-02493",
                         commandBuffer,
    ...
    for (uint32_t i = 0; i < accelerationStructureCount; ++i) {
        ...
        skip |= ValidateMemoryIsBoundToBuffer(commandBuffer,
                                              *as_state->buffer_state,
                                              as_loc.dot(Field::buffer),
                                              "VUID-vkCmdWriteAccelerationStructuresPropertiesKHR-buffer-03736");

        if (!as_state->is_built) {
            skip |= LogError("VUID-vkCmdWriteAccelerationStructuresPropertiesKHR-pAccelerationStructures-04964",
                             commandBuffer,
                             as_loc,
                             "has not been built.");
        } else {
            if (queryType == VK_QUERY_TYPE_ACCELERATION_STRUCTURE_COMPACTED_SIZE_KHR &&
                as_state->build_info_khr.has_value()) {
                if (!(as_state->build_info_khr->flags & VK_BUILD_ACCELERATION_STRUCTURE_ALLOW_COMPACTION_BIT_KHR)) {
                    skip |= LogError(
...
bool CoreChecks::PreCallValidateCmdWriteAccelerationStructuresPropertiesNV(
    ...
    const auto& query_pool_ci = query_pool_state->create_info;
    if (query_pool_ci.queryType != queryType) {
        skip |= LogError("VUID-vkCmdWriteAccelerationStructuresPropertiesNV-queryPool-03755",
                         commandBuffer,
                         error_obj.location.dot(Field::queryType),
...
bool CoreChecks::ValidateCopyAccelerationStructureInfoKHR(const VkCopyAccelerationStructureInfoKHR& as_info,
    ...
    if (src_as_state) {
        if (!src_as_state->is_built) {
            skip |= LogError("VUID-VkCopyAccelerationStructureInfoKHR-src-04963",
                             device,
                             info_loc.dot(Field::src),
                             "has not been built.");
        }
        ...
        if (as_info.mode == VK_COPY_ACCELERATION_STRUCTURE_MODE_COMPACT_KHR &&
            ...
            if (!(src_as_state->build_info_khr->flags & VK_BUILD_ACCELERATION_STRUCTURE_ALLOW_COMPACTION_BIT_KHR)) {
                ...
                skip |=
                    LogError("VUID-VkCopyAccelerationStructureInfoKHR-src-03411",
                             objlist,
                             info_loc.dot(Field::src),
    ...
    if (dst_as_state) {
        if (auto buffer_state = Get<vvl::Buffer>(dst_as_state->create_info.buffer)) {
            skip |= ValidateMemoryIsBoundToBuffer(device,
                                                  *buffer_state,
                                                  info_loc.dot(Field::dst),
                                                  "VUID-VkCopyAccelerationStructureInfoKHR-buffer-03719");
        }
    }

    if (src_as_state && dst_as_state) {
        skip |= ValidateAccelStructsMemoryDoNotOverlap(info_loc.function,
                                                       LogObjectList(),
                                                       *src_as_state,
                                                       info_loc.dot(Field::src),
                                                       *dst_as_state,
...
bool CoreChecks::PreCallValidateDestroyDeferredOperationKHR(VkDevice device,
    ...
    skip |= ValidateDeferredOperation(device,
                                      operation,
                                      error_obj.location.dot(Field::operation),
...
bool CoreChecks::PreCallValidateCopyAccelerationStructureKHR(VkDevice device,
    ...
    if (auto src_accel_state = Get<vvl::AccelerationStructureKHR>(pInfo->src)) {
        skip |= ValidateAccelStructBufferMemoryIsHostVisible(
            *src_accel_state, info_loc.dot(Field::src), "VUID-vkCopyAccelerationStructureKHR-buffer-03727");

        skip |= ValidateAccelStructBufferMemoryIsNotMultiInstance(
            *src_accel_state, info_loc.dot(Field::src), "VUID-vkCopyAccelerationStructureKHR-buffer-03780");
    }

    if (auto dst_accel_state = Get<vvl::AccelerationStructureKHR>(pInfo->dst)) {
        skip |= ValidateAccelStructBufferMemoryIsHostVisible(
            *dst_accel_state, info_loc.dot(Field::dst), "VUID-vkCopyAccelerationStructureKHR-buffer-03728");

        skip |= ValidateAccelStructBufferMemoryIsNotMultiInstance(
            *dst_accel_state, info_loc.dot(Field::dst), "VUID-vkCopyAccelerationStructureKHR-buffer-03781");
    }
    return skip;
}

bool CoreChecks::ValidateVkCopyAccelerationStructureToMemoryInfoKHR(
    const vvl::AccelerationStructureKHR& src_accel_struct, LogObjectList objlist, const Location& loc) const {
    bool skip = false;
    if (!src_accel_struct.is_built) {
        objlist.add(src_accel_struct.Handle());
        skip |= LogError("VUID-VkCopyAccelerationStructureToMemoryInfoKHR-src-04959",
                         objlist,
                         loc.dot(Field::src),
                         "has not been built.");
    }
...
bool CoreChecks::PreCallValidateCopyAccelerationStructureToMemoryKHR(
    VkDevice device,
    VkDeferredOperationKHR deferredOperation,
...
bool CoreChecks::PreCallValidateCmdCopyAccelerationStructureToMemoryKHR(
    ...
    if (auto src_accel_struct = Get<vvl::AccelerationStructureKHR>(pInfo->src)) {
        skip |= ValidateVkCopyAccelerationStructureToMemoryInfoKHR(
            *src_accel_struct, LogObjectList(commandBuffer), error_obj.location.dot(Field::pInfo));
    ...
    if (buffer_states.empty()) {
        skip |= LogError("VUID-vkCmdCopyAccelerationStructureToMemoryKHR-pInfo-03739",
                         commandBuffer,
                         error_obj.location.dot(Field::pInfo).dot(Field::dst).dot(Field::deviceAddress),
                         "(0x%" PRIx64 ") is not a valid buffer address.",
                         dst_address);
    } else {
        BufferAddressValidation<1> buffer_address_validator = {
...
bool CoreChecks::PreCallValidateCopyMemoryToAccelerationStructureKHR(
    ...
    if (auto accel_state = Get<vvl::AccelerationStructureKHR>(pInfo->dst)) {
        skip |=
            ValidateAccelStructBufferMemoryIsHostVisible(*accel_state,
...
bool CoreChecks::PreCallValidateCmdCopyMemoryToAccelerationStructureKHR(
    ...
    if (buffer_states.empty()) {
        skip |= LogError("VUID-vkCmdCopyMemoryToAccelerationStructureKHR-pInfo-03742",
                         commandBuffer,
...
bool CoreChecks::PreCallValidateGetRayTracingShaderGroupHandlesKHR(VkDevice device,
                                                                   VkPipeline pipeline,
                                                                   uint32_t firstGroup,
                                                                   uint32_t groupCount,
                                                                   size_t dataSize,
    ...
    if (pipeline_ptr->pipeline_type != VK_PIPELINE_BIND_POINT_RAY_TRACING_KHR) {
        skip |= LogError("VUID-vkGetRayTracingShaderGroupHandlesKHR-pipeline-04619",
                         pipeline,
                         error_obj.location.dot(Field::pipeline),
                         "is a %s pipeline.",
                         string_VkPipelineBindPoint(pipeline_ptr->pipeline_type));
        return skip;
    }
    ...
    if (pipeline_state.create_flags & VK_PIPELINE_CREATE_LIBRARY_BIT_KHR) {
        if (!enabled_features.pipelineLibraryGroupHandles) {
            skip |= LogError("VUID-vkGetRayTracingShaderGroupHandlesKHR-pipeline-07828",
                             pipeline,
                             error_obj.location.dot(Field::pipeline),
    ...
    if (dataSize < (phys_dev_ext_props.ray_tracing_props_khr.shaderGroupHandleSize * groupCount)) {
        skip |= LogError("VUID-vkGetRayTracingShaderGroupHandlesKHR-dataSize-02420",
                         device,
                         error_obj.location.dot(Field::dataSize),
                         "(%zu) must be at least "
                         "shaderGroupHandleSize (%" PRIu32 ") * groupCount (%" PRIu32 ").",
                         dataSize,
                         phys_dev_ext_props.ray_tracing_props_khr.shaderGroupHandleSize,
                         groupCount);
    }
    ...
    if (firstGroup >= total_group_count) {
        skip |= LogError("VUID-vkGetRayTracingShaderGroupHandlesKHR-firstGroup-04050",
                         device,
                         error_obj.location.dot(Field::firstGroup),
                         "(%" PRIu32 ") must be less than the number of shader groups in pipeline (%" PRIu32 ").",
                         firstGroup,
                         total_group_count);
    }
    if ((firstGroup + groupCount) > total_group_count) {
        skip |= LogError("VUID-vkGetRayTracingShaderGroupHandlesKHR-firstGroup-02419",
                         device,
                         error_obj.location.dot(Field::firstGroup),
                         "(%" PRIu32 ") + groupCount (%" PRIu32 ") must be less than or equal to the number "
...
bool CoreChecks::PreCallValidateGetRayTracingCaptureReplayShaderGroupHandlesKHR(VkDevice device,
                                                                                VkPipeline pipeline,
                                                                                uint32_t firstGroup,
                                                                                uint32_t groupCount,
                                                                                size_t dataSize,
    ...
    if (dataSize < (phys_dev_ext_props.ray_tracing_props_khr.shaderGroupHandleCaptureReplaySize * groupCount)) {
        skip |= LogError("VUID-vkGetRayTracingCaptureReplayShaderGroupHandlesKHR-dataSize-03484",
                         device,
                         error_obj.location.dot(Field::dataSize),
                         "(%zu) must be at least "
                         "shaderGroupHandleCaptureReplaySize (%" PRIu32 ") * groupCount (%" PRIu32 ").",
                         dataSize,
                         phys_dev_ext_props.ray_tracing_props_khr.shaderGroupHandleCaptureReplaySize,
                         groupCount);
    }
    ...
    if (pipeline_state->pipeline_type != VK_PIPELINE_BIND_POINT_RAY_TRACING_KHR) {
        skip |= LogError("VUID-vkGetRayTracingCaptureReplayShaderGroupHandlesKHR-pipeline-04620",
                         pipeline,
                         error_obj.location.dot(Field::pipeline),
                         "is a %s pipeline.",
    ...
    if (create_info.flags & VK_PIPELINE_CREATE_LIBRARY_BIT_KHR) {
        if (!enabled_features.pipelineLibraryGroupHandles) {
            skip |= LogError("VUID-vkGetRayTracingCaptureReplayShaderGroupHandlesKHR-pipeline-07829",
                             pipeline,
                             error_obj.location.dot(Field::pipeline),
                             "was created with %s, but the pipelineLibraryGroupHandles feature was not enabled.",
    ...
    if (firstGroup >= total_group_count) {
        skip |= LogError("VUID-vkGetRayTracingCaptureReplayShaderGroupHandlesKHR-firstGroup-04051",
                         device,
                         error_obj.location.dot(Field::firstGroup),
                         "(%" PRIu32 ") must be less than the number of shader "
                         "groups in pipeline (%" PRIu32 ").",
                         firstGroup,
                         total_group_count);
    }
    if ((firstGroup + groupCount) > total_group_count) {
        skip |= LogError("VUID-vkGetRayTracingCaptureReplayShaderGroupHandlesKHR-firstGroup-03483",
                         device,
                         error_obj.location.dot(Field::firstGroup),
    ...
    if (!(create_info.flags & VK_PIPELINE_CREATE_RAY_TRACING_SHADER_GROUP_HANDLE_CAPTURE_REPLAY_BIT_KHR)) {
        skip |= LogError("VUID-vkGetRayTracingCaptureReplayShaderGroupHandlesKHR-pipeline-03607",
                         pipeline,
                         error_obj.location.dot(Field::pipeline),
                         "was created with %s.",
                         string_VkPipelineCreateFlags(create_info.flags).c_str());
...
bool CoreChecks::PreCallValidateGetRayTracingShaderGroupStackSizeKHR(VkDevice device,
                                                                     VkPipeline pipeline,
                                                                     uint32_t group,
    ...
    if (pipeline_state->pipeline_type != VK_PIPELINE_BIND_POINT_RAY_TRACING_KHR) {
        skip |= LogError("VUID-vkGetRayTracingShaderGroupStackSizeKHR-pipeline-04622",
                         pipeline,
                         error_obj.location.dot(Field::pipeline),
                         "is a %s pipeline.",
                         string_VkPipelineBindPoint(pipeline_state->pipeline_type));
        ...
        if (group >= create_info.groupCount) {
            skip |= LogError("VUID-vkGetRayTracingShaderGroupStackSizeKHR-group-03608",
                             pipeline,
            ...
            if (unused_group) {
                ...
                skip |= LogError("VUID-vkGetRayTracingShaderGroupStackSizeKHR-groupShader-03609",
                                 objlist,
                                 error_obj.location.dot(Field::groupShader),
                                 "is %s but the corresponding shader in shader group (%" PRIu32
                                 ") is VK_SHADER_UNUSED_KHR.",
                                 string_VkShaderGroupShaderKHR(groupShader),
                                 group);
            }
        }
    }
...
bool CoreChecks::ValidateRaytracingShaderBindingTable(VkCommandBuffer commandBuffer,
                                                      const Location& table_loc,
                                                      const char* vuid_single_device_memory,
    ...
    if (buffer_states.empty()) {
        skip |= LogError("VUID-VkStridedDeviceAddressRegionKHR-size-04631",
                         ...
                         "(0x%" PRIx64 ") has no buffer associated with it.",
        ...
        BufferAddressValidation<4> buffer_address_validator = { { {
            ...
            { "VUID-VkStridedDeviceAddressRegionKHR-size-04631",
              ...
              [table_loc, requested_range_string = string_range_hex(requested_range)]() {
                  return "The following buffers do not include " + table_loc.Fields() +
        ...
        skip |= buffer_address_validator.LogErrorsIfNoValidBuffer(*this,
                                                                  buffer_states,
                                                                  table_loc.dot(Field::deviceAddress),
                                                                  LogObjectList(commandBuffer),
                                                                  binding_table.deviceAddress);
...
bool CoreChecks::ValidateDeferredOperation(VkDevice device,
    ...
    if (deferred_operation != VK_NULL_HANDLE) {
        ...
        if (result == VK_NOT_READY) {
            skip |= LogError(vuid,
                             deferred_operation,
                             loc.dot(Field::deferredOperation),
                             "%s is not completed.",
