...
bool CoreChecks::ValidateInterfaceVertexInput(const vvl::Pipeline& pipeline,
                                              const spirv::Module& module_state,
    ...
    for (uint32_t i = 0; i < input_state->vertexAttributeDescriptionCount; ++i) {
        ...
        // depth/multi-plane/compressed will never be used here because they would mean nothing. So we can ensure these
        // are "standard" color formats being used
    ...
    for (const auto& [location, attribute_info] : location_map) {
        ...
        if (attribute_input && !shader_input) {
            skip |= LogPerformanceWarning("WARNING-Shader-OutputNotConsumed",
                                          module_state.handle(),
                                          vi_loc,
            ...
            if (!enabled_features.vertexAttributeRobustness) {
                skip |= LogError("VUID-VkGraphicsPipelineCreateInfo-Input-07904",
                                 module_state.handle(),
            ...
            if ((attribute_type & var_numeric_type) == 0) {
                ...
                if (attribute_components < input_components) {
                    skip |=
                        LogError("VUID-VkGraphicsPipelineCreateInfo-pVertexInputState-09198",
                                 module_state.handle(),
                                 vi_loc.dot(Field::pVertexAttributeDescriptions, attribute_info.attribute_index)
                                     .dot(Field::format),
...
bool CoreChecks::ValidateInterfaceFragmentOutput(const vvl::Pipeline& pipeline,
                                                 const spirv::Module& module_state,
                                                 const spirv::EntryPoint& entrypoint,
                                                 const Location& create_info_loc) const {
    bool skip = false;
    const auto* ms_state = pipeline.MultisampleState();
    if (!pipeline.IsDynamic(CB_DYNAMIC_STATE_ALPHA_TO_COVERAGE_ENABLE_EXT) && ms_state &&
        ms_state->alphaToCoverageEnable) {
        if (!entrypoint.has_alpha_to_coverage_variable) {
            skip |= LogError("VUID-VkGraphicsPipelineCreateInfo-alphaToCoverageEnable-08891",
                             module_state.handle(),
...
bool CoreChecks::ValidateBuiltinLimits(const spirv::Module& module_state,
                                       const spirv::EntryPoint& entrypoint,
    ...
    for (const auto* variable : entrypoint.built_in_variables) {
        ...
        if (variable->decorations.builtin == spv::BuiltInSampleMask &&
            ...
            skip |= LogError(vuid,
                             module_state.handle(),
                             loc,
                             "The BuiltIns SampleMask array sizes is %" PRIu32 " which exceeds "
...
bool CoreChecks::ValidatePrimitiveTopology(const spirv::Module& module_state,
                                           const spirv::EntryPoint& entrypoint,
                                           const vvl::Pipeline& pipeline,
                                           const Location& loc) const {
    bool skip = false;

    if (!pipeline.pre_raster_state || !pipeline.InputAssemblyState() ||
        entrypoint.stage != VK_SHADER_STAGE_GEOMETRY_BIT || pipeline.IsDynamic(CB_DYNAMIC_STATE_PRIMITIVE_TOPOLOGY)) {
        return skip;
    }
    ...
    if (mismatch) {
        if (has_tess) {
            skip |= LogError("VUID-VkGraphicsPipelineCreateInfo-pStages-00739",
                             module_state.handle(),
                             loc,
                             "SPIR-V (Geometry stage) expects input topology %s, but tessellation evaluation shader "
                             "output topology is %s.",
                             string_VkPrimitiveTopology(geom_topology),
                             string_VkPrimitiveTopology(topology));
        } else {
            skip |= LogError("VUID-VkGraphicsPipelineCreateInfo-pStages-00738",
                             module_state.handle(),
                             loc,
                             "SPIR-V (Geometry stage) expects input topology %s, but pipeline was created with "
...
bool CoreChecks::ValidateShaderStageInputOutputLimits(const spirv::Module& module_state,
                                                      const spirv::EntryPoint& entrypoint,
    ...
    switch (stage) {
        case VK_SHADER_STAGE_VERTEX_BIT:
            if (total_output_components >= limits.maxVertexOutputComponents) {
                skip |= LogError("VUID-RuntimeSpirv-Location-06272",
                                 module_state.handle(),
                                 loc,
                                 ...
                                 max_output_slot.Describe().c_str(),
                                 entrypoint.builtin_output_components,
        ...
        case VK_SHADER_STAGE_TESSELLATION_CONTROL_BIT:
            if (max_input_slot.slot >= limits.maxTessellationControlPerVertexInputComponents) {
                skip |=
                    LogError("VUID-RuntimeSpirv-Location-06272",
                             module_state.handle(),
                             loc,
        ...
        case VK_SHADER_STAGE_TESSELLATION_EVALUATION_BIT:
            ...
            if (IsExtEnabled(device_extensions.vk_khr_portability_subset)) {
                if (is_iso_lines && (VK_FALSE == enabled_features.tessellationIsolines)) {
                    skip |=
                        LogError("VUID-RuntimeSpirv-tessellationShader-06326",
                                 module_state.handle(),
                                 loc,
        ...
        case VK_SHADER_STAGE_GEOMETRY_BIT:
            if (total_input_components >= limits.maxGeometryInputComponents) {
                skip |= LogError("VUID-RuntimeSpirv-Location-06272",
                                 ...
                                 max_input_slot.Describe().c_str(),
                                 entrypoint.builtin_input_components,
        ...
        case VK_SHADER_STAGE_FRAGMENT_BIT:
            ...
            if (max_output_slot.Location() >= limits.maxFragmentOutputAttachments) {
                skip |= LogError("VUID-RuntimeSpirv-Location-06272",
                                 ...
                                 "exceeds the limit maxFragmentOutputAttachments (%" PRIu32
                                 ") (note: Location are zero index based).",
                                 max_output_slot.Location(),
        ...
        case VK_SHADER_STAGE_MESH_BIT_EXT:
            if (entrypoint.execution_model == spv::ExecutionModelMeshNV) {
                if (num_vertices > phys_dev_ext_props.mesh_shader_props_nv.maxMeshOutputVertices) {
                    skip |= LogError("VUID-RuntimeSpirv-MeshNV-07113",
                                     module_state.handle(),
                                     loc,
                ...
                if (num_primitives > phys_dev_ext_props.mesh_shader_props_nv.maxMeshOutputPrimitives) {
                    skip |= LogError("VUID-RuntimeSpirv-MeshNV-07114",
                                     module_state.handle(),
                                     loc,
                ...
                if (num_vertices > phys_dev_ext_props.mesh_shader_props_ext.maxMeshOutputVertices) {
                    skip |= LogError("VUID-RuntimeSpirv-MeshEXT-07115",
                                     module_state.handle(),
                                     loc,
                                     "SPIR-V (Mesh stage) output vertices count exceeds the "
                                     "maxMeshOutputVertices of %" PRIu32 " by %" PRIu32 ".",
                ...
                if (num_primitives > phys_dev_ext_props.mesh_shader_props_ext.maxMeshOutputPrimitives) {
                    skip |= LogError("VUID-RuntimeSpirv-MeshEXT-07116",
                                     module_state.handle(),
                                     loc,
                                     "SPIR-V (Mesh stage) output primitives count exceeds the "
                                     "maxMeshOutputPrimitives of %" PRIu32 " by %" PRIu32 ".",
    ...
    //
    // This limit was created from Vulkan 1.0, with the move to bindless, this limit has slowly become less relevant, if
    // using descriptor indexing, the limit should basically be UINT32_MAX
    if (stage == VK_SHADER_STAGE_FRAGMENT_BIT && !IsExtEnabled(device_extensions.vk_ext_descriptor_indexing)) {
        ...
        for (const auto* variable : entrypoint.user_defined_interface_variables) {
            if (variable->storage_class == spv::StorageClassOutput &&
                variable->decorations.location != spirv::kInvalidValue) {
                // even if using an array of attachments in the shader, each used variable of the array is represented
        ...
        const uint32_t total_output =
            (uint32_t)(color_attachments.size() + storage_buffers.size() + storage_images.size());
...
bool CoreChecks::ValidateInterfaceBetweenStages(const spirv::Module& producer,
                                                const spirv::EntryPoint& producer_entrypoint,
                                                const spirv::Module& consumer,
                                                const spirv::EntryPoint& consumer_entrypoint,
                                                const Location& create_info_loc) const {
    bool skip = false;
        ...
        for (uint32_t component = 0; component < 4; component++) {
            ...
            if ((input_var == nullptr) && (output_var == nullptr)) {
                ...
                if ((component_info.output_type != component_info.input_type) ||
                    ...
                    skip |= LogError("VUID-RuntimeSpirv-OpEntryPoint-07754",
                                     objlist,
                                     create_info_loc,
                                     ...
                                     consumer.DescribeVariable(input_var->id).c_str(),
                ...
                if (!enabled_features.maintenance4 && (output_var->base_type.Opcode() == spv::OpTypeVector) &&
                    ...
                    // Note the "Component Count" in the VU refers to OpTypeVector's operand and NOT the "Component
                    // slot"
                ...
                if (!enabled_features.maintenance4 && (output_var->base_type.Opcode() != spv::OpTypeVector)) {
                    ...
                    skip |= LogPerformanceWarning("WARNING-Shader-OutputNotConsumed",
                                                  objlist,
                                                  create_info_loc,
                                                  "(SPIR-V Interface) %s declared to output location %" PRIu32
                ...
                const LogObjectList objlist(producer.handle(), consumer.handle());
                skip |= LogError("VUID-RuntimeSpirv-OpEntryPoint-08743",
                                 objlist,
                                 create_info_loc,
                                 ...
                                 location,
                                 component,
                                 input_var->is_patch ? "(Tessellation Patch) " : "",
                                 string_VkShaderStageFlagBits(producer_stage));
                break; // Only need to report for the first component found
            }
        }
    ...
    if (mismatch) {
        ...
        msg << "}\n";
        const LogObjectList objlist(producer.handle(), consumer.handle());
        skip |= LogError("VUID-RuntimeSpirv-OpVariable-08746",
                         objlist,
                         create_info_loc,
...
bool CoreChecks::ValidateFsOutputsAgainstRenderPass(const spirv::Module& module_state,
                                                    const spirv::EntryPoint& entrypoint,
                                                    const vvl::Pipeline& pipeline,
    ...
    for (const auto& [location, attachment_info] : location_map) {
        ...
        if (attachment && !output) {
            ...
            if (location < attachment_states.size() && attachment_states[location].colorWriteMask != 0) {
                skip |=
                    LogUndefinedValue("Undefined-Value-ShaderInputNotProduced",
                                      module_state.handle(),
                                      create_info_loc,
                                      "Attachment %" PRIu32
            ...
            if (!(alpha_to_coverage_enabled && location == 0)) {
                skip |= LogUndefinedValue("Undefined-Value-ShaderOutputNotConsumed",
                                          module_state.handle(),
                                          create_info_loc,
                                          "fragment shader writes to output location %" PRIu32
...
bool CoreChecks::ValidateFsOutputsAgainstDynamicRenderingRenderPass(const spirv::Module& module_state,
    ...
    for (uint32_t location = 0; location < location_map.size(); ++location) {
        ...
        if (!output && location < attachment_states.size() && attachment_states[location].colorWriteMask != 0) {
            skip |= LogUndefinedValue("Undefined-Value-ShaderInputNotProduced-DynamicRendering",
                                      module_state.handle(),
                                      create_info_loc,
                                      "Attachment %" PRIu32
                                      " not written by fragment shader; undefined values will be written to attachment",
...
bool CoreChecks::ValidatePipelineTessellationStages(const spirv::Module& tesc_module_state,
    ...
    if (tesc_subdivision == 0 && tese_subdivision == 0) {
        ...
        skip |= LogError("VUID-VkGraphicsPipelineCreateInfo-pStages-00732",
                         objlist,
                         create_info_loc,
        ...
        skip |= LogError(
            ...
            string_SpvExecutionMode(tesc_subdivision),
            string_SpvExecutionMode(tese_subdivision));
    }
    if (tesc_patch_size == vvl::kU32Max && tese_patch_size == vvl::kU32Max) {
        const LogObjectList objlist(tesc_module_state.handle(), tese_module_state.handle());
        skip |= LogError("VUID-VkGraphicsPipelineCreateInfo-pStages-00734",
                         objlist,
                         create_info_loc,
                         "Output patch size is not specified in either of tessellation stages");
    } else if (tesc_patch_size != vvl::kU32Max && tese_patch_size != vvl::kU32Max &&
               tesc_patch_size != tese_patch_size) {
        const LogObjectList objlist(tesc_module_state.handle(), tese_module_state.handle());
        skip |= LogError("VUID-VkGraphicsPipelineCreateInfo-pStages-00735",
                         objlist,
                         create_info_loc,
...
bool CoreChecks::ValidateGraphicsPipelineShaderState(const vvl::Pipeline& pipeline,
                                                     const Location& create_info_loc) const {
    bool skip = false;
    ...
    const ShaderStageState *vertex_stage = nullptr, *tesc_stage = nullptr, *tese_stage = nullptr,
                           *fragment_stage = nullptr;
    for (uint32_t i = 0; i < pipeline.stage_states.size(); i++) {
        auto& stage_state = pipeline.stage_states[i];
        const VkShaderStageFlagBits stage = stage_state.GetStage();
    ...
    if (pipeline.vertex_input_state && vertex_stage && vertex_stage->entrypoint && vertex_stage->spirv_state &&
        ...
        skip |= ValidateInterfaceVertexInput(
            pipeline, *vertex_stage->spirv_state.get(), *vertex_stage->entrypoint, create_info_loc);
    }

    if (pipeline.fragment_shader_state && fragment_stage && fragment_stage->entrypoint && fragment_stage->spirv_state) {
        skip |= ValidateInterfaceFragmentOutput(
            pipeline, *fragment_stage->spirv_state.get(), *fragment_stage->entrypoint, create_info_loc);
    }

    // We need to order the stages not how they are supplied in VkGraphicsPipelineCreateInfo::pStages but rather how
    // they are chained together in the pipeline. Note, we could be in the PreRaster GPL path, so there may not be a
    // Fragment Shader see https://github.com/KhronosGroup/Vulkan-ValidationLayers/issues/8443
    if (pipeline.stage_states.size() > 1) {
        ...
        for (size_t i = 1; i < pipeline.stage_states.size(); i++) {
            ...
            if (consumer_spirv && producer_spirv && consumer.entrypoint && producer.entrypoint) {
                skip |= ValidateInterfaceBetweenStages(*producer_spirv.get(),
                                                       *producer.entrypoint,
                                                       *consumer_spirv.get(),
                                                       *consumer.entrypoint,
                                                       create_info_loc);
    ...
    if (fragment_stage && fragment_stage->entrypoint && fragment_stage->spirv_state) {
        ...
        if (rp_state && rp_state->UsesDynamicRendering()) {
            ...
            skip |= ValidateFsOutputsAgainstRenderPass(*fragment_stage->spirv_state.get(),
                                                       *fragment_stage->entrypoint,
                                                       pipeline,
                                                       pipeline.Subpass(),
                                                       create_info_loc);
        }
    ...
    if (tesc_stage && tesc_stage->spirv_state && tesc_stage->entrypoint && tese_stage && tese_stage->spirv_state &&
        ...
        skip |= ValidatePipelineTessellationStages(*tesc_stage->spirv_state,
                                                   *tesc_stage->entrypoint,
                                                   *tese_stage->spirv_state,
                                                   *tese_stage->entrypoint,
                                                   create_info_loc);
    }
