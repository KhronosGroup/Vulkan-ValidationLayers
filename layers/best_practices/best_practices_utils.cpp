...
std::shared_ptr<vvl::CommandBuffer> BestPractices::CreateCmdBufferState(VkCommandBuffer handle,
    ...
    if (cb) {
        cb->SetSubState(container_type, std::make_unique<bp_state::CommandBufferSubState>(*this, *cb));
    }
    return cb;
}

bp_state::CommandBufferSubState::CommandBufferSubState(BestPractices& bp, vvl::CommandBuffer& cb)
