...
bool BestPractices::ValidateMultisampledBlendingArm(const VkGraphicsPipelineCreateInfo& create_info,
    ...
    uint32_t num_color_attachments =
        std::min(subpass.colorAttachmentCount, create_info.pColorBlendState->attachmentCount);
    ...
    for (uint32_t j = 0; j < num_color_attachments; j++) {
        ...
        if (att != VK_ATTACHMENT_UNUSED && blend_att.blendEnable && blend_att.colorWriteMask) {
            if (!FormatHasFullThroughputBlendingArm(rp_state->create_info.pAttachments[att].format)) {
                skip |= LogPerformanceWarning("BestPractices-Arm-vkCreatePipelines-multisampled-blending",
                                              device,
                                              create_info_loc,
...
void BestPractices::ManualPostCallRecordCreateComputePipelines(VkDevice device,
...
bool BestPractices::ValidateCreateGraphicsPipeline(const VkGraphicsPipelineCreateInfo& create_info,
    ...
    if (!(pipeline.active_shaders & VK_SHADER_STAGE_MESH_BIT_EXT) && create_info.pVertexInputState) {
        ...
        if (count > kMaxInstancedVertexBuffers) {
            skip |= LogPerformanceWarning(
                ...
                device,
                create_info_loc,
                "The pipeline is using %u instanced vertex buffers (current limit: %u), but this can be inefficient on "
                "the "
                "GPU. If using instanced vertex attributes prefer interleaving them in a single buffer.",
                count,
                kMaxInstancedVertexBuffers);
        }
    }
    ...
    if ((create_info.pRasterizationState) && (create_info.pRasterizationState->depthBiasEnable) &&
        ...
        skip |= LogPerformanceWarning(
            "BestPractices-Arm-vkCreatePipelines-depthbias-zero",
            device,
            create_info_loc,
    ...
    if (create_info.renderPass == VK_NULL_HANDLE &&
        ...
        skip |= LogWarning("BestPractices-Pipeline-NoRendering",
                           device,
                           create_info_loc,
                           "renderPass is VK_NULL_HANDLE and pNext chain does not contain an instance of "
    ...
    if (VendorCheckEnabled(kBPVendorAMD)) {
        if (create_info.pInputAssemblyState && create_info.pInputAssemblyState->primitiveRestartEnable) {
            skip |= LogPerformanceWarning("BestPractices-AMD-CreatePipelines-AvoidPrimitiveRestart",
                                          device,
                                          create_info_loc,
                                          "%s Use of primitive restart is not recommended",
...
bool BestPractices::PreCallValidateCreateGraphicsPipelines(VkDevice device,
                                                           VkPipelineCache pipelineCache,
                                                           uint32_t createInfoCount,
    ...
    bool skip = BaseClass::PreCallValidateCreateGraphicsPipelines(device,
                                                                  pipelineCache,
                                                                  createInfoCount,
                                                                  pCreateInfos,
                                                                  pAllocator,
    ...
    if ((createInfoCount > 1) && (!pipelineCache)) {
        skip |= LogPerformanceWarning("BestPractices-vkCreateGraphicsPipelines-multiple-pipelines-no-cache",
                                      device,
                                      error_obj.location,
                                      "creating multiple pipelines (createInfoCount is %" PRIu32 ") but is not using a "
    ...
    for (uint32_t i = 0; i < createInfoCount; i++) {
        ...
        skip |=
            ValidateCreateGraphicsPipeline(pCreateInfos[i], *pipeline, error_obj.location.dot(Field::pCreateInfos, i));
    }
    ...
    if (VendorCheckEnabled(kBPVendorAMD) || VendorCheckEnabled(kBPVendorNVIDIA)) {
        ...
        if (pipelineCache && prev_pipeline && pipelineCache != prev_pipeline) {
            skip |= LogPerformanceWarning("BestPractices-vkCreatePipelines-multiple-pipelines-caches",
                                          device,
                                          error_obj.location,
    ...
    if (VendorCheckEnabled(kBPVendorAMD)) {
        ...
        if (pso_count > kMaxRecommendedNumberOfPSOAMD) {
            skip |= LogPerformanceWarning("BestPractices-AMD-CreatePipelines-TooManyPipelines",
                                          device,
                                          error_obj.location,
...
static std::vector<bp_state::AttachmentInfo> GetAttachmentAccess(bp_state::Pipeline& pipe_state) {
    ...
    // NOTE: see PIPELINE_LAYOUT and vku::safe_VkGraphicsPipelineCreateInfo constructors. pColorBlendState and
    // pDepthStencilState are only non-null if they are enabled.
    ...
    if (create_info.pDepthStencilState &&
        ...
        if (attachment != VK_ATTACHMENT_UNUSED) {
            ...
            if (create_info.pDepthStencilState->depthTestEnable ||
                create_info.pDepthStencilState->depthBoundsTestEnable) {
...
bp_state::Pipeline::Pipeline(const ValidationStateTracker& state_data,
...
BestPractices::CreateGraphicsPipelineState(const VkGraphicsPipelineCreateInfo* create_info,
...
void BestPractices::ManualPostCallRecordCreateGraphicsPipelines(VkDevice device,
                                                                VkPipelineCache pipelineCache,
                                                                uint32_t count,
                                                                const VkGraphicsPipelineCreateInfo* pCreateInfos,
                                                                const VkAllocationCallbacks* pAllocator,
...
bool BestPractices::PreCallValidateCreateComputePipelines(VkDevice device,
                                                          VkPipelineCache pipelineCache,
                                                          uint32_t createInfoCount,
                                                          const VkComputePipelineCreateInfo* pCreateInfos,
                                                          const VkAllocationCallbacks* pAllocator,
    ...
    bool skip = BaseClass::PreCallValidateCreateComputePipelines(device,
                                                                 pipelineCache,
                                                                 createInfoCount,
                                                                 pCreateInfos,
                                                                 pAllocator,
    ...
    if ((createInfoCount > 1) && (!pipelineCache)) {
        skip |= LogPerformanceWarning("BestPractices-vkCreateComputePipelines-multiple-pipelines-no-cache",
                                      device,
                                      error_obj.location,
                                      "creating multiple pipelines (createInfoCount is %" PRIu32 ") but is not using a "
                                      "pipeline cache, which may help with performance",
    ...
    if (VendorCheckEnabled(kBPVendorAMD)) {
        ...
        if (pipelineCache && prev_pipeline && pipelineCache != prev_pipeline) {
            skip |=
                LogPerformanceWarning("BestPractices-vkCreateComputePipelines-multiple-cache",
                                      device,
                                      error_obj.location,
    ...
    for (uint32_t i = 0; i < createInfoCount; i++) {
        ...
        if (IsExtEnabled(device_extensions.vk_khr_maintenance4)) {
            ...
            if (module_state && module_state->spirv->static_data_
                                    .has_builtin_workgroup_size) { // No module if creating from module identifier
                skip |= LogWarning(
...
bool BestPractices::ValidateCreateComputePipelineArm(const VkComputePipelineCreateInfo& create_info,
    ...
    if (thread_count > kMaxEfficientWorkGroupThreadCountArm) {
        skip |= LogPerformanceWarning(
            ...
            z,
            thread_count,
            kMaxEfficientWorkGroupThreadCountArm);
    }
    ...
    if (thread_count == 1 || ((x > 1) && (x & (kThreadGroupDispatchCountAlignmentArm - 1))) ||
        ...
        skip |= LogPerformanceWarning("BestPractices-Arm-vkCreateComputePipelines-compute-thread-group-alignment",
                                      device,
                                      create_info_loc,
    ...
    if (accesses_2d && dimensions < 2) {
        LogPerformanceWarning("BestPractices-Arm-vkCreateComputePipelines-compute-spatial-locality",
                              device,
                              create_info_loc,
...
bool BestPractices::ValidateCreateComputePipelineAmd(const VkComputePipelineCreateInfo& create_info,
    ...
    if (!multiple_64) {
        skip |=
            LogPerformanceWarning("BestPractices-AMD-LocalWorkgroup-Multiple64",
                                  device,
                                  create_info_loc,
...
void BestPractices::PostCallRecordCmdBindPipeline(VkCommandBuffer commandBuffer,
    ...
    if (pipelineBindPoint == VK_PIPELINE_BIND_POINT_GRAPHICS) {
        ...
        if (auto pipeline_state = Get<bp_state::Pipeline>(pipeline)) {
            ...
            if (VendorCheckEnabled(kBPVendorNVIDIA)) {
                ...
                auto tgm_stages = VK_SHADER_STAGE_TESSELLATION_CONTROL_BIT |
                                  VK_SHADER_STAGE_TESSELLATION_EVALUATION_BIT | VK_SHADER_STAGE_GEOMETRY_BIT |
                                  VK_SHADER_STAGE_TASK_BIT_EXT | VK_SHADER_STAGE_MESH_BIT_EXT;
                auto new_tgm_state = (pipeline_state->active_shaders & tgm_stages) != 0
                ...
                if (depth_stencil_state && dynamic_state) {
                    ...
                    const bool dynamic_depth_test_enable =
                        std::find(dynamic_state_begin, dynamic_state_end, VK_DYNAMIC_STATE_DEPTH_TEST_ENABLE) !=
                        dynamic_state_end;
                    const bool dynamic_depth_func =
                        std::find(dynamic_state_begin, dynamic_state_end, VK_DYNAMIC_STATE_DEPTH_COMPARE_OP) !=
                        dynamic_state_end;

                    if (!dynamic_depth_test_enable) {
                        RecordSetDepthTestState(
                            *cb_state, cb_state->nv.depth_compare_op, depth_stencil_state->depthTestEnable != VK_FALSE);
                    }
                    if (!dynamic_depth_func) {
                        RecordSetDepthTestState(
                            *cb_state, depth_stencil_state->depthCompareOp, cb_state->nv.depth_test_enable);
                    }
                }
            }
        }
...
void BestPractices::PreCallRecordCreateGraphicsPipelines(VkDevice device,
                                                         VkPipelineCache pipelineCache,
                                                         uint32_t createInfoCount,
                                                         const VkGraphicsPipelineCreateInfo* pCreateInfos,
                                                         const VkAllocationCallbacks* pAllocator,
...
bool BestPractices::PreCallValidateCreatePipelineLayout(VkDevice device,
    ...
    if (VendorCheckEnabled(kBPVendorAMD)) {
        ...
        if (pipeline_size > kPipelineLayoutSizeWarningLimitAMD) {
            skip |= LogPerformanceWarning("BestPractices-AMD-CreatePipelinesLayout-KeepLayoutSmall",
                                          device,
                                          error_obj.location,
    ...
    if (VendorCheckEnabled(kBPVendorNVIDIA)) {
        ...
        for (uint32_t i = 0; i < pCreateInfo->setLayoutCount; ++i) {
            ...
            for (const auto& binding : descriptor_set_layout_state->GetBindings()) {
                ...
                if ((descriptor_set_layout_state->GetCreateFlags() &
                     VK_DESCRIPTOR_SET_LAYOUT_CREATE_UPDATE_AFTER_BIND_POOL_BIT) == 0U) {
        ...
        if (has_separate_sampler) {
            skip |= LogPerformanceWarning("BestPractices-NVIDIA-CreatePipelineLayout-SeparateSampler",
                                          device,
                                          error_obj.location,
                                          "%s Consider using combined image samplers instead of separate samplers for "
        ...
        if (fast_space_usage > kPipelineLayoutFastDescriptorSpaceNVIDIA) {
            skip |= LogPerformanceWarning(
                ...
                VendorSpecificTag(kBPVendorNVIDIA),
                kPipelineLayoutFastDescriptorSpaceNVIDIA);
        }
    }
...
bool BestPractices::PreCallValidateCmdBindPipeline(VkCommandBuffer commandBuffer,
                                                   VkPipelineBindPoint pipelineBindPoint,
                                                   VkPipeline pipeline,
                                                   const ErrorObject& error_obj) const {
    bool skip = false;
    ...
    if (VendorCheckEnabled(kBPVendorNVIDIA)) {
        ...
        if (tgm.num_switches >= kNumBindPipelineTessGeometryMeshSwitchesThresholdNVIDIA && !tgm.threshold_signaled) {
            LogPerformanceWarning("BestPractices-NVIDIA-BindPipeline-SwitchTessGeometryMesh",
                                  commandBuffer,
                                  error_obj.location,
