...
bool BestPractices::PreCallValidateCreateImage(VkDevice device,
    ...
    if ((pCreateInfo->queueFamilyIndexCount > 1) && (pCreateInfo->sharingMode == VK_SHARING_MODE_EXCLUSIVE)) {
        skip |= LogWarning("BestPractices-vkCreateImage-sharing-mode-exclusive",
                           device,
    ...
    if ((pCreateInfo->flags & VK_IMAGE_CREATE_EXTENDED_USAGE_BIT) &&
        !(pCreateInfo->flags & VK_IMAGE_CREATE_MUTABLE_FORMAT_BIT)) {
        skip |= LogWarning(
            "BestPractices-vkCreateImage-CreateFlags",
            device,
    ...
    if (VendorCheckEnabled(kBPVendorArm) && pCreateInfo->samples > kMaxEfficientSamplesArm) {
        skip |= LogPerformanceWarning(
            "BestPractices-Arm-vkCreateImage-too-large-sample-count",
            device,
            error_obj.location,
            ...
            VendorSpecificTag(kBPVendorArm),
            static_cast<uint32_t>(pCreateInfo->samples),
            kMaxEfficientSamplesArm);
    }

    if (VendorCheckEnabled(kBPVendorIMG) && pCreateInfo->samples > kMaxEfficientSamplesImg) {
        skip |= LogPerformanceWarning(
            "BestPractices-IMG-vkCreateImage-too-large-sample-count",
            device,
            error_obj.location,
            "%s Trying to create an image with %u samples. "
    ...
    if (VendorCheckEnabled(kBPVendorIMG) && (pCreateInfo->format == VK_FORMAT_PVRTC1_2BPP_SRGB_BLOCK_IMG ||
        ...
        skip |=
            LogPerformanceWarning("BestPractices-IMG-Texture-Format-PVRTC-Outdated",
                                  device,
                                  error_obj.location,
    ...
    if (VendorCheckEnabled(kBPVendorAMD)) {
        if ((pCreateInfo->usage &
            ...
            skip |= LogPerformanceWarning(
                "BestPractices-AMD-vkImage-AvoidConcurrentRenderTargets",
                device,
                error_obj.location,
        ...
        if ((pCreateInfo->usage & (VK_IMAGE_USAGE_COLOR_ATTACHMENT_BIT | VK_IMAGE_USAGE_STORAGE_BIT |
                                   VK_IMAGE_USAGE_DEPTH_STENCIL_ATTACHMENT_BIT)) &&
            (pCreateInfo->flags & VK_IMAGE_CREATE_MUTABLE_FORMAT_BIT)) {
            skip |= LogPerformanceWarning(
                "BestPractices-AMD-vkImage-DontUseMutableRenderTargets",
                device,
                error_obj.location,
        ...
        if ((pCreateInfo->usage &
            ...
            skip |= LogPerformanceWarning(
                "BestPractices-AMD-vkImage-DontUseStorageRenderTargets",
                device,
                error_obj.location,
    ...
    if (VendorCheckEnabled(kBPVendorNVIDIA)) {
        if (pCreateInfo->tiling == VK_IMAGE_TILING_LINEAR) {
            skip |= LogPerformanceWarning("BestPractices-NVIDIA-CreateImage-TilingLinear",
                                          device,
                                          error_obj.location,
...
void BestPractices::QueueValidateImageView(QueueCallbacks& funcs,
                                           Func command,
                                           vvl::ImageView* view,
...
void BestPractices::QueueValidateImage(QueueCallbacks& funcs,
                                       Func command,
    ...
    const uint32_t base_array_layer =
        state->create_info.imageType == VK_IMAGE_TYPE_3D ? 0 : subresource_range.baseArrayLayer;
    ...
    for (uint32_t layer = 0; layer < array_layers; layer++) {
        for (uint32_t level = 0; level < mip_levels; level++) {
            QueueValidateImage(
                funcs, command, state, usage, layer + base_array_layer, level + subresource_range.baseMipLevel);
        }
    }
}
...
void BestPractices::QueueValidateImage(QueueCallbacks& funcs,
    ...
    for (uint32_t layer = 0; layer < array_layers; layer++) {
        QueueValidateImage(
            funcs, command, state, usage, layer + subresource_layers.baseArrayLayer, subresource_layers.mipLevel);
    }
}
...
void BestPractices::QueueValidateImage(QueueCallbacks& funcs,
                                       ...
                                       uint32_t mip_level) {
    funcs.emplace_back([this, command, state, usage, array_layer, mip_level](const ValidationStateTracker& vst,
                                                                             const vvl::Queue& qs,
...
void BestPractices::ValidateImageInQueueArmImg(Func command,
                                               const bp_state::Image& image,
    ...
    if (usage == IMAGE_SUBRESOURCE_USAGE_BP::RENDER_PASS_CLEARED &&
        last_usage == IMAGE_SUBRESOURCE_USAGE_BP::RENDER_PASS_STORED && !image.IsSwapchainImage()) {
        LogPerformanceWarning(
            "BestPractices-RenderPass-redundant-store",
            device,
            loc,
            "%s %s Subresource (arrayLayer: %u, mipLevel: %u) of image was cleared as part of LOAD_OP_CLEAR, but last "
            "time "
            ...
            array_layer,
            mip_level);
    } else if (usage == IMAGE_SUBRESOURCE_USAGE_BP::RENDER_PASS_CLEARED &&
        ...
        switch (last_usage) {
            ...
            case IMAGE_SUBRESOURCE_USAGE_BP::CLEARED:
                ...
                suggestion = "Clearing the image with vkCmdClear*Image() is probably redundant in this case, and "
                             "wastes bandwidth on "
                             "tile-based architectures. "
            ...
            case IMAGE_SUBRESOURCE_USAGE_BP::RESOLVE_WRITE:
                ...
                suggestion = "The resolve is probably redundant in this case, and wastes a lot of bandwidth on "
                             "tile-based architectures. "
        ...
        LogPerformanceWarning(vuid,
                              device,
                              loc,
                              "%s %s Subresource (arrayLayer: %u, mipLevel: %u) of image was loaded to tile as part of "
                              "LOAD_OP_LOAD, but last "
...
void BestPractices::ValidateImageInQueue(const vvl::Queue& qs,
                                         const vvl::CommandBuffer& cbs,
                                         Func command,
                                         bp_state::Image& state,
                                         IMAGE_SUBRESOURCE_USAGE_BP usage,
                                         uint32_t array_layer,
                                         uint32_t mip_level) {
    ...
    if (state.create_info.sharingMode == VK_SHARING_MODE_EXCLUSIVE && last_usage.queue_family_index != queue_family) {
        ...
        if (last_usage.type != IMAGE_SUBRESOURCE_USAGE_BP::UNDEFINED) {
            ...
            if (usage == IMAGE_SUBRESOURCE_USAGE_BP::RENDER_PASS_READ_TO_TILE ||
                usage == IMAGE_SUBRESOURCE_USAGE_BP::BLIT_READ || usage == IMAGE_SUBRESOURCE_USAGE_BP::COPY_READ ||
                usage == IMAGE_SUBRESOURCE_USAGE_BP::DESCRIPTOR_ACCESS ||
                usage == IMAGE_SUBRESOURCE_USAGE_BP::RESOLVE_READ) {
                Location loc(command);
                LogWarning("BestPractices-ConcurrentUsageOfExclusiveImage",
                           state.Handle(),
                           loc,
                           "Subresource (arrayLayer: %" PRIu32 ", mipLevel: %" PRIu32
                           ") of image is used on queue family index %" PRIu32 " after being used on "
                           "queue family index %" PRIu32 ", "
                           "but has VK_SHARING_MODE_EXCLUSIVE, and has not been acquired and released with a ownership "
                           "transfer operation",
                           array_layer,
                           mip_level,
                           queue_family,
                           last_usage.queue_family_index);
            }
        }
    }
    ...
    if (last_usage.type == IMAGE_SUBRESOURCE_USAGE_BP::RENDER_PASS_DISCARDED &&
        ...
        Location loc(command);
        LogWarning("BestPractices-StoreOpDontCareThenLoadOpLoad",
                   device,
                   loc,
                   "Trying to load an attachment with LOAD_OP_LOAD that was previously stored with STORE_OP_DONT_CARE. "
                   "This may "
...
std::shared_ptr<vvl::Image>
BestPractices::CreateImageState(VkImage handle, const VkImageCreateInfo* create_info, VkFormatFeatureFlags2 features) {
    return std::make_shared<bp_state::Image>(*this, handle, create_info, features);
}

std::shared_ptr<vvl::Image> BestPractices::CreateImageState(VkImage handle,
