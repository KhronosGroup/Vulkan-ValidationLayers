...
bool BestPractices::CheckPipelineStageFlags(const LogObjectList& objlist,
                                            const Location& loc,
    ...
    if (flags & VK_PIPELINE_STAGE_ALL_GRAPHICS_BIT) {
        skip |= LogWarning(
            "BestPractices-pipeline-stage-flags-graphics", objlist, loc, "using VK_PIPELINE_STAGE_ALL_GRAPHICS_BIT");
    } else if (flags & VK_PIPELINE_STAGE_ALL_COMMANDS_BIT) {
        skip |= LogWarning(
            "BestPractices-pipeline-stage-flags-compute", objlist, loc, "using VK_PIPELINE_STAGE_ALL_COMMANDS_BIT");
    }
...
bool BestPractices::CheckPipelineStageFlags(const LogObjectList& objlist,
    ...
    if (flags & VK_PIPELINE_STAGE_2_ALL_GRAPHICS_BIT) {
        skip |= LogWarning(
            "BestPractices-pipeline-stage-flags2-graphics", objlist, loc, "using VK_PIPELINE_STAGE_2_ALL_GRAPHICS_BIT");
    } else if (flags & VK_PIPELINE_STAGE_2_ALL_COMMANDS_BIT) {
        skip |= LogWarning(
            "BestPractices-pipeline-stage-flags2-compute", objlist, loc, "using VK_PIPELINE_STAGE_2_ALL_COMMANDS_BIT");
    }

    return skip;
}

bool BestPractices::CheckDependencyInfo(const LogObjectList& objlist,
                                        const Location& dep_loc,
    ...
    for (uint32_t i = 0; i < dep_info.imageMemoryBarrierCount; ++i) {
        skip |= ValidateImageMemoryBarrier(dep_loc.dot(Field::pImageMemoryBarriers, i),
...
bool BestPractices::CheckEventSignalingState(const bp_state::CommandBuffer& command_buffer,
                                             VkEvent event,
                                             const Location& cb_loc) const {
    bool skip = false;
    if (auto* signaling_info = vvl::Find(command_buffer.event_signaling_state, event);
        signaling_info && signaling_info->signaled) {
        const LogObjectList objlist(command_buffer.VkHandle(), event);
        skip |= LogWarning(
            "BestPractices-Event-SignalSignaledEvent",
            objlist,
            cb_loc,
            ...
            FormatHandle(command_buffer.VkHandle()).c_str(),
...
bool BestPractices::PreCallValidateCmdSetEvent(VkCommandBuffer commandBuffer,
                                               VkEvent event,
...
void BestPractices::PreCallRecordCmdSetEvent(VkCommandBuffer commandBuffer,
                                             VkEvent event,
                                             VkPipelineStageFlags stageMask,
...
bool BestPractices::PreCallValidateCmdSetEvent2KHR(VkCommandBuffer commandBuffer,
                                                   VkEvent event,
...
bool BestPractices::PreCallValidateCmdSetEvent2(VkCommandBuffer commandBuffer,
                                                VkEvent event,
...
void BestPractices::PreCallRecordCmdSetEvent2KHR(VkCommandBuffer commandBuffer,
                                                 VkEvent event,
                                                 const VkDependencyInfoKHR* pDependencyInfo,
                                                 const RecordObject& record_obj) {
...
void BestPractices::PreCallRecordCmdSetEvent2(VkCommandBuffer commandBuffer,
                                              VkEvent event,
                                              const VkDependencyInfo* pDependencyInfo,
                                              const RecordObject& record_obj) {
...
bool BestPractices::PreCallValidateCmdResetEvent(VkCommandBuffer commandBuffer,
                                                 VkEvent event,
                                                 VkPipelineStageFlags stageMask,
                                                 const ErrorObject& error_obj) const {
    bool skip = false;
...
void BestPractices::PreCallRecordCmdResetEvent(VkCommandBuffer commandBuffer,
                                               VkEvent event,
                                               VkPipelineStageFlags stageMask,
                                               const RecordObject& record_obj) {
...
bool BestPractices::PreCallValidateCmdResetEvent2KHR(VkCommandBuffer commandBuffer,
                                                     VkEvent event,
...
bool BestPractices::PreCallValidateCmdResetEvent2(VkCommandBuffer commandBuffer,
                                                  VkEvent event,
...
void BestPractices::PreCallRecordCmdResetEvent2KHR(VkCommandBuffer commandBuffer,
                                                   VkEvent event,
                                                   VkPipelineStageFlags2KHR stageMask,
                                                   const RecordObject& record_obj) {
...
void BestPractices::PreCallRecordCmdResetEvent2(VkCommandBuffer commandBuffer,
                                                VkEvent event,
                                                VkPipelineStageFlags2 stageMask,
                                                const RecordObject& record_obj) {
...
bool BestPractices::PreCallValidateCmdWaitEvents(VkCommandBuffer commandBuffer,
                                                 uint32_t eventCount,
...
bool BestPractices::PreCallValidateCmdWaitEvents2KHR(VkCommandBuffer commandBuffer,
                                                     uint32_t eventCount,
                                                     const VkEvent* pEvents,
...
bool BestPractices::PreCallValidateCmdWaitEvents2(VkCommandBuffer commandBuffer,
                                                  uint32_t eventCount,
                                                  const VkEvent* pEvents,
    ...
    for (uint32_t i = 0; i < eventCount; i++) {
        skip |=
            CheckDependencyInfo(commandBuffer, error_obj.location.dot(Field::pDependencyInfos, i), pDependencyInfos[i]);
    }

    return skip;
}

bool BestPractices::ValidateAccessLayoutCombination(
    const Location& loc, VkImage image, VkAccessFlags2 access, VkImageLayout layout, VkImageAspectFlags aspect) const {
    bool skip = false;
    ...
    switch (layout) {
        ...
        case VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL:
            allowed = VK_ACCESS_INPUT_ATTACHMENT_READ_BIT | VK_ACCESS_SHADER_READ_BIT |
                      VK_ACCESS_COLOR_ATTACHMENT_READ_BIT | VK_ACCESS_2_SHADER_SAMPLED_READ_BIT |
...
bool BestPractices::ValidateImageMemoryBarrier(const Location& loc,
                                               VkImage image,
...
bool BestPractices::PreCallValidateCmdPipelineBarrier(VkCommandBuffer commandBuffer,
                                                      VkPipelineStageFlags srcStageMask,
                                                      VkPipelineStageFlags dstStageMask,
    ...
    for (uint32_t i = 0; i < imageMemoryBarrierCount; ++i) {
        skip |= ValidateImageMemoryBarrier(error_obj.location.dot(Field::pImageMemoryBarriers, i),
    ...
    if (VendorCheckEnabled(kBPVendorAMD)) {
        ...
        if (total_barriers > kMaxRecommendedBarriersSizeAMD) {
            skip |= LogPerformanceWarning(
                "BestPractices-AMD-CmdBuffer-highBarrierCount",
                commandBuffer,
                error_obj.location,
                ...
                num,
                total_barriers,
                kMaxRecommendedBarriersSizeAMD);
        }
    }
    if (VendorCheckEnabled(kBPVendorAMD) || VendorCheckEnabled(kBPVendorNVIDIA)) {
        ...
        for (uint32_t i = 0; i < imageMemoryBarrierCount; i++) {
            ...
            if (old_is_read_layout && new_is_read_layout) {
                skip |= LogPerformanceWarning("BestPractices-PipelineBarrier-readToReadBarrier",
                                              commandBuffer,
                                              error_obj.location,
            ...
            if (VendorCheckEnabled(kBPVendorAMD) && image_barrier.newLayout == VK_IMAGE_LAYOUT_GENERAL) {
                ...
                if (image_state && !(image_state->create_info.usage & VK_IMAGE_USAGE_STORAGE_BIT)) {
                    ...
                    skip |=
                        LogPerformanceWarning("BestPractices-AMD-vkImage-AvoidGeneral",
                                              objlist,
    ...
    for (uint32_t i = 0; i < imageMemoryBarrierCount; ++i) {
        skip |= ValidateCmdPipelineBarrierImageBarrier(
            commandBuffer, pImageMemoryBarriers[i], error_obj.location.dot(Field::pImageMemoryBarriers, i));
    }
...
bool BestPractices::PreCallValidateCmdPipelineBarrier2(VkCommandBuffer commandBuffer,
    ...
    for (uint32_t i = 0; i < pDependencyInfo->imageMemoryBarrierCount; ++i) {
        skip |= ValidateCmdPipelineBarrierImageBarrier(
            commandBuffer, pDependencyInfo->pImageMemoryBarriers[i], dep_info_loc.dot(Field::pImageMemoryBarriers, i));
    }
...
template <typename ImageMemoryBarrier>
void BestPractices::RecordCmdPipelineBarrierImageBarrier(VkCommandBuffer commandBuffer,
                                                         const ImageMemoryBarrier& barrier) {
    auto cb_state = Get<bp_state::CommandBuffer>(commandBuffer);
...
void BestPractices::PostCallRecordCmdPipelineBarrier(VkCommandBuffer commandBuffer,
                                                     VkPipelineStageFlags srcStageMask,
                                                     VkPipelineStageFlags dstStageMask,
                                                     VkDependencyFlags dependencyFlags,
                                                     uint32_t memoryBarrierCount,
                                                     ...
                                                     const VkImageMemoryBarrier* pImageMemoryBarriers,
                                                     const RecordObject& record_obj) {
    BaseClass::PostCallRecordCmdPipelineBarrier(commandBuffer,
                                                srcStageMask,
                                                dstStageMask,
                                                dependencyFlags,
                                                memoryBarrierCount,
...
void BestPractices::PostCallRecordCmdPipelineBarrier2(VkCommandBuffer commandBuffer,
                                                      const VkDependencyInfo* pDependencyInfo,
                                                      const RecordObject& record_obj) {
...
void BestPractices::PostCallRecordCmdPipelineBarrier2KHR(VkCommandBuffer commandBuffer,
                                                         const VkDependencyInfo* pDependencyInfo,
                                                         const RecordObject& record_obj) {
    PostCallRecordCmdPipelineBarrier2(commandBuffer, pDependencyInfo, record_obj);
}

bool BestPractices::PreCallValidateCreateSemaphore(VkDevice device,
    ...
    if (VendorCheckEnabled(kBPVendorAMD) || VendorCheckEnabled(kBPVendorNVIDIA)) {
        ...
        if (count > kMaxRecommendedSemaphoreObjectsSizeAMD) {
            skip |= LogPerformanceWarning("BestPractices-SyncObjects-HighNumberOfSemaphores",
                                          device,
                                          error_obj.location,
...
bool BestPractices::PreCallValidateCreateFence(VkDevice device,
    ...
    if (VendorCheckEnabled(kBPVendorAMD) || VendorCheckEnabled(kBPVendorNVIDIA)) {
        ...
        if (count > kMaxRecommendedFenceObjectsSizeAMD) {
            skip |= LogPerformanceWarning("BestPractices-SyncObjects-HighNumberOfFences",
                                          device,
                                          error_obj.location,
