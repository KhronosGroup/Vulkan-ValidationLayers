...
bool BestPractices::PreCallValidateGetDisplayPlaneSupportedDisplaysKHR(VkPhysicalDevice physicalDevice,
                                                                       uint32_t planeIndex,
...
bool BestPractices::PreCallValidateGetDisplayPlaneCapabilitiesKHR(VkPhysicalDevice physicalDevice,
...
bool BestPractices::PreCallValidateCreateSwapchainKHR(VkDevice device,
    ...
    if (bp_pd_state) {
        if (bp_pd_state->vkGetPhysicalDeviceSurfaceCapabilitiesKHRState == UNCALLED) {
            skip |= LogWarning("BestPractices-vkCreateSwapchainKHR-capabilities-no-surface",
                               device,
                               error_obj.location,
        ...
        if ((pCreateInfo->presentMode != VK_PRESENT_MODE_FIFO_KHR) &&
            ...
            skip |= LogWarning("BestPractices-vkCreateSwapchainKHR-present-mode-no-surface",
                               device,
                               error_obj.location,
        ...
        if (bp_pd_state->vkGetPhysicalDeviceSurfaceFormatsKHRState != QUERY_DETAILS) {
            skip |= LogWarning("BestPractices-vkCreateSwapchainKHR-formats-no-surface",
                               device,
                               error_obj.location,
    ...
    if ((pCreateInfo->queueFamilyIndexCount > 1) && (pCreateInfo->imageSharingMode == VK_SHARING_MODE_EXCLUSIVE)) {
        skip |=
            LogWarning("BestPractices-vkCreateSwapchainKHR-sharing-mode-exclusive",
                       device,
                       error_obj.location,
    ...
    if (((present_mode == VK_PRESENT_MODE_MAILBOX_KHR) || (present_mode == VK_PRESENT_MODE_FIFO_KHR)) &&
        ...
        skip |= LogPerformanceWarning("BestPractices-vkCreateSwapchainKHR-suboptimal-swapchain-image-count",
                                      device,
                                      error_obj.location,
                                      "A Swapchain is being created with minImageCount set to %" PRIu32
                                      ", which means double buffering is going "
                                      "to be used. Using double buffering and vsync locks rendering to an integer "
                                      "fraction of the vsync rate. In turn, "
                                      "reducing the performance of the application if rendering is slower than vsync. "
                                      "Consider setting minImageCount to "
    ...
    if (IsExtEnabled(device_extensions.vk_ext_swapchain_maintenance1) &&
        ...
        skip |=
            LogWarning("BestPractices-vkCreateSwapchainKHR-no-VkSwapchainPresentModesCreateInfoEXT-provided",
                       device,
                       error_obj.location,
    ...
    if (VendorCheckEnabled(kBPVendorArm) && (pCreateInfo->presentMode != VK_PRESENT_MODE_FIFO_KHR)) {
        skip |= LogWarning(
            "BestPractices-Arm-vkCreateSwapchainKHR-swapchain-presentmode-not-fifo",
            device,
            error_obj.location,
...
bool BestPractices::PreCallValidateCreateSharedSwapchainsKHR(VkDevice device,
    ...
    for (uint32_t i = 0; i < swapchainCount; i++) {
        if ((pCreateInfos[i].queueFamilyIndexCount > 1) &&
            (pCreateInfos[i].imageSharingMode == VK_SHARING_MODE_EXCLUSIVE)) {
            skip |= LogWarning(
                "BestPractices-vkCreateSharedSwapchainsKHR-sharing-mode-exclusive",
                device,
...
void BestPractices::ManualPostCallRecordQueuePresentKHR(VkQueue queue,
    ...
    for (uint32_t i = 0; i < pPresentInfo->swapchainCount; ++i) {
        ...
        if (swapchains_result == VK_SUBOPTIMAL_KHR) {
            LogPerformanceWarning(
                ...
                "subject to the window resize behavior, but the swapchain (%s) is no longer configured optimally for "
                "the surface "
                "it "
                "targets. Applications should query updated surface information and recreate their swapchain at the "
                "next "
...
bool BestPractices::PreCallValidateGetPhysicalDeviceSurfaceFormatsKHR(VkPhysicalDevice physicalDevice,
    ...
    if (!bp_pd_state || !pSurfaceFormats)
        return skip;

    if (pSurfaceFormatCount && *pSurfaceFormatCount > bp_pd_state->surface_formats_count) {
        skip |=
            LogWarning("BestPractices-GetPhysicalDeviceSurfaceFormatsKHR-CountMismatch",
                       physicalDevice,
                       error_obj.location.dot(Field::pSurfaceFormatCount),
                       "(%" PRIu32
                       ") is greater than the value that was returned when pSurfaceFormatCount was NULL (%" PRIu32 ").",
                       *pSurfaceFormatCount,
                       bp_pd_state->surface_formats_count);
    }
    return skip;
}

bool BestPractices::PreCallValidateQueuePresentKHR(VkQueue queue,
                                                   const VkPresentInfoKHR* pPresentInfo,
                                                   const ErrorObject& error_obj) const {
    bool skip = false;
    ...
    if (VendorCheckEnabled(kBPVendorAMD) || VendorCheckEnabled(kBPVendorNVIDIA)) {
        ...
        if (num > kNumberOfSubmissionWarningLimitAMD) {
            skip |= LogPerformanceWarning("BestPractices-Submission-ReduceNumberOfSubmissions",
                                          device,
                                          error_obj.location,
...
bool BestPractices::PreCallValidateAcquireNextImageKHR(VkDevice device,
    ...
    if (swapchain_data && swapchain_data->images.empty()) {
        skip |= LogWarning("BestPractices-vkAcquireNextImageKHR-SwapchainImagesNotFound",
                           swapchain,
                           error_obj.location,
...
void BestPractices::ManualPostCallRecordGetPhysicalDeviceSurfaceCapabilitiesKHR(
...
void BestPractices::ManualPostCallRecordGetPhysicalDeviceSurfaceCapabilities2EXT(
    VkPhysicalDevice physicalDevice,
    VkSurfaceKHR surface,
...
void BestPractices::ManualPostCallRecordGetPhysicalDeviceSurfaceFormatsKHR(VkPhysicalDevice physicalDevice,
                                                                           VkSurfaceKHR surface,
                                                                           uint32_t* pSurfaceFormatCount,
                                                                           VkSurfaceFormatKHR* pSurfaceFormats,
                                                                           const RecordObject& record_obj) {
...
void BestPractices::ManualPostCallRecordGetPhysicalDeviceSurfaceFormats2KHR(
    VkPhysicalDevice physicalDevice,
    const VkPhysicalDeviceSurfaceInfo2KHR* pSurfaceInfo,
    uint32_t* pSurfaceFormatCount,
...
void BestPractices::ManualPostCallRecordGetPhysicalDeviceDisplayPlanePropertiesKHR(
    VkPhysicalDevice physicalDevice,
...
void BestPractices::ManualPostCallRecordGetSwapchainImagesKHR(VkDevice device,
                                                              VkSwapchainKHR swapchain,
