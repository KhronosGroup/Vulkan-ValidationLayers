...
static inline bool RenderPassUsesAttachmentAsResolve(const vku::safe_VkRenderPassCreateInfo2& create_info,
                                                     uint32_t attachment) {
    for (uint32_t subpass = 0; subpass < create_info.subpassCount; subpass++) {
        ...
        if (subpass_info.pResolveAttachments) {
            for (uint32_t i = 0; i < subpass_info.colorAttachmentCount; i++) {
                if (subpass_info.pResolveAttachments[i].attachment == attachment)
                    return true;
            }
        }
    }
...
static inline bool RenderPassUsesAttachmentOnTile(const vku::safe_VkRenderPassCreateInfo2& create_info,
                                                  uint32_t attachment) {
    for (uint32_t subpass = 0; subpass < create_info.subpassCount; subpass++) {
        const auto& subpass_info = create_info.pSubpasses[subpass];
        ...
        for (uint32_t i = 0; i < subpass_info.colorAttachmentCount; i++) {
            if (subpass_info.pColorAttachments[i].attachment == attachment)
                return true;
        }
        ...
        if (subpass_info.pDepthStencilAttachment && subpass_info.pDepthStencilAttachment->attachment == attachment)
            return true;
    }
...
static inline bool RenderPassUsesAttachmentAsImageOnly(const vku::safe_VkRenderPassCreateInfo2& create_info,
                                                       uint32_t attachment) {
...
bool BestPractices::PreCallValidateCreateRenderPass(VkDevice device,
    ...
    for (uint32_t i = 0; i < pCreateInfo->attachmentCount; ++i) {
        ...
        if (pCreateInfo->pAttachments[i].initialLayout == VK_IMAGE_LAYOUT_UNDEFINED) {
            if ((vkuFormatIsColor(format) || vkuFormatHasDepth(format)) &&
                ...
                skip |= LogWarning(
                    "BestPractices-vkCreateRenderPass-attatchment-color-depth",
                    device,
                    attachment_loc,
            ...
            if (vkuFormatHasStencil(format) &&
                ...
                skip |= LogWarning(
                    "BestPractices-vkCreateRenderPass-attatchment-stencil",
                    device,
                    attachment_loc,
        ...
        if (attachment.samples > VK_SAMPLE_COUNT_1_BIT) {
            ...
            if (access_requires_memory) {
                skip |= LogPerformanceWarning(
                    ...
                    device,
                    attachment_loc,
                    "Attachment %u in the VkRenderPass is a multisampled image with %u samples, but it uses "
                    "loadOp/storeOp "
                    "which requires accessing data from memory. Multisampled images should always be loadOp = CLEAR or "
                    "DONT_CARE, "
                    ...
                    if (performance_query.optimal == VK_FALSE) {
                        skip |= LogPerformanceWarning(
                            ...
                            create_info_loc.dot(Field::pSubpasses, i)
                                .dot(Field::pResolveAttachments, j)
                                .dot(Field::attachment),
                            "(%" PRIu32 ") in the VkRenderPass has the format %s and is used as a resolve attachment, "
    ...
    for (uint32_t dependency = 0; dependency < pCreateInfo->dependencyCount; dependency++) {
        ...
        skip |= CheckPipelineStageFlags(
            device, dependency_loc.dot(Field::srcStageMask), pCreateInfo->pDependencies[dependency].srcStageMask);
        skip |= CheckPipelineStageFlags(
            device, dependency_loc.dot(Field::dstStageMask), pCreateInfo->pDependencies[dependency].dstStageMask);
    }

    return skip;
}

bool BestPractices::ValidateCmdBeginRenderPass(VkCommandBuffer commandBuffer,
    ...
    if (!clearing && pRenderPassBegin->clearValueCount > 0) {
        ...
        const LogObjectList objlist(commandBuffer, pRenderPassBegin->renderPass);
        skip |= LogWarning(
            "BestPractices-ClearValueWithoutLoadOpClear",
            objlist,
            loc,
    ...
    if (pRenderPassBegin->clearValueCount > rp_state->create_info.attachmentCount) {
        // Flag as warning because the overflowing clearValues will be ignored and could even be undefined on certain
        // platforms. This could signal a bug and there seems to be no reason for this to happen on purpose.
        const LogObjectList objlist(commandBuffer, pRenderPassBegin->renderPass);
        skip |= LogWarning(
            "BestPractices-ClearValueCountHigherThanAttachmentCount",
            objlist,
            loc,
...
bool BestPractices::ValidateCmdBeginRendering(VkCommandBuffer commandBuffer,
    ...
    for (uint32_t i = 0; i < pRenderingInfo->colorAttachmentCount; ++i) {
        ...
        if (VendorCheckEnabled(kBPVendorNVIDIA)) {
            if (color_attachment.loadOp == VK_ATTACHMENT_LOAD_OP_CLEAR) {
                ...
                skip |=
                    ValidateClearColor(commandBuffer, format, color_attachment.clearValue.color, color_attachment_loc);
            }
        }
        ...
        if (color_attachment.resolveMode == VK_RESOLVE_MODE_NONE &&
            ...
            if (resolve_image_view_state &&
                resolve_image_view_state->image_state->create_info.samples == VK_SAMPLE_COUNT_1_BIT &&
                image_view_state->image_state->create_info.samples != VK_SAMPLE_COUNT_1_BIT) {
                const LogObjectList objlist(
                    commandBuffer, resolve_image_view_state->Handle(), image_view_state->Handle());
                skip |=
                    LogWarning("BestPractices-VkRenderingInfo-ResolveModeNone",
                               commandBuffer,
...
bool BestPractices::PreCallValidateCmdBeginRendering(VkCommandBuffer commandBuffer,
                                                     const VkRenderingInfo* pRenderingInfo,
                                                     const ErrorObject& error_obj) const {
...
void BestPractices::PreCallRecordCmdEndRenderPass2(VkCommandBuffer commandBuffer,
                                                   const VkSubpassEndInfo* pSubpassInfo,
...
void BestPractices::PreCallRecordCmdEndRenderPass2KHR(VkCommandBuffer commandBuffer,
                                                      const VkSubpassEndInfoKHR* pSubpassInfo,
                                                      const RecordObject& record_obj) {
...
void BestPractices::PostCallRecordCmdBeginRendering(VkCommandBuffer commandBuffer,
                                                    const VkRenderingInfo* pRenderingInfo,
                                                    const RecordObject& record_obj) {
...
void BestPractices::PostCallRecordCmdBeginRenderingKHR(VkCommandBuffer commandBuffer,
                                                       const VkRenderingInfo* pRenderingInfo,
                                                       const RecordObject& record_obj) {
    PostCallRecordCmdBeginRendering(commandBuffer, pRenderingInfo, record_obj);
}

void BestPractices::PostCallRecordCmdNextSubpass(VkCommandBuffer commandBuffer,
    ...
    if (VendorCheckEnabled(kBPVendorNVIDIA)) {
        ...
        if (depth_image_view &&
            (depth_image_view->create_info.subresourceRange.aspectMask & VK_IMAGE_ASPECT_DEPTH_BIT) != 0U) {
...
void BestPractices::RecordCmdBeginRenderPass(bp_state::CommandBuffer& cb_state,
                                             const VkRenderPassBeginInfo* pRenderPassBegin) {
    if (!pRenderPassBegin)
        return;
    ...
    for (uint32_t att = 0; att < rp_state->create_info.attachmentCount; att++) {
        ...
        if ((!vkuFormatIsStencilOnly(attachment.format) && attachment.loadOp == VK_ATTACHMENT_LOAD_OP_LOAD) ||
            ...
            usage = IMAGE_SUBRESOURCE_USAGE_BP::RENDER_PASS_READ_TO_TILE;
                   ...
                   (vkuFormatHasStencil(attachment.format) &&
    ...
    for (uint32_t att = 0; att < rp_state->create_info.attachmentCount; att++) {
        ...
        QueueValidateImageView(
            cb_state.queue_submit_functions_after_render_pass, Func::vkCmdEndRenderPass, image_view.get(), usage);
    }
}

void BestPractices::RecordCmdBeginRenderingCommon(bp_state::CommandBuffer& cb_state) {
    auto rp = cb_state.activeRenderPass.get();
    ASSERT_AND_RETURN(rp);
...
bool BestPractices::PreCallValidateCmdBeginRenderPass(VkCommandBuffer commandBuffer,
                                                      const VkRenderPassBeginInfo* pRenderPassBegin,
                                                      VkSubpassContents contents,
                                                      const ErrorObject& error_obj) const {
...
bool BestPractices::PreCallValidateCmdBeginRenderPass2(VkCommandBuffer commandBuffer,
                                                       const VkRenderPassBeginInfo* pRenderPassBegin,
                                                       const VkSubpassBeginInfo* pSubpassBeginInfo,
                                                       const ErrorObject& error_obj) const {
    return ValidateCmdBeginRenderPass(commandBuffer, pRenderPassBegin, error_obj.location);
...
void BestPractices::PostCallRecordCmdNextSubpass2KHR(VkCommandBuffer commandBuffer,
                                                     const VkSubpassBeginInfo* pSubpassBeginInfo,
...
void BestPractices::PostCallRecordCmdNextSubpass2(VkCommandBuffer commandBuffer,
                                                  const VkSubpassBeginInfo* pSubpassBeginInfo,
                                                  const VkSubpassEndInfo* pSubpassEndInfo,
                                                  const RecordObject& record_obj) {
...
void BestPractices::PostCallRecordCmdPushConstants(VkCommandBuffer commandBuffer,
...
void BestPractices::PostCallRecordCmdPushConstants2(VkCommandBuffer commandBuffer,
...
void BestPractices::PostRecordCmdBeginRenderPass(bp_state::CommandBuffer& cb_state,
                                                 const VkRenderPassBeginInfo* pRenderPassBegin) {
    // Reset the renderpass state
    // TODO - move this logic to the Render Pass state as cb->has_draw_cmd should stay true for lifetime of command
    // buffer
    ...
    if (auto rp_state = Get<vvl::RenderPass>(pRenderPassBegin->renderPass)) {
        ...
        for (size_t i = 0; i < rp_state->create_info.subpassCount; i++) {
            ...
            if (rp_state->create_info.pSubpasses[i].pDepthStencilAttachment != nullptr)
                render_pass_state.depthAttachment = true;

            if (rp_state->create_info.pSubpasses[i].colorAttachmentCount > 0)
                render_pass_state.colorAttachment = true;
        }
        if (cb_state.activeRenderPass) {
            // Spec states that after BeginRenderPass all resources should be rebound
            if (cb_state.activeRenderPass->has_multiview_enabled) {
...
void BestPractices::PostCallRecordCmdBeginRenderPass(VkCommandBuffer commandBuffer,
                                                     const VkRenderPassBeginInfo* pRenderPassBegin,
                                                     VkSubpassContents contents,
                                                     const RecordObject& record_obj) {
...
void BestPractices::PostCallRecordCmdBeginRenderPass2(VkCommandBuffer commandBuffer,
                                                      const VkRenderPassBeginInfo* pRenderPassBegin,
                                                      const VkSubpassBeginInfo* pSubpassBeginInfo,
                                                      const RecordObject& record_obj) {
...
bool BestPractices::PreCallValidateCmdEndRenderPass2(VkCommandBuffer commandBuffer,
                                                     const VkSubpassEndInfo* pSubpassEndInfo,
                                                     const ErrorObject& error_obj) const {
...
bool BestPractices::PreCallValidateCmdEndRenderingKHR(VkCommandBuffer commandBuffer,
                                                      const ErrorObject& error_obj) const {
...
bool BestPractices::ValidateCmdEndRenderPass(VkCommandBuffer commandBuffer, const Location& loc) const {
    ...
    if ((VendorCheckEnabled(kBPVendorArm) || VendorCheckEnabled(kBPVendorIMG)) && rp) {
        ...
        for (uint32_t i = 0; i < num_attachments; i++) {
            if (!RenderPassUsesAttachmentOnTile(rp->create_info, i) ||
                RenderPassUsesAttachmentAsResolve(rp->create_info, i)) {
                continue;
            }
            ...
            auto itr =
                std::find_if(render_pass_state.touchesAttachments.begin(),
                             render_pass_state.touchesAttachments.end(),
            ...
            if (untouched_aspects) {
                skip |= LogPerformanceWarning(
                    ...
                    "On tile-based architectures, LOAD_OP_LOAD and STORE_OP_STORE consume bandwidth and should not be "
                    "part of the "
...
void BestPractices::RecordAttachmentAccess(bp_state::CommandBuffer& cb_state,
                                           uint32_t fb_attachment,
    ...
    auto itr = std::find_if(
        rp_state.touchesAttachments.begin(),
        rp_state.touchesAttachments.end(),
...
void BestPractices::RecordAttachmentClearAttachments(bp_state::CommandBuffer& cb_state,
                                                     uint32_t fb_attachment,
