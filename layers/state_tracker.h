...
#define VALSTATETRACK_MAP_AND_TRAITS_IMPL(handle_type, state_type, map_member, instance_scope)        \
    ...
    template<>                                                                                        \
    struct HandleTraits<handle_type> {                                                                \
        using StateType = state_type;                                                                 \
    };
...
class ValidationStateTracker : public ValidationObject {
  ...
  public:
    template <typename Handle>
    typename AccessorTraits<typename HandleTraits<Handle>::StateType>::ReturnType Get(Handle handle) {
        using Traits = AccessorTraits<typename HandleTraits<Handle>::StateType>;
    ...
    template <typename Handle>
    const typename AccessorTraits<typename HandleTraits<Handle>::StateType>::ReturnType Get(Handle handle) const {
        using Traits = AccessorTraits<typename HandleTraits<Handle>::StateType>;
        auto map_member = Traits::Map();
        const typename Traits::MapType& map =
            (Traits::kInstanceScope && (this->*map_member).size() == 0) ? instance_state->*map_member : this->*map_member;
    ...
    template <typename Handle>
    typename AccessorTraits<typename HandleTraits<Handle>::StateType>::SharedType GetShared(Handle handle) {
        using Traits = AccessorTraits<typename HandleTraits<Handle>::StateType>;
        auto map_member = Traits::Map();
        const typename Traits::MapType& map =
            (Traits::kInstanceScope && (this->*map_member).size() == 0) ? instance_state->*map_member : this->*map_member;
    ...
    template <typename Handle>
    typename AccessorTraits<typename HandleTraits<Handle>::StateType>::ConstSharedType GetShared(Handle handle) const {
        using Traits = AccessorTraits<typename HandleTraits<Handle>::StateType>;
        auto map_member = Traits::Map();
        const typename Traits::MapType& map =
            (Traits::kInstanceScope && (this->*map_member).size() == 0) ? instance_state->*map_member : this->*map_member;
