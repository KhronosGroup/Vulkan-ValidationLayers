...
bool StatelessValidation::manual_PreCallValidateCreateImage(VkDevice device,
    ...
    if (pCreateInfo->sharingMode == VK_SHARING_MODE_CONCURRENT) {
        ...
        if (queue_family_index_count <= 1) {
            skip |= LogError("VUID-VkImageCreateInfo-sharingMode-00942",
                             device,
                             create_info_loc.dot(Field::queueFamilyIndexCount),
                             "is %" PRIu32 ".",
        ...
        if (pCreateInfo->pQueueFamilyIndices == nullptr) {
            skip |= LogError("VUID-VkImageCreateInfo-sharingMode-00941",
                             device,
                             create_info_loc.dot(Field::pQueueFamilyIndices),
                             "is NULL.");
        }
    }

    skip |= ValidateNotZero(pCreateInfo->extent.width == 0,
                            "VUID-VkImageCreateInfo-extent-00944",
                            create_info_loc.dot(Field::extent).dot(Field::width));
    skip |= ValidateNotZero(pCreateInfo->extent.height == 0,
                            "VUID-VkImageCreateInfo-extent-00945",
                            create_info_loc.dot(Field::extent).dot(Field::height));
    skip |= ValidateNotZero(pCreateInfo->extent.depth == 0,
                            "VUID-VkImageCreateInfo-extent-00946",
                            create_info_loc.dot(Field::extent).dot(Field::depth));

    skip |= ValidateNotZero(
        pCreateInfo->mipLevels == 0, "VUID-VkImageCreateInfo-mipLevels-00947", create_info_loc.dot(Field::mipLevels));
    skip |= ValidateNotZero(pCreateInfo->arrayLayers == 0,
    ...
    if ((pCreateInfo->initialLayout != VK_IMAGE_LAYOUT_UNDEFINED) &&
        ...
        skip |= LogError("VUID-VkImageCreateInfo-initialLayout-00993",
                         device,
                         create_info_loc.dot(Field::initialLayout),
                         "is %s, but must be UNDEFINED or PREINITIALIZED.",
                         string_VkImageLayout(pCreateInfo->initialLayout));
    }

    if ((pCreateInfo->imageType == VK_IMAGE_TYPE_1D) &&
        ((pCreateInfo->extent.height != 1) || (pCreateInfo->extent.depth != 1))) {
        skip |= LogError("VUID-VkImageCreateInfo-imageType-00956",
                         device,
                         create_info_loc.dot(Field::imageType),
                         "is VK_IMAGE_TYPE_1D but extent.height (%" PRIu32 ") and extent.depth (%" PRIu32
                         ") must both be 1.",
    ...
    if (pCreateInfo->flags & VK_IMAGE_CREATE_CUBE_COMPATIBLE_BIT) {
        if (pCreateInfo->imageType != VK_IMAGE_TYPE_2D) {
            skip |= LogError("VUID-VkImageCreateInfo-flags-00949",
                             device,
        ...
        if (pCreateInfo->extent.width != pCreateInfo->extent.height) {
            skip |= LogError("VUID-VkImageCreateInfo-flags-08865",
                             device,
                             create_info_loc.dot(Field::flags),
        ...
        if (pCreateInfo->arrayLayers < 6) {
            skip |=
                LogError("VUID-VkImageCreateInfo-flags-08866",
                         device,
                         create_info_loc.dot(Field::flags),
    ...
    if (pCreateInfo->imageType == VK_IMAGE_TYPE_2D) {
        if (pCreateInfo->extent.depth != 1) {
            skip |= LogError("VUID-VkImageCreateInfo-imageType-00957",
                             device,
                             create_info_loc.dot(Field::imageType),
                             "is VK_IMAGE_TYPE_2D but extent.depth (%" PRIu32 ") must be 1.",
                             pCreateInfo->extent.depth);
        }
    }
    ...
    if ((pCreateInfo->imageType == VK_IMAGE_TYPE_3D) && (pCreateInfo->arrayLayers != 1)) {
        skip |= LogError("VUID-VkImageCreateInfo-imageType-00961",
                         device,
                         create_info_loc.dot(Field::imageType),
                         "is VK_IMAGE_TYPE_3D but arrayLayers (%" PRIu32 ") must be 1.",
                         pCreateInfo->arrayLayers);
    }
    ...
    if (0 != (pCreateInfo->usage & VK_IMAGE_USAGE_TRANSIENT_ATTACHMENT_BIT)) {
        ...
        if (0 == (pCreateInfo->usage & legal_flags)) {
            skip |= LogError("VUID-VkImageCreateInfo-usage-00966",
                             device,
    ...
    if (max_dim > 0 && pCreateInfo->mipLevels > max_mip_levels) {
        skip |= LogError("VUID-VkImageCreateInfo-mipLevels-00958",
                         device,
                         create_info_loc.dot(Field::mipLevels),
                         "(%" PRIu32 ") must be less than or equal to %" PRIu32 " (based on extent of %s)",
                         pCreateInfo->mipLevels,
                         max_mip_levels,
                         string_VkExtent3D(pCreateInfo->extent).c_str());
    }

    if ((image_flags & VK_IMAGE_CREATE_2D_ARRAY_COMPATIBLE_BIT) && (pCreateInfo->imageType != VK_IMAGE_TYPE_3D)) {
        skip |= LogError("VUID-VkImageCreateInfo-flags-00950",
                         device,
                         create_info_loc.dot(Field::flags),
    ...
    if ((image_flags & VK_IMAGE_CREATE_2D_VIEW_COMPATIBLE_BIT_EXT) && (pCreateInfo->imageType != VK_IMAGE_TYPE_3D)) {
        skip |= LogError("VUID-VkImageCreateInfo-flags-07755",
                         device,
                         create_info_loc.dot(Field::flags),
    ...
    if (((image_flags & VK_IMAGE_CREATE_SAMPLE_LOCATIONS_COMPATIBLE_DEPTH_BIT_EXT) != 0) &&
        ...
        skip |= LogError("VUID-VkImageCreateInfo-flags-01533",
                         device,
                         create_info_loc.dot(Field::flags),
    ...
    if (!enabled_features.hostImageCopy && (pCreateInfo->usage & VK_IMAGE_USAGE_HOST_TRANSFER_BIT) != 0) {
        skip |= LogError("VUID-VkImageCreateInfo-usage-10245",
                         device,
                         create_info_loc.dot(Field::usage),
    ...
    if (pCreateInfo->tiling == VK_IMAGE_TILING_LINEAR) {
        ...
        image_create_maybe_linear = (std::find(image_create_drm_format_modifiers.begin(),
                                               image_create_drm_format_modifiers.end(),
    ...
    if ((pCreateInfo->samples != VK_SAMPLE_COUNT_1_BIT) &&
        ...
        skip |= LogError("VUID-VkImageCreateInfo-samples-02257",
                         device,
                         create_info_loc,
                         "image created with\n"
                         "samples = %s\n"
    ...
    if ((image_flags & VK_IMAGE_CREATE_SPLIT_INSTANCE_BIND_REGIONS_BIT) &&
        ((pCreateInfo->mipLevels != 1) || (pCreateInfo->arrayLayers != 1) ||
         (pCreateInfo->imageType != VK_IMAGE_TYPE_2D) || image_create_maybe_linear)) {
        skip |= LogError("VUID-VkImageCreateInfo-flags-02259",
                         device,
                         create_info_loc,
                         "image created with\n"
                         "imageType = %s\n"
    ...
    if (pCreateInfo->usage & VK_IMAGE_USAGE_FRAGMENT_DENSITY_MAP_BIT_EXT) {
        if (pCreateInfo->imageType != VK_IMAGE_TYPE_2D) {
            skip |= LogError("VUID-VkImageCreateInfo-flags-02557",
                             device,
                             create_info_loc.dot(Field::usage),
        ...
        if (pCreateInfo->samples != VK_SAMPLE_COUNT_1_BIT) {
            skip |= LogError("VUID-VkImageCreateInfo-samples-02558",
                             device,
                             create_info_loc.dot(Field::usage),
    ...
    if (image_flags & VK_IMAGE_CREATE_SUBSAMPLED_BIT_EXT) {
        if (pCreateInfo->tiling != VK_IMAGE_TILING_OPTIMAL) {
            skip |= LogError("VUID-VkImageCreateInfo-flags-02565",
                             device,
                             create_info_loc.dot(Field::flags),
        ...
        if (pCreateInfo->imageType != VK_IMAGE_TYPE_2D) {
            skip |= LogError("VUID-VkImageCreateInfo-flags-02566",
                             device,
                             create_info_loc.dot(Field::flags),
        ...
        if (pCreateInfo->mipLevels != 1) {
            skip |= LogError("VUID-VkImageCreateInfo-flags-02568",
                             device,
                             create_info_loc.dot(Field::flags),
                             "includes VK_IMAGE_CREATE_SUBSAMPLED_BIT_EXT, but mipLevels is %" PRIu32 ".",
                             pCreateInfo->mipLevels);
        }
    }
    ...
    if (vkuFormatIsXChromaSubsampled(image_format) && (SafeModulo(pCreateInfo->extent.width, 2) != 0)) {
        skip |= LogError("VUID-VkImageCreateInfo-format-04712",
                         device,
    ...
    if (vkuFormatIsYChromaSubsampled(image_format) && (SafeModulo(pCreateInfo->extent.height, 2) != 0)) {
        skip |= LogError("VUID-VkImageCreateInfo-format-04713",
                         device,
                         create_info_loc.dot(Field::format),
                         "(%s) is Y Chroma Subsampled (has _420 suffix) so the height (%" PRIu32
                         ") must be a multiple of 2.",
                         string_VkFormat(image_format),
                         pCreateInfo->extent.height);
    }
...
bool StatelessValidation::ValidateCreateImageSparse(const VkImageCreateInfo& create_info,
    ...
    if (has_sparse_flags) {
        ...
        if (image_flags & VK_IMAGE_CREATE_2D_VIEW_COMPATIBLE_BIT_EXT) {
            skip |= LogError("VUID-VkImageCreateInfo-imageType-10197",
                             device,
                             create_info_loc.dot(Field::flags),
                             "is %s.",
                             string_VkImageCreateFlags(image_flags).c_str());
        ...
        if (image_flags & VK_IMAGE_CREATE_2D_ARRAY_COMPATIBLE_BIT) {
            skip |= LogError("VUID-VkImageCreateInfo-flags-09403",
                             device,
                             create_info_loc.dot(Field::flags),
                             "is %s.",
                             string_VkImageCreateFlags(image_flags).c_str());
    ...
    if ((image_flags & VK_IMAGE_CREATE_SPARSE_BINDING_BIT) && (!enabled_features.sparseBinding)) {
        skip |= LogError("VUID-VkImageCreateInfo-flags-00969",
                         device,
                         create_info_loc.dot(Field::flags),
    ...
    // If flags contains VK_IMAGE_CREATE_SPARSE_RESIDENCY_BIT or VK_IMAGE_CREATE_SPARSE_ALIASED_BIT, it must also
    ...
    if (((image_flags & (VK_IMAGE_CREATE_SPARSE_RESIDENCY_BIT | VK_IMAGE_CREATE_SPARSE_ALIASED_BIT)) != 0) &&
        ...
        skip |= LogError("VUID-VkImageCreateInfo-flags-00987",
                         device,
                         create_info_loc.dot(Field::flags),
                         "is %s.",
                         string_VkImageCreateFlags(image_flags).c_str());
    ...
    if ((image_flags & VK_IMAGE_CREATE_SPARSE_RESIDENCY_BIT) != 0) {
        ...
        if (VK_IMAGE_TILING_LINEAR == create_info.tiling) {
            skip |= LogError("VUID-VkImageCreateInfo-tiling-04121",
                             device,
                             create_info_loc.dot(Field::flags),
        ...
        if (VK_IMAGE_TYPE_1D == create_info.imageType) {
            skip |= LogError("VUID-VkImageCreateInfo-imageType-00970",
                             device,
                             create_info_loc.dot(Field::flags),
        ...
        if ((!enabled_features.sparseResidencyImage2D) && (VK_IMAGE_TYPE_2D == create_info.imageType)) {
            skip |= LogError("VUID-VkImageCreateInfo-imageType-00971",
                             device,
                             create_info_loc.dot(Field::flags),
        ...
        if ((!enabled_features.sparseResidencyImage3D) && (VK_IMAGE_TYPE_3D == create_info.imageType)) {
            skip |= LogError("VUID-VkImageCreateInfo-imageType-00972",
                             device,
                             create_info_loc.dot(Field::flags),
        ...
        if (VK_IMAGE_TYPE_2D == create_info.imageType) {
            ...
            } else if ((!enabled_features.sparseResidency16Samples) &&
                       (VK_SAMPLE_COUNT_16_BIT == create_info.samples)) {
                skip |= LogError(
...
bool StatelessValidation::ValidateCreateImageFragmentShadingRate(const VkImageCreateInfo& create_info,
    ...
    if ((create_info.usage & VK_IMAGE_USAGE_FRAGMENT_SHADING_RATE_ATTACHMENT_BIT_KHR) == 0)
        return skip;

    if (create_info.imageType != VK_IMAGE_TYPE_2D) {
        skip |= LogError("VUID-VkImageCreateInfo-imageType-02082",
                         device,
                         create_info_loc.dot(Field::usage),
    ...
    if (create_info.samples != VK_SAMPLE_COUNT_1_BIT) {
        skip |= LogError("VUID-VkImageCreateInfo-samples-02083",
                         device,
                         create_info_loc.dot(Field::usage),
                         "includes VK_IMAGE_USAGE_FRAGMENT_SHADING_RATE_ATTACHMENT_BIT_KHR (or the "
    ...
    if (enabled_features.shadingRateImage && create_info.tiling != VK_IMAGE_TILING_OPTIMAL) {
        ...
        skip |= LogError("VUID-VkImageCreateInfo-shadingRateImage-07727",
                         device,
                         create_info_loc.dot(Field::usage),
...
bool StatelessValidation::ValidateCreateImageCornerSampled(const VkImageCreateInfo& create_info,
                                                           const Location& create_info_loc) const {
    bool skip = false;
    if ((create_info.flags & VK_IMAGE_CREATE_CORNER_SAMPLED_BIT_NV) == 0)
        return skip;

    if (create_info.imageType != VK_IMAGE_TYPE_2D && create_info.imageType != VK_IMAGE_TYPE_3D) {
        skip |= LogError("VUID-VkImageCreateInfo-flags-02050",
                         device,
                         create_info_loc.dot(Field::flags),
    ...
    if ((create_info.flags & VK_IMAGE_CREATE_CUBE_COMPATIBLE_BIT) || vkuFormatIsDepthOrStencil(create_info.format)) {
        skip |= LogError("VUID-VkImageCreateInfo-flags-02051",
                         device,
                         create_info_loc.dot(Field::flags),
                         "includes VK_IMAGE_CREATE_CORNER_SAMPLED_BIT_NV, "
    ...
    if (create_info.imageType == VK_IMAGE_TYPE_2D &&
        (create_info.extent.width == 1 || create_info.extent.height == 1)) {
        skip |= LogError("VUID-VkImageCreateInfo-flags-02052",
                         device,
                         create_info_loc.dot(Field::flags),
        ...
        skip |= LogError("VUID-VkImageCreateInfo-flags-02053",
                         device,
                         create_info_loc.dot(Field::flags),
                         "includes VK_IMAGE_CREATE_CORNER_SAMPLED_BIT_NV and "
...
bool StatelessValidation::ValidateCreateImageStencilUsage(const VkImageCreateInfo& create_info,
    ...
    if ((image_stencil_struct->stencilUsage & VK_IMAGE_USAGE_TRANSIENT_ATTACHMENT_BIT) != 0) {
        ...
        if ((image_stencil_struct->stencilUsage & ~legal_flags) != 0) {
            skip |= LogError("VUID-VkImageStencilUsageCreateInfo-stencilUsage-02539",
                             device,
    ...
    if (vkuFormatIsDepthOrStencil(create_info.format)) {
        if ((image_stencil_struct->stencilUsage & VK_IMAGE_USAGE_INPUT_ATTACHMENT_BIT) != 0) {
            if (create_info.extent.width > device_limits.maxFramebufferWidth) {
                skip |= LogError("VUID-VkImageCreateInfo-Format-02536",
                                 device,
                                 create_info_loc.pNext(Struct::VkImageStencilUsageCreateInfo, Field::stencilUsage),
            ...
            if (create_info.extent.height > device_limits.maxFramebufferHeight) {
                skip |= LogError("VUID-VkImageCreateInfo-format-02537",
                                 device,
                                 create_info_loc.pNext(Struct::VkImageStencilUsageCreateInfo, Field::stencilUsage),
        ...
        if (!enabled_features.shaderStorageImageMultisample &&
            ...
            skip |= LogError("VUID-VkImageCreateInfo-format-02538",
                             device,
                             create_info_loc.pNext(Struct::VkImageStencilUsageCreateInfo, Field::stencilUsage),
...
bool StatelessValidation::ValidateCreateImageCompressionControl(const VkImageCreateInfo& create_info,
    ...
    skip |= ValidateFlags(create_info_loc.pNext(Struct::VkImageCompressionControlEXT, Field::flags),
                          vvl::FlagBitmask::VkImageCompressionFlagBitsEXT,
                          AllVkImageCompressionFlagBitsEXT,
                          image_compression_control->flags,
                          kOptionalSingleBit,
                          VK_NULL_HANDLE,
    ...
    if (image_compression_control->flags == VK_IMAGE_COMPRESSION_FIXED_RATE_EXPLICIT_EXT &&
        ...
        skip |= LogError("VUID-VkImageCompressionControlEXT-flags-06748",
                         device,
...
bool StatelessValidation::ValidateCreateImageSwapchain(const VkImageCreateInfo& create_info,
    ...
    // to be returned when this error occurs. Check for matching Swapchain flags is done later in state tracking
    // validation
    ...
    if (create_info.imageType != VK_IMAGE_TYPE_2D) {
        ...
        skip |= LogError(vuid,
                         ...
                         swapchain_loc,
                         "is not NULL, but imageType (%s) is not VK_IMAGE_TYPE_2D.",
                         string_VkImageType(create_info.imageType));
    }
    if (create_info.mipLevels != 1) {
        skip |= LogError(vuid,
                         swapchain_create_info->swapchain,
                         swapchain_loc,
                         "is not NULL, but mipLevels (%" PRIu32 ") is not 1.",
    ...
    if (create_info.tiling != VK_IMAGE_TILING_OPTIMAL) {
        skip |= LogError(vuid,
                         swapchain_create_info->swapchain,
                         swapchain_loc,
                         "is not NULL, but tiling (%s) is not VK_IMAGE_TILING_OPTIMAL.",
    ...
    if ((create_info.flags & ~valid_flags) != 0) {
        skip |= LogError(vuid,
                         ...
                         string_VkImageCreateFlags(valid_flags).c_str());
    }
...
bool StatelessValidation::ValidateCreateImageFormatList(const VkImageCreateInfo& create_info,
    ...
    if (!mutable_image && view_format_count > 1) {
        skip |= LogError("VUID-VkImageCreateInfo-flags-04738",
                         device,
    ...
    for (uint32_t i = 0; i < view_format_count; i++) {
        ...
        if (view_format == VK_FORMAT_UNDEFINED) {
            skip |= LogError("VUID-VkImageFormatListCreateInfo-viewFormatCount-09540",
                             device,
                             format_loc,
            ...
            if (vkuFormatIsMultiplane(view_format)) {
                ...
                const VkFormat plane_0_format =
                    vkuFindMultiplaneCompatibleFormat(image_format, VK_IMAGE_ASPECT_PLANE_0_BIT);
                const VkFormat plane_1_format =
                    vkuFindMultiplaneCompatibleFormat(image_format, VK_IMAGE_ASPECT_PLANE_1_BIT);
                const VkFormat plane_2_format =
                    vkuFindMultiplaneCompatibleFormat(image_format, VK_IMAGE_ASPECT_PLANE_2_BIT);
                ...
                if (!found_compatible) {
                    ...
                    skip |= LogError("VUID-VkImageCreateInfo-pNext-10062",
                                     device,
                                     format_loc,
            ...
            if (image_flags & VK_IMAGE_CREATE_BLOCK_TEXEL_VIEW_COMPATIBLE_BIT) {
                ...
                if (!size_compatible) {
                    skip |= LogError("VUID-VkImageCreateInfo-pNext-06722",
                                     device,
                                     format_loc,
...
bool StatelessValidation::ValidateCreateImageMetalObject(const VkImageCreateInfo& create_info,
    ...
    while (export_metal_object_info) {
        if ((export_metal_object_info->exportObjectType != VK_EXPORT_METAL_OBJECT_TYPE_METAL_TEXTURE_BIT_EXT) &&
            ...
            skip |= LogError("VUID-VkImageCreateInfo-pNext-06783",
                             device,
    ...
    while (import_metal_texture_info) {
        ...
        if ((import_metal_texture_info->plane != VK_IMAGE_ASPECT_PLANE_0_BIT) &&
            ...
            skip |= LogError("VUID-VkImageCreateInfo-pNext-06784",
                             device,
                             texture_info_loc,
        ...
        if ((format_plane_count <= 1) && (import_metal_texture_info->plane != VK_IMAGE_ASPECT_PLANE_0_BIT)) {
            skip |=
                LogError("VUID-VkImageCreateInfo-pNext-06785",
                         device,
                         texture_info_loc,
        ...
        if ((format_plane_count == 2) && (import_metal_texture_info->plane == VK_IMAGE_ASPECT_PLANE_2_BIT)) {
            skip |=
                LogError("VUID-VkImageCreateInfo-pNext-06786",
                         device,
                         texture_info_loc,
        ...
        import_metal_texture_info =
            vku::FindStructInPNextChain<VkImportMetalTextureInfoEXT>(import_metal_texture_info->pNext);
    }
#endif // VK_USE_PLATFORM_METAL_EXT
    return skip;
}

bool StatelessValidation::ValidateCreateImageDrmFormatModifiers(
    ...
    const auto drm_format_mod_list =
    ...
    if (create_info.tiling == VK_IMAGE_TILING_DRM_FORMAT_MODIFIER_EXT) {
        if ((!drm_format_mod_list) && (!drm_format_mod_explict)) {
            skip |= LogError("VUID-VkImageCreateInfo-tiling-02261",
                             device,
                             create_info_loc.dot(Field::tiling),
        ...
        if (create_info.flags & VK_IMAGE_CREATE_MUTABLE_FORMAT_BIT) {
            ...
            if (!format_list_info) {
                skip |=
                    LogError("VUID-VkImageCreateInfo-tiling-02353",
                             device,
                             create_info_loc.dot(Field::tiling),
        ...
        skip |= LogError("VUID-VkImageCreateInfo-pNext-02262",
                         device,
                         create_info_loc.dot(Field::tiling),
                         "is %s, but there is a "
    ...
    if (drm_format_mod_explict && drm_format_mod_explict->pPlaneLayouts) {
        for (uint32_t i = 0; i < drm_format_mod_explict->drmFormatModifierPlaneCount; ++i) {
            ...
            if (drm_format_mod_explict->pPlaneLayouts[i].size != 0) {
                skip |= LogError("VUID-VkImageDrmFormatModifierExplicitCreateInfoEXT-size-02267",
                                 device,
                                 drm_loc.dot(Field::size),
                                 "is not zero (%" PRIu64 ").",
                                 drm_format_mod_explict->pPlaneLayouts[i].size);
            }
            if (create_info.arrayLayers == 1 && drm_format_mod_explict->pPlaneLayouts[i].arrayPitch != 0) {
                skip |= LogError("VUID-VkImageDrmFormatModifierExplicitCreateInfoEXT-arrayPitch-02268",
                                 device,
                                 drm_loc.dot(Field::arrayPitch),
                                 "is %" PRIu64 " and arrayLayers is 1.",
            ...
            if (create_info.extent.depth == 1 && drm_format_mod_explict->pPlaneLayouts[i].depthPitch != 0) {
                skip |= LogError("VUID-VkImageDrmFormatModifierExplicitCreateInfoEXT-depthPitch-02269",
                                 device,
                                 drm_loc.dot(Field::depthPitch),
                                 "is %" PRIu64 " and extext.depth is 1.",
    ...
    if (drm_format_mod_explict && compression_control) {
        skip |= LogError("VUID-VkImageCreateInfo-pNext-06746",
                         device,
...
bool StatelessValidation::manual_PreCallValidateCreateImageView(VkDevice device,
    ...
    if ((pCreateInfo->viewType == VK_IMAGE_VIEW_TYPE_CUBE_ARRAY) && (!enabled_features.imageCubeArray)) {
        skip |= LogError("VUID-VkImageViewCreateInfo-viewType-01004",
                         pCreateInfo->image,
    ...
    if (pCreateInfo->subresourceRange.layerCount != VK_REMAINING_ARRAY_LAYERS) {
        if (pCreateInfo->viewType == VK_IMAGE_VIEW_TYPE_CUBE && pCreateInfo->subresourceRange.layerCount != 6) {
            skip |= LogError("VUID-VkImageViewCreateInfo-viewType-02960",
                             pCreateInfo->image,
                             create_info_loc.dot(Field::subresourceRange).dot(Field::layerCount),
                             "(%" PRIu32 ") must be 6 or VK_REMAINING_ARRAY_LAYERS.",
                             pCreateInfo->subresourceRange.layerCount);
        }
        if (pCreateInfo->viewType == VK_IMAGE_VIEW_TYPE_CUBE_ARRAY &&
            (pCreateInfo->subresourceRange.layerCount % 6) != 0) {
            skip |= LogError("VUID-VkImageViewCreateInfo-viewType-02961",
                             pCreateInfo->image,
                             create_info_loc.dot(Field::subresourceRange).dot(Field::layerCount),
    ...
    if (astc_decode_mode != nullptr) {
        if ((astc_decode_mode->decodeMode != VK_FORMAT_R16G16B16A16_SFLOAT) &&
            ...
            skip |= LogError("VUID-VkImageViewASTCDecodeModeEXT-decodeMode-02230",
                             pCreateInfo->image,
    ...
    if (ycbcr_conversion != nullptr) {
        if (ycbcr_conversion->conversion != VK_NULL_HANDLE) {
            if (IsIdentitySwizzle(pCreateInfo->components) == false) {
                skip |= LogError("VUID-VkImageViewCreateInfo-pNext-01970",
                                 pCreateInfo->image,
                                 create_info_loc,
...
bool StatelessValidation::manual_PreCallValidateGetDeviceImageSubresourceLayout(
    VkDevice device,
    ...
    if (GetBitSetCount(aspect_mask) != 1) {
        skip |= LogError("VUID-VkDeviceImageSubresourceInfo-aspectMask-00997",
                         device,
                         subresource_loc.dot(Field::aspectMask),
                         "(%s) must have exactly 1 bit set.",
                         string_VkImageAspectFlags(aspect_mask).c_str());
    }

    if (subresource.mipLevel >= create_info.mipLevels) {
        skip |= LogError("VUID-VkDeviceImageSubresourceInfo-mipLevel-01716",
                         device,
                         subresource_loc.dot(Field::mipLevel),
                         "(%" PRIu32 ") must be less than %s (%" PRIu32 ").",
                         subresource.mipLevel,
                         create_info_loc.dot(Field::mipLevel).Fields().c_str(),
                         create_info.mipLevels);
    }

    if (subresource.arrayLayer >= create_info.arrayLayers) {
        skip |= LogError("VUID-VkDeviceImageSubresourceInfo-arrayLayer-01717",
                         device,
                         subresource_loc.dot(Field::arrayLayer),
                         "(%" PRIu32 ") must be less than %s (%" PRIu32 ").",
                         subresource.arrayLayer,
                         create_info_loc.dot(Field::arrayLayers).Fields().c_str(),
    ...
    if (vkuFormatIsColor(image_format) && !vkuFormatIsMultiplane(image_format) &&
        (aspect_mask != VK_IMAGE_ASPECT_COLOR_BIT) && tiling_linear_optimal) {
        skip |= LogError("VUID-VkDeviceImageSubresourceInfo-format-08886",
                         device,
                         subresource_loc.dot(Field::aspectMask),
                         "(%s) is invalid with %s (%s).",
                         string_VkImageAspectFlags(aspect_mask).c_str(),
                         create_info_loc.dot(Field::format).Fields().c_str(),
                         string_VkFormat(image_format));
    }

    if (vkuFormatHasDepth(image_format) && ((aspect_mask & VK_IMAGE_ASPECT_DEPTH_BIT) == 0)) {
        skip |= LogError("VUID-VkDeviceImageSubresourceInfo-format-04462",
                         device,
                         subresource_loc.dot(Field::aspectMask),
                         "(%s) is invalid with %s (%s).",
                         string_VkImageAspectFlags(aspect_mask).c_str(),
    ...
    if (vkuFormatHasStencil(image_format) && ((aspect_mask & VK_IMAGE_ASPECT_STENCIL_BIT) == 0)) {
        skip |= LogError("VUID-VkDeviceImageSubresourceInfo-format-04463",
                         device,
                         subresource_loc.dot(Field::aspectMask),
                         "(%s) is invalid with %s (%s).",
                         string_VkImageAspectFlags(aspect_mask).c_str(),
    ...
    if (!vkuFormatHasDepth(image_format) && !vkuFormatHasStencil(image_format)) {
        if ((aspect_mask & (VK_IMAGE_ASPECT_DEPTH_BIT | VK_IMAGE_ASPECT_STENCIL_BIT)) != 0) {
            skip |= LogError("VUID-VkDeviceImageSubresourceInfo-format-04464",
                             device,
                             subresource_loc.dot(Field::aspectMask),
                             "(%s) is invalid with %s (%s).",
                             string_VkImageAspectFlags(aspect_mask).c_str(),
    ...
    if (create_info.tiling == VK_IMAGE_TILING_LINEAR) {
        if (vkuFormatIsMultiplane(image_format) && !IsOnlyOneValidPlaneAspect(image_format, aspect_mask)) {
            skip |= LogError("VUID-VkDeviceImageSubresourceInfo-tiling-08717",
                             device,
                             subresource_loc.dot(Field::aspectMask),
                             "(%s) is invalid for %s (%s).",
                             string_VkImageAspectFlags(aspect_mask).c_str(),
                             create_info_loc.dot(Field::format).Fields().c_str(),
                             string_VkFormat(image_format));
        }
