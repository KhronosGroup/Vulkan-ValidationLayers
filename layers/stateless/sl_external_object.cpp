...
bool StatelessValidation::manual_PreCallValidateGetMemoryFdKHR(VkDevice device,
                                                               const VkMemoryGetFdInfoKHR* pGetFdInfo,
                                                               int* pFd,
                                                               const ErrorObject& error_obj) const {
    constexpr auto allowed_types =
    ...
    if (0 == (pGetFdInfo->handleType & allowed_types)) {
        skip |= LogError("VUID-VkMemoryGetFdInfoKHR-handleType-00672",
                         pGetFdInfo->memory,
...
bool StatelessValidation::manual_PreCallValidateGetMemoryFdPropertiesKHR(VkDevice device,
    ...
    bool skip = false;
    if (fd < 0) {
        skip |= LogError("VUID-vkGetMemoryFdPropertiesKHR-fd-00673",
                         device,
                         error_obj.location.dot(Field::fd),
                         "handle (%d) is not a valid POSIX file descriptor.",
                         fd);
    }
    if (handleType == VK_EXTERNAL_MEMORY_HANDLE_TYPE_OPAQUE_FD_BIT) {
        skip |= LogError("VUID-vkGetMemoryFdPropertiesKHR-handleType-00674",
                         device,
...
bool StatelessValidation::ValidateExternalSemaphoreHandleType(VkSemaphore semaphore,
                                                              const char* vuid,
    ...
    if (0 == (handle_type & allowed_types)) {
        skip |= LogError(vuid,
                         semaphore,
                         handle_type_loc,
                         "%s is not one of the supported handleTypes (%s).",
...
bool StatelessValidation::ValidateExternalFenceHandleType(VkFence fence,
                                                          const char* vuid,
                                                          const Location& handle_type_loc,
...
bool StatelessValidation::manual_PreCallValidateGetSemaphoreFdKHR(VkDevice device,
                                                                  const VkSemaphoreGetFdInfoKHR* pGetFdInfo,
                                                                  int* pFd,
                                                                  const ErrorObject& error_obj) const {
    return ValidateExternalSemaphoreHandleType(pGetFdInfo->semaphore,
...
bool StatelessValidation::manual_PreCallValidateImportSemaphoreFdKHR(
    VkDevice device, const VkImportSemaphoreFdInfoKHR* pImportSemaphoreFdInfo, const ErrorObject& error_obj) const {
    bool skip = false;
    const Location info_loc = error_obj.location.dot(Field::pImportSemaphoreFdInfo);
    skip |= ValidateExternalSemaphoreHandleType(pImportSemaphoreFdInfo->semaphore,
...
bool StatelessValidation::manual_PreCallValidateGetFenceFdKHR(VkDevice device,
                                                              const VkFenceGetFdInfoKHR* pGetFdInfo,
                                                              int* pFd,
                                                              const ErrorObject& error_obj) const {
    return ValidateExternalFenceHandleType(pGetFdInfo->fence,
...
bool StatelessValidation::manual_PreCallValidateImportFenceFdKHR(VkDevice device,
                                                                 const VkImportFenceFdInfoKHR* pImportFenceFdInfo,
                                                                 const ErrorObject& error_obj) const {
    bool skip = false;
    const Location info_loc = error_obj.location.dot(Field::pImportFenceFdInfo);
    skip |= ValidateExternalFenceHandleType(pImportFenceFdInfo->fence,
    ...
    if (pImportFenceFdInfo->handleType == VK_EXTERNAL_FENCE_HANDLE_TYPE_SYNC_FD_BIT &&
        ...
        skip |= LogError("VUID-VkImportFenceFdInfoKHR-handleType-07306",
...
bool StatelessValidation::manual_PreCallValidateGetMemoryHostPointerPropertiesEXT(
    ...
    if (handleType != VK_EXTERNAL_MEMORY_HANDLE_TYPE_HOST_ALLOCATION_BIT_EXT &&
        ...
        skip |= LogError("VUID-vkGetMemoryHostPointerPropertiesEXT-handleType-01752",
                         device,
                         error_obj.location.dot(Field::handleType),
                         "is %s.",
    ...
    if (SafeModulo(host_pointer, phys_dev_ext_props.external_memory_host_props.minImportedHostPointerAlignment) != 0) {
        skip |= LogError("VUID-vkGetMemoryHostPointerPropertiesEXT-pHostPointer-01753",
                         device,
                         error_obj.location.dot(Field::pHostPointer),
                         "(0x%" PRIxLEAST64 ") is not aligned "
...
bool StatelessValidation::manual_PreCallValidateGetMemoryWin32HandleKHR(
    VkDevice device,
    ...
    if ((pGetWin32HandleInfo->handleType & (nt_handles | global_share_handles)) == 0) {
        skip |= LogError("VUID-VkMemoryGetWin32HandleInfoKHR-handleType-00664",
...
bool StatelessValidation::manual_PreCallValidateGetMemoryWin32HandlePropertiesKHR(
    ...
    if (handle == NULL || handle == INVALID_HANDLE_VALUE) {
        ...
        skip |= LogError("VUID-vkGetMemoryWin32HandlePropertiesKHR-handle-00665",
                         device,
                         error_obj.location.dot(Field::handle),
                         "(0x%" PRIxPTR ") is not a valid Windows handle.",
    ...
    if (handleType == VK_EXTERNAL_MEMORY_HANDLE_TYPE_OPAQUE_WIN32_BIT ||
        ...
        skip |= LogError("VUID-vkGetMemoryWin32HandlePropertiesKHR-handleType-00666",
                         device,
                         error_obj.location.dot(Field::handleType),
                         "%s is not allowed.",
                         string_VkExternalMemoryHandleTypeFlagBits(handleType));
...
bool StatelessValidation::manual_PreCallValidateImportSemaphoreWin32HandleKHR(
    VkDevice device, const VkImportSemaphoreWin32HandleInfoKHR* info, const ErrorObject& error_obj) const {
    bool skip = false;

    skip |= ValidateExternalSemaphoreHandleType(
        info->semaphore,
    ...
    if ((info->handleType & kNameAllowedTypes) == 0 && info->name) {
        skip |= LogError("VUID-VkImportSemaphoreWin32HandleInfoKHR-handleType-01466",
                         info->semaphore,
                         error_obj.location.dot(Field::pImportSemaphoreWin32HandleInfo).dot(Field::name),
                         "(%p) must be NULL if handleType is %s",
                         reinterpret_cast<const void*>(info->name),
                         string_VkExternalSemaphoreHandleTypeFlagBits(info->handleType));
    }
    if (info->handle && info->name) {
        skip |= LogError("VUID-VkImportSemaphoreWin32HandleInfoKHR-handle-01469",
                         info->semaphore,
                         error_obj.location.dot(Field::pImportSemaphoreWin32HandleInfo),
                         "both handle (%p) and name (%p) are non-NULL",
                         info->handle,
                         reinterpret_cast<const void*>(info->name));
    }
    return skip;
...
bool StatelessValidation::manual_PreCallValidateGetSemaphoreWin32HandleKHR(VkDevice device,
                                                                           ...
                                                                           HANDLE* pHandle,
                                                                           const ErrorObject& error_obj) const {
    return ValidateExternalSemaphoreHandleType(
        info->semaphore,
...
bool StatelessValidation::manual_PreCallValidateImportFenceWin32HandleKHR(VkDevice device,
                                                                          ...
                                                                          const ErrorObject& error_obj) const {
    bool skip = false;
    skip |= ValidateExternalFenceHandleType(
        info->fence,
    ...
    if ((info->handleType & kNameAllowedTypes) == 0 && info->name) {
        skip |= LogError("VUID-VkImportFenceWin32HandleInfoKHR-handleType-01459",
                         info->fence,
    ...
    if (info->handle && info->name) {
        skip |= LogError("VUID-VkImportFenceWin32HandleInfoKHR-handle-01462",
                         info->fence,
...
bool StatelessValidation::manual_PreCallValidateGetFenceWin32HandleKHR(VkDevice device,
                                                                       const VkFenceGetWin32HandleInfoKHR* info,
                                                                       HANDLE* pHandle,
                                                                       const ErrorObject& error_obj) const {
    return ValidateExternalFenceHandleType(info->fence,
...
bool StatelessValidation::ExportMetalObjectsPNextUtil(VkExportMetalObjectTypeFlagBitsEXT bit,
                                                      const char* vuid,
                                                      const Location& loc,
                                                      const char* sType,
...
ExternalOperationsInfo GetExternalOperationsInfo(const void* pNext) {
    ...
    // NOTE: There's no handleType on VkImportMemoryBufferCollectionFUCHSIA, so we can't check that, and from the "Valid
    // Usage (Implicit)" collection has to  always be valid.
...
bool StatelessValidation::ValidateAllocateMemoryExternal(VkDevice device,
    ...
    if (!ext.has_export && ext.total_import_ops == 0 && allocate_info.allocationSize == 0) {
        skip |= LogError("VUID-VkMemoryAllocateInfo-allocationSize-07897",
                         device,
    ...
    if (opaque_alloc_info && opaque_alloc_info->opaqueCaptureAddress != 0) {
        ...
        if (!(flags & VK_MEMORY_ALLOCATE_DEVICE_ADDRESS_CAPTURE_REPLAY_BIT)) {
            skip |= LogError("VUID-VkMemoryAllocateInfo-opaqueCaptureAddress-03329",
                             device,
                             address_loc,
                             "is non-zero (%" PRIu64 ") so VkMemoryAllocateFlagsInfo::flags must include "
                             "VK_MEMORY_ALLOCATE_DEVICE_ADDRESS_CAPTURE_REPLAY_BIT.",
                             opaque_alloc_info->opaqueCaptureAddress);
        }

        if (ext.import_info_host_pointer) {
            skip |= LogError("VUID-VkMemoryAllocateInfo-pNext-03332",
                             device,
                             address_loc,
                             "is non-zero (%" PRIu64
                             ") but the pNext chain includes a VkImportMemoryHostPointerInfoEXT structure.",
                             opaque_alloc_info->opaqueCaptureAddress);
        }

        if (ext.total_import_ops > 0) {
            skip |= LogError("VUID-VkMemoryAllocateInfo-opaqueCaptureAddress-03333",
                             device,
                             address_loc,
    ...
    if (ext.total_import_ops > 1) {
        skip |= LogError("VUID-VkMemoryAllocateInfo-None-06657",
                         device,
                         allocate_info_loc,
                         "%" PRIu32 " import operations are defined",
                         ext.total_import_ops);
    }

    if (ext.export_info) {
        if (ext.export_info_nv) {
            skip |= LogError("VUID-VkMemoryAllocateInfo-pNext-00640",
                             device,
                             allocate_info_loc,
    ...
    if (ext.import_info_win32 && ext.import_info_win32_nv) {
        skip |= LogError("VUID-VkMemoryAllocateInfo-pNext-00641",
                         device,
                         allocate_info_loc,
    ...
    if (ext.import_info_fd && ext.import_info_fd->handleType != 0) {
        if (ext.import_info_fd->fd < 0) {
            skip |= LogError("VUID-VkImportMemoryFdInfoKHR-handleType-00670",
                             device,
                             allocate_info_loc.pNext(Struct::VkImportMemoryFdInfoKHR, Field::fd),
                             "(%d) is not a valid POSIX file descriptor.",
                             ext.import_info_fd->fd);
        }
        if (ext.import_info_fd->handleType != VK_EXTERNAL_MEMORY_HANDLE_TYPE_OPAQUE_FD_BIT &&
            ...
            skip |= LogError("VUID-VkImportMemoryFdInfoKHR-handleType-00669",
                             device,
    ...
    if (ext.import_info_host_pointer && ext.import_info_host_pointer->handleType != 0) {
        if (ext.import_info_host_pointer->handleType != VK_EXTERNAL_MEMORY_HANDLE_TYPE_HOST_ALLOCATION_BIT_EXT &&
            ext.import_info_host_pointer->handleType !=
                VK_EXTERNAL_MEMORY_HANDLE_TYPE_HOST_MAPPED_FOREIGN_MEMORY_BIT_EXT) {
            skip |= LogError("VUID-VkImportMemoryHostPointerInfoEXT-handleType-01748",
                             device,
                             allocate_info_loc.pNext(Struct::VkImportMemoryHostPointerInfoEXT, Field::handleType),
                             "is %s.",
        ...
        if (SafeModulo(host_pointer, phys_dev_ext_props.external_memory_host_props.minImportedHostPointerAlignment) !=
            0) {
            skip |= LogError("VUID-VkImportMemoryHostPointerInfoEXT-pHostPointer-01749",
                             device,
        ...
        if (SafeModulo(allocate_info.allocationSize,
            ...
            skip |=
                LogError("VUID-VkMemoryAllocateInfo-allocationSize-01745",
                         device,
                         allocate_info_loc.dot(Field::allocationSize),
        ...
        if (dedicated_allocate_info) {
            if (dedicated_allocate_info->buffer != VK_NULL_HANDLE) {
                skip |= LogError("VUID-VkMemoryAllocateInfo-pNext-02806",
                                 device,
