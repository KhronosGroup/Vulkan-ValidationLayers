...
bool StatelessValidation::manual_PreCallValidateCreateShaderModule(VkDevice device,
    ...
    skip |= ValidateStructPnext(error_obj.location.dot(Field::pCreateInfo),
                                pCreateInfo->pNext,
                                allowed_structs.size(),
                                allowed_structs.data(),
...
bool StatelessValidation::manual_PreCallValidateCreatePipelineLayout(VkDevice device,
    ...
    if (pCreateInfo->setLayoutCount > device_limits.maxBoundDescriptorSets) {
        skip |= LogError("VUID-VkPipelineLayoutCreateInfo-setLayoutCount-00286",
                         device,
                         create_info_loc.dot(Field::setLayoutCount),
    ...
    skip |= ValidatePushConstantRange(
        pCreateInfo->pushConstantRangeCount, pCreateInfo->pPushConstantRanges, create_info_loc);
...
bool StatelessValidation::ValidatePushConstantRange(uint32_t push_constant_range_count,
    ...
    for (uint32_t i = 0; i < push_constant_range_count; ++i) {
        ...
        if (offset >= max_push_constants_size) {
            skip |= LogError("VUID-VkPushConstantRange-offset-00294",
                             device,
                             pc_loc.dot(Field::offset),
        ...
        if (size > max_push_constants_size - offset) {
            skip |=
                LogError("VUID-VkPushConstantRange-size-00298",
                         device,
                         pc_loc.dot(Field::size),
                         "(%" PRIu32 ") plus offset (%" PRIu32 ") is greater than maxPushConstantSize (%" PRIu32 ").",
                         size,
                         offset,
                         max_push_constants_size);
        }
        ...
        if (size == 0) {
            skip |= LogError("VUID-VkPushConstantRange-size-00296",
                             device,
                             pc_loc.dot(Field::size),
                             "(%" PRIu32 ") is not greater than zero.",
                             size);
        }
        if (size & 0x3) {
            skip |= LogError("VUID-VkPushConstantRange-size-00297",
                             device,
                             pc_loc.dot(Field::size),
        ...
        if ((offset & 0x3) != 0) {
            skip |= LogError("VUID-VkPushConstantRange-offset-00295",
                             device,
                             pc_loc.dot(Field::offset),
                             "(%" PRIu32 ") is not a multiple of 4.",
                             offset);
    ...
    // As of 1.0.28, there is a VU that states that a stage flag cannot appear more than once in the list of push
    // constant ranges.
    for (uint32_t i = 0; i < push_constant_range_count; ++i) {
        for (uint32_t j = i + 1; j < push_constant_range_count; ++j) {
            if (0 != (push_constant_ranges[i].stageFlags & push_constant_ranges[j].stageFlags)) {
                ...
                skip |= LogError(vuid,
                                 ...
                                 i,
                                 string_VkShaderStageFlags(push_constant_ranges[i].stageFlags).c_str(),
                                 j);
...
bool StatelessValidation::ValidatePipelineShaderStageCreateInfoCommon(
    const VkPipelineShaderStageCreateInfo& create_info, const Location& loc) const {
    bool skip = false;

    if (create_info.pName) {
        skip |= ValidateString(
            loc.dot(Field::pName), "VUID-VkPipelineShaderStageCreateInfo-pName-parameter", create_info.pName);
    }
    ...
    if (vku::FindStructInPNextChain<VkPipelineShaderStageRequiredSubgroupSizeCreateInfo>(create_info.pNext)) {
        if ((create_info.flags & VK_PIPELINE_SHADER_STAGE_CREATE_ALLOW_VARYING_SUBGROUP_SIZE_BIT) != 0) {
            skip |= LogError("VUID-VkPipelineShaderStageCreateInfo-pNext-02754",
                             device,
                             loc.dot(Field::flags),
...
// Called from graphics, compute, raytracing, etc
bool StatelessValidation::ValidatePipelineBinaryInfo(const void* next,
    ...
    if (temp_flags_2 && (create_flags_2 & VK_PIPELINE_CREATE_2_CAPTURE_DATA_BIT_KHR) &&
        (pipelineCache != VK_NULL_HANDLE)) {
        skip |= LogError(GetPipelineBinaryInfoVUID(flag_loc, vvl::PipelineBinaryInfoError::PNext_09617),
                         device,
                         flag_loc,
    ...
    if (binary_info && (binary_info->binaryCount > 0)) {
        if (pipelineCache != VK_NULL_HANDLE) {
            skip |= LogError(GetPipelineBinaryInfoVUID(flag_loc, vvl::PipelineBinaryInfoError::PNext_09616),
                             device,
        ...
        if (creation_feedback) {
            if (creation_feedback->pPipelineCreationFeedback->flags &
                ...
                skip |= LogError(
                    GetPipelineBinaryInfoVUID(flag_loc, vvl::PipelineBinaryInfoError::BinaryCount_09620),
                    device,
                    loc.pNext(Struct::VkPipelineCreationFeedbackCreateInfo, Field::pPipelineCreationFeedback)
                        .dot(Field::flags),
            ...
            if (creation_feedback->pPipelineCreationFeedback->flags &
                ...
                skip |= LogError(
                    GetPipelineBinaryInfoVUID(flag_loc, vvl::PipelineBinaryInfoError::BinaryCount_09621),
                    device,
                    loc.pNext(Struct::VkPipelineCreationFeedbackCreateInfo, Field::pPipelineCreationFeedback)
                        .dot(Field::flags),
        ...
        if (create_flags_2 & VK_PIPELINE_CREATE_FAIL_ON_PIPELINE_COMPILE_REQUIRED_BIT) {
            skip |= LogError(GetPipelineBinaryInfoVUID(flag_loc, vvl::PipelineBinaryInfoError::BinaryCount_09622),
                             device,
                             flag_loc,
...
bool StatelessValidation::ValidatePipelineRenderingCreateInfo(const VkPipelineRenderingCreateInfo& rendering_struct,
    ...
    if ((rendering_struct.depthAttachmentFormat != VK_FORMAT_UNDEFINED)) {
        skip |= ValidateRangedEnum(loc.pNext(Struct::VkPipelineRenderingCreateInfo, Field::depthAttachmentFormat),
                                   vvl::Enum::VkFormat,
                                   rendering_struct.depthAttachmentFormat,
                                   "VUID-VkGraphicsPipelineCreateInfo-renderPass-06583");

        if (!vkuFormatHasDepth(rendering_struct.depthAttachmentFormat)) {
            skip |= LogError("VUID-VkGraphicsPipelineCreateInfo-renderPass-06587",
                             device,
    ...
    if ((rendering_struct.stencilAttachmentFormat != VK_FORMAT_UNDEFINED)) {
        ...
        if (!vkuFormatHasStencil(rendering_struct.stencilAttachmentFormat)) {
            skip |= LogError("VUID-VkGraphicsPipelineCreateInfo-renderPass-06588",
                             device,
    ...
    if (rendering_struct.colorAttachmentCount != 0) {
        skip |=
            ValidateRangedEnumArray(loc.pNext(Struct::VkPipelineRenderingCreateInfo, Field::colorAttachmentCount),
                                    ...
                                    rendering_struct.pColorAttachmentFormats,
                                    true,
                                    true,
        ...
        if (rendering_struct.colorAttachmentCount > device_limits.maxColorAttachments) {
            skip |= LogError("VUID-VkPipelineRenderingCreateInfo-colorAttachmentCount-09533",
                             device,
    ...
    if (rendering_struct.pColorAttachmentFormats) {
        for (uint32_t j = 0; j < rendering_struct.colorAttachmentCount; ++j) {
            skip |=
                ValidateRangedEnum(loc.pNext(Struct::VkPipelineRenderingCreateInfo, Field::pColorAttachmentFormats, j),
                                   vvl::Enum::VkFormat,
...
bool StatelessValidation::ValidatePipelineRobustnessCreateInfo(
    const VkPipelineRobustnessCreateInfo& robustness_create_info, const Location& loc) const {
    bool skip = false;
    ...
    if (!supported_robustness_2_features.robustBufferAccess2) {
        if (robustness_create_info.storageBuffers == VK_PIPELINE_ROBUSTNESS_BUFFER_BEHAVIOR_ROBUST_BUFFER_ACCESS_2) {
            skip |= LogError("VUID-VkPipelineRobustnessCreateInfo-robustBufferAccess2-06931",
                             device,
                             loc.pNext(Struct::VkPipelineRobustnessCreateInfo, Field::storageBuffers),
                             "is VK_PIPELINE_ROBUSTNESS_BUFFER_BEHAVIOR_ROBUST_BUFFER_ACCESS_2, but "
        ...
        if (robustness_create_info.uniformBuffers == VK_PIPELINE_ROBUSTNESS_BUFFER_BEHAVIOR_ROBUST_BUFFER_ACCESS_2) {
            skip |= LogError("VUID-VkPipelineRobustnessCreateInfo-robustBufferAccess2-06932",
                             device,
        ...
        if (robustness_create_info.vertexInputs == VK_PIPELINE_ROBUSTNESS_BUFFER_BEHAVIOR_ROBUST_BUFFER_ACCESS_2) {
            skip |= LogError("VUID-VkPipelineRobustnessCreateInfo-robustBufferAccess2-06933",
                             device,
    ...
    if (!supported_robustness_2_features.robustImageAccess2) {
        if (robustness_create_info.images == VK_PIPELINE_ROBUSTNESS_IMAGE_BEHAVIOR_ROBUST_IMAGE_ACCESS_2) {
            skip |= LogError("VUID-VkPipelineRobustnessCreateInfo-robustImageAccess2-06934",
                             device,
                             loc.pNext(Struct::VkPipelineRobustnessCreateInfo, Field::images),
                             "is VK_PIPELINE_ROBUSTNESS_IMAGE_BEHAVIOR_ROBUST_IMAGE_ACCESS_2, but robustImageAccess2 "
                             "is not supported.");
        }
    }
...
bool StatelessValidation::ValidateCreateGraphicsPipelinesFlags(const VkPipelineCreateFlags2KHR flags,
    ...
    if ((flags & VK_PIPELINE_CREATE_RETAIN_LINK_TIME_OPTIMIZATION_INFO_BIT_EXT) != 0 &&
        ...
        skip |= LogError("VUID-VkGraphicsPipelineCreateInfo-flags-09245",
                         device,
                         flags_loc,
                         "is (%s).",
                         string_VkPipelineCreateFlags2(flags).c_str());
    ...
    if ((flags & VK_PIPELINE_CREATE_INDIRECT_BINDABLE_BIT_NV) != 0) {
        if (!enabled_features.deviceGeneratedCommands) {
            skip |= LogError("VUID-VkGraphicsPipelineCreateInfo-flags-02877",
                             device,
                             flags_loc,
                             "(%s) contains VK_PIPELINE_CREATE_INDIRECT_BINDABLE_BIT_NV, but deviceGeneratedCommands "
                             "was not enabled.",
                             string_VkPipelineCreateFlags2(flags).c_str());
        }
    }
    ...
    if ((flags & VK_PIPELINE_CREATE_2_INDIRECT_BINDABLE_BIT_EXT) != 0) {
        if (!enabled_features.deviceGeneratedCommands) {
            skip |=
                LogError("VUID-VkGraphicsPipelineCreateInfo-flags-11000",
                         device,
                         flags_loc,
    ...
    if ((flags & VK_PIPELINE_CREATE_DISPATCH_BASE) != 0) {
        skip |= LogError("VUID-VkGraphicsPipelineCreateInfo-flags-00764",
                         device,
                         flags_loc,
                         "(%s) must not include "
    ...
    if ((flags & VK_PIPELINE_CREATE_RAY_TRACING_NO_NULL_ANY_HIT_SHADERS_BIT_KHR) != 0) {
        skip |= LogError("VUID-VkGraphicsPipelineCreateInfo-flags-03372",
                         device,
                         flags_loc,
    ...
    if ((flags & VK_PIPELINE_CREATE_RAY_TRACING_NO_NULL_CLOSEST_HIT_SHADERS_BIT_KHR) != 0) {
        skip |= LogError("VUID-VkGraphicsPipelineCreateInfo-flags-03373",
                         device,
                         flags_loc,
    ...
    if ((flags & VK_PIPELINE_CREATE_RAY_TRACING_NO_NULL_MISS_SHADERS_BIT_KHR) != 0) {
        skip |= LogError("VUID-VkGraphicsPipelineCreateInfo-flags-03374",
                         device,
                         flags_loc,
    ...
    if ((flags & VK_PIPELINE_CREATE_RAY_TRACING_NO_NULL_INTERSECTION_SHADERS_BIT_KHR) != 0) {
        skip |= LogError("VUID-VkGraphicsPipelineCreateInfo-flags-03375",
                         device,
                         flags_loc,
    ...
    if ((flags & VK_PIPELINE_CREATE_RAY_TRACING_SKIP_TRIANGLES_BIT_KHR) != 0) {
        skip |= LogError("VUID-VkGraphicsPipelineCreateInfo-flags-03376",
                         device,
                         flags_loc,
    ...
    if ((flags & VK_PIPELINE_CREATE_RAY_TRACING_SKIP_AABBS_BIT_KHR) != 0) {
        skip |= LogError("VUID-VkGraphicsPipelineCreateInfo-flags-03377",
                         device,
                         flags_loc,
    ...
    if ((flags & VK_PIPELINE_CREATE_RAY_TRACING_SHADER_GROUP_HANDLE_CAPTURE_REPLAY_BIT_KHR) != 0) {
        skip |=
            LogError("VUID-VkGraphicsPipelineCreateInfo-flags-03577",
                     device,
                     flags_loc,
    ...
    if ((flags & VK_PIPELINE_CREATE_RAY_TRACING_ALLOW_MOTION_BIT_NV) != 0) {
        skip |= LogError("VUID-VkGraphicsPipelineCreateInfo-flags-04947",
                         device,
                         flags_loc,
    ...
    if ((flags & VK_PIPELINE_CREATE_RAY_TRACING_OPACITY_MICROMAP_BIT_EXT) != 0) {
        skip |= LogError("VUID-VkGraphicsPipelineCreateInfo-flags-07401",
                         device,
                         flags_loc,
    ...
    if ((flags & VK_PIPELINE_CREATE_RAY_TRACING_DISPLACEMENT_MICROMAP_BIT_NV) != 0) {
        skip |= LogError("VUID-VkGraphicsPipelineCreateInfo-flags-07997",
                         device,
                         flags_loc,
...
bool StatelessValidation::manual_PreCallValidateCreateGraphicsPipelines(
    ...
    for (uint32_t i = 0; i < createInfoCount; ++i) {
        ...
        const auto* graphics_lib_info =
            vku::FindStructInPNextChain<VkGraphicsPipelineLibraryCreateInfoEXT>(create_info.pNext);
        ...
        if (!create_info.renderPass) {
            ...
            if ((flags & VK_PIPELINE_CREATE_LIBRARY_BIT_KHR) == 0 ||
                ...
                auto attachment_sample_count_info =
                    vku::FindStructInPNextChain<VkAttachmentSampleCountInfoAMD>(create_info.pNext);
                if (attachment_sample_count_info && attachment_sample_count_info->pColorAttachmentSamples) {
                    for (uint32_t j = 0; j < attachment_sample_count_info->colorAttachmentCount; ++j) {
                        skip |= ValidateFlags(create_info_loc.pNext(Struct::VkAttachmentSampleCountInfoAMD,
                                                                    Field::pColorAttachmentSamples),
                                              vvl::FlagBitmask::VkSampleCountFlagBits,
                                              AllVkSampleCountFlagBits,
                                              attachment_sample_count_info->pColorAttachmentSamples[j],
                                              kRequiredFlags,
                                              VK_NULL_HANDLE,
        ...
        if (!IsExtEnabled(device_extensions.vk_ext_graphics_pipeline_library)) {
            if (create_info.stageCount == 0) {
                ...
                skip |= LogError("VUID-VkGraphicsPipelineCreateInfo-stageCount-09530",
                                 device,
                                 create_info_loc.dot(Field::stageCount),
                                 "is 0, but %s is not enabled",
                                 VK_EXT_GRAPHICS_PIPELINE_LIBRARY_EXTENSION_NAME);
            }

            skip |= ValidateStructTypeArray(create_info_loc.dot(Field::stageCount),
        ...
        if (graphics_lib_info &&
            (graphics_lib_info->flags & (VK_GRAPHICS_PIPELINE_LIBRARY_PRE_RASTERIZATION_SHADERS_BIT_EXT))) {
            skip |= ValidateArray(create_info_loc.dot(Field::stageCount),
                                  create_info_loc.dot(Field::pStages),
                                  create_info.stageCount,
                                  &create_info.pStages,
                                  true,
        ...
        // TODO probably should check dynamic state from graphics libraries, at least when creating an "executable
        // pipeline"
        if (create_info.pDynamicState != nullptr) {
            ...
            for (uint32_t state_index = 0; state_index < dynamic_state_info.dynamicStateCount; ++state_index) {
                ...
                if (vvl::Contains(dynamic_state_map, dynamic_state)) {
                    skip |= LogError("VUID-VkPipelineDynamicStateCreateInfo-pDynamicStates-01442",
                                     device,
                                     create_info_loc.dot(Field::pDynamicState),
                                     "has %s at both pDynamicStates[%" PRIu32 "] and pDynamicStates[%" PRIu32 "].",
                                     string_VkDynamicState(dynamic_state),
                                     dynamic_state_map[dynamic_state],
                                     state_index);
                }
        ...
        if (vvl::Contains(dynamic_state_map, VK_DYNAMIC_STATE_RAY_TRACING_PIPELINE_STACK_SIZE_KHR)) {
            ...
            skip |= LogError("VUID-VkGraphicsPipelineCreateInfo-pDynamicStates-03578",
                             device,
                             create_info_loc.dot(Field::pDynamicState)
                                 .dot(Field::pDynamicStates,
                                      dynamic_state_map[VK_DYNAMIC_STATE_RAY_TRACING_PIPELINE_STACK_SIZE_KHR]),
        ...
        if (has_pre_raster_state && vvl::Contains(dynamic_state_map, VK_DYNAMIC_STATE_VIEWPORT_WITH_COUNT) &&
            ...
            skip |= LogError("VUID-VkGraphicsPipelineCreateInfo-pDynamicStates-04132",
                             device,
                             create_info_loc,
                             "pDynamicStates[%" PRIu32
                             "] is VK_DYNAMIC_STATE_VIEWPORT_WITH_COUNT but pDynamicStates[%" PRIu32
                             "] is VK_DYNAMIC_STATE_VIEWPORT.",
                             dynamic_state_map[VK_DYNAMIC_STATE_VIEWPORT_WITH_COUNT],
        ...
        if (has_pre_raster_state && vvl::Contains(dynamic_state_map, VK_DYNAMIC_STATE_SCISSOR_WITH_COUNT) &&
            ...
            skip |= LogError("VUID-VkGraphicsPipelineCreateInfo-pDynamicStates-04133",
                             device,
                             create_info_loc,
                             "pDynamicStates[%" PRIu32
                             "] is VK_DYNAMIC_STATE_SCISSOR_WITH_COUNT but pDynamicStates[%" PRIu32
                             "] is VK_DYNAMIC_STATE_SCISSOR.",
                             dynamic_state_map[VK_DYNAMIC_STATE_SCISSOR_WITH_COUNT],
                             dynamic_state_map[VK_DYNAMIC_STATE_SCISSOR]);
        }
        ...
        if (vvl::Contains(dynamic_state_map, VK_DYNAMIC_STATE_DISCARD_RECTANGLE_ENABLE_EXT) &&
            ...
            skip |= LogError(
                ...
                "is VK_DYNAMIC_STATE_DISCARD_RECTANGLE_ENABLE_EXT without support for version 2 of "
        ...
        if (vvl::Contains(dynamic_state_map, VK_DYNAMIC_STATE_DISCARD_RECTANGLE_MODE_EXT) &&
            ...
            skip |= LogError(
                ...
                "is VK_DYNAMIC_STATE_DISCARD_RECTANGLE_MODE_EXT without support for version 2 of "
                "VK_EXT_discard_rectangles.");
        }
        if (vvl::Contains(dynamic_state_map, VK_DYNAMIC_STATE_EXCLUSIVE_SCISSOR_ENABLE_NV) &&
            ...
            skip |= LogError(
                ...
                "is VK_DYNAMIC_STATE_EXCLUSIVE_SCISSOR_ENABLE_NV without support for version 2 of "
        ...
        if ((feedback_struct != nullptr) &&
            (feedback_struct->pipelineStageCreationFeedbackCount != 0 &&
        ...
        const bool has_dynamic_viewport_with_count =
            vvl::Contains(dynamic_state_map, VK_DYNAMIC_STATE_VIEWPORT_WITH_COUNT);
        const bool has_dynamic_scissor_with_count =
            vvl::Contains(dynamic_state_map, VK_DYNAMIC_STATE_SCISSOR_WITH_COUNT);
        ...
        if (create_info.pStages != nullptr) {
            for (uint32_t stage_index = 0; stage_index < create_info.stageCount; ++stage_index) {
                ...
                skip |= ValidateStructType(stage_loc,
        ...
        if (has_pre_raster_state && (active_shaders & VK_SHADER_STAGE_TESSELLATION_CONTROL_BIT) &&
            ...
            if (create_info.pTessellationState) {
                ...
                if (!has_dynamic_patch_control_points &&
                    ...
                    skip |=
                        LogError("VUID-VkPipelineTessellationStateCreateInfo-patchControlPoints-01214",
                                 device,
        ...
        if (!has_dynamic_vertex_input && !(active_shaders & VK_SHADER_STAGE_MESH_BIT_EXT) &&
            ...
            if (vertex_input_state->vertexBindingDescriptionCount > device_limits.maxVertexInputBindings) {
                skip |= LogError("VUID-VkPipelineVertexInputStateCreateInfo-vertexBindingDescriptionCount-00613",
                                 device,
            ...
            if (vertex_input_state->vertexAttributeDescriptionCount > device_limits.maxVertexInputAttributes) {
                skip |= LogError("VUID-VkPipelineVertexInputStateCreateInfo-vertexAttributeDescriptionCount-00614",
                                 device,
            ...
            const bool has_dynamic_binding_stride =
                vvl::Contains(dynamic_state_map, VK_DYNAMIC_STATE_VERTEX_INPUT_BINDING_STRIDE);
            ...
            for (uint32_t d = 0; d < vertex_input_state->vertexBindingDescriptionCount; ++d) {
                ...
                if (binding_it != vertex_bindings.cend()) {
                    skip |= LogError("VUID-VkPipelineVertexInputStateCreateInfo-pVertexBindingDescriptions-00616",
                                     device,
                                     binding_loc.dot(Field::binding),
                                     "(%" PRIu32 ") is already in pVertexBindingDescription[%" PRIu32 "].",
                                     vertex_bind_desc.binding,
                                     *binding_it);
                }
            ...
            for (uint32_t d = 0; d < vertex_input_state->vertexAttributeDescriptionCount; ++d) {
                ...
                if (location_it != attribute_locations.cend()) {
                    skip |= LogError("VUID-VkPipelineVertexInputStateCreateInfo-pVertexAttributeDescriptions-00617",
                                     device,
                                     attribute_loc.dot(Field::location),
                                     "(%" PRIu32 ") is already in pVertexAttributeDescriptions[%" PRIu32 "].",
                                     vertex_attrib_desc.location,
                                     *location_it);
                }
                ...
                if (binding_it == vertex_bindings.cend()) {
                    skip |= LogError("VUID-VkPipelineVertexInputStateCreateInfo-binding-00615",
                                     device,
                                     attribute_loc.dot(Field::binding),
                                     "(%" PRIu32 ") does not exist in pVertexBindingDescription.",
                                     vertex_attrib_desc.binding);
                }

                if (vertex_attrib_desc.location >= device_limits.maxVertexInputAttributes) {
                    skip |= LogError("VUID-VkVertexInputAttributeDescription-location-00620",
                                     device,
                                     attribute_loc.dot(Field::location),
                ...
                if (vertex_attrib_desc.binding >= device_limits.maxVertexInputBindings) {
                    skip |= LogError("VUID-VkVertexInputAttributeDescription-binding-00621",
                                     device,
                                     attribute_loc.dot(Field::binding),
                ...
                if (vertex_attrib_desc.offset > device_limits.maxVertexInputAttributeOffset) {
                    skip |= LogError("VUID-VkVertexInputAttributeDescription-offset-00622",
                                     device,
                                     attribute_loc.dot(Field::offset),
                                     "(%" PRIu32 ") is larger than maxVertexInputAttributeOffset (%" PRIu32 ").",
                                     vertex_attrib_desc.offset,
                ...
                if (vkuFormatIsDepthOrStencil(vertex_attrib_desc.format)) {
                    ...
                    skip |= LogError(
                        "UNASSIGNED-VkVertexInputAttributeDescription-depthStencil-format",
                        device,
                ...
                if ((properties.bufferFeatures & VK_FORMAT_FEATURE_VERTEX_BUFFER_BIT) == 0) {
                    skip |= LogError("VUID-VkVertexInputAttributeDescription-format-00623",
                                     device,
                                     attribute_loc.dot(Field::format),
        ...
        // pViewportState, pMultisampleState, pDepthStencilState, and pColorBlendState ignored when rasterization is
        // disabled
        if ((create_info.pRasterizationState != nullptr) &&
            ...
            if (create_info.pViewportState) {
                ...
                const auto* exclusive_scissor_struct =
                    vku::FindStructInPNextChain<VkPipelineViewportExclusiveScissorStateCreateInfoNV>(
                        viewport_state.pNext);
                const auto* shading_rate_image_struct =
                    vku::FindStructInPNextChain<VkPipelineViewportShadingRateImageStateCreateInfoNV>(
                        viewport_state.pNext);
                const auto* coarse_sample_order_struct =
                    vku::FindStructInPNextChain<VkPipelineViewportCoarseSampleOrderStateCreateInfoNV>(
                        viewport_state.pNext);
                ...
                if (!enabled_features.multiViewport) {
                    ...
                    if (shading_rate_image_struct && (shading_rate_image_struct->viewportCount != 0 &&
                                                      shading_rate_image_struct->viewportCount != 1)) {
                        skip |= LogError("VUID-VkPipelineViewportShadingRateImageStateCreateInfoNV-viewportCount-02054",
                                         device,
                ...
                if (viewport_state.viewportCount > device_limits.maxViewports) {
                    skip |= LogError("VUID-VkPipelineViewportStateCreateInfo-viewportCount-01218",
                                     device,
                ...
                if (has_dynamic_viewport_with_count) {
                    if (viewport_state.viewportCount != 0) {
                        skip |=
                            LogError("VUID-VkGraphicsPipelineCreateInfo-pDynamicStates-03379",
                                     device,
                                     viewport_loc.dot(Field::viewportCount),
                    ...
                    if (viewport_state.viewportCount == 0) {
                        skip |= LogError("VUID-VkPipelineViewportStateCreateInfo-viewportCount-04135",
                                         device,
                                         viewport_loc.dot(Field::viewportCount),
                    ...
                    if (!enabled_features.multiViewport && (viewport_state.viewportCount > 1)) {
                        skip |= LogError("VUID-VkPipelineViewportStateCreateInfo-viewportCount-01216",
                                         device,
                                         viewport_loc.dot(Field::viewportCount),
                                         "is %" PRIu32 " but multiViewport feature is not enabled.",
                                         viewport_state.viewportCount);
                ...
                if (viewport_state.scissorCount > device_limits.maxViewports) {
                    skip |= LogError("VUID-VkPipelineViewportStateCreateInfo-scissorCount-01219",
                                     device,
                ...
                if (has_dynamic_scissor_with_count) {
                    if (viewport_state.scissorCount != 0) {
                        skip |= LogError("VUID-VkGraphicsPipelineCreateInfo-pDynamicStates-03380",
                                         device,
                                         viewport_loc.dot(Field::scissorCount),
                    ...
                    if (viewport_state.scissorCount == 0) {
                        skip |= LogError("VUID-VkPipelineViewportStateCreateInfo-scissorCount-04136",
                                         device,
                                         viewport_loc.dot(Field::scissorCount),
                    ...
                    if (!enabled_features.multiViewport && (viewport_state.scissorCount > 1)) {
                        skip |= LogError("VUID-VkPipelineViewportStateCreateInfo-scissorCount-01217",
                                         device,
                                         viewport_loc.dot(Field::scissorCount),
                                         "is %" PRIu32 " but multiViewport feature is not enabled.",
                                         viewport_state.scissorCount);
                ...
                if (!has_dynamic_scissor && viewport_state.pScissors) {
                    for (uint32_t scissor_i = 0; scissor_i < viewport_state.scissorCount; ++scissor_i) {
                        ...
                        if (scissor.offset.x < 0) {
                            skip |= LogError("VUID-VkPipelineViewportStateCreateInfo-x-02821",
                                             device,
                        ...
                        if (x_sum > std::numeric_limits<int32_t>::max()) {
                            skip |= LogError("VUID-VkPipelineViewportStateCreateInfo-offset-02822",
                                             device,
                                             scissor_loc,
                        ...
                        if (y_sum > std::numeric_limits<int32_t>::max()) {
                            skip |= LogError("VUID-VkPipelineViewportStateCreateInfo-offset-02823",
                                             device,
                                             scissor_loc,
                ...
                if (exclusive_scissor_struct &&
                    ...
                    skip |=
                        LogError("VUID-VkPipelineViewportExclusiveScissorStateCreateInfoNV-exclusiveScissorCount-02028",
                                 device,
                                 viewport_loc.pNext(Struct::VkPipelineViewportExclusiveScissorStateCreateInfoNV,
                                                    Field::exclusiveScissorCount),
                                 "(%" PRIu32 ") is larger than maxViewports (%" PRIu32 ").",
                ...
                if (shading_rate_image_struct &&
                    ...
                    skip |= LogError("VUID-VkPipelineViewportShadingRateImageStateCreateInfoNV-viewportCount-02055",
                                     device,
                                     viewport_loc.pNext(Struct::VkPipelineViewportShadingRateImageStateCreateInfoNV,
                                                        Field::viewportCount),
                                     "(%" PRIu32 ") is larger than maxViewports (%" PRIu32 ").",
                ...
                if (viewport_state.scissorCount != viewport_state.viewportCount) {
                    if (!IsExtEnabled(device_extensions.vk_ext_extended_dynamic_state) ||
                        ...
                        skip |= LogError("VUID-VkPipelineViewportStateCreateInfo-scissorCount-04134",
                                         device,
                                         viewport_loc,
                ...
                if (exclusive_scissor_struct && exclusive_scissor_struct->exclusiveScissorCount != 0 &&
                    ...
                    skip |=
                        LogError("VUID-VkPipelineViewportExclusiveScissorStateCreateInfoNV-exclusiveScissorCount-02029",
                                 device,
                                 viewport_loc.pNext(Struct::VkPipelineViewportExclusiveScissorStateCreateInfoNV,
                                                    Field::exclusiveScissorCount),
                ...
                if (shading_rate_image_struct && shading_rate_image_struct->shadingRateImageEnable &&
                    ...
                    skip |= LogError(
                        ...
                        viewport_loc.pNext(Struct::VkPipelineViewportShadingRateImageStateCreateInfoNV,
                                           Field::viewportCount),
                        "(%" PRIu32 ") is greater than viewportCount (%" PRIu32 ").",
                        shading_rate_image_struct->viewportCount,
                        viewport_state.viewportCount);
                }
                ...
                if (!has_dynamic_viewport && !has_dynamic_viewport_with_count && viewport_state.viewportCount > 0 &&
                    ...
                    skip |= LogError("VUID-VkGraphicsPipelineCreateInfo-pDynamicStates-04130",
                                     device,
                                     viewport_loc.dot(Field::pViewports),
                                     "is NULL, but the viewport state is not dynamic.");
                }

                if (!has_dynamic_scissor && !has_dynamic_scissor_with_count && viewport_state.scissorCount > 0 &&
                    viewport_state.pScissors == nullptr) {
                    skip |= LogError("VUID-VkGraphicsPipelineCreateInfo-pDynamicStates-04131",
                                     device,
                                     viewport_loc.dot(Field::pScissors),
                                     "is NULL, but the scissor state is not dynamic.");
                }
                ...
                if (!vvl::Contains(dynamic_state_map, VK_DYNAMIC_STATE_VIEWPORT_SHADING_RATE_PALETTE_NV) &&
                    ...
                    skip |= LogError("VUID-VkGraphicsPipelineCreateInfo-pDynamicStates-04057",
                                     device,
                                     viewport_loc,
                                     "The shading rate palette state is static (pCreateInfos[%" PRIu32
                                     "].pDynamicState->pDynamicStates does not contain "
                ...
                if (vp_swizzle_struct) {
                    ...
                    if (swizzle_viewport_count < viewport_count) {
                        skip |= LogError("VUID-VkPipelineViewportSwizzleStateCreateInfoNV-viewportCount-01215",
                                         device,
                                         viewport_loc.pNext(Struct::VkPipelineViewportSwizzleStateCreateInfoNV,
                                                            Field::viewportCount),
                                         "(%" PRIu32 ") less than viewportCount (%" PRIu32 ").",
                                         swizzle_viewport_count,
                                         viewport_count);
                    }
                }
                ...
                if (!has_dynamic_viewport && viewport_state.pViewports) {
                    for (uint32_t viewport_i = 0; viewport_i < viewport_state.viewportCount; ++viewport_i) {
                        ...
                        skip |= ValidateViewport(
                            viewport, VkCommandBuffer(0), viewport_loc.dot(Field::pViewports, viewport_i));
                    }
                }
                ...
                if (coarse_sample_order_struct) {
                    ...
                    for (uint32_t order_i = 0; order_i < coarse_sample_order_struct->customSampleOrderCount;
                         ++order_i) {
                        skip |= ValidateCoarseSampleOrderCustomNV(
                ...
                if (vp_w_scaling_struct && (vp_w_scaling_struct->viewportWScalingEnable == VK_TRUE)) {
                    if (vp_w_scaling_struct->viewportCount != viewport_state.viewportCount) {
                        skip |= LogError("VUID-VkPipelineViewportStateCreateInfo-viewportWScalingEnable-01726",
                                         device,
                                         viewport_loc.pNext(Struct::VkPipelineViewportWScalingStateCreateInfoNV,
                                                            Field::viewportCount),
                                         "(%" PRIu32 ") is not equal to viewportCount (%" PRIu32 ").",
                                         vp_w_scaling_struct->viewportCount,
                                         viewport_state.viewportCount);
                    }
                    if (!has_dynamic_viewport_w_scaling_nv && !vp_w_scaling_struct->pViewportWScalings) {
                        skip |= LogError("VUID-VkGraphicsPipelineCreateInfo-pDynamicStates-01715",
                                         device,
                                         viewport_loc.pNext(Struct::VkPipelineViewportWScalingStateCreateInfoNV,
                                                            Field::pViewportWScalings),
                                         "is NULL.");
                    }
                }

                const bool has_dynamic_depth_clamp_range =
                    vvl::Contains(dynamic_state_map, VK_DYNAMIC_STATE_DEPTH_CLAMP_RANGE_EXT);
                if (!has_dynamic_depth_clamp_range && depth_clamp_control_struct &&
                    ...
                    if (!depth_clamp_control_struct->pDepthClampRange) {
                        skip |= LogError("VUID-VkPipelineViewportDepthClampControlCreateInfoEXT-pDepthClampRange-09646",
                                         device,
                                         viewport_loc.pNext(Struct::VkPipelineViewportDepthClampControlCreateInfoEXT,
                                                            Field::pDepthClampRange),
                                         "is NULL.");
                    } else {
                ...
                if (depth_clip_control_struct) {
                    if (depth_clip_control_struct->negativeOneToOne && !enabled_features.depthClipControl) {
                        skip |= LogError("VUID-VkPipelineViewportDepthClipControlCreateInfoEXT-negativeOneToOne-06470",
                                         device,
                                         viewport_loc.pNext(Struct::VkPhysicalDeviceDepthClipControlFeaturesEXT,
                                                            Field::negativeOneToOne),
                ...
                skip |=
                    LogError("VUID-VkGraphicsPipelineCreateInfo-rasterizerDiscardEnable-09024",
                             device,
                             create_info_loc,
            ...
            if (create_info.pMultisampleState) {
                ...
                if (create_info.pMultisampleState->sampleShadingEnable == VK_TRUE) {
                    if (!enabled_features.sampleRateShading) {
                        skip |= LogError("VUID-VkPipelineMultisampleStateCreateInfo-sampleShadingEnable-00784",
                                         device,
                    ...
                    if (!IsBetweenInclusive(create_info.pMultisampleState->minSampleShading, 0.F, 1.0F)) {
                        skip |= LogError("VUID-VkPipelineMultisampleStateCreateInfo-minSampleShading-00786",
                                         device,
                                         ms_loc.dot(Field::minSampleShading),
                                         "is %f.",
            ...
            if (create_info.pColorBlendState != nullptr && uses_color_attachment) {
                ...
                if (color_blend_state.logicOpEnable == VK_TRUE) {
                    skip |= ValidateRangedEnum(color_loc.dot(Field::logicOp),
                                               vvl::Enum::VkLogicOp,
                ...
                const bool dynamic_not_set =
                    (!vvl::Contains(dynamic_state_map, VK_DYNAMIC_STATE_COLOR_BLEND_ADVANCED_EXT) ||
                ...
                if ((color_blend_state.attachmentCount > 0) && dynamic_not_set) {
                    skip |= ValidateArray(color_loc.dot(Field::attachmentCount),
                                          color_loc.dot(Field::pAttachments),
                ...
                auto color_write =
                    vku::FindStructInPNextChain<VkPipelineColorWriteCreateInfoEXT>(color_blend_state.pNext);
                if (color_write && (color_write->attachmentCount != color_blend_state.attachmentCount) &&
                    dynamic_not_set) {
                    skip |= LogError("VUID-VkPipelineColorWriteCreateInfoEXT-attachmentCount-07608",
                                     device,
        ...
        if (create_info.pRasterizationState) {
            ...
            if (!IsExtEnabled(device_extensions.vk_nv_fill_rectangle)) {
                if (create_info.pRasterizationState->polygonMode == VK_POLYGON_MODE_FILL_RECTANGLE_NV) {
                    skip |= LogError("VUID-VkPipelineRasterizationStateCreateInfo-polygonMode-01414",
                                     device,
            ...
            if ((create_info.pRasterizationState->polygonMode != VK_POLYGON_MODE_FILL) &&
                ...
                skip |= LogError("VUID-VkPipelineRasterizationStateCreateInfo-polygonMode-01507",
                                 device,
                                 rasterization_loc.dot(Field::polygonMode),
                                 "is %s, but fillModeNonSolid feature is not enabled.",
            ...
            if (!vvl::Contains(dynamic_state_map, VK_DYNAMIC_STATE_LINE_WIDTH) && !enabled_features.wideLines &&
                ...
                skip |= LogError("VUID-VkGraphicsPipelineCreateInfo-pDynamicStates-00749",
                                 device,
            ...
            const auto* line_state = vku::FindStructInPNextChain<VkPipelineRasterizationLineStateCreateInfo>(
                create_info.pRasterizationState->pNext);
            ...
            if (line_state) {
                if (line_state->stippledLineEnable && !dynamic_line_stipple_enable) {
                    if (line_state->lineStippleFactor < 1 || line_state->lineStippleFactor > 256) {
                        skip |= LogError("VUID-VkGraphicsPipelineCreateInfo-stippledLineEnable-02767",
                                         device,
                                         rasterization_loc.pNext(Struct::VkPipelineRasterizationLineStateCreateInfo,
                                                                 Field::lineStippleFactor),
                                         "is %" PRIu32 ".",
                                         line_state->lineStippleFactor);
                    }
                }
                ...
                if (!dynamic_line_raster_mode) {
                    if (create_info.pMultisampleState &&
                        ...
                        if (create_info.pMultisampleState->alphaToCoverageEnable) {
                            skip |= LogError("VUID-VkGraphicsPipelineCreateInfo-lineRasterizationMode-02766",
                                             device,
                                             rasterization_loc.pNext(Struct::VkPipelineRasterizationLineStateCreateInfo,
                                                                     Field::lineStippleFactor),
                                             "is %s, but pCreateInfos[%" PRIu32
                                             "].pMultisampleState->alphaToCoverageEnable is VK_TRUE.",
                                             string_VkLineRasterizationMode(line_state->lineRasterizationMode),
                                             i);
                        }
                        if (create_info.pMultisampleState->alphaToOneEnable) {
                            skip |= LogError("VUID-VkGraphicsPipelineCreateInfo-lineRasterizationMode-02766",
                                             ...
                                             rasterization_loc.pNext(Struct::VkPipelineRasterizationLineStateCreateInfo,
                                                                     Field::lineStippleFactor),
                                             "is %s, but pCreateInfos[%" PRIu32
                                             "].pMultisampleState->alphaToOneEnable is VK_TRUE.",
                                             string_VkLineRasterizationMode(line_state->lineRasterizationMode),
                                             i);
                        }
                        if (create_info.pMultisampleState->sampleShadingEnable) {
                            skip |= LogError("VUID-VkGraphicsPipelineCreateInfo-lineRasterizationMode-02766",
                                             ...
                                             rasterization_loc.pNext(Struct::VkPipelineRasterizationLineStateCreateInfo,
                                                                     Field::lineStippleFactor),
                                             "is %s, but pCreateInfos[%" PRIu32
                                             "].pMultisampleState->sampleShadingEnable is VK_TRUE.",
                                             string_VkLineRasterizationMode(line_state->lineRasterizationMode),
                                             i);
                        }
                    }
                    ...
                    if (line_state->lineRasterizationMode == VK_LINE_RASTERIZATION_MODE_RECTANGULAR &&
                        ...
                        skip |= LogError("VUID-VkPipelineRasterizationLineStateCreateInfo-lineRasterizationMode-02768",
                                         device,
                                         rasterization_loc.pNext(Struct::VkPhysicalDeviceLineRasterizationFeatures,
                                                                 Field::lineRasterizationMode),
                                         "is VK_LINE_RASTERIZATION_MODE_RECTANGULAR but the rectangularLines feature "
                                         "was not enabled.");
                    }
                    ...
                    if (line_state->stippledLineEnable && !dynamic_line_stipple_enable) {
                        ...
                        if (line_state->lineRasterizationMode == VK_LINE_RASTERIZATION_MODE_RECTANGULAR_SMOOTH &&
                            ...
                            skip |=
                                LogError("VUID-VkPipelineRasterizationLineStateCreateInfo-stippledLineEnable-02773",
                                         device,
                                         rasterization_loc.pNext(Struct::VkPhysicalDeviceLineRasterizationFeatures,
                                                                 Field::lineRasterizationMode),
        ...
        if (flags & VK_PIPELINE_CREATE_DERIVATIVE_BIT) {
            if (create_info.basePipelineHandle != VK_NULL_HANDLE) {
                if (create_info.basePipelineIndex != -1) {
                    skip |= LogError("VUID-VkGraphicsPipelineCreateInfo-flags-07986",
                                     device,
                                     create_info_loc.dot(Field::basePipelineIndex),
                                     "%" PRIu32 " and basePipelineHandle is not VK_NULL_HANDLE.",
                                     create_info.basePipelineIndex);
                }
            } else if (static_cast<uint32_t>(create_info.basePipelineIndex) >= createInfoCount) {
                skip |= LogError("VUID-VkGraphicsPipelineCreateInfo-flags-07985",
                                 device,
                                 create_info_loc.dot(Field::basePipelineIndex),
...
bool StatelessValidation::ValidateCreateComputePipelinesFlags(const VkPipelineCreateFlags2KHR flags,
    ...
    if ((flags & VK_PIPELINE_CREATE_LIBRARY_BIT_KHR) != 0) {
        if (!enabled_features.shaderEnqueue) {
            skip |= LogError("VUID-VkComputePipelineCreateInfo-shaderEnqueue-09177",
                             device,
                             flags_loc,
    ...
    if ((flags & VK_PIPELINE_CREATE_RAY_TRACING_NO_NULL_ANY_HIT_SHADERS_BIT_KHR) != 0) {
        skip |= LogError("VUID-VkComputePipelineCreateInfo-flags-03365",
                         device,
                         flags_loc,
                         "(%s) must not include VK_PIPELINE_CREATE_RAY_TRACING_NO_NULL_ANY_HIT_SHADERS_BIT_KHR.",
                         string_VkPipelineCreateFlags2(flags).c_str());
    ...
    if ((flags & VK_PIPELINE_CREATE_RAY_TRACING_NO_NULL_CLOSEST_HIT_SHADERS_BIT_KHR) != 0) {
        skip |= LogError("VUID-VkComputePipelineCreateInfo-flags-03366",
                         device,
                         flags_loc,
                         "(%s) must not include VK_PIPELINE_CREATE_RAY_TRACING_NO_NULL_CLOSEST_HIT_SHADERS_BIT_KHR.",
                         string_VkPipelineCreateFlags2(flags).c_str());
    ...
    if ((flags & VK_PIPELINE_CREATE_RAY_TRACING_NO_NULL_MISS_SHADERS_BIT_KHR) != 0) {
        skip |= LogError("VUID-VkComputePipelineCreateInfo-flags-03367",
                         device,
                         flags_loc,
                         "(%s) must not include VK_PIPELINE_CREATE_RAY_TRACING_NO_NULL_MISS_SHADERS_BIT_KHR.",
                         string_VkPipelineCreateFlags2(flags).c_str());
    ...
    if ((flags & VK_PIPELINE_CREATE_RAY_TRACING_NO_NULL_INTERSECTION_SHADERS_BIT_KHR) != 0) {
        skip |= LogError("VUID-VkComputePipelineCreateInfo-flags-03368",
                         device,
                         flags_loc,
                         "(%s) must not include VK_PIPELINE_CREATE_RAY_TRACING_NO_NULL_INTERSECTION_SHADERS_BIT_KHR.",
                         string_VkPipelineCreateFlags2(flags).c_str());
    ...
    if ((flags & VK_PIPELINE_CREATE_RAY_TRACING_SKIP_TRIANGLES_BIT_KHR) != 0) {
        skip |= LogError("VUID-VkComputePipelineCreateInfo-flags-03369",
                         device,
                         flags_loc,
                         "(%s) must not include VK_PIPELINE_CREATE_RAY_TRACING_SKIP_TRIANGLES_BIT_KHR.",
                         string_VkPipelineCreateFlags2(flags).c_str());
    ...
    if ((flags & VK_PIPELINE_CREATE_RAY_TRACING_SKIP_AABBS_BIT_KHR) != 0) {
        skip |= LogError("VUID-VkComputePipelineCreateInfo-flags-03370",
                         device,
                         flags_loc,
                         "(%s) must not include VK_PIPELINE_CREATE_RAY_TRACING_SKIP_AABBS_BIT_KHR.",
                         string_VkPipelineCreateFlags2(flags).c_str());
    ...
    if ((flags & VK_PIPELINE_CREATE_RAY_TRACING_SHADER_GROUP_HANDLE_CAPTURE_REPLAY_BIT_KHR) != 0) {
        skip |=
            LogError("VUID-VkComputePipelineCreateInfo-flags-03576",
                     device,
                     flags_loc,
                     "(%s) must not include VK_PIPELINE_CREATE_RAY_TRACING_SHADER_GROUP_HANDLE_CAPTURE_REPLAY_BIT_KHR.",
                     string_VkPipelineCreateFlags2(flags).c_str());
    ...
    if ((flags & VK_PIPELINE_CREATE_RAY_TRACING_ALLOW_MOTION_BIT_NV) != 0) {
        skip |= LogError("VUID-VkComputePipelineCreateInfo-flags-04945",
                         device,
                         flags_loc,
                         "(%s) must not include VK_PIPELINE_CREATE_RAY_TRACING_ALLOW_MOTION_BIT_NV.",
                         string_VkPipelineCreateFlags2(flags).c_str());
    ...
    if ((flags & VK_PIPELINE_CREATE_RAY_TRACING_OPACITY_MICROMAP_BIT_EXT) != 0) {
        skip |= LogError("VUID-VkComputePipelineCreateInfo-flags-07367",
                         device,
                         flags_loc,
                         "(%s) must not include VK_PIPELINE_CREATE_RAY_TRACING_OPACITY_MICROMAP_BIT_EXT.",
                         string_VkPipelineCreateFlags2(flags).c_str());
    ...
    if ((flags & VK_PIPELINE_CREATE_RAY_TRACING_DISPLACEMENT_MICROMAP_BIT_NV) != 0) {
        skip |= LogError("VUID-VkComputePipelineCreateInfo-flags-07996",
                         device,
                         flags_loc,
                         "(%s) must not include VK_PIPELINE_CREATE_RAY_TRACING_DISPLACEMENT_MICROMAP_BIT_NV.",
                         string_VkPipelineCreateFlags2(flags).c_str());
    ...
    if ((flags & VK_PIPELINE_CREATE_2_INDIRECT_BINDABLE_BIT_EXT) != 0) {
        if (!enabled_features.deviceGeneratedCommands) {
            skip |=
                LogError("VUID-VkComputePipelineCreateInfo-flags-11007",
                         device,
                         flags_loc,
                         "(%s) contains VK_PIPELINE_CREATE_2_INDIRECT_BINDABLE_BIT_EXT, but "
                         "VkPhysicalDeviceDeviceGeneratedCommandsFeaturesEXT::deviceGeneratedCommands is not enabled.",
...
bool StatelessValidation::manual_PreCallValidateCreateComputePipelines(VkDevice device,
                                                                       VkPipelineCache pipelineCache,
                                                                       uint32_t createInfoCount,
    ...
    for (uint32_t i = 0; i < createInfoCount; i++) {
        ...
        if (create_info.stage.stage != VK_SHADER_STAGE_COMPUTE_BIT) {
            skip |= LogError("VUID-VkComputePipelineCreateInfo-stage-00701",
                             device,
                             create_info_loc.dot(Field::stage).dot(Field::stage),
                             "is %s.",
        ...
        if (flags & VK_PIPELINE_CREATE_DERIVATIVE_BIT) {
            if (create_info.basePipelineHandle != VK_NULL_HANDLE) {
                if (create_info.basePipelineIndex != -1) {
                    skip |= LogError("VUID-VkComputePipelineCreateInfo-flags-07986",
                                     device,
                                     create_info_loc.dot(Field::basePipelineIndex),
                                     "(%" PRIu32 ") and basePipelineHandle is not VK_NULL_HANDLE.",
                                     create_info.basePipelineIndex);
                ...
                if (static_cast<uint32_t>(create_info.basePipelineIndex) >= createInfoCount) {
                    skip |= LogError("VUID-VkComputePipelineCreateInfo-flags-07985",
                                     device,
                                     create_info_loc.dot(Field::basePipelineIndex),
                                     "(%" PRIu32 ") is greater than or equal to createInfoCount %" PRIu32 ".",
                                     create_info.basePipelineIndex,
...
bool StatelessValidation::ValidateDepthClampRange(const VkDepthClampRangeEXT& depth_clamp_range,
                                                  const Location& loc) const {
    bool skip = false;
    if (depth_clamp_range.minDepthClamp > depth_clamp_range.maxDepthClamp) {
        skip |= LogError("VUID-VkDepthClampRangeEXT-pDepthClampRange-00999",
                         device,
                         loc.dot(Field::minDepthClamp),
                         "(%f) is greater than maxDepthClamp (%f).",
    ...
    if (!IsExtEnabled(device_extensions.vk_ext_depth_range_unrestricted)) {
        if (depth_clamp_range.minDepthClamp < 0.0) {
            skip |= LogError("VUID-VkDepthClampRangeEXT-pDepthClampRange-09648",
                             device,
                             loc.dot(Field::minDepth),
                             "(%f) is below 0.0 (and VK_EXT_depth_range_unrestricted is not enabled).",
                             depth_clamp_range.minDepthClamp);
        }
        if (depth_clamp_range.maxDepthClamp > 1.0) {
            skip |= LogError("VUID-VkDepthClampRangeEXT-pDepthClampRange-09649",
                             device,
                             loc.dot(Field::maxDepthClamp),
...
bool StatelessValidation::manual_PreCallValidateCreatePipelineCache(VkDevice device,
    ...
    if (!enabled_features.pipelineCreationCacheControl && has_externally_sync) {
        skip |=
            LogError("VUID-VkPipelineCacheCreateInfo-pipelineCreationCacheControl-02892",
                     device,
...
bool StatelessValidation::manual_PreCallValidateMergePipelineCaches(VkDevice device,
    ...
    if (pSrcCaches) {
        for (uint32_t index0 = 0; index0 < srcCacheCount; ++index0) {
            if (pSrcCaches[index0] == dstCache) {
                skip |= LogError("VUID-vkMergePipelineCaches-dstCache-00770",
                                 instance,
                                 error_obj.location.dot(Field::dstCache),
                                 "%s is in pSrcCaches list.",
                                 FormatHandle(dstCache).c_str());
                break;
            }
        }
    }
...
bool StatelessValidation::manual_PreCallValidateGetPipelinePropertiesEXT(VkDevice device,
    ...
    if (!enabled_features.pipelinePropertiesIdentifier) {
        skip |= LogError("VUID-vkGetPipelinePropertiesEXT-None-06766",
                         device,
                         error_obj.location,
    ...
    if (pPipelineProperties) {
        if (pPipelineProperties->sType != VK_STRUCTURE_TYPE_PIPELINE_PROPERTIES_IDENTIFIER_EXT) {
            skip |= LogError("VUID-VkPipelinePropertiesIdentifierEXT-sType-sType",
                             device,
                             pipeline_properties_loc.dot(Field::sType),
                             "is not VK_STRUCTURE_TYPE_PIPELINE_PROPERTIES_IDENTIFIER_EXT.");
        }
        if (pPipelineProperties->pNext != nullptr) {
            skip |= LogError("VUID-VkPipelinePropertiesIdentifierEXT-pNext-pNext",
                             device,
                             pipeline_properties_loc.dot(Field::sType),
                             "is not NULL.");
        }
        ...
        skip |= LogError(
            "VUID-vkGetPipelinePropertiesEXT-pPipelineProperties-06739", device, pipeline_properties_loc, "is NULL.");
    }
    return skip;
}
