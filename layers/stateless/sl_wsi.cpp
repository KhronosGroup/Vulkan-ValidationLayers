...
bool StatelessValidation::manual_PreCallValidateAcquireNextImageKHR(VkDevice device,
    ...
    if (semaphore == VK_NULL_HANDLE && fence == VK_NULL_HANDLE) {
        skip |= LogError("VUID-vkAcquireNextImageKHR-semaphore-01780",
                         swapchain,
                         error_obj.location,
...
bool StatelessValidation::manual_PreCallValidateAcquireNextImage2KHR(VkDevice device,
    ...
    if (pAcquireInfo->semaphore == VK_NULL_HANDLE && pAcquireInfo->fence == VK_NULL_HANDLE) {
        skip |= LogError("VUID-VkAcquireNextImageInfoKHR-semaphore-01782",
                         pAcquireInfo->swapchain,
                         error_obj.location.dot(Field::pAcquireInfo),
                         "semaphore and fence are both VK_NULL_HANDLE.");
    }
...
bool StatelessValidation::ValidateSwapchainCreateInfo(const VkSwapchainCreateInfoKHR& create_info,
    ...
    if (create_info.imageSharingMode == VK_SHARING_MODE_CONCURRENT) {
        ...
        if (create_info.queueFamilyIndexCount <= 1) {
            skip |= LogError("VUID-VkSwapchainCreateInfoKHR-imageSharingMode-01278",
                             device,
                             loc.dot(Field::imageSharingMode),
        ...
        if (create_info.pQueueFamilyIndices == nullptr) {
            skip |= LogError("VUID-VkSwapchainCreateInfoKHR-imageSharingMode-01277",
                             device,
                             loc.dot(Field::imageSharingMode),
    ...
    skip |= ValidateNotZero(create_info.imageArrayLayers == 0,
    ...
    if (format_list_info) {
        ...
        if (((create_info.flags & VK_SWAPCHAIN_CREATE_MUTABLE_FORMAT_BIT_KHR) == 0) && (view_format_count > 1)) {
            skip |=
                LogError("VUID-VkSwapchainCreateInfoKHR-flags-04100",
                         device,
        ...
        for (uint32_t i = 1; i < view_format_count; i++) {
            if (vkuFormatCompatibilityClass(format_list_info->pViewFormats[0]) !=
                ...
                skip |= LogError("VUID-VkSwapchainCreateInfoKHR-pNext-04099",
                                 device,
    ...
    if ((create_info.flags & VK_SWAPCHAIN_CREATE_MUTABLE_FORMAT_BIT_KHR) != 0) {
        if (format_list_info == nullptr) {
            ...
            skip |= LogError("VUID-VkSwapchainCreateInfoKHR-flags-03168",
                             device,
                             loc.dot(Field::flags),
    ...
    if (create_info.presentMode == VK_PRESENT_MODE_FIFO_LATEST_READY_EXT &&
        !enabled_features.presentModeFifoLatestReady) {
        skip |= LogError("VUID-VkSwapchainCreateInfoKHR-presentModeFifoLatestReady-10161",
                         device,
                         loc.dot(Field::presentMode),
...
bool StatelessValidation::manual_PreCallValidateCreateSwapchainKHR(VkDevice device,
...
bool StatelessValidation::manual_PreCallValidateCreateSharedSwapchainsKHR(VkDevice device,
...
bool StatelessValidation::manual_PreCallValidateQueuePresentKHR(VkQueue queue,
    ...
    for (uint32_t i = 0; i < pPresentInfo->swapchainCount; ++i) {
        for (uint32_t j = i + 1; j < pPresentInfo->swapchainCount; ++j) {
            if (pPresentInfo->pSwapchains[i] == pPresentInfo->pSwapchains[j]) {
                skip |= LogError("VUID-VkPresentInfoKHR-pSwapchain-09231",
                                 device,
                                 error_obj.location.dot(Field::pSwapchain),
                                 "[%" PRIu32 "] and pSwapchain[%" PRIu32 "] are both %s.",
                                 i,
                                 j,
...
bool StatelessValidation::manual_PreCallValidateCreateDisplayModeKHR(VkPhysicalDevice physicalDevice,
    ...
    skip |= ValidateNotZero(display_mode_parameters.visibleRegion.width == 0,
                            "VUID-VkDisplayModeParametersKHR-width-01990",
                            param_loc.dot(Field::visibleRegion).dot(Field::width));
    skip |= ValidateNotZero(display_mode_parameters.visibleRegion.height == 0,
                            "VUID-VkDisplayModeParametersKHR-height-01991",
                            param_loc.dot(Field::visibleRegion).dot(Field::width));
    skip |= ValidateNotZero(display_mode_parameters.refreshRate == 0,
...
bool StatelessValidation::manual_PreCallValidateGetPhysicalDeviceSurfaceFormatsKHR(VkPhysicalDevice physicalDevice,
    ...
    if (surface == VK_NULL_HANDLE && !IsExtEnabled(instance_extensions.vk_google_surfaceless_query)) {
        skip |= LogError("VUID-vkGetPhysicalDeviceSurfaceFormatsKHR-surface-06524",
                         physicalDevice,
...
bool StatelessValidation::manual_PreCallValidateGetPhysicalDeviceSurfacePresentModesKHR(
    ...
    bool skip = false;
    if (surface == VK_NULL_HANDLE && !IsExtEnabled(instance_extensions.vk_google_surfaceless_query)) {
        skip |= LogError("VUID-vkGetPhysicalDeviceSurfacePresentModesKHR-surface-06524",
                         physicalDevice,
                         error_obj.location.dot(Field::surface),
                         "is VK_NULL_HANDLE and VK_GOOGLE_surfaceless_query is not enabled.");
    }
...
bool StatelessValidation::manual_PreCallValidateGetPhysicalDeviceSurfaceCapabilities2KHR(
    ...
    const ErrorObject& error_obj) const {
    bool skip = false;
    if (pSurfaceInfo && pSurfaceInfo->surface == VK_NULL_HANDLE &&
        !IsExtEnabled(instance_extensions.vk_google_surfaceless_query)) {
        skip |= LogError("VUID-vkGetPhysicalDeviceSurfaceCapabilities2KHR-pSurfaceInfo-06521",
                         physicalDevice,
    ...
    if (capabilities_full_screen_exclusive) {
        ...
        if (!full_screen_exclusive_win32_info) {
            skip |= LogError("VUID-vkGetPhysicalDeviceSurfaceCapabilities2KHR-pNext-02671",
                             physicalDevice,
                             error_obj.location,
    ...
    if (!(vku::FindStructInPNextChain<VkSurfacePresentModeEXT>(pSurfaceInfo->pNext))) {
        if (surface_present_mode_compatibility) {
            skip |= LogError("VUID-vkGetPhysicalDeviceSurfaceCapabilities2KHR-pNext-07776",
                             physicalDevice,
                             error_obj.location,
        ...
        if (surface_present_scaling_compatibilities) {
            skip |= LogError("VUID-vkGetPhysicalDeviceSurfaceCapabilities2KHR-pNext-07777",
                             physicalDevice,
                             error_obj.location,
...
bool StatelessValidation::manual_PreCallValidateGetPhysicalDeviceSurfaceFormats2KHR(
    ...
    if (pSurfaceInfo && pSurfaceInfo->surface == VK_NULL_HANDLE &&
        !IsExtEnabled(instance_extensions.vk_google_surfaceless_query)) {
        skip |= LogError("VUID-vkGetPhysicalDeviceSurfaceFormats2KHR-pSurfaceInfo-06521",
                         physicalDevice,
                         error_obj.location.dot(Field::pSurfaceInfo).dot(Field::surface),
                         "is VK_NULL_HANDLE and VK_GOOGLE_surfaceless_query is not enabled.");
    }
        ...
        for (uint32_t i = 0; i < *pSurfaceFormatCount; ++i) {
            if (vku::FindStructInPNextChain<VkImageCompressionPropertiesEXT>(pSurfaceFormats[i].pNext) &&
                ...
                skip |= LogError(
                    ...
                    device,
                    error_obj.location.dot(Field::pNext),
...
bool StatelessValidation::manual_PreCallValidateGetPhysicalDeviceSurfacePresentModes2EXT(
    ...
    if (pSurfaceInfo && pSurfaceInfo->surface == VK_NULL_HANDLE &&
        !IsExtEnabled(instance_extensions.vk_google_surfaceless_query)) {
        skip |= LogError("VUID-vkGetPhysicalDeviceSurfacePresentModes2EXT-pSurfaceInfo-06521",
                         physicalDevice,
                         error_obj.location.dot(Field::pSurfaceInfo).dot(Field::surface),
                         "is VK_NULL_HANDLE and VK_GOOGLE_surfaceless_query is not enabled.");
    }
...
bool StatelessValidation::manual_PreCallValidateCreateWin32SurfaceKHR(VkInstance instance,
    ...
    if (pCreateInfo->hwnd == nullptr) {
        skip |= LogError(
            "VUID-VkWin32SurfaceCreateInfoKHR-hwnd-01308", instance, error_obj.location, "pCreateInfo->hwnd is NULL.");
    }

    return skip;
}

bool StatelessValidation::PreCallValidateGetDeviceGroupSurfacePresentModes2EXT(
    VkDevice device,
    const VkPhysicalDeviceSurfaceInfo2KHR* pSurfaceInfo,
    ...
    if (!pModes) {
        skip |= LogError("VUID-vkGetDeviceGroupSurfacePresentModes2EXT-pModes-parameter",
                         device,
                         error_obj.location.dot(Field::pModes),
                         "is NULL.");
    }

    skip |= ValidateStructType(error_obj.location.dot(Field::pSurfaceInfo),
                               pSurfaceInfo,
    ...
    if (pSurfaceInfo != NULL) {
        ...
        skip |= ValidateRequiredHandle(error_obj.location.dot(Field::pSurfaceInfo).dot(Field::surface),
...
bool StatelessValidation::manual_PreCallValidateCreateWaylandSurfaceKHR(
    VkInstance instance,
    ...
    if (display == nullptr) {
        skip |= LogError("VUID-VkWaylandSurfaceCreateInfoKHR-display-01304",
                         instance,
                         error_obj.location.dot(Field::pCreateInfo).dot(Field::display),
                         "is NULL!");
    }

    if (surface == nullptr) {
        skip |= LogError("VUID-VkWaylandSurfaceCreateInfoKHR-surface-01305",
                         instance,
                         error_obj.location.dot(Field::pCreateInfo).dot(Field::surface),
                         "is NULL!");
    }
...
bool StatelessValidation::manual_PreCallValidateCreateXcbSurfaceKHR(VkInstance instance,
    ...
    if (connection == nullptr) {
        skip |= LogError("VUID-VkXcbSurfaceCreateInfoKHR-connection-01310",
                         instance,
                         error_obj.location.dot(Field::pCreateInfo).dot(Field::connection),
                         "is NULL!");
    }

    skip |= ValidateNotZero(window == 0,
...
bool StatelessValidation::manual_PreCallValidateCreateXlibSurfaceKHR(VkInstance instance,
    ...
    if (display == nullptr) {
        skip |= LogError("VUID-VkXlibSurfaceCreateInfoKHR-dpy-01313",
                         instance,
                         error_obj.location.dot(Field::pCreateInfo).dot(Field::dpy),
                         "is NULL!");
    }
...
bool StatelessValidation::manual_PreCallValidateCreateAndroidSurfaceKHR(
    VkInstance instance,
    ...
    if (pCreateInfo->window == nullptr) {
        skip |= LogError("VUID-VkAndroidSurfaceCreateInfoKHR-window-01248",
                         instance,
                         error_obj.location.dot(Field::pCreateInfo).dot(Field::window),
                         "is NULL.");
    }
