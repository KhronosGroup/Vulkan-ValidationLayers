...
bool StatelessValidation::ValidateGeometryTrianglesNV(const VkGeometryTrianglesNV& triangles,
    ...
    if (triangles.vertexFormat != VK_FORMAT_R32G32B32_SFLOAT && triangles.vertexFormat != VK_FORMAT_R16G16B16_SFLOAT &&
        ...
        } else if (triangles.vertexFormat == VK_FORMAT_R16G16B16_SFLOAT ||
                   triangles.vertexFormat == VK_FORMAT_R16G16B16_SNORM ||
                   triangles.vertexFormat == VK_FORMAT_R16G16_SFLOAT ||
                   triangles.vertexFormat == VK_FORMAT_R16G16_SNORM) {
...
bool StatelessValidation::ValidateGeometryAABBNV(const VkGeometryAABBNV& aabbs,
                                                 VkAccelerationStructureNV object_handle,
                                                 const Location& loc) const {
    bool skip = false;
...
bool StatelessValidation::ValidateGeometryNV(const VkGeometryNV& geometry,
                                             VkAccelerationStructureNV object_handle,
                                             const Location& loc) const {
    bool skip = false;
...
bool StatelessValidation::ValidateAccelerationStructureInfoNV(const VkAccelerationStructureInfoNV& info,
    ...
    if (info.type == VK_ACCELERATION_STRUCTURE_TYPE_TOP_LEVEL_NV && info.geometryCount != 0) {
        skip |= LogError("VUID-VkAccelerationStructureInfoNV-type-02425",
                         object_handle,
                         loc,
    ...
    if (info.type == VK_ACCELERATION_STRUCTURE_TYPE_BOTTOM_LEVEL_NV && info.instanceCount != 0) {
        skip |= LogError("VUID-VkAccelerationStructureInfoNV-type-02426",
                         object_handle,
                         loc,
    ...
    if (info.type == VK_ACCELERATION_STRUCTURE_TYPE_GENERIC_KHR) {
        skip |= LogError("VUID-VkAccelerationStructureInfoNV-type-04623",
                         object_handle,
                         loc,
    ...
    if (info.instanceCount > phys_dev_ext_props.ray_tracing_props_nv.maxInstanceCount) {
        skip |= LogError("VUID-VkAccelerationStructureInfoNV-instanceCount-02423",
                         object_handle,
                         loc,
    ...
    if (info.type == VK_ACCELERATION_STRUCTURE_TYPE_BOTTOM_LEVEL_NV && info.geometryCount > 0) {
        ...
        if (total_triangle_count > phys_dev_ext_props.ray_tracing_props_nv.maxTriangleCount) {
            skip |= LogError("VUID-VkAccelerationStructureInfoNV-maxTriangleCount-02424",
                             object_handle,
                             loc,
    ...
    if (info.type == VK_ACCELERATION_STRUCTURE_TYPE_BOTTOM_LEVEL_NV && info.geometryCount > 1) {
        ...
        for (uint32_t i = 1; i < info.geometryCount; i++) {
            ...
            if (geometry.geometryType != first_geometry_type) {
                skip |= LogError("VUID-VkAccelerationStructureInfoNV-type-02786",
                                 object_handle,
                                 loc,
                                 "info.pGeometries[%" PRIu32 "].geometryType does not match "
    ...
    for (uint32_t geometry_index = 0; geometry_index < info.geometryCount; ++geometry_index) {
        if (!(info.pGeometries[geometry_index].geometryType == VK_GEOMETRY_TYPE_TRIANGLES_NV ||
            ...
            skip |= LogError("VUID-VkGeometryNV-geometryType-03503",
                             object_handle,
                             loc,
    ...
    skip |= ValidateFlags(loc.dot(Field::flags),
                          vvl::FlagBitmask::VkBuildAccelerationStructureFlagBitsKHR,
                          AllVkBuildAccelerationStructureFlagBitsKHR,
                          info.flags,
                          kOptionalFlags,
                          VK_NULL_HANDLE,
...
bool StatelessValidation::manual_PreCallValidateCreateAccelerationStructureNV(
    ...
    if ((pCreateInfo->compactedSize != 0) &&
        ...
        skip |= LogError("VUID-VkAccelerationStructureCreateInfoNV-compactedSize-02421",
                         device,
                         error_obj.location,
...
bool StatelessValidation::manual_PreCallValidateCreateAccelerationStructureKHR(
    ...
    if (!enabled_features.accelerationStructure) {
        skip |= LogError("VUID-vkCreateAccelerationStructureKHR-accelerationStructure-03611",
                         device,
                         error_obj.location,
    ...
    if (pCreateInfo->createFlags & VK_ACCELERATION_STRUCTURE_CREATE_DEVICE_ADDRESS_CAPTURE_REPLAY_BIT_KHR &&
        ...
        skip |= LogError("VUID-VkAccelerationStructureCreateInfoKHR-createFlags-03613",
                         device,
    ...
    if (pCreateInfo->deviceAddress &&
        ...
        skip |= LogError("VUID-VkAccelerationStructureCreateInfoKHR-deviceAddress-03612",
                         device,
                         create_info_loc.dot(Field::createFlags),
                         "includes VK_ACCELERATION_STRUCTURE_CREATE_DEVICE_ADDRESS_CAPTURE_REPLAY_BIT_KHR but the "
                         "deviceAddress (%" PRIu64 ") is not zero.",
                         pCreateInfo->deviceAddress);
    }
    if (pCreateInfo->deviceAddress && !enabled_features.accelerationStructureCaptureReplay) {
        skip |= LogError("VUID-vkCreateAccelerationStructureKHR-deviceAddress-03488",
                         device,
                         create_info_loc.dot(Field::deviceAddress),
                         "is %" PRIu64 " but accelerationStructureCaptureReplay feature was not enabled.",
                         pCreateInfo->deviceAddress);
    }
    if (SafeModulo(pCreateInfo->offset, 256) != 0) {
        skip |= LogError("VUID-VkAccelerationStructureCreateInfoKHR-offset-03734",
                         device,
                         create_info_loc.dot(Field::offset),
                         "(%" PRIu64 ") must be a multiple of 256 bytes",
                         pCreateInfo->offset);
    }
    ...
    if ((pCreateInfo->createFlags & VK_ACCELERATION_STRUCTURE_CREATE_DESCRIPTOR_BUFFER_CAPTURE_REPLAY_BIT_EXT) &&
        ...
        skip |= LogError("VUID-VkAccelerationStructureCreateInfoKHR-createFlags-08108",
                         device,
                         create_info_loc.dot(Field::createFlags),
    ...
    if (opaque_capture_descriptor_buffer &&
        ...
        skip |= LogError("VUID-VkAccelerationStructureCreateInfoKHR-pNext-08109",
                         device,
                         create_info_loc.dot(Field::createFlags),
...
bool StatelessValidation::manual_PreCallValidateDestroyAccelerationStructureKHR(
    VkDevice device,
    ...
    bool skip = false;
    if (!enabled_features.accelerationStructure) {
        skip |= LogError("VUID-vkDestroyAccelerationStructureKHR-accelerationStructure-08934",
                         device,
                         error_obj.location,
                         "accelerationStructure feature was not enabled.");
    }
...
bool StatelessValidation::manual_PreCallValidateGetAccelerationStructureHandleNV(
    VkDevice device,
    ...
    bool skip = false;
    if (dataSize < 8) {
        skip |= LogError("VUID-vkGetAccelerationStructureHandleNV-dataSize-02240",
...
bool StatelessValidation::manual_PreCallValidateCmdWriteAccelerationStructuresPropertiesNV(
    ...
    if (queryType != VK_QUERY_TYPE_ACCELERATION_STRUCTURE_COMPACTED_SIZE_NV) {
        skip |= LogError("VUID-vkCmdWriteAccelerationStructuresPropertiesNV-queryType-06216",
                         device,
                         error_obj.location,
                         "queryType must be "
...
bool StatelessValidation::ValidateCreateRayTracingPipelinesFlagsNV(const VkPipelineCreateFlags2KHR flags,
    ...
    if (flags & VK_PIPELINE_CREATE_INDIRECT_BINDABLE_BIT_NV) {
        skip |= LogError("VUID-VkRayTracingPipelineCreateInfoNV-flags-02904",
                         device,
                         flags_loc,
                         "is %s.",
    ...
    if (flags & VK_PIPELINE_CREATE_2_INDIRECT_BINDABLE_BIT_EXT) {
        skip |= LogError("VUID-VkRayTracingPipelineCreateInfoNV-flags-11008",
                         device,
                         flags_loc,
                         "is %s.",
                         string_VkPipelineCreateFlags2(flags).c_str());
    ...
    if ((flags & VK_PIPELINE_CREATE_DEFER_COMPILE_BIT_NV) &&
        ...
        skip |= LogError("VUID-VkRayTracingPipelineCreateInfoNV-flags-02957",
                         device,
                         flags_loc,
                         "is %s.",
                         string_VkPipelineCreateFlags2(flags).c_str());
    ...
    if (flags & VK_PIPELINE_CREATE_LIBRARY_BIT_KHR) {
        skip |= LogError("VUID-VkRayTracingPipelineCreateInfoNV-flags-03456",
                         device,
                         flags_loc,
                         "is %s.",
                         string_VkPipelineCreateFlags2(flags).c_str());
    ...
    if (flags & VK_PIPELINE_CREATE_RAY_TRACING_NO_NULL_ANY_HIT_SHADERS_BIT_KHR) {
        skip |= LogError("VUID-VkRayTracingPipelineCreateInfoNV-flags-03458",
                         device,
                         flags_loc,
                         "is %s.",
                         string_VkPipelineCreateFlags2(flags).c_str());
    ...
    if (flags & VK_PIPELINE_CREATE_RAY_TRACING_NO_NULL_CLOSEST_HIT_SHADERS_BIT_KHR) {
        skip |= LogError("VUID-VkRayTracingPipelineCreateInfoNV-flags-03459",
                         device,
                         flags_loc,
                         "is %s.",
                         string_VkPipelineCreateFlags2(flags).c_str());
    ...
    if (flags & VK_PIPELINE_CREATE_RAY_TRACING_NO_NULL_MISS_SHADERS_BIT_KHR) {
        skip |= LogError("VUID-VkRayTracingPipelineCreateInfoNV-flags-03460",
                         device,
                         flags_loc,
                         "is %s",
                         string_VkPipelineCreateFlags2(flags).c_str());
    ...
    if (flags & VK_PIPELINE_CREATE_RAY_TRACING_NO_NULL_INTERSECTION_SHADERS_BIT_KHR) {
        skip |= LogError("VUID-VkRayTracingPipelineCreateInfoNV-flags-03461",
                         device,
                         flags_loc,
                         "is %s",
                         string_VkPipelineCreateFlags2(flags).c_str());
    ...
    if (flags & VK_PIPELINE_CREATE_RAY_TRACING_SKIP_AABBS_BIT_KHR) {
        skip |= LogError("VUID-VkRayTracingPipelineCreateInfoNV-flags-03462",
                         device,
                         flags_loc,
                         "is %s",
                         string_VkPipelineCreateFlags2(flags).c_str());
    ...
    if (flags & VK_PIPELINE_CREATE_RAY_TRACING_SKIP_TRIANGLES_BIT_KHR) {
        skip |= LogError("VUID-VkRayTracingPipelineCreateInfoNV-flags-03463",
                         device,
                         flags_loc,
                         "is %s",
                         string_VkPipelineCreateFlags2(flags).c_str());
    ...
    if (flags & VK_PIPELINE_CREATE_RAY_TRACING_SHADER_GROUP_HANDLE_CAPTURE_REPLAY_BIT_KHR) {
        skip |= LogError("VUID-VkRayTracingPipelineCreateInfoNV-flags-03588",
                         device,
                         flags_loc,
                         "is %s",
                         string_VkPipelineCreateFlags2(flags).c_str());
    ...
    if (flags & VK_PIPELINE_CREATE_DISPATCH_BASE) {
        skip |= LogError("VUID-vkCreateRayTracingPipelinesNV-flags-03816",
                         device,
                         flags_loc,
                         "is %s",
                         string_VkPipelineCreateFlags2(flags).c_str());
    ...
    if (flags & VK_PIPELINE_CREATE_RAY_TRACING_ALLOW_MOTION_BIT_NV) {
        skip |= LogError("VUID-VkRayTracingPipelineCreateInfoNV-flags-04948",
                         device,
                         flags_loc,
                         "is %s",
                         string_VkPipelineCreateFlags2(flags).c_str());
...
bool StatelessValidation::manual_PreCallValidateCreateRayTracingPipelinesNV(
    ...
    for (uint32_t i = 0; i < createInfoCount; i++) {
        ...
        if ((feedback_struct != nullptr) && (feedback_struct->pipelineStageCreationFeedbackCount != 0) &&
            ...
            skip |=
                LogError("VUID-VkRayTracingPipelineCreateInfoNV-pipelineStageCreationFeedbackCount-06651",
                         device,
                         create_info_loc.dot(Field::stageCount),
                         "(%" PRIu32
                         ") must equal VkPipelineCreationFeedback::pipelineStageCreationFeedbackCount (%" PRIu32 ").",
                         create_info.stageCount,
                         feedback_struct->pipelineStageCreationFeedbackCount);
        }
        ...
        const Location flags_loc = create_flags_2
                                       ? create_info_loc.pNext(Struct::VkPipelineCreateFlags2CreateInfo, Field::flags)
                                       : create_info_loc.dot(Field::flags);
        if (!create_flags_2) {
            skip |= ValidateFlags(flags_loc,
                                  vvl::FlagBitmask::VkPipelineCreateFlagBits,
                                  AllVkPipelineCreateFlagBits,
                                  create_info.flags,
                                  kOptionalFlags,
                                  VK_NULL_HANDLE,
        ...
        if (!enabled_features.pipelineCreationCacheControl) {
            if (flags & (VK_PIPELINE_CREATE_FAIL_ON_PIPELINE_COMPILE_REQUIRED_BIT |
                ...
                skip |= LogError("VUID-VkRayTracingPipelineCreateInfoNV-pipelineCreationCacheControl-02905",
                                 device,
                                 flags_loc,
        ...
        if (flags & VK_PIPELINE_CREATE_DERIVATIVE_BIT) {
            if (create_info.basePipelineIndex != -1) {
                if (create_info.basePipelineHandle != VK_NULL_HANDLE) {
                    skip |= LogError("VUID-VkRayTracingPipelineCreateInfoNV-flags-07986",
                                     device,
                                     flags_loc,
                                     "is %s, %s is %" PRId32 ", but %s is %s.",
                                     string_VkPipelineCreateFlags2(flags).c_str(),
                                     create_info_loc.dot(Field::basePipelineIndex).Fields().c_str(),
                ...
                if (create_info.basePipelineIndex > static_cast<int32_t>(i)) {
                    skip |= LogError("VUID-vkCreateRayTracingPipelinesNV-flags-03415",
                                     device,
                                     flags_loc,
                                     "is %s, but %s is %" PRId32 ".",
                                     string_VkPipelineCreateFlags2(flags).c_str(),
                                     create_info_loc.dot(Field::basePipelineIndex).Fields().c_str(),
                                     create_info.basePipelineIndex);
                }
            ...
            if (create_info.basePipelineHandle == VK_NULL_HANDLE) {
                if (static_cast<uint32_t>(create_info.basePipelineIndex) >= createInfoCount) {
                    skip |= LogError("VUID-VkRayTracingPipelineCreateInfoNV-flags-07985",
                                     device,
                                     create_info_loc.dot(Field::basePipelineHandle),
                                     "is VK_NULL_HANDLE, but %s is %s.",
                                     flags_loc.Fields().c_str(),
                                     string_VkPipelineCreateFlags2(flags).c_str());
                }
            } else {
                if (create_info.basePipelineIndex != -1) {
...
bool StatelessValidation::ValidateCreateRayTracingPipelinesFlagsKHR(const VkPipelineCreateFlags2KHR flags,
    ...
    if (flags & VK_PIPELINE_CREATE_INDIRECT_BINDABLE_BIT_NV) {
        skip |= LogError("VUID-VkRayTracingPipelineCreateInfoKHR-flags-02904",
                         device,
                         flags_loc,
                         "is %s.",
                         string_VkPipelineCreateFlags2(flags).c_str());
    ...
    if (flags & VK_PIPELINE_CREATE_DISPATCH_BASE) {
        skip |= LogError("VUID-vkCreateRayTracingPipelinesKHR-flags-03816",
                         device,
                         flags_loc,
                         "is %s.",
                         string_VkPipelineCreateFlags2(flags).c_str());
    ...
    if (flags & VK_PIPELINE_CREATE_RAY_TRACING_SHADER_GROUP_HANDLE_CAPTURE_REPLAY_BIT_KHR &&
        ...
        skip |= LogError("VUID-VkRayTracingPipelineCreateInfoKHR-flags-03598",
                         device,
                         flags_loc,
                         "is %s.",
                         string_VkPipelineCreateFlags2(flags).c_str());
    ...
    if (!enabled_features.rayTraversalPrimitiveCulling) {
        if (flags & VK_PIPELINE_CREATE_RAY_TRACING_SKIP_AABBS_BIT_KHR) {
            skip |= LogError("VUID-VkRayTracingPipelineCreateInfoKHR-rayTraversalPrimitiveCulling-03596",
                             device,
                             flags_loc,
                             "is %s.",
                             string_VkPipelineCreateFlags2(flags).c_str());
        ...
        if (flags & VK_PIPELINE_CREATE_RAY_TRACING_SKIP_TRIANGLES_BIT_KHR) {
            skip |= LogError("VUID-VkRayTracingPipelineCreateInfoKHR-rayTraversalPrimitiveCulling-03597",
                             device,
                             flags_loc,
                             "is %s.",
                             string_VkPipelineCreateFlags2(flags).c_str());
...
bool StatelessValidation::manual_PreCallValidateCreateRayTracingPipelinesKHR(
    ...
    if (!enabled_features.rayTracingPipeline) {
        skip |= LogError("VUID-vkCreateRayTracingPipelinesKHR-rayTracingPipeline-03586",
                         device,
                         error_obj.location,
    ...
    for (uint32_t i = 0; i < createInfoCount; i++) {
        ...
        for (uint32_t stage_index = 0; stage_index < create_info.stageCount; ++stage_index) {
            ...
            if ((stage & kShaderStageAllRayTracing) == 0) {
                skip |= LogError("VUID-VkRayTracingPipelineCreateInfoKHR-stage-06899",
                                 device,
                                 stage_loc.dot(Field::stage),
                                 "is %s.",
        ...
        if ((feedback_struct != nullptr) && (feedback_struct->pipelineStageCreationFeedbackCount != 0) &&
            (feedback_struct->pipelineStageCreationFeedbackCount != create_info.stageCount)) {
            skip |= LogError("VUID-VkRayTracingPipelineCreateInfoKHR-pipelineStageCreationFeedbackCount-06652",
                             device,
                             create_info_loc.pNext(Struct::VkPipelineCreationFeedbackCreateInfo,
                                                   Field::pipelineStageCreationFeedbackCount),
                             "(%" PRIu32 ") is not equal to %s (%" PRIu32 ").",
                             feedback_struct->pipelineStageCreationFeedbackCount,
                             create_info_loc.Fields().c_str(),
                             create_info.stageCount);
        }
        ...
        if (!enabled_features.pipelineCreationCacheControl) {
            if (flags & (VK_PIPELINE_CREATE_FAIL_ON_PIPELINE_COMPILE_REQUIRED_BIT |
                         VK_PIPELINE_CREATE_EARLY_RETURN_ON_FAILURE_BIT)) {
                skip |= LogError("VUID-VkRayTracingPipelineCreateInfoKHR-pipelineCreationCacheControl-02905",
                                 device,
                                 flags_loc,
                                 "is %s but the pipelineCreationCacheControl feature is not enabled.",
                                 string_VkPipelineCreateFlags2(flags).c_str());
        ...
        for (uint32_t group_index = 0; group_index < create_info.groupCount; ++group_index) {
            ...
            if ((create_info.pGroups[group_index].type == VK_RAY_TRACING_SHADER_GROUP_TYPE_TRIANGLES_HIT_GROUP_KHR) ||
                ...
                if ((flags & VK_PIPELINE_CREATE_RAY_TRACING_NO_NULL_ANY_HIT_SHADERS_BIT_KHR) &&
                    ...
                    skip |= LogError("VUID-VkRayTracingPipelineCreateInfoKHR-flags-03470",
                                     device,
                                     flags_loc,
                                     "includes VK_PIPELINE_CREATE_RAY_TRACING_NO_NULL_ANY_HIT_SHADERS_BIT_KHR but %s "
                                     "is VK_SHADER_UNUSED_KHR.",
        ...
        if (flags & VK_PIPELINE_CREATE_DERIVATIVE_BIT) {
            if (create_info.basePipelineIndex != -1) {
                if (create_info.basePipelineHandle != VK_NULL_HANDLE) {
                    skip |= LogError("VUID-VkRayTracingPipelineCreateInfoKHR-flags-07986",
                                     device,
                                     create_info_loc.dot(Field::basePipelineIndex),
                                     "is %" PRId32 " and basePipelineHandle is not VK_NULL_HANDLE.",
                                     create_info.basePipelineIndex);
                }
                if (create_info.basePipelineIndex > static_cast<int32_t>(i)) {
                    skip |= LogError("VUID-vkCreateRayTracingPipelinesKHR-flags-03415",
                                     device,
                                     create_info_loc.dot(Field::basePipelineIndex),
                                     "is %" PRId32 ".",
                                     create_info.basePipelineIndex);
            ...
            if (create_info.basePipelineHandle == VK_NULL_HANDLE) {
                if (create_info.basePipelineIndex < 0 ||
                    static_cast<uint32_t>(create_info.basePipelineIndex) >= createInfoCount) {
                    skip |= LogError("VUID-VkRayTracingPipelineCreateInfoKHR-flags-07985",
                                     device,
                                     flags_loc,
                                     "includes VK_PIPELINE_CREATE_DERIVATIVE_BIT but basePipelineIndex has invalid "
                                     "index value %" PRId32 ".",
                                     create_info.basePipelineIndex);
                }
            }
        ...
        if (flags & VK_PIPELINE_CREATE_LIBRARY_BIT_KHR) {
            if (create_info.pLibraryInterface == nullptr) {
                skip |= LogError("VUID-VkRayTracingPipelineCreateInfoKHR-flags-03465",
                                 device,
                                 flags_loc,
        ...
        if (!library_enabled) {
            if (create_info.pLibraryInfo) {
                skip |= LogError("VUID-VkRayTracingPipelineCreateInfoKHR-pLibraryInfo-03595",
                                 device,
                                 create_info_loc.dot(Field::pLibraryInfo),
                                 "is not NULL.");
            }
            if (create_info.pLibraryInterface) {
                skip |= LogError("VUID-VkRayTracingPipelineCreateInfoKHR-pLibraryInfo-03595",
                                 device,
                                 create_info_loc.dot(Field::pLibraryInterface),
                                 "is not NULL.");
            }
        }
        ...
        if (create_info.pLibraryInfo) {
            if ((create_info.pLibraryInfo->libraryCount > 0) && (create_info.pLibraryInterface == nullptr)) {
                skip |= LogError("VUID-VkRayTracingPipelineCreateInfoKHR-pLibraryInfo-03590",
                                 device,
                                 create_info_loc.dot(Field::pLibraryInfo).dot(Field::libraryCount),
                                 "is %" PRIu32 ", but pLibraryInterface is NULL.",
                                 create_info.pLibraryInfo->libraryCount);
            }
        }

        if (create_info.pLibraryInfo == nullptr) {
            if (create_info.stageCount == 0) {
                skip |= LogError("VUID-VkRayTracingPipelineCreateInfoKHR-pLibraryInfo-07999",
                                 device,
                                 create_info_loc.dot(Field::pLibraryInfo),
                                 "is NULL and stageCount is zero.");
            }
            if (((flags & VK_PIPELINE_CREATE_LIBRARY_BIT_KHR) == 0) && (create_info.groupCount == 0)) {
                skip |= LogError("VUID-VkRayTracingPipelineCreateInfoKHR-flags-08700",
                                 device,
                                 create_info_loc.dot(Field::pLibraryInfo),
            ...
            if (create_info.stageCount == 0) {
                skip |= LogError("VUID-VkRayTracingPipelineCreateInfoKHR-pLibraryInfo-07999",
                                 device,
                                 create_info_loc.dot(Field::pLibraryInfo).dot(Field::libraryCount),
                                 "is zero and stageCount is zero.");
            }
            if (((flags & VK_PIPELINE_CREATE_LIBRARY_BIT_KHR) == 0) && (create_info.groupCount == 0)) {
                skip |= LogError("VUID-VkRayTracingPipelineCreateInfoKHR-flags-08700",
                                 device,
        ...
        if (create_info.pLibraryInterface) {
            if (create_info.pLibraryInterface->maxPipelineRayHitAttributeSize >
                ...
                skip |= LogError(
                    ...
                    "(%" PRIu32
                    ") is larger than VkPhysicalDeviceRayTracingPipelinePropertiesKHR::maxRayHitAttributeSize (%" PRIu32
                    ").",
        ...
        if (deferredOperation != VK_NULL_HANDLE) {
            if (flags & VK_PIPELINE_CREATE_EARLY_RETURN_ON_FAILURE_BIT) {
                skip |= LogError("VUID-vkCreateRayTracingPipelinesKHR-deferredOperation-03587",
                                 device,
                                 flags_loc,
                                 "includes VK_PIPELINE_CREATE_EARLY_RETURN_ON_FAILURE_BIT but deferredOperation is not "
                                 "VK_NULL_HANDLE.");
            }
        }
        ...
        if (create_info.pDynamicState) {
            for (uint32_t j = 0; j < create_info.pDynamicState->dynamicStateCount; ++j) {
                if (create_info.pDynamicState->pDynamicStates[j] !=
                    VK_DYNAMIC_STATE_RAY_TRACING_PIPELINE_STACK_SIZE_KHR) {
                    skip |= LogError("VUID-VkRayTracingPipelineCreateInfoKHR-pDynamicStates-03602",
                                     device,
                                     create_info_loc.dot(Field::pDynamicState).dot(Field::pDynamicStates, j),
                                     "is %s.",
...
bool StatelessValidation::manual_PreCallValidateCopyAccelerationStructureToMemoryKHR(
    ...
    if (pInfo->mode != VK_COPY_ACCELERATION_STRUCTURE_MODE_SERIALIZE_KHR) {
        skip |= LogError("VUID-VkCopyAccelerationStructureToMemoryInfoKHR-mode-03412",
                         device,
    ...
    if (!enabled_features.accelerationStructureHostCommands) {
        skip |= LogError("VUID-vkCopyAccelerationStructureToMemoryKHR-accelerationStructureHostCommands-03584",
                         device,
                         error_obj.location,
                         "accelerationStructureHostCommands feature was not enabled.");
    }
    skip |= ValidateRequiredPointer(info_loc.dot(Field::dst).dot(Field::hostAddress),
    ...
    if (SafeModulo((VkDeviceSize)pInfo->dst.hostAddress, 16) != 0) {
        skip |= LogError("VUID-vkCopyAccelerationStructureToMemoryKHR-pInfo-03751",
                         device,
                         info_loc.dot(Field::dst).dot(Field::hostAddress),
                         "(0x%" PRIx64 ") must be aligned to 16 bytes.",
...
bool StatelessValidation::manual_PreCallValidateCmdCopyAccelerationStructureToMemoryKHR(
    ...
    if (!enabled_features.accelerationStructure) {
        skip |= LogError("VUID-vkCmdCopyAccelerationStructureToMemoryKHR-accelerationStructure-08926",
                         device,
                         error_obj.location,
                         "accelerationStructure feature was not enabled.");
    }
    ...
    if (SafeModulo(pInfo->dst.deviceAddress, 256) != 0) {
        skip |= LogError("VUID-vkCmdCopyAccelerationStructureToMemoryKHR-pInfo-03740",
                         commandBuffer,
                         info_loc.dot(Field::dst).dot(Field::deviceAddress),
                         "(0x%" PRIx64 ") must be aligned to 256 bytes.",
...
bool StatelessValidation::ValidateCopyAccelerationStructureInfoKHR(const VkCopyAccelerationStructureInfoKHR& as_info,
    ...
    if (!(as_info.mode == VK_COPY_ACCELERATION_STRUCTURE_MODE_COMPACT_KHR ||
        ...
        skip |= LogError("VUID-VkCopyAccelerationStructureInfoKHR-mode-03410",
                         objlist,
                         info_loc.dot(Field::mode),
                         "is %s.",
...
bool StatelessValidation::manual_PreCallValidateCopyAccelerationStructureKHR(
    ...
    if (!enabled_features.accelerationStructureHostCommands) {
        skip |= LogError("VUID-vkCopyAccelerationStructureKHR-accelerationStructureHostCommands-03582",
                         device,
                         error_obj.location,
...
bool StatelessValidation::manual_PreCallValidateCmdCopyAccelerationStructureKHR(
    ...
    if (!enabled_features.accelerationStructure) {
        skip |= LogError("VUID-vkCmdCopyAccelerationStructureKHR-accelerationStructure-08925",
                         device,
                         error_obj.location,
                         "accelerationStructure feature was not enabled.");
    }
...
bool StatelessValidation::ValidateCopyMemoryToAccelerationStructureInfoKHR(
    const VkCopyMemoryToAccelerationStructureInfoKHR& as_info,
    const VulkanTypedHandle& handle,
    const Location& loc) const {
    bool skip = false;
    if (as_info.mode != VK_COPY_ACCELERATION_STRUCTURE_MODE_DESERIALIZE_KHR) {
        const LogObjectList objlist(handle);
        skip |= LogError("VUID-VkCopyMemoryToAccelerationStructureInfoKHR-mode-03413",
                         objlist,
                         loc.dot(Field::mode),
                         "is %s.",
                         string_VkCopyAccelerationStructureModeKHR(as_info.mode));
...
bool StatelessValidation::manual_PreCallValidateCopyMemoryToAccelerationStructureKHR(
    ...
    if (!enabled_features.accelerationStructureHostCommands) {
        skip |= LogError("VUID-vkCopyMemoryToAccelerationStructureKHR-accelerationStructureHostCommands-03583",
                         device,
                         error_obj.location,
                         "accelerationStructureHostCommands feature was not enabled.");
    }
    skip |= ValidateRequiredPointer(info_loc.dot(Field::src).dot(Field::hostAddress),
    ...
    if (SafeModulo((VkDeviceAddress)pInfo->src.hostAddress, 16) != 0) {
        skip |= LogError("VUID-vkCopyMemoryToAccelerationStructureKHR-pInfo-03750",
                         device,
                         info_loc.dot(Field::src).dot(Field::hostAddress),
                         "(0x%" PRIx64 ") must be aligned to 16 bytes.",
...
bool StatelessValidation::manual_PreCallValidateCmdCopyMemoryToAccelerationStructureKHR(
    ...
    if (!enabled_features.accelerationStructure) {
        skip |= LogError("VUID-vkCmdCopyMemoryToAccelerationStructureKHR-accelerationStructure-08927",
                         device,
                         error_obj.location,
                         "accelerationStructure feature was not enabled.");
    }
    ...
    if (SafeModulo(pInfo->src.deviceAddress, 256) != 0) {
        skip |= LogError("VUID-vkCmdCopyMemoryToAccelerationStructureKHR-pInfo-03743",
                         commandBuffer,
...
bool StatelessValidation::manual_PreCallValidateCmdWriteAccelerationStructuresPropertiesKHR(
    ...
    if (!enabled_features.accelerationStructure) {
        skip |= LogError("VUID-vkCmdWriteAccelerationStructuresPropertiesKHR-accelerationStructure-08924",
                         commandBuffer,
                         error_obj.location,
                         "accelerationStructure feature was not enabled.");
    }
    ...
    if (!(queryType == VK_QUERY_TYPE_ACCELERATION_STRUCTURE_COMPACTED_SIZE_KHR ||
        ...
        skip |= LogError("VUID-vkCmdWriteAccelerationStructuresPropertiesKHR-queryType-06742",
                         commandBuffer,
...
bool StatelessValidation::manual_PreCallValidateWriteAccelerationStructuresPropertiesKHR(
    ...
    bool skip = false;
    if (!enabled_features.accelerationStructureHostCommands) {
        skip |= LogError("VUID-vkWriteAccelerationStructuresPropertiesKHR-accelerationStructureHostCommands-03585",
                         device,
                         error_obj.location,
                         "accelerationStructureHostCommands feature was not enabled.");
    }
    if (dataSize < accelerationStructureCount * stride) {
        skip |= LogError("VUID-vkWriteAccelerationStructuresPropertiesKHR-dataSize-03452",
                         device,
                         error_obj.location.dot(Field::dataSize),
                         "(%zu) is less than "
    ...
    if (dataSize < sizeof(VkDeviceSize)) {
        switch (queryType) {
            case VK_QUERY_TYPE_ACCELERATION_STRUCTURE_COMPACTED_SIZE_KHR:
                skip |= LogError("VUID-vkWriteAccelerationStructuresPropertiesKHR-queryType-03449",
                                 device,
                                 query_type_loc,
                                 "is %s, but %s is %zu.",
                                 string_VkQueryType(queryType),
                                 data_size_loc.Fields().c_str(),
                                 dataSize);
                break;
            case VK_QUERY_TYPE_ACCELERATION_STRUCTURE_SERIALIZATION_SIZE_KHR:
                skip |= LogError("VUID-vkWriteAccelerationStructuresPropertiesKHR-queryType-03451",
                                 device,
                                 query_type_loc,
                                 "is %s, but %s is %zu.",
                                 string_VkQueryType(queryType),
            ...
            case VK_QUERY_TYPE_ACCELERATION_STRUCTURE_SERIALIZATION_BOTTOM_LEVEL_POINTERS_KHR:
                skip |= LogError("VUID-vkWriteAccelerationStructuresPropertiesKHR-queryType-06734",
                                 device,
                                 query_type_loc,
                                 "is %s, but %s is %zu.",
                                 string_VkQueryType(queryType),
            ...
            case VK_QUERY_TYPE_ACCELERATION_STRUCTURE_SIZE_KHR:
                skip |= LogError("VUID-vkWriteAccelerationStructuresPropertiesKHR-queryType-06732",
                                 device,
                                 query_type_loc,
                                 "is %s, but %s is %zu.",
                                 string_VkQueryType(queryType),
    ...
    if (!(queryType == VK_QUERY_TYPE_ACCELERATION_STRUCTURE_COMPACTED_SIZE_KHR ||
          ...
          queryType == VK_QUERY_TYPE_ACCELERATION_STRUCTURE_SIZE_KHR ||
          queryType == VK_QUERY_TYPE_ACCELERATION_STRUCTURE_SERIALIZATION_BOTTOM_LEVEL_POINTERS_KHR)) {
        skip |= LogError("VUID-vkWriteAccelerationStructuresPropertiesKHR-queryType-06742",
                         device,
                         error_obj.location.dot(Field::queryType),
                         "is %s.",
                         string_VkQueryType(queryType));
...
bool StatelessValidation::manual_PreCallValidateGetRayTracingCaptureReplayShaderGroupHandlesKHR(
    ...
    bool skip = false;
    if (!enabled_features.rayTracingPipelineShaderGroupHandleCaptureReplay) {
        skip |= LogError("VUID-vkGetRayTracingCaptureReplayShaderGroupHandlesKHR-"
                         "rayTracingPipelineShaderGroupHandleCaptureReplay-03606",
                         device,
                         error_obj.location,
                         "rayTracingPipelineShaderGroupHandleCaptureReplay feature was not enabled.");
    }
    return skip;
}
...
bool StatelessValidation::manual_PreCallValidateGetDeviceAccelerationStructureCompatibilityKHR(
    ...
    bool skip = false;
    if (!enabled_features.accelerationStructure) {
        skip |= LogError("VUID-vkGetDeviceAccelerationStructureCompatibilityKHR-accelerationStructure-08928",
                         device,
                         error_obj.location,
                         "accelerationStructure feature was not enabled.");
    }
...
bool StatelessValidation::ValidateTotalPrimitivesCount(uint64_t total_triangles_count,
                                                       uint64_t total_aabbs_count,
                                                       const VulkanTypedHandle& handle,
                                                       const Location& loc) const {
    bool skip = false;

    if (total_triangles_count > phys_dev_ext_props.acc_structure_props.maxPrimitiveCount) {
        skip |= LogError("VUID-VkAccelerationStructureBuildGeometryInfoKHR-type-03795",
                         handle,
                         loc,
                         "total number of triangles in all geometries (%" PRIu64 ") is larger than maxPrimitiveCount "
                         "(%" PRIu64 ")",
    ...
    if (total_aabbs_count > phys_dev_ext_props.acc_structure_props.maxPrimitiveCount) {
        skip |= LogError("VUID-VkAccelerationStructureBuildGeometryInfoKHR-type-03794",
                         handle,
                         loc,
                         "total number of AABBs in all geometries (%" PRIu64 ") is larger than maxPrimitiveCount "
                         "(%" PRIu64 ")",
                         total_aabbs_count,
                         phys_dev_ext_props.acc_structure_props.maxPrimitiveCount);
    }
...
bool StatelessValidation::ValidateAccelerationStructureBuildGeometryInfoKHR(
    ...
    if (info.type == VK_ACCELERATION_STRUCTURE_TYPE_GENERIC_KHR) {
        skip |= LogError("VUID-VkAccelerationStructureBuildGeometryInfoKHR-type-03654",
                         handle,
    ...
    if (info.flags & VK_BUILD_ACCELERATION_STRUCTURE_PREFER_FAST_TRACE_BIT_KHR &&
        ...
        skip |=
            LogError("VUID-VkAccelerationStructureBuildGeometryInfoKHR-flags-03796",
                     handle,
    ...
    if (info.pGeometries && info.ppGeometries) {
        skip |= LogError("VUID-VkAccelerationStructureBuildGeometryInfoKHR-pGeometries-03788",
                         handle,
                         info_loc,
    ...
    if (info.type == VK_ACCELERATION_STRUCTURE_TYPE_TOP_LEVEL_KHR && info.geometryCount != 1) {
        skip |= LogError("VUID-VkAccelerationStructureBuildGeometryInfoKHR-type-03790",
                         handle,
                         info_loc.dot(Field::type),
    ...
    if (info.type == VK_ACCELERATION_STRUCTURE_TYPE_BOTTOM_LEVEL_KHR &&
        ...
        skip |= LogError("VUID-VkAccelerationStructureBuildGeometryInfoKHR-type-03793",
                         handle,
                         info_loc.dot(Field::type),
                         "is VK_ACCELERATION_STRUCTURE_TYPE_BOTTOM_LEVEL_KHR but geometryCount (%" PRIu32
                         ") is greater than maxGeometryCount (%" PRIu64 ").",
                         info.geometryCount,
                         phys_dev_ext_props.acc_structure_props.maxGeometryCount);
    }
    ...
    if (info.geometryCount > 0 && !info.pGeometries && !info.ppGeometries) {
        skip |= LogError("VUID-VkAccelerationStructureBuildGeometryInfoKHR-pGeometries-03788",
                         ...
                         "is (%" PRIu32 ") but both pGeometries and ppGeometries are both NULL.",
                         info.geometryCount);
        return skip;
    }
    ...
    for (uint32_t geom_i = 0; geom_i < info.geometryCount; ++geom_i) {
        ...
        skip |= ValidateRangedEnum(geometry_ptr_loc.dot(Field::geometryType),
                                   vvl::Enum::VkGeometryTypeKHR,
                                   geom.geometryType,
        ...
        if (geom.geometryType == VK_GEOMETRY_TYPE_TRIANGLES_KHR) {
            ...
            if (geom.geometry.triangles.vertexStride > vvl::kU32Max) {
                skip |= LogError("VUID-VkAccelerationStructureGeometryTrianglesDataKHR-vertexStride-03819",
                                 handle,
                                 triangles_loc.dot(Field::vertexStride),
            ...
            if (geom.geometry.triangles.indexType != VK_INDEX_TYPE_UINT16 &&
                ...
                skip |= LogError("VUID-VkAccelerationStructureGeometryTrianglesDataKHR-indexType-03798",
                                 handle,
                                 triangles_loc.dot(Field::indexType),
                                 "is %s.",
            ...
            if (geom.geometry.aabbs.stride % 8) {
                skip |= LogError("VUID-VkAccelerationStructureGeometryAabbsDataKHR-stride-03545",
                                 handle,
                                 aabbs_loc.dot(Field::stride),
                                 "(%" PRIu64 ") is not a multiple of 8.",
                                 geom.geometry.aabbs.stride);
            }
            if (geom.geometry.aabbs.stride > vvl::kU32Max) {
                skip |= LogError("VUID-VkAccelerationStructureGeometryAabbsDataKHR-stride-03820",
                                 handle,
                                 aabbs_loc.dot(Field::stride),
                                 "(%" PRIu64 ") must be less than or equal to 2^32-1.",
                                 geom.geometry.aabbs.stride);
        ...
        if (info.type == VK_ACCELERATION_STRUCTURE_TYPE_TOP_LEVEL_KHR &&
            ...
            skip |= LogError("VUID-VkAccelerationStructureBuildGeometryInfoKHR-type-03789",
                             handle,
        ...
        if (info.type == VK_ACCELERATION_STRUCTURE_TYPE_BOTTOM_LEVEL_KHR) {
            if (geom.geometryType == VK_GEOMETRY_TYPE_INSTANCES_KHR) {
                skip |= LogError("VUID-VkAccelerationStructureBuildGeometryInfoKHR-type-03791",
                                 handle,
                                 geometry_ptr_loc.dot(Field::geometryType),
            ...
            if (geom.geometryType != rt::GetGeometry(info, 0).geometryType) {
                skip |= LogError("VUID-VkAccelerationStructureBuildGeometryInfoKHR-type-03792",
                                 handle,
                                 geometry_ptr_loc.dot(Field::geometryType),
...
static void
ComputeTotalPrimitiveCountWithBuildRanges(uint32_t info_count,
                                          ...
                                          uint64_t* out_total_triangles_count,
                                          uint64_t* out_total_aabbs_count) {
...
static void ComputeTotalPrimitiveCountWithMaxPrimitivesCount(
    uint32_t info_count,
    const VkAccelerationStructureBuildGeometryInfoKHR* build_geometry_infos,
    const uint32_t* const* max_primitives,
    uint64_t* out_total_triangles_count,
    uint64_t* out_total_aabbs_count) {
    *out_total_triangles_count = 0;
    *out_total_aabbs_count = 0;
...
bool StatelessValidation::manual_PreCallValidateCmdBuildAccelerationStructuresKHR(
    ...
    if (!enabled_features.accelerationStructure) {
        skip |= LogError("VUID-vkCmdBuildAccelerationStructuresKHR-accelerationStructure-08923",
                         commandBuffer,
                         error_obj.location,
                         "accelerationStructure feature was not enabled.");
    }
    ...
    ComputeTotalPrimitiveCountWithBuildRanges(
        infoCount, pInfos, ppBuildRangeInfos, &total_triangles_count, &total_aabbs_count);
    skip |=
    ...
    for (const auto [info_i, info] : vvl::enumerate(pInfos, infoCount)) {
        ...
        if (SafeModulo(info->scratchData.deviceAddress,
            ...
            skip |= LogError("VUID-vkCmdBuildAccelerationStructuresKHR-pInfos-03710",
                             commandBuffer,
                             info_loc.dot(Field::scratchData).dot(Field::deviceAddress),
                             "(%" PRIu64
                             ") must be a multiple of minAccelerationStructureScratchOffsetAlignment (%" PRIu32 ").",
                             info->scratchData.deviceAddress,
                             phys_dev_ext_props.acc_structure_props.minAccelerationStructureScratchOffsetAlignment);
        }
        skip |= ValidateRangedEnum(info_loc.dot(Field::mode),
                                   vvl::Enum::VkBuildAccelerationStructureModeKHR,
                                   info->mode,
        ...
        for (const auto [other_info_j, other_info] : vvl::enumerate(pInfos, infoCount)) {
            ...
            if (info->dstAccelerationStructure == other_info->dstAccelerationStructure) {
                ...
                skip |= LogError("VUID-vkCmdBuildAccelerationStructuresKHR-dstAccelerationStructure-03698",
                                 objlist,
                                 info_loc.dot(Field::dstAccelerationStructure),
                                 "and pInfos[%" PRIu32 "].dstAccelerationStructure are both %s.",
                                 other_info_j,
            ...
            if (info->srcAccelerationStructure == other_info->dstAccelerationStructure) {
                ...
                skip |= LogError("VUID-vkCmdBuildAccelerationStructuresKHR-pInfos-03403",
                                 objlist,
        ...
        for (uint32_t geom_i = 0; geom_i < info->geometryCount; ++geom_i) {
            ...
            const Location p_geom_loc =
                info_loc.dot(info->pGeometries ? Field::pGeometries : Field::ppGeometries, geom_i);
            const Location p_geom_geom_loc = p_geom_loc.dot(Field::geometry);
            switch (as_geometry.geometryType) {
                case VK_GEOMETRY_TYPE_TRIANGLES_KHR: {
                    const VkDeviceSize index_buffer_alignment =
                        GetIndexAlignment(as_geometry.geometry.triangles.indexType);
                    if (SafeModulo(as_geometry.geometry.triangles.indexData.deviceAddress, index_buffer_alignment) !=
                        0) {
                        skip |= LogError(
                ...
                case VK_GEOMETRY_TYPE_AABBS_KHR: {
                    if (SafeModulo(as_geometry.geometry.aabbs.data.deviceAddress, 8) != 0) {
                        skip |=
                            LogError("VUID-vkCmdBuildAccelerationStructuresKHR-pInfos-03714",
                                     commandBuffer,
        ...
        skip |= ValidateArray(info_loc.dot(Field::geometryCount),
                              error_obj.location.dot(Field::ppBuildRangeInfos, info_i),
                              info->geometryCount,
                              &ppBuildRangeInfos[info_i],
                              false,
                              true,
                              kVUIDUndefined,
...
bool StatelessValidation::manual_PreCallValidateCmdBuildAccelerationStructuresIndirectKHR(
    ...
    if (!enabled_features.accelerationStructureIndirectBuild) {
        skip |= LogError("VUID-vkCmdBuildAccelerationStructuresIndirectKHR-accelerationStructureIndirectBuild-03650",
                         commandBuffer,
                         error_obj.location,
                         "the accelerationStructureIndirectBuild feature was not enabled.");
    }

    uint64_t total_triangles_count = 0;
    uint64_t total_aabbs_count = 0;
    ComputeTotalPrimitiveCountWithMaxPrimitivesCount(
        infoCount, pInfos, ppMaxPrimitiveCounts, &total_triangles_count, &total_aabbs_count);
    skip |=
        ValidateTotalPrimitivesCount(total_triangles_count, total_aabbs_count, error_obj.handle, error_obj.location);
    ...
    for (const auto [info_i, info] : vvl::enumerate(pInfos, infoCount)) {
        ...
        if (SafeModulo(info->scratchData.deviceAddress,
                       phys_dev_ext_props.acc_structure_props.minAccelerationStructureScratchOffsetAlignment) != 0) {
            skip |= LogError("VUID-vkCmdBuildAccelerationStructuresIndirectKHR-pInfos-03710",
                             commandBuffer,
                             info_loc.dot(Field::scratchData).dot(Field::deviceAddress),
                             "(%" PRIu64
                             ") must be a multiple of minAccelerationStructureScratchOffsetAlignment (%" PRIu32 ").",
        ...
        for (uint32_t info_k = 0; info_k < infoCount; ++info_k) {
            ...
            if (info->dstAccelerationStructure == pInfos[info_k].dstAccelerationStructure) {
                const LogObjectList objlist(commandBuffer, info->dstAccelerationStructure);
                skip |= LogError("VUID-vkCmdBuildAccelerationStructuresIndirectKHR-dstAccelerationStructure-03698",
                                 objlist,
                                 info_loc.dot(Field::dstAccelerationStructure),
                                 "and pInfos[%" PRIu32 "].dstAccelerationStructure are both %s.",
                                 info_k,
            ...
            if (info->srcAccelerationStructure == pInfos[info_k].dstAccelerationStructure) {
                const LogObjectList objlist(commandBuffer, info->srcAccelerationStructure);
                skip |= LogError("VUID-vkCmdBuildAccelerationStructuresIndirectKHR-pInfos-03403",
                                 objlist,
                                 info_loc.dot(Field::srcAccelerationStructure),
                                 "and pInfos[%" PRIu32 "].dstAccelerationStructure are both %s.",
                                 info_k,
        ...
        for (uint32_t j = 0; j < info->geometryCount; ++j) {
            if (info->pGeometries) {
                ...
                if (as_geometry.geometryType == VK_GEOMETRY_TYPE_TRIANGLES_KHR) {
                    ...
                    if (SafeModulo(as_geometry.geometry.triangles.indexData.deviceAddress, index_buffer_alignment) !=
                        0) {
                        skip |= LogError("VUID-vkCmdBuildAccelerationStructuresIndirectKHR-pInfos-03712",
                                         commandBuffer,
                                         geometry_loc.dot(Field::geometry)
                                             .dot(Field::triangles)
                                             .dot(Field::indexData)
        ...
        if (SafeModulo(pIndirectDeviceAddresses[info_i], 4) != 0) {
            skip |= LogError("VUID-vkCmdBuildAccelerationStructuresIndirectKHR-pIndirectDeviceAddresses-03648",
                             commandBuffer,
                             error_obj.location.dot(Field::pIndirectStrides, info_i),
                             "is 0x%" PRIx64 ".",
                             pIndirectDeviceAddresses[info_i]);
        }

        if (SafeModulo(pIndirectStrides[info_i], 4) != 0) {
            skip |= LogError("VUID-vkCmdBuildAccelerationStructuresIndirectKHR-pIndirectStrides-03787",
                             commandBuffer,
                             error_obj.location.dot(Field::pIndirectStrides, info_i),
                             "is %" PRIu32 ".",
...
bool StatelessValidation::manual_PreCallValidateBuildAccelerationStructuresKHR(
    ...
    bool skip = false;
    if (!enabled_features.accelerationStructureHostCommands) {
        skip |= LogError("VUID-vkBuildAccelerationStructuresKHR-accelerationStructureHostCommands-03581",
                         device,
                         error_obj.location,
                         "accelerationStructureHostCommands feature was not enabled.");
    }
    ...
    for (const auto [info_i, info] : vvl::enumerate(pInfos, infoCount)) {
        ...
        if (info->mode == VK_BUILD_ACCELERATION_STRUCTURE_MODE_BUILD_KHR) {
            if (info->scratchData.hostAddress == nullptr) {
                skip |= LogError("VUID-vkBuildAccelerationStructuresKHR-pInfos-03725",
                                 device,
                                 info_loc.dot(Field::scratchData).dot(Field::hostAddress),
                                 "is NULL.");
            }
        } else if (info->mode == VK_BUILD_ACCELERATION_STRUCTURE_MODE_UPDATE_KHR) {
            if (info->scratchData.hostAddress == nullptr) {
                skip |= LogError("VUID-vkBuildAccelerationStructuresKHR-pInfos-03726",
                                 device,
                                 info_loc.dot(Field::scratchData).dot(Field::hostAddress),
                                 "is NULL.");
            }
            ...
            if (info->srcAccelerationStructure == VK_NULL_HANDLE) {
                skip |= LogError("VUID-vkBuildAccelerationStructuresKHR-pInfos-04630",
                                 device,
                                 info_loc.dot(Field::mode),
            ...
            switch (geom.geometryType) {
                case VK_GEOMETRY_TYPE_TRIANGLES_KHR: {
                    if (geom.geometry.triangles.vertexData.hostAddress == nullptr) {
                        skip |= LogError("VUID-vkBuildAccelerationStructuresKHR-pInfos-03771",
                                         device,
                                         info_loc.dot(Field::triangles).dot(Field::vertexData).dot(Field::hostAddress),
                                         "is NULL.");
                    }
                    if (geom.geometry.triangles.indexType != VK_INDEX_TYPE_NONE_KHR) {
                        if (geom.geometry.triangles.indexData.hostAddress == nullptr) {
                            skip |=
                                LogError("VUID-vkBuildAccelerationStructuresKHR-pInfos-03772",
                                         device,
                                         info_loc.dot(Field::triangles).dot(Field::indexData).dot(Field::hostAddress),
                                         "is NULL.");
                        }
                    }
                    break;
                ...
                case VK_GEOMETRY_TYPE_AABBS_KHR: {
                    if (geom.geometry.aabbs.data.hostAddress == nullptr) {
                        skip |= LogError("VUID-vkBuildAccelerationStructuresKHR-pInfos-03774",
                                         device,
                                         info_loc.dot(Field::aabbs).dot(Field::data).dot(Field::hostAddress),
                                         "is NULL.");
                    }
                    break;
                }
                case VK_GEOMETRY_TYPE_INSTANCES_KHR: {
                    if (geom.geometry.instances.data.hostAddress == nullptr) {
                        skip |= LogError("VUID-vkBuildAccelerationStructuresKHR-pInfos-03778",
                                         device,
                                         info_loc.dot(Field::instances).dot(Field::data).dot(Field::hostAddress),
                                         "is NULL.");
                    }
                    break;
                }
        ...
        for (uint32_t info_k = 0; info_k < infoCount; ++info_k) {
            if (info_i == info_k)
                continue;
            if (info->dstAccelerationStructure == pInfos[info_k].dstAccelerationStructure) {
                skip |= LogError("VUID-vkBuildAccelerationStructuresKHR-dstAccelerationStructure-03698",
                                 device,
                                 info_loc.dot(Field::dstAccelerationStructure),
                                 "and pInfos[%" PRIu32 "].dstAccelerationStructure are both %s.",
                                 info_k,
            ...
            if (info->srcAccelerationStructure == pInfos[info_k].dstAccelerationStructure) {
                skip |= LogError("VUID-vkBuildAccelerationStructuresKHR-pInfos-03403",
                                 device,
                                 info_loc.dot(Field::srcAccelerationStructure),
                                 "and pInfos[%" PRIu32 "].dstAccelerationStructure are both %s.",
                                 info_k,
...
bool StatelessValidation::manual_PreCallValidateGetAccelerationStructureBuildSizesKHR(
    ...
    uint64_t total_aabbs_count = 0;
    ComputeTotalPrimitiveCountWithMaxPrimitivesCount(
        1, pBuildInfo, &pMaxPrimitiveCounts, &total_triangles_count, &total_aabbs_count);
    skip |=
        ValidateTotalPrimitivesCount(total_triangles_count, total_aabbs_count, error_obj.handle, error_obj.location);

    skip |= ValidateAccelerationStructureBuildGeometryInfoKHR(
        *pBuildInfo, error_obj.handle, error_obj.location.dot(Field::pBuildInfo, 0));
    if (!enabled_features.accelerationStructure) {
        skip |= LogError("VUID-vkGetAccelerationStructureBuildSizesKHR-accelerationStructure-08933",
                         device,
                         error_obj.location,
                         "accelerationStructure feature was not enabled.");
    }
    if (pBuildInfo) {
        if (pBuildInfo->geometryCount != 0 && !pMaxPrimitiveCounts) {
            skip |= LogError("VUID-vkGetAccelerationStructureBuildSizesKHR-pBuildInfo-03619",
                             device,
                             error_obj.location.dot(Field::pBuildInfo).dot(Field::geometryCount),
                             "is %" PRIu32 ", but pMaxPrimitiveCounts is NULL.",
                    ...
                    case VK_GEOMETRY_TYPE_INSTANCES_KHR: {
                        if (pMaxPrimitiveCounts[geom_i] > phys_dev_ext_props.acc_structure_props.maxInstanceCount) {
                            ...
                            skip |= LogError(
                                ...
                                ") is larger than VkPhysicalDeviceAccelerationStructurePropertiesKHR::maxInstanceCount "
                                "(%" PRIu64 ").",
                                string_VkGeometryTypeKHR(geom.geometryType),
                                geom_i,
                                pMaxPrimitiveCounts[geom_i],
...
bool StatelessValidation::ValidateTraceRaysRaygenShaderBindingTable(
    ...
    if (raygen_shader_binding_table.size != raygen_shader_binding_table.stride) {
        ...
        skip |= LogError(vuid,
                         commandBuffer,
                         table_loc.dot(Field::size),
                         "(%" PRIu64 ") is not equal to stride (%" PRIu64 ").",
                         raygen_shader_binding_table.size,
                         raygen_shader_binding_table.stride);
    }

    if (SafeModulo(raygen_shader_binding_table.deviceAddress,
                   phys_dev_ext_props.ray_tracing_props_khr.shaderGroupBaseAlignment) != 0) {
...
bool StatelessValidation::ValidateTraceRaysMissShaderBindingTable(
    VkCommandBuffer commandBuffer,
    ...
    if (SafeModulo(miss_shader_binding_table.stride, props.shaderGroupHandleAlignment) != 0) {
        const char* vuid =
            indirect ? "VUID-vkCmdTraceRaysIndirectKHR-stride-03686" : "VUID-vkCmdTraceRaysKHR-stride-03686";
        skip |= LogError(vuid,
                         commandBuffer,
                         table_loc.dot(Field::stride),
                         "(%" PRIu64 ") must be a multiple of "
                         "VkPhysicalDeviceRayTracingPipelinePropertiesKHR::shaderGroupHandleAlignment (%" PRIu32 ").",
                         miss_shader_binding_table.stride,
                         props.shaderGroupHandleAlignment);
    }
    if (miss_shader_binding_table.stride > props.maxShaderGroupStride) {
        const char* vuid =
            indirect ? "VUID-vkCmdTraceRaysIndirectKHR-stride-04029" : "VUID-vkCmdTraceRaysKHR-stride-04029";
        skip |= LogError(
            vuid,
            commandBuffer,
            table_loc.dot(Field::stride),
    ...
    if (SafeModulo(miss_shader_binding_table.deviceAddress, props.shaderGroupBaseAlignment) != 0) {
        ...
        skip |= LogError(vuid,
                         ...
                         "(%" PRIu64 ") must be a multiple of "
                         "VkPhysicalDeviceRayTracingPipelinePropertiesKHR::shaderGroupBaseAlignment (%" PRIu32 ").",
                         miss_shader_binding_table.deviceAddress,
...
bool StatelessValidation::ValidateTraceRaysHitShaderBindingTable(
    VkCommandBuffer commandBuffer,
    ...
    if (SafeModulo(hit_shader_binding_table.stride, props.shaderGroupHandleAlignment) != 0) {
        const char* vuid =
            indirect ? "VUID-vkCmdTraceRaysIndirectKHR-stride-03690" : "VUID-vkCmdTraceRaysKHR-stride-03690";
        skip |= LogError(vuid,
                         commandBuffer,
                         table_loc.dot(Field::stride),
                         "(%" PRIu64 ") must be a multiple of "
                         "VkPhysicalDeviceRayTracingPipelinePropertiesKHR::shaderGroupHandleAlignment (%" PRIu32 ").",
                         hit_shader_binding_table.stride,
                         props.shaderGroupHandleAlignment);
    }
    if (hit_shader_binding_table.stride > props.maxShaderGroupStride) {
        const char* vuid =
            indirect ? "VUID-vkCmdTraceRaysIndirectKHR-stride-04035" : "VUID-vkCmdTraceRaysKHR-stride-04035";
        skip |= LogError(vuid,
                         commandBuffer,
                         table_loc.dot(Field::stride),
                         "(%" PRIu64 ") must be less than or equal to "
    ...
    if (SafeModulo(hit_shader_binding_table.deviceAddress, props.shaderGroupBaseAlignment) != 0) {
        ...
        skip |= LogError(vuid,
                         ...
                         "(%" PRIu64 ") must be a multiple of "
                         "VkPhysicalDeviceRayTracingPipelinePropertiesKHR::shaderGroupBaseAlignment (%" PRIu32 ").",
                         hit_shader_binding_table.deviceAddress,
                         props.shaderGroupBaseAlignment);
    }
...
bool StatelessValidation::ValidateTraceRaysCallableShaderBindingTable(
    ...
    if (SafeModulo(callable_shader_binding_table.stride, props.shaderGroupHandleAlignment) != 0) {
        const char* vuid =
            indirect ? "VUID-vkCmdTraceRaysIndirectKHR-stride-03694" : "VUID-vkCmdTraceRaysKHR-stride-03694";
        skip |= LogError(vuid,
                         commandBuffer,
                         table_loc.dot(Field::stride),
                         "(%" PRIu64 ") must be a multiple of "
                         "VkPhysicalDeviceRayTracingPipelinePropertiesKHR::shaderGroupHandleAlignment (%" PRIu32 ").",
                         callable_shader_binding_table.stride,
                         props.shaderGroupHandleAlignment);
    }

    if (callable_shader_binding_table.stride > props.maxShaderGroupStride) {
        const char* vuid =
            indirect ? "VUID-vkCmdTraceRaysIndirectKHR-stride-04041" : "VUID-vkCmdTraceRaysKHR-stride-04041";
        skip |= LogError(vuid,
                         commandBuffer,
                         table_loc.dot(Field::stride),
                         "(%" PRIu64 ") must be less than or equal to "
    ...
    if (SafeModulo(callable_shader_binding_table.deviceAddress, props.shaderGroupBaseAlignment) != 0) {
        ...
        skip |= LogError(vuid,
                         ...
                         "(%" PRIu64 ") must be a multiple of "
                         "VkPhysicalDeviceRayTracingPipelinePropertiesKHR::shaderGroupBaseAlignment (%" PRIu32 ").",
                         callable_shader_binding_table.deviceAddress,
                         props.shaderGroupBaseAlignment);
    }
...
bool StatelessValidation::manual_PreCallValidateCmdTraceRaysKHR(
    VkCommandBuffer commandBuffer,
    ...
    if (pRaygenShaderBindingTable) {
        skip |= ValidateTraceRaysRaygenShaderBindingTable(
            commandBuffer, *pRaygenShaderBindingTable, error_obj.location.dot(Field::pRaygenShaderBindingTable));
    }
    if (pMissShaderBindingTable) {
        skip |= ValidateTraceRaysMissShaderBindingTable(
            commandBuffer, *pMissShaderBindingTable, error_obj.location.dot(Field::pMissShaderBindingTable));
    }
    if (pHitShaderBindingTable) {
        skip |= ValidateTraceRaysHitShaderBindingTable(
            commandBuffer, *pHitShaderBindingTable, error_obj.location.dot(Field::pHitShaderBindingTable));
    }
    if (pCallableShaderBindingTable) {
        skip |= ValidateTraceRaysCallableShaderBindingTable(
            commandBuffer, *pCallableShaderBindingTable, error_obj.location.dot(Field::pCallableShaderBindingTable));
    }

    if (width * depth * height > phys_dev_ext_props.ray_tracing_props_khr.maxRayDispatchInvocationCount) {
        skip |=
            LogError("VUID-vkCmdTraceRaysKHR-width-03641",
                     commandBuffer,
                     error_obj.location,
                     "width x height x depth (%" PRIu32 " x %" PRIu32 " x %" PRIu32 ") must be less than or equal to "
                     "VkPhysicalDeviceRayTracingPipelinePropertiesKHR::maxRayDispatchInvocationCount (%" PRIu32 ").",
                     width,
                     depth,
                     height,
                     phys_dev_ext_props.ray_tracing_props_khr.maxRayDispatchInvocationCount);
    }
    if (width > device_limits.maxComputeWorkGroupCount[0] * device_limits.maxComputeWorkGroupSize[0]) {
        skip |=
            LogError("VUID-vkCmdTraceRaysKHR-width-03638",
                     commandBuffer,
                     error_obj.location.dot(Field::width),
                     "(%" PRIu32 ") must be less than or equal to VkPhysicalDeviceLimits::maxComputeWorkGroupCount[0] "
...
bool StatelessValidation::manual_PreCallValidateCmdTraceRaysIndirectKHR(
    ...
    if (SafeModulo(indirectDeviceAddress, 4) != 0) {
        skip |= LogError("VUID-vkCmdTraceRaysIndirectKHR-indirectDeviceAddress-03634",
                         commandBuffer,
                         error_obj.location.dot(Field::indirectDeviceAddress),
                         "(%" PRIu64 ") must be a multiple of 4.",
...
bool StatelessValidation::manual_PreCallValidateCreateMicromapEXT(VkDevice device,
    ...
    if (!enabled_features.micromap) {
        skip |= LogError(
            "VUID-vkCreateMicromapEXT-micromap-07430", device, error_obj.location, "micromap feature was not enabled.");
    }

    if ((pCreateInfo->deviceAddress != 0ULL) && !enabled_features.micromapCaptureReplay) {
        skip |= LogError("VUID-vkCreateMicromapEXT-deviceAddress-07431",
                         device,
                         error_obj.location,
...
bool StatelessValidation::manual_PreCallValidateDestroyMicromapEXT(VkDevice device,
    ...
    if (!enabled_features.micromap) {
        skip |= LogError("VUID-vkDestroyMicromapEXT-micromap-10382",
                         device,
                         error_obj.location,
...
bool StatelessValidation::manual_PreCallValidateCmdBuildMicromapsEXT(VkCommandBuffer commandBuffer,
                                                                     uint32_t infoCount,
    ...
    for (const auto [info_i, info] : vvl::enumerate(pInfos, infoCount)) {
        ...
        if (SafeModulo(info->scratchData.deviceAddress,
                       phys_dev_ext_props.acc_structure_props.minAccelerationStructureScratchOffsetAlignment) != 0) {
            skip |= LogError("VUID-vkCmdBuildMicromapsEXT-pInfos-07514",
                             commandBuffer,
                             info_loc.dot(Field::scratchData).dot(Field::deviceAddress),
                             "(%" PRIu64
                             ") must be a multiple of minAccelerationStructureScratchOffsetAlignment (%" PRIu32 ").",
        ...
        if (SafeModulo(info->triangleArray.deviceAddress, 256) != 0) {
            skip |= LogError("VUID-vkCmdBuildMicromapsEXT-pInfos-07515",
                             commandBuffer,
        ...
        if (info->pUsageCounts && info->ppUsageCounts) {
            skip |= LogError("VUID-VkMicromapBuildInfoEXT-pUsageCounts-07516",
                             commandBuffer,
                             info_loc,
...
bool StatelessValidation::manual_PreCallValidateBuildMicromapsEXT(VkDevice device,
                                                                  VkDeferredOperationKHR deferredOperation,
                                                                  uint32_t infoCount,
                                                                  const VkMicromapBuildInfoEXT* pInfos,
                                                                  const ErrorObject& error_obj) const {
    ...
    if (!enabled_features.micromapHostCommands) {
        skip |= LogError("VUID-vkBuildMicromapsEXT-micromapHostCommands-07555",
                         device,
                         error_obj.location,
...
bool StatelessValidation::manual_PreCallValidateCopyMicromapEXT(VkDevice device,
                                                                VkDeferredOperationKHR deferredOperation,
    ...
    if (!enabled_features.micromapHostCommands) {
        skip |= LogError("VUID-vkCopyMicromapEXT-micromapHostCommands-07560",
                         device,
                         error_obj.location,
                         "micromapHostCommands feature was not enabled.");
    }
    ...
    if (pInfo->mode != VK_COPY_MICROMAP_MODE_COMPACT_EXT && pInfo->mode != VK_COPY_MICROMAP_MODE_CLONE_EXT) {
        skip |= LogError("VUID-VkCopyMicromapInfoEXT-mode-07531",
                         device,
                         info_loc.dot(Field::mode),
                         "is %s.",
...
bool StatelessValidation::manual_PreCallValidateCopyMicromapToMemoryEXT(VkDevice device,
                                                                        VkDeferredOperationKHR deferredOperation,
    ...
    if (!enabled_features.micromapHostCommands) {
        skip |= LogError("VUID-vkCopyMicromapToMemoryEXT-micromapHostCommands-07571",
                         device,
                         error_obj.location,
                         "micromapHostCommands feature was not enabled.");
    }
    ...
    if (pInfo->mode != VK_COPY_MICROMAP_MODE_SERIALIZE_EXT) {
        skip |= LogError("VUID-VkCopyMicromapToMemoryInfoEXT-mode-07542",
                         device,
                         info_loc.dot(Field::mode),
                         "is %s.",
                         string_VkCopyMicromapModeEXT(pInfo->mode));
...
bool StatelessValidation::manual_PreCallValidateCopyMemoryToMicromapEXT(VkDevice device,
                                                                        VkDeferredOperationKHR deferredOperation,
    ...
    if (!enabled_features.micromapHostCommands) {
        skip |= LogError("VUID-vkCopyMemoryToMicromapEXT-micromapHostCommands-07566",
                         device,
                         error_obj.location,
                         "micromapHostCommands feature was not enabled.");
    }
    ...
    if (pInfo->mode != VK_COPY_MICROMAP_MODE_DESERIALIZE_EXT) {
        skip |= LogError("VUID-VkCopyMemoryToMicromapInfoEXT-mode-07548",
                         device,
                         info_loc.dot(Field::mode),
                         "is %s.",
                         string_VkCopyMicromapModeEXT(pInfo->mode));
...
bool StatelessValidation::manual_PreCallValidateWriteMicromapsPropertiesEXT(VkDevice device,
    ...
    if (queryType != VK_QUERY_TYPE_MICROMAP_COMPACTED_SIZE_EXT &&
        ...
        skip |= LogError("VUID-vkWriteMicromapsPropertiesEXT-queryType-07503",
                         device,
                         error_obj.location,
                         "is %s.",
                         string_VkQueryType(queryType));
...
bool StatelessValidation::manual_PreCallValidateCmdWriteMicromapsPropertiesEXT(VkCommandBuffer commandBuffer,
                                                                               uint32_t micromapCount,
                                                                               const VkMicromapEXT* pMicromaps,
                                                                               VkQueryType queryType,
                                                                               VkQueryPool queryPool,
    ...
    if (queryType != VK_QUERY_TYPE_MICROMAP_COMPACTED_SIZE_EXT &&
        queryType != VK_QUERY_TYPE_MICROMAP_SERIALIZATION_SIZE_EXT) {
        skip |= LogError("VUID-vkCmdWriteMicromapsPropertiesEXT-queryType-07503",
                         commandBuffer,
                         error_obj.location,
                         "is %s.",
                         string_VkQueryType(queryType));
...
bool StatelessValidation::manual_PreCallValidateGetDeviceMicromapCompatibilityEXT(
    ...
    if (!enabled_features.micromap) {
        skip |= LogError("VUID-vkGetDeviceMicromapCompatibilityEXT-micromap-07551",
                         device,
                         error_obj.location,
                         "micromap feature was not enabled.");
    }
...
bool StatelessValidation::manual_PreCallValidateGetMicromapBuildSizesEXT(VkDevice device,
    ...
    if (!enabled_features.micromap) {
        skip |= LogError("VUID-vkGetMicromapBuildSizesEXT-micromap-07439",
                         device,
                         error_obj.location,
                         "micromap feature was not enabled.");
    }
