...
bool StatelessValidation::ValidateCoarseSampleOrderCustomNV(const VkCoarseSampleOrderCustomNV& order,
    ...
    if (sample_order_info == nullptr) {
        skip |= LogError("VUID-VkCoarseSampleOrderCustomNV-shadingRate-02073",
                         device,
                         order_loc,
    ...
    if (order.sampleCount == 0 || (order.sampleCount & (order.sampleCount - 1)) ||
        ...
        skip |= LogError("VUID-VkCoarseSampleOrderCustomNV-sampleCount-02074",
                         device,
                         order_loc.dot(Field::sampleCount),
                         "(%" PRIu32
    ...
    if (order.sampleLocationCount != order.sampleCount * sample_order_info->width * sample_order_info->height) {
        skip |= LogError("VUID-VkCoarseSampleOrderCustomNV-sampleLocationCount-02075",
                         device,
                         ...
                         order.sampleLocationCount,
                         order.sampleCount,
                         sample_order_info->width,
                         sample_order_info->height);
    }

    if (order.sampleLocationCount > phys_dev_ext_props.shading_rate_image_props.shadingRateMaxCoarseSamples) {
        skip |= LogError("VUID-VkCoarseSampleOrderCustomNV-sampleLocationCount-02076",
                         device,
                         order_loc.dot(Field::sampleLocationCount),
                         "(%" PRIu32 ") must be less than or equal to shadingRateMaxCoarseSamples (%" PRIu32 ").",
                         order.sampleLocationCount,
                         phys_dev_ext_props.shading_rate_image_props.shadingRateMaxCoarseSamples);
    }
    ...
    for (uint32_t i = 0; i < order.sampleLocationCount; ++i) {
        ...
        if (sample_loc->pixelX >= sample_order_info->width) {
            skip |=
                LogError("VUID-VkCoarseSampleLocationNV-pixelX-02078",
                         device,
                         order_loc,
        ...
        if (sample_loc->pixelY >= sample_order_info->height) {
            skip |=
                LogError("VUID-VkCoarseSampleLocationNV-pixelY-02079",
                         device,
                         order_loc,
        ...
        uint32_t idx = sample_loc->sample +
                       order.sampleCount * (sample_loc->pixelX + sample_order_info->width * sample_loc->pixelY);
    ...
    if (sample_locations_mask != expected_mask) {
        skip |= LogError("VUID-VkCoarseSampleOrderCustomNV-pSampleLocations-02077",
                         device,
                         order_loc,
                         "The array pSampleLocations must contain exactly one entry for "
                         "every combination of valid values for pixelX, pixelY, and sample in the structure "
...
bool StatelessValidation::ValidateSamplerFilterMinMax(const VkSamplerCreateInfo& create_info,
    ...
    if (sampler_reduction->reductionMode != VK_SAMPLER_REDUCTION_MODE_WEIGHTED_AVERAGE) {
        if ((api_version >= VK_API_VERSION_1_2) && !enabled_features.samplerFilterMinmax) {
            skip |= LogError("VUID-VkSamplerCreateInfo-pNext-06726",
                             device,
                   ...
                   !IsExtEnabled(device_extensions.vk_ext_sampler_filter_minmax)) {
            // NOTE: technically this VUID is only if the corresponding _feature_ is not enabled, and only if on
            // api_version
            // >= 1.2, but there doesn't appear to be a similar VUID for when api_version < 1.2
            skip |= LogError("VUID-VkSamplerCreateInfo-pNext-06726",
                             device,
                             create_info_loc.pNext(Struct::VkSamplerReductionModeCreateInfo, Field::reductionMode),
                             "is %s, but extension %s is not enabled.",
                             string_VkSamplerReductionMode(sampler_reduction->reductionMode),
        ...
        if (vku::FindStructInPNextChain<VkSamplerYcbcrConversionInfo>(create_info.pNext)) {
            skip |=
                LogError("VUID-VkSamplerCreateInfo-None-01647",
                         device,
                         create_info_loc.pNext(Struct::VkSamplerReductionModeCreateInfo, Field::reductionMode),
        ...
        if (create_info.compareEnable == VK_TRUE) {
            skip |= LogError("VUID-VkSamplerCreateInfo-compareEnable-01423",
                             device,
                             create_info_loc.pNext(Struct::VkSamplerReductionModeCreateInfo, Field::reductionMode),
        ...
        if (create_info.magFilter == VK_FILTER_CUBIC_IMG || create_info.minFilter == VK_FILTER_CUBIC_IMG) {
            if (!IsExtEnabled(device_extensions.vk_ext_filter_cubic)) {
                skip |= LogError("VUID-VkSamplerCreateInfo-magFilter-07911",
                                 device,
                                 create_info_loc.pNext(Struct::VkSamplerReductionModeCreateInfo, Field::reductionMode),
                                 "is %s, magFilter is %s and minFilter is %s, but "
                                 "extension %s is not enabled.",
                                 string_VkSamplerReductionMode(sampler_reduction->reductionMode),
                                 string_VkFilter(create_info.magFilter),
                                 string_VkFilter(create_info.minFilter),
...
bool StatelessValidation::ValidateSamplerCustomBoarderColor(const VkSamplerCreateInfo& create_info,
    ...
    if (create_info.borderColor == VK_BORDER_COLOR_INT_CUSTOM_EXT ||
        create_info.borderColor == VK_BORDER_COLOR_FLOAT_CUSTOM_EXT) {
        if (!enabled_features.customBorderColors) {
            skip |= LogError("VUID-VkSamplerCreateInfo-customBorderColors-04085",
                             device,
                             create_info_loc.dot(Field::borderColor),
                             "is %s but customBorderColors feature was not enabled.",
                             string_VkBorderColor(create_info.borderColor));
        }

        auto custom_create_info =
            vku::FindStructInPNextChain<VkSamplerCustomBorderColorCreateInfoEXT>(create_info.pNext);
        if (!custom_create_info) {
            skip |= LogError("VUID-VkSamplerCreateInfo-borderColor-04011",
                             device,
                             create_info_loc.dot(Field::borderColor),
            ...
            if ((custom_create_info->format != VK_FORMAT_UNDEFINED) &&
                !vkuFormatIsDepthAndStencil(custom_create_info->format) &&
                ...
                skip |= LogError("VUID-VkSamplerCustomBorderColorCreateInfoEXT-format-07605",
                                 device,
                                 create_info_loc.pNext(Struct::VkSamplerCustomBorderColorCreateInfoEXT, Field::format),
                                 "%s does not match borderColor (%s).",
                                 string_VkFormat(custom_create_info->format),
                                 string_VkBorderColor(create_info.borderColor));
            }

            if (custom_create_info->format == VK_FORMAT_UNDEFINED && !enabled_features.customBorderColorWithoutFormat) {
                skip |= LogError("VUID-VkSamplerCustomBorderColorCreateInfoEXT-format-04014",
                                 device,
                                 create_info_loc.pNext(Struct::VkSamplerCustomBorderColorCreateInfoEXT, Field::format),
...
bool StatelessValidation::ValidateSamplerSubsampled(const VkSamplerCreateInfo& create_info,
                                                    const Location& create_info_loc) const {
    bool skip = false;
    if ((create_info.flags & VK_SAMPLER_CREATE_SUBSAMPLED_BIT_EXT) == 0)
        return skip;

    if (create_info.minFilter != create_info.magFilter) {
        skip |= LogError("VUID-VkSamplerCreateInfo-flags-02574",
                         device,
    ...
    if (create_info.mipmapMode != VK_SAMPLER_MIPMAP_MODE_NEAREST) {
        skip |= LogError("VUID-VkSamplerCreateInfo-flags-02575",
                         device,
                         create_info_loc.dot(Field::flags),
                         "includes VK_SAMPLER_CREATE_SUBSAMPLED_BIT_EXT, but "
    ...
    if (create_info.minLod != 0.0 || create_info.maxLod != 0.0) {
        skip |= LogError("VUID-VkSamplerCreateInfo-flags-02576",
                         device,
                         create_info_loc.dot(Field::flags),
                         "includes VK_SAMPLER_CREATE_SUBSAMPLED_BIT_EXT, but "
    ...
    if (((create_info.addressModeU != VK_SAMPLER_ADDRESS_MODE_CLAMP_TO_EDGE) &&
        ...
        skip |= LogError("VUID-VkSamplerCreateInfo-flags-02577",
                         device,
                         create_info_loc.dot(Field::flags),
    ...
    if (create_info.anisotropyEnable) {
        skip |= LogError("VUID-VkSamplerCreateInfo-flags-02578",
                         device,
                         create_info_loc.dot(Field::flags),
    ...
    if (create_info.compareEnable) {
        skip |= LogError("VUID-VkSamplerCreateInfo-flags-02579",
                         device,
                         create_info_loc.dot(Field::flags),
                         "includes VK_SAMPLER_CREATE_SUBSAMPLED_BIT_EXT, "
    ...
    if (create_info.unnormalizedCoordinates) {
        skip |= LogError("VUID-VkSamplerCreateInfo-flags-02580",
                         device,
                         create_info_loc.dot(Field::flags),
                         "includes VK_SAMPLER_CREATE_SUBSAMPLED_BIT_EXT, "
...
bool StatelessValidation::ValidateSamplerImageProcessingQCOM(const VkSamplerCreateInfo& create_info,
                                                             const Location& create_info_loc) const {
    bool skip = false;
    if ((create_info.flags & VK_SAMPLER_CREATE_IMAGE_PROCESSING_BIT_QCOM) == 0)
        return skip;

    if ((create_info.minFilter != VK_FILTER_NEAREST) || (create_info.magFilter != VK_FILTER_NEAREST)) {
        skip |= LogError("VUID-VkSamplerCreateInfo-flags-06964",
                         device,
                         create_info_loc.dot(Field::flags),
    ...
    if (create_info.mipmapMode != VK_SAMPLER_MIPMAP_MODE_NEAREST) {
        skip |= LogError("VUID-VkSamplerCreateInfo-flags-06965",
                         device,
                         create_info_loc.dot(Field::flags),
                         "includes VK_SAMPLER_CREATE_IMAGE_PROCESSING_BIT_QCOM, so "
    ...
    if ((create_info.minLod != 0) || (create_info.maxLod != 0)) {
        skip |= LogError("VUID-VkSamplerCreateInfo-flags-06966",
                         device,
                         create_info_loc.dot(Field::flags),
                         "includes VK_SAMPLER_CREATE_IMAGE_PROCESSING_BIT_QCOM, so "
    ...
    if (((create_info.addressModeU != VK_SAMPLER_ADDRESS_MODE_CLAMP_TO_EDGE) &&
        ...
        ((create_info.addressModeV != VK_SAMPLER_ADDRESS_MODE_CLAMP_TO_EDGE) &&
         (create_info.addressModeV != VK_SAMPLER_ADDRESS_MODE_CLAMP_TO_BORDER))) {
        skip |= LogError("VUID-VkSamplerCreateInfo-flags-06967",
                         device,
                         create_info_loc.dot(Field::flags),
                         "includes VK_SAMPLER_CREATE_IMAGE_PROCESSING_BIT_QCOM, so "
    ...
    if (((create_info.addressModeU == VK_SAMPLER_ADDRESS_MODE_CLAMP_TO_BORDER) ||
        ...
        skip |= LogError("VUID-VkSamplerCreateInfo-flags-06968",
                         device,
                         create_info_loc.dot(Field::flags),
    ...
    if (create_info.anisotropyEnable) {
        skip |= LogError("VUID-VkSamplerCreateInfo-flags-06969",
                         device,
                         create_info_loc.dot(Field::flags),
                         "includes VK_SAMPLER_CREATE_IMAGE_PROCESSING_BIT_QCOM, "
                         "but anisotropyEnable is VK_TRUE.");
    ...
    if (create_info.compareEnable) {
        skip |= LogError("VUID-VkSamplerCreateInfo-flags-06970",
                         device,
                         create_info_loc.dot(Field::flags),
                         "includes VK_SAMPLER_CREATE_IMAGE_PROCESSING_BIT_QCOM, "
                         "but compareEnable is VK_TRUE.");
...
bool StatelessValidation::manual_PreCallValidateCreateSampler(VkDevice device,
    ...
    if (pCreateInfo->anisotropyEnable == VK_TRUE) {
        if (!IsBetweenInclusive(pCreateInfo->maxAnisotropy, 1.0F, limits.maxSamplerAnisotropy)) {
            skip |= LogError("VUID-VkSamplerCreateInfo-anisotropyEnable-01071",
                             device,
                             create_info_loc.dot(Field::maxAnisotropy),
                             "is %f but must be in the range of [1.0, %f] (maxSamplerAnistropy).",
        ...
        if (enabled_features.samplerAnisotropy == VK_FALSE) {
            skip |= LogError("VUID-VkSamplerCreateInfo-anisotropyEnable-01070",
                             device,
    ...
    if (pCreateInfo->unnormalizedCoordinates == VK_TRUE) {
        if (pCreateInfo->minFilter != pCreateInfo->magFilter) {
            skip |= LogError("VUID-VkSamplerCreateInfo-unnormalizedCoordinates-01072",
                             device,
        ...
        if (pCreateInfo->mipmapMode != VK_SAMPLER_MIPMAP_MODE_NEAREST) {
            skip |= LogError("VUID-VkSamplerCreateInfo-unnormalizedCoordinates-01073",
                             device,
                             create_info_loc.dot(Field::unnormalizedCoordinates),
        ...
        if (pCreateInfo->minLod != 0.0f || pCreateInfo->maxLod != 0.0f) {
            skip |= LogError("VUID-VkSamplerCreateInfo-unnormalizedCoordinates-01074",
                             device,
                             create_info_loc.dot(Field::unnormalizedCoordinates),
                             "is VK_TRUE, but minLod (%f) and maxLod (%f) must both be zero.",
                             pCreateInfo->minLod,
                             pCreateInfo->maxLod);
        }
        if ((pCreateInfo->addressModeU != VK_SAMPLER_ADDRESS_MODE_CLAMP_TO_EDGE &&
            ...
            skip |= LogError("VUID-VkSamplerCreateInfo-unnormalizedCoordinates-01075",
                             device,
                             create_info_loc.dot(Field::unnormalizedCoordinates),
        ...
        if (pCreateInfo->anisotropyEnable == VK_TRUE) {
            skip |= LogError("VUID-VkSamplerCreateInfo-unnormalizedCoordinates-01076",
                             device,
                             create_info_loc,
                             "anisotropyEnable and unnormalizedCoordinates are both VK_TRUE.");
        }
        if (pCreateInfo->compareEnable == VK_TRUE) {
            skip |= LogError("VUID-VkSamplerCreateInfo-unnormalizedCoordinates-01077",
                             device,
                             create_info_loc,
    ...
    if (pCreateInfo->compareEnable == VK_TRUE) {
        skip |= ValidateRangedEnum(create_info_loc.dot(Field::compareOp),
                                   vvl::Enum::VkCompareOp,
    ...
    if ((pCreateInfo->addressModeU == VK_SAMPLER_ADDRESS_MODE_CLAMP_TO_BORDER) ||
        ...
        skip |= ValidateRangedEnum(create_info_loc.dot(Field::borderColor),
                                   vvl::Enum::VkBorderColor,
    ...
    if (enabled_features.samplerMirrorClampToEdge == VK_FALSE) {
        ...
        if (pCreateInfo->addressModeU == VK_SAMPLER_ADDRESS_MODE_MIRROR_CLAMP_TO_EDGE) {
            skip |= LogError(
                ...
                device,
                create_info_loc.dot(Field::addressModeU),
            ...
            skip |= LogError(
                "VUID-VkSamplerCreateInfo-addressModeU-01079",
                device,
                create_info_loc.dot(Field::addressModeV),
                "is VK_SAMPLER_ADDRESS_MODE_MIRROR_CLAMP_TO_EDGE but the "
                "VK_KHR_sampler_mirror_clamp_to_edge extension or samplerMirrorClampToEdge feature was not enabled.");
            ...
            skip |= LogError(
                "VUID-VkSamplerCreateInfo-addressModeU-01079",
                device,
                create_info_loc.dot(Field::addressModeW),
                "is VK_SAMPLER_ADDRESS_MODE_MIRROR_CLAMP_TO_EDGE but the "
                "VK_KHR_sampler_mirror_clamp_to_edge extension or samplerMirrorClampToEdge feature was not enabled.");
        }
    }
    ...
    if (IsExtEnabled(device_extensions.vk_img_filter_cubic)) {
        if ((pCreateInfo->anisotropyEnable == VK_TRUE) &&
            ...
            skip |= LogError("VUID-VkSamplerCreateInfo-magFilter-01081",
                             device,
                             create_info_loc.dot(Field::anisotropyEnable),
    ...
    if (pCreateInfo->minLod > pCreateInfo->maxLod) {
        skip |= LogError("VUID-VkSamplerCreateInfo-maxLod-01973",
                         device,
                         create_info_loc.dot(Field::minLod),
                         "(%f) is greater than maxLod (%f)",
                         pCreateInfo->minLod,
                         pCreateInfo->maxLod);
    }
    ...
    if (pCreateInfo->mipLodBias > limits.maxSamplerLodBias) {
        skip |= LogError("VUID-VkSamplerCreateInfo-mipLodBias-01069",
                         device,
                         create_info_loc.dot(Field::mipLodBias),
                         "(%f) is greater than maxSamplerLodBias (%f)",
    ...
    if (vku::FindStructInPNextChain<VkSamplerYcbcrConversionInfo>(pCreateInfo->pNext)) {
        if ((pCreateInfo->addressModeU != VK_SAMPLER_ADDRESS_MODE_CLAMP_TO_EDGE) ||
            ...
            skip |= LogError("VUID-VkSamplerCreateInfo-addressModeU-01646",
                             device,
    ...
    if (vku::FindStructInPNextChain<VkSamplerBorderColorComponentMappingCreateInfoEXT>(pCreateInfo->pNext)) {
        if (!enabled_features.borderColorSwizzle) {
            skip |= LogError("VUID-VkSamplerBorderColorComponentMappingCreateInfoEXT-borderColorSwizzle-06437",
                             device,
                             create_info_loc,
    ...
    if ((pCreateInfo->flags & VK_SAMPLER_CREATE_NON_SEAMLESS_CUBE_MAP_BIT_EXT) &&
        (!enabled_features.nonSeamlessCubeMap)) {
        skip |= LogError("VUID-VkSamplerCreateInfo-nonSeamlessCubeMap-06788",
                         device,
                         create_info_loc.dot(Field::flags),
    ...
    if (vku::FindStructInPNextChain<VkOpaqueCaptureDescriptorDataCreateInfoEXT>(pCreateInfo->pNext)) {
        if (!(pCreateInfo->flags & VK_SAMPLER_CREATE_DESCRIPTOR_BUFFER_CAPTURE_REPLAY_BIT_EXT)) {
            skip |= LogError("VUID-VkSamplerCreateInfo-pNext-08111",
                             device,
                             create_info_loc.dot(Field::flags),
...
bool StatelessValidation::ValidateMutableDescriptorTypeCreateInfo(
    ...
    for (uint32_t i = 0; i < create_info.bindingCount; ++i) {
        ...
        if (create_info.pBindings[i].descriptorType == VK_DESCRIPTOR_TYPE_MUTABLE_EXT) {
            if (mutable_type_count == 0) {
                skip |= LogError("VUID-VkMutableDescriptorTypeListEXT-descriptorTypeCount-04597",
                                 device,
            ...
            if (mutable_type_count > 0) {
                skip |= LogError("VUID-VkMutableDescriptorTypeListEXT-descriptorTypeCount-04599",
                                 device,
                                 binding_loc.dot(Field::descriptorType),
                                 "is %s, but "
                                 "VkMutableDescriptorTypeCreateInfoEXT::pMutableDescriptorTypeLists[%" PRIu32
        ...
        for (uint32_t k = 0; k < mutable_create_info.pMutableDescriptorTypeLists[j].descriptorTypeCount; ++k) {
            ...
            switch (mutable_create_info.pMutableDescriptorTypeLists[j].pDescriptorTypes[k]) {
                case VK_DESCRIPTOR_TYPE_MUTABLE_EXT:
                    skip |= LogError("VUID-VkMutableDescriptorTypeListEXT-pDescriptorTypes-04600",
                                     device,
                                     type_loc,
                                     "is VK_DESCRIPTOR_TYPE_MUTABLE_EXT.");
                    break;
                case VK_DESCRIPTOR_TYPE_STORAGE_BUFFER_DYNAMIC:
                    skip |= LogError("VUID-VkMutableDescriptorTypeListEXT-pDescriptorTypes-04601",
                                     device,
                                     type_loc,
                                     "is VK_DESCRIPTOR_TYPE_STORAGE_BUFFER_DYNAMIC.");
                    break;
                case VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER_DYNAMIC:
                    skip |= LogError("VUID-VkMutableDescriptorTypeListEXT-pDescriptorTypes-04602",
                                     device,
                                     type_loc,
                                     "is VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER_DYNAMIC.");
                    break;
                case VK_DESCRIPTOR_TYPE_INLINE_UNIFORM_BLOCK:
                    skip |= LogError("VUID-VkMutableDescriptorTypeListEXT-pDescriptorTypes-04603",
                                     device,
                                     type_loc,
            ...
            for (uint32_t l = k + 1; l < mutable_create_info.pMutableDescriptorTypeLists[j].descriptorTypeCount; ++l) {
                if (mutable_create_info.pMutableDescriptorTypeLists[j].pDescriptorTypes[k] ==
                    ...
                    skip |= LogError("VUID-VkMutableDescriptorTypeListEXT-pDescriptorTypes-04598",
                                     device,
                                     type_loc,
                                     "and pDescriptorTypes[%" PRIu32 "] are both %s.",
                                     l,
                                     string_VkDescriptorType(
                                         mutable_create_info.pMutableDescriptorTypeLists[j].pDescriptorTypes[k]));
                }
            }
        }
    }
...
bool StatelessValidation::ValidateDescriptorSetLayoutCreateInfo(const VkDescriptorSetLayoutCreateInfo& create_info,
    ...
    const bool has_descriptor_buffer_flag =
        (create_info.flags & VK_DESCRIPTOR_SET_LAYOUT_CREATE_DESCRIPTOR_BUFFER_BIT_EXT) != 0;
    const bool has_push_descriptor_flag =
        (create_info.flags & VK_DESCRIPTOR_SET_LAYOUT_CREATE_PUSH_DESCRIPTOR_BIT) != 0;
    // Validation for parameters excluded from the generated validation code due to a 'noautovalidity' tag in vk.xml
    if (create_info.pBindings != nullptr) {
        const auto* mutable_descriptor_type =
            vku::FindStructInPNextChain<VkMutableDescriptorTypeCreateInfoEXT>(create_info.pNext);
        for (const auto [i, binding] : vvl::enumerate(create_info.pBindings, create_info.bindingCount)) {
            ...
            if (stage_flags != 0) {
                if (stage_flags != VK_SHADER_STAGE_ALL &&
                    ((stage_flags & (~AllVkShaderStageFlagBitsExcludingStageAll)) != 0)) {
                    skip |= LogError("VUID-VkDescriptorSetLayoutBinding-descriptorCount-09465",
                                     device,
                ...
                if ((binding->descriptorType == VK_DESCRIPTOR_TYPE_INPUT_ATTACHMENT) &&
                    ...
                    skip |= LogError("VUID-VkDescriptorSetLayoutBinding-descriptorType-01510",
                                     device,
            ...
            if (binding->descriptorType == VK_DESCRIPTOR_TYPE_MUTABLE_EXT) {
                if (mutable_descriptor_type) {
                    if (i >= mutable_descriptor_type->mutableDescriptorTypeListCount) {
                        skip |= LogError("VUID-VkDescriptorSetLayoutCreateInfo-pBindings-07303",
                                         device,
                                         binding_loc.pNext(Struct::VkMutableDescriptorTypeCreateInfoEXT,
                                                           Field::mutableDescriptorTypeListCount),
                                         "(%" PRIu32 ") is less than %" PRIu32
                                         " but descriptorType is VK_DESCRIPTOR_TYPE_MUTABLE_EXT",
                                         i,
                                         mutable_descriptor_type->mutableDescriptorTypeListCount);
                    }
                } else {
                    skip |=
                        LogError("VUID-VkDescriptorSetLayoutCreateInfo-pBindings-07303",
                                 device,
                                 binding_loc.dot(Field::descriptorType),
                ...
                if (binding->pImmutableSamplers) {
                    skip |= LogError("VUID-VkDescriptorSetLayoutCreateInfo-descriptorType-04594",
                                     device,
                                     binding_loc.dot(Field::descriptorType),
                                     "is VK_DESCRIPTOR_TYPE_MUTABLE_EXT but pImmutableSamplers is not NULL.");
                }
                if (!enabled_features.mutableDescriptorType) {
                    skip |= LogError("VUID-VkDescriptorSetLayoutCreateInfo-mutableDescriptorType-04595",
                                     device,
                                     binding_loc.dot(Field::descriptorType),
                                     "is VK_DESCRIPTOR_TYPE_MUTABLE_EXT but "
            ...
            if (has_push_descriptor_flag && binding->descriptorType == VK_DESCRIPTOR_TYPE_MUTABLE_EXT) {
                skip |= LogError("VUID-VkDescriptorSetLayoutCreateInfo-flags-04591",
                                 device,
                                 binding_loc.dot(Field::descriptorType),
            ...
            if (has_descriptor_buffer_flag &&
                ((binding->descriptorType == VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER_DYNAMIC) ||
                 (binding->descriptorType == VK_DESCRIPTOR_TYPE_STORAGE_BUFFER_DYNAMIC))) {
                skip |= LogError("VUID-VkDescriptorSetLayoutCreateInfo-flags-08000",
                                 device,
                                 binding_loc.dot(Field::descriptorType),
    ...
    const bool has_host_only_pool_flag =
        (create_info.flags & VK_DESCRIPTOR_SET_LAYOUT_CREATE_HOST_ONLY_POOL_BIT_EXT) != 0;
    const bool has_update_after_bind_flag =
        (create_info.flags & VK_DESCRIPTOR_SET_LAYOUT_CREATE_UPDATE_AFTER_BIND_POOL_BIT) != 0;
    ...
    if (has_push_descriptor_flag && has_host_only_pool_flag) {
        skip |= LogError("VUID-VkDescriptorSetLayoutCreateInfo-flags-04590",
                         device,
                         create_info_loc.dot(Field::flags),
                         "is %s.",
    ...
    if (has_update_after_bind_flag && has_host_only_pool_flag) {
        skip |= LogError("VUID-VkDescriptorSetLayoutCreateInfo-flags-04592",
                         device,
                         create_info_loc.dot(Field::flags),
                         "is %s.",
                         string_VkDescriptorSetLayoutCreateFlags(create_info.flags).c_str());
    ...
    if (has_host_only_pool_flag && !enabled_features.mutableDescriptorType) {
        skip |= LogError("VUID-VkDescriptorSetLayoutCreateInfo-flags-04596",
                         device,
                         create_info_loc.dot(Field::flags),
    ...
    if (has_embedded_immutable_sampler_flag && !has_descriptor_buffer_flag) {
        skip |= LogError("VUID-VkDescriptorSetLayoutCreateInfo-flags-08001",
                         device,
                         create_info_loc.dot(Field::flags),
                         "is %s.",
                         string_VkDescriptorSetLayoutCreateFlags(create_info.flags).c_str());
    ...
    if (has_descriptor_buffer_flag && has_update_after_bind_flag) {
        skip |= LogError("VUID-VkDescriptorSetLayoutCreateInfo-flags-08002",
                         device,
                         create_info_loc.dot(Field::flags),
                         "is %s.",
                         string_VkDescriptorSetLayoutCreateFlags(create_info.flags).c_str());
    ...
    if (has_descriptor_buffer_flag && has_host_only_pool_flag) {
        skip |= LogError("VUID-VkDescriptorSetLayoutCreateInfo-flags-08003",
                         device,
                         create_info_loc.dot(Field::flags),
                         "is %s.",
                         string_VkDescriptorSetLayoutCreateFlags(create_info.flags).c_str());
...
bool StatelessValidation::manual_PreCallValidateCreateDescriptorSetLayout(
    VkDevice device,
...
bool StatelessValidation::manual_PreCallValidateGetDescriptorSetLayoutSupport(
    VkDevice device,
    const VkDescriptorSetLayoutCreateInfo* pCreateInfo,
...
bool StatelessValidation::manual_PreCallValidateFreeDescriptorSets(VkDevice device,
    ...
    return ValidateArray(error_obj.location.dot(Field::descriptorSetCount),
                         error_obj.location.dot(Field::pDescriptorSets),
                         descriptorSetCount,
                         &pDescriptorSets,
                         true,
                         true,
...
bool StatelessValidation::ValidateWriteDescriptorSet(const Location& loc,
    ...
    const bool is_push_descriptor = IsValueIn(loc.function,
    ...
    for (uint32_t i = 0; i < descriptorWriteCount; ++i) {
        ...
        if ((descriptor_type == VK_DESCRIPTOR_TYPE_SAMPLER) ||
            ...
            if (descriptor_writes.pImageInfo != nullptr && descriptor_type != VK_DESCRIPTOR_TYPE_SAMPLER) {
                for (uint32_t descriptor_index = 0; descriptor_index < descriptor_writes.descriptorCount;
                     ++descriptor_index) {
                    skip |=
                        ValidateRangedEnum(writes_loc.dot(Field::pImageInfo, descriptor_index).dot(Field::imageLayout),
                                           vvl::Enum::VkImageLayout,
                                           descriptor_writes.pImageInfo[descriptor_index].imageLayout,
                                           kVUIDUndefined);
                }
            }
        }
...
bool StatelessValidation::manual_PreCallValidateUpdateDescriptorSets(VkDevice device,
...
static bool MutableDescriptorTypePartialOverlap(const VkDescriptorPoolCreateInfo* pCreateInfo, uint32_t i, uint32_t j) {
    ...
    const auto* mutable_descriptor_type =
        vku::FindStructInPNextChain<VkMutableDescriptorTypeCreateInfoEXT>(pCreateInfo->pNext);
    if (mutable_descriptor_type) {
        vvl::span<const VkDescriptorType> first_types, second_types;

        if (mutable_descriptor_type->mutableDescriptorTypeListCount > i) {
            const uint32_t descriptor_type_count =
                mutable_descriptor_type->pMutableDescriptorTypeLists[i].descriptorTypeCount;
        ...
        if (mutable_descriptor_type->mutableDescriptorTypeListCount > j) {
            const uint32_t descriptor_type_count =
                mutable_descriptor_type->pMutableDescriptorTypeLists[j].descriptorTypeCount;
...
bool StatelessValidation::manual_PreCallValidateCreateDescriptorPool(VkDevice device,
    ...
    if (pCreateInfo->maxSets == 0 &&
        ((pCreateInfo->flags & VK_DESCRIPTOR_POOL_CREATE_ALLOW_OVERALLOCATION_SETS_BIT_NV) == 0)) {
        skip |= LogError("VUID-VkDescriptorPoolCreateInfo-descriptorPoolOverallocation-09227",
                         device,
                         create_info_loc.dot(Field::maxSets),
                         "is zero.");
    }

    const auto* inline_uniform_info =
        vku::FindStructInPNextChain<VkDescriptorPoolInlineUniformBlockCreateInfo>(pCreateInfo->pNext);
    const bool non_zero_inline_uniform_count =
        inline_uniform_info && inline_uniform_info->maxInlineUniformBlockBindings != 0;
    ...
    if (pCreateInfo->pPoolSizes) {
        for (uint32_t i = 0; i < pCreateInfo->poolSizeCount; ++i) {
            ...
            if (pCreateInfo->pPoolSizes[i].descriptorCount <= 0) {
                skip |= LogError("VUID-VkDescriptorPoolSize-descriptorCount-00302",
                                 device,
                                 pool_loc.dot(Field::descriptorCount),
                                 "is zero.");
            }
            if (pCreateInfo->pPoolSizes[i].type == VK_DESCRIPTOR_TYPE_INLINE_UNIFORM_BLOCK) {
                if (SafeModulo(pCreateInfo->pPoolSizes[i].descriptorCount, 4) != 0) {
                    skip |= LogError("VUID-VkDescriptorPoolSize-type-02218",
                                     device,
                                     pool_loc.dot(Field::descriptorCount),
                                     "is %" PRIu32
                                     " (not a multiple of 4), but type is VK_DESCRIPTOR_TYPE_INLINE_UNIFORM_BLOCK.",
                                     pCreateInfo->pPoolSizes[i].descriptorCount);
                }
                if (!non_zero_inline_uniform_count) {
                    skip |= LogError("VUID-VkDescriptorPoolCreateInfo-pPoolSizes-09424",
                                     device,
                                     pool_loc.dot(Field::type),
                                     "is VK_DESCRIPTOR_TYPE_INLINE_UNIFORM_BLOCK but no maxInlineUniformBlockBindings "
                                     "was provided.");
                }
            }
            if (pCreateInfo->pPoolSizes[i].type == VK_DESCRIPTOR_TYPE_MUTABLE_EXT &&
                !enabled_features.mutableDescriptorType) {
                skip |= LogError("VUID-VkDescriptorPoolCreateInfo-mutableDescriptorType-04608",
                                 device,
                                 pool_loc.dot(Field::type),
                                 "is VK_DESCRIPTOR_TYPE_MUTABLE_EXT "
                ...
                for (uint32_t j = i + 1; j < pCreateInfo->poolSizeCount; ++j) {
                    if (pCreateInfo->pPoolSizes[j].type == VK_DESCRIPTOR_TYPE_MUTABLE_EXT) {
                        if (MutableDescriptorTypePartialOverlap(pCreateInfo, i, j)) {
                            skip |= LogError("VUID-VkDescriptorPoolCreateInfo-pPoolSizes-04787",
                                             device,
                                             pool_loc.dot(Field::type),
                                             "and pPoolSizes[%" PRIu32 "].type are both VK_DESCRIPTOR_TYPE_MUTABLE_EXT "
    ...
    if (pCreateInfo->flags & VK_DESCRIPTOR_POOL_CREATE_HOST_ONLY_BIT_EXT && !enabled_features.mutableDescriptorType) {
        skip |= LogError("VUID-VkDescriptorPoolCreateInfo-flags-04609",
                         device,
                         create_info_loc.dot(Field::flags),
    ...
    if ((pCreateInfo->flags & VK_DESCRIPTOR_POOL_CREATE_HOST_ONLY_BIT_EXT) &&
        ...
        skip |=
            LogError("VUID-VkDescriptorPoolCreateInfo-flags-04607",
                     device,
                     create_info_loc.dot(Field::flags),
                     "includes both "
...
bool StatelessValidation::manual_PreCallValidateCreateQueryPool(VkDevice device,
    ...
    switch (pCreateInfo->queryType) {
        case VK_QUERY_TYPE_PIPELINE_STATISTICS: {
            ...
            } else if ((pCreateInfo->pipelineStatistics &
                        (VK_QUERY_PIPELINE_STATISTIC_TASK_SHADER_INVOCATIONS_BIT_EXT |
                         VK_QUERY_PIPELINE_STATISTIC_MESH_SHADER_INVOCATIONS_BIT_EXT)) &&
                       !enabled_features.meshShaderQueries) {
                skip |= LogError("VUID-VkQueryPoolCreateInfo-meshShaderQueries-07069",
                                 device,
            ...
            if (pCreateInfo->pipelineStatistics == 0) {
                skip |= LogError("VUID-VkQueryPoolCreateInfo-queryType-09534",
                                 device,
                                 create_info_loc.dot(Field::queryType),
                ...
                skip |= LogError("VUID-VkQueryPoolCreateInfo-queryType-00792",
                                 device,
                                 create_info_loc.dot(Field::queryType),
    ...
    if (pCreateInfo->queryCount == 0) {
        skip |= LogError(
            "VUID-VkQueryPoolCreateInfo-queryCount-02763", device, create_info_loc.dot(Field::queryCount), "is zero.");
    }
    return skip;
}

bool StatelessValidation::manual_PreCallValidateCreateSamplerYcbcrConversion(
    VkDevice device,
    ...
    if (!enabled_features.samplerYcbcrConversion) {
        skip |= LogError("VUID-vkCreateSamplerYcbcrConversion-None-01648",
                         device,
                         error_obj.location,
    ...
    if (vkuFormatIsXChromaSubsampled(format) == true) {
        if ((components.g != VK_COMPONENT_SWIZZLE_G) && (components.g != VK_COMPONENT_SWIZZLE_IDENTITY)) {
            skip |= LogError(
                ...
                string_VkComponentSwizzle(components.g));
        }
        ...
        if ((components.a != VK_COMPONENT_SWIZZLE_A) && (components.a != VK_COMPONENT_SWIZZLE_IDENTITY) &&
            ...
            skip |= LogError("VUID-VkSamplerYcbcrConversionCreateInfo-components-02582",
                             device,
                             create_info_loc,
                             " When using a XChroma subsampled format (%s) the components.a (%s) needs to be "
                             "VK_COMPONENT_SWIZZLE_A or "
                             "VK_COMPONENT_SWIZZLE_IDENTITY or VK_COMPONENT_SWIZZLE_ONE or VK_COMPONENT_SWIZZLE_ZERO.",
                             string_VkFormat(format),
                             string_VkComponentSwizzle(components.a));
        }
        ...
        if ((components.r != VK_COMPONENT_SWIZZLE_R) && (components.r != VK_COMPONENT_SWIZZLE_IDENTITY) &&
            ...
            skip |= LogError(
                ...
                string_VkFormat(format),
                string_VkComponentSwizzle(components.r));
        }
        ...
        const bool r_identity =
            ((components.r == VK_COMPONENT_SWIZZLE_R) || (components.r == VK_COMPONENT_SWIZZLE_IDENTITY));
        const bool b_identity =
    ...
    if (pCreateInfo->ycbcrModel != VK_SAMPLER_YCBCR_MODEL_CONVERSION_RGB_IDENTITY) {
        ...
        if ((components.r == VK_COMPONENT_SWIZZLE_ONE) || (components.r == VK_COMPONENT_SWIZZLE_ZERO) ||
            ...
            skip |= LogError(
                ...
                "components.g (%s), nor components.b (%s) can't be VK_COMPONENT_SWIZZLE_ZERO or "
        ...
        const uint32_t component_count =
            (vkuFormatIsDepthOrStencil(format) == true) ? 1 : vkuFormatComponentCount(format);

        if ((component_count < 4) &&
            ((components.r == VK_COMPONENT_SWIZZLE_A) || (components.g == VK_COMPONENT_SWIZZLE_A) ||
...
bool StatelessValidation::manual_PreCallValidateGetDescriptorEXT(VkDevice device,
    ...
    if (!enabled_features.descriptorBuffer) {
        skip |= LogError("VUID-vkGetDescriptorEXT-None-08015",
                         device,
                         error_obj.location,
    ...
    switch (pDescriptorInfo->type) {
        ...
        case VK_DESCRIPTOR_TYPE_STORAGE_BUFFER_DYNAMIC:
        case VK_DESCRIPTOR_TYPE_INLINE_UNIFORM_BLOCK:
            skip |= LogError("VUID-VkDescriptorGetInfoEXT-type-08018",
                             device,
        ...
        case VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER:
            if (!pDescriptorInfo->data.pCombinedImageSampler) {
                skip |= LogError("VUID-VkDescriptorGetInfoEXT-pCombinedImageSampler-parameter",
                                 device,
                                 descriptor_info_loc.dot(Field::type),
        ...
        case VK_DESCRIPTOR_TYPE_INPUT_ATTACHMENT:
            if (!pDescriptorInfo->data.pInputAttachmentImage) {
                skip |= LogError("VUID-VkDescriptorGetInfoEXT-pInputAttachmentImage-parameter",
                                 device,
                                 descriptor_info_loc.dot(Field::type),
    ...
    if (address_info) {
        ...
        if (address_info->address != 0) {
            if ((pDescriptorInfo->type == VK_DESCRIPTOR_TYPE_UNIFORM_TEXEL_BUFFER ||
                ...
                skip |= LogError("VUID-VkDescriptorAddressInfoEXT-None-09508",
                                 device,
                                 address_loc.dot(Field::format),
...
bool StatelessValidation::manual_PreCallValidateCmdSetDescriptorBufferOffsets2EXT(
    ...
    if (pSetDescriptorBufferOffsetsInfo->layout == VK_NULL_HANDLE) {
        if (!enabled_features.dynamicPipelineLayout) {
            skip |= LogError("VUID-VkSetDescriptorBufferOffsetsInfoEXT-None-09495",
                             commandBuffer,
            ...
            skip |= LogError("VUID-VkSetDescriptorBufferOffsetsInfoEXT-layout-09496",
                             commandBuffer,
                             error_obj.location.dot(Field::pSetDescriptorBufferOffsetsInfo).dot(Field::layout),
...
bool StatelessValidation::manual_PreCallValidateCmdBindDescriptorBufferEmbeddedSamplers2EXT(
    ...
    if (pBindDescriptorBufferEmbeddedSamplersInfo->layout == VK_NULL_HANDLE) {
        if (!enabled_features.dynamicPipelineLayout) {
            skip |=
                LogError("VUID-VkBindDescriptorBufferEmbeddedSamplersInfoEXT-None-09495",
                         commandBuffer,
                         error_obj.location.dot(Field::pBindDescriptorBufferEmbeddedSamplersInfo).dot(Field::layout),
                         "is VK_NULL_HANDLE.");
        } else if (!vku::FindStructInPNextChain<VkPipelineLayoutCreateInfo>(
                       pBindDescriptorBufferEmbeddedSamplersInfo->pNext)) {
            skip |=
                LogError("VUID-VkBindDescriptorBufferEmbeddedSamplersInfoEXT-layout-09496",
                         commandBuffer,
                         error_obj.location.dot(Field::pBindDescriptorBufferEmbeddedSamplersInfo).dot(Field::layout),
                         "is VK_NULL_HANDLE and pNext is missing VkPipelineLayoutCreateInfo.");
        }
...
bool StatelessValidation::manual_PreCallValidateCmdPushDescriptorSetWithTemplate2(
    ...
    if (pPushDescriptorSetWithTemplateInfo->layout == VK_NULL_HANDLE) {
        if (!enabled_features.dynamicPipelineLayout) {
            skip |= LogError("VUID-VkPushDescriptorSetWithTemplateInfo-None-09495",
                             commandBuffer,
                             error_obj.location.dot(Field::pPushDescriptorSetWithTemplateInfo).dot(Field::layout),
                             "is VK_NULL_HANDLE.");
        } else if (!vku::FindStructInPNextChain<VkPipelineLayoutCreateInfo>(
                       pPushDescriptorSetWithTemplateInfo->pNext)) {
            skip |= LogError("VUID-VkPushDescriptorSetWithTemplateInfo-layout-09496",
                             commandBuffer,
                             error_obj.location.dot(Field::pPushDescriptorSetWithTemplateInfo).dot(Field::layout),
                             "is VK_NULL_HANDLE and pNext is missing VkPipelineLayoutCreateInfo.");
        }
...
bool StatelessValidation::manual_PreCallValidateCmdBindDescriptorSets2(
    VkCommandBuffer commandBuffer,
    ...
    if (pBindDescriptorSetsInfo->layout == VK_NULL_HANDLE) {
        if (!enabled_features.dynamicPipelineLayout) {
            skip |= LogError("VUID-VkBindDescriptorSetsInfo-None-09495",
                             commandBuffer,
            ...
            skip |= LogError("VUID-VkBindDescriptorSetsInfo-layout-09496",
                             commandBuffer,
                             error_obj.location.dot(Field::pBindDescriptorSetsInfo).dot(Field::layout),
                             "is VK_NULL_HANDLE and pNext is missing VkPipelineLayoutCreateInfo.");
        }
