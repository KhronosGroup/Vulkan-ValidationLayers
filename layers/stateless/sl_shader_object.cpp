...
bool StatelessValidation::manual_PreCallValidateCreateShadersEXT(VkDevice device,
    ...
    for (uint32_t i = 0; i < createInfoCount; ++i) {
        ...
        if (createInfo.codeType == VK_SHADER_CODE_TYPE_SPIRV_EXT && SafeModulo(createInfo.codeSize, 4) != 0) {
            skip |= LogError("VUID-VkShaderCreateInfoEXT-codeSize-08735",
                             device,
                             create_info_loc.dot(Field::codeSize),
        ...
        if (createInfo.codeType == VK_SHADER_CODE_TYPE_BINARY_EXT) {
            if (SafeModulo(pCode, 16 * sizeof(unsigned char)) != 0) {
                skip |= LogError("VUID-VkShaderCreateInfoEXT-pCode-08492",
                                 device,
                                 create_info_loc.dot(Field::codeType),
            ...
            if (SafeModulo(pCode, 4 * sizeof(unsigned char)) != 0) {
                skip |= LogError("VUID-VkShaderCreateInfoEXT-pCode-08493",
                                 device,
                                 create_info_loc.dot(Field::codeType),
        ...
        if ((createInfo.stage & linkedStages) == 0 && (createInfo.flags & VK_SHADER_CREATE_LINK_STAGE_BIT_EXT) != 0) {
            skip |= LogError("VUID-VkShaderCreateInfoEXT-flags-08412",
                             device,
        ...
        if ((createInfo.stage != VK_SHADER_STAGE_COMPUTE_BIT) &&
            ...
            skip |= LogError("VUID-VkShaderCreateInfoEXT-flags-08485",
                             device,
                             create_info_loc.dot(Field::flags),
        ...
        if (createInfo.stage != VK_SHADER_STAGE_FRAGMENT_BIT) {
            if ((createInfo.flags & VK_SHADER_CREATE_FRAGMENT_SHADING_RATE_ATTACHMENT_BIT_EXT) != 0) {
                skip |=
                    LogError("VUID-VkShaderCreateInfoEXT-flags-08486",
                             device,
                             create_info_loc.dot(Field::flags),
            ...
            if ((createInfo.flags & VK_SHADER_CREATE_FRAGMENT_DENSITY_MAP_ATTACHMENT_BIT_EXT) != 0) {
                skip |=
                    LogError("VUID-VkShaderCreateInfoEXT-flags-08488",
                             device,
                             create_info_loc.dot(Field::flags),
        ...
        if (createInfo.stage != VK_SHADER_STAGE_MESH_BIT_EXT &&
            ...
            skip |= LogError("VUID-VkShaderCreateInfoEXT-flags-08414",
                             device,
                             create_info_loc.dot(Field::flags),
        ...
        if (createInfo.stage == VK_SHADER_STAGE_VERTEX_BIT) {
            if ((createInfo.nextStage & ~(VK_SHADER_STAGE_TESSELLATION_CONTROL_BIT | VK_SHADER_STAGE_GEOMETRY_BIT |
                                          VK_SHADER_STAGE_FRAGMENT_BIT)) != 0) {
                skip |= LogError("VUID-VkShaderCreateInfoEXT-nextStage-08427",
                                 device,
        ...
        } else if (createInfo.stage == VK_SHADER_STAGE_FRAGMENT_BIT ||
            ...
            skip |= LogError("VUID-VkShaderCreateInfoEXT-stage-08418",
                             device,
                             create_info_loc.dot(Field::stage),
        ...
        if (((createInfo.flags & VK_SHADER_CREATE_REQUIRE_FULL_SUBGROUPS_BIT_EXT) != 0) &&
            ...
            skip |= LogError("VUID-VkShaderCreateInfoEXT-flags-08992",
                             device,
                             create_info_loc.dot(Field::flags),
        ...
        skip |= ValidatePushConstantRange(
            createInfo.pushConstantRangeCount, createInfo.pPushConstantRanges, create_info_loc);
    }

    return skip;
}

bool StatelessValidation::manual_PreCallValidateGetShaderBinaryDataEXT(
    ...
    if (pData) {
        ...
        if (SafeModulo(ptr, 16 * sizeof(unsigned char)) != 0) {
            skip |= LogError("VUID-vkGetShaderBinaryDataEXT-None-08499",
                             device,
                             error_obj.location.dot(Field::pData),
