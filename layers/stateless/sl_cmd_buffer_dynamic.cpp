...
bool StatelessValidation::manual_PreCallValidateCmdSetViewportWithCount(VkCommandBuffer    commandBuffer,
                                                                        ...
                                                                        const ErrorObject& error_obj) const
{
    ...
    if (!enabled_features.multiViewport)
    {
        if (viewportCount != 1)
        {
            skip |= LogError("VUID-vkCmdSetViewportWithCount-viewportCount-03395",
                             commandBuffer,
                             error_obj.location.dot(Field::viewportCount),
                             "(%" PRIu32 ") is not 1, but the multiViewport feature is not enabled.",
                             viewportCount);
        }
    }
    else
    { // multiViewport enabled
        if (viewportCount < 1 || viewportCount > device_limits.maxViewports)
        {
            skip |= LogError("VUID-vkCmdSetViewportWithCount-viewportCount-03394",
                             commandBuffer,
                             error_obj.location.dot(Field::viewportCount),
                             "(%" PRIu32 ") must "
...
bool StatelessValidation::manual_PreCallValidateCmdSetScissorWithCount(VkCommandBuffer    commandBuffer,
                                                                       ...
                                                                       const ErrorObject& error_obj) const
{
    ...
    if (!enabled_features.multiViewport)
    {
        if (scissorCount != 1)
        {
            skip |= LogError("VUID-vkCmdSetScissorWithCount-scissorCount-03398",
                             commandBuffer,
    ...
    { // multiViewport enabled
        if (scissorCount == 0)
        {
            skip |= LogError("VUID-vkCmdSetScissorWithCount-scissorCount-03397",
                             commandBuffer,
                             error_obj.location.dot(Field::scissorCount),
                             "(%" PRIu32 ") must "
                             "be great than zero.",
    ...
    if (pScissors)
    {
        for (uint32_t scissor_i = 0; scissor_i < scissorCount; ++scissor_i)
        {
            ...
            {
                skip |= LogError("VUID-vkCmdSetScissorWithCount-x-03399",
                                 commandBuffer,
                                 scissor_loc.dot(Field::offset).dot(Field::x),
                                 "(%" PRId32 ") is negative.",
                                 scissor.offset.x);
            }
            ...
            if (scissor.offset.y < 0)
            {
                skip |= LogError("VUID-vkCmdSetScissorWithCount-x-03399",
                                 commandBuffer,
                                 scissor_loc.dot(Field::offset).dot(Field::y),
                                 "(%" PRId32 ") is negative.",
                                 scissor.offset.y);
            }
            ...
            {
                skip |= LogError("VUID-vkCmdSetScissorWithCount-offset-03400",
                                 commandBuffer,
                                 scissor_loc,
                                 "offset.x (%" PRId32 ") + extent.width (%" PRIu32 ") is %" PRIi64
            ...
            {
                skip |= LogError("VUID-vkCmdSetScissorWithCount-offset-03401",
                                 commandBuffer,
                                 scissor_loc,
                                 "offset.y (%" PRId32 ") + extent.height (%" PRIu32 ") is %" PRIi64
                                 " which will overflow int32_t.",
                                 scissor.offset.y,
    ...
    const ErrorObject&                           error_obj) const
{
    ...
    {
        skip |= LogError("VUID-vkCmdSetVertexInputEXT-vertexBindingDescriptionCount-04791",
                         commandBuffer,
    ...
    {
        skip |= LogError("VUID-vkCmdSetVertexInputEXT-vertexAttributeDescriptionCount-04792",
                         commandBuffer,
    ...
    for (uint32_t attribute = 0; attribute < vertexAttributeDescriptionCount; ++attribute)
    {
        ...
        if (!binding_found)
        {
            skip |= LogError("VUID-vkCmdSetVertexInputEXT-binding-04793",
                             commandBuffer,
    ...
    // check for distinct values
    {
        ...
        for (uint32_t i = 0; i < vertexBindingDescriptionCount; ++i)
        {
            ...
            {
                skip |= LogError("VUID-vkCmdSetVertexInputEXT-pVertexBindingDescriptions-04794",
                                 commandBuffer,
        ...
        for (uint32_t i = 0; i < vertexAttributeDescriptionCount; ++i)
        {
            ...
            {
                skip |= LogError("VUID-vkCmdSetVertexInputEXT-pVertexAttributeDescriptions-04795",
                                 commandBuffer,
    ...
    for (uint32_t binding = 0; binding < vertexBindingDescriptionCount; ++binding)
    {
        ...
        {
            skip |= LogError("VUID-VkVertexInputBindingDescription2EXT-binding-04796",
                             commandBuffer,
                             binding_loc.dot(Field::binding),
                             "(%" PRIu32 ") is greater than maxVertexInputBindings (%" PRIu32 ").",
                             pVertexBindingDescriptions[binding].binding,
                             device_limits.maxVertexInputBindings);
        }

        if (pVertexBindingDescriptions[binding].stride > device_limits.maxVertexInputBindingStride)
        {
            skip |= LogError("VUID-VkVertexInputBindingDescription2EXT-stride-04797",
                             commandBuffer,
                             binding_loc.dot(Field::stride),
                             "(%" PRIu32 ") is greater than maxVertexInputBindingStride (%" PRIu32 ").",
                             pVertexBindingDescriptions[binding].stride,
                             device_limits.maxVertexInputBindingStride);
        }

        if (pVertexBindingDescriptions[binding].divisor == 0 &&
            (!enabled_features.vertexAttributeInstanceRateZeroDivisor))
        {
            skip |= LogError("VUID-VkVertexInputBindingDescription2EXT-divisor-04798",
                             commandBuffer,
        ...
        if (pVertexBindingDescriptions[binding].divisor > 1)
        {
            ...
            {
                skip |= LogError("VUID-VkVertexInputBindingDescription2EXT-divisor-04799",
                                 commandBuffer,
                                 binding_loc.dot(Field::divisor),
                                 "is %" PRIu32 " (greater than 1) but "
            ...
            else
            {
                ...
                {
                    skip |= LogError("VUID-VkVertexInputBindingDescription2EXT-divisor-06226",
                                     commandBuffer,
                                     binding_loc.dot(Field::divisor),
                ...
                {
                    skip |= LogError("VUID-VkVertexInputBindingDescription2EXT-divisor-06227",
                                     commandBuffer,
                                     binding_loc.dot(Field::divisor),
                                     "is %" PRIu32 " (greater than 1) but inputRate is %s.",
                                     pVertexBindingDescriptions[binding].divisor,
    ...
    for (uint32_t attribute = 0; attribute < vertexAttributeDescriptionCount; ++attribute)
    {
        ...
        {
            skip |= LogError("VUID-VkVertexInputAttributeDescription2EXT-location-06228",
                             commandBuffer,
                             attribute_loc.dot(Field::location),
                             "(%" PRIu32 ") is greater than maxVertexInputAttributes (%" PRIu32 ").",
                             pVertexAttributeDescriptions[attribute].location,
                             device_limits.maxVertexInputAttributes);
        }

        if (pVertexAttributeDescriptions[attribute].binding > device_limits.maxVertexInputBindings)
        {
            skip |= LogError("VUID-VkVertexInputAttributeDescription2EXT-binding-06229",
                             commandBuffer,
        ...
        {
            skip |= LogError("VUID-VkVertexInputAttributeDescription2EXT-offset-06230",
                             commandBuffer,
                             attribute_loc.dot(Field::offset),
                             "(%" PRIu32 ") is greater than maxVertexInputAttributeOffset (%" PRIu32 ").",
                             pVertexAttributeDescriptions[attribute].offset,
                             device_limits.maxVertexInputAttributeOffset);
        }

        VkFormatProperties properties;
        DispatchGetPhysicalDeviceFormatProperties(
            physical_device, pVertexAttributeDescriptions[attribute].format, &properties);
        if ((properties.bufferFeatures & VK_FORMAT_FEATURE_VERTEX_BUFFER_BIT) == 0)
        {
            skip |= LogError("VUID-VkVertexInputAttributeDescription2EXT-format-04805",
                             commandBuffer,
                                                                          ...
                                                                          const ErrorObject& error_obj) const
{
    ...
    for (uint32_t i = 0; i < discardRectangleCount; ++i)
    {
        ...
        const int64_t  x_sum = static_cast<int64_t>(pDiscardRectangles[i].offset.x) +
                              static_cast<int64_t>(pDiscardRectangles[i].extent.width);
        if (x_sum > std::numeric_limits<int32_t>::max())
        {
            skip |= LogError("VUID-vkCmdSetDiscardRectangleEXT-offset-00588",
                             commandBuffer,
                             loc,
                             "offset.x (%" PRId32 ") + extent.width (%" PRIu32 ") is %" PRIi64
                             " which will overflow int32_t.",
                             pDiscardRectangles[i].offset.x,
                             pDiscardRectangles[i].extent.width,
                             x_sum);
        }

        const int64_t y_sum = static_cast<int64_t>(pDiscardRectangles[i].offset.y) +
                              static_cast<int64_t>(pDiscardRectangles[i].extent.height);
        if (y_sum > std::numeric_limits<int32_t>::max())
        {
            skip |= LogError("VUID-vkCmdSetDiscardRectangleEXT-offset-00589",
                             commandBuffer,
                             loc,
                             "offset.y (%" PRId32 ") + extent.height (%" PRIu32 ") is %" PRIi64
                             " which will overflow int32_t.",
                             pDiscardRectangles[i].offset.y,
                             pDiscardRectangles[i].extent.height,
                             y_sum);
        }
    }
                                                                                ...
                                                                                const ErrorObject& error_obj) const
{
    ...
    {
        skip |= LogError("VUID-vkCmdSetDiscardRectangleEnableEXT-specVersion-07851",
                         commandBuffer,
                         error_obj.location,
...
bool StatelessValidation::manual_PreCallValidateCmdSetDiscardRectangleModeEXT(
    VkCommandBuffer commandBuffer, VkDiscardRectangleModeEXT discardRectangleMode, const ErrorObject& error_obj) const
{
    bool skip = false;
    if (discard_rectangles_extension_version < 2)
    {
        skip |= LogError("VUID-vkCmdSetDiscardRectangleModeEXT-specVersion-07852",
                         commandBuffer,
                         error_obj.location,
                         "Requires support for version 2 of VK_EXT_discard_rectangles.");
    }
                                                                               ...
                                                                               const ErrorObject& error_obj) const
{
    ...
    {
        skip |= LogError("VUID-vkCmdSetExclusiveScissorEnableNV-exclusiveScissor-07853",
                         commandBuffer,
                         error_obj.location,
                                                                         ...
                                                                         const ErrorObject& error_obj) const
{
    ...
    if (!enabled_features.multiViewport)
    {
        ...
        {
            skip |= LogError("VUID-vkCmdSetExclusiveScissorNV-firstExclusiveScissor-02035",
                             commandBuffer,
                             error_obj.location.dot(Field::firstExclusiveScissor),
                             "is %" PRIu32 " but the multiViewport feature is not enabled.",
                             firstExclusiveScissor);
        }
        if (exclusiveScissorCount > 1)
        {
            skip |= LogError("VUID-vkCmdSetExclusiveScissorNV-exclusiveScissorCount-02036",
                             commandBuffer,
    ...
    { // multiViewport enabled
        const uint64_t sum =
            static_cast<uint64_t>(firstExclusiveScissor) + static_cast<uint64_t>(exclusiveScissorCount);
        if (sum > device_limits.maxViewports)
        {
            skip |= LogError("VUID-vkCmdSetExclusiveScissorNV-firstExclusiveScissor-02034",
                             commandBuffer,
                             error_obj.location,
    ...
    if (pExclusiveScissors)
    {
        for (uint32_t scissor_i = 0; scissor_i < exclusiveScissorCount; ++scissor_i)
        {
            ...
            if (scissor.offset.x < 0)
            {
                skip |= LogError("VUID-vkCmdSetExclusiveScissorNV-x-02037",
                                 commandBuffer,
                                 scissor_loc.dot(Field::offset).dot(Field::x),
                                 "(%" PRId32 ") is negative.",
                                 scissor.offset.x);
            ...
            if (x_sum > vvl::kI32Max)
            {
                skip |= LogError("VUID-vkCmdSetExclusiveScissorNV-offset-02038",
                                 commandBuffer,
                                 scissor_loc,
                                 "offset.x (%" PRId32 ") + extent.width (%" PRIu32 ") is %" PRIi64
                                 " which will overflow int32_t.",
            ...
            if (y_sum > vvl::kI32Max)
            {
                skip |= LogError("VUID-vkCmdSetExclusiveScissorNV-offset-02039",
                                 commandBuffer,
                                 scissor_loc,
                                 "offset.y (%" PRId32 ") + extent.height (%" PRIu32 ") is %" PRIi64
                                 " which will overflow int32_t.",
...
bool StatelessValidation::manual_PreCallValidateCmdSetViewportWScalingNV(VkCommandBuffer             commandBuffer,
                                                                         ...
                                                                         const ErrorObject&          error_obj) const
{
    ...
    {
        skip |= LogError("VUID-vkCmdSetViewportWScalingNV-firstViewport-01324",
                         commandBuffer,
                         error_obj.location,
    ...
    const ErrorObject&            error_obj) const
{
    ...
    if (!enabled_features.multiViewport)
    {
        if (firstViewport != 0)
        {
            skip |= LogError("VUID-vkCmdSetViewportShadingRatePaletteNV-firstViewport-02068",
                             commandBuffer,
                             error_obj.location.dot(Field::firstViewport),
                             "is %" PRIu32 " but the multiViewport feature was not enabled.",
                             firstViewport);
        }
        if (viewportCount > 1)
        {
            skip |= LogError("VUID-vkCmdSetViewportShadingRatePaletteNV-viewportCount-02069",
                             commandBuffer,
                             error_obj.location.dot(Field::viewportCount),
                             "is %" PRIu32 " but the multiViewport feature was not enabled.",
                             viewportCount);
    ...
    if (sum > device_limits.maxViewports)
    {
        skip |= LogError("VUID-vkCmdSetViewportShadingRatePaletteNV-firstViewport-02067",
                         commandBuffer,
                         error_obj.location,
                         "firstViewport (%" PRIu32 ") + viewportCount (%" PRIu32 ") is %" PRIu64
                         ") which is greater than VkPhysicalDeviceLimits::maxViewports (%" PRIu32 ").",
    ...
    const ErrorObject&                 error_obj) const
{
    ...
    {
        skip |= LogError("VUID-vkCmdSetCoarseSampleOrderNV-sampleOrderType-02081",
                         commandBuffer,
                         error_obj.location,
...
bool StatelessValidation::manual_PreCallValidateCmdSetViewport(VkCommandBuffer    commandBuffer,
                                                               uint32_t           firstViewport,
                                                               uint32_t           viewportCount,
                                                               const VkViewport*  pViewports,
                                                               const ErrorObject& error_obj) const
{
    ...
    if (!enabled_features.multiViewport)
    {
        if (firstViewport != 0)
        {
            skip |= LogError("VUID-vkCmdSetViewport-firstViewport-01224",
                             commandBuffer,
                             error_obj.location.dot(Field::firstViewport),
                             "is %" PRIu32 " but the multiViewport feature was not enabled.",
                             firstViewport);
        ...
        if (viewportCount > 1)
        {
            skip |= LogError("VUID-vkCmdSetViewport-viewportCount-01225",
                             commandBuffer,
                             error_obj.location.dot(Field::viewportCount),
                             "is %" PRIu32 " but the multiViewport feature was not enabled.",
                             viewportCount);
    ...
    { // multiViewport enabled
        ...
        if (sum > device_limits.maxViewports)
        {
            skip |= LogError("VUID-vkCmdSetViewport-firstViewport-01223",
                             commandBuffer,
                             error_obj.location,
                             "firstViewport (%" PRIu32 ") + viewportCount (%" PRIu32 ") is %" PRIu64
                                                                         ...
                                                                         const ErrorObject&          error_obj) const
{
    ...
    if (depthClampMode == VK_DEPTH_CLAMP_MODE_USER_DEFINED_RANGE_EXT)
    {
        if (!pDepthClampRange)
        {
            skip |= LogError("VUID-vkCmdSetDepthClampRangeEXT-pDepthClampRange-09647",
                             device,
                             error_obj.location.dot(Field::pDepthClampRange),
                             "is NULL.");
        }
        else
        {
...
bool StatelessValidation::manual_PreCallValidateCmdSetScissor(VkCommandBuffer    commandBuffer,
                                                              ...
                                                              const ErrorObject& error_obj) const
{
    ...
    if (!enabled_features.multiViewport)
    {
        if (firstScissor != 0)
        {
            skip |= LogError("VUID-vkCmdSetScissor-firstScissor-00593",
                             commandBuffer,
                             error_obj.location.dot(Field::firstScissor),
                             "is %" PRIu32 " but the multiViewport feature was not enabled.",
                             firstScissor);
        }
        if (scissorCount > 1)
        {
            skip |= LogError("VUID-vkCmdSetScissor-scissorCount-00594",
                             commandBuffer,
                             error_obj.location.dot(Field::scissorCount),
                             "is %" PRIu32 " but the multiViewport feature was not enabled.",
                             scissorCount);
    ...
    { // multiViewport enabled
        ...
        if (sum > device_limits.maxViewports)
        {
            skip |= LogError("VUID-vkCmdSetScissor-firstScissor-00592",
                             commandBuffer,
                             error_obj.location,
    ...
    if (pScissors)
    {
        for (uint32_t scissor_i = 0; scissor_i < scissorCount; ++scissor_i)
        {
            ...
            if (scissor.offset.x < 0)
            {
                skip |= LogError("VUID-vkCmdSetScissor-x-00595",
                                 commandBuffer,
                                 scissor_loc.dot(Field::offset).dot(Field::x),
                                 "(%" PRId32 ") is negative.",
                                 scissor.offset.x);
            ...
            if (x_sum > vvl::kI32Max)
            {
                skip |= LogError("VUID-vkCmdSetScissor-offset-00596",
                                 commandBuffer,
                                 scissor_loc,
                                 "offset.x (%" PRId32 ") + extent.width (%" PRIu32 ") is %" PRIi64
                                 " which will overflow int32_t.",
            ...
            if (y_sum > vvl::kI32Max)
            {
                skip |= LogError("VUID-vkCmdSetScissor-offset-00597",
                                 commandBuffer,
                                 scissor_loc,
                                 "offset.y (%" PRId32 ") + extent.height (%" PRIu32 ") is %" PRIi64
                                 " which will overflow int32_t.",
...
bool StatelessValidation::manual_PreCallValidateCmdSetLineWidth(VkCommandBuffer    commandBuffer,
                                                                float              lineWidth,
                                                                const ErrorObject& error_obj) const
{
    bool skip = false;
    ...
    {
        skip |= LogError("VUID-vkCmdSetLineWidth-lineWidth-00788",
                         commandBuffer,
                         error_obj.location.dot(Field::lineWidth),
                         "is %f (not 1.0), but wideLines was not enabled.",
                         lineWidth);
    }
...
bool StatelessValidation::manual_PreCallValidateCmdSetLineStipple(VkCommandBuffer    commandBuffer,
                                                                  ...
                                                                  const ErrorObject& error_obj) const
{
    ...
    {
        skip |= LogError("VUID-vkCmdSetLineStipple-lineStippleFactor-02776",
                         commandBuffer,
                         error_obj.location.dot(Field::lineStippleFactor),
                         "%" PRIu32 " is not in [1,256].",
                         lineStippleFactor);
    }
