...
bool StatelessValidation::ValidateSubpassGraphicsFlags(VkDevice device,
    ...
    const auto kGraphicsStages =
        (sync_utils::ExpandPipelineStages(VK_PIPELINE_STAGE_ALL_GRAPHICS_BIT, VK_QUEUE_GRAPHICS_BIT) |
         kMetaGraphicsStages) &
        ~kExcludeStages;
...
bool StatelessValidation::ValidateCreateRenderPass(VkDevice device,
                                                   const VkRenderPassCreateInfo2* pCreateInfo,
    ...
    for (uint32_t i = 0; i < pCreateInfo->attachmentCount; ++i) {
        ...
        const void* pNext =
            (use_rp2) ? reinterpret_cast<VkAttachmentDescription2 const*>(&pCreateInfo->pAttachments[i])->pNext
                      : nullptr;
        ...
        if (attachment_format == VK_FORMAT_UNDEFINED) {
            if (use_rp2 && android_external_format_resolve_feature) {
                if (GetExternalFormat(pNext) == 0) {
                    skip |= LogError("VUID-VkAttachmentDescription2-format-09334",
                                     device,
                                     attachment_loc.dot(Field::format),
                                     "is VK_FORMAT_UNDEFINED.");
                }
            } else {
                vuid = use_rp2 ? "VUID-VkAttachmentDescription2-format-09332"
        ...
        if (final_layout == VK_IMAGE_LAYOUT_UNDEFINED || final_layout == VK_IMAGE_LAYOUT_PREINITIALIZED) {
            vuid = use_rp2 ? "VUID-VkAttachmentDescription2-finalLayout-00843"
                           : "VUID-VkAttachmentDescription-finalLayout-00843";
            skip |= LogError(
                vuid, device, attachment_loc.dot(Field::finalLayout), "is %s.", string_VkImageLayout(final_layout));
        }
        ...
        if (!enabled_features.synchronization2) {
            if (initial_layout == VK_IMAGE_LAYOUT_ATTACHMENT_OPTIMAL ||
                initial_layout == VK_IMAGE_LAYOUT_READ_ONLY_OPTIMAL) {
            ...
            if (final_layout == VK_IMAGE_LAYOUT_ATTACHMENT_OPTIMAL ||
                ...
                skip |= LogError(vuid,
                                 ...
                                 attachment_loc.dot(Field::finalLayout),
                                 "is %s but the synchronization2 feature is not enabled.",
                                 string_VkImageLayout(final_layout));
            }
        }
        ...
        if (!vkuFormatIsDepthOrStencil(attachment_format)) { // color format
            if (IsImageLayoutDepthOnly(initial_layout) || IsImageLayoutStencilOnly(initial_layout)) {
                vuid = use_rp2 ? "VUID-VkAttachmentDescription2-format-03286"
            ...
            if (IsImageLayoutDepthOnly(final_layout) || IsImageLayoutStencilOnly(final_layout)) {
                vuid = use_rp2 ? "VUID-VkAttachmentDescription2-format-03287"
            ...
            if (IsImageLayoutStencilOnly(initial_layout)) {
                vuid = use_rp2 ? "VUID-VkAttachmentDescription2-format-06906"
            ...
            if (IsImageLayoutStencilOnly(final_layout)) {
                vuid = use_rp2 ? "VUID-VkAttachmentDescription2-format-06907"
            ...
            if (!attachment_description_stencil_layout) {
                if (IsImageLayoutDepthOnly(initial_layout)) {
                    vuid = use_rp2 ? "VUID-VkAttachmentDescription2-format-06249"
                ...
                if (IsImageLayoutDepthOnly(final_layout)) {
                    vuid = use_rp2 ? "VUID-VkAttachmentDescription2-format-06250"
            ...
            if (IsImageLayoutStencilOnly(initial_layout)) {
                vuid = use_rp2 ? "VUID-VkAttachmentDescription2-format-03290"
            ...
            if (IsImageLayoutStencilOnly(final_layout)) {
                vuid = use_rp2 ? "VUID-VkAttachmentDescription2-format-03291"
            ...
            if (IsImageLayoutDepthOnly(initial_layout)) {
                vuid = use_rp2 ? "VUID-VkAttachmentDescription2-format-06247"
            ...
            if (IsImageLayoutDepthOnly(final_layout)) {
                vuid = use_rp2 ? "VUID-VkAttachmentDescription2-format-06248"
        ...
        if (attachment_description_stencil_layout) {
            ...
            if (stencil_initial_layout == VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL ||
                ...
                skip |= LogError(
                    ...
                    "is %s.",
                    string_VkImageLayout(stencil_initial_layout));
            }
            if (stencil_final_layout == VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL ||
                ...
                skip |= LogError(
                    ...
                    "is %s.",
                    string_VkImageLayout(stencil_final_layout));
            }
            if (stencil_final_layout == VK_IMAGE_LAYOUT_UNDEFINED ||
                stencil_final_layout == VK_IMAGE_LAYOUT_PREINITIALIZED) {
                skip |= LogError(
        ...
        if (vkuFormatIsDepthOrStencil(attachment_format)) {
            if (initial_layout == VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL) {
                vuid = use_rp2 ? "VUID-VkAttachmentDescription2-format-03281"
            ...
            if (final_layout == VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL) {
                vuid = use_rp2 ? "VUID-VkAttachmentDescription2-format-03283"
        ...
        if (vkuFormatIsColor(attachment_format)) {
            if (initial_layout == VK_IMAGE_LAYOUT_DEPTH_STENCIL_ATTACHMENT_OPTIMAL ||
                ...
                vuid = use_rp2 ? "VUID-VkAttachmentDescription2-format-03280"
                               : "VUID-VkAttachmentDescription-format-03280";
                skip |= LogError(vuid,
                                 device,
                                 attachment_loc.dot(Field::initialLayout),
                                 "is %s, but using a Color format (%s)",
                                 string_VkImageLayout(initial_layout),
                                 string_VkFormat(attachment_format));
                ...
                vuid = use_rp2 ? "VUID-VkAttachmentDescription2-format-06487"
            ...
            if (final_layout == VK_IMAGE_LAYOUT_DEPTH_STENCIL_ATTACHMENT_OPTIMAL ||
                ...
                vuid = use_rp2 ? "VUID-VkAttachmentDescription2-format-03282"
                ...
                vuid = use_rp2 ? "VUID-VkAttachmentDescription2-format-06488"
        ...
        if (vkuFormatIsColor(attachment_format) || vkuFormatHasDepth(attachment_format)) {
            if (pCreateInfo->pAttachments[i].loadOp == VK_ATTACHMENT_LOAD_OP_LOAD &&
                ...
                vuid = use_rp2 ? "VUID-VkAttachmentDescription2-format-06699"
                               : "VUID-VkAttachmentDescription-format-06699";
                skip |= LogError(vuid,
                                 device,
                                 attachment_loc,
                                 "format is %s and loadOp is VK_ATTACHMENT_LOAD_OP_LOAD, but initialLayout is "
        ...
        if (vkuFormatHasStencil(attachment_format) &&
            ...
            if (initial_layout == VK_IMAGE_LAYOUT_UNDEFINED) {
                vuid =
                    use_rp2 ? "VUID-VkAttachmentDescription2-pNext-06704" : "VUID-VkAttachmentDescription-format-06700";
                skip |=
                    LogError(vuid,
                             device,
                             attachment_loc,
    ...
    for (uint32_t i = 0; i < pCreateInfo->subpassCount; ++i) {
        if (pCreateInfo->pSubpasses[i].colorAttachmentCount > max_color_attachments) {
            ...
            skip |= LogError(vuid,
                             ...
                             "cannot be used to create a render pass. maxColorAttachments is %d.",
    ...
    for (uint32_t i = 0; i < pCreateInfo->dependencyCount; ++i) {
        ...
        if ((dependency.srcSubpass != VK_SUBPASS_EXTERNAL) && (dependency.srcSubpass >= pCreateInfo->subpassCount)) {
            vuid = use_rp2 ? "VUID-VkRenderPassCreateInfo2-srcSubpass-02526"
                           : "VUID-VkRenderPassCreateInfo-pDependencies-06866";
            skip |= LogError(vuid,
                             device,
                             create_info_loc.dot(Field::pDependencies, i).dot(Field::srcSubpass),
                             "index (%" PRIu32 ") has to be less than subpassCount (%" PRIu32 ")",
        ...
        if ((dependency.dstSubpass != VK_SUBPASS_EXTERNAL) && (dependency.dstSubpass >= pCreateInfo->subpassCount)) {
            vuid = use_rp2 ? "VUID-VkRenderPassCreateInfo2-dstSubpass-02527"
        ...
        if (const auto barrier = vku::FindStructInPNextChain<VkMemoryBarrier2>(pCreateInfo->pDependencies[i].pNext);
            barrier) {
        ...
        vuid = use_rp2 ? "VUID-VkRenderPassCreateInfo2-pDependencies-03054"
                       : "VUID-VkRenderPassCreateInfo-pDependencies-00837";
        skip |= ValidateSubpassGraphicsFlags(device,
                                             pCreateInfo,
                                             dependency.srcSubpass,
                                             srcStageMask,
                                             vuid,
        ...
        vuid = use_rp2 ? "VUID-VkRenderPassCreateInfo2-pDependencies-03055"
...
bool StatelessValidation::manual_PreCallValidateCreateRenderPass(VkDevice device,
...
bool StatelessValidation::manual_PreCallValidateCreateRenderPass2(VkDevice device,
                                                                  const VkRenderPassCreateInfo2* pCreateInfo,
                                                                  const VkAllocationCallbacks* pAllocator,
                                                                  VkRenderPass* pRenderPass,
                                                                  const ErrorObject& error_obj) const {
...
void StatelessValidation::PostCallRecordCreateRenderPass(VkDevice device,
                                                         const VkRenderPassCreateInfo* pCreateInfo,
                                                         const VkAllocationCallbacks* pAllocator,
                                                         VkRenderPass* pRenderPass,
...
void StatelessValidation::PostCallRecordCreateRenderPass2KHR(VkDevice device,
                                                             const VkRenderPassCreateInfo2* pCreateInfo,
                                                             const VkAllocationCallbacks* pAllocator,
                                                             VkRenderPass* pRenderPass,
                                                             const RecordObject& record_obj) {
...
void StatelessValidation::PostCallRecordDestroyRenderPass(VkDevice device,
...
bool StatelessValidation::ValidateRenderPassStripeBeginInfo(VkCommandBuffer commandBuffer,
                                                            const void* pNext,
    ...
    if (rp_stripe_begin->stripeInfoCount > phys_dev_ext_props.renderpass_striped_props.maxRenderPassStripes) {
        skip |= LogError("VUID-VkRenderPassStripeBeginInfoARM-stripeInfoCount-09450",
                         commandBuffer,
                         loc.pNext(Struct::VkRenderPassStripeBeginInfoARM, Field::stripeInfoCount),
                         "(%" PRIu32 ") is greater than maxRenderPassStripes (%" PRIu32 ").",
                         rp_stripe_begin->stripeInfoCount,
    ...
    for (uint32_t i = 0; i < rp_stripe_begin->stripeInfoCount; ++i) {
        ...
        for (uint32_t index = i + 1; (!has_overlapping_stripes && i != last_stripe_index && index <= last_stripe_index);
             ++index) {
            ...
            if (has_overlapping_stripes) {
                skip |= LogError("VUID-VkRenderPassStripeBeginInfoARM-stripeArea-09451",
                                 commandBuffer,
                                 stripe_info_loc,
                                 "(offset{%s} extent{%s}) is overlapping with pStripeInfos[%" PRIu32
                                 "] (offset{%s} extent {%s}).",
                                 string_VkOffset2D(stripe_area.offset).c_str(),
                                 string_VkExtent2D(stripe_area.extent).c_str(),
                                 index,
                                 string_VkOffset2D(rect.offset).c_str(),
                                 string_VkExtent2D(rect.extent).c_str());
                break;
            }
        }

        if (width_granularity > 0 && (stripe_area.offset.x % width_granularity) != 0) {
            skip |= LogError("VUID-VkRenderPassStripeInfoARM-stripeArea-09452",
                             commandBuffer,
                             stripe_info_loc.dot(Field::stripeArea).dot(Field::offset).dot(Field::x),
                             "(%" PRIu32 ") is not a multiple of %" PRIu32 ".",
                             stripe_area.offset.x,
                             width_granularity);
        }
        ...
        if (width_granularity > 0 && (stripe_area.extent.width % width_granularity) != 0 &&
            ...
            skip |= LogError("VUID-VkRenderPassStripeInfoARM-stripeArea-09453",
                             commandBuffer,
        ...
        if (height_granularity > 0 && (stripe_area.extent.height % height_granularity) != 0 &&
            ...
            skip |=
                LogError("VUID-VkRenderPassStripeInfoARM-stripeArea-09455",
                         commandBuffer,
    ...
    if (!has_overlapping_stripes && (total_stripe_area != total_render_area)) {
        ...
        skip |=
            LogError(vuid.data(),
                     commandBuffer,
                     loc.pNext(Struct::VkRenderPassStripeBeginInfoARM, Field::pStripeInfos),
                     "has total of stripe area of %" PRIu32 " is not covering whole render area of %" PRIu32 " (%s).",
                     total_stripe_area,
                     total_render_area,
                     string_VkExtent2D(render_area.extent).c_str());
    }

    return skip;
}

bool StatelessValidation::ValidateCmdBeginRenderPass(VkCommandBuffer commandBuffer,
                                                     const VkRenderPassBeginInfo* const rp_begin,
                                                     const ErrorObject& error_obj) const {
    bool skip = false;
    if ((rp_begin->clearValueCount != 0) && !rp_begin->pClearValues) {
        const LogObjectList objlist(commandBuffer, rp_begin->renderPass);
        skip |= LogError("VUID-VkRenderPassBeginInfo-clearValueCount-04962",
                         objlist,
                         error_obj.location.dot(Field::pRenderPassBegin).dot(Field::clearValueCount),
                         "(%" PRIu32 ") is not zero, but pRenderPassBegin->pClearValues is NULL.",
...
bool StatelessValidation::manual_PreCallValidateCmdBeginRendering(VkCommandBuffer commandBuffer,
    ...
    if (!enabled_features.dynamicRendering) {
        skip |= LogError("VUID-vkCmdBeginRendering-dynamicRendering-06446",
                         commandBuffer,
                         error_obj.location,
    ...
    if (pRenderingInfo->viewMask == 0 && pRenderingInfo->layerCount == 0) {
        skip |= LogError("VUID-VkRenderingInfo-viewMask-06069",
                         commandBuffer,
                         rendering_info_loc,
    ...
    if (pRenderingInfo->colorAttachmentCount > device_limits.maxColorAttachments) {
        skip |= LogError("VUID-VkRenderingInfo-colorAttachmentCount-06106",
                         commandBuffer,
    ...
    if ((pRenderingInfo->flags & VK_RENDERING_CONTENTS_INLINE_BIT_KHR) != 0 && !enabled_features.nestedCommandBuffer &&
        ...
        skip |= LogError("VUID-VkRenderingInfo-flags-10012",
                         commandBuffer,
    ...
    if (pRenderingInfo->layerCount > device_limits.maxFramebufferLayers) {
        skip |= LogError("VUID-VkRenderingInfo-layerCount-07817",
                         commandBuffer,
    ...
    if (!enabled_features.multiview && (pRenderingInfo->viewMask != 0)) {
        skip |= LogError("VUID-VkRenderingInfo-multiview-06127",
                         commandBuffer,
                         rendering_info_loc.dot(Field::viewMask),
                         "is %" PRId32 " but the multiview feature is not enabled.",
    ...
    if (rendering_fsr_attachment_info) {
        skip |= ValidateBeginRenderingFragmentShadingRateAttachment(
            commandBuffer, *pRenderingInfo, *rendering_fsr_attachment_info, rendering_info_loc);
    }
    ...
    skip |= ValidateRenderPassStripeBeginInfo(
        commandBuffer, pRenderingInfo->pNext, pRenderingInfo->renderArea, rendering_info_loc);
...
bool StatelessValidation::ValidateBeginRenderingColorAttachment(VkCommandBuffer commandBuffer,
    ...
    for (uint32_t i = 0; i < rendering_info.colorAttachmentCount; ++i) {
        ...
        if (image_layout == VK_IMAGE_LAYOUT_DEPTH_STENCIL_ATTACHMENT_OPTIMAL ||
            ...
            skip |= LogError("VUID-VkRenderingInfo-colorAttachmentCount-06090",
                             commandBuffer,
                             color_attachment_loc.dot(Field::imageLayout),
                             "is %s.",
        ...
        if (resolve_mode != VK_RESOLVE_MODE_NONE) {
            if (resolve_image_layout == VK_IMAGE_LAYOUT_DEPTH_STENCIL_ATTACHMENT_OPTIMAL ||
                ...
                skip |= LogError("VUID-VkRenderingInfo-colorAttachmentCount-06091",
                                 commandBuffer,
                                 color_attachment_loc.dot(Field::resolveImageLayout),
                                 "is %s and resolveMode is %s.",
                                 string_VkImageLayout(resolve_image_layout),
                                 string_VkResolveModeFlagBits(resolve_mode));
            }
        }
        ...
        if (IsExtEnabled(device_extensions.vk_khr_maintenance2)) {
            if (image_layout == VK_IMAGE_LAYOUT_DEPTH_READ_ONLY_STENCIL_ATTACHMENT_OPTIMAL ||
                ...
                skip |= LogError("VUID-VkRenderingInfo-colorAttachmentCount-06096",
                                 commandBuffer,
                                 color_attachment_loc.dot(Field::imageLayout),
                                 "is %s.",
                                 string_VkImageLayout(image_layout));
            ...
            if (resolve_mode != VK_RESOLVE_MODE_NONE) {
                if (resolve_image_layout == VK_IMAGE_LAYOUT_DEPTH_READ_ONLY_STENCIL_ATTACHMENT_OPTIMAL ||
                    ...
                    skip |= LogError("VUID-VkRenderingInfo-colorAttachmentCount-06097",
                                     commandBuffer,
                                     color_attachment_loc.dot(Field::resolveImageLayout),
                                     "is %s and resolveMode is %s.",
                                     string_VkImageLayout(resolve_image_layout),
        ...
        if (IsImageLayoutDepthOnly(image_layout) || IsImageLayoutStencilOnly(image_layout)) {
            skip |= LogError("VUID-VkRenderingInfo-colorAttachmentCount-06100",
                             commandBuffer,
                             color_attachment_loc.dot(Field::imageLayout),
                             "is %s.",
                             string_VkImageLayout(image_layout));
        ...
        if (resolve_mode != VK_RESOLVE_MODE_NONE) {
            if (resolve_image_layout == VK_IMAGE_LAYOUT_DEPTH_ATTACHMENT_OPTIMAL ||
                ...
                skip |= LogError("VUID-VkRenderingInfo-colorAttachmentCount-06101",
                                 commandBuffer,
                                 color_attachment_loc.dot(Field::resolveImageLayout),
                                 "is %s and resolveMode is %s.",
                                 string_VkImageLayout(resolve_image_layout),
...
bool StatelessValidation::ValidateBeginRenderingDepthAttachment(VkCommandBuffer commandBuffer,
    ...
    if (depth_attachment.imageLayout == VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL) {
        skip |= LogError("VUID-VkRenderingInfo-pDepthAttachment-06092",
                         commandBuffer,
        ...
        skip |= LogError("VUID-VkRenderingInfo-pDepthAttachment-07732",
                         commandBuffer,
                         attachment_loc.dot(Field::imageLayout),
                         "is %s.",
    ...
    if (depth_attachment.resolveMode != VK_RESOLVE_MODE_NONE) {
        ...
        if (resolve_layout == VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL) {
            skip |= LogError("VUID-VkRenderingInfo-pDepthAttachment-06093",
                             commandBuffer,
            ...
            skip |= LogError("VUID-VkRenderingInfo-pDepthAttachment-07733",
                             commandBuffer,
                             attachment_loc.dot(Field::resolveImageLayout),
                             "is %s and resolveMode is %s.",
        ...
        if (IsExtEnabled(device_extensions.vk_khr_maintenance2) &&
            ...
            skip |= LogError("VUID-VkRenderingInfo-pDepthAttachment-06098",
                             commandBuffer,
                             attachment_loc.dot(Field::resolveImageLayout),
                             "is VK_IMAGE_LAYOUT_DEPTH_READ_ONLY_STENCIL_ATTACHMENT_OPTIMAL.");
        }

        if (!(depth_attachment.resolveMode &
            ...
            skip |= LogError(
                ...
                string_VkResolveModeFlagBits(depth_attachment.resolveMode),
                string_VkResolveModeFlags(phys_dev_ext_props.depth_stencil_resolve_props.supportedDepthResolveModes)
                    .c_str());
        }
    }
...
bool StatelessValidation::ValidateBeginRenderingStencilAttachment(VkCommandBuffer commandBuffer,
    ...
    if (stencil_attachment.imageLayout == VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL) {
        skip |= LogError("VUID-VkRenderingInfo-pStencilAttachment-06094",
                         commandBuffer,
                         attachment_loc.dot(Field::imageLayout),
                         "is VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL.");
    } else if (IsImageLayoutDepthOnly(stencil_attachment.imageLayout)) {
        skip |= LogError("VUID-VkRenderingInfo-pStencilAttachment-07734",
                         commandBuffer,
                         attachment_loc.dot(Field::imageLayout),
                         "is %s.",
    ...
    if (stencil_attachment.resolveMode != VK_RESOLVE_MODE_NONE) {
        ...
        if (resolve_layout == VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL) {
            skip |= LogError("VUID-VkRenderingInfo-pStencilAttachment-06095",
                             commandBuffer,
                             attachment_loc.dot(Field::resolveImageLayout),
                             "is VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL and resolveMode is %s.",
                             string_VkResolveModeFlagBits(stencil_attachment.resolveMode));
        } else if (IsImageLayoutDepthOnly(resolve_layout)) {
            skip |= LogError("VUID-VkRenderingInfo-pStencilAttachment-07735",
                             commandBuffer,
                             attachment_loc.dot(Field::resolveImageLayout),
                             "is %s and resolveMode is %s.",
                             string_VkImageLayout(resolve_layout),
        ...
        if (IsExtEnabled(device_extensions.vk_khr_maintenance2) &&
            ...
            skip |= LogError("VUID-VkRenderingInfo-pStencilAttachment-06099",
                             commandBuffer,
                             attachment_loc.dot(Field::resolveImageLayout),
                             "is VK_IMAGE_LAYOUT_DEPTH_READ_ONLY_STENCIL_ATTACHMENT_OPTIMAL.");
        }

        if (!(stencil_attachment.resolveMode &
            ...
            skip |= LogError(
                ...
                string_VkResolveModeFlagBits(stencil_attachment.resolveMode),
                string_VkResolveModeFlags(phys_dev_ext_props.depth_stencil_resolve_props.supportedStencilResolveModes)
                    .c_str());
        }
    }
...
bool StatelessValidation::ValidateBeginRenderingFragmentShadingRateAttachment(
    ...
    const Location& rendering_info_loc) const {
    bool skip = false;
    if (rendering_fsr_attachment_info.imageView == VK_NULL_HANDLE)
        return skip;
    ...
    if (UniqueRenderingInfoImageViews(rendering_info, rendering_fsr_attachment_info.imageView) == false) {
        skip |= LogError(
            ...
            "is %s.",
            FormatHandle(rendering_fsr_attachment_info.imageView).c_str());
    }
    ...
    if (image_layout != VK_IMAGE_LAYOUT_GENERAL &&
        ...
        skip |=
            LogError("VUID-VkRenderingFragmentShadingRateAttachmentInfoKHR-imageView-06147",
                     commandBuffer,
    ...
    if (rendering_fsr_attachment_info.shadingRateAttachmentTexelSize.width < min_frs_attach_texel_width) {
        skip |= LogError(
            ...
            rendering_info_loc
                .pNext(Struct::VkRenderingFragmentShadingRateAttachmentInfoKHR, Field::shadingRateAttachmentTexelSize)
                .dot(Field::width),
            "(%" PRIu32 ") must be greater than or equal to "
    ...
    if ((rendering_fsr_attachment_info.shadingRateAttachmentTexelSize.width /
        ...
        skip |= LogError("VUID-VkRenderingFragmentShadingRateAttachmentInfoKHR-imageView-06155",
                         commandBuffer,
    ...
    if ((rendering_fsr_attachment_info.shadingRateAttachmentTexelSize.height /
        ...
        skip |= LogError("VUID-VkRenderingFragmentShadingRateAttachmentInfoKHR-imageView-06156",
                         commandBuffer,
                         rendering_info_loc.pNext(Struct::VkRenderingFragmentShadingRateAttachmentInfoKHR,
                                                  Field::shadingRateAttachmentTexelSize),
