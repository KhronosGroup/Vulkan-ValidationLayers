...
bool StatelessValidation::manual_PreCallValidateCreateBuffer(VkDevice device,
    ...
    skip |=
        ValidateNotZero(pCreateInfo->size == 0, "VUID-VkBufferCreateInfo-size-00912", create_info_loc.dot(Field::size));

    // Validation for parameters excluded from the generated validation code due to a 'noautovalidity' tag in vk.xml
    if (pCreateInfo->sharingMode == VK_SHARING_MODE_CONCURRENT) {
        // If sharingMode is VK_SHARING_MODE_CONCURRENT, queueFamilyIndexCount must be greater than 1
        if (pCreateInfo->queueFamilyIndexCount <= 1) {
            skip |= LogError("VUID-VkBufferCreateInfo-sharingMode-00914",
                             device,
                             create_info_loc.dot(Field::sharingMode),
        ...
        if (pCreateInfo->pQueueFamilyIndices == nullptr) {
            skip |= LogError("VUID-VkBufferCreateInfo-sharingMode-00913",
                             device,
                             create_info_loc.dot(Field::sharingMode),
    ...
    if ((pCreateInfo->flags & VK_BUFFER_CREATE_SPARSE_BINDING_BIT) && (!enabled_features.sparseBinding)) {
        skip |= LogError("VUID-VkBufferCreateInfo-flags-00915",
                         device,
    ...
    // If flags contains VK_BUFFER_CREATE_SPARSE_RESIDENCY_BIT or VK_BUFFER_CREATE_SPARSE_ALIASED_BIT, it must also
    ...
    if (((pCreateInfo->flags & (VK_BUFFER_CREATE_SPARSE_RESIDENCY_BIT | VK_BUFFER_CREATE_SPARSE_ALIASED_BIT)) != 0) &&
        ...
        skip |= LogError("VUID-VkBufferCreateInfo-flags-00918",
                         device,
                         create_info_loc.dot(Field::flags),
                         "is %s.",
    ...
    if (!vku::FindStructInPNextChain<VkBufferUsageFlags2CreateInfo>(pCreateInfo->pNext)) {
        skip |= ValidateFlags(create_info_loc.dot(Field::usage),
    ...
    if (pCreateInfo->flags & VK_BUFFER_CREATE_PROTECTED_BIT) {
        const VkBufferUsageFlags invalid =
            ...
            VK_BUFFER_USAGE_TRANSFORM_FEEDBACK_COUNTER_BUFFER_BIT_EXT | VK_BUFFER_USAGE_CONDITIONAL_RENDERING_BIT_EXT |
            ...
            VK_BUFFER_USAGE_MICROMAP_BUILD_INPUT_READ_ONLY_BIT_EXT | VK_BUFFER_USAGE_MICROMAP_STORAGE_BIT_EXT;
        if (pCreateInfo->usage & invalid) {
            skip |= LogError("VUID-VkBufferCreateInfo-flags-09641",
                             device,
                             create_info_loc.dot(Field::flags),
...
bool StatelessValidation::manual_PreCallValidateCreateBufferView(VkDevice device,
