...
template <typename String = std::string,
          typename StringCollection = std::vector<String>,
...
bool StatelessValidation::ValidateExtensionReqs(const ExtensionState& extensions,
                                                const char* vuid,
                                                const char* extension_type,
                                                vvl::Extension extension,
    ...
    if (missing.size()) {
        ...
        skip |= LogError(vuid,
                         instance,
                         extension_loc,
                         "Missing extension%s required by the %s extension %s: %s.",
                         ((missing.size() > 1) ? "s" : ""),
                         extension_type,
                         String(extension),
                         missing_joined_list.c_str());
    }
    return skip;
}
...
bool StatelessValidation::manual_PreCallValidateCreateInstance(const VkInstanceCreateInfo* pCreateInfo,
    ...
    // Note: From the spec--
    //  Providing a NULL VkInstanceCreateInfo::pApplicationInfo or providing an apiVersion of 0 is equivalent to
    //  providing an apiVersion of VK_MAKE_VERSION(1, 0, 0).  (a.k.a. VK_API_VERSION_1_0)
    uint32_t local_api_version = (pCreateInfo->pApplicationInfo && pCreateInfo->pApplicationInfo->apiVersion)
                                     ? pCreateInfo->pApplicationInfo->apiVersion
                                     : VK_API_VERSION_1_0;

    uint32_t api_version_nopatch =
        VK_MAKE_VERSION(VK_VERSION_MAJOR(local_api_version), VK_VERSION_MINOR(local_api_version), 0);
    if (api_version != api_version_nopatch) {
        if ((api_version_nopatch < VK_API_VERSION_1_0) && (local_api_version != 0)) {
            skip |= LogError("VUID-VkApplicationInfo-apiVersion-04010",
                             instance,
            ...
            skip |= LogWarning(kVUIDUndefined,
                               instance,
                               create_info_loc.dot(Field::pApplicationInfo).dot(Field::apiVersion),
                               "is (0x%08x). "
    ...
    for (uint32_t i = 0; i < pCreateInfo->enabledExtensionCount; i++) {
        ...
        skip |= ValidateExtensionReqs(local_instance_extensions,
    ...
    if (pCreateInfo->flags & VK_INSTANCE_CREATE_ENUMERATE_PORTABILITY_BIT_KHR &&
        ...
        skip |= LogError("VUID-VkInstanceCreateInfo-flags-06559",
                         instance,
    ...
    const auto* debug_report_callback =
    ...
    const auto* debug_utils_messenger =
    ...
    const auto* direct_driver_loading_list =
    ...
    if (direct_driver_loading_list && !local_instance_extensions.vk_lunarg_direct_driver_loading) {
        skip |= LogError("VUID-VkInstanceCreateInfo-pNext-09400",
                         instance,
                         create_info_loc.dot(Field::ppEnabledExtensionNames),
                         "does not include VK_LUNARG_direct_driver_loading, but the pNext chain includes "
...
void StatelessValidation::CommonPostCallRecordEnumeratePhysicalDevice(const VkPhysicalDevice* phys_devices,
                                                                      const int count) {
...
void StatelessValidation::PostCallRecordEnumeratePhysicalDevices(VkInstance instance,
...
void StatelessValidation::PreCallRecordDestroyInstance(VkInstance instance,
                                                       const VkAllocationCallbacks* pAllocator,
                                                       const RecordObject& record_obj) {
...
void StatelessValidation::PostCallRecordCreateDevice(VkPhysicalDevice physicalDevice,
    ...
    if (record_obj.result != VK_SUCCESS)
        return;
    auto stateless_validation =
        static_cast<StatelessValidation*>(device_data->GetValidationObject(LayerObjectTypeParameterValidation));
...
bool StatelessValidation::manual_PreCallValidateCreateDevice(VkPhysicalDevice physicalDevice,
                                                             const VkDeviceCreateInfo* pCreateInfo,
                                                             const VkAllocationCallbacks* pAllocator,
                                                             VkDevice* pDevice,
                                                             const ErrorObject& error_obj) const {
    ...
    for (uint32_t i = 0; i < pCreateInfo->enabledExtensionCount; i++) {
        ...
        skip |= ValidateString(create_info_loc.dot(Field::ppEnabledExtensionNames),
                               "VUID-VkDeviceCreateInfo-ppEnabledExtensionNames-parameter",
                               pCreateInfo->ppEnabledExtensionNames[i]);
        skip |= ValidateExtensionReqs(device_extensions,
    ...
    if (portability_supported && !portability_requested) {
        skip |= LogError("VUID-VkDeviceCreateInfo-pProperties-04451",
                         physicalDevice,
                         error_obj.location,
    ...
    if (IsExtEnabledByCreateinfo(
            ExtensionStateByName(device_extensions, vvl::Extension::_VK_AMD_negative_viewport_height))) {
        const bool maint1 =
            IsExtEnabledByCreateinfo(ExtensionStateByName(device_extensions, vvl::Extension::_VK_KHR_maintenance1));
        // Only need to check for VK_KHR_MAINTENANCE_1_EXTENSION_NAME if api version is 1.0, otherwise it's deprecated
        // due to integration into api version 1.1
        if (api_version >= VK_API_VERSION_1_1) {
            skip |= LogError("VUID-VkDeviceCreateInfo-ppEnabledExtensionNames-01840",
                             physicalDevice,
                             create_info_loc.dot(Field::ppEnabledExtensionNames),
                             "must not include "
                             "VK_AMD_negative_viewport_height if api version is greater than or equal to 1.1.");
        } else if (maint1) {
            skip |= LogError("VUID-VkDeviceCreateInfo-ppEnabledExtensionNames-00374",
                             physicalDevice,
                             create_info_loc.dot(Field::ppEnabledExtensionNames),
    ...
    if (fragmentmask_requested) {
        ...
        if (descriptor_buffer_features && descriptor_buffer_features->descriptorBuffer) {
            skip |=
                LogError("VUID-VkDeviceCreateInfo-None-08095",
                         physicalDevice,
                         create_info_loc.dot(Field::ppEnabledExtensionNames),
    ...
    {
        const bool khr_bda = IsExtEnabledByCreateinfo(
            ExtensionStateByName(device_extensions, vvl::Extension::_VK_KHR_buffer_device_address));
        const bool ext_bda = IsExtEnabledByCreateinfo(
            ExtensionStateByName(device_extensions, vvl::Extension::_VK_EXT_buffer_device_address));
        if (khr_bda && ext_bda) {
            skip |= LogError("VUID-VkDeviceCreateInfo-ppEnabledExtensionNames-03328",
                             physicalDevice,
                             create_info_loc.dot(Field::ppEnabledExtensionNames),
    ...
    if (pCreateInfo->pNext != nullptr && pCreateInfo->pEnabledFeatures && features2) {
        ...
        skip |= LogError("VUID-VkDeviceCreateInfo-pNext-00373",
                         physicalDevice,
    ...
    if (const auto* robustness2_features =
        ...
        if (features && robustness2_features->robustBufferAccess2 && !features->robustBufferAccess) {
            skip |= LogError("VUID-VkPhysicalDeviceRobustness2FeaturesEXT-robustBufferAccess2-04000",
                             physicalDevice,
                             error_obj.location,
    ...
    if (const auto* raytracing_features =
        ...
        if (raytracing_features->rayTracingPipelineShaderGroupHandleCaptureReplayMixed &&
            ...
            skip |= LogError("VUID-VkPhysicalDeviceRayTracingPipelineFeaturesKHR-"
                             "rayTracingPipelineShaderGroupHandleCaptureReplayMixed-03575",
                             physicalDevice,
                             error_obj.location,
    ...
    if (const auto* di_features =
            vku::FindStructInPNextChain<VkPhysicalDeviceDescriptorIndexingFeatures>(pCreateInfo->pNext)) {
    ...
    if (vulkan_11_features) {
        ...
        while (current) {
            if (current->sType == VK_STRUCTURE_TYPE_PHYSICAL_DEVICE_16BIT_STORAGE_FEATURES ||
                ...
                skip |= LogError("VUID-VkDeviceCreateInfo-pNext-02829",
                                 physicalDevice,
                                 error_obj.location,
        ...
        if (vulkan_11_features->shaderDrawParameters == VK_FALSE &&
            ...
            skip |= LogError("VUID-VkDeviceCreateInfo-ppEnabledExtensionNames-04476",
                             physicalDevice,
                             error_obj.location,
    ...
    if (vulkan_12_features) {
        ...
        while (current) {
            if (current->sType == VK_STRUCTURE_TYPE_PHYSICAL_DEVICE_8BIT_STORAGE_FEATURES ||
                ...
                skip |= LogError("VUID-VkDeviceCreateInfo-pNext-02830",
                                 physicalDevice,
                                 create_info_loc.dot(Field::pNext),
        ...
        if (vulkan_12_features->drawIndirectCount == VK_FALSE &&
            ...
            skip |= LogError("VUID-VkDeviceCreateInfo-ppEnabledExtensionNames-02831",
                             physicalDevice,
                             error_obj.location,
        ...
        if (vulkan_12_features->samplerMirrorClampToEdge == VK_FALSE &&
            ...
            skip |= LogError("VUID-VkDeviceCreateInfo-ppEnabledExtensionNames-02832",
                             physicalDevice,
                             error_obj.location,
        ...
        if (vulkan_12_features->descriptorIndexing == VK_FALSE &&
            ...
            skip |= LogError("VUID-VkDeviceCreateInfo-ppEnabledExtensionNames-02833",
                             physicalDevice,
                             error_obj.location,
        ...
        if (vulkan_12_features->samplerFilterMinmax == VK_FALSE &&
            ...
            skip |= LogError("VUID-VkDeviceCreateInfo-ppEnabledExtensionNames-02834",
                             physicalDevice,
                             error_obj.location,
        ...
        if ((vulkan_12_features->shaderOutputViewportIndex == VK_FALSE ||
            ...
            skip |= LogError("VUID-VkDeviceCreateInfo-ppEnabledExtensionNames-02835",
                             physicalDevice,
                             error_obj.location,
        ...
        if (vulkan_12_features->bufferDeviceAddress == VK_TRUE) {
            if (IsExtEnabledByCreateinfo(
                    ExtensionStateByName(device_extensions, vvl::Extension::_VK_EXT_buffer_device_address))) {
                skip |= LogError("VUID-VkDeviceCreateInfo-pNext-04748",
                                 physicalDevice,
                                 create_info_loc.dot(Field::pNext),
    ...
    if (vulkan_13_features) {
        ...
        while (current) {
            if (current->sType == VK_STRUCTURE_TYPE_PHYSICAL_DEVICE_DYNAMIC_RENDERING_FEATURES ||
                ...
                skip |= LogError("VUID-VkDeviceCreateInfo-pNext-06532",
                                 physicalDevice,
                                 create_info_loc.dot(Field::pNext),
    ...
    if (vulkan_14_features) {
        ...
        while (current) {
            if (IsValueIn(current->sType, illegal_feature_structs_with_14)) {
                skip |= LogError("VUID-VkDeviceCreateInfo-pNext-10360",
                                 physicalDevice,
                                 create_info_loc.dot(Field::pNext),
    ...
    if (pCreateInfo->pQueueCreateInfos) {
        for (uint32_t i = 0; i < pCreateInfo->queueCreateInfoCount; ++i) {
            ...
            if (queue_create_info.pQueuePriorities != nullptr) {
                for (uint32_t j = 0; j < queue_create_info.queueCount; ++j) {
                    ...
                    if (!(queue_priority >= 0.f) || !(queue_priority <= 1.f)) {
                        skip |=
                            LogError("VUID-VkDeviceQueueCreateInfo-pQueuePriorities-00383",
                                     physicalDevice,
            ...
            if (((queue_create_info.flags & VK_DEVICE_QUEUE_CREATE_PROTECTED_BIT) != 0) &&
                (protected_memory == VK_FALSE)) {
                skip |= LogError("VUID-VkDeviceQueueCreateInfo-flags-02861",
                                 physicalDevice,
                                 create_info_loc.dot(Field::flags),
                                 "includes VK_DEVICE_QUEUE_CREATE_PROTECTED_BIT, but the protectedMemory feature is "
                                 "not being enabled.");
            }
        }
    }
    ...
    // feature dependencies for VK_KHR_variable_pointers
    {
        ...
        if ((variable_pointers == VK_TRUE) && (variable_pointers_storage_buffer == VK_FALSE)) {
            skip |=
                LogError("VUID-VkPhysicalDeviceVariablePointersFeatures-variablePointers-01431",
                         physicalDevice,
                         error_obj.location,
    ...
    // feature dependencies for VK_KHR_multiview
    {
        const auto* multiview_features =
            vku::FindStructInPNextChain<VkPhysicalDeviceMultiviewFeatures>(pCreateInfo->pNext);
        ...
        if ((multiview == VK_FALSE) && (multiview_geometry_shader == VK_TRUE)) {
            skip |= LogError("VUID-VkPhysicalDeviceMultiviewFeatures-multiviewGeometryShader-00580",
                             physicalDevice,
                             error_obj.location,
        ...
        if ((multiview == VK_FALSE) && (multiview_tessellation_shader == VK_TRUE)) {
            skip |= LogError("VUID-VkPhysicalDeviceMultiviewFeatures-multiviewTessellationShader-00581",
                             physicalDevice,
                             error_obj.location,
                             "If multiviewTessellationShader is VK_TRUE then multiview also needs to be VK_TRUE");
        }

        const auto* fsr_features =
        ...
        const auto* mesh_shader_features =
            vku::FindStructInPNextChain<VkPhysicalDeviceMeshShaderFeaturesEXT>(pCreateInfo->pNext);
        if (mesh_shader_features) {
            if ((multiview == VK_FALSE) && (mesh_shader_features->multiviewMeshShader)) {
                skip |= LogError("VUID-VkPhysicalDeviceMeshShaderFeaturesEXT-multiviewMeshShader-07032",
                                 physicalDevice,
                                 error_obj.location,
                                 "If multiviewMeshShader is VK_TRUE then multiview also needs to be VK_TRUE");
            }
    ...
    if (features && features->robustBufferAccess && any_update_after_bind_feature) {
        ...
        if (!di_props.robustBufferAccessUpdateAfterBind) {
            skip |= LogError("VUID-VkDeviceCreateInfo-robustBufferAccess-10247",
                             physicalDevice,
                             error_obj.location,
    ...
    if (const auto* fragment_shading_rate_features =
        ...
        if (shading_rate_image_features && shading_rate_image_features->shadingRateImage) {
            if (fragment_shading_rate_features->pipelineFragmentShadingRate) {
                skip |=
                    LogError("VUID-VkDeviceCreateInfo-shadingRateImage-04478",
                             physicalDevice,
                             error_obj.location,
            ...
            if (fragment_shading_rate_features->attachmentFragmentShadingRate) {
                skip |= LogError("VUID-VkDeviceCreateInfo-shadingRateImage-04480",
                                 physicalDevice,
                                 error_obj.location,
        ...
        if (fragment_density_map_features && fragment_density_map_features->fragmentDensityMap) {
            if (fragment_shading_rate_features->pipelineFragmentShadingRate) {
                skip |= LogError("VUID-VkDeviceCreateInfo-fragmentDensityMap-04481",
                                 physicalDevice,
                                 error_obj.location,
            ...
            if (fragment_shading_rate_features->primitiveFragmentShadingRate) {
                skip |= LogError("VUID-VkDeviceCreateInfo-fragmentDensityMap-04482",
                                 physicalDevice,
                                 error_obj.location,
            ...
            if (fragment_shading_rate_features->attachmentFragmentShadingRate) {
                skip |= LogError("VUID-VkDeviceCreateInfo-fragmentDensityMap-04483",
                                 physicalDevice,
                                 error_obj.location,
    ...
    if (const auto* shader_image_atomic_int64_features =
        ...
        if (shader_image_atomic_int64_features->sparseImageInt64Atomics &&
            ...
            skip |= LogError("VUID-VkDeviceCreateInfo-None-04896",
                             physicalDevice,
                             error_obj.location,
    ...
    if (const auto* shader_atomic_float_features =
        ...
        if (shader_atomic_float_features->sparseImageFloat32Atomics &&
            ...
            skip |= LogError("VUID-VkDeviceCreateInfo-None-04897",
                             physicalDevice,
                             error_obj.location,
        ...
        if (shader_atomic_float_features->sparseImageFloat32AtomicAdd &&
            ...
            skip |= LogError("VUID-VkDeviceCreateInfo-None-04898",
                             physicalDevice,
                             error_obj.location,
    ...
    if (const auto* shader_atomic_float2_features =
        ...
        if (shader_atomic_float2_features->sparseImageFloat32AtomicMinMax &&
            ...
            skip |= LogError("VUID-VkDeviceCreateInfo-sparseImageFloat32AtomicMinMax-04975",
                             physicalDevice,
                             error_obj.location,
        ...
        for (uint32_t i = 0; i < device_group_ci->physicalDeviceCount - 1; ++i) {
            for (uint32_t j = i + 1; j < device_group_ci->physicalDeviceCount; ++j) {
                if (device_group_ci->pPhysicalDevices[i] == device_group_ci->pPhysicalDevices[j]) {
                    skip |= LogError("VUID-VkDeviceGroupDeviceCreateInfo-pPhysicalDevices-00375",
                                     physicalDevice,
                                     error_obj.location,
    ...
    const auto* cache_control =
        vku::FindStructInPNextChain<VkDevicePipelineBinaryInternalCacheControlKHR>(pCreateInfo->pNext);
...
bool StatelessValidation::manual_PreCallValidateGetPhysicalDeviceImageFormatProperties2(
    ...
    if (pImageFormatInfo != nullptr) {
        ...
        const auto image_stencil_struct =
            vku::FindStructInPNextChain<VkImageStencilUsageCreateInfo>(pImageFormatInfo->pNext);
        ...
        if (image_drm_format) {
            if (pImageFormatInfo->tiling != VK_IMAGE_TILING_DRM_FORMAT_MODIFIER_EXT) {
                skip |= LogError("VUID-VkPhysicalDeviceImageFormatInfo2-tiling-02249",
                                 physicalDevice,
            ...
            if (image_drm_format->sharingMode == VK_SHARING_MODE_CONCURRENT) {
                if (image_drm_format->queueFamilyIndexCount <= 1) {
                    skip |= LogError("VUID-VkPhysicalDeviceImageDrmFormatModifierInfoEXT-sharingMode-02315",
                                     physicalDevice,
                    ...
                    skip |= LogError("VUID-VkPhysicalDeviceImageDrmFormatModifierInfoEXT-sharingMode-02314",
                                     physicalDevice,
                                     format_info_loc.pNext(Struct::VkPhysicalDeviceImageDrmFormatModifierInfoEXT,
                                                           Field::sharingMode),
                                     "is VK_SHARING_MODE_CONCURRENT, queueFamilyIndexCount is %" PRIu32
                                     ", but pQueueFamilyIndices is NULL.",
                                     image_drm_format->queueFamilyIndexCount);
                } else {
                    uint32_t queue_family_property_count = 0;
                    DispatchGetPhysicalDeviceQueueFamilyProperties2Helper(
                        physicalDevice, &queue_family_property_count, nullptr);
                    ...
                    for (uint32_t i = 0; i < image_drm_format->queueFamilyIndexCount; i++) {
                        ...
                        if (queue_family_indices_set.find(queue_index) != queue_family_indices_set.end()) {
                            skip |=
                                LogError("VUID-VkPhysicalDeviceImageDrmFormatModifierInfoEXT-sharingMode-02316",
                                         physicalDevice,
                                         format_info_loc.pNext(Struct::VkPhysicalDeviceImageDrmFormatModifierInfoEXT,
                                                               Field::pQueueFamilyIndices,
                                                               i),
                                         "is %" PRIu32 ", but is duplicated in pQueueFamilyIndices.",
                                         queue_index);
                            break;
                            ...
                            skip |=
                                LogError("VUID-VkPhysicalDeviceImageDrmFormatModifierInfoEXT-sharingMode-02316",
                                         ...
                                         format_info_loc.pNext(Struct::VkPhysicalDeviceImageDrmFormatModifierInfoEXT,
                                                               ...
                                                               i),
                                         "is %" PRIu32
                                         ", but vkGetPhysicalDeviceQueueFamilyProperties2::pQueueFamilyPropertyCount "
                                         "returned %" PRIu32 ".",
                                         queue_index,
        ...
        if (pImageFormatInfo->tiling == VK_IMAGE_TILING_DRM_FORMAT_MODIFIER_EXT &&
            ...
            if (!format_list || format_list->viewFormatCount == 0) {
                skip |= LogError("VUID-VkPhysicalDeviceImageFormatInfo2-tiling-02313",
                                 physicalDevice,
                                 format_info_loc,
                                 "tiling is VK_IMAGE_TILING_DRM_FORMAT_MODIFIER_EXT and flags contain "
                                 "VK_IMAGE_CREATE_MUTABLE_FORMAT_BIT "
                                 "bit, but the pNext chain does not contain an instance of VkImageFormatListCreateInfo "
                                 "with non-zero "
                                 "viewFormatCount.");
            }
        }
...
bool StatelessValidation::manual_PreCallValidateGetPhysicalDeviceImageFormatProperties(
    ...
    if (tiling == VK_IMAGE_TILING_DRM_FORMAT_MODIFIER_EXT) {
        skip |= LogError("VUID-vkGetPhysicalDeviceImageFormatProperties-tiling-02248",
                         physicalDevice,
...
bool StatelessValidation::manual_PreCallValidateSetDebugUtilsObjectNameEXT(
    VkDevice device, const VkDebugUtilsObjectNameInfoEXT* pNameInfo, const ErrorObject& error_obj) const {
    bool skip = false;
    const Location name_info_loc = error_obj.location.dot(Field::pNameInfo);
    if (pNameInfo->objectType == VK_OBJECT_TYPE_UNKNOWN) {
        skip |= LogError("VUID-vkSetDebugUtilsObjectNameEXT-pNameInfo-02587",
                         device,
                         name_info_loc.dot(Field::objectType),
                         "cannot be VK_OBJECT_TYPE_UNKNOWN.");
    }
    if (pNameInfo->objectHandle == HandleToUint64(VK_NULL_HANDLE)) {
        skip |= LogError("VUID-vkSetDebugUtilsObjectNameEXT-pNameInfo-02588",
                         device,
                         name_info_loc.dot(Field::objectHandle),
                         "cannot be VK_NULL_HANDLE.");
    }

    if ((pNameInfo->objectType == VK_OBJECT_TYPE_UNKNOWN) &&
        (pNameInfo->objectHandle == HandleToUint64(VK_NULL_HANDLE))) {
        skip |= LogError("VUID-VkDebugUtilsObjectNameInfoEXT-objectType-02589",
                         device,
                         name_info_loc.dot(Field::objectType),
...
bool StatelessValidation::manual_PreCallValidateSetDebugUtilsObjectTagEXT(VkDevice device,
    ...
    if (pTagInfo->objectType == VK_OBJECT_TYPE_UNKNOWN) {
        skip |= LogError("VUID-VkDebugUtilsObjectTagInfoEXT-objectType-01908",
                         device,
...
bool StatelessValidation::manual_PreCallValidateGetPhysicalDeviceProperties2(VkPhysicalDevice physicalDevice,
                                                                             ...
                                                                             const ErrorObject& error_obj) const {
    bool skip = false;
    const auto* api_props_lists =
        vku::FindStructInPNextChain<VkPhysicalDeviceLayeredApiPropertiesListKHR>(pProperties->pNext);
    if (api_props_lists && api_props_lists->pLayeredApis) {
        for (uint32_t i = 0; i < api_props_lists->layeredApiCount; i++) {
            if (const auto* api_vulkan_props =
                    vku::FindStructInPNextChain<VkPhysicalDeviceLayeredApiVulkanPropertiesKHR>(
                        api_props_lists->pLayeredApis[i].pNext)) {
                const VkBaseOutStructure* current =
                    static_cast<const VkBaseOutStructure*>(api_vulkan_props->properties.pNext);
                while (current) {
