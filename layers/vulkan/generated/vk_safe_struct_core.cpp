    ...
    basePipelineIndex(in_struct->basePipelineIndex)
{
    ...
    if (stageCount && pStages)
        for (uint32_t i = 0; i < stageCount; ++i)
        {
            ...
            if (pStages[i].stage == VK_SHADER_STAGE_FRAGMENT_BIT)
    ...
    if (in_struct->pRasterizationState)
        ...
        pRasterizationState = nullptr;
    if (in_struct->pMultisampleState && (has_rasterization && (renderPass != VK_NULL_HANDLE || is_graphics_library || has_fragment_stage)))
