...
class ValidateResolveAction {
  public:
    ValidateResolveAction(VkRenderPass render_pass,
                          uint32_t subpass,
                          const AccessContext& context,
                          const CommandBufferAccessContext& cb_context,
                          vvl::Func command) :
        render_pass_(render_pass),
        subpass_(subpass), context_(context), cb_context_(cb_context), command_(command), skip_(false) {}

    void operator()(const char* aspect_name,
                    const char* attachment_name,
                    uint32_t src_at,
                    uint32_t dst_at,
                    const AttachmentViewGen& view_gen,
                    AttachmentViewGen::Gen gen_type,
                    SyncAccessIndex current_usage,
...
void InitSubpassContexts(VkQueueFlags queue_flags,
    ...
    for (uint32_t pass = 0; pass < create_info.subpassCount; pass++) {
        subpass_contexts.emplace_back(
            pass, queue_flags, rp_state.subpass_dependencies, subpass_contexts, external_context);
    }
}
...
static SyncAccessIndex GetLoadOpUsageIndex(VkAttachmentLoadOp load_op, syncval_state::AttachmentType type) {
    ...
    if (load_op == VK_ATTACHMENT_LOAD_OP_NONE) {
        ...
        access_index = (load_op == VK_ATTACHMENT_LOAD_OP_LOAD)
...
static AccessContext* CreateStoreResolveProxyContext(const AccessContext& context,
                                                     const vvl::RenderPass& rp_state,
                                                     uint32_t subpass,
...
bool RenderPassAccessContext::ValidateLayoutTransitions(const CommandBufferAccessContext& cb_context,
    ...
    // previous subpass, we have to validate them against a copy of the AccessContext, with resolve operations applied,
    // as those affects have not been recorded yet.
    //
    // Note: we could be more efficient by tracking whether or not we actually *have* any changes (e.g. attachment
    // resolve) to apply and only copy then, if this proves a hot spot.
    ...
    for (const auto& transition : transitions) {
        const bool prev_needs_proxy =
            transition.prev_pass != VK_SUBPASS_EXTERNAL && (transition.prev_pass + 1 == subpass);

        const auto* track_back = access_context.GetTrackBackFromSubpass(transition.prev_pass);
        assert(track_back);
        if (prev_needs_proxy) {
            if (!proxy_for_prev) {
                proxy_for_prev.reset(CreateStoreResolveProxyContext(
                    *track_back->source_subpass, rp_state, transition.prev_pass, attachment_views));
        ...
        auto hazard =
            access_context.DetectSubpassTransitionHazard(*track_back, attachment_views[transition.attachment]);
        if (hazard.IsHazard()) {
            ...
            if (hazard.Tag() == kInvalidTag) {
                ...
                const auto error =
                    cb_context.GetSyncState().error_messages_.RenderPassLayoutTransitionVsStoreOrResolveError(
                        hazard,
                        subpass,
...
bool RenderPassAccessContext::ValidateLoadOperation(const CommandBufferAccessContext& cb_context,
    ...
    for (uint32_t i = 0; i < rp_state.create_info.attachmentCount; i++) {
        if (subpass == rp_state.attachment_first_subpass[i]) {
            ...
            // 1) if the usage bit isn't in the dest_access_scope, and there is layout traniition for initial use,
            // report hazard
            //    vs. transition
            // 2) if there isn't a layout transition, we need to look at the  external context with a "detect hazard"
            // operation
            ...
            const SyncAccessIndex stencil_load_index =
                has_stencil ? DepthStencilLoadUsage(ci.stencilLoadOp) : load_index;
            ...
            if (is_color && (load_index != SYNC_ACCESS_INDEX_NONE)) {
                hazard = access_context.DetectHazard(
                    view_gen, AttachmentViewGen::Gen::kRenderArea, load_index, SyncOrdering::kColorAttachment);
                aspect = "color";
            } else {
                if (has_depth && (load_index != SYNC_ACCESS_INDEX_NONE)) {
                    hazard = access_context.DetectHazard(view_gen,
                                                         AttachmentViewGen::Gen::kDepthOnlyRenderArea,
                                                         load_index,
                ...
                if (!hazard.IsHazard() && has_stencil && (stencil_load_index != SYNC_ACCESS_INDEX_NONE)) {
                    hazard = access_context.DetectHazard(view_gen,
                                                         AttachmentViewGen::Gen::kStencilOnlyRenderArea,
                                                         stencil_load_index,
                                                         SyncOrdering::kDepthStencilAttachment);
                    aspect = "stencil";
            ...
            if (hazard.IsHazard()) {
                ...
                if (hazard.Tag() == kInvalidTag) {
                    ...
                    const auto error = sync_state.error_messages_.RenderPassLoadOpVsLayoutTransitionError(
                        hazard, subpass, i, aspect, load_op);
                    skip |= sync_state.SyncError(hazard.Hazard(), rp_state.Handle(), loc, error);
                } else {
                    const auto error = sync_state.error_messages_.RenderPassLoadOpError(
                        hazard, cb_context, subpass, i, aspect, load_op);
                    skip |= sync_state.SyncError(hazard.Hazard(), rp_state.Handle(), loc, error);
                }
            }
        }
...
// because of the ordering guarantees w.r.t. sample access and that the resolve validation hasn't altered the state,
// because store is part of the same Next/End operation. The latter is handled in layout transistion validation directly
bool RenderPassAccessContext::ValidateStoreOperation(const CommandBufferAccessContext& cb_context,
    ...
    for (uint32_t i = 0; i < rp_state_->create_info.attachmentCount; i++) {
        if (current_subpass_ == rp_state_->attachment_last_subpass[i]) {
            ...
            if (!has_stencil && !store_op_stores)
                continue;
            ...
            bool checked_stencil = false;
            if (is_color) {
                hazard = CurrentContext().DetectHazard(view_gen,
                                                       AttachmentViewGen::Gen::kRenderArea,
                                                       SYNC_COLOR_ATTACHMENT_OUTPUT_COLOR_ATTACHMENT_WRITE,
...
bool IsStencilAttachmentWriteable(const LastBound& last_bound_state,
...
void ResolveOperation(Action& action,
                      const vvl::RenderPass& rp_state,
                      const AttachmentViewGenVector& attachment_views,
                      uint32_t subpass) {
        ...
        for (uint32_t i = 0; i < subpass_ci.colorAttachmentCount; i++) {
            ...
            if ((color_attach != VK_ATTACHMENT_UNUSED) && (resolve_attach != VK_ATTACHMENT_UNUSED)) {
                ...
                action("color",
                       ...
                       color_attach,
                       resolve_attach,
                       attachment_views[resolve_attach],
                       AttachmentViewGen::Gen::kRenderArea,
                       SYNC_COLOR_ATTACHMENT_OUTPUT_COLOR_ATTACHMENT_WRITE,
                       SyncOrdering::kColorAttachment);
            }
    ...
    if (ds_resolve && ds_resolve->pDepthStencilResolveAttachment &&
        ...
        subpass_ci.pDepthStencilAttachment &&
        ...
        const bool resolve_depth =
            (ds_resolve->depthResolveMode != VK_RESOLVE_MODE_NONE) && vkuFormatHasDepth(src_ci.format);
        const bool resolve_stencil =
            (ds_resolve->stencilResolveMode != VK_RESOLVE_MODE_NONE) && vkuFormatHasStencil(src_ci.format);
        ...
        if (aspect_string) {
            action(aspect_string,
                   "resolve read",
                   src_at,
                   dst_at,
                   attachment_views[src_at],
                   gen_type,
                   SYNC_COLOR_ATTACHMENT_OUTPUT_COLOR_ATTACHMENT_READ,
                   SyncOrdering::kRaster);
            action(aspect_string,
...
bool RenderPassAccessContext::ValidateResolveOperations(const CommandBufferAccessContext& cb_context,
                                                        vvl::Func command) const {
    ValidateResolveAction validate_action(
        rp_state_->VkHandle(), current_subpass_, CurrentContext(), cb_context, command);
    ...
    for (uint32_t i = 0; i < rp_state.create_info.attachmentCount; i++) {
        if (rp_state.attachment_last_subpass[i] == subpass) {
            ...
            if (is_color && store_op_stores) {
                access_context.UpdateAccessState(view_gen,
                                                 AttachmentViewGen::Gen::kRenderArea,
                                                 SYNC_COLOR_ATTACHMENT_OUTPUT_COLOR_ATTACHMENT_WRITE,
                                                 SyncOrdering::kRaster,
                                                 tag);
...
void RenderPassAccessContext::RecordLayoutTransitions(const vvl::RenderPass& rp_state,
                                                      uint32_t subpass,
                                                      const AttachmentViewGenVector& attachment_views,
                                                      const ResourceUsageTag tag,
                                                      AccessContext& access_context) {
    ...
    for (const auto& transition : transitions) {
        ...
        const std::optional<ImageRangeGen>& attachment_gen =
            view_gen.GetRangeGen(AttachmentViewGen::Gen::kViewSubresource);
        assert(attachment_gen);

        access_context.ResolveFromContext(
            barrier_action, *prev_context, *attachment_gen, &empty_infill, true /* recur to infill */);
        assert(attachment_gen);
    }
...
bool RenderPassAccessContext::ValidateDrawSubpassAttachment(const CommandBufferAccessContext& cb_context,
    ...
    if (subpass.pColorAttachments && subpass.colorAttachmentCount && !list.empty()) {
        for (const auto location : list) {
            ...
            if (!view_gen.IsValid())
                continue;
            HazardResult hazard = current_context.DetectHazard(view_gen,
                                                               AttachmentViewGen::Gen::kRenderArea,
                                                               SYNC_COLOR_ATTACHMENT_OUTPUT_COLOR_ATTACHMENT_WRITE,
                                                               SyncOrdering::kColorAttachment);
            if (hazard.IsHazard()) {
                ...
                const auto error = sync_state.error_messages_.RenderPassColorAttachmentError(
                    hazard, cb_context, *view_gen.GetViewState(), location);
    ...
    if ((depth_stencil_attachment != VK_ATTACHMENT_UNUSED) && attachment_views_[depth_stencil_attachment].IsValid()) {
        ...
        if (depth_write) {
            ...
            if (hazard.IsHazard()) {
                const Location loc(command);
                const auto error = sync_state.error_messages_.RenderPassDepthStencilAttachmentError(
                    hazard, cb_context, view_state, true);
        ...
        if (stencil_write) {
            ...
            if (hazard.IsHazard()) {
                const Location loc(command);
                const auto error = sync_state.error_messages_.RenderPassDepthStencilAttachmentError(
                    hazard, cb_context, view_state, false);
                skip |= sync_state.SyncError(hazard.Hazard(), view_state.Handle(), loc, error);
            }
        }
    }
...
void RenderPassAccessContext::RecordDrawSubpassAttachment(const vvl::CommandBuffer& cmd_buffer,
                                                          const ResourceUsageTag tag) {
    const auto lv_bind_point = ConvertToLvlBindPoint(VK_PIPELINE_BIND_POINT_GRAPHICS);
    const auto& last_bound_state = cmd_buffer.lastBound[lv_bind_point];
    const auto* pipe = last_bound_state.pipeline_state;
    ...
    if (subpass.pColorAttachments && subpass.colorAttachmentCount && !list.empty()) {
        for (const auto location : list) {
            ...
            const AttachmentViewGen& view_gen = attachment_views_[subpass.pColorAttachments[location].attachment];
            current_context.UpdateAccessState(view_gen,
                                              AttachmentViewGen::Gen::kRenderArea,
                                              SYNC_COLOR_ATTACHMENT_OUTPUT_COLOR_ATTACHMENT_WRITE,
                                              SyncOrdering::kColorAttachment,
                                              tag);
    ...
    if ((depth_stencil_attachment != VK_ATTACHMENT_UNUSED) && attachment_views_[depth_stencil_attachment].IsValid()) {
        ...
        const bool has_stencil =
            0 != (view_state.normalized_subresource_range.aspectMask & VK_IMAGE_ASPECT_STENCIL_BIT);

        const bool depth_write_enable =
            last_bound_state.IsDepthWriteEnable(); // implicitly means DepthTestEnable is set
        const bool stencil_test_enable = last_bound_state.IsStencilTestEnable();
...
uint32_t RenderPassAccessContext::GetAttachmentIndex(const VkClearAttachment& clear_attachment) const {
    ...
    // As _UNUSED is UINT32_MAX (~0U) this catches all "no attachment" cases -- unknown aspectMask, UNUSED, and out of
    // bounds
...
VkImageAspectFlags ClearAttachmentInfo::GetAspectsToClear(VkImageAspectFlags clear_aspect_mask,
                                                          const ImageViewState& view) {
...
ClearAttachmentInfo::ClearAttachmentInfo(const VkClearAttachment& clear_attachment,
                                         const VkClearRect& rect,
                                         const ImageViewState& view_,
                                         uint32_t attachment_index_,
                                         uint32_t subpass_) :
    view(&view_),
    aspects_to_clear(GetAspectsToClear(clear_attachment.aspectMask, view_)),
    subresource_range(RestrictSubresourceRange(rect, view_)), offset(CastTo3D(rect.rect.offset)),
    extent(CastTo3D(rect.rect.extent)), attachment_index(attachment_index_), subpass(subpass_) {}
...
VkImageSubresourceRange ClearAttachmentInfo::RestrictSubresourceRange(const VkClearRect& clear_rect,
                                                                      const ImageViewState& view) {
...
bool RenderPassAccessContext::ValidateNextSubpass(const CommandBufferAccessContext& cb_context,
                                                  vvl::Func command) const {
    ...
    skip |= ValidateLayoutTransitions(
        cb_context, next_context, *rp_state_, render_area_, next_subpass, attachment_views_, command);
    if (!skip) {
        // To avoid complex (and buggy) duplication of the affect of layout transitions on load operations, we'll record
        // them on a copy of the (empty) next context. Note: The resource access map should be empty so hopefully this
        // copy isn't too horrible from a perf POV.
        ...
        skip |= ValidateLoadOperation(
            cb_context, temp_context, *rp_state_, render_area_, next_subpass, attachment_views_, command);
    }
    return skip;
}
bool RenderPassAccessContext::ValidateEndRenderPass(const CommandBufferAccessContext& cb_context,
                                                    vvl::Func command) const {
...
bool RenderPassAccessContext::ValidateFinalSubpassLayoutTransitions(const CommandBufferAccessContext& cb_context,
    ...
    for (const auto& transition : final_transitions) {
        ...
        assert(
            trackback
                .source_subpass); // Transitions are given implicit transitions if the StateTracker is working correctly
        auto* context = trackback.source_subpass;

        if (transition.prev_pass == current_subpass_) {
            if (!proxy_for_current) {
                // We haven't recorded resolve ofor the current_subpass, so we need to copy current and update it *as
                // if*
        ...
        auto hazard =
            context->DetectImageBarrierHazard(view_gen, merged_barrier, AccessContext::DetectOptions::kDetectPrevious);
        if (hazard.IsHazard()) {
            const Location loc(command);
            if (hazard.Tag() == kInvalidTag) {
                // Hazard vs. store/resolve
                const auto error =
                    cb_context.GetSyncState().error_messages_.RenderPassFinalLayoutTransitionVsStoreOrResolveError(
                        hazard,
                        cb_context,
                        transition.prev_pass,
                        transition.attachment,
        ...
        if (rp_state_->attachment_first_subpass[i] == current_subpass_) {
            ...
            if (is_color) {
                ...
                if (load_op != SYNC_ACCESS_INDEX_NONE) {
                    subpass_context.UpdateAccessState(
                        view_gen, AttachmentViewGen::Gen::kRenderArea, load_op, SyncOrdering::kColorAttachment, tag);
                }
            } else {
                if (has_depth) {
                    const SyncAccessIndex load_op = DepthStencilLoadUsage(ci.loadOp);
                    if (load_op != SYNC_ACCESS_INDEX_NONE) {
                        subpass_context.UpdateAccessState(view_gen,
                                                          AttachmentViewGen::Gen::kDepthOnlyRenderArea,
                                                          load_op,
                                                          SyncOrdering::kDepthStencilAttachment,
                                                          tag);
...
RenderPassAccessContext::RenderPassAccessContext(
    const vvl::RenderPass& rp_state,
    const VkRect2D& render_area,
    ...
    const AccessContext* external_context) :
    rp_state_(&rp_state),
    render_area_(render_area), current_subpass_(0U), attachment_views_() {
...
void RenderPassAccessContext::RecordBeginRenderPass(const ResourceUsageTag barrier_tag,
...
void RenderPassAccessContext::RecordNextSubpass(const ResourceUsageTag store_tag,
...
void RenderPassAccessContext::RecordEndRenderPass(AccessContext* external_context,
    ...
    // TODO Aliasing we may need to reconsider barrier accumulation... though I don't know that it would be valid for
    // aliasing
    ...
    for (const auto& transition : final_transitions) {
        ...
        ApplyBarrierOpsFunctor<PipelineBarrierOp> barrier_action(
            true /* resolve */, last_trackback.barriers.size(), barrier_tag);
...
void syncval_state::BeginRenderingCmdState::AddRenderingInfo(const SyncValidator& state,
...
syncval_state::DynamicRenderingInfo::DynamicRenderingInfo(const SyncValidator& state,
                                                          const VkRenderingInfo& rendering_info) :
    info(&rendering_info) {
    uint32_t attachment_count =
        info.colorAttachmentCount + (info.pDepthAttachment ? 1 : 0) + (info.pStencilAttachment ? 1 : 0);
    ...
    for (uint32_t i = 0; i < info.colorAttachmentCount; i++) {
        attachments.emplace_back(
            state, info.pColorAttachments[i], syncval_state::AttachmentType::kColor, offset, extent);
    }
    ...
    if (info.pStencilAttachment) {
        attachments.emplace_back(
            state, *info.pStencilAttachment, syncval_state::AttachmentType::kStencil, offset, extent);
    }
}

ClearAttachmentInfo
syncval_state::DynamicRenderingInfo::GetClearAttachmentInfo(const VkClearAttachment& clear_attachment,
                                                            const VkClearRect& rect) const {
...
syncval_state::DynamicRenderingInfo::Attachment::Attachment(const SyncValidator& state,
    ...
    view(state.Get<ImageViewState>(attachment_info.imageView)), view_gen(), type(type_) {
    if (view) {
...
Location syncval_state::DynamicRenderingInfo::Attachment::GetLocation(const Location& loc,
