...
const SyncAccessFlags kDepthStencilAttachmentAccessScope =
    ...
    SYNC_FRAGMENT_SHADER_INPUT_ATTACHMENT_READ_BIT; // Note: this is intentionally not in the exec scope
constexpr VkPipelineStageFlags2 kRasterAttachmentExecScope =
    kDepthStencilAttachmentExecScope | kColorAttachmentExecScope;
...
class HazardResult {
  public:
    struct HazardState {
        ...
        HazardState(const ResourceAccessState* access_state,
    ...
    static HazardResult HazardVsPriorWrite(const ResourceAccessState* access_state,
                                           const SyncAccessInfo& usage_info,
                                           SyncHazard hazard,
                                           const WriteState& prior_write);
    static HazardResult HazardVsPriorRead(const ResourceAccessState* access_state,
                                          const SyncAccessInfo& usage_info,
                                          SyncHazard hazard,
...
struct SyncExecScope {
    ...
    VkPipelineStageFlags2
        exec_scope;                 // all earlier or later stages that would be affected by a barrier using this scope.
    ...
    SyncExecScope(VkPipelineStageFlags2 mask_param_,
                  VkPipelineStageFlags2 expanded_mask_,
                  VkPipelineStageFlags2 exec_scope_,
                  const SyncAccessFlags& valid_accesses_) :
        mask_param(mask_param_),
        expanded_mask(expanded_mask_), exec_scope(exec_scope_), valid_accesses(valid_accesses_) {}

    static SyncExecScope MakeSrc(VkQueueFlags queue_flags,
...
struct SyncBarrier {
    ...
    SyncBarrier(const SyncExecScope& src_exec,
                const SyncAccessFlags& src_access,
                const SyncExecScope& dst_exec,
                const SyncAccessFlags& dst_access) :
        src_exec_scope(src_exec),
        src_access_scope(src_access), dst_exec_scope(dst_exec), dst_access_scope(dst_access) {}
...
struct ReadState {
    ...
    bool IsReadBarrierHazard(QueueId barrier_queue,
    ...
    bool ReadInEventScope(VkPipelineStageFlags2 exec_scope, QueueId scope_queue, ResourceUsageTag scope_tag) const {
        ...
        // NOTE: That's not really correct... this read stage might *not* have been included in the setevent, and the
        // barriers representing the chain might have changed since then (that would be an odd usage), so as a first
        // approximation we'll assume the barriers *haven't* been changed since (if the tag hasn't), and while this
        // could be a false positive in the case of Set; SomeBarrier; Wait; we'll live with it until we can add more
        // state to the first scope capture (the specific write and read stages that *were* in scope at the moment of
        // SetEvents.
...
class WriteState {
  public:
    ...
    bool operator==(const WriteState& rhs) const {
        return (access_ == rhs.access_) && (barriers_ == rhs.barriers_) && (tag_ == rhs.tag_) &&
               (queue_ == rhs.queue_) && (dependency_chain_ == rhs.dependency_chain_);
    }
    ...
    bool WriteInQueueSourceScopeOrChain(QueueId queue,
                                        VkPipelineStageFlags2 src_exec_scope,
                                        const SyncAccessFlags& src_access_scope) const;

    bool WriteInEventScope(VkPipelineStageFlags2 src_exec_scope,
    ...
    bool IsWriteBarrierHazard(QueueId queue_id,
                              VkPipelineStageFlags2 src_exec_scope,
                              const SyncAccessFlags& src_access_scope) const;
...
class ResourceAccessState : public SyncStageAccess {
  ...
  public:
    ...
    HazardResult
    DetectHazard(const SyncAccessInfo& usage_info, const OrderingBarrier& ordering, QueueId queue_id) const;
    HazardResult
    DetectHazard(const ResourceAccessState& recorded_use, QueueId queue_id, const ResourceUsageRange& tag_range) const;

    HazardResult
    DetectAsyncHazard(const SyncAccessInfo& usage_info, ResourceUsageTag start_tag, QueueId queue_id) const;
    HazardResult DetectAsyncHazard(const ResourceAccessState& recorded_use,
                                   const ResourceUsageRange& tag_range,
                                   ResourceUsageTag start_tag,
                                   QueueId queue_id) const;

    HazardResult DetectBarrierHazard(const SyncAccessInfo& usage_info,
                                     QueueId queue_id,
                                     VkPipelineStageFlags2 source_exec_scope,
                                     const SyncAccessFlags& source_access_scope) const;
    HazardResult DetectBarrierHazard(const SyncAccessInfo& usage_info,
                                     const ResourceAccessState& scope_state,
                                     VkPipelineStageFlags2 source_exec_scope,
                                     const SyncAccessFlags& source_access_scope,
                                     QueueId event_queue,
    ...
    bool HasPendingState() const {
        return (0 != pending_layout_transition) || (last_write && last_write->HasPendingState());
    }
    bool HasWriteOp() const { return last_write.has_value(); }
    SyncAccessIndex LastWriteOp() const {
        return last_write.has_value() ? last_write->Index() : SYNC_ACCESS_INDEX_NONE;
    }
    ...
    bool operator==(const ResourceAccessState& rhs) const {
        ...
        const bool read_write_same =
            write_same && (last_read_stages == rhs.last_read_stages) && (last_reads == rhs.last_reads);
    ...
    SyncAccessFlags GetWriteBarriers() const {
        return last_write.has_value() ? last_write->Barriers() : SyncAccessFlags();
    }
    void SetQueueId(QueueId id);
    ...
    struct QueueScopeOps {
        bool WriteInScope(const SyncBarrier& barrier, const ResourceAccessState& access) const {
            return access.WriteInQueueSourceScopeOrChain(
                queue, barrier.src_exec_scope.exec_scope, barrier.src_access_scope);
        }
        bool ReadInScope(const SyncBarrier& barrier, const ReadState& read_state) const {
    ...
    struct EventScopeOps {
        bool WriteInScope(const SyncBarrier& barrier, const ResourceAccessState& access) const {
            return access.WriteInEventScope(
                barrier.src_exec_scope.exec_scope, barrier.src_access_scope, scope_queue, scope_tag);
        }
        bool ReadInScope(const SyncBarrier& barrier, const ReadState& read_state) const {
  ...
  private:
    ...
    // VkPipelineStageFlags2 write_dependency_chain;  // intiially zero, but accumulating the dstStages of barriers if
    // they chain. ResourceUsageTag write_tag; QueueId write_queue;
...
using ResourceRangeMergeIterator =
    sparse_container::parallel_iterator<ResourceAccessRangeMap, const ResourceAccessRangeMap>;
    ...
    if (layout_transition) {
        ...
        if (!pending_layout_transition) {
            ...
            for (auto& read_access : last_reads) {
                // The | implements the "dependency chain" logic for this access, as the barriers field stores the
                // second sync scope
            ...
            for (auto& read_access : last_reads) {
                if (0 != ((read_access.stage | read_access.sync_stages) & stages_in_scope)) {
                    // If this stage, or any stage known to be synchronized after it are in scope, apply the barrier to
                    // this read NOTE: Forwarding barriers to known prior stages changes the sync_stages from shallow to
                    // deep, because the
...
bool ResourceAccessState::ApplyPredicatedWait(Predicate& predicate) {
    ...
    for (auto& read_access : last_reads) {
        if (predicate(read_access)) {
            // If we know this stage is before any stage we syncing, or if the predicate tells us that we are waited
            // for..
            sync_reads |= read_access.stage;
        }
    }

    // Now that we know the reads directly in scopejust need to go over the list again to pick up the "known earlier"
    // stages. NOTE: sync_stages is "deep" catching all stages synchronized after it because we forward barriers
...
SyncBarrier::SyncBarrier(const Barrier& barrier, const SyncExecScope& src, const SyncExecScope& dst) :
    src_exec_scope(src), src_access_scope(SyncStageAccess::AccessScope(src.valid_accesses, barrier.srcAccessMask)),
    dst_exec_scope(dst), dst_access_scope(SyncStageAccess::AccessScope(dst.valid_accesses, barrier.dstAccessMask)) {}
