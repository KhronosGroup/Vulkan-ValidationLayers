...
CommandBufferAccessContext::CommandBufferAccessContext(const SyncValidator& sync_validator, VkQueueFlags queue_flags) :
    CommandExecutionContext(sync_validator, queue_flags), cb_state_(), access_log_(std::make_shared<AccessLog>()),
    cbs_referenced_(std::make_shared<CommandBufferSet>()), command_number_(0), subcommand_number_(0), reset_count_(0),
    cb_access_context_(), current_context_(&cb_access_context_), events_context_(), render_pass_contexts_(),
    current_renderpass_context_(), sync_ops_()
{}
    ...
    CommandBufferAccessContext(from.sync_state_, from.cb_state_->GetQueueFlags())
{
    ...
    access_log_ =
        std::make_shared<AccessLog>(*from.access_log_); // potentially large, but no choice given tagging lookup.
    ...
    // The proxy has flatten the current render pass context (if any), but the async contexts are needed for hazard
    // detection
                                                      ...
                                                      const RecordObject&                    record_obj)
{
    ...
    if (0 == (info.info.flags & VK_RENDERING_RESUMING_BIT))
    {
        ...
        for (uint32_t i = 0; i < attachment_count; i++)
        {
            ...
            GetCurrentAccessContext()->UpdateAccessState(
                attachment.view_gen, load_index, attachment.GetOrdering(), ResourceUsageTagEx{ tag });
        }
    }
...
bool CommandBufferAccessContext::ValidateEndRendering(const ErrorObject& error_obj) const
{
    ...
    if (dynamic_rendering_info_ && (0 == (dynamic_rendering_info_->info.flags & VK_RENDERING_SUSPENDING_BIT)))
    {
        ...
        auto report_resolve_hazard = [this](const HazardResult&         hazard,
                                            const Location&             loc,
            ...
            const auto    error =
                sync_state_.error_messages_.EndRenderingResolveError(hazard, image_handle, resolve_mode, *this);
        ...
        for (uint32_t i = 0; i < attachment_count && !skip; i++)
        {
            ...
            if (attachment.resolve_gen)
            {
                ...
                {
                    Location loc = attachment.GetLocation(error_obj.location, i);
                    skip |= report_resolve_hazard(
                        hazard, loc.dot(vvl::Field::imageView), attachment.view->Handle(), attachment.info.resolveMode);
                }
                if (!skip)
                {
                    ...
                    {
                        Location loc = attachment.GetLocation(error_obj.location, i);
                        skip |= report_resolve_hazard(hazard,
                                                      loc.dot(vvl::Field::resolveImageView),
                                                      attachment.resolve_view->Handle(),
            ...
            if (store_usage != SYNC_ACCESS_INDEX_NONE)
            {
                ...
                if (hazard.IsHazard())
                {
                    ...
                    const auto              error = sync_state_.error_messages_.EndRenderingStoreError(
                        hazard, image_handle, attachment.info.storeOp, *this);
                    skip |= sync_state_.SyncError(hazard.Hazard(), objlist, loc.dot(vvl::Field::imageView), error);
                }
            }
        }
        ...
        for (uint32_t i = 0; i < attachment_count; i++)
        {
            ...
            if (attachment.resolve_gen)
            {
                ...
                access_context->UpdateAccessState(
                    attachment.view_gen, kResolveRead, kResolveOrder, ResourceUsageTagEx{ store_tag });
                access_context->UpdateAccessState(
                    *attachment.resolve_gen, kResolveWrite, kResolveOrder, ResourceUsageTagEx{ store_tag });
            }
            ...
            if (store_index == SYNC_ACCESS_INDEX_NONE)
                continue;
            access_context->UpdateAccessState(
                attachment.view_gen, store_index, kStoreOrder, ResourceUsageTagEx{ store_tag });
        }
    }
            ...
            for (uint32_t index = 0; index < binding->count; index++)
            {
                ...
                switch (descriptor->GetClass())
                {
                    ...
                    case DescriptorClass::Image:
                    {
                        ...
                        // NOTE: ImageSamplerDescriptor inherits from ImageDescriptor, so this cast works for both
                        // types.
                        ...
                        if (img_view_state->IsDepthSliced())
                        {
                            ...
                            // Descriptors, unless VK_EXT_image_2d_view_of_3d is supported, which it isn't at the
                            // moment. See: VUID 00343
                            continue;
                        }
                        ...
                        if (sync_index == SYNC_FRAGMENT_SHADER_INPUT_ATTACHMENT_READ)
                        {
                            const VkExtent3D extent =
                                CastTo3D(cb_state_->active_render_pass_begin_info.renderArea.extent);
                            const VkOffset3D offset =
                                CastTo3D(cb_state_->active_render_pass_begin_info.renderArea.offset);
                            // Input attachments are subject to raster ordering rules
                            hazard = current_context_->DetectHazard(
                                *img_view_state, offset, extent, sync_index, SyncOrdering::kRaster);
                        }
                        else
                        {
                        ...
                        {
                            const auto error = error_messages_.DrawDispatchImageError(hazard,
                                                                                      *this,
                                                                                      *img_view_state,
                                                                                      *pipe,
                                                                                      *descriptor_set,
                    ...
                    case DescriptorClass::TexelBuffer:
                    {
                        ...
                        {
                            const auto error =
                                error_messages_.DrawDispatchTexelBufferError(hazard,
                                                                             *this,
                                                                             *buf_view_state,
                                                                             *pipe,
                                                                             *descriptor_set,
                    ...
                    case DescriptorClass::GeneralBuffer:
                    {
                        ...
                        if (hazard.IsHazard() && !sync_state_.SupressedBoundDescriptorWAW(hazard))
                        {
                            const auto error = error_messages_.DrawDispatchBufferError(hazard,
                                                                                       *this,
                                                                                       *buf_state,
                                                                                       *pipe,
                                                                                       *descriptor_set,
                ...
                switch (descriptor->GetClass())
                {
                    ...
                    case DescriptorClass::Image:
                    {
                        ...
                        const ResourceUsageTagEx tag_ex =
                            AddCommandHandle(tag, img_view_state->GetImageState()->Handle());
                        if (sync_index == SYNC_FRAGMENT_SHADER_INPUT_ATTACHMENT_READ)
                        {
                            const VkExtent3D extent =
                                CastTo3D(cb_state_->active_render_pass_begin_info.renderArea.extent);
                            ...
                            current_context_->UpdateAccessState(
                                *img_view_state, sync_index, SyncOrdering::kRaster, offset, extent, tag_ex);
                        }
                        else
                        {
                            current_context_->UpdateAccessState(
                                *img_view_state, sync_index, SyncOrdering::kNonAttachment, tag_ex);
                        }
                        break;
                    }
                    case DescriptorClass::TexelBuffer:
                    {
                        ...
                        current_context_->UpdateAccessState(
                            *buf_state, sync_index, SyncOrdering::kNonAttachment, range, tag_ex);
                        break;
                    }
                    case DescriptorClass::GeneralBuffer:
                    {
...
bool CommandBufferAccessContext::ValidateDrawVertex(std::optional<uint32_t> vertexCount,
                                                    ...
                                                    const Location&         loc) const
{
    ...
    for (const auto& [_, binding_state] : vertex_bindings)
    {
        ...
        if (const auto* vertex_buffer = vvl::Find(binding_buffers, binding_desc.binding))
        {
            ...
            { // entire vertex buffer
                range = MakeRange(*vertex_buffer);
            }

            auto hazard =
                current_context_->DetectHazard(*buf_state, SYNC_VERTEX_ATTRIBUTE_INPUT_VERTEX_ATTRIBUTE_READ, range);
            if (hazard.IsHazard())
            {
...
void CommandBufferAccessContext::RecordDrawVertex(std::optional<uint32_t> vertexCount,
                                                  uint32_t                firstVertex,
                                                  const ResourceUsageTag  tag)
{
    const auto* pipe = cb_state_->GetCurrentPipeline(VK_PIPELINE_BIND_POINT_GRAPHICS);
    ...
    for (const auto& [_, binding_state] : vertex_bindings)
    {
        ...
        if (const auto* vertex_buffer = vvl::Find(binding_buffers, binding_desc.binding))
        {
            ...
            const ResourceUsageTagEx tag_ex = AddCommandHandle(tag, buf_state->Handle());
            current_context_->UpdateAccessState(*buf_state,
...
bool CommandBufferAccessContext::ValidateDrawVertexIndex(uint32_t        index_count,
                                                         uint32_t        firstIndex,
                                                         const Location& loc) const
{
    bool        skip            = false;
...
void CommandBufferAccessContext::RecordDrawVertexIndex(uint32_t               indexCount,
                                                       uint32_t               firstIndex,
                                                       const ResourceUsageTag tag)
{
    const auto& index_binding   = cb_state_->index_buffer_binding;
    ...
    current_context_->UpdateAccessState(
        *index_buf_state, SYNC_INDEX_INPUT_INDEX_READ, SyncOrdering::kNonAttachment, range, tag_ex);
...
bool CommandBufferAccessContext::ValidateDrawDynamicRenderingAttachment(const Location& location) const
{
    ...
    for (const auto output_location : list)
    {
        ...
        HazardResult hazard = access_context.DetectHazard(
            attachment.view_gen, SYNC_COLOR_ATTACHMENT_OUTPUT_COLOR_ATTACHMENT_WRITE, SyncOrdering::kColorAttachment);
        if (hazard.IsHazard())
        {
    ...
    for (uint32_t i = info.info.colorAttachmentCount; i < attachment_count; i++)
    {
        ...
        if (writeable)
        {
            HazardResult hazard = access_context.DetectHazard(attachment.view_gen,
...
void CommandBufferAccessContext::RecordDrawDynamicRenderingAttachment(ResourceUsageTag tag)
{
    ...
    for (const auto output_location : list)
    {
        ...
        access_context.UpdateAccessState(attachment.view_gen,
...
bool CommandBufferAccessContext::ValidateClearAttachment(const Location&          loc,
...
void CommandBufferAccessContext::RecordClearAttachment(ResourceUsageTag         tag,
                                                       const VkClearAttachment& clear_attachment,
    ...
    const std::vector<const syncval_state::ImageViewState*>& attachment_views)
{
    ...
    render_pass_contexts_.emplace_back(std::make_unique<RenderPassAccessContext>(
        rp_state, render_area, GetQueueFlags(), attachment_views, &cb_access_context_));
...
void CommandBufferAccessContext::RecordExecutedCommandBuffer(const CommandBufferAccessContext& recorded_cb_context)
{
    ...
    // Just run through the barriers ignoring the usage from the recorded context, as Resolve will overwrite outdated
    // state
...
void CommandBufferAccessContext::ResolveExecutedCommandBuffer(const AccessContext& recorded_context,
                                                              ResourceUsageTag     offset)
{
...
void CommandBufferAccessContext::ImportRecordedAccessLog(const CommandBufferAccessContext& recorded_context)
{
    ...
    access_log_->insert(
        access_log_->end(), recorded_context.access_log_->cbegin(), recorded_context.access_log_->cend());
...
ResourceUsageTag CommandBufferAccessContext::NextCommandTag(vvl::Func                           command,
...
ResourceUsageTag CommandBufferAccessContext::NextSubcommandTag(vvl::Func                           command,
                                                               ResourceUsageRecord::SubcommandType subcommand)
{
    subcommand_number_++;
...
ResourceUsageTagEx CommandBufferAccessContext::AddCommandHandle(ResourceUsageTag         tag,
...
void CommandBufferAccessContext::AddSubcommandHandle(ResourceUsageTag         tag,
                                                     const VulkanTypedHandle& typed_handle,
                                                     uint32_t                 index)
{
    assert(tag < access_log_->size());
...
bool CommandBufferAccessContext::ValidateClearAttachment(const Location& loc, const ClearAttachmentInfo& info) const
{
    ...
    if (info.aspects_to_clear & kColorAspects)
    {
        ...
        HazardResult hazard = access_context->DetectHazard(*info.view->GetImageState(),
                                                           subresource_range,
                                                           info.offset,
                                                           info.extent,
                                                           info.view->IsDepthSliced(),
                                                           SYNC_COLOR_ATTACHMENT_OUTPUT_COLOR_ATTACHMENT_WRITE,
                                                           SyncOrdering::kColorAttachment);
        if (hazard.IsHazard())
        {
            ...
            const auto          error =
                error_messages_.ClearColorAttachmentError(hazard, *this, info.GetSubpassAttachmentText());
            skip |= sync_state_.SyncError(hazard.Hazard(), objlist, loc, error);
        }
    }

    constexpr VkImageAspectFlagBits depth_stencil_aspects[2] = { VK_IMAGE_ASPECT_DEPTH_BIT,
    ...
    for (const auto aspect : depth_stencil_aspects)
    {
        if (info.aspects_to_clear & aspect)
        {
            ...
            // vkCmdClearAttachments depth/stencil writes are executed by the EARLY_FRAGMENT_TESTS_BIT and
            // LATE_FRAGMENT_TESTS_BIT stages. The implementation tracks the most recent access, which happens in the
            // LATE_FRAGMENT_TESTS_BIT stage.
            HazardResult hazard = access_context->DetectHazard(*info.view->GetImageState(),
            ...
            {
                const LogObjectList objlist(cb_state_->Handle(), info.view->Handle());
                const auto          error = error_messages_.ClearDepthStencilAttachmentError(
                    hazard, *this, info.GetSubpassAttachmentText(), aspect);
                skip |= sync_state_.SyncError(hazard.Hazard(), objlist, loc, error);
            }
        }
    }
...
void CommandBufferAccessContext::CheckCommandTagDebugCheckpoint()
{
    ...
    if (sync_state_.debug_command_number == command_number_ && sync_state_.debug_reset_count == reset_count_)
    {
        ...
        if (cmdbuf_match)
        {
            sync_state_.LogInfo("SYNCVAL_DEBUG_COMMAND",
                                LogObjectList(),
                                Location(access_log_->back().command),
                                "Command stream has reached command #%" PRIu32
                                " in command buffer %s with reset count #%" PRIu32,
                                sync_state_.debug_command_number,
...
syncval_state::CommandBuffer::CommandBuffer(SyncValidator&                     dev,
                                            VkCommandBuffer                    handle,
                                            const VkCommandBufferAllocateInfo* allocate_info,
                                            const vvl::CommandPool*            pool) :
    vvl::CommandBuffer(dev, handle, allocate_info, pool),
    access_context(dev, this)
{}
