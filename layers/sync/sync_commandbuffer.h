...
class AlternateResourceUsage {
  public:
    ...
    std::ostream& Format(std::ostream& out, const SyncValidator& sync_state) const {
        return record_->Format(out, sync_state);
    };
...
struct ResourceCmdUsageRecord {
    ...
    ResourceCmdUsageRecord(vvl::Func command_,
                           uint32_t seq_num_,
                           SubcommandType sub_type_,
                           uint32_t sub_command_,
                           const vvl::CommandBuffer* cb_state_,
                           uint32_t reset_count_) :
        command(command_),
        seq_num(seq_num_), sub_command_type(sub_type_), sub_command(sub_command_), cb_state(cb_state_),
    ...
    // This is somewhat repetitive, but it prevents the need for Exec/Submit time touchup, after which usage records can
    // be from different command buffers and resets. plain pointer as a shared pointer is held by the context storing
    // this record
...
struct ResourceUsageRecord : public ResourceCmdUsageRecord {
    struct FormatterState {
        FormatterState(const SyncValidator& sync_state_,
                       const ResourceUsageRecord& record_,
                       const vvl::CommandBuffer* cb_state_,
                       const DebugNameProvider* debug_name_provider_,
                       uint32_t handle_index) :
            sync_state(sync_state_),
            record(record_), ex_cb_state(cb_state_), debug_name_provider(debug_name_provider_),
    ...
    FormatterState Formatter(const SyncValidator& sync_state,
                             const vvl::CommandBuffer* ex_cb_state,
                             const DebugNameProvider* debug_name_provider,
    ...
    ResourceUsageRecord(vvl::Func command_,
                        uint32_t seq_num_,
                        SubcommandType sub_type_,
                        uint32_t sub_command_,
                        const vvl::CommandBuffer* cb_state_,
...
class CommandBufferAccessContext : public CommandExecutionContext, DebugNameProvider {
  public:
    ...
    ResourceUsageTag RecordBeginRenderPass(vvl::Func command,
    ...
    bool ValidateClearAttachment(const Location& loc,
                                 const VkClearAttachment& clear_attachment,
                                 const VkClearRect& rect) const;
    void
    RecordClearAttachment(ResourceUsageTag tag, const VkClearAttachment& clear_attachment, const VkClearRect& rect);
    ...
    ResourceUsageTag
    NextCommandTag(vvl::Func command,
    ...
    ResourceUsageTagEx
    AddCommandHandle(ResourceUsageTag tag, const VulkanTypedHandle& typed_handle, uint32_t index = vvl::kNoIndex32);
    ...
    void
    AddSubcommandHandle(ResourceUsageTag tag, const VulkanTypedHandle& typed_handle, uint32_t index = vvl::kNoIndex32);
...
class CommandBuffer : public vvl::CommandBuffer {
  public:
    ...
    CommandBuffer(SyncValidator& dev,
