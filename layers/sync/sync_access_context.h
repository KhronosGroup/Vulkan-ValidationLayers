...
// This functor applies a collection of barriers, updating the "pending state" in each touched memory range, and
// optionally resolves the pending state. Suitable for processing Global memory barriers, or Subpass Barriers when the
// "final" barrier of a collection is known/present.
...
class ApplyBarrierOpsFunctor {
  public:
    ...
    inline Iterator
    Infill(ResourceAccessRangeMap* accesses, const Iterator& pos, const ResourceAccessRange& range) const {
        if (!infill_default_) {
            return pos;
        }
    ...
    void operator()(const Iterator& pos) const {
        ...
        if (resolve_) {
            // If this is the last (or only) batch, we can do the pending resolve as the last step in this operation to
            // avoid another walk
...
class ResolvePendingBarrierFunctor
    : public ApplyBarrierOpsFunctor<NoopBarrierAction, small_vector<NoopBarrierAction, 1>> {
...
struct ApplyTrackbackStackAction {
    explicit ApplyTrackbackStackAction(const std::vector<SyncBarrier>& barriers_,
        ...
        barriers(barriers_),
        previous_barrier(previous_barrier_) {}
    void operator()(ResourceAccessState* access) const {
        assert(access);
...
class AttachmentViewGen {
  public:
    enum Gen {
        kViewSubresource = 0,
        kRenderArea = 1,
        kDepthOnlyRenderArea = 2,
        kStencilOnlyRenderArea = 3,
        kGenSize = 4
    };
    AttachmentViewGen(const syncval_state::ImageViewState* image_view,
...
class AccessContext {
  public:
    ...
    HazardResult
    DetectHazard(const vvl::Buffer& buffer, SyncAccessIndex access_index, const ResourceAccessRange& range) const;
    HazardResult DetectHazard(const ImageState& image,
    ...
    HazardResult DetectHazard(const ImageRangeGen& ref_range_gen,
                              SyncAccessIndex current_usage,
                              SyncOrdering ordering_rule = SyncOrdering::kOrderingNone) const;
    HazardResult DetectHazard(const ImageViewState& image_view,
                              const VkOffset3D& offset,
                              const VkExtent3D& extent,
                              SyncAccessIndex current_usage,
                              SyncOrdering ordering_rule) const;
    HazardResult DetectHazard(const AttachmentViewGen& view_gen,
                              AttachmentViewGen::Gen gen_type,
                              SyncAccessIndex current_usage,
                              SyncOrdering ordering_rule) const;
    HazardResult DetectHazard(const vvl::VideoSession& vs_state,
    ...
    HazardResult DetectImageBarrierHazard(const ImageState& image,
                                          const VkImageSubresourceRange& subresource_range,
    ...
    HazardResult DetectImageBarrierHazard(const AttachmentViewGen& attachment_view,
    ...
    HazardResult DetectFirstUseHazard(QueueId queue_id,
    ...
    void ResolveFromContext(ResolveOp&& resolve_op,
                            const AccessContext& from_context,
                            const ResourceAccessState* infill_state = nullptr,
                            bool recur_to_infill = false);
    template <typename ResolveOp, typename RangeGenerator>
    void ResolveFromContext(ResolveOp&& resolve_op,
                            const AccessContext& from_context,
                            RangeGenerator range_gen,
                            const ResourceAccessState* infill_state = nullptr,
                            bool recur_to_infill = false);

    void UpdateAccessState(const vvl::Buffer& buffer,
                           SyncAccessIndex current_usage,
                           SyncOrdering ordering_rule,
                           const ResourceAccessRange& range,
                           ResourceUsageTagEx tag_ex);
    void UpdateAccessState(const ImageState& image,
                           SyncAccessIndex current_usage,
                           SyncOrdering ordering_rule,
                           const VkImageSubresourceRange& subresource_range,
    ...
    void UpdateAccessState(const ImageState& image,
                           ...
                           const VkExtent3D& extent,
                           ResourceUsageTagEx tag_ex);
    void UpdateAccessState(const AttachmentViewGen& view_gen,
                           AttachmentViewGen::Gen gen_type,
                           SyncAccessIndex current_usage,
                           SyncOrdering ordering_rule,
                           ResourceUsageTag tag);
    void UpdateAccessState(const ImageViewState& image_view,
                           SyncAccessIndex current_usage,
                           SyncOrdering ordering_rule,
                           const VkOffset3D& offset,
                           const VkExtent3D& extent,
    ...
    void UpdateAccessState(const ImageViewState& image_view,
                           ...
                           SyncOrdering ordering_rule,
                           ResourceUsageTagEx tag_ex);
    void UpdateAccessState(const ImageRangeGen& range_gen,
                           SyncAccessIndex current_usage,
                           SyncOrdering ordering_rule,
                           ResourceUsageTagEx tag_ex);
    void UpdateAccessState(ImageRangeGen& range_gen,
                           SyncAccessIndex current_usage,
                           SyncOrdering ordering_rule,
                           ResourceUsageTagEx tag_ex);
    void UpdateAccessState(const vvl::VideoSession& vs_state,
                           const vvl::VideoPictureResource& resource,
                           SyncAccessIndex current_usage,
                           ResourceUsageTag tag);
    ...
    AccessContext(uint32_t subpass,
  ...
  private:
    template <typename Action>
    static void
    UpdateMemoryAccessRangeState(ResourceAccessRangeMap& accesses, Action& action, const ResourceAccessRange& range);
    ...
    struct UpdateMemoryAccessStateFunctor {
        ...
        void operator()(const Iterator& pos) const;
        UpdateMemoryAccessStateFunctor(const AccessContext& context_,
                                       SyncAccessIndex usage_,
                                       SyncOrdering ordering_rule_,
                                       ResourceUsageTagEx tag_ex) :
            context(context_),
            usage_info(SyncStageAccess::AccessInfo(usage_)), ordering_rule(ordering_rule_), tag_ex(tag_ex) {}
    ...
    // Follow the context previous to access the access state, supporting "lazy" import into the context. Not intended
    // for subpass layout transition, as the pending state handling is more complex
    ...
    void ResolvePreviousAccess(const ResourceAccessRange& range,
    ...
    void ResolvePreviousAccessStack(const ResourceAccessRange& range,
                                    ResourceAccessRangeMap* descent_map,
                                    const ResourceAccessState* infill_state,
                                    const BarrierAction& previous_barrie) const;
    template <typename BarrierAction>
    void ResolveAccessRange(const ResourceAccessRange& range,
    ...
    template <typename Detector, typename RangeGen>
    HazardResult
    DetectHazardGeneratedRanges(Detector& detector, const RangeGen& range_gen, DetectOptions options) const {
        RangeGen mutable_gen(range_gen);
        return DetectHazardGeneratedRanges<Detector, RangeGen>(detector, mutable_gen, options);
    }
    template <typename Detector>
    HazardResult DetectHazard(Detector& detector,
    ...
    template <typename Detector>
    HazardResult DetectHazardOneRange(Detector& detector,
                                      bool detect_prev,
                                      ResourceAccessRangeMap::const_iterator& pos,
                                      const ResourceAccessRangeMap::const_iterator& the_end,
                                      const ResourceAccessRange& range) const;

    template <typename Detector, typename RangeGen>
    HazardResult DetectAsyncHazard(const Detector& detector,
                                   const RangeGen& const_range_gen,
                                   ResourceUsageTag async_tag,
    ...
    // These contexts *must* have the same lifespan as this context, or be cleared, before the referenced contexts can
    // expire
...
// The semantics of the InfillUpdateOps of infill_update_range are slightly different than for the
// UpdateMemoryAccessState Action operations, as this simplifies the generic traversal.  So we wrap them in a semantics
// Adapter to get the same effect.
template <typename Action>
struct ActionToOpsAdapter {
    ...
    void infill(Map& accesses, const Iterator& pos, const Range& infill_range) const {
        ...
        if (infill == accesses.end())
            return; // Allow action to 'pass' on filling in the blanks

        // Need to apply the action to the Infill.  'infill_update_range' expect ops.infill to be completely done with
        // the infill_range, where as Action::Infill assumes the caller will apply the action() logic to the
        // infill_range
...
void AccessContext::ApplyToContext(const Action& barrier_action) {
    // Note: Barriers do *not* cross context boundaries, applying to accessess within.... (at least for renderpass
    // subpasses)
    UpdateMemoryAccessRangeState(access_state_map_, barrier_action, kFullRange);
}

template <typename Action>
void AccessContext::UpdateMemoryAccessRangeState(ResourceAccessRangeMap& accesses,
                                                 Action& action,
                                                 const ResourceAccessRange& range) {
...
template <typename Action>
void AccessContext::ApplyUpdateAction(const AttachmentViewGen& view_gen,
                                      AttachmentViewGen::Gen gen_type,
                                      const Action& action) {
...
template <typename Detector, typename RangeGen>
HazardResult AccessContext::DetectAsyncHazard(const Detector& detector,
                                              const RangeGen& const_range_gen,
                                              ResourceUsageTag async_tag,
                                              QueueId async_queue_id) const {
    using RangeType = typename RangeGen::RangeType;
    ...
    auto do_async_hazard_check = [&detector, async_tag, async_queue_id, &hazard](
                                     const RangeType& range, const ConstIterator& end, ConstIterator& pos) {
...
template <typename Detector>
HazardResult AccessContext::DetectHazardOneRange(Detector& detector,
                                                 bool detect_prev,
                                                 ResourceAccessRangeMap::const_iterator& pos,
                                                 const ResourceAccessRangeMap::const_iterator& the_end,
                                                 const ResourceAccessRange& range) const {
...
template <typename Detector>
HazardResult
AccessContext::DetectHazardRange(Detector& detector, const ResourceAccessRange& range, DetectOptions options) const {
    SingleRangeGenerator range_gen(range);
    return DetectHazardGeneratedRanges(detector, range_gen, options);
}

template <typename BarrierAction>
void AccessContext::ResolveAccessRange(const ResourceAccessRange& range,
                                       BarrierAction& barrier_action,
                                       ResourceAccessRangeMap* resolve_map,
                                       const ResourceAccessState* infill_state,
    ...
    while (current->range.non_empty() && range.includes(current->range.begin)) {
        ...
        if (current->pos_B->valid) {
            ...
            if (recur_to_infill) {
                ...
                // Since we will be recurring back up the DAG, expand the gap descent to cover the full range for which
                // B is not valid, to minimize that recurrence
                ...
                // Given that there could be gaps we need to seek carefully to not repeatedly search the same gaps in
                // the next iterator of the outer while.
                ...
                const auto seek_to =
                    recurrence_range.end - 1; // The subtraction is safe as range can't be empty (loop condition)
                ...
                auto inserted =
                    resolve_map->insert(current->pos_A->lower_bound, std::make_pair(current->range, *infill_state));
                current.invalidate_A(
                    inserted); // Update the parallel iterator to point at the correct segment after insert
            }
        }
...
template <typename Detector, typename RangeGen>
HazardResult
AccessContext::DetectHazardGeneratedRanges(Detector& detector, RangeGen& range_gen, DetectOptions options) const {
    HazardResult hazard;
    ...
    if (static_cast<uint32_t>(options) & DetectOptions::kDetectAsync) {
        ...
        for (const auto& async_ref : async_) {
            hazard = async_ref.Context().DetectAsyncHazard(
                detector, range_gen, async_ref.StartTag(), async_ref.GetQueueId());
            if (hazard.IsHazard())
                return hazard;
        }
    }
    ...
    using RangeType = typename RangeGen::RangeType;
    using ConstIterator = ResourceAccessRangeMap::const_iterator;
    auto do_detect_hazard_range =
        [this, &detector, &hazard, detect_prev](const RangeType& range, const ConstIterator& end, ConstIterator& pos) {
...
template <typename ResolveOp>
void AccessContext::ResolveFromContext(ResolveOp&& resolve_op,
                                       const AccessContext& from_context,
                                       const ResourceAccessState* infill_state,
...
template <typename BarrierAction>
void AccessContext::ResolvePreviousAccessStack(const ResourceAccessRange& range,
                                               ResourceAccessRangeMap* descent_map,
                                               const ResourceAccessState* infill_state,
