...
bool AcquiredImage::Invalid() const {
    return vvl::StateObject::Invalid(image);
}

SignalInfo::SignalInfo(const std::shared_ptr<const vvl::Semaphore>& semaphore_state,
                       const QueueBatchContext::Ptr& batch,
...
void SignalsUpdate::OnBinarySignal(const vvl::Semaphore& semaphore_state,
                                   const QueueBatchContext::Ptr& batch,
    ...
    if (!found_unsignal_request) {
        if (vvl::Contains(binary_signal_requests, semaphore) ||
...
bool SignalsUpdate::OnTimelineSignal(const vvl::Semaphore& semaphore_state,
...
bool SignalsUpdate::RegisterSignals(const BatchContextPtr& batch,
                                    const vvl::span<const VkSemaphoreSubmitInfo>& submit_signals) {
...
syncval_state::Swapchain::Swapchain(ValidationStateTracker& dev_data,
...
PresentedImage syncval_state::Swapchain::MovePresentedImage(uint32_t image_index) {
    ...
    if (ret_val.Invalid()) {
        // If this is the first time the image has been acquired, then it's valid to have no present record, so we
        // create one Note: It's also possible this is an invalid acquire... but that's CoreChecks/Parameter
        // validation's job to report
...
class ApplySemaphoreBarrierAction {
  public:
    ApplySemaphoreBarrierAction(const SemaphoreScope& signal, const SemaphoreScope& wait) :
...
class ApplyAcquireNextSemaphoreAction {
  public:
    ...
    void operator()(ResourceAccessState* access) const {
        // Note that the present operations may or may not be present, given that the fence wait may have cleared them
        // out. Also, if a subsequent present has happened, we *don't* want to protect that...
...
QueueBatchContext::QueueBatchContext(const SyncValidator& sync_state, const QueueSyncState& queue_state) :
    CommandExecutionContext(sync_state, queue_state.GetQueueFlags()), queue_state_(&queue_state), tag_range_(0, 0),
    current_access_context_(&access_context_), batch_log_(),
...
QueueBatchContext::QueueBatchContext(const SyncValidator& sync_state) :
    CommandExecutionContext(sync_state, 0), queue_state_(), tag_range_(0, 0), current_access_context_(&access_context_),
    batch_log_(), queue_sync_tag_(sync_state.GetQueueIdLimit(), ResourceUsageTag(0)) {
    sync_state_.stats.AddQueueBatchContext();
}

QueueBatchContext::~QueueBatchContext() {
    sync_state_.stats.RemoveQueueBatchContext();
}
...
void QueueBatchContext::OnResourceDestroyed(const ResourceAccessRange& resource_range) {
    ...
    access_context_.EraseIf([&resource_range](ResourceAccessRangeMap::value_type& access) {
        return resource_range.includes(access.first);
    });
}

void QueueBatchContext::BeginRenderPassReplaySetup(ReplayState& replay, const SyncOpBeginRenderPass& begin_op) {
    current_access_context_ =
        replay.ReplayStateRenderPassBegin(queue_state_->GetQueueFlags(), begin_op, access_context_);
}
...
void QueueBatchContext::ResolvePresentSemaphoreWait(const SignalInfo& signal_info,
    ...
    SemaphoreScope wait_scope{ queue_id,
                               SyncExecScope::MakeDst(queue_flags, VK_PIPELINE_STAGE_2_PRESENT_ENGINE_BIT_SYNCVAL) };

    // If signal queue == wait queue, signal is treated as a memory barrier with an access scope equal to the present
    // accesses
...
void QueueBatchContext::ResolveSubmitSemaphoreWait(const SignalInfo& signal_info, VkPipelineStageFlags2 wait_mask) {
    ...
    if (signal_info.acquired_image) {
        ...
        // Grab the reset of the presenting QBC, with no effective barrier, won't overwrite the acquire, as the tag is
        // newer
        SyncBarrier noop_barrier;
        const BatchBarrierOp noop_barrier_op(wait_scope.queue, noop_barrier);
        access_context_.ResolveFromContext(noop_barrier_op, from_context);
        ...
        if (signal_scope.queue == wait_scope.queue) {
            ...
            events_context_.ApplyBarrier(
                sem_barrier.src_exec_scope, sem_barrier.dst_exec_scope, ResourceUsageRecord::kMaxIndex);
        } else {
...
QueueBatchContext::ResolvePresentWaits(vvl::span<const VkSemaphore> wait_semaphores,
                                       const PresentedImages& presented_images,
...
bool QueueBatchContext::DoQueuePresentValidate(const Location& loc, const PresentedImages& presented_images) {
    ...
    for (size_t index = 0; index < presented_images.size(); ++index) {
        ...
        if (hazard.IsHazard()) {
            ...
            const auto error = sync_state_.error_messages_.PresentError(
                hazard, *this, presented.present_index, swap_handle, presented.image_index, image_handle);
...
void QueueBatchContext::DoPresentOperations(const PresentedImages& presented_images) {
    ...
    for (const auto& presented : presented_images) {
        // Update memory state
        presented.UpdateMemoryAccess(
            SYNC_PRESENT_ENGINE_SYNCVAL_PRESENT_PRESENTED_SYNCVAL, presented.tag, access_context_);
    }
}
...
void QueueBatchContext::DoAcquireOperation(const PresentedImage& presented) {
    ...
    presented.UpdateMemoryAccess(
        SYNC_PRESENT_ENGINE_SYNCVAL_PRESENT_ACQUIRE_READ_SYNCVAL, tag_range_.begin, access_context_);
}
...
std::vector<QueueBatchContext::ConstPtr>
QueueBatchContext::RegisterAsyncContexts(const std::vector<ConstPtr>& batches_resolved) {
...
std::vector<BatchContextConstPtr>
QueueBatchContext::ResolveSubmitWaits(vvl::span<const VkSemaphoreSubmitInfo> wait_infos,
...
bool QueueBatchContext::ValidateSubmit(const std::vector<CommandBufferConstPtr>& command_buffers,
...
std::ostream& QueueBatchContext::PresentResourceRecord::Format(std::ostream& out,
                                                               const SyncValidator& sync_state) const {
...
std::ostream& QueueBatchContext::AcquireResourceRecord::Format(std::ostream& out,
                                                               const SyncValidator& sync_state) const {
...
std::vector<QueueBatchContext::ConstPtr>
SyncValidator::GetLastBatches(std::function<bool(const QueueBatchContext::ConstPtr&)> filter) const {
...
std::vector<QueueBatchContext::Ptr>
SyncValidator::GetLastBatches(std::function<bool(const QueueBatchContext::ConstPtr&)> filter) {
...
std::vector<QueueBatchContext::ConstPtr>
SyncValidator::GetLastPendingBatches(std::function<bool(const QueueBatchContext::ConstPtr&)> filter) const {
    std::vector<QueueBatchContext::ConstPtr> snapshot;
    for (const auto& queue_sync_state : queue_sync_states_) {
...
// Note that function is const, but updates mutable submit_index to allow Validate to create correct tagging for command
// invocation scope state. Given that queue submits are supposed to be externally synchronized for the same queue, this
// should safe without being atomic... but as the ops are per submit, the performance cost is negible for the peace of
// mind.
uint64_t QueueSyncState::ReserveSubmitId() const {
    return submit_index_.fetch_add(1);
}

void QueueSyncState::SetPendingLastBatch(QueueBatchContext::Ptr&& last) const {
    pending_last_batch_ = std::move(last);
}
...
void QueueSyncState::ApplyPendingLastBatch() {
    ...
    if (pending_last_batch_) {
        // Clean up the events data in the previous last batch on queue, as only the subsequent batches have valid use
        // for them and the QueueBatchContext::Setup calls have be copying them along from batch to batch during submit.
        if (last_batch_) {
...
void BatchAccessLog::Import(const BatchRecord& batch,
...
void BatchAccessLog::Insert(const BatchRecord& batch,
...
// the Trim call removes any AccessLog references that do not correspond to any tags in use. The set of referenced tag,
// used_tags, is generated by scanning the AccessContext and EventContext of the containing QueueBatchContext.
//
...
//  * when ranges are detected that have no tags referenced, all ranges between the last tag and the current tag are
//  erased
//  * when used tags prior to the current range are found, all tags up to the current range are skipped
//  * when a tag is found within the current range, that range is skipped (and thus kept in the map), and further used
//  tags
//    within the range are skipped.
//
// Note that for each subcase, any "next steps" logic is designed to be handled within the subsequent iteration --
// meaning that each subcase simply handles the specifics of the current update/skip/erase action needed, and leaves the
// iterators in a sensible state for the top of loop... intentionally eliding special case handling.
...
BatchAccessLog::CBSubmitLog::CBSubmitLog(const BatchRecord& batch,
                                         ...
                                         const std::vector<std::string>& initial_label_stack) :
    batch_(batch),
    cbs_(cb.GetCBReferencesShared()), log_(cb.GetAccessLogShared()), initial_label_stack_(initial_label_stack) {}

PresentedImage::PresentedImage(const SyncValidator& sync_state,
                               QueueBatchContext::Ptr batch_,
...
void PresentedImage::UpdateMemoryAccess(SyncAccessIndex usage,
