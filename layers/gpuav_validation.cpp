...
GpuAVDescriptorSetManager::GpuAVDescriptorSetManager(GpuAV *dev_data) { dev_data_ = dev_data; }

GpuAVDescriptorSetManager::~GpuAVDescriptorSetManager() {
...
VkResult GpuAVDescriptorSetManager::GetDescriptorSet(VkDescriptorPool *desc_pool, VkDescriptorSet *desc_set) {
...
VkResult GpuAVDescriptorSetManager::GetDescriptorSets(uint32_t count, VkDescriptorPool *pool,
                                                      std::vector<VkDescriptorSet> *desc_sets) {
    ...
    std::vector<VkDescriptorSetLayout> desc_layouts(count, dev_data_->gpuav_validation_state->debug_desc_layout);
...
void GpuAVDescriptorSetManager::PutBackDescriptorSet(VkDescriptorPool desc_pool, VkDescriptorSet desc_set) {
...
VkResult GpuAV::InitializeVma(VkPhysicalDevice physical_device, VkDevice device, VmaAllocator *pAllocator) {
...
void GpuAV::ReportSetupProblem(VkDebugReportObjectTypeEXT object_type, uint64_t object_handle, const char *const specific_message) {
...
void GpuAV::PreCallRecordCreateBuffer(VkDevice device, const VkBufferCreateInfo *pCreateInfo,
...
void GpuAV::PreCallRecordCreateDevice(VkPhysicalDevice gpu, const VkDeviceCreateInfo *create_info,
...
void GpuAV::PostCallRecordCreateDevice(VkPhysicalDevice physicalDevice, const VkDeviceCreateInfo *pCreateInfo,
                                       const VkAllocationCallbacks *pAllocator, VkDevice *pDevice, VkResult result) {
    // The state tracker sets up the device state
    StateTracker::PostCallRecordCreateDevice(physicalDevice, pCreateInfo, pAllocator, pDevice, result);

    ValidationObject *device_object = GetLayerDataPtr(get_dispatch_key(*pDevice), layer_data_map);
    ValidationObject *validation_data = GetValidationObject(device_object->object_dispatch, this->container_type);
    GpuAV *device_gpuav = static_cast<GpuAV *>(validation_data);
    device_gpuav->gpuav_validation_state = std::unique_ptr<GpuAVValidationState>(new GpuAVValidationState);

    if (device_gpuav->phys_dev_props.apiVersion < VK_API_VERSION_1_1) {
        ...
        device_gpuav->gpuav_validation_state->aborted = true;
        return;
    }

    if (!device_gpuav->enabled_features.core.fragmentStoresAndAtomics ||
        !device_gpuav->enabled_features.core.vertexPipelineStoresAndAtomics) {
        ReportSetupProblem(VK_DEBUG_REPORT_OBJECT_TYPE_DEVICE_EXT, HandleToUint64(device),
    ...
    if (device_extensions.vk_ext_buffer_device_address && !device_gpuav->enabled_features.core.shaderInt64) {
    ...
    device_gpuav->gpuav_validation_state->shaderInt64 = device_gpuav->enabled_features.core.shaderInt64;
    ...
    device_gpuav->gpuav_validation_state->vkSetDeviceLoaderData = chain_info->u.pfnSetDeviceLoaderData;
    ...
    device_gpuav->gpuav_validation_state->adjusted_max_desc_sets = device_gpuav->phys_dev_props.limits.maxBoundDescriptorSets;
    device_gpuav->gpuav_validation_state->adjusted_max_desc_sets =
        std::min(33U, device_gpuav->gpuav_validation_state->adjusted_max_desc_sets);
    ...
    if (device_gpuav->gpuav_validation_state->adjusted_max_desc_sets == 1) {
        ReportSetupProblem(VK_DEBUG_REPORT_OBJECT_TYPE_DEVICE_EXT, HandleToUint64(device),
                           "Device can bind only a single descriptor set.  GPU-Assisted Validation disabled.");
        device_gpuav->gpuav_validation_state->aborted = true;
        return;
    }
    device_gpuav->gpuav_validation_state->desc_set_bind_index = device_gpuav->gpuav_validation_state->adjusted_max_desc_sets - 1;
    log_msg(report_data, VK_DEBUG_REPORT_INFORMATION_BIT_EXT, VK_DEBUG_REPORT_OBJECT_TYPE_DEVICE_EXT, HandleToUint64(device),
            ...
            device_gpuav->gpuav_validation_state->desc_set_bind_index);

    device_gpuav->gpuav_validation_state->output_buffer_size = sizeof(uint32_t) * (spvtools::kInst2MaxOutCnt + 1);
    VkResult result1 = InitializeVma(physicalDevice, *pDevice, &device_gpuav->gpuav_validation_state->vmaAllocator);
    assert(result1 == VK_SUCCESS);
    std::unique_ptr<GpuAVDescriptorSetManager> desc_set_manager(new GpuAVDescriptorSetManager(device_gpuav));
    ...
    result1 = DispatchCreateDescriptorSetLayout(*pDevice, &debug_desc_layout_info, NULL,
                                                &device_gpuav->gpuav_validation_state->debug_desc_layout);

    // This is a layout used to "pad" a pipeline layout to fill in any gaps to the selected bind index.
    VkResult result2 = DispatchCreateDescriptorSetLayout(*pDevice, &dummy_desc_layout_info, NULL,
                                                         &device_gpuav->gpuav_validation_state->dummy_desc_layout);
    ...
    if ((result1 != VK_SUCCESS) || (result2 != VK_SUCCESS)) {
        ...
        if (result == VK_SUCCESS) {
            DispatchDestroyDescriptorSetLayout(*pDevice, device_gpuav->gpuav_validation_state->debug_desc_layout, NULL);
        }
        if (result2 == VK_SUCCESS) {
            DispatchDestroyDescriptorSetLayout(*pDevice, device_gpuav->gpuav_validation_state->dummy_desc_layout, NULL);
        }
        device_gpuav->gpuav_validation_state->debug_desc_layout = VK_NULL_HANDLE;
        device_gpuav->gpuav_validation_state->dummy_desc_layout = VK_NULL_HANDLE;
        device_gpuav->gpuav_validation_state->aborted = true;
        return;
    }
    device_gpuav->gpuav_validation_state->desc_set_manager = std::move(desc_set_manager);

    // Register callback to be called at any ResetCommandBuffer time
    device_gpuav->SetCommandBufferResetCallback(
        [device_gpuav](VkCommandBuffer command_buffer) -> void { device_gpuav->ResetCommandBuffer(command_buffer); });

    CreateAccelerationStructureBuildValidationState(device_gpuav);
}

void GpuAV::PostCallRecordGetBufferDeviceAddressEXT(VkDevice device, const VkBufferDeviceAddressInfoEXT *pInfo,
                                                    VkDeviceAddress address) {
    BUFFER_STATE *buffer_state = GetBufferState(pInfo->buffer);
    // Validate against the size requested when the buffer was created
    if (buffer_state) {
        gpuav_validation_state->buffer_map[address] = buffer_state->createInfo.size;
        buffer_state->deviceAddress = address;
    }
}

void GpuAV::PreCallRecordDestroyBuffer(VkDevice device, VkBuffer buffer, const VkAllocationCallbacks *pAllocator) {
    BUFFER_STATE *buffer_state = GetBufferState(buffer);
    if (buffer_state) gpuav_validation_state->buffer_map.erase(buffer_state->deviceAddress);
}
// Clean up device-related resources
void GpuAV::PreCallRecordDestroyDevice(VkDevice device, const VkAllocationCallbacks *pAllocator) {
    for (auto &queue_barrier_command_info_kv : gpuav_validation_state->queue_barrier_command_infos) {
        GpuAVQueueBarrierCommandInfo &queue_barrier_command_info = queue_barrier_command_info_kv.second;
    ...
    gpuav_validation_state->queue_barrier_command_infos.clear();
    if (gpuav_validation_state->debug_desc_layout) {
        DispatchDestroyDescriptorSetLayout(device, gpuav_validation_state->debug_desc_layout, NULL);
        gpuav_validation_state->debug_desc_layout = VK_NULL_HANDLE;
    }
    if (gpuav_validation_state->dummy_desc_layout) {
        DispatchDestroyDescriptorSetLayout(device, gpuav_validation_state->dummy_desc_layout, NULL);
        gpuav_validation_state->dummy_desc_layout = VK_NULL_HANDLE;
    }
    gpuav_validation_state->desc_set_manager.reset();

    DestroyAccelerationStructureBuildValidationState();

    if (gpuav_validation_state->vmaAllocator) {
        vmaDestroyAllocator(gpuav_validation_state->vmaAllocator);
    }
}
void GpuAV::CreateAccelerationStructureBuildValidationState(GpuAV *device_gpuav) {
    if (device_gpuav->gpuav_validation_state->aborted) {
        return;
    }

    auto &as_validation_state = device_gpuav->gpuav_validation_state->acceleration_structure_validation_state;
    ...
    if (result == VK_SUCCESS) {
        ...
        result =
            vmaCreateBuffer(device_gpuav->gpuav_validation_state->vmaAllocator, &vbo_ci, &vbo_ai, &vbo, &vbo_allocation, nullptr);
        if (result != VK_SUCCESS) {
            ReportSetupProblem(VK_DEBUG_REPORT_OBJECT_TYPE_DEVICE_EXT, HandleToUint64(device),
    ...
    if (result == VK_SUCCESS) {
        ...
        result = vmaMapMemory(device_gpuav->gpuav_validation_state->vmaAllocator, vbo_allocation, (void **)&mapped_vbo_buffer);
        if (result != VK_SUCCESS) {
            ReportSetupProblem(VK_DEBUG_REPORT_OBJECT_TYPE_DEVICE_EXT, HandleToUint64(device),
            ...
            vmaUnmapMemory(device_gpuav->gpuav_validation_state->vmaAllocator, vbo_allocation);
        }
    }
    ...
    if (result == VK_SUCCESS) {
        ...
        result =
            vmaCreateBuffer(device_gpuav->gpuav_validation_state->vmaAllocator, &ibo_ci, &ibo_ai, &ibo, &ibo_allocation, nullptr);
        if (result != VK_SUCCESS) {
            ReportSetupProblem(VK_DEBUG_REPORT_OBJECT_TYPE_DEVICE_EXT, HandleToUint64(device),
    ...
    if (result == VK_SUCCESS) {
        ...
        result = vmaMapMemory(device_gpuav->gpuav_validation_state->vmaAllocator, ibo_allocation, (void **)&mapped_ibo_buffer);
        if (result != VK_SUCCESS) {
            ReportSetupProblem(VK_DEBUG_REPORT_OBJECT_TYPE_DEVICE_EXT, HandleToUint64(device),
            ...
            vmaUnmapMemory(device_gpuav->gpuav_validation_state->vmaAllocator, ibo_allocation);
        }
    }
    ...
    if (result == VK_SUCCESS) {
        ...
        result = vmaAllocateMemory(device_gpuav->gpuav_validation_state->vmaAllocator, &as_mem_requirements.memoryRequirements,
                                   &as_memory_aci, &as_validation_state.replacement_as_allocation, &as_memory_ai);
        if (result != VK_SUCCESS) {
            ReportSetupProblem(VK_DEBUG_REPORT_OBJECT_TYPE_DEVICE_EXT, HandleToUint64(device_gpuav->device),
    ...
    if (result == VK_SUCCESS) {
        ...
        result = vmaAllocateMemory(device_gpuav->gpuav_validation_state->vmaAllocator, &scratch_mem_requirements.memoryRequirements,
                                   &scratch_aci, &scratch_allocation, &scratch_allocation_info);
        if (result != VK_SUCCESS) {
            ReportSetupProblem(VK_DEBUG_REPORT_OBJECT_TYPE_DEVICE_EXT, HandleToUint64(device_gpuav->device),
    ...
    if (result == VK_SUCCESS) {
        ...
        device_gpuav->gpuav_validation_state->vkSetDeviceLoaderData(device_gpuav->device, command_buffer);
    }
    ...
    if (result == VK_SUCCESS) {
        ...
        device_gpuav->gpuav_validation_state->vkSetDeviceLoaderData(device_gpuav->device, queue);
    ...
    if (vbo != VK_NULL_HANDLE) {
        vmaDestroyBuffer(device_gpuav->gpuav_validation_state->vmaAllocator, vbo, vbo_allocation);
    }
    if (ibo != VK_NULL_HANDLE) {
        vmaDestroyBuffer(device_gpuav->gpuav_validation_state->vmaAllocator, ibo, ibo_allocation);
    }
    if (scratch != VK_NULL_HANDLE) {
        DispatchDestroyBuffer(device_gpuav->device, scratch, nullptr);
        vmaFreeMemory(device_gpuav->gpuav_validation_state->vmaAllocator, scratch_allocation);
    }
    if (command_pool != VK_NULL_HANDLE) {
        DispatchDestroyCommandPool(device_gpuav->device, command_pool, nullptr);
    }

    if (device_gpuav->gpuav_validation_state->debug_desc_layout == VK_NULL_HANDLE) {
        ReportSetupProblem(VK_DEBUG_REPORT_OBJECT_TYPE_DEVICE_EXT, HandleToUint64(device_gpuav->device),
    ...
    if (result == VK_SUCCESS) {
        ...
        pipeline_layout_ci.pSetLayouts = &device_gpuav->gpuav_validation_state->debug_desc_layout;
...
void GpuAV::DestroyAccelerationStructureBuildValidationState() {
    auto &as_validation_state = gpuav_validation_state->acceleration_structure_validation_state;
    ...
    if (as_validation_state.replacement_as_allocation != VK_NULL_HANDLE) {
        vmaFreeMemory(gpuav_validation_state->vmaAllocator, as_validation_state.replacement_as_allocation);
    }
}
...
void GpuAV::PreCallRecordCmdBuildAccelerationStructureNV(VkCommandBuffer commandBuffer, const VkAccelerationStructureInfoNV *pInfo,
    ...
    GpuAVAccelerationStructureBuildValidationBufferInfo as_validation_buffer_info = {};
    ...
    VkResult result = vmaCreateBuffer(gpuav_validation_state->vmaAllocator, &validation_buffer_create_info,
    ...
    if (result != VK_SUCCESS) {
        ...
        gpuav_validation_state->aborted = true;
        return;
    }

    GpuAccelerationStructureBuildValidationBuffer *mapped_validation_buffer = nullptr;
    result = vmaMapMemory(gpuav_validation_state->vmaAllocator, as_validation_buffer_info.validation_buffer_allocation,
    ...
    vmaUnmapMemory(gpuav_validation_state->vmaAllocator, as_validation_buffer_info.validation_buffer_allocation);
    ...
    result = gpuav_validation_state->desc_set_manager->GetDescriptorSet(&as_validation_buffer_info.descriptor_pool,
...
void GpuAV::ProcessAccelerationStructureBuildValidationBuffer(VkQueue queue, CMD_BUFFER_STATE *cb_node) {
    if (cb_node == nullptr || !cb_node->hasBuildAccelerationStructureCmd) {
        return;
    }

    auto &as_validation_info = gpuav_validation_state->acceleration_structure_validation_state;
    ...
    for (const auto &as_validation_buffer_info : as_validation_buffer_infos) {
        ...
        VkResult result = vmaMapMemory(gpuav_validation_state->vmaAllocator, as_validation_buffer_info.validation_buffer_allocation,
                                       (void **)&mapped_validation_buffer);
        if (result == VK_SUCCESS) {
...
void GpuAV::PostCallRecordBindAccelerationStructureMemoryNV(VkDevice device, uint32_t bindInfoCount,
                                                            const VkBindAccelerationStructureMemoryInfoNV *pBindInfos,
                                                            VkResult result) {
    if (VK_SUCCESS != result) return;
    StateTracker::PostCallRecordBindAccelerationStructureMemoryNV(device, bindInfoCount, pBindInfos, result);
...
void GpuAV::PreCallRecordCreatePipelineLayout(VkDevice device, const VkPipelineLayoutCreateInfo *pCreateInfo,
                                              const VkAllocationCallbacks *pAllocator, VkPipelineLayout *pPipelineLayout,
                                              void *cpl_state_data) {
    if (gpuav_validation_state->aborted) {
        return;
    }
    create_pipeline_layout_api_state *cpl_state = reinterpret_cast<create_pipeline_layout_api_state *>(cpl_state_data);

    if (cpl_state->modified_create_info.setLayoutCount >= gpuav_validation_state->adjusted_max_desc_sets) {
        std::ostringstream strm;
        strm << "Pipeline Layout conflict with validation's descriptor set at slot " << gpuav_validation_state->desc_set_bind_index
             << ". "
        ...
        cpl_state->new_layouts.reserve(gpuav_validation_state->adjusted_max_desc_sets);
        ...
        for (uint32_t i = pCreateInfo->setLayoutCount; i < gpuav_validation_state->adjusted_max_desc_sets - 1; ++i) {
            cpl_state->new_layouts.push_back(gpuav_validation_state->dummy_desc_layout);
        }
        cpl_state->new_layouts.push_back(gpuav_validation_state->debug_desc_layout);
        cpl_state->modified_create_info.pSetLayouts = cpl_state->new_layouts.data();
        cpl_state->modified_create_info.setLayoutCount = gpuav_validation_state->adjusted_max_desc_sets;
    }
}

void GpuAV::PostCallRecordCreatePipelineLayout(VkDevice device, const VkPipelineLayoutCreateInfo *pCreateInfo,
                                               const VkAllocationCallbacks *pAllocator, VkPipelineLayout *pPipelineLayout,
                                               VkResult result) {
...
void GpuAV::ResetCommandBuffer(VkCommandBuffer commandBuffer) {
    if (gpuav_validation_state->aborted) {
        return;
    }
    auto gpuav_buffer_list = gpuav_validation_state->GetGpuAVBufferInfo(commandBuffer);
    for (auto buffer_info : gpuav_buffer_list) {
        vmaDestroyBuffer(gpuav_validation_state->vmaAllocator, buffer_info.output_mem_block.buffer,
                         buffer_info.output_mem_block.allocation);
        if (buffer_info.di_input_mem_block.buffer) {
            vmaDestroyBuffer(gpuav_validation_state->vmaAllocator, buffer_info.di_input_mem_block.buffer,
                             buffer_info.di_input_mem_block.allocation);
        }
        if (buffer_info.bda_input_mem_block.buffer) {
            vmaDestroyBuffer(gpuav_validation_state->vmaAllocator, buffer_info.bda_input_mem_block.buffer,
                             buffer_info.bda_input_mem_block.allocation);
        }
        if (buffer_info.desc_set != VK_NULL_HANDLE) {
            gpuav_validation_state->desc_set_manager->PutBackDescriptorSet(buffer_info.desc_pool, buffer_info.desc_set);
        }
    }
    gpuav_validation_state->command_buffer_map.erase(commandBuffer);
    ...
    for (auto &as_validation_buffer_info : as_validation_buffer_infos) {
        vmaDestroyBuffer(gpuav_validation_state->vmaAllocator, as_validation_buffer_info.validation_buffer,
                         as_validation_buffer_info.validation_buffer_allocation);

        if (as_validation_buffer_info.descriptor_set != VK_NULL_HANDLE) {
            gpuav_validation_state->desc_set_manager->PutBackDescriptorSet(as_validation_buffer_info.descriptor_pool,
                                                                           as_validation_buffer_info.descriptor_set);
        }
    }
...
bool GpuAV::PreCallValidateCmdWaitEvents(VkCommandBuffer commandBuffer, uint32_t eventCount, const VkEvent *pEvents,
...
void GpuAV::PostCallRecordGetPhysicalDeviceProperties(VkPhysicalDevice physicalDevice,
                                                      VkPhysicalDeviceProperties *pPhysicalDeviceProperties) {
    // There is an implicit layer that can cause this call to return 0 for maxBoundDescriptorSets - Ignore such calls
    if (enabled.gpu_av && enabled.gpu_validation_reserve_binding_slot &&
...
void GpuAV::PostCallRecordGetPhysicalDeviceProperties2(VkPhysicalDevice physicalDevice,
...
void GpuAV::PreCallRecordCreateGraphicsPipelines(VkDevice device, VkPipelineCache pipelineCache, uint32_t count,
...
void GpuAV::PreCallRecordCreateComputePipelines(VkDevice device, VkPipelineCache pipelineCache, uint32_t count,
...
void GpuAV::PreCallRecordCreateRayTracingPipelinesNV(VkDevice device, VkPipelineCache pipelineCache, uint32_t count,
...
void GpuAV::PreCallRecordPipelineCreations(uint32_t count, const CreateInfo *pCreateInfos, const VkAllocationCallbacks *pAllocator,
    ...
    for (uint32_t pipeline = 0; pipeline < count; ++pipeline) {
        ...
        if (pipe_state[pipeline]->active_slots.find(gpuav_validation_state->desc_set_bind_index) !=
        ...
        if (pipe_state[pipeline]->pipeline_layout.set_layouts.size() >= gpuav_validation_state->adjusted_max_desc_sets) {
            replace_shaders = true;
        }
...
void GpuAV::PostCallRecordCreateGraphicsPipelines(VkDevice device, VkPipelineCache pipelineCache, uint32_t count,
                                                  const VkGraphicsPipelineCreateInfo *pCreateInfos,
...
void GpuAV::PostCallRecordCreateComputePipelines(VkDevice device, VkPipelineCache pipelineCache, uint32_t count,
                                                 const VkComputePipelineCreateInfo *pCreateInfos,
                                                 const VkAllocationCallbacks *pAllocator, VkPipeline *pPipelines, VkResult result,
                                                 void *ccpl_state_data) {
...
void GpuAV::PostCallRecordCreateRayTracingPipelinesNV(VkDevice device, VkPipelineCache pipelineCache, uint32_t count,
                                                      const VkRayTracingPipelineCreateInfoNV *pCreateInfos,
                                                      const VkAllocationCallbacks *pAllocator, VkPipeline *pPipelines,
...
template <typename CreateInfo>
void GpuAV::PostCallRecordPipelineCreations(const uint32_t count, const CreateInfo *pCreateInfos,
                                            const VkAllocationCallbacks *pAllocator, VkPipeline *pPipelines,
                                            const VkPipelineBindPoint bind_point) {
    using Accessor = CreatePipelineTraits<CreateInfo>;
    if (bind_point != VK_PIPELINE_BIND_POINT_GRAPHICS && bind_point != VK_PIPELINE_BIND_POINT_COMPUTE &&
    ...
    for (uint32_t pipeline = 0; pipeline < count; ++pipeline) {
        ...
        for (uint32_t stage = 0; stage < stageCount; ++stage) {
            if (pipeline_state->active_slots.find(gpuav_validation_state->desc_set_bind_index) !=
            ...
            gpuav_validation_state->shader_map[shader_state->gpu_validation_shader_id].pipeline = pipeline_state->pipeline;
            ...
            gpuav_validation_state->shader_map[shader_state->gpu_validation_shader_id].shader_module = shader_module;
            gpuav_validation_state->shader_map[shader_state->gpu_validation_shader_id].pgm = std::move(code);
        }
    }
}

// Remove all the shader trackers associated with this destroyed pipeline.
void GpuAV::PreCallRecordDestroyPipeline(VkDevice device, VkPipeline pipeline, const VkAllocationCallbacks *pAllocator) {
    for (auto it = gpuav_validation_state->shader_map.begin(); it != gpuav_validation_state->shader_map.end();) {
        if (it->second.pipeline == pipeline) {
            it = gpuav_validation_state->shader_map.erase(it);
        } else {
            ++it;
        }
    }
...
bool GpuAV::InstrumentShader(const VkShaderModuleCreateInfo *pCreateInfo, std::vector<unsigned int> &new_pgm,
                             uint32_t *unique_shader_id) {
    if (gpuav_validation_state->aborted) return false;
    ...
    optimizer.RegisterPass(CreateInstBindlessCheckPass(gpuav_validation_state->desc_set_bind_index,
                                                       gpuav_validation_state->unique_shader_module_id, descriptor_indexing,
                                                       descriptor_indexing, 2));
    optimizer.RegisterPass(CreateAggressiveDCEPass());
    if (device_extensions.vk_ext_buffer_device_address && gpuav_validation_state->shaderInt64)
        optimizer.RegisterPass(CreateInstBuffAddrCheckPass(gpuav_validation_state->desc_set_bind_index,
                                                           gpuav_validation_state->unique_shader_module_id));
    ...
    *unique_shader_id = gpuav_validation_state->unique_shader_module_id++;
    return pass;
}
// Create the instrumented shader data to provide to the driver.
void GpuAV::PreCallRecordCreateShaderModule(VkDevice device, const VkShaderModuleCreateInfo *pCreateInfo,
...
//
void GpuAV::AnalyzeAndReportError(CMD_BUFFER_STATE *cb_node, VkQueue queue, VkPipelineBindPoint pipeline_bind_point,
    ...
    auto it = gpuav_validation_state->shader_map.find(debug_record[kInstCommonOutShaderId]);
    if (it != gpuav_validation_state->shader_map.end()) {
...
void GpuAV::ProcessInstrumentationBuffer(VkQueue queue, CMD_BUFFER_STATE *cb_node) {
    if (cb_node && (cb_node->hasDrawCmd || cb_node->hasTraceRaysCmd || cb_node->hasDispatchCmd)) {
        auto gpu_buffer_list = gpuav_validation_state->GetGpuAVBufferInfo(cb_node->commandBuffer);
        ...
        for (auto &buffer_info : gpu_buffer_list) {
            char *pData;
            VkResult result =
                vmaMapMemory(gpuav_validation_state->vmaAllocator, buffer_info.output_mem_block.allocation, (void **)&pData);
            ...
            if (result == VK_SUCCESS) {
                ...
                vmaUnmapMemory(gpuav_validation_state->vmaAllocator, buffer_info.output_mem_block.allocation);
            }
...
void GpuAV::UpdateInstrumentationBuffer(CMD_BUFFER_STATE *cb_node) {
    auto gpu_buffer_list = gpuav_validation_state->GetGpuAVBufferInfo(cb_node->commandBuffer);
    uint32_t *pData;
    for (auto &buffer_info : gpu_buffer_list) {
        if (buffer_info.di_input_mem_block.update_at_submit.size() > 0) {
            VkResult result =
                vmaMapMemory(gpuav_validation_state->vmaAllocator, buffer_info.di_input_mem_block.allocation, (void **)&pData);
            if (result == VK_SUCCESS) {
                for (auto update : buffer_info.di_input_mem_block.update_at_submit) {
                    if (update.second->updated) pData[update.first] = 1;
                }
                vmaUnmapMemory(gpuav_validation_state->vmaAllocator, buffer_info.di_input_mem_block.allocation);
            }
        }
    }
}
...
void GpuAV::SubmitBarrier(VkQueue queue) {
    auto queue_barrier_command_info_it =
        gpuav_validation_state->queue_barrier_command_infos.emplace(queue, GpuAVQueueBarrierCommandInfo{});
    if (queue_barrier_command_info_it.second) {
        GpuAVQueueBarrierCommandInfo &quere_barrier_command_info = queue_barrier_command_info_it.first->second;
        ...
        // Hook up command buffer dispatch
        gpuav_validation_state->vkSetDeviceLoaderData(device, quere_barrier_command_info.barrier_command_buffer);
...
void GpuAV::PreCallRecordQueueSubmit(VkQueue queue, uint32_t submitCount, const VkSubmitInfo *pSubmits, VkFence fence) {
...
void GpuAV::PostCallRecordQueueSubmit(VkQueue queue, uint32_t submitCount, const VkSubmitInfo *pSubmits, VkFence fence,
                                      VkResult result) {
    StateTracker::PostCallRecordQueueSubmit(queue, submitCount, pSubmits, fence, result);

    if (gpuav_validation_state->aborted) return;
    ...
    for (uint32_t submit_idx = 0; submit_idx < submitCount; submit_idx++) {
        ...
        for (uint32_t i = 0; i < submit->commandBufferCount; i++) {
            auto cb_node = GetCBState(submit->pCommandBuffers[i]);
            if (gpuav_validation_state->GetGpuAVBufferInfo(cb_node->commandBuffer).size() ||
                cb_node->hasBuildAccelerationStructureCmd)
                buffers_present = true;
            for (auto secondaryCmdBuffer : cb_node->linkedCommandBuffers) {
                if (gpuav_validation_state->GetGpuAVBufferInfo(secondaryCmdBuffer->commandBuffer).size() ||
                    cb_node->hasBuildAccelerationStructureCmd)
                    buffers_present = true;
            }
        }
...
void GpuAV::PreCallRecordCmdDraw(VkCommandBuffer commandBuffer, uint32_t vertexCount, uint32_t instanceCount, uint32_t firstVertex,
                                 uint32_t firstInstance) {
    AllocateValidationResources(commandBuffer, VK_PIPELINE_BIND_POINT_GRAPHICS);
}

void GpuAV::PreCallRecordCmdDrawIndexed(VkCommandBuffer commandBuffer, uint32_t indexCount, uint32_t instanceCount,
                                        uint32_t firstIndex, int32_t vertexOffset, uint32_t firstInstance) {
    AllocateValidationResources(commandBuffer, VK_PIPELINE_BIND_POINT_GRAPHICS);
}

void GpuAV::PreCallRecordCmdDrawIndirect(VkCommandBuffer commandBuffer, VkBuffer buffer, VkDeviceSize offset, uint32_t count,
                                         uint32_t stride) {
    AllocateValidationResources(commandBuffer, VK_PIPELINE_BIND_POINT_GRAPHICS);
}

void GpuAV::PreCallRecordCmdDrawIndexedIndirect(VkCommandBuffer commandBuffer, VkBuffer buffer, VkDeviceSize offset, uint32_t count,
                                                uint32_t stride) {
    AllocateValidationResources(commandBuffer, VK_PIPELINE_BIND_POINT_GRAPHICS);
}

void GpuAV::PreCallRecordCmdDispatch(VkCommandBuffer commandBuffer, uint32_t x, uint32_t y, uint32_t z) {
    AllocateValidationResources(commandBuffer, VK_PIPELINE_BIND_POINT_COMPUTE);
}

void GpuAV::PreCallRecordCmdDispatchIndirect(VkCommandBuffer commandBuffer, VkBuffer buffer, VkDeviceSize offset) {
    AllocateValidationResources(commandBuffer, VK_PIPELINE_BIND_POINT_COMPUTE);
}

void GpuAV::PreCallRecordCmdTraceRaysNV(VkCommandBuffer commandBuffer, VkBuffer raygenShaderBindingTableBuffer,
...
void GpuAV::PostCallRecordCmdTraceRaysNV(VkCommandBuffer commandBuffer, VkBuffer raygenShaderBindingTableBuffer,
                                         VkDeviceSize raygenShaderBindingOffset, VkBuffer missShaderBindingTableBuffer,
                                         VkDeviceSize missShaderBindingOffset, VkDeviceSize missShaderBindingStride,
                                         VkBuffer hitShaderBindingTableBuffer, VkDeviceSize hitShaderBindingOffset,
                                         VkDeviceSize hitShaderBindingStride, VkBuffer callableShaderBindingTableBuffer,
...
void GpuAV::AllocateValidationResources(const VkCommandBuffer cmd_buffer, const VkPipelineBindPoint bind_point) {
    if (bind_point != VK_PIPELINE_BIND_POINT_GRAPHICS && bind_point != VK_PIPELINE_BIND_POINT_COMPUTE &&
        bind_point != VK_PIPELINE_BIND_POINT_RAY_TRACING_NV) {
        return;
    }
    ...
    result = gpuav_validation_state->desc_set_manager->GetDescriptorSets(1, &desc_pool, &desc_sets);
    assert(result == VK_SUCCESS);
    if (result != VK_SUCCESS) {
        ReportSetupProblem(VK_DEBUG_REPORT_OBJECT_TYPE_DEVICE_EXT, HandleToUint64(device),
    ...
    output_desc_buffer_info.range = gpuav_validation_state->output_buffer_size;
    ...
    GpuAVDeviceMemoryBlock output_block = {};
    VkBufferCreateInfo bufferInfo = {VK_STRUCTURE_TYPE_BUFFER_CREATE_INFO};
    bufferInfo.size = gpuav_validation_state->output_buffer_size;
    bufferInfo.usage = VK_BUFFER_USAGE_STORAGE_BUFFER_BIT;
    VmaAllocationCreateInfo allocInfo = {};
    allocInfo.usage = VMA_MEMORY_USAGE_GPU_TO_CPU;
    result = vmaCreateBuffer(gpuav_validation_state->vmaAllocator, &bufferInfo, &allocInfo, &output_block.buffer,
    ...
    uint32_t *pData;
    result = vmaMapMemory(gpuav_validation_state->vmaAllocator, output_block.allocation, (void **)&pData);
    if (result == VK_SUCCESS) {
        memset(pData, 0, gpuav_validation_state->output_buffer_size);
        vmaUnmapMemory(gpuav_validation_state->vmaAllocator, output_block.allocation);
    }

    GpuAVDeviceMemoryBlock di_input_block = {}, bda_input_block = {};
    ...
    if (number_of_sets > 0 && device_extensions.vk_ext_descriptor_indexing) {
        ...
        result = vmaCreateBuffer(gpuav_validation_state->vmaAllocator, &bufferInfo, &allocInfo, &di_input_block.buffer,
        ...
        result = vmaMapMemory(gpuav_validation_state->vmaAllocator, di_input_block.allocation, (void **)&pData);
        ...
        vmaUnmapMemory(gpuav_validation_state->vmaAllocator, di_input_block.allocation);
    ...
    if (number_of_sets > 0 && device_extensions.vk_ext_buffer_device_address && gpuav_validation_state->buffer_map.size() &&
        gpuav_validation_state->shaderInt64) {
        ...
        uint32_t num_buffers = static_cast<uint32_t>(gpuav_validation_state->buffer_map.size());
        uint32_t words_needed = (num_buffers + 3) + (num_buffers + 2);
        allocInfo.usage = VMA_MEMORY_USAGE_CPU_TO_GPU;
        bufferInfo.size = words_needed * 8;  // 64 bit words
        result = vmaCreateBuffer(gpuav_validation_state->vmaAllocator, &bufferInfo, &allocInfo, &bda_input_block.buffer,
                                 &bda_input_block.allocation, nullptr);
        if (result != VK_SUCCESS) {
            ReportSetupProblem(VK_DEBUG_REPORT_OBJECT_TYPE_DEVICE_EXT, HandleToUint64(device),
                               "Unable to allocate device memory.  Device could become unstable.");
            gpuav_validation_state->aborted = true;
        ...
        uint64_t *bda_data;
        result = vmaMapMemory(gpuav_validation_state->vmaAllocator, bda_input_block.allocation, (void **)&bda_data);
        ...
        for (auto const &value : gpuav_validation_state->buffer_map) {
            bda_data[address_index++] = value.first;
            bda_data[size_index++] = value.second;
        }
        bda_data[address_index] = UINTPTR_MAX;
        bda_data[size_index] = 0;
        vmaUnmapMemory(gpuav_validation_state->vmaAllocator, bda_input_block.allocation);
    ...
    if (iter != cb_node->lastBound.end()) {
        ...
        if (pipeline_state && (pipeline_state->pipeline_layout.set_layouts.size() <= gpuav_validation_state->desc_set_bind_index)) {
            DispatchCmdBindDescriptorSets(cmd_buffer, bind_point, pipeline_state->pipeline_layout.layout,
                                          gpuav_validation_state->desc_set_bind_index, 1, desc_sets.data(), 0, nullptr);
        }
        // Record buffer and memory info in CB state tracking
        gpuav_validation_state->GetGpuAVBufferInfo(cmd_buffer)
        ...
        vmaDestroyBuffer(gpuav_validation_state->vmaAllocator, di_input_block.buffer, di_input_block.allocation);
        vmaDestroyBuffer(gpuav_validation_state->vmaAllocator, bda_input_block.buffer, bda_input_block.allocation);
        vmaDestroyBuffer(gpuav_validation_state->vmaAllocator, output_block.buffer, output_block.allocation);
        gpuav_validation_state->aborted = true;
        return;
    }
}
