...
bool CommandBuffer::UpdateBdaRangesBuffer(const Location &loc) {
    ...
    // ---
    auto bda_table_ptr = (uint32_t *)bda_ranges_snapshot_.MapMemory(loc);

    // Buffer device address table layout
    // Ranges are sorted from low to high, and do not overlap
    // QWord 0 | split up into two dwords
    //     DWord 0 | Cache for which index was used last
    //     DWord 1 | Number of *ranges* (1 range occupies 2 QWords)
    ...
    bda_table_ptr[0] = 0;
    bda_table_ptr[1] = static_cast<uint32_t>(ranges_to_update_count);
