...
layout(set = kInstDefaultDescriptorSet, binding = kBindingInstSmallestVertexBufferBinding, std430) 
readonly buffer MaxVertexIndex {
    uint has_smallest_vbb_vertex_input_rate;
    uint smallest_vbb_vertex_input_rate;
    uint has_smallest_vbb_instance_input_rate;
    uint smallest_vbb_instance_input_rate;
};
...
void inst_vertex_attribute_fetch_oob(const uvec4 stage_info)
{
    ...
    if (has_smallest_vbb_vertex_input_rate == 1u) {
        valid_vertex_attribute_fetch_vertex_input_rate = vertex_index < smallest_vbb_vertex_input_rate;
    }
    if (has_smallest_vbb_instance_input_rate == 1u) {
        valid_vertex_attribute_fetch_instance_input_rate = instance_index < smallest_vbb_instance_input_rate;
    }
