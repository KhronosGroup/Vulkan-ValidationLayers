...
DescriptorSet::DescriptorSet(const vvl::DescriptorSet &set, Validator &state_data)
    : vvl::DescriptorSetProxy(set),
...
void DescriptorSet::BuildBindingLayouts() {
    const uint32_t binding_count = (set.GetBindingCount() > 0) ? set.GetLayout()->GetMaxBinding() + 1 : 0;
...
VkDeviceAddress DescriptorSet::GetTypeAddress(Validator &gpuav, const Location &loc) {
    ...
    if (set.GetNonInlineDescriptorCount() == 0) {
    ...
    buffer_info.size = set.GetNonInlineDescriptorCount() * sizeof(glsl::DescriptorState);
...
VkDeviceAddress DescriptorSet::GetPostProcessBuffer(Validator &gpuav, const Location &loc) {
    ...
    buffer_info.size = set.GetNonInlineDescriptorCount() * sizeof(glsl::PostProcessDescriptorIndexSlot);
    buffer_info.usage = VK_BUFFER_USAGE_STORAGE_BUFFER_BIT | VK_BUFFER_USAGE_SHADER_DEVICE_ADDRESS_BIT;
...
void DescriptorSet::NotifyUpdate() { current_version_++; }

void DescriptorSet::NotifyInvalidate(const vvl::StateObject::NodeList &invalid_nodes, bool unlink) {
