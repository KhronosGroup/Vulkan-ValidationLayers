...
static glsl::DescriptorState GetInData(const vvl::TensorDescriptor &desc) {
    auto tensor_view_state = static_cast<const TensorView *>(desc.GetTensorViewState());
    return glsl::DescriptorState(DescriptorClass::Tensor, tensor_view_state ? tensor_view_state->id : glsl::kNullDescriptor);
}
...
static glsl::DescriptorState GetInData(const vvl::MutableDescriptor &desc) {
    ...
    switch (desc_class) {
        ...
        case DescriptorClass::Tensor: {
            auto tensor_state = std::static_pointer_cast<const Tensor>(desc.GetSharedTensor());
            return glsl::DescriptorState(desc_class, tensor_state ? tensor_state->id : glsl::kNullDescriptor);
        }
