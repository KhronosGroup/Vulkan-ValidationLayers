...
void UpdateBoundDescriptors(Validator &gpuav, CommandBufferSubState &cb_state, VkPipelineBindPoint pipeline_bind_point,
    ...
    CommandBufferDescriptorBindings &cb_desc_bindings =
        cb_state.shared_resources_cache.GetOrCreate<CommandBufferDescriptorBindings>();
    for (auto &descriptor_binding_func : cb_desc_bindings.on_descriptor_binding_functions) {
        descriptor_binding_func(cb_state, descriptor_binding_cmd);
    }
    cb_desc_bindings.descriptor_binding_commands.emplace_back(std::move(descriptor_binding_cmd));
}
...
[[nodiscard]] bool UpdateDescriptorStateSSBO(Validator &gpuav, CommandBufferSubState &cb_state, const Location &loc) {
    ...
    for (DescriptorBindingCommand &descriptor_command_binding : cb_desc_bindings->descriptor_binding_commands) {
