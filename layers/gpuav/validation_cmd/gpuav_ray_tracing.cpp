...
void BuildAccelerationStructures(Validator& gpuav, const Location& loc, CommandBufferSubState& cb_state,
    ...
    // ---
    {
        ...
        for (size_t tlas_build_i = 0; tlas_build_i < blas_arrays.size(); ++tlas_build_i) {
            const auto blas_array_buffers = gpuav.GetBuffersByAddress(blas_arrays[tlas_build_i].array_start_addr);
            ...
            const uint32_t is_array_of_pointers = blas_arrays[tlas_build_i].is_array_of_pointers;
            if (is_array_of_pointers == 0) {
                ...
                shader_resources.push_constants.blas_ptr_array_start_addr = blas_arrays[tlas_build_i].array_start_addr;
                shader_resources.push_constants.blas_array_start_addr = 0;
            }

            shader_resources.push_constants.blas_array_size = blas_arrays[tlas_build_i].size;
            ...
            const uint32_t as_instances_count = blas_arrays[tlas_build_i].size;
