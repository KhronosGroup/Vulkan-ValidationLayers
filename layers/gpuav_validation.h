...
struct GpuAVDeviceMemoryBlock {
    VkBuffer buffer;
    VmaAllocation allocation;
    std::unordered_map<uint32_t, const cvdescriptorset::Descriptor*> update_at_submit;
};

struct GpuAVBufferInfo {
    GpuAVDeviceMemoryBlock output_mem_block;
    GpuAVDeviceMemoryBlock di_input_mem_block;   // Descriptor Indexing input
    GpuAVDeviceMemoryBlock bda_input_mem_block;  // Buffer Device Address input
    VkDescriptorSet desc_set;
    VkDescriptorPool desc_pool;
    VkPipelineBindPoint pipeline_bind_point;
    GpuAVBufferInfo(GpuAVDeviceMemoryBlock output_mem_block, GpuAVDeviceMemoryBlock di_input_mem_block,
                    GpuAVDeviceMemoryBlock bda_input_mem_block, VkDescriptorSet desc_set, VkDescriptorPool desc_pool,
                    VkPipelineBindPoint pipeline_bind_point)
...
struct GpuAVQueueBarrierCommandInfo {
...
class GpuAVDescriptorSetManager {
  public:
    GpuAVDescriptorSetManager(GpuAV* dev_data);
    ~GpuAVDescriptorSetManager();
...
struct GpuAVShaderTracker {
    VkPipeline pipeline;
    VkShaderModule shader_module;
    std::vector<unsigned int> pgm;
};

struct GpuAVAccelerationStructureBuildValidationBufferInfo {
...
struct GpuAVAccelerationStructureBuildValidationState {
    ...
    std::unordered_map<VkCommandBuffer, std::vector<GpuAVAccelerationStructureBuildValidationBufferInfo>> validation_buffers;
};

struct GpuAVValidationState {
    bool aborted;
    ...
    std::unordered_map<uint32_t, GpuAVShaderTracker> shader_map;
    std::unique_ptr<GpuAVDescriptorSetManager> desc_set_manager;
    std::map<VkQueue, GpuAVQueueBarrierCommandInfo> queue_barrier_command_infos;
    std::unordered_map<VkCommandBuffer, std::vector<GpuAVBufferInfo>> command_buffer_map;  // gpu_buffer_list;
    uint32_t output_buffer_size;
    VmaAllocator vmaAllocator;
    PFN_vkSetDeviceLoaderData vkSetDeviceLoaderData;
    std::map<VkDeviceAddress, VkDeviceSize> buffer_map;
    GpuAVAccelerationStructureBuildValidationState acceleration_structure_validation_state;
    GpuAVValidationState(bool aborted = false, uint32_t unique_shader_module_id = 0, VmaAllocator vmaAllocator = {})
        : aborted(aborted), unique_shader_module_id(unique_shader_module_id), vmaAllocator(vmaAllocator){};

    std::vector<GpuAVBufferInfo>& GetGpuAVBufferInfo(const VkCommandBuffer command_buffer) {
        auto buffer_list = command_buffer_map.find(command_buffer);
        if (buffer_list == command_buffer_map.end()) {
            std::vector<GpuAVBufferInfo> new_list{};
...
class GpuAV : public ValidationStateTracker {
  public:
    using StateTracker = ValidationStateTracker;
    std::unique_ptr<GpuAVValidationState> gpuav_validation_state;
    ...
    void CreateAccelerationStructureBuildValidationState(GpuAV* device_gpuav);
