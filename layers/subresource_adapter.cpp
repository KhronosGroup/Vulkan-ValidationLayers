...
IndexType RangeEncoder::Encode1AspectArrayOnly(const Subresource& pos) const { return pos.array_layer; }
IndexType RangeEncoder::Encode1AspectMipArray(const Subresource& pos) const { return pos.array_layer + pos.mip_level * mip_size_; }
IndexType RangeEncoder::Encode1AspectMipOnly(const Subresource& pos) const { return pos.mip_level; }

IndexType RangeEncoder::EncodeAspectArrayOnly(const Subresource& pos) const {
    return pos.array_layer + aspect_base_[pos.aspect_index];
}
IndexType RangeEncoder::EncodeAspectMipArray(const Subresource& pos) const {
    return pos.array_layer + pos.mip_level * mip_size_ + aspect_base_[pos.aspect_index];
}
IndexType RangeEncoder::EncodeAspectMipOnly(const Subresource& pos) const { return pos.mip_level + aspect_base_[pos.aspect_index]; }

bool RangeEncoder::InRange(const VkImageSubresource& subres) const {
    bool in_range = (subres.mipLevel < limits_.mip_level) && (subres.arrayLayer < limits_.array_layer) &&
                    (subres.aspectMask & limits_.selected_aspects);
    return in_range;
}

bool RangeEncoder::InRange(const VkImageSubresourceRange& range) const {
    bool in_range = (range.baseMipLevel < limits_.mip_level) && ((range.baseMipLevel + range.levelCount) <= limits_.mip_level) &&
                    (range.baseArrayLayer < limits_.array_layer) &&
                    ((range.baseArrayLayer + range.layerCount) <= limits_.array_layer) &&
                    (range.aspectMask & limits_.selected_aspects);
    return in_range;
}
...
uint32_t RangeEncoder::UpperBoundFromMask(VkImageAspectFlags mask) const {
    uint32_t aspect_index = limits_.aspect_index;
    while (0 < limits_.aspect_index) {
        --aspect_index;
        if (mask & aspect_bits_[aspect_index]) return aspect_index + 1;  // Point to the one past the upper index of the upper bit
    }
    return limits_.aspect_index;  // None found, point to end
}

uint32_t RangeEncoder::LowerBoundFromMask(VkImageAspectFlags mask, uint32_t starting_index) const {
    uint32_t aspect_index = starting_index;
    for (; aspect_index < limits_.aspect_index; ++aspect_index) {
        if (mask & aspect_bits_[aspect_index]) return aspect_index;
    }
    return aspect_index;  // This is the at end condition. Test vs. RangeEncoder::Limits().aspect_index
}

Subresource RangeEncoder::MakeSubresource(const VkImageSubresource& subres) const {
    return Subresource(subres.arrayLayer, subres.mipLevel, LowerBoundFromMask(subres.aspectMask), subres.aspectMask);
}

SubresourceRange RangeEncoder::MakeSubresourceRange(const VkImageSubresourceRange& range) const {
    SubresourceRange bounds;
    bounds.end = Subresource(range.baseArrayLayer + range.layerCount, range.baseMipLevel + range.levelCount,
                             UpperBoundFromMask(range.aspectMask), range.aspectMask);
    if (InRange(range)) {
        bounds.begin =
            Subresource(range.baseArrayLayer, range.baseMipLevel, LowerBoundFromMask(range.aspectMask), range.aspectMask);
    } else {
...
RangeEncoder::RangeEncoder(const VkImageSubresourceRange& full_range, const AspectParameters* param)
    : limits_(full_range.layerCount, full_range.levelCount, param->AspectCount(), param->AspectMask()),
    ...
    assert(full_range.aspectMask == limits_.selected_aspects);
...
static bool IsValid(const SubresourceRange& bounds) {
    return (bounds.begin.aspect_index < bounds.end.aspect_index) && (bounds.begin.mip_level < bounds.end.mip_level) &&
           (bounds.begin.array_layer < bounds.end.array_layer);
}
...
//
// For sake of encode simplicity, spanning across *aspects* is not done by the generator
// Given the "merge" function in the range_map, the decode iterator will likely have to handle it.
RangeGenerator::RangeGenerator(const RangeEncoder& encoder, const VkImageSubresourceRange& subres_range)
    ...
    const auto& bounds = isr_pos_.Limits();
    assert(IsValid(bounds));
    auto base = encoder.Encode(bounds.begin);

    if ((subres_range.baseArrayLayer == 0 && subres_range.layerCount == encoder.Limits().array_layer)) {
        // Full set of array layers at a time, thus we can span across all selected mip levels
        IndexType mip_range_size = subres_range.levelCount * encoder.MipSize();
        pos_ = {base, base + mip_range_size};
        ...
        IndexType array_range_size = subres_range.layerCount;
        pos_ = {base, base + array_range_size};
    ...
    aspect_base_ = pos_;
    mip_index_ = 0;
    selected_aspects_ = subres_range.aspectMask;
    aspect_index_ = bounds.begin.aspect_index;
    mask_limit_ = bounds.end.aspect_index;
}
...
RangeGenerator& RangeGenerator::operator++() {
    ...
    if (mip_index_ >= mip_count_) {
        // Next aspect
        aspect_index_ = encoder_->LowerBoundFromMask(selected_aspects_, aspect_index_ + 1);
        if (aspect_index_ < mask_limit_) {
            aspect_base_ += encoder_->AspectBase(aspect_index_);
            pos_ = aspect_base_;
            mip_index_ = 0;
        } else {
            // Tombstone for "At end"
            pos_ = {0, 0};
        }
        // Allow the isr generator to tombstone as well by setting unconditionally
        ...
        isr_pos_.seek_mip(isr_pos_.Limits().begin.mip_level + mip_index_);
    }
    return *this;
}
