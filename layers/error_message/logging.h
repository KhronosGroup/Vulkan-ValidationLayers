...
struct LogObjectList {
    small_vector<VulkanTypedHandle, 4, u32> object_list;
...
template <typename HANDLE_T>
static inline u64 HandleToUint64(HANDLE_T h) {
    return CastToUint64<HANDLE_T>(h);
}

static inline u64 HandleToUint64(u64 h) { return h; }
...
class DebugReport {
  public:
    ...
    vvl::unordered_set<u32> filter_message_ids{};
    ...
    std::string GetUtilsObjectNameNoLock(const u64 object) const;
    std::string GetMarkerObjectNameNoLock(const u64 object) const;

    void SetDebugUtilsSeverityFlags(std::vector<VkLayerDbgFunctionState> &callbacks);
    void RemoveDebugUtilsCallback(u64 callback);

    std::string FormatHandle(const char *handle_type_name, u64 handle) const;
  ...
  private:
    bool UpdateLogMsgCounts(i32 vuid_hash) const;
    ...
    mutable vvl::unordered_map<u32, u32> duplicate_message_count_map{};
    ...
    vvl::unordered_map<u64, std::string> debug_object_name_map;
    vvl::unordered_map<u64, std::string> debug_utils_object_name_map;
};
