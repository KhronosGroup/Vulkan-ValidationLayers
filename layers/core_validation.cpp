    ...
    if (current_vertex_state.binding_descriptions_.size() > 0) {
        ...
        for (size_t i = 0; i < current_vertex_state.attribute_descriptions_.size(); i++) {
            ...
            if ((vertex_binding_map_it != current_vertex_state.binding_to_index_map_.cend()) &&
                ...
                (current_vtx_bfr_binding_info[vertex_binding].buffer != VK_NULL_HANDLE)) {
                const auto vertex_buffer_stride = current_vertex_state.binding_descriptions_[vertex_binding_map_it->second].stride;
...
bool CoreChecks::PreCallValidateCmdExecuteGeneratedCommandsNV(VkCommandBuffer commandBuffer, VkBool32 isPreprocessed,
    ...
    const CMD_BUFFER_STATE *cb_state = GetCBState(commandBuffer);
    const auto last_bound_it = cb_state->lastBound.find(pGeneratedCommandsInfo->pipelineBindPoint);
    const PIPELINE_STATE *pPipe = nullptr;
    if (last_bound_it != cb_state->lastBound.cend()) {
        pPipe = last_bound_it->second.pipeline_state;
    }
