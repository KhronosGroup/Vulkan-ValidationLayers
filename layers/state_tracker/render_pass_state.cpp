...
static void RecordRenderPassDAG(const VkRenderPassCreateInfo2* pCreateInfo, vvl::RenderPass* render_pass)
{
    ...
    for (auto& subpass_dep : subpass_dependencies)
    {
        ...
        if (subpass_dep.barrier_to_external.empty())
        {
            ...
            subpass_dep.implicit_barrier_to_external =
                std::make_unique<VkSubpassDependency2>(ImplicitDependencyToExternal(pass));
    ...
    // syncronization is only interested in asyncronous stages *earlier* that the current one... so we'll only look
    // towards those. NOTE: This is O(N^3), which we could shrink to O(N^2logN) using sets instead of arrays, but given
    // that N is likely to be small and the K for |= from the prev is must less than for set, we'll accept the brute
    // force.
...
{ // This is really only of local interest, but a bit big for a lambda
    ...
    explicit AttachmentTracker(vvl::RenderPass* render_pass) :
        rp(render_pass), first(const_cast<vvl::RenderPass::SubpassVec&>(rp->attachment_first_subpass)),
        ...
        attachment_count(rp->create_info.attachmentCount), attachment_layout(), subpass_attachment_layout()
    {
    ...
    void Update(uint32_t subpass, const VkAttachmentReference2* attach_ref, uint32_t count, bool is_read)
    {
        ...
        for (uint32_t j = 0; j < count; ++j)
        {
            ...
            if (attachment != VK_ATTACHMENT_UNUSED)
            {
                ...
                if (first[attachment] == VK_SUBPASS_EXTERNAL)
                {
                    ...
                    if (initial_layout != layout)
                    {
                        subpass_transitions[subpass].emplace_back(
                            VK_SUBPASS_EXTERNAL, attachment, initial_layout, layout);
                ...
                {
                    // This will insert a layout transition when dependencies are missing between first and subsequent
                    // use but is consistent with the idea of an implicit external dependency
                    if (initial_layout != layout)
                    {
                        subpass_transitions[subpass].emplace_back(
                            VK_SUBPASS_EXTERNAL, attachment, initial_layout, layout);
    ...
    void FinalTransitions()
    {
        ...
        for (uint32_t attachment = 0; attachment < attachment_count; ++attachment)
        {
            ...
            {
                final_transitions.emplace_back(
                    last[attachment], attachment, attachment_layout[attachment], final_layout);
            }
        }
    }
};
...
RenderPass::RenderPass(VkRenderPass handle, VkRenderPassCreateInfo2 const* pCreateInfo) :
    StateObject(handle, kVulkanObjectTypeRenderPass), use_dynamic_rendering(false),
    use_dynamic_rendering_inherited(false), has_multiview_enabled(false), create_info(pCreateInfo)
{
...
RenderPass::RenderPass(VkRenderPass handle, VkRenderPassCreateInfo const* pCreateInfo) :
    StateObject(handle, kVulkanObjectTypeRenderPass), use_dynamic_rendering(false),
    use_dynamic_rendering_inherited(false), has_multiview_enabled(false), create_info(ConvertCreateInfo(*pCreateInfo))
{
    InitRenderPassState(this);
}
...
RenderPass::RenderPass(VkPipelineRenderingCreateInfo const* pPipelineRenderingCreateInfo, bool rasterization_enabled) :
    StateObject(static_cast<VkRenderPass>(VK_NULL_HANDLE), kVulkanObjectTypeRenderPass), use_dynamic_rendering(true),
    use_dynamic_rendering_inherited(false), has_multiview_enabled(false), rasterization_enabled(rasterization_enabled),
...
const VkMultisampledRenderToSingleSampledInfoEXT* RenderPass::GetMSRTSSInfo(uint32_t subpass) const
{
    ...
    return vku::FindStructInPNextChain<VkMultisampledRenderToSingleSampledInfoEXT>(
        create_info.pSubpasses[subpass].pNext);
}

RenderPass::RenderPass(VkRenderingInfo const* pRenderingInfo, bool rasterization_enabled) :
    StateObject(static_cast<VkRenderPass>(VK_NULL_HANDLE), kVulkanObjectTypeRenderPass), use_dynamic_rendering(true),
    use_dynamic_rendering_inherited(false), has_multiview_enabled(IsDynamicRenderingMultiviewEnabled(
                                                (pRenderingInfo && rasterization_enabled) ? pRenderingInfo : nullptr)),
...
RenderPass::RenderPass(VkCommandBufferInheritanceRenderingInfo const* pInheritanceRenderingInfo) :
    StateObject(static_cast<VkRenderPass>(VK_NULL_HANDLE), kVulkanObjectTypeRenderPass), use_dynamic_rendering(false),
    use_dynamic_rendering_inherited(true), has_multiview_enabled(false),
    inheritance_rendering_info(pInheritanceRenderingInfo)
{}

Framebuffer::Framebuffer(VkFramebuffer                                  handle,
                         const VkFramebufferCreateInfo*                 pCreateInfo,
                         std::shared_ptr<RenderPass>&&                  rpstate,
                         std::vector<std::shared_ptr<vvl::ImageView>>&& attachments) :
    StateObject(handle, kVulkanObjectTypeFramebuffer),
    safe_create_info(pCreateInfo), create_info(*safe_create_info.ptr()), rp_state(rpstate),
