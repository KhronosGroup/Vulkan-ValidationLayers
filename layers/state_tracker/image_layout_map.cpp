...
static bool UpdateLayoutStateImpl(LayoutsMap& layouts,
                                  InitialLayoutStates& initial_layout_states,
                                  const IndexRange& range,
                                  LayoutEntry& new_entry,
        ...
        if (pos->valid) {
            ...
            if (!intersected_range.empty() && pos->lower_bound->second.CurrentWillChange(new_entry.current_layout)) {
                ...
                auto overwrite_result =
                    layouts.overwrite_range(pos->lower_bound, std::make_pair(intersected_range, orig_entry));
...
static bool SetSubresourceRangeLayoutImpl(LayoutMap& layouts,
                                          InitialLayoutStates& initial_layout_states,
...
bool ImageLayoutRegistry::SetSubresourceRangeLayout(const vvl::CommandBuffer& cb_state,
    ...
    if (!InRange(range))
        return false; // Don't even try to track bogus subreources

    RangeGenerator range_gen(encoder_, range);
    if (layout_map_.SmallMode()) {
        return SetSubresourceRangeLayoutImpl(
            layout_map_.GetSmallMap(), initial_layout_states_, range_gen, cb_state, layout, expected_layout);
    } else {
        assert(!layout_map_.Tristate());
        return SetSubresourceRangeLayoutImpl(
            layout_map_.GetBigMap(), initial_layout_states_, range_gen, cb_state, layout, expected_layout);
    }
}

// Use the unwrapped maps from the BothMap in the actual implementation
template <typename LayoutMap>
static void SetSubresourceRangeInitialLayoutImpl(LayoutMap& layouts,
                                                 InitialLayoutStates& initial_layout_states,
                                                 RangeGenerator& range_gen,
                                                 const vvl::CommandBuffer& cb_state,
                                                 VkImageLayout layout,
...
void ImageLayoutRegistry::SetSubresourceRangeInitialLayout(const vvl::CommandBuffer& cb_state,
                                                           const VkImageSubresourceRange& range,
                                                           VkImageLayout layout) {
    if (!InRange(range))
    ...
    RangeGenerator range_gen(encoder_, range);
    if (layout_map_.SmallMode()) {
        SetSubresourceRangeInitialLayoutImpl(
            layout_map_.GetSmallMap(), initial_layout_states_, range_gen, cb_state, layout, nullptr);
    } else {
        assert(!layout_map_.Tristate());
        SetSubresourceRangeInitialLayoutImpl(
            layout_map_.GetBigMap(), initial_layout_states_, range_gen, cb_state, layout, nullptr);
    }
}
...
void ImageLayoutRegistry::SetSubresourceRangeInitialLayout(const vvl::CommandBuffer& cb_state,
    ...
    if (layout_map_.SmallMode()) {
        SetSubresourceRangeInitialLayoutImpl(
            layout_map_.GetSmallMap(), initial_layout_states_, range_gen, cb_state, layout, &view_state);
    } else {
        assert(!layout_map_.Tristate());
        SetSubresourceRangeInitialLayoutImpl(
            layout_map_.GetBigMap(), initial_layout_states_, range_gen, cb_state, layout, &view_state);
    }
}
...
uint32_t ImageLayoutRegistry::GetImageId() const {
    return image_state_.GetId();
}
