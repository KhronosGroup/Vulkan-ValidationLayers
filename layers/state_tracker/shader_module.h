 ...
 *
 * The Shader Module file is in charge of all things around creating and parsing an internal representation of a shader
 * module
 */
...
// Can't have nested structs with OpMemberDecorate, this class prevents accidently creating a 2nd level of member
// decorations,
...
struct InterfaceSlot {
    ...
    // Having a single uint32_t slot allows a 64-bit Vec3 to pass in (Loc 0, Comp 5) and have it automatically mean (Loc
    // 1, Comp 1)
...
struct VariableBase {
    ...
    VariableBase(const Module& module_state,
                 const Instruction& insn,
    ...
    // We need to store a std::string since the original SPIR-V string will be gone when we need to print this in an
    // error message
    ...
    // (It would require a lot of GPU-AV overhead to detect if the access is dynamic and that level of fine control is
    // currently not required) While SPIR-V has its own terms/concepts, the following is designed to match the Vulkan
    // Spec.
    // -------
    ...
    // Read/Write here refer to the variable itself. For a buffer this means the memory has been accessed. For an Image,
    // this only means the descriptor itself has been accessed
    ...
    // Images are special and will first have a read/write to the descriptor, then an Image Operation to the image
    // memory itself.
    // - Some operations such as ImageSize() will read data from the descriptor and never actually read the image memory
    // (it would return false for IsImageReadFrom()).
    // - A storage image is always "loaded" before it is written, but it will only return true for IsImageReadFrom() if
    // the image memory was read
...
//
// These include both BuiltIns and User Defined, while there are difference in member variables, the variables are
// needed for the common logic so its easier using the same object in the end
struct StageInterfaceVariable : public VariableBase {
    ...
    StageInterfaceVariable(const Module& module_state,
                           const Instruction& insn,
                           VkShaderStageFlagBits stage,
                           const VariableAccessMap& variable_access_map,
                           const DebugNameMap& debug_name_map);
...
struct ResourceInterfaceVariable : public VariableBase {
    ...
    struct Info {
        ...
        // vkspec.html#spirvenv-image-signedness describes how SignExtend/ZeroExtend can be used per-access to adjust
        // the Signedness Only need to check if one access has explicit signedness, mixing should be caught in spirv-val
        ...
        // If a variable is used as a function arguement, but never actually used, it will be found in
        // EntryPoint::accessible_ids so we need to have a dedicated mark if it was accessed. We use this for variable
        // hashing, but the VariableBase has the helper functions to read this value.
        uint32_t access_mask{ AccessBit::empty };
    } info;
    ...
    ResourceInterfaceVariable(const Module& module_state,
...
struct PushConstantVariable : public VariableBase {
    // This info could be found/saved in TypeStructInfo, but since Push Constants are the only ones using it right now,
    // no point to do it for every struct
    uint32_t offset; // where first member is
    uint32_t size;   // total size of block

    PushConstantVariable(const Module& module_state,
                         const Instruction& insn,
                         VkShaderStageFlagBits stage,
                         const VariableAccessMap& variable_access_map,
                         const DebugNameMap& debug_name_map);
...
struct EntryPoint {
    ...
    EntryPoint(const Module& module_state,
               const Instruction& entrypoint_insn,
               const ImageAccessMap& image_access_map,
               const AccessChainVariableMap& access_chain_map,
               const VariableAccessMap& variable_access_map,
               const DebugNameMap& debug_name_map);
  ...
  protected:
    ...
    static std::vector<StageInterfaceVariable> GetStageInterfaceVariables(const Module& module_state,
                                                                          const EntryPoint& entrypoint,
                                                                          const VariableAccessMap& variable_access_map,
                                                                          const DebugNameMap& debug_name_map);
    static std::vector<ResourceInterfaceVariable>
    GetResourceInterfaceVariables(const Module& module_state,
                                  EntryPoint& entrypoint,
                                  const ImageAccessMap& image_access_map,
                                  const AccessChainVariableMap& access_chain_map,
                                  const VariableAccessMap& variable_access_map,
                                  const DebugNameMap& debug_name_map);
    static bool IsBuiltInWritten(spv::BuiltIn built_in,
                                 const Module& module_state,
                                 const StageInterfaceVariable& variable,
                                 const AccessChainVariableMap& access_chain_map);
};

// Info to capture while parsing the SPIR-V, but will only be used by ValidateSpirvStateless and don't need to save
// after
struct StatelessData {
    ...
    // These instruction mapping were designed to quickly find the few instructions without having to loop the entire
    // pass In theory, these could be removed checked during the 2nd pass in ValidateSpirvStateless()
...
struct Module {
    ...
    struct StaticData {
        ...
        ExecutionModeSet
            empty_execution_mode; // all zero values, allows use to return a reference and not a copy each time
    ...
    // VK_KHR_maintenance5 allows VkShaderModuleCreateInfo (the SPIR-V binary) to be passed at pipeline creation time,
    // because the way we create our pipeline state objects first, we need to still create a valid Module object, but
    // can signal that the underlying spirv is not worth validating further
    ...
    // Used for when modifying the SPIR-V (spirv-opt, GPU-AV instrumentation, etc) and need reparse it for VVL
    // validation
    Module(vvl::span<const uint32_t> code) : valid_spirv(true), words_(code.begin(), code.end()), static_data_(*this) {}

    // StatelessData is a pointer as we have cases were we don't need it and simpler to just null check the few cases
    // that use it
    ...
    const Instruction* FindDef(uint32_t id) const {
        ...
        if (it == static_data_.definitions.end())
            return nullptr;
        return it->second;
    }
    ...
    bool FindLocalSize(const EntryPoint& entrypoint,
    ...
    bool HasCapability(spv::Capability find_capability) const {
        return std::any_of(
            static_data_.capability_list.begin(),
            static_data_.capability_list.end(),
