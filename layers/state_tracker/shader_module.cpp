...
EntryPoint::EntryPoint(const Module& module_state, const Instruction& entrypoint_insn, const StaticImageAccessMap& image_access_map,
    ...
    : entrypoint_insn(entrypoint_insn),
      execution_model((entrypoint_insn.Opcode() == spv::OpGraphEntryPointARM) ? spv::ExecutionModelGLCompute
                                                                              : spv::ExecutionModel(entrypoint_insn.Word(1))),
      stage((entrypoint_insn.Opcode() == spv::OpGraphEntryPointARM)
                ? VK_SHADER_STAGE_ALL
                : static_cast<VkShaderStageFlagBits>(ExecutionModelToShaderStageFlagBits(execution_model))),
      id((entrypoint_insn.Opcode() == spv::OpGraphEntryPointARM) ? entrypoint_insn.Word(1) : entrypoint_insn.Word(2)),
      name((entrypoint_insn.Opcode() == spv::OpGraphEntryPointARM) ? entrypoint_insn.GetAsString(2)
                                                                   : entrypoint_insn.GetAsString(3)),
