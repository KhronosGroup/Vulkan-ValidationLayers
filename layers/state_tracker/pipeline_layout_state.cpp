...
std::string PipelineLayoutCompatDef::DescribeDifference(const PipelineLayoutCompatDef& other) const {
    ...
    if (set != other.set) {
        ...
        ss << "Pipeline layout pipeline bound with last call to vkCmdBindDescriptorSets has following push constant "
              "ranges:\n";
        if (push_constant_ranges->empty()) {
            ss << "Empty\n";
        } else {
            for (const auto& [pcr_i, pcr] :
                 vvl::enumerate(push_constant_ranges->data(), push_constant_ranges->size())) {
...
static PipelineLayoutCompatId GetCanonicalId(const uint32_t set_index,
...
static PushConstantRangesId
GetPushConstantRangesFromLayouts(const vvl::span<const vvl::PipelineLayout* const>& layouts) {
            ...
            if (layout) {
                if (layout->set_layouts.size() > i) {
                    ...
                    if (layout->set_layouts[i]) {
                        // This is the layout we're looking for. Any subsequent ones that match must be identical to
                        // this one.
                        break;
                    }
                }
...
PipelineLayout::PipelineLayout(ValidationStateTracker& dev_data,
    ...
    set_compat_ids(GetCompatForSet(set_layouts, push_constant_ranges_layout)), create_flags(pCreateInfo->flags) {}
...
PipelineLayout::PipelineLayout(const vvl::span<const PipelineLayout* const>& layouts) :
    ...
    set_compat_ids(GetCompatForSet(set_layouts, push_constant_ranges_layout)), create_flags(GetCreateFlags(layouts)) {}
