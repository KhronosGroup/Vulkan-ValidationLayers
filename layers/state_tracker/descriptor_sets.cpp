...
vvl::DescriptorPool::DescriptorPool(ValidationStateTracker& dev,
                                    const VkDescriptorPool handle,
                                    const VkDescriptorPoolCreateInfo* pCreateInfo) :
    StateObject(handle, kVulkanObjectTypeDescriptorPool),
    safe_create_info(pCreateInfo), create_info(*safe_create_info.ptr()), maxSets(pCreateInfo->maxSets),
    maxDescriptorTypeCount(GetMaxTypeCounts(pCreateInfo)), available_sets_(pCreateInfo->maxSets),
    available_counts_(maxDescriptorTypeCount), dev_data_(dev) {}

void vvl::DescriptorPool::Allocate(const VkDescriptorSetAllocateInfo* alloc_info,
...
struct ExtendedBinding {
    ExtendedBinding(const VkDescriptorSetLayoutBinding* l, VkDescriptorBindingFlags f) :
        layout_binding(l), binding_flags(f) {}
...
bool operator==(const DescriptorSetLayoutDef& lhs, const DescriptorSetLayoutDef& rhs) {
    ...
    for (uint32_t i = 0; i < lhs_bindings.size(); i++) {
        ...
        // For things where we are comparing with the bound pipeline, the binding will always be right, but when
        // comparing two arbitrary layouts (ex. templates, Device Generated Commands, etc) the bindings might be
        // different
        if (l.binding != r.binding) {
            return false;
        }
        if (l.descriptorType != r.descriptorType || l.descriptorCount != r.descriptorCount ||
...
std::string DescriptorSetLayoutDef::DescribeDifference(uint32_t index, const DescriptorSetLayoutDef& other) const {
    ...
    if (GetCreateFlags() != other.GetCreateFlags()) {
        ss << "VkDescriptorSetLayoutCreateFlags " << string_VkDescriptorSetLayoutCreateFlags(GetCreateFlags())
           << " doesn't match " << string_VkDescriptorSetLayoutCreateFlags(other.GetCreateFlags());
    } else if (lhs_binding_flags.size() != rhs_binding_flags.size()) {
        ss << "VkDescriptorSetLayoutBindingFlagsCreateInfo::bindingCount " << lhs_binding_flags.size()
           << " doesn't match " << rhs_binding_flags.size();
        ...
        for (uint32_t i = 0; i < lhs_bindings.size(); i++) {
            ...
            if (l.binding != r.binding) {
                ...
                ss << "binding " << i << " descriptorType " << string_VkDescriptorType(l.descriptorType)
                   << " doesn't match " << string_VkDescriptorType(r.descriptorType);
                break;
            } else if (l.descriptorCount != r.descriptorCount) {
                ss << "binding " << i << " descriptorCount " << l.descriptorCount << " doesn't match "
                   << r.descriptorCount;
                break;
                ...
                for (uint32_t s = 0; s < l.descriptorCount; s++) {
                    if (l.pImmutableSamplers[s] != r.pImmutableSamplers[s]) {
                        ss << "binding " << i << " pImmutableSamplers[" << s << "] " << l.pImmutableSamplers[s]
                           << " doesn't match " << r.pImmutableSamplers[s];
                        break;
                    }
                }
...
vvl::DescriptorSetLayoutDef::DescriptorSetLayoutDef(const VkDescriptorSetLayoutCreateInfo* p_create_info) :
    flags_(p_create_info->flags), binding_count_(0), descriptor_count_(0), non_inline_descriptor_count_(0),
    dynamic_descriptor_count_(0) {
    const auto* flags_create_info =
        vku::FindStructInPNextChain<VkDescriptorSetLayoutBindingFlagsCreateInfo>(p_create_info->pNext);
    ...
    const auto* mutable_descriptor_type_create_info =
...
uint32_t vvl::DescriptorSetLayoutDef::GetIndexFromBinding(uint32_t binding) const {
    ...
    if (bi_itr != binding_to_index_map_.cend())
        return bi_itr->second;
    return GetBindingCount();
}
VkDescriptorSetLayoutBinding const*
vvl::DescriptorSetLayoutDef::GetDescriptorSetLayoutBindingPtrFromIndex(const uint32_t index) const {
...
uint32_t vvl::DescriptorSetLayoutDef::GetNextValidBinding(const uint32_t binding) const {
    ...
    if (it != non_empty_bindings_.cend())
        return *it;
...
bool vvl::DescriptorSetLayoutDef::IsTypeMutable(const VkDescriptorType type, uint32_t binding) const {
    ...
    // If mutableDescriptorTypeListCount is zero or if VkMutableDescriptorTypeCreateInfoEXT structure is not included in
    // the pNext chain, the VkMutableDescriptorTypeListEXT for each element is considered to be zero or NULL for each
    // member.
    return false;
}
...
// The DescriptorSetLayout stores the per handle data for a descriptor set layout, and references the common defintion
// for the handle invariant portion
...
vvl::DescriptorSet::DescriptorSet(const VkDescriptorSet handle,
                                  vvl::DescriptorPool* pool_state,
    ...
    some_update_(false), pool_state_(pool_state), layout_(layout), state_data_(state_data),
    variable_count_(variable_count), change_count_(0) {
    ...
    for (uint32_t i = 0; i < binding_count; ++i) {
        ...
        switch (descriptor_class) {
            ...
            case DescriptorClass::InlineUniform: {
                bindings_.push_back(
                    MakeBinding<InlineUniformBinding>(free_binding++, *create_info, descriptor_count, flags));
                break;
            }
...
// Prereq: This should be called for a set that has been confirmed to be active for the given cb_state, meaning it's
// going
//   to be used in a draw by the given cb_state
void vvl::DescriptorSet::UpdateDrawStates(ValidationStateTracker* device_data,
    ...
    // For the active slots, use set# to look up descriptorSet from boundDescriptorSets, and bind all of that descriptor
    // set's resources
...
bool vvl::DescriptorSet::ValidateBindingOnGPU(const DescriptorBinding& binding,
                                              bool is_runtime_descriptor_array) const {
    // Some applications (notably Doom Eternal) might have large non-bindless descriptors attached (basically doing
    // Descriptor Indexing without the extension). Trying to loop through these on the CPU will bring FPS down by over
    // 50% so we make use of the post processing to detect which descriptors were actually accessed
...
static void ReplaceStatePtr(DescriptorSet& set_state, T& dst, const T& src, bool is_bindless) {
    ...
    dst = src;
    // For descriptor bindings with UPDATE_AFTER_BIND or PARTIALLY_BOUND only set the object as a child, but not the
    // descriptor as a parent, so that destroying the object wont invalidate the descriptor
    if (dst && !is_bindless) {
        dst->AddParent(&set_state);
    }
}

void vvl::SamplerDescriptor::WriteUpdate(DescriptorSet& set_state,
                                         const ValidationStateTracker& dev_data,
                                         const VkWriteDescriptorSet& update,
                                         const uint32_t index,
                                         bool is_bindless) {
    if (!immutable_ && update.pImageInfo) {
        ReplaceStatePtr(set_state,
                        sampler_state_,
                        dev_data.GetConstCastShared<vvl::Sampler>(update.pImageInfo[index].sampler),
                        is_bindless);
    }
}

void vvl::SamplerDescriptor::CopyUpdate(DescriptorSet& set_state,
                                        const ValidationStateTracker& dev_data,
...
VkSampler vvl::SamplerDescriptor::GetSampler() const {
    return sampler_state_ ? sampler_state_->VkHandle() : VK_NULL_HANDLE;
}
...
bool vvl::SamplerDescriptor::Invalid() const {
    return !sampler_state_ || sampler_state_->Invalid();
}

void vvl::ImageSamplerDescriptor::WriteUpdate(DescriptorSet& set_state,
                                              const ValidationStateTracker& dev_data,
                                              const VkWriteDescriptorSet& update,
                                              const uint32_t index,
                                              bool is_bindless) {
    ...
    if (!immutable_) {
        ReplaceStatePtr(
            set_state, sampler_state_, dev_data.GetConstCastShared<vvl::Sampler>(image_info.sampler), is_bindless);
    }
    image_layout_ = image_info.imageLayout;
    ReplaceStatePtr(
        set_state, image_view_state_, dev_data.GetConstCastShared<vvl::ImageView>(image_info.imageView), is_bindless);
    UpdateKnownValidView(is_bindless);
}

void vvl::ImageSamplerDescriptor::CopyUpdate(DescriptorSet& set_state,
                                             const ValidationStateTracker& dev_data,
                                             const Descriptor& src,
                                             bool is_bindless,
...
void vvl::ImageDescriptor::WriteUpdate(DescriptorSet& set_state,
                                       const ValidationStateTracker& dev_data,
                                       const VkWriteDescriptorSet& update,
                                       const uint32_t index,
                                       bool is_bindless) {
...
void vvl::ImageDescriptor::CopyUpdate(DescriptorSet& set_state,
                                      const ValidationStateTracker& dev_data,
                                      const Descriptor& src,
                                      bool is_bindless,
                                      VkDescriptorType src_type) {
...
bool vvl::ImageDescriptor::Invalid() const {
    return !known_valid_view_ && ComputeInvalid();
}
...
void vvl::ImageDescriptor::UpdateKnownValidView(bool is_bindless) {
    known_valid_view_ = !is_bindless && !ComputeInvalid();
}

void vvl::BufferDescriptor::WriteUpdate(DescriptorSet& set_state,
                                        const ValidationStateTracker& dev_data,
                                        const VkWriteDescriptorSet& update,
                                        const uint32_t index,
                                        bool is_bindless) {
...
void vvl::BufferDescriptor::CopyUpdate(DescriptorSet& set_state,
                                       const ValidationStateTracker& dev_data,
                                       const Descriptor& src,
                                       bool is_bindless,
                                       VkDescriptorType src_type) {
...
VkBuffer vvl::BufferDescriptor::GetBuffer() const {
    return buffer_state_ ? buffer_state_->VkHandle() : VK_NULL_HANDLE;
}
...
VkDeviceSize vvl::BufferDescriptor::GetEffectiveRange() const {
    ...
    if (range_ == VK_WHOLE_SIZE && buffer_state_) {
        // When range is VK_WHOLE_SIZE the effective range is calculated at vkUpdateDescriptorSets is by taking the size
        // of buffer minus the offset.
...
void vvl::TexelDescriptor::WriteUpdate(DescriptorSet& set_state,
                                       const ValidationStateTracker& dev_data,
                                       const VkWriteDescriptorSet& update,
                                       const uint32_t index,
                                       bool is_bindless) {
...
void vvl::TexelDescriptor::CopyUpdate(DescriptorSet& set_state,
                                      const ValidationStateTracker& dev_data,
                                      const Descriptor& src,
                                      bool is_bindless,
                                      VkDescriptorType src_type) {
    if (src.GetClass() == DescriptorClass::Mutable) {
        ReplaceStatePtr(set_state,
                        buffer_view_state_,
                        static_cast<const MutableDescriptor&>(src).GetSharedBufferViewState(),
                        is_bindless);
        return;
    }
    ReplaceStatePtr(
        set_state, buffer_view_state_, static_cast<const TexelDescriptor&>(src).buffer_view_state_, is_bindless);
}
...
void vvl::AccelerationStructureDescriptor::WriteUpdate(DescriptorSet& set_state,
                                                       const ValidationStateTracker& dev_data,
                                                       const VkWriteDescriptorSet& update,
                                                       const uint32_t index,
                                                       bool is_bindless) {
    ...
    if (is_khr_) {
        ...
        ReplaceStatePtr(
            set_state, acc_state_, dev_data.GetConstCastShared<vvl::AccelerationStructureKHR>(acc_), is_bindless);
    } else {
        acc_nv_ = acc_info_nv->pAccelerationStructures[index];
        ReplaceStatePtr(
            set_state, acc_state_nv_, dev_data.GetConstCastShared<vvl::AccelerationStructureNV>(acc_nv_), is_bindless);
    }
}

void vvl::AccelerationStructureDescriptor::CopyUpdate(DescriptorSet& set_state,
                                                      const ValidationStateTracker& dev_data,
                                                      const Descriptor& src,
                                                      bool is_bindless,
                                                      VkDescriptorType src_type) {
    if (src.GetClass() == DescriptorClass::Mutable) {
        ...
        if (is_khr_) {
            ...
            ReplaceStatePtr(set_state,
                            acc_state_nv_,
                            dev_data.GetConstCastShared<vvl::AccelerationStructureNV>(acc_nv_),
                            is_bindless);
        }
        return;
    }
...
vvl::MutableDescriptor::MutableDescriptor() :
    Descriptor(), buffer_size_(0), active_descriptor_type_(VK_DESCRIPTOR_TYPE_MUTABLE_EXT), immutable_(false),
    image_layout_(VK_IMAGE_LAYOUT_UNDEFINED), offset_(0), range_(0), is_khr_(false), acc_(VK_NULL_HANDLE) {}

void vvl::MutableDescriptor::WriteUpdate(DescriptorSet& set_state,
                                         const ValidationStateTracker& dev_data,
                                         const VkWriteDescriptorSet& update,
                                         const uint32_t index,
                                         bool is_bindless) {
    ...
    switch (DescriptorTypeToClass(update.descriptorType)) {
        ...
        case DescriptorClass::ImageSampler: {
            if (update.pImageInfo) {
                ...
                if (!immutable_) {
                    ReplaceStatePtr(set_state,
                                    sampler_state_,
                                    dev_data.GetConstCastShared<vvl::Sampler>(image_info.sampler),
                                    is_bindless);
                }
                image_layout_ = image_info.imageLayout;
                ReplaceStatePtr(set_state,
                                image_view_state_,
                                dev_data.GetConstCastShared<vvl::ImageView>(image_info.imageView),
                                is_bindless);
            }
            break;
        }
        ...
        case DescriptorClass::AccelerationStructure: {
            const auto* acc_info =
                vku::FindStructInPNextChain<VkWriteDescriptorSetAccelerationStructureKHR>(update.pNext);
            const auto* acc_info_nv =
            ...
            if (is_khr_) {
                ...
                ReplaceStatePtr(set_state,
                                acc_state_,
                                dev_data.GetConstCastShared<vvl::AccelerationStructureKHR>(acc_),
                                is_bindless);
            } else {
                acc_nv_ = acc_info_nv->pAccelerationStructures[index];
                ReplaceStatePtr(set_state,
...
void vvl::MutableDescriptor::CopyUpdate(DescriptorSet& set_state,
                                        const ValidationStateTracker& dev_data,
                                        const Descriptor& src,
                                        bool is_bindless,
                                        VkDescriptorType src_type) {
    ...
    switch (src.GetClass()) {
        ...
        case DescriptorClass::TexelBuffer: {
            ReplaceStatePtr(set_state,
                            buffer_view_state_,
                            static_cast<const TexelDescriptor&>(src).GetSharedBufferViewState(),
                            is_bindless);
