...
class Semaphore : public RefcountedStateObject {
  public:
    ...
    Semaphore(ValidationStateTracker& dev, VkSemaphore handle, const VkSemaphoreCreateInfo* pCreateInfo) :
        Semaphore(
            dev, handle, vku::FindStructInPNextChain<VkSemaphoreTypeCreateInfo>(pCreateInfo->pNext), pCreateInfo) {}
    ...
    std::optional<SemOp>
    LastOp(const std::function<bool(OpType op_type, uint64_t payload, bool is_pending)>& filter) const;
  ...
  private:
    Semaphore(ValidationStateTracker& dev,
              VkSemaphore handle,
    ...
    // (validation object has to use {Begin/End}BlockingOperation() when waiting for the timepoint)
    void WaitTimePoint(std::shared_future<void>&& waiter,
                       uint64_t payload,
                       bool unblock_validation_object,
                       const Location& loc);
