...
static bool IsMultiInstance(const VkMemoryAllocateInfo* p_alloc_info,
...
namespace vvl {
DeviceMemory::DeviceMemory(VkDeviceMemory handle,
    ...
    safe_allocate_info(allocate_info), allocate_info(*safe_allocate_info.ptr()),
    export_handle_types(GetExportHandleTypes(allocate_info)), import_handle_type(GetImportHandleType(allocate_info)),
    ...
    dedicated(std::move(dedicated_binding)), mapped_range{},
#ifdef VK_USE_PLATFORM_METAL_EXT
    metal_buffer_export(GetMetalExport(allocate_info)),
#endif // VK_USE_PLATFORM_METAL_EXT
    p_driver_data(nullptr), fake_base_address(fake_address) {
}
} // namespace vvl

void vvl::BindableLinearMemoryTracker::BindMemory(StateObject* parent,
    ...
    if (!is_resident_) {
        VkDeviceSize current_offset = 0u;
        {
            ...
            for (const auto& range : binding_map_) {
                if (current_offset != range.first.begin || !range.second.memory_state ||
                    range.second.memory_state->Invalid()) {
                    return false;
                }
        ...
        if (current_offset != resource_size_)
            return false;
    }
...
void vvl::BindableSparseMemoryTracker::BindMemory(StateObject* parent,
                                                  std::shared_ptr<vvl::DeviceMemory>& mem_state,
                                                  VkDeviceSize memory_offset,
                                                  VkDeviceSize resource_offset,
                                                  VkDeviceSize size) {
    ...
    for (auto& value_pair : binding_map_) {
        if (value_pair.second.memory_state)
            value_pair.second.memory_state->RemoveParent(parent);
    }
    binding_map_.overwrite_range(item);

    for (auto& value_pair : binding_map_) {
        if (value_pair.second.memory_state)
            value_pair.second.memory_state->AddParent(parent);
    }
}
    ...
    for (auto it = range_bounds.begin; it != range_bounds.end; ++it) {
        ...
        if (memory_data.memory_state && memory_data.memory_state->VkHandle() != VK_NULL_HANDLE) {
            ...
            const VkDeviceSize memory_range_end =
                std::min(range.end, memory_data.resource_offset + resource_range.distance()) -
                memory_data.resource_offset + memory_data.memory_offset;
...
BoundRanges vvl::BindableSparseMemoryTracker::GetBoundRanges(const BufferRange& ranges_bounds,
    ...
    for (auto it = bound_memory_ranges.begin; it != bound_memory_ranges.end; ++it) {
        ...
        if (bounds_buffer_range_memory.memory_state &&
            bounds_buffer_range_memory.memory_state->VkHandle() != VK_NULL_HANDLE) {
            ...
            bounds_memory_range.begin = std::max(ranges_bounds.begin, bounds_buffer_range_memory.resource_offset) -
                                        bounds_buffer_range_memory.resource_offset +
                                        bounds_buffer_range_memory.memory_offset;
            bounds_memory_range.end =
                std::min(ranges_bounds.end,
            ...
            for (const BufferRange& buffer_range : buffer_ranges) {
                ...
                auto insert_pos = std::lower_bound(
                    vk_memory_ranges_vec.begin(),
                    vk_memory_ranges_vec.end(),
                    mem_and_buffer_range,
                    [](const std::pair<MemoryRange, BufferRange>& lhs, const std::pair<MemoryRange, BufferRange>& rhs) {
...
DeviceMemoryState vvl::BindableSparseMemoryTracker::GetBoundMemoryStates() const {
...

    {
        ...
        for (auto& binding : binding_map_) {
            if (binding.second.memory_state)
                dev_mem_states.emplace(binding.second.memory_state);
        }
    }
...
void vvl::BindableMultiplanarMemoryTracker::BindMemory(StateObject* parent,
                                                       std::shared_ptr<vvl::DeviceMemory>& mem_state,
                                                       VkDeviceSize memory_offset,
                                                       VkDeviceSize resource_offset,
                                                       VkDeviceSize size) {
...
std::pair<VkDeviceMemory, MemoryRange> vvl::Bindable::GetResourceMemoryOverlap(
    const MemoryRange& memory_region, const Bindable* other_resource, const MemoryRange& other_memory_region) const {
    if (!other_resource)
