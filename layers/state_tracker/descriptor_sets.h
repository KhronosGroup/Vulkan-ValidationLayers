...
class DescriptorPool : public StateObject {
  public:
    ...
    void Allocate(const VkDescriptorSetAllocateInfo* alloc_info,
...
class DescriptorUpdateTemplate : public StateObject {
  public:
    ...
    DescriptorUpdateTemplate(VkDescriptorUpdateTemplate handle,
...
class DescriptorSetLayoutDef {
  public:
    ...
    const VkDescriptorSetLayoutBinding* GetBindingInfoFromIndex(const uint32_t index) const {
    ...
    VkDescriptorType GetTypeFromBinding(const uint32_t binding) const {
        return GetTypeFromIndex(GetIndexFromBinding(binding));
    }
  ...
  private:
    // Only the first three data members are used for hash and equality checks, the other members are derived from them,
    // and are used to speed up the various lookups/queries/validations
...
using DescriptorSetLayoutDict =
    hash_util::Dictionary<DescriptorSetLayoutDef, hash_util::HasHashMember<DescriptorSetLayoutDef>>;
        ...
        if (l.pImmutableSamplers) {
            for (uint32_t s = 0; s < l.descriptorCount; s++) {
                if (l.pImmutableSamplers[s] != r.pImmutableSamplers[s]) {
                    ...
                    // This just checks pointers, but two different VkSampler handles could be created with same
                    // createInfo. Since this is rare enough, mark as "not the same" and check later when checking for
                    // compatibility.
                    return false;
                }
            }
...
class DescriptorSetLayout : public StateObject {
  public:
    ...
    uint32_t GetDescriptorCountFromIndex(const uint32_t index) const {
        return layout_id_->GetDescriptorCountFromIndex(index);
    }
    uint32_t GetDescriptorCountFromBinding(const uint32_t binding) const {
    ...
    VkDescriptorType GetTypeFromBinding(const uint32_t binding) const {
        return layout_id_->GetTypeFromBinding(binding);
    }
    ...
    bool IsTypeMutable(const VkDescriptorType type, uint32_t binding) const {
        return layout_id_->IsTypeMutable(type, binding);
    }
    const std::vector<VkDescriptorType>& GetMutableTypes(uint32_t binding) const {
        return layout_id_->GetMutableTypes(binding);
    }
    ...
    const IndexRange& GetGlobalIndexRangeFromIndex(uint32_t index) const {
        return layout_id_->GetGlobalIndexRangeFromIndex(index);
    }
...
// This allows the WriteUpdate() and CopyUpdate() operations to be specialized per descriptor type, but all descriptors
// in a set can be accessed via the common Descriptor.
class Descriptor {
  public:
    ...
    Descriptor() {}
    virtual ~Descriptor() {}
    virtual void WriteUpdate(DescriptorSet& set_state,
                             const ValidationStateTracker& dev_data,
                             const VkWriteDescriptorSet&,
                             const uint32_t,
                             bool is_bindless) = 0;
    virtual void CopyUpdate(DescriptorSet& set_state,
                            const ValidationStateTracker& dev_data,
                            const Descriptor&,
                            bool is_bindless,
...
inline bool IsBufferDescriptor(VkDescriptorType type) {
    return ((type == VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER_DYNAMIC) ||
            (type == VK_DESCRIPTOR_TYPE_STORAGE_BUFFER_DYNAMIC) || (type == VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER) ||
...
class SamplerDescriptor : public Descriptor {
  public:
    ...
    void WriteUpdate(DescriptorSet& set_state,
                     const ValidationStateTracker& dev_data,
                     const VkWriteDescriptorSet&,
                     const uint32_t,
                     bool is_bindless) override;
    void CopyUpdate(DescriptorSet& set_state,
                    const ValidationStateTracker& dev_data,
                    const Descriptor&,
                    bool is_bindless,
...
class MutableDescriptor : public Descriptor {
  public:
    ...
    const vvl::AccelerationStructureNV* GetAccelerationStructureStateNV() const { return acc_state_nv_.get(); }
    vvl::AccelerationStructureNV* GetAccelerationStructureStateNV() { return acc_state_nv_.get(); }
    // Returns true if there is a stored KHR acceleration structure and false if there is a stored NV acceleration
    // structure. Asserts that there is only one of the two.
...
void PerformUpdateDescriptorSets(
    ValidationStateTracker&, uint32_t, const VkWriteDescriptorSet*, uint32_t, const VkCopyDescriptorSet*);

class DescriptorBinding {
  public:
    using NodeList = StateObject::NodeList;
    DescriptorBinding(const VkDescriptorSetLayoutBinding& create_info,
                      uint32_t count_,
                      VkDescriptorBindingFlags binding_flags_) :
        binding(create_info.binding),
        type(create_info.descriptorType), descriptor_class(DescriptorTypeToClass(type)),
        stage_flags(create_info.stageFlags), binding_flags(binding_flags_), count(count_),
        has_immutable_samplers(create_info.pImmutableSamplers != nullptr), updated(count_, false) {}
...
class DescriptorBindingImpl : public DescriptorBinding {
  public:
    DescriptorBindingImpl(const VkDescriptorSetLayoutBinding& create_info,
                          uint32_t count_,
                          VkDescriptorBindingFlags binding_flags_) :
    ...
    const Descriptor* GetDescriptor(const uint32_t index) const override {
        return index < count ? &descriptors[index] : nullptr;
    }
...
struct DecodedTemplateUpdate {
    ...
    DecodedTemplateUpdate(const ValidationStateTracker& device_data,
                          VkDescriptorSet descriptorSet,
                          const DescriptorUpdateTemplate* template_state,
                          const void* pData,
...
class DescriptorSet : public StateObject {
  public:
    ...
    // Given that we are providing placement new allocation for bindings, the deleter needs to *only* call the
    // destructor
    ...
    DescriptorSet(const VkDescriptorSet handle,
                  vvl::DescriptorPool*,
                  const std::shared_ptr<DescriptorSetLayout const>&,
                  uint32_t variable_count,
                  StateTracker* state_data);
  ...
  protected:
    ...
    template <typename T>
    std::unique_ptr<T, BindingDeleter> MakeBinding(BindingBackingStore* location,
                                                   const VkDescriptorSetLayoutBinding& create_info,
    ...
    // "Destructors for nonstatic member objects are called in the reverse order in which they appear in the class
    // declaration."
...
//
// This object is created to allow both code paths to share same logic, but still provide error messages that fit for
// each incoming call
