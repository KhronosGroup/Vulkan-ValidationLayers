    ...
    inline bool IsQueueFamilyIndicesEqual(const VkImageCreateInfo& other_create_info) const {
        return (create_info.queueFamilyIndexCount == other_create_info.queueFamilyIndexCount) &&
               (create_info.queueFamilyIndexCount == 0 ||
                memcmp(create_info.pQueueFamilyIndices,
                       other_create_info.pQueueFamilyIndices,
  ...
  protected:
    ...
    bool AnyImageAliasOf(const UnaryPredicate& pred) const {
        ...
        for (auto const& memory_state : GetBoundMemoryStates()) {
            if (AnyAliasBindingOf(memory_state->ObjectBindings(), pred))
                return true;
        }
        return false;
    }
...
class ImageView : public StateObject {
  public:
    ...
    const VkSamplerYcbcrConversion
        samplerConversion; // Handle of the ycbcr sampler conversion the image was created with, if any
...
class Surface : public StateObject {
  public:
    ...
    GetPresentModes(VkPhysicalDevice phys_dev, const Location& loc, const ValidationObject* validation_obj) const;

    void SetFormats(VkPhysicalDevice phys_dev, std::vector<vku::safe_VkSurfaceFormat2KHR>&& fmts);
    vvl::span<const vku::safe_VkSurfaceFormat2KHR> GetFormats(bool get_surface_capabilities2,
    ...
    void UpdateCapabilitiesCache(VkPhysicalDevice phys_dev,
  ...
  private:
    ...
    // TODO: make mutex shared, so multiple Validate can read simultaneously. Remove remaining mutations in Validate
    // first
