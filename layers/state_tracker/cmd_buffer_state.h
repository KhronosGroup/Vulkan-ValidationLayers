...
struct AttachmentInfo {
    ...
    bool IsResolve() const {
        return type == Type::ColorResolve || type == Type::DepthResolve || type == Type::StencilResolve;
    }
...
struct SubpassInfo {
    ...
    SubpassInfo() :
        used(false), usage(VkImageUsageFlagBits(0)), layout(VK_IMAGE_LAYOUT_UNDEFINED),
...
class CommandPool : public StateObject {
  public:
    ...
    CommandPool(ValidationStateTracker& dev,
...
class CommandBuffer : public RefcountedStateObject {
  ...
  public:
    ...
    ImageLayoutUpdateCount
        image_layout_change_count; // The sequence number for changes to image layout (for cached validation)
    ...
    struct DynamicStateStatus {
        ...
        CBDynamicFlags
            history; // for lifetime of CommandBuffer, regardless if invalidated, used for better error messages
    ...
    struct DynamicStateValue {
        ...
        // When the Command Buffer resets, the value most things in this struct don't matter because if they are read
        // without setting the state, it will fail in ValidateDynamicStateIsSet() for us. Some values (ex. the bitset)
        // are tracking in replacement for static_status/dynamic_status so this needs to reset along with those.
        //
    ...
    // If VK_NV_inherited_viewport_scissor is enabled and
    // VkCommandBufferInheritanceViewportScissorInfoNV::viewportScissor2D is true, then is the nonempty list of
    // viewports passed in pViewportDepths. Otherwise, this is empty.
    ...
    //  * v_G be the viewportCount of g_D (0 if g_D disables rasterization or enables
    //  VK_DYNAMIC_STATE_VIEWPORT_WITH_COUNT)
    //  * s_G be the scissorCount  of g_D (0 if g_D disables rasterization or enables
    //  VK_DYNAMIC_STATE_SCISSOR_WITH_COUNT)
    ...
    // Bits set when binding graphics pipeline defining corresponding static state, or executing any secondary command
    // buffer. Bits unset by calling a corresponding vkCmdSet[State] cmd.
    ...
    // There is no concept of "attachment index" with dynamic rendering, we use this for both dynamic/non-dynamic
    // rendering though. The attachments are packed the following: | color | color resolve | depth | depth resolve |
    ...
    std::optional<VkSampleCountFlagBits> GetActiveSubpassRasterizationSampleCount() const {
        return active_subpass_sample_count_;
    }
    ...
    using QueueCallback = std::function<bool(
        const ValidationStateTracker& device_data, const class vvl::Queue& queue_state, const CommandBuffer& cb_state)>;
    ...
    std::vector<
        std::function<bool(const CommandBuffer& secondary, const CommandBuffer* primary, const vvl::Framebuffer*)>>
        cmd_execute_commands_functions;

    using EventCallback = std::function<bool(CommandBuffer& cb_state,
                                             bool do_validate,
    ...
    std::vector<std::function<bool(CommandBuffer& cb_state,
                                   bool do_validate,
    ...
    CommandBuffer(ValidationStateTracker& dev,
    ...
    const QFOTransferBarrierSets<QFOImageTransferBarrier>&
    GetQFOBarrierSets(const QFOImageTransferBarrier& type_tag) const {
        return qfo_transfer_image_barriers;
    }

    const QFOTransferBarrierSets<QFOBufferTransferBarrier>&
    GetQFOBarrierSets(const QFOBufferTransferBarrier& type_tag) const {
    ...
    void GetCurrentPipelineAndDesriptorSets(VkPipelineBindPoint pipelineBindPoint,
                                            const vvl::Pipeline** rtn_pipe,
    ...
    void UpdateLastBoundDescriptorBuffers(VkPipelineBindPoint pipeline_bind_point,
                                          ...
                                          uint32_t first_set,
                                          uint32_t set_count,
                                          const uint32_t* buffer_indicies,
                                          const VkDeviceSize* buffer_offsets);

    void PushDescriptorSetState(VkPipelineBindPoint pipelineBindPoint,
                                const vvl::PipelineLayout& pipeline_layout,
                                vvl::Func bound_command,
                                uint32_t set,
    ...
    virtual void RecordWaitEvents(Func command,
                                  uint32_t eventCount,
                                  const VkEvent* pEvents,
                                  VkPipelineStageFlags2KHR src_stage_mask);
    void
    RecordWriteTimestamp(Func command, VkPipelineStageFlags2KHR pipelineStage, VkQueryPool queryPool, uint32_t slot);

    void RecordBarriers(uint32_t memoryBarrierCount,
    ...
    void SetImageLayout(const vvl::Image& image_state,
    ...
    void
    SetImageInitialLayout(const vvl::Image& image_state, const VkImageSubresourceRange& range, VkImageLayout layout);
    void
    SetImageInitialLayout(const vvl::Image& image_state, const VkImageSubresourceLayers& layers, VkImageLayout layout);
    ...
    uint32_t GetDynamicColorResolveAttachmentImageIndex(uint32_t index) const {
        return index + GetDynamicColorAttachmentCount();
    }
    ...
    static void ReplayLabelCommands(const vvl::span<const LabelCommand>& label_commands,
                                    std::vector<std::string>& label_stack);
    // Computes debug region by replaying given commands on top initial label stack.
    static std::string GetDebugRegionName(const std::vector<LabelCommand>& label_commands,
                                          uint32_t label_command_index,
                                          const std::vector<std::string>& initial_label_stack = {});

  private:
    void ResetCBState();

    // Keep track of how many CmdBeginDebugUtilsLabelEXT calls have been made without a matching
    // CmdEndDebugUtilsLabelEXT. Negative value for a secondary command buffer indicates invalid state. Negative value
    // for a primary command buffer is allowed. Validation is done at submit time accross all command buffers.
    ...
    // Stores rasterization samples count obtained from the first pipeline with a pMultisampleState in the active
    // subpass, or std::nullopt
