...
class DeviceMemory : public StateObject {
  public:
    ...
    const bool
        metal_buffer_export; // Can be used in a VkExportMetalBufferInfoEXT struct in a VkExportMetalObjectsEXT call
#endif                       // VK_USE_PLATFORM_METAL_EXT
    void* p_driver_data;     // Pointer to application's actual memory
    const VkDeviceSize
        fake_base_address; // To allow a unified view of allocations, useful to Synchronization Validation
    ...
    VkImage GetDedicatedImage() const {
        return (dedicated && dedicated->handle.type == kVulkanObjectTypeImage) ? dedicated->handle.Cast<VkImage>()
                                                                               : VK_NULL_HANDLE;
    }
...
class BindableMemoryTracker {
  public:
    ...
    virtual void
    BindMemory(StateObject*, std::shared_ptr<vvl::DeviceMemory>&, VkDeviceSize, VkDeviceSize, VkDeviceSize) = 0;

    virtual BoundMemoryRange GetBoundMemoryRange(const MemoryRange&) const = 0;
    virtual BoundRanges GetBoundRanges(const BufferRange& ranges_bounds,
                                       const std::vector<BufferRange>& ranges) const = 0;
...
class BindableNoMemoryTracker : public BindableMemoryTracker {
  public:
    ...
    void
    BindMemory(StateObject*, std::shared_ptr<vvl::DeviceMemory>&, VkDeviceSize, VkDeviceSize, VkDeviceSize) override {}

    BoundMemoryRange GetBoundMemoryRange(const MemoryRange&) const override { return BoundMemoryRange{}; }
    BoundRanges GetBoundRanges(const BufferRange& ranges_bounds,
                               const std::vector<BufferRange>& ranges) const override {
        return {};
    }
...
class BindableLinearMemoryTracker : public BindableMemoryTracker {
  public:
    ...
    void BindMemory(StateObject* parent,
...
class BindableSparseMemoryTracker : public BindableMemoryTracker {
  public:
    ...
    BoundMemoryRange GetBoundMemoryRange(const MemoryRange& range) const override;
    // With a list of (VALID) buffer ranges as input, and `ranges_bounds` being a range that contains all of those
    // buffer ranges, find what tracked VkDeviceMemory maps to what buffer range. Result is stored in a "VkDeviceMemory
    // -> vector<(memory range, buffer range)>" map. Pairs in vector<(memory range, buffer range)> are sorted in
    // ascending order with respect to memory ranges Note: The stored buffer range in the map are all subranges of the
    // ranges listed in `buffer_ranges`, since an input range could be mapped to multiple VkDeviceMemory
    BoundRanges GetBoundRanges(const BufferRange& ranges_bounds,
                               const std::vector<BufferRange>& buffer_ranges) const override;
...
class Bindable : public StateObject {
  public:
    ...
    Bindable(Handle handle,
             VulkanObjectType type,
             bool is_sparse,
             bool is_unprotected,
             VkExternalMemoryHandleTypeFlags handle_types) :
        StateObject(handle, type),
        external_memory_handle_types(handle_types), sparse(is_sparse), unprotected(is_unprotected),
    ...
    bool IsExternalBuffer() const {
        return ((external_memory_handle_types & VK_EXTERNAL_MEMORY_HANDLE_TYPE_ANDROID_HARDWARE_BUFFER_BIT_ANDROID) !=
                0) ||
    ...
    void BindMemory(StateObject* parent,
                    std::shared_ptr<vvl::DeviceMemory>& mem,
                    const VkDeviceSize memory_offset,
                    const VkDeviceSize resource_offset,
    ...
    std::pair<VkDeviceMemory, BindableMemoryTracker::MemoryRange>
    GetResourceMemoryOverlap(const BindableMemoryTracker::MemoryRange& memory_region,
                             const Bindable* other_resource,
                             const BindableMemoryTracker::MemoryRange& other_memory_region) const;

    bool DoesResourceMemoryOverlap(const BindableMemoryTracker::MemoryRange& memory_region,
                                   const Bindable* other_resource,
                                   const BindableMemoryTracker::MemoryRange& other_memory_region) const {
    ...
    BindableLinearMemoryTracker::BoundRanges
    GetBoundRanges(const BindableMemoryTracker::BufferRange& ranges_bounds,
