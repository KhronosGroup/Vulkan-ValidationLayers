    ...
    for (auto insn : *this) {
        ...
        switch (insn.opcode()) {
            ...
            case spv::OpMemberDecorate: {
                memberDecorate_lists.insert({insn.word(1), insn.offset()});
            } break;
...
struct shader_struct_member {
    uint32_t offset;
    uint32_t size;                                 // scalar size. If the data is a struct, size is 0.
    std::vector<uint32_t> array_length_hierarchy;  // e.g.: multi-dimensional array, mat, vec
    std::vector<bool> used_arrays;
    std::vector<std::pair<bool, shader_struct_member>> used_members;  // If the data is not a struct, it's empty.

    shader_struct_member() : offset(0), size(0) {}

    void SetUsedArrays(const std::vector<uint32_t> &used_array_index) {
        auto size = array_length_hierarchy.size();
        uint32_t block_size = 1;
        for (uint32_t i = used_array_index.size(); i < size; ++i) {
            block_size *= array_length_hierarchy[i];
        }
        used_members[used_array_index[0]][used_array_index[1]];
    }
};
...
void DefineStructMember(const SHADER_MODULE_STATE &src, spirv_inst_iter &member_it, shader_struct_member &data) {
    ...
    data.used_members.resize(struct_it.len() - 2);
    auto range = src.memberDecorate_lists.equal_range(struct_it.word(1));

    uint32_t i = 2;
    for (auto it = range.first; it != range.second; ++it) {
        shader_struct_member data1 = {};
        data1.offset = it->second;
        ...
        while (def_member.opcode() == spv::OpTypeArray) {
            ...
            data1.array_length_hierarchy.emplace_back(def_len.word(3));
            len *= def_len.word(3);  // array length
        ...
        if (def_member.opcode() == spv::OpTypeStruct || def_member.opcode() == spv::OpTypePointer) {
            ...
            data1.size = 0;
            data1.used_arrays.resize(len, false);
            DefineStructMember(src, def_member, data1);
        } else {
            data1.used_members.clear();
            data1.size = 1;
            if (def_member.opcode() == spv::OpTypeMatrix) {
                data1.array_length_hierarchy.emplace_back(def_member.word(3));
                len *= def_member.word(3);  // matrix's columns. matrix's row is vector.
                def_member = src.get_def(def_member.word(2));
            }

            if (def_member.opcode() == spv::OpTypeVector) {
                data1.array_length_hierarchy.emplace_back(def_member.word(3));
                len *= def_member.word(3);  // vector length
                def_member = src.get_def(def_member.word(2));
            }
            data1.used_arrays.resize(len, false);
        ...
        data.used_members.emplace_back(std::pair<bool, shader_struct_member>(false, data1));
        ++i;
    }
}
...
void SetUsedStructMember(const SHADER_MODULE_STATE &src, spirv_inst_iter &member_it, const function_set &func_set,
    ...
    for (auto it = range.first; it != range.second; ++it) {
        ...
        if (access_chain.word(3) == member_it.word(1)) {
            auto *data1 = &data;
            ...
            for (uint32_t i = 4; i < access_chain_len; ++i) {
                ...
                data1->used_members[strcut_member_index].first = true;

                // if the member is a array, vec, mat, the index is behind member id.
                const auto array_length_hierarchy_len = data1->array_length_hierarchy.size();
                for (uint32_t i2 = 0; i2 < array_length_hierarchy_len && (i + 1) < access_chain.len(); ++i2) {
                    ++i;
                    array_index_list.emplace_back(GetConstantIntValue(src, access_chain.word(i)));
                }
                data1->SetUsedArrays(array_index_list);
                data1 = &(data1->used_members[strcut_member_index].second);
            }
        }
    }
    ...
    for (auto it = range.first; it != range.second; ++it) {
        ...
        if (called_function_id.find(function_id) == called_function_id.end()) {
            ...
            if (it2 != src.function_sets.end()) {
                SetUsedStructMember(src, member_it, it2->second, data, called_function_id);
            }
        }
    }
}

shader_struct_member GetUsedMembersInFunctions(const SHADER_MODULE_STATE &src, spirv_inst_iter &member_it,
                                               const function_set &func_set) {
    shader_struct_member used_member;
    used_member.used_arrays.resize(1, false);
    DefineStructMember(src, member_it, used_member);
    std::set<uint32_t> called_function_id;
    SetUsedStructMember(src, member_it, func_set, used_member, called_function_id);
    return used_member;
}

void SetStructMemberPushConstant(const SHADER_MODULE_STATE &src, const function_set &entrypoint_function, spirv_inst_iter &type,
                                 std::vector<uint8_t> &push_constant_used_in_shader, const uint32_t struct_offset,
                                 const uint32_t struct_len) {
    shader_struct_member struct_member = GetUsedMembersInFunctions(src, type, entrypoint_function);
    ...
    for (auto insn : src) {
        if (insn.opcode() == spv::OpMemberDecorate && insn.word(1) == type.word(1)) {
            if (insn.word(3) == spv::DecorationOffset) {
                ...
                if (def_member.opcode() == spv::OpTypeStruct || def_member.opcode() == spv::OpTypePointer) {
                    // If it's OpTypePointer. it means the member is a buffer, the type will be TypePointer, and then struct
                    SetStructMemberPushConstant(src, entrypoint_function, def_member, push_constant_used_in_shader, offset, size);
...
void SetPushConstantUsedInShader(const SHADER_MODULE_STATE &src, const spirv_inst_iter &entrypoint,
    ...
    for (auto id : accessible_ids) {
        ...
        if (def_insn.opcode() == spv::OpVariable && def_insn.word(3) == spv::StorageClassPushConstant) {
            ...
            SetStructMemberPushConstant(src, entrypoint_function->second, type, push_constant_used_in_shader, 0, 1);
        }
    }
}
