...
VkResult DispatchCreateRayTracingPipelinesKHR(VkDevice device, VkDeferredOperationKHR deferredOperation,
    ...
    // Operation may be deffered, dedifinite answer know looking at dispatch VkResult,
...
VkResult DispatchGetDeferredOperationResultKHR(VkDevice device, VkDeferredOperationKHR operation) {
    ...
    if (result == VK_SUCCESS) {
        ...
        auto post_op__completion_fns = layer_data->deferred_operation_post_completion.pop(operation);
        if (post_op__completion_fns != layer_data->deferred_operation_post_completion.end()) {
            for (auto &post_op__completion_fn : post_op__completion_fns->second) {
