...
StatelessDeviceData::StatelessDeviceData(vvl::dispatch::Instance *instance, VkPhysicalDevice physical_device,
    ...
    DispatchEnumerateDeviceExtensionProperties(physical_device, nullptr, &extensionCount, nullptr);
    std::vector<VkExtensionProperties> supportedExtensions(extensionCount);
    DispatchEnumerateDeviceExtensionProperties(physical_device, nullptr, &extensionCount, supportedExtensions.data());
    for (uint32_t i = 0; i < extensionCount; i++) {
        vvl::Extension extension = GetExtension(supportedExtensions[i].extensionName);
...
void Instance::FindSupportedExtensions() {
    ...
    instance_dispatch_table.EnumeratePhysicalDevices(instance, &physicalDeviceCount, nullptr);
    std::vector<VkPhysicalDevice> physicalDevices(physicalDeviceCount);
    instance_dispatch_table.EnumeratePhysicalDevices(instance, &physicalDeviceCount, physicalDevices.data());
    for (const auto physicalDevice : physicalDevices) {
        uint32_t extensionCount = 0u;
        instance_dispatch_table.EnumerateDeviceExtensionProperties(physicalDevice, nullptr, &extensionCount, nullptr);
        std::vector<VkExtensionProperties> supportedExtensions(extensionCount);
        instance_dispatch_table.EnumerateDeviceExtensionProperties(physicalDevice, nullptr, &extensionCount,
