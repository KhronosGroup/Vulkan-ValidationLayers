...
template<>
vvl::span<VkPipelineCacheHeaderVersion> ValidationObject::ValidParamValues() const {
    ...
    static vvl::unordered_map<Key, std::vector<VkPipelineCacheHeaderVersion>> values_map;

    Key k = (Key(CastToUint64(instance)) << handle_size) | Key(CastToUint64(device));
    auto& values = values_map[k];
    if (values.empty()) {
        values.insert(values.begin(), CoreVkPipelineCacheHeaderVersionEnums.cbegin(), CoreVkPipelineCacheHeaderVersionEnums.cend());
...
template<>
vvl::span<VkImageLayout> ValidationObject::ValidParamValues() const {
    // TODO (ncesario) This is not ideal as we compute the enabled extensions every time this function is called.
    //      Ideally "values" would be something like a static variable that is built once and this function returns
    //      a span of the container. This does not work for applications which create and destroy many instances and
    //      devices over the lifespan of the project (e.g., VLT).
    ...
    constexpr int handle_size = sizeof(uint64_t);
    using Key = std::bitset<handle_size * 2>; // Key: instance handle in upper 64 bits, device handle in lower 64 bits
    static vvl::unordered_map<Key, std::vector<VkImageLayout>> values_map;

    Key k = (Key(CastToUint64(instance)) << handle_size) | Key(CastToUint64(device));
    auto& values = values_map[k];
    if (values.empty()) {
        values.insert(values.begin(), CoreVkImageLayoutEnums.cbegin(), CoreVkImageLayoutEnums.cend());
...
template<>
vvl::span<VkObjectType> ValidationObject::ValidParamValues() const {
    // TODO (ncesario) This is not ideal as we compute the enabled extensions every time this function is called.
    //      Ideally "values" would be something like a static variable that is built once and this function returns
    //      a span of the container. This does not work for applications which create and destroy many instances and
    //      devices over the lifespan of the project (e.g., VLT).
    ...
    constexpr int handle_size = sizeof(uint64_t);
    using Key = std::bitset<handle_size * 2>; // Key: instance handle in upper 64 bits, device handle in lower 64 bits
    static vvl::unordered_map<Key, std::vector<VkObjectType>> values_map;

    Key k = (Key(CastToUint64(instance)) << handle_size) | Key(CastToUint64(device));
    auto& values = values_map[k];
    if (values.empty()) {
        values.insert(values.begin(), CoreVkObjectTypeEnums.cbegin(), CoreVkObjectTypeEnums.cend());
...
template<>
vvl::span<VkFormat> ValidationObject::ValidParamValues() const {
    // TODO (ncesario) This is not ideal as we compute the enabled extensions every time this function is called.
    //      Ideally "values" would be something like a static variable that is built once and this function returns
    //      a span of the container. This does not work for applications which create and destroy many instances and
    //      devices over the lifespan of the project (e.g., VLT).
    ...
    constexpr int handle_size = sizeof(uint64_t);
    using Key = std::bitset<handle_size * 2>; // Key: instance handle in upper 64 bits, device handle in lower 64 bits
    static vvl::unordered_map<Key, std::vector<VkFormat>> values_map;

    Key k = (Key(CastToUint64(instance)) << handle_size) | Key(CastToUint64(device));
    auto& values = values_map[k];
    if (values.empty()) {
        values.insert(values.begin(), CoreVkFormatEnums.cbegin(), CoreVkFormatEnums.cend());
...
template<>
vvl::span<VkImageTiling> ValidationObject::ValidParamValues() const {
    // TODO (ncesario) This is not ideal as we compute the enabled extensions every time this function is called.
    //      Ideally "values" would be something like a static variable that is built once and this function returns
    //      a span of the container. This does not work for applications which create and destroy many instances and
    //      devices over the lifespan of the project (e.g., VLT).
    ...
    constexpr int handle_size = sizeof(uint64_t);
    using Key = std::bitset<handle_size * 2>; // Key: instance handle in upper 64 bits, device handle in lower 64 bits
    static vvl::unordered_map<Key, std::vector<VkImageTiling>> values_map;

    Key k = (Key(CastToUint64(instance)) << handle_size) | Key(CastToUint64(device));
    auto& values = values_map[k];
    if (values.empty()) {
        values.insert(values.begin(), CoreVkImageTilingEnums.cbegin(), CoreVkImageTilingEnums.cend());
...
template<>
vvl::span<VkImageType> ValidationObject::ValidParamValues() const {
    // TODO (ncesario) This is not ideal as we compute the enabled extensions every time this function is called.
    //      Ideally "values" would be something like a static variable that is built once and this function returns
    //      a span of the container. This does not work for applications which create and destroy many instances and
    //      devices over the lifespan of the project (e.g., VLT).
    ...
    constexpr int handle_size = sizeof(uint64_t);
    using Key = std::bitset<handle_size * 2>; // Key: instance handle in upper 64 bits, device handle in lower 64 bits
    static vvl::unordered_map<Key, std::vector<VkImageType>> values_map;

    Key k = (Key(CastToUint64(instance)) << handle_size) | Key(CastToUint64(device));
    auto& values = values_map[k];
    if (values.empty()) {
        values.insert(values.begin(), CoreVkImageTypeEnums.cbegin(), CoreVkImageTypeEnums.cend());
...
template<>
vvl::span<VkQueryType> ValidationObject::ValidParamValues() const {
    // TODO (ncesario) This is not ideal as we compute the enabled extensions every time this function is called.
    //      Ideally "values" would be something like a static variable that is built once and this function returns
    //      a span of the container. This does not work for applications which create and destroy many instances and
    //      devices over the lifespan of the project (e.g., VLT).
    ...
    constexpr int handle_size = sizeof(uint64_t);
    using Key = std::bitset<handle_size * 2>; // Key: instance handle in upper 64 bits, device handle in lower 64 bits
    static vvl::unordered_map<Key, std::vector<VkQueryType>> values_map;

    Key k = (Key(CastToUint64(instance)) << handle_size) | Key(CastToUint64(device));
    auto& values = values_map[k];
    if (values.empty()) {
        values.insert(values.begin(), CoreVkQueryTypeEnums.cbegin(), CoreVkQueryTypeEnums.cend());
...
template<>
vvl::span<VkSharingMode> ValidationObject::ValidParamValues() const {
    // TODO (ncesario) This is not ideal as we compute the enabled extensions every time this function is called.
    //      Ideally "values" would be something like a static variable that is built once and this function returns
    //      a span of the container. This does not work for applications which create and destroy many instances and
    //      devices over the lifespan of the project (e.g., VLT).
    ...
    constexpr int handle_size = sizeof(uint64_t);
    using Key = std::bitset<handle_size * 2>; // Key: instance handle in upper 64 bits, device handle in lower 64 bits
    static vvl::unordered_map<Key, std::vector<VkSharingMode>> values_map;

    Key k = (Key(CastToUint64(instance)) << handle_size) | Key(CastToUint64(device));
    auto& values = values_map[k];
    if (values.empty()) {
        values.insert(values.begin(), CoreVkSharingModeEnums.cbegin(), CoreVkSharingModeEnums.cend());
...
template<>
vvl::span<VkComponentSwizzle> ValidationObject::ValidParamValues() const {
    // TODO (ncesario) This is not ideal as we compute the enabled extensions every time this function is called.
    //      Ideally "values" would be something like a static variable that is built once and this function returns
    //      a span of the container. This does not work for applications which create and destroy many instances and
    //      devices over the lifespan of the project (e.g., VLT).
    ...
    constexpr int handle_size = sizeof(uint64_t);
    using Key = std::bitset<handle_size * 2>; // Key: instance handle in upper 64 bits, device handle in lower 64 bits
    static vvl::unordered_map<Key, std::vector<VkComponentSwizzle>> values_map;

    Key k = (Key(CastToUint64(instance)) << handle_size) | Key(CastToUint64(device));
    auto& values = values_map[k];
    if (values.empty()) {
        values.insert(values.begin(), CoreVkComponentSwizzleEnums.cbegin(), CoreVkComponentSwizzleEnums.cend());
...
template<>
vvl::span<VkImageViewType> ValidationObject::ValidParamValues() const {
    // TODO (ncesario) This is not ideal as we compute the enabled extensions every time this function is called.
    //      Ideally "values" would be something like a static variable that is built once and this function returns
    //      a span of the container. This does not work for applications which create and destroy many instances and
    //      devices over the lifespan of the project (e.g., VLT).
    ...
    constexpr int handle_size = sizeof(uint64_t);
    using Key = std::bitset<handle_size * 2>; // Key: instance handle in upper 64 bits, device handle in lower 64 bits
    static vvl::unordered_map<Key, std::vector<VkImageViewType>> values_map;

    Key k = (Key(CastToUint64(instance)) << handle_size) | Key(CastToUint64(device));
    auto& values = values_map[k];
    if (values.empty()) {
        values.insert(values.begin(), CoreVkImageViewTypeEnums.cbegin(), CoreVkImageViewTypeEnums.cend());
...
template<>
vvl::span<VkBlendFactor> ValidationObject::ValidParamValues() const {
    // TODO (ncesario) This is not ideal as we compute the enabled extensions every time this function is called.
    //      Ideally "values" would be something like a static variable that is built once and this function returns
    //      a span of the container. This does not work for applications which create and destroy many instances and
    //      devices over the lifespan of the project (e.g., VLT).
    ...
    constexpr int handle_size = sizeof(uint64_t);
    using Key = std::bitset<handle_size * 2>; // Key: instance handle in upper 64 bits, device handle in lower 64 bits
    static vvl::unordered_map<Key, std::vector<VkBlendFactor>> values_map;

    Key k = (Key(CastToUint64(instance)) << handle_size) | Key(CastToUint64(device));
    auto& values = values_map[k];
    if (values.empty()) {
        values.insert(values.begin(), CoreVkBlendFactorEnums.cbegin(), CoreVkBlendFactorEnums.cend());
...
template<>
vvl::span<VkBlendOp> ValidationObject::ValidParamValues() const {
    // TODO (ncesario) This is not ideal as we compute the enabled extensions every time this function is called.
    //      Ideally "values" would be something like a static variable that is built once and this function returns
    //      a span of the container. This does not work for applications which create and destroy many instances and
    //      devices over the lifespan of the project (e.g., VLT).
    ...
    constexpr int handle_size = sizeof(uint64_t);
    using Key = std::bitset<handle_size * 2>; // Key: instance handle in upper 64 bits, device handle in lower 64 bits
    static vvl::unordered_map<Key, std::vector<VkBlendOp>> values_map;

    Key k = (Key(CastToUint64(instance)) << handle_size) | Key(CastToUint64(device));
    auto& values = values_map[k];
    if (values.empty()) {
        values.insert(values.begin(), CoreVkBlendOpEnums.cbegin(), CoreVkBlendOpEnums.cend());
...
template<>
vvl::span<VkCompareOp> ValidationObject::ValidParamValues() const {
    // TODO (ncesario) This is not ideal as we compute the enabled extensions every time this function is called.
    //      Ideally "values" would be something like a static variable that is built once and this function returns
    //      a span of the container. This does not work for applications which create and destroy many instances and
    //      devices over the lifespan of the project (e.g., VLT).
    ...
    constexpr int handle_size = sizeof(uint64_t);
    using Key = std::bitset<handle_size * 2>; // Key: instance handle in upper 64 bits, device handle in lower 64 bits
    static vvl::unordered_map<Key, std::vector<VkCompareOp>> values_map;

    Key k = (Key(CastToUint64(instance)) << handle_size) | Key(CastToUint64(device));
    auto& values = values_map[k];
    if (values.empty()) {
        values.insert(values.begin(), CoreVkCompareOpEnums.cbegin(), CoreVkCompareOpEnums.cend());
...
template<>
vvl::span<VkDynamicState> ValidationObject::ValidParamValues() const {
    // TODO (ncesario) This is not ideal as we compute the enabled extensions every time this function is called.
    //      Ideally "values" would be something like a static variable that is built once and this function returns
    //      a span of the container. This does not work for applications which create and destroy many instances and
    //      devices over the lifespan of the project (e.g., VLT).
    ...
    constexpr int handle_size = sizeof(uint64_t);
    using Key = std::bitset<handle_size * 2>; // Key: instance handle in upper 64 bits, device handle in lower 64 bits
    static vvl::unordered_map<Key, std::vector<VkDynamicState>> values_map;

    Key k = (Key(CastToUint64(instance)) << handle_size) | Key(CastToUint64(device));
    auto& values = values_map[k];
    if (values.empty()) {
        values.insert(values.begin(), CoreVkDynamicStateEnums.cbegin(), CoreVkDynamicStateEnums.cend());
...
template<>
vvl::span<VkFrontFace> ValidationObject::ValidParamValues() const {
    // TODO (ncesario) This is not ideal as we compute the enabled extensions every time this function is called.
    //      Ideally "values" would be something like a static variable that is built once and this function returns
    //      a span of the container. This does not work for applications which create and destroy many instances and
    //      devices over the lifespan of the project (e.g., VLT).
    ...
    constexpr int handle_size = sizeof(uint64_t);
    using Key = std::bitset<handle_size * 2>; // Key: instance handle in upper 64 bits, device handle in lower 64 bits
    static vvl::unordered_map<Key, std::vector<VkFrontFace>> values_map;

    Key k = (Key(CastToUint64(instance)) << handle_size) | Key(CastToUint64(device));
    auto& values = values_map[k];
    if (values.empty()) {
        values.insert(values.begin(), CoreVkFrontFaceEnums.cbegin(), CoreVkFrontFaceEnums.cend());
...
template<>
vvl::span<VkVertexInputRate> ValidationObject::ValidParamValues() const {
    // TODO (ncesario) This is not ideal as we compute the enabled extensions every time this function is called.
    //      Ideally "values" would be something like a static variable that is built once and this function returns
    //      a span of the container. This does not work for applications which create and destroy many instances and
    //      devices over the lifespan of the project (e.g., VLT).
    ...
    constexpr int handle_size = sizeof(uint64_t);
    using Key = std::bitset<handle_size * 2>; // Key: instance handle in upper 64 bits, device handle in lower 64 bits
    static vvl::unordered_map<Key, std::vector<VkVertexInputRate>> values_map;

    Key k = (Key(CastToUint64(instance)) << handle_size) | Key(CastToUint64(device));
    auto& values = values_map[k];
    if (values.empty()) {
        values.insert(values.begin(), CoreVkVertexInputRateEnums.cbegin(), CoreVkVertexInputRateEnums.cend());
...
template<>
vvl::span<VkPrimitiveTopology> ValidationObject::ValidParamValues() const {
    // TODO (ncesario) This is not ideal as we compute the enabled extensions every time this function is called.
    //      Ideally "values" would be something like a static variable that is built once and this function returns
    //      a span of the container. This does not work for applications which create and destroy many instances and
    //      devices over the lifespan of the project (e.g., VLT).
    ...
    constexpr int handle_size = sizeof(uint64_t);
    using Key = std::bitset<handle_size * 2>; // Key: instance handle in upper 64 bits, device handle in lower 64 bits
    static vvl::unordered_map<Key, std::vector<VkPrimitiveTopology>> values_map;

    Key k = (Key(CastToUint64(instance)) << handle_size) | Key(CastToUint64(device));
    auto& values = values_map[k];
    if (values.empty()) {
        values.insert(values.begin(), CoreVkPrimitiveTopologyEnums.cbegin(), CoreVkPrimitiveTopologyEnums.cend());
...
template<>
vvl::span<VkPolygonMode> ValidationObject::ValidParamValues() const {
    // TODO (ncesario) This is not ideal as we compute the enabled extensions every time this function is called.
    //      Ideally "values" would be something like a static variable that is built once and this function returns
    //      a span of the container. This does not work for applications which create and destroy many instances and
    //      devices over the lifespan of the project (e.g., VLT).
    ...
    constexpr int handle_size = sizeof(uint64_t);
    using Key = std::bitset<handle_size * 2>; // Key: instance handle in upper 64 bits, device handle in lower 64 bits
    static vvl::unordered_map<Key, std::vector<VkPolygonMode>> values_map;

    Key k = (Key(CastToUint64(instance)) << handle_size) | Key(CastToUint64(device));
    auto& values = values_map[k];
    if (values.empty()) {
        values.insert(values.begin(), CoreVkPolygonModeEnums.cbegin(), CoreVkPolygonModeEnums.cend());
...
template<>
vvl::span<VkStencilOp> ValidationObject::ValidParamValues() const {
    // TODO (ncesario) This is not ideal as we compute the enabled extensions every time this function is called.
    //      Ideally "values" would be something like a static variable that is built once and this function returns
    //      a span of the container. This does not work for applications which create and destroy many instances and
    //      devices over the lifespan of the project (e.g., VLT).
    ...
    constexpr int handle_size = sizeof(uint64_t);
    using Key = std::bitset<handle_size * 2>; // Key: instance handle in upper 64 bits, device handle in lower 64 bits
    static vvl::unordered_map<Key, std::vector<VkStencilOp>> values_map;

    Key k = (Key(CastToUint64(instance)) << handle_size) | Key(CastToUint64(device));
    auto& values = values_map[k];
    if (values.empty()) {
        values.insert(values.begin(), CoreVkStencilOpEnums.cbegin(), CoreVkStencilOpEnums.cend());
...
template<>
vvl::span<VkLogicOp> ValidationObject::ValidParamValues() const {
    // TODO (ncesario) This is not ideal as we compute the enabled extensions every time this function is called.
    //      Ideally "values" would be something like a static variable that is built once and this function returns
    //      a span of the container. This does not work for applications which create and destroy many instances and
    //      devices over the lifespan of the project (e.g., VLT).
    ...
    constexpr int handle_size = sizeof(uint64_t);
    using Key = std::bitset<handle_size * 2>; // Key: instance handle in upper 64 bits, device handle in lower 64 bits
    static vvl::unordered_map<Key, std::vector<VkLogicOp>> values_map;

    Key k = (Key(CastToUint64(instance)) << handle_size) | Key(CastToUint64(device));
    auto& values = values_map[k];
    if (values.empty()) {
        values.insert(values.begin(), CoreVkLogicOpEnums.cbegin(), CoreVkLogicOpEnums.cend());
...
template<>
vvl::span<VkBorderColor> ValidationObject::ValidParamValues() const {
    // TODO (ncesario) This is not ideal as we compute the enabled extensions every time this function is called.
    //      Ideally "values" would be something like a static variable that is built once and this function returns
    //      a span of the container. This does not work for applications which create and destroy many instances and
    //      devices over the lifespan of the project (e.g., VLT).
    ...
    constexpr int handle_size = sizeof(uint64_t);
    using Key = std::bitset<handle_size * 2>; // Key: instance handle in upper 64 bits, device handle in lower 64 bits
    static vvl::unordered_map<Key, std::vector<VkBorderColor>> values_map;

    Key k = (Key(CastToUint64(instance)) << handle_size) | Key(CastToUint64(device));
    auto& values = values_map[k];
    if (values.empty()) {
        values.insert(values.begin(), CoreVkBorderColorEnums.cbegin(), CoreVkBorderColorEnums.cend());
...
template<>
vvl::span<VkFilter> ValidationObject::ValidParamValues() const {
    // TODO (ncesario) This is not ideal as we compute the enabled extensions every time this function is called.
    //      Ideally "values" would be something like a static variable that is built once and this function returns
    //      a span of the container. This does not work for applications which create and destroy many instances and
    //      devices over the lifespan of the project (e.g., VLT).
    ...
    constexpr int handle_size = sizeof(uint64_t);
    using Key = std::bitset<handle_size * 2>; // Key: instance handle in upper 64 bits, device handle in lower 64 bits
    static vvl::unordered_map<Key, std::vector<VkFilter>> values_map;

    Key k = (Key(CastToUint64(instance)) << handle_size) | Key(CastToUint64(device));
    auto& values = values_map[k];
    if (values.empty()) {
        values.insert(values.begin(), CoreVkFilterEnums.cbegin(), CoreVkFilterEnums.cend());
...
template<>
vvl::span<VkSamplerAddressMode> ValidationObject::ValidParamValues() const {
    // TODO (ncesario) This is not ideal as we compute the enabled extensions every time this function is called.
    //      Ideally "values" would be something like a static variable that is built once and this function returns
    //      a span of the container. This does not work for applications which create and destroy many instances and
    //      devices over the lifespan of the project (e.g., VLT).
    ...
    constexpr int handle_size = sizeof(uint64_t);
    using Key = std::bitset<handle_size * 2>; // Key: instance handle in upper 64 bits, device handle in lower 64 bits
    static vvl::unordered_map<Key, std::vector<VkSamplerAddressMode>> values_map;

    Key k = (Key(CastToUint64(instance)) << handle_size) | Key(CastToUint64(device));
    auto& values = values_map[k];
    if (values.empty()) {
        values.insert(values.begin(), CoreVkSamplerAddressModeEnums.cbegin(), CoreVkSamplerAddressModeEnums.cend());
...
template<>
vvl::span<VkSamplerMipmapMode> ValidationObject::ValidParamValues() const {
    // TODO (ncesario) This is not ideal as we compute the enabled extensions every time this function is called.
    //      Ideally "values" would be something like a static variable that is built once and this function returns
    //      a span of the container. This does not work for applications which create and destroy many instances and
    //      devices over the lifespan of the project (e.g., VLT).
    ...
    constexpr int handle_size = sizeof(uint64_t);
    using Key = std::bitset<handle_size * 2>; // Key: instance handle in upper 64 bits, device handle in lower 64 bits
    static vvl::unordered_map<Key, std::vector<VkSamplerMipmapMode>> values_map;

    Key k = (Key(CastToUint64(instance)) << handle_size) | Key(CastToUint64(device));
    auto& values = values_map[k];
    if (values.empty()) {
        values.insert(values.begin(), CoreVkSamplerMipmapModeEnums.cbegin(), CoreVkSamplerMipmapModeEnums.cend());
...
template<>
vvl::span<VkDescriptorType> ValidationObject::ValidParamValues() const {
    // TODO (ncesario) This is not ideal as we compute the enabled extensions every time this function is called.
    //      Ideally "values" would be something like a static variable that is built once and this function returns
    //      a span of the container. This does not work for applications which create and destroy many instances and
    //      devices over the lifespan of the project (e.g., VLT).
    ...
    constexpr int handle_size = sizeof(uint64_t);
    using Key = std::bitset<handle_size * 2>; // Key: instance handle in upper 64 bits, device handle in lower 64 bits
    static vvl::unordered_map<Key, std::vector<VkDescriptorType>> values_map;

    Key k = (Key(CastToUint64(instance)) << handle_size) | Key(CastToUint64(device));
    auto& values = values_map[k];
    if (values.empty()) {
        values.insert(values.begin(), CoreVkDescriptorTypeEnums.cbegin(), CoreVkDescriptorTypeEnums.cend());
...
template<>
vvl::span<VkAttachmentLoadOp> ValidationObject::ValidParamValues() const {
    // TODO (ncesario) This is not ideal as we compute the enabled extensions every time this function is called.
    //      Ideally "values" would be something like a static variable that is built once and this function returns
    //      a span of the container. This does not work for applications which create and destroy many instances and
    //      devices over the lifespan of the project (e.g., VLT).
    ...
    constexpr int handle_size = sizeof(uint64_t);
    using Key = std::bitset<handle_size * 2>; // Key: instance handle in upper 64 bits, device handle in lower 64 bits
    static vvl::unordered_map<Key, std::vector<VkAttachmentLoadOp>> values_map;

    Key k = (Key(CastToUint64(instance)) << handle_size) | Key(CastToUint64(device));
    auto& values = values_map[k];
    if (values.empty()) {
        values.insert(values.begin(), CoreVkAttachmentLoadOpEnums.cbegin(), CoreVkAttachmentLoadOpEnums.cend());
...
template<>
vvl::span<VkAttachmentStoreOp> ValidationObject::ValidParamValues() const {
    // TODO (ncesario) This is not ideal as we compute the enabled extensions every time this function is called.
    //      Ideally "values" would be something like a static variable that is built once and this function returns
    //      a span of the container. This does not work for applications which create and destroy many instances and
    //      devices over the lifespan of the project (e.g., VLT).
    ...
    constexpr int handle_size = sizeof(uint64_t);
    using Key = std::bitset<handle_size * 2>; // Key: instance handle in upper 64 bits, device handle in lower 64 bits
    static vvl::unordered_map<Key, std::vector<VkAttachmentStoreOp>> values_map;

    Key k = (Key(CastToUint64(instance)) << handle_size) | Key(CastToUint64(device));
    auto& values = values_map[k];
    if (values.empty()) {
        values.insert(values.begin(), CoreVkAttachmentStoreOpEnums.cbegin(), CoreVkAttachmentStoreOpEnums.cend());
...
template<>
vvl::span<VkPipelineBindPoint> ValidationObject::ValidParamValues() const {
    // TODO (ncesario) This is not ideal as we compute the enabled extensions every time this function is called.
    //      Ideally "values" would be something like a static variable that is built once and this function returns
    //      a span of the container. This does not work for applications which create and destroy many instances and
    //      devices over the lifespan of the project (e.g., VLT).
    ...
    constexpr int handle_size = sizeof(uint64_t);
    using Key = std::bitset<handle_size * 2>; // Key: instance handle in upper 64 bits, device handle in lower 64 bits
    static vvl::unordered_map<Key, std::vector<VkPipelineBindPoint>> values_map;

    Key k = (Key(CastToUint64(instance)) << handle_size) | Key(CastToUint64(device));
    auto& values = values_map[k];
    if (values.empty()) {
        values.insert(values.begin(), CoreVkPipelineBindPointEnums.cbegin(), CoreVkPipelineBindPointEnums.cend());
...
template<>
vvl::span<VkCommandBufferLevel> ValidationObject::ValidParamValues() const {
    // TODO (ncesario) This is not ideal as we compute the enabled extensions every time this function is called.
    //      Ideally "values" would be something like a static variable that is built once and this function returns
    //      a span of the container. This does not work for applications which create and destroy many instances and
    //      devices over the lifespan of the project (e.g., VLT).
    ...
    constexpr int handle_size = sizeof(uint64_t);
    using Key = std::bitset<handle_size * 2>; // Key: instance handle in upper 64 bits, device handle in lower 64 bits
    static vvl::unordered_map<Key, std::vector<VkCommandBufferLevel>> values_map;

    Key k = (Key(CastToUint64(instance)) << handle_size) | Key(CastToUint64(device));
    auto& values = values_map[k];
    if (values.empty()) {
        values.insert(values.begin(), CoreVkCommandBufferLevelEnums.cbegin(), CoreVkCommandBufferLevelEnums.cend());
...
template<>
vvl::span<VkIndexType> ValidationObject::ValidParamValues() const {
    // TODO (ncesario) This is not ideal as we compute the enabled extensions every time this function is called.
    //      Ideally "values" would be something like a static variable that is built once and this function returns
    //      a span of the container. This does not work for applications which create and destroy many instances and
    //      devices over the lifespan of the project (e.g., VLT).
    ...
    constexpr int handle_size = sizeof(uint64_t);
    using Key = std::bitset<handle_size * 2>; // Key: instance handle in upper 64 bits, device handle in lower 64 bits
    static vvl::unordered_map<Key, std::vector<VkIndexType>> values_map;

    Key k = (Key(CastToUint64(instance)) << handle_size) | Key(CastToUint64(device));
    auto& values = values_map[k];
    if (values.empty()) {
        values.insert(values.begin(), CoreVkIndexTypeEnums.cbegin(), CoreVkIndexTypeEnums.cend());
...
template<>
vvl::span<VkSubpassContents> ValidationObject::ValidParamValues() const {
    // TODO (ncesario) This is not ideal as we compute the enabled extensions every time this function is called.
    //      Ideally "values" would be something like a static variable that is built once and this function returns
    //      a span of the container. This does not work for applications which create and destroy many instances and
    //      devices over the lifespan of the project (e.g., VLT).
    ...
    constexpr int handle_size = sizeof(uint64_t);
    using Key = std::bitset<handle_size * 2>; // Key: instance handle in upper 64 bits, device handle in lower 64 bits
    static vvl::unordered_map<Key, std::vector<VkSubpassContents>> values_map;

    Key k = (Key(CastToUint64(instance)) << handle_size) | Key(CastToUint64(device));
    auto& values = values_map[k];
    if (values.empty()) {
        values.insert(values.begin(), CoreVkSubpassContentsEnums.cbegin(), CoreVkSubpassContentsEnums.cend());
...
template<>
vvl::span<VkTessellationDomainOrigin> ValidationObject::ValidParamValues() const {
    // TODO (ncesario) This is not ideal as we compute the enabled extensions every time this function is called.
    //      Ideally "values" would be something like a static variable that is built once and this function returns
    //      a span of the container. This does not work for applications which create and destroy many instances and
    //      devices over the lifespan of the project (e.g., VLT).
    ...
    constexpr int handle_size = sizeof(uint64_t);
    using Key = std::bitset<handle_size * 2>; // Key: instance handle in upper 64 bits, device handle in lower 64 bits
    static vvl::unordered_map<Key, std::vector<VkTessellationDomainOrigin>> values_map;

    Key k = (Key(CastToUint64(instance)) << handle_size) | Key(CastToUint64(device));
    auto& values = values_map[k];
    if (values.empty()) {
        values.insert(values.begin(), CoreVkTessellationDomainOriginEnums.cbegin(), CoreVkTessellationDomainOriginEnums.cend());
...
template<>
vvl::span<VkSamplerYcbcrModelConversion> ValidationObject::ValidParamValues() const {
    // TODO (ncesario) This is not ideal as we compute the enabled extensions every time this function is called.
    //      Ideally "values" would be something like a static variable that is built once and this function returns
    //      a span of the container. This does not work for applications which create and destroy many instances and
    //      devices over the lifespan of the project (e.g., VLT).
    ...
    constexpr int handle_size = sizeof(uint64_t);
    using Key = std::bitset<handle_size * 2>; // Key: instance handle in upper 64 bits, device handle in lower 64 bits
    static vvl::unordered_map<Key, std::vector<VkSamplerYcbcrModelConversion>> values_map;

    Key k = (Key(CastToUint64(instance)) << handle_size) | Key(CastToUint64(device));
    auto& values = values_map[k];
    if (values.empty()) {
        values.insert(values.begin(), CoreVkSamplerYcbcrModelConversionEnums.cbegin(), CoreVkSamplerYcbcrModelConversionEnums.cend());
...
template<>
vvl::span<VkSamplerYcbcrRange> ValidationObject::ValidParamValues() const {
    // TODO (ncesario) This is not ideal as we compute the enabled extensions every time this function is called.
    //      Ideally "values" would be something like a static variable that is built once and this function returns
    //      a span of the container. This does not work for applications which create and destroy many instances and
    //      devices over the lifespan of the project (e.g., VLT).
    ...
    constexpr int handle_size = sizeof(uint64_t);
    using Key = std::bitset<handle_size * 2>; // Key: instance handle in upper 64 bits, device handle in lower 64 bits
    static vvl::unordered_map<Key, std::vector<VkSamplerYcbcrRange>> values_map;

    Key k = (Key(CastToUint64(instance)) << handle_size) | Key(CastToUint64(device));
    auto& values = values_map[k];
    if (values.empty()) {
        values.insert(values.begin(), CoreVkSamplerYcbcrRangeEnums.cbegin(), CoreVkSamplerYcbcrRangeEnums.cend());
...
template<>
vvl::span<VkChromaLocation> ValidationObject::ValidParamValues() const {
    // TODO (ncesario) This is not ideal as we compute the enabled extensions every time this function is called.
    //      Ideally "values" would be something like a static variable that is built once and this function returns
    //      a span of the container. This does not work for applications which create and destroy many instances and
    //      devices over the lifespan of the project (e.g., VLT).
    ...
    constexpr int handle_size = sizeof(uint64_t);
    using Key = std::bitset<handle_size * 2>; // Key: instance handle in upper 64 bits, device handle in lower 64 bits
    static vvl::unordered_map<Key, std::vector<VkChromaLocation>> values_map;

    Key k = (Key(CastToUint64(instance)) << handle_size) | Key(CastToUint64(device));
    auto& values = values_map[k];
    if (values.empty()) {
        values.insert(values.begin(), CoreVkChromaLocationEnums.cbegin(), CoreVkChromaLocationEnums.cend());
...
template<>
vvl::span<VkDescriptorUpdateTemplateType> ValidationObject::ValidParamValues() const {
    // TODO (ncesario) This is not ideal as we compute the enabled extensions every time this function is called.
    //      Ideally "values" would be something like a static variable that is built once and this function returns
    //      a span of the container. This does not work for applications which create and destroy many instances and
    //      devices over the lifespan of the project (e.g., VLT).
    ...
    constexpr int handle_size = sizeof(uint64_t);
    using Key = std::bitset<handle_size * 2>; // Key: instance handle in upper 64 bits, device handle in lower 64 bits
    static vvl::unordered_map<Key, std::vector<VkDescriptorUpdateTemplateType>> values_map;

    Key k = (Key(CastToUint64(instance)) << handle_size) | Key(CastToUint64(device));
    auto& values = values_map[k];
    if (values.empty()) {
        values.insert(values.begin(), CoreVkDescriptorUpdateTemplateTypeEnums.cbegin(), CoreVkDescriptorUpdateTemplateTypeEnums.cend());
...
template<>
vvl::span<VkSamplerReductionMode> ValidationObject::ValidParamValues() const {
    // TODO (ncesario) This is not ideal as we compute the enabled extensions every time this function is called.
    //      Ideally "values" would be something like a static variable that is built once and this function returns
    //      a span of the container. This does not work for applications which create and destroy many instances and
    //      devices over the lifespan of the project (e.g., VLT).
    ...
    constexpr int handle_size = sizeof(uint64_t);
    using Key = std::bitset<handle_size * 2>; // Key: instance handle in upper 64 bits, device handle in lower 64 bits
    static vvl::unordered_map<Key, std::vector<VkSamplerReductionMode>> values_map;

    Key k = (Key(CastToUint64(instance)) << handle_size) | Key(CastToUint64(device));
    auto& values = values_map[k];
    if (values.empty()) {
        values.insert(values.begin(), CoreVkSamplerReductionModeEnums.cbegin(), CoreVkSamplerReductionModeEnums.cend());
...
template<>
vvl::span<VkSemaphoreType> ValidationObject::ValidParamValues() const {
    // TODO (ncesario) This is not ideal as we compute the enabled extensions every time this function is called.
    //      Ideally "values" would be something like a static variable that is built once and this function returns
    //      a span of the container. This does not work for applications which create and destroy many instances and
    //      devices over the lifespan of the project (e.g., VLT).
    ...
    constexpr int handle_size = sizeof(uint64_t);
    using Key = std::bitset<handle_size * 2>; // Key: instance handle in upper 64 bits, device handle in lower 64 bits
    static vvl::unordered_map<Key, std::vector<VkSemaphoreType>> values_map;

    Key k = (Key(CastToUint64(instance)) << handle_size) | Key(CastToUint64(device));
    auto& values = values_map[k];
    if (values.empty()) {
        values.insert(values.begin(), CoreVkSemaphoreTypeEnums.cbegin(), CoreVkSemaphoreTypeEnums.cend());
...
template<>
vvl::span<VkPresentModeKHR> ValidationObject::ValidParamValues() const {
    // TODO (ncesario) This is not ideal as we compute the enabled extensions every time this function is called.
    //      Ideally "values" would be something like a static variable that is built once and this function returns
    //      a span of the container. This does not work for applications which create and destroy many instances and
    //      devices over the lifespan of the project (e.g., VLT).
    ...
    constexpr int handle_size = sizeof(uint64_t);
    using Key = std::bitset<handle_size * 2>; // Key: instance handle in upper 64 bits, device handle in lower 64 bits
    static vvl::unordered_map<Key, std::vector<VkPresentModeKHR>> values_map;

    Key k = (Key(CastToUint64(instance)) << handle_size) | Key(CastToUint64(device));
    auto& values = values_map[k];
    if (values.empty()) {
        values.insert(values.begin(), CoreVkPresentModeKHREnums.cbegin(), CoreVkPresentModeKHREnums.cend());
...
template<>
vvl::span<VkColorSpaceKHR> ValidationObject::ValidParamValues() const {
    // TODO (ncesario) This is not ideal as we compute the enabled extensions every time this function is called.
    //      Ideally "values" would be something like a static variable that is built once and this function returns
    //      a span of the container. This does not work for applications which create and destroy many instances and
    //      devices over the lifespan of the project (e.g., VLT).
    ...
    constexpr int handle_size = sizeof(uint64_t);
    using Key = std::bitset<handle_size * 2>; // Key: instance handle in upper 64 bits, device handle in lower 64 bits
    static vvl::unordered_map<Key, std::vector<VkColorSpaceKHR>> values_map;

    Key k = (Key(CastToUint64(instance)) << handle_size) | Key(CastToUint64(device));
    auto& values = values_map[k];
    if (values.empty()) {
        values.insert(values.begin(), CoreVkColorSpaceKHREnums.cbegin(), CoreVkColorSpaceKHREnums.cend());
...
template<>
vvl::span<VkQueueGlobalPriorityKHR> ValidationObject::ValidParamValues() const {
    // TODO (ncesario) This is not ideal as we compute the enabled extensions every time this function is called.
    //      Ideally "values" would be something like a static variable that is built once and this function returns
    //      a span of the container. This does not work for applications which create and destroy many instances and
    //      devices over the lifespan of the project (e.g., VLT).
    ...
    constexpr int handle_size = sizeof(uint64_t);
    using Key = std::bitset<handle_size * 2>; // Key: instance handle in upper 64 bits, device handle in lower 64 bits
    static vvl::unordered_map<Key, std::vector<VkQueueGlobalPriorityKHR>> values_map;

    Key k = (Key(CastToUint64(instance)) << handle_size) | Key(CastToUint64(device));
    auto& values = values_map[k];
    if (values.empty()) {
        values.insert(values.begin(), CoreVkQueueGlobalPriorityKHREnums.cbegin(), CoreVkQueueGlobalPriorityKHREnums.cend());
...
template<>
vvl::span<VkFragmentShadingRateCombinerOpKHR> ValidationObject::ValidParamValues() const {
    // TODO (ncesario) This is not ideal as we compute the enabled extensions every time this function is called.
    //      Ideally "values" would be something like a static variable that is built once and this function returns
    //      a span of the container. This does not work for applications which create and destroy many instances and
    //      devices over the lifespan of the project (e.g., VLT).
    ...
    constexpr int handle_size = sizeof(uint64_t);
    using Key = std::bitset<handle_size * 2>; // Key: instance handle in upper 64 bits, device handle in lower 64 bits
    static vvl::unordered_map<Key, std::vector<VkFragmentShadingRateCombinerOpKHR>> values_map;

    Key k = (Key(CastToUint64(instance)) << handle_size) | Key(CastToUint64(device));
    auto& values = values_map[k];
    if (values.empty()) {
        values.insert(values.begin(), CoreVkFragmentShadingRateCombinerOpKHREnums.cbegin(), CoreVkFragmentShadingRateCombinerOpKHREnums.cend());
...
template<>
vvl::span<VkVideoEncodeTuningModeKHR> ValidationObject::ValidParamValues() const {
    // TODO (ncesario) This is not ideal as we compute the enabled extensions every time this function is called.
    //      Ideally "values" would be something like a static variable that is built once and this function returns
    //      a span of the container. This does not work for applications which create and destroy many instances and
    //      devices over the lifespan of the project (e.g., VLT).
    ...
    constexpr int handle_size = sizeof(uint64_t);
    using Key = std::bitset<handle_size * 2>; // Key: instance handle in upper 64 bits, device handle in lower 64 bits
    static vvl::unordered_map<Key, std::vector<VkVideoEncodeTuningModeKHR>> values_map;

    Key k = (Key(CastToUint64(instance)) << handle_size) | Key(CastToUint64(device));
    auto& values = values_map[k];
    if (values.empty()) {
        values.insert(values.begin(), CoreVkVideoEncodeTuningModeKHREnums.cbegin(), CoreVkVideoEncodeTuningModeKHREnums.cend());
...
template<>
vvl::span<VkDebugReportObjectTypeEXT> ValidationObject::ValidParamValues() const {
    // TODO (ncesario) This is not ideal as we compute the enabled extensions every time this function is called.
    //      Ideally "values" would be something like a static variable that is built once and this function returns
    //      a span of the container. This does not work for applications which create and destroy many instances and
    //      devices over the lifespan of the project (e.g., VLT).
    ...
    constexpr int handle_size = sizeof(uint64_t);
    using Key = std::bitset<handle_size * 2>; // Key: instance handle in upper 64 bits, device handle in lower 64 bits
    static vvl::unordered_map<Key, std::vector<VkDebugReportObjectTypeEXT>> values_map;

    Key k = (Key(CastToUint64(instance)) << handle_size) | Key(CastToUint64(device));
    auto& values = values_map[k];
    if (values.empty()) {
        values.insert(values.begin(), CoreVkDebugReportObjectTypeEXTEnums.cbegin(), CoreVkDebugReportObjectTypeEXTEnums.cend());
...
template<>
vvl::span<VkRasterizationOrderAMD> ValidationObject::ValidParamValues() const {
    // TODO (ncesario) This is not ideal as we compute the enabled extensions every time this function is called.
    //      Ideally "values" would be something like a static variable that is built once and this function returns
    //      a span of the container. This does not work for applications which create and destroy many instances and
    //      devices over the lifespan of the project (e.g., VLT).
    ...
    constexpr int handle_size = sizeof(uint64_t);
    using Key = std::bitset<handle_size * 2>; // Key: instance handle in upper 64 bits, device handle in lower 64 bits
    static vvl::unordered_map<Key, std::vector<VkRasterizationOrderAMD>> values_map;

    Key k = (Key(CastToUint64(instance)) << handle_size) | Key(CastToUint64(device));
    auto& values = values_map[k];
    if (values.empty()) {
        values.insert(values.begin(), CoreVkRasterizationOrderAMDEnums.cbegin(), CoreVkRasterizationOrderAMDEnums.cend());
...
template<>
vvl::span<VkVideoEncodeH264RateControlStructureEXT> ValidationObject::ValidParamValues() const {
    // TODO (ncesario) This is not ideal as we compute the enabled extensions every time this function is called.
    //      Ideally "values" would be something like a static variable that is built once and this function returns
    //      a span of the container. This does not work for applications which create and destroy many instances and
    //      devices over the lifespan of the project (e.g., VLT).
    ...
    constexpr int handle_size = sizeof(uint64_t);
    using Key = std::bitset<handle_size * 2>; // Key: instance handle in upper 64 bits, device handle in lower 64 bits
    static vvl::unordered_map<Key, std::vector<VkVideoEncodeH264RateControlStructureEXT>> values_map;

    Key k = (Key(CastToUint64(instance)) << handle_size) | Key(CastToUint64(device));
    auto& values = values_map[k];
    if (values.empty()) {
        values.insert(values.begin(), CoreVkVideoEncodeH264RateControlStructureEXTEnums.cbegin(), CoreVkVideoEncodeH264RateControlStructureEXTEnums.cend());
...
template<>
vvl::span<VkVideoEncodeH265RateControlStructureEXT> ValidationObject::ValidParamValues() const {
    // TODO (ncesario) This is not ideal as we compute the enabled extensions every time this function is called.
    //      Ideally "values" would be something like a static variable that is built once and this function returns
    //      a span of the container. This does not work for applications which create and destroy many instances and
    //      devices over the lifespan of the project (e.g., VLT).
    ...
    constexpr int handle_size = sizeof(uint64_t);
    using Key = std::bitset<handle_size * 2>; // Key: instance handle in upper 64 bits, device handle in lower 64 bits
    static vvl::unordered_map<Key, std::vector<VkVideoEncodeH265RateControlStructureEXT>> values_map;

    Key k = (Key(CastToUint64(instance)) << handle_size) | Key(CastToUint64(device));
    auto& values = values_map[k];
    if (values.empty()) {
        values.insert(values.begin(), CoreVkVideoEncodeH265RateControlStructureEXTEnums.cbegin(), CoreVkVideoEncodeH265RateControlStructureEXTEnums.cend());
...
template<>
vvl::span<VkShaderInfoTypeAMD> ValidationObject::ValidParamValues() const {
    // TODO (ncesario) This is not ideal as we compute the enabled extensions every time this function is called.
    //      Ideally "values" would be something like a static variable that is built once and this function returns
    //      a span of the container. This does not work for applications which create and destroy many instances and
    //      devices over the lifespan of the project (e.g., VLT).
    ...
    constexpr int handle_size = sizeof(uint64_t);
    using Key = std::bitset<handle_size * 2>; // Key: instance handle in upper 64 bits, device handle in lower 64 bits
    static vvl::unordered_map<Key, std::vector<VkShaderInfoTypeAMD>> values_map;

    Key k = (Key(CastToUint64(instance)) << handle_size) | Key(CastToUint64(device));
    auto& values = values_map[k];
    if (values.empty()) {
        values.insert(values.begin(), CoreVkShaderInfoTypeAMDEnums.cbegin(), CoreVkShaderInfoTypeAMDEnums.cend());
...
template<>
vvl::span<VkValidationCheckEXT> ValidationObject::ValidParamValues() const {
    // TODO (ncesario) This is not ideal as we compute the enabled extensions every time this function is called.
    //      Ideally "values" would be something like a static variable that is built once and this function returns
    //      a span of the container. This does not work for applications which create and destroy many instances and
    //      devices over the lifespan of the project (e.g., VLT).
    ...
    constexpr int handle_size = sizeof(uint64_t);
    using Key = std::bitset<handle_size * 2>; // Key: instance handle in upper 64 bits, device handle in lower 64 bits
    static vvl::unordered_map<Key, std::vector<VkValidationCheckEXT>> values_map;

    Key k = (Key(CastToUint64(instance)) << handle_size) | Key(CastToUint64(device));
    auto& values = values_map[k];
    if (values.empty()) {
        values.insert(values.begin(), CoreVkValidationCheckEXTEnums.cbegin(), CoreVkValidationCheckEXTEnums.cend());
...
template<>
vvl::span<VkPipelineRobustnessBufferBehaviorEXT> ValidationObject::ValidParamValues() const {
    // TODO (ncesario) This is not ideal as we compute the enabled extensions every time this function is called.
    //      Ideally "values" would be something like a static variable that is built once and this function returns
    //      a span of the container. This does not work for applications which create and destroy many instances and
    //      devices over the lifespan of the project (e.g., VLT).
    ...
    constexpr int handle_size = sizeof(uint64_t);
    using Key = std::bitset<handle_size * 2>; // Key: instance handle in upper 64 bits, device handle in lower 64 bits
    static vvl::unordered_map<Key, std::vector<VkPipelineRobustnessBufferBehaviorEXT>> values_map;

    Key k = (Key(CastToUint64(instance)) << handle_size) | Key(CastToUint64(device));
    auto& values = values_map[k];
    if (values.empty()) {
        values.insert(values.begin(), CoreVkPipelineRobustnessBufferBehaviorEXTEnums.cbegin(), CoreVkPipelineRobustnessBufferBehaviorEXTEnums.cend());
...
template<>
vvl::span<VkPipelineRobustnessImageBehaviorEXT> ValidationObject::ValidParamValues() const {
    // TODO (ncesario) This is not ideal as we compute the enabled extensions every time this function is called.
    //      Ideally "values" would be something like a static variable that is built once and this function returns
    //      a span of the container. This does not work for applications which create and destroy many instances and
    //      devices over the lifespan of the project (e.g., VLT).
    ...
    constexpr int handle_size = sizeof(uint64_t);
    using Key = std::bitset<handle_size * 2>; // Key: instance handle in upper 64 bits, device handle in lower 64 bits
    static vvl::unordered_map<Key, std::vector<VkPipelineRobustnessImageBehaviorEXT>> values_map;

    Key k = (Key(CastToUint64(instance)) << handle_size) | Key(CastToUint64(device));
    auto& values = values_map[k];
    if (values.empty()) {
        values.insert(values.begin(), CoreVkPipelineRobustnessImageBehaviorEXTEnums.cbegin(), CoreVkPipelineRobustnessImageBehaviorEXTEnums.cend());
...
template<>
vvl::span<VkDisplayPowerStateEXT> ValidationObject::ValidParamValues() const {
    // TODO (ncesario) This is not ideal as we compute the enabled extensions every time this function is called.
    //      Ideally "values" would be something like a static variable that is built once and this function returns
    //      a span of the container. This does not work for applications which create and destroy many instances and
    //      devices over the lifespan of the project (e.g., VLT).
    ...
    constexpr int handle_size = sizeof(uint64_t);
    using Key = std::bitset<handle_size * 2>; // Key: instance handle in upper 64 bits, device handle in lower 64 bits
    static vvl::unordered_map<Key, std::vector<VkDisplayPowerStateEXT>> values_map;

    Key k = (Key(CastToUint64(instance)) << handle_size) | Key(CastToUint64(device));
    auto& values = values_map[k];
    if (values.empty()) {
        values.insert(values.begin(), CoreVkDisplayPowerStateEXTEnums.cbegin(), CoreVkDisplayPowerStateEXTEnums.cend());
...
template<>
vvl::span<VkDeviceEventTypeEXT> ValidationObject::ValidParamValues() const {
    // TODO (ncesario) This is not ideal as we compute the enabled extensions every time this function is called.
    //      Ideally "values" would be something like a static variable that is built once and this function returns
    //      a span of the container. This does not work for applications which create and destroy many instances and
    //      devices over the lifespan of the project (e.g., VLT).
    ...
    constexpr int handle_size = sizeof(uint64_t);
    using Key = std::bitset<handle_size * 2>; // Key: instance handle in upper 64 bits, device handle in lower 64 bits
    static vvl::unordered_map<Key, std::vector<VkDeviceEventTypeEXT>> values_map;

    Key k = (Key(CastToUint64(instance)) << handle_size) | Key(CastToUint64(device));
    auto& values = values_map[k];
    if (values.empty()) {
        values.insert(values.begin(), CoreVkDeviceEventTypeEXTEnums.cbegin(), CoreVkDeviceEventTypeEXTEnums.cend());
...
template<>
vvl::span<VkDisplayEventTypeEXT> ValidationObject::ValidParamValues() const {
    // TODO (ncesario) This is not ideal as we compute the enabled extensions every time this function is called.
    //      Ideally "values" would be something like a static variable that is built once and this function returns
    //      a span of the container. This does not work for applications which create and destroy many instances and
    //      devices over the lifespan of the project (e.g., VLT).
    ...
    constexpr int handle_size = sizeof(uint64_t);
    using Key = std::bitset<handle_size * 2>; // Key: instance handle in upper 64 bits, device handle in lower 64 bits
    static vvl::unordered_map<Key, std::vector<VkDisplayEventTypeEXT>> values_map;

    Key k = (Key(CastToUint64(instance)) << handle_size) | Key(CastToUint64(device));
    auto& values = values_map[k];
    if (values.empty()) {
        values.insert(values.begin(), CoreVkDisplayEventTypeEXTEnums.cbegin(), CoreVkDisplayEventTypeEXTEnums.cend());
...
template<>
vvl::span<VkViewportCoordinateSwizzleNV> ValidationObject::ValidParamValues() const {
    // TODO (ncesario) This is not ideal as we compute the enabled extensions every time this function is called.
    //      Ideally "values" would be something like a static variable that is built once and this function returns
    //      a span of the container. This does not work for applications which create and destroy many instances and
    //      devices over the lifespan of the project (e.g., VLT).
    ...
    constexpr int handle_size = sizeof(uint64_t);
    using Key = std::bitset<handle_size * 2>; // Key: instance handle in upper 64 bits, device handle in lower 64 bits
    static vvl::unordered_map<Key, std::vector<VkViewportCoordinateSwizzleNV>> values_map;

    Key k = (Key(CastToUint64(instance)) << handle_size) | Key(CastToUint64(device));
    auto& values = values_map[k];
    if (values.empty()) {
        values.insert(values.begin(), CoreVkViewportCoordinateSwizzleNVEnums.cbegin(), CoreVkViewportCoordinateSwizzleNVEnums.cend());
...
template<>
vvl::span<VkDiscardRectangleModeEXT> ValidationObject::ValidParamValues() const {
    // TODO (ncesario) This is not ideal as we compute the enabled extensions every time this function is called.
    //      Ideally "values" would be something like a static variable that is built once and this function returns
    //      a span of the container. This does not work for applications which create and destroy many instances and
    //      devices over the lifespan of the project (e.g., VLT).
    ...
    constexpr int handle_size = sizeof(uint64_t);
    using Key = std::bitset<handle_size * 2>; // Key: instance handle in upper 64 bits, device handle in lower 64 bits
    static vvl::unordered_map<Key, std::vector<VkDiscardRectangleModeEXT>> values_map;

    Key k = (Key(CastToUint64(instance)) << handle_size) | Key(CastToUint64(device));
    auto& values = values_map[k];
    if (values.empty()) {
        values.insert(values.begin(), CoreVkDiscardRectangleModeEXTEnums.cbegin(), CoreVkDiscardRectangleModeEXTEnums.cend());
...
template<>
vvl::span<VkConservativeRasterizationModeEXT> ValidationObject::ValidParamValues() const {
    // TODO (ncesario) This is not ideal as we compute the enabled extensions every time this function is called.
    //      Ideally "values" would be something like a static variable that is built once and this function returns
    //      a span of the container. This does not work for applications which create and destroy many instances and
    //      devices over the lifespan of the project (e.g., VLT).
    ...
    constexpr int handle_size = sizeof(uint64_t);
    using Key = std::bitset<handle_size * 2>; // Key: instance handle in upper 64 bits, device handle in lower 64 bits
    static vvl::unordered_map<Key, std::vector<VkConservativeRasterizationModeEXT>> values_map;

    Key k = (Key(CastToUint64(instance)) << handle_size) | Key(CastToUint64(device));
    auto& values = values_map[k];
    if (values.empty()) {
        values.insert(values.begin(), CoreVkConservativeRasterizationModeEXTEnums.cbegin(), CoreVkConservativeRasterizationModeEXTEnums.cend());
...
template<>
vvl::span<VkBlendOverlapEXT> ValidationObject::ValidParamValues() const {
    // TODO (ncesario) This is not ideal as we compute the enabled extensions every time this function is called.
    //      Ideally "values" would be something like a static variable that is built once and this function returns
    //      a span of the container. This does not work for applications which create and destroy many instances and
    //      devices over the lifespan of the project (e.g., VLT).
    ...
    constexpr int handle_size = sizeof(uint64_t);
    using Key = std::bitset<handle_size * 2>; // Key: instance handle in upper 64 bits, device handle in lower 64 bits
    static vvl::unordered_map<Key, std::vector<VkBlendOverlapEXT>> values_map;

    Key k = (Key(CastToUint64(instance)) << handle_size) | Key(CastToUint64(device));
    auto& values = values_map[k];
    if (values.empty()) {
        values.insert(values.begin(), CoreVkBlendOverlapEXTEnums.cbegin(), CoreVkBlendOverlapEXTEnums.cend());
...
template<>
vvl::span<VkCoverageModulationModeNV> ValidationObject::ValidParamValues() const {
    // TODO (ncesario) This is not ideal as we compute the enabled extensions every time this function is called.
    //      Ideally "values" would be something like a static variable that is built once and this function returns
    //      a span of the container. This does not work for applications which create and destroy many instances and
    //      devices over the lifespan of the project (e.g., VLT).
    ...
    constexpr int handle_size = sizeof(uint64_t);
    using Key = std::bitset<handle_size * 2>; // Key: instance handle in upper 64 bits, device handle in lower 64 bits
    static vvl::unordered_map<Key, std::vector<VkCoverageModulationModeNV>> values_map;

    Key k = (Key(CastToUint64(instance)) << handle_size) | Key(CastToUint64(device));
    auto& values = values_map[k];
    if (values.empty()) {
        values.insert(values.begin(), CoreVkCoverageModulationModeNVEnums.cbegin(), CoreVkCoverageModulationModeNVEnums.cend());
...
template<>
vvl::span<VkShadingRatePaletteEntryNV> ValidationObject::ValidParamValues() const {
    // TODO (ncesario) This is not ideal as we compute the enabled extensions every time this function is called.
    //      Ideally "values" would be something like a static variable that is built once and this function returns
    //      a span of the container. This does not work for applications which create and destroy many instances and
    //      devices over the lifespan of the project (e.g., VLT).
    ...
    constexpr int handle_size = sizeof(uint64_t);
    using Key = std::bitset<handle_size * 2>; // Key: instance handle in upper 64 bits, device handle in lower 64 bits
    static vvl::unordered_map<Key, std::vector<VkShadingRatePaletteEntryNV>> values_map;

    Key k = (Key(CastToUint64(instance)) << handle_size) | Key(CastToUint64(device));
    auto& values = values_map[k];
    if (values.empty()) {
        values.insert(values.begin(), CoreVkShadingRatePaletteEntryNVEnums.cbegin(), CoreVkShadingRatePaletteEntryNVEnums.cend());
...
template<>
vvl::span<VkCoarseSampleOrderTypeNV> ValidationObject::ValidParamValues() const {
    // TODO (ncesario) This is not ideal as we compute the enabled extensions every time this function is called.
    //      Ideally "values" would be something like a static variable that is built once and this function returns
    //      a span of the container. This does not work for applications which create and destroy many instances and
    //      devices over the lifespan of the project (e.g., VLT).
    ...
    constexpr int handle_size = sizeof(uint64_t);
    using Key = std::bitset<handle_size * 2>; // Key: instance handle in upper 64 bits, device handle in lower 64 bits
    static vvl::unordered_map<Key, std::vector<VkCoarseSampleOrderTypeNV>> values_map;

    Key k = (Key(CastToUint64(instance)) << handle_size) | Key(CastToUint64(device));
    auto& values = values_map[k];
    if (values.empty()) {
        values.insert(values.begin(), CoreVkCoarseSampleOrderTypeNVEnums.cbegin(), CoreVkCoarseSampleOrderTypeNVEnums.cend());
...
template<>
vvl::span<VkRayTracingShaderGroupTypeKHR> ValidationObject::ValidParamValues() const {
    // TODO (ncesario) This is not ideal as we compute the enabled extensions every time this function is called.
    //      Ideally "values" would be something like a static variable that is built once and this function returns
    //      a span of the container. This does not work for applications which create and destroy many instances and
    //      devices over the lifespan of the project (e.g., VLT).
    ...
    constexpr int handle_size = sizeof(uint64_t);
    using Key = std::bitset<handle_size * 2>; // Key: instance handle in upper 64 bits, device handle in lower 64 bits
    static vvl::unordered_map<Key, std::vector<VkRayTracingShaderGroupTypeKHR>> values_map;

    Key k = (Key(CastToUint64(instance)) << handle_size) | Key(CastToUint64(device));
    auto& values = values_map[k];
    if (values.empty()) {
        values.insert(values.begin(), CoreVkRayTracingShaderGroupTypeKHREnums.cbegin(), CoreVkRayTracingShaderGroupTypeKHREnums.cend());
...
template<>
vvl::span<VkGeometryTypeKHR> ValidationObject::ValidParamValues() const {
    // TODO (ncesario) This is not ideal as we compute the enabled extensions every time this function is called.
    //      Ideally "values" would be something like a static variable that is built once and this function returns
    //      a span of the container. This does not work for applications which create and destroy many instances and
    //      devices over the lifespan of the project (e.g., VLT).
    ...
    constexpr int handle_size = sizeof(uint64_t);
    using Key = std::bitset<handle_size * 2>; // Key: instance handle in upper 64 bits, device handle in lower 64 bits
    static vvl::unordered_map<Key, std::vector<VkGeometryTypeKHR>> values_map;

    Key k = (Key(CastToUint64(instance)) << handle_size) | Key(CastToUint64(device));
    auto& values = values_map[k];
    if (values.empty()) {
        values.insert(values.begin(), CoreVkGeometryTypeKHREnums.cbegin(), CoreVkGeometryTypeKHREnums.cend());
...
template<>
vvl::span<VkAccelerationStructureTypeKHR> ValidationObject::ValidParamValues() const {
    // TODO (ncesario) This is not ideal as we compute the enabled extensions every time this function is called.
    //      Ideally "values" would be something like a static variable that is built once and this function returns
    //      a span of the container. This does not work for applications which create and destroy many instances and
    //      devices over the lifespan of the project (e.g., VLT).
    ...
    constexpr int handle_size = sizeof(uint64_t);
    using Key = std::bitset<handle_size * 2>; // Key: instance handle in upper 64 bits, device handle in lower 64 bits
    static vvl::unordered_map<Key, std::vector<VkAccelerationStructureTypeKHR>> values_map;

    Key k = (Key(CastToUint64(instance)) << handle_size) | Key(CastToUint64(device));
    auto& values = values_map[k];
    if (values.empty()) {
        values.insert(values.begin(), CoreVkAccelerationStructureTypeKHREnums.cbegin(), CoreVkAccelerationStructureTypeKHREnums.cend());
...
template<>
vvl::span<VkCopyAccelerationStructureModeKHR> ValidationObject::ValidParamValues() const {
    // TODO (ncesario) This is not ideal as we compute the enabled extensions every time this function is called.
    //      Ideally "values" would be something like a static variable that is built once and this function returns
    //      a span of the container. This does not work for applications which create and destroy many instances and
    //      devices over the lifespan of the project (e.g., VLT).
    ...
    constexpr int handle_size = sizeof(uint64_t);
    using Key = std::bitset<handle_size * 2>; // Key: instance handle in upper 64 bits, device handle in lower 64 bits
    static vvl::unordered_map<Key, std::vector<VkCopyAccelerationStructureModeKHR>> values_map;

    Key k = (Key(CastToUint64(instance)) << handle_size) | Key(CastToUint64(device));
    auto& values = values_map[k];
    if (values.empty()) {
        values.insert(values.begin(), CoreVkCopyAccelerationStructureModeKHREnums.cbegin(), CoreVkCopyAccelerationStructureModeKHREnums.cend());
...
template<>
vvl::span<VkAccelerationStructureMemoryRequirementsTypeNV> ValidationObject::ValidParamValues() const {
    // TODO (ncesario) This is not ideal as we compute the enabled extensions every time this function is called.
    //      Ideally "values" would be something like a static variable that is built once and this function returns
    //      a span of the container. This does not work for applications which create and destroy many instances and
    //      devices over the lifespan of the project (e.g., VLT).
    ...
    constexpr int handle_size = sizeof(uint64_t);
    using Key = std::bitset<handle_size * 2>; // Key: instance handle in upper 64 bits, device handle in lower 64 bits
    static vvl::unordered_map<Key, std::vector<VkAccelerationStructureMemoryRequirementsTypeNV>> values_map;

    Key k = (Key(CastToUint64(instance)) << handle_size) | Key(CastToUint64(device));
    auto& values = values_map[k];
    if (values.empty()) {
        values.insert(values.begin(), CoreVkAccelerationStructureMemoryRequirementsTypeNVEnums.cbegin(), CoreVkAccelerationStructureMemoryRequirementsTypeNVEnums.cend());
...
template<>
vvl::span<VkTimeDomainEXT> ValidationObject::ValidParamValues() const {
    // TODO (ncesario) This is not ideal as we compute the enabled extensions every time this function is called.
    //      Ideally "values" would be something like a static variable that is built once and this function returns
    //      a span of the container. This does not work for applications which create and destroy many instances and
    //      devices over the lifespan of the project (e.g., VLT).
    ...
    constexpr int handle_size = sizeof(uint64_t);
    using Key = std::bitset<handle_size * 2>; // Key: instance handle in upper 64 bits, device handle in lower 64 bits
    static vvl::unordered_map<Key, std::vector<VkTimeDomainEXT>> values_map;

    Key k = (Key(CastToUint64(instance)) << handle_size) | Key(CastToUint64(device));
    auto& values = values_map[k];
    if (values.empty()) {
        values.insert(values.begin(), CoreVkTimeDomainEXTEnums.cbegin(), CoreVkTimeDomainEXTEnums.cend());
...
template<>
vvl::span<VkMemoryOverallocationBehaviorAMD> ValidationObject::ValidParamValues() const {
    // TODO (ncesario) This is not ideal as we compute the enabled extensions every time this function is called.
    //      Ideally "values" would be something like a static variable that is built once and this function returns
    //      a span of the container. This does not work for applications which create and destroy many instances and
    //      devices over the lifespan of the project (e.g., VLT).
    ...
    constexpr int handle_size = sizeof(uint64_t);
    using Key = std::bitset<handle_size * 2>; // Key: instance handle in upper 64 bits, device handle in lower 64 bits
    static vvl::unordered_map<Key, std::vector<VkMemoryOverallocationBehaviorAMD>> values_map;

    Key k = (Key(CastToUint64(instance)) << handle_size) | Key(CastToUint64(device));
    auto& values = values_map[k];
    if (values.empty()) {
        values.insert(values.begin(), CoreVkMemoryOverallocationBehaviorAMDEnums.cbegin(), CoreVkMemoryOverallocationBehaviorAMDEnums.cend());
...
template<>
vvl::span<VkPerformanceConfigurationTypeINTEL> ValidationObject::ValidParamValues() const {
    // TODO (ncesario) This is not ideal as we compute the enabled extensions every time this function is called.
    //      Ideally "values" would be something like a static variable that is built once and this function returns
    //      a span of the container. This does not work for applications which create and destroy many instances and
    //      devices over the lifespan of the project (e.g., VLT).
    ...
    constexpr int handle_size = sizeof(uint64_t);
    using Key = std::bitset<handle_size * 2>; // Key: instance handle in upper 64 bits, device handle in lower 64 bits
    static vvl::unordered_map<Key, std::vector<VkPerformanceConfigurationTypeINTEL>> values_map;

    Key k = (Key(CastToUint64(instance)) << handle_size) | Key(CastToUint64(device));
    auto& values = values_map[k];
    if (values.empty()) {
        values.insert(values.begin(), CoreVkPerformanceConfigurationTypeINTELEnums.cbegin(), CoreVkPerformanceConfigurationTypeINTELEnums.cend());
...
template<>
vvl::span<VkQueryPoolSamplingModeINTEL> ValidationObject::ValidParamValues() const {
    // TODO (ncesario) This is not ideal as we compute the enabled extensions every time this function is called.
    //      Ideally "values" would be something like a static variable that is built once and this function returns
    //      a span of the container. This does not work for applications which create and destroy many instances and
    //      devices over the lifespan of the project (e.g., VLT).
    ...
    constexpr int handle_size = sizeof(uint64_t);
    using Key = std::bitset<handle_size * 2>; // Key: instance handle in upper 64 bits, device handle in lower 64 bits
    static vvl::unordered_map<Key, std::vector<VkQueryPoolSamplingModeINTEL>> values_map;

    Key k = (Key(CastToUint64(instance)) << handle_size) | Key(CastToUint64(device));
    auto& values = values_map[k];
    if (values.empty()) {
        values.insert(values.begin(), CoreVkQueryPoolSamplingModeINTELEnums.cbegin(), CoreVkQueryPoolSamplingModeINTELEnums.cend());
...
template<>
vvl::span<VkPerformanceOverrideTypeINTEL> ValidationObject::ValidParamValues() const {
    // TODO (ncesario) This is not ideal as we compute the enabled extensions every time this function is called.
    //      Ideally "values" would be something like a static variable that is built once and this function returns
    //      a span of the container. This does not work for applications which create and destroy many instances and
    //      devices over the lifespan of the project (e.g., VLT).
    ...
    constexpr int handle_size = sizeof(uint64_t);
    using Key = std::bitset<handle_size * 2>; // Key: instance handle in upper 64 bits, device handle in lower 64 bits
    static vvl::unordered_map<Key, std::vector<VkPerformanceOverrideTypeINTEL>> values_map;

    Key k = (Key(CastToUint64(instance)) << handle_size) | Key(CastToUint64(device));
    auto& values = values_map[k];
    if (values.empty()) {
        values.insert(values.begin(), CoreVkPerformanceOverrideTypeINTELEnums.cbegin(), CoreVkPerformanceOverrideTypeINTELEnums.cend());
...
template<>
vvl::span<VkPerformanceParameterTypeINTEL> ValidationObject::ValidParamValues() const {
    // TODO (ncesario) This is not ideal as we compute the enabled extensions every time this function is called.
    //      Ideally "values" would be something like a static variable that is built once and this function returns
    //      a span of the container. This does not work for applications which create and destroy many instances and
    //      devices over the lifespan of the project (e.g., VLT).
    ...
    constexpr int handle_size = sizeof(uint64_t);
    using Key = std::bitset<handle_size * 2>; // Key: instance handle in upper 64 bits, device handle in lower 64 bits
    static vvl::unordered_map<Key, std::vector<VkPerformanceParameterTypeINTEL>> values_map;

    Key k = (Key(CastToUint64(instance)) << handle_size) | Key(CastToUint64(device));
    auto& values = values_map[k];
    if (values.empty()) {
        values.insert(values.begin(), CoreVkPerformanceParameterTypeINTELEnums.cbegin(), CoreVkPerformanceParameterTypeINTELEnums.cend());
...
template<>
vvl::span<VkPerformanceValueTypeINTEL> ValidationObject::ValidParamValues() const {
    // TODO (ncesario) This is not ideal as we compute the enabled extensions every time this function is called.
    //      Ideally "values" would be something like a static variable that is built once and this function returns
    //      a span of the container. This does not work for applications which create and destroy many instances and
    //      devices over the lifespan of the project (e.g., VLT).
    ...
    constexpr int handle_size = sizeof(uint64_t);
    using Key = std::bitset<handle_size * 2>; // Key: instance handle in upper 64 bits, device handle in lower 64 bits
    static vvl::unordered_map<Key, std::vector<VkPerformanceValueTypeINTEL>> values_map;

    Key k = (Key(CastToUint64(instance)) << handle_size) | Key(CastToUint64(device));
    auto& values = values_map[k];
    if (values.empty()) {
        values.insert(values.begin(), CoreVkPerformanceValueTypeINTELEnums.cbegin(), CoreVkPerformanceValueTypeINTELEnums.cend());
...
template<>
vvl::span<VkValidationFeatureEnableEXT> ValidationObject::ValidParamValues() const {
    // TODO (ncesario) This is not ideal as we compute the enabled extensions every time this function is called.
    //      Ideally "values" would be something like a static variable that is built once and this function returns
    //      a span of the container. This does not work for applications which create and destroy many instances and
    //      devices over the lifespan of the project (e.g., VLT).
    ...
    constexpr int handle_size = sizeof(uint64_t);
    using Key = std::bitset<handle_size * 2>; // Key: instance handle in upper 64 bits, device handle in lower 64 bits
    static vvl::unordered_map<Key, std::vector<VkValidationFeatureEnableEXT>> values_map;

    Key k = (Key(CastToUint64(instance)) << handle_size) | Key(CastToUint64(device));
    auto& values = values_map[k];
    if (values.empty()) {
        values.insert(values.begin(), CoreVkValidationFeatureEnableEXTEnums.cbegin(), CoreVkValidationFeatureEnableEXTEnums.cend());
...
template<>
vvl::span<VkValidationFeatureDisableEXT> ValidationObject::ValidParamValues() const {
    // TODO (ncesario) This is not ideal as we compute the enabled extensions every time this function is called.
    //      Ideally "values" would be something like a static variable that is built once and this function returns
    //      a span of the container. This does not work for applications which create and destroy many instances and
    //      devices over the lifespan of the project (e.g., VLT).
    ...
    constexpr int handle_size = sizeof(uint64_t);
    using Key = std::bitset<handle_size * 2>; // Key: instance handle in upper 64 bits, device handle in lower 64 bits
    static vvl::unordered_map<Key, std::vector<VkValidationFeatureDisableEXT>> values_map;

    Key k = (Key(CastToUint64(instance)) << handle_size) | Key(CastToUint64(device));
    auto& values = values_map[k];
    if (values.empty()) {
        values.insert(values.begin(), CoreVkValidationFeatureDisableEXTEnums.cbegin(), CoreVkValidationFeatureDisableEXTEnums.cend());
...
template<>
vvl::span<VkComponentTypeNV> ValidationObject::ValidParamValues() const {
    // TODO (ncesario) This is not ideal as we compute the enabled extensions every time this function is called.
    //      Ideally "values" would be something like a static variable that is built once and this function returns
    //      a span of the container. This does not work for applications which create and destroy many instances and
    //      devices over the lifespan of the project (e.g., VLT).
    ...
    constexpr int handle_size = sizeof(uint64_t);
    using Key = std::bitset<handle_size * 2>; // Key: instance handle in upper 64 bits, device handle in lower 64 bits
    static vvl::unordered_map<Key, std::vector<VkComponentTypeNV>> values_map;

    Key k = (Key(CastToUint64(instance)) << handle_size) | Key(CastToUint64(device));
    auto& values = values_map[k];
    if (values.empty()) {
        values.insert(values.begin(), CoreVkComponentTypeNVEnums.cbegin(), CoreVkComponentTypeNVEnums.cend());
...
template<>
vvl::span<VkScopeNV> ValidationObject::ValidParamValues() const {
    // TODO (ncesario) This is not ideal as we compute the enabled extensions every time this function is called.
    //      Ideally "values" would be something like a static variable that is built once and this function returns
    //      a span of the container. This does not work for applications which create and destroy many instances and
    //      devices over the lifespan of the project (e.g., VLT).
    ...
    constexpr int handle_size = sizeof(uint64_t);
    using Key = std::bitset<handle_size * 2>; // Key: instance handle in upper 64 bits, device handle in lower 64 bits
    static vvl::unordered_map<Key, std::vector<VkScopeNV>> values_map;

    Key k = (Key(CastToUint64(instance)) << handle_size) | Key(CastToUint64(device));
    auto& values = values_map[k];
    if (values.empty()) {
        values.insert(values.begin(), CoreVkScopeNVEnums.cbegin(), CoreVkScopeNVEnums.cend());
...
template<>
vvl::span<VkCoverageReductionModeNV> ValidationObject::ValidParamValues() const {
    // TODO (ncesario) This is not ideal as we compute the enabled extensions every time this function is called.
    //      Ideally "values" would be something like a static variable that is built once and this function returns
    //      a span of the container. This does not work for applications which create and destroy many instances and
    //      devices over the lifespan of the project (e.g., VLT).
    ...
    constexpr int handle_size = sizeof(uint64_t);
    using Key = std::bitset<handle_size * 2>; // Key: instance handle in upper 64 bits, device handle in lower 64 bits
    static vvl::unordered_map<Key, std::vector<VkCoverageReductionModeNV>> values_map;

    Key k = (Key(CastToUint64(instance)) << handle_size) | Key(CastToUint64(device));
    auto& values = values_map[k];
    if (values.empty()) {
        values.insert(values.begin(), CoreVkCoverageReductionModeNVEnums.cbegin(), CoreVkCoverageReductionModeNVEnums.cend());
...
template<>
vvl::span<VkProvokingVertexModeEXT> ValidationObject::ValidParamValues() const {
    // TODO (ncesario) This is not ideal as we compute the enabled extensions every time this function is called.
    //      Ideally "values" would be something like a static variable that is built once and this function returns
    //      a span of the container. This does not work for applications which create and destroy many instances and
    //      devices over the lifespan of the project (e.g., VLT).
    ...
    constexpr int handle_size = sizeof(uint64_t);
    using Key = std::bitset<handle_size * 2>; // Key: instance handle in upper 64 bits, device handle in lower 64 bits
    static vvl::unordered_map<Key, std::vector<VkProvokingVertexModeEXT>> values_map;

    Key k = (Key(CastToUint64(instance)) << handle_size) | Key(CastToUint64(device));
    auto& values = values_map[k];
    if (values.empty()) {
        values.insert(values.begin(), CoreVkProvokingVertexModeEXTEnums.cbegin(), CoreVkProvokingVertexModeEXTEnums.cend());
...
template<>
vvl::span<VkFullScreenExclusiveEXT> ValidationObject::ValidParamValues() const {
    // TODO (ncesario) This is not ideal as we compute the enabled extensions every time this function is called.
    //      Ideally "values" would be something like a static variable that is built once and this function returns
    //      a span of the container. This does not work for applications which create and destroy many instances and
    //      devices over the lifespan of the project (e.g., VLT).
    ...
    constexpr int handle_size = sizeof(uint64_t);
    using Key = std::bitset<handle_size * 2>; // Key: instance handle in upper 64 bits, device handle in lower 64 bits
    static vvl::unordered_map<Key, std::vector<VkFullScreenExclusiveEXT>> values_map;

    Key k = (Key(CastToUint64(instance)) << handle_size) | Key(CastToUint64(device));
    auto& values = values_map[k];
    if (values.empty()) {
        values.insert(values.begin(), CoreVkFullScreenExclusiveEXTEnums.cbegin(), CoreVkFullScreenExclusiveEXTEnums.cend());
...
template<>
vvl::span<VkLineRasterizationModeEXT> ValidationObject::ValidParamValues() const {
    // TODO (ncesario) This is not ideal as we compute the enabled extensions every time this function is called.
    //      Ideally "values" would be something like a static variable that is built once and this function returns
    //      a span of the container. This does not work for applications which create and destroy many instances and
    //      devices over the lifespan of the project (e.g., VLT).
    ...
    constexpr int handle_size = sizeof(uint64_t);
    using Key = std::bitset<handle_size * 2>; // Key: instance handle in upper 64 bits, device handle in lower 64 bits
    static vvl::unordered_map<Key, std::vector<VkLineRasterizationModeEXT>> values_map;

    Key k = (Key(CastToUint64(instance)) << handle_size) | Key(CastToUint64(device));
    auto& values = values_map[k];
    if (values.empty()) {
        values.insert(values.begin(), CoreVkLineRasterizationModeEXTEnums.cbegin(), CoreVkLineRasterizationModeEXTEnums.cend());
...
template<>
vvl::span<VkIndirectCommandsTokenTypeNV> ValidationObject::ValidParamValues() const {
    // TODO (ncesario) This is not ideal as we compute the enabled extensions every time this function is called.
    //      Ideally "values" would be something like a static variable that is built once and this function returns
    //      a span of the container. This does not work for applications which create and destroy many instances and
    //      devices over the lifespan of the project (e.g., VLT).
    ...
    constexpr int handle_size = sizeof(uint64_t);
    using Key = std::bitset<handle_size * 2>; // Key: instance handle in upper 64 bits, device handle in lower 64 bits
    static vvl::unordered_map<Key, std::vector<VkIndirectCommandsTokenTypeNV>> values_map;

    Key k = (Key(CastToUint64(instance)) << handle_size) | Key(CastToUint64(device));
    auto& values = values_map[k];
    if (values.empty()) {
        values.insert(values.begin(), CoreVkIndirectCommandsTokenTypeNVEnums.cbegin(), CoreVkIndirectCommandsTokenTypeNVEnums.cend());
...
template<>
vvl::span<VkFragmentShadingRateNV> ValidationObject::ValidParamValues() const {
    // TODO (ncesario) This is not ideal as we compute the enabled extensions every time this function is called.
    //      Ideally "values" would be something like a static variable that is built once and this function returns
    //      a span of the container. This does not work for applications which create and destroy many instances and
    //      devices over the lifespan of the project (e.g., VLT).
    ...
    constexpr int handle_size = sizeof(uint64_t);
    using Key = std::bitset<handle_size * 2>; // Key: instance handle in upper 64 bits, device handle in lower 64 bits
    static vvl::unordered_map<Key, std::vector<VkFragmentShadingRateNV>> values_map;

    Key k = (Key(CastToUint64(instance)) << handle_size) | Key(CastToUint64(device));
    auto& values = values_map[k];
    if (values.empty()) {
        values.insert(values.begin(), CoreVkFragmentShadingRateNVEnums.cbegin(), CoreVkFragmentShadingRateNVEnums.cend());
...
template<>
vvl::span<VkAccelerationStructureMotionInstanceTypeNV> ValidationObject::ValidParamValues() const {
    // TODO (ncesario) This is not ideal as we compute the enabled extensions every time this function is called.
    //      Ideally "values" would be something like a static variable that is built once and this function returns
    //      a span of the container. This does not work for applications which create and destroy many instances and
    //      devices over the lifespan of the project (e.g., VLT).
    ...
    constexpr int handle_size = sizeof(uint64_t);
    using Key = std::bitset<handle_size * 2>; // Key: instance handle in upper 64 bits, device handle in lower 64 bits
    static vvl::unordered_map<Key, std::vector<VkAccelerationStructureMotionInstanceTypeNV>> values_map;

    Key k = (Key(CastToUint64(instance)) << handle_size) | Key(CastToUint64(device));
    auto& values = values_map[k];
    if (values.empty()) {
        values.insert(values.begin(), CoreVkAccelerationStructureMotionInstanceTypeNVEnums.cbegin(), CoreVkAccelerationStructureMotionInstanceTypeNVEnums.cend());
...
template<>
vvl::span<VkDeviceFaultAddressTypeEXT> ValidationObject::ValidParamValues() const {
    // TODO (ncesario) This is not ideal as we compute the enabled extensions every time this function is called.
    //      Ideally "values" would be something like a static variable that is built once and this function returns
    //      a span of the container. This does not work for applications which create and destroy many instances and
    //      devices over the lifespan of the project (e.g., VLT).
    ...
    constexpr int handle_size = sizeof(uint64_t);
    using Key = std::bitset<handle_size * 2>; // Key: instance handle in upper 64 bits, device handle in lower 64 bits
    static vvl::unordered_map<Key, std::vector<VkDeviceFaultAddressTypeEXT>> values_map;

    Key k = (Key(CastToUint64(instance)) << handle_size) | Key(CastToUint64(device));
    auto& values = values_map[k];
    if (values.empty()) {
        values.insert(values.begin(), CoreVkDeviceFaultAddressTypeEXTEnums.cbegin(), CoreVkDeviceFaultAddressTypeEXTEnums.cend());
...
template<>
vvl::span<VkDeviceFaultVendorBinaryHeaderVersionEXT> ValidationObject::ValidParamValues() const {
    // TODO (ncesario) This is not ideal as we compute the enabled extensions every time this function is called.
    //      Ideally "values" would be something like a static variable that is built once and this function returns
    //      a span of the container. This does not work for applications which create and destroy many instances and
    //      devices over the lifespan of the project (e.g., VLT).
    ...
    constexpr int handle_size = sizeof(uint64_t);
    using Key = std::bitset<handle_size * 2>; // Key: instance handle in upper 64 bits, device handle in lower 64 bits
    static vvl::unordered_map<Key, std::vector<VkDeviceFaultVendorBinaryHeaderVersionEXT>> values_map;

    Key k = (Key(CastToUint64(instance)) << handle_size) | Key(CastToUint64(device));
    auto& values = values_map[k];
    if (values.empty()) {
        values.insert(values.begin(), CoreVkDeviceFaultVendorBinaryHeaderVersionEXTEnums.cbegin(), CoreVkDeviceFaultVendorBinaryHeaderVersionEXTEnums.cend());
...
template<>
vvl::span<VkDeviceAddressBindingTypeEXT> ValidationObject::ValidParamValues() const {
    // TODO (ncesario) This is not ideal as we compute the enabled extensions every time this function is called.
    //      Ideally "values" would be something like a static variable that is built once and this function returns
    //      a span of the container. This does not work for applications which create and destroy many instances and
    //      devices over the lifespan of the project (e.g., VLT).
    ...
    constexpr int handle_size = sizeof(uint64_t);
    using Key = std::bitset<handle_size * 2>; // Key: instance handle in upper 64 bits, device handle in lower 64 bits
    static vvl::unordered_map<Key, std::vector<VkDeviceAddressBindingTypeEXT>> values_map;

    Key k = (Key(CastToUint64(instance)) << handle_size) | Key(CastToUint64(device));
    auto& values = values_map[k];
    if (values.empty()) {
        values.insert(values.begin(), CoreVkDeviceAddressBindingTypeEXTEnums.cbegin(), CoreVkDeviceAddressBindingTypeEXTEnums.cend());
...
template<>
vvl::span<VkMicromapTypeEXT> ValidationObject::ValidParamValues() const {
    // TODO (ncesario) This is not ideal as we compute the enabled extensions every time this function is called.
    //      Ideally "values" would be something like a static variable that is built once and this function returns
    //      a span of the container. This does not work for applications which create and destroy many instances and
    //      devices over the lifespan of the project (e.g., VLT).
    ...
    constexpr int handle_size = sizeof(uint64_t);
    using Key = std::bitset<handle_size * 2>; // Key: instance handle in upper 64 bits, device handle in lower 64 bits
    static vvl::unordered_map<Key, std::vector<VkMicromapTypeEXT>> values_map;

    Key k = (Key(CastToUint64(instance)) << handle_size) | Key(CastToUint64(device));
    auto& values = values_map[k];
    if (values.empty()) {
        values.insert(values.begin(), CoreVkMicromapTypeEXTEnums.cbegin(), CoreVkMicromapTypeEXTEnums.cend());
...
template<>
vvl::span<VkCopyMicromapModeEXT> ValidationObject::ValidParamValues() const {
    // TODO (ncesario) This is not ideal as we compute the enabled extensions every time this function is called.
    //      Ideally "values" would be something like a static variable that is built once and this function returns
    //      a span of the container. This does not work for applications which create and destroy many instances and
    //      devices over the lifespan of the project (e.g., VLT).
    ...
    constexpr int handle_size = sizeof(uint64_t);
    using Key = std::bitset<handle_size * 2>; // Key: instance handle in upper 64 bits, device handle in lower 64 bits
    static vvl::unordered_map<Key, std::vector<VkCopyMicromapModeEXT>> values_map;

    Key k = (Key(CastToUint64(instance)) << handle_size) | Key(CastToUint64(device));
    auto& values = values_map[k];
    if (values.empty()) {
        values.insert(values.begin(), CoreVkCopyMicromapModeEXTEnums.cbegin(), CoreVkCopyMicromapModeEXTEnums.cend());
...
template<>
vvl::span<VkAccelerationStructureBuildTypeKHR> ValidationObject::ValidParamValues() const {
    // TODO (ncesario) This is not ideal as we compute the enabled extensions every time this function is called.
    //      Ideally "values" would be something like a static variable that is built once and this function returns
    //      a span of the container. This does not work for applications which create and destroy many instances and
    //      devices over the lifespan of the project (e.g., VLT).
    ...
    constexpr int handle_size = sizeof(uint64_t);
    using Key = std::bitset<handle_size * 2>; // Key: instance handle in upper 64 bits, device handle in lower 64 bits
    static vvl::unordered_map<Key, std::vector<VkAccelerationStructureBuildTypeKHR>> values_map;

    Key k = (Key(CastToUint64(instance)) << handle_size) | Key(CastToUint64(device));
    auto& values = values_map[k];
    if (values.empty()) {
        values.insert(values.begin(), CoreVkAccelerationStructureBuildTypeKHREnums.cbegin(), CoreVkAccelerationStructureBuildTypeKHREnums.cend());
...
template<>
vvl::span<VkDirectDriverLoadingModeLUNARG> ValidationObject::ValidParamValues() const {
    // TODO (ncesario) This is not ideal as we compute the enabled extensions every time this function is called.
    //      Ideally "values" would be something like a static variable that is built once and this function returns
    //      a span of the container. This does not work for applications which create and destroy many instances and
    //      devices over the lifespan of the project (e.g., VLT).
    ...
    constexpr int handle_size = sizeof(uint64_t);
    using Key = std::bitset<handle_size * 2>; // Key: instance handle in upper 64 bits, device handle in lower 64 bits
    static vvl::unordered_map<Key, std::vector<VkDirectDriverLoadingModeLUNARG>> values_map;

    Key k = (Key(CastToUint64(instance)) << handle_size) | Key(CastToUint64(device));
    auto& values = values_map[k];
    if (values.empty()) {
        values.insert(values.begin(), CoreVkDirectDriverLoadingModeLUNARGEnums.cbegin(), CoreVkDirectDriverLoadingModeLUNARGEnums.cend());
...
template<>
vvl::span<VkOpticalFlowPerformanceLevelNV> ValidationObject::ValidParamValues() const {
    // TODO (ncesario) This is not ideal as we compute the enabled extensions every time this function is called.
    //      Ideally "values" would be something like a static variable that is built once and this function returns
    //      a span of the container. This does not work for applications which create and destroy many instances and
    //      devices over the lifespan of the project (e.g., VLT).
    ...
    constexpr int handle_size = sizeof(uint64_t);
    using Key = std::bitset<handle_size * 2>; // Key: instance handle in upper 64 bits, device handle in lower 64 bits
    static vvl::unordered_map<Key, std::vector<VkOpticalFlowPerformanceLevelNV>> values_map;

    Key k = (Key(CastToUint64(instance)) << handle_size) | Key(CastToUint64(device));
    auto& values = values_map[k];
    if (values.empty()) {
        values.insert(values.begin(), CoreVkOpticalFlowPerformanceLevelNVEnums.cbegin(), CoreVkOpticalFlowPerformanceLevelNVEnums.cend());
...
template<>
vvl::span<VkOpticalFlowSessionBindingPointNV> ValidationObject::ValidParamValues() const {
    // TODO (ncesario) This is not ideal as we compute the enabled extensions every time this function is called.
    //      Ideally "values" would be something like a static variable that is built once and this function returns
    //      a span of the container. This does not work for applications which create and destroy many instances and
    //      devices over the lifespan of the project (e.g., VLT).
    ...
    constexpr int handle_size = sizeof(uint64_t);
    using Key = std::bitset<handle_size * 2>; // Key: instance handle in upper 64 bits, device handle in lower 64 bits
    static vvl::unordered_map<Key, std::vector<VkOpticalFlowSessionBindingPointNV>> values_map;

    Key k = (Key(CastToUint64(instance)) << handle_size) | Key(CastToUint64(device));
    auto& values = values_map[k];
    if (values.empty()) {
        values.insert(values.begin(), CoreVkOpticalFlowSessionBindingPointNVEnums.cbegin(), CoreVkOpticalFlowSessionBindingPointNVEnums.cend());
...
template<>
vvl::span<VkShaderGroupShaderKHR> ValidationObject::ValidParamValues() const {
    // TODO (ncesario) This is not ideal as we compute the enabled extensions every time this function is called.
    //      Ideally "values" would be something like a static variable that is built once and this function returns
    //      a span of the container. This does not work for applications which create and destroy many instances and
    //      devices over the lifespan of the project (e.g., VLT).
    ...
    constexpr int handle_size = sizeof(uint64_t);
    using Key = std::bitset<handle_size * 2>; // Key: instance handle in upper 64 bits, device handle in lower 64 bits
    static vvl::unordered_map<Key, std::vector<VkShaderGroupShaderKHR>> values_map;

    Key k = (Key(CastToUint64(instance)) << handle_size) | Key(CastToUint64(device));
    auto& values = values_map[k];
    if (values.empty()) {
        values.insert(values.begin(), CoreVkShaderGroupShaderKHREnums.cbegin(), CoreVkShaderGroupShaderKHREnums.cend());
