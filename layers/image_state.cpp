...
std::vector<VkPresentModeKHR> SURFACE_STATE::GetPresentModes(VkPhysicalDevice phys_dev,
    ...
    if (const auto search = present_modes_.find(phys_dev); search != present_modes_.end()) {
    ...
    const auto log_internal_error = [validation_obj](VkResult err, auto &&...objects) {
        if (validation_obj) {
            ...
            validation_obj->LogInternalError(__PRETTY_FUNCTION__, obj_list, "vkGetPhysicalDeviceSurfacePresentModesKHR", err);
        }
    };
...
layer_data::span<const safe_VkSurfaceFormat2KHR> SURFACE_STATE::GetFormats(bool get_surface_capabilities2,
    ...
    if (get_surface_capabilities2) {
        const auto log_internal_error = [validation_obj](VkResult err, auto &&...objects) {
            if (validation_obj) {
                LogObjectList obj_list(std::forward<decltype(objects)>(objects)...);
                validation_obj->LogInternalError(__PRETTY_FUNCTION__, obj_list, "vkGetPhysicalDeviceSurfaceFormats2KHR", err);
            }
        };
        ...
        const auto log_internal_error = [validation_obj](VkResult err, auto &&...objects) {
            if (validation_obj) {
                LogObjectList obj_list(std::forward<decltype(objects)>(objects)...);
                validation_obj->LogInternalError(__PRETTY_FUNCTION__, obj_list, "vkGetPhysicalDeviceSurfaceFormatsKHR", err);
            }
        };
...
safe_VkSurfaceCapabilities2KHR SURFACE_STATE::GetCapabilities(bool get_surface_capabilities2, VkPhysicalDevice phys_dev,
    ...
    const auto log_internal_error = [validation_obj](VkResult err, auto &&...objects) {
        if (validation_obj) {
            LogObjectList obj_list(std::forward<decltype(objects)>(objects)...);
            validation_obj->LogInternalError(__PRETTY_FUNCTION__, obj_list, "vkGetPhysicalDeviceSurfaceCapabilities2KHR", err);
        }
    };
