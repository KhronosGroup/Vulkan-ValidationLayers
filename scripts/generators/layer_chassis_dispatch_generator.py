...
class LayerChassisDispatchOutputGenerator(BaseGenerator):
    ...
    def __init__(self,
        ...
        # List of all extension structs containing handles
    ...
    def decIndent(self, indent):
        return indent[:-self.INDENT_SPACES] if indent and (len(indent) > self.INDENT_SPACES) else ''
...
''')
        ...
        for command in [x for x in self.vk.commands.values() if x.name not in self.no_autogen_list]:
            ...
            for param in command.params:
                ...
                isLocal = (self.isNonDispatchable(param.type) and param.length and param.const) or (struct and self.containsNonDispatchableObject(struct))
    ...
    def uniquifyMembers(self, members: List[Member], indent: str, prefix: str, arrayIndex: int, isCreate: bool, isDestroy: bool, topLevel: bool):
        ...
        for member in members:
            ...
            # Handle Structs that contain NDOs at some level
                ...
                if self.containsNonDispatchableObject(struct) or process_pnext:
