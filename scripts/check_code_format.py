...
#
#
# Check clang-tidy source code diff
def VerifyClangTidySource(compile_commands_dir):
    # Check if clang-tidy is available
    clang_tidy = shutil.which('clang-tidy')
    if clang_tidy is None:
        CPrint('WARNING_MSG', 'WARNING: clang-tidy is not in your PATH; not checking for clang-tidy error.')
        return 0
    # Check if compile commands were generated by cmake
    compile_commands_file = 'compile_commands.json'
    if not os.path.exists(os.path.join(compile_commands_dir, compile_commands_file)):
        CPrint('WARNING_MSG', 'WARNING: can\'t find build directory containing compile_commands.json' +
                           ' (generated by cmake for Makefile and Ninja generators). Not checking' +
                           ' for clang-tidy errors.')
        return 0
    CPrint('', "\nChecking PR source code for clang-tidy errors:")
    retval = 0
    good_file_pattern = re.compile('.*\\.(cpp|cc|c\+\+|cxx|c|h|hpp)$')
    diff_files_list = [item for item in target_files if good_file_pattern.search(item)]
    retval = 0
    if len(diff_files_list) > 0:
        clang_tidy_process = subprocess.Popen([clang_tidy, '-p', compile_commands_dir, '--fix-errors'] + diff_files_list, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
        clang_tidy_process.communicate()
        git_diff = subprocess.check_output(['git', 'diff', '--'] + diff_files_list)
        if len(git_diff) > 0:
            CPrint('ERR_MSG', "\nFound clang-tidy errors!")
            CPrint('CONTENT', "\n" + git_diff.decode('utf-8'))
            retval = 1
        else:
            CPrint('SUCCESS_MSG', "\nThe modified source code in PR has no clang-tidy errors.\n\n")
    return retval
#
#
...
def main():
    ...
    parser.add_argument('--build-dir', metavar='BUILD_DIR', type=str, dest='build_dir', help='Build directory containing '
            + 'compile_commands.json (default = "build")', default='build')
    ...
    clang_tidy_failure = VerifyClangTidySource(args.build_dir)

    if clang_format_failure or copyright_failure or commit_msg_failure or clang_tidy_failure:
