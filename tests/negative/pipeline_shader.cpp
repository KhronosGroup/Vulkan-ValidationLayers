...
TEST_F(VkLayerTest, TestCreatingPipelineWithShaderTileImage) {
    ...
    if (shader_tile_image_features.shaderTileImageDepthReadAccess) {
        char const *fsSource = R"glsl(
            ...
            layout(early_fragment_tests) in;
            layout(location = 0) out vec4 depth_output;
            void main() {
                highp float depth = depthAttachmentReadEXT();
                depth_output = vec4(depth);
            }
        )glsl";
    ...
    if (shader_tile_image_features.shaderTileImageStencilReadAccess) {
        char const *fsSource = R"glsl(
            ...
            #extension GL_EXT_shader_tile_image : require
            layout(early_fragment_tests) in;
            layout(location = 0) out vec4 stencil_output;
            void main()
            {
               uint stencil = stencilAttachmentReadEXT();
               stencil_output = vec4(float(stencil));
            }
        )glsl";
    ...
    if (shader_tile_image_features.shaderTileImageColorReadAccess) {
        char const *fsSource = R"glsl(
            #version 450
            #extension GL_EXT_shader_tile_image : require
            layout(location = 1) tileImageEXT highp attachmentEXT color_f;
            layout(location = 0) out vec4 color_output;
            void main()
            {
                color_output = colorAttachmentReadEXT(color_f);
            }
        )glsl";
