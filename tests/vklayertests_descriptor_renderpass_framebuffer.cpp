...
TEST_F(VkLayerTest, FramebufferCreateErrors) {
...

    {
        if (!push_fragment_density_support || !imageless_framebuffer_support) {
            ...
            rpci_fragment_density_map.pNext = &fragment_density_map_create_info;
...
TEST_F(VkLayerTest, DepthStencilResolveMode) {
    ...
    depthStencilResolveAttachmentReference.layout = VK_IMAGE_LAYOUT_GENERAL;
    depthStencilResolveAttachmentReference.attachment = 1;
    VkSubpassDescriptionDepthStencilResolveKHR subpassDescriptionDSR = {};
    subpassDescriptionDSR.sType = VK_STRUCTURE_TYPE_SUBPASS_DESCRIPTION_DEPTH_STENCIL_RESOLVE_KHR;
    ...
    // test invalid VkResolveModeFlagBits
    {
        ...
        subpassDescriptionDSR.stencilResolveMode = VK_RESOLVE_MODE_FLAG_BITS_MAX_ENUM;  // ignored
        attachmentDescriptions[0].format = depthFormat;
        attachmentDescriptions[1].format = depthFormat;
        m_errorMonitor->ExpectSuccess();
        ASSERT_VK_SUCCESS(vkCreateRenderPass2KHR(m_device->device(), &renderPassCreateInfo, nullptr, &renderPass));
        vk::DestroyRenderPass(m_device->device(), renderPass, nullptr);
        m_errorMonitor->VerifyNotFound();
        ...
        subpassDescriptionDSR.depthResolveMode = VK_RESOLVE_MODE_FLAG_BITS_MAX_ENUM;  // ignored
