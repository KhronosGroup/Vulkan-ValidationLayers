...
TEST_F(VkLayerTest, DeviceCoherentMemoryDisabledAMD) {
    ...
    if (!coherent_memory_features_amd.deviceCoherentMemory) {
        GTEST_SKIP() << "device coherent memory amd not supported";
    }
    if (!AreRequiredExtensionsEnabled()) {
        GTEST_SKIP() << RequiredExtensionsNotSupported() << " not supported";
    }
...
TEST_F(VkLayerTest, InvalidExportExternalImageHandleType) {
    ...
    auto image_info = LvlInitStruct<VkImageCreateInfo>(&external_info);
    image_info.imageType = VK_IMAGE_TYPE_2D;
    image_info.arrayLayers = 1;
    image_info.extent = {64, 64, 1};
    image_info.format = VK_FORMAT_R8G8B8A8_UNORM;
    ...
    const auto handle_type = static_cast<VkExternalMemoryHandleTypeFlagBits>(1 << LeastSignificantBit(exportable_types));
    exportable_types &= ~handle_type;
    const auto handle_type2 = static_cast<VkExternalMemoryHandleTypeFlagBits>(1 << LeastSignificantBit(exportable_types));
    assert(handle_type != handle_type2);

    external_info.handleTypes = handle_type;
    VkImage image_export = VK_NULL_HANDLE;
    if (vk::CreateImage(device(), &image_info, NULL, &image_export) != VK_SUCCESS) {
    ...
    auto memory = AllocateImageMemory(*this, image_export, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, &export_memory_info);
...
TEST_F(VkLayerTest, BufferMemoryWithUnsupportedExternalHandleType) {
    ...
    auto buffer_info = LvlInitStruct<VkBufferCreateInfo>(&external_buffer_info);
    ...
    VkBuffer buffer;
    if (vk::CreateBuffer(device(), &buffer_info, NULL, &buffer) != VK_SUCCESS) {
        GTEST_SKIP() << "Unable to create a valid external buffer";
    }

    // Create export memory with both supported and unsupported handle types
...
TEST_F(VkLayerTest, BufferMemoryWithNotCompatibleExternalHandleTypes) {
    ...
    // Create export memory with supported but not compatible handle types
...
TEST_F(VkLayerTest, ImageMemoryWithUnsupportedExternalHandleType) {
    ...
    const auto exportable_types =
        FindSupportedExternalMemoryHandleTypes(*this, image_info, VK_EXTERNAL_MEMORY_FEATURE_EXPORTABLE_BIT);
    if (!exportable_types) {
        GTEST_SKIP() << "Unable to find exportable handle type";
    }
    if (exportable_types == AllVkExternalMemoryHandleTypeFlagBits) {
    ...
    VkImage image;
    if (vk::CreateImage(device(), &image_info, NULL, &image) != VK_SUCCESS) {
        GTEST_SKIP() << "Unable to create a valid external image";
    }
...
TEST_F(VkLayerTest, InvalidExportExternalBufferHandleType) {
    ...
    if (vk::CreateBuffer(device(), &buffer_info, NULL, &buffer_export) != VK_SUCCESS) {
        GTEST_SKIP() << "Unable to create a valid external buffer";
    }
    ...
    auto export_memory_info = LvlInitStruct<VkExportMemoryAllocateInfo>();
    export_memory_info.handleTypes = handle_type2;
    auto memory = AllocateBufferMemory(*this, buffer_export, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, &export_memory_info);
    ASSERT_TRUE(memory != VK_NULL_HANDLE);
...
TEST_F(VkLayerTest, QueueBindSparse_InvalidSparseMemoryBindSize) {
...
TEST_F(VkLayerTest, QueueBindSparse_InvalidSparseMemoryBindResourceOffset) {
...
TEST_F(VkLayerTest, QueueBindSparse_InvalidSparseMemoryBindSizeMemoryOffset) {
...
TEST_F(VkLayerTest, InvalidImageCompressionControl) {
    ...
    auto features2 = LvlInitStruct<VkPhysicalDeviceFeatures2KHR>(&image_compression_control);
