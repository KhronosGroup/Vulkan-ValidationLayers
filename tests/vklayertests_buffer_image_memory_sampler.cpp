...
TEST_F(VkLayerTest, ImageBarrierSubpassConflicts) {
    ...
    VkFramebufferCreateInfo fbci = {VK_STRUCTURE_TYPE_FRAMEBUFFER_CREATE_INFO, nullptr, 0, rp_noselfdep, 1, &imageView, 32, 32, 1};
    VkFramebuffer fb;
...
TEST_F(VkLayerTest, CopyImageOverlap) {
    TEST_DESCRIPTION("Try to copy an image on same region.");
    ...
    VkImageObj image_2D_shared_memory{m_device};
    image_2D_shared_memory.init_no_mem(*m_device, image_2D.info());
    image_2D_shared_memory.bind_memory(image_2D.Image::memory(), 0u);
    image_2D_shared_memory.SetLayout(VK_IMAGE_ASPECT_COLOR_BIT, VK_IMAGE_LAYOUT_GENERAL);
    ...
    copy_region.dstOffset = {15, 15, 0};
    copy_region.extent = {16, 16, 1};
    m_errorMonitor->SetDesiredFailureMsg(kErrorBit, "VUID-vkCmdCopyImage-pRegions-00124");
    vk::CmdCopyImage(m_commandBuffer->handle(), image_2D.image(), image_2D.Layout(), image_2D.image(), image_2D.Layout(), 1,
                     &copy_region);
    m_errorMonitor->VerifyFound();
    m_errorMonitor->SetDesiredFailureMsg(kErrorBit, "VUID-vkCmdCopyImage-pRegions-00124");
    vk::CmdCopyImage(m_commandBuffer->handle(), image_2D.image(), image_2D.Layout(), image_2D_shared_memory.image(),
                     image_2D_shared_memory.Layout(), 1, &copy_region);
    m_errorMonitor->VerifyFound();

    m_commandBuffer->end();
}

TEST_F(VkLayerTest, SparseImageOverlapCopy) {
    TEST_DESCRIPTION("Test overlapping sparse images' copy");
    ...
    // Sparse no-residency test
    {
        ...
        // Copy not legal since we won't update the bound device memory for either of the images
