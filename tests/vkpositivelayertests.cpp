...
TEST_F(VkPositiveLayerTest, ShaderAtomicFloat2) {
    ...
    if (DeviceExtensionSupported(gpu(), nullptr, VK_KHR_16BIT_STORAGE_EXTENSION_NAME)) {
        ...
        printf("%s %s Extension not supported, skipping tests\n", kSkipPrefix, VK_KHR_16BIT_STORAGE_EXTENSION_NAME);
        return;
    }
    ...
    auto float16int8_features = LvlInitStruct<VkPhysicalDeviceFloat16Int8FeaturesKHR>(&atomic_float2_features);
    auto storage_16_bit_features = LvlInitStruct<VkPhysicalDevice16BitStorageFeaturesKHR>(&float16int8_features);
    auto features2 = lvl_init_struct<VkPhysicalDeviceFeatures2KHR>(&storage_16_bit_features);
    vkGetPhysicalDeviceFeatures2KHR(gpu(), &features2);
