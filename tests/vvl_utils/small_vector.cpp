...
bool operator!=(const NoDefaultCons& lhs, const NoDefaultCons& rhs) {
    return lhs.x != rhs.x;
}
...
TEST(CustomContainer, SmallVectorAssign) {
    ...
    v_dst = ref_xl;
    v_dst = std::move(v_src);
    ASSERT_TRUE(v_src.empty()); // Since large moves move the large store, the source is empty, but not required by spec
                                // of vector
    ASSERT_TRUE(HaveSameElements(ref_large, v_dst));
