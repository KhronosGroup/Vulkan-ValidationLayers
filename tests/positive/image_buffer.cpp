...
TEST_F(VkPositiveLayerTest, CorrectSparseBufferOverlapCopy) {
...
TEST_F(VkPositiveLayerTest, CorrectSparseImageOverlapBlit) {
    TEST_DESCRIPTION("Test correct non overlapping sparse images' blit");
    ...
    // Sparse residency test
    {
        ...
        // image_sparse2's memory on queue submission, or viceversa
        vk::CmdBlitImage(m_commandBuffer->handle(), image_sparse.handle(), image_sparse.Layout(), image_sparse2.handle(),
                         image_sparse2.Layout(), 1, &blit_info, VK_FILTER_LINEAR);
        m_commandBuffer->end();
