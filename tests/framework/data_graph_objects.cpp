...
void CreateDataGraphPipelineHelper::CreateShaderModule(const char* spirv_source) {
    ...
    pipeline_shader_module_ = vku::InitStructHelper();
    pipeline_shader_module_.module = shader_.handle();
    pipeline_shader_module_.pName = "main";

    vvl::PnextChainAdd(&pipeline_create_info_, &pipeline_shader_module_);
}

// default graph module creation.
// playing with `inserted_line` we can cause different errors
void CreateDataGraphPipelineHelper::CreateGraphShaderModule(const char *inserted_line) {
    std::stringstream ss;
    ss << R"(
    ...
    CreateShaderModule(ss.str().c_str());
}

void CreateDataGraphPipelineHelper::CreateComputeShaderModule() {
    constexpr char const* spirv_source = R"spirv(
; SPIR-V
; Version: 1.6
...
%_ptr_UniformConstant_11 = OpTypePointer UniformConstant %11
    ...
    CreateShaderModule(spirv_source);
}

void CreateDataGraphPipelineHelper::InitPipelineResources(const std::vector<vkt::Tensor*>& tensors, VkDescriptorType desc_type,
    ...
    pipeline_create_info_.resourceInfoCount = resources_.size();
    pipeline_create_info_.pResourceInfos = resources_.data();
...
void CreateDataGraphPipelineHelper::CreatePipelineLayout(const std::vector<VkPushConstantRange>& push_constant_ranges) {
    ...
    pipeline_create_info_.layout = pipeline_layout_;
}
...
void CreateDataGraphPipelineHelper::InitTensor(vkt::Tensor &tensor, vkt::TensorView &tensor_view, const std::vector<int64_t> &tensor_dims, bool is_protected) {
...
CreateDataGraphPipelineHelper::CreateDataGraphPipelineHelper(VkLayerTest& test, bool create_valid, bool protected_tensors, const char *inserted_line) : layer_test_(test) {
    device_ = layer_test_.DeviceObj();

    pipeline_create_info_ = vku::InitStructHelper();
    ...
    if (!create_valid) {
        ...
        CreateGraphShaderModule(inserted_line);
...
VkResult CreateDataGraphPipelineHelper::CreateDataGraphPipeline() {
    return vk::CreateDataGraphPipelinesARM(device_->handle(), VK_NULL_HANDLE, VK_NULL_HANDLE, 1, &pipeline_create_info_, nullptr,
                                           &pipeline_);
}

void CreateDataGraphPipelineHelper::Destroy() {
...
CreateDataGraphPipelineHelper::~CreateDataGraphPipelineHelper() { Destroy(); }
}  // namespace dg
}  // namespace vkt
