...
//
bool GLSLtoSPV(const VkPhysicalDeviceLimits& device_limits,
               const VkShaderStageFlagBits shader_type,
               const char* p_shader,
               std::vector<uint32_t>& spirv,
               const spv_target_env spv_env) {
...
VkShaderObj::VkShaderObj(VkRenderFramework* framework,
                         const char* source,
                         ...
                         const void* pNext) :
    m_device(*(framework->DeviceObj())),
    m_source(source), m_spv_env(env) {
...
// creation due to supplying an invalid/unknown SPIR-V capability/operation. This is called after VkShaderObj creation
// when tests are found to crash on a CI device
...
// static
std::unique_ptr<VkShaderObj> VkShaderObj::CreateFromGLSL(VkRenderFramework* framework,
                                                         const char* source,
                                                         VkShaderStageFlagBits stage,
                                                         const spv_target_env spv_env,
                                                         const VkSpecializationInfo* spec_info,
    ...
    auto shader =
        std::make_unique<VkShaderObj>(framework, source, stage, spv_env, SPV_SOURCE_GLSL_TRY, spec_info, entry_point);
...
// static
std::unique_ptr<VkShaderObj> VkShaderObj::CreateFromASM(VkRenderFramework* framework,
                                                        const char* source,
                                                        VkShaderStageFlagBits stage,
                                                        const spv_target_env spv_env,
                                                        const VkSpecializationInfo* spec_info,
                                                        const char* entry_point) {
    auto shader =
        std::make_unique<VkShaderObj>(framework, source, stage, spv_env, SPV_SOURCE_ASM_TRY, spec_info, entry_point);
