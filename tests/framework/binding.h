...
std::vector<Dst> MakeVkHandles(const std::vector<Src*>& v) {
    std::vector<Dst> handles;
    handles.reserve(v.size());
    std::transform(v.begin(), v.end(), std::back_inserter(handles), [](const Src* o) {
        return (o) ? o->handle() : VK_NULL_HANDLE;
    });
    return handles;
}
...
class PhysicalDevice : public internal::Handle<VkPhysicalDevice> {
  public:
    explicit PhysicalDevice(VkPhysicalDevice phy) :
        Handle(phy), properties_(Properties()), limits_(properties_.limits), memory_properties_(MemoryProperties()),
    ...
    bool SetMemoryType(const uint32_t type_bits,
...
class Device : public internal::Handle<VkDevice> {
  public:
    ...
    void init(std::vector<const char*>& extensions,
              VkPhysicalDeviceFeatures* features = nullptr,
              void* create_device_pnext = nullptr,
              bool all_queue_count = false); // all queues, all extensions, etc
    void init() {
    ...
    static VkWriteDescriptorSet WriteDescriptorSet(const DescriptorSet& set,
                                                   uint32_t binding,
    ...
    static VkWriteDescriptorSet WriteDescriptorSet(const DescriptorSet& set,
                                                   ...
                                                   VkDescriptorType type,
                                                   uint32_t count,
                                                   const VkBufferView* buffer_views);
    static VkWriteDescriptorSet WriteDescriptorSet(const DescriptorSet& set,
                                                   uint32_t binding,
                                                   uint32_t array_element,
                                                   VkDescriptorType type,
...
class DeviceMemory : public internal::NonDispHandle<VkDeviceMemory> {
  public:
    ...
    static VkMemoryAllocateInfo GetResourceAllocInfo(const Device& dev,
...
class Fence : public internal::NonDispHandle<VkFence> {
  public:
    ...
    VkResult
    ImportHandle(HANDLE win32_handle, VkExternalFenceHandleTypeFlagBits handle_type, VkFenceImportFlags flags = 0);
#endif
...
class Semaphore : public internal::NonDispHandle<VkSemaphore> {
  public:
    ...
    VkResult ImportHandle(HANDLE win32_handle,
    ...
    VkResult
    ImportHandle(int fd_handle, VkExternalSemaphoreHandleTypeFlagBits handle_type, VkSemaphoreImportFlags flags = 0);
};
...
class Event : public internal::NonDispHandle<VkEvent> {
  public:
    ...
    void CmdWait(const CommandBuffer& cmd,
...
struct Signal {
    ...
    explicit Signal(const Semaphore& semaphore,
...
struct TimelineWait {
    ...
    explicit TimelineWait(const Semaphore& semaphore,
                          uint64_t value,
                          VkPipelineStageFlags2 stage_mask = VK_PIPELINE_STAGE_2_ALL_COMMANDS_BIT) :
        semaphore(semaphore),
        value(value), stage_mask(stage_mask) {}
};
...
struct TimelineSignal {
    ...
    explicit TimelineSignal(const Semaphore& semaphore,
                            uint64_t value,
                            VkPipelineStageFlags2 stage_mask = VK_PIPELINE_STAGE_2_ALL_COMMANDS_BIT) :
        semaphore(semaphore),
        value(value), stage_mask(stage_mask) {}
...
class Queue : public internal::Handle<VkQueue> {
  public:
    ...
    void SetName(const Device& device, const char* name) {
        Handle<VkQueue>::SetName(device.handle(), VK_OBJECT_TYPE_QUEUE, name);
    }
    ...
    VkResult
    Submit2(const CommandBuffer& cmd, const Signal& signal, const Fence& fence = no_fence, bool use_khr = false);
    ...
    VkResult
    Submit2(const CommandBuffer& cmd, const TimelineWait& wait, const Fence& fence = no_fence, bool use_khr = false);
    VkResult Submit2(const CommandBuffer& cmd,
                     const TimelineSignal& signal,
                     const Fence& fence = no_fence,
                     bool use_khr = false);
    ...
    VkResult
    Submit2(const CommandBuffer& cmd, const Wait& wait, const TimelineSignal& signal, const Fence& fence = no_fence);
    VkResult
    Submit2(const CommandBuffer& cmd, const TimelineWait& wait, const Signal& signal, const Fence& fence = no_fence);
...
class Buffer : public internal::NonDispHandle<VkBuffer> {
  public:
    ...
    explicit Buffer(const Device& dev,
    ...
    void InitHostVisibleWithData(const Device& dev,
                                 VkBufferUsageFlags usage,
                                 const void* data,
                                 size_t data_size,
                                 const vvl::span<uint32_t>& queue_families = {});
    ...
    void
    AllocateAndBindMemory(const Device& dev, VkMemoryPropertyFlags mem_props = 0, void* alloc_info_pnext = nullptr);
    ...
    static VkBufferCreateInfo CreateInfo(VkDeviceSize size,
                                         VkFlags usage,
                                         const vvl::span<uint32_t>& queue_families = {},
                                         void* create_info_pnext = nullptr);

    VkBufferMemoryBarrier
    BufferMemoryBarrier(VkFlags output_mask, VkFlags input_mask, VkDeviceSize offset, VkDeviceSize size) const {
    ...
    VkBufferMemoryBarrier2 BufferMemoryBarrier(VkPipelineStageFlags2KHR src_stage,
...
class BufferView : public internal::NonDispHandle<VkBufferView> {
  public:
    ...
    CreateInfo(VkBuffer buffer, VkFormat format, VkDeviceSize offset = 0, VkDeviceSize range = VK_WHOLE_SIZE);
...
BufferView::CreateInfo(VkBuffer buffer, VkFormat format, VkDeviceSize offset, VkDeviceSize range) {
...
class Image : public internal::NonDispHandle<VkImage> {
  public:
    ...
    static VkImageCreateInfo ImageCreateInfo2D(uint32_t const width,
                                               uint32_t const height,
                                               uint32_t const mip_levels,
    ...
    VkImageMemoryBarrier ImageMemoryBarrier(VkFlags output_mask,
                                            VkFlags input_mask,
    ...
    VkImageMemoryBarrier2 ImageMemoryBarrier(VkPipelineStageFlags2KHR src_stage,
                                             VkPipelineStageFlags2KHR dst_stage,
                                             VkAccessFlags2KHR src_access,
                                             VkAccessFlags2KHR dst_access,
                                             VkImageLayout old_layout,
    ...
    void ImageMemoryBarrier(CommandBuffer& cmd,
    ...
    Subresource(const VkImageSubresourceRange& range, uint32_t mip_level, uint32_t array_layer);
    static VkImageSubresourceLayers
    Subresource(VkImageAspectFlags aspect, uint32_t mip_level, uint32_t array_layer, uint32_t array_size);
    static VkImageSubresourceLayers
    Subresource(const VkImageSubresourceRange& range, uint32_t mip_level, uint32_t array_layer, uint32_t array_size);

    VkImageSubresourceRange SubresourceRange(VkImageAspectFlags aspect) const {
        return SubresourceRange(create_info_, aspect);
    }
    static VkImageSubresourceRange SubresourceRange(VkImageAspectFlags aspect_mask,
...
class AccelerationStructureNV : public internal::NonDispHandle<VkAccelerationStructureNV> {
  public:
    explicit AccelerationStructureNV(const Device& dev,
    ...
    [[nodiscard]] Buffer CreateScratchBuffer(const Device& device,
...
class Shader : public internal::NonDispHandle<VkShaderEXT> {
  public:
    ...
    Shader(const Device& dev,
           ...
           const VkPushConstantRange* pushConstRange = nullptr);
    Shader(const Device& dev,
           const VkShaderStageFlagBits stage,
...
class Pipeline : public internal::NonDispHandle<VkPipeline> {
  public:
    ...
    void
    InitDeferred(const Device& dev, const VkRayTracingPipelineCreateInfoKHR& info, VkDeferredOperationKHR deferred_op);
    // vkLoadPipeline()
...
class PipelineLayout : public internal::NonDispHandle<VkPipelineLayout> {
  public:
    ...
    void
    init(const Device& dev, VkPipelineLayoutCreateInfo& info, const std::vector<const DescriptorSetLayout*>& layouts);
...
class DescriptorPool : public internal::NonDispHandle<VkDescriptorPool> {
  public:
    ...
    CreateInfo(VkDescriptorPoolCreateFlags flags, uint32_t max_sets, const PoolSizes& pool_sizes);
...
DescriptorPool::CreateInfo(VkDescriptorPoolCreateFlags flags, uint32_t max_sets, const PoolSizes& pool_sizes) {
...
class DescriptorSet : public internal::NonDispHandle<VkDescriptorSet> {
  public:
    ...
    explicit DescriptorSet(const Device& dev, DescriptorPool* pool, VkDescriptorSet set) :
...
class DescriptorUpdateTemplate : public internal::NonDispHandle<VkDescriptorUpdateTemplate> {
  public:
    ...
    explicit DescriptorUpdateTemplate(const Device& dev, const VkDescriptorUpdateTemplateCreateInfo& info) {
        Init(dev, info);
    }
...
class CommandBuffer : public internal::Handle<VkCommandBuffer> {
  public:
    ...
    void BeginRenderPass(VkRenderPass rp,
                         VkFramebuffer fb,
                         uint32_t render_area_width = 1,
                         uint32_t render_area_height = 1,
...
class Framebuffer : public internal::NonDispHandle<VkFramebuffer> {
  public:
    ...
    Framebuffer(const Device& dev,
                VkRenderPass rp,
                uint32_t attchment_count,
                const VkImageView* attchments,
                uint32_t width = 32,
                uint32_t height = 32) {
...
inline VkBufferCreateInfo Buffer::CreateInfo(VkDeviceSize size,
                                             VkFlags usage,
                                             const vvl::span<uint32_t>& queue_families,
...
Image::Subresource(const VkImageSubresourceRange& range, uint32_t mip_level, uint32_t array_layer) {
...
Image::Subresource(VkImageAspectFlags aspect, uint32_t mip_level, uint32_t array_layer, uint32_t array_size) {
...
inline VkImageSubresourceLayers Image::Subresource(const VkImageSubresourceRange& range,
                                                   uint32_t mip_level,
                                                   uint32_t array_layer,
                                                   uint32_t array_size) {
    return Subresource(
        range.aspectMask, range.baseMipLevel + mip_level, range.baseArrayLayer + array_layer, array_size);
}

inline VkImageSubresourceRange Image::SubresourceRange(VkImageAspectFlags aspect_mask,
                                                       uint32_t base_mip_level,
                                                       uint32_t mip_levels,
                                                       uint32_t base_array_layer,
                                                       uint32_t num_layers) {
...
inline VkWriteDescriptorSet Device::WriteDescriptorSet(const DescriptorSet& set,
                                                       uint32_t binding,
                                                       uint32_t array_element,
                                                       VkDescriptorType type,
                                                       uint32_t count,
...
inline VkWriteDescriptorSet Device::WriteDescriptorSet(const DescriptorSet& set,
    ...
    return WriteDescriptorSet(
        set, binding, array_element, type, static_cast<uint32_t>(image_info.size()), &image_info[0]);
}
...
inline VkWriteDescriptorSet Device::WriteDescriptorSet(const DescriptorSet& set,
                                                       ...
                                                       const std::vector<VkDescriptorBufferInfo>& buffer_info) {
    return WriteDescriptorSet(
        set, binding, array_element, type, static_cast<uint32_t>(buffer_info.size()), &buffer_info[0]);
}
...
inline VkWriteDescriptorSet Device::WriteDescriptorSet(const DescriptorSet& set,
                                                       ...
                                                       const std::vector<VkBufferView>& buffer_views) {
    return WriteDescriptorSet(
        set, binding, array_element, type, static_cast<uint32_t>(buffer_views.size()), &buffer_views[0]);
}
...
class IndirectCommandsLayout : public internal::NonDispHandle<VkIndirectCommandsLayoutEXT> {
  public:
    ...
    explicit IndirectCommandsLayout(const Device& dev, const VkIndirectCommandsLayoutCreateInfoEXT& info) {
        Init(dev, info);
    }
...
class IndirectExecutionSet : public internal::NonDispHandle<VkIndirectExecutionSetEXT> {
  public:
    ...
    explicit IndirectExecutionSet(const Device& dev, const VkIndirectExecutionSetCreateInfoEXT& info) {
        Init(dev, info);
    }
