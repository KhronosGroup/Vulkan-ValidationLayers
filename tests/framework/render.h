...
class VkRenderFramework : public VkTestFramework {
  public:
    ...
    VkResult
    CreateSurface(SurfaceContext& surface_context, vkt::Surface& surface, VkInstance custom_instance = VK_NULL_HANDLE);
    SurfaceInformation GetSwapchainInfo(const VkSurfaceKHR surface);
    vkt::Swapchain CreateSwapchain(VkSurfaceKHR surface,
    ...
    void SetDefaultDynamicStatesExclude(const std::vector<VkDynamicState>& exclude = {},
    ...
    bool DeviceExtensionSupported(VkPhysicalDevice,
                                  const char*,
                                  const char* name,
    ...
    // Add a feature required for the test to be executed. The currently targeted API version is used to add the correct
    // struct, so be sure to call SetTargetApiVersion before. Only features added with this or the AddOptionalFeature
    // method will be enabled.
    void AddRequiredFeature(vkt::Feature feature);
    // Add an optional feature for the test to be executed. The currently targeted API version is used to add the
    // correct struct, so be sure to call SetTargetApiVersion before Only features added with this or the
    // AddRequiredFeature method will be enabled.
    void AddOptionalFeature(vkt::Feature feature);

    std::vector<uint32_t>
    GLSLToSPV(VkShaderStageFlagBits stage, const char* code, const spv_target_env env = SPV_ENV_VULKAN_1_0);
  ...
  private:
    ...
    // Add ext_name, the names of all instance extensions required by ext_name, and return true if ext_name is
    // supported. If the extension is not supported, no extension names are added for instance creation. `ext_name` can
    // refer to a device or instance extension.
    bool AddRequestedInstanceExtensions(const char* ext_name);
    // Returns true if the instance extension is promoted to core in the target API version requested using
    // SetTargetApiVersion().
    ...
    // `AddRequired*Extensions` calls and InitFramework has been called. `inst_ext_name` must be an instance extension
    // name; false is returned for all device extension names. This function also returns true if the instance extension
    // is implicitly supported in the target API version requested using SetTargetApiVersion().
    bool CanEnableInstanceExtension(const std::string& inst_ext_name) const;
    // Add dev_ext_name, then names of _device_ extensions required by dev_ext_name, and return true if dev_ext_name is
    // supported. If the extension is not supported, no extension names are added for device creation. This function has
    // no effect if dev_ext_name refers to an instance extension.
    ...
    // Returns true if the device extension is enabled. This call is only valid _after_ previous
    // `AddRequired*Extensions` calls and InitFramework has been called. `dev_ext_name` must be an instance extension
    // name; false is returned for all instance extension names. This function also returns true if the device extension
    // is implicitly supported by the API version supported by the device, as queriable using DeviceValidationVersion().
