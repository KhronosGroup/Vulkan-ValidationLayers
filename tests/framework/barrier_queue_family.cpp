...
void BarrierQueueFamilyBase::QueueFamilyObjs::Init(vkt::Device* device,
                                                   uint32_t qf_index,
                                                   VkQueue qf_queue,
...
void BarrierQueueFamilyTestHelper::Init(std::vector<uint32_t>* families, bool image_memory, bool buffer_memory) {
    ...
    auto image_ci = vkt::Image::ImageCreateInfo2D(32,
                                                  ...
                                                  families ? vvl::make_span(families->data(), families->size())
                                                           : vvl::span<uint32_t>{});
    if (image_memory) {
    ...
    image_barrier_ = image_.ImageMemoryBarrier(VK_ACCESS_TRANSFER_READ_BIT,
                                               VK_ACCESS_TRANSFER_READ_BIT,
                                               image_.Layout(),
                                               image_.Layout(),
                                               image_.SubresourceRange(VK_IMAGE_ASPECT_COLOR_BIT, 0, 1, 0, 1));

    VkMemoryPropertyFlags mem_prop = VK_MEMORY_PROPERTY_HOST_COHERENT_BIT;
    auto buffer_ci =
        vkt::Buffer::CreateInfo(256,
    ...
    buffer_barrier_ =
        buffer_.BufferMemoryBarrier(VK_ACCESS_TRANSFER_READ_BIT, VK_ACCESS_TRANSFER_READ_BIT, 0, VK_WHOLE_SIZE);
}
...
void Barrier2QueueFamilyTestHelper::Init(bool image_memory, bool buffer_memory) {
    ...
    image_barrier_ = image_.ImageMemoryBarrier(VK_PIPELINE_STAGE_TRANSFER_BIT,
    ...
    ASSERT_TRUE(buffer_.initialized());
    buffer_barrier_ = buffer_.BufferMemoryBarrier(VK_PIPELINE_STAGE_TRANSFER_BIT,
                                                  VK_PIPELINE_STAGE_TRANSFER_BIT,
                                                  VK_ACCESS_TRANSFER_READ_BIT,
                                                  VK_ACCESS_TRANSFER_READ_BIT,
                                                  0,
...
void BarrierQueueFamilyTestHelper::operator()(const std::string& img_err,
    ...
    for (int cb_repeat = 0; cb_repeat < (mod == Modifier::DOUBLE_COMMAND_BUFFER ? 2 : 1); cb_repeat++) {
        ...
        for (int repeat = 0; repeat < (mod == Modifier::DOUBLE_RECORD ? 2 : 1); repeat++) {
            vk::CmdPipelineBarrier(command_buffer->handle(),
                                   VK_PIPELINE_STAGE_TRANSFER_BIT,
                                   VK_PIPELINE_STAGE_TRANSFER_BIT,
...
void Barrier2QueueFamilyTestHelper::operator()(const std::string& img_err,
                                               const std::string& buf_err,
                                               uint32_t src,
                                               uint32_t dst,
                                               uint32_t queue_family_index,
...
void ValidOwnershipTransferOp(ErrorMonitor* monitor,
    ...
    vk::CmdPipelineBarrier(
        cb.handle(), src_stages, dst_stages, 0, 0, nullptr, num_buf_barrier, buf_barrier, num_img_barrier, img_barrier);
    cb.End();
    queue->Submit(cb);
    queue->Wait();
}
...
void ValidOwnershipTransfer(ErrorMonitor* monitor,
    ...
    ValidOwnershipTransferOp(
        monitor, queue_from, cb_from, src_stages, VK_PIPELINE_STAGE_BOTTOM_OF_PIPE_BIT, buf_barrier, img_barrier);
    ValidOwnershipTransferOp(
        monitor, queue_to, cb_to, VK_PIPELINE_STAGE_TOP_OF_PIPE_BIT, dst_stages, buf_barrier, img_barrier);
}
...
void ValidOwnershipTransfer(ErrorMonitor* monitor,
    ...
    ValidOwnershipTransferOp(monitor,
                             queue_from,
                             cb_from,
                             buf_barrier ? &fixup_buf_barrier : nullptr,
