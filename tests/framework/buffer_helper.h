...
Buffer VertexBuffer(const Device& dev, const std::vector<VertexT>& vertices) {
    vkt::Buffer vertex_buffer(
        dev, vertices.size() * sizeof(VertexT), VK_BUFFER_USAGE_VERTEX_BUFFER_BIT, VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT);
...
Buffer IndexBuffer(const Device& dev, const std::vector<IndexT>& indices) {
    vkt::Buffer index_buffer(
        dev, indices.size() * sizeof(IndexT), VK_BUFFER_USAGE_INDEX_BUFFER_BIT, VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT);
...
Buffer IndirectBuffer(const Device& dev, const std::vector<IndirectCmdT>& indirect_cmds) {
    vkt::Buffer indirect_buffer(dev,
                                indirect_cmds.size() * sizeof(IndirectCmdT),
