...
// GTest has a GTEST_SKIP macro, but the test can't be actually "skipped" unless you are at the top function in the
// test. The macro will call the function and then call 'return' if a skip has been called inside
...
inline std::ostream& operator<<(std::ostream& os, const VkResult& result) {
    return os << string_VkResult(result);
}
