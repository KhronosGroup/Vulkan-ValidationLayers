...
TEST_F(NegativeGpuAVDescriptorClassGeneralBuffer, ReachMaxActionsCommandValidationLimit) {
    ...
    VkShaderObj vs_1(*m_device, vs_source_1, VK_SHADER_STAGE_VERTEX_BIT);
    ...
    VkShaderObj vs_2(*m_device, vs_source_2, VK_SHADER_STAGE_VERTEX_BIT);
    ...
    *data = 8;
    ...
    m_errorMonitor->SetDesiredError("GPUAV-Overflow-Unknown", 2);  // 2 descriptor accesses
    ...
    const char vs_source[] = R"glsl(
        ...
        void main() {
            ...
            if (u_index.index[0] == 1) {
                Data.data[0] = Null.data[40]; // won't crash with nullDescriptor
...
TEST_F(NegativeGpuAVDescriptorClassGeneralBuffer, DISABLED_TessellationControl) {
    ...
    VkShaderObj vs(*m_device, kVertexDrawPassthroughGlsl, VK_SHADER_STAGE_VERTEX_BIT);
    VkShaderObj tcs(*m_device, shader_source, VK_SHADER_STAGE_TESSELLATION_CONTROL_BIT);
...
TEST_F(NegativeGpuAVDescriptorClassGeneralBuffer, DISABLED_TessellationEvaluation) {
    ...
    VkShaderObj tes(*m_device, shader_source, VK_SHADER_STAGE_TESSELLATION_EVALUATION_BIT);
...
TEST_F(NegativeGpuAVDescriptorClassGeneralBuffer, GPLOneLibraryTwoLinkedPipelines) {
    TEST_DESCRIPTION("Use one library to link two pipelines - second one should be instrumented");
    AddRequiredExtensions(VK_EXT_GRAPHICS_PIPELINE_LIBRARY_EXTENSION_NAME);
    AddRequiredFeature(vkt::Feature::graphicsPipelineLibrary);
    AddRequiredFeature(vkt::Feature::vertexPipelineStoresAndAtomics);
...
TEST_F(NegativeGpuAVDescriptorClassGeneralBuffer, GPLOneLibraryTwoLinkedPipelinesOneDestroyed) {
    TEST_DESCRIPTION(
