...
TEST_F(NegativeDebugPrintf, Int64Vector) {
    ...
    BasicComputeTest(shader_source,
                     "vector of ul 2000000000000001, 2000000000000001, 2000000000000001, 2000000000000001");
}
...
TEST_F(NegativeDebugPrintf, MultiDraw) {
    ...
    vk::CmdDrawMultiIndexedEXT(
        m_command_buffer.handle(), 3, multi_draw_indices, 1, 0, sizeof(VkMultiDrawIndexedInfoEXT), 0);
    m_command_buffer.EndRenderPass();
    m_command_buffer.End();
...
TEST_F(NegativeDebugPrintf, GPLFragment) {
    ...
    OneOffDescriptorSet fragment_set(
        m_device, { { 0, VK_DESCRIPTOR_TYPE_STORAGE_BUFFER, 1, VK_SHADER_STAGE_FRAGMENT_BIT, nullptr } });
...
TEST_F(NegativeDebugPrintf, GPLFragmentIndependentSets) {
    ...
    // Independent sets
    const vkt::PipelineLayout pipeline_layout_vs(
        *m_device, { &vertex_set.layout_, nullptr }, {}, VK_PIPELINE_LAYOUT_CREATE_INDEPENDENT_SETS_BIT_EXT);
    const auto vs_layout = pipeline_layout_vs.handle();
    const vkt::PipelineLayout pipeline_layout_fs(
        *m_device, { nullptr, &fragment_set.layout_ }, {}, VK_PIPELINE_LAYOUT_CREATE_INDEPENDENT_SETS_BIT_EXT);
    const auto fs_layout = pipeline_layout_fs.handle();
    const vkt::PipelineLayout pipeline_layout(*m_device,
                                              { &vertex_set.layout_, &fragment_set.layout_ },
                                              {},
...
TEST_F(NegativeDebugPrintf, ShaderObjectFragment) {
    ...
    const vkt::Shader vert_shader(
        *m_device, VK_SHADER_STAGE_VERTEX_BIT, GLSLToSPV(VK_SHADER_STAGE_VERTEX_BIT, kVertexDrawPassthroughGlsl));
    const vkt::Shader frag_shader(
        *m_device, VK_SHADER_STAGE_FRAGMENT_BIT, GLSLToSPV(VK_SHADER_STAGE_FRAGMENT_BIT, fs_source));
...
TEST_F(NegativeDebugPrintf, ShaderObjectCompute) {
    ...
    const vkt::Shader comp_shader(
        *m_device, VK_SHADER_STAGE_COMPUTE_BIT, GLSLToSPV(VK_SHADER_STAGE_COMPUTE_BIT, cs_source));
...
TEST_F(NegativeDebugPrintf, SetupErrorVersion) {
    ...
    VkValidationFeatureEnableEXT enables[] = { VK_VALIDATION_FEATURE_ENABLE_DEBUG_PRINTF_EXT };
    VkValidationFeatureDisableEXT disables[] = { VK_VALIDATION_FEATURE_DISABLE_THREAD_SAFETY_EXT,
...
%_ptr_Input_uint = OpTypePointer Input %uint
    ...
    CreateComputePipelineHelper pipe(*this);
    pipe.cs_ = std::make_unique<VkShaderObj>(
        this, shader_source, VK_SHADER_STAGE_COMPUTE_BIT, SPV_ENV_VULKAN_1_3, SPV_SOURCE_ASM, &specialization_info);
    pipe.CreateComputePipeline();
...
TEST_F(NegativeDebugPrintf, DISABLED_UseAllDescriptorSlotsShaderObjectReserved) {
    ...
    {
        m_errorMonitor->SetDesiredWarning("This Shader Object has too many descriptor sets that will not allow GPU "
                                          "shader instrumentation to be setup for "
                                          "VkShaderEXT created with it");

        const vkt::Shader comp_shader(
            *m_device, ShaderCreateInfo(cs_spirv, VK_SHADER_STAGE_COMPUTE_BIT, set_limit, layouts.data()));
        m_errorMonitor->VerifyFound();
    ...
    {
        const vkt::Shader comp_shader(
            *m_device, ShaderCreateInfo(cs_spirv, VK_SHADER_STAGE_COMPUTE_BIT, set_limit - 1, layouts.data()));
...
TEST_F(NegativeDebugPrintf, DualCommandBufferBothPrint) {
    ...
    cb0.Begin();
    vk::CmdPushConstants(
        cb0.handle(), pipeline_layout.handle(), VK_SHADER_STAGE_COMPUTE_BIT, 0, sizeof(uint32_t), &data);
    ...
    data = 8;
    vk::CmdPushConstants(
        cb1.handle(), pipeline_layout.handle(), VK_SHADER_STAGE_COMPUTE_BIT, 0, sizeof(uint32_t), &data);
...
TEST_F(NegativeDebugPrintf, DrawIndexedIndirect) {
    ...
    vkt::Buffer indirect_buffer(
        *m_device, sizeof(VkDrawIndexedIndirectCommand), VK_BUFFER_USAGE_INDIRECT_BUFFER_BIT, kHostVisibleMemProps);
    ...
    vk::CmdDrawIndexedIndirect(
        m_command_buffer.handle(), indirect_buffer.handle(), 0, 1, sizeof(VkDrawIndexedIndirectCommand));
    m_command_buffer.EndRenderPass();
    m_command_buffer.End();
...
TEST_F(NegativeDebugPrintf, SpecConstant) {
    ...
    pipe_44.cs_ = std::make_unique<VkShaderObj>(
        this, shader_source, VK_SHADER_STAGE_COMPUTE_BIT, SPV_ENV_VULKAN_1_0, SPV_SOURCE_GLSL, &spec_info_44);
    pipe_44.CreateComputePipeline();

    CreateComputePipelineHelper pipe_88(*this);
    pipe_88.cs_ = std::make_unique<VkShaderObj>(
        this, shader_source, VK_SHADER_STAGE_COMPUTE_BIT, SPV_ENV_VULKAN_1_0, SPV_SOURCE_GLSL, &spec_info_88);
