...
TEST_F(NegativeRayTracingMicromap, CaptureReplayFeatureDisableEXT) {
    ...
    maCreateInfo.createFlags = VK_MICROMAP_CREATE_DEVICE_ADDRESS_CAPTURE_REPLAY_BIT_EXT;
    maCreateInfo.buffer = buffer;
    maCreateInfo.offset = 0;
    maCreateInfo.size = 0;
    maCreateInfo.type = VK_MICROMAP_TYPE_OPACITY_MICROMAP_EXT;
...
TEST_F(NegativeRayTracingMicromap, CmdBuildAccelerationStructureInvalidFlags) {
...

    {
        ...
        vk::CmdBuildAccelerationStructuresKHR(m_command_buffer, 1, &build_info, &pRangeInfo);
        m_errorMonitor->VerifyFound();
        m_command_buffer.End();
    }
...
TEST_F(NegativeRayTracingMicromap, CmdBuildAccelerationStructureGeometry) {
    ...
    vkt::Buffer triangle_buffer(*m_device, 4096,
                                VK_BUFFER_USAGE_ACCELERATION_STRUCTURE_BUILD_INPUT_READ_ONLY_BIT_KHR |
                                    VK_BUFFER_USAGE_SHADER_DEVICE_ADDRESS_BIT | VK_BUFFER_USAGE_STORAGE_BUFFER_BIT,
                                VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT, &alloc_flags_info);
    ...
    std::array<VkAccelerationStructureGeometryKHR, 2> geometryArray = {geometry, geometry};
...

    {
        ...
        build_info.pGeometries = geometryArray.data();
...
TEST_F(NegativeRayTracingMicromap, CmdBuildAccelerationStructureTriangleMicromap) {
...

    {
        ...
        triangleMicromap.indexBuffer = micromap_buffer.Address();
        triangleMicromap.indexStride = 0;
        triangleMicromap.micromap = NULL;
        geometry.geometry.triangles.pNext = &triangleMicromap;
        build_info.pGeometries = &geometry;
...

    {
        ...
        geometry.geometry.triangles.vertexData.deviceAddress = triangle_buffer.Address();
        geometry.geometry.triangles.vertexFormat = VK_FORMAT_R32G32B32_SFLOAT;
        triangleMicromap.indexType = VK_INDEX_TYPE_UINT16;
        triangleMicromap.indexBuffer = 0;
        triangleMicromap.indexStride = 1;
        triangleMicromap.micromap = blas3->handle();  // intentionally wrong type
        geometry.geometry.triangles.pNext = &triangleMicromap;
        build_info.pGeometries = &geometry;
...

    {
        ...
        geometry.geometry.triangles.vertexData.deviceAddress = triangle_buffer.Address();
        geometry.geometry.triangles.vertexFormat = VK_FORMAT_R32G32B32_SFLOAT;
        triangleMicromap.indexType = VK_INDEX_TYPE_UINT32;
        triangleMicromap.indexBuffer = micromap_buffer.Address();
        triangleMicromap.indexStride = UINT64_MAX;
        triangleMicromap.micromap = micromap_as->handle();
        geometry.geometry.triangles.pNext = &triangleMicromap;
        build_info.pGeometries = &geometry;
...
TEST_F(NegativeRayTracingMicromap, CmdBuildAccelerationStructureTriangleMicromapIndexValidation) {
    ...
    m_command_buffer.Begin();
    vk::CmdBuildAccelerationStructuresKHR(m_command_buffer, 1, &micromap_build_info, &pRangeNullInfo);
    m_command_buffer.End();
...

    {
        vkt::Buffer small_index_buffer(
            *m_device, 4, VK_BUFFER_USAGE_ACCELERATION_STRUCTURE_STORAGE_BIT_KHR | VK_BUFFER_USAGE_SHADER_DEVICE_ADDRESS_BIT,
            VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT, &alloc_flags_info);
        triangleMicromap = vku::InitStructHelper();
        triangleMicromap.indexType = VK_INDEX_TYPE_UINT32;
        triangleMicromap.indexBuffer = small_index_buffer.Address();
        triangleMicromap.indexStride = 4;
        triangleMicromap.baseTriangle = 0;
        triangleMicromap.micromap = micromap_as->handle();
        geometry.geometry.triangles.pNext = &triangleMicromap;
        m_command_buffer.Begin();
...

    {
        ...
        triangleMicromap.indexType = VK_INDEX_TYPE_NONE_KHR;
        triangleMicromap.indexBuffer = 0;
        triangleMicromap.baseTriangle = 2;
        triangleMicromap.micromap = micromap_as->handle();
        geometry.geometry.triangles.pNext = &triangleMicromap;
        m_command_buffer.Begin();
...
TEST_F(NegativeRayTracingMicromap, CmdBuildAccelerationStructureTriangleMicromapEXT) {
    ...
    geometry.geometry.triangles.pNext = &triangleMicromapEXT;
...
TEST_F(NegativeRayTracingMicromap, CmdBuildAccelerationStructureIndexType) {
    ...
    VkAccelerationStructureTrianglesOpacityMicromapKHR triangleMicromap = vku::InitStructHelper();
    triangleMicromap.indexType = VK_INDEX_TYPE_MAX_ENUM;
    triangleMicromap.indexBuffer = micromap_buffer.Address();
    triangleMicromap.indexStride = 4;
    triangleMicromap.micromap = micromap_as->handle();
    ...
    VkAccelerationStructureGeometryKHR geometry = vku::InitStructHelper();
    geometry.geometryType = VK_GEOMETRY_TYPE_TRIANGLES_KHR;
    geometry.geometry.triangles = vku::InitStructHelper(&triangleMicromap);
    geometry.geometry.triangles.indexType = VK_INDEX_TYPE_NONE_KHR;
    geometry.geometry.triangles.maxVertex = 3;
    geometry.geometry.triangles.transformData.deviceAddress = triangle_buffer.Address();
    geometry.geometry.triangles.vertexData.deviceAddress = triangle_buffer.Address();
...
TEST_F(NegativeRayTracingMicromap, CmdBuildAccelerationStructureUpdateMicromapConsistency) {
    ...
    triangleMicromap.indexType = VK_INDEX_TYPE_NONE_KHR;
    triangleMicromap.micromap = micromap_as->handle();
    triangle_geometry.geometry.triangles.pNext = &triangleMicromap;
    ...
    m_command_buffer.Begin();
    vk::CmdBuildAccelerationStructuresKHR(m_command_buffer, 1, &blas_build_info, &pRangeInfo);
    m_command_buffer.End();
    m_default_queue->Submit(m_command_buffer);
    m_default_queue->Wait();
    ...
    {
        m_command_buffer.Begin();
        triangleMicromap.micromap = VK_NULL_HANDLE;
        triangleMicromap.indexType = VK_INDEX_TYPE_UINT32;
        triangleMicromap.indexBuffer = micromap_buffer.Address();
...

    {
        ...
        m_command_buffer.Begin();
        triangleMicromap.micromap = unbuilt_micromap_as->handle();
        blas_build_info.mode = VK_BUILD_ACCELERATION_STRUCTURE_MODE_UPDATE_KHR;
