...
TEST_F(NegativeGpuAVRayTracing, CmdTraceRaysIndirect) {
    ...
    vkt::Buffer trace_rays_big_width(*m_device,
                                     4096,
    ...
    vkt::Buffer trace_rays_big_height(*m_device,
                                      4096,
                                      VK_BUFFER_USAGE_SHADER_DEVICE_ADDRESS_BIT | VK_BUFFER_USAGE_STORAGE_BUFFER_BIT,
                                      vkt::device_address);
    ...
    vkt::Buffer trace_ray_big_depth(*m_device,
                                    4096,
                                    VK_BUFFER_USAGE_SHADER_DEVICE_ADDRESS_BIT | VK_BUFFER_USAGE_STORAGE_BUFFER_BIT,
                                    vkt::device_address);
...
TEST_F(NegativeGpuAVRayTracing, DISABLED_BasicTraceRaysDeferredBuild) {
    ...
    vkt::Buffer rt_params_buffer(
        *m_device, 4 * sizeof(float), 0, vkt::device_address); // missing space for Tmin and Tmax
    ...
    pipeline.GetDescriptorSet().WriteDescriptorBufferInfo(
        1, uniform_buffer.handle(), 0, 16, VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER);
