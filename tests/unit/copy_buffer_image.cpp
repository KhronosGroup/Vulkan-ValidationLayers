...
TEST_F(NegativeCopyBufferImage, ImageBufferCopy) {
    ...
    missing_rgba_support |=
        (props.bufferFeatures == 0 && props.linearTilingFeatures == 0 && props.optimalTilingFeatures == 0);
    ...
    vk::CmdCopyBufferToImage(
        m_command_buffer.handle(), buffer_64k.handle(), image_64k.handle(), VK_IMAGE_LAYOUT_GENERAL, 1, &region);
    m_errorMonitor->VerifyFound();

    m_errorMonitor->SetDesiredError("VUID-vkCmdCopyImageToBuffer-commandBuffer-recording");
    vk::CmdCopyImageToBuffer(
        m_command_buffer.handle(), image_64k.handle(), VK_IMAGE_LAYOUT_GENERAL, buffer_64k.handle(), 1, &region);
    m_errorMonitor->VerifyFound();
    ...
    // successful copies
    vk::CmdCopyImageToBuffer(
        m_command_buffer.handle(), image_16k.handle(), VK_IMAGE_LAYOUT_GENERAL, buffer_16k.handle(), 1, &region);
    ...
    vk::CmdPipelineBarrier(m_command_buffer.handle(),
                           ...
                           nullptr);
    vk::CmdCopyBufferToImage(
        m_command_buffer.handle(), buffer_16k.handle(), image_16k.handle(), VK_IMAGE_LAYOUT_GENERAL, 1, &region);
    ...
    vk::CmdPipelineBarrier(m_command_buffer.handle(),
                           ...
                           nullptr);
    vk::CmdCopyImageToBuffer(
        m_command_buffer.handle(), image_64k.handle(), VK_IMAGE_LAYOUT_GENERAL, buffer_16k.handle(), 1, &region);
    ...
    vk::CmdPipelineBarrier(m_command_buffer.handle(),
                           ...
                           nullptr);
    vk::CmdCopyImageToBuffer(
        m_command_buffer.handle(), image_16k.handle(), VK_IMAGE_LAYOUT_GENERAL, buffer_64k.handle(), 1, &region);
    ...
    vk::CmdCopyBufferToImage(
        m_command_buffer.handle(), buffer_16k.handle(), image_64k.handle(), VK_IMAGE_LAYOUT_GENERAL, 1, &region);
    m_errorMonitor->VerifyFound();
    ...
    vk::CmdCopyBufferToImage(
        m_command_buffer.handle(), buffer_64k.handle(), image_16k.handle(), VK_IMAGE_LAYOUT_GENERAL, 1, &region);
    m_errorMonitor->VerifyFound();
    ...
    m_errorMonitor->SetDesiredError(
        "VUID-vkCmdCopyImageToBuffer-imageSubresource-07970"); // unavoidable "region exceeds image
    ...
    m_errorMonitor->SetDesiredError(
        "VUID-vkCmdCopyBufferToImage-imageSubresource-07970"); // unavoidable "region exceeds image
                                                               // bounds" for non-existent mip
    vk::CmdCopyBufferToImage(
        m_command_buffer.handle(), buffer_16k.handle(), image_16k.handle(), VK_IMAGE_LAYOUT_GENERAL, 1, &region);
    m_errorMonitor->VerifyFound();
    ...
    vk::CmdCopyImageToBuffer(m_command_buffer.handle(),
                             image_16k.handle(),
...
TEST_F(NegativeCopyBufferImage, ImageBufferCopyDepthStencil) {
    ...
    vkt::Image image_16k_depth(*m_device,
                               64,
                               64,
                               1,
    ...
    vkt::Image ds_image_4D_1S(*m_device,
                              256,
                              256,
                              1,
    ...
    vkt::Image ds_image_3D_1S(*m_device,
                              256,
                              256,
                              1,
                              VK_FORMAT_D24_UNORM_S8_UINT,
    ...
    vkt::Image ds_image_2D(*m_device,
                           256,
                           256,
                           1,
                           VK_FORMAT_D16_UNORM,
    ...
    vkt::Image ds_image_1S(*m_device,
                           256,
                           256,
                           1,
                           VK_FORMAT_S8_UINT,
...
TEST_F(NegativeCopyBufferImage, ImageBufferCopyCompression) {
    ...
    if (device_features.textureCompressionBC && (!missing_bc_support)) {
        ...
        image_16k_4x4comp.Init(
            *m_device, 128, 128, 1, VK_FORMAT_ETC2_R8G8B8A8_UNORM_BLOCK, VK_IMAGE_USAGE_TRANSFER_SRC_BIT);
        image_16k_4x4comp.SetLayout(VK_IMAGE_LAYOUT_GENERAL);
        image_NPOT_4x4comp.Init(
            *m_device, 130, 130, 1, VK_FORMAT_ETC2_R8G8B8A8_UNORM_BLOCK, VK_IMAGE_USAGE_TRANSFER_SRC_BIT);
        image_NPOT_4x4comp.SetLayout(VK_IMAGE_LAYOUT_GENERAL);
        ...
        image_16k_4x4comp.SetLayout(VK_IMAGE_LAYOUT_GENERAL);
        image_NPOT_4x4comp.Init(
            *m_device, 130, 130, 1, VK_FORMAT_ASTC_4x4_UNORM_BLOCK, VK_IMAGE_USAGE_TRANSFER_SRC_BIT);
        image_NPOT_4x4comp.SetLayout(VK_IMAGE_LAYOUT_GENERAL);
    } else {
...
TEST_F(NegativeCopyBufferImage, ImageBufferCopyMultiPlanar) {
    ...
    // Try to use G8_B8R8_2PLANE_420_UNORM because need 2-plane format for some tests and likely supported due to copy
    // support being required with samplerYcbcrConversion feature
    ...
    vkt::Image image_multi_planar(
        *m_device, 128, 128, 1, VK_FORMAT_G8_B8R8_2PLANE_420_UNORM, VK_IMAGE_USAGE_TRANSFER_DST_BIT);
...
TEST_F(NegativeCopyBufferImage, ImageLayerCountMismatch) {
    TEST_DESCRIPTION("Try to copy between images with the source subresource having a different layerCount than the "
    ...
    const char* vuid =
        (maintenance1 == true) ? "VUID-vkCmdCopyImage-srcImage-08793" : "VUID-VkImageCopy-apiVersion-07941";
...
TEST_F(NegativeCopyBufferImage, CompressedImageMip) {
    ...
    auto image_ci = vkt::Image::ImageCreateInfo2D(
        32, 32, 6, 1, VK_FORMAT_BC3_SRGB_BLOCK, VK_IMAGE_USAGE_TRANSFER_SRC_BIT | VK_IMAGE_USAGE_TRANSFER_DST_BIT);
    ...
    region.imageSubresource.mipLevel = 0;
    vk::CmdCopyImageToBuffer(
        m_command_buffer.handle(), image.handle(), VK_IMAGE_LAYOUT_GENERAL, buffer_1024.handle(), 1, &region);
    ...
    vk::CmdPipelineBarrier(m_command_buffer.handle(),
                           ...
                           nullptr);
    vk::CmdCopyBufferToImage(
        m_command_buffer.handle(), buffer_1024.handle(), image.handle(), VK_IMAGE_LAYOUT_GENERAL, 1, &region);

    // Mip 2 should fit in 64b buffer - 64 texels @ 1b each
    region.imageExtent = { 8, 8, 1 };
    region.imageSubresource.mipLevel = 2;
    vk::CmdCopyImageToBuffer(
        m_command_buffer.handle(), image.handle(), VK_IMAGE_LAYOUT_GENERAL, buffer_64.handle(), 1, &region);
    ...
    vk::CmdPipelineBarrier(m_command_buffer.handle(),
                           ...
                           nullptr);
    vk::CmdCopyBufferToImage(
        m_command_buffer.handle(), buffer_64.handle(), image.handle(), VK_IMAGE_LAYOUT_GENERAL, 1, &region);

    // Mip 3 should fit in 16b buffer - 16 texels @ 1b each
    region.imageExtent = { 4, 4, 1 };
    region.imageSubresource.mipLevel = 3;
    vk::CmdCopyImageToBuffer(
        m_command_buffer.handle(), image.handle(), VK_IMAGE_LAYOUT_GENERAL, buffer_16.handle(), 1, &region);
    ...
    vk::CmdPipelineBarrier(m_command_buffer.handle(),
                           ...
                           nullptr);
    vk::CmdCopyBufferToImage(
        m_command_buffer.handle(), buffer_16.handle(), image.handle(), VK_IMAGE_LAYOUT_GENERAL, 1, &region);
    ...
    m_errorMonitor->SetDesiredError("VUID-vkCmdCopyImageToBuffer-pRegions-00183");
    vk::CmdCopyImageToBuffer(
        m_command_buffer.handle(), image.handle(), VK_IMAGE_LAYOUT_GENERAL, buffer_8.handle(), 1, &region);
    m_errorMonitor->VerifyFound();
    m_errorMonitor->SetDesiredError("VUID-vkCmdCopyBufferToImage-pRegions-00171");
    vk::CmdCopyBufferToImage(
        m_command_buffer.handle(), buffer_8.handle(), image.handle(), VK_IMAGE_LAYOUT_GENERAL, 1, &region);
    m_errorMonitor->VerifyFound();
    ...
    vk::CmdPipelineBarrier(m_command_buffer.handle(),
                           ...
                           nullptr);
    vk::CmdCopyImageToBuffer(
        m_command_buffer.handle(), odd_image.handle(), VK_IMAGE_LAYOUT_GENERAL, buffer_16.handle(), 1, &region);
    ...
    vk::CmdPipelineBarrier(m_command_buffer.handle(),
                           ...
                           nullptr);
    vk::CmdCopyBufferToImage(
        m_command_buffer.handle(), buffer_16.handle(), odd_image.handle(), VK_IMAGE_LAYOUT_GENERAL, 1, &region);

    // Offset + extent width < mip width and not a multiple of block width - should fail
    region.imageExtent = { 3, 3, 1 };
    m_errorMonitor->SetDesiredError(
        "VUID-vkCmdCopyImageToBuffer-srcImage-00208"); // offset+extent not a multiple of block width
    m_errorMonitor->SetDesiredError("VUID-vkCmdCopyImageToBuffer-imageOffset-07747"); // image transfer granularity
    vk::CmdCopyImageToBuffer(
        m_command_buffer.handle(), odd_image.handle(), VK_IMAGE_LAYOUT_GENERAL, buffer_16.handle(), 1, &region);
    m_errorMonitor->VerifyFound();
    m_errorMonitor->SetDesiredError(
        "VUID-vkCmdCopyBufferToImage-dstImage-00208"); // offset+extent not a multiple of block width
    m_errorMonitor->SetDesiredError("VUID-vkCmdCopyBufferToImage-imageOffset-07738"); // image transfer granularity
    vk::CmdCopyBufferToImage(
        m_command_buffer.handle(), buffer_16.handle(), odd_image.handle(), VK_IMAGE_LAYOUT_GENERAL, 1, &region);
    m_errorMonitor->VerifyFound();
...
TEST_F(NegativeCopyBufferImage, Compressed) {
    ...
    if (!FormatFeaturesAreSupported(Gpu(),
        ...
        !FormatFeaturesAreSupported(Gpu(),
    ...
    vkt::Image image_bc2(*m_device,
                         60,
                         60,
                         1,
                         VK_FORMAT_BC2_UNORM_BLOCK,
                         VK_IMAGE_USAGE_TRANSFER_SRC_BIT | VK_IMAGE_USAGE_TRANSFER_DST_BIT);
    vkt::Image image_bc3(*m_device,
                         60,
                         60,
                         1,
                         VK_FORMAT_BC3_UNORM_BLOCK,
...
TEST_F(NegativeCopyBufferImage, CompressedMipLevels) {
    ...
    // Mip 1 - 30 x 30
    // Mip 2 - 15 x 15
    vkt::Image image_src(*m_device,
                         60,
                         60,
                         3,
                         VK_FORMAT_BC2_UNORM_BLOCK,
                         VK_IMAGE_USAGE_TRANSFER_SRC_BIT | VK_IMAGE_USAGE_TRANSFER_DST_BIT);
    vkt::Image image_dst(*m_device,
                         60,
                         60,
                         3,
                         VK_FORMAT_BC3_UNORM_BLOCK,
...
TEST_F(NegativeCopyBufferImage, MiscImageLayer) {
    ...
    if (!FormatFeaturesAreSupported(
            Gpu(), VK_FORMAT_R16G16B16A16_UINT, VK_IMAGE_TILING_OPTIMAL, VK_FORMAT_FEATURE_TRANSFER_DST_BIT)) {
        GTEST_SKIP() << "Required VK_FORMAT_R16G16B16A16_UINT features not supported";
    } else if (!FormatFeaturesAreSupported(
                   Gpu(), VK_FORMAT_R8G8_UNORM, VK_IMAGE_TILING_OPTIMAL, VK_FORMAT_FEATURE_TRANSFER_DST_BIT)) {
    ...
    vk::CmdCopyBufferToImage(
        m_command_buffer.handle(), buffer.handle(), image.handle(), VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, 1, &region);
    m_errorMonitor->VerifyFound();
...
TEST_F(NegativeCopyBufferImage, ImageTypeExtentMismatch) {
    ...
    // Create 1D image
    VkImageCreateInfo ci = vkt::Image::ImageCreateInfo2D(
        32, 32, 1, 1, VK_FORMAT_R8G8B8A8_UNORM, VK_IMAGE_USAGE_TRANSFER_SRC_BIT | VK_IMAGE_USAGE_TRANSFER_DST_BIT);
...
TEST_F(NegativeCopyBufferImage, ImageTypeExtentMismatch3D) {
    ...
    // Create 1D image
    VkImageCreateInfo ci = vkt::Image::ImageCreateInfo2D(
        32, 1, 1, 1, VK_FORMAT_R8G8B8A8_UNORM, VK_IMAGE_USAGE_TRANSFER_SRC_BIT | VK_IMAGE_USAGE_TRANSFER_DST_BIT);
    ci.extent = { 32, 32, 1 };
    vkt::Image image_2D(*m_device, ci, vkt::set_layout);
...
TEST_F(NegativeCopyBufferImage, ImageCompressedBlockAlignment) {
    ...
    auto image_ci = vkt::Image::ImageCreateInfo2D(
        64, 64, 1, 1, VK_FORMAT_BC3_SRGB_BLOCK, VK_IMAGE_USAGE_TRANSFER_SRC_BIT | VK_IMAGE_USAGE_TRANSFER_DST_BIT);
    ...
    bool ycbcr = (IsExtensionsEnabled(VK_KHR_SAMPLER_YCBCR_CONVERSION_EXTENSION_NAME) ||
    ...
    // Note: "VUID-vkCmdCopyImage-srcImage-01730", "VUID-vkCmdCopyImage-dstImage-01734",
    // "VUID-vkCmdCopyImage-srcImage-01730", "VUID-vkCmdCopyImage-dstImage-01734"
...
TEST_F(NegativeCopyBufferImage, ImageZeroSize) {
    ...
    vk::CmdCopyImageToBuffer(
        m_command_buffer.handle(), src_image.handle(), VK_IMAGE_LAYOUT_GENERAL, buffer.handle(), 1, &buffer_image_copy);
    m_errorMonitor->VerifyFound();

    m_errorMonitor->SetDesiredError("VUID-VkBufferImageCopy-imageExtent-06660");
    vk::CmdCopyBufferToImage(
        m_command_buffer.handle(), buffer.handle(), dst_image.handle(), VK_IMAGE_LAYOUT_GENERAL, 1, &buffer_image_copy);
    m_errorMonitor->VerifyFound();
...
TEST_F(NegativeCopyBufferImage, ImageMultiPlaneSizeExceeded) {
    ...
    AddRequiredExtensions(VK_KHR_SAMPLER_YCBCR_CONVERSION_EXTENSION_NAME);
    RETURN_IF_SKIP(Init());
    // Try to use VK_FORMAT_G8_B8_R8_3PLANE_420_UNORM because need multi-plane format for some tests and likely
    // supported due to copy support being required with samplerYcbcrConversion feature
    VkFormatProperties props = { 0, 0, 0 };
...
TEST_F(NegativeCopyBufferImage, ImageFormatSizeMismatch) {
    ...
    } else if (!FormatFeaturesAreSupported(Gpu(),
                                           VK_FORMAT_R8_UINT,
                                           VK_IMAGE_TILING_OPTIMAL,
                                           VK_FORMAT_FEATURE_TRANSFER_SRC_BIT | VK_FORMAT_FEATURE_TRANSFER_DST_BIT)) {
    ...
    auto image_ci = vkt::Image::ImageCreateInfo2D(
        32, 32, 1, 1, VK_FORMAT_R8_UNORM, VK_IMAGE_USAGE_TRANSFER_SRC_BIT | VK_IMAGE_USAGE_TRANSFER_DST_BIT);
    ...
    vk::CmdCopyImage(m_command_buffer.handle(),
                     image_8b_unorm.handle(),
                     VK_IMAGE_LAYOUT_GENERAL,
                     image_8b_uint.handle(),
                     VK_IMAGE_LAYOUT_GENERAL,
                     1,
                     &copy_region);
...
TEST_F(NegativeCopyBufferImage, ImageFormatSizeMismatch2) {
    ...
    vk::CmdCopyImage(m_command_buffer.handle(),
                     image_8b_unorm.handle(),
                     VK_IMAGE_LAYOUT_GENERAL,
                     image_8b_16b_420_unorm.handle(),
                     VK_IMAGE_LAYOUT_GENERAL,
                     1,
                     &copy_region);

    image_8b_16b_420_unorm.ImageMemoryBarrier(m_command_buffer,
    ...
    image_8b_unorm.ImageMemoryBarrier(m_command_buffer,
...
TEST_F(NegativeCopyBufferImage, ImageSampleCountMismatch) {
    ...
    auto image_ci = vkt::Image::ImageCreateInfo2D(
        128, 128, 1, 1, VK_FORMAT_R8G8B8A8_UNORM, VK_IMAGE_USAGE_TRANSFER_SRC_BIT | VK_IMAGE_USAGE_TRANSFER_DST_BIT);
...
TEST_F(NegativeCopyBufferImage, ImageAspectMismatch) {
    ...
    vkt::Image color_image(*m_device,
                           128,
                           128,
                           1,
                           VK_FORMAT_R32_SFLOAT,
                           VK_IMAGE_USAGE_TRANSFER_SRC_BIT | VK_IMAGE_USAGE_TRANSFER_DST_BIT);
    vkt::Image depth_image(*m_device,
                           128,
                           128,
                           1,
                           VK_FORMAT_D32_SFLOAT,
                           VK_IMAGE_USAGE_TRANSFER_SRC_BIT | VK_IMAGE_USAGE_TRANSFER_DST_BIT);
    vkt::Image ds_image(
        *m_device, 128, 128, 1, ds_format, VK_IMAGE_USAGE_TRANSFER_SRC_BIT | VK_IMAGE_USAGE_TRANSFER_DST_BIT);
    ...
    copy_region.srcSubresource.aspectMask =
        VK_IMAGE_ASPECT_COLOR_BIT | VK_IMAGE_ASPECT_DEPTH_BIT; // color must be alone
    copy_region.dstSubresource.aspectMask = VK_IMAGE_ASPECT_COLOR_BIT;
...
TEST_F(NegativeCopyBufferImage, DepthStencilImageCopyNoGraphicsQueueFlags) {
    TEST_DESCRIPTION("Allocate a command buffer on a queue that does not support graphics and try to issue a "
                     "depth/stencil image copy to "
                     "buffer");
    ...
    vkt::Image ds_image(
        *m_device, 64, 64, 1, ds_format, VK_IMAGE_USAGE_TRANSFER_DST_BIT | VK_IMAGE_USAGE_DEPTH_STENCIL_ATTACHMENT_BIT);
    ds_image.SetLayout(VK_IMAGE_LAYOUT_GENERAL);
    ...
    vk::CmdCopyBufferToImage(
        command_buffer.handle(), buffer.handle(), ds_image.handle(), VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, 1, &region);
    m_errorMonitor->VerifyFound();
}

TEST_F(NegativeCopyBufferImage, ImageTransferQueueFlags) {
    TEST_DESCRIPTION("Allocate a command buffer on a queue that does not support graphics/compute and try to issue an "
                     "invalid image copy to "
                     "buffer");
    ...
    vk::CmdCopyBufferToImage(
        command_buffer.handle(), buffer.handle(), image.handle(), VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, 1, &region);
    m_errorMonitor->VerifyFound();
}
...
TEST_F(NegativeCopyBufferImage, ImageOverlappingMemory) {
    ...
    auto image_ci = vkt::Image::ImageCreateInfo2D(
        32, 32, 1, 1, VK_FORMAT_R8G8B8A8_UNORM, VK_IMAGE_USAGE_TRANSFER_DST_BIT | VK_IMAGE_USAGE_TRANSFER_SRC_BIT);
    ...
    vk::CmdCopyImageToBuffer(
        m_command_buffer.handle(), image.handle(), VK_IMAGE_LAYOUT_GENERAL, buffer.handle(), 1, &region);
    m_errorMonitor->VerifyFound();
    m_errorMonitor->SetDesiredError("VUID-vkCmdCopyBufferToImage-pRegions-00173");
    vk::CmdCopyBufferToImage(
        m_command_buffer.handle(), buffer.handle(), image.handle(), VK_IMAGE_LAYOUT_GENERAL, 1, &region);
    m_errorMonitor->VerifyFound();
...
TEST_F(NegativeCopyBufferImage, ImageRemainingLayers) {
    ...
    copy_region.srcSubresource.layerCount =
        VK_REMAINING_ARRAY_LAYERS; // This value is unsupported by VkImageSubresourceLayer
    ...
    buffer_copy.imageSubresource.layerCount =
        VK_REMAINING_ARRAY_LAYERS; // This value is unsupported by VkImageSubresourceLayers
    buffer_copy.imageSubresource.mipLevel = 0;
    buffer_copy.imageSubresource.baseArrayLayer = 5;

    // This error will trigger first stating that the copy is too big for the buffer, because of
...
TEST_F(NegativeCopyBufferImage, BufferToCompressedImage) {
    ...
    if (!FormatFeaturesAreSupported(
            Gpu(), VK_FORMAT_BC1_RGBA_SRGB_BLOCK, VK_IMAGE_TILING_OPTIMAL, VK_FORMAT_FEATURE_TRANSFER_DST_BIT)) {
        GTEST_SKIP() << "Required formats/features not supported";
    }
    ...
    vk::CmdCopyBufferToImage(
        m_command_buffer.handle(), buffer.handle(), width_image.handle(), VK_IMAGE_LAYOUT_GENERAL, 1, &region);
    m_errorMonitor->VerifyFound();
    ...
    vk::CmdCopyBufferToImage(
        m_command_buffer.handle(), buffer.handle(), depth_image, VK_IMAGE_LAYOUT_GENERAL, 1, &region);
    m_errorMonitor->VerifyFound();
...
TEST_F(NegativeCopyBufferImage, SameImage) {
    ...
    auto image_ci = vkt::Image::ImageCreateInfo2D(
        32, 32, 1, 1, VK_FORMAT_B8G8R8A8_UNORM, VK_IMAGE_USAGE_TRANSFER_SRC_BIT | VK_IMAGE_USAGE_TRANSFER_DST_BIT);
    vkt::Image src_image(*m_device, image_ci, vkt::set_layout);
...
TEST_F(NegativeCopyBufferImage, ImageRemainingArrayLayers) {
    ...
    auto image_ci = vkt::Image::ImageCreateInfo2D(
        64, 64, 1, 4, VK_FORMAT_R8G8B8A8_UNORM, VK_IMAGE_USAGE_TRANSFER_SRC_BIT | VK_IMAGE_USAGE_TRANSFER_DST_BIT);
    vkt::Image image(*m_device, image_ci, vkt::set_layout);
...
TEST_F(NegativeCopyBufferImage, ImageMemory) {
    ...
    auto image_ci = vkt::Image::ImageCreateInfo2D(
        64, 64, 1, 1, VK_FORMAT_R8G8B8A8_UNORM, VK_IMAGE_USAGE_TRANSFER_SRC_BIT | VK_IMAGE_USAGE_TRANSFER_DST_BIT);
...
TEST_F(NegativeCopyBufferImage, ImageMissingUsage) {
    ...
    vk::CmdCopyImage(m_command_buffer.handle(),
                     separate_stencil_sampled_image.handle(),
                     VK_IMAGE_LAYOUT_GENERAL,
                     separate_stencil_transfer_image.handle(),
                     VK_IMAGE_LAYOUT_GENERAL,
                     1,
                     &region);
    m_errorMonitor->VerifyFound();
...
TEST_F(NegativeCopyBufferImage, OverlappingImage) {
    ...
    vk::CmdCopyImage(
        m_command_buffer.handle(), image, VK_IMAGE_LAYOUT_GENERAL, image, VK_IMAGE_LAYOUT_GENERAL, 1, &image_copy);
    m_errorMonitor->VerifyFound();
...
TEST_F(NegativeCopyBufferImage, MinImageTransferGranularity) {
    ...
    auto large_granularity_family = std::find_if(
        queue_family_properties.begin(), queue_family_properties.end(), [](VkQueueFamilyProperties family_properties) {
            VkExtent3D family_granularity = family_properties.minImageTransferGranularity;
            // We need a queue family that supports copy operations and has a large enough minImageTransferGranularity
            // for the tests below to make sense.
            return (family_properties.queueFlags & VK_QUEUE_TRANSFER_BIT ||
    ...
    m_errorMonitor->SetDesiredError("VUID-vkCmdCopyImage-srcOffset-01783"); // srcOffset image transfer granularity
    vk::CmdCopyImage(command_buffer.handle(),
                     src_image,
                     VK_IMAGE_LAYOUT_GENERAL,
                     dst_image,
                     VK_IMAGE_LAYOUT_GENERAL,
    ...
    m_errorMonitor->SetDesiredError("VUID-vkCmdCopyImageToBuffer-imageOffset-07747"); // image transfer granularity
    vk::CmdCopyImageToBuffer(
        command_buffer.handle(), src_image, VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL, buffer.handle(), 1, &region);
    m_errorMonitor->VerifyFound();
    ...
    m_errorMonitor->SetDesiredError("VUID-vkCmdCopyBufferToImage-imageOffset-07738"); // image transfer granularity
    vk::CmdCopyBufferToImage(
        command_buffer.handle(), buffer.handle(), dst_image, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, 1, &region);
    m_errorMonitor->VerifyFound();
...
TEST_F(NegativeCopyBufferImage, Extents) {
    ...
    // equivalent test using KHR_copy_commands2
    if (copy_commands2) {
        const VkBufferCopy2 copy_info2 = {
            VK_STRUCTURE_TYPE_BUFFER_COPY_2, NULL, copy_info.srcOffset, copy_info.dstOffset, copy_info.size
        };
...
TEST_F(NegativeCopyBufferImage, CompletelyOverlappingBuffer) {
    ...
    copy_info.size = 256;
    vkt::Buffer buffer(
        *m_device, copy_info.size, VK_BUFFER_USAGE_TRANSFER_SRC_BIT | VK_BUFFER_USAGE_TRANSFER_DST_BIT, 0);
