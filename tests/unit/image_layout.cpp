...
TEST_F(NegativeImageLayout, Blit) {
    ...
    vkt::Image img_src_transfer(
        *m_device, 64, 64, 1, fmt, VK_IMAGE_USAGE_TRANSFER_SRC_BIT | VK_IMAGE_USAGE_TRANSFER_DST_BIT);
    vkt::Image img_dst_transfer(
        *m_device, 64, 64, 1, fmt, VK_IMAGE_USAGE_TRANSFER_SRC_BIT | VK_IMAGE_USAGE_TRANSFER_DST_BIT);
    vkt::Image img_general(
        *m_device, 64, 64, 1, fmt, VK_IMAGE_USAGE_TRANSFER_SRC_BIT | VK_IMAGE_USAGE_TRANSFER_DST_BIT);
    vkt::Image img_color(*m_device,
                         64,
                         64,
                         1,
                         fmt,
    ...
    vk::CmdBlitImage(m_command_buffer.handle(),
                     img_src_transfer.handle(),
                     VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL,
                     img_dst_transfer.handle(),
                     img_dst_transfer.Layout(),
                     1,
                     &blit_region,
                     VK_FILTER_LINEAR);
...
TEST_F(NegativeImageLayout, Compute) {
    ...
    { // Verify invalid image layout with CmdDispatch
        ...
        cmd.Begin();
        vk::CmdBindDescriptorSets(cmd.handle(),
...
TEST_F(NegativeImageLayout, Basic) {
    ...
    m_errorMonitor->SetDesiredError("VUID-vkCmdClearColorImage-imageLayout-00004");
    vk::CmdClearColorImage(
        m_command_buffer.handle(), dst_image.handle(), VK_IMAGE_LAYOUT_GENERAL, &color_clear_value, 1, &clear_range);
    m_errorMonitor->VerifyFound();
    ...
    vk::CmdClearDepthStencilImage(
        m_command_buffer.handle(), depth_image.handle(), VK_IMAGE_LAYOUT_GENERAL, &depth_clear_value, 1, &clear_range);
    m_errorMonitor->VerifyFound();
