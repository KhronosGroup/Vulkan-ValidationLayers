...
TEST_F(PositiveImageLayout, ImagelessTracking) {
    ...
    auto image_ci = vkt::Image::ImageCreateInfo2D(
        attachmentWidth, attachmentHeight, 1, 1, attachmentFormat, VK_IMAGE_USAGE_COLOR_ATTACHMENT_BIT);
    ...
    VkRenderPassAttachmentBeginInfo renderPassAttachmentBeginInfo = {
        VK_STRUCTURE_TYPE_RENDER_PASS_ATTACHMENT_BEGIN_INFO_KHR, nullptr, 1, &imageView.handle()
    };
    VkRenderPassBeginInfo renderPassBeginInfo =
        vku::InitStruct<VkRenderPassBeginInfo>(&renderPassAttachmentBeginInfo,
                                               rp.Handle(),
                                               framebuffer.handle(),
                                               VkRect2D{ { 0, 0 }, { attachmentWidth, attachmentHeight } },
                                               0u,
                                               nullptr);
...
TEST_F(PositiveImageLayout, Subresource) {
    ...
    auto barrier = image.ImageMemoryBarrier(0,
...
TEST_F(PositiveImageLayout, DescriptorSubresource) {
    ...
    for (TestType test_type : test_list) {
        ...
        if (test_type == kExternal) {
            // The image layout is external to the command buffer we are recording to test.  Submit to push to instance
            // scope.
            cmd_buf.End();
...
TEST_F(PositiveImageLayout, Descriptor3D2DSubresource) {
    ...
    VkSubpassDescription subpass = {
        0,      VK_PIPELINE_BIND_POINT_GRAPHICS, 0, nullptr, (uint32_t)color.size(), color.data(), nullptr, nullptr, 0,
        nullptr
    };
    ...
    std::vector<VkSubpassDependency> deps = {
        { VK_SUBPASS_EXTERNAL,
          ...
          (VK_ACCESS_COLOR_ATTACHMENT_READ_BIT | VK_ACCESS_COLOR_ATTACHMENT_WRITE_BIT | VK_ACCESS_MEMORY_WRITE_BIT),
          0 },
        { 0,
          VK_SUBPASS_EXTERNAL,
          VK_PIPELINE_STAGE_COLOR_ATTACHMENT_OUTPUT_BIT,
