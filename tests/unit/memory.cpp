...
TEST_F(NegativeMemory, MemoryDecompressionIndirectCount) {
    ...
    m_errorMonitor->SetDesiredError("UNASSIGNED-vkCmdDecompressMemoryIndirectCountEXT-indirectCommandsAddress-range-same-buffer");
    vk::CmdDecompressMemoryIndirectCountEXT(m_command_buffer.handle(), decompression_method, ic_buffer.Address(),
                                            icc_buffer.Address(), max_decompression_count, stride);
    m_errorMonitor->VerifyFound();
    m_command_buffer.End();
...
TEST_F(NegativeMemory, MaxDecompressionCount) {
    ...
    m_command_buffer.Begin();
    m_errorMonitor->SetDesiredError("VUID-vkCmdDecompressMemoryIndirectCountEXT-indirectCommandsCountAddress-07699");
    vk::CmdDecompressMemoryIndirectCountEXT(m_command_buffer.handle(), decompression_method, ic_buffer.Address(),
...
TEST_F(NegativeMemory, MemoryDecompressionIndirectAddressRangeSameBuffer) {
    ...
    const VkDeviceAddress start = indirect_a.Address() + sizeof(cmds) - 1;
...
TEST_F(NegativeMemory, MemoryDecompression) {
    ...
    m_command_buffer.Begin();
    m_errorMonitor->SetDesiredError("VUID-VkDecompressMemoryInfoEXT-decompressionMethod-09395");
...
TEST_F(NegativeMemory, MemoryDecompressionBufferUsage) {
    ...
    m_command_buffer.Begin();
    m_errorMonitor->SetDesiredError("UNASSIGNED-VkDecompressMemoryRegionEXT-srcAddress-memory-decompression-usage");
    m_errorMonitor->SetDesiredError("UNASSIGNED-VkDecompressMemoryRegionEXT-dstAddress-memory-decompression-usage");
    vk::CmdDecompressMemoryEXT(m_command_buffer.handle(), &decompress_info);
    m_errorMonitor->VerifyFound();
    m_command_buffer.End();
}
