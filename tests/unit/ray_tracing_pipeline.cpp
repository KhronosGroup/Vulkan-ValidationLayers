...
TEST_F(NegativeRayTracingPipeline, ClusterAccelerationStructureFeatureDisabled) {
    TEST_DESCRIPTION("Test VUID-10576: clusterAccelerationStructure feature must be enabled when using cluster acceleration structure pipeline info");
    ...
    m_errorMonitor->SetDesiredError("VUID-VkRayTracingPipelineClusterAccelerationStructureCreateInfoNV-clusterAccelerationStructure-10576");
    vk::CreateRayTracingPipelinesKHR(*m_device, VK_NULL_HANDLE, VK_NULL_HANDLE, 1, &pipeline_ci, nullptr, &pipeline);
    m_errorMonitor->VerifyFound();
}
