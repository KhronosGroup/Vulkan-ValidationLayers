...
TEST_F(NegativeGpuAVIndirectBuffer, DrawCountDeviceLimit) {
    ...
    RETURN_IF_SKIP(
        InitState(nullptr, (features13.dynamicRendering || mesh_shader_enabled) ? (void*)&features13 : nullptr));
    InitRenderTarget();

    vkt::Buffer draw_buffer(
        *m_device, 2 * sizeof(VkDrawIndirectCommand), VK_BUFFER_USAGE_INDIRECT_BUFFER_BIT, kHostVisibleMemProps);
    ...
    *count_ptr = 2; // Fits in buffer but exceeds (fake) limit
    ...
    vk::CmdDrawIndirectCountKHR(
        m_command_buffer.handle(), draw_buffer.handle(), 0, count_buffer.handle(), 0, 2, sizeof(VkDrawIndirectCommand));
...
TEST_F(NegativeGpuAVIndirectBuffer, DrawCountDeviceLimitSubmit2) {
    ...
    vkt::Buffer draw_buffer(
        *m_device, 2 * sizeof(VkDrawIndexedIndirectCommand), VK_BUFFER_USAGE_INDIRECT_BUFFER_BIT, kHostVisibleMemProps);
...
TEST_F(NegativeGpuAVIndirectBuffer, DrawCount) {
    ...
    *count_ptr = 2;
    ...
    m_errorMonitor->SetDesiredWarningRegex(
        ...
        ".* = 32");
    vk::CmdDrawIndirectCountKHR(
        m_command_buffer.handle(), draw_buffer.handle(), 0, count_buffer.handle(), 0, 1, sizeof(VkDrawIndirectCommand));
    m_command_buffer.EndRenderPass();
    m_command_buffer.End();
    m_default_queue->Submit(m_command_buffer);
    m_default_queue->Wait();
    ...
    *count_ptr = 1;
    ...
    m_errorMonitor->SetDesiredWarningRegex(
        ...
        ".* = 20");
    vk::CmdDrawIndirectCountKHR(
        m_command_buffer.handle(), draw_buffer.handle(), 4, count_buffer.handle(), 0, 1, sizeof(VkDrawIndirectCommand));
    m_command_buffer.EndRenderPass();
    m_command_buffer.End();
    m_default_queue->Submit(m_command_buffer);
    m_default_queue->Wait();
    ...
    *count_ptr = 1;
    ...
    if (mesh_shader_enabled) {
        ...
        mesh_pipe.shader_stages_[0] = mesh_shader.GetStageCreateInfo();
        mesh_pipe.CreateGraphicsPipeline();
        vkt::Buffer mesh_draw_buffer(*m_device,
                                     sizeof(VkDrawMeshTasksIndirectCommandEXT),
...
TEST_F(NegativeGpuAVIndirectBuffer, Mesh) {
    ...
    uint32_t buffer_size =
        num_commands * (sizeof(VkDrawMeshTasksIndirectCommandEXT) + 4); // 4 byte pad between commands
...
TEST_F(NegativeGpuAVIndirectBuffer, FirstInstance) {
    ...
    vkt::Buffer draw_params_buffer = vkt::IndirectBuffer<VkDrawIndirectCommand>(
        *m_device,
        { draw_params, draw_params_invalid_first_instance_1, draw_params, draw_params_invalid_first_instance_42 });
...
TEST_F(NegativeGpuAVIndirectBuffer, DispatchWorkgroupSize) {
    ...
    vkt::Buffer indirect_buffer(
        *m_device, 5 * sizeof(VkDispatchIndirectCommand), VK_BUFFER_USAGE_INDIRECT_BUFFER_BIT, kHostVisibleMemProps);
