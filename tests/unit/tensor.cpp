...
static VkTensorDescriptionARM default_desc() {
    static std::vector<int64_t> dimensions{2ul};
    static std::vector<int64_t> strides{1l};
    static VkTensorDescriptionARM desc;
...
static VkTensorCreateInfoARM DefaultCreateInfo(VkTensorDescriptionARM* desc = nullptr) {
    static VkTensorCreateInfoARM info;
...
TEST_F(NegativeTensor, ConcurrentTensor) {
    ...
    VkTensorDescriptionARM desc = default_desc();
...
TEST_F(NegativeTensor, OpaqueCaptureMissingFlag) {
    ...
    VkOpaqueCaptureDescriptorDataCreateInfoEXT opaque_tensor{};
    opaque_tensor.sType = VK_STRUCTURE_TYPE_OPAQUE_CAPTURE_DESCRIPTOR_DATA_CREATE_INFO_EXT;
    opaque_tensor.pNext = nullptr;
...
TEST_F(NegativeTensor, ProtectedMemory) {
    ...
    if (protected_memory_features.protectedMemory == VK_TRUE) {
        GTEST_SKIP() << "protectedMemory feature is supported";
    };
...
TEST_F(NegativeTensor, FormatFeatureLinUsage) {
    ...
    if (src_support && dst_support) {
        GTEST_SKIP() << "Linear Feature Flags supported";
    }
...
TEST_F(NegativeTensor, FormatFeatureOptUsage) {
    ...
    bool src_support = opt_features & VK_FORMAT_FEATURE_2_TRANSFER_SRC_BIT;
    bool dst_support = opt_features & VK_FORMAT_FEATURE_2_TRANSFER_DST_BIT;
    if (src_support && dst_support) {
        GTEST_SKIP() << "Optimal Feature Flags supported";
    }
...
TEST_F(NegativeTensor, FormatTwoComponent) {
    ...
    static std::vector<int64_t> strides{16l};
    desc.pStrides = strides.data();
...
TEST_F(NegativeTensor, FormatThreeComponent) {
    ...
    static std::vector<int64_t> strides{24l};
    desc.pStrides = strides.data();
...
TEST_F(NegativeTensor, FormatFourComponent) {
    ...
    static std::vector<int64_t> strides{32l};
    desc.pStrides = strides.data();
...
TEST_F(NegativeTensor, InnermostStrideWrongSize) {
    ...
    VkTensorDescriptionARM desc = default_desc();
    VkTensorCreateInfoARM info = DefaultCreateInfo(&desc);
    static std::vector<int64_t> strides{2l};
    desc.pStrides = strides.data();
...
TEST_F(NegativeTensor, StridesNotMultiple) {
    ...
    VkTensorDescriptionARM desc = default_desc();
    VkTensorCreateInfoARM info = DefaultCreateInfo(&desc);
    static std::vector<int64_t> dimensions{4ul, 1ul};
    static std::vector<int64_t> strides{3l, 2l};
    desc.pDimensions = dimensions.data();
...
TEST_F(NegativeTensor, StridesExtremes) {
    ...
    static std::vector<int64_t> dimensions{2ul, 2ul, 2ul, 2ul};
    static std::vector<int64_t> strides{-1l, 0l, static_cast<int64_t>(tensor_props.maxTensorStride + 1), 1l};
    desc.pDimensions = dimensions.data();
    desc.dimensionCount = dimensions.size();
    desc.pStrides = strides.data();
...
TEST_F(NegativeTensor, StridesOverlap) {
    ...
    static std::vector<int64_t> dimensions{1ul, 1ul, 1ul};
    static std::vector<int64_t> strides{1l, 4ul, 1l};
    desc.pDimensions = dimensions.data();
    desc.dimensionCount = dimensions.size();
    desc.pStrides = strides.data();
...
TEST_F(NegativeTensor, NonPacked) {
    ...
    if (tensor_features.tensorNonPacked) {
        GTEST_SKIP() << "NonPacked Tensors supported";
    }

    VkTensorDescriptionARM desc = default_desc();
    VkTensorCreateInfoARM info = DefaultCreateInfo(&desc);

    static std::vector<int64_t> dimensions{1ul, 3ul, 1ul};
    static std::vector<int64_t> strides{4l, 1l, 1l};
    desc.pDimensions = dimensions.data();
    desc.dimensionCount = dimensions.size();
    desc.pStrides = strides.data();
...
TEST_F(NegativeTensor, OptimalAliased) {
    ...
    static std::vector<int64_t> dimensions{1ul, 1ul, 5ul};
    desc.pDimensions = dimensions.data();
    desc.dimensionCount = dimensions.size();
    desc.pStrides = nullptr;
...
TEST_F(NegativeTensor, RebindTensor) {
    ...
    VkMemoryRequirements2 mem_reqs = tensor.get_memory_reqs();
...
TEST_F(NegativeTensor, BindTensorMemoryTooSmall) {
    TEST_DESCRIPTION("Test binding a tensor when the allocated memory is smaller than the size of the memory");
    SetTargetApiVersion(VK_API_VERSION_1_4);
    AddRequiredExtensions(VK_ARM_TENSORS_EXTENSION_NAME);
    AddRequiredFeature(vkt::Feature::tensors);
    RETURN_IF_SKIP(Init());
    ...
    VkMemoryAllocateInfo tensor_alloc_info = vku::InitStructHelper();
    tensor_alloc_info.allocationSize = mem_reqs.memoryRequirements.size - 1;
    m_device->Physical().SetMemoryType(mem_reqs.memoryRequirements.memoryTypeBits, &tensor_alloc_info, 0);
    vkt::DeviceMemory memory(*m_device, tensor_alloc_info);
...
TEST_F(NegativeTensor, BindTensorNotProtectedToProtectedMemory) {
    ...
    unprotected_tensor.get_memory_reqs();

    /* get protected memory (use requirements for protected tensor) */
    VkMemoryRequirements2 mem_reqs = protected_tensor.get_memory_reqs();
...
TEST_F(NegativeTensor, BindTensorProtectedToNotProtectedMemory) {
    ...
    VkMemoryRequirements2 mem_reqs = unprotected_tensor.get_memory_reqs();
    VkMemoryAllocateInfo mem_alloc = vku::InitStructHelper();
...
TEST_F(NegativeTensor, TensorViewOpaqueCaptureMissingFlag) {
    ...
    std::vector<uint32_t> opaque_capture_descriptor_data(64, 0);
    VkOpaqueCaptureDescriptorDataCreateInfoEXT opaque_capture{};
    opaque_capture.sType = VK_STRUCTURE_TYPE_OPAQUE_CAPTURE_DESCRIPTOR_DATA_CREATE_INFO_EXT;
    opaque_capture.pNext = nullptr;
...
TEST_F(NegativeTensor, DestroyTensorInUse) {
    ...
    // Create a timline semaphore block the command buffer
...
TEST_F(NegativeTensor, DestroyTensorCreateWithDestroyWithoutCallbacks) {
    ...
    auto desc = default_desc();
    auto info = DefaultCreateInfo(&desc);
    VkTensorARM tensor;
    vk::CreateTensorARM(*m_device, &info, vkt::default_allocator(), &tensor);
    ...
    vk::DestroyTensorARM(*m_device, tensor, vkt::default_allocator());
}
...
TEST_F(NegativeTensor, DestroyTensorViewInUse) {
    ...
    vkt::Buffer buffer(*m_device, tensor.get_memory_reqs().memoryRequirements.size, VK_BUFFER_USAGE_STORAGE_BUFFER_BIT);
    ...
    pipe.cs_ = VkShaderObj::CreateFromGLSL(this, cs_source, VK_SHADER_STAGE_COMPUTE_BIT);
...
TEST_F(NegativeTensor, DestroyTensorViewCreateWithDestroyWithoutCallbacks) {
    ...
    vk::CreateTensorViewARM(*m_device, &tensor_view_create_info, vkt::default_allocator(), &view);
    m_errorMonitor->SetDesiredError("VUID-vkDestroyTensorViewARM-tensorView-09751");
    vk::DestroyTensorViewARM(*m_device, view, nullptr);
    m_errorMonitor->VerifyFound();
    // Clean up by correctly destroying the tensor
    vk::DestroyTensorViewARM(*m_device, view, vkt::default_allocator());
}
...
TEST_F(NegativeTensor, WriteDescriptorSetTensorInfoWrongCount) {
    ...
    static constexpr uint32_t tensor_binding_count = 5;
    ...
    descriptor_write.descriptorCount = tensor_binding_count - 1;
    descriptor_write.descriptorType = VK_DESCRIPTOR_TYPE_TENSOR_ARM;

    m_errorMonitor->SetDesiredError("VUID-VkWriteDescriptorSetTensorARM-tensorViewCount-09700");
...
TEST_F(NegativeTensor, TensorMemoryBarrierSharingModeConcurrentSrcQueueFamilyNotIgnored) {
    ...
    VkDependencyInfoKHR dependency_info = {VK_STRUCTURE_TYPE_DEPENDENCY_INFO_KHR, &barrier};
...
TEST_F(NegativeTensor, TensorMemoryBarrierWrongQueueFamilySets) {
    TEST_DESCRIPTION(
        "Test setting a tensor memory barrier when neither src or dstQueueFamilyIndex are the same as the family of the queue that "
...
TEST_F(NegativeTensor, BindTensorDedicatedMemoryOffsetNotZero) {
    ...
    // We expect this VUID failure as the allocated size is always going to be smaller than the dedicated size allocated
    m_errorMonitor->SetDesiredError("VUID-VkBindTensorMemoryInfoARM-size-09716");
