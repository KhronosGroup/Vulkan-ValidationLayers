...
TEST_F(PositiveDescriptorHeap, Basic) {
    ...
    descriptor_host[0].size = resource_stride;
    descriptor_host[1].address = resource_heap_data_ + resource_stride;
    descriptor_host[1].size = resource_stride;
...
TEST_F(PositiveDescriptorHeap, ComputeBuffer) {
    ...
    VkHostAddressRangeEXT out_descriptor{resource_heap_data_, resource_stride};
    ...
    VkHostAddressRangeEXT in_descriptor{resource_heap_data_ + resource_stride, resource_stride};
...
TEST_F(PositiveDescriptorHeap, ResourceParameterDataNull) {
    ...
    for (auto type : types) {
        ...
        VkHostAddressRangeEXT descriptors = {data.get(), size};
...
TEST_F(PositiveDescriptorHeap, PushData) {
    ...
    VkHostAddressRangeEXT descriptor_host = {resource_heap_data_, resource_stride};
...
TEST_F(PositiveDescriptorHeap, Sampler) {
    ...
    VkHostAddressRangeEXT sampler_host = {sampler_heap_data + sampler_offset, sampler_size};
...
TEST_F(PositiveDescriptorHeap, CombinedImageSampler) {
    ...
    VkHostAddressRangeEXT resource_host[2];
    resource_host[0].address = resource_heap_data_ + image_offset;
    resource_host[0].size = heap_props.imageDescriptorSize;
    resource_host[1].address = resource_heap_data_ + buffer_offset;
    resource_host[1].size = heap_props.bufferDescriptorSize;
...
TEST_F(PositiveDescriptorHeap, MappingSourceHeapWithPushIndex) {
    ...
    VkHostAddressRangeEXT descriptor_host = {resource_heap_data_ + offset, resource_stride};
    VkDeviceAddressRangeEXT device_range = {buffer.Address(), buffer.CreateInfo().size};
...
TEST_F(PositiveDescriptorHeap, MappingSourceHeapData) {
    ...
    VkHostAddressRangeEXT descriptor_host = {resource_heap_data_ + write_offset, resource_stride};
...
TEST_F(PositiveDescriptorHeap, CmdPushData) {
    ...
    std::vector<uint8_t> payload(heap_props.maxPushDataSize);
    ...
    push_data_info.offset = 0u;
    push_data_info.data.size = heap_props.maxPushDataSize;
    vk::CmdPushDataEXT(m_command_buffer, &push_data_info);

    push_data_info.offset = (uint32_t)heap_props.maxPushDataSize / 2u;
    push_data_info.data.size = heap_props.maxPushDataSize / 2u;
    vk::CmdPushDataEXT(m_command_buffer, &push_data_info);
...
TEST_F(PositiveDescriptorHeap, UntypedImageAndSampler) {
    ...
    VkHostAddressRangeEXT sampler_host = {sampler_heap_data_ + sampler_offset, sampler_size};
    vk::WriteSamplerDescriptorsEXT(*m_device, 1u, &sampler_info, &sampler_host);
...
TEST_F(PositiveDescriptorHeap, HardcodedOffsetIntoStruct) {
    ...
    VkHostAddressRangeEXT descriptor_host = {host_decriptor_a, resource_stride};
    ...
    memcpy(&resource_heap_data_[0], host_decriptor_a, resource_stride);
    memcpy(&resource_heap_data_[resource_stride], host_decriptor_b, resource_stride);
    memcpy(&resource_heap_data_[resource_stride * 2], host_decriptor_a, resource_stride);
    memcpy(&resource_heap_data_[resource_stride * 3], host_decriptor_a, resource_stride);
