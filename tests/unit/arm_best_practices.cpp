...
class VkArmBestPracticesLayerTest : public VkBestPracticesLayerTest {
  public:
    std::unique_ptr<vkt::Image> CreateImage(VkFormat format,
    ...
    VkFramebuffer
    CreateFramebuffer(const uint32_t width, const uint32_t height, VkImageView image_view, VkRenderPass renderpass);
};

class VkConstantBufferObj : public vkt::Buffer {
  public:
    VkConstantBufferObj(vkt::Device* device,
                        VkDeviceSize size,
                        const void* data,
...
VkFramebuffer VkArmBestPracticesLayerTest::CreateFramebuffer(const uint32_t width,
                                                             const uint32_t height,
...
std::unique_ptr<vkt::Image> VkArmBestPracticesLayerTest::CreateImage(VkFormat format,
                                                                     const uint32_t width,
                                                                     const uint32_t height,
                                                                     VkImageUsageFlags attachment_usage) {
    auto img = std::unique_ptr<vkt::Image>(new vkt::Image(*m_device,
                                                          width,
                                                          height,
                                                          1,
                                                          format,
...
VkRenderPass VkArmBestPracticesLayerTest::CreateRenderPass(VkFormat format,
...
TEST_F(VkArmBestPracticesLayerTest, NonTransientMSImage) {
    ...
    auto image_ci =
        vkt::Image::ImageCreateInfo2D(1920, 1080, 1, 1, VK_FORMAT_R8G8B8A8_UNORM, VK_IMAGE_USAGE_COLOR_ATTACHMENT_BIT);
...
TEST_F(VkArmBestPracticesLayerTest, MultisampledBlending) {
    ...
    m_errorMonitor->SetDesiredFailureMsg(kPerformanceWarningBit,
                                         "BestPractices-Arm-vkCreatePipelines-multisampled-blending");
...
TEST_F(VkArmBestPracticesLayerTest, SuboptimalDescriptorReuseTest) {
    ...
    m_errorMonitor->SetDesiredFailureMsg(kPerformanceWarningBit,
                                         "BestPractices-Arm-vkAllocateDescriptorSets-suboptimal-reuse");
...
TEST_F(VkArmBestPracticesLayerTest, SparseIndexBufferTest) {
    TEST_DESCRIPTION("Test for appropriate warnings to be thrown when recording an indexed draw call with "
                     "sparse/non-sparse index buffers.");
    ...
    VkConstantBufferObj nonsparse_ibo(m_device,
                                      nonsparse_indices.size() * sizeof(uint16_t),
                                      nonsparse_indices.data(),
                                      VK_BUFFER_USAGE_INDEX_BUFFER_BIT);
    VkConstantBufferObj nonsparse_ibo_2(m_device,
                                        nonsparse_indices_2.size() * sizeof(uint16_t),
                                        nonsparse_indices_2.data(),
                                        VK_BUFFER_USAGE_INDEX_BUFFER_BIT);
    VkConstantBufferObj nonsparse_ibo_3(m_device,
                                        nonsparse_indices_3.size() * sizeof(uint16_t),
                                        nonsparse_indices_3.data(),
                                        VK_BUFFER_USAGE_INDEX_BUFFER_BIT);
    VkConstantBufferObj nonsparse_ibo_4(m_device,
                                        nonsparse_indices_4.size() * sizeof(uint16_t),
                                        nonsparse_indices_4.data(),
                                        VK_BUFFER_USAGE_INDEX_BUFFER_BIT);
    VkConstantBufferObj sparse_ibo(
        m_device, sparse_indices.size() * sizeof(uint16_t), sparse_indices.data(), VK_BUFFER_USAGE_INDEX_BUFFER_BIT);
    ...
    auto test_pipelines = [&](VkConstantBufferObj& ibo, size_t index_count, bool expect_error) -> void {
        ...
        vk::CmdBindIndexBuffer(
            m_command_buffer.handle(), ibo.handle(), static_cast<VkDeviceSize>(0), VK_INDEX_TYPE_UINT16);

        // the validation layer will only be able to analyse mapped memory, it's too expensive otherwise to do in the
        // layer itself
        ibo.Memory().Map();
        if (expect_error) {
            m_errorMonitor->SetDesiredFailureMsg(kPerformanceWarningBit,
...
TEST_F(VkArmBestPracticesLayerTest, PostTransformVertexCacheThrashingIndicesTest) {
    ...
    VkConstantBufferObj worst_ibo(
        m_device, worst_indices.size() * sizeof(uint16_t), worst_indices.data(), VK_BUFFER_USAGE_INDEX_BUFFER_BIT);
    vk::CmdBindIndexBuffer(
        m_command_buffer.handle(), worst_ibo.handle(), static_cast<VkDeviceSize>(0), VK_INDEX_TYPE_UINT16);

    // the validation layer will only be able to analyse mapped memory, it's too expensive otherwise to do in the layer
    // itself
    ...
    VkConstantBufferObj best_ibo(
        m_device, best_indices.size() * sizeof(uint16_t), best_indices.data(), VK_BUFFER_USAGE_INDEX_BUFFER_BIT);
    vk::CmdBindIndexBuffer(
        m_command_buffer.handle(), best_ibo.handle(), static_cast<VkDeviceSize>(0), VK_INDEX_TYPE_UINT16);
...
TEST_F(VkArmBestPracticesLayerTest, PipelineDepthBiasZeroTest) {
    TEST_DESCRIPTION(
        "Test for unnecessary rasterization due to using 0 for depthBiasConstantFactor and depthBiasSlopeFactor");
...
TEST_F(VkArmBestPracticesLayerTest, DepthPrePassUsage) {
    ...
    VkConstantBufferObj ibo(
        m_device, sizeof(uint32_t) * indices.size(), indices.data(), VK_BUFFER_USAGE_INDEX_BUFFER_BIT);
...
TEST_F(VkArmBestPracticesLayerTest, ComputeShaderBadWorkGroupThreadAlignmentTest) {
    TEST_DESCRIPTION("Testing for cases where compute shaders will be dispatched in an inefficient way, due to work "
                     "group dispatch counts on "
...

    {
        ...
        m_errorMonitor->SetDesiredFailureMsg(
            kPerformanceWarningBit, "BestPractices-Arm-vkCreateComputePipelines-compute-thread-group-alignment");
        pipe.CreateComputePipeline();
        m_errorMonitor->VerifyFound();
    }
...
TEST_F(VkArmBestPracticesLayerTest, ComputeShaderBadWorkGroupThreadCountTest) {
    TEST_DESCRIPTION("Testing for cases where the number of work groups spawned is greater than advised for Arm Mali "
                     "architectures.");
...
TEST_F(VkArmBestPracticesLayerTest, ComputeShaderBadSpatialLocalityTest) {
    TEST_DESCRIPTION("Testing for cases where a compute shader's configuration makes poor use of spatial locality, on "
                     "Arm Mali architectures, "
    ...
    auto make_pipeline_with_shader = [this](const VkPipelineShaderStageCreateInfo& stage) {
        ...
        auto ds_layout =
            std::unique_ptr<vkt::DescriptorSetLayout>(new vkt::DescriptorSetLayout(*m_device, { sampler_binding }));
        auto pipe_layout =
            std::unique_ptr<vkt::PipelineLayout>(new vkt::PipelineLayout(*m_device, { ds_layout.get() }));
    ...
    auto test_spatial_locality = [=](const VkPipelineShaderStageCreateInfo& stage, bool positive_test) {
        if (!positive_test) {
            this_ptr->m_errorMonitor->SetDesiredFailureMsg(
                kPerformanceWarningBit, "BestPractices-Arm-vkCreateComputePipelines-compute-spatial-locality");
        }
...
TEST_F(VkArmBestPracticesLayerTest, RedundantRenderPassStore) {
    ...
    auto img = std::unique_ptr<vkt::Image>(
        new vkt::Image(*m_device,
                       WIDTH,
                       HEIGHT,
                       1,
                       FMT,
...
TEST_F(VkArmBestPracticesLayerTest, InefficientRenderPassClear) {
    TEST_DESCRIPTION(
        "Test for appropriate warnings to be thrown when a redundant clear is used on a LOAD_OP_LOAD attachment.");
    ...
    vk::CmdClearColorImage(
        m_command_buffer.handle(), image->handle(), VK_IMAGE_LAYOUT_GENERAL, &clear_color_value, 1, &subresource_range);
...
TEST_F(VkArmBestPracticesLayerTest, DescriptorTracking) {
    ...
    m_command_buffer.BeginRenderPass(rp.handle(), framebuffers[0], 1, 1, 3, clear_values);
    {
        vk::CmdBindDescriptorSets(m_command_buffer.handle(),
                                  ...
                                  graphics_pipeline.pipeline_layout_.handle(),
                                  0,
                                  1,
                                  &descriptor_set,
                                  0,
