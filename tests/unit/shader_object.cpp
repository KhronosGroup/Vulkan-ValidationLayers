...
// 02721 is not validated correctly https://github.com/KhronosGroup/Vulkan-ValidationLayers/issues/10798
TEST_F(NegativeShaderObject, DISABLED_VertexOOBAccess) {
    TEST_DESCRIPTION("Draw with vertex format components not matching vertex input format components.");
    ...
    m_vertex_buffer = new vkt::Buffer(*m_device, 4u, VK_BUFFER_USAGE_VERTEX_BUFFER_BIT, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT);
    ...
    vk::CmdDraw(m_command_buffer, 24, 1, 0, 0);
    m_errorMonitor->VerifyFound();
