...
TEST_F(PositiveRenderPass, InitialLayoutUndefined) {
    TEST_DESCRIPTION("Ensure that CmdBeginRenderPass with an attachment's initialLayout of VK_IMAGE_LAYOUT_UNDEFINED "
                     "works when the command "
...
TEST_F(PositiveRenderPass, AttachmentLayoutWithLoadOpThenReadOnly) {
    TEST_DESCRIPTION("Positive test where we create a renderpass with an attachment that uses LOAD_OP_CLEAR, the first "
                     "subpass has a valid "
...
TEST_F(PositiveRenderPass, BeginTransitionsAttachmentUnused) {
    TEST_DESCRIPTION("Ensure that layout transitions work correctly without errors, when an attachment reference is "
...
TEST_F(PositiveRenderPass, BeginStencilLoadOp) {
    TEST_DESCRIPTION(
        "Create a stencil-only attachment with a LOAD_OP set to CLEAR. stencil[Load|Store]Op used to be ignored.");
    RETURN_IF_SKIP(Init());
    ...
    RenderPassSingleSubpass rp(*this);
    rp.AddAttachmentDescription(
        depth_stencil_fmt, VK_IMAGE_LAYOUT_UNDEFINED, VK_IMAGE_LAYOUT_DEPTH_STENCIL_ATTACHMENT_OPTIMAL);
...
TEST_F(PositiveRenderPass, BeginDepthStencilLayoutTransitionFromUndefined) {
    TEST_DESCRIPTION("Create a render pass with depth-stencil attachment where layout transition from UNDEFINED TO "
                     "DS_READ_ONLY_OPTIMAL is set "
                     "by render pass and verify that transition has correctly occurred at queue submit time with no "
                     "validation errors.");
...
TEST_F(PositiveRenderPass, ImagelessFramebufferNonZeroBaseMip) {
    ...
    auto image_ci = vkt::Image::ImageCreateInfo2D(
        width, 1, 2, 1, formats[0], VK_IMAGE_USAGE_COLOR_ATTACHMENT_BIT | VK_IMAGE_USAGE_TRANSFER_SRC_BIT);
...
TEST_F(PositiveRenderPass, SingleMipTransition) {
    ...
    vkt::ImageView depth_view = depthImage.CreateView(VK_IMAGE_VIEW_TYPE_2D,
                                                      /*baseMipLevel*/ 0,
                                                      /*levelCount*/ 1,
                                                      0,
    ...
    descriptor_set.WriteDescriptorImageInfo(2,
                                            fullViews[1],
                                            sampler,
    ...
    // Note that these image barriers attempt to transition *all* miplevels, even though only 1 miplevel has
    // transitioned.

    colorImage.Layout(VK_IMAGE_LAYOUT_GENERAL);

    colorImage.ImageMemoryBarrier(m_command_buffer,
                                  VK_IMAGE_ASPECT_COLOR_BIT,
                                  VK_ACCESS_COLOR_ATTACHMENT_WRITE_BIT,
                                  VK_ACCESS_COLOR_ATTACHMENT_WRITE_BIT,
...
TEST_F(PositiveRenderPass, BeginDedicatedStencilLayout) {
    ...
    vkt::Image ds_image(
        *m_device, 32, 32, 1, ds_format, VK_IMAGE_USAGE_TRANSFER_DST_BIT | VK_IMAGE_USAGE_DEPTH_STENCIL_ATTACHMENT_BIT);
...
TEST_F(PositiveRenderPass, QueriesInMultiview) {
    ...
    auto image_ci =
        vkt::Image::ImageCreateInfo2D(32, 32, 2, 3, VK_FORMAT_R8G8B8A8_UNORM, VK_IMAGE_USAGE_COLOR_ATTACHMENT_BIT);
    vkt::Image image(*m_device, image_ci, vkt::set_layout);
...
TEST_F(PositiveRenderPass, FramebufferCreateDepthStencilLayoutTransitionForDepthOnlyImageView) {
    ...
    RenderPassSingleSubpass rp(*this);
    rp.AddAttachmentDescription(VK_FORMAT_D32_SFLOAT_S8_UINT,
...
TEST_F(PositiveRenderPass, FramebufferWithAttachmentsTo3DImageMultipleSubpasses) {
    TEST_DESCRIPTION("Test no false overlap is reported with multi attachment framebuffer (attachments are slices of a "
                     "3D image). Multiple "
    ...
    vkt::Framebuffer framebuffer(
        *m_device, renderpass.handle(), depth_count, views, image_info.extent.width, image_info.extent.height);

    m_command_buffer.Begin();
    m_command_buffer.BeginRenderPass(
        renderpass.handle(), framebuffer.handle(), image_info.extent.width, image_info.extent.height);
...
TEST_F(PositiveRenderPass, ImageLayoutTransitionOf3dImageWith2dViews) {
    TEST_DESCRIPTION("Test that transitioning the layout of a mip level of a 3D image using a view of one of its slice "
                     "applies to the entire 3D "
                     "image: all views referencing different slices of the same mip level should also see their layout "
                     "transitioned");
    ...
    rp_1.AddAttachmentDescription(
        image_info.format, VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL, VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL);
    rp_1.AddAttachmentReference({ 0, VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL });
    rp_1.AddInputAttachment(0);
    rp_1.CreateRenderPass();

    vkt::Framebuffer framebuffer_1(
        *m_device, rp_1.Handle(), 1, &views[0], image_info.extent.width, image_info.extent.height);
    ...
    // VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL
    rp_2.AddAttachmentDescription(
        image_info.format, VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL, VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL);
    rp_2.AddAttachmentReference({ 0, VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL });
    rp_2.AddColorAttachment(0);
    rp_2.CreateRenderPass();

    vkt::Framebuffer framebuffer_2(
        *m_device, rp_2.Handle(), 1, &views[1], image_info.extent.width, image_info.extent.height);

    m_command_buffer.Begin();

    m_command_buffer.BeginRenderPass(
        rp_1.Handle(), framebuffer_1.handle(), image_info.extent.width, image_info.extent.height);
    m_command_buffer.EndRenderPass();

    m_command_buffer.BeginRenderPass(
        rp_2.Handle(), framebuffer_2.handle(), image_info.extent.width, image_info.extent.height);
    m_command_buffer.EndRenderPass();
...
TEST_F(PositiveRenderPass, SubpassWithReadOnlyLayoutWithoutDependency) {
    TEST_DESCRIPTION(
        "When both subpasses' attachments are the same and layouts are read-only, they don't need dependency.");
    RETURN_IF_SKIP(Init());
    ...
    subpasses[0] = {
        0, VK_PIPELINE_BIND_POINT_GRAPHICS, 0, 0, 0, nullptr, nullptr, &att_ref_depth_stencil, 0, nullptr
    };
    subpasses[1] = {
        0, VK_PIPELINE_BIND_POINT_GRAPHICS, 0, 0, 0, nullptr, nullptr, &att_ref_depth_stencil, 0, nullptr
    };
...
TEST_F(PositiveRenderPass, SeparateDepthStencilSubresourceLayout) {
    ...
    {
        m_command_buffer.Begin();
        auto depth_barrier = image.ImageMemoryBarrier(
            0, 0, VK_IMAGE_LAYOUT_UNDEFINED, VK_IMAGE_LAYOUT_DEPTH_ATTACHMENT_OPTIMAL, depth_range);
        auto stencil_barrier = image.ImageMemoryBarrier(
            0, 0, VK_IMAGE_LAYOUT_UNDEFINED, VK_IMAGE_LAYOUT_STENCIL_READ_ONLY_OPTIMAL, stencil_range);
        vk::CmdPipelineBarrier(m_command_buffer.handle(),
    ...
    barriers.push_back(image.ImageMemoryBarrier(0,
                                                0,
    ...
    barriers.push_back(image.ImageMemoryBarrier(
        0, 0, VK_IMAGE_LAYOUT_DEPTH_ATTACHMENT_OPTIMAL, VK_IMAGE_LAYOUT_DEPTH_READ_ONLY_OPTIMAL, depth_range));

    barriers.push_back(image.ImageMemoryBarrier(
        0, 0, VK_IMAGE_LAYOUT_STENCIL_ATTACHMENT_OPTIMAL, VK_IMAGE_LAYOUT_STENCIL_READ_ONLY_OPTIMAL, stencil_range));

    // Test that transition from UNDEFINED on depth aspect does not clobber stencil layout.
    barriers.push_back(image.ImageMemoryBarrier(
        0, 0, VK_IMAGE_LAYOUT_UNDEFINED, VK_IMAGE_LAYOUT_DEPTH_ATTACHMENT_OPTIMAL, depth_range));

    // Test that we can transition aspects separately and use combined layouts. (Only care about the aspect in
    // question).
    barriers.push_back(image.ImageMemoryBarrier(0,
                                                0,
                                                VK_IMAGE_LAYOUT_DEPTH_STENCIL_ATTACHMENT_OPTIMAL,
...
TEST_F(PositiveRenderPass, TestDepthStencilRenderPassTransition) {
    TEST_DESCRIPTION("Create framebuffer with a depth/stencil attachment that has only depth or stencil aspect and "
                     "transition it in render "
                     "pass. Then create a barrier on both aspect, for this to be valid *both* aspects must have been "
                     "correctly tracked as "
    ...
    for (size_t i = 0; i < 2; i++) {
        ...
        RenderPassSingleSubpass rp(*this);
        rp.AddAttachmentDescription(
            ds_format, VK_IMAGE_LAYOUT_UNDEFINED, VK_IMAGE_LAYOUT_DEPTH_STENCIL_ATTACHMENT_OPTIMAL);
        rp.AddAttachmentReference({ 0, VK_IMAGE_LAYOUT_DEPTH_STENCIL_ATTACHMENT_OPTIMAL });
        rp.AddDepthStencilAttachment(0);
        rp.CreateRenderPass();
...
TEST_F(PositiveRenderPass, NestedCommandBuffersFeatureMaintenance7) {
    TEST_DESCRIPTION("Use VK_SUBPASS_CONTENTS_INLINE_AND_SECONDARY_COMMAND_BUFFERS_KHR with maintenance7, but not "
                     "nextedCommandBuffers is not "
                     "enabled");
    SetTargetApiVersion(VK_API_VERSION_1_1);
    ...
    m_command_buffer.Begin();
    vk::CmdBeginRenderPass(m_command_buffer.handle(),
