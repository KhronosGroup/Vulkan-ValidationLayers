...
TEST_F(NegativeDataGraph, ResourceVariableNotDeclared) {
    TEST_DESCRIPTION(
        "Try creating a datagraph pipeline where pResourceInfos contains an object where descriptorSet and binding do not "
...
TEST_F(NegativeDataGraph, GraphConstantIsTensorConstantMissingDescription) {
    TEST_DESCRIPTION(
        "Try creating a datagraph pipeline with a graph pipeline constant which corresponds to a tensor constant but there is no "
...
TEST_F(NegativeDataGraph, GraphConstantIsTensorConstantDescriptionMissingUsageFlags) {
    TEST_DESCRIPTION(
        "Try creating a datagraph pipeline with a graph pipeline constant which corresponds to a tensor constant but the supplied "
...
TEST_F(NegativeDataGraph, ResourceIsTensorMissingDescription) {
    TEST_DESCRIPTION(
        "Try creating a datagraph pipeline when the resources in resourceInfo do not have a VkTensorDescriptionARM in the pNext chain");
    InitBasicDataGraph();
    InitTypes();
    RETURN_IF_SKIP(Init());
...
TEST_F(NegativeDataGraph, DataGraphShaderModuleCreateInfoIncorrectName) {
    TEST_DESCRIPTION("Create a datagraph pipeline where VkDataGraphPipelineShaderModuleCreateInfoARM::pName doesn't match the name in the spirv code.");
    InitBasicDataGraph();
    InitTypes();
    RETURN_IF_SKIP(Init());

    vkt::dg::CreateDataGraphPipelineHelper pipeline(*this, true);
    pipeline.pipeline_shader_module_.pName = "NOT_the_correct_name";
    m_errorMonitor->SetDesiredError("VUID-VkDataGraphPipelineShaderModuleCreateInfoARM-pName-09872");
    pipeline.CreateDataGraphPipeline();
    m_errorMonitor->VerifyFound();
}

TEST_F(NegativeDataGraph, DISABLED_DataGraphShaderModuleCreateInfoHasModuleAndShaderModuleCreateInfo) {
    ...
    // the helper constructor adds the shader module in the "normal" way, as VkDataGraphPipelineShaderModuleCreateInfoARM::module
    vkt::dg::CreateDataGraphPipelineHelper pipeline(*this, true);

    // and now also add the same ShaderModule in the pNext chain
    ...
    pipeline.pipeline_shader_module_.pNext = &shader_module_create_info;
...
TEST_F(NegativeDataGraph, DataGraphShaderModuleCreateInfoInvalidModule) {
    ...
    vkt::dg::CreateDataGraphPipelineHelper pipeline(*this, true);
    pipeline.pipeline_shader_module_.module = VK_NULL_HANDLE;
    m_errorMonitor->SetDesiredError("VUID-VkDataGraphPipelineShaderModuleCreateInfoARM-pNext-09874");
    // missing module will also cause missing resources
    m_errorMonitor->SetAllowedFailureMsg("VUID-VkDataGraphPipelineCreateInfoARM-pResourceInfos-09771");
    pipeline.CreateDataGraphPipeline();
    m_errorMonitor->VerifyFound();
}
