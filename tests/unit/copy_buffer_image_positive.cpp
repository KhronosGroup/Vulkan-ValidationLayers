...
TEST_F(PositiveCopyBufferImage, ImageRemainingLayersMaintenance5) {
    TEST_DESCRIPTION("Test copying an image with VkImageSubresourceLayers.layerCount = VK_REMAINING_ARRAY_LAYERS using "
                     "VK_KHR_maintenance5");
...
TEST_F(PositiveCopyBufferImage, BufferToRemaingImageLayers) {
    ...
    vkt::Buffer buffer(
        *m_device, 32u * 32u * 4u, VK_BUFFER_USAGE_TRANSFER_SRC_BIT, VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT);
...
TEST_F(PositiveCopyBufferImage, UncompressedToCompressedImage) {
    ...
    // Size-compatible (64-bit) formats. Uncompressed is 64 bits per texel, compressed is 64 bits per 4x4 block (or
    // 4bpt).
    ...
    // Size = 10 * 10 * 64 = 6400
    vkt::Image uncomp_10x10t_image(*m_device,
                                   10,
                                   10,
                                   1,
                                   VK_FORMAT_R16G16B16A16_UINT,
                                   VK_IMAGE_USAGE_TRANSFER_SRC_BIT | VK_IMAGE_USAGE_TRANSFER_DST_BIT);
    // Size = 40 * 40 * 4  = 6400
    vkt::Image comp_10x10b_40x40t_image(*m_device,
                                        40,
                                        40,
                                        1,
                                        VK_FORMAT_BC1_RGBA_SRGB_BLOCK,
    ...
    vk::CmdCopyImage(m_command_buffer.handle(),
                     uncomp_10x10t_image.handle(),
                     VK_IMAGE_LAYOUT_GENERAL,
                     comp_10x10b_40x40t_image.handle(),
                     VK_IMAGE_LAYOUT_GENERAL,
                     1,
                     &copy_region);
    // The next copy swaps source and dest s.t. we need an execution barrier on for the prior source and an access
    // barrier for prior dest
...
TEST_F(PositiveCopyBufferImage, UncompressedToCompressedImage2) {
    ...
    // Size = 3 * 3 * 64 = 576
    vkt::Image uncomp_image(*m_device,
                            3,
                            3,
                            1,
                            VK_FORMAT_R16G16B16A16_UINT,
...
TEST_F(PositiveCopyBufferImage, BufferCopiesStressTest) {
    ...
    vk::CmdCopyBuffer(
        m_command_buffer, src_buffer.handle(), dst_buffer.handle(), size32(copy_info_list), copy_info_list.data());
