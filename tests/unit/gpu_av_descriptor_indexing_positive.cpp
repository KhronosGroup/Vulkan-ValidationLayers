...
TEST_F(PositiveGpuAVDescriptorIndexing, SharedPipelineLayoutSubsetGraphicsPushConstants) {
    ...
    vk::CmdPushConstants(m_commandBuffer->handle(), pipeline_layout_2.handle(), VK_SHADER_STAGE_VERTEX_BIT, 0,
                         static_cast<uint32_t>(2 * sizeof(uint32_t)), &push_constants_data[0]);
    vk::CmdPushConstants(m_commandBuffer->handle(), pipeline_layout_2.handle(), VK_SHADER_STAGE_FRAGMENT_BIT,
