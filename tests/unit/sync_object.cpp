...
TEST_F(NegativeSyncObject, Transition3dImageSlice) {
    ...
    m_command_buffer.Begin();
    m_errorMonitor->SetDesiredError("VUID-VkImageMemoryBarrier-subresourceRange-Todo");
    ...
    m_errorMonitor->SetDesiredError("VUID-VkImageMemoryBarrier-subresourceRange-Todo2");
    vk::CmdPipelineBarrier(m_command_buffer.handle(), VK_PIPELINE_STAGE_TRANSFER_BIT, VK_PIPELINE_STAGE_TRANSFER_BIT, 0u, 0u,
                           nullptr, 0u, nullptr, 1u, &image_memory_barrier);
    m_errorMonitor->VerifyFound();
...
TEST_F(NegativeSyncObject, Transition3dImageWithMipLevels) {
    ...
    m_command_buffer.Begin();
    m_errorMonitor->SetDesiredError("VUID-VkImageMemoryBarrier-subresourceRange-Todo3");
    vk::CmdPipelineBarrier(m_command_buffer.handle(), VK_PIPELINE_STAGE_TRANSFER_BIT, VK_PIPELINE_STAGE_TRANSFER_BIT, 0u, 0u,
                           nullptr, 0u, nullptr, 1u, &image_memory_barrier);
    m_errorMonitor->VerifyFound();
...
TEST_F(NegativeSyncObject, AsymmetricSetEvent2) {
    ...
    m_errorMonitor->SetDesiredError("VUID-vkCmdSetEvent2-pDependencyInfo-Todo1");
    vk::CmdSetEvent2(m_command_buffer, event, &dependency_info);
    m_errorMonitor->VerifyFound();

    dependency_info.bufferMemoryBarrierCount = 0u;
    dependency_info.memoryBarrierCount = 2u;
    m_errorMonitor->SetDesiredError("VUID-vkCmdSetEvent2-pDependencyInfo-Todo2");
    vk::CmdSetEvent2(m_command_buffer, event, &dependency_info);
    m_errorMonitor->VerifyFound();

    dependency_info.memoryBarrierCount = 1u;
    barriers[0].srcAccessMask = VK_ACCESS_TRANSFER_WRITE_BIT;
    m_errorMonitor->SetDesiredError("VUID-vkCmdSetEvent2-pDependencyInfo-Todo3");
    vk::CmdSetEvent2(m_command_buffer, event, &dependency_info);
    m_errorMonitor->VerifyFound();
...
TEST_F(NegativeSyncObject, AsymmetricWaitEvent2) {
    ...
    m_errorMonitor->SetDesiredError("VUID-vkCmdWaitEvents2-pDependencyInfo-Todo4");
    m_default_queue->Submit(m_command_buffer);
    m_errorMonitor->VerifyFound();
    m_default_queue->Wait();
}

TEST_F(NegativeSyncObject, AsymmetricWaitEvent2StageMask) {
    SetTargetApiVersion(VK_API_VERSION_1_3);
    AddRequiredExtensions(VK_KHR_MAINTENANCE_9_EXTENSION_NAME);
    AddRequiredFeature(vkt::Feature::maintenance9);
    AddRequiredFeature(vkt::Feature::synchronization2);
    ...
    //m_errorMonitor->SetDesiredError("VUID-vkCmdWaitEvents2-pDependencyInfo-Todo5");
    m_default_queue->Submit(m_command_buffer);
    //m_errorMonitor->VerifyFound();
    m_default_queue->Wait();
}
