...
struct layer_data {
    ...
    // For VkPhysicalDeviceProperties2 instead of generating and storing every pNext, just list those needed as there
    // will probably not end up being that many
...
typedef void(VKAPI_PTR* PFN_vkSetPhysicalDeviceFeaturesEXT)(VkPhysicalDevice physicalDevice,
                                                            const VkFormatProperties2 newFeatures);
...
VKAPI_ATTR void VKAPI_CALL GetOriginalPhysicalDeviceLimitsEXT(VkPhysicalDevice physicalDevice,
...
VKAPI_ATTR void VKAPI_CALL GetOriginalPhysicalDeviceFormatPropertiesEXT(VkPhysicalDevice physicalDevice,
                                                                        VkFormat format,
...
VKAPI_ATTR void VKAPI_CALL SetPhysicalDeviceFormatPropertiesEXT(VkPhysicalDevice physicalDevice,
                                                                VkFormat format,
                                                                const VkFormatProperties newProperties) {
    std::lock_guard<std::mutex> lock(global_lock);
    layer_data* phy_dev_data = GetLayerDataPtr(physicalDevice, device_profile_api_dev_data_map);
    ...
    // Need to set in the case a test uses SetPhysicalDeviceFormatProperties1() functions but because it is a 1.3 device
    // and the validation code will call GetPhysicalDeviceFormatProperties2() expecting VkFormatProperties3 to be filled
...
VKAPI_ATTR void VKAPI_CALL GetOriginalPhysicalDeviceFormatProperties2EXT(VkPhysicalDevice physicalDevice,
                                                                         VkFormat format,
...
VKAPI_ATTR void VKAPI_CALL SetPhysicalDeviceFormatProperties2EXT(VkPhysicalDevice physicalDevice,
                                                                 VkFormat format,
                                                                 const VkFormatProperties2 newProperties) {
    std::lock_guard<std::mutex> lock(global_lock);
    layer_data* phy_dev_data = GetLayerDataPtr(physicalDevice, device_profile_api_dev_data_map);
    ...
    memcpy(
        &(phy_dev_data->format_properties_map[format]), &(newProperties.formatProperties), sizeof(VkFormatProperties));
...
VKAPI_ATTR VkResult VKAPI_CALL CreateInstance(const VkInstanceCreateInfo* pCreateInfo,
    ...
    result = instance_data->dispatch_table.EnumeratePhysicalDevices(
        *pInstance, &physical_device_count, physical_devices.data());
    if (result != VK_SUCCESS)
        return result;
...
VKAPI_ATTR void VKAPI_CALL DestroyInstance(VkInstance instance, const VkAllocationCallbacks* pAllocator) {
    ...
    std::vector<VkPhysicalDevice> physical_devices(physical_device_count);
    VkResult result = instance_data->dispatch_table.EnumeratePhysicalDevices(
...
VKAPI_ATTR void VKAPI_CALL GetPhysicalDeviceFormatProperties2(VkPhysicalDevice physicalDevice,
    ...
    auto device_format_map_it = phy_dev_data->format_properties_map.find(format);
    if (device_format_map_it != phy_dev_data->format_properties_map.end()) {
        memcpy((void*)&(pProperties->formatProperties),
               &phy_dev_data->format_properties_map[format],
...
VKAPI_ATTR PFN_vkVoidFunction VKAPI_CALL GetPhysicalDeviceProcAddr(VkInstance instance, const char* name) {
    if (!strcmp(name, "vkSetPhysicalDeviceLimitsEXT"))
        return (PFN_vkVoidFunction)SetPhysicalDeviceLimitsEXT;
    if (!strcmp(name, "vkGetOriginalPhysicalDeviceLimitsEXT"))
        return (PFN_vkVoidFunction)GetOriginalPhysicalDeviceLimitsEXT;
    if (!strcmp(name, "vkSetPhysicalDeviceFormatPropertiesEXT"))
        return (PFN_vkVoidFunction)SetPhysicalDeviceFormatPropertiesEXT;
    ...
    if (!strcmp(name, "vkSetPhysicalDeviceFormatProperties2EXT"))
        return (PFN_vkVoidFunction)SetPhysicalDeviceFormatProperties2EXT;
    ...
    if (!strcmp(name, "vkGetOriginalPhysicalDeviceFeaturesEXT"))
        return (PFN_vkVoidFunction)GetOriginalPhysicalDeviceFeaturesEXT;
    if (!strcmp(name, "vkSetPhysicalDeviceFeaturesEXT"))
        return (PFN_vkVoidFunction)SetPhysicalDeviceFeaturesEXT;
    if (!strcmp(name, "vkSetPhysicalDeviceProperties2EXT"))
        return (PFN_vkVoidFunction)SetPhysicalDeviceProperties2EXT;
    layer_data* instance_data = GetLayerDataPtr(instance, device_profile_api_dev_data_map);
...
VKAPI_ATTR PFN_vkVoidFunction VKAPI_CALL GetInstanceProcAddr(VkInstance instance, const char* name) {
    ...
    if (!strcmp(name, "vkGetPhysicalDeviceProperties2"))
        return (PFN_vkVoidFunction)GetPhysicalDeviceProperties2;
    ...
    if (!strcmp(name, "vkGetPhysicalDeviceFormatProperties2"))
        return (PFN_vkVoidFunction)GetPhysicalDeviceFormatProperties2;
    ...
    if (!strcmp(name, "vkSetPhysicalDeviceProperties2EXT"))
        return (PFN_vkVoidFunction)SetPhysicalDeviceProperties2EXT;
    if (!strcmp(name, "vk_layerGetPhysicalDeviceProcAddr"))
...
VVL_EXPORT VKAPI_ATTR VkResult VKAPI_CALL vkEnumerateInstanceLayerProperties(uint32_t* pCount,
                                                                             VkLayerProperties* pProperties) {
    return device_profile_api::EnumerateInstanceLayerProperties(pCount, pProperties);
}

VVL_EXPORT VKAPI_ATTR VkResult VKAPI_CALL vkEnumerateInstanceExtensionProperties(const char* pLayerName,
                                                                                 uint32_t* pCount,
                                                                                 VkExtensionProperties* pProperties) {
...
VVL_EXPORT VKAPI_ATTR PFN_vkVoidFunction VKAPI_CALL vk_layerGetPhysicalDeviceProcAddr(VkInstance instance,
                                                                                      const char* funcName) {
    return device_profile_api::GetPhysicalDeviceProcAddr(instance, funcName);
}

VVL_EXPORT VKAPI_ATTR VkResult VKAPI_CALL
