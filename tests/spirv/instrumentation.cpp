...
static bool ray_tracing_runtime_pass = false;
...
USAGE: %s <input> -o <output> <passes>
    ...
    printf(R"(
  --ray-tracing-runtime
               Adds validation hooks for SPV_KHR_ray_tracing instructions that could
               not be caught in spirv-val becuase values are not known.)");
    printf(R"(
  -h, --help
               Print this help.)");
    printf("\n");
...
bool ParseFlags(int argc, char** argv, const char** out_file) {
    ...
    for (int argi = 1; argi < argc; ++argi) {
        ...
        } else if (0 == strcmp(cur_arg, "--ray-tracing-runtime")) {
            ray_tracing_runtime_pass = true;
        }
    }
