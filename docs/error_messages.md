...
By default, the Validation Layers will format the messsage, but it is possible for the user can provide their own `PFN_vkDebugUtilsMessengerCallbackEXT` and pasre the string in `VkDebugUtilsMessengerCallbackDataEXT::pMessage` ([working example using vk-bootstrap](https://github.com/charles-lunarg/vk-bootstrap/blob/main/example/custom_debug_callback.cpp#L9-L20)).
...
5. Which function the error occured
6. Which struct/location the error occured
7. The "main message"
